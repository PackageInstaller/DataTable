...
function UIActivityN5Stage:autoFightBtnOnClick()
  if self._autoBtnEnable then
    if self._powerID == RoleAssetID.RoleAssetPhyPoint then
      self:ShowDialog("UISetAutoFightCount", MatchType.MT_Campaign, self._needPower, 0, nil, nil, ECampaignType.CAMPAIGN_TYPE_N5)
    else
      self:ShowDialog("UISetAutoFightCount", MatchType.MT_Campaign, self._needPower, 0, nil, self._pointComp, ECampaignType.CAMPAIGN_TYPE_N5)
    end
  else
