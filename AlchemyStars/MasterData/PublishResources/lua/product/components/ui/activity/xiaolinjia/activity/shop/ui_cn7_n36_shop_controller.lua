...
_class("UICN7N36ShopController", UIController)
UICN7N36ShopController = UICN7N36ShopController

function UICN7N36ShopController:Constructor()
...
function UICN7N36ShopController:LoadDataOnEnter(TT, res, uiParams)
  self._campaignModule = GameGlobal.GetModule(CampaignModule)
  self._svrTimeModule = self:GetModule(SvrTimeModule)
  self._campaign = UIActivityCampaign:New()
  self._campaign:LoadCampaignInfo(TT, res, ECampaignType.CAMPAIGN_TYPE_INLAND_N7, ECampaignCN7ComponentID.ECAMPAIGN_N7_LOTTERY)
  if res and not res:GetSucc() then
    self._campaignModule:CheckErrorCode(res.m_result, self._campaign._id, nil, nil)
    return
  end
  self._lotteryComponent = self._campaign:GetComponent(ECampaignCN7ComponentID.ECAMPAIGN_N7_LOTTERY)
  self._lotteryComponentInfo = self._campaign:GetComponentInfo(ECampaignCN7ComponentID.ECAMPAIGN_N7_LOTTERY)
end

function UICN7N36ShopController:OnShow(uiParams)
  ...
  local lockName = "UICN7N36ShopController_ShopAwardCell_uieff_ShopCell"
...
function UICN7N36ShopController:OnHide()
  if self._spine then
...
function UICN7N36ShopController:_AttachEvents()
  self:AttachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
  self:AttachEvent(GameEventType.ItemCountChanged, self._OnItemCountChanged)
end

function UICN7N36ShopController:_CheckActivityClose(id)
  if self._campaign and self._campaign._id == id then
    self:SwitchState(UIStateType.UIMain)
  end
end

function UICN7N36ShopController:_ForceRefresh(isOpenNew)
  self:Lock("UICN7N36ShopController:_ForceRefresh")
  ...
  self._timeEvents._delayUnlockTimeEvent = GameGlobal.Timer():AddEvent(1, function()
    self:UnLock("UICN7N36ShopController:_ForceRefresh")
  end)
end

function UICN7N36ShopController:_GetComponents()
  ...
  self.backBtns = backBtnGen:SpawnObject("UINewCommonTopButton")
...
function UICN7N36ShopController:_SetStencil(rawImage)
...
function UICN7N36ShopController:RegUIEventTriggerListener(onBeginDrag, onDrag, onEndDrag)
...
function UICN7N36ShopController:OnBeginDrag(ped)
  local deltaX = ped.delta.x
  if self:IsEdge(deltaX) then
    return
  end
  local pos = UICN7N36ShopController.ScreenPointToLocalPointInRectangle(self.selfRect, ped)
  self._xBegainDrag = pos.x
end

function UICN7N36ShopController:OnDrag(ped)
  local pos = UICN7N36ShopController.ScreenPointToLocalPointInRectangle(self.selfRect, ped)
  self._xCurDrag = pos.x
end

function UICN7N36ShopController:OnEndDrag(ped)
  self:InitDragField()
end

function UICN7N36ShopController:OnPress()
end

function UICN7N36ShopController:_UpdateDragSwitch()
...
function UICN7N36ShopController.ScreenPointToLocalPointInRectangle(rect, ped)
...
function UICN7N36ShopController:IsEdge(deltaX)
...
function UICN7N36ShopController:InitDragField()
  self._xBegainDrag = nil
  self._xCurDrag = nil
  self._posBGLogo = Vector2(-667, 0)
end

function UICN7N36ShopController:_InitData(isOpenNew, isOnShow)
  self.unlockBoxs = {}
  self.uiData = {}
...
function UICN7N36ShopController:CloseDialogWithAnim()
...
function UICN7N36ShopController:ShowMainUI()
  if self._lotteryState ~= LotteryState.None then
    return
  end
  self:SwitchState(UIStateType.UIMain)
...
function UICN7N36ShopController:_InitAwardListUi()
...
function UICN7N36ShopController:_InitBoxCoverSpine()
...
function UICN7N36ShopController:_OnGetAwardBoxCell(scrollview, index)
  ...
  if item.IsInitHandlerCalled == false then
    ...
    cellPool:SpawnObjects("UICN7N36ShopAwardBoxCell", 1)
  end
...
function UICN7N36ShopController:SetPointSelect(index, select)
  if select then
...
function UICN7N36ShopController:BoxLeftBtnOnClick(go)
  if self._lotteryState ~= LotteryState.None then
    return
  end
...
function UICN7N36ShopController:BoxRightBtnOnClick(go)
  if self._lotteryState ~= LotteryState.None then
    return
  end
  local nextIndex = self._curPageIndex + 1
...
function UICN7N36ShopController:OnUpdate(deltaTimeMS)
...
function UICN7N36ShopController:_ShowItemTips(data)
  if self._lotteryState ~= LotteryState.None then
    return
  end
  local End = self:CheckActivityEnd()
  ...
  self:ShowDialog("UICN7N36ShopTipsController", data)
end

function UICN7N36ShopController:NoRestItemTipsCoverOnClick(go)
  ToastManager.ShowToast(StringTable.Get("str_cn7_n36_shop_no_item_tips"))
end

function UICN7N36ShopController:_IsBoxUnlock(boxIndex)
...
function UICN7N36ShopController:_SetAwardListToPageIndex(index)
...
function UICN7N36ShopController:_InitBoxPosPoint()
  for index = 1, 10 do
...
function UICN7N36ShopController:_RefreshDrawBtnArea()
...
function UICN7N36ShopController:_RefreshLockInfoText()
  local str = StringTable.Get("str_cn7_n36_shop_box_unlock_intro", self._curPageIndex - 1)
  self.lockInfoText:SetText(str)
end

function UICN7N36ShopController:_RefreshDrawBtnNoItemCover()
  local hasRestItem = self:_CheckAwardRestSingle()
  self.noRestItemTipsCoverGo:SetActive(not hasRestItem)
end

function UICN7N36ShopController:_RefreshSingleDrawBtn()
...
function UICN7N36ShopController:_RefreshMultiDrawBtn()
  local bEnableBtn = true
  if not self:_IsBoxUnlock(self._curPageIndex) then
    bEnableBtn = false
  end
...
function UICN7N36ShopController:_RefreshBoxTitle()
  local boxNameText = StringTable.Get("str_cn7_n36_lottery_pool", self._curPageIndex)
  self.awardListTitleText:SetText(boxNameText)
end

function UICN7N36ShopController:_RefreshCurBoxRest()
  local curBoxRest = 0
  local curBoxTotal = 0
  ...
  local tmpText = "(" .. StringTable.Get("str_cn7_n36_shop_award_rest_num") .. " " .. "<color=#F6e834>" .. curBoxRest .. "</color>" .. "/" .. curBoxTotal .. ")"
  self.curBoxRestText:SetText(tmpText)
end

function UICN7N36ShopController:_RefreshCurBoxCover()
  local uiCfg = Cfg.cfg_n10_shop_box_ui_client[self._curPageIndex]
  if uiCfg then
...
function UICN7N36ShopController:_CheckCurrencyEnable(price)
...
function UICN7N36ShopController:_OnItemCountChanged()
  self:_RefreshSingleDrawBtn()
  self:_RefreshMultiDrawBtn()
  self:_RefreshDrawBtnNoItemCover()
  local totalNum = ClientCampaignDrawShop.GetMoney(self.currencyId)
...
function UICN7N36ShopController:_GetDefaultPageIndexOnShow()
  local pageIndex = 1
  if self._lotteryComponent then
...
function UICN7N36ShopController:_CheckAwardRestSingle()
  if self._lotteryComponent then
...
function UICN7N36ShopController:_CheckAwardRestMulti()
  if self._lotteryComponent then
...
function UICN7N36ShopController:DrawSingleBtnOnClick(go)
  self:_DoDraw(ECampaignLotteryType.E_CLT_SINGLE)
end

function UICN7N36ShopController:DrawMultiBtnOnClick(go)
  self:_DoDraw(ECampaignLotteryType.E_CLT_MULTI)
end

function UICN7N36ShopController:_EnableSingleDrawBtn(enable)
...
function UICN7N36ShopController:_EnableMultiDrawBtn(enable)
...
function UICN7N36ShopController:GetNumberCN(num)
  if num <= 0 then
    return nil
  elseif num < 11 then
    return StringTable.Get("str_cn7_n36_shop_number_" .. num)
  elseif num < 100 then
    local gewei = num % 10
    local shiwei = math.floor(num / 10)
    local str = ""
    if shiwei == 1 then
      str = str .. StringTable.Get("str_cn7_n36_shop_number_10")
    else
      str = str .. StringTable.Get("str_cn7_n36_shop_number_" .. shiwei) .. StringTable.Get("str_cn7_n36_shop_number_10")
    end
    if gewei == 0 then
      return str
    else
      return str .. StringTable.Get("str_cn7_n36_shop_number_" .. gewei)
    end
  else
    return StringTable.Get("str_cn7_n36_shop_number_99")
  end
end

function UICN7N36ShopController:_CheckCanDrawOnceMore(lotteryType)
...
function UICN7N36ShopController:_DoDraw(lotteryType)
  if self._lotteryState ~= LotteryState.None then
    return
  end
  local End = self:CheckActivityEnd()
...
function UICN7N36ShopController:_SendDrawReq(TT, res, boxIndex, lotteryType)
  if self._lotteryComponent then
...
function UICN7N36ShopController:_ShowGetReward(record)
  self._lotteryState = LotteryState.None
  self._uiModule:LockAchievementFinishPanel(false)
  if not record then
    return
  ...
  if 0 < #tempPets then
    self:ShowDialog("UIPetObtain", tempPets, function()
      ...
      self:ShowDialog(getItemCtrl, assetAwards, function()
...
function UICN7N36ShopController:_ConfirmToNextBox()
  local strTitle = StringTable.Get("str_cn7_n36_shop_new_box_unlock_title")
  local strText = StringTable.Get("str_cn7_n36_shop_open_next_text", self._curPageIndex, self._curPageIndex + 1)
  local curCost = 10
...
function UICN7N36ShopController:_LoopBoxRestTips()
  local strTitle = ""
  local strText = StringTable.Get("str_cn7_n36_shop_loop_box_reset_tips")
  local curCost = 10
...
function UICN7N36ShopController:_ConfirmDrawOnceMore(lotteryType)
  local price = self.singlePrice
  if lotteryType == ECampaignLotteryType.E_CLT_MULTI then
    price = self.multiPrice
  end
  local strText = StringTable.Get("str_cn7_n36_shop_draw_once_more_text", price)
...
function UICN7N36ShopController:_RecordRewardsInfo(getRewards, lotteryType, curBoxHasRest, isOpenNew, canDrawOnceMore)
...
function UICN7N36ShopController:_ShowDrawSpineAnim(TT, lotteryType)
  self._lotterySpine = nil
  self._lotterySpineTime = 0
  self._lotteryResultSpine = nil
  self._lotteryResultSpineTime = 0
...
function UICN7N36ShopController:SpineAreaOnClick()
  if self._lotteryState ~= LotteryState.LotterySpine then
    return
  end
  self._lotteryState = LotteryState.LotteryResultSpine
...
function UICN7N36ShopController:_CheckIsRestRepeatBox()
...
function UICN7N36ShopController:RuleDescriptionBtnOnClick(go)
  if self._lotteryState ~= LotteryState.None then
    return
  end
  local End = self:CheckActivityEnd()
  ...
  self:ShowDialog("UICN7N36IntroController", "UICN7N36ShopController")
end

function UICN7N36ShopController:DrawSingleBtnGreyCoverOnClick(go)
  local End = self:CheckActivityEnd()
  if End then
    ToastManager.ShowToast(StringTable.Get("str_activity_finished"))
    self:SwitchState(UIStateType.UIMain)
  ...
  local priceEnable = self:_CheckCurrencyEnable(self.singlePrice)
  local remainEnable = self:_CheckAwardRestSingle()
  if not priceEnable and remainEnable then
    ToastManager.ShowToast(StringTable.Get("str_activity_cost_not_enough"))
  end
end

function UICN7N36ShopController:DrawMultiBtnGreyCoverOnClick(go)
  local End = self:CheckActivityEnd()
  if End then
    ToastManager.ShowToast(StringTable.Get("str_activity_finished"))
    self:SwitchState(UIStateType.UIMain)
  ...
  local priceEnable = self:_CheckCurrencyEnable(self.multiPrice)
  local remainEnable = self:_CheckAwardRestMulti()
  if not priceEnable and remainEnable then
    ToastManager.ShowToast(StringTable.Get("str_activity_cost_not_enough"))
  end
end

function UICN7N36ShopController:CheckActivityEnd()
