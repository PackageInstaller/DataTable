...
function UIN20MiniGameStageController:LoadDataOnEnter(TT, res, uiParams)
  ...
  self._campaign:LoadCampaignInfo(TT, res, ECampaignType.CAMPAIGN_TYPE_N20, ECampaignN20ComponentID.ECAMPAIGN_N20_MINI_GAME)
...
function UIN20MiniGameStageController:_GetComponents()
  ...
  self._commonTopBtn:SetData(function()
    ...
    self:ShowDialog("UIN20MiniGameHelp", "UIN20MiniGameStageController")
  end)
  ...
  if table.icontains(self._current_stage_cfg.StoryActiveType, 1) and self._componentInfo.mission_info_list[self._curMissionID].mission_info.story_mask & 1 == 0 then
    ...
    GameGlobal.GetModule(StoryModule):StartStory(self._current_stage_cfg.StoryID[1], function()
      self:StartTask(function(TT)
        ...
        if res:GetSucc() then
          self:ShowDialog("UIN20MiniGameController", self._current_stage_cfg.ID, self._component, self._componentInfo, self._lastBGMResName, function()
            self:_hide(false)
