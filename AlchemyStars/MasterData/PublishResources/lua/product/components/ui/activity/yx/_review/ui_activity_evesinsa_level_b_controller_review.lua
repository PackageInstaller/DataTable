...
function UIActivityEveSinsaLevelBController_Review:LoadDataOnEnter(TT, res, uiParams)
  ...
  if res and res:GetSucc() then
    self._camp = self._campaign:GetComponent(ECampaignReviewEvaRescuePlanComponentID.ECAMPAIGN_REVIEW_EVARESCUEPLAN_TREE_MISSION)
    self._campInfo = self._camp:GetComponentInfo()
    self._phase = UIActivityEveSinsaHelper.CheckTimePhase(self._campaign)
    if self._phase ~= EActivityEveSinsaTimePhase.EPhase_Over and not self._campaign:CheckComponentOpen(ECampaignReviewEvaRescuePlanComponentID.ECAMPAIGN_REVIEW_EVARESCUEPLAN_TREE_MISSION) then
...
function UIActivityEveSinsaLevelBController_Review:_EnterMiddleStage(stageIndex)
  ...
  self:ShowDialog("UIActivityLevelStageNew", middleStageID, self._passInfo[middleStageID], self._camp, false, nil, true, true)
end
...
function UIActivityEveSinsaLevelBController_Review:_EnterSmallStage(middleStageIndex, smallStageIndex)
  ...
  self:ShowDialog("UIActivityLevelStageNew", smallStageID, self._passInfo[smallStageID], self._camp, false, nil, true, true)
end
