_class("UIN48LineMissionMapNode", UICustomWidget)
UIN48LineMissionMapNode = UIN48LineMissionMapNode

function UIN48LineMissionMapNode:OnShow(uiParams)
  self:InitWidget()
end

function UIN48LineMissionMapNode:InitWidget()
  ...
  self._atlas = self:GetAsset("UIN48.spriteatlas", LoadType.SpriteAtlas)
  self._anim = self:GetGameObject():GetComponent("Animation")
end

function UIN48LineMissionMapNode:SetData(lineCfg, passInfo, cb)
  ...
  if lineCfg.WayPointType == 4 then
    typeCfg = UIN48LineMissionController.NodeCfg[UIN48LineMissionController.SLeval]
  else
    typeCfg = UIN48LineMissionController.NodeCfg[missionCfg.Type]
  end
  local bg
...
function UIN48LineMissionMapNode:btnOnClick(go)
