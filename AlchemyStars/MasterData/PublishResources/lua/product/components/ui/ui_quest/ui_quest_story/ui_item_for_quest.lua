...
_class("UIItemForQuest", UICustomWidget)
UIItemForQuest = UIItemForQuest

function UIItemForQuest:Constructor()
...
function UIItemForQuest:OnShow(uiParams)
...
function UIItemForQuest:OnHide()
...
function UIItemForQuest:PlayAni(aniName)
  self._anim:Play(aniName)
end

function UIItemForQuest:SetForm(type, scale, dontPlayAni)
...
function UIItemForQuest:EnableNode(nodeId, isEnable)
...
function UIItemForQuest:SetData(params)
...
function UIItemForQuest:ShowNodes(uiItemNode, enable)
...
function UIItemForQuest:SetScale(scale)
  scale = scale or 1
...
function UIItemForQuest:Select(select, noAnim)
...
function UIItemForQuest:SetClickCallBack(callBack, param)
...
function UIItemForQuest:BtnOnClick(go)
  if self._clickCallBack then
    self._clickCallBack(go)
  end
end

function UIItemForQuest:SetBtnImage(bImage)
  if bImage then
...
function UIItemForQuest:SetLongPressCallBack(longPressCallBack, longPressUpCallBack, pressTime, update)
...
function UIItemForQuest:LongEvent()
...
function UIItemForQuest:LongEventUpLv(gradeLv)
  local cfg = Cfg.cfg_up_lv_long_press[gradeLv]
  if not cfg then
    Log.fatal("###[UIItemForQuest] LongEventUpLv cfg is nil ! grade:", gradeLv)
  end
  local arr = cfg.Value
  local datas = {}
  for i = 1, #arr do
...
function UIItemForQuest:Calculate(count)
...
function UIItemForQuest:GetBtn()
  return self._btnGO
end

function UIItemForQuest:SetToggleGroup(group)
...
function UIItemForQuest:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  end
end

function UIItemForQuest:SetToggleValue(isOn)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleValue(isOn)
  end
end

function UIItemForQuest:GetItemData()
