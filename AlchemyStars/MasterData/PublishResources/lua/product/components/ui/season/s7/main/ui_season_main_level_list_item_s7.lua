_class("UISeasonMainLevelListItemS7", UICustomWidget)
UISeasonMainLevelListItemS7 = UISeasonMainLevelListItemS7

function UISeasonMainLevelListItemS7:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonMainLevelListItemS7:InitWidget()
...
function UISeasonMainLevelListItemS7:SetData(data, onClick)
  self._data = data
  self._onClick = onClick
  self._lockTip:SetActive(not self._data:IsUnlock())
end

function UISeasonMainLevelListItemS7:RefreshByDiff(curDiff)
  ...
  if isHasProduceLevel then
    ...
    awards = tmp
    local items = self.item:SpawnObjects("UISeasonMainLevelListAssetS7", #awards)
    for i = 1, #awards do
      local item = items[i]
  ...
  local atlas = self:GetAsset("UIS7Scene.spriteatlas", LoadType.SpriteAtlas)
  if isStoryLevel then
    self._bg.sprite = atlas:GetSprite("exp_s7_map_gqlb_di01")
  else
    self._bg.sprite = atlas:GetSprite("exp_s7_map_gqlb_di02")
  end
end

function UISeasonMainLevelListItemS7:PrepareAnim()
  self._root:SetActive(false)
end

function UISeasonMainLevelListItemS7:PlayEnterAnim()
  self._root:SetActive(true)
  self._anim:Play("uieff_UISeasonMainLevelListItemS3_in")
end

function UISeasonMainLevelListItemS7:PlaySwitchAnim()
  self._root:SetActive(true)
end

function UISeasonMainLevelListItemS7:PlayOutAnim()
  self._anim:Play("uieff_UISeasonMainLevelListItemS2_out")
end

function UISeasonMainLevelListItemS7:RootOnClick(go)
