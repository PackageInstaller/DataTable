...
_class("UISeasonResultItemS4", UICustomWidget)
UISeasonResultItemS4 = UISeasonResultItemS4

function UISeasonResultItemS4:Constructor()
...
function UISeasonResultItemS4:OnShow(uiParams)
  self._transform = self:GetGameObject().transform
  self._nodes = {}
  self._nodes[UIItemNode.Normal] = UIItemNormalNodeForSeasonS4:New(self:GetUIComponent("UIView", "g_normal"), self)
...
function UISeasonResultItemS4:OnHide()
...
function UISeasonResultItemS4:PlayAni(aniName)
  self._anim:Play(aniName)
end

function UISeasonResultItemS4:SetForm(type, scale, dontPlayAni)
...
function UISeasonResultItemS4:EnableNode(nodeId, isEnable)
...
function UISeasonResultItemS4:SetData(params)
...
function UISeasonResultItemS4:ShowNodes(uiItemNode, enable)
...
function UISeasonResultItemS4:SetScale(scale)
  scale = scale or 1
...
function UISeasonResultItemS4:Select(select, noAnim)
...
function UISeasonResultItemS4:SetClickCallBack(callBack, param)
...
function UISeasonResultItemS4:BtnOnClick(go)
  if self._clickCallBack then
    self._clickCallBack(go)
  end
end

function UISeasonResultItemS4:SetBtnImage(bImage)
  if bImage then
...
function UISeasonResultItemS4:SetLongPressCallBack(longPressCallBack, longPressUpCallBack, pressTime, update)
...
function UISeasonResultItemS4:SetReduceLongPressCallBack(sec)
  if self._nodes[UIItemNode.Reduce] then
    self._nodes[UIItemNode.Reduce]:SetReduceLongPressCallBack(sec)
  end
end

function UISeasonResultItemS4:InitLongPress()
...
function UISeasonResultItemS4:LongEvent()
...
function UISeasonResultItemS4:LongEventUpLv(gradeLv)
  local cfg = Cfg.cfg_up_lv_long_press[gradeLv]
  if not cfg then
    Log.fatal("###[UISeasonResultItemS4] LongEventUpLv cfg is nil ! grade:", gradeLv)
  end
  local arr = cfg.Value
  local datas = {}
  for i = 1, #arr do
...
function UISeasonResultItemS4:Calculate(count)
...
function UISeasonResultItemS4:GetBtn()
  return self._btnGO
end

function UISeasonResultItemS4:SetToggleGroup(group)
...
function UISeasonResultItemS4:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  end
end

function UISeasonResultItemS4:SetToggleValue(isOn)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleValue(isOn)
  end
end

function UISeasonResultItemS4:GetItemData()
  return self._uiItemData
end

function UISeasonResultItemS4:ClearItemData()
  self._uiItemData = nil
end

function UISeasonResultItemS4:SetBtnImageByName(imgName, atlasName)
...
_class("UIItemNormalNodeForSeasonS4", UIItemNormalNode)
UIItemNormalNodeForSeasonS4 = UIItemNormalNodeForSeasonS4

function UIItemNormalNodeForSeasonS4:SetQuality(quality)
  if quality <= 0 then
    self:ShowQuality(false)
    return
  end
  local qualityName = "exp_s4_map_daoju_pinji0" .. tostring(quality)
  if qualityName ~= "" then
    self:ShowQuality(true)
    local atlas = self._owner:GetAsset("UIS4Scene.spriteatlas", LoadType.SpriteAtlas)
...
function UIItemNormalNodeForSeasonS4:SetIconOffset(itemId)
...
function UIItemNormalNodeForSeasonS4:SetOffset(showText1)
  if showText1 then
    self._qualityRect.anchoredPosition = Vector2(0, -33)
  else
    self._qualityRect.anchoredPosition = Vector2(0, -33)
  end
end
