_class("UISeasonShowAwardsS6", UIController)
UISeasonShowAwardsS6 = UISeasonShowAwardsS6

function UISeasonShowAwardsS6:GetComponents()
...
function UISeasonShowAwardsS6:OnShow(uiParams)
  ...
  if uiParams[1] then
    if table.count(uiParams[1]) == 0 then
      Log.fatal("###[UISeasonShowAwardsS6] table.count(uiParams[1]) == 0 !")
    end
  else
    Log.fatal("###[UISeasonShowAwardsS6] uiParams[1] is nil !")
  end
  if uiParams[3] then
    itemlist = uiParams[1]
  else
...
function UISeasonShowAwardsS6:CreateData(itemlist)
...
function UISeasonShowAwardsS6:DoAnimation()
  self._canvasGroup = self:GetUIComponent("CanvasGroup", "canvasGroup")
  self._canvasGroup.alpha = 0
  self._tweener = self._canvasGroup:DOFade(1, 0.02)
end

function UISeasonShowAwardsS6:ClosePanel()
  self:CloseDialog()
end

function UISeasonShowAwardsS6:OnHide()
  if self._closeCallback then
    self._closeCallback()
  end
end

function UISeasonShowAwardsS6:_InitListView(scrollView, index)
  if index < 0 then
    return nil
  end
  local count
  ...
  if item.IsInitHandlerCalled == false then
    ...
    rowPool:SpawnObjects("UISeasonShowAwardsItemS6", count)
  end
...
function UISeasonShowAwardsS6:_ShowItem(giftItem, index, tweenIdx)
  local beforeTime = 0
...
function UISeasonShowAwardsS6:OnItemSelect(id, count, go)
  GameGlobal.EventDispatcher():Dispatch(GameEventType.ShowItemTips, id, self._trans.transform.position)
end

function UISeasonShowAwardsS6:NextOnClick(go)
...
function UISeasonShowAwardsS6:CalcPage()
...
function UISeasonShowAwardsS6:_GetNextPageIndex()
...
function UISeasonShowAwardsS6:_GetCurPageFirstIndex()
  return self._curPageFirstIndex
end

function UISeasonShowAwardsS6:_GetItemDataByIndex(index)
...
function UISeasonShowAwardsS6:_FormatItemCount(itemCount)
  return HelperProxy:GetInstance():FormatItemCount(itemCount)
end

function UISeasonShowAwardsS6:ShowTips(itemId, pos)
