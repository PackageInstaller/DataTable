_class("UISeasonS3MusicTab", UICustomWidget)
UISeasonS3MusicTab = UISeasonS3MusicTab

function UISeasonS3MusicTab:OnShow(uiParams)
  self:InitWidget()
...
function UISeasonS3MusicTab:OnHide()
...
function UISeasonS3MusicTab:InitWidget()
...
function UISeasonS3MusicTab:SetData(data)
  self._collageData = data
  self._musicCount = self._collageData:GetMusicCount()
  self._items = self.content:SpawnObjects("UISeasonS3CollageMusicItem", self._musicCount)
...
function UISeasonS3MusicTab:SetShow(show)
...
function UISeasonS3MusicTab:_OnSelect(data, isInit)
...
function UISeasonS3MusicTab:PlayOnClick(go)
...
function UISeasonS3MusicTab:PauseOnClick(go)
  if not self._curIdx then
    return
  end
  if self._pause then
...
function UISeasonS3MusicTab:LastOnClick(go)
  if not self._curIdx then
    return
  end
...
function UISeasonS3MusicTab:NextOnClick(go)
  if not self._curIdx then
    return
  end
  if self._unlockCount == 1 then
...
function UISeasonS3MusicTab:_RefreshPlayBar()
  if self._curIdx then
...
function UISeasonS3MusicTab:_ResetProgress()
  self.progress.fillAmount = 0
  self.time:SetText("")
end

function UISeasonS3MusicTab:_PlayingTick()
  if self._pause then
    return
  end
  local data = self._collageData:GetMusicByIndex(self._curIdx)
...
function UISeasonS3MusicTab:_SnapTo(index, animate)
...
function UISeasonS3MusicTab:PlayExitAnim()
end
