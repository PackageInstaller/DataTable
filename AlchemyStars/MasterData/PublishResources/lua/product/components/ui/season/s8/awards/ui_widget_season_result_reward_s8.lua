_class("UIWidgetSeasonResultRewardS8", UICustomWidget)
UIWidgetSeasonResultRewardS8 = UIWidgetSeasonResultRewardS8

function UIWidgetSeasonResultRewardS8:OnShow()
  self._trans = self:GetGameObject()
  self._itemID = nil
  local sop = self:GetUIComponent("UISelectObjectPath", "uiitem")
  self.uiItem = sop:SpawnObject("UISeasonResultItemS8")
...
function UIWidgetSeasonResultRewardS8:SetShotEffStartTime()
  self.effStartTime = 750
end

function UIWidgetSeasonResultRewardS8:Init(count, iconID, threeStar, returnPrism, firstPass, extAward, doubleExtAward, activityAward, returnHelpAward)
...
function UIWidgetSeasonResultRewardS8:GetNormalTxt()
...
function UIWidgetSeasonResultRewardS8:HasItem(dataList)
...
function UIWidgetSeasonResultRewardS8:BGOnClick(go)
  GameGlobal.GameRecorder():RecordAction(GameRecordAction.UIInput, {
    ui = "UIWidgetSeasonResultRewardS8",
    input = "BGOnClick",
    args = {}
  })
...
function UIWidgetSeasonResultRewardS8:SetLevelDiff(levelDiff)
  local typeMainBg = "exp_S8_map_daoju_di01"
  if levelDiff then
    if levelDiff == UISeasonLevelDiff.Normal then
      typeMainBg = "exp_S8map_daoju_di01"
    elseif levelDiff == UISeasonLevelDiff.Hard then
      typeMainBg = "exp_S8_map_daoju_di02"
    end
  end
  local atlasName = "UIS8Scene.spriteatlas"
