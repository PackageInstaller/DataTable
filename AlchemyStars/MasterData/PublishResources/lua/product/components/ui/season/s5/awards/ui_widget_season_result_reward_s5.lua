_class("UIWidgetSeasonResultRewardS5", UICustomWidget)
UIWidgetSeasonResultRewardS5 = UIWidgetSeasonResultRewardS5

function UIWidgetSeasonResultRewardS5:OnShow()
  self._trans = self:GetGameObject()
  self._itemID = nil
  local sop = self:GetUIComponent("UISelectObjectPath", "uiitem")
  self.uiItem = sop:SpawnObject("UISeasonResultItemS5")
...
function UIWidgetSeasonResultRewardS5:SetShotEffStartTime()
  self.effStartTime = 750
end

function UIWidgetSeasonResultRewardS5:Init(count, iconID, threeStar, returnPrism, firstPass, extAward, doubleExtAward, activityAward, returnHelpAward)
...
function UIWidgetSeasonResultRewardS5:GetNormalTxt()
...
function UIWidgetSeasonResultRewardS5:HasItem(dataList)
...
function UIWidgetSeasonResultRewardS5:BGOnClick(go)
  GameGlobal.GameRecorder():RecordAction(GameRecordAction.UIInput, {
    ui = "UIWidgetSeasonResultRewardS5",
    input = "BGOnClick",
    args = {}
  })
...
function UIWidgetSeasonResultRewardS5:SetLevelDiff(levelDiff)
  local typeMainBg = "exp_s5_map_daoju_di01"
  if levelDiff then
    if levelDiff == UISeasonLevelDiff.Normal then
      typeMainBg = "exp_s5map_daoju_di01"
    elseif levelDiff == UISeasonLevelDiff.Hard then
      typeMainBg = "exp_s5_map_daoju_di02"
    end
  end
  local atlasName = "UIS5Scene.spriteatlas"
