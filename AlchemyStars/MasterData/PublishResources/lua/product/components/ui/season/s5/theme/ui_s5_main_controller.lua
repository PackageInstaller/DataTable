_class("UIS5MainController", UIController)
UIS5MainController = UIS5MainController

function UIS5MainController:Constructor()
  self._isReview = false
  self._canShare = self:GetModule(ShareModule):CanShare()
  self._svrTimeModule = GameGlobal.GetModule(SvrTimeModule)
end

function UIS5MainController:_SetRemainingTime(widgetName, extraId, descId, endTime)
...
function UIS5MainController:_SetCommonTopButton()
...
function UIS5MainController:_Back()
  if self:Manager():CurUIStateType() == UIStateType.UIS5Main or self:Manager():CurUIStateType() == UIStateType.UISeasonMazeMain then
    self:SwitchState(UIStateType.UIMain)
  else
    self:_Shot(function()
      local frogNode = self:GetGameObject("FrogNode1")
      if frogNode then
        frogNode:SetActive(false)
      end
...
function UIS5MainController:_PlayAnim(idx, callback)
  local tb = {
    ["in"] = {
      animName = "uieffanim_UIS5MainController_in",
      duration = 200
    },
    out = {
      animName = "uieffanim_UIS5MainController_out",
      duration = 500
    },
    hide = {
      animName = "uieffanim_UIS5MainController_hide",
      duration = 500
    },
    show = {
      animName = "uieffanim_UIS5MainController_show",
      duration = 500
    }
  }
...
function UIS5MainController:_CheckGuide()
  GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UIS5MainController)
end

function UIS5MainController:LoadDataOnEnter(TT, res, uiParams)
...
function UIS5MainController:OnShow(uiParams)
  ...
  local obj = UIWidgetHelper.SpawnObject(self, "Title", "UIS5Title")
  self:_Refresh(true)
  self:_AttachEvents()
end

function UIS5MainController:OnHide()
  self:_DetachEvents()
end

function UIS5MainController:_Refresh(first)
  ...
  self:_SetBtn_SeasonMaze(first)
end

function UIS5MainController:_CalcTime()
  ...
  if curTime < time_action then
    extra, time = "str_season_s5_main_time_cd_action", time_action
  elseif curTime < time_exchange then
    extra, time = "str_season_s5_main_time_cd_exchange", time_exchange
  end
  self:_SetRemainingTime("_time", extra, nil, time)
end

function UIS5MainController:_SetBtn_Collage(first)
  local obj = UIWidgetHelper.SpawnObject(self, "CollageBtn", "UIS5CollageBtn")
  obj:SetData(self._seasonId, first)
end

function UIS5MainController:_SetBtn_Exchange(first)
  local obj = UIWidgetHelper.SpawnObject(self, "ExchangeBtn", "UIS5ExchangeBtn")
  obj:SetData(self._seasonId, self._component_exchange, first)
end

function UIS5MainController:_SetBtn_Medal(first)
  local obj = UIWidgetHelper.SpawnObject(self, "MedalBtn", "UIS5MedalBtn")
  obj:SetData(self._seasonId, first)
end

function UIS5MainController:_SetBtn_Quest(first)
  local obj = UIWidgetHelper.SpawnObject(self, "QuestBtn", "UIS5QuestBtn")
  obj:SetData(self._seasonId, self._component_quest, first)
end

function UIS5MainController:_SetBtn_Go(first)
  local obj = UIWidgetHelper.SpawnObject(self, "GoBtn", "UIS5GoBtn")
  obj:SetData(self._seasonId, self._component_action, first)
end

function UIS5MainController:_SetBtn_SeasonMaze(first)
...
function UIS5MainController:_Shot(callback)
...
function UIS5MainController:_AfterShot()
...
function UIS5MainController:ShowBtnOnClick(go)
  self:_ShowUI()
end

function UIS5MainController:StoryBtnOnClick(go)
end

function UIS5MainController:IntroBtnOnClick(go)
  UISeasonHelper.ShowSeasonHelperBook(UISeasonHelperTabIndex.S1Main)
end

function UIS5MainController:_AttachEvents()
  self:AttachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
  self:AttachEvent(GameEventType.AfterUILayerChanged, self._AfterUILayerChanged)
end

function UIS5MainController:_DetachEvents()
  self:DetachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
  self:DetachEvent(GameEventType.AfterUILayerChanged, self._AfterUILayerChanged)
end

function UIS5MainController:_CheckActivityClose(id)
  if self._seasonId == id then
    self:SwitchState(UIStateType.UIMain)
  end
end

function UIS5MainController:_AfterUILayerChanged()
