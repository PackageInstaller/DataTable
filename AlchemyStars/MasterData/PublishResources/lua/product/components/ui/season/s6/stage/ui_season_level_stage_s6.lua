_class("UISeasonLevelStageS6", UIController)
UISeasonLevelStageS6 = UISeasonLevelStageS6

function UISeasonLevelStageS6:_GetComponents()
...
function UISeasonLevelStageS6:OnShow(uiParams)
...
function UISeasonLevelStageS6:LoadDataOnEnter(TT, res)
end

function UISeasonLevelStageS6.GetAutoOpenState(matchType, stageId)
...
function UISeasonLevelStageS6.SetAutoOpenState(matchType, stageId, isOpen)
  local roleModule = GameGlobal.GetModule(RoleModule)
  local pstId = roleModule:GetPstId()
  local playerPrefsKey = pstId .. "AutoOpenState" .. matchType
  if stageId then
...
function UISeasonLevelStageS6:_CanAutoFight()
  local tipsStr = ""
...
function UISeasonLevelStageS6:_CanShowAutoFight()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS6:InitAutoBtnState()
...
function UISeasonLevelStageS6:_CheckShowWordBuff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS6:OnHide()
...
function UISeasonLevelStageS6:HasPassThreeStar(missionCfg)
...
function UISeasonLevelStageS6:GetSortedArr(awardType, cfg, stageAwardType)
...
function UISeasonLevelStageS6:ProcessAward(missionCfg, noNormal)
  local missionId = missionCfg.ID
  local awards = {}
...
function UISeasonLevelStageS6:RefreshAwardsShowState()
  self._noDiffAwardGo = self:GetGameObject("NoDiffAward")
  self._seasonAwardGo = self:GetGameObject("SeasonAward")
  self._seasonNormalAwardGo = self:GetGameObject("MultiAwardGroup1")
  self._seasonHardAwardGo = self:GetGameObject("MultiAwardGroup2")
...
function UISeasonLevelStageS6:RefreshWordsArea()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS6:InitWords()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS6:_InitWords(sop, missionId)
...
function UISeasonLevelStageS6:_GetWordDesc(levelId, wordId)
...
function UISeasonLevelStageS6:RefreshBtnUnknown()
  local show = self:_CheckShowWordBuff()
  self.btnWord:SetActive(show)
end

function UISeasonLevelStageS6:InitForMultiDiff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS6:RefreshPanelShow()
...
function UISeasonLevelStageS6:RefreshPanelBg()
end

function UISeasonLevelStageS6:RefreshPanelText()
  if self._curPanel == UISeasonLevelStagePanelMode.AwardsInfo then
    self._awardsPanelBtnText.color = Color(1, 0.9882352941176471, 0.9607843137254902, 1)
    self._battleInfoPanelBtnText.color = Color(0.788235294117647, 0.611764705882353, 0.43137254901960786, 1)
  else
    self._awardsPanelBtnText.color = Color(0.788235294117647, 0.611764705882353, 0.43137254901960786, 1)
    self._battleInfoPanelBtnText.color = Color(1, 0.9882352941176471, 0.9607843137254902, 1)
  end
end

function UISeasonLevelStageS6:RefreshDiffArea()
end

function UISeasonLevelStageS6:InitAllAwards()
  if self._hasMultiDiff then
    ...
    self._normalMultiAwardList = normalAwardGen:SpawnObject("UISeasonStageMultiAwardListS6")
    ...
    self._hardMultiAwardList = hardAwardGen:SpawnObject("UISeasonStageMultiAwardListS6")
...
function UISeasonLevelStageS6:PlayAnimAwardList(totalAnimDelay)
...
function UISeasonLevelStageS6:_ProcessLevelDiffAwards(missionId, levelDiff, noNormal)
  if missionId then
...
function UISeasonLevelStageS6:Init()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS6:UpdateCondition(three_star_condition, conditions)
...
function UISeasonLevelStageS6:AutoFightBtnOnClick()
...
function UISeasonLevelStageS6:BtnFightOnClick(go)
  self:_DoFight()
end

function UISeasonLevelStageS6:_DoFight(autoFightCount)
  if not self._seasonModule:CheckSeasonAndMissionCoseAndJump() then
    return
  end
...
function UISeasonLevelStageS6:IsPowerEnough()
...
function UISeasonLevelStageS6:ShowTips(itemId, pos)
  self._tips:SetData(itemId, pos)
end

function UISeasonLevelStageS6:BgOnClick()
  self:Close()
end

function UISeasonLevelStageS6:ThreeStarTipsBtnOnClick(go)
  self:ShowDialog("UIThreeStarTips")
end

function UISeasonLevelStageS6:BtnUnknownOnClick(go)
  local buffData = {}
  buffData.name = ""
  buffData.des = ""
...
function UISeasonLevelStageS6:_SetStoryBtn()
...
function UISeasonLevelStageS6:BtnPlotOnClick()
  local storyList = DiscoveryStoryList:New()
  storyList:Init(self._missionID)
...
function UISeasonLevelStageS6:SliderHardOnClick()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS6:SliderNormalOnClick()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
      return
    end
...
function UISeasonLevelStageS6:_SwitchDiff()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
...
function UISeasonLevelStageS6:DiffInfoBtnOnClick(go)
  self:ShowDialog("UISeasonStageDiffTips")
end

function UISeasonLevelStageS6:RefreshPowerTips()
...
function UISeasonLevelStageS6:RefreshPowerCost()
  local needPowerText = self._needPower
  if self._powerID == RoleAssetID.RoleAssetPhyPoint then
  else
    if not self._pointComp then
...
function UISeasonLevelStageS6:InitChapterName()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS6:_InitChapterName(text, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
...
function UISeasonLevelStageS6:RefreshChapter()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS6:RefreshThreeStarArea(playAnim, animDelay)
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS6:PlayAnimThreeStarArea(totalAnimDelay)
...
function UISeasonLevelStageS6:InitEnemys()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS6:_InitEnemy(sop, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
  local progress = self._curLevelProgress
...
function UISeasonLevelStageS6:RefreshEnemyArea()
...
function UISeasonLevelStageS6:RefreshRecommendLv()
...
function UISeasonLevelStageS6:RefreshAreasTitleStyle()
end

function UISeasonLevelStageS6:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffStageAreaS6")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonLevelStageS6:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonLevelStageS6:Close()
  self:_CloseDialogWithAnim()
end

function UISeasonLevelStageS6:_CloseDialogWithAnim(callback)
  UIWidgetHelper.PlayAnimation(self, "UICanvas", "uieff_UISeasonLevelStageS6_out", 333, function()
    if callback then
      callback()
    end
    self:_Resume()
...
function UISeasonLevelStageS6:_Resume()
...
function UISeasonLevelStageS6:BattleInfoPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.BattleInfo
  self:RefreshPanelShow()
  self:RefreshPanelText()
end

function UISeasonLevelStageS6:AwardsPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.AwardsInfo
  self:RefreshPanelShow()
  self:RefreshPanelText()
end

function UISeasonLevelStageS6:PowerTopTipsOnClick(go)
  self:ShowDialog("UISeasonActionPointTipS6", self._pointComp, go.transform.anchoredPosition)
end

function UISeasonLevelStageS6:_Countdown()
...
function UISeasonLevelStageS6:_ReqFlushPower(TT)
...
function UISeasonLevelStageS6:OnAfterUILayerChanged()
...
function UISeasonLevelStageS6:ShowMainUI(show)
...
function UISeasonLevelStageS6:RecordOnClick(go)
  self:ShowDialog("UIActivityLevelRecordController", self._component, self._missionID)
end

function UISeasonLevelStageS6:_TryFixStory(TT)
  local normalMissionID = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
...
function UISeasonLevelStageS6:_ReqFinishStory(TT, id, type)
