_class("UIS5ExchangeBtn", UICustomWidget)
UIS5ExchangeBtn = UIS5ExchangeBtn

function UIS5ExchangeBtn:OnShow()
  self._constBtnName = self:GetName()
  self:_AttachEvents()
end

function UIS5ExchangeBtn:OnHide()
  self:_DetachEvents()
end

function UIS5ExchangeBtn:SetData(seasonId, component, playAnim)
  ...
  self:_Refresh()
  if playAnim then
    UIWidgetHelper.PlayAnimationInSequence(self, "_anim", "_anim", "uieffanim_UIS5_ExchangeBtn_in", 0, 667)
  end
end

function UIS5ExchangeBtn:_Refresh()
...
function UIS5ExchangeBtn:_SetState(state)
  self:GetGameObject("_over"):SetActive(state == 2)
end

function UIS5ExchangeBtn:_SetIconText(itemId, widgetIcon, widgetText)
...
function UIS5ExchangeBtn:_AttachEvents()
  self:AttachEvent(GameEventType.ItemCountChanged, self._Refresh)
end

function UIS5ExchangeBtn:_DetachEvents()
