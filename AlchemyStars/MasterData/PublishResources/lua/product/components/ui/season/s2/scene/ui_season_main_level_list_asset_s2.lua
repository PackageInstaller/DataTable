_class("UISeasonMainLevelListAssetS2", UICustomWidget)
UISeasonMainLevelListAssetS2 = UISeasonMainLevelListAssetS2

function UISeasonMainLevelListAssetS2:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonMainLevelListAssetS2:InitWidget()
  self.icon = self:GetUIComponent("Image", "icon")
  self.count = self:GetUIComponent("UILocalizationText", "count")
end

function UISeasonMainLevelListAssetS2:SetData(id, count)
  local zeros
  if 99999 < count then
  ...
  if string.isnullorempty(zeros) then
    ...
    self.count:SetText("<color=#625542>" .. zeros .. "</color>" .. count)
  end
