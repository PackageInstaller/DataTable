_class("UISeasonMainS7", UIController)
UISeasonMainS7 = UISeasonMainS7

function UISeasonMainS7:LoadDataOnEnter(TT, res)
  res:SetSucc(true)
...
function UISeasonMainS7:OnShow(uiParams)
  ...
  self._topBtn:SetData(function()
    ...
    self:Lock("UISeasonMainS7:Back")
    GameGlobal.Timer():AddEvent(300, function()
      self:UnLock("UISeasonMainS7:Back")
      if self.view then
        self._active = false
        if self._component:ComponentIsOpen() then
          self._uiModule:ExitSeasonTo(UIStateType.UIS7Main)
        else
  ...
  self._ovalArea = self.ovalAreaPool:SpawnObject("UISeasonMainOvalAreaS7")
  ...
  self:AttachEvents()
  self._levelList = self.levelListPool:SpawnObject("UISeasonMainLevelListS7")
  ...
  self:_ResetQuestRed()
  self._modeSwitch = self._modeSwitchLoader:SpawnObject("UISeasonModeSwitchS7")
  ...
  if not self._uiModule:IsBackTrack() then
    self:Lock("PlayS7EnterAnim")
    self._levelList:PlayEnterAnim()
    GameGlobal.Timer():AddEvent(1000, function()
      self:UnLock("PlayS7EnterAnim")
      if self._active then
      end
    end)
    self._levelList:PlayEnterAnim()
    self._animation:Play("uieff_UISeasonMainS7_in")
  end
  local seasonID = GameGlobal.GetUIModule(SeasonModule):GetSeasonID()
  if UISeasonHelper._GetSeasonCfg(seasonID, "SceneState") ~= UIStateType.UISeasonS7 then
...
function UISeasonMainS7:_OnOnceMissonControllerCosed()
  self:_RefreshThirdBtnVisible()
end

function UISeasonMainS7:AttachEvents()
...
function UISeasonMainS7:DisableOpenList(bDisable, reason)
...
function UISeasonMainS7:IsDisableOpenList()
...
function UISeasonMainS7:OnAfterUILayerChanged()
  self:_TryShowOpenList()
end

function UISeasonMainS7:_TryShowOpenList()
...
function UISeasonMainS7:ShowOpenList()
...
function UISeasonMainS7:CheckShowExpressBubbleOnEnter()
...
function UISeasonMainS7:OnUIGetItemCloseInQuest(type)
  self:RefreshBuffArea()
end

function UISeasonMainS7:OnUpdate(dtMS)
...
function UISeasonMainS7:OnHide()
  self._active = false
...
function UISeasonMainS7:InitWidget()
...
function UISeasonMainS7:_SetTaskListNavi()
...
function UISeasonMainS7:_SetTaskListBtn()
...
function UISeasonMainS7:_RefreshThirdBtnVisible()
...
function UISeasonMainS7:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffMainAreaS7")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonMainS7:InitFinalPlotEnterArea()
...
function UISeasonMainS7:SwitchTestFuncEntry(show)
  if self._testFunc ~= nil then
    self._testFunc:Switch(show)
  end
end

function UISeasonMainS7:SetShow(show)
  self.root:SetActive(show)
  self.showUI:SetActive(not show)
end

function UISeasonMainS7:PlayerIconOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS7:ArchieveBtnOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS7:CollectionBtnOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS7:TopTipOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
  self:_TryStopPlayer()
  self:_HideActionPointTip()
  self:ShowDialog("UISeasonActionPointTipS7", self._actionPointCpt, go.transform.anchoredPosition, function()
    self:_RefreshActionPointTip()
  end)
end

function UISeasonMainS7:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonMainS7:LeaveToBattle()
  self._active = false
  self._uiModule:ExitSeasonGame()
end

function UISeasonMainS7:LeaveToMain()
  self._active = false
end

function UISeasonMainS7:ShowUIOnClick()
  self:SetShow(true)
end

function UISeasonMainS7:AfterPlayEnterVideo(played)
...
function UISeasonMainS7:AfterPlayEnterStory(played)
  if played then
...
function UISeasonMainS7:ShowSerialRewards()
  self._isWaitShowSerialRewards = true
end

function UISeasonMainS7:CheckShowSerialRewards()
...
function UISeasonMainS7:CheckShowCollectionRewardOnEnter()
...
function UISeasonMainS7:CalcBuffLevelOnEnter()
  local seasonModule = self:GetModule(SeasonModule)
  ...
  elseif curLevel ~= oldLevel then
    ...
    self:ShowDialog("UISeasonBuffLevelUpS7", oldLevel, curLevel, componentID, function()
...
function UISeasonMainS7:CheckShowCollectionComposePlotOnEnter()
...
function UISeasonMainS7:OnCollectionComposePlotEnd(completedQuest)
  if not self.questCmpt then
...
function UISeasonMainS7:_OnGetCollection()
  self:_ResetCollectionInfo()
  self:_TryShowOpenList()
end

function UISeasonMainS7:_ResetCollectionInfo()
...
function UISeasonMainS7:_OnActionPointChanged()
  local cur, ceil = self._actionPointCpt:GetItemCount()
  self._pointCount:SetText(string.format("%s/%s", cur, ceil))
end

function UISeasonMainS7:_OnAutoFightSweepFinished()
  self:_OnActionPointChanged()
  self:_ResetCollectionInfo()
  self:_ResetQuestRed()
end

function UISeasonMainS7:_TryStopPlayer()
  self._seasonManager:SeasonPlayerManager():GetPlayer():Stop(false)
end

function UISeasonMainS7:_PlayEnterAnim()
...
function UISeasonMainS7:_OnSeasonDailyResetSucc()
  if self._seasonDaily then
    self._seasonDaily:SetData()
  end
end

function UISeasonMainS7:IsPlayAnimation()
  if self._animation then
    local name = "uieff_UISeasonMainS7_in"
...
function UISeasonMainS7:_OnModeChanged(mode)
...
function UISeasonMainS7:_TriggerGuide()
  self:_CheckGuide()
  self:_SetBackpackInfo()
end

function UISeasonMainS7:_CheckGuide()
  if self:_CheckModeValid(SeasonMapMode.Mode2) then
    GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UISeasonMainS7)
  else
    GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UISeasonMainS7)
  end
end

function UISeasonMainS7:_CheckModeValid(mode)
...
function UISeasonMainS7:_OnSeasonStoryChanged(isOpen)
  self:SetShow(not isOpen)
end

function UISeasonMainS7:_SetBackTrackUI()
  if self._uiModule:IsBackTrack() then
...
function UISeasonMainS7:BackSeasonBtnOnClick(go)
...
function UISeasonMainS7:_IsBackpackOpen()
  if self._uiModule:IsBackTrack() then
    return false
  end
...
function UISeasonMainS7:_SetBackpackInfo()
  local loginModule = self:GetModule(LoginModule)
  local svrTimeModule = self:GetModule(SvrTimeModule)
  local curTime = svrTimeModule:GetServerTime() * 0.001
...
function UISeasonMainS7:BackpackBtnOnClick(go)
...
function UISeasonMainS7:ThirdBtnOnClick(go)
  local onceMissionData = self._seasonModule:GetOnceMissionData()
  local bShow = onceMissionData:AnyComponentOpening()
  if not bShow then
    ToastManager.ShowToast(StringTable.Get("str_activity_error_109"))
  ...
  if mode == 1 then
    ...
    self:ShowDialog("UISeasonConfirm2", title, content, function()
      self._modeSwitch:Switch2Modle2(function()
        self:_MoveToEvnentPointById(8007301)
      end)
    end)
    return
  end
...
function UISeasonMainS7:_MoveToEvnentPointById(eventId)
...
function UISeasonMainS7:_OnSeasonBackpackClose()
  self:_SetBackpackInfo()
end

function UISeasonMainS7:_HideActionPointTip()
...
function UISeasonMainS7:_RefreshActionPointTip()
