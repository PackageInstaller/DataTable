_class("UISeasonMainS6", UIController)
UISeasonMainS6 = UISeasonMainS6

function UISeasonMainS6:LoadDataOnEnter(TT, res)
  res:SetSucc(true)
...
function UISeasonMainS6:OnShow(uiParams)
  ...
  self._topBtn:SetData(function()
    ...
    self:Lock("UISeasonMainS6:Back")
    GameGlobal.Timer():AddEvent(300, function()
      self:UnLock("UISeasonMainS6:Back")
      if self.view then
        self._active = false
        if self._component:ComponentIsOpen() then
          self._uiModule:ExitSeasonTo(UIStateType.UIS6Main)
        else
  ...
  self._ovalArea = self.ovalAreaPool:SpawnObject("UISeasonMainOvalAreaS6")
  ...
  self:AttachEvents()
  self._levelList = self.levelListPool:SpawnObject("UISeasonMainLevelListS6")
  ...
  self:_ResetQuestRed()
  self._modeSwitch = self._modeSwitchLoader:SpawnObject("UISeasonModeSwitchS6")
  ...
  if not self._uiModule:IsBackTrack() then
    self:Lock("PlayS6EnterAnim")
    self._animation:Play("uieff_UISeasonMainS6_in")
    self._levelList:PlayEnterAnim()
    GameGlobal.Timer():AddEvent(1000, function()
      self:UnLock("PlayS6EnterAnim")
      if self._active then
        self._seasonAnim:Play("uieff_UISeasonDailyS6_in")
      end
    end)
    self._levelList:PlayEnterAnim()
    self._animation:Play("uieff_UISeasonMainS6_in")
...
function UISeasonMainS6:_OnOnceMissonControllerCosed()
  self:_RefreshThirdBtnVisible()
end

function UISeasonMainS6:AttachEvents()
...
function UISeasonMainS6:DisableOpenList(bDisable, reason)
...
function UISeasonMainS6:IsDisableOpenList()
...
function UISeasonMainS6:OnAfterUILayerChanged()
  self:_TryShowOpenList()
end

function UISeasonMainS6:_TryShowOpenList()
...
function UISeasonMainS6:ShowOpenList()
...
function UISeasonMainS6:CheckShowExpressBubbleOnEnter()
...
function UISeasonMainS6:OnUIGetItemCloseInQuest(type)
  self:RefreshBuffArea()
end

function UISeasonMainS6:OnUpdate(dtMS)
...
function UISeasonMainS6:OnHide()
  self._active = false
...
function UISeasonMainS6:InitWidget()
...
function UISeasonMainS6:_SetTaskListNavi()
...
function UISeasonMainS6:_SetTaskListBtn()
...
function UISeasonMainS6:_RefreshThirdBtnVisible()
...
function UISeasonMainS6:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffMainAreaS6")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonMainS6:InitFinalPlotEnterArea()
...
function UISeasonMainS6:SwitchTestFuncEntry(show)
  if self._testFunc ~= nil then
    self._testFunc:Switch(show)
  end
end

function UISeasonMainS6:SetShow(show)
  self.root:SetActive(show)
  self.showUI:SetActive(not show)
end

function UISeasonMainS6:PlayerIconOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS6:ArchieveBtnOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS6:CollectionBtnOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS6:TopTipOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
  self:_TryStopPlayer()
  self:_HideActionPointTip()
  self:ShowDialog("UISeasonActionPointTipS6", self._actionPointCpt, go.transform.anchoredPosition, function()
    self:_RefreshActionPointTip()
  end)
end

function UISeasonMainS6:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonMainS6:LeaveToBattle()
  self._active = false
  self._uiModule:ExitSeasonGame()
end

function UISeasonMainS6:LeaveToMain()
  self._active = false
end

function UISeasonMainS6:ShowUIOnClick()
  self:SetShow(true)
end

function UISeasonMainS6:AfterPlayEnterVideo(played)
...
function UISeasonMainS6:AfterPlayEnterStory(played)
  if played then
...
function UISeasonMainS6:ShowSerialRewards()
  self._isWaitShowSerialRewards = true
end

function UISeasonMainS6:CheckShowSerialRewards()
...
function UISeasonMainS6:CheckShowCollectionRewardOnEnter()
...
function UISeasonMainS6:CalcBuffLevelOnEnter()
  local seasonModule = self:GetModule(SeasonModule)
  ...
  elseif curLevel ~= oldLevel then
    ...
    self:ShowDialog("UISeasonBuffLevelUpS6", oldLevel, curLevel, componentID, function()
...
function UISeasonMainS6:CheckShowCollectionComposePlotOnEnter()
...
function UISeasonMainS6:OnCollectionComposePlotEnd(completedQuest)
  if not self.questCmpt then
...
function UISeasonMainS6:_OnGetCollection()
  self:_ResetCollectionInfo()
  self:_TryShowOpenList()
end

function UISeasonMainS6:_ResetCollectionInfo()
...
function UISeasonMainS6:_OnActionPointChanged()
  local cur, ceil = self._actionPointCpt:GetItemCount()
  self._pointCount:SetText(string.format("%s/%s", cur, ceil))
end

function UISeasonMainS6:_OnAutoFightSweepFinished()
  self:_OnActionPointChanged()
  self:_ResetCollectionInfo()
  self:_ResetQuestRed()
end

function UISeasonMainS6:_TryStopPlayer()
  self._seasonManager:SeasonPlayerManager():GetPlayer():Stop(false)
end

function UISeasonMainS6:_PlayEnterAnim()
...
function UISeasonMainS6:_OnSeasonDailyResetSucc()
  if self._seasonDaily then
    self._seasonDaily:SetData()
  end
end

function UISeasonMainS6:IsPlayAnimation()
  if self._animation then
    local name = "uieff_UISeasonMainS6_in"
...
function UISeasonMainS6:_OnModeChanged(mode)
...
function UISeasonMainS6:_TriggerGuide()
  self:_CheckGuide()
  self:_SetBackpackInfo()
end

function UISeasonMainS6:_CheckGuide()
  if self:_CheckModeValid(SeasonMapMode.Mode2) then
    GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UISeasonMainS6)
  else
    GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UISeasonMainS6)
  end
end

function UISeasonMainS6:_CheckModeValid(mode)
...
function UISeasonMainS6:_OnSeasonStoryChanged(isOpen)
  self:SetShow(not isOpen)
end

function UISeasonMainS6:_SetBackTrackUI()
  if self._uiModule:IsBackTrack() then
...
function UISeasonMainS6:BackSeasonBtnOnClick(go)
...
function UISeasonMainS6:_IsBackpackOpen()
  if self._uiModule:IsBackTrack() then
    return false
  end
...
function UISeasonMainS6:_SetBackpackInfo()
  local loginModule = self:GetModule(LoginModule)
  local svrTimeModule = self:GetModule(SvrTimeModule)
  local curTime = svrTimeModule:GetServerTime() * 0.001
...
function UISeasonMainS6:BackpackBtnOnClick(go)
...
function UISeasonMainS6:ThirdBtnOnClick(go)
  local onceMissionData = self._seasonModule:GetOnceMissionData()
  local bShow = onceMissionData:AnyComponentOpening()
  if not bShow then
    ToastManager.ShowToast(StringTable.Get("str_activity_error_109"))
  ...
  local mode = self._seasonManager:SeasonMapManager():Mode()
  if mode == 1 then
    local title = StringTable.Get("str_season_s6_branch_change_mode_title")
    local content = StringTable.Get("str_season_s6_branch_change_mode_message")
    self:ShowDialog("UISeasonConfirm2", title, content, function()
      self._modeSwitch:Switch2Modle2(function()
        self:_MoveToEvnentPointById(8006301)
      end)
    end)
    return
  end
...
function UISeasonMainS6:_MoveToEvnentPointById(eventId)
...
function UISeasonMainS6:_OnSeasonBackpackClose()
  self:_SetBackpackInfo()
end

function UISeasonMainS6:_HideActionPointTip()
...
function UISeasonMainS6:_RefreshActionPointTip()
  ...
  if self.view then
    ...
    self._apCountdownTip:SetText(StringTable.Get("str_season_s6_ticket_refresh_time", t))
  end
