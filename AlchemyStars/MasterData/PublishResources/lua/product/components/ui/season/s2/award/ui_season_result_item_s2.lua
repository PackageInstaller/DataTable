...
_class("UISeasonResultItemS2", UICustomWidget)
UISeasonResultItemS2 = UISeasonResultItemS2

function UISeasonResultItemS2:Constructor()
...
function UISeasonResultItemS2:OnShow(uiParams)
  self._transform = self:GetGameObject().transform
  self._nodes = {}
  self._nodes[UIItemNode.Normal] = UIItemNormalNodeForSeasonS2:New(self:GetUIComponent("UIView", "g_normal"), self)
...
function UISeasonResultItemS2:OnHide()
...
function UISeasonResultItemS2:PlayAni(aniName)
  self._anim:Play(aniName)
end

function UISeasonResultItemS2:SetForm(type, scale, dontPlayAni)
...
function UISeasonResultItemS2:EnableNode(nodeId, isEnable)
...
function UISeasonResultItemS2:SetData(params)
...
function UISeasonResultItemS2:ShowNodes(uiItemNode, enable)
...
function UISeasonResultItemS2:SetScale(scale)
  scale = scale or 1
...
function UISeasonResultItemS2:Select(select, noAnim)
...
function UISeasonResultItemS2:SetClickCallBack(callBack, param)
...
function UISeasonResultItemS2:BtnOnClick(go)
  if self._clickCallBack then
    self._clickCallBack(go)
  end
end

function UISeasonResultItemS2:SetBtnImage(bImage)
  if bImage then
...
function UISeasonResultItemS2:SetLongPressCallBack(longPressCallBack, longPressUpCallBack, pressTime, update)
...
function UISeasonResultItemS2:SetReduceLongPressCallBack(sec)
  if self._nodes[UIItemNode.Reduce] then
    self._nodes[UIItemNode.Reduce]:SetReduceLongPressCallBack(sec)
  end
end

function UISeasonResultItemS2:InitLongPress()
...
function UISeasonResultItemS2:LongEvent()
...
function UISeasonResultItemS2:LongEventUpLv(gradeLv)
  local cfg = Cfg.cfg_up_lv_long_press[gradeLv]
  if not cfg then
    Log.fatal("###[UISeasonResultItemS2] LongEventUpLv cfg is nil ! grade:", gradeLv)
  end
  local arr = cfg.Value
  local datas = {}
  for i = 1, #arr do
...
function UISeasonResultItemS2:Calculate(count)
...
function UISeasonResultItemS2:GetBtn()
  return self._btnGO
end

function UISeasonResultItemS2:SetToggleGroup(group)
...
function UISeasonResultItemS2:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  end
end

function UISeasonResultItemS2:SetToggleValue(isOn)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleValue(isOn)
  end
end

function UISeasonResultItemS2:GetItemData()
  return self._uiItemData
end

function UISeasonResultItemS2:ClearItemData()
  self._uiItemData = nil
end

function UISeasonResultItemS2:SetBtnImageByName(imgName, atlasName)
...
_class("UIItemNormalNodeForSeasonS2", UIItemNormalNode)
UIItemNormalNodeForSeasonS2 = UIItemNormalNodeForSeasonS2

function UIItemNormalNodeForSeasonS2:SetQuality(quality)
  if quality <= 0 then
    self:ShowQuality(false)
    return
  end
  local qualityName = "exp_s2_map_se" .. tostring(quality)
  if qualityName ~= "" then
    self:ShowQuality(true)
    local atlas = self._owner:GetAsset("UIS2Stage.spriteatlas", LoadType.SpriteAtlas)
...
function UIItemNormalNodeForSeasonS2:SetIconOffset(itemId)
