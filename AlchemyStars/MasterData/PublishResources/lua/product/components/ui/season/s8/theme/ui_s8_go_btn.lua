_class("UIS8GoBtn", UICustomWidget)
UIS8GoBtn = UIS8GoBtn

function UIS8GoBtn:OnShow()
  self._constBtnName = GameGlobal.GetUIModule(SeasonModule):GetGoBtnKey()
end

function UIS8GoBtn:OnHide()
end

function UIS8GoBtn:SetData(seasonId, component, playAnim)
...
function UIS8GoBtn:_Refresh(component)
...
function UIS8GoBtn:_SetState(state)
  self:GetGameObject("_lock"):SetActive(state == 2)
end

function UIS8GoBtn:_CalcNew()
...
function UIS8GoBtn:_CalcRed()
  local isOpen = self._component and self._component:ComponentIsOpen()
  if not isOpen then
    return isOpen
  end
  local lastTime = UISeasonLocalDBHelper.SeasonBtn_Get(self._constBtnName, "Red")
  Log.debug("UIS8GoBtn:_CalcRed() lastTime = ", lastTime)
