_class("UISeasonStageMultiAwardListS5", UICustomWidget)
UISeasonStageMultiAwardListS5 = UISeasonStageMultiAwardListS5

function UISeasonStageMultiAwardListS5:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonStageMultiAwardListS5:InitWidget()
  self._seasonAwardGen = self:GetUIComponent("UISelectObjectPath", "Content")
  self._sr = self:GetUIComponent("ScrollRect", "ScrollView")
end

function UISeasonStageMultiAwardListS5:OnHide()
end

function UISeasonStageMultiAwardListS5:SetData(multiAwardList)
  self._multiAwardList = multiAwardList
  local count = #self._multiAwardList
  self._seasonAwardGen:SpawnObjects("UISeasonStageSingleAwardListS5", count)
...
function UISeasonStageMultiAwardListS5:ResetScrollPos()
  self._sr.horizontalNormalizedPosition = 0
end

function UISeasonStageMultiAwardListS5:PlayAnim(totalAnimDelay)
  if self._list then
...
function UISeasonStageMultiAwardListS5:SetWaitAnim()
  if self._list then
    for index, singleList in ipairs(self._list) do
