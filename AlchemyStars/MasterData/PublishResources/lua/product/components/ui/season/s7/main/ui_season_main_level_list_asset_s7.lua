_class("UISeasonMainLevelListAssetS7", UICustomWidget)
UISeasonMainLevelListAssetS7 = UISeasonMainLevelListAssetS7

function UISeasonMainLevelListAssetS7:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonMainLevelListAssetS7:InitWidget()
  self.icon = self:GetUIComponent("Image", "icon")
  self.count = self:GetUIComponent("UILocalizationText", "count")
end

function UISeasonMainLevelListAssetS7:SetData(id, count)
  local zeros
  if 9999 < count then
