_class("UIS6ExchangeBtn", UICustomWidget)
UIS6ExchangeBtn = UIS6ExchangeBtn

function UIS6ExchangeBtn:OnShow()
  self._constBtnName = self:GetName()
  self:_AttachEvents()
end

function UIS6ExchangeBtn:OnHide()
  self:_DetachEvents()
end

function UIS6ExchangeBtn:SetData(seasonId, component, playAnim)
  ...
  self:_Refresh()
  if playAnim then
    UIWidgetHelper.PlayAnimationInSequence(self, "_anim", "_anim", "uieffanim_UIS6_ExchangeBtn_in", 0, 667)
  end
end

function UIS6ExchangeBtn:_Refresh()
...
function UIS6ExchangeBtn:_SetState(state)
  self:GetGameObject("_over"):SetActive(state == 2)
end

function UIS6ExchangeBtn:_SetIconText(itemId, widgetIcon, widgetText)
...
function UIS6ExchangeBtn:_AttachEvents()
  self:AttachEvent(GameEventType.ItemCountChanged, self._Refresh)
end

function UIS6ExchangeBtn:_DetachEvents()
