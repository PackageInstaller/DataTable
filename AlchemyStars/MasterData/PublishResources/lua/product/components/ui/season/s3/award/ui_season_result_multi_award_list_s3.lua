_class("UISeasonResultMultiAwardListS3", UICustomWidget)
UISeasonResultMultiAwardListS3 = UISeasonResultMultiAwardListS3

function UISeasonResultMultiAwardListS3:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonResultMultiAwardListS3:InitWidget()
  self._seasonAwardGen = self:GetUIComponent("UISelectObjectPath", "Content")
  self._sr = self:GetUIComponent("ScrollRect", "ScrollView")
end

function UISeasonResultMultiAwardListS3:OnHide()
end

function UISeasonResultMultiAwardListS3:SetData(matchRes, seasonMissionInfo)
  ...
  self._seasonAwardGen:SpawnObjects("UISeasonResultSingleAwardListS3", count)
...
function UISeasonResultMultiAwardListS3:ProcessCollectionItem(awardList, collectionList)
  local retList = {}
...
function UISeasonResultMultiAwardListS3:ResetScrollPos()
  self._sr.horizontalNormalizedPosition = 0
end

function UISeasonResultMultiAwardListS3:_GetItemCount(vecItem)
  local nItemCount = 0
  if vecItem then
    for i = 1, #vecItem do
...
function UISeasonResultMultiAwardListS3:BattleNormalResultSortAsset(assets)
...
function UISeasonResultMultiAwardListS3:GetPassAward()
  local awardHeadType, cfgId
  return UICommonHelper:GetInstance():GetPassAward(awardHeadType, cfgId)
end

function UISeasonResultMultiAwardListS3:HasItem(dataList, itemId)
  local isNormal = 0
  if dataList then
