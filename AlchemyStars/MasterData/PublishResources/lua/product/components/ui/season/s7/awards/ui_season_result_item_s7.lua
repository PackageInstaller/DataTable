...
_class("UISeasonResultItemS7", UICustomWidget)
UISeasonResultItemS7 = UISeasonResultItemS7

function UISeasonResultItemS7:Constructor()
...
function UISeasonResultItemS7:OnShow(uiParams)
  self._transform = self:GetGameObject().transform
  self._nodes = {}
  self._nodes[UIItemNode.Normal] = UIItemNormalNodeForSeasonS7:New(self:GetUIComponent("UIView", "g_normal"), self)
...
function UISeasonResultItemS7:OnHide()
...
function UISeasonResultItemS7:PlayAni(aniName)
  self._anim:Play(aniName)
end

function UISeasonResultItemS7:SetForm(type, scale, dontPlayAni)
...
function UISeasonResultItemS7:EnableNode(nodeId, isEnable)
...
function UISeasonResultItemS7:SetData(params)
...
function UISeasonResultItemS7:ShowNodes(uiItemNode, enable)
...
function UISeasonResultItemS7:SetScale(scale)
  scale = scale or 1
...
function UISeasonResultItemS7:Select(select, noAnim)
...
function UISeasonResultItemS7:SetClickCallBack(callBack, param)
...
function UISeasonResultItemS7:BtnOnClick(go)
  if self._clickCallBack then
    self._clickCallBack(go)
  end
end

function UISeasonResultItemS7:SetBtnImage(bImage)
end

function UISeasonResultItemS7:SetLongPressCallBack(longPressCallBack, longPressUpCallBack, pressTime, update)
...
function UISeasonResultItemS7:SetReduceLongPressCallBack(sec)
  if self._nodes[UIItemNode.Reduce] then
    self._nodes[UIItemNode.Reduce]:SetReduceLongPressCallBack(sec)
  end
end

function UISeasonResultItemS7:InitLongPress()
...
function UISeasonResultItemS7:LongEvent()
...
function UISeasonResultItemS7:LongEventUpLv(gradeLv)
  local cfg = Cfg.cfg_up_lv_long_press[gradeLv]
  if not cfg then
    Log.fatal("###[UISeasonResultItemS7] LongEventUpLv cfg is nil ! grade:", gradeLv)
  end
  local arr = cfg.Value
  local datas = {}
  for i = 1, #arr do
...
function UISeasonResultItemS7:Calculate(count)
...
function UISeasonResultItemS7:GetBtn()
  return self._btnGO
end

function UISeasonResultItemS7:SetToggleGroup(group)
...
function UISeasonResultItemS7:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  end
end

function UISeasonResultItemS7:SetToggleValue(isOn)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleValue(isOn)
  end
end

function UISeasonResultItemS7:GetItemData()
  return self._uiItemData
end

function UISeasonResultItemS7:ClearItemData()
  self._uiItemData = nil
end

function UISeasonResultItemS7:SetBtnImageByName(imgName, atlasName)
end

_class("UIItemNormalNodeForSeasonS7", UIItemNormalNode)
UIItemNormalNodeForSeasonS7 = UIItemNormalNodeForSeasonS7

function UIItemNormalNodeForSeasonS7:SetQuality(quality)
  if quality <= 0 then
...
function UIItemNormalNodeForSeasonS7:SetIconOffset(itemId)
...
function UIItemNormalNodeForSeasonS7:SetOffset(showText1)
end
