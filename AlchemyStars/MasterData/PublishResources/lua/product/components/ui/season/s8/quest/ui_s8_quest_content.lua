_class("UIS8QuestContent", UICustomWidget)
UIS8QuestContent = UIS8QuestContent

function UIS8QuestContent:Constructor()
  self._scrollViewInit = false
end

function UIS8QuestContent:CloseDialogWithAnim(callback)
  self:_PlayAnim("out", callback)
end

function UIS8QuestContent:OnShow(uiParams)
...
function UIS8QuestContent:OnHide()
  self:_Detach()
end

function UIS8QuestContent:SetData(params)
...
function UIS8QuestContent:_Refresh(isFirst)
  self:_DynamicSv(isFirst)
  self:_SetClaimAllBtn()
end

function UIS8QuestContent:_PlayAnim(type, callback)
  local tb = {
    ["in"] = {
...
function UIS8QuestContent:_SetGoBtn()
  ...
  local obj = UIWidgetHelper.SpawnObject(self, widgetName, "UIS8Title")
end

function UIS8QuestContent:_SetClaimAllBtn()
  local isShow = self._component:HasQuestCanClaim(self._cellDatas)
  self:GetGameObject("ClaimAllBtn"):SetActive(isShow)
end

function UIS8QuestContent:_SetCellListData()
...
function UIS8QuestContent:_DynamicSv(isFirst)
...
function UIS8QuestContent:DynamicSvInfo(scrollView, index)
...
function UIS8QuestContent:_Attach()
  self:AttachEvent(GameEventType.OnUIGetItemCloseInQuest, self.OnUIGetItemCloseInQuest)
end

function UIS8QuestContent:_Detach()
  self:DetachEvent(GameEventType.OnUIGetItemCloseInQuest, self.OnUIGetItemCloseInQuest)
end

function UIS8QuestContent:OnUIGetItemCloseInQuest()
...
function UIS8QuestContent:SetResponseEvent(val)
