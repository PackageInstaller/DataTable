_class("UISeasonLevelStageS7", UIController)
UISeasonLevelStageS7 = UISeasonLevelStageS7

function UISeasonLevelStageS7:_GetComponents()
...
function UISeasonLevelStageS7:OnShow(uiParams)
...
function UISeasonLevelStageS7:LoadDataOnEnter(TT, res)
end

function UISeasonLevelStageS7.GetAutoOpenState(matchType, stageId)
...
function UISeasonLevelStageS7.SetAutoOpenState(matchType, stageId, isOpen)
  local roleModule = GameGlobal.GetModule(RoleModule)
  local pstId = roleModule:GetPstId()
  local playerPrefsKey = pstId .. "AutoOpenState" .. matchType
  if stageId then
...
function UISeasonLevelStageS7:_CanAutoFight()
  local tipsStr = ""
...
function UISeasonLevelStageS7:_CanShowAutoFight()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS7:InitAutoBtnState()
...
function UISeasonLevelStageS7:_CheckShowWordBuff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS7:OnHide()
...
function UISeasonLevelStageS7:HasPassThreeStar(missionCfg)
...
function UISeasonLevelStageS7:GetSortedArr(awardType, cfg, stageAwardType)
...
function UISeasonLevelStageS7:ProcessAward(missionCfg, noNormal)
  local missionId = missionCfg.ID
  local awards = {}
...
function UISeasonLevelStageS7:RefreshAwardsShowState()
  self._noDiffAwardGo = self:GetGameObject("NoDiffAward")
  self._seasonAwardGo = self:GetGameObject("SeasonAward")
  self._seasonNormalAwardGo = self:GetGameObject("MultiAwardGroup1")
  self._seasonHardAwardGo = self:GetGameObject("MultiAwardGroup2")
...
function UISeasonLevelStageS7:RefreshWordsArea()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS7:InitWords()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS7:_InitWords(sop, missionId)
...
function UISeasonLevelStageS7:RefreshBtnUnknown()
  local show = self:_CheckShowWordBuff()
  self.btnWord:SetActive(show)
end

function UISeasonLevelStageS7:InitForMultiDiff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS7:RefreshPanelShow()
...
function UISeasonLevelStageS7:RefreshPanelBg()
end

function UISeasonLevelStageS7:RefreshPanelText()
  if self._curPanel == UISeasonLevelStagePanelMode.AwardsInfo then
    self._awardsPanelBtnText.color = Color(0.996078431372549, 0.9137254901960784, 0.6784313725490196, 1)
    self._battleInfoPanelBtnText.color = Color(0.7215686274509804, 0.5882352941176471, 0.4588235294117647, 1)
  else
    self._awardsPanelBtnText.color = Color(0.7215686274509804, 0.5882352941176471, 0.4588235294117647, 1)
    self._battleInfoPanelBtnText.color = Color(0.996078431372549, 0.9137254901960784, 0.6784313725490196, 1)
  end
end

function UISeasonLevelStageS7:RefreshDiffArea()
end

function UISeasonLevelStageS7:InitAllAwards()
  if self._hasMultiDiff then
    ...
    self._normalMultiAwardList = normalAwardGen:SpawnObject("UISeasonStageMultiAwardListS7")
    ...
    self._hardMultiAwardList = hardAwardGen:SpawnObject("UISeasonStageMultiAwardListS7")
...
function UISeasonLevelStageS7:PlayAnimAwardList(totalAnimDelay)
...
function UISeasonLevelStageS7:_ProcessLevelDiffAwards(missionId, levelDiff, noNormal)
  if missionId then
...
function UISeasonLevelStageS7:Init()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS7:UpdateCondition(three_star_condition, conditions)
...
function UISeasonLevelStageS7:AutoFightBtnOnClick()
...
function UISeasonLevelStageS7:BtnFightOnClick(go)
  self:_DoFight()
end

function UISeasonLevelStageS7:_DoFight(autoFightCount)
  if not self._seasonModule:CheckSeasonAndMissionCoseAndJump() then
    return
  end
...
function UISeasonLevelStageS7:IsPowerEnough()
...
function UISeasonLevelStageS7:ShowTips(itemId, pos)
  self._tips:SetData(itemId, pos)
end

function UISeasonLevelStageS7:BgOnClick()
  self:Close()
end

function UISeasonLevelStageS7:ThreeStarTipsBtnOnClick(go)
  self:ShowDialog("UIThreeStarTips")
end

function UISeasonLevelStageS7:BtnUnknownOnClick(go)
  local buffData = {}
  buffData.name = ""
  buffData.des = ""
...
function UISeasonLevelStageS7:_SetStoryBtn()
...
function UISeasonLevelStageS7:BtnPlotOnClick()
  local storyList = DiscoveryStoryList:New()
  storyList:Init(self._missionID)
...
function UISeasonLevelStageS7:SliderHardOnClick()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS7:SliderNormalOnClick()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
      return
    end
...
function UISeasonLevelStageS7:_SwitchDiff()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
...
function UISeasonLevelStageS7:DiffInfoBtnOnClick(go)
  self:ShowDialog("UISeasonStageDiffTips")
end

function UISeasonLevelStageS7:RefreshPowerTips()
  ...
  local cur, ceil = self._pointComp:GetItemCount()
  contentText:SetText(string.format("%s/%s", cur, ceil))
end

function UISeasonLevelStageS7:RefreshPowerCost()
  local needPowerText = self._needPower
  if self._powerID == RoleAssetID.RoleAssetPhyPoint then
  else
    if not self._pointComp then
  ...
  if self._mapManager:Mode() == SeasonMapMode.Mode1 then
    self._txtCost.gameObject.transform.parent.gameObject:SetActive(false)
    self.fightBtnTextTr.gameObject.transform.localPosition = Vector3(0, self.fightBtnTextTr.gameObject.transform.localPosition.y, self.fightBtnTextTr.gameObject.transform.localPosition.z)
  else
    self._txtCost.gameObject.transform.parent.gameObject:SetActive(true)
  end
  self._txtCost:SetText(needPowerText)
end

function UISeasonLevelStageS7:InitChapterName()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS7:_InitChapterName(text, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
...
function UISeasonLevelStageS7:RefreshChapter()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS7:RefreshThreeStarArea(playAnim, animDelay)
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS7:PlayAnimThreeStarArea(totalAnimDelay)
...
function UISeasonLevelStageS7:InitEnemys()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS7:_InitEnemy(sop, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
  local progress = self._curLevelProgress
...
function UISeasonLevelStageS7:RefreshEnemyArea()
...
function UISeasonLevelStageS7:RefreshRecommendLv()
...
function UISeasonLevelStageS7:RefreshAreasTitleStyle()
end

function UISeasonLevelStageS7:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffStageAreaS7")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonLevelStageS7:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonLevelStageS7:Close()
  self:_CloseDialogWithAnim()
end

function UISeasonLevelStageS7:_CloseDialogWithAnim(callback)
  UIWidgetHelper.PlayAnimation(self, "UICanvas", "uieff_UISeasonLevelStageS7_out", 333, function()
    if callback then
      callback()
    end
    self:_Resume()
...
function UISeasonLevelStageS7:_Resume()
...
function UISeasonLevelStageS7:BattleInfoPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.BattleInfo
  self:RefreshPanelShow()
  self:RefreshPanelText()
end

function UISeasonLevelStageS7:AwardsPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.AwardsInfo
  self:RefreshPanelShow()
  self:RefreshPanelText()
end

function UISeasonLevelStageS7:PowerTopTipsOnClick(go)
  self:ShowDialog("UISeasonActionPointTipS7", self._pointComp, go.transform.anchoredPosition)
end

function UISeasonLevelStageS7:_Countdown()
...
function UISeasonLevelStageS7:_ReqFlushPower(TT)
...
function UISeasonLevelStageS7:OnAfterUILayerChanged()
...
function UISeasonLevelStageS7:ShowMainUI(show)
...
function UISeasonLevelStageS7:RecordOnClick(go)
  self:ShowDialog("UIActivityLevelRecordController", self._component, self._missionID)
end

function UISeasonLevelStageS7:_TryFixStory(TT)
  local normalMissionID = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
...
function UISeasonLevelStageS7:_ReqFinishStory(TT, id, type)
