_class("UISeasonStageAwardItemS8", UICustomWidget)
UISeasonStageAwardItemS8 = UISeasonStageAwardItemS8

function UISeasonStageAwardItemS8:OnShow()
  self._trans = self:GetGameObject()
  self:GetComponents()
end

function UISeasonStageAwardItemS8:GetComponents()
  self._trans = self:GetGameObject()
  ...
  self.atlas = self:GetAsset("UIS7Scene.spriteatlas", LoadType.SpriteAtlas)
...
function UISeasonStageAwardItemS8:Flush(v, levelDiff)
  if not v then
    return
  end
  self._v = v
  ...
  if awardType == StageAwardType.First then
    ...
    textColor = Color(0.47058823529411764, 0.6039215686274509, 0.7215686274509804, 1)
  elseif awardType == StageAwardType.Star then
    strKey = "str_season_3star_award"
    textColor = Color(0.8823529411764706, 0.2980392156862745, 0.20392156862745098, 1)
  elseif awardType == StageAwardType.Activity then
    ...
    textColor = Color(0.7215686274509804, 0.6862745098039216, 0.6274509803921569, 1)
  elseif awardType == StageAwardType.HasGen then
    strKey = "str_season_already_collect"
  else
    strKey = "str_season_normal_award"
    textColor = Color(0.12941176470588237, 0.47058823529411764, 0.8156862745098039, 1)
  end
  self:SetText1(text1)
...
function UISeasonStageAwardItemS8:SetIcon(name, itemId)
...
function UISeasonStageAwardItemS8:ShowIcon(show, itemId)
  self._iconGO:SetActive(show)
  self:SetIconOffset(itemId)
end

function UISeasonStageAwardItemS8:SetIconOffset(itemId)
...
function UISeasonStageAwardItemS8:_IsPet(id)
  local cfg = Cfg.cfg_pet({ID = id})
  return cfg and true or false
end

function UISeasonStageAwardItemS8:SetQuality(quality)
  if quality <= 0 then
    quality = 1
  end
  local qualityName = "exp_s7_map_daoju_pinji0" .. tostring(quality)
  if qualityName ~= "" then
...
function UISeasonStageAwardItemS8:ShowQuality(show)
  self._qualityGO:SetActive(show)
end

function UISeasonStageAwardItemS8:SetText1(text, showNumber)
  if text ~= nil then
    local show = false
...
function UISeasonStageAwardItemS8:ShowText1(show)
  self._text1GO:SetActive(show)
  self:SetOffset(show)
end

function UISeasonStageAwardItemS8:SetOffset(showText1)
end

function UISeasonStageAwardItemS8:SetAwardText(awardText, color)
  self._awardTxt:SetText(awardText)
  self._awardTxt.color = color
end

function UISeasonStageAwardItemS8:SetTypeBg(typeBgName)
end

function UISeasonStageAwardItemS8:SetTypeMainBg(typeBgName)
end

function UISeasonStageAwardItemS8:SetTopAward()
  self:SetTopText(self._topTextStr)
end

function UISeasonStageAwardItemS8:SetTopText(text)
...
function UISeasonStageAwardItemS8:ShowTopText(show)
  self._topGo:SetActive(show)
end

function UISeasonStageAwardItemS8:SetAct()
...
function UISeasonStageAwardItemS8:ShowText(show)
  self._actGo:SetActive(show)
end

function UISeasonStageAwardItemS8:BtnOnClick(go)
...
function UISeasonStageAwardItemS8:PlayAnim(totalDelay)
...
function UISeasonStageAwardItemS8:_PlayInAnim()
  if self._anim then
...
function UISeasonStageAwardItemS8:OnHide()
  if self._animTimer then
    GameGlobal.Timer():CancelEvent(self._animTimer)
    self._animTimer = nil
  end
...
function UISeasonStageAwardItemS8:SetWaitAnim()
  if self._rootGo then
