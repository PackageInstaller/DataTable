_class("UISeasonResultMultiAwardListS6", UICustomWidget)
UISeasonResultMultiAwardListS6 = UISeasonResultMultiAwardListS6

function UISeasonResultMultiAwardListS6:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonResultMultiAwardListS6:InitWidget()
  self._seasonAwardGen = self:GetUIComponent("UISelectObjectPath", "Content")
  self._sr = self:GetUIComponent("ScrollRect", "ScrollView")
end

function UISeasonResultMultiAwardListS6:OnHide()
end

function UISeasonResultMultiAwardListS6:SetData(matchRes, seasonMissionInfo)
  ...
  self._seasonAwardGen:SpawnObjects("UISeasonResultSingleAwardListS6", count)
...
function UISeasonResultMultiAwardListS6:ProcessCollectionItem(awardList, collectionList)
  local retList = {}
...
function UISeasonResultMultiAwardListS6:ResetScrollPos()
  self._sr.horizontalNormalizedPosition = 0
end

function UISeasonResultMultiAwardListS6:_GetItemCount(vecItem)
  local nItemCount = 0
  if vecItem then
    for i = 1, #vecItem do
...
function UISeasonResultMultiAwardListS6:BattleNormalResultSortAsset(assets)
...
function UISeasonResultMultiAwardListS6:GetPassAward()
  local awardHeadType, cfgId
  return UICommonHelper:GetInstance():GetPassAward(awardHeadType, cfgId)
end

function UISeasonResultMultiAwardListS6:HasItem(dataList, itemId)
  local isNormal = 0
  if dataList then
