_class("UIS4ExchangeConfirm", UIController)
UIS4ExchangeConfirm = UIS4ExchangeConfirm

function UIS4ExchangeConfirm:LoadDataOnEnter(TT, res, uiParams)
  ...
  if self._info == nil then
    Log.error("UIS4ExchangeConfirm:LoadDataOnEnter() param = nil")
    res:SetSucc(false)
    return
  end
end

function UIS4ExchangeConfirm:OnShow(uiParams)
...
function UIS4ExchangeConfirm:OnHide()
  self:_DetachEvents()
end

function UIS4ExchangeConfirm:_Refresh()
  self:_SetBuyCount(self._buyCount)
  self:_SetCoinCount(self._buyCount)
end

function UIS4ExchangeConfirm:_UpdateBuyCount(newCount)
  local min, max = self:_ConstMin(), self:_CalcMaxCount()
  self._buyCount = Mathf.Clamp(newCount, min, max)
  self:_Refresh()
end

function UIS4ExchangeConfirm:_SetState(state)
...
function UIS4ExchangeConfirm:_SetItem(itemId, itemCount)
...
function UIS4ExchangeConfirm:_SetCoin()
...
function UIS4ExchangeConfirm:_SetCoinCount(buyCount)
...
function UIS4ExchangeConfirm:_ConstMin()
  return 1
end

function UIS4ExchangeConfirm:_ConstMax()
  local constMax = 99
  local max = self._component:GetCanExchangeCount(self._info, constMax)
  return Mathf.Max(max, 1)
end

function UIS4ExchangeConfirm:_CalcMinCount()
  local min, max = self:_ConstMin(), self:_ConstMax()
  return min
end

function UIS4ExchangeConfirm:_CalcMaxCount()
  local min, max = self:_ConstMin(), self:_ConstMax()
...
function UIS4ExchangeConfirm:_CalcHaveCount()
  local itemId = self._info.m_cost_item_id
...
function UIS4ExchangeConfirm:_CalcTotalPrice(buyCount)
  local price = self._info.m_cost_count
  local total = price * buyCount
  return total
end

function UIS4ExchangeConfirm:_CheckCoinEnough(buyCount)
  local haveCount = self:_CalcHaveCount()
  local total = self:_CalcTotalPrice(buyCount)
  return haveCount >= total
end

function UIS4ExchangeConfirm:MinBtnOnClick()
  local newCount = self:_CalcMinCount()
  self:_UpdateBuyCount(newCount)
end

function UIS4ExchangeConfirm:SubBtnOnClick()
  local newCount = self._buyCount - 1
  self:_UpdateBuyCount(newCount)
end

function UIS4ExchangeConfirm:AddBtnOnClick()
  local newCount = self._buyCount + 1
  self:_UpdateBuyCount(newCount)
end

function UIS4ExchangeConfirm:MaxBtnOnClick()
  local newCount = self:_CalcMaxCount()
  self:_UpdateBuyCount(newCount)
end

function UIS4ExchangeConfirm:BuyBtnOnClick()
...
function UIS4ExchangeConfirm:_OnReceiveRewards(res, rewards)
...
function UIS4ExchangeConfirm:PetDetailBtnOnClick(go)
  self:ShowDialog("UIShopPetDetailController", self.goodData:GetItemId())
end

function UIS4ExchangeConfirm:CloseBtnOnClick(go)
  self:_CloseDialogWithAnim()
end

function UIS4ExchangeConfirm:_AttachEvents()
  self:AttachEvent(GameEventType.ItemCountChanged, self._Refresh)
end

function UIS4ExchangeConfirm:_DetachEvents()
  self:DetachEvent(GameEventType.ItemCountChanged, self._Refresh)
end

function UIS4ExchangeConfirm:_CloseDialogWithAnim(callback)
  self:CloseDialog()
end
