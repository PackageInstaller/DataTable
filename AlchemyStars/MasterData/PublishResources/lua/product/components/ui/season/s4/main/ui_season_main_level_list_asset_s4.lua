_class("UISeasonMainLevelListAssetS4", UICustomWidget)
UISeasonMainLevelListAssetS4 = UISeasonMainLevelListAssetS4

function UISeasonMainLevelListAssetS4:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonMainLevelListAssetS4:InitWidget()
  self.icon = self:GetUIComponent("Image", "icon")
  self.count = self:GetUIComponent("UILocalizationText", "count")
end

function UISeasonMainLevelListAssetS4:SetData(id, count)
  local zeros
  if 9999 < count then
