_class("UISeasonShowAwardsS7", UIController)
UISeasonShowAwardsS7 = UISeasonShowAwardsS7

function UISeasonShowAwardsS7:GetComponents()
...
function UISeasonShowAwardsS7:OnShow(uiParams)
  ...
  if uiParams[1] then
    if table.count(uiParams[1]) == 0 then
      Log.fatal("###[UISeasonShowAwardsS7] table.count(uiParams[1]) == 0 !")
    end
  else
    Log.fatal("###[UISeasonShowAwardsS7] uiParams[1] is nil !")
  end
  if uiParams[3] then
    itemlist = uiParams[1]
  else
...
function UISeasonShowAwardsS7:CreateData(itemlist)
...
function UISeasonShowAwardsS7:DoAnimation()
  self._canvasGroup = self:GetUIComponent("CanvasGroup", "canvasGroup")
  self._canvasGroup.alpha = 0
  self._tweener = self._canvasGroup:DOFade(1, 0.02)
end

function UISeasonShowAwardsS7:ClosePanel()
  self:CloseDialog()
end

function UISeasonShowAwardsS7:OnHide()
  if self._closeCallback then
    self._closeCallback()
  end
end

function UISeasonShowAwardsS7:_InitListView(scrollView, index)
  if index < 0 then
    return nil
  end
  local count
  ...
  if item.IsInitHandlerCalled == false then
    ...
    rowPool:SpawnObjects("UISeasonShowAwardsItemS7", count)
  end
...
function UISeasonShowAwardsS7:_ShowItem(giftItem, index, tweenIdx)
  local beforeTime = 0
...
function UISeasonShowAwardsS7:OnItemSelect(id, count, go)
  GameGlobal.EventDispatcher():Dispatch(GameEventType.ShowItemTips, id, self._trans.transform.position)
end

function UISeasonShowAwardsS7:NextOnClick(go)
...
function UISeasonShowAwardsS7:CalcPage()
...
function UISeasonShowAwardsS7:_GetNextPageIndex()
...
function UISeasonShowAwardsS7:_GetCurPageFirstIndex()
  return self._curPageFirstIndex
end

function UISeasonShowAwardsS7:_GetItemDataByIndex(index)
...
function UISeasonShowAwardsS7:_FormatItemCount(itemCount)
  return HelperProxy:GetInstance():FormatItemCount(itemCount)
end

function UISeasonShowAwardsS7:ShowTips(itemId, pos)
