_class("UISeasonMainLevelListAssetS3", UICustomWidget)
UISeasonMainLevelListAssetS3 = UISeasonMainLevelListAssetS3

function UISeasonMainLevelListAssetS3:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonMainLevelListAssetS3:InitWidget()
  self.icon = self:GetUIComponent("Image", "icon")
  self.count = self:GetUIComponent("UILocalizationText", "count")
end

function UISeasonMainLevelListAssetS3:SetData(id, count)
  local zeros
  if 9999 < count then
