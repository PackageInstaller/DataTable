_class("UISeasonResultMultiAwardListS2", UICustomWidget)
UISeasonResultMultiAwardListS2 = UISeasonResultMultiAwardListS2

function UISeasonResultMultiAwardListS2:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonResultMultiAwardListS2:InitWidget()
  self._seasonAwardGen = self:GetUIComponent("UISelectObjectPath", "Content")
  self._sr = self:GetUIComponent("ScrollRect", "ScrollView")
end

function UISeasonResultMultiAwardListS2:OnHide()
end

function UISeasonResultMultiAwardListS2:SetData(matchRes, seasonMissionInfo)
  ...
  self._seasonAwardGen:SpawnObjects("UISeasonResultSingleAwardListS2", count)
...
function UISeasonResultMultiAwardListS2:ProcessCollectionItem(awardList, collectionList)
  local retList = {}
...
function UISeasonResultMultiAwardListS2:ResetScrollPos()
  self._sr.horizontalNormalizedPosition = 0
end

function UISeasonResultMultiAwardListS2:_GetItemCount(vecItem)
  local nItemCount = 0
  if vecItem then
    for i = 1, #vecItem do
...
function UISeasonResultMultiAwardListS2:BattleNormalResultSortAsset(assets)
...
function UISeasonResultMultiAwardListS2:GetPassAward()
  local awardHeadType, cfgId
  return UICommonHelper:GetInstance():GetPassAward(awardHeadType, cfgId)
end

function UISeasonResultMultiAwardListS2:HasItem(dataList, itemId)
  local isNormal = 0
  if dataList then
