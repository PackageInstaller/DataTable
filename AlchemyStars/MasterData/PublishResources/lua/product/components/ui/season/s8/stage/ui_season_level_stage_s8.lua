_class("UISeasonLevelStageS8", UIController)
UISeasonLevelStageS8 = UISeasonLevelStageS8

function UISeasonLevelStageS8:_GetComponents()
...
function UISeasonLevelStageS8:OnShow(uiParams)
...
function UISeasonLevelStageS8:LoadDataOnEnter(TT, res)
end

function UISeasonLevelStageS8.GetAutoOpenState(matchType, stageId)
...
function UISeasonLevelStageS8.SetAutoOpenState(matchType, stageId, isOpen)
  local roleModule = GameGlobal.GetModule(RoleModule)
  local pstId = roleModule:GetPstId()
  local playerPrefsKey = pstId .. "AutoOpenState" .. matchType
  if stageId then
...
function UISeasonLevelStageS8:_CanAutoFight()
  local tipsStr = ""
...
function UISeasonLevelStageS8:_CanShowAutoFight()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS8:InitAutoBtnState()
...
function UISeasonLevelStageS8:_CheckShowWordBuff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS8:OnHide()
...
function UISeasonLevelStageS8:HasPassThreeStar(missionCfg)
...
function UISeasonLevelStageS8:GetSortedArr(awardType, cfg, stageAwardType)
...
function UISeasonLevelStageS8:ProcessAward(missionCfg, noNormal)
  local missionId = missionCfg.ID
  local awards = {}
...
function UISeasonLevelStageS8:RefreshAwardsShowState()
  self._noDiffAwardGo = self:GetGameObject("NoDiffAward")
  self._seasonAwardGo = self:GetGameObject("SeasonAward")
  self._seasonNormalAwardGo = self:GetGameObject("MultiAwardGroup1")
  self._seasonHardAwardGo = self:GetGameObject("MultiAwardGroup2")
...
function UISeasonLevelStageS8:RefreshWordsArea()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS8:InitWords()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS8:_InitWords(sop, missionId)
...
function UISeasonLevelStageS8:_GetWordDesc(levelId, wordId)
...
function UISeasonLevelStageS8:RefreshBtnUnknown()
  local show = self:_CheckShowWordBuff()
  self.btnWord:SetActive(show)
end

function UISeasonLevelStageS8:InitForMultiDiff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS8:RefreshPanelShow()
...
function UISeasonLevelStageS8:RefreshPanelBg()
end

function UISeasonLevelStageS8:RefreshPanelText()
  if self._curPanel == UISeasonLevelStagePanelMode.AwardsInfo then
    self._awardsPanelBtnText.color = Color(1.0, 1.0, 1.0, 1)
    self._battleInfoPanelBtnText.color = Color(1.0, 0.9764705882352941, 0.9725490196078431, 0.8)
  else
    self._awardsPanelBtnText.color = Color(1.0, 1.0, 1.0, 1)
    self._battleInfoPanelBtnText.color = Color(1.0, 0.9764705882352941, 0.9725490196078431, 0.8)
  end
...
function UISeasonLevelStageS8:RefreshDiffArea()
end

function UISeasonLevelStageS8:InitAllAwards()
  if self._hasMultiDiff then
    ...
    self._normalMultiAwardList = normalAwardGen:SpawnObject("UISeasonStageMultiAwardListS8")
    ...
    self._hardMultiAwardList = hardAwardGen:SpawnObject("UISeasonStageMultiAwardListS8")
...
function UISeasonLevelStageS8:PlayAnimAwardList(totalAnimDelay)
...
function UISeasonLevelStageS8:_ProcessLevelDiffAwards(missionId, levelDiff, noNormal)
  if missionId then
...
function UISeasonLevelStageS8:Init()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS8:UpdateCondition(three_star_condition, conditions)
...
function UISeasonLevelStageS8:AutoFightBtnOnClick()
...
function UISeasonLevelStageS8:BtnFightOnClick(go)
  self:_DoFight()
end

function UISeasonLevelStageS8:_DoFight(autoFightCount)
  if not self._seasonModule:CheckSeasonAndMissionCoseAndJump() then
    return
  end
...
function UISeasonLevelStageS8:IsPowerEnough()
...
function UISeasonLevelStageS8:ShowTips(itemId, pos)
  self._tips:SetData(itemId, pos)
end

function UISeasonLevelStageS8:BgOnClick()
  self:Close()
end

function UISeasonLevelStageS8:ThreeStarTipsBtnOnClick(go)
  self:ShowDialog("UIThreeStarTips")
end

function UISeasonLevelStageS8:BtnUnknownOnClick(go)
  local buffData = {}
  buffData.name = ""
  buffData.des = ""
...
function UISeasonLevelStageS8:_SetStoryBtn()
...
function UISeasonLevelStageS8:BtnPlotOnClick()
  local storyList = DiscoveryStoryList:New()
  storyList:Init(self._missionID)
...
function UISeasonLevelStageS8:SliderHardOnClick()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS8:SliderNormalOnClick()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
      return
    end
...
function UISeasonLevelStageS8:_SwitchDiff()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
...
function UISeasonLevelStageS8:DiffInfoBtnOnClick(go)
  self:ShowDialog("UISeasonStageDiffTips")
end

function UISeasonLevelStageS8:RefreshPowerTips()
...
function UISeasonLevelStageS8:RefreshPowerCost()
  local needPowerText = self._needPower
  if self._powerID == RoleAssetID.RoleAssetPhyPoint then
  else
    if not self._pointComp then
...
function UISeasonLevelStageS8:InitChapterName()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS8:_InitChapterName(text, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
...
function UISeasonLevelStageS8:RefreshChapter()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS8:RefreshThreeStarArea(playAnim, animDelay)
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS8:PlayAnimThreeStarArea(totalAnimDelay)
...
function UISeasonLevelStageS8:InitEnemys()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS8:_InitEnemy(sop, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
  local progress = self._curLevelProgress
...
function UISeasonLevelStageS8:RefreshEnemyArea()
...
function UISeasonLevelStageS8:RefreshRecommendLv()
...
function UISeasonLevelStageS8:RefreshAreasTitleStyle()
end

function UISeasonLevelStageS8:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffStageAreaS8")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonLevelStageS8:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonLevelStageS8:Close()
  self:_CloseDialogWithAnim()
end

function UISeasonLevelStageS8:_CloseDialogWithAnim(callback)
  UIWidgetHelper.PlayAnimation(self, "UICanvas", "uieff_UISeasonLevelStageS8_out", 333, function()
    if callback then
      callback()
    end
    self:_Resume()
...
function UISeasonLevelStageS8:_Resume()
...
function UISeasonLevelStageS8:BattleInfoPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.BattleInfo
  self:RefreshPanelShow()
  self:RefreshPanelText()
end

function UISeasonLevelStageS8:AwardsPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.AwardsInfo
  self:RefreshPanelShow()
  self:RefreshPanelText()
end

function UISeasonLevelStageS8:PowerTopTipsOnClick(go)
  self:ShowDialog("UISeasonActionPointTipS8", self._pointComp, go.transform.anchoredPosition)
end

function UISeasonLevelStageS8:_Countdown()
...
function UISeasonLevelStageS8:_ReqFlushPower(TT)
...
function UISeasonLevelStageS8:OnAfterUILayerChanged()
...
function UISeasonLevelStageS8:ShowMainUI(show)
...
function UISeasonLevelStageS8:RecordOnClick(go)
  self:ShowDialog("UIActivityLevelRecordController", self._component, self._missionID)
end

function UISeasonLevelStageS8:_TryFixStory(TT)
  local normalMissionID = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
...
function UISeasonLevelStageS8:_ReqFinishStory(TT, id, type)
