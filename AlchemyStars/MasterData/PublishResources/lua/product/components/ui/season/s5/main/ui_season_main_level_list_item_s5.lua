_class("UISeasonMainLevelListItemS5", UICustomWidget)
UISeasonMainLevelListItemS5 = UISeasonMainLevelListItemS5

function UISeasonMainLevelListItemS5:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonMainLevelListItemS5:InitWidget()
...
function UISeasonMainLevelListItemS5:SetData(data, onClick)
  self._data = data
  self._onClick = onClick
  self._lockTip:SetActive(not self._data:IsUnlock())
end

function UISeasonMainLevelListItemS5:RefreshByDiff(curDiff)
  ...
  if isHasProduceLevel then
    ...
    awards = tmp
    local items = self.item:SpawnObjects("UISeasonMainLevelListAssetS5", #awards)
    for i = 1, #awards do
      local item = items[i]
  ...
  if isStoryLevel then
    self._bg.sprite = atlas:GetSprite("exp_s5_map_gqlb_di01")
  else
    self._bg.sprite = atlas:GetSprite("exp_s5_map_gqlb_di02")
  end
end

function UISeasonMainLevelListItemS5:PrepareAnim()
  self._root:SetActive(false)
end

function UISeasonMainLevelListItemS5:PlayEnterAnim()
  self._root:SetActive(true)
  self._anim:Play("uieff_UISeasonMainLevelListItemS3_in")
end

function UISeasonMainLevelListItemS5:PlaySwitchAnim()
  self._root:SetActive(true)
end

function UISeasonMainLevelListItemS5:PlayOutAnim()
  self._anim:Play("uieff_UISeasonMainLevelListItemS2_out")
end

function UISeasonMainLevelListItemS5:RootOnClick(go)
