_class("UISeasonLevelStageS4", UIController)
UISeasonLevelStageS4 = UISeasonLevelStageS4

function UISeasonLevelStageS4:_GetComponents()
...
function UISeasonLevelStageS4:OnShow(uiParams)
...
function UISeasonLevelStageS4:LoadDataOnEnter(TT, res)
end

function UISeasonLevelStageS4.GetAutoOpenState(matchType, stageId)
...
function UISeasonLevelStageS4.SetAutoOpenState(matchType, stageId, isOpen)
  local roleModule = GameGlobal.GetModule(RoleModule)
  local pstId = roleModule:GetPstId()
  local playerPrefsKey = pstId .. "AutoOpenState" .. matchType
  if stageId then
...
function UISeasonLevelStageS4:_CanAutoFight()
  local tipsStr = ""
...
function UISeasonLevelStageS4:_CanShowAutoFight()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS4:InitAutoBtnState()
...
function UISeasonLevelStageS4:_CheckShowWordBuff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS4:OnHide()
...
function UISeasonLevelStageS4:HasPassThreeStar(missionCfg)
...
function UISeasonLevelStageS4:GetSortedArr(awardType, cfg, stageAwardType)
...
function UISeasonLevelStageS4:ProcessAward(missionCfg, noNormal)
  local missionId = missionCfg.ID
  local awards = {}
...
function UISeasonLevelStageS4:RefreshAwardsShowState()
  self._noDiffAwardGo = self:GetGameObject("NoDiffAward")
  self._seasonAwardGo = self:GetGameObject("SeasonAward")
  self._seasonNormalAwardGo = self:GetGameObject("MultiAwardGroup1")
  self._seasonHardAwardGo = self:GetGameObject("MultiAwardGroup2")
...
function UISeasonLevelStageS4:RefreshWordsArea()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS4:InitWords()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS4:_InitWords(sop, missionId)
...
function UISeasonLevelStageS4:_GetWordDesc(levelId, wordId)
...
function UISeasonLevelStageS4:RefreshBtnUnknown()
  local show = self:_CheckShowWordBuff()
  self.btnWord:SetActive(show)
end

function UISeasonLevelStageS4:InitForMultiDiff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS4:RefreshPanelShow()
...
function UISeasonLevelStageS4:RefreshPanelText()
  if self._missionDiff == UISeasonLevelDiff.Normal then
    if self._curPanel == UISeasonLevelStagePanelMode.AwardsInfo then
...
function UISeasonLevelStageS4:RefreshDiffArea()
  if self._missionDiff == UISeasonLevelDiff.Normal then
    self._normalBtnText.color = Color(0.98, 0.957, 0.875, 1)
    self._hardBtnText.color = Color(0.624, 0.608, 0.561, 1)
    self._normalMask:SetActive(false)
    self._hardMask:SetActive(true)
  elseif self._missionDiff == UISeasonLevelDiff.Hard then
    self._normalBtnText.color = Color(0.624, 0.608, 0.561, 1)
    self._hardBtnText.color = Color(0.98, 0.957, 0.875, 1)
    self._hardMask:SetActive(false)
    self._normalMask:SetActive(true)
  end
end

function UISeasonLevelStageS4:InitAllAwards()
  if self._hasMultiDiff then
    ...
    self._normalMultiAwardList = normalAwardGen:SpawnObject("UISeasonStageMultiAwardListS4")
    ...
    self._hardMultiAwardList = hardAwardGen:SpawnObject("UISeasonStageMultiAwardListS4")
...
function UISeasonLevelStageS4:PlayAnimAwardList(totalAnimDelay)
...
function UISeasonLevelStageS4:_ProcessLevelDiffAwards(missionId, levelDiff, noNormal)
  if missionId then
...
function UISeasonLevelStageS4:Init()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS4:UpdateCondition(three_star_condition, conditions)
...
function UISeasonLevelStageS4:AutoFightBtnOnClick()
...
function UISeasonLevelStageS4:BtnFightOnClick(go)
  self:_DoFight()
end

function UISeasonLevelStageS4:_DoFight(autoFightCount)
  if not self._seasonModule:CheckSeasonAndMissionCoseAndJump() then
    return
  end
...
function UISeasonLevelStageS4:IsPowerEnough()
...
function UISeasonLevelStageS4:ShowTips(itemId, pos)
  self._tips:SetData(itemId, pos)
end

function UISeasonLevelStageS4:BgOnClick()
  self:Close()
end

function UISeasonLevelStageS4:ThreeStarTipsBtnOnClick(go)
  self:ShowDialog("UIThreeStarTips")
end

function UISeasonLevelStageS4:BtnUnknownOnClick(go)
  local buffData = {}
  buffData.name = ""
  buffData.des = ""
...
function UISeasonLevelStageS4:_SetStoryBtn()
...
function UISeasonLevelStageS4:BtnPlotOnClick()
  local storyList = DiscoveryStoryList:New()
  storyList:Init(self._missionID)
...
function UISeasonLevelStageS4:SliderHardOnClick()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS4:SliderNormalOnClick()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
      return
    end
...
function UISeasonLevelStageS4:_SwitchDiff()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
...
function UISeasonLevelStageS4:DiffInfoBtnOnClick(go)
  self:ShowDialog("UISeasonStageDiffTips")
end

function UISeasonLevelStageS4:RefreshPowerTips()
...
function UISeasonLevelStageS4:RefreshPowerCost()
  local needPowerText = self._needPower
  if self._powerID == RoleAssetID.RoleAssetPhyPoint then
  else
    if not self._pointComp then
...
function UISeasonLevelStageS4:InitChapterName()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS4:_InitChapterName(text, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
...
function UISeasonLevelStageS4:RefreshChapter()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS4:RefreshThreeStarArea(playAnim, animDelay)
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS4:PlayAnimThreeStarArea(totalAnimDelay)
...
function UISeasonLevelStageS4:InitEnemys()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS4:_InitEnemy(sop, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
  local progress = self._curLevelProgress
...
function UISeasonLevelStageS4:RefreshEnemyArea()
...
function UISeasonLevelStageS4:RefreshRecommendLv()
...
function UISeasonLevelStageS4:RefreshAreasTitleStyle()
end

function UISeasonLevelStageS4:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffStageAreaS4")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonLevelStageS4:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonLevelStageS4:Close()
  self:_CloseDialogWithAnim()
end

function UISeasonLevelStageS4:_CloseDialogWithAnim(callback)
...
function UISeasonLevelStageS4:_Resume()
...
function UISeasonLevelStageS4:BattleInfoPanelBtnOnClick()
...
function UISeasonLevelStageS4:AwardsPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.AwardsInfo
  self:RefreshPanelShow()
  self:RefreshPanelBg()
  self:RefreshPanelText()
...
function UISeasonLevelStageS4:PowerTopTipsOnClick(go)
  self:ShowDialog("UISeasonActionPointTipS4", self._pointComp, go.transform.anchoredPosition)
end

function UISeasonLevelStageS4:_Countdown()
...
function UISeasonLevelStageS4:_ReqFlushPower(TT)
...
function UISeasonLevelStageS4:OnAfterUILayerChanged()
...
function UISeasonLevelStageS4:ShowMainUI(show)
...
function UISeasonLevelStageS4:RecordOnClick(go)
  self:ShowDialog("UIActivityLevelRecordController", self._component, self._missionID)
end

function UISeasonLevelStageS4:_TryFixStory(TT)
  local normalMissionID = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
...
function UISeasonLevelStageS4:_ReqFinishStory(TT, id, type)
