_class("UIS4ExchangeController", UIController)
UIS4ExchangeController = UIS4ExchangeController

function UIS4ExchangeController:_SetRemainingTime(widgetName, descId, endTime)
  ...
  UIWidgetHelper.SetAnimationPlay(obj, "_anim", "uieff_UIS4TimeExchange")
end

function UIS4ExchangeController:_SetCommonTopButton()
...
function UIS4ExchangeController:_Back()
  self:_PlayAnim("out", function()
    self:CloseDialog()
  end)
end

function UIS4ExchangeController:_SetSpine()
  local obj = self:GetUIComponent("SpineLoader", "_spine")
  obj:LoadSpine("1500994_spine_idle")
end

function UIS4ExchangeController:_SetImgRT(imgRT)
  if imgRT ~= nil then
...
function UIS4ExchangeController:_PlayAnim(type, callback)
  local tb = {
    ["in"] = {
      animName = "uieff_UIS4ExchangeController_in",
      duration = 667
    },
    out = {
      animName = "uieff_UIS4ExchangeController_out",
      duration = 567
    }
  }
...
function UIS4ExchangeController:_CheckGuide()
end

function UIS4ExchangeController:LoadDataOnEnter(TT, res, uiParams)
...
function UIS4ExchangeController:OnShow(uiParams)
...
function UIS4ExchangeController:OnHide()
  self:_DetachEvents()
end

function UIS4ExchangeController:_Refresh(isFirst)
  self:_SetTaken()
  self:_SetTopTips()
...
function UIS4ExchangeController:_SetTaken()
...
function UIS4ExchangeController:_SetTopTips()
...
function UIS4ExchangeController:_SetDynamicListData()
...
function UIS4ExchangeController:_SetDynamicList()
...
function UIS4ExchangeController:_RefreshList(count, list)
...
function UIS4ExchangeController:_SpawnListItem(scrollView, index)
  if index < 0 then
    return nil
  end
  ...
  local className = "UIS4ExchangeCell"
...
function UIS4ExchangeController:_SetListItemData(item, index)
  local info = self._infos[index]
  item:SetData(index, info, self._seasonId, self._component, self._tipsCallback)
end

function UIS4ExchangeController:_DynamicListPlayAnimation(isPlay)
  if isPlay ~= true then
    return
  end
