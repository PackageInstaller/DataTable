_class("UICN18N47HardLevel", UIController)
UICN18N47HardLevel = UICN18N47HardLevel

function UICN18N47HardLevel:Constructor()
  self._isReview = false
end

function UICN18N47HardLevel:LoadDataOnEnter(TT, res, uiParams)
  local campaignModule = GameGlobal.GetModule(CampaignModule)
  self._campaignType = UICN18N47Helper.GetCampaignType(self._isReview)
  self._componentId_BlackMission = UICN18N47Helper.GetComponentId("black", self._isReview)
...
function UICN18N47HardLevel:OnShow(uiParams)
...
function UICN18N47HardLevel:_InitLevelData()
...
function UICN18N47HardLevel:_SortCfg(cfgs)
...
function UICN18N47HardLevel:_Refresh(playAnim)
  for i = 1, 6 do
...
function UICN18N47HardLevel:InitCommonTopButton()
  self.topButtonWidget = self.topbuttons:SpawnObject("UINewCommonTopButton")
  self.topButtonWidget:SetData(function()
    local campaignModule = GameGlobal.GetModule(CampaignModule)
    campaignModule:CampaignSwitchState(true, UIStateType.UICN18N47MainController, UIStateType.UIMain, nil, self._campaign._id)
  end, nil, function()
    self:SwitchState(UIStateType.UIMain)
  end)
end

function UICN18N47HardLevel:RefreshTime()
  ...
  if 0 < remainTime then
    local timeStr = StringTable.Get("str_cn18_n47_shop_close_time_tips", self:GetFormatTimerStr(remainTime))
...
function UICN18N47HardLevel:GetFormatTimerStr(time, id)
  local default_id = {
...
function UICN18N47HardLevel:OnUpdate()
  self:RefreshTime()
end

function UICN18N47HardLevel:OnHide()
...
function UICN18N47HardLevel:ClearTeam()
  self:_InitLevelData()
end

function UICN18N47HardLevel:OnComponentClose(componentCfgId)
end

function UICN18N47HardLevel:_Back()
...
function UICN18N47HardLevel:InitWidget()
  self.topbuttons = self:GetUIComponent("UISelectObjectPath", "topbuttons")
  self._levels = {}
  for i = 1, 6 do
    self._levels[i] = UICN18N47HardLevelItem:New(self:GetUIComponent("UIView", "Level" .. i))
  end
...
function UICN18N47HardLevel:_EnterLevel(idx)
...
function UICN18N47HardLevel:Press1OnClick()
  self:_EnterLevel(1)
end

function UICN18N47HardLevel:Press2OnClick()
  self:_EnterLevel(2)
end

function UICN18N47HardLevel:Press3OnClick()
  self:_EnterLevel(3)
end

function UICN18N47HardLevel:Press4OnClick()
  self:_EnterLevel(4)
end

function UICN18N47HardLevel:Press5OnClick()
  self:_EnterLevel(5)
end

function UICN18N47HardLevel:Press6OnClick()
  self:_EnterLevel(6)
end

function UICN18N47HardLevel:InfoBtnOnClick()
  local endTime = self._blackHardCompInfo.m_close_time
  local svrTimeModule = self:GetModule(SvrTimeModule)
  local curTime = math.floor(svrTimeModule:GetServerTime() * 0.001)
  local remainTime = endTime - curTime
  ...
  if remainTime <= 0 then
    local campaignModule = GameGlobal.GetModule(CampaignModule)
    campaignModule:CampaignSwitchState(true, UIStateType.UICN18N47HardLevel, UIStateType.UIMain, nil, self._campaign._id)
    return
  end
  self:ShowDialog("UIIntroLoader", "UICN18N47HardLevel", MaskType.MT_BlurMask)
end

function UICN18N47HardLevel:_SpawnObject(widgetName, className)
