...
local TreeGetCsv = require("csvdata.treeget")
...
local TreePackageLayer = class("TreePackageLayer", UIBase)

function TreePackageLayer:ctor(params)
  UIBase.ctor(self, uiData, params)
end

function TreePackageLayer:init(params)
  self.showId = 0
  local num = 0
  self.items = {}
  for _, data in pairs(TreeGetCsv) do
    local count = game.role.items[data.itemId] or 0
    if 0 < count and data.itemId ~= 1098 then
      num = num + count
...
function TreePackageLayer:showData(params)
  self:refreshContent()
end

function TreePackageLayer:loadList(row)
...
function TreePackageLayer:loadPage(topRow, buttomRow)
...
function TreePackageLayer:refreshContent()
...
function TreePackageLayer:createCard(index, curRow)
...
function TreePackageLayer:showItemDetail(id)
  self.showId = id
