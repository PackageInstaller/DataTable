...
local ChooseLtyHeroLayer = class("ChooseLtyHeroLayer", UIBase)

function ChooseLtyHeroLayer:ctor(params)
  UIBase.ctor(self, uiData, params)
end

function ChooseLtyHeroLayer:init(params)
  ...
  self.chooseList = {}
  self.unChooseList = {}
  self.herosInfo = json.decode(game.role.ltyHerosInfo)
  self.formatInfo = json.decode(game.role.ltyFormatInfo) or {}
  self.formatInfo.list = self.formatInfo.list or {}
  self.formatInfo.pos = self.formatInfo.pos or {}
  self.formatInfo.skillOder = self.formatInfo.skillOder or {}
  for i = 2, 3 do
...
function ChooseLtyHeroLayer:refreshContent()
  ...
  for index, hero in pairs(heros) do
    ...
    elseif self.herosInfo[tostring(hero.id)] then
      table.insert(self.heros, 1, hero.id)
    else
  ...
  if self.multiChoose and table.nums(self.formatInfo.list) ~= 5 then
    local index = 1
    for slot = 1, 5 do
      local id = self.formatInfo.list[tostring(slot)]
      if id then
...
function ChooseLtyHeroLayer:loadPage(topRow, buttomRow)
...
function ChooseLtyHeroLayer:loadList(row)
...
function ChooseLtyHeroLayer:createCard(curRow, heroId, index)
  local card
  if heroId == 0 then
    ...
    card = SmallCard.new({
      heroId = heroId,
      tower = true,
      hpPercent = self.herosInfo[tostring(heroId)] or 1000,
      showLove = self.love
    })
    ...
    card:setForcetouch(function()
      ...
      BackManager:push(function()
        ...
        layer:close()
        self:show()
        TopBar:show(TopBarType.full, "道馆")
      end)
    end)
    local curFormat = false
    for _, id in pairs(self.formatInfo.list) do
      if id == heroId then
        curFormat = true
        break
      end
    ...
    if curFormat then
      display.newSprite(chooseRes .. "format_cur.png"):pos(101, 120):addTo(card)
    elseif self.herosInfo[tostring(heroId)] then
      display.newSprite(chooseRes .. "format.png"):pos(55, 193):addTo(card)
    end
...
function ChooseLtyHeroLayer:choose(heroId, card)
  if not self.multiChoose then
...
function ChooseLtyHeroLayer:checkFormationSlot()
  local isFull = self:checkChangeCountFull() and 2
  ...
  for _, rowData in pairs(self.unChooseList) do
    for heroId, card in pairs(rowData) do
      if isFull and not self.chooseList[heroId] then
        ...
        elseif not lock and isFull == 2 and not self.herosInfo[tostring(heroId)] then
          lock = display.newSprite("ui/kitchen/strength/mask.png"):scale(0.93):pos(101, 108):addTo(card):name("lock")
          local bg = display.newSprite(chooseRes .. "text_bg.png"):pos(101, 114):addTo(lock)
          display.newTTFLabel({
            text = "上阵数达到上限",
...
function ChooseLtyHeroLayer:calculateFormatSlot()
...
function ChooseLtyHeroLayer:checkFormationSameHero(heroId)
  for index, data in ipairs(self.formatList) do
...
function ChooseLtyHeroLayer:checkChangeCountFull()
  local cur = table.nums(self.herosInfo)
  if self.multiChoose then
    for index, data in pairs(self.formatList) do
      if not self.herosInfo[tostring(data.id)] then
        cur = cur + 1
      end
    end
  end
...
function ChooseLtyHeroLayer:findSlotById(heroId)
  for index, data in ipairs(self.formatList) do
...
function ChooseLtyHeroLayer:setFilter(params)
  params = params or {}
...
function ChooseLtyHeroLayer:showType(types)
  self.typeBg:removeAllChildren()
  local num = table.nums(types)
  if num == 0 then
    display.newTTFLabel({
      text = "全部",
      size = 18,
      color = UIHelper.hex2rgb("#ffffff")
    }):enableOutline(UIHelper.hex2rgb("#000000"), 1):center(self.typeBg):addTo(self.typeBg)
  else
    local xPos, index = 68 - (num - 1) / 2 * 25, 0
    for type, _ in pairs(types) do
      display.newSprite(string.format("ui/hero/choose/%d.png", type)):pos(xPos + index * 25, 15):addTo(self.typeBg)
      index = index + 1
    end
  end
end

return ChooseLtyHeroLayer
