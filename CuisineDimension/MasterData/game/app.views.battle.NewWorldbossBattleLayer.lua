...
local ChapterBattleCsv = require("csvdata.newboss_battle")
...
local NewWorldbossBattleLayer = class("NewWorldbossBattleLayer", BaseBattleLayer)
local uiData = {
...
function NewWorldbossBattleLayer:ctor(params)
  params = params or {}
  ...
  self.carbonData = ChapterBattleCsv[self.bossIdx][bossDegree]
  ...
  self.killTime = globalCsv.NewWorldBossBattleLimitTime
  self.lastKillTime = 0
  self.currentPhase = 1
...
function NewWorldbossBattleLayer:__init()
...
function NewWorldbossBattleLayer:sendGameEndRequest(starNum)
  local function endBattle()
    ...
    function sendGameInfo()
      ...
      game:sendData(actionCodes.Activity_newWorldBossEndBattleRpc, MsgPack.pack({
        score = bossData.hurt,
        bossIdx = self.bossIdx,
        is_win = true
      }))
      UIHelper.showWaiting()
      game:addResponseHandler(actionCodes.Activity_newWorldBossEndBattleRpc, function(event)
...
function NewWorldbossBattleLayer:initLeftInfo()
  local heroData = {}
...
function NewWorldbossBattleLayer:loadTMXMapData()
...
function NewWorldbossBattleLayer:enterNextPhase()
...
function NewWorldbossBattleLayer:loadPhaseEnemys()
...
function NewWorldbossBattleLayer:onPlayerDead(player)
...
function NewWorldbossBattleLayer:step(dt)
  self:baseStep(dt)
  self:updateTime(dt)
...
function NewWorldbossBattleLayer:updateTime(dt)
...
function NewWorldbossBattleLayer:showView()
...
return NewWorldbossBattleLayer
