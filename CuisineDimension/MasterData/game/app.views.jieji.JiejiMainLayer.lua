local taskCsv = require("csvdata.activity_96_task")
local drawCsv = require("csvdata.activity_96_drawgift")
local itemCsv = require("csvdata.item")
local shopCsv = require("res.csvdata.shop")
local JiejiMainRes = "ui/jieji/main/"
local uiData = {
  csbFile = "ui/jieji/JiejiMainLayer.csb",
  widgets = {
    content = "content",
    bg = "content/bg",
    bg2 = "content/bg2",
...
local activityType = "96"
local color_active = "#e4582c"
local color_inactive = "#7c79c5"
...
function JiejiMainLayer:init(params)
  ...
  if globalCsv.Activity96Spine ~= "" then
    local heroSpine = UIHelper.createSpineNodeByRes("live2d/homespine/" .. globalCsv.Activity96Spine)
    heroSpine:setAnimation(0, "idle", true)
    local posX = globalCsv.Activity96SpinePos.x
    local posY = globalCsv.Activity96SpinePos.y
    local scale = globalCsv.Activity96SpineScale
    heroSpine:scale(scale, scale):pos(posX, posY):addTo(self.bg)
  end
  local costId = globalCsv.Activity96CostItem
...
function JiejiMainLayer:createTaskCell(data)
  ...
  local icon = display.newSprite(itemData.icon):scale(0.7):pos(60, 53):addTo(bg)
  if 0 < record:getv(data.id, 0) then
  ...
  }):anch(0.5, 0.5):pos(45, 97):addTo(viewNode)
  local str1 = string.format("<div shadow=0,-2,0.5,%s outline=1,%s >%s</div>", color_inactive, color_inactive, set[2])
  local str2 = string.format("<div shadow=0,-2,0.5,%s outline=1,%s >%s</div>", color_active, color_active, set[2])
  MRichText.new({
...
function JiejiMainLayer:createdarwCell(data, pos, index)
  ...
  local str1 = string.format("<div shadow=0,-2,0.5,%s outline=1,%s >%s</div>", color_inactive, color_inactive, itemData.name)
  local str2 = string.format("<div shadow=0,-2,0.5,%s outline=1,%s >%s</div>", color_active, color_active, itemData.name)
  local label = MRichText.new({
    text = str1,
    size = 16,
    color = UIHelper.hex2rgb("#ffffff")
  }):rotate(0):anch(0.5, 0.5):pos(59, 21):addTo(bg)
  ...
  game:addResponseHandler(actionCodes.Activity_act96DrawRpc, function(event)
    ...
    self:handleAction(function()
      self:selectNodeRunAction(posList, 1, function()
        game:createView("jieji.JiejiRewardDialog", {
          rewards = msg.reward,
          callBack = function()
            self.countLabel:setString(game.role.activityStatus[activityType].coin or 0)
            self:showSelect(-1)
