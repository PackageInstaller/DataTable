...
local MoonPveBattleLayer = class("MoonPveBattleLayer", BaseBattleLayer)
local uiData = {
...
function MoonPveBattleLayer:ctor(params)
  params = params or {}
  ...
  self.activityBuff = params.activityBuff
...
function MoonPveBattleLayer:__init()
  ...
  if game.role.midBattleCount:getv(self.carbonId, 0) == 0 and 0 < self.monsterData.story1 then
...
function MoonPveBattleLayer:sendGameEndRequest(starNum)
  local function endBattle()
    ...
    function sendGameInfo()
      ...
      game:sendData(actionCodes.Moon_endGameRpc, MsgPack.pack({
      ...
      game:addResponseHandler(actionCodes.Moon_endGameRpc, function(event)
        ...
        msg.carbonType = BattleType.Moon
  ...
  if game.role.midBattleCount:getv(self.carbonId, 0) == 0 and 0 < self.monsterData.story2 then
    game:createView("talk.PlotTalkLayer", {
...
function MoonPveBattleLayer:getActivityBuff()
  local activityStatus = json.decode(game.role.activityBackStatus)
  local activityBuffs = {}
  if activityStatus["31"] or activityStatus["54"] or activityStatus["38"] or activityStatus["58"] then
    if not self.activityBuff or self.activityBuff == 0 then
      return
    end
    for _, heroInfo in ipairs(self.leftInfo.heros) do
      local buff = {}
      for _, buffId in pairs(globalCsv.ActivityBuffItem[self.activityBuff] or {}) do
        table.insert(buff, buffId)
      end
      activityBuffs[heroInfo.type] = buff
    end
  else
    return
  end
  return activityBuffs
end

function MoonPveBattleLayer:initLeftInfo()
  local activityBuffs = self:getActivityBuff()
  local heroData = {}
  for _, heroInfo in ipairs(self.leftInfo.heros) do
  ...
  for index, info in ipairs(heroData) do
    ...
    if activityBuffs and activityBuffs[info.type] then
      for _, buffId in ipairs(activityBuffs[info.type]) do
        player:addBuff({buffId = buffId, releaser = player})
      end
    end
  end
...
function MoonPveBattleLayer:loadTMXMapData()
...
function MoonPveBattleLayer:updateMaxXposByPhase()
  local xPos = {
...
function MoonPveBattleLayer:enterNextPhase()
...
function MoonPveBattleLayer:loadPhaseEnemys()
...
function MoonPveBattleLayer:onPlayerDead(player)
...
function MoonPveBattleLayer:step(dt)
  self:baseStep(dt)
...
function MoonPveBattleLayer:showView()
...
function MoonPveBattleLayer:checkGuide()
  if game.role.majorGuideStep == 11 then
