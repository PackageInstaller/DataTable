...
local NewWorldBossSelectHeroLayer = class("NewWorldBossSelectHeroLayer", UIBase)

function NewWorldBossSelectHeroLayer:ctor(params)
  UIBase.ctor(self, uiData, params)
end

function NewWorldBossSelectHeroLayer:init(params)
  ...
  self.chooseList = {}
  self.unChooseList = {}
  local activityData = game.role.activityStatus["107"] or {}
  self.formatInfo = json.decode(activityData.formatInfo or {})
  for i = 2, 3 do
...
function NewWorldBossSelectHeroLayer:refreshContent()
  ...
  for index, hero in pairs(heros) do
    ...
    elseif not self.rule(hero) then
...
function NewWorldBossSelectHeroLayer:loadPage(topRow, buttomRow)
...
function NewWorldBossSelectHeroLayer:loadList(row)
...
function NewWorldBossSelectHeroLayer:createCard(curRow, heroId, index)
  local card
  if heroId == 0 then
...
function NewWorldBossSelectHeroLayer:choose(heroId, card)
  if not self.multiChoose then
...
function NewWorldBossSelectHeroLayer:checkFormationSlot()
...
function NewWorldBossSelectHeroLayer:calculateFormatSlot()
...
function NewWorldBossSelectHeroLayer:checkFormationSameHero(heroId)
  for index, data in ipairs(self.formatList) do
...
function NewWorldBossSelectHeroLayer:checkChangeCountFull()
  return table.nums(self.chooseList) >= self.max
end

function NewWorldBossSelectHeroLayer:findSlotById(heroId)
  for index, data in ipairs(self.formatList) do
...
function NewWorldBossSelectHeroLayer:setFilter(params)
  params = params or {}
...
return NewWorldBossSelectHeroLayer
