-- chunkname: @E:/AntMan/AntMan_Android_pub1/Assets/Code/.LuaCode/Debug\\Modules\\Demo\\DemoStateBattleTest.lua
...
local strClassName = "DemoStateBattleTest"
local DemoStateBattleTest = Class(strClassName, GameState)
local StateBattleMixin = require("Logic/battle/StateBattleMixin")

if StateBattleMixin then
	MixinClass(DemoStateBattleTest, StateBattleMixin)
end
...
function DemoStateBattleTest:ctor(name)
	self:_initData()
end

function DemoStateBattleTest:_initData()
	self.mEntityDict = {}
...
function DemoStateBattleTest:onEnter(preStateName)
	DemoStateBattleTest.super.onEnter(self, preStateName)
	EventCenter.addEventListenerGroup(self, listenerFuncConfig)
end

function DemoStateBattleTest:onExit(nextState)
	DemoStateBattleTest.super.onExit(self, nextState)
...
function DemoStateBattleTest:split(input, delimiter)
...
function DemoStateBattleTest:initBattle(sceneNo, battleNo, demoNo, isRobot, randomSeed, monsterAtkEnhance, monsterMhpEnhance, formation)
...
function DemoStateBattleTest:onLoadEnded()
	DemoStateBattleTest.super.onLoadEnded(self)
...
function DemoStateBattleTest:initConfig()
	self:clear()
...
function DemoStateBattleTest:_initSpeData(mainData, camp)
...
function DemoStateBattleTest:initObjInfo(battleNo, demoNo)
	self.speData = {}
	...
	if self.isRobot then
		local mainData = ResBattleToolMain[demoNo]
...
function DemoStateBattleTest:onMatrixOver(loseCamp, speResultType)
	UIManager.getUI("battleRecordDlg", true):onShow()
end

function DemoStateBattleTest:startGame()
...
function DemoStateBattleTest:clear()
...
function DemoStateBattleTest:onActorsReady()
	self.actorsReady = true
end

function DemoStateBattleTest:onRaiseMatrixInput(matrixOpType, ...)
...
function DemoStateBattleTest:getPlayers()
...
function DemoStateBattleTest:refreshInfoLevel(infoLevel, refreshAll)
...
function DemoStateBattleTest:onPause()
	if self.frameMgr then
		self.frameMgr:pause()
...
function DemoStateBattleTest:onResume()
	if self.frameMgr then
...
function DemoStateBattleTest:getCenterPointPos()
...
function DemoStateBattleTest:getReplayData()
	local data = {}
...
function DemoStateBattleTest:recordBattle(sender, menu, value)
