...
public class T0DragOnSmoothPathHandler : MonoBehaviour, IBeginDragHandler, IEventSystemHandler, IEndDragHandler, IDragHandler, IInitializePotentialDragHandler
{
	...
	public T0DragHandlerType dragType;
	...
	{
		if (dragType == T0DragHandlerType.Normal)
		{
			OnNormalDrag(eventData);
		}
		else if (dragType == T0DragHandlerType.Wipe)
		{
			OnWipeDrag(eventData);
		}
	}

	private void OnNormalDrag(PointerEventData eventData)
	{
		...
		{
			if (item.gameObject.transform.IsChildOf(dragPath.transform))
			{
				BoxCollider component = item.gameObject.GetComponent<BoxCollider>();
	...
	private void OnWipeDrag(PointerEventData eventData)
	{
		Vector3 worldPosition = eventData.pointerCurrentRaycast.worldPosition;
		List<RaycastResult> list = new List<RaycastResult>();
		EventSystem.current.RaycastAll(eventData, list);
		...
		if (!useSmoothValue && lastProgress >= 0f)
		{
			Mathf.Abs(lastProgress - progress);
			OnProgressUpdate(Mathf.Abs(lastProgress - progress));
		}
	...
	{
		dragStarted = false;
		T0WorldScene.Scene.dataContext.OnDragEnd(dragKey);
	}
	...
	private void OnProgressUpdate(float progress)
	{
		if (dragType == T0DragHandlerType.Normal)
		{
			OnNormalProgressUpdate(progress);
		}
		else if (dragType == T0DragHandlerType.Wipe)
		{
			OnWipeProgressUpdate(progress);
		}
	}

	private void OnNormalProgressUpdate(float progress)
	{
		T0WorldScene.Scene.dataContext.OnDragValueChange(dragKey, progress);
		if (!dragFinish && progress >= dragFinishThreashold)
		{
			T0WorldScene.Scene.dataContext.OnDragFinish(dragKey);
			dragFinish = true;
		}
	}

	private void OnWipeProgressUpdate(float deltaProgress)
	{
		T0WorldScene.Scene.dataContext.OnDragValueAddChange(dragKey, deltaProgress);
	}
