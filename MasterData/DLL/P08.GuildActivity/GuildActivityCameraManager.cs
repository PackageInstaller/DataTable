...
public class GuildActivityCameraManager : MonoBehaviour
{
	...
	private GuildActivityWarFieldNode _tweeningTarget;
	...
	private void Awake()
	{
		...
		inputManager.onZoomChange = (Action<int, float>)Delegate.Combine(inputManager.onZoomChange, new Action<int, float>(OnZoomChange));
		GuildActivityInputManager inputManager2 = _inputManager;
		inputManager2.onSingleFingerMove = (Action<Vector2>)Delegate.Combine(inputManager2.onSingleFingerMove, new Action<Vector2>(OnSingleFingerMove));
	}
	...
	{
		if (status == GuildActivityInputManager.ZOOM_BEGIN)
		{
	...
	public void OnNodeClick(GuildActivityWarFieldNode node)
	{
		MoveCameraToNode(node, _cameraMinSize);
	}

	public void MoveCameraToNode(GuildActivityWarFieldNode node, float finalOrthographicSize = -1f)
	{
	...
	{
		SpriteRenderer component = GetComponentsInChildren<GuildActivityWarFieldLevel>()[0].GetComponent<SpriteRenderer>();
