...
[Name("更换摇杆为跑酷控制杆(渲染)", 0)]
[Category("Render/UI")]
[Description("更换摇杆为跑酷控制杆(本质还是摇杆)")]
public class ChangeJoystickRunLogic : FlowControlNode
{
				...
				else
				{
					...
					else
					{
						if (component.joystickController != null)
						{
							...
							component.joystickController.Init(Vector3.zero, 0f, new P08JoystickOnlyXAxisControllerGameLogic());
						}
						output.Call(f);
					}
				}
