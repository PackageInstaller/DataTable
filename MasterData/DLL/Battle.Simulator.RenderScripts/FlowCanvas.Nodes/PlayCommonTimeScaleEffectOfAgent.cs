...
[Name("创建特效(绑定)(渲染)", 0)]
[Category("Render/函数")]
[Description("注意!!!该节点需要和【移除特效(绑定)】配套使用，否则第二次添加的时候无法添加通过角色身上的通用特效播放器创建特效\n特效路径为:从Effect开始,例如:Effect/Hero/1037/fx_1037_skill1_1;\n挂点路径从tpose开始, 如果要挂在指定点, 可以用 获取挂点路径 先获取\n其他参数意义问文杰\n创建出来的通用特效需要使用【移除角色通用特效】节点进行关闭\n这个节点创建的特效会跟随角色显隐")]
public class PlayCommonTimeScaleEffectOfAgent : EffectCreateBaseNode
{
