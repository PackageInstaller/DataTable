...
[Name("左侧目标-数量或时间", 0)]
[Category("Render/UI")]
[Description("显示左侧目标-数量, 和之前的显示左侧目标不是同一个")]
public class LeftMapTargetContentNumber : FlowNode
{
	private MapTargetContentNumber com;
	...
	protected override void RegisterPorts()
	{
		...
		ValueInput<string> instNameInput = AddValueInput<string>("实例名").SetDefaultAndSerializedValue("Number");
		ValueInput<string> prefabPathInput = AddValueInput<string>("预制体路径").SetDefaultAndSerializedValue("UI/BattleTips/Target_UI_Content_Number");
		...
		AddFlowInput("Init", delegate(Flow f)
		{
			...
			if (battleScene != null && battleScene.battlePanelGameObject != null)
			{
				...
				if (transform != null)
				{
					...
					if (transform2 != null)
					{
						com = transform2.gameObject.GetComponent<MapTargetContentNumber>();
					}
				}
			}
			if (com == null)
		...
		ValueInput<string> numberInput = AddValueInput<string>("数量");
		...
		AddFlowInput("设置数量/时间", delegate(Flow f)
		{
			if (com == null)
			{
				errorOutput.Call(f);
			...
			{
				com.SetNumber(numberInput.value, playAnimationInput.value);
				output.Call(f);
			}
		});
		ValueInput<Color> colorInput = AddValueInput<Color>("指定颜色");
		AddFlowInput("设置颜色", delegate(Flow f)
		{
			if (com == null)
			{
				errorOutput.Call(f);
