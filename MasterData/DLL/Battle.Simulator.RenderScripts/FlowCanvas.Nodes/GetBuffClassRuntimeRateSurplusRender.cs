...
[Name("得到Buff剩余时长(百分比)(渲染)", 0)]
[Category("Render/Buff/通用")]
[Description("得到实体身上指定类型BUFF的剩余调用时长,由于一个类型的buff可能有多个,故而需要用list来存放")]
public class GetBuffClassRuntimeRateSurplusRender : FlowControlNode
{
			...
			else
			{
				...
				else
				{
					for (int i = 0; i < buffList.Count; i++)
					{
						...
						{
							cdsVar.value.Add((float)buffList[i].mBuffRuntime / (float)buffList[i].mTick);
						}
					}
					output.Call(f);
				}
