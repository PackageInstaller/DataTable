...
public class UpdateSourceSpaceKeepTime : FlowControlNode
{
	...
	protected override void RegisterPorts()
	{
		...
		ValueInput<int> AddTimeInput = AddValueInput<int>("增加的时长(毫秒)", "AddTimeInput");
		...
		AddFlowInput("In", "input", delegate(Flow f)
		{
			...
			if (entitySourceSpace != null && entitySourceSpace.Enable && entitySourceSpace.SourceSpaceKeepTime > 0)
			{
				...
				sourceSpaceKeepTime = (entitySourceSpace.SourceSpaceKeepTime = sourceSpaceKeepTime + AddTimeInput.GetValue());
				m_newKeepTime = sourceSpaceKeepTime;
