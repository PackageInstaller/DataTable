...
[Name("遍历处理范围内实体(多重阵营)", 0)]
[Category("Logic/PosAndRot")]
[Description("遍历处理范围内实体(多重阵营)")]
public class GetEntityByRangeConditionByMultipleCamps : FlowControlNode
{
	private int m_count;
	...
	protected override void RegisterPorts()
	{
		...
		AddFlowInput("In", delegate(Flow f)
		{
			...
			case RangeType.Sector:
				...
				SimPhysics.OverlapSector(practicalCenterPos, radiusVar.value, angleVar.value, forwardVar.value, list, mask);
				break;
			case RangeType.Roundness:
				practicalCenterPos = IntMath.Transform(centerPosOffsetVar.value, forwardVar.value, originCenterPosVar.value);
				SimPhysics.Overlap(practicalCenterPos, radiusVar.value, list, mask);
				break;
			case RangeType.Rectangle:
				practicalCenterPos = IntMath.Transform(centerPosOffsetVar.value, forwardVar.value, originCenterPosVar.value);
				SimPhysics.OverlapBox(originCenterPosVar.value, forwardVar.value, centerPosOffsetVar.value, new Int3(widthVar.value, heightVar.value, lengthVar.value), list, mask);
				break;
			}
			for (int j = 0; j < list.Count; j++)
			{
				SimEntity simEntityByMultiColliders = CommonProcessor.GetSimEntityByMultiColliders(list[j]);
