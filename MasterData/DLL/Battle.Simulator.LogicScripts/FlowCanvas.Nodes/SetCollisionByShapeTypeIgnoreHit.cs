...
[Name("设置指定碰撞(类型)忽略受击", 0)]
[Category("Logic/Attribute")]
[Description("设置指定碰撞(类型)忽略受击")]
public class SetCollisionByShapeTypeIgnoreHit : FlowNode
{
	...
	protected override void RegisterPorts()
	{
		...
		ValueInput<bool> valueInput = AddValueInput<bool>("忽略受击");
		...
		AddFlowInput("", delegate(Flow f)
		{
			...
			else
			{
				...
				for (int i = 0; i < shapes.Count; i++)
				{
					...
					{
						shapes[i].mShape.IgnoreHit = valueInput.value;
						flag = true;
					}
				}
				if (flag)
