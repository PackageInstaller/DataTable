...
[Name("销毁抛掷物实体(阵营)(过时)(逻辑)", 0)]
[Category("Logic/抛掷物")]
[Description("销毁指定阵营的所有抛掷物，销毁每个抛掷物后发出抛掷物销毁事件。无论抛掷物是否待销毁都会发出")]
public class DestroyThrownByCamp : FlowControlNode
{
	...
	protected override void RegisterPorts()
	{
		...
		AddFlowInput("In", "intput", delegate(Flow f)
		{
			...
			for (int i = 0; i < mThrowns.Count; i++)
			{
				...
				if (thrownState.mCamp == (uint)_CampInput.GetValue())
				{
