...
[Name("[格挡]格挡成功事件(实体类型ID)", 0)]
[Category("Common/事件/格挡")]
[Description("格挡成功时候,抛出来的事件;\n位置:目标身上作为判定的碰撞盒位置;")]
public class ParriedSucceededEventHandlerOfRoleID : EventNode
{
	...
	private ValueInput<int> _specifyRoleID;
	...
	{
		_specifyRoleID = AddValueInput<int>("攻击者实体类型ID", "roleID");
	...
	private void onEventHandler(EntityParrySucceededEvent evt)
	{
		...
		SimEntity entityWithEntityID = base.graph.mSimContext.GetEntityWithEntityID(_attackerID);
		if (entityWithEntityID != null && entityWithEntityID.hasEntityConfig && !(entityWithEntityID.ENTITY_CONFIG_ID != _specifyRoleID.value))
		{
			Flow f = new Flow
			{
