...
[Description("具体聚性的行为逻辑,在原本聚怪逻辑的基础上增加对聚怪目标怪物类型的考量;\n被设定怪物类型才可以被该节点施加被聚效果;\n可以通过移除Buff(创建ID)移除对应的聚怪Buff来提前结束聚怪效果")]
public class GatherWithEnmeyType : FlowControlNodeOfLogic
{
	...
	private ValueInput<GatherTargetType> _targetTypeInput;
	...
	protected override void RegisterPorts()
	{
		...
		_targetTypeInput = AddValueInput<GatherTargetType>("聚怪目标类型", "targetType");
		...
		AddFlowInput("In", delegate(Flow f)
		{
			...
			case RangeType.Roundness:
			{
				...
				SimVar param3 = new Int3(_speedInput.value, (int)_targetTypeInput.value, 0);
			...
			{
				SimVar param = new Int3((int)_rangeTypeInput.value, _speedInput.value, (int)_targetTypeInput.value);
	...
	private static void ProcessGather(List<int> collisions, int speed, Int3 center, int targetType)
	{
		for (int i = 0; i < collisions.Count; i++)
		{
			...
			if (((targetType >> (int)simEntityByMultiColliders.entityBlackboard.var.enemyType) & 1) == 1)
			{
				Debug.Log("===>>>> 目标被聚怪处理:" + simEntityByMultiColliders.ENTITY_CONFIG_ID.ToString());
	...
	private static void Impl(SimEntity simEntity, int buffIndex)
	{
		...
		case RangeType.Sector:
		{
			...
			targetType = buffParams.v4.int3Var.y;
			break;
		}
		case RangeType.Roundness:
		{
		...
		case RangeType.Rectangle:
			...
			speed = int3Var3.y;
			targetType = int3Var3.z;
			break;
		}
		ProcessGather(list, speed, int3Var, targetType);
