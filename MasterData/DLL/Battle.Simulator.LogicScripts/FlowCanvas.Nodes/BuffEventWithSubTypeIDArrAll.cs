...
[Name("BUFF事件(子类型)(全部)(逻辑)", 0)]
[Category("Logic/事件")]
[Description("身上的buff处理")]
public class BuffEventWithSubTypeIDArrAll : EventNode
{
	...
	private ValueInput<List<string>> m_subTypeArrInput;
	...
	protected override void RegisterPorts()
	{
		...
		m_subTypeArrInput = AddValueInput<List<string>>("子类型组", "expectationSubTypeArr");
	...
	public void LogicBuffUpdate(int ownerID, int buffIndex, int buffClass, EBuffLife buffLife, bool isTimeout)
	{
		...
		if (config != null && BuffSubTypeComparer.ContainsAll(m_subTypeArrInput.value, config.SubType))
		{
			m_ownerID = ownerID;
