...
public class ChessBoardCameraManager : MonoBehaviour
{
	...
	public static ChessBoardCameraManager _instance;
	...
	public CinemachineFreeLook playerDiceFreeLook;

	public CinemachineFreeLook playerCloseFreeLook;

	public CinemachineFreeLook groupFreeLook;

	public CinemachineTargetGroup targetGroup;

	public CinemachineTargetGroup targetDiceGroup;
	...
	public Transform playerDiceTrs;

	private CinemachineTargetGroup.Target playerDiceTarget;
	...
	public static ChessBoardCameraManager Instance => _instance;
	...
	public Transform PlayerDiceTrs
	{
		get
		{
			return playerDiceTrs;
		}
		set
		{
			playerDiceTrs = value;
			playerDiceTarget = default(CinemachineTargetGroup.Target);
			playerDiceTarget.target = playerDiceTrs;
			playerDiceTarget.weight = 1f;
			playerDiceTarget.radius = 0f;
			targetDiceGroup.m_Targets = new CinemachineTargetGroup.Target[1];
			targetDiceGroup.m_Targets[0] = playerDiceTarget;
		}
	}
	...
	{
		_instance = this;
		MapConfig mapConfig = ChessBoardMapCfgManager.Instance.MapConfig;
	...
	private void Start()
	{
		...
		playerDiceFreeLook.LookAt = targetDiceGroup.transform;
		playerDiceFreeLook.Follow = targetDiceGroup.transform;
		playerCloseFreeLook.LookAt = targetDiceGroup.transform;
		playerCloseFreeLook.Follow = targetDiceGroup.transform;
	...
	private void Update()
	{
		...
		case CAMERASTATE.Switch_Start:
		{
			...
			if (null == component || component.ActiveBlend == null)
			{
				if ((bool)component)
				{
					component.IsLive(playerFreeLook);
				}
	...
	{
		targetGroup.m_Targets[0].target = PlayerTrs;
		targetDiceGroup.m_Targets[0].target = PlayerDiceTrs;
	...
	public void ChangeCameraToDiceCamera()
	{
		ChangeCameraLookAt(CAMERA_LOOKAT_STATUS.PLAYER_DICE);
	}

	public void ChangeCameraToCloseCamera()
	{
		ChangeCameraLookAt(CAMERA_LOOKAT_STATUS.PLAYER_CLOSE);
	}
	...
	private CinemachineFreeLook GetFreeLookCameraByStatus(CAMERA_LOOKAT_STATUS lookAtStatus)
	{
		return lookAtStatus switch
		{
			...
			CAMERA_LOOKAT_STATUS.PLAYER_DICE => playerDiceFreeLook, 
			CAMERA_LOOKAT_STATUS.PLAYER_CLOSE => playerCloseFreeLook, 
	...
	public void SetTargetTransform(Transform trans, Transform diceTrans)
	{
		targetGroup.m_Targets[0].target = trans;
		targetDiceGroup.m_Targets[0].target = diceTrans;
		if (state != CAMERASTATE.LookAt)
		{
			state = CAMERASTATE.Switch_Ready;
		}
