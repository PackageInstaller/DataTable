...
public class NewHexCameraManager : MonoBehaviour
{
	...
	public static NewHexCameraManager _instance;
	...
	private NewChessCameraTarget.cameraTarget target1;
	...
	private Dictionary<int, NewChessCameraTarget.cameraTarget> _callBackDic = new Dictionary<int, NewChessCameraTarget.cameraTarget>(new IntComparer());
	...
	public static NewHexCameraManager Instance => _instance;
	...
	{
		_instance = this;
		NewMapConfig mapConfig = NewChessMapCfgManager.Instance.MapConfig;
	...
	public void MoveGameplayCamera(Vector2 amount)
	{
		if (CAMERASTATE.Move != state)
		{
			...
			gameplayDummy.position = new Vector3(Mathf.Clamp(gameplayDummy.position.x, Mathf.Max(minFreeLookX, NewHexFogManager.Instance.CameraMinX), Mathf.Min(maxFreeLookX, NewHexFogManager.Instance.CameraMaxX)), gameplayDummy.position.y, Mathf.Clamp(gameplayDummy.position.z, Mathf.Max(minFreeLookZ, NewHexFogManager.Instance.CameraMinZ), Mathf.Min(maxFreeLookZ, NewHexFogManager.Instance.CameraMaxZ)));
		}
	}
	...
	public void MoveTargetSmooth(Vector3 _target, float speed = 10f, bool _force = true, Action callBack = null, bool isUniform = true)
	{
		...
		if (_force || CAMERASTATE.Move != state)
		{
			...
			target1 = NewChessCameraTarget.SetNewTarget(_target, callBack);
			state = CAMERASTATE.Move;
	...
	private void CheckCallBack(NewChessCameraTarget.cameraTarget tar)
	{
