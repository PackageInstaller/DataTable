...
function Controller:SetElement(element, elementIndex)
  ...
  if index - 1 < 10 then
    ...
    element.Group_Level.Txt_Index:SetText(index - 1)
  end
  if isCurrent then
    element.Group_Level.Txt_Index:SetColor("#FFFFFF")
  elseif isUnLock then
    element.Group_Level.Txt_Index:SetColor("#FFFFFF")
  else
    element.Group_Level.Txt_Index:SetColor("#74A1A7")
  end
  local y = 0
  if (index - 1) % 2 == 1 then
    y = 80
...
function Controller:TryBattle()
  ...
  if not levelCA.isEnemyLvEquilsPlayer and levelCA.recomGrade + levelCA.extraLevelOffset * (levelDifficulty - 1) - PlayerData:GetPlayerLevel() >= 5 then
    ...
    checkTipParam.checkTipKey = "SignBattleLevelEmilieTip"
