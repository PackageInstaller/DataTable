...
function Controller:Init()
  ...
  if DataModel.curPos > 0 then
    ...
    local isDeep = false
    local pos = PlayerData:GetPlayerPrefs("int", "FlameLevelPos")
    if 0 < pos then
      isDeep = pos == 2
    end
    Controller:SwitchDeep(isDeep)
...
function Controller:ShowLevelDetail(idx, isRefresh)
  ...
  if isRefresh ~= true then
    if hideLockDiff then
      ...
      local initLock = PlayerData:GetPlayerPrefs("int", "FlameSpLevelLock") == 1
  ...
  levelGroup.Txt_Grade:SetText(string.format(GetText(80612162), lv))
...
function Controller:RefreshAutoBattleRemainTime()
  if DataModel.remainAutoBattleTime <= 0 then
    ...
    PlayerData.BattleCallBackPage = "UI/Activity/Flame/FlameLevel"
...
function Controller:LockDiffClick(isLock, isInit)
  ...
  PlayerData:SetPlayerPrefs("int", "FlameSpLevelLock", isLock and 1 or 0)
end
