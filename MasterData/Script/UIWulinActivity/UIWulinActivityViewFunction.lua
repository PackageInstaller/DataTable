...
local Controller = require("UIWulinActivity/UIWulinActivityController")
local ViewFunction = {
  WulinActivity_Group_NotJoin_Group_Preview_ScrollGrid_Reward_SetGrid = function(element, elementIndex)
  ...
  WulinActivity_Group_NotJoin_Group_Preview_ScrollGrid_Reward_Group_Item_Group_Item_Btn_Item_Click = function(btn, str)
    local id = tonumber(str)
    CommonTips.OpenPreRewardDetailTips(id)
  end,
  WulinActivity_Group_NotJoin_Group_Add_Btn__Click = function(btn, str)
    if PlayerData.IsInStation() == false then
      CommonTips.OpenTips(80612463)
      return
    end
    ...
    if DataModel.plotId == -1 or DataModel.plotId == 0 or DataModel.plotId == nil then
      ...
      UIManager:Open("UI/Dialog/Dialog", Json.encode({
        ...
        Controller.JoinActivity(true)
      end, function()
        Controller.JoinActivity(true)
      end)
    end
  end,
  WulinActivity_Group_Join_Group_Quest_Btn__Click = function(btn, str)
    local parms = {}
    parms.activityId = DataModel.activityId
    UIManager:Open("UI/Activity/Wulin/WulinMission", Json.encode(parms))
  end,
  WulinActivity_Group_Join_Group_Store_Btn__Click = function(btn, str)
    local parms = {}
    parms.activityId = DataModel.activityId
    parms.shopId = DataModel.shopId
    Net:SendProto("shop.info", function(json)
      UIManager:Open("UI/Activity/Wulin/WulinStore", Json.encode(parms))
    end)
  end,
  WulinActivity_Group_Join_Group_Card_Btn__Click = function(btn, str)
  ...
  WulinActivity_Group_Join_Group_Skip_Btn__Click = function(btn, str)
  ...
  WulinActivity_Group_Join_Group_Rank_Btn__Click = function(btn, str)
  ...
  WulinActivity_Group_Join_Group_Level_Group_Can_Btn__Click = function(btn, str)
    ...
    UIManager:Open("UI/Activity/Wulin/WulinLevel")
  end,
  WulinActivity_Group_Join_Group_Level_Group_Skin_Btn__Click = function(btn, str)
    if not DataModel.levelQuestIsComplete then
      local questName = PlayerData:GetFactoryData(DataModel.levelQuestId).name
      CommonTips.OpenTips(string.format(GetText(80607102), questName))
      return
  ...
  WulinActivity_Group_Join_Group_Level_Group_SignBattle_Btn_SignBattle_Click = function(btn, str)
    if not DataModel.levelQuestIsComplete then
      local questName = PlayerData:GetFactoryData(DataModel.levelQuestId).name
      CommonTips.OpenTips(string.format(GetText(80607102), questName))
      return
    end
    local signBattleId = 86000244
    if TimeUtil:IsActive(DataModel.startTime, DataModel.endTime) then
  ...
  WulinActivity_Group_Join_Btn_Gacha_Click = function(btn, str)
    local ca = PlayerData:GetFactoryData(DataModel.activityId)
  ...
  WulinActivity_Btn_Help_Click = function(btn, str)
    CommonTips.OpenHelp(DataModel.helpId)
  end,
  WulinActivity_Btn_Video_Click = function(btn, str)
    UIManager:Open("UI/Activity/ActivitySignBattle/SignBattleTipCommon", Json.encode({id = 86000321, IsGacha = true}))
  end
}
return ViewFunction
