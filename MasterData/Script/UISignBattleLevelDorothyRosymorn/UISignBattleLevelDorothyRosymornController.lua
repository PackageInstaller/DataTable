local View = require("UISignBattleLevelDorothyRosymorn/UISignBattleLevelDorothyRosymornView")
local DataModel = require("UISignBattleLevelDorothyRosymorn/UISignBattleLevelDorothyRosymornDataModel")
local Controller = {}

function Controller:Init()
  View.ScrollGrid_Map.grid.self:SetActive(true)
  ...
  View.Btn_Gacha.Img_B:SetSprite(GetResPath(88300323))
end
...
function Controller:SetElement(element, elementIndex)
  ...
  local isUnLock = index <= DataModel.GetUnlockIndex() + 2 and not DataModel.GetFinishIndex(index - 1)
  local isCurrent = currentState or index == DataModel.listCount - 1 and DataModel.GetFinishIndex(index - 1)
  local isFinish = DataModel.GetFinishIndex(index - 1) and index ~= DataModel.listCount - 1
  local isLock = index > DataModel.GetUnlockIndex() + 2
  element.Group_Level.Group_Current:SetActive(isCurrent)
  ...
  element.Group_Level.Group_UnLock:SetActive(isUnLock and not isCurrent)
  element.Group_Level.Group_Finish:SetActive(isFinish)
  element.Group_Level.Group_Lock:SetActive(isLock)
  ...
  element.Group_Level.Img_Index:SetSprite(DataModel.imgPath .. index - 1)
  local y = 0
  if (index - 1) % 2 == 1 then
    y = 80
  else
...
function Controller:OpenRightPage(index)
  ...
  local levelData = PlayerData:GetFactoryData(data.id)
...
function Controller:TryBattle()
  ...
  local function doSquad()
    ...
    PlayerData.BattleCallBackPage = "UI/Activity/ActivitySignBattle/SignBattleLevelDorothyRosymorn"
  ...
  if not levelCA.isEnemyLvEquilsPlayer and levelCA.recomGrade + levelCA.extraLevelOffset * (levelDifficulty - 1) - PlayerData:GetPlayerLevel() >= 5 then
    ...
    checkTipParam.checkTipKey = "SignBattleLevelDorothyRosymornTip"
