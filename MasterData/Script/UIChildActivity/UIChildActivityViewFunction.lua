...
local ViewFunction = {
  ChildActivity_Group_NotJoin_Group_Preview_ScrollGrid_Reward_SetGrid = function(element, elementIndex)
  ...
  ChildActivity_Group_NotJoin_Group_Preview_ScrollGrid_Reward_Group_Item_Group_Item_Btn_Item_Click = function(btn, str)
    local id = tonumber(str)
    CommonTips.OpenPreRewardDetailTips(id)
  end,
  ChildActivity_Group_NotJoin_Group_Add_Btn__Click = function(btn, str)
  ...
  ChildActivity_Group_Join_Group_Quest_Btn__Click = function(btn, str)
    local parms = {}
    parms.activityId = DataModel.activityId
    UIManager:Open("UI/Activity/Child/ChildMission", Json.encode(parms))
  end,
  ChildActivity_Group_Join_Group_Store_Btn__Click = function(btn, str)
    local parms = {}
    parms.activityId = DataModel.activityId
  ...
  ChildActivity_Group_Join_Group_Card_Btn__Click = function(btn, str)
  ...
  ChildActivity_Group_Join_Group_Skip_Btn__Click = function(btn, str)
  ...
  ChildActivity_Group_Join_Group_Rank_Btn__Click = function(btn, str)
  ...
  ChildActivity_Group_Join_Group_Level_Group_Can_Btn__Click = function(btn, str)
    ...
    UIManager:Open("UI/Activity/Child/ChildLevel")
  end,
  ChildActivity_Group_Join_Group_Level_Group_Skin_Btn__Click = function(btn, str)
    if not DataModel.levelQuestIsComplete then
      local questName = PlayerData:GetFactoryData(DataModel.levelQuestId).name
      CommonTips.OpenTips(string.format(GetText(80607102), questName))
      return
  ...
  ChildActivity_Group_Join_Group_Level_Group_SignBattle_Btn_SignBattle_Click = function(btn, str)
    if not DataModel.levelQuestIsComplete then
      local questName = PlayerData:GetFactoryData(DataModel.levelQuestId).name
      CommonTips.OpenTips(string.format(GetText(80607102), questName))
      return
    end
    local signBattleId = PlayerData:GetFactoryData(DataModel.activityId)
    if not signBattleId then
      return
    end
    if TimeUtil:IsActive(DataModel.startTime, DataModel.endTime) then
      local caBat = PlayerData:GetFactoryData(signBattleId.signBattleActivityId)
      if caBat and not string.nilorempty(caBat.signBattleUI) then
        UIManager:Open(caBat.signBattleUI, Json.encode({
          activityId = signBattleId.signBattleActivityId,
          mainActivityId = DataModel.activityId
        }))
      end
    end
  end,
  ChildActivity_Group_Join_Btn_Gacha_Click = function(btn, str)
    local ca = PlayerData:GetFactoryData(DataModel.activityId)
  ...
  ChildActivity_Btn_Help_Click = function(btn, str)
    CommonTips.OpenHelp(DataModel.helpId)
  end,
  ChildActivity_Btn_Video_Click = function(btn, str)
