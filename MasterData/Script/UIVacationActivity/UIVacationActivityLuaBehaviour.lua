local View = require("UIVacationActivity/UIVacationActivityView")
local DataModel = require("UIVacationActivity/UIVacationActivityDataModel")
local ViewFunction = require("UIVacationActivity/UIVacationActivityViewFunction")
local Controller = require("UIVacationActivity/UIVacationActivityController")
local param
local Luabehaviour = {
  serialize = function()
    return param
  ...
  deserialize = function(initParams)
    ...
    View.Group_Bg.Img_Icon:SetSprite(GetResPath(88300310))
    ...
    View.Group_Time.Txt_StartTime:SetText(string.format(GetText(80611406), startY .. "/" .. startMonth .. "/" .. startDay))
    ...
    if not DataModel.isJoin then
      ...
      if questCa then
        View.Group_Join.Group_Level.Group_Quest.Txt_:SetText(string.format(GetText(80611435), questCa.name or ""))
      end
      if isOver then
        View.Group_Join.Group_Finish.Txt_Time:SetText(DataModel.GetCloseTimeInfo())
      end
      View.Group_Join.Group_Store.Txt_Num:SetText(PlayerData:GetGoodsById(11401054).num or 0)
    end
  end,
  awake = function()
  end,
