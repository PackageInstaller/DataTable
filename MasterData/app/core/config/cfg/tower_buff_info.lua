...
local var_0_11 = {
	...
	"武将热点加成（一）",
	1,
	"玩法内所有战斗中享受如下技能：我方攻击最高1人第一次受到致命伤害时，免疫一次死亡，获得相当于自身血量上限300%的治疗，过量转护盾，并获得1回合的无敌，清除自身所有伤害限制类效果，获得1次额外出手机会。",
	1900
}
local var_0_12 = 7
local var_0_13 = {
	__index = function(self, arg_1_1)
		...
		if arg_1_1 == "toObject" then
			...
			assert(var_0_9[arg_1_1], "cannot find " .. arg_1_1 .. " in record_tower_buff_info")
...
function var_0_7._getData()
	if not var_0_7._originData then
		var_0_7._originData = loadCfg("core.config.cfg.tower_buff_info_data")
...
function var_0_7._getKey()
	if not var_0_8 then
		var_0_8 = loadCfg("core.config.cfg.tower_buff_info_key")
...
function var_0_7.checkDataClear()
	if var_0_7._lastUseTime > 0 and os.time() - var_0_7._lastUseTime > g.core.const.ConstMgr.BASE_CONST.CONFIG_CHECK_DATA_DELTA then
		...
		package.loaded["app.core.config.cfg.tower_buff_info_data"] = nil
		package.loaded["app.core.config.cfg.tower_buff_info_key"] = nil
	end
end
