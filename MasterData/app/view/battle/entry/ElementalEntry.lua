local Entry = require("app.view.battle.entry.Entry")
local ElementalEntry = class("ElementalEntry", Entry)

function ElementalEntry:ctor(arg_1_1)
	self._battleLayer = arg_1_1.battleLayer
	self._identity = arg_1_1.identity
	self._skillInfo = arg_1_1.skillInfo
	self._skillName = arg_1_1.skillName
	self._elemental = arg_1_1.elemental or 1
	self._isDone = false

	ElementalEntry.super.ctor(self)
end

function ElementalEntry:initEntry()
	self.super.initEntry(self)

	local var_2_0 = Entry.new()

	var_2_0:addEntryToNewQueue(nil, handler(self, self._showUI))
	self:addEntryToQueue(var_2_0, var_2_0.updateEntry)
end

function ElementalEntry:_showUI()
	if g.core.battle.BattleProxy:getBattleLayer():isCurPaused() then
		return false
	end

	local var_3_0 = fgui.UIPackage:createObject("battle", "BattleElementalCurtainComp")

	if self._identity ~= 1 then
		var_3_0:setScaleX(-1)
		var_3_0:setPosition(cc.p(display.width - 0, 0))
	else
		var_3_0:setPosition(cc.p(0, 0))
	end

	var_3_0:updateView({
		identity = self._identity,
		skillInfo = self._skillInfo,
		skillName = self._skillName,
		timeScale = self._battleLayer:getCurSpeed(),
		elemental = self._elemental,
		callback = function()
			self._isDone = true
		end
	})
	self._battleLayer:addNodeToUILayer(var_3_0, 1)

	return true
end

function ElementalEntry:isDone()
	return self._isDone
end

function ElementalEntry:destroyEntry()
	ElementalEntry.super.destroyEntry(self)

	self._battleLayer = nil
	self._identity = nil
	self._skillInfo = nil
	self._skillName = nil
end

return ElementalEntry
