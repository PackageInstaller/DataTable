...
local BattleResultBlazingArenaRecordComp = class("BattleResultBlazingArenaRecordComp", require("app.fairyGUI.battleResult.UI_BattleResultBlazingArenaRecordComp"))

function BattleResultBlazingArenaRecordComp:ctor()
...
function BattleResultBlazingArenaRecordComp:update(arg_2_1)
	...
	if var_2_0 < var_0_0:getLastRank() then
...
function BattleResultBlazingArenaRecordComp:_arrangeUnitsByPos(arg_3_1)
	if not arg_3_1 then
		return
	end
...
function BattleResultBlazingArenaRecordComp:_onRenderResultList(arg_4_1, arg_4_2)
	arg_4_2:updateCell({
		result = self._resultList[arg_4_1 + 1],
		roundNum = arg_4_1 + 1,
		myMulTeam = var_0_1:getFormationStructDict(g.core.const.ConstMgr.LineUpConst.MulTeamType.BLAZING_ARENA),
		formations = var_0_0:getEnemyFormations(),
		ownUnits = self._ownUnits[arg_4_1 + 1],
		enemyUnits = self._enemyUnits[arg_4_1 + 1]
	})
end

return BattleResultBlazingArenaRecordComp
