	...
	if self:isSidValid(arg_4_2) then
		if self:isKnightLineUp(arg_4_2) then
			...
			if self:isStationPosLineupArtifact(arg_4_1) then
				local var_4_2 = g.core.model.User.teamBattleData:getFormationData()

				if not g.core.model.User.teamBattleData:getFormationData():getArtifactByServerId(var_4_2):checkWearKnight(var_4_2:getKnightByServerId(arg_4_2):getAdvanceId()) then
