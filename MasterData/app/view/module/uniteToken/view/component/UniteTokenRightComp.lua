local UniteTokenRightComp = class("UniteTokenRightComp", require("app.fairyGUI.uniteToken.UI_UniteTokenRightComp"))

function UniteTokenRightComp:ctor()
	self.m_bgLoader:setURL("pic/base_new/bg_yht_rightdi.png")
end

function UniteTokenRightComp:onLoad()
...
function UniteTokenRightComp:init(arg_3_1, arg_3_2)
	self._childComp = {}
...
function UniteTokenRightComp:setViewData(arg_4_1, arg_4_2)
...
function UniteTokenRightComp:onExitAnimPlayEnd()
...
function UniteTokenRightComp:_onEnterAnimFinish()
	self.m_tabList:setTouchable(true)
end

function UniteTokenRightComp:updateView(arg_7_1)
...
function UniteTokenRightComp:_onTabClicked(arg_8_1)
	local var_8_0 = self.m_tabList:getSelectedIndex()

	if var_8_0 + 1 ~= self._selectIndex then
		local var_8_1 = arg_8_1:getSender()

		if var_8_1 then
			var_8_1:getChildAt(var_8_0):getChild("effectHolder"):addEffectSpine({
