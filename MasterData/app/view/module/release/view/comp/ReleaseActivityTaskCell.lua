local var_0_0 = g.core.const.ConstMgr.ReleaseActivityConst
local var_0_1 = g.core.model.User.releaseActivityData
local ReleaseActivityTaskCell = class("ReleaseActivityTaskCell", require("app.fairyGUI.release.UI_ReleaseActivityTaskCell"))

function ReleaseActivityTaskCell:ctor()
	self._taskdata = {}
...
function ReleaseActivityTaskCell:updateTaskCell(arg_2_1)
	self._taskInfo = arg_2_1
	self._taskData = var_0_1:getTaskData()

	local var_2_0 = self._taskData:getTaskValue(arg_2_1.action_key, arg_2_1.action_type)
	...
	self.m_typeController:setSelectedIndex(self._taskInfo.reset_type == var_0_0.TASK_TYPE.PERIOD and 1 or 0)
end

function ReleaseActivityTaskCell:_onAwardItemRender(arg_3_1, arg_3_2)
	arg_3_2:updateIcon(self._awards[arg_3_1 + 1])
end

function ReleaseActivityTaskCell:_onGoBtnClick()
	...
	elseif self._state == var_0_0.TASK_STATE.FINISH then
		...
		table.insert(var_4_0, self._taskInfo.id)
		g.core.network.GameNetProxy:send_C2S_Release_TaskAward({
			ids = var_4_0
		})
...
return ReleaseActivityTaskCell
