...
function RechargeSkinComp:_onTouchSkinBegin(arg_3_1)
	...
	local var_3_1 = self.m_knightPicComp:getPosition()

	self._verticalDiff = var_3_1.y - var_3_0.y
	self._horizontalDiff = self:_coordinateRevert(var_3_0:getTouch():getLocation()).x - var_3_1.x
end
...
function RechargeSkinComp:_onTouchSkinMove(arg_4_1)
	...
	self.m_knightPicComp:setPosition(cc.p(math.clamp(self:_coordinateRevert(var_4_0:getTouch():getLocation()).x - self._horizontalDiff, var_4_1.x, display.width - var_4_1.x), (math.clamp(var_4_0.y + self._verticalDiff, -50, display.height - 130))))
end
