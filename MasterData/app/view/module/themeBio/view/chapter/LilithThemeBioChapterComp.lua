local LilithThemeBioChapterComp = class("LilithThemeBioChapterComp", require("app.fairyGUI.themeBio.UI_LilithThemeBioChapterComp"))
...
function LilithThemeBioChapterComp:ctor()
	self._info = nil
	self._chapterId = nil
...
function LilithThemeBioChapterComp:updateData(arg_2_1, arg_2_2)
...
function LilithThemeBioChapterComp:_changeAnimateMode(arg_3_1)
...
function LilithThemeBioChapterComp:playAnimateUnlock(arg_4_1)
	return
end

function LilithThemeBioChapterComp:_checkUnlock(arg_5_1)
	if not arg_5_1 then
		return
	end
...
function LilithThemeBioChapterComp:_onOpenBioMapClick()
...
function LilithThemeBioChapterComp:checkPlayedPassEffect()
	if (self._unlockMode == ThemeBioConst.BIO_CHAPTER_PASSED or self._unlockMode == ThemeBioConst.BIO_CHAPTER_PERFECT) and not self._themeBioData:checkPlayedChapterPassEffect(self._chapterId, self._unlockMode) then
		...
		if self._unlockMode == ThemeBioConst.BIO_CHAPTER_PASSED then
			...
			self.m_effPerfect:removeAllEffect()
			self.m_effPerfect:addEffectSpine({
				remove = true,
				name = "eff_ui_themeBioVote_pass",
				isLoop = false,
				anim = "pass",
				eventHandler = handler(self, self._delayDispatchCompEvent)
			})
...
function LilithThemeBioChapterComp:_delayDispatchCompEvent(arg_8_1)
	if arg_8_1.type == "complete" then
		self:dispatchCompEvent("new_chapter_unlock", {
			chapterIdx = self._info.number
		})
	end
end

function LilithThemeBioChapterComp:playEnterAnim(arg_9_1)
	self.m_enterTransition:play()
	self.m_effEnter:removeAllEffect()
	self.m_effEnter:addEffectSpine({
		remove = true,
		name = "eff_ui_themeBioLilith_mainLayerEnter",
		isLoop = false,
		anim = "play" .. arg_9_1
	})
end

return LilithThemeBioChapterComp
