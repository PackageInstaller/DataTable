...
local BeautyCustomActivityPop = class("BeautyCustomActivityPop", require("app.fairyGUI.signActivity.UI_BeautyCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "BeautyCustomActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function BeautyCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.BEAUTY_SIGN_ACTIVITY
...
function BeautyCustomActivityPop:onLoad()
	...
	self:getChild("n59"):setVisible(false)
end

function BeautyCustomActivityPop:_updateMainView()
...
function BeautyCustomActivityPop:_updateTimeText()
...
function BeautyCustomActivityPop:_checkAutoSignAward()
...
function BeautyCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function BeautyCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function BeautyCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function BeautyCustomActivityPop:homeLayerAutoPopup()
...
function BeautyCustomActivityPop:_playEffect()
...
return BeautyCustomActivityPop
