...
local HeraCustomActivityPop = class("HeraCustomActivityPop", require("app.fairyGUI.signActivity.UI_HeraCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "HeraCustomActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function HeraCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.HERA_SIGN_ACTIVITY
	...
	self.m_bgLoader:setURL("pic/signBigPic/pic_bg_7sign_6_1.png")
	self.m_skinPic:setURL("pic/signBigPic/pic_bg_7sign_6_2.png")
	self.m_awardBg:setURL("pic/signBigPic/pic_7sign_tianshuqiandaodi_2.png")
end

function HeraCustomActivityPop:onLoad()
...
function HeraCustomActivityPop:_updateMainView()
...
function HeraCustomActivityPop:_updateTimeText()
...
function HeraCustomActivityPop:_checkAutoSignAward()
...
function HeraCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function HeraCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function HeraCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function HeraCustomActivityPop:homeLayerAutoPopup()
...
function HeraCustomActivityPop:_playEffect()
	self.m_effBg:removeAllEffect()
	self.m_effBg:addEffectSpine({
		isLoop = true,
		name = "eff_ui_activity_sdOLbg"
	})
end

return HeraCustomActivityPop
