...
local LinkCustomActivityPop = class("LinkCustomActivityPop", require("app.fairyGUI.signActivity.UI_LinkCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "LinkCustomActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function LinkCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.LINK_SIGN_ACTIVITY
...
function LinkCustomActivityPop:onLoad()
...
function LinkCustomActivityPop:_updateMainView()
...
function LinkCustomActivityPop:_updateTimeText()
...
function LinkCustomActivityPop:_checkAutoSignAward()
...
function LinkCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function LinkCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function LinkCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function LinkCustomActivityPop:homeLayerAutoPopup()
