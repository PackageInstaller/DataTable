...
local DoctorVolenceCustomActivityPop = class("DoctorVolenceCustomActivityPop", require("app.fairyGUI.signActivity.UI_DoctorVolenceCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "DoctorVolenceCustomActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function DoctorVolenceCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.DOCTORVOLENCE_SIGN_ACTIVITY

	self:showAtCenter()
	self:setSize(display.width, display.height)
	self.m_roleLoader:setURL("pic/signActivity/bg_3signly_juese8.png")
...
function DoctorVolenceCustomActivityPop:onLoad()
...
function DoctorVolenceCustomActivityPop:_updateMainView()
...
function DoctorVolenceCustomActivityPop:_updateTimeText()
...
function DoctorVolenceCustomActivityPop:_checkAutoSignAward()
...
function DoctorVolenceCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function DoctorVolenceCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function DoctorVolenceCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function DoctorVolenceCustomActivityPop:homeLayerAutoPopup()
...
return DoctorVolenceCustomActivityPop
