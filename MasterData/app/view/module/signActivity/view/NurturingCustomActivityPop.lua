...
local NurturingCustomActivityPop = class("NurturingCustomActivityPop", require("app.fairyGUI.signActivity.UI_NurturingCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "NurturingCustomActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function NurturingCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.NURTURING_SIGN_ACTIVITY

	self:showAtCenter()
	self:setSize(display.width, display.height)
	self.m_roleLoader:setURL("pic/signActivity/bg_3signly_juese5.png")
...
function NurturingCustomActivityPop:onLoad()
...
function NurturingCustomActivityPop:_updateMainView()
...
function NurturingCustomActivityPop:_updateTimeText()
...
function NurturingCustomActivityPop:_checkAutoSignAward()
...
function NurturingCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function NurturingCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function NurturingCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function NurturingCustomActivityPop:homeLayerAutoPopup()
...
return NurturingCustomActivityPop
