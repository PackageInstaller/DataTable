...
local SakuraCustomActivityPop = class("SakuraCustomActivityPop", require("app.fairyGUI.signActivity.UI_SakuraCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "SakuraCustomActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function SakuraCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.SAKURA_SIGN_ACTIVITY
...
function SakuraCustomActivityPop:onLoad()
...
function SakuraCustomActivityPop:_updateMainView()
...
function SakuraCustomActivityPop:_updateTimeText()
...
function SakuraCustomActivityPop:_checkAutoSignAward()
...
function SakuraCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function SakuraCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function SakuraCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function SakuraCustomActivityPop:homeLayerAutoPopup()
...
function SakuraCustomActivityPop:_playEffect()
...
return SakuraCustomActivityPop
