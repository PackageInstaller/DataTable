...
local ListenWithSwearActivityPop = class("ListenWithSwearActivityPop", require("app.fairyGUI.signActivity.UI_ListenWithSwearActivityPop"), function()
	return fgui.GComponent:create({
		resName = "ListenWithSwearActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function ListenWithSwearActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.LISTEN_SWEAR_ACTIVITY
...
function ListenWithSwearActivityPop:onLoad()
...
function ListenWithSwearActivityPop:_updateMainView()
...
function ListenWithSwearActivityPop:_updateTimeText()
...
function ListenWithSwearActivityPop:_checkAutoSignAward()
...
function ListenWithSwearActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function ListenWithSwearActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function ListenWithSwearActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function ListenWithSwearActivityPop:homeLayerAutoPopup()
...
function ListenWithSwearActivityPop:_playEffect()
	self.m_effBg:removeAllEffect()
end

return ListenWithSwearActivityPop
