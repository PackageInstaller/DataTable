...
local ChildrenDayCustomActivityPop = class("ChildrenDayCustomActivityPop", require("app.fairyGUI.signActivity.UI_ChildrenDayCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "ChildrenDayCustomActivityPop",
		pkgPath = "ui/signActivity/signActivity",
		pkgName = "signActivity"
	})
end)

function ChildrenDayCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.CHILDRENDAY_SIGN_ACTIVITY
...
function ChildrenDayCustomActivityPop:onLoad()
...
function ChildrenDayCustomActivityPop:_updateMainView()
...
function ChildrenDayCustomActivityPop:_updateTimeText()
...
function ChildrenDayCustomActivityPop:_checkAutoSignAward()
...
function ChildrenDayCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function ChildrenDayCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function ChildrenDayCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function ChildrenDayCustomActivityPop:homeLayerAutoPopup()
...
return ChildrenDayCustomActivityPop
