...
local DomainFormationLayer = class("DomainFormationLayer", require("app.fairyGUI.domainDungeon.UI_DomainFormationLayer"), function()
	return fgui.GComponent:create({
		resName = "DomainFormationLayer",
...
function DomainFormationLayer:ctor(arg_2_1)
...
function DomainFormationLayer:_initView()
	self.m_saveBtn:addClickListener(handler(self, self._onSaveBtnClicked))
	self:addBg("bg/domain/" .. self._dungeonId .. "/bg_mnly_biandui.jpg", true, nil, 1)
	self.m_descTxt:setText(var_0_0.FORMATION_DESC[self._dungeonId])
	self.m_tabBtn:updateView(1, self._formationType)
	self.m_enterTransition:play()
end

function DomainFormationLayer:onLoad()
...
function DomainFormationLayer:_onSaveBtnClicked()
...
function DomainFormationLayer:_doSaveFormation()
...
function DomainFormationLayer:_updateFormationView()
	self.m_teamFormationComp:updateComp(self._formationType, self._formationIndex)
end

function DomainFormationLayer:_onSaveSucc(arg_8_1, arg_8_2, arg_8_3)
