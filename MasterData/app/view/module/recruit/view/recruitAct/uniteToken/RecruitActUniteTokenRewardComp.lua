local RecruitActUniteTokenRewardComp = class("RecruitActUniteTokenRewardComp", require("app.fairyGUI.recruitActUniteToken.UI_RecruitActUniteTokenRewardComp"))

function RecruitActUniteTokenRewardComp:ctor()
	self._isNew = false

	self.m_enter1Transition:setHook("eff", handler(self, self._onPlayEnterFlagEff))
end

function RecruitActUniteTokenRewardComp:updateItem(arg_2_1, arg_2_2, arg_2_3)
	self:clearEffect()
	...
	if var_2_0.type == g.core.common.Goods.TYPE_UNITETOKEN then
		...
		local var_2_7 = var_2_0.quality + 1
		...
		if var_2_7 >= 2 then
			self.m_eff_item_idel_holder:addEffectSpine({
				anim = "play",
				isLoop = true,
				name = "eff_ui_recruitUniteToken_frag_" .. var_2_7
			})
		end
	end
...
function RecruitActUniteTokenRewardComp:setShareState()
	self.m_fragShow:setVisible(false)
	self.m_newImg:setVisible(false)
end

function RecruitActUniteTokenRewardComp:_onPlayEnterFlagEff()
...
function RecruitActUniteTokenRewardComp:resetShareState()
	if self._isNew then
		self.m_fragShow:setVisible(false)
		self.m_newImg:setVisible(true)
	else
...
function RecruitActUniteTokenRewardComp:getIsNew()
	return self._isNew
end

function RecruitActUniteTokenRewardComp:clearEffect()
...
return RecruitActUniteTokenRewardComp
