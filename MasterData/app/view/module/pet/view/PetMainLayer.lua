...
function PetMainLayer:_onTouchMove(arg_27_1)
	...
	local var_27_0 = arg_27_1:getInput()
	local var_27_1 = var_27_0:getTouch():getLocation()
	local var_27_2 = math.abs(var_27_0.x + self._touchOffsetX - self._petSpineX)
	local var_27_3 = math.min(var_27_2, var_0_4) / var_0_4

	if self._isChangeUIAction then
		if var_27_0.x + self._touchOffsetX - self._petSpineX > 0 then
			self.m_leftPetSpineDadComp:setAlpha(var_27_3)
			self.m_leftPetSpineDadComp:setX(self._leftPetSpineX + (var_27_0.x + self._touchOffsetX - self._petSpineX))
		else
			self.m_rightPetSpineDadComp:setAlpha(var_27_3)
			self.m_rightPetSpineDadComp:setX(self._rightPetSpineX + (var_27_0.x + self._touchOffsetX - self._petSpineX))
		end
	elseif var_27_2 >= var_0_5 then
	...
	self.m_petSpineDadComp:setX(var_27_1.x + self._touchOffsetX)
	self.m_petSpineDadComp:setAlpha(1 - var_27_3)
end
...
function PetMainLayer:_onTouchEnd(arg_28_1)
	...
	local var_28_0 = arg_28_1:getInput():getTouch():getLocation().x + self._touchOffsetX - self._petSpineX
	...
	if math.abs(var_28_0) >= var_0_4 and self:_isTherePetToChange() then
		...
		self.m_petSpineDadComp:runAction((cc.Sequence:create(cc.Spawn:create(cc.EaseSineOut:create((cc.MoveBy:create(0.5, cc.p(-var_28_0, 0)))), (cc.FadeIn:create(0.3))), (cc.CallFunc:create(handler(self, function()
			self._canTouchPet = true
		end))))))
