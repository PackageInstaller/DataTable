...
local TowerHardMapComp = class("TowerHardMapComp", require("app.fairyGUI.tower.UI_TowerHardMapComp"))

function TowerHardMapComp:ctor()
...
function TowerHardMapComp:initView()
	for iter_2_0 = 1, var_0_1.SUB_TOWER_MAP_COUNT do
		if self["m_stage" .. iter_2_0] then
			...
			if var_2_0 then
				var_2_0:setSelectedIndex((self._stageType - 2) % 5)
			end
		end
	end
...
function TowerHardMapComp:onClickStage(arg_4_1)
	...
	if var_4_0 == self._imageStageIndex + arg_4_1 + self._addCount then
		if var_4_2 then
			...
			if var_0_3:getLeftCount(var_0_4.PLAY_TYPE["TOWER_HARD_CHALLENGE_COUNT_" .. self._stageType]) <= 0 then
				var_0_2:tip(g.core.lang:get(308024))

				return
			end

			if var_0_0:checkIsNeedPopFormation(self._stageType) then
				var_0_2:pushPopup(require("app.view.module.tower.view.hard.TowerHardFormationLayer").new(self._stageType), {
					touchDisappear = false,
					ignoreTouch = false
				})
			else
				local var_4_3, var_4_4 = var_0_0:getFormationPos(self._stageType)

				g.core.network.GameNetProxy:send_C2S_Tower_ChallengeStageBegin({
					play_type = 2,
					id = var_4_3,
					formation_knight_ids = var_4_4
				})
			end
		end
...
function TowerHardMapComp:setInitData(arg_5_1, arg_5_2)
	self._stageType = arg_5_1
	self._addCount = arg_5_2
end

function TowerHardMapComp:setImageStageIndex(arg_6_1)
	self._imageStageIndex = arg_6_1
end

function TowerHardMapComp:updateImageNodeInScroll(arg_7_1, arg_7_2, arg_7_3, arg_7_4)
...
function TowerHardMapComp:updateImageNode(arg_8_1)
		...
		if arg_8_1 then
			...
			elseif var_8_7 == var_8_0 then
				...
				if not var_8_2 then
					var_8_8:addEffectSpine({
						anim = "play",
						isLoop = true,
						name = var_8_0 % 5 == 0 and "eff_ui_towerHard_specialIdle" or "eff_ui_towerHard_commonIdle"
					})
				end
			else
				var_8_9 = 2
			...
			elseif var_8_7 == var_8_10 then
				...
				var_8_8:addEffectSpine({
					...
					remove = true,
					isLoop = false,
					name = var_8_7 % 5 == 0 and "eff_ui_towerHard_improve" or "eff_ui_towerHard_pass"
				})
