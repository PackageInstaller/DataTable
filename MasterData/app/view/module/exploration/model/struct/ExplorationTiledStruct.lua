...
function ExplorationTiledStruct:setTiledMonster(arg_21_1, arg_21_2, arg_21_3)
	...
	if var_21_0.attr_type > 0 then
		...
		if g.core.config.exploration_map_info.get(var_21_1, (var_21_1:getCurMapId())).difficulty > 1 then
	...
	while g.core.config.exploration_monster_info.hasKey("monster_" .. 1) do
		...
		if var_21_0["monster_" .. 1] ~= 0 then
			local var_21_4 = g.core.config.massive_monster_team_info.get(var_21_0["monster_" .. 1])

			if 0 < var_21_4.fight then
				var_21_3 = var_21_4
				var_21_5 = var_21_4.fight
			end
		end
	...
	self._monsterInfo.spineId = g.core.config.knight_info.get(var_21_3.knight_id_1).res_id
	self._monsterInfo.fight = var_21_3.fight
	self._monsterInfo.direction = arg_21_3
	self._monsterInfo.roomId = self:getBelongRoom()
	self._state = ExplorationConst.MAP.TILED_STATE_UNLOCK
end
