...
function ExplorationLayerMiniRoomComp:_onTouchMove(arg_4_1)
	local var_4_0 = arg_4_1:getInput()
	local var_4_1 = var_4_0:getTouch():getDelta()

	var_4_1.y = -var_4_0.y

	local var_4_2 = cc.pAdd(self.m_miniMapComp:getPosition(), var_4_1)
	local KsMath = require("app.core.utils.KsMath")

	var_4_2.x = KsMath.rangeIn(var_4_2.x, self._moveRange.x, self._moveRange.x + self._moveRange.width)
	var_4_2.y = KsMath.rangeIn(var_4_2.y, self._moveRange.y, self._moveRange.y + self._moveRange.height)

	self.m_miniMapComp:setPosition(var_4_2)
	self.m_lineComp:setPosition(var_4_2)
	self.m_topComp:setPosition(var_4_2)
end
