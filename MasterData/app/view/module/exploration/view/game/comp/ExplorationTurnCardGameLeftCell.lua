local ExplorationTurnCardGameLeftCell = class("ExplorationTurnCardGameLeftCell", require("app.fairyGUI.exploration.UI_ExplorationTurnCardGameLeftCell"))

function ExplorationTurnCardGameLeftCell:ctor()
	self._rewardArray = nil

	self.m_iconList:setItemRenderer(handler(self, self._onRenderIconList))
end

function ExplorationTurnCardGameLeftCell:updateLeftCell(arg_2_1, arg_2_2)
...
function ExplorationTurnCardGameLeftCell:_initRewardList()
	for iter_3_0 = 1, 3 do
		self["m_reward" .. iter_3_0]:updateRewardCell(self._rewardArray[iter_3_0])
	end
end

function ExplorationTurnCardGameLeftCell:_onRenderIconList(arg_4_1, arg_4_2)
...
function ExplorationTurnCardGameLeftCell:updateCellStatus(arg_5_1, arg_5_2)
	...
	if var_5_0 >= 0 and var_5_0 < self.m_iconList:numChildren() then
		...
		var_5_4:addEffectSpine({
			anim = "play",
			name = "eff_ui_exploration_activate",
			remove = true,
			isLoop = false
		})
...
function ExplorationTurnCardGameLeftCell:playFinishAnim(arg_6_1, arg_6_2)
	self.m_eff:removeAllEffect()
	self.m_eff:addEffectSpine({
		anim = "play",
		name = "eff_ui_exploration_get",
		isLoop = false,
		remove = true,
...
function ExplorationTurnCardGameLeftCell:_playIconAnim(arg_7_1)
...
return ExplorationTurnCardGameLeftCell
