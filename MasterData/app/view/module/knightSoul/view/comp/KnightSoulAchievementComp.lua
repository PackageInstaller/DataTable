...
function KnightSoulAchievementComp:updateActiveState()
	...
	if not self._selData then
		...
		var_4_0 = self._selData.cfg
	end

	local var_4_1 = var_0_1:getActivationHonorId() >= self._selData.cfg.id

	self.m_activeStateController:setSelectedIndex(var_4_1 and 1 or 0)

	if not var_4_1 then
		if var_4_0.honor_num > var_0_1:getCurHonorNum() then
			self.m_ksValueNeedTxt:setText(g.core.lang:get(410620, {
				num = var_4_0.honor_num
			}))
		else
