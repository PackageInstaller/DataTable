...
function GveMapComp:updatePlayerIcon()
	if self._stage then
		local var_11_0 = self:getMapWorld()
		local var_11_1, var_11_2 = self:getMapWorld():getMapGround():coordToPixel(var_11_0.x, var_0_2:getLastAddPoint().y)

		return self._stage:updatePlayerIcon(var_11_0:convertToNodeSpace({
			x = display.cx,
			y = display.cy
		}), {
			x = var_11_1,
