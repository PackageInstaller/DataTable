__reload_all = true
local System = require("System.System")
local readonly = System.readonly
local Text_PanelText = readonly({
  PanelText_Battle_UltiSkill_Painting_EX_Text_C_UltiSkill_Content = {
    Text = "Over-Exaltation"
  },
  PanelText_Btn_Block_Window_Text_C_Display_Content = {
    Text = "Display Card"
  },
  PanelText_Btn_Block_Window_Text_C_Hide_Content = {Text = "Hide Cards"},
  PanelText_Btn_Get_Text_Name_Content = {Text = "Claim"},
  PanelText_Com_RedDot_Text_C_GroupNew_Content = {Text = "NEW"},
  PanelText_Com_RedDot_Text_C_New_Content = {Text = "NEW"},
  PanelText_Com_Text_New_Text_C_New_Content = {Text = "NEW"},
  ["PanelText_GmPanel_Text_C_Battle_Test (1)_Content"] = {Text = "Battle 2.0"},
  PanelText_GmPanel_Text_C_Battle_Test2_Content = {Text = "PvP Battle"},
  PanelText_GmPanel_Text_C_Battle_Test_Content = {
    Text = "PvP Matchmaking"
  },
  PanelText_GmPanel_Text_C_Gb_Content = {Text = "Close"},
  PanelText_GmPanel_Text_C_Hm_Content = {
    Text = "Graphic Quality: High/Low"
  },
  PanelText_GmPanel_Text_C_Placeholder_Content = {Text = "Search"},
  PanelText_GmPanel_Text_C_Xy_Content = {
    Text = "Test Protocol"
  },
  PanelText_GmPanel_Text_C_Yc_Content = {
    Text = "Hide GM Panel"
  },
  PanelText_GmParamEquipPanel_Text_C_Fs_Content = {Text = "Send"},
  PanelText_GmParamEquipPanel_Text_C_Tc_Content = {Text = "Exit"},
  PanelText_GmParamPanel_Text_C_Fs_Content = {Text = "Send"},
  PanelText_GmParamPanel_Text_C_Param_Content = {Text = "Settings"},
  PanelText_GmParamPanel_Text_C_Tc_Content = {Text = "Exit"},
  PanelText_Icon_Common_Text_C_New_Content = {Text = "NEW"},
  PanelText_Icon_Common_Text_C_NoEquip_Content = {
    Text = "Cannot be equipped"
  },
  PanelText_Item_Battle_AwakerSkill_1_Text_C_CD_Content = {
    Text = "On Cooldown..."
  },
  PanelText_Item_Battle_AwakerSkill_1_Text_C_Ready_Content = {Text = "Exalt"},
  PanelText_Panel_Announcement_Text_C_Panel_Title_Content = {Text = "Notice"},
  PanelText_Panel_Battle_Monster_Information_Text_C_Monster_Content = {
    Text = "Monster's Intention"
  },
  PanelText_Panel_Battle_Monster_Information_Text_C_Story_Content = {
    Text = "Monster Story"
  },
  PanelText_Panel_Gm_Battle_Art_Entry_Text_C_Canel_Content = {Text = "Cancel"},
  PanelText_Panel_Gm_Battle_Art_Entry_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_Panel_Gm_Battle_Dbg_Entry_Text_C_Canel_Content = {Text = "Cancel"},
  PanelText_Panel_Gm_Battle_Dbg_Entry_Text_C_Enter_Content = {Text = "Challenge"},
  PanelText_Panel_Gm_PVP_Text_C_Canel_Content = {Text = "Cancel"},
  PanelText_Panel_Gm_PVP_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_Popup_Gm_Effect_Side_Text_Content = {
    Text = "Edit Effect"
  },
  PanelText_Popup_Gm_Effect_Text_Cancel_Content = {Text = "Close"},
  PanelText_Popup_Gm_Effect_Text_Confirm_Content = {Text = "Confirm"},
  PanelText_RedPoint_New_Text_C_New_Content = {Text = "NEW"},
  PanelText_Simple_List_Panel_Text_Content = {Text = "Close"},
  PanelText_Text_C_Activate_Text_C_Activate_Content = {
    Text = "Stimulant Materials"
  },
  PanelText_UI_Address_Panel_Main_Text_C_NoTask1_Content = {
    Text = "Select Contact"
  },
  PanelText_UI_Address_Panel_Main_Text_C_Title_Content = {
    Text = "Communicator"
  },
  PanelText_UI_Address_Panel_Main_Text_Time_Content = {
    Text = "Click the list to select"
  },
  PanelText_UI_Address_Panel_NewMessage_Text_C_NoTask_Content = {
    Text = "No new messages"
  },
  PanelText_UI_Address_Panel_NewMessage_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Address_Panel_NewMessage_Text_Title_Content = {
    Text = "Communicator"
  },
  PanelText_UI_Alchemy_Panel_Main_Text_C_Achievement_Content = {
    Text = "Covenant Dismantle"
  },
  PanelText_UI_Alchemy_Panel_Main_Text_C_Alchemy_Content = {Text = "Synthesis"},
  PanelText_UI_Alchemy_Panel_Main_Text_C_Permute_Content = {
    Text = "Material Conversion"
  },
  PanelText_UI_Alchemy_Panel_Main_Text_C_Recast_Content = {
    Text = "Covenant Reprint"
  },
  PanelText_UI_Alchemy_Panel_Main_Text_C_ResolveItem_Content = {
    Text = "Material Breakdown"
  },
  PanelText_UI_Alchemy_Panel_Main_Text_C_ResolveWeapon_Content = {
    Text = "Wheel Dismantle"
  },
  PanelText_UI_Alchemy_Panel_Main_Text_Title_Content = {Text = "Foundry"},
  PanelText_UI_Alchemy_Popup_Break_Down_Placeholder_Content = {
    Text = "Click to enter the answer"
  },
  PanelText_UI_Alchemy_Popup_MyTeam_Awaker_List_Optimize_Text_Btn_Save_Content = {Text = "Save"},
  PanelText_UI_Alchemy_Popup_Permute_Text_C_Name_Content = {
    Text = "Substitution"
  },
  PanelText_UI_Alchemy_Popup_Permute_Text_C_Possess_Tips_Content = {
    Text = "About to Replace"
  },
  PanelText_UI_Alchemy_Popup_Permute_Text_C_Tips_Content = {
    Text = "There are no items available for dismantling"
  },
  PanelText_UI_Alchemy_Popup_Permute_Text_C_Title_Content = {
    Text = "Materials Required for Substitution"
  },
  PanelText_UI_Alchemy_Popup_Permute_Text_C_Whole_Content = {Text = "Select All"},
  PanelText_UI_Alchemy_Popup_Recast_Text_C_Name_Content = {Text = "Reprint"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_All_Content = {Text = "Select All"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_Cap_Content = {
    Text = "Level Maxed"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_GoodsNum_Content = {Text = "Quantity"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_Mail_Content = {Text = "Rules"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_MaterialsNull_Content = {
    Text = "No related Covenant"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_NormotopiaCap_Content = {
    Text = "Align limit has been reached"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_Null_Content = {
    Text = "Please select the item to dismantle"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_Resolve_Content = {
    Text = "Dismantle to obtain"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Optimize_Text_C_Sel_Content = {Text = "Selected"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_C_Screen_Content = {Text = "Filter"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_C_Tips1_Content = {Text = "I"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_C_Tips2_Content = {Text = "II"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_C_Tips3_Content = {Text = "III"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_C_Tips4_Content = {Text = "IV"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_C_Tips5_Content = {Text = "V"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_C_Tips6_Content = {Text = "VI"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_Confirm_Content = {Text = "Confirm"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_Reset_Content = {Text = "Reset"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_SiftTitle1_Content = {Text = "Set"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_SiftTitle2_Content = {Text = "Position"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_SiftTitle3_Content = {Text = "Mainstat"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Sift2_Text_SiftTitle4_Content = {Text = "Substat"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_SiftTitle_Text_C_Tips_Content = {Text = "Select"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_All_Content = {Text = "Select All"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_Cap_Content = {
    Text = "Level Maxed"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_GoodsNum_Content = {Text = "Quantity"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_MaterialsNull_Content = {
    Text = "No related Covenant"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_NormotopiaCap_Content = {
    Text = "Ascension Maxed"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_Null_Content = {
    Text = "Please select the item to dismantle"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_Resolve_Content = {
    Text = "Dismantle to obtain"
  },
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_C_Sel_Content = {Text = "Selected"},
  PanelText_UI_Alchemy_Popup_ResolveMaterials_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_All_Content = {Text = "Select All"},
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_Cap_Content = {
    Text = "Level Maxed"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_GoodsNum_Content = {Text = "Quantity"},
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_Mail_Content = {Text = "Rules"},
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_NormotopiaCap_Content = {
    Text = "Align limit has been reached"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_Null_Content = {
    Text = "Please select the item to dismantle"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_Resolve_Content = {
    Text = "Dismantle to obtain"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_Sel_Content = {Text = "Selected"},
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Optimize_Text_C_WeaponNull_Content = {
    Text = "No related Wheel of Destiny"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_All_Content = {Text = "Select All"},
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_Cap_Content = {
    Text = "Level Maxed"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_GoodsNum_Content = {Text = "Quantity"},
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_NormotopiaCap_Content = {
    Text = "Ascension Maxed"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_Null_Content = {
    Text = "Please select the item to dismantle"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_Resolve_Content = {
    Text = "Dismantle to obtain"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_Sel_Content = {Text = "Selected"},
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_C_WeaponNull_Content = {
    Text = "No related Wheel of Destiny"
  },
  PanelText_UI_Alchemy_Popup_ResolveWeapon_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Alchemy_Popup_Resolve_Text_C_Name_Content = {Text = "Dismantle"},
  PanelText_UI_Alchemy_Popup_Resolve_Text_C_Whole_Content = {Text = "Select All"},
  PanelText_UI_Alchemy_Popup_Resolve_Text_Need_Content = {Text = "Consumes"},
  PanelText_UI_Alchemy_Popup_Resolve_Text_None_Tips_Content = {
    Text = "Select the items to dismantle from the left."
  },
  PanelText_UI_Alchemy_Popup_Resolve_Text_Possess_Tips_Content = {
    Text = "About to Dismantle"
  },
  PanelText_UI_Alchemy_Popup_Resolve_Text_Slider_Tips_Content = {
    Text = "Number of Dismantles"
  },
  PanelText_UI_Alchemy_Popup_Resolve_Text_Tips_Content = {
    Text = "No materials available for dismantling"
  },
  PanelText_UI_Alchemy_Popup_Resolve_Text_Title_Content = {
    Text = "Materials obtained from dismantling"
  },
  PanelText_UI_Alchemy_Popup_Suit_Item1_Text_C_Have_Content = {Text = "YES"},
  PanelText_UI_Alchemy_Popup_Suit_Item1_Text_C_Not_Content = {Text = "NO"},
  PanelText_UI_Announcement_Item_Title_Text_New_Info_Content = {Text = "NEW"},
  PanelText_UI_Awaker_Button_Addition_Text_C_Tips_Content = {Text = "Bonus"},
  PanelText_UI_Awaker_Item_Info_Text_C_Attribute_Content = {Text = "Attributes"},
  PanelText_UI_Awaker_Item_Info_Text_C_Attributes_Title_Content = {Text = "Attributes"},
  PanelText_UI_Awaker_Item_Info_Text_C_Configuration_Content = {Text = "Lineup"},
  PanelText_UI_Awaker_Item_Info_Text_C_Grade_Content = {Text = "Level"},
  PanelText_UI_Awaker_Item_Info_Text_C_High_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Info_Text_C_Love_Content = {Text = "Affinity"},
  PanelText_UI_Awaker_Item_Info_Text_C_Love_Title_Content = {Text = "Affinity"},
  PanelText_UI_Awaker_Item_Info_Text_C_Name1_Content = {Text = "Name"},
  PanelText_UI_Awaker_Item_Info_Text_C_No_Content = {
    Text = "Not Obtained"
  },
  PanelText_UI_Awaker_Item_Info_Text_C_Normal_Content = {Text = "Level 60"},
  PanelText_UI_Awaker_Item_Info_Text_C_Potential_Content = {
    Text = "Gnostic Advance"
  },
  PanelText_UI_Awaker_Item_Info_Text_C_Score_Force_Content = {
    Text = "Combat Power"
  },
  PanelText_UI_Awaker_Item_Info_Text_C_Title_Content = {Text = "Realm"},
  PanelText_UI_Awaker_Item_Info_Text_C_Xx_Content = {Text = "Details"},
  PanelText_UI_Awaker_Item_List_3_Text_C_Full_Content = {
    Text = "Enlighten is full"
  },
  PanelText_UI_Awaker_Item_List_3_Text_C_Got_Content = {Text = "Awakened"},
  PanelText_UI_Awaker_Item_List_Text_C_Already_Content = {
    Text = "Already Owned"
  },
  PanelText_UI_Awaker_Item_List_Text_C_Disable_Content = {
    Text = "Cannot be replaced"
  },
  PanelText_UI_Awaker_Item_List_Text_C_Full_Content = {
    Text = "Fully Enlightened"
  },
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Backtrace_Click_Content = {Text = "Reset"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Backtrace_Content = {Text = "Reset"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Enlightenment_Click_Content = {Text = "Enlighten"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Enlightenment_Content = {Text = "Enlighten"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Information_Click_Content = {Text = "Archives"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Information_Content = {Text = "Archives"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Michi_Click_Content = {Text = "Covenant"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Michi_Content = {Text = "Covenant"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Skill_Click_Content = {Text = "Skills"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Skill_Content = {Text = "Skills"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_State_Click_Content = {Text = "Overview"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_State_Content = {Text = "Overview"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_TalentSelected_Content = {Text = "Talents"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Talent_Content = {Text = "Talents"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Weapons_Click_Content = {Text = "WoD"},
  PanelText_UI_Awaker_Item_Minihead_List_Text_C_Weapons_Content = {Text = "WoD"},
  PanelText_UI_Awaker_Item_Origin_Tip_Text_C_Consume_Content = {
    Text = "The following items will be consumed to unlock Genesis Form. Are you sure?"
  },
  PanelText_UI_Awaker_Item_Origin_Tip_Text_C_Name_1_Content = {Text = "Cancel"},
  PanelText_UI_Awaker_Item_Origin_Tip_Text_C_Name_2_Content = {Text = "Confirm"},
  PanelText_UI_Awaker_Item_Origin_Tip_Text_C_Title_Content = {
    Text = "Genesis Unlock Confirmation"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Common_Click_Content = {Text = "Universal"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Common_Content = {Text = "Universal"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Course_Click_Content = {
    Text = "Challenge Mode"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Course_Content = {
    Text = "Challenge Mode"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Current_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Lower_Content = {Text = "Next Level"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Mad_Content = {
    Text = "Over-Exalt is Unlocked"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_MaxMad_Solved_Content = {Text = "Unlocked"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_MaxMad_Unsolved_Content = {Text = "Locked"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_MaxMad_WillSolved_Content = {
    Text = "Will Unlock"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Ranking_Content = {Text = "Preview"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Skill_Story_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Solved_Content = {Text = "Unlocked"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Survey_Click_Content = {
    Text = "Investigation Mode"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Survey_Content = {
    Text = "Investigation Mode"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Unsolved_Content = {Text = "Locked"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_Warning_CN_Content = {Text = "Activated"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_C_WillSolved_Content = {
    Text = "Will Unlock"
  },
  PanelText_UI_Awaker_Item_Potency_Detail_Text_MaxMad_Solved_Content = {Text = "Unlocked"},
  PanelText_UI_Awaker_Item_Potency_Detail_Text_MaxMad_Unsolved_Content = {Text = "Locked"},
  PanelText_UI_Awaker_Item_Preview_Text_C_Confirm_1_Content = {
    Text = "Genesis Return"
  },
  PanelText_UI_Awaker_Item_Preview_Text_C_Confirm_2_Content = {
    Text = "Development Reset"
  },
  PanelText_UI_Awaker_Item_Preview_Text_C_Preview_1_Content = {
    Text = "Reset Preview"
  },
  PanelText_UI_Awaker_Item_Preview_Text_C_Preview_2_Content = {
    Text = "Reset Preview"
  },
  PanelText_UI_Awaker_Item_Preview_Text_C_Preview_3_Content = {
    Text = "Reset Preview"
  },
  PanelText_UI_Awaker_Item_Preview_Text_C_TipDesc_Content = {
    Text = "After 'Development Reset' or 'Enlightenment Reset', you can no longer perform 'Genesis Return'."
  },
  PanelText_UI_Awaker_Item_Skill_Detail_Text_C_Problem_Click_Content = {
    Text = "Challenge Mode"
  },
  PanelText_UI_Awaker_Item_Skill_Detail_Text_C_Problem_Content = {
    Text = "Challenge Mode"
  },
  PanelText_UI_Awaker_Item_Skill_Detail_Text_C_Special_Click_Content = {Text = "Special"},
  PanelText_UI_Awaker_Item_Skill_Detail_Text_C_Special_Content = {Text = "Special"},
  PanelText_UI_Awaker_Item_Skill_Detail_Text_C_Survey_Click_Content = {
    Text = "Investigation Mode"
  },
  PanelText_UI_Awaker_Item_Skill_Detail_Text_C_Survey_Content = {
    Text = "Investigation Mode"
  },
  PanelText_UI_Awaker_Item_Skill_Detail_Text_C_Warning_CN_Content = {Text = "Activated"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Attribute_Content = {Text = "Attributes"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Full_Content = {
    Text = "Max Lv. Reached"
  },
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_High_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Insufficient_Content = {
    Text = "Insufficient Level"
  },
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Maniac_Content = {Text = "Exalt"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Material_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Name_1_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Name_2_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Name_3_Content = {Text = "Next"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Name_4_Content = {Text = "Next"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Normal_Content = {Text = "Max Lv."},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_PressTip_Content = {
    Text = "Long press each card to zoom in for details."
  },
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Skill_Tag_Content = {Text = "Exalt"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Tale_Content = {Text = "Story"},
  PanelText_UI_Awaker_Item_Skill_Training_Text_C_Text_Need_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Attribute_Content = {Text = "Attributes"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Maniac_Content = {Text = "Exalt"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Material_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Name_1_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Name_2_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Name_3_Content = {Text = "Next Level"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Name_4_Content = {Text = "Next Level"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Tale_Content = {Text = "Story"},
  PanelText_UI_Awaker_Item_Skill_Upgrades_Text_C_Text_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Item_Topped_DetailUp_Text_C_Material_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Topped_DetailUp_Text_C_Tips_Content = {
    Text = "Auto Insert"
  },
  PanelText_UI_Awaker_Item_Topped_DetailUp_Text_Confirm_Content = {Text = "Upgrade"},
  PanelText_UI_Awaker_Item_Topped_Detail_Text_C_Condition_2_Content = {Text = "Option Two"},
  PanelText_UI_Awaker_Item_Topped_Detail_Text_C_Condition_3_Content = {
    Text = "Option Three"
  },
  PanelText_UI_Awaker_Item_Topped_Detail_Text_C_Condition_Content = {
    Text = "Prerequisites"
  },
  PanelText_UI_Awaker_Item_Topped_Detail_Text_C_Material_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Topped_Detail_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Item_Topped_Detail_Text_C_New_Content = {
    Text = "New Skill Can Be Activated"
  },
  PanelText_UI_Awaker_Item_Topped_Detail_Text_C_Title_Content = {
    Text = "Prerequisites <WordSize20:(Choose One)>"
  },
  PanelText_UI_Awaker_Item_Trinket_Change_Text_C_Prop_Title_Content = {Text = "Attributes"},
  PanelText_UI_Awaker_Item_Trinket_Change_Text_C_Tips_Content = {Text = "Details"},
  PanelText_UI_Awaker_Item_Trinket_Detail_Text_C_Empty_Content = {
    Text = "Select Covenants"
  },
  PanelText_UI_Awaker_Item_Trinket_Refinement_Text_C_Deplete_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Trinket_Refinement_Text_C_Fast_Content = {
    Text = "Quick Insert"
  },
  PanelText_UI_Awaker_Item_Trinket_Refinement_Text_C_Name_1_Content = {Text = "Upgrade"},
  PanelText_UI_Awaker_Item_Trinket_Refinement_Text_C_Name_2_Content = {
    Text = "Transcription"
  },
  PanelText_UI_Awaker_Item_Trinket_Refinement_Text_C_Name_3_Content = {Text = "Upgrade"},
  PanelText_UI_Awaker_Item_Trinket_Refinement_Text_C_Refinement_Content = {
    Text = "Covenant Details"
  },
  ["PanelText_UI_Awaker_Item_Voice_Text_C_ Voice_Content"] = {
    Text = "Voice Actor"
  },
  PanelText_UI_Awaker_Item_Voice_Text_C_Battle_Normal_Content = {Text = "Battle"},
  PanelText_UI_Awaker_Item_Voice_Text_C_Battle_Select_Content = {Text = "Battle"},
  PanelText_UI_Awaker_Item_Voice_Text_C_Daily_Normal_Content = {Text = "Daily"},
  PanelText_UI_Awaker_Item_Voice_Text_C_Daily_Select_Content = {Text = "Daily"},
  PanelText_UI_Awaker_Item_Voice_Text_C_Pvp_Normal_Content = {Text = "Traphase"},
  PanelText_UI_Awaker_Item_Voice_Text_C_Pvp_Select_Content = {Text = "Traphase"},
  PanelText_UI_Awaker_Item_Weapon_Refine_Text_C_Coin_Need_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Blackpool_Content = {
    Text = "Return to Preview"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Confirm_3_Content = {
    Text = "Genesis Return"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Confirm_4_Content = {
    Text = "Stack Traceback"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Cost_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Current_Content = {
    Text = "Current Effect"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Expend_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Fast_Content = {
    Text = "Quick Insert"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Full_Content = {Text = "Max Level"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Name_1_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Name_2_Content = {Text = "Current"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Name_3_Content = {Text = "Stacked"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Name_4_Content = {Text = "Stacked"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Name_5_Content = {Text = "Next"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Name_6_Content = {Text = "Next"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Next_Content = {
    Text = "<Blue:Next Phase Effect>"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Overlay_Content = {
    Text = "Return to Preview"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Refine_Level_Content = {
    Text = "WoD Ascension Rank"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Refine_Name_Content = {Text = "Ascension"},
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Title_Content = {
    Text = "Select Material"
  },
  PanelText_UI_Awaker_Item_Weapon_Refining_Text_C_Warning_Content = {
    Text = "Ascension Level Cap Reached"
  },
  PanelText_UI_Awaker_Item_Weapon_Strengthen2_Text_C_BackTraceTip_Content = {
    Text = "After using \"Ascension Reset,\" \"Genesis Return\" can no longer be performed."
  },
  PanelText_UI_Awaker_Item_Weapon_Strengthen2_Text_C_Title_Content = {
    Text = "WoD Details"
  },
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Backtrack_Click_Content = {Text = "Reset"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Backtrack_Content = {Text = "Reset"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Breakthrough_Content = {Text = "Edify"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Enhance_Content = {Text = "Enhance"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Equip_Content = {Text = "Equipped"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Fast_Content = {
    Text = "Quick Insert"
  },
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Need_1_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Refine_Content = {Text = "Ascension"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Skill_Click_Content = {Text = "Ascension"},
  PanelText_UI_Awaker_Item_Weapon_Strengthen_Text_C_Skill_Content = {Text = "Ascension"},
  ["PanelText_UI_Awaker_PanelStory_Main_Text_C_ Describe_Content"] = {
    Text = "Description"
  },
  PanelText_UI_Awaker_PanelStory_Main_Text_C_Message_Content = {
    Text = "Awakener Info"
  },
  PanelText_UI_Awaker_PanelStory_Main_Text_C_Story_Content = {Text = "Story"},
  PanelText_UI_Awaker_PanelStory_Main_Text_C_Title_Content = {Text = "Basic Info"},
  PanelText_UI_Awaker_Panel_Main_Text_C_Ordinary_1_Content = {
    Text = "Normal Form"
  },
  PanelText_UI_Awaker_Panel_Main_Text_C_Ordinary_2_Content = {
    Text = "Normal Form"
  },
  PanelText_UI_Awaker_Panel_Main_Text_C_Ordinary_3_Content = {
    Text = "Normal Form"
  },
  PanelText_UI_Awaker_Panel_Main_Text_C_Origin_1_Content = {
    Text = "Genesis Form"
  },
  PanelText_UI_Awaker_Panel_Main_Text_C_Origin_2_Content = {
    Text = "Genesis Form"
  },
  PanelText_UI_Awaker_Panel_Main_Text_C_Origin_3_Content = {
    Text = "Genesis Form"
  },
  PanelText_UI_Awaker_Panel_Trinket_BindDetail_Text_C_Null_Content = {
    Text = "No available Covenant for this part"
  },
  PanelText_UI_Awaker_Panel_Trinket_BindDetail_Text_C_Strengthen_Content = {Text = "Enhance"},
  PanelText_UI_Awaker_Panel_Trinket_BindDetail_Text_C_Title_Content = {
    Text = "Covenant Bind"
  },
  PanelText_UI_Awaker_Panel_Trinket_BindDetail_Text_C_UnBindAll_Content = {Text = "Unbind All"},
  PanelText_UI_Awaker_Panel_Weapon_Main_Text_C_Contrast_Content = {Text = "Compare"},
  PanelText_UI_Awaker_Panel_Weapon_Main_Text_C_Info1_Content = {
    Text = "Wheel of Destiny"
  },
  PanelText_UI_Awaker_Panel_Weapon_Main_Text_Empty1_Content = {
    Text = "Select a Wheel from the left"
  },
  PanelText_UI_Awaker_Panel_Weapon_Main_Text_Empty_Content = {
    Text = "Select a Wheel from the left"
  },
  PanelText_UI_Awaker_Panel_Weapon_Main_Text_Unlock_Content = {
    Text = [[
Edify the Awakener to
Unlock the Second WoD]]
  },
  PanelText_UI_Awaker_Popup_Attribute_Detail_Text_C_Title_Content = {
    Text = "Attribute Details"
  },
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_C_Name_1_Content = {
    Text = "View Details"
  },
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_C_Name_2_Content = {Text = "Confirm"},
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_C_NoTask_Content = {
    Text = "No items available for Reset"
  },
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_C_Title_Content = {
    Text = "Select an Item"
  },
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_C_Toggle_1_Content = {
    Text = "Standard Awakener"
  },
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_C_Toggle_2_Content = {
    Text = "Special Awakener"
  },
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_C_Toggle_3_Content = {
    Text = "Wheel of Destiny"
  },
  PanelText_UI_Awaker_Popup_Backtrace_Tip_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Awaker_Popup_BindTrinket_Text_C_BindTip_Content = {
    Text = "Covenants equipped by other Awakeners or present in lineups/plans/support have been automatically removed"
  },
  PanelText_UI_Awaker_Popup_BindTrinket_Text_C_Tip_Content = {
    Text = "Confirm binding the following Covenant?"
  },
  PanelText_UI_Awaker_Popup_BindTrinket_Text_C_Title_Content = {
    Text = "Bind Confirmation"
  },
  PanelText_UI_Awaker_Popup_Exchange_Tip_Text_C_Need_Content = {Text = "Needs"},
  PanelText_UI_Awaker_Popup_Exchange_Tip_Text_Cancel_Name_Content = {Text = "Cancel"},
  PanelText_UI_Awaker_Popup_Exchange_Tip_Text_Confirm_Name_Content = {Text = "Confirm"},
  PanelText_UI_Awaker_Popup_Exchange_Tip_Text_Tip_Content = {
    Text = "Insufficient materials. Substitute with the following?"
  },
  PanelText_UI_Awaker_Popup_Exchange_Tip_Text_Title_Content = {Text = "Hint"},
  PanelText_UI_Awaker_Popup_Expired_Tip_Text_C_Expired_Content = {
    Text = "The following items are no longer available."
  },
  PanelText_UI_Awaker_Popup_Expired_Tip_Text_C_Title_Content = {
    Text = "Items Expired"
  },
  PanelText_UI_Awaker_Popup_Jewelry_Text_C_No_Trinket_Content = {
    Text = "No available materials"
  },
  PanelText_UI_Awaker_Popup_Jewelry_Text_C_Title_Content = {
    Text = "Select Covenant"
  },
  PanelText_UI_Awaker_Popup_Levelup_Tip_Text_C_Content_Content = {Text = "Level Max"},
  PanelText_UI_Awaker_Popup_Levelup_Tip_Text_C_Title_Content = {Text = "Level Up"},
  PanelText_UI_Awaker_Popup_Levelup_Tip_Text_C_Xuyao_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Popup_MyTeam_List_Text_C_William_Content = {
    Text = "No upgrade materials available"
  },
  PanelText_UI_Awaker_Popup_Potential_Text_C_Coin_Count_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Popup_Potential_Text_C_Material_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Popup_Potential_Text_C_Name_Content = {
    Text = "Gnostic Advance"
  },
  PanelText_UI_Awaker_Popup_Potential_Text_C_Title_Content = {
    Text = "Level Cap Increase"
  },
  PanelText_UI_Awaker_Popup_Preview_Tip_Text_C_Preview_Content = {
    Text = "After the Reset, you can obtain the following materials."
  },
  PanelText_UI_Awaker_Popup_Preview_Tip_Text_C_Title_Content = {
    Text = "Reset Preview"
  },
  PanelText_UI_Awaker_Popup_Skillup_Tip_Text_C_Content_Content = {Text = "Level Cap"},
  PanelText_UI_Awaker_Popup_Skillup_Tip_Text_C_Title_Content = {
    Text = "Skill Upgrade"
  },
  PanelText_UI_Awaker_Popup_Skillup_Tip_Text_C_Xuyao_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Popup_Talent_Main_Text_C_Condition_Content = {
    Text = "Unlock Conditions"
  },
  PanelText_UI_Awaker_Popup_Talent_Main_Text_C_Current_Content = {Text = "Current"},
  PanelText_UI_Awaker_Popup_Talent_Main_Text_C_High_Content = {Text = "Current"},
  PanelText_UI_Awaker_Popup_Talent_Main_Text_C_Lower_Content = {Text = "Next"},
  PanelText_UI_Awaker_Popup_Talent_Main_Text_C_Normal_Content = {Text = "Max Lv."},
  PanelText_UI_Awaker_Popup_Talent_Main_Text_C_Warning_EN_Content = {
    Text = "Prerequisites"
  },
  PanelText_UI_Awaker_Popup_Talent_Main_Text_Require_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Popup_Tip_Preview_Text_C_Title_Content = {
    Text = "Materials Preview"
  },
  PanelText_UI_Awaker_Popup_Tip_Succeed_Text_C_Info_Content = {
    Text = "Click anywhere to close"
  },
  PanelText_UI_Awaker_Popup_Tip_Succeed_Text_C_New_Content = {
    Text = "New Skill Can Be Activated"
  },
  PanelText_UI_Awaker_Popup_Tip_Succeed_Text_C_Succeed_Content = {
    Text = "Successfully Edified"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_Breakthrough_Content = {
    Text = "Sync Requirement"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_Now_Content = {Text = "Current"},
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_Reward_Content = {
    Text = "Level Rewards"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_Screening_Content = {
    Text = "Reward Claimed"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_Tips_Content = {Text = "View"},
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_Title_Content = {Text = "Affinity"},
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_Unlock_Content = {
    Text = "Archive Unlocked"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_C_illustrate_Content = {Text = "Rules"},
  PanelText_UI_Awaker_Popup_Topped_Tip2_Text_Name_Content = {Text = "Claim"},
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Activation_Content = {Text = "Activated"},
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Lv_Content = {
    Text = "Level Cap Increase"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Mad_Content = {
    Text = "Level Cap Increase"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Mad_Name_Content = {
    Text = "Over-Exaltation"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_MaxMad_Activation_Content = {Text = "Activated"},
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_MaxMad_Name_Content = {
    Text = "Absolute Axiom"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Not_Activation_Content = {Text = "Inactive"},
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Not_MaxMad_Activation_Content = {Text = "Inactive"},
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Solved_Content = {Text = "Unlocked"},
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Title_Content = {
    Text = "Psyche Surge"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Unlock_Content = {
    Text = "Attribute Enhancement"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_Unsolved_Content = {Text = "Locked"},
  PanelText_UI_Awaker_Popup_Topped_Tip3_Text_C_WillSolved_Content = {
    Text = "Will Unlock"
  },
  PanelText_UI_Awaker_Popup_Topped_Tip_Text_C_Material_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Popup_Topped_Tip_Text_C_Need_Content = {Text = "Needs"},
  PanelText_UI_Awaker_Popup_Topped_Tip_Text_C_Reward_Content = {Text = "Reward"},
  PanelText_UI_Awaker_Popup_Topped_Tip_Text_C_Title_Content = {Text = "Preview"},
  PanelText_UI_Awaker_Popup_Topped_Tip_Text_Screening_Content = {
    Text = "Reward Claimed"
  },
  PanelText_UI_Awaker_Popup_Trinket_Bind_Text_C_BindTip_Content = {
    Text = "After binding, the Covenant's main attribute is increased by an additional 50%!"
  },
  PanelText_UI_Awaker_Popup_Trinket_Bind_Text_C_Bind_Content = {
    Text = "Bind Covenant"
  },
  PanelText_UI_Awaker_Popup_Trinket_Bind_Text_C_Empty_Suit_Content = {
    Text = "No active set effects"
  },
  PanelText_UI_Awaker_Popup_Trinket_Bind_Text_C_Foundation_Content = {Text = "Attributes"},
  PanelText_UI_Awaker_Popup_Trinket_Bind_Text_C_Rule_Content = {Text = "Rules"},
  PanelText_UI_Awaker_Popup_Trinket_Bind_Text_C_Suit_Content = {Text = "Set Effect"},
  PanelText_UI_Awaker_Popup_Trinket_Change_Text_C_Equipped_Content = {Text = "Equipped"},
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_AutoRefinement_Content = {
    Text = "Auto-Transcription"
  },
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Empty_Content = {
    Text = "Transcribe to roll new Substats"
  },
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Exchange_Content = {Text = "Settings"},
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Foundation_Content = {
    Text = "Original Substats"
  },
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Hoist_Content = {
    Text = "New Substats"
  },
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Name_1_Content = {Text = "Transcribe"},
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Name_2_Content = {Text = "Replace"},
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_No_Entries_Content = {
    Text = "No attributes"
  },
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Skip_Content = {
    Text = "Skip Animation"
  },
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_Text_Content = {Text = "Consumes"},
  PanelText_UI_Awaker_Popup_Trinket_Conversion_Text_C_illustrate_Content = {
    Text = "Transcription Instructions"
  },
  PanelText_UI_Awaker_Popup_Trinket_Intensify_Detail_Text_C_Filtrate_Content = {
    Text = "SSR and Below Materials"
  },
  PanelText_UI_Awaker_Popup_Trinket_Intensify_Detail_Text_C_Shortcut_Content = {Text = "Quick Add"},
  PanelText_UI_Awaker_Popup_Trinket_Intensify_Detail_Text_C_Upper_Limit_Content = {
    Text = "Maximum Level Reached"
  },
  PanelText_UI_Awaker_Popup_Trinket_Intensify_List_Text_C_None_Content = {
    Text = "No unequipped Wheel of Destiny"
  },
  PanelText_UI_Awaker_Popup_Trinket_Main3_Text_C_Empty_1_Content = {
    Text = "Select Covenants"
  },
  PanelText_UI_Awaker_Popup_Trinket_Main3_Text_C_Empty_2_Content = {
    Text = "No active set effects"
  },
  PanelText_UI_Awaker_Popup_Trinket_Main3_Text_C_Foundation_Content = {Text = "Attributes"},
  PanelText_UI_Awaker_Popup_Trinket_Main3_Text_C_Suit_Content = {Text = "Set Effect"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_1_Normal_Content = {Text = "I"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_1_Select_Content = {Text = "I"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_2_Normal_Content = {Text = "II"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_2_Select_Content = {Text = "II"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_3_Normal_Content = {Text = "III"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_3_Select_Content = {Text = "III"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_4_Normal_Content = {Text = "IV"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_4_Select_Content = {Text = "IV"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_5_Normal_Content = {Text = "V"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_5_Select_Content = {Text = "V"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_6_Normal_Content = {Text = "VI"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_6_Select_Content = {Text = "VI"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_Contrast_Content = {Text = "Compare"},
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_Empty_Content = {
    Text = "Select Covenants"
  },
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_None_Content = {
    Text = "No available Covenant for this part"
  },
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_Notselected_Content = {
    Text = "Select Covenants"
  },
  PanelText_UI_Awaker_Popup_Trinket_Replace_Text_C_Suit_Content = {Text = "Set Effect"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_1_Normal_Content = {Text = "I"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_1_Select_Content = {Text = "I"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_2_Normal_Content = {Text = "II"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_2_Select_Content = {Text = "II"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_3_Normal_Content = {Text = "III"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_3_Select_Content = {Text = "III"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_4_Normal_Content = {Text = "IV"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_4_Select_Content = {Text = "IV"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_5_Normal_Content = {Text = "V"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_5_Select_Content = {Text = "V"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_6_Normal_Content = {Text = "VI"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_6_Select_Content = {Text = "VI"},
  PanelText_UI_Awaker_Popup_Trinket_Select_List_Text_C_None_Content = {
    Text = "No unequipped Covenant"
  },
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Consume_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Deplete_Content = {Text = "Materials"},
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Exchange_Content = {Text = "Settings"},
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Fast_Content = {
    Text = "Quick Insert"
  },
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Fast_High_Content = {Text = "Level +1"},
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Fast_Normal_Content = {Text = "Level +1"},
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_MaxLevel_Content = {Text = "Enhance"},
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Max_High_Content = {
    Text = "Level Up to Maximum"
  },
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Max_Normal_Content = {
    Text = "Level Up to Maximum"
  },
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Name_3_Content = {Text = "Upgrade"},
  PanelText_UI_Awaker_Popup_Trinket_Upgrade_Text_C_Title_Content = {Text = "Keeper EXP"},
  PanelText_UI_Awaker_Popup_UnBindTrinket_Text_C_AutoSelect_Content = {
    Text = "Auto-Select"
  },
  PanelText_UI_Awaker_Popup_UnBindTrinket_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Awaker_Popup_UnBindTrinket_Text_C_Confirm_Content = {
    Text = "Unbind Covenant"
  },
  PanelText_UI_Awaker_Popup_UnBindTrinket_Text_C_NeedTrinket_Content = {
    Text = "Covenant Required"
  },
  PanelText_UI_Awaker_Popup_UnBindTrinket_Text_C_Title_Content = {
    Text = "Unbind Covenant"
  },
  PanelText_UI_Awaker_Popup_Whole_Body_Text_C_Reveal_Content = {Text = "Hidden"},
  PanelText_UI_Awaker_Popup_illustrate_Tips_Text_C_Title_Content = {
    Text = "Affix Description"
  },
  PanelText_UI_Awaker_Special_Skill_Main_Text_C_Tips_Content = {Text = "Overview"},
  PanelText_UI_Awaker_Special_Skill_Main_Text_C_Title_Content = {
    Text = "Awakener Talent"
  },
  PanelText_UI_Awaker_Special_Skill_Main_Text_C_Type_Content = {Text = "Type"},
  PanelText_UI_Awaker_Weapon_Enhance_Success_Text_C_Succeed_Content = {
    Text = "Enhancing Success"
  },
  PanelText_UI_Awaker_Weapon_Refine_Success_Text_C_Info_Content = {
    Text = "Click anywhere to close"
  },
  PanelText_UI_Awaker_Weapon_Refine_Success_Text_C_Succeed_Content = {
    Text = "Ascension Success"
  },
  PanelText_UI_Bag_Helicopter_Tips_Text_C_Attribute_Content = {Text = "Attributes"},
  PanelText_UI_Bag_Helicopter_Tips_Text_C_Choose_Content = {
    Text = "Select Awakener"
  },
  PanelText_UI_Bag_Helicopter_Tips_Text_C_Helicopter_Content = {
    Text = "Amplification Preview"
  },
  PanelText_UI_Bag_Helicopter_Tips_Text_C_Skill_Content = {Text = "Skill"},
  PanelText_UI_Bag_Helicopter_Tips_Text_C_Title_Content = {
    Text = "Awakener Amplification"
  },
  PanelText_UI_Bag_Item_Appearance_Text_C_Got_Content = {Text = "Acquired"},
  PanelText_UI_Bag_Item_Detail_Optimized_Text_C_AavtarName_Content = {
    Text = "Avatar and Expression"
  },
  PanelText_UI_Bag_Item_Detail_Optimized_Text_C_Avatar_Info_Content = {
    Text = "Detail Preview"
  },
  PanelText_UI_Bag_Item_Detail_Optimized_Text_C_Source_Content = {Text = "Source"},
  PanelText_UI_Bag_Item_Detail_Optimized_Text_C_Time_Content = {
    Text = "Remaining Valid Time:"
  },
  PanelText_UI_Bag_Item_Detail_Text_C_AavtarName_Content = {
    Text = "Avatar and Expression"
  },
  PanelText_UI_Bag_Item_Detail_Text_C_Avatar_Info_Content = {
    Text = "Detail Preview"
  },
  PanelText_UI_Bag_Item_Detail_Text_C_KeeperSkill_Title_Content = {
    Text = "Posse Effect:"
  },
  PanelText_UI_Bag_Item_Detail_Text_C_Source_Content = {
    Text = "Where to Get"
  },
  PanelText_UI_Bag_Item_Detail_Text_C_Time_Content = {
    Text = "Remaining Valid Time"
  },
  PanelText_UI_Bag_Item_Detail_Text_C_Weapon_Attack_Content = {Text = "Base ATK"},
  PanelText_UI_Bag_Item_Detail_Text_C_Weapon_Defense_Content = {Text = "Base DEF"},
  PanelText_UI_Bag_Item_Detail_Text_C_Weapon_Physique_Content = {Text = "Base CON"},
  PanelText_UI_Bag_Obtain_Awaken_Tips_Text_C_Full_Content = {
    Text = "Fully Enlightened"
  },
  PanelText_UI_Bag_Obtain_Awaken_Tips_Text_C_Title_Content = {
    Text = "Select an Awakener"
  },
  PanelText_UI_Bag_Obtain_Awaken_Tips_Text_C_Yes_Content = {
    Text = "Already Owned"
  },
  PanelText_UI_Bag_Obtain_Awaken_Tips_Text_Cancel_Name_Content = {
    Text = "View Details"
  },
  PanelText_UI_Bag_Obtain_Awaken_Tips_Text_Confirm_Name_Content = {Text = "Confirm"},
  PanelText_UI_Bag_Panel_AppearancePreview_Text_C_BtnName_Content = {Text = "Claim"},
  PanelText_UI_Bag_Panel_AppearancePreview_Text_C_Emoji_Content = {Text = "Emote"},
  PanelText_UI_Bag_Panel_AppearancePreview_Text_C_Head_Content = {Text = "Avatar"},
  PanelText_UI_Bag_Panel_AppearancePreview_Text_C_NoTask_Content = {
    Text = "All Skins have been obtained"
  },
  PanelText_UI_Bag_Panel_AppearancePreview_Text_C_RatePreview_Content = {
    Text = "Probability Preview"
  },
  PanelText_UI_Bag_Panel_AppearancePreview_Text_C_Title_Content = {
    Text = "Appearance Preview"
  },
  PanelText_UI_Bag_Panel_Main_Text_AmountName_Content = {Text = "Quantity"},
  PanelText_UI_Bag_Panel_Main_Text_C_2_Select_Content = {Text = "Order"},
  PanelText_UI_Bag_Panel_Main_Text_C_Cap_Content = {
    Text = "Level Maxed"
  },
  PanelText_UI_Bag_Panel_Main_Text_C_None2_Content = {Text = "No Item"},
  PanelText_UI_Bag_Panel_Main_Text_C_NormotopiaCap_Content = {
    Text = "Ascension Maxed"
  },
  PanelText_UI_Bag_Panel_Main_Text_C_Replacement_Content = {Text = "Substitute"},
  PanelText_UI_Bag_Panel_Main_Text_C_Resolve_Content = {Text = "Dismantle"},
  PanelText_UI_Bag_Panel_Main_Text_C_Synthesis_Content = {Text = "Synthesize"},
  PanelText_UI_Bag_Panel_Main_Text_C_recast_Content = {Text = "Reforge"},
  PanelText_UI_Bag_Panel_Main_Text_Intensify_Content = {Text = "Enhance"},
  PanelText_UI_Bag_Panel_Main_Text_Normotopia_Content = {Text = "Ascension"},
  PanelText_UI_Bag_Panel_Main_Text_Title_Content = {Text = "Inventory"},
  PanelText_UI_Bag_Panel_OptionalAppearance_Text_C_BtnName_Content = {Text = "Claim"},
  PanelText_UI_Bag_Panel_OptionalAppearance_Text_C_Emoji_Content = {Text = "Expression"},
  PanelText_UI_Bag_Panel_OptionalAppearance_Text_C_Head_Content = {Text = "Avatar"},
  PanelText_UI_Bag_Panel_OptionalAppearance_Text_C_NoTask_Content = {
    Text = "No data available"
  },
  PanelText_UI_Bag_Panel_OptionalAppearance_Text_C_Title_Content = {
    Text = "Appearance Preview"
  },
  PanelText_UI_Battle_Item_CounterAwaker_Text_C_CounterAwaker_Content = {
    Text = "Attack Bonus Awakeners"
  },
  PanelText_UI_Battle_Panel_CardStack_Text_C_Disabled_CN_Content = {Text = "Confirm"},
  PanelText_UI_Battle_Panel_CardStack_Text_C_Display_Content = {
    Text = "Display Card"
  },
  PanelText_UI_Battle_Panel_CardStack_Text_C_Hide_Content = {Text = "Hide Cards"},
  PanelText_UI_Battle_Panel_CardStack_Text_C_High_CN_Content = {Text = "Confirm"},
  PanelText_UI_Battle_Panel_CardStack_Text_C_Normal_CN_Content = {Text = "Confirm"},
  PanelText_UI_Battle_Panel_Dbg_Text_C_Count_Content = {Text = "D-Tide"},
  PanelText_UI_Battle_Panel_Dbg_Text_C_End_EN_No_Content = {Text = "End"},
  PanelText_UI_Battle_Panel_Dbg_Text_C_Insufficient_Content = {
    Text = "Insufficient Space"
  },
  PanelText_UI_Battle_Panel_Dbg_Text_C_Name_Content = {Text = "End"},
  PanelText_UI_Battle_Panel_Dbg_Text_C_Setting_Content = {Text = "Settings"},
  PanelText_UI_Battle_Panel_Dbg_Text_C_Unclickable_Content = {Text = "End"},
  PanelText_UI_Battle_Panel_Dbg_Text_C_Zongshanghai_Content = {
    Text = "Total Damage"
  },
  PanelText_UI_Battle_Panel_Dbg_Text_C_insufficient_Content = {
    Text = "Insufficient Space"
  },
  PanelText_UI_Battle_Popup_Anger_Overrun_Text_C_CD_Content = {
    Text = "On Cooldown..."
  },
  PanelText_UI_Battle_Popup_Anger_Overrun_Text_C_Cooling_Content = {
    Text = "On cooldown, can be used again next turn"
  },
  PanelText_UI_Battle_Popup_Anger_Overrun_Text_C_Title_Content = {
    Text = "Over-Exaltation"
  },
  PanelText_UI_Battle_Popup_Anger_Text_C_CD_Content = {
    Text = "On Cooldown..."
  },
  PanelText_UI_Battle_Popup_Anger_Text_C_Cooling_Content = {
    Text = "On cooldown, can be used again next turn"
  },
  PanelText_UI_Battle_Popup_Anger_Text_Title_Content = {Text = "Exalt"},
  PanelText_UI_Battle_Popup_AttachPost_Text_C_AttachPost_Content = {Text = "Pursuit"},
  PanelText_UI_Battle_Popup_CardGroup_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_UI_Battle_Popup_CardGroup_Text_C_Title_Content = {
    Text = "Select a Set of Cards"
  },
  PanelText_UI_Battle_Popup_Elite_Text_C_Victory_Content = {
    Text = "Danger Alert"
  },
  PanelText_UI_Battle_Popup_Elite_Tips_Text_C_Hang_Content = {
    Text = "Battle Start"
  },
  PanelText_UI_Battle_Popup_Elite_Tips_Text_C_Title_Content = {
    Text = "Highest Alert"
  },
  PanelText_UI_Battle_Popup_Fail_Text_C_Continue_Content = {
    Text = "Click to continue"
  },
  PanelText_UI_Battle_Popup_Fail_Text_C_Failure_Content = {
    Text = "You Lost..."
  },
  PanelText_UI_Battle_Popup_Fail_Text_C_Victory_Content = {
    Text = "You Lost..."
  },
  PanelText_UI_Battle_Popup_Furnace_Text_C_Name_1_Content = {Text = "Cancel"},
  PanelText_UI_Battle_Popup_Furnace_Text_C_Name_2_Content = {Text = "Confirm"},
  PanelText_UI_Battle_Popup_Furnace_Text_C_Title_Content = {
    Text = "Crimson Furnace"
  },
  PanelText_UI_Battle_Popup_Keeper_Skill_Tip_Text_C_Awaker_School_Content = {Text = "Keeper"},
  PanelText_UI_Battle_Popup_Keeper_Skill_Tip_Text_C_Nature_Content = {
    Text = "Team Information"
  },
  PanelText_UI_Battle_Popup_Keeper_Skill_Tip_Text_C_Property_Content = {
    Text = "Emergency Inventory"
  },
  PanelText_UI_Battle_Popup_Keeper_Skill_Tip_Text_C_Skill2_Content = {Text = "Keyflare"},
  PanelText_UI_Battle_Popup_Keeper_Skill_Tip_Text_C_Skill_Tag_Content = {Text = "Posse"},
  PanelText_UI_Battle_Popup_KeyTokenAwaken_Text_C_KeeperSkill_Content = {Text = "Release"},
  PanelText_UI_Battle_Popup_KeyTokenAwaken_Text_C_KeyTokenAwaken_Content = {Text = "Release"},
  PanelText_UI_Battle_Popup_KeyTokenAwaken_Text_C_KeyTokenNewChaos_Content = {Text = "Release"},
  PanelText_UI_Battle_Popup_KeyTokenAwaken_Text_Title_Content = {
    Text = "Select a Command"
  },
  PanelText_UI_Battle_Popup_KeyTokenAwaken_Txt_C_AwakenTitle_Content = {
    Text = "Keyflare Rouse"
  },
  PanelText_UI_Battle_Popup_KeyTokenAwaken_Txt_C_NewChaosTitle_Content = {
    Text = "Compound Posse"
  },
  PanelText_UI_Battle_Popup_Open_Tips_Text_C_Hang_Content = {
    Text = "Battle Start"
  },
  PanelText_UI_Battle_Popup_Open_Tips_Text_C_Title_Content = {
    Text = "Battle Start"
  },
  PanelText_UI_Battle_Popup_RebirthTip_Text_C_CostTips2_Content = {Text = "Redeem?"},
  PanelText_UI_Battle_Popup_RebirthTip_Text_C_CostTips_Content = {
    Text = "Emergency Gnosis is depleted. Use the following item"
  },
  PanelText_UI_Battle_Popup_RebirthTip_Text_C_Cost_Content = {Text = "Consumes"},
  PanelText_UI_Battle_Popup_RebirthTip_Text_C_Tips_Content = {
    Text = "(Redeeming will directly consume it and trigger a revival; if retreating, the Emergency Gnosis will be refunded)"
  },
  PanelText_UI_Battle_Popup_RebirthTip_Text_Cancel_Name_Content = {Text = "Cancel"},
  PanelText_UI_Battle_Popup_RebirthTip_Text_Confirm_Name_Content = {Text = "Confirm"},
  PanelText_UI_Battle_Popup_RebirthTip_Text_Title_Content = {
    Text = "Revive Now?"
  },
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_AffixInfo_Content = {
    Text = "Affix Description"
  },
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Arousal_Content = {Text = "Rouse"},
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Deepen_Tag_Content = {
    Text = "Psyche Surge"
  },
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Destiny_Content = {
    Text = "Wheel of Destiny"
  },
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Gnosisi_Content = {Text = "Rouse"},
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Maniac_Content = {
    Text = "Aliemus Value"
  },
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Property_Content = {
    Text = "Attribute Details"
  },
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Secret_Content = {Text = "Covenant"},
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Skill_Tag_Content = {Text = "Exalt"},
  PanelText_UI_Battle_Popup_Record_Tip_New_Text_C_Talent_Content = {Text = "Talent"},
  PanelText_UI_Battle_Popup_Record_Tip_Text_C_Title_Awakening_Content = {Text = "Rouse"},
  PanelText_UI_Battle_Popup_Record_Tip_Text_C_Title_Trinket_Content = {Text = "Covenant"},
  PanelText_UI_Battle_Popup_Record_Tip_Text_C_Title_Weapon_Content = {
    Text = "Wheel of Destiny"
  },
  PanelText_UI_Battle_Popup_Round_Enemy_Text_C_Our_Content = {Text = "Enemy Turn"},
  PanelText_UI_Battle_Popup_Round_Mine_Text_C_Our_Content = {Text = "Your Turn"},
  PanelText_UI_Battle_Popup_SelectAwaker_Text_C_Title_Content = {
    Text = "Select an Awakener"
  },
  PanelText_UI_Battle_Popup_Settle_Text_C_Succeed_Content = {Text = "Victory"},
  PanelText_UI_Battle_Popup_Settle_Text_C_Victory_Content = {Text = "You Win!"},
  PanelText_UI_Battle_ShenHai_Switch_Text_C_Activate_Content = {
    Text = "Current Command"
  },
  PanelText_UI_Battle_ShenHai_Switch_Text_C_Defense_Content = {Text = "Defense"},
  PanelText_UI_Battle_ShenHai_Switch_Text_C_Disable_1_Content = {
    Text = "Cannot Select"
  },
  PanelText_UI_Battle_ShenHai_Switch_Text_C_Disable_2_Content = {
    Text = "Cannot Select"
  },
  PanelText_UI_Battle_ShenHai_Switch_Text_C_Frantic_Content = {Text = "Madness"},
  PanelText_UI_Battle_ShenHai_Switch_Text_C_Name_Content = {Text = "Battle"},
  PanelText_UI_Battle_ShenHai_Switch_Text_C_Title_Content = {
    Text = "Select a Tentacle Stance"
  },
  PanelText_UI_Battle_Tip_Backtrack_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Battle_Tip_Backtrack_Text_C_Lock_Content = {
    Text = "Time Reversal: Rewind combat back to a previous turn"
  },
  PanelText_UI_Battle_Tip_Backtrack_Text_C_Sure_Content = {Text = "Confirm"},
  PanelText_UI_Battle_Tip_Backtrack_Text_C_Title_Content = {
    Text = "Time Reversal"
  },
  PanelText_UI_Battle_Tip_CounterAwaker_Text_C_Title_Content = {
    Text = "Enemy Characteristics"
  },
  PanelText_UI_Card_Panel_Discard_Text_C_Title_Content = {Text = "Hand Pile"},
  PanelText_UI_Chaper_Popup_Award_Optimized_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Chaper_Popup_Award_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Chaper_Popup_Award_Text_C_Title_Content = {
    Text = "Cumulative Rewards"
  },
  PanelText_UI_Chaper_Popup_Award_Text_C_Title_En_Content = {Text = "Drops"},
  PanelText_UI_Chaper_Popup_BsetPlot_Text_C_Cancel_Content = {Text = "Exit"},
  PanelText_UI_Chaper_Popup_BsetPlot_Text_C_Confirm_Content = {
    Text = "Proceed Anyway"
  },
  PanelText_UI_Chaper_Popup_BsetPlot_Text_C_Joinin_Content = {
    Text = "You are recommended to complete the following stages first"
  },
  PanelText_UI_Chaper_Popup_BsetPlot_Text_C_Screening_Content = {
    Text = "No more reminders today"
  },
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Activity_Bright_Content = {Text = "Orison"},
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Activity_Content = {Text = "Orison"},
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Begone_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Creation_Content = {Text = "Relic"},
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Darkactivity_Content = {Text = "Orison"},
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Darkgame_Content = {Text = "Relic"},
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Demon_Content = {
    Text = "No Orisons available"
  },
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Game_Bright_Content = {Text = "Relic"},
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Game_Content = {Text = "Relic"},
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Not_Content = {
    Text = "No Relics available"
  },
  PanelText_UI_Chaper_Popup_Detailed_Text_C_Walk_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Chaper_Popup_Level_Success_Text_C_Title_Content = {
    Text = "Stage Achievement"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Achievement_Content = {
    Text = "Enemy Preview"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Cost_Tips_Content = {
    Text = "Requirements:"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Grade_Content = {
    Text = "Recommended Level"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Monster_Content = {
    Text = "Stage Relics"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Recommend_Content = {
    Text = "Realm Recommendation"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Reward_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Suppress_Content = {
    Text = "D-Tide Onslaught"
  },
  PanelText_UI_Chaper_Popup_Particulars_New_Text_C_Title_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Chaper_Popup_Particulars_Text_C_Monster_Content = {Text = "Monster"},
  PanelText_UI_Chaper_Popup_Particulars_Text_C_Title_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Chaper_Popup_Particulars_View_Text_C_Replay_Content = {Text = "Record"},
  PanelText_UI_Chaper_Popup_StageAchieve_View_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Chapter_Dream_View_Text_C_Difficulty_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Dream_View_Text_C_Nightmare_Content = {Text = "Madness"},
  PanelText_UI_Chapter_Dream_View_Text_C_Remember_Content = {Text = "Story"},
  PanelText_UI_Chapter_Dream_View_Text_C_Simple_Content = {Text = "Normal"},
  PanelText_UI_Chapter_Hard_Swich_Group_New_Text_C_Difficulty_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Hard_Swich_Group_New_Text_C_Nightmare_Content = {Text = "Nightmare"},
  PanelText_UI_Chapter_Hard_Swich_Group_New_Text_C_Remember_Content = {Text = "Story"},
  PanelText_UI_Chapter_Hard_Swich_Group_New_Text_C_Simple_Content = {Text = "Easy"},
  PanelText_UI_Chapter_Item_Achievement_List_Text_C_Lost_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Chapter_Item_Achievement_List_Text_Lost_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Chapter_Item_Chapter_Big_Text_C_Expect_Content = {Text = "Stay tuned"},
  PanelText_UI_Chapter_Item_Chapter_Big_Text_C_Tips_Content = {Text = "Progress"},
  PanelText_UI_Chapter_Item_Chapter_Small_Text_C_Expect1_Content = {
    Text = "Not yet unlocked"
  },
  PanelText_UI_Chapter_Item_Chapter_Small_Text_C_Expect2_Content = {
    Text = "Not yet unlocked"
  },
  PanelText_UI_Chapter_Item_Chapter_Small_Text_C_Expect_Content = {Text = "Stay tuned"},
  PanelText_UI_Chapter_Item_Chapter_Small_Text_C_Tips_Content = {Text = "Progress"},
  PanelText_UI_Chapter_Item_Dream_01_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Item_Dream_02_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Item_Dream_03_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Item_Dream_04_Text_C_Free_Unlock_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Chapter_Item_Dream_05_Text_C_Chaoter_Content = {Text = "Dreamscape"},
  PanelText_UI_Chapter_Item_Dream_05_Text_C_Free_Unlock_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Chapter_Item_Skill_Text_C_Level_Content = {Text = "State"},
  PanelText_UI_Chapter_Item_Special_01_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Item_Special_02_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Item_Special_03_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Item_StageAchieve_Text_C_Lost_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Chapter_Item_Success_List_Text_Lost_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Chapter_Panel_Main2_ItemBranch_Text_C_Award_Content = {
    Text = "All reward conditions have been achieved."
  },
  PanelText_UI_Chapter_Panel_Main2_ItemBranch_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Panel_Main2_SmallBranch_Text_C_Award_Content = {Text = "Achieved"},
  PanelText_UI_Chapter_Panel_Main2_SmallBranch_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Chapter_Panel_Main2_Text_C_Difficulty_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Panel_Main2_Text_C_Nightmare_Content = {Text = "Madness"},
  PanelText_UI_Chapter_Panel_Main2_Text_C_NoTask_Content = {
    Text = "No Special Ops Available"
  },
  PanelText_UI_Chapter_Panel_Main2_Text_C_Simple_Content = {Text = "Normal"},
  PanelText_UI_Chapter_Panel_Main2_Text_C_Time_Content = {
    Text = "12/21/2023 05: 00 New arrivals, stay tuned"
  },
  PanelText_UI_Chapter_Panel_Main2_Text_C_Title_Content = {
    Text = "Investigate"
  },
  PanelText_UI_Chapter_Panel_Main_Text_C_Tishi_2_Content = {
    Text = "Achievement"
  },
  PanelText_UI_Chapter_Panel_Main_Text_C_Tishi_Content = {
    Text = "Level Score"
  },
  PanelText_UI_Chapter_Panel_Spur_Text_C_Difficulty_01_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Difficulty_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Difficulty_Now_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Nightmare_01_Content = {Text = "Madness"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Nightmare_Content = {Text = "Madness"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Nightmare_Now_Content = {Text = "Madness"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Progress_Content = {
    Text = "Investigation Progress"
  },
  PanelText_UI_Chapter_Panel_Spur_Text_C_Simple_01_Content = {Text = "Normal"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Simple_Content = {Text = "Normal"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Simple_Now_Content = {Text = "Normal"},
  PanelText_UI_Chapter_Panel_Spur_Text_C_Tab_Special_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Chapter_Panel_Spur_Text_C_Tishi_Content = {
    Text = "Achievement"
  },
  PanelText_UI_Chapter_Popup_Achievement_Text_C_Title_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Chapter_Popup_Chapter_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Chapter_Popup_Monster_Details_Text_C_Title_Content = {
    Text = "Enemy Details"
  },
  PanelText_UI_Chapter_Popup_Vibes_List_Text_C_Effect_Content = {Text = "Effect"},
  PanelText_UI_Chapter_Popup_Vibes_List_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Chapter_Popup_Vibes_List_Text_C_Story_Content = {Text = "Story"},
  PanelText_UI_Chapter_Popup_Yuanxing_Text_Title_Content = {Text = "Evaluation"},
  PanelText_UI_Chapter_Special_Spur_View_Text_C_Difficulty_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Special_Spur_View_Text_C_Nightmare_Content = {Text = "Madness"},
  PanelText_UI_Chapter_Special_Spur_View_Text_C_Progress_Content = {Text = "Progress"},
  PanelText_UI_Chapter_Special_Spur_View_Text_C_Remember_Content = {Text = "Story"},
  PanelText_UI_Chapter_Special_Spur_View_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Chapter_Special_Spur_View_Text_C_Simple_Content = {Text = "Normal"},
  PanelText_UI_Chapter_Special_Spur_View_Text_C_Tab_Special_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Chapter_Spur_View_Text_C_Difficulty_Content = {Text = "Hard"},
  PanelText_UI_Chapter_Spur_View_Text_C_Nightmare_Content = {Text = "Madness"},
  PanelText_UI_Chapter_Spur_View_Text_C_Progress_Content = {Text = "Progress"},
  PanelText_UI_Chapter_Spur_View_Text_C_Remember_Content = {Text = "Story"},
  PanelText_UI_Chapter_Spur_View_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Chapter_Spur_View_Text_C_Review_Content = {Text = "Prologue"},
  PanelText_UI_Chapter_Spur_View_Text_C_Simple_Content = {Text = "Normal"},
  PanelText_UI_Chapter_Spur_View_Text_C_Tab_Special_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Chapter_Vindicate_View_Text_C_Condition_Content = {
    Text = "Not Unlocked"
  },
  PanelText_UI_Chapter_Vindicate_View_Text_C_Name_Content = {
    Text = "Start Story"
  },
  PanelText_UI_Chapter_Vindicate_View_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Chapter_Vindicate_View_Text_C_NoTask_Content = {
    Text = "Reproduction Frenzy not yet unlocked"
  },
  PanelText_UI_Chapter_Vindicate_View_Text_C_StoryDesc_Content = {
    Text = "As the mysterious influence of \"Carnal Desires\" continues, the Awakeners have fallen into \"Reproduction Frenzy\"... Please be careful, Keepers!"
  },
  PanelText_UI_Chapter_Vindicate_View_Text_C_Tip_Content = {
    Text = "During \"Carnal Desires,\" Frenzy Stories no longer require Affinity and only cost \"Reality Verge\"*1!"
  },
  PanelText_UI_Chapter_Vindicate_View_Text_C_Title_Content = {
    Text = "Party Invitation"
  },
  PanelText_UI_Chapter_Vindicate_View_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Collection_History_Details_Text_C_Lock_Content = {
    Text = "Image materials have not yet been acquired."
  },
  PanelText_UI_Collection_History_Details_Text_Title_Content = {Text = "Record"},
  PanelText_UI_Collection_Item_Comprehensive_Text_C_Placeholder_Content = {
    Text = "Enter a term..."
  },
  PanelText_UI_Collection_Item_Music_Alone_Text_C_High_Content = {Text = "Reset BGM"},
  PanelText_UI_Collection_Item_Music_Alone_Text_C_Label_Content = {Text = "Phonograph"},
  PanelText_UI_Collection_Item_Music_Alone_Text_C_Mail_Content = {Text = "Progress"},
  PanelText_UI_Collection_Item_Music_Alone_Text_C_Normal_Content = {Text = "Set BGM"},
  PanelText_UI_Collection_Item_Music_Alone_Text_C_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Item_Music_Text_C_Label_Content = {Text = "Phonograph"},
  PanelText_UI_Collection_Item_Music_Text_C_Mail_Content = {Text = "Progress"},
  PanelText_UI_Collection_Item_Music_Text_C_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Item_Music_Toggle_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Collection_Item_Music_Toggle_Text_C_UnLock_Content = {Text = "Unlock"},
  PanelText_UI_Collection_Item_Video_Text_C_UnLock_Content = {Text = "Unlock"},
  PanelText_UI_Collection_Panel_EquipDetail_Text_C_Title_Content = {Text = "A Story..."},
  PanelText_UI_Collection_Panel_Equip_Text_C_None_Content = {
    Text = "No related collections yet"
  },
  PanelText_UI_Collection_Panel_Equip_Text_C_Time_Content = {
    Text = "Nothing here. Go look somewhere else?"
  },
  PanelText_UI_Collection_Panel_Event_Text_C_Placeholder_Content = {
    Text = "Search for a term..."
  },
  PanelText_UI_Collection_Panel_History_Text_C_Label_Content = {Text = "Chronicles"},
  PanelText_UI_Collection_Panel_History_Text_C_Mail_Content = {Text = "Progress"},
  PanelText_UI_Collection_Panel_History_Text_C_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Panel_LoginPV_Text_Title_Content = {
    Text = "Change Login Background"
  },
  PanelText_UI_Collection_Panel_LoginPV_Text_Title_En_Content = {
    Text = "Change Login Background"
  },
  PanelText_UI_Collection_Panel_Main_Text_C_Creation_Content = {Text = "Relic"},
  PanelText_UI_Collection_Panel_Main_Text_C_Event_Content = {Text = "Event"},
  PanelText_UI_Collection_Panel_Main_Text_C_Image_Content = {Text = "Media"},
  PanelText_UI_Collection_Panel_Main_Text_C_Music_Content = {Text = "Phonograph"},
  PanelText_UI_Collection_Panel_Main_Text_C_Progress_Content = {Text = "Progress"},
  PanelText_UI_Collection_Panel_Main_Text_C_Role_Content = {Text = "Characters"},
  PanelText_UI_Collection_Panel_Main_Text_C_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Panel_Main_Text_C_Trinket_Content = {Text = "Covenant"},
  PanelText_UI_Collection_Panel_Main_Text_C_Weapon_Content = {
    Text = "Wheel of Destiny"
  },
  PanelText_UI_Collection_Panel_Main_Text_C_World_Content = {
    Text = "Worldbuilding"
  },
  PanelText_UI_Collection_Panel_Music_Play_Text_C_Mail_Content = {Text = "Progress"},
  PanelText_UI_Collection_Panel_Music_Play_Text_C_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Panel_Organize_Text_C_Mail_Content = {Text = "Progress"},
  PanelText_UI_Collection_Panel_Organize_Text_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Panel_Relic_Text_C_Mail_Content = {Text = "Progress"},
  PanelText_UI_Collection_Panel_Relic_Text_C_Title_Content = {Text = "A Story..."},
  PanelText_UI_Collection_Panel_Relic_Text_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_Display_Content = {
    Text = "Display Awakener"
  },
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_DownLoadCG_Content = {
    Text = "Download Original Image"
  },
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_HideAwaker_Content = {
    Text = "After setting this image as the dormitory background, the visiting character's illustration will be hidden."
  },
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_Hide_Content = {
    Text = "Hidden Awakener"
  },
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_High_Content = {
    Text = "Reset Background"
  },
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_Normal_Content = {
    Text = "Set as Dormitory Background"
  },
  PanelText_UI_Collection_Panel_SetMainBg_Text_C_Save_Content = {Text = "Save"},
  PanelText_UI_Collection_Panel_Unlock_Text_C_Cost_Content = {Text = "Consumes"},
  PanelText_UI_Collection_Panel_World_Text_C_Chronicle_Content = {Text = "Chronicles"},
  PanelText_UI_Collection_Panel_World_Text_C_Comprehensive_Content = {
    Text = "Encyclopedia"
  },
  PanelText_UI_Collection_Panel_World_Text_C_Label_Content = {Text = "World"},
  PanelText_UI_Collection_Panel_World_Text_C_Organize_Content = {Text = "Factions"},
  PanelText_UI_Collection_Panel_World_Text_C_Title_Content = {Text = "Library"},
  PanelText_UI_Collection_Panel_World_Text_Progress_Content = {Text = "Progress"},
  PanelText_UI_Common_Awaker_Favor_Text_C_Sync_Content = {
    Text = "Awakener Affinity Increase"
  },
  PanelText_UI_Common_Btn_Receive_Small_Text_C_Name_Content = {Text = "Claim"},
  PanelText_UI_Common_Btn_Share_Text_C_Across_Content = {
    Text = "First Share Reward"
  },
  PanelText_UI_Common_Btn_Share_Text_C_Share_Content = {
    Text = "First Share Reward"
  },
  PanelText_UI_Common_Btn_Small_240x90_Text_Name_Content = {
    Text = "Activate Talent"
  },
  PanelText_UI_Common_Group_Coin_Text_C_Text_Content = {Text = "Consumes"},
  PanelText_UI_Common_Item_Detail_Text_C_AavtarName_Content = {
    Text = "Avatar and Expression"
  },
  PanelText_UI_Common_Item_Detail_Text_C_Avatar_Info_Content = {
    Text = "Detail Preview"
  },
  PanelText_UI_Common_Item_Detail_Text_C_Close_Content = {Text = "Collapse"},
  PanelText_UI_Common_Item_Detail_Text_C_Equipped_Content = {Text = "Equipped"},
  PanelText_UI_Common_Item_Detail_Text_C_KeeperSkill_Title_Content = {
    Text = "Posse Effect:"
  },
  PanelText_UI_Common_Item_Detail_Text_C_Source_Content = {
    Text = "Where to Get"
  },
  PanelText_UI_Common_Item_Hint_Text_C_Recommend_Content = {Text = "HOT"},
  PanelText_UI_Common_Item_Potency_Text_C_Max_Content = {Text = "Max"},
  PanelText_UI_Common_Item_Tip_Bg_Text_C_Name_Content = {Text = "Sort"},
  PanelText_UI_Common_Item_WuPin_Type1_Text_C_Dispatch_Content = {Text = "On Duty..."},
  PanelText_UI_Common_Item_WuPin_Type1_Text_C_Expired_Content = {Text = "Expired"},
  PanelText_UI_Common_Item_WuPin_Type1_Text_C_NoEquip_Content = {
    Text = "Cannot be equipped"
  },
  PanelText_UI_Common_Item_WuPin_Type1_Text_C_NotActive_Content = {Text = "Inactive"},
  PanelText_UI_Common_Item_WuPin_Type1_Text_C_UpperLimit_Content = {
    Text = "Limit Reached"
  },
  PanelText_UI_Common_Item_WuPin_Type1_Text_C_Using_Content = {Text = "In Use"},
  PanelText_UI_Common_Item_WuPin_Type2_Text_C_Full_Content = {
    Text = "Fully Ascended"
  },
  PanelText_UI_Common_Item_WuPin_Type2_Text_C_Tab_Double_Content = {Text = "Double"},
  PanelText_UI_Common_Item_WuPin_Type2_Text_C_Tab_Nor_Content = {Text = "Extra"},
  PanelText_UI_Common_Melt_Disaster_Text_C_Suppress_Content = {Text = "D-tide"},
  PanelText_UI_Common_Popup_Card_List_Text_C_AbandonCard_Content = {
    Text = "Discard pile"
  },
  PanelText_UI_Common_Popup_Card_List_Text_C_DrawCard_Content = {Text = "Draw pile"},
  PanelText_UI_Common_Popup_Card_List_Text_C_Empty_Content = {
    Text = "The current Draw Pile is empty"
  },
  PanelText_UI_Common_Popup_Card_List_Text_C_NoTask_Content = {
    Text = "No cards available"
  },
  PanelText_UI_Common_Popup_Card_List_Text_C_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Common_Popup_Consume_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Common_Popup_Consume_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_UI_Common_Popup_Consume_Text_C_Title_Content = {
    Text = "Item Use Confirmation"
  },
  PanelText_UI_Common_Popup_Tip_01_Text_C_Screening_Content = {
    Text = "Do not show again today"
  },
  PanelText_UI_Common_Popup_Tip_3_Text_C_Screening_Content = {
    Text = "Do not show again today"
  },
  PanelText_UI_Common_Popup_Tip_3_Text_C_ShopNeed_Content = {
    Text = "Current Voucher Count"
  },
  PanelText_UI_Common_Popup_Tip_3_Text_Screening_Content = {
    Text = "Do not show again today"
  },
  PanelText_UI_Common_Popup_Tip_4_Text_C_Info_Content = {
    Text = "<WhiteQuality:Click to close>"
  },
  PanelText_UI_Common_Popup_Tip_5_Text_C_Info_Content = {
    Text = "Click blank area to close"
  },
  PanelText_UI_Common_Popup_Tip_6_Text_C_Tips_Content = {
    Text = "You cannot change your name again for 72 hours after the modification."
  },
  PanelText_UI_Common_Popup_Tip_6_Text_NameCancel_Content = {Text = "Cancel"},
  PanelText_UI_Common_Popup_Tip_6_Text_NameConfirm_Content = {Text = "Confirm"},
  PanelText_UI_Common_Popup_Tip_6_Text_Placeholder_Content = {
    Text = "Please enter your name"
  },
  PanelText_UI_Common_Popup_Tip_6_Text_Title_Content = {
    Text = "Change Name"
  },
  PanelText_UI_Common_Popup_Tips_6_Text_null_Content = {Text = "(Damaged)"},
  PanelText_UI_Common_Popup_Tips_Confirm_L_Text_C_Material_Content = {Text = "Materials"},
  PanelText_UI_Common_Popup_Tips_Confirm_L_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Common_Popup_Tips_Confirm_L_Text_C_Reward_Content = {Text = "Reward"},
  PanelText_UI_Common_Popup_Tips_Confirm_L_Text_C_Title_Content = {Text = "Preview"},
  PanelText_UI_Common_Popup_Tips_Confirm_S_Text_C_Day_Content = {Text = "d"},
  PanelText_UI_Common_Popup_Tips_Confirm_S_Text_C_Month_Content = {Text = "/"},
  PanelText_UI_Common_Popup_Tips_Small_Text_C_Scroll_Tips_Content = {
    Text = "Swipe up and down to view more rewards"
  },
  PanelText_UI_Community_Acclaim_Tips_Text_C_Name_1_Content = {
    Text = "Proceed to Evaluation"
  },
  PanelText_UI_Community_Steer_Tips_Text_Thanks_Name_Content = {
    Text = "Special Thanks"
  },
  PanelText_UI_Community_ThanksLetter_Text_C_LetterTitle_Content = {
    Text = "Dear Localization Volunteers:"
  },
  PanelText_UI_Community_ThanksLetter_Text_C_Name_Content = {
    Text = "Localization Volunteer"
  },
  PanelText_UI_Community_ThanksLetter_Text_C_Signature_Content = {
    Text = "Mythag University Committee"
  },
  PanelText_UI_Community_ThanksLetter_Text_C_Sincerely_Content = {
    Text = "With Sincere Gratitude"
  },
  PanelText_UI_Community_ThanksLetter_Text_Prefix_Content = {
    Text = [[
With your keen linguistic sense and passion for gaming, you transform rigid machine-translated texts into vivid language. It is precisely this selfless enthusiasm that allows Keepers around the world to enjoy the authentic experience of Morimens. 
Mythag University sincerely thanks the following Keepers for their outstanding contributions:]]
  },
  PanelText_UI_Community_ThanksLetter_Text_Suffix_Content = {
    Text = "May the Silver Key guide us."
  },
  PanelText_UI_Community_ThanksLetter_Text_Title_Content = {
    Text = "Special Thanks"
  },
  PanelText_UI_Copy_Item_StoryChapter_Text_C_Award_Content = {Text = "Archived"},
  PanelText_UI_Copy_Item_StoryChapter_Text_C_Free_Unlock_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Copy_Item_StoryChapter_Text_C_Lock_Content = {
    Text = "Not yet unlocked"
  },
  PanelText_UI_Copy_Story_View_Text_C_ReadTip_Content = {
    Text = "Suggested order is for reading guidance only, not strict chronology."
  },
  PanelText_UI_Course_Panel_Main_Text_C_NoTask_Content = {
    Text = "Currently, there are no tutorials of this type available."
  },
  PanelText_UI_Course_Panel_Main_Text_C_Placeholder_Content = {
    Text = "Searching tutorial..."
  },
  PanelText_UI_Course_Panel_Text_Title_Content = {Text = "Tutorial"},
  PanelText_UI_Dbgcopy_Item_Select_Goods_Text_C_Sellout_Content = {Text = "Sold Out"},
  PanelText_UI_Dbgcopy_Item_Target_Title_Text_C_Achievement_Content = {
    Text = "Stage Achievement"
  },
  PanelText_UI_Dbgcopy_Item_Target_Title_Text_C_Star_Content = {Text = "Rating"},
  PanelText_UI_Dbgcopy_Panel_Action_Text_C_Arousal_Sellout_Content = {Text = "Sold Out"},
  PanelText_UI_Dbgcopy_Panel_Action_Text_C_Curse_Content = {
    Text = "Between Curses"
  },
  PanelText_UI_Dbgcopy_Panel_Action_Text_C_Sell_Sellout_Content = {Text = "Sold Out"},
  PanelText_UI_Dbgcopy_Panel_Action_Text_C_Spend_Content = {Text = "Cost"},
  PanelText_UI_Dbgcopy_Panel_Main_New_Text_C_Count_Content = {Text = "D-Tide"},
  PanelText_UI_Dbgcopy_Panel_RelicPreview_Text_Name_Content = {Text = "Back"},
  PanelText_UI_Dbgcopy_Panel_RelicStrength_Text_C_Title_Content = {
    Text = "Enhance Relic"
  },
  PanelText_UI_Dbgcopy_Panel_Select_Arousal_Text_C_Name_Content = {Text = "Select"},
  PanelText_UI_Dbgcopy_Panel_Select_Extra_Text_C_Name_Content = {Text = "Abandon"},
  PanelText_UI_Dbgcopy_Popup_Event_New_Text_C_Display_Content = {Text = "Show Event"},
  PanelText_UI_Dbgcopy_Popup_Event_New_Text_C_Hide_Content = {Text = "Hide Event"},
  PanelText_UI_Dbgcopy_Popup_LevelChapter_Text_C_Monster_Content = {
    Text = "Stage Relics"
  },
  PanelText_UI_Dbgcopy_Popup_LevelChapter_Text_C_Tip_Content = {
    Text = "*Different chapters bring their own combat mechanics and starting Relics."
  },
  PanelText_UI_Dbgcopy_Popup_LevelChapter_Text_C_Title_Content = {
    Text = "Stage Effect Description"
  },
  PanelText_UI_Dbgcopy_Popup_ReplaceAwaker_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_UI_Dbgcopy_Popup_ReplaceAwaker_Text_C_Replace_Content = {Text = "Replace"},
  PanelText_UI_Dbgcopy_Popup_ReplaceAwaker_Text_C_TeamAwaker_Content = {
    Text = "Current Lineup"
  },
  PanelText_UI_Dbgcopy_Popup_ReplaceAwaker_Text_C_Title_Content = {
    Text = "Switch Characters"
  },
  PanelText_UI_Dbgcopy_Popup_Vibes_Text_C_Suppress_Content = {Text = "Breeding"},
  PanelText_UI_Dbgcopy_Popup_Vibes_Text_C_Title_Content = {
    Text = "D-Tide Effect Description"
  },
  PanelText_UI_Dbgcopy_Popup_ydbj_yty_01_Text_C_Dcks_Content = {
    Text = "Investigation Begins"
  },
  PanelText_UI_Dungeons_BattleTeaching_View_Text_BtnName_Content = {
    Text = "Start Training"
  },
  PanelText_UI_Dungeons_BattleTeaching_View_Text_C_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Dungeons_Item_Common_Element_Text_C_Award_Content = {
    Text = "Reward +100%"
  },
  PanelText_UI_Dungeons_Item_Common_Tier2_Text_C_Current_Content = {Text = "Completed"},
  PanelText_UI_Dungeons_Item_Level_Content_Text_C_Extra_Content = {
    Text = "Extra Clear"
  },
  PanelText_UI_Dungeons_Item_Level_Content_Text_C_Name_Content = {Text = "Reset"},
  PanelText_UI_Dungeons_Item_Level_Content_Text_C_Name_Extra_Content = {Text = "Reset"},
  PanelText_UI_Dungeons_Item_Level_Content_Text_C_Undone_Content = {
    Text = "Incomplete Challenge"
  },
  PanelText_UI_Dungeons_Item_RailWayRank_Text_C_Null_Content = {
    Text = "No lineup available"
  },
  PanelText_UI_Dungeons_Material_View_Text_C_ActivityAchievement_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_All_Content = {Text = "All"},
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Awaker_Content = {Text = "Battle"},
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_BattleStatistics_Content = {Text = "Overview"},
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Damage_Content = {Text = "DMG"},
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Heal_Content = {Text = "Healing"},
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Null_BattleStatistics_Content = {
    Text = "No data available"
  },
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Null_RoundStatistics_Content = {
    Text = "No data available"
  },
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_RoundStatistics_Content = {Text = "By Turn"},
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Shield_Content = {Text = "Shield"},
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Time_BattleStatistics_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Dungeons_Panel_CopyStatistics_Text_C_Time_RoundStatistics_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_C_Experience_Content = {Text = "Dive Level"},
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_C_Highest_Content = {
    Text = "Weekly High Score"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_C_Rank_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_C_Record_Content = {
    Text = "Weekly Performance"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_C_Store_Content = {Text = "Exchange"},
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_C_Title_Content = {
    Text = "Investigation Title"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_Diving_Record_Content = {Text = "Records"},
  PanelText_UI_Dungeons_Panel_Daily_Challenge_New_Text_Main_Title_Content = {
    Text = "Phantasmal Dive"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_C_Highest_Content = {
    Text = "Seasonal Record"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_C_Record2_Content = {
    Text = "Weekly Summary"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_C_Record_Content = {
    Text = "Weekly Summary"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_C_Reward2_Content = {
    Text = "Deep Dive Records"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_C_Store_Content = {
    Text = "Exchange Store"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_C_Title_Content = {
    Text = "Highest Title"
  },
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_C_illustrate_Content = {Text = "Rules"},
  PanelText_UI_Dungeons_Panel_Daily_Challenge_Text_list_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeons_Panel_Daily_Out_Text_C_Appraise_Content = {
    Text = "Points Breakdown"
  },
  PanelText_UI_Dungeons_Panel_Daily_Out_Text_C_Finish_Content = {
    Text = "Investigation Successful"
  },
  PanelText_UI_Dungeons_Panel_Daily_Out_Text_C_High_Content = {
    Text = "New Record!"
  },
  PanelText_UI_Dungeons_Panel_Daily_Out_Text_C_Remind_Content = {
    Text = "Enhance the Awakeners' Ability before challenging strong enemies!"
  },
  PanelText_UI_Dungeons_Panel_Daily_Out_Text_C_Score_Content = {
    Text = "Total Points"
  },
  PanelText_UI_Dungeons_Panel_Daily_Out_Text_C_Synthesis_Content = {Text = "(=sum ×3)"},
  PanelText_UI_Dungeons_Panel_Daily_Text_C_Award_Content = {
    Text = "Completion Rewards"
  },
  PanelText_UI_Dungeons_Panel_Daily_Text_C_Highest_Content = {
    Text = "Highest Record"
  },
  PanelText_UI_Dungeons_Panel_Daily_Text_C_Record_Content = {
    Text = "Today's Achievements"
  },
  PanelText_UI_Dungeons_Panel_Daily_Text_C_Reward_Content = {
    Text = "Weekly Trial"
  },
  PanelText_UI_Dungeons_Panel_Daily_Text_C_Score_Content = {
    Text = "Weekly Cumulative Score"
  },
  PanelText_UI_Dungeons_Panel_Daily_Text_C_Store_Content = {
    Text = "Exchange Store"
  },
  PanelText_UI_Dungeons_Panel_Daily_Text_C_Title_Content = {
    Text = "Investigation Title"
  },
  PanelText_UI_Dungeons_Panel_Material_Text_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_Appraise_Content = {
    Text = "Score Statistics"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_BattleData_Content = {Text = "Overview"},
  PanelText_UI_Dungeons_Panel_Out_Text_C_Finish_Content = {
    Text = "Investigation Complete"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_High_Content = {
    Text = "New Record!"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_Huodewuzi_Content = {
    Text = "Rewards Settlement"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_Integral_Content = {Text = "Evaluation"},
  PanelText_UI_Dungeons_Panel_Out_Text_C_Limit_Content = {
    Text = "※Item limit has been reached"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_Score_Content = {
    Text = "Total Score"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_Statistics_Content = {
    Text = "Battle Statistics"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_Sync_Content = {
    Text = "Awakener Affinity Increase"
  },
  PanelText_UI_Dungeons_Panel_Out_Text_C_Synthesis_Content = {Text = "(=sum ×3)"},
  PanelText_UI_Dungeons_Panel_Out_Text_C_TeamInfo_Content = {
    Text = "Team Information"
  },
  PanelText_UI_Dungeons_Panel_Realms_Text_C_Interval1_Content = {Text = "Interval"},
  PanelText_UI_Dungeons_Panel_Realms_Text_C_Interval2_Content = {
    Text = "Rotation gap"
  },
  PanelText_UI_Dungeons_Panel_Realms_Text_C_Name_1_Content = {
    Text = "Realm of Chaos"
  },
  PanelText_UI_Dungeons_Panel_Realms_Text_C_Name_2_Content = {
    Text = "Bloodmoon Tower"
  },
  PanelText_UI_Dungeons_Panel_Realms_Text_C_Name_3_Content = {
    Text = "Aequor Abyss"
  },
  PanelText_UI_Dungeons_Panel_Realms_Text_C_Name_4_Content = {
    Text = "Ultra Journey"
  },
  PanelText_UI_Dungeons_Panel_Realms_Text_C_NotUnlocked_1_Content = {Text = "Locked"},
  PanelText_UI_Dungeons_Panel_Realms_Text_C_NotUnlocked_2_Content = {Text = "Locked"},
  PanelText_UI_Dungeons_Panel_Realms_Text_C_NotUnlocked_3_Content = {Text = "Locked"},
  PanelText_UI_Dungeons_Panel_Realms_Text_C_NotUnlocked_4_Content = {Text = "Locked"},
  PanelText_UI_Dungeons_Panel_Realms_Text_C_Tips_Content = {
    Text = "Lightless Exchange"
  },
  PanelText_UI_Dungeons_Panel_Realms_Text_C_illustrate_Content = {
    Text = "Gameplay Rules"
  },
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Card2_Content = {
    Text = "(With Orison)"
  },
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Card_Content = {Text = "Cards"},
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_KeyToken_Content = {Text = "Posse"},
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Null_Content = {
    Text = "No Relics Available"
  },
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_PlayerLevel_Content = {
    Text = "Keeper Level"
  },
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Relic_Content = {Text = "Relic"},
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Reset_Content = {Text = "Reset"},
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_TeamData_Content = {Text = "Party Data"},
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_TeamProperty_Content = {
    Text = "Team Attributes"
  },
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Team_Content = {
    Text = "Investigation Team"
  },
  PanelText_UI_Dungeons_Panel_SurveyData_Text_C_Uid_Content = {Text = "Keeper ID:"},
  PanelText_UI_Dungeons_Panel_SurveyData_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Dungeons_Panel_Trinket_Text_C_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Dungeons_Panel_Try_Text_C_Activity_Awaker_Content = {
    Text = "Limited: Faded Legacy"
  },
  PanelText_UI_Dungeons_Panel_Try_Text_C_Award_Content = {Text = "Reward"},
  PanelText_UI_Dungeons_Panel_Try_Text_C_Emoji_Content = {Text = "Expression"},
  PanelText_UI_Dungeons_Panel_Try_Text_C_Introduce_Content = {
    Text = "Ability Introduction"
  },
  PanelText_UI_Dungeons_Panel_Try_Text_C_Name_1_Content = {Text = "Claim"},
  PanelText_UI_Dungeons_Panel_Try_Text_C_Name_Content = {
    Text = "Start Trial"
  },
  PanelText_UI_Dungeons_Panel_Try_Text_C_Normal_Awaker_Content = {Text = "Permanent"},
  PanelText_UI_Dungeons_Panel_Try_Text_C_Star_Awaker_Content = {
    Text = "Limited: Astral Reign"
  },
  PanelText_UI_Dungeons_Panel_Try_Text_C_Undone_Content = {Text = "Incomplete"},
  PanelText_UI_Dungeons_Panel_Try_Text_C_Wake_Content = {Text = "Awaken"},
  PanelText_UI_Dungeons_Panel_WeekBOSS_Optimized_Text_C_ActivityAchievement_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Dungeons_Panel_WeekBOSS_Optimized_Text_C_AwardText_Content = {
    Text = "Weekly Rewards"
  },
  PanelText_UI_Dungeons_Panel_WeekBOSS_Optimized_Text_C_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Dungeons_Panel_WeekBOSS_Text_C_AwardText_Content = {
    Text = "Weekly Rewards"
  },
  PanelText_UI_Dungeons_Panel_WeekBOSS_Text_C_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Dungeons_Popup_AbyssRank_Text_C_Info_Content = {
    Text = "Keeper Information"
  },
  PanelText_UI_Dungeons_Popup_AbyssRank_Text_C_More_Content = {Text = "Details"},
  PanelText_UI_Dungeons_Popup_AbyssRank_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Dungeons_Popup_AbyssRank_Text_C_Rank_Content = {Text = "Rank"},
  PanelText_UI_Dungeons_Popup_AbyssRank_Text_C_Replay_Content = {Text = "Replay"},
  PanelText_UI_Dungeons_Popup_AbyssRank_Text_C_Value_Content = {
    Text = "Training Points"
  },
  PanelText_UI_Dungeons_Popup_AbyssRank_Text_Title_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeons_Popup_Award_Optimized_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Dungeons_Popup_Award_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Dungeons_Popup_BattleStatistics_Text_C_Awaker_Content = {Text = "Battle"},
  PanelText_UI_Dungeons_Popup_BattleStatistics_Text_C_Damage_Content = {Text = "DMG Dealt"},
  PanelText_UI_Dungeons_Popup_BattleStatistics_Text_C_Heal_Content = {
    Text = "Healing Done"
  },
  PanelText_UI_Dungeons_Popup_BattleStatistics_Text_C_Shield_Content = {
    Text = "Shield Generated"
  },
  PanelText_UI_Dungeons_Popup_BattleStatistics_Text_C_Title_Content = {
    Text = "Battle Statistics"
  },
  PanelText_UI_Dungeons_Popup_Clearance_Text_C_FilterAwaker_Content = {Text = "Filter"},
  PanelText_UI_Dungeons_Popup_Clearance_Text_C_No_Records_Content = {
    Text = "No Record Yet"
  },
  PanelText_UI_Dungeons_Popup_Clearance_Text_C_Refresh_Content = {Text = "Refresh"},
  PanelText_UI_Dungeons_Popup_Clearance_Text_C_Title_Content = {
    Text = "Clear Record"
  },
  PanelText_UI_Dungeons_Popup_Clearance_Text_Fliter_Type_Content = {
    Text = "Total Turns"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Award_Content = {
    Text = "High Score Rankings"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Big_Content = {Text = "Rank Range"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Current_Content = {
    Text = "Current Rewards"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Empty_Content = {
    Text = "No records available"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_FilterAwaker_Content = {Text = "Filter"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Grade_Content = {
    Text = "Investigation Title"
  },
  ["PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Haven't_Content"] = {
    Text = "No rewards available"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Highest_Content = {
    Text = "Highest Record"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Information_1_Content = {
    Text = "Keeper Information"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Information_2_Content = {
    Text = "Ranking Title"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Information_3_Content = {
    Text = "Highest Record"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Information_4_Content = {Text = "Rank"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Information_5_Content = {Text = "Lineup"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Information_6_Content = {Text = "Record"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Me_Content = {Text = "My Ranking"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Prompt_Content = {
    Text = "Rewards are determined based on the investigation titles and will be sent to your mail at 9:00 AM every Monday"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_RankAward1_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_RankAward2_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Ranking_Content = {Text = "Rank"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Reward_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Season1_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Season2_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Small_Content = {
    Text = "Less than or equal to"
  },
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_SurveyRanking1_Content = {Text = "Ranking"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_SurveyRanking2_Content = {Text = "Ranking"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_Title_Content = {Text = "Ranking"},
  PanelText_UI_Dungeons_Popup_List_Tip_Text_C_illustrate_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Big_Content = {Text = "Rank Range"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_DivingTitle_Content = {
    Text = "Ranking Title"
  },
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Exp_Content = {
    Text = "Highest Pts"
  },
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Grade_Content = {
    Text = "Ranking Title"
  },
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Info_Content = {
    Text = "Keeper Information"
  },
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_RankAward_Nor_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_RankAward_Sel_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Rank_Content = {Text = "Rank"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Rank_Nor_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Rank_Sel_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Replay_Content = {Text = "Record"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Reward_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Round_Content = {Text = "Turns"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Team_Content = {Text = "Lineup"},
  PanelText_UI_Dungeons_Popup_RailWayRank_Text_C_Title_Content = {Text = "VIP List"},
  PanelText_UI_Dungeons_Popup_Reward_Tip_Text_C_Big_Content = {
    Text = "Greater than"
  },
  PanelText_UI_Dungeons_Popup_Reward_Tip_Text_C_Reward_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_Reward_Tip_Text_C_Small_Content = {
    Text = "Less than or equal to"
  },
  PanelText_UI_Dungeons_Popup_Reward_Tip_Text_C_Title_Content = {Text = "Rewards"},
  PanelText_UI_Dungeons_Popup_Rules_Tip_Text_C_Title_Content = {
    Text = "Event Rules"
  },
  PanelText_UI_Dungeons_RogueStage_View_Text_C_Award_Content = {
    Text = "Rewards Preview"
  },
  PanelText_UI_Dungeons_RogueStage_View_Text_C_Resonance_2_Content = {Text = "Resonance"},
  PanelText_UI_Dungeons_RogueStage_View_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Dungeons_RogueStage_View_Text_C_Title_Content = {
    Text = "Spire Roguelite"
  },
  PanelText_UI_Dungeons_Rogue_View_Text_C_Award_Content = {
    Text = "Rewards Preview"
  },
  PanelText_UI_Dungeons_Rogue_View_Text_C_Challenge_Content = {
    Text = "Standard Mode"
  },
  PanelText_UI_Dungeons_Rogue_View_Text_C_Daily_Content = {
    Text = "Daily Challenge"
  },
  PanelText_UI_Dungeons_Rogue_View_Text_C_Name_Content = {
    Text = "Spire Roguelite"
  },
  PanelText_UI_Dungeons_Rogue_View_Text_C_NoTask_Content = {Text = "Vacant"},
  PanelText_UI_Dungeons_Tip_AwakerFavor_Text_C_EnergyCost_Content = {
    Text = "Arithmetica"
  },
  PanelText_UI_Dungeons_Tip_AwakerFavor_Text_C_Extra_Content = {
    Text = "Extra Bonus"
  },
  PanelText_UI_Dungeons_Tip_AwakerFavor_Text_C_KeeperSkill_Content = {Text = "Keyflare"},
  PanelText_UI_Dungeons_Tip_AwakerFavor_Text_C_Manual_Content = {Text = "Manual"},
  PanelText_UI_Dungeons_Tip_AwakerFavor_Text_C_UltiEnergy_Content = {Text = "Aliemus"},
  PanelText_UI_Dungeons_Tip_AwakerFavor_Text_C_UseCard_Content = {Text = "Play card"},
  PanelText_UI_Dungeons_Trinket_View_Text_C_ActivityAchievement_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Dungeous_Item_Popup_Award2_Text_C_Lost_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Dungeous_Item_Popup_Award2_Text_C_Title2_Content = {
    Text = "Total Score"
  },
  PanelText_UI_Dungeous_Item_Popup_Award2_Text_C_Title_Content = {
    Text = "Total Stars"
  },
  PanelText_UI_Dungeous_Item_Popup_Award_Text_C_Lost_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Dungeous_Item_Popup_Award_Text_C_Title_Content = {
    Text = "Total Stars Reach:"
  },
  PanelText_UI_Dungeous_Item_Ranking_Text_C_Null_Content = {
    Text = "No lineup available"
  },
  PanelText_UI_Dungeous_Item_Ranking_Text_C_Uid_Title_Content = {Text = "UID:"},
  PanelText_UI_Dungeous_Item_Ranking_Text_C_Underway_Content = {
    Text = "Battle data transmission in progress"
  },
  PanelText_UI_Dungeous_Item_Reward_Text_C_Present_Content = {Text = "Current"},
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_Additional_Content = {
    Text = "Reward +50% Today"
  },
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_AwardNum_Content = {
    Text = "2× Reward Chances Today"
  },
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_AwardText_Content = {
    Text = "Weekly Rewards"
  },
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_Boundaries_Tips_Content = {
    Text = "Recommended Realm"
  },
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_Creation_Content = {
    Text = "Stage Relics"
  },
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_Name_Content = {Text = "Reenact"},
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_Recommend_Content = {
    Text = "Recommended Realm"
  },
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_School_1_Content = {Text = "Chaos"},
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_School_2_Content = {Text = "Ultra"},
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_School_3_Content = {Text = "Caro"},
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_School_4_Content = {Text = "Aequor"},
  PanelText_UI_Dungeous_Panel_BossSel1_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_BossSel_Text_C_AwardText_Content = {
    Text = "Weekly Rewards"
  },
  PanelText_UI_Dungeous_Panel_BossSel_Text_C_Award_Content = {
    Text = "Enemy Details"
  },
  PanelText_UI_Dungeous_Panel_BossSel_Text_C_Boundaries_Tips_Content = {
    Text = "Recommended Realm"
  },
  PanelText_UI_Dungeous_Panel_BossSel_Text_C_Reward_Content = {
    Text = "Completion Rewards"
  },
  PanelText_UI_Dungeous_Panel_BossSel_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_BossSel_Text_Title_Content = {
    Text = "Arcane Dominion"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_C_Award_Content = {
    Text = "Boss Preview"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_C_Boundaries_Tips_Content = {Text = "Rec. Realm"},
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_C_Creation_Content = {
    Text = "Stage Relic"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_C_Difficulty_Content = {
    Text = "Descent Depth"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_C_Reward_Content = {
    Text = "Rewards Preview"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_C_Score_Content = {
    Text = "Seasonal High"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_RewardShow_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Optimized_Text_StrongAwakers_Content = {
    Text = "Enhanced Awakener"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Text_C_Creation_Content = {
    Text = "Stage Relic"
  },
  PanelText_UI_Dungeous_Panel_Difficulty_Text_C_Suppress_Content = {Text = "D-Tide"},
  PanelText_UI_Dungeous_Panel_Lake_City_Text_C_Creation_Content = {
    Text = "Stagel Relic"
  },
  PanelText_UI_Dungeous_Panel_Lake_City_Text_C_Remuneration_Content = {
    Text = "Character Rewards"
  },
  PanelText_UI_Dungeous_Panel_Lake_City_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Dungeous_Panel_Lake_City_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Optimized_Text_C_Hint_Content = {
    Text = "*In Regional D-Effect Zones, deployed Awakeners and Wheels of Destiny cannot be deployed in other zones."
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Optimized_Text_C_Rank_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Optimized_Text_C_Title_Content = {
    Text = "Regional D-Effect Zone"
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Optimized_Text_C_Train_Content = {
    Text = "Current Points"
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Optimized_Text_C_Weekly_Content = {
    Text = "Special Training Rewards"
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Text_C_Hint_Content = {
    Text = "*After clearing, the Awakeners, Wheels of Destiny, and Posse used in this battle cannot be deployed in other Regional D-Effect Zones."
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Text_C_Title_Content = {
    Text = "Regional D-Effect Zone"
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Text_C_Train_Content = {
    Text = "Current Points"
  },
  PanelText_UI_Dungeous_Panel_Melt_Erosion_Text_C_Weekly_Content = {
    Text = "Training Rewards"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Creation_Content = {
    Text = "Stage Relic"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Extra_Content = {
    Text = "Extra Clear Lineup"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Integral_Content = {
    Text = "Clear Score"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_LineUp_Content = {
    Text = "Clear Lineup"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Monster_Content = {
    Text = "Monster Preview"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Name_Content = {Text = "Reset"},
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Name_Extra_Content = {Text = "Reset"},
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_None_Content = {
    Text = "No Lineup Available"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Replay_Content = {Text = "Record"},
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_Molten_Optimized_Text_Extra_None_Content = {
    Text = "Challenge not yet completed"
  },
  PanelText_UI_Dungeous_Panel_Molten_Text_C_Creation_Content = {
    Text = "Stage Relics"
  },
  PanelText_UI_Dungeous_Panel_Molten_Text_C_Integral_Content = {
    Text = "Training Points"
  },
  PanelText_UI_Dungeous_Panel_Molten_Text_C_LineUp_Content = {Text = "Lineup"},
  PanelText_UI_Dungeous_Panel_Molten_Text_C_Monster_Content = {
    Text = "Monster Preview"
  },
  PanelText_UI_Dungeous_Panel_Molten_Text_C_Name_Content = {Text = "Reset"},
  PanelText_UI_Dungeous_Panel_Molten_Text_C_None_Content = {
    Text = "Challenge not yet completed"
  },
  PanelText_UI_Dungeous_Panel_Molten_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_Molten_Text_C_Suppress_Content = {Text = "D-Tide"},
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Challenge_Content = {Text = "Challenge"},
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Choose_Content = {
    Text = "Select Difficulty"
  },
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Experience_Content = {
    Text = "Train Upgrades"
  },
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Hard_Content = {Text = "Hard"},
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Madness_Content = {Text = "Madness"},
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Normal_Content = {Text = "Normal"},
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Rank_Content = {Text = "VIP List"},
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Record_Content = {
    Text = "Wayfare Frames"
  },
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Resonance_Content = {
    Text = "Little N's Help"
  },
  PanelText_UI_Dungeous_Panel_RailWay_Text_C_Title_Content = {
    Text = "Hectic Skybound Rail"
  },
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Award2_Content = {
    Text = "Domain Laws"
  },
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Award_Content = {
    Text = "Enemy Details"
  },
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Boundaries_Tips_Content = {Text = "Rec. Realm"},
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Cost_Tips_Content = {
    Text = "Requirements:"
  },
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Creation_Content = {
    Text = "Stage Relic"
  },
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Replay_Content = {Text = "Record"},
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Reward_Content = {
    Text = "Rewards Preview"
  },
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_Realms_New_Text_C_Tishi_Content = {
    Text = "Rewards Preview"
  },
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Award2_Content = {
    Text = "Domain Laws"
  },
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Award_Content = {
    Text = "Enemy Details"
  },
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Boundaries_Tips_Content = {Text = "Rec. Realm"},
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Cost_Tips_Content = {
    Text = "Requirements:"
  },
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Creation_Content = {
    Text = "Stage Relics"
  },
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Reward_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_Realms_Text_C_Tishi_Content = {
    Text = "Star Progress"
  },
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Additional_Content = {
    Text = "Reward +50% Today"
  },
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_AwardNum_Content = {
    Text = "2× Reward Chances"
  },
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Boundaries_Tips_Content = {
    Text = "Recommended Realm"
  },
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Cw_Content = {Text = "Ultra"},
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Hd_Content = {Text = "Chaos"},
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Recommend_Content = {
    Text = "Recommended Realm"
  },
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Sh_Content = {Text = "Aequor"},
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Special_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_C_Xr_Content = {Text = "Caro"},
  PanelText_UI_Dungeous_Panel_SecEntrance_Text_Name_Content = {Text = "Reenact"},
  PanelText_UI_Dungeous_Popup_Awaker_List_Text_C_Hint_1_Content = {
    Text = "No Wheel of Destiny selected"
  },
  PanelText_UI_Dungeous_Popup_Awaker_List_Text_C_Hint_2_Content = {
    Text = "Click the left side to make a selection"
  },
  PanelText_UI_Dungeous_Popup_Awaker_List_Text_C_Hint_Content = {
    Text = "*An Awakener can equip only one SSR Wheel of Destiny."
  },
  PanelText_UI_Dungeous_Popup_Awaker_List_Text_C_Title_Content = {
    Text = "WoD Selection"
  },
  PanelText_UI_Dungeous_Popup_FailTip_Text_C_Title_Content = {
    Text = "Power-up Tips"
  },
  PanelText_UI_Dungeous_Popup_Prop_Award_Text_C_Title_Content = {
    Text = "Star Reward Preview"
  },
  PanelText_UI_Dungeous_Popup_Prop_Award_Text_C_Title_En_Content = {Text = "Drops"},
  PanelText_UI_Dungeous_Popup_Score_Tip_Text_C_Title_Content = {
    Text = "Points Breakdown"
  },
  PanelText_UI_Dungeous_Popup_Score_Tip_Text_C_Total_Content = {
    Text = "Current Total Points"
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip2_Text_C_Coin_Content = {
    Text = "Insufficient Currency"
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip2_Text_C_Explain_Content = {Text = "Rules"},
  PanelText_UI_Dungeous_Popup_Swept_Tip2_Text_C_RewardPreview_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip2_Text_C_Swept_Content = {
    Text = "Number of Reenacts"
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip2_Text_C_Text_Content = {Text = "Consumes"},
  PanelText_UI_Dungeous_Popup_Swept_Tip2_Text_C_Tips_Content = {
    Text = "Unlock upon achieving a three-star investigation rating."
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip2_Text_Title_Content = {
    Text = "Reenact Now?"
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip_Text_C_Deplete_Content = {
    Text = "Reenact Cost"
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip_Text_C_Swept_Content = {
    Text = "Reenact Times"
  },
  PanelText_UI_Dungeous_Popup_Swept_Tip_Text_C_Team_Content = {Text = "Team"},
  PanelText_UI_Dungeous_Popup_Swept_Tip_Text_C_Title_Content = {
    Text = "Confirm Reenact"
  },
  PanelText_UI_Dungeous_Popup_Team_Tip_Text_C_Difficulty_Content = {Text = "Reset"},
  PanelText_UI_Dungeous_Popup_Team_Tip_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Dungeous_Popup_Team_Tip_Text_C_Prohibit_Content = {
    Text = "Posse disabled for this Stage"
  },
  PanelText_UI_Dungeous_Popup_Team_Tip_Text_C_Skill_Content = {
    Text = "Keeper Skills"
  },
  PanelText_UI_Dungeous_Popup_Team_Tip_Text_C_Title_Content = {
    Text = "Challenge Team"
  },
  PanelText_UI_Dungeous_Popup_Tips_New_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Dungeous_Popup_Tips_Text_C_Activity2_Content = {
    Text = "First Clear Reward"
  },
  PanelText_UI_Dungeous_Popup_Tips_Text_C_Activity_Content = {
    Text = "First Clear Reward"
  },
  PanelText_UI_Dungeous_Popup_Tips_Text_C_Game2_Content = {
    Text = "Cumulative Rewards"
  },
  PanelText_UI_Dungeous_Popup_Tips_Text_C_Game_Content = {
    Text = "Cumulative Rewards"
  },
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Again_Content = {
    Text = "Reset Challenge"
  },
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Boundaries_Tips_Content = {
    Text = "Recommended Realm"
  },
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Creation_Content = {
    Text = "Stage Relics"
  },
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Cw_Content = {Text = "Ultra"},
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Hd_Content = {Text = "Chaos"},
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Monster_Content = {
    Text = "Monster Preview"
  },
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Sh_Content = {Text = "Aequor"},
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Title_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_Xr_Content = {Text = "Caro"},
  PanelText_UI_Dungeous_Weekly_Difficulty_Text_C_illustrate_Content = {Text = "Rules"},
  PanelText_UI_Dungeous_Weekly_Main_Text_C_Again_Content = {
    Text = "Reset Challenge"
  },
  PanelText_UI_Dungeous_Weekly_Main_Text_C_Tips_Content = {
    Text = "Reward Details"
  },
  PanelText_UI_Dungeous_Weekly_Main_Text_C_Title_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Dungeous_Weekly_Main_Text_C_illustrate_Content = {Text = "Rules"},
  PanelText_UI_Dungeous_illustrate_Tip_Text_C_Title_Content = {
    Text = "Lightless Realm"
  },
  PanelText_UI_Event_Activity_Castor_Text_C_Finish_Content = {
    Text = "The event has ended, and acquisition has stopped."
  },
  PanelText_UI_Event_Activity_Castor_Text_C_Full_Content = {
    Text = "Storage limit has been reached."
  },
  PanelText_UI_Event_Activity_Castor_Text_C_Reward_Content = {
    Text = "Obtain Inventory"
  },
  PanelText_UI_Event_Activity_Castor_Text_C_SpeedTitle_Content = {
    Text = "Storage Speed:"
  },
  PanelText_UI_Event_Activity_Castor_Text_None_Content = {
    Text = "Nothing Yet..."
  },
  PanelText_UI_Event_Activity_DonationBox_Text_C_Finish_Content = {
    Text = "The event has ended."
  },
  PanelText_UI_Event_Activity_DonationBox_Text_C_Full_Content = {
    Text = "Inventory has reached the limit"
  },
  PanelText_UI_Event_Activity_DonationBox_Text_C_None_Content = {
    Text = "No donated inventory"
  },
  PanelText_UI_Event_Activity_DonationBox_Text_C_Reward_Content = {Text = "Inventory"},
  PanelText_UI_Event_Activity_DonationBox_Text_C_SpeedTitle_Content = {
    Text = "Acquisition Speed:"
  },
  PanelText_UI_Event_Activity_Horla_Text_C_Finish_Content = {
    Text = "The event has ended."
  },
  PanelText_UI_Event_Activity_Horla_Text_C_Full_Content = {
    Text = "Storage limit has been reached."
  },
  PanelText_UI_Event_Activity_Horla_Text_C_Reward_Content = {
    Text = "Store Information"
  },
  PanelText_UI_Event_Activity_Horla_Text_C_SpeedTitle_Content = {
    Text = "Storage Speed"
  },
  PanelText_UI_Event_Activity_KathiguRa_Text_C_Finish_Content = {
    Text = "The event has ended, and acquisition has stopped."
  },
  PanelText_UI_Event_Activity_KathiguRa_Text_C_Full_Content = {
    Text = "Storage Limit Reached"
  },
  PanelText_UI_Event_Activity_KathiguRa_Text_C_Reward_Content = {
    Text = "Obtain Items"
  },
  PanelText_UI_Event_Activity_KathiguRa_Text_C_SpeedTitle_Content = {
    Text = "Storage Speed:"
  },
  PanelText_UI_Event_Activity_KathiguRa_Text_None_Content = {Text = "No Item"},
  PanelText_UI_Event_Activity_Main_Text_C_Label_Content = {
    Text = "Mrs. Gray's Shop"
  },
  PanelText_UI_Event_Activity_Main_Text_C_Mail_Content = {Text = "Rules"},
  PanelText_UI_Event_Activity_Main_Text_C_Normal_1_Content = {Text = "I"},
  PanelText_UI_Event_Activity_Main_Text_C_Normal_2_Content = {Text = "II"},
  PanelText_UI_Event_Activity_Main_Text_C_Normal_3_Content = {Text = "III"},
  PanelText_UI_Event_Activity_Main_Text_C_Select_1_Content = {Text = "I"},
  PanelText_UI_Event_Activity_Main_Text_C_Select_2_Content = {Text = "II"},
  PanelText_UI_Event_Activity_Main_Text_C_Select_3_Content = {Text = "III"},
  PanelText_UI_Event_Activity_Main_Text_C_Title_Content = {
    Text = "Church Charity Bazaar"
  },
  PanelText_UI_Event_Activity_Miriam_Text_C_Finish_Content = {
    Text = "Event has ended. Refill has stopped."
  },
  PanelText_UI_Event_Activity_Miriam_Text_C_Full_Content = {
    Text = "Storage Limit Reached"
  },
  PanelText_UI_Event_Activity_Miriam_Text_C_Reward_Content = {Text = "Supplies"},
  PanelText_UI_Event_Activity_Miriam_Text_C_SpeedTitle_Content = {
    Text = "Acquisition Speed"
  },
  PanelText_UI_Event_Activity_Miriam_Text_None_Content = {
    Text = "No inventory"
  },
  PanelText_UI_Event_Activity_OriMurphy_Text_C_Finish_Content = {
    Text = "The event has ended, and acquisition has stopped."
  },
  PanelText_UI_Event_Activity_OriMurphy_Text_C_Full_Content = {
    Text = "Storage Limit Reached"
  },
  PanelText_UI_Event_Activity_OriMurphy_Text_C_Reward_Content = {
    Text = "Obtain Items"
  },
  PanelText_UI_Event_Activity_OriMurphy_Text_C_SpeedTitle_Content = {
    Text = "Storage Speed:"
  },
  PanelText_UI_Event_Activity_OriMurphy_Text_None_Content = {Text = "No Item"},
  PanelText_UI_Event_Activity_Pollux_Text_None_Content = {Text = "No Item"},
  PanelText_UI_Event_Activity_RewardList_Text_C_Reward_Content = {
    Text = "Charity Sale List"
  },
  PanelText_UI_Event_Activity_RewardPool_Text_C_Name_Content = {Text = "Reset"},
  PanelText_UI_Event_Activity_RewardPool_Text_Name_Content = {
    Text = "Exchange All"
  },
  PanelText_UI_Event_Activity_RewardResult_Text_C_Close_Content = {
    Text = "Click anywhere to close"
  },
  PanelText_UI_Event_Activity_RewardResult_Text_C_CoreReward_Content = {
    Text = "Special Seed"
  },
  PanelText_UI_Event_Activity_RewardResult_Text_C_Result_Content = {
    Text = "Exchange Results"
  },
  PanelText_UI_Event_Activity_SevenDaySign_Text_C_Desc_Content = {
    Text = "Has the Master come to help Karen clean today too? That's great; this is a small token of Karen's appreciation, please make sure to accept it!"
  },
  PanelText_UI_Event_Activity_SevenDaySign_Text_Receive_1_Content = {Text = "Claimed"},
  PanelText_UI_Event_Activity_SevenDaySign_Text_Receive_2_Content = {Text = "Claimed"},
  PanelText_UI_Event_Activity_SevenDaySign_Text_Receive_3_Content = {Text = "Claimed"},
  PanelText_UI_Event_Activity_SevenDaySign_Text_Receive_4_Content = {Text = "Claimed"},
  PanelText_UI_Event_Activity_SevenDaySign_Text_Receive_5_Content = {Text = "Claimed"},
  PanelText_UI_Event_Activity_SevenDaySign_Text_Receive_6_Content = {Text = "Claimed"},
  PanelText_UI_Event_Activity_SevenDaySign_Text_Receive_7_Content = {Text = "Claimed"},
  PanelText_UI_Event_Activity_SevenDaySign_Text_Title_Content = {
    Text = "Karen's Gratitude"
  },
  PanelText_UI_Event_Activity_SummerSign_Text_C_Desc_Content = {
    Text = "Sunshine, sand, waves, and countless smiling faces—in this summer dream, let's have an energetic volleyball match!"
  },
  PanelText_UI_Event_Activity_SummerSign_Text_Title_Content = {
    Text = "Summer Fiesta"
  },
  PanelText_UI_Event_Activity_SummonBenefits_Text_C_Summon_Content = {
    Text = "Free Awaken"
  },
  PanelText_UI_Event_Activity_Thing_Item_Text_C_Sold_Out_Content = {Text = "Sold Out"},
  PanelText_UI_Event_Activity_Thing_Item_Text_C_Special_Tips_Content = {Text = "Special"},
  PanelText_UI_Event_Activity_TwentyFour_Text_C_Finish_Content = {
    Text = "The event has ended, and storage has been stopped."
  },
  PanelText_UI_Event_Activity_TwentyFour_Text_C_Full_Content = {
    Text = "Storage capacity has reached the limit"
  },
  PanelText_UI_Event_Activity_TwentyFour_Text_C_Reward_Content = {
    Text = "Stored Matter"
  },
  PanelText_UI_Event_Activity_TwentyFour_Text_C_SpeedTitle_Content = {
    Text = "Storage Speed:"
  },
  PanelText_UI_Event_Activity_TwentyFour_Text_C_Title_Content = {
    Text = "Manic Accumulation"
  },
  PanelText_UI_Event_Activity_TwentyFour_Text_Name_Content = {Text = "Claim"},
  PanelText_UI_Event_Activity_TwentyFour_Text_None_Content = {
    Text = "No inventory"
  },
  PanelText_UI_Event_Activity_XiaomaoGift_Text_C_Receive_Content = {
    Text = "Remaining Awakenings:"
  },
  PanelText_UI_Event_Challenge_Item_Text_C_Finish_Content = {Text = "Last Time"},
  PanelText_UI_Event_Item_Return_Text_C_Close_Content = {
    Text = "Click anywhere to close"
  },
  PanelText_UI_Event_Item_Return_Text_C_Finish_Content = {Text = "Claimed"},
  PanelText_UI_Event_Item_SignDay_Text_C_CanReceive_Content = {Text = "Claimable:"},
  PanelText_UI_Event_Item_SignDay_Text_C_Got_Content = {Text = "Claimed"},
  PanelText_UI_Event_Item_SignDay_Text_C_Nor_Content = {
    Text = "Rewards Ready"
  },
  PanelText_UI_Event_Item_WeaponOptional_Text_C_Got_Content = {
    Text = "Already Owned"
  },
  PanelText_UI_Event_Panel_Activity_Fundraising_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Event_Panel_Activity_Fundraising_Text_C_Title_Content = {
    Text = "Church Area"
  },
  PanelText_UI_Event_Panel_Activity_TwentyFour_Text_C_Introduction_Content = {
    Text = "Shh... take a deep breath, relax your tentacles. Now you will delve into the depths of \"24\"'s consciousness, exploring hidden memories..."
  },
  PanelText_UI_Event_Panel_Activity_TwentyFour_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Event_Panel_Activity_TwentyFour_Text_C_Title_Content = {
    Text = "Memory Code"
  },
  PanelText_UI_Event_Panel_ChallengeDifficulty_Text_C_Awaker_Content = {
    Text = "Trial Awakener"
  },
  PanelText_UI_Event_Panel_ChallengeDifficulty_Text_C_FristCreation_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Event_Panel_ChallengeDifficulty_Text_C_Leader_Content = {
    Text = "Boss Preview"
  },
  PanelText_UI_Event_Panel_ChallengeDifficulty_Text_C_Record_Content = {Text = "Challenge"},
  PanelText_UI_Event_Panel_ChallengeDifficulty_Text_C_StarTitle_Content = {Text = "Star Goals"},
  PanelText_UI_Event_Panel_ChallengeDifficulty_Text_C_Title_Content = {
    Text = "Plotted Mystery"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_Creation_Content = {
    Text = "Stage Relics"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_Double_Content = {
    Text = "Double Challenge"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_FristCreation_Content = {
    Text = "Initial Relic"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_Leader_Content = {
    Text = "Boss Preview<WordSize20:>"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_MeltErosion_Content = {Text = "D-Tide"},
  PanelText_UI_Event_Panel_Challenge_Text_C_Monster_Content = {
    Text = "Monster Preview"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_Record_Content = {Text = "Challenge"},
  PanelText_UI_Event_Panel_Challenge_Text_C_RemainNum_Content = {
    Text = "Number of Remaining Clones"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_Replay_Content = {Text = "Record"},
  PanelText_UI_Event_Panel_Challenge_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Event_Panel_Challenge_Text_C_RewardTitle_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Event_Panel_Challenge_Text_C_SweepName_Content = {Text = "Sweep"},
  PanelText_UI_Event_Panel_Challenge_Text_C_Title_Content = {
    Text = "Plotted Mystery"
  },
  PanelText_UI_Event_Panel_Challenge_Text_Career_Content = {
    Text = "Requirement: Realm 1"
  },
  PanelText_UI_Event_RewardGroup_Text_C_Reset_Content = {
    Text = "Remaining Resets:"
  },
  PanelText_UI_Event_Reward_Item_Text_C_Not_Content = {Text = "Incomplete"},
  PanelText_UI_Event_Reward_Item_Text_C_Receive_Content = {Text = "Claim"},
  PanelText_UI_Event_Task_Item_Text_C_Not_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Event_Task_Item_Text_C_Receive_Content = {Text = "Claim"},
  PanelText_UI_Event_WeaponOptional_Text_C_Receive_Content = {
    Text = "Remaining Awakenings:"
  },
  PanelText_UI_Events_2ndAnnivWB_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Come! With madness, carnage, and agony as the notes, let us hymn this requiem shimmering amongst the stars!
That Blind Idiot God, the Supreme Being of Folly and Chaos, must not awaken, neither now nor in the future.]]
  },
  PanelText_UI_Events_2ndAnnivWB_StageGroup_Text_C_Title_Content = {
    Text = "Stellar Requiem"
  },
  PanelText_UI_Events_Activity_StageGroup_Text_C_Introduction_Content = {
    Text = [[
That which Our Lord promised is nigh.
The waiting of millennia shall meet a joyous finality, and the promised land shall rise from the sea.
He shall be reborn from the Divine Mother's chest; when the seventh day concludes, a new world shall descend.]]
  },
  PanelText_UI_Events_Activity_StageGroup_Text_C_Title_Content = {Text = "Seven Days"},
  PanelText_UI_Events_Item_Castor_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Castor_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Clementine_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Clementine_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Daffodil_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Daffodil_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Doresain_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Doresain_Text_C_LockTips_Content = {
    Text = "Unlock Tips"
  },
  PanelText_UI_Events_Item_GiftBag_Text_C_Free_Content = {Text = "Free"},
  PanelText_UI_Events_Item_GiftBag_Text_C_High_Content = {Text = "Free"},
  PanelText_UI_Events_Item_GiftBag_Text_C_Noraml_Content = {Text = "Claim"},
  PanelText_UI_Events_Item_GiftBag_Text_C_Soldout_Content = {Text = "Claimed"},
  PanelText_UI_Events_Item_Hamline_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Hamline_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_KathiguRa_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_KathiguRa_Text_C_LockTips_Content = {
    Text = "Unlock Notification"
  },
  PanelText_UI_Events_Item_Lake_City_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Lake_City_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Miriam_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Miriam_Text_C_LockTips_Content = {
    Text = "How to Unlock"
  },
  PanelText_UI_Events_Item_Murphy_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Murphy_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_OceanBoss_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_OceanBoss_Text_C_Killed_Content = {Text = "Killed!"},
  PanelText_UI_Events_Item_OceanBoss_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_OceanBoss_Text_C_Progress_Content = {
    Text = "Challengeable"
  },
  PanelText_UI_Events_Item_Optional_Text_C_Full_Content = {
    Text = "Fully Ascended"
  },
  PanelText_UI_Events_Item_Optional_Text_C_Have_Content = {
    Text = "Already Owned"
  },
  PanelText_UI_Events_Item_OriDoll_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_OriDoll_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_OriMurphy_Text_C_Fall_Content = {
    Text = "Drops Preview"
  },
  PanelText_UI_Events_Item_OriMurphy_Text_C_LockTips_Content = {
    Text = "Unlock Notification"
  },
  PanelText_UI_Events_Item_Pickman_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Pickman_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Pollux_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Pollux_Text_C_LockTips_Content = {
    Text = "How to Unlock"
  },
  PanelText_UI_Events_Item_PreOrderGift_Text_C_Lock_Content = {
    Text = "Purchase to Claim"
  },
  PanelText_UI_Events_Item_RAIDProgressGroup_Text_C_Kill_Content = {
    Text = "Total Captures"
  },
  PanelText_UI_Events_Item_RAID_Text_C_Career_Content = {Text = "Require:"},
  PanelText_UI_Events_Item_RAID_Text_C_Finish_Content = {Text = "Ended"},
  PanelText_UI_Events_Item_RAID_Text_C_Kill_Content = {Text = "Captured"},
  PanelText_UI_Events_Item_RAID_Text_CareerLimit_Content = {Text = "Unlimited"},
  PanelText_UI_Events_Item_RAID_Text_Lock_Content = {
    Text = "Not Yet Available"
  },
  PanelText_UI_Events_Item_Rome_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Rome_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Ryker_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Ryker_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_SignDiamond_Text_C_Receive_Content = {Text = "Claimed"},
  PanelText_UI_Events_Item_Sorel_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Sorel_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_StElmosFire_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_StElmosFire_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Stage_Text_C_Undone_Content = {Text = "Incomplete"},
  PanelText_UI_Events_Item_Summer1_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Summer1_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_Summer2_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Summer2_Text_C_LockTips_Content = {
    Text = "Stage Unlock Guide"
  },
  PanelText_UI_Events_Item_Summer2_Text_C_NormalTip_Content = {
    Text = "Normal Difficulty Pending Clear"
  },
  PanelText_UI_Events_Item_SurveyDiary_Text_Name_Content = {Text = "Claim"},
  PanelText_UI_Events_Item_Tab_Text_C_Return_Content = {
    Text = "Homecoming Event"
  },
  PanelText_UI_Events_Item_Tawil_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Tawil_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_TeamSchool_Text_C_Career_Content = {
    Text = "Party Requirements"
  },
  PanelText_UI_Events_Item_TeamSchool_Text_C_Finish_Content = {Text = "Ended"},
  PanelText_UI_Events_Item_TeamSchool_Text_CareerLimit_Content = {Text = "Unlimited"},
  PanelText_UI_Events_Item_Tulu_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Tulu_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_TwentyFour_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_TwentyFour_Text_C_LockTips_Content = {
    Text = "Complete the prerequisite levels to unlock Task Force Records"
  },
  PanelText_UI_Events_Item_Wanda_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_Wanda_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_WorldBoss_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_Item_WorldBoss_Text_C_Killed_Content = {
    Text = "Eliminated!"
  },
  PanelText_UI_Events_Item_WorldBoss_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Item_WorldBoss_Text_C_Progress_Content = {
    Text = "Number of Remaining Clones"
  },
  PanelText_UI_Events_OceanBoss_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Abandon your body, abandon your sanity, and in this world of pure will, feel His presence.
He is your only one. He is also your all.]]
  },
  PanelText_UI_Events_OceanBoss_StageGroup_Text_C_Title_Content = {
    Text = "Anomalous Tide"
  },
  PanelText_UI_Events_OriHelot_StageGroup_Text_C_Introduction_Content = {
    Text = "\nOpen wide and bite down hard.\nTo devour or be devoured—this is the rule of battle, this is the fate of a beast."
  },
  PanelText_UI_Events_OriHelot_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_OriHelot_StageGroup_Text_C_Title_Content = {
    Text = "Beastly Ambition"
  },
  PanelText_UI_Events_OriHelot_Text_C_Fall_Content = {Text = "Drops"},
  PanelText_UI_Events_OriHelot_Text_C_LockTips_Content = {
    Text = "Level Unlock Notification"
  },
  PanelText_UI_Events_Panel_2ndAnnivWorldBoss_Text_C_Challenge_Content = {
    Text = "Stellar Requiem"
  },
  PanelText_UI_Events_Panel_2ndAnnivWorldBoss_Text_C_ProgressTitle_Content = {
    Text = "Global Progress"
  },
  PanelText_UI_Events_Panel_ActivityPassport_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Events_Panel_ActivityPassport_Text_C_Ordinary_Content = {
    Text = "Standard Archives"
  },
  PanelText_UI_Events_Panel_ActivityPassport_Text_C_PassportUp_Content = {Text = "Upgrade"},
  PanelText_UI_Events_Panel_ActivityPassport_Text_C_Senior_Content = {
    Text = "Premium Archives"
  },
  PanelText_UI_Events_Panel_ActivityPassport_Text_C_illustrate_Content = {
    Text = "Event Description"
  },
  PanelText_UI_Events_Panel_ActivityPassport_Text_PayTip_Content = {
    Text = "The displayed price is for reference only. Actual payments will be processed in the applicable currency at the current exchange rate."
  },
  PanelText_UI_Events_Panel_Arachne_StageGroup_Text_C_Introduction_Content = {
    Text = "The lights flare up. The applause dies down. The audience waits in anticipation.\nIt's time to take the stage, my dear children. Go forth—perform, tear open your very souls, and sell your deepest secrets. Take your most devastating wounds and fashion them into sweets for the audience’s pleasure. Bask greedily in that single beam of light lit only for you, until the curtain falls to bury both you and your audience alike.\n\nThis is the stage you were destined for—ascend! Do not be afraid; there is no need for stage fright."
  },
  PanelText_UI_Events_Panel_Arachne_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Arachne_StageGroup_Text_C_Title_Content = {
    Text = "The Puppet's Stage"
  },
  PanelText_UI_Events_Panel_Arachne_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Arachne_Text_C_Challenge_Content = {
    Text = "The Puppet's Stage"
  },
  PanelText_UI_Events_Panel_Arachne_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Arachne_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_Arachne_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Arachne_Text_C_Shop_Content = {
    Text = "Theater Souvernirs"
  },
  PanelText_UI_Events_Panel_Arachne_Text_C_Topic_Content = {
    Text = "Bound Weaver"
  },
  PanelText_UI_Events_Panel_Attendance_Text_C_GiftTitle_Content = {
    Text = "Claim a Skin Voucher!"
  },
  PanelText_UI_Events_Panel_Award_Text_C_Award_1_Content = {
    Text = "Phantasmal Dive"
  },
  PanelText_UI_Events_Panel_Award_Text_C_Award_2_Content = {Text = "Traphase"},
  PanelText_UI_Events_Panel_Award_Text_C_Award_Content = {
    Text = "Phantasmal Dive"
  },
  PanelText_UI_Events_Panel_Award_Text_C_Over_Content = {
    Text = "Today's Keeper EXP"
  },
  PanelText_UI_Events_Panel_Award_Text_C_Title_Content = {
    Text = "Today's Keeper EXP"
  },
  PanelText_UI_Events_Panel_Back_To_School_Text_C_Not_Unlocked_Content = {
    Text = "Not Unlocked"
  },
  PanelText_UI_Events_Panel_Back_To_School_Text_C_Shop_Content = {
    Text = "Back-to-School Store"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Banquet_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Challenge_Content = {
    Text = "Sacrosanct Path"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Fill_Up_Content = {
    Text = "(Til Maxed:"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Finish_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Banquet_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Banquet_Text_C_Fundraising_Content = {
    Text = "Goodwill of the Citizens"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Not_unlocked_Content = {
    Text = "<Gray:Unlocked>"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Pv_Content = {Text = "Story"},
  PanelText_UI_Events_Panel_Banquet_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Record_Content = {
    Text = "The Color of Blood"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Resonance_Content = {
    Text = "Resonance: Soul's Fork"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Shop_Content = {
    Text = "Church Charity Bazaar"
  },
  PanelText_UI_Events_Panel_Banquet_Text_C_Topic_Content = {
    Text = "Soul Shepherd: Rerun"
  },
  PanelText_UI_Events_Panel_Branch_Text_C_Award_Content = {
    Text = "Limited Time Rewards"
  },
  PanelText_UI_Events_Panel_Branch_Text_C_Limit_Content = {
    Text = "Survey Rewards"
  },
  PanelText_UI_Events_Panel_Branch_Text_C_Over_Content = {
    Text = "All rewards have been claimed"
  },
  PanelText_UI_Events_Panel_Branch_Text_C_Pass_Content = {
    Text = "Completion Reward"
  },
  PanelText_UI_Events_Panel_Branch_Text_C_Title_Content = {
    Text = "Reward Preview"
  },
  PanelText_UI_Events_Panel_Caraboo_StageGroup_Text_C_Introduction_Content = {
    Text = "♪~~♫~~♪~~♫~~♪~~♫~~\nBeneath the snow mountain, every guide remembers it, and every child still sings it. She is the will of the towering mountain made flesh, she is the merciful goddess Umapati. Her divine power lies scattered through the mortal world; her treasures cannot be found. They are near enough to touch; they leave no trace at all. \nSurrender something. Hold onto something. Any whose heart is false toward the goddess's true form shall be buried in the snow mountain forever, alongside the mountain that died."
  },
  PanelText_UI_Events_Panel_Caraboo_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Caraboo_StageGroup_Text_C_Title_Content = {
    Text = "Umapati's Treasures"
  },
  PanelText_UI_Events_Panel_Caraboo_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Caraboo_Text_C_Challenge_Content = {
    Text = "Umapati's Treasures"
  },
  PanelText_UI_Events_Panel_Caraboo_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Caraboo_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_Caraboo_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Caraboo_Text_C_Shop_Content = {
    Text = "Fairy's Rituals"
  },
  PanelText_UI_Events_Panel_Caraboo_Text_C_Topic_Content = {
    Text = "New Moon and Deep Snow"
  },
  PanelText_UI_Events_Panel_Castor_StageGroup_Text_C_Introduction_Content = {
    Text = "A bird's life is destined to be a life of freedom. You should have seen it at the moment of hatching, wresting the freedom of life from the hands of death. You should have seen it when it first learned to fly, wresting the freedom of survival from the comfort of a warm nest. Now, if the Key to freedom were cast into the abyss of death, it would leap without hesitation—choosing to face the raging storm of death over a narrow life, embracing infinite freedom instead."
  },
  PanelText_UI_Events_Panel_Castor_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Castor_StageGroup_Text_C_Title_Content = {
    Text = "Skybound Spirit"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Can_Unlockable_Content = {Text = "Claimable"},
  PanelText_UI_Events_Panel_Castor_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Challenge_Content = {
    Text = "Skybound Spirit"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Fill_Up_Content = {
    Text = "(Countdown:"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Castor_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Castor_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Record_Content = {Text = "Dreamscape"},
  PanelText_UI_Events_Panel_Castor_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Shop_Content = {
    Text = "Thirst for Knowledge"
  },
  PanelText_UI_Events_Panel_Castor_Text_C_Topic_Content = {
    Text = "Mirrored Prisoner"
  },
  PanelText_UI_Events_Panel_Clementine_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Since then, she has not shed a single tear.
Wandering alone, drifting in a world full of outcasts.
A grand plan is about to begin, as her final farewell.
She is the most successful psychologist, and this is her most successful treatment.]]
  },
  PanelText_UI_Events_Panel_Clementine_StageGroup_Text_C_Title_Content = {
    Text = "Fear Therapy"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Clementine_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Challenge_Content = {
    Text = "Fear Therapy"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Clementine_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Clementine_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Record_Content = {
    Text = "Psyche Deepdive"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Resonance_Content = {
    Text = "Resonance: Null"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Shop_Content = {
    Text = "Neurotherapy Room"
  },
  PanelText_UI_Events_Panel_Clementine_Text_C_Topic_Content = {
    Text = "Buried in Memory"
  },
  PanelText_UI_Events_Panel_ClotherLottery_Text_C_Empty_Content = {
    Text = "All rewards have been obtained"
  },
  PanelText_UI_Events_Panel_ClotherLottery_Text_C_Lottery_Content = {Text = "1× Pull"},
  PanelText_UI_Events_Panel_ClotherLottery_Text_C_Rate_Content = {
    Text = "Probability Description"
  },
  PanelText_UI_Events_Panel_ClotherLottery_Text_C_Tip_Content = {
    Text = "<color=#e3c96e>Premium Skin</color> may drop starting from the <color=#e3c96e>5</color>th pull"
  },
  PanelText_UI_Events_Panel_ClotherLottery_Text_C_Topic_Content = {
    Text = "Soul Shepherd"
  },
  PanelText_UI_Events_Panel_ClotherLottery_Text_Free_Content = {Text = "Free"},
  PanelText_UI_Events_Panel_Daffodil_StageGroup_Text_C_Introduction_Content = {
    Text = [[
In this seaside town, there is a bustling commercial street. Maritime trade has brought countless trendy goods here; as long as you pay enough Leonian Florins, you can always find satisfactory products.
The only exception is a small shop on the corner, a quaint little place filled with magical antiques. Amid the smoke billowing from a pipe, the shopkeeper, wearing a wide-brimmed hat, will slowly reveal a smile to you.
"Welcome to the Worry-Free Shop, where all your needs can be met, as long as you offer enough compensation..."
"Of course, of course, the prices are fair, and we are honest with everyone."]]
  },
  PanelText_UI_Events_Panel_Daffodil_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Daffodil_StageGroup_Text_C_Title_Content = {
    Text = "Witch's Shop"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Daffodil_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Challenge_Content = {
    Text = "Witch's Shop"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Daffodil_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Daffodil_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Shop_Content = {
    Text = "Witch's Vault"
  },
  PanelText_UI_Events_Panel_Daffodil_Text_C_Topic_Content = {
    Text = "Phantom of the Rainy Town"
  },
  PanelText_UI_Events_Panel_Doresain_StageGroup_Text_C_Introduction_Content = {
    Text = [[
The orchestra has swelled to its zenith, and the thirteenth bell tolls.
Tonight, the poet's knife will carve its verse.
Tonight, the mother's throat envelops us whole.]]
  },
  PanelText_UI_Events_Panel_Doresain_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Doresain_StageGroup_Text_C_Title_Content = {Text = "Soulsfeast"},
  PanelText_UI_Events_Panel_Doresain_Text_C_Can_Unlockable_Content = {Text = "Available"},
  PanelText_UI_Events_Panel_Doresain_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_Challenge_Content = {Text = "Soulsfeast"},
  PanelText_UI_Events_Panel_Doresain_Text_C_Fill_Up_Content = {
    Text = "(Till Refilled:"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Doresain_Text_C_Not_unlocked_Content = {Text = "Locked"},
  PanelText_UI_Events_Panel_Doresain_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_Record_Content = {
    Text = "Assignments Record"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_Resonance_Content = {
    Text = "Resonance: Devour the Mountains"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_Shop_Content = {
    Text = "Ossuary Banquet"
  },
  PanelText_UI_Events_Panel_Doresain_Text_C_Topic_Content = {
    Text = "Necrotic Aesthetics"
  },
  PanelText_UI_Events_Panel_GamePassport_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Events_Panel_GamePassport_Text_C_Ordinary_Content = {
    Text = "Standard Archives"
  },
  PanelText_UI_Events_Panel_GamePassport_Text_C_PassportUp_Content = {Text = "Upgrade"},
  PanelText_UI_Events_Panel_GamePassport_Text_C_Senior_Content = {
    Text = "Premium Archives"
  },
  PanelText_UI_Events_Panel_GamePassport_Text_C_illustrate_Content = {
    Text = "Event Description"
  },
  PanelText_UI_Events_Panel_Hameln_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Roaring Machines, played on the tip of his spear.
Immersed in a Long Dream, until the Destruction's final chapter.]]
  },
  PanelText_UI_Events_Panel_Hameln_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Hameln_StageGroup_Text_C_Title_Content = {
    Text = "Roaring Machines"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Hamline_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Challenge_Content = {
    Text = "Roaring Machines"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Hamline_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Hamline_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Resonance_Content = {
    Text = "Resonance: Null"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Shop_Content = {
    Text = "Somnus Citadel"
  },
  PanelText_UI_Events_Panel_Hamline_Text_C_Topic_Content = {
    Text = "Invisible Symphony"
  },
  PanelText_UI_Events_Panel_Horla_StageGroup_Text_C_Introduction_Content = {
    Text = "In the dilapidated greenhouse, a lonely rose is imprisoned."
  },
  PanelText_UI_Events_Panel_Horla_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Horla_StageGroup_Text_C_Title_Content = {
    Text = "Garden of Recollections"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Horla_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Challenge_Content = {
    Text = "Garden Memories"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Fill_Up_Content = {
    Text = "(Till Refilled:"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Horla_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Horla_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Resonance_Content = {
    Text = "Resonance: Garden Elegy"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Shop_Content = {
    Text = "Colette's Flower House"
  },
  PanelText_UI_Events_Panel_Horla_Text_C_Topic_Content = {
    Text = "Before Spring Fades"
  },
  PanelText_UI_Events_Panel_Karen_Text_C_Reward_Content = {
    Text = "Dust Removal Allowance"
  },
  PanelText_UI_Events_Panel_Karen_Text_C_Sign_Content = {
    Text = "Karen's Gratitude"
  },
  PanelText_UI_Events_Panel_Karen_Text_C_Task_Content = {
    Text = "Cleaning Progress"
  },
  PanelText_UI_Events_Panel_Karen_Text_C_Topic_Content = {
    Text = "Mop & Wonder"
  },
  PanelText_UI_Events_Panel_KathiguRa_StageGroup_Text_C_Introduction_Content = {
    Text = [[
She stands once again upon the Earth, and the revenge that consumes all arrives as promised.

It is an unending Pursuit, it is the searing fire of The Living Flame.

She will traverse every corner, hunting down all who have signed a Contract with Death.

Run, run, flee into the deeper Darkness, flee to a place you think will not be found.

Though it is meaningless, for He will tread through every world.

He will hunt down everything until all is reduced to ash.]]
  },
  PanelText_UI_Events_Panel_KathiguRa_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_KathiguRa_StageGroup_Text_C_Title_Content = {
    Text = "Hunt of the Charred"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Can_Unlockable_Content = {Text = "Available"},
  PanelText_UI_Events_Panel_KathiguRa_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Challenge_Content = {
    Text = "Hunt of the Charred"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Fill_Up_Content = {
    Text = "(Til Storage Limit:"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Not_unlocked_Content = {Text = "Locked"},
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_RecordTip_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Record_Content = {
    Text = "Psyche Deepdive"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Resonance_Content = {
    Text = "Resonance: Devour the Mountains"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Shop_Content = {
    Text = "A Hidden Base"
  },
  PanelText_UI_Events_Panel_KathiguRa_Text_C_Topic_Content = {
    Text = "A Million Suns Descend"
  },
  PanelText_UI_Events_Panel_Lake_City_Text_C_Describe_Content = {
    Text = "What you witness is not just the choice of a soul, but also the fluctuating journey of its life."
  },
  PanelText_UI_Events_Panel_Lake_City_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Lake_City_Text_C_Title_Content = {
    Text = "Sacrosanct Path"
  },
  PanelText_UI_Events_Panel_Lake_City_Text_C_illustrate_Content = {Text = "Rules"},
  PanelText_UI_Events_Panel_Lily_StageGroup_Text_C_Introduction_Content = {
    Text = [[
The tasks assigned by Doll are never simple.
Even though she repeatedly reminds everyone to "have fun," how can the Keeper not feel nervous when taking a group of Awakener out for the first time?
But remember to "have fun," okay.]]
  },
  PanelText_UI_Events_Panel_Lily_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Lily_StageGroup_Text_C_Title_Content = {
    Text = "Plotted Mystery"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_Challenge_Content = {
    Text = "Plotted Mystery"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_Lily_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_Resonance_Content = {
    Text = "Resonance: Director's Code"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_Shop_Content = {
    Text = "Arcane codex emporium"
  },
  PanelText_UI_Events_Panel_Lily_Text_C_Topic_Content = {
    Text = "The Magic of Stories: Rerun"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Strength, power, wealth, freedom... Upon Leviathan's lands, It shall grant you all you crave.
Provided, of course, that you are the last one standing.]]
  },
  PanelText_UI_Events_Panel_LotanCetarchon_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_LotanCetarchon_StageGroup_Text_C_Title_Content = {
    Text = "Greed and Bloodlust"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_Text_C_Challenge_Content = {
    Text = "Greed and Bloodlust"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_Text_C_Record_Content = {
    Text = "Psyche Deepdive"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_Text_C_Shop_Content = {
    Text = "Arena Altar"
  },
  PanelText_UI_Events_Panel_LotanCetarchon_Text_C_Topic_Content = {
    Text = "Leviathan's Game"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Challenge_Content = {
    Text = "Plotted Mystery"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Desc_Content = {
    Text = [[
The tasks assigned by Doll are never simple.
Even though she repeatedly reminds everyone to "have fun," how can the Keeper not feel nervous when taking a group of Awakener out for the first time?
But remember to "have fun," okay.]]
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Own_Content = {
    Text = "Currently Owned"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Resonance_Content = {
    Text = "Resonance: Director's Code"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Shop_Content = {
    Text = "Arcane codex emporium"
  },
  PanelText_UI_Events_Panel_MagicStory_Text_C_Topic_Content = {
    Text = "The Magic of Stories"
  },
  PanelText_UI_Events_Panel_Main_Text_C_PermanentActivity_Content = {Text = "Arcade"},
  PanelText_UI_Events_Panel_Main_Text_C_PermanentRule_Content = {Text = "Arcade"},
  PanelText_UI_Events_Panel_Main_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_Main_Text_C_SevenDays_Content = {
    Text = "Seven-Day Sign-In"
  },
  PanelText_UI_Events_Panel_Main_Text_C_illustrate_Content = {
    Text = "Event Description"
  },
  PanelText_UI_Events_Panel_Main_Text_Title_Content = {Text = "Rewards"},
  PanelText_UI_Events_Panel_Miriam_StageGroup_Text_C_Introduction_Content = {
    Text = "Take this arduous and sacred path. The final Gate will open for you at the end of it all."
  },
  PanelText_UI_Events_Panel_Miriam_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Miriam_StageGroup_Text_C_Title_Content = {
    Text = "Divine Rebirth"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Can_Unlockable_Content = {
    Text = "Available to Claim"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Challenge_Content = {
    Text = "Divine Rebirth"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Fill_Up_Content = {
    Text = "(Till Full Refill:"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Miriam_Text_C_Not_unlocked_Content = {Text = "Locked"},
  PanelText_UI_Events_Panel_Miriam_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_RecordTip_Content = {
    Text = "Limited-Time Free Unlock"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Record_Content = {Text = "Dreamscape"},
  PanelText_UI_Events_Panel_Miriam_Text_C_Resonance_Content = {
    Text = "Resonance: Abyssal Rite"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Shop_Content = {
    Text = "The Abyss Responds"
  },
  PanelText_UI_Events_Panel_Miriam_Text_C_Topic_Content = {
    Text = "Abyss Sacrifice"
  },
  PanelText_UI_Events_Panel_Mosk_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Sky~ so bright~ sea~ so calm~
Vortice~ and friends~ Coalsie~ set sail!
Storms? No fear! Lightning? No fear!
Vortice, friends, Coalsie~
For our freedom! Here! We! Go!]]
  },
  PanelText_UI_Events_Panel_Mosk_StageGroup_Text_C_Title_Content = {
    Text = "Vortice and the Sea"
  },
  PanelText_UI_Events_Panel_Mosk_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Mosk_Text_C_Challenge_Content = {
    Text = "Vortice and the Sea"
  },
  PanelText_UI_Events_Panel_Mosk_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Mosk_Text_C_Record_Content = {
    Text = "No More the Shore"
  },
  PanelText_UI_Events_Panel_Mosk_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Mosk_Text_C_Shop_Content = {
    Text = "Vortice's Hoard"
  },
  PanelText_UI_Events_Panel_Mosk_Text_C_Topic_Content = {
    Text = "Call of the Moskstraumen"
  },
  PanelText_UI_Events_Panel_Mouchette_StageGroup_Text_C_Introduction_Content = {
    Text = [[
One day, you suddenly realize that you've changed.
No one discovered this because you have the same behavior and demeanor, the same stunning appearance.
But you know very well that the originally bright red heart has already been covered by a mask made of mist
If I can keep pretending, am I still the same person I was before?
Congratulations, this question didn't linger in your mind for more than a second; in fact, before that mask is lifted, no one knows your true thoughts.
Then, let's set off first~]]
  },
  PanelText_UI_Events_Panel_Mouchette_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Mouchette_StageGroup_Text_C_Title_Content = {
    Text = "The Lost Self"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_Challenge_Content = {
    Text = "The Lost Self"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_RecordTip_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_Record_Content = {
    Text = "Multiverse Link"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_Resonance_Content = {
    Text = "Resonance: Devour the Mountains"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_Shop_Content = {
    Text = "Mouchette's Cottage"
  },
  PanelText_UI_Events_Panel_Mouchette_Text_C_Topic_Content = {
    Text = "Mist-Shrouded Sacrament"
  },
  PanelText_UI_Events_Panel_Murphy_StageGroup_Text_C_Introduction_Content = {
    Text = "In His Dau@2ter's Darkling @2mb, the Great @4 is Born"
  },
  PanelText_UI_Events_Panel_Murphy_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Murphy_StageGroup_Text_C_Title_Content = {
    Text = "Darkling Womb"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_Challenge_Content = {
    Text = "Dark Uterus"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_Homology_Content = {
    Text = "During the event, Affinity Gain is doubled!"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_Murphy_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_Record_Content = {
    Text = "Tribute to the Rose"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_Shop_Content = {
    Text = "Celestial Gestation"
  },
  PanelText_UI_Events_Panel_Murphy_Text_C_Topic_Content = {
    Text = "The Stars Are Right: Rerun"
  },
  PanelText_UI_Events_Panel_OceanBoss_Text_C_Challenge_Content = {
    Text = "Anomalous Tide"
  },
  PanelText_UI_Events_Panel_OceanBoss_Text_C_GiftTitle_Content = {
    Text = "Claim a Skin Voucher!"
  },
  PanelText_UI_Events_Panel_OceanBoss_Text_C_ProgressTitle_Content = {
    Text = "Bleak Tide Kill Progress"
  },
  PanelText_UI_Events_Panel_OceanBoss_Text_C_Topic_Content = {
    Text = "Divine Realm Resurfaces"
  },
  PanelText_UI_Events_Panel_OriDoll_StageGroup_Text_C_Introduction_Content = {
    Text = [[
She gazes at the great Gate at the end of the world, then steps forward without hesitation.
Click. The handle turns. Her eyes search eagerly beyond the threshold.
But there is only nothingness. She has stepped into the Illusion.
She should have known: gifts given so freely always demand the steepest price.]]
  },
  PanelText_UI_Events_Panel_OriDoll_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_OriDoll_StageGroup_Text_C_Title_Content = {
    Text = "Gate of Oblivion"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_OriDoll_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Challenge_Content = {
    Text = "Gate of Oblivion"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_OriDoll_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_OriDoll_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Record_Content = {
    Text = "Psyche Deepdive"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Resonance_Content = {
    Text = "Resonance: Null"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Shop_Content = {
    Text = "Mentor's Teachings"
  },
  PanelText_UI_Events_Panel_OriDoll_Text_C_Topic_Content = {
    Text = "Half Step Away"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_OriHelot_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Challenge_Content = {
    Text = "Beastly Ambition"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_OriHelot_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_OriHelot_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Shop_Content = {
    Text = "Mountain Maw"
  },
  PanelText_UI_Events_Panel_OriHelot_Text_C_Topic_Content = {
    Text = "Til the Snow Melts: Rerun"
  },
  PanelText_UI_Events_Panel_OriMurphy_StageGroup_Text_C_Introduction_Content = {
    Text = [[
That which Our Lord promised is nigh.
The waiting of millennia shall meet a joyous finality, and the promised land shall rise from the sea.
He shall be reborn from the Divine Mother's chest; when the seventh day concludes, a new world shall descend.]]
  },
  PanelText_UI_Events_Panel_OriMurphy_StageGroup_Text_C_Title_Content = {
    Text = "Week of Theophany"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Can_Unlockable_Content = {Text = "Available"},
  PanelText_UI_Events_Panel_OriMurphy_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Challenge_Content = {
    Text = "Week of Theophany"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Fill_Up_Content = {
    Text = "(Till Refilled:"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Not_unlocked_Content = {Text = "Locked"},
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_RecordTip_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Shop_Content = {
    Text = "Holy Bedchamber"
  },
  PanelText_UI_Events_Panel_OriMurphy_Text_C_Topic_Content = {
    Text = "Rites of Divine Birth"
  },
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_PVPReRound_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Collaboration"
  },
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Challenge_Content = {Text = "Traphase"},
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_PVPReRound_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Record_Content = {Text = "My Turn"},
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Shop_Content = {Text = "Shop Name"},
  PanelText_UI_Events_Panel_PVPReRound_Text_C_Topic_Content = {
    Text = "Phantom of the Rainy Town"
  },
  PanelText_UI_Events_Panel_PermanentActivity_Text_C_Reward_Content = {
    Text = "Remaining Rewards"
  },
  PanelText_UI_Events_Panel_Pickman_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Between sanity and breakdown, a bizarre scroll unfolds.
The hallucinations you despise paint brilliant colors,
The loss of control you fear takes the form of loyalty.
Here, madness is the instinct of art, the origin of the world,
The only truth that never lies.]]
  },
  PanelText_UI_Events_Panel_Pickman_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Pickman_StageGroup_Text_C_Title_Content = {
    Text = "Bizzare Sketch"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_Challenge_Content = {
    Text = "Bizzare Sketch"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_RecordTip_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_Record_Content = {
    Text = "Psyche Deepdive"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_Resonance_Content = {
    Text = "Resonance: Devour the Mountains"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_Shop_Content = {
    Text = "Pickman's Studio"
  },
  PanelText_UI_Events_Panel_Pickman_Text_C_Topic_Content = {
    Text = "Dark Side of the Canvas"
  },
  PanelText_UI_Events_Panel_Pollux_StageGroup_Text_C_Introduction_Content = {
    Text = "Hate and love, warmth and pain, what we once held and what we lost—it is all intertwined now, impossible to untangle.\nLet our shared blood flow together and converge, forging our inseparable fate."
  },
  PanelText_UI_Events_Panel_Pollux_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Pollux_StageGroup_Text_C_Title_Content = {
    Text = "Shared Bitter Blood"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Can_Unlockable_Content = {Text = "Available"},
  PanelText_UI_Events_Panel_Pollux_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Challenge_Content = {
    Text = "Shared Bitter Blood"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Fill_Up_Content = {
    Text = "(Till Refilled:"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Pollux_Text_C_Not_unlocked_Content = {Text = "Locked"},
  PanelText_UI_Events_Panel_Pollux_Text_C_Receive_Content = {
    Text = "Till Claimable"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_RecordTip_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Record_Content = {
    Text = "Beyond the Skies: Part II"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Resonance_Content = {
    Text = "Resonance: Devour the Mountains"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Shop_Content = {
    Text = "Rite of Benediction"
  },
  PanelText_UI_Events_Panel_Pollux_Text_C_Topic_Content = {
    Text = "Everbright Nightfall"
  },
  PanelText_UI_Events_Panel_Pontos_StageGroup_Text_C_Introduction_Content = {
    Text = "As the ominous white light pierces the casements of the sky-perched house once more, His invitation awaits.\nAbsorbing, engulfing, sweeping, crowding...\nAnswer the call from the ocean's abyss and join this endless hunt. Your past, your self—all is lost to the mist."
  },
  PanelText_UI_Events_Panel_Pontos_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Pontos_StageGroup_Text_C_Title_Content = {
    Text = "The High House Hunt"
  },
  PanelText_UI_Events_Panel_Pontos_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Pontos_Text_C_Challenge_Content = {
    Text = "The High House Hunt"
  },
  PanelText_UI_Events_Panel_Pontos_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Pontos_Text_C_Record_Content = {
    Text = "Psyche Deepdive"
  },
  PanelText_UI_Events_Panel_Pontos_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Pontos_Text_C_Shop_Content = {
    Text = "Hunting Grounds"
  },
  PanelText_UI_Events_Panel_Pontos_Text_C_Topic_Content = {
    Text = "Ebwynnos Evermore"
  },
  PanelText_UI_Events_Panel_RAID_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_RAID_Text_C_Challenge_Content = {
    Text = "Tide of Silver Paws"
  },
  PanelText_UI_Events_Panel_RAID_Text_C_Rank_Content = {
    Text = "Capture Rankings"
  },
  PanelText_UI_Events_Panel_RAID_Text_C_Reward_Content = {
    Text = "Black Cat Capture Log"
  },
  PanelText_UI_Events_Panel_RAID_Text_C_Topic_Content = {
    Text = "Meowch Obliged!"
  },
  PanelText_UI_Events_Panel_RailWay_StageGroup_Text_C_Title_Content = {
    Text = "Board the Train"
  },
  PanelText_UI_Events_Panel_RailWay_Text_C_Challenge_Content = {
    Text = "Board the Train"
  },
  PanelText_UI_Events_Panel_RailWay_Text_C_Rank_Content = {Text = "VIP List"},
  PanelText_UI_Events_Panel_RailWay_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_RailWay_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_RailWay_Text_C_Resonance_Content = {
    Text = "Little N's Help"
  },
  PanelText_UI_Events_Panel_RailWay_Text_C_Reward_Content = {
    Text = "Track Traces"
  },
  PanelText_UI_Events_Panel_RailWay_Text_C_Topic_Content = {
    Text = "Hectic Skybound Rail"
  },
  PanelText_UI_Events_Panel_Reward_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Events_Panel_Reward_Text_Title_Content = {Text = "Rewards"},
  PanelText_UI_Events_Panel_Rome_StageGroup_Text_C_Introduction_Content = {
    Text = "We are carnivores, children of the wolf. The meat we crave—rich, red, and dripping—will find its way to our teeth and tongues, as all things must."
  },
  PanelText_UI_Events_Panel_Rome_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Rome_StageGroup_Text_C_Title_Content = {
    Text = "Divine Predation"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Rome_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Challenge_Content = {
    Text = "Divine Predation"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Rome_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Rome_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Record_Content = {Text = "Dreamscape"},
  PanelText_UI_Events_Panel_Rome_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Shop_Content = {
    Text = "Offer Tribute"
  },
  PanelText_UI_Events_Panel_Rome_Text_C_Topic_Content = {
    Text = "Rome! Special Event"
  },
  PanelText_UI_Events_Panel_Ryker_StageGroup_Text_C_Introduction_Content = {
    Text = [[
That's right, it's no longer popular to be an ordinary detective without any flair.
If you can't fly or disappear, and you don't have a scientist assistant, please rethink your career plan.]]
  },
  PanelText_UI_Events_Panel_Ryker_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Ryker_StageGroup_Text_C_Title_Content = {
    Text = "Moonlit Tales"
  },
  PanelText_UI_Events_Panel_Ryker_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Ryker_Text_C_Challenge_Content = {
    Text = "Moonlit Tales"
  },
  PanelText_UI_Events_Panel_Ryker_Text_C_Homology_Content = {
    Text = "During the event, Affinity Gain is doubled!"
  },
  PanelText_UI_Events_Panel_Ryker_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_Ryker_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Ryker_Text_C_Record_Content = {
    Text = "Happy Hunting!"
  },
  PanelText_UI_Events_Panel_Ryker_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Ryker_Text_C_Shop_Content = {Text = "Lucky Club"},
  PanelText_UI_Events_Panel_Ryker_Text_C_Topic_Content = {
    Text = "Detective Handbook: Rerun"
  },
  PanelText_UI_Events_Panel_Saya_StageGroup_Text_C_Introduction_Content = {
    Text = "Science? Of no consequence. Ethics? Of no consequence. Morality? Of no consequence. \nLife, death, hope, despair... in the end, on a notebook in the basement, the same proof is written in two different hands. \nThey say: <Italic:\"Look—love is the one and only eternal theme.\">"
  },
  PanelText_UI_Events_Panel_Saya_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Saya_StageGroup_Text_C_Title_Content = {
    Text = "Dark Chamber Notes"
  },
  PanelText_UI_Events_Panel_Saya_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Saya_Text_C_Challenge_Content = {
    Text = "Dark Chamber Notes"
  },
  PanelText_UI_Events_Panel_Saya_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Saya_Text_C_Record_Content = {
    Text = "Forgotten Seed of Love"
  },
  PanelText_UI_Events_Panel_Saya_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Saya_Text_C_Shop_Content = {
    Text = "Dandelion Garden"
  },
  PanelText_UI_Events_Panel_Saya_Text_C_Topic_Content = {
    Text = "Inverted Rebirth"
  },
  PanelText_UI_Events_Panel_Sorel_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Years later, a biographer knocked on her door one evening.
That day, they talked about many things, from agate stones to the decaying rose mansion...
Now, the writer has compiled these fragmented words into a book, dedicated to all of you present.]]
  },
  PanelText_UI_Events_Panel_Sorel_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Sorel_StageGroup_Text_C_Title_Content = {
    Text = "Rose's Memories"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_Challenge_Content = {
    Text = "Rose's Memories"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_Homology_Content = {
    Text = "During the event, Affinity Gain is doubled!"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_Sorel_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_Record_Content = {
    Text = "Tribute to the Rose"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_Shop_Content = {
    Text = "Crimson Revel"
  },
  PanelText_UI_Events_Panel_Sorel_Text_C_Topic_Content = {
    Text = "Epistle to the Rose"
  },
  PanelText_UI_Events_Panel_Spur_Text_C_Limit_Content = {
    Text = "Investigation Reward"
  },
  PanelText_UI_Events_Panel_Spur_Text_C_Over_Content = {
    Text = "All reward conditions have been achieved."
  },
  PanelText_UI_Events_Panel_Spur_Text_C_Pass_Content = {
    Text = "Completion Rewards"
  },
  PanelText_UI_Events_Panel_Spur_Text_C_Title_Content = {
    Text = "Complete the Awakener trial to receive a reward."
  },
  PanelText_UI_Events_Panel_StElmosFire_StageGroup_Text_C_Introduction_Content = {
    Text = [[
The old Sextant determines the angles between the stars, ancient sea charts delineate the expected course, and the worn compass decides the direction to move Forward...
Even in the deep, cold Black Sea, there are still Explorers who rely on ordinary copper and iron to guide the way for Everyone.
May All guides never become lost.]]
  },
  PanelText_UI_Events_Panel_StElmosFire_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_StElmosFire_StageGroup_Text_C_Title_Content = {
    Text = "Lost in Deep Sea"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Can_Unlockable_Content = {Text = "Available"},
  PanelText_UI_Events_Panel_StElmosFire_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Challenge_Content = {
    Text = "Lost in Deep Sea"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Fill_Up_Content = {
    Text = "(Remaining:"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Finished_Content = {Text = "Event End"},
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Not_unlocked_Content = {Text = "Locked"},
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Record_Content = {
    Text = "Final Steersmanship"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Resonance_Content = {
    Text = "Resonance: Mountain's Maw"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Shop_Content = {
    Text = "Crewmates' Treasures"
  },
  PanelText_UI_Events_Panel_StElmosFire_Text_C_Topic_Content = {
    Text = "Beneath the Tempest"
  },
  PanelText_UI_Events_Panel_Summer1_StageGroup_Text_C_Introduction_Content = {
    Text = "Friendship first, competition second—just kidding!\nSweating under the blazing sun, striving for the championship!"
  },
  PanelText_UI_Events_Panel_Summer1_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Summer1_StageGroup_Text_C_Title_Content = {
    Text = "Beach Volleyball Tournament"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Summer1_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Challenge_Content = {
    Text = "Beach Volleyball Tournament"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Fill_Up_Content = {
    Text = "(Till Refilled:"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Summer1_Text_C_Not_unlocked_Content = {Text = "Locked"},
  PanelText_UI_Events_Panel_Summer1_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_RecordTip_Content = {
    Text = "Limited Free Unlock"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Record_Content = {Text = "Dreamscape"},
  PanelText_UI_Events_Panel_Summer1_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Shop_Content = {
    Text = "Seaside Inn"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Sign_Content = {
    Text = "Summer Fiesta"
  },
  PanelText_UI_Events_Panel_Summer1_Text_C_Topic_Content = {
    Text = "Summer Special: I"
  },
  PanelText_UI_Events_Panel_Summer2_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Under the warm sunlight, everything feels so cozy and happy.
You gaze at the azure sky, just like in a fairy tale, and decide to tear open the canvas with your own hands, breaking out of the false world.]]
  },
  PanelText_UI_Events_Panel_Summer2_StageGroup_Text_C_Progress_Content = {
    Text = "Star Progress"
  },
  PanelText_UI_Events_Panel_Summer2_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Summer2_StageGroup_Text_C_Title_Content = {
    Text = "New Thing Under the Sun"
  },
  PanelText_UI_Events_Panel_Summer2_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Summer2_Text_C_Challenge_Content = {
    Text = "New Thing Under the Sun"
  },
  PanelText_UI_Events_Panel_Summer2_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Summer2_Text_C_Record_Content = {Text = "Dreamscape"},
  PanelText_UI_Events_Panel_Summer2_Text_C_Resonance_Content = {
    Text = "Resonance: Devour the Mountains"
  },
  PanelText_UI_Events_Panel_Summer2_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Summer2_Text_C_Shop_Content = {
    Text = "Student Union Building"
  },
  PanelText_UI_Events_Panel_Summer2_Text_C_Topic_Content = {
    Text = "If Life Was As Usual"
  },
  PanelText_UI_Events_Panel_SurveyDiary_Text_C_Return_Content = {
    Text = "Return to School Reward"
  },
  PanelText_UI_Events_Panel_Task_Text_C_OneKeyName_Content = {Text = "Claim All"},
  PanelText_UI_Events_Panel_Task_Text_Title_Content = {
    Text = "Cleaning Progress"
  },
  PanelText_UI_Events_Panel_Tawil_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Cross the Ultimate Gate and reach the realm of infinite wisdom.
Confront the desecrated and understand what it means to be "yourself."
This is the Final Test bestowed upon you by Them.]]
  },
  PanelText_UI_Events_Panel_Tawil_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Tawil_StageGroup_Text_C_Title_Content = {Text = "Final Test"},
  PanelText_UI_Events_Panel_Tawil_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Tawil_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Tawil_Text_C_Challenge_Content = {Text = "Final Test"},
  PanelText_UI_Events_Panel_Tawil_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_Tawil_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Tawil_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Tawil_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Tawil_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Tawil_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_Tawil_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_Tawil_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Tawil_Text_C_Shop_Content = {Text = "Echo"},
  PanelText_UI_Events_Panel_Tawil_Text_C_Topic_Content = {
    Text = "Gateway of All Realms"
  },
  PanelText_UI_Events_Panel_TeamSchool_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_TeamSchool_Text_C_Challenge_Content = {
    Text = "Cats Carnival"
  },
  PanelText_UI_Events_Panel_TeamSchool_Text_C_Tip_Content = {
    Text = "Different School Cat levels open every day, and you can earn rich rewards for successfully completing the challenges!"
  },
  PanelText_UI_Events_Panel_TeamSchool_Text_C_Topic_Content = {
    Text = "Uzzah Cat's Law"
  },
  PanelText_UI_Events_Panel_Thais_StageGroup_Text_C_Describe_Content = {
    Text = "Time flows back and forth, and the withered flowers bloom again."
  },
  PanelText_UI_Events_Panel_Thais_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Thais_StageGroup_Text_C_Title_Content = {
    Text = "Chaotic Desire"
  },
  PanelText_UI_Events_Panel_Thais_StageGroup_Text_C_illustrate_Content = {
    Text = "Gameplay Rules"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_Challenge_Content = {
    Text = "Chaotic Desire"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_Homology_Content = {
    Text = "During the event, Affinity Gain is doubled!"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_Thais_Text_C_RecordTip_Content = {
    Text = "Limited-Time Unlock"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_Record_Content = {
    Text = "Reproduction Frenzy"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_Shop_Content = {
    Text = "Spawn of Lust"
  },
  PanelText_UI_Events_Panel_Thais_Text_C_Topic_Content = {
    Text = "Carnal Desires"
  },
  PanelText_UI_Events_Panel_Try_Text_C_Fail_Content = {Text = "Incomplete"},
  PanelText_UI_Events_Panel_Try_Text_C_Summon_Content = {Text = "Awaken"},
  PanelText_UI_Events_Panel_Tulu_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Prayer, Dedication, Sacrifice, the profundity of humanity is merely a cacophonous farce to Him.
Yet this farce occasionally leaves traces, in the gaze of the God-King... or in His heart.]]
  },
  PanelText_UI_Events_Panel_Tulu_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Tulu_StageGroup_Text_C_Title_Content = {
    Text = "Gaze of the Abyss"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Tulu_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Challenge_Content = {
    Text = "Gaze of the Abyss"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Fill_Up_Content = {
    Text = "(Til Refill:"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Tulu_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Tulu_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Record_Content = {
    Text = "Day of Coronation"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Resonance_Content = {
    Text = "Resonance: Devour Mountains"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Shop_Content = {
    Text = "Abyssborn Epiphany"
  },
  PanelText_UI_Events_Panel_Tulu_Text_C_Topic_Content = {
    Text = "The Sovereign's Return"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_TwentyFour_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Challenge_Content = {
    Text = "Memory Code"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Fill_Up_Content = {
    Text = "(Til Maxed:"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Finish_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Not_unlocked_Content = {
    Text = "<Gray:Unlocked>"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Record_Content = {
    Text = "Serene Fission"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Resonance_Content = {
    Text = "Resonance: Dissociative Syndrome"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Shop_Content = {
    Text = "Ego Dissection"
  },
  PanelText_UI_Events_Panel_TwentyFour_Text_C_Topic_Content = {
    Text = "Manifold Healing: Rerun"
  },
  PanelText_UI_Events_Panel_Wanda_StageGroup_Text_C_Describe_Content = {
    Text = [[
Isarawu, City in the Lake, is said to be the homeland of Wanda.
This time you will pass through the Gate of Dreams and arrive at the ancient towers filled with clock tongues...]]
  },
  PanelText_UI_Events_Panel_Wanda_StageGroup_Text_C_Resonance_Content = {Text = "Resonance"},
  PanelText_UI_Events_Panel_Wanda_StageGroup_Text_C_Title_Content = {
    Text = "City in the Lake"
  },
  PanelText_UI_Events_Panel_Wanda_StageGroup_Text_C_illustrate_Content = {
    Text = "Gameplay Rules"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Can_Unlockable_Content = {Text = "Claimable:"},
  PanelText_UI_Events_Panel_Wanda_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Challenge_Content = {
    Text = "City in the Lake"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Fill_Up_Content = {
    Text = "(Till Refilled:"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Finished_Content = {Text = "Ended"},
  PanelText_UI_Events_Panel_Wanda_Text_C_Not_unlocked_Content = {Text = "Unlocked"},
  PanelText_UI_Events_Panel_Wanda_Text_C_Pv_Content = {Text = "Play Video"},
  PanelText_UI_Events_Panel_Wanda_Text_C_Receive_Content = {
    Text = "Till Claimable:"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Record_Content = {
    Text = "Feast of Flames"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Resonance_Content = {
    Text = "Resonance: Secret Order"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Shop_Content = {
    Text = "Wanda's Treasures"
  },
  PanelText_UI_Events_Panel_Wanda_Text_C_Topic_Content = {
    Text = "Feast of Flames"
  },
  PanelText_UI_Events_Panel_WorldBoss_Text_C_Challenge_Content = {
    Text = "Stellar Requiem"
  },
  PanelText_UI_Events_Panel_WorldBoss_Text_C_ProgressTitle_Content = {
    Text = "Stage Progress"
  },
  PanelText_UI_Events_Panel_WorldBoss_Text_C_Rank_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Events_Panel_WorldBoss_Text_C_Record_Content = {
    Text = "Special Ops"
  },
  PanelText_UI_Events_Panel_WorldBoss_Text_C_Topic_Content = {
    Text = "Final Gambit: Primal Shadow"
  },
  PanelText_UI_Events_Panel_Xu_StageGroup_Text_C_Introduction_Content = {
    Text = "From love is born resentment; from resentment, a thick and suffocating hate. The heavens remain silent to your pleas, and the earth offers no grave for escape. Only the laughter drifting through the Night Haze answers your call—and for this, she asks no price. Look upon her. Follow her. Dissolve into her. Become her."
  },
  PanelText_UI_Events_Panel_Xu_StageGroup_Text_C_Title_Content = {
    Text = "Obsidian Matriarch"
  },
  PanelText_UI_Events_Panel_Xu_Text_C_ChallengeTip_Content = {
    Text = "Limited-Time Event"
  },
  PanelText_UI_Events_Panel_Xu_Text_C_Challenge_Content = {
    Text = "Obsidian Matriarch"
  },
  PanelText_UI_Events_Panel_Xu_Text_C_RecordTip_Content = {
    Text = "Free Unlock Event"
  },
  PanelText_UI_Events_Panel_Xu_Text_C_Record_Content = {
    Text = "Psyche Deepdive"
  },
  PanelText_UI_Events_Panel_Xu_Text_C_Reward_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Events_Panel_Xu_Text_C_Shop_Content = {
    Text = "Desolate Offering"
  },
  PanelText_UI_Events_Panel_Xu_Text_C_Topic_Content = {
    Text = "Huangpu Whispers"
  },
  PanelText_UI_Events_Popup_ClotherLotteryRate_Text_C_Awards_Content = {Text = "Rewards"},
  PanelText_UI_Events_Popup_ClotherLotteryRate_Text_C_CurRate_Content = {
    Text = "Current Probability"
  },
  PanelText_UI_Events_Popup_ClotherLotteryRate_Text_C_InitRate_Content = {
    Text = "Initial Probability"
  },
  PanelText_UI_Events_Popup_ClotherLotteryRate_Text_C_Item_Content = {Text = "Rewards"},
  PanelText_UI_Events_Popup_ClotherLotteryRate_Text_C_Rate_Content = {
    Text = "Probability"
  },
  PanelText_UI_Events_Popup_GiftBag_Text_C_Desc_Content = {
    Text = "Event Description"
  },
  PanelText_UI_Events_Popup_GiftBag_Text_C_InfoTitle_Content = {
    Text = "Event Summary"
  },
  PanelText_UI_Events_Popup_GiftBag_Text_C_OptionWeapon_Content = {
    Text = "View All Optional Wheels of Destiny"
  },
  PanelText_UI_Events_Popup_GiftBag_Text_C_Slogan_Content = {
    Text = [[
24 Hours Only!
Choose 1 Standard Awakener!]]
  },
  PanelText_UI_Events_Popup_GiftBag_Text_C_SpecialTime_Content = {Text = "Remaining"},
  PanelText_UI_Events_Popup_GiftBag_Text_PayTip_Content = {
    Text = "The displayed price is for reference only. Actual payments will be processed in the applicable currency at the current exchange rate."
  },
  PanelText_UI_Events_Popup_Optional_Text_C_Name_1_Content = {
    Text = "View Details"
  },
  PanelText_UI_Events_Popup_Optional_Text_C_Name_2_Content = {Text = "Claim"},
  PanelText_UI_Events_Popup_Optional_Text_C_Title_Content = {
    Text = "WoD Selector"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_AssistAwaker_Content = {
    Text = "Support Awakener"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_AssistCount_Content = {
    Text = "Supports Count"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Assist_Content = {
    Text = "Support Ranking"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Empty_Assist_Content = {
    Text = "No records available"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Empty_Kill_Content = {
    Text = "No records available"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Information_Assist_Content = {
    Text = "Keeper Info"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Information_Kill_Content = {
    Text = "Keeper Info"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_KillCount_Content = {
    Text = "Total Capture Count"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Kill_Content = {
    Text = "Keeper Ranking"
  },
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Rank_Assist_Content = {Text = "Ranking"},
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Rank_Kill_Content = {Text = "Rank"},
  PanelText_UI_Events_Popup_WorldBossRank_Text_C_Title_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Events_RAID_StageGroup_Text_C_Introduction_Content = {
    Text = "Black, black, black...\nThe pitch-black monsters shriek, roar, and dance wildly, surging toward you like a tide.\n\"SPLISH—\"\n\"Ugh, this one barely had any Silver. Remember to bring more next time, cat.\""
  },
  PanelText_UI_Events_RAID_StageGroup_Text_C_Title_Content = {
    Text = "Tide of Silver Paws"
  },
  PanelText_UI_Events_Stage_Tip_Text_C_Title_Content = {
    Text = "Investigation Reward"
  },
  PanelText_UI_Events_TeamSchool_StageGroup_Text_C_Introduction_Content = {
    Text = [[
Black, the black tide surges endlessly.
The maddened cats surrounded you, rushing towards you from countless dimensions.]]
  },
  PanelText_UI_Events_TeamSchool_StageGroup_Text_C_Title_Content = {
    Text = "Cats Carnival"
  },
  PanelText_UI_Events_WorldBoss_StageGroup_Text_C_Introduction_Content = {
    Text = "Face your most primordial despair. Face your most primordial fear.  \nStand beside each other invisibly, and then defeat Him"
  },
  PanelText_UI_Events_WorldBoss_StageGroup_Text_C_Title_Content = {
    Text = "Stellar Requiem"
  },
  PanelText_UI_Events_illustrate_Tip_Text_C_Title_Content = {
    Text = "Awakener Description"
  },
  PanelText_UI_GuideNote_Item_Material_Text_C_Full_Content = {
    Text = "Materials Ready"
  },
  PanelText_UI_GuideNote_Item_Material_Text_C_Goto_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Item_Material_Text_C_Source_Content = {
    Text = "Where to Get"
  },
  PanelText_UI_GuideNote_Item_Material_Text_C_Sweep_Content = {Text = "Reenact"},
  PanelText_UI_GuideNote_Item_Task_Text_C_Recieve_Content = {Text = "Claim"},
  PanelText_UI_GuideNote_Item_Trinket_Text_C_Goto_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Item_Trinket_Text_C_Source_Content = {
    Text = "Where to Get"
  },
  PanelText_UI_GuideNote_Item_Trinket_Text_C_Sweep_Content = {Text = "Reenact"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_Change_Content = {Text = "Replace"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_Choose_Content = {
    Text = "Select Awakener"
  },
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_Detail_Content = {Text = "Details"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_ExcludeTalent_Content = {Text = "Talent"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_Level_Content = {Text = "Keeper EXP"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_More_Content = {Text = "More"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_NoTask_Content = {
    Text = "Select an Awakener"
  },
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_Skill_Content = {Text = "Skill"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_Talent_Content = {Text = "Talent"},
  PanelText_UI_GuideNote_Panel_AwakerStrength_Text_C_Time_Content = {
    Text = "Nothing here. Go look somewhere else?"
  },
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Goto_Abyss_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Goto_DailyChallenge_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Goto_Pvp_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Goto_RailyWay_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Goto_RotationMode_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Rank_Abyss_Content = {Text = "Rank"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Reward_Abyss_Content = {
    Text = "Remaining Rewards"
  },
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Reward_DailyChallenge_Content = {Text = "Rewards"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Reward_Pvp_Content = {Text = "Rewards"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Reward_RailyWay_Content = {Text = "Rewards"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Reward_RotationMode_Content = {Text = "Rewards"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Score_DailyChallenge_Content = {Text = "Points"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Score_Pvp_Content = {Text = "Points"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Score_RailyWay_Content = {
    Text = "This Season"
  },
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Score_RotationMode_Content = {Text = "Points"},
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Title_Abyss_Content = {
    Text = "D-Effect Zone"
  },
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Title_DailyChallenge_Content = {
    Text = "Phantasmal Dive"
  },
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Title_Pvp_Content = {
    Text = "Traphase - All Pick"
  },
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Title_RailyWay_Content = {
    Text = "Hectic Skybound Rail"
  },
  PanelText_UI_GuideNote_Panel_PermTrain_Text_C_Title_RotationMode_Content = {
    Text = "Traphase - Draft Pick"
  },
  PanelText_UI_GuideNote_PermTrainBase_Text_C_Goto_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Popup_ResSummary_Text_C_Goto_Content = {Text = "Proceed"},
  PanelText_UI_GuideNote_Popup_ResSummary_Text_C_ItemsPreview_Content = {
    Text = "Rewards to be Claimed:"
  },
  PanelText_UI_GuideNote_Popup_ResSummary_Text_C_Null_Content = {
    Text = "Congrats. You've Claimed Them All!"
  },
  PanelText_UI_GuideNote_Popup_ResSummary_Text_C_ProgressTitle_Content = {Text = "Progress"},
  PanelText_UI_GuideNote_Popup_ResSummary_Text_C_Title_Content = {
    Text = "Investigation"
  },
  PanelText_UI_Guide_ChangeName_Image_Confim_Dis_Bg_Content = {Text = "Engrave"},
  PanelText_UI_Guide_ChangeName_Text_C_Confim_Dis_Content = {Text = "Sign Name"},
  PanelText_UI_Guide_ChangeName_Text_C_Confim_Nor_Content = {Text = "Sign Name"},
  PanelText_UI_Guide_ChangeName_Text_C_Myself_Content = {Text = "Your Name"},
  PanelText_UI_Guide_ChangeName_Text_C_Return_Content = {
    Text = "Return to Select Gender"
  },
  PanelText_UI_Guide_ChangeName_Text_C_Tips_Content = {
    Text = "Please enter your name"
  },
  PanelText_UI_Guide_ChangeName_Text_Confim_Nor_Content = {Text = "Engrave"},
  PanelText_UI_Home_Panel_Main_Text_C_Title_Content = {
    Text = "Research Center"
  },
  PanelText_UI_Homeland_Item_Building_Text_C_Done_Content = {Text = "Full"},
  PanelText_UI_Homeland_Panel_Main_Text_C_Tips_Content = {
    Text = "Bonus Overview"
  },
  PanelText_UI_InvitationCode_Item_Player_Text_Name_Content = {Text = "Claim"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_BindingTip_Content = {
    Text = [[
Bind with a guiding Keeper.
In a universe shimmering with countless Silver Keys,
you are never alone.]]
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Binding_High_Content = {
    Text = "Bind Invitation"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Binding_Nor_Content = {
    Text = "Bind Invitation"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Clean_Content = {Text = "Clear"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Condition_Content = {
    Text = "Invited Keepers can receive rewards upon reaching Lv. 5/10/15/20."
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Copy_Content = {Text = "COPY"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Desc_Content = {
    Text = [[
Copy your Invitation Code.
Lead souls across the infinite universe on a journey of Dissolution.]]
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_InputTip_Content = {
    Text = "Enter Invitation Code to claim:"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_InviteCount_Content = {
    Text = "Invited Keepers"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Invite_High_Content = {
    Text = "Mythag Invite"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Invite_Nor_Content = {
    Text = "Mythag Invite"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_InvitedCount_Content = {Text = "300!"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_MyCode_Content = {
    Text = "My Invitation Code"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Paste_Content = {Text = "Paste"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Share_Content = {Text = "Share"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Tips_Content = {Text = "Enter Code"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_Value_Content = {Text = "Glimmer"},
  PanelText_UI_InvitationCode_Popup_Main_Text_C_illustrate_Content = {
    Text = "Event Description"
  },
  PanelText_UI_InvitationCode_Popup_Main_Text_Title_Content = {
    Text = "Mythag Enrollment Invite"
  },
  PanelText_UI_InvitationCode_Popup_PlayerList_Text_C_Null_Content = {
    Text = "No invited Keeper has reached Lv.5 yet"
  },
  PanelText_UI_InvitationCode_Popup_PlayerList_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_InvitationCode_Popup_PlayerList_Text_C_Title2_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_InvitationCode_Popup_PlayerList_Text_C_Title_Content = {
    Text = "Invites List"
  },
  PanelText_UI_Keeper_Panel_Acquire_Text_C_Acquire_Content = {
    Text = "New Posse Unlocked"
  },
  PanelText_UI_Keeper_Panel_Acquire_Text_C_Tips_Content = {
    Text = "Click anywhere to close"
  },
  PanelText_UI_Keeper_Skill_Item_Text_C_Current_Content = {Text = "Current"},
  PanelText_UI_Keeper_Skill_Item_Text_C_Disable_Content = {
    Text = "Unavailable"
  },
  PanelText_UI_Keeper_Skill_Panel_Text_C_Name_1_Content = {Text = "Select"},
  PanelText_UI_Keeper_Skill_Panel_Text_C_Name_2_Content = {Text = "Cancel"},
  PanelText_UI_Keeper_Skill_Panel_Text_C_NoTask_Content = {
    Text = "No Posse Available"
  },
  PanelText_UI_Keeper_Skill_Panel_Text_C_Title_Content = {Text = "Posse"},
  PanelText_UI_Keeper_Skill_Panel_Text_Time_Content = {
    Text = "Nothing here. Go look somewhere else?"
  },
  PanelText_UI_Login_Panel_ServerSelect_Text_Panel_Title_Content = {
    Text = "Server Selection"
  },
  PanelText_UI_Login_Popup_Tips_Text_C_Activity_Content = {
    Text = "Event Notice"
  },
  PanelText_UI_Login_Popup_Tips_Text_C_BugFix_Content = {
    Text = "Problem Fix"
  },
  PanelText_UI_Login_Popup_Tips_Text_C_Game_Content = {Text = "Notice"},
  PanelText_UI_Login_Popup_Tips_Text_C_Title2_Content = {
    Text = "Nothing here. Go look somewhere else?"
  },
  PanelText_UI_Login_Popup_Tips_Text_C_Title_Content = {
    Text = "Nothing here yet..."
  },
  PanelText_UI_Login_Popup_Tips_Text_Title_Content = {Text = "Notice"},
  PanelText_UI_Login_Text_C_Announcement_Content = {Text = "Notice"},
  PanelText_UI_Login_Text_C_Deal_Content = {Text = "Agreement"},
  PanelText_UI_Login_Text_C_Enter_Content = {
    Text = "Enter Mythag"
  },
  PanelText_UI_Login_Text_C_Fight_Content = {Text = "Battle 2.0"},
  PanelText_UI_Login_Text_C_Plot_Content = {
    Text = "Avg Story Test"
  },
  PanelText_UI_Login_Text_C_Quit_Content = {Text = "Exit"},
  PanelText_UI_Login_Text_C_Repair_Content = {Text = "Fix"},
  PanelText_UI_Login_Text_C_Service_Content = {
    Text = "Customer Support"
  },
  PanelText_UI_Login_Text_C_Set_Content = {Text = "Settings"},
  PanelText_UI_Login_Text_C_Setting_Content = {Text = "Settings"},
  PanelText_UI_Login_Text_C_Sweep_Content = {
    Text = "Scan QR Code"
  },
  PanelText_UI_Login_Text_C_Title1_Content = {Text = "Morimens"},
  PanelText_UI_Login_Text_C_User_Content = {
    Text = "User Center"
  },
  PanelText_UI_Mail_Text_C_All_Content = {Text = "Claim All"},
  PanelText_UI_Mail_Text_C_Collection_Content = {
    Text = "Saved Mails"
  },
  PanelText_UI_Mail_Text_C_Delete01_Content = {Text = "Delete"},
  PanelText_UI_Mail_Text_C_Delete_Content = {
    Text = "Delete Read"
  },
  PanelText_UI_Mail_Text_C_Empty_Content = {
    Text = "Inbox Empty"
  },
  PanelText_UI_Mail_Text_C_Mail_Content = {Text = "Inbox"},
  PanelText_UI_Mail_Text_C_Name_Content = {Text = "Claim"},
  PanelText_UI_Mail_Text_C_NoTask1_Content = {
    Text = "No mail available"
  },
  PanelText_UI_Mail_Text_C_Select_Letter_Content = {
    Text = "Select Mail"
  },
  PanelText_UI_Mail_Text_C_Time_Content = {Text = "Time:"},
  PanelText_UI_Mail_Text_C_Tips_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Mail_Text_C_Title_Content = {Text = "Mail Box"},
  PanelText_UI_Mail_Text_Time2_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Mail_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Main_Daily_Tips_Text_C_Close_Content = {
    Text = "Click anywhere to close"
  },
  PanelText_UI_Main_Daily_Tips_Text_C_Daily_Content = {
    Text = "Daily Quest"
  },
  PanelText_UI_Main_Daily_Tips_Text_C_Finish_Content = {
    Text = "Tasks completed today"
  },
  PanelText_UI_Main_Daily_Tips_Text_C_Receive_Content = {Text = "Claimed"},
  PanelText_UI_Main_Daily_Tips_Text_C_Reward_Content = {
    Text = "Completion Rewards"
  },
  PanelText_UI_Main_Daily_Tips_Text_C_Skill_Tag_Content = {
    Text = "Investigation Approach"
  },
  PanelText_UI_Main_Favor_Tip_Text_C_Name_Content = {Text = "Affinity"},
  PanelText_UI_Main_Helicopter_Tips_Text_C_Awaken_Content = {
    Text = "Awaken Date"
  },
  PanelText_UI_Main_Helicopter_Tips_Text_C_Favor_Content = {
    Text = "Current Affinity"
  },
  PanelText_UI_Main_Helicopter_Tips_Text_C_Tip_Content = {
    Text = "Invited Awakener will gain Affinity over time."
  },
  PanelText_UI_Main_Helicopter_Tips_Text_C_Title_Content = {Text = "Dormitory"},
  PanelText_UI_Main_Helicopter_Tips_Text_Name_1_Content = {Text = "Invite"},
  PanelText_UI_Main_Helicopter_Tips_Text_Name_2_Content = {Text = "Cancel"},
  PanelText_UI_Main_Helicopter_Tips_Text_ResetBg_Content = {
    Text = "Default Setting"
  },
  PanelText_UI_Main_Item_ActiveTasks_Text_C_Not_Content = {Text = "Incomplete"},
  PanelText_UI_Main_Item_ActiveTasks_Text_C_Proxy_Content = {Text = "Use Proxy"},
  PanelText_UI_Main_Item_ActiveTasks_Text_C_Receive_Content = {Text = "Claim"},
  PanelText_UI_Main_Panel_1_Text_Abyss_Content = {Text = "D-Zone"},
  PanelText_UI_Main_Panel_1_Text_C_Acclaim_Content = {
    Text = "Leave a Review"
  },
  PanelText_UI_Main_Panel_1_Text_C_Activity_Content = {Text = "Event"},
  PanelText_UI_Main_Panel_1_Text_C_Awaker_Content = {Text = "Awakener"},
  PanelText_UI_Main_Panel_1_Text_C_Bag_Content = {Text = "Inventory"},
  PanelText_UI_Main_Panel_1_Text_C_Close_1_Content = {Text = "Closing"},
  PanelText_UI_Main_Panel_1_Text_C_Close_Content = {Text = "Closing"},
  PanelText_UI_Main_Panel_1_Text_C_Collection_Content = {Text = "Library"},
  PanelText_UI_Main_Panel_1_Text_C_Community_Content = {Text = "Community"},
  PanelText_UI_Main_Panel_1_Text_C_Daily_Content = {
    Text = "Daily Quest"
  },
  PanelText_UI_Main_Panel_1_Text_C_Dbg_Content = {Text = "Interlude"},
  PanelText_UI_Main_Panel_1_Text_C_DungeonNote_Content = {Text = "Handbook"},
  PanelText_UI_Main_Panel_1_Text_C_Formation_Content = {Text = "Lineup"},
  PanelText_UI_Main_Panel_1_Text_C_Full_Content = {Text = "Completed"},
  PanelText_UI_Main_Panel_1_Text_C_GamePassport_Content = {Text = "Archives"},
  PanelText_UI_Main_Panel_1_Text_C_Lesson_Content = {Text = "Curriculum"},
  PanelText_UI_Main_Panel_1_Text_C_Main_Content = {Text = "NEXT"},
  PanelText_UI_Main_Panel_1_Text_C_Mission_Content = {Text = "Records"},
  PanelText_UI_Main_Panel_1_Text_C_Pvp_Content = {Text = "Traphase\n"},
  PanelText_UI_Main_Panel_1_Text_C_Recall_Content = {Text = "Arcade"},
  PanelText_UI_Main_Panel_1_Text_C_Recruit_Content = {Text = "Awaken"},
  PanelText_UI_Main_Panel_1_Text_C_Recruit_Nor_Content = {Text = "Awaken"},
  PanelText_UI_Main_Panel_1_Text_C_Store_Content = {
    Text = "Mythag Shop"
  },
  PanelText_UI_Main_Panel_1_Text_C_Welfare_Content = {
    Text = "Claim Various Limited-Time Rewards!"
  },
  PanelText_UI_Main_Panel_1_Text_Diving_Content = {Text = "P-Dive"},
  PanelText_UI_Main_Popup_ActiveTasks_Text_C_OneKey_Content = {
    Text = "Collect All"
  },
  PanelText_UI_Main_Popup_ActiveTasks_Text_C_Title_Content = {
    Text = "Training Tasks"
  },
  PanelText_UI_Michi_Popup_Strengthen_Succeed_Text_C_Info_Content = {
    Text = "Click anywhere to continue"
  },
  PanelText_UI_Other_Donate_Tips_Text_C_Consume_Content = {Text = "Consumes"},
  PanelText_UI_Other_Donate_Tips_Text_C_Donated_Content = {
    Text = "Total Donations"
  },
  PanelText_UI_Other_Donate_Tips_Text_C_Fund_Content = {
    Text = "Alumni Donation Fund"
  },
  PanelText_UI_Other_Donate_Tips_Text_C_Least_Content = {Text = "Min"},
  PanelText_UI_Other_Donate_Tips_Text_C_Title_Content = {
    Text = "Silver Prime Donation"
  },
  PanelText_UI_Other_Donate_Tips_Text_C_Utmost_Content = {Text = "Max"},
  PanelText_UI_Other_Fund_Tips_Text_C_Describe_Content = {
    Text = "Donate Silver Prime to accumulate contributions; your good deeds will be rewarded. Your \"Total Contribution Value\" is the sum of the donated Silver Prime."
  },
  PanelText_UI_Other_Fund_Tips_Text_C_Donated_Content = {
    Text = "Current Total Contribution"
  },
  PanelText_UI_Other_Fund_Tips_Text_C_Title_Content = {
    Text = "Alumni Donation Fund"
  },
  PanelText_UI_Other_Fund_Tips_Text_C_illustrate_Content = {Text = "Rules"},
  PanelText_UI_Other_Physical_Tips_Text_C_All_Content = {
    Text = "Full Recovery:"
  },
  PanelText_UI_Other_Physical_Tips_Text_C_Auto_Content = {
    Text = "Auto-Recovery"
  },
  PanelText_UI_Other_Physical_Tips_Text_C_Name_1_Content = {Text = "Cancel"},
  PanelText_UI_Other_Physical_Tips_Text_C_Name_2_Content = {Text = "Confirm"},
  PanelText_UI_Other_Physical_Tips_Text_C_Name_Content = {Text = "Cancel"},
  PanelText_UI_Other_Physical_Tips_Text_C_Next_Content = {
    Text = "Next Recovery:"
  },
  PanelText_UI_Other_Physical_Tips_Text_Name_Content = {Text = "Cancel"},
  PanelText_UI_Other_Popup_Key_Obtain_Text_C_Energy_Content = {
    Text = "Energy Consumption:"
  },
  PanelText_UI_Other_Popup_Key_Obtain_Text_C_Info_Content = {
    Text = "Click on the blank area to close"
  },
  PanelText_UI_Other_Popup_Key_Time_Revert_Text_C_Cost_Title_Next_Content = {
    Text = "Consume Silver Crystals"
  },
  PanelText_UI_Other_Popup_Key_Time_Revert_Text_C_Cost_Title_Now_Content = {
    Text = "Current Silver Crystals"
  },
  PanelText_UI_Other_Popup_Key_Time_Revert_Text_C_Question_Content = {
    Text = "Claim the Key now?"
  },
  PanelText_UI_Other_Popup_Key_Time_Revert_Text_C_Restore_Title_Next_Content = {
    Text = "Consume Key"
  },
  PanelText_UI_Other_Popup_Key_Time_Revert_Text_C_Restore_Title_Now_Content = {Text = "Keys Held"},
  PanelText_UI_Other_Popup_Key_Time_Revert_Text_C_Title_Content = {
    Text = "Key Acquisition"
  },
  PanelText_UI_Other_Popup_Trinket_LevelUp_Text_C_Success_Content = {Text = "Enhanced"},
  PanelText_UI_PVP_Item_RotationPlayer_Text_C_HP_Content = {Text = "HP"},
  PanelText_UI_PVP_Item_RotationPlayer_Text_C_Wait_Content = {
    Text = "Waiting for Draft Pick Info"
  },
  PanelText_UI_PVP_Item_RotationTeamInfo_Text_C_Attack_Content = {Text = "First Move"},
  PanelText_UI_PVP_Item_RotationTeamInfo_Text_C_Defend_Content = {Text = "Late Move"},
  PanelText_UI_PVP_Panel_Rotation_Adjust_Text_C_Adjust_Content = {
    Text = "Click to change Awakener positions"
  },
  PanelText_UI_PVP_Panel_Rotation_Adjust_Text_C_StandBy_My_Content = {Text = "Ready!"},
  PanelText_UI_PVP_Panel_Rotation_Adjust_Text_C_StandBy_Other_Content = {Text = "Ready!"},
  PanelText_UI_PVP_Panel_TeamRotation_Text_BtnName_Content = {Text = "Select"},
  PanelText_UI_Passport_Choose_Tip_Text_C_Title_Content = {
    Text = "Please select rewards"
  },
  PanelText_UI_Passport_Exchange_Text_C_Buy_Title_Content = {Text = "Level Up"},
  PanelText_UI_Passport_Exchange_Text_C_Cost_Tips_Content = {Text = "Consumes"},
  PanelText_UI_Passport_Exchange_Text_C_Title_Content = {
    Text = "Upgrade Topic Level"
  },
  PanelText_UI_Passport_Instructions_Text_C_Remainder_Content = {
    Text = "Remaining Time for This Period"
  },
  PanelText_UI_Passport_Instructions_Text_C_Title_Content = {Text = "Curriculum"},
  PanelText_UI_Passport_Money_Item_New_Text_C_Additional_Content = {Text = "Claim Now"},
  PanelText_UI_Passport_Money_Item_New_Text_C_Condition_Content = {
    Text = "Upgrade to Claim"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Hoist_1_Content = {Text = "+"},
  PanelText_UI_Passport_Money_Item_New_Text_C_Hoist_3_Content = {Text = "Lv."},
  PanelText_UI_Passport_Money_Item_New_Text_C_Hoist_4_Content = {
    Text = "Topic Level"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Hoist_Content = {
    Text = "Topic Level immediately <color=#5EF2FF>+<size=32>10</size></color>!"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Introduce_Content = {
    Text = "Unlock Advanced Topics and reach level 40 to select a Wheel of Destiny or Luminous Core ×5."
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Name_Content = {
    Text = "Extra Curriculum"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Privilege_1_Content = {
    Text = "Dailies\nRewards ×3"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Privilege_2_Content = {
    Text = [[
Combat
Undo Turns]]
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Privilege_3_Content = {
    Text = [[
Traphase
All Cards]]
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Privilege_Content = {
    Text = "Purchase to Unlock Privileges During This Curriculum:"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Prompt_Content = {
    Text = "Please enjoy responsibly and spend wisely."
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_Purchase_1_Content = {Text = "Purchased"},
  PanelText_UI_Passport_Money_Item_New_Text_C_Purchase_2_Content = {Text = "Purchased"},
  ["PanelText_UI_Passport_Money_Item_New_Text_C_Senior (1)_Content"] = {Text = "Advanced"},
  PanelText_UI_Passport_Money_Item_New_Text_C_Senior_Content = {
    Text = "Advanced Topics"
  },
  ["PanelText_UI_Passport_Money_Item_New_Text_C_Super (1)_Content"] = {Text = "Superior"},
  PanelText_UI_Passport_Money_Item_New_Text_C_Super_Content = {
    Text = "Premium Topics"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_TempUnlock_Content = {
    Text = "Premium Topics Exclusive"
  },
  PanelText_UI_Passport_Money_Item_New_Text_C_ThisIssue_Content = {
    Text = "Upgrade to Claim"
  },
  PanelText_UI_Passport_Money_Item_Text_C_Condition_Content = {
    Text = "Claimable from Topic Levels"
  },
  PanelText_UI_Passport_Money_Item_Text_C_Hoist_1_Content = {Text = "+"},
  PanelText_UI_Passport_Money_Item_Text_C_Hoist_3_Content = {Text = "Lv."},
  PanelText_UI_Passport_Money_Item_Text_C_Hoist_4_Content = {
    Text = "Topic Level, and obtain"
  },
  PanelText_UI_Passport_Money_Item_Text_C_Introduce_Content = {
    Text = "A finely crafted honor box containing a precious Wheel of Destiny, handmade.\nUnlock Advanced Topics and reach level 40 to select a Wheel of Destiny or Luminous Core ×5."
  },
  PanelText_UI_Passport_Money_Item_Text_C_Name_Content = {
    Text = "Curriculum Honor"
  },
  PanelText_UI_Passport_Money_Item_Text_C_Prompt_Content = {
    Text = "Please enjoy responsibly and spend wisely."
  },
  PanelText_UI_Passport_Money_Item_Text_C_Purchase_1_Content = {Text = "Purchased"},
  PanelText_UI_Passport_Money_Item_Text_C_Purchase_2_Content = {Text = "Purchased"},
  ["PanelText_UI_Passport_Money_Item_Text_C_Senior (1)_Content"] = {Text = "Advanced"},
  PanelText_UI_Passport_Money_Item_Text_C_Senior_Content = {
    Text = "Advanced Topics"
  },
  ["PanelText_UI_Passport_Money_Item_Text_C_Super (1)_Content"] = {Text = "Superior"},
  PanelText_UI_Passport_Money_Item_Text_C_Super_Content = {
    Text = "Premium Topics"
  },
  PanelText_UI_Passport_Money_Item_Text_C_ThisIssue_Content = {
    Text = "All Advanced Topics content"
  },
  PanelText_UI_Passport_Panel_Main_New_Text_C_Mail_Content = {Text = "Rules"},
  PanelText_UI_Passport_Panel_Main_New_Text_C_Title_Content = {Text = "Curriculum"},
  PanelText_UI_Passport_Panel_Main_Text_C_Experience_Content = {Text = "Topic EXP"},
  PanelText_UI_Passport_Panel_Main_Text_C_Full_Content = {
    Text = "Level Maxed"
  },
  PanelText_UI_Passport_Panel_Main_Text_C_Instructions_Grade_Content = {
    Text = "Topic Level"
  },
  PanelText_UI_Passport_Panel_Main_Text_C_Mail_Content = {Text = "Rules"},
  PanelText_UI_Passport_Panel_Main_Text_C_Name_Content = {Text = "Level Up"},
  PanelText_UI_Passport_Panel_Main_Text_C_Online_Content = {
    Text = "Weekly EXP Cap"
  },
  PanelText_UI_Passport_Panel_Main_Text_C_Title_Content = {Text = "Curriculum"},
  PanelText_UI_Passport_Popup_Player_LevelUp_Text_C_Title_Content = {
    Text = "Topic Level Up"
  },
  PanelText_UI_Passport_Popup_Recharge_Text_Cancel_Name_Content = {Text = "Cancel"},
  PanelText_UI_Passport_Popup_Recharge_Text_Confirm_Name_Content = {Text = "Confirm"},
  PanelText_UI_Passport_Popup_Recharge_Text_Title_Content = {
    Text = "Purchase Confirmation"
  },
  PanelText_UI_Passport_Praising_Item_Text_C_Content_Content = {
    Text = "A finely crafted honor box containing a precious Wheel of Destiny, handmade.\nUnlock Advanced Topics and reach level 40 to select a Wheel of Destiny or Luminous Core ×5."
  },
  PanelText_UI_Passport_Praising_Item_Text_C_Title_Content = {
    Text = "Curriculum Honor"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_Experience_Content = {Text = "Topic EXP"},
  PanelText_UI_Passport_Reward_Item_New_Text_C_Full_Content = {
    Text = "Level Maxed"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_GainExperience_Content = {Text = "Gain EXP"},
  PanelText_UI_Passport_Reward_Item_New_Text_C_Name_2_Content = {
    Text = "Purchase Level"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_Name_Content = {
    Text = "Collect All"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_Online_Content = {
    Text = "Weekly Limit"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_Ordinary_Title_Content = {
    Text = "Basic Topics"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_Senior_Content = {
    Text = "Extra Curriculum"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_Senior_Title_Content = {
    Text = "Advanced Topics"
  },
  PanelText_UI_Passport_Reward_Item_New_Text_C_Timing_Content = {Text = "This Issue"},
  PanelText_UI_Passport_Reward_Item_New_Text_C_Unlock_Content = {
    Text = "Unlock to Get:"
  },
  PanelText_UI_Passport_Reward_Item_Text_C_Experience_Content = {Text = "Topic EXP"},
  PanelText_UI_Passport_Reward_Item_Text_C_Full_Content = {
    Text = "Max Level Reached"
  },
  PanelText_UI_Passport_Reward_Item_Text_C_Name_2_Content = {
    Text = "Purchase Level"
  },
  PanelText_UI_Passport_Reward_Item_Text_C_Name_Content = {Text = "Claim All"},
  PanelText_UI_Passport_Reward_Item_Text_C_Online_Content = {
    Text = "This Week's Limit"
  },
  PanelText_UI_Passport_Reward_Item_Text_C_Ordinary_Title_Content = {
    Text = "Basic Topics"
  },
  PanelText_UI_Passport_Reward_Item_Text_C_Senior_Content = {
    Text = "Extra curriculum"
  },
  PanelText_UI_Passport_Reward_Item_Text_C_Senior_Title_Content = {
    Text = "Advanced Topics"
  },
  PanelText_UI_Passport_Reward_Item_Text_C_Tab_Special_Content = {Text = "Advanced"},
  PanelText_UI_Passport_Reward_Item_Text_C_Timing_Content = {
    Text = "Remaining Time for This Period"
  },
  PanelText_UI_Passport_Show_Item_Text_C_Introduce_Content = {
    Text = "Unlock Advanced Topics and reach level 40 to select a Wheel of Destiny or Luminous Core ×5."
  },
  PanelText_UI_Passport_Show_Item_Text_C_Reward_Content = {
    Text = "Current Gift Box Preview"
  },
  PanelText_UI_Passport_Show_Item_Text_C_Time_1_Content = {
    Text = "Event Time Remaining"
  },
  PanelText_UI_Passport_Task_Choose_Text_C_Double_Content = {Text = "Double EXP"},
  PanelText_UI_Passport_Task_Choose_Text_C_Tab_Special_Content = {Text = "Event"},
  PanelText_UI_Passport_Task_Description_Text_C_Name_Content = {Text = "Claim"},
  PanelText_UI_Passport_Task_Description_Text_C_Tab_Special_Content = {Text = "Event"},
  PanelText_UI_Passport_Task_Item_Text_C_Experience_Content = {Text = "Topic EXP"},
  PanelText_UI_Passport_Task_Item_Text_C_Full_Content = {
    Text = "Max Level Reached"
  },
  PanelText_UI_Passport_Task_Item_Text_C_Name_3_Content = {
    Text = "Purchase Level"
  },
  PanelText_UI_Passport_Task_Item_Text_C_Name_Content = {
    Text = "Collect All"
  },
  PanelText_UI_Passport_Task_Item_Text_C_Online_Content = {
    Text = "This Week's Limit"
  },
  PanelText_UI_Passport_Task_Item_Text_C_Time_Content = {
    Text = "Remaining Time:"
  },
  PanelText_UI_Passport_illustrate_Tip_Text_C_Title_Content = {Text = "Rules"},
  PanelText_UI_Pocket_Mail_Text_C_Del_Content = {Text = "Delete"},
  PanelText_UI_Pocket_Mail_Text_C_Time_Content = {Text = "Time:"},
  PanelText_UI_Pocket_Mail_Text_C_Tips_Content = {
    Text = "Number of Collections"
  },
  PanelText_UI_Pocket_Mail_Text_C_Title2_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Pocket_Mail_Text_C_Title_Content = {
    Text = "No mail available"
  },
  PanelText_UI_Pocket_Mail_Text_C_select_Title_Content = {
    Text = "Select a Letter"
  },
  PanelText_UI_Popup_DonationBox_Text_C_Reward_Content = {Text = "Inventory"},
  PanelText_UI_Popup_DonationBox_Text_C_SpeedTitle_Content = {
    Text = "Acquisition Speed:"
  },
  PanelText_UI_Protagonist_Item_Skill_Text_C_Current_Content = {Text = "Current"},
  PanelText_UI_Protagonist_Item_Skill_Text_C_No_Content = {
    Text = "Not Obtained"
  },
  PanelText_UI_Pvp_Blood_Share_Text_C_Release_Content = {Text = "Exalt"},
  PanelText_UI_Pvp_Item_Card_Text_C_Replace_Content = {Text = "Replace"},
  PanelText_UI_Pvp_Item_Collection_Comment_Text_C_Done_Content = {
    Text = "Followed Me"
  },
  PanelText_UI_Pvp_Item_Collection_Comment_Text_C_Mutual_Content = {Text = "Mutual"},
  PanelText_UI_Pvp_Item_Dan_Text_C_Current_Content = {
    Text = "Current Tier"
  },
  PanelText_UI_Pvp_Item_Formation_Text_C_Conflict_Content = {
    Text = "Position Conflict"
  },
  PanelText_UI_Pvp_Item_List_Text_C_Uid_Title_Content = {Text = "UID:"},
  PanelText_UI_Pvp_Item_Out_Obtain_Text_C_Tab_Double_Content = {
    Text = "Double Goods"
  },
  PanelText_UI_Pvp_Item_Out_Obtain_Text_C_Tab_Topic_Content = {
    Text = "Research Privileges"
  },
  PanelText_UI_Pvp_Item_Ranking_Text_C_Current_Content = {
    Text = "Current Rewards"
  },
  PanelText_UI_Pvp_Item_Role_Text_C_Decks_Content = {Text = "Deck"},
  PanelText_UI_Pvp_Item_Role_Text_C_Maniac_Content = {Text = "Aliemus"},
  PanelText_UI_Pvp_Item_Role_Text_C_Role_Wins_Content = {Text = "Wins"},
  PanelText_UI_Pvp_Panel_Battle_Text_High_Content = {Text = "Surrender"},
  PanelText_UI_Pvp_Panel_Collection_Comment_Text_C_NoTask_Content = {
    Text = "No comments yet"
  },
  PanelText_UI_Pvp_Panel_Collection_Comment_Text_Time_Content = {
    Text = "Make the first comment"
  },
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Decks_Content = {Text = "Deck"},
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Effect_Content = {Text = "Effect"},
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Hoist_Content = {
    Text = "Arithmetica Cost"
  },
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Maniac_Content = {Text = "Aliemus"},
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Role_TotalWins_Content = {Text = "Total Wins"},
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Role_Wins_Content = {
    Text = "Monthly Wins"
  },
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Skill_Tag_Content = {Text = "Exalt"},
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Source_Content = {Text = "Source"},
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_TotalWins_Content = {Text = "Total Wins"},
  PanelText_UI_Pvp_Panel_Collection_Detail_Text_C_Wins_Content = {
    Text = "Season Wins"
  },
  PanelText_UI_Pvp_Panel_Collection_List_Text_C_None_Content = {
    Text = "No related collections yet"
  },
  PanelText_UI_Pvp_Panel_Collection_List_Text_C_Time_Content = {
    Text = "This Realm is not yet available for Traphase."
  },
  PanelText_UI_Pvp_Panel_Collection_Main_Text_C_Awaker_Name_Content = {Text = "Awakener"},
  PanelText_UI_Pvp_Panel_Collection_Main_Text_C_KeeperSkill_Name_Content = {Text = "Posse"},
  PanelText_UI_Pvp_Panel_Collection_Main_Text_C_Toggle_1_Content = {Text = "Details"},
  PanelText_UI_Pvp_Panel_Collection_Main_Text_C_Toggle_2_Content = {Text = "Ranking"},
  PanelText_UI_Pvp_Panel_Collection_Main_Text_C_Toggle_3_Content = {Text = "Comment"},
  PanelText_UI_Pvp_Panel_Collection_Main_Text_C_Train_Name_Content = {
    Text = "Awakener trial"
  },
  PanelText_UI_Pvp_Panel_Collection_Main_Text_C_Weapon_Name_Content = {
    Text = "Wheel of Destiny"
  },
  PanelText_UI_Pvp_Panel_Collection_Rank_Text_C_NoTask_Content = {Text = "No Records"},
  PanelText_UI_Pvp_Panel_Collection_Rank_Text_C_SeasonWins_Content = {
    Text = "Season Wins"
  },
  PanelText_UI_Pvp_Panel_Collection_Rank_Text_C_TotalWins_Content = {Text = "Total Wins"},
  PanelText_UI_Pvp_Panel_Detail_Text_C_Decks_Content = {Text = "Deck"},
  PanelText_UI_Pvp_Panel_Detail_Text_C_Effect_Content = {Text = "Effect"},
  PanelText_UI_Pvp_Panel_Detail_Text_C_Maniac_Content = {Text = "Aliemus"},
  PanelText_UI_Pvp_Panel_Detail_Text_C_Role_Wins_Content = {Text = "Wins"},
  PanelText_UI_Pvp_Panel_Detail_Text_C_Wins_Content = {Text = "Wins"},
  PanelText_UI_Pvp_Panel_Formation_Text_C_Detail_Content = {
    Text = "Please select from the left to enter the field."
  },
  PanelText_UI_Pvp_Panel_Formation_Text_C_Empty_1_Content = {
    Text = "Please select from the left to enter the field."
  },
  PanelText_UI_Pvp_Panel_Formation_Text_C_Empty_Content = {
    Text = "Select from the left"
  },
  PanelText_UI_Pvp_Panel_Formation_Text_C_Name_Content = {Text = "Details"},
  PanelText_UI_Pvp_Panel_Formation_Text_C_None_Content = {
    Text = "No relevant filter results"
  },
  PanelText_UI_Pvp_Panel_Formation_Text_C_Time_Content = {
    Text = "Can be obtained through specific methods"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Award_Content = {Text = "Rewards"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_CardEdit_Content = {Text = "Teams"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Close_Robot_Content = {Text = "Close"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Close_Speed_Content = {Text = "Close"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Collection_Content = {
    Text = "Collections"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_DraftPick_Content = {
    Text = "Draft Pick Lv."
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Expression_Content = {Text = "Emotes"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Integral_Content = {
    Text = "Season Points"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Matching_Content = {
    Text = "Start Matching"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Open_Robot_Content = {Text = "Open"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Open_Speed_Content = {Text = "Open"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Password_Content = {
    Text = "Password Battle"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Precon_Content = {
    Text = "All Pick Lv."
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Progress_Content = {
    Text = "Rank Rewards"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Pvp_Content = {Text = "All Pick"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Pvp_Score_Content = {Text = "PTS"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Ranking_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Record_Content = {Text = "Records"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Reward_Content = {
    Text = "Reach higher ranks to earn more rewards."
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Robot_Content = {
    Text = "Simulated Rival"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_RotationMode_Content = {Text = "Draft Pick"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_RotationMode_Score_Content = {Text = "PTS"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Rule_Content = {Text = "Rules"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Speed_Content = {
    Text = "Precise Match"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Store_Content = {Text = "Store"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Time_Content = {
    Text = "Current Season:"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Title_Content = {Text = "Traphase"},
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Victory_Content = {
    Text = "Total Wins:"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_Weekly_Content = {
    Text = "Weekly Rewards"
  },
  PanelText_UI_Pvp_Panel_Main_1_Text_C_list_Content = {Text = "Ranking"},
  PanelText_UI_Pvp_Panel_Main_Text_C_Name_Content = {Text = "End"},
  PanelText_UI_Pvp_Panel_Main_Text_C_Unclickable_Content = {Text = "End"},
  PanelText_UI_Pvp_Panel_Main_Text_C_Zongshanghai_Content = {
    Text = "Total Damage"
  },
  PanelText_UI_Pvp_Panel_Match_Text_C_Second_Content = {Text = "Second"},
  PanelText_UI_Pvp_Panel_Match_Text_C_Win_Content = {Text = "Game On!"},
  PanelText_UI_Pvp_Panel_Match_Text_Name_Content = {
    Text = "Start Matching"
  },
  PanelText_UI_Pvp_Panel_Out_Text_C_Huodewuzi_Content = {
    Text = "Rewards Settlement"
  },
  PanelText_UI_Pvp_Panel_Out_Text_C_Integral_Content = {
    Text = "Season Points"
  },
  PanelText_UI_Pvp_Panel_Out_Text_C_Show_Content = {Text = "Next Step"},
  PanelText_UI_Pvp_Panel_Out_Text_C_Sync_Content = {
    Text = "Awakener Affinity Increase"
  },
  PanelText_UI_Pvp_Panel_Playback_Text_C_Angle_Content = {
    Text = "Switch View Angle"
  },
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Attention_1_Content = {Text = "Follow"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Attention_2_Content = {Text = "Follow"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Commend_1_Content = {Text = "Praise"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Commend_2_Content = {Text = "Praised"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Enemy_Content = {Text = "Enemy"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Fail_Content = {Text = "Failure"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Harm_Content = {Text = "Dmg"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Ourside_Content = {Text = "You"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Outcome_Content = {Text = "Victory"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_OwnRespectable_Content = {
    Text = "Honorable opponent!"
  },
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Point_01_Content = {Text = "Points"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Point_02_Content = {Text = "Points"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Respectable_Content = {
    Text = "Honorable opponent!"
  },
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Shields_Content = {Text = "Shield"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Treat_Content = {Text = "Heal"},
  PanelText_UI_Pvp_Panel_Settlement_Text_C_Victory_Content = {Text = "Victory"},
  PanelText_UI_Pvp_Panel_Train_Text_C_KeeperSkills_Content = {Text = "Posse"},
  PanelText_UI_Pvp_Panel_Train_Text_C_Position_Content = {Text = "Shift"},
  PanelText_UI_Pvp_Panel_Train_Text_C_Reset_Content = {Text = "Reset"},
  PanelText_UI_Pvp_Panel_Train_Text_C_SelectRelic_Content = {Text = "Relic"},
  PanelText_UI_Pvp_Panel_Train_Text_C_Weapons_Content = {Text = "Wheel"},
  PanelText_UI_Pvp_Panel_lineup_Show_Text_C_Point_01_Content = {Text = "Points"},
  PanelText_UI_Pvp_Panel_lineup_Show_Text_C_Point_02_Content = {Text = "Points"},
  PanelText_UI_Pvp_Popup_Breaking_Text_C_Breaking_Content = {
    Text = "Disconnected from the server"
  },
  PanelText_UI_Pvp_Popup_Breaking_Text_C_Quit_Content = {
    Text = "Return to the Login Screen"
  },
  PanelText_UI_Pvp_Popup_Collection_Detail_Text_C_Decks_Content = {Text = "Deck"},
  PanelText_UI_Pvp_Popup_Collection_Detail_Text_C_Effect_Content = {Text = "Effect"},
  PanelText_UI_Pvp_Popup_Collection_Detail_Text_C_Maniac_Content = {Text = "Aliemus"},
  PanelText_UI_Pvp_Popup_Collection_Detail_Text_C_Role_Wins_Content = {Text = "Wins"},
  PanelText_UI_Pvp_Popup_Collection_Detail_Text_C_Wins_Content = {Text = "Wins"},
  PanelText_UI_Pvp_Popup_Dan_Text_C_Bonus_Content = {
    Text = "Season Bonus"
  },
  PanelText_UI_Pvp_Popup_Dan_Text_C_Continue_Content = {Text = "Continue"},
  PanelText_UI_Pvp_Popup_Dan_Text_C_Streak_Content = {
    Text = "Winning Streak Bonus"
  },
  PanelText_UI_Pvp_Popup_Enlightenment_Adjust_Text_C_Title_Content = {Text = "Enlighten"},
  PanelText_UI_Pvp_Popup_Enlightenment_Adjust_Text_C_Use_Content = {
    Text = "Currently in use"
  },
  PanelText_UI_Pvp_Popup_Enlightenment_Tips_Text_C_Title_Content = {Text = "Enlighten"},
  PanelText_UI_Pvp_Popup_Expression_Manage_Text_C_Emoji_Content = {
    Text = "Manage Emotes"
  },
  PanelText_UI_Pvp_Popup_Expression_Manage_Text_C_Emoji_Overview_Content = {Text = "All Emotes"},
  PanelText_UI_Pvp_Popup_Expression_Manage_Text_C_Emoji_Overview_Sel_Content = {Text = "All Emotes"},
  PanelText_UI_Pvp_Popup_Expression_Manage_Text_C_Emoji_Sel_Content = {
    Text = "Manage Emotes"
  },
  PanelText_UI_Pvp_Popup_Expression_Manage_Text_C_Title_Content = {
    Text = "Emotes Management"
  },
  PanelText_UI_Pvp_Popup_Expression_Manage_Text_C_Use_Content = {Text = "Emotes"},
  PanelText_UI_Pvp_Popup_Fail_Text_C_Close_Content = {
    Text = "Click anywhere to continue"
  },
  PanelText_UI_Pvp_Popup_Fail_Text_C_Continue_Content = {Text = "Continue"},
  PanelText_UI_Pvp_Popup_Fail_Text_C_Fail_Content = {Text = "Failure"},
  PanelText_UI_Pvp_Popup_Fail_Text_C_Failure_Content = {
    Text = "You Lost..."
  },
  PanelText_UI_Pvp_Popup_Fail_Text_C_Victory_Content = {
    Text = "Battle Failed"
  },
  PanelText_UI_Pvp_Popup_Fail_Text_Reward_1_Content = {
    Text = "Battle Style Rewards"
  },
  PanelText_UI_Pvp_Popup_Fight_Text_C_Create_Content = {
    Text = "Create Battle"
  },
  PanelText_UI_Pvp_Popup_Fight_Text_C_Joinin_Content = {
    Text = "Join Battle"
  },
  PanelText_UI_Pvp_Popup_Fight_Text_Title_Content = {
    Text = "Password Battle"
  },
  PanelText_UI_Pvp_Popup_ImportTeam_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Pvp_Popup_ImportTeam_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_UI_Pvp_Popup_ImportTeam_Text_C_Title_Content = {
    Text = "Import Lineup"
  },
  PanelText_UI_Pvp_Popup_PasswordCode_Placeholder_Content = {
    Text = "Enter a 6-digit <color=#5EF2FF>Password</color>"
  },
  PanelText_UI_Pvp_Popup_PasswordCode_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Pvp_Popup_PasswordCode_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_UI_Pvp_Popup_PasswordCode_Text_Title_Content = {
    Text = "Input Password"
  },
  PanelText_UI_Pvp_Popup_PasswordMatch_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Pvp_Popup_PasswordMatch_Text_C_Copy_Content = {
    Text = "Copy Password"
  },
  PanelText_UI_Pvp_Popup_PasswordMatch_Text_C_Wait_Content = {Text = "Waiting..."},
  PanelText_UI_Pvp_Popup_PasswordMatch_Text_Title_Content = {
    Text = "Password Battle"
  },
  PanelText_UI_Pvp_Popup_PasswordReady_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Pvp_Popup_PasswordReady_Text_C_Done_Content = {
    Text = "Followed me"
  },
  PanelText_UI_Pvp_Popup_PasswordReady_Text_C_Mutual_Content = {Text = "Mutual"},
  PanelText_UI_Pvp_Popup_PasswordReady_Text_C_Ready_Content = {Text = "Prepare"},
  PanelText_UI_Pvp_Popup_PasswordReady_Text_C_Reject_Content = {Text = "Reject"},
  PanelText_UI_Pvp_Popup_PasswordReady_Text_C_Wait_Content = {
    Text = "Waiting for the opponent..."
  },
  PanelText_UI_Pvp_Popup_PlaybackAngle_Text_C_Copy_Content = {Text = "Copy"},
  PanelText_UI_Pvp_Popup_PlaybackAngle_Text_Name_Content = {
    Text = "Watch Replay"
  },
  PanelText_UI_Pvp_Popup_PlaybackAngle_Text_Title_Content = {Text = "View Angle"},
  PanelText_UI_Pvp_Popup_Replacement_Text_C_Initial_Content = {
    Text = "Starting Hand"
  },
  PanelText_UI_Pvp_Popup_Replacement_Text_C_Prompt_Content = {
    Text = "Click to replace unwanted cards"
  },
  PanelText_UI_Pvp_Popup_Reward_Tips_Text_C_Title_Content = {
    Text = "Rank Rewards"
  },
  PanelText_UI_Pvp_Popup_SelectCreation_Text_C_Confirm_Content = {Text = "Confirm"},
  PanelText_UI_Pvp_Popup_SelectCreation_Text_C_Title_Content = {
    Text = "Choose 1 Relic"
  },
  PanelText_UI_Pvp_Popup_Settle_Text_C_Close_Content = {
    Text = "Click anywhere to continue"
  },
  PanelText_UI_Pvp_Popup_Settle_Text_C_Succeed_Content = {Text = "Victory!"},
  PanelText_UI_Pvp_Popup_Tips2_Text_C_Highest_Content = {
    Text = "Highest Points"
  },
  PanelText_UI_Pvp_Popup_Tips2_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Pvp_Popup_Tips2_Text_C_Title_Content = {Text = "Records"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Activity1_Content = {Text = "Rewards"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Activity2_Content = {Text = "Rankings"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Activity3_Content = {Text = "Rankings"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Activity4_Content = {Text = "Rewards"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Award_Content = {Text = "Rewards"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Big_Content = {
    Text = "Keeper Info"
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_Game1_Content = {
    Text = "Weekly Ranking"
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_Game2_Content = {
    Text = "Weekly Ranking"
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_Grade_Content = {Text = "Rank"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Hint_Content = {
    Text = "Win one match this season to receive settlement rewards at the end of the season."
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_RankTip_Content = {
    Text = "Participate in one match this season to enter the Seasonal Leaderboard."
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_Rank_Pvp_Content = {Text = "All Pick"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Rank_RotationMode_Content = {Text = "Draft Pick"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Reward_Content = {
    Text = "Season High Score"
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_Scope_Content = {Text = "Rank Range"},
  PanelText_UI_Pvp_Popup_Tips_Text_C_Small_Content = {
    Text = "Traphase Title"
  },
  PanelText_UI_Pvp_Popup_Tips_Text_C_Title_2_Content = {
    Text = "Traphase Title"
  },
  PanelText_UI_Pvp_Popup_Tips_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Pvp_Popup_Victory_Text_C_Fail_Content = {Text = "Failure"},
  PanelText_UI_Pvp_Popup_Victory_Text_C_Red_Content = {Text = "Continue"},
  PanelText_UI_Pvp_Popup_Victory_Text_C_Victory_Content = {Text = "Victory"},
  PanelText_UI_Pvp_Popup_Victory_Text_C_Yellow_Content = {Text = "Continue"},
  PanelText_UI_Pvp_Popup_list_Tips_Text_C_Title_Content = {
    Text = "Leaderboard"
  },
  PanelText_UI_Pvp_Popup_list_Tips_Text_C_Uid_Title_Content = {Text = "UID:"},
  PanelText_UI_Pvp_Team_Main_Text_C_Name_10_Content = {
    Text = "Equip Wheel of Destiny"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Name_1_Content = {
    Text = "Equip Posse"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Name_2_Content = {Text = "Slot 4"},
  PanelText_UI_Pvp_Team_Main_Text_C_Name_3_Content = {Text = "Slot 3"},
  PanelText_UI_Pvp_Team_Main_Text_C_Name_4_Content = {Text = "Slot 2"},
  PanelText_UI_Pvp_Team_Main_Text_C_Name_5_Content = {Text = "Slot 1"},
  PanelText_UI_Pvp_Team_Main_Text_C_Name_6_Content = {
    Text = "Equip Posse"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Name_7_Content = {
    Text = "Equip Wheel of Destiny"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Name_8_Content = {
    Text = "Equip Wheel of Destiny"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Name_9_Content = {
    Text = "Equip Wheel of Destiny"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Name_Content = {
    Text = "Quick Lineup"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Realms_Content = {
    Text = "Realm Not Activated"
  },
  PanelText_UI_Pvp_Team_Main_Text_C_Title_Content = {
    Text = "Preset Lineup"
  },
  PanelText_UI_Pvp_Team_Main_Text_Name_Content = {
    Text = "Quick Lineup"
  },
  PanelText_UI_Pvp_Tip_Comment_Text_C_Copy_Content = {Text = "Copy"},
  PanelText_UI_Pvp_Tip_Comment_Text_C_Done_Content = {
    Text = "Followed Me"
  },
  PanelText_UI_Pvp_Tip_Comment_Text_C_Mutual_Content = {Text = "Mutual"},
  PanelText_UI_Pvp_Tip_Comment_Text_C_Proficiency_Content = {Text = "Total Wins"},
  PanelText_UI_Pvp_Tip_Comment_Text_C_Remove_Content = {Text = "Edit"},
  PanelText_UI_Pvp_Tip_Comment_Text_C_Save_Content = {Text = "Delete"},
  PanelText_UI_Pvp_Tip_Comment_Text_C_Title_Content = {
    Text = "Comment Details"
  },
  PanelText_UI_Recharge_Courtesy_Item_Text_C_Additional_Content = {Text = "Bonus"},
  PanelText_UI_Recharge_Courtesy_Item_Text_C_DiscountMultipleTitle_Content = {Text = "VALUE"},
  PanelText_UI_Recharge_Courtesy_Item_Text_C_Haved_Content = {
    Text = "Already Owned"
  },
  PanelText_UI_Recharge_Courtesy_Item_Text_C_Sellout_Content = {Text = "Sold Out"},
  PanelText_UI_Recharge_Exchange_Text_C_Buy_Title_Content = {
    Text = "Silvers Redeemed"
  },
  PanelText_UI_Recharge_Exchange_Text_C_Cost_Tips_Content = {Text = "Consumes"},
  PanelText_UI_Recharge_Exchange_Text_C_Least_Content = {Text = "Min"},
  PanelText_UI_Recharge_Exchange_Text_C_Redeemed_Content = {Text = "Total"},
  PanelText_UI_Recharge_Exchange_Text_C_Utmost_Content = {Text = "Max"},
  PanelText_UI_Recharge_Exchange_Tips_Text_C_Not_Content = {
    Text = "Insufficient Currency"
  },
  PanelText_UI_Recharge_Exchange_Tips_Text_C_Title_Content = {Text = "Confirm"},
  PanelText_UI_Recharge_Exchange_Tips_Text_C_illustrate_Content = {Text = "Rules"},
  PanelText_UI_Recharge_Instructions_Text_C_Title_Content = {Text = "Curriculum"},
  PanelText_UI_Recharge_Instructions_Text_Name_Content = {Text = "Proceed"},
  PanelText_UI_Recharge_OnekeyPurchase_Text_C_Cost_Tips_Content = {Text = "Total"},
  PanelText_UI_Recharge_OnekeyPurchase_Text_C_Tips_Content = {
    Text = "You can deselect items you don't want."
  },
  PanelText_UI_Recharge_OnekeyPurchase_Text_C_Title_Content = {
    Text = "Confirmation"
  },
  PanelText_UI_Recharge_Panel_Extract_Text_C_Buttom_Content = {
    Text = "First purchase of each tier grants an equal amount of bonus \"Silver Prime,\" one time only"
  },
  PanelText_UI_Recharge_Panel_Extract_Text_C_Law1_Content = {
    Text = "Special Sales"
  },
  PanelText_UI_Recharge_Panel_Extract_Text_C_Law2_Content = {
    Text = "Fund Settlement Law"
  },
  PanelText_UI_Recharge_Panel_Extract_Text_C_Refresh_Content = {
    Text = "Customer Support"
  },
  PanelText_UI_Recharge_Panel_Extract_Text_C_Title_Content = {
    Text = "Prime Extraction"
  },
  PanelText_UI_Recharge_Panel_Extract_Text_PayTip_Content = {
    Text = "The displayed price is for reference only. Actual payments will be processed in the applicable currency at the current exchange rate."
  },
  PanelText_UI_Recharge_Panel_GiftBag_Text_C_Desc_Content = {
    Text = "Event Description"
  },
  PanelText_UI_Recharge_Panel_GiftBag_Text_C_Slogan_Content = {
    Text = [[
24 Hours Only!
Choose 1 Standard Awakener!]]
  },
  PanelText_UI_Recharge_Panel_GiftBag_Text_C_SpecialTime_Content = {Text = "Remaining"},
  PanelText_UI_Recharge_Panel_GiftBag_Text_PayTip_Content = {
    Text = "The displayed price is for reference only. Actual payments will be processed in the applicable currency at the current exchange rate."
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Bigitle_Content = {
    Text = "Moonphase Vigil"
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Buttom_Tips_Content = {
    Text = "Get double the amount of Silver. One-time offer."
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Cost_Tips_Content = {Text = "Consumes"},
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Expired_Content = {Text = "Expired"},
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Flushed_Content = {Text = "Refresh"},
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Get1_Title_Content = {Text = "Obtain Now"},
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Get2_Title_Content = {Text = "Daily:"},
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Illustrate_Content = {
    Text = "This item may be purchased multiple times."
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_OnekeyPurchase_Content = {
    Text = "Purchase All"
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Refresh_Content = {
    Text = "Customer Support"
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Tips_Content = {
    Text = "300 + 3000 Silver in total!"
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_C_Today_Content = {
    Text = "Claimed Today"
  },
  PanelText_UI_Recharge_Panel_Main_Optimized_Text_Get2_Title2_Content = {Text = "(30d)"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Bigitle_Content = {
    Text = "Moonphase Vigil"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Bigtitle_Content = {
    Text = "Moonphase Vigil"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Buttom_Tips_Content = {
    Text = "Get bonus Silver equal to your top-up amount for each first-time tier purchase. (One-time offer per tier)"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Buy_Content = {Text = "Purchase"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Cost_Tips_Content = {Text = "Consumes"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Double_Content = {Text = "Extra"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Expired_Content = {Text = "Expired"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Extra_Content = {Text = "Bonus"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Flushed_Content = {Text = "Refresh"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Get1_NumName_Content = {
    Text = "Silver Prime"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Get1_Title_Content = {Text = "Obtain Now"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Get2_NumName_Content = {Text = "Silver"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Get2_Title2_Content = {Text = "(30d)"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Get2_Title_Content = {Text = "Daily:"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Illustrate_Content = {
    Text = "Multiple purchase available!"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Need_Content = {Text = "Consumes"},
  PanelText_UI_Recharge_Panel_Main_Text_C_OnekeyPurchase_Content = {
    Text = "Purchase All"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Refresh_Content = {
    Text = "Customer Support"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_RewardTip1_Content = {
    Text = "Log in daily to claim\nSilver <color=#57F8FE><size=38>×6000</size></color>!"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_RewardTip2_Content = {
    Text = [[
Login daily to claim
<color=#57F8FE><size=38>6000</size></color> Menophin and<color=#57F8FE><size=38>600</size></color> Remembrance Quill!]]
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Stamina_Get1_Title_Content = {Text = "Claim Now"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Stamina_Illustrate_Content = {
    Text = "Gameplay Rules"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Stamina_Need_Content = {Text = "Requires"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Stamina_Tips_Content = {
    Text = "Delivered Daily via Mail (30d)"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Stamina_Title_Content = {
    Text = "In-Sheath Supply"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Tips_Content = {
    Text = "*300 + 6000 Silver in total!"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Title_Content = {
    Text = "Mythag Shop"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Today_Content = {
    Text = "Claimed Today"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Toggle_Normal_Month_Content = {
    Text = "Moonphase Vigil"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Toggle_Normal_Recharge_Content = {Text = "Recharge"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Toggle_Normal_Root_Content = {Text = "Radix Foam"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Toggle_Select_Month_Content = {
    Text = "Moonphase Vigil"
  },
  PanelText_UI_Recharge_Panel_Main_Text_C_Toggle_Select_Recharge_Content = {Text = "Recharge"},
  PanelText_UI_Recharge_Panel_Main_Text_C_Toggle_Select_Root_Content = {Text = "Radix Foam"},
  PanelText_UI_Recharge_Panel_Main_Text_Get2_Title2_Content = {Text = "(30d)"},
  PanelText_UI_Recharge_Panel_Main_Text_Name_Content = {Text = "Purchase"},
  PanelText_UI_Recharge_Panel_Main_Text_Tips_Content = {
    Text = "*300 + 6000 Silver in total!"
  },
  PanelText_UI_Recharge_Panel_Main_Text_Toggle_Normal_Month_Content = {
    Text = "Moonphase Vigil"
  },
  PanelText_UI_Recharge_Panel_Main_Text_Toggle_Normal_Recharge_Content = {Text = "Recharge"},
  PanelText_UI_Recharge_Panel_Main_Text_Toggle_Normal_Root_Content = {Text = "Radix Foam"},
  PanelText_UI_Recharge_Panel_Main_Text_Toggle_Select_Month_Content = {
    Text = "Moonphase Vigil"
  },
  PanelText_UI_Recharge_Panel_Main_Text_Toggle_Select_Recharge_Content = {Text = "Recharge"},
  PanelText_UI_Recharge_Panel_Main_Text_Toggle_Select_Root_Content = {Text = "Radix Foam"},
  PanelText_UI_Recharge_Panel_Month_Tips_Text_C_Today_Content = {
    Text = "Your Moonphase Vigil reward is available!"
  },
  PanelText_UI_Recharge_Panel_OptionGift_Text_C_Disable_Content = {Text = "Purchase"},
  PanelText_UI_Recharge_Panel_OptionGift_Text_C_Normal_Content = {Text = "Purchase"},
  PanelText_UI_Recharge_Panel_OptionGift_Text_C_Title_Content = {
    Text = "Mythag Shop"
  },
  PanelText_UI_Recharge_Popup_OptionGift_Text_C_Cancel_Content = {Text = "Cancel"},
  PanelText_UI_Recharge_Popup_OptionGift_Text_C_Confirm_Content = {Text = "Purchase"},
  PanelText_UI_Recharge_Popup_OptionGift_Text_C_Cost_Tips_Content = {Text = "Consumes"},
  PanelText_UI_Recharge_Popup_OptionGift_Text_C_Title_Content = {Text = "Confirm"},
  PanelText_UI_Recharge_Popup_OptionGift_Text_C_Title_En_Content = {Text = "Title"},
  PanelText_UI_Recharge_Reset_Item_Text_C_Double_Content = {
    Text = "First Purchase"
  },
  PanelText_UI_Recharge_Reset_Item_Text_C_Extra_Content = {Text = "Extra"},
  PanelText_UI_Research_Panel_Ceri_Item_Text_C_Activate_Content = {Text = "Activated"},
  PanelText_UI_Research_Panel_Ceri_Item_Text_C_NoeGet_Content = {
    Text = "Awakener Locked"
  },
  PanelText_UI_Research_Panel_Ceri_Item_Text_Name_Content = {
    Text = "Activate Talent"
  },
  PanelText_UI_Research_Panel_Ceri_Text_C_Activate_Content = {Text = "Activated"},
  PanelText_UI_Research_Panel_Ceri_Text_C_Title_Content = {
    Text = "Talents Overview"
  },
  PanelText_UI_Research_Panel_Challenge_Item_Text_C_Undone_Content = {Text = "Incomplete"},
  PanelText_UI_Research_Panel_Challenge_Text_C_PlanName_Content = {
    Text = "Current Progress"
  },
  PanelText_UI_Research_Panel_Challenge_Text_C_ReachName_Content = {
    Text = "All Achievements Unlocked"
  },
  PanelText_UI_Research_Panel_Challenge_Text_Title_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Research_Panel_Dispatch_Content_Text_C_Award_Content = {Text = "Rewards"},
  PanelText_UI_Research_Panel_Dispatch_Content_Text_C_Count_Content = {Text = "Rules"},
  PanelText_UI_Research_Panel_Dispatch_Content_Text_C_Extra_Content = {Text = "[EXTRA]"},
  PanelText_UI_Research_Panel_Dispatch_Content_Text_C_NoTask_Content = {
    Text = "No new assignments for now"
  },
  PanelText_UI_Research_Panel_Dispatch_Content_Text_C_Special2_Content = {
    Text = "Every 20 surplus levels, +1 Reward."
  },
  PanelText_UI_Research_Panel_Dispatch_Content_Text_C_Special_Content = {Text = "Conditions"},
  PanelText_UI_Research_Panel_Dispatch_Content_Text_Underway_Content = {Text = "Ongoing..."},
  PanelText_UI_Research_Panel_Dispatch_Item_Text_C_Tip_Content = {Text = "Claimable"},
  PanelText_UI_Research_Panel_Dispatch_Item_Text_C_Unclickable_Content = {Text = "Locked"},
  PanelText_UI_Research_Panel_Dispatch_Item_Text_C_Underway_Content = {
    Text = "In Progress"
  },
  PanelText_UI_Research_Panel_Dispatch_Item_Text_Tine_Title_Content = {Text = "Remaining"},
  PanelText_UI_Research_Panel_Dispatch_Item_Text_Tine_Title_Normal_Content = {Text = "Consumes"},
  PanelText_UI_Research_Panel_Dispatch_Text_Title_Content = {Text = "Dispatch"},
  PanelText_UI_Research_Popup_Dispatch_Text_Title_Content = {
    Text = "Bonuses Overview"
  },
  PanelText_UI_Research_Popup_Recall2_Text_Agin_Content = {Text = "Reassign"},
  PanelText_UI_Research_Popup_Recall2_Text_C_Dispatch_Content = {
    Text = "Deploy Awakeners"
  },
  PanelText_UI_Research_Popup_Recall2_Text_C_Reward_Content = {Text = "Rewards"},
  PanelText_UI_Research_Popup_Recall2_Text_C_Title_Content = {
    Text = "Assignment Report"
  },
  PanelText_UI_Research_Popup_Recall2_Text_Close_Content = {Text = "Close"},
  PanelText_UI_Research_Popup_Recall_Text_C_Award_Content = {Text = "Rewards"},
  PanelText_UI_Research_Popup_Recall_Text_C_Dispatch_Content = {
    Text = "Current Total Level"
  },
  PanelText_UI_Research_Popup_Recall_Text_C_Extra_Content = {Text = "[EXTRA]"},
  PanelText_UI_Research_Popup_Recall_Text_C_Level_Content = {
    Text = "Total Deployed Level"
  },
  PanelText_UI_Research_Popup_Recall_Text_C_Special2_Content = {
    Text = "Every 20 surplus levels, +1 Reward."
  },
  PanelText_UI_Research_Popup_Recall_Text_C_Special_Content = {Text = "Conditions"},
  PanelText_UI_Research_Popup_Recall_Text_C_Title_Content = {
    Text = "Select Awakeners"
  },
  PanelText_UI_Research_Popup_Report_Text_C_Title_Content = {
    Text = "Investigation Report"
  },
  PanelText_UI_Setting_Popup_ChooseLang_Text_C_Tip_Content = {
    Text = "Can be reset later in \"Settings - Language\""
  },
  PanelText_UI_Setting_Popup_ChooseLang_Text_Name_Content = {Text = "Confirm"},
  PanelText_UI_Setting_Popup_ChooseLang_Text_Title_Content = {
    Text = "Select Language"
  },
  PanelText_UI_Setting_Popup_Code_Text_C_Clean_Content = {Text = "Clear"},
  PanelText_UI_Setting_Popup_Code_Text_C_Paste_Content = {Text = "Paste"},
  PanelText_UI_Setting_Popup_Code_Text_C_Tips_Content = {
    Text = "Enter Redemption Code"
  },
  PanelText_UI_Setting_Popup_Down_Text_C_Downing_Content = {
    Text = "Downloading.."
  },
  PanelText_UI_Setting_Popup_Examine_Text_C_Examine_Content = {
    Text = "Checking the integrity of game resources"
  },
  PanelText_UI_Setting_Popup_Main_Text_C_Tips_Content = {
    Text = "Internal GM"
  },
  PanelText_UI_Setting_Popup_Main_Text_C_Title_Content = {Text = "Settings"},
  PanelText_UI_Setting_Popup_PlatformBinding_Text_C_Receive_1_Content = {Text = "Claim"},
  PanelText_UI_Setting_Popup_PlatformBinding_Text_C_Receive_2_Content = {Text = "Claim"},
  PanelText_UI_Setting_Popup_PlatformBinding_Text_C_Title_Content = {
    Text = "Log-in Rewards"
  },
  PanelText_UI_Setting_Popup_PlatformBinding_Text_C_UnDone_1_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Setting_Popup_PlatformBinding_Text_C_UnDone_2_Content = {
    Text = "Not Achieved"
  },
  PanelText_UI_Shop_Item_Commodity_Text_C_Head_Content = {Text = "Avatar"},
  PanelText_UI_Shop_Item_Commodity_Text_C_Value_Content = {Text = "DEAL"},
  PanelText_UI_Shop_Item_Toggle_Text_C_Recommend_Content = {Text = "HOT"},
  PanelText_UI_Shop_Popup_Tips_Text_C_Tips_Content = {
    Text = "Insufficient Currency"
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Assist_Content = {
    Text = "Support Count"
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Awaker_Content = {Text = "Awakeners"},
  PanelText_UI_Social_Item_Data_Left_Text_C_Birthday_Content = {Text = "Birthday"},
  PanelText_UI_Social_Item_Data_Left_Text_C_Concern2_Content = {Text = "Follow"},
  PanelText_UI_Social_Item_Data_Left_Text_C_Conclude_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Difficulty_Content = {Text = "Operation"},
  PanelText_UI_Social_Item_Data_Left_Text_C_Dream_Content = {
    Text = "Phantasmal Dive"
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Fans_Content = {Text = "Follower"},
  PanelText_UI_Social_Item_Data_Left_Text_C_Highest_Content = {Text = "Score"},
  PanelText_UI_Social_Item_Data_Left_Text_C_Indestructible_Content = {
    Text = "Lightless Realm: Rotation Gap"
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Pagoda_Content = {
    Text = "Lightless Realm"
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Privacy_Content = {
    Text = "This Keeper has privated their progress."
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Register_Content = {
    Text = "Active Days"
  },
  PanelText_UI_Social_Item_Data_Left_Text_C_Society_Content = {Text = "Guild"},
  PanelText_UI_Social_Item_Data_Left_Text_C_Survey_Content = {
    Text = "Investigation Progress"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Placeholder_Content = {
    Text = "Edit your bio..."
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Abyss_Content = {
    Text = "D-Effect Zone"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Assist_Content = {
    Text = "Support Count"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Attention_Content = {Text = "Follow"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Awaker_Content = {Text = "Awakeners"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Ban_Content = {Text = "Block"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Birthday_Content = {Text = "Birthday"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Collection_Content = {Text = "Progress"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Concern2_Content = {Text = "Followed"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Conclude_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Difficulty_Content = {Text = "Main Story"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Dream_Content = {
    Text = "Phantasmal Dive"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Fans_Content = {Text = "Follower"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Gender_Content = {Text = "Gender"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Hard_Content = {Text = "Hard"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Highest_Content = {Text = "Score"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Indestructible_Content = {
    Text = "Lightless Realm: Rotation Gap"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Interrelated_Content = {Text = "Mutual"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Normal_Content = {Text = "Normal"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Notset_Content = {Text = "Not Set"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Pagoda_Content = {
    Text = "Lightless Realm: Fissure"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Privacy_Content = {
    Text = "This Keeper has privated their progress."
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Register_Content = {
    Text = "Active Days"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Society_Content = {Text = "Guild"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Survey_Content = {
    Text = "Investigation Progress"
  },
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Total_1_Content = {Text = "Total"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_C_Total_2_Content = {Text = "Total"},
  PanelText_UI_Social_Item_Data_Overview_Left_Text_NickNamePre_Content = {Text = "Note:"},
  PanelText_UI_Social_Item_Data_Right_Text_C_Help_Content = {
    Text = "Support Awakener"
  },
  PanelText_UI_Social_Item_Data_Right_Text_C_Not_Content = {
    Text = "No Support Awakener"
  },
  PanelText_UI_Social_Item_Data_Right_Text_C_Null_1_Content = {
    Text = "No Awakeners Displayed"
  },
  PanelText_UI_Social_Item_Data_Right_Text_C_Team_Content = {
    Text = "Awakener Display"
  },
  PanelText_UI_Social_Item_FriendlyInvite_Text_C_Accept_Content = {Text = "Accept"},
  PanelText_UI_Social_Item_FriendlyInvite_Text_C_Cancel_Content = {Text = "Cancelled"},
  PanelText_UI_Social_Item_FriendlyInvite_Text_C_Done_Content = {
    Text = "Follows You"
  },
  PanelText_UI_Social_Item_FriendlyInvite_Text_C_Mutual_Content = {Text = "Mutual"},
  PanelText_UI_Social_Item_FriendlyInvite_Text_C_Refuse2_Content = {Text = "Rejected"},
  PanelText_UI_Social_Item_FriendlyInvite_Text_C_Refuse_Content = {Text = "Reject"},
  PanelText_UI_Social_Item_FriendlyInvite_Text_C_Timeout_Content = {Text = "Timed Out"},
  PanelText_UI_Social_Item_PveRecord_Text_Fail_Content = {Text = "Failure"},
  PanelText_UI_Social_Item_PveRecord_Text_Victory_Content = {Text = "Victory"},
  PanelText_UI_Social_Item_PvpData_Text_C_Point_01_Content = {Text = "Pts"},
  PanelText_UI_Social_Item_PvpData_Text_Fail_Content = {Text = "Failure"},
  PanelText_UI_Social_Item_PvpData_Text_Victory_Content = {Text = "Victory"},
  PanelText_UI_Social_Item_PvpRecord_Text_Fail_Content = {Text = "Failure"},
  PanelText_UI_Social_Item_PvpRecord_Text_Victory_Content = {Text = "Victory"},
  PanelText_UI_Social_Item_Pvp_Text_C_Dispaly_Content = {
    Text = "Open Combat Log"
  },
  PanelText_UI_Social_Item_Pvp_Text_C_Input_Content = {
    Text = "Enter Replay Code to Watch..."
  },
  PanelText_UI_Social_Item_Pvp_Text_C_NoTask_Content = {
    Text = "No Traphase Record"
  },
  PanelText_UI_Social_Item_Pvp_Text_C_Pve_Nor_Content = {
    Text = "Investigation"
  },
  PanelText_UI_Social_Item_Pvp_Text_C_Pve_Sel_Content = {
    Text = "Investigation"
  },
  PanelText_UI_Social_Item_Pvp_Text_C_Pvp_Nor_Content = {Text = "Traphase"},
  PanelText_UI_Social_Item_Pvp_Text_C_Pvp_Sel_Content = {Text = "Traphase"},
  PanelText_UI_Social_Item_Pvp_Text_C_Search_Content = {
    Text = "Watch Replay"
  },
  PanelText_UI_Social_Item_Pvp_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Social_Item_Rank_Text_C_Attention_Content = {Text = "Follow"},
  PanelText_UI_Social_Item_Rank_Text_C_Following_Content = {Text = "Followed"},
  PanelText_UI_Social_Item_Rank_Text_C_Interrelated_Content = {Text = "Mutual"},
  PanelText_UI_Social_Panel_Data_Text_C_Assist_Content = {
    Text = "Support Count"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Awaker_Content = {Text = "Awakeners"},
  PanelText_UI_Social_Panel_Data_Text_C_Birthday_Content = {Text = "Birthday"},
  PanelText_UI_Social_Panel_Data_Text_C_Concern2_Content = {Text = "Follow"},
  PanelText_UI_Social_Panel_Data_Text_C_Conclude_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Difficulty_Content = {Text = "Hard"},
  PanelText_UI_Social_Panel_Data_Text_C_Dream_Content = {
    Text = "Phantasmal Dive"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Fans_Content = {Text = "Follower"},
  PanelText_UI_Social_Panel_Data_Text_C_Help_Content = {
    Text = "Support Awakener"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Indestructible_Content = {
    Text = "Unspeakable"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Not_Content = {
    Text = "No Support Awakener"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Null_1_Content = {
    Text = "No Awakeners Displayed"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Pagoda_Content = {
    Text = "Lightless Tower"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Privacy_Content = {
    Text = "This Keeper has privated their progress."
  },
  PanelText_UI_Social_Panel_Data_Text_C_Register_Content = {
    Text = "Active Days"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Society_Content = {Text = "Guild"},
  PanelText_UI_Social_Panel_Data_Text_C_Survey_Content = {
    Text = "Investigation Progress"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Team_Content = {
    Text = "Awakener Display"
  },
  PanelText_UI_Social_Panel_Data_Text_C_Title_Content = {
    Text = "Keeper Profile"
  },
  PanelText_UI_Social_Panel_HelpList_Filter_Text_C_Selected_Content = {Text = "Selected"},
  ["PanelText_UI_Social_Panel_HelpList_Item_Text_C_ Interrelated_Content"] = {Text = "Mutual"},
  PanelText_UI_Social_Panel_HelpList_Item_Text_C_Attention_Content = {Text = "Follow"},
  PanelText_UI_Social_Panel_HelpList_Item_Text_C_Commonly_Content = {Text = "Favorite"},
  PanelText_UI_Social_Panel_HelpList_Item_Text_C_Conflict_Content = {
    Text = "Realm Conflict"
  },
  PanelText_UI_Social_Panel_HelpList_Item_Text_C_Fans_Content = {Text = "Follower"},
  PanelText_UI_Social_Panel_HelpList_Item_Text_C_Following_Content = {Text = "Followed"},
  PanelText_UI_Social_Panel_HelpList_Item_Text_C_Recent_Content = {
    Text = "Recent Supports"
  },
  PanelText_UI_Social_Panel_HelpList_Item_Text_C_Recommend_Content = {
    Text = "Recommend Follow"
  },
  PanelText_UI_Social_Panel_HelpList_Text_C_CommonlyAssist_Content = {Text = "Favorite"},
  PanelText_UI_Social_Panel_HelpList_Text_C_Confim_Dis_1_Content = {Text = "Refresh"},
  PanelText_UI_Social_Panel_HelpList_Text_C_Confim_Dis_Content = {Text = "Deploy"},
  PanelText_UI_Social_Panel_HelpList_Text_C_Confim_Nor_1_Content = {Text = "Refresh"},
  PanelText_UI_Social_Panel_HelpList_Text_C_Confim_Nor_Content = {Text = "Deploy"},
  PanelText_UI_Social_Panel_HelpList_Text_C_Mail_Content = {Text = "Rules"},
  PanelText_UI_Social_Panel_HelpList_Text_C_NoTask_Content = {
    Text = "No Support available. Please readjust your filter setting"
  },
  PanelText_UI_Social_Panel_HelpList_Text_C_Time_Content = {
    Text = "You may borrow the Awakeners of mutuals."
  },
  PanelText_UI_Social_Panel_HelpList_Text_C_Title_Content = {
    Text = "Select Support"
  },
  PanelText_UI_Social_Panel_Main_Item_Text_C_Attention_Content = {Text = "Follow"},
  PanelText_UI_Social_Panel_Main_Item_Text_C_Done_Content = {
    Text = "Following You"
  },
  PanelText_UI_Social_Panel_Main_Item_Text_C_Following_Content = {Text = "Followed"},
  PanelText_UI_Social_Panel_Main_Item_Text_C_Interrelated_Content = {Text = "Mutual"},
  PanelText_UI_Social_Panel_Main_Item_Text_C_Mutual_Content = {Text = "Mutual"},
  PanelText_UI_Social_Panel_Main_Item_Text_C_Unblock_Content = {Text = "Unblock"},
  PanelText_UI_Social_Panel_Main_Item_Text_Unfollow_Content = {Text = "Unfollow"},
  PanelText_UI_Social_Panel_Main_SubPanel_1_Text_C_Concern_Content = {Text = "New"},
  PanelText_UI_Social_Panel_Main_SubPanel_1_Text_C_NoTask_Content = {
    Text = "Not following anyone yet"
  },
  PanelText_UI_Social_Panel_Main_SubPanel_1_Text_C_Time_Content = {
    Text = "Nothing here. Go look somewhere else?"
  },
  PanelText_UI_Social_Panel_Main_SubPanel_2_Text_C_NoTask_Content = {Text = "No results"},
  PanelText_UI_Social_Panel_Main_SubPanel_2_Text_C_Time_Content = {
    Text = "Nothing here. Go look somewhere else?"
  },
  PanelText_UI_Social_Panel_Main_SubPanel_2_Text_Name_Content = {Text = "Follow All"},
  PanelText_UI_Social_Panel_Main_Text_C_Concern_Content = {Text = "New:"},
  PanelText_UI_Social_Panel_Main_Text_C_NoTask_Content = {Text = "No Follows"},
  PanelText_UI_Social_Panel_Main_Text_C_Stranger_Content = {Text = "Stranger"},
  PanelText_UI_Social_Panel_Main_Text_C_Time_Content = {
    Text = "Nothing here. Go look somewhere else?"
  },
  PanelText_UI_Social_Panel_Main_Text_C_Tips_1_Content = {
    Text = "Search by Name"
  },
  PanelText_UI_Social_Panel_Main_Text_C_Tips_2_Content = {
    Text = "Search by Name"
  },
  PanelText_UI_Social_Panel_RankList_Text_C_Interaction_Content = {Text = "Interact"},
  PanelText_UI_Social_Panel_RankList_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Social_Panel_RankList_Text_C_PlayerInfo_Content = {Text = "Master"},
  PanelText_UI_Social_Panel_RankList_Text_C_Rank_Content = {Text = "Rank"},
  PanelText_UI_Social_Panel_RankList_Text_C_Uid_Title_Content = {Text = "UID:"},
  PanelText_UI_Social_Panel_Rank_Text_C_All_Content = {Text = "All"},
  PanelText_UI_Social_Panel_Rank_Text_C_Friend_Content = {Text = "Follow"},
  PanelText_UI_Social_Panel_Rank_Text_C_Interaction_Content = {Text = "Interact"},
  PanelText_UI_Social_Panel_Rank_Text_C_NoTask_Content = {
    Text = "No records available"
  },
  PanelText_UI_Social_Panel_Rank_Text_C_PlayerInfo_Content = {
    Text = "Keeper Information"
  },
  PanelText_UI_Social_Panel_Rank_Text_C_Rank_Content = {Text = "Rank"},
  PanelText_UI_Social_Panel_Rank_Text_C_Sign_Content = {Text = "Bio"},
  PanelText_UI_Social_Panel_Rank_Text_C_Uid_Title_Content = {Text = "UID:"},
  PanelText_UI_Social_Popup_Awaker_Text_C_Owned_Content = {Text = "Owned"},
  PanelText_UI_Social_Popup_Birthday_Text_C_Day_Content = {Text = "d"},
  PanelText_UI_Social_Popup_Birthday_Text_C_Month_Content = {Text = "/"},
  PanelText_UI_Social_Popup_Birthday_Text_C_Title_Content = {
    Text = "Edit Birthday"
  },
  PanelText_UI_Social_Popup_CommonlyAssist_Text_C_NoTask_Content = {
    Text = "No Supports"
  },
  PanelText_UI_Social_Popup_CommonlyAssist_Text_C_SearchTip_Content = {Text = "Enter UID"},
  PanelText_UI_Social_Popup_CommonlyAssist_Text_C_Search_Content = {Text = "Search"},
  PanelText_UI_Social_Popup_CommonlyAssist_Text_C_Title_Content = {Text = "Favorite"},
  PanelText_UI_Social_Popup_CommonlyAssist_Text_Time_Content = {
    Text = "Check out other places"
  },
  PanelText_UI_Social_Popup_FriendlyInvite_Text_Title_Content = {
    Text = "Traphase Match Invite"
  },
  PanelText_UI_Social_Popup_Introduction_Text_C_Placeholder_Content = {
    Text = "Edit your bio..."
  },
  PanelText_UI_Social_Popup_MatchMode_Text_C_FriendlyMatch_Content = {Text = "Select"},
  PanelText_UI_Social_Popup_MatchMode_Text_C_FriendlyMatch_Desc_Content = {
    Text = "<size=25>The standard Traphase mode. Both sides use their pre-prepared teams for battle.</size>"
  },
  PanelText_UI_Social_Popup_MatchMode_Text_C_FriendlyMatch_Name_Content = {Text = "All Pick"},
  PanelText_UI_Social_Popup_MatchMode_Text_C_RotationMode_Content = {Text = "Select"},
  PanelText_UI_Social_Popup_MatchMode_Text_C_RotationMode_Desc_Content = {
    Text = "<size=25>The advanced Traphase mode. Both sides take turns to select cards from a randomized pool.</size>"
  },
  PanelText_UI_Social_Popup_MatchMode_Text_C_RotationMode_Name_Content = {Text = "Draft Pick"},
  PanelText_UI_Social_Popup_MatchMode_Text_Title_Content = {
    Text = "Select Mode"
  },
  PanelText_UI_Social_Popup_Match_Text_C_Waiting_Content = {Text = "Waiting..."},
  PanelText_UI_Social_Popup_Match_Text_Name_Content = {Text = "Cancel"},
  PanelText_UI_Social_Popup_Match_Text_Title_Content = {
    Text = "Friendly Match"
  },
  PanelText_UI_Social_Popup_PlayerHead_Text_C_HeadFrame_Content = {
    Text = "Avatar Frame"
  },
  PanelText_UI_Social_Popup_PlayerHead_Text_C_HeadFrame_Sel_Content = {
    Text = "Avatar Frame"
  },
  PanelText_UI_Social_Popup_PlayerHead_Text_C_Head_Content = {Text = "Avatar"},
  PanelText_UI_Social_Popup_PlayerHead_Text_C_Head_Sel_Content = {Text = "Avatar"},
  PanelText_UI_Social_Popup_PlayerHead_Text_C_Using_Content = {Text = "Equipped"},
  PanelText_UI_Social_Popup_Situation_Tips_Text_C_AffixInfo_Content = {
    Text = "Investigation Status"
  },
  PanelText_UI_Social_Popup_SwitchGender_Text_C_Cost_Content = {Text = "Consumes"},
  PanelText_UI_Social_Popup_SwitchGender_Text_Cancel_Name_Content = {Text = "Cancel"},
  PanelText_UI_Social_Popup_SwitchGender_Text_Confirm_Name_Content = {Text = "Confirm"},
  PanelText_UI_Social_Popup_SwitchGender_Text_Title_Content = {
    Text = "Switch Gender"
  },
  PanelText_UI_Social_Popup_WatchList_Text_C_NoTask_Content = {
    Text = "No new followers yet"
  },
  PanelText_UI_Social_Popup_WatchList_Text_Name_FollowAll_Content = {Text = "Follow All"},
  PanelText_UI_Social_Popup_WatchList_Text_Tip_Content = {
    Text = "View the 100 most recent followers"
  },
  PanelText_UI_Social_Tips_InviteBubble_Text_Name_Content = {
    Text = "Friendly Match"
  },
  PanelText_UI_Story_Item_History_Text_Text_C_Choice_1_Content = {Text = ">>>"},
  PanelText_UI_Story_Item_History_Text_Text_C_Choice_2_Content = {Text = ">>>"},
  PanelText_UI_Story_Item_History_Text_Text_C_Choice_3_Content = {Text = ">>>"},
  PanelText_UI_Story_Item_History_Text_Text_C_Current_Content = {Text = ">>>"},
  PanelText_UI_Story_Panel_Main_Text_C_High_Content = {
    Text = "Reset Login Background"
  },
  PanelText_UI_Story_Panel_Main_Text_C_Normal_Content = {
    Text = "Set as Login Background"
  },
  PanelText_UI_Story_Popup_Curtain_Call_Text_C_Continue_Content = {Text = "Continue"},
  PanelText_UI_Story_Popup_Curtain_Call_Text_C_Title_Content = {
    Text = "Memory Archived"
  },
  PanelText_UI_Story_Portrait_Test_Text_C_Close_Content = {Text = "Close"},
  PanelText_UI_Story_Portrait_Test_Text_C_Hide_Content = {Text = "Hidden"},
  PanelText_UI_Summon_Btn_Jump_Text_C_Look_Content = {Text = "View"},
  PanelText_UI_Summon_Btn_Realms_Text_C_Difficulty_Content = {Text = "Difficulty"},
  PanelText_UI_Summon_Btn_Realms_Text_C_Directional_Content = {
    Text = "Current Choice"
  },
  PanelText_UI_Summon_Btn_Realms_Text_C_Name_Content = {Text = "Select"},
  PanelText_UI_Summon_Item_OptionalLimit_Text_C_Full_Content = {
    Text = "Fully Ascended"
  },
  PanelText_UI_Summon_Item_OptionalLimit_Text_C_Have_Content = {
    Text = "Already Owned"
  },
  PanelText_UI_Summon_Item_Probability_Text_C_Name_Content = {Text = "Name"},
  PanelText_UI_Summon_Item_Probability_Text_C_Role_Content = {
    Text = "Comprehensive Probability"
  },
  PanelText_UI_Summon_Item_Probability_Text_C_Time_Content = {Text = "Name"},
  PanelText_UI_Summon_Item_Probability_Text_C_Type_Content = {Text = "Type"},
  PanelText_UI_Summon_Item_Probability_Text_Limit_Content = {Text = "Limited"},
  PanelText_UI_Summon_Panel_LuckyBag_Text_C_GiftTitle_Content = {
    Text = "Claim a Skin Voucher!"
  },
  PanelText_UI_Summon_Panel_LuckyBag_Text_C_LuckyBag_1_Content = {
    Text = "×2 Guaranteed"
  },
  PanelText_UI_Summon_Panel_LuckyBag_Text_C_LuckyBag_2_Content = {
    Text = "×3 Guaranteed"
  },
  PanelText_UI_Summon_Panel_Main_Text_Award_Content = {
    Text = "Extra Rewards"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_Choose_Content = {Text = "Select"},
  PanelText_UI_Summon_Panel_Main_Text_C_Difficult_Title_Content = {Text = "Difficulty"},
  PanelText_UI_Summon_Panel_Main_Text_C_FirstLimit_Content = {
    Text = "First-Time Limited"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_FreeLimit_Content = {Text = "Free"},
  PanelText_UI_Summon_Panel_Main_Text_C_Hint_Content = {
    Text = "Switch Realms"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_LimitName_Content = {
    Text = "5× Awakening"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_Limited_Content = {
    Text = "Limited Awakener"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_Look_Content = {Text = "View"},
  PanelText_UI_Summon_Panel_Main_Text_C_Name_Content = {Text = "Permanent"},
  PanelText_UI_Summon_Panel_Main_Text_C_Realms_Content = {
    Text = "Select a Realm"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_RecAwaker_Content = {
    Text = "Recommended Awakener"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_School_Intro_Title_Content = {
    Text = "Realm Characteristics:"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_Summon_Content = {
    Text = "Awake 5 times"
  },
  PanelText_UI_Summon_Panel_Main_Text_C_System_Name_Content = {Text = "Awaken"},
  PanelText_UI_Summon_Panel_Main_Text_C_TryName_Content = {
    Text = "Awakener Trial"
  },
  PanelText_UI_Summon_Panel_Main_Text_Desc_2_Content = {
    Text = "Description"
  },
  PanelText_UI_Summon_Panel_Main_Text_FreeTen_Content = {Text = "Free"},
  PanelText_UI_Summon_Panel_Main_Text_Recommend_Content = {
    Text = "Freshman Awakening Event"
  },
  PanelText_UI_Summon_Panel_SelectLimit_Text_C_Choose_Content = {
    Text = "Select Awakeners"
  },
  PanelText_UI_Summon_Panel_SelectLimit_Text_C_Title_Content = {
    Text = "Awakeners Selected"
  },
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Detail_Content = {Text = "Rules"},
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Inventory_Content = {Text = "Inventory"},
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Name_Content = {Text = "Name"},
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Not_Content = {Text = "No records"},
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Prompt_Content = {
    Text = "You can check the past 6 months of Awakening records on this page. Data may be delayed by about an hour."
  },
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_REcording_Content = {Text = "Records"},
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Role_Content = {
    Text = "Awakening Type"
  },
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Time_Content = {
    Text = "Awakening Time"
  },
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Type_1_Content = {
    Text = "Awakening Type"
  },
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Type_Content = {Text = "Type"},
  PanelText_UI_Summon_Popup_Detail_Tip_Text_C_Up_Content = {
    Text = "Rate-Up Item"
  },
  PanelText_UI_Summon_Popup_Main_Text_C_Name_Content = {Text = "Name"},
  PanelText_UI_Summon_Popup_Main_Text_C_Not_Content = {
    Text = "No records available"
  },
  PanelText_UI_Summon_Popup_Main_Text_C_Prompt_Content = {
    Text = "You can check the past 6 months of Awakening records on this page. Data may be delayed by about an hour."
  },
  PanelText_UI_Summon_Popup_Main_Text_C_Role_Content = {
    Text = "Awakening Type"
  },
  PanelText_UI_Summon_Popup_Main_Text_C_Time_Content = {
    Text = "Awakening Time"
  },
  PanelText_UI_Summon_Popup_Main_Text_C_Title_Content = {Text = "Records"},
  PanelText_UI_Summon_Popup_Main_Text_C_Type_Content = {Text = "Type"},
  PanelText_UI_Summon_Popup_MixPoolLimit_Text_C_Change_Content = {Text = "Reselect"},
  PanelText_UI_Summon_Popup_MixPoolLimit_Text_C_ChooseTitle_Content = {
    Text = "Select Awakener"
  },
  PanelText_UI_Summon_Popup_MixPoolLimit_Text_C_Choose_Content = {Text = "Select Now"},
  PanelText_UI_Summon_Popup_MixPoolLimit_Text_C_InfoTitle_Content = {
    Text = "Instructions"
  },
  PanelText_UI_Summon_Popup_MixPoolLimit_Text_Title_Content = {
    Text = "Choose an Awakener"
  },
  PanelText_UI_Summon_Popup_OptionalLimit_Text_C_Confirm_Content = {
    Text = "Confirm Selection"
  },
  PanelText_UI_Summon_Popup_OptionalLimit_Text_C_Tip_Content = {
    Text = "Long press an Awakener to view details"
  },
  PanelText_UI_Summon_Popup_OptionalMixPoolLimit_Text_C_Confirm_Content = {
    Text = "Confirm Selection"
  },
  PanelText_UI_Summon_Popup_OptionalMixPoolLimit_Text_C_Tip_Content = {
    Text = "Long press an Awakener to view details"
  },
  PanelText_UI_Summon_Popup_OptionalMixPoolLimit_Text_C_Title_Content = {
    Text = "Choose an Awakener"
  },
  PanelText_UI_Summon_Popup_Presented_Tip_Text_C_Activity_Content = {
    Text = "Free Wheel of Destiny"
  },
  PanelText_UI_Summon_Popup_Presented_Tip_Text_C_Game_Content = {
    Text = "Free Awakener"
  },
  PanelText_UI_Summon_Popup_Presented_Tip_Text_C_Info_Content = {
    Text = "For every 150 [Realm Study] Awakenings, you can obtain 1 Awakener/Wheel of Destiny of your choice from the ones listed above."
  },
  PanelText_UI_Summon_Popup_Presented_Tip_Text_C_Plan_Content = {Text = "Progress"},
  PanelText_UI_Summon_Popup_Presented_Tip_Text_Check_Content = {Text = "About"},
  PanelText_UI_Summon_Popup_Presented_Tip_Text_Git_Content = {Text = "Claim"},
  PanelText_UI_Summon_Popup_Prob_Tip_Text_C_Detail_Content = {Text = "Details"},
  PanelText_UI_Summon_Popup_Prob_Tip_Text_C_Inventory_Content = {Text = "Inventory"},
  PanelText_UI_Summon_Popup_Prob_Tip_Text_C_Up_Content = {
    Text = "Rate-Up Item"
  },
  PanelText_UI_Summon_Popup_Realms_Text_C_Introduce_Content = {
    Text = "Choose a Realm to only obtain its Awakeners. You can change your selection anytime."
  },
  PanelText_UI_Summon_Popup_Realms_Text_C_Title_Content = {
    Text = "Realm Selection"
  },
  PanelText_UI_Summon_Popup_Recording_Text_C_Prompt_Content = {
    Text = "You can check the past 6 months of Awakening records on this page. Data may be delayed by about an hour."
  },
  PanelText_UI_Summon_Popup_SelectLimit_Text_C_ChooseTitle_Content = {
    Text = "Select Awakeners"
  },
  PanelText_UI_Summon_Popup_SelectLimit_Text_C_Choose_Content = {
    Text = "Select Awakeners"
  },
  PanelText_UI_Summon_Popup_SelectLimit_Text_C_InfoTitle_Content = {
    Text = "Event Rules"
  },
  PanelText_UI_Summon_Popup_SelectLimit_Text_Title_Content = {
    Text = "Choose an Awakener"
  },
  PanelText_UI_Summon_Popup_Share_Text_C_Share_Content = {Text = "Share to:"},
  PanelText_UI_Summon_Popup_Show_Legend_Text_Autofine_Content = {
    Text = "Auto Ascension"
  },
  PanelText_UI_Summon_Popup_Show_Legend_Text_C_Name_Content = {Text = "Awaken ×1"},
  PanelText_UI_Summon_Popup_Show_Ordinary_Text_Autofine_Content = {
    Text = "Auto Ascension"
  },
  PanelText_UI_Summon_Popup_Show_Ordinary_Text_C_Name_Content = {Text = "Awaken ×1"},
  PanelText_UI_Summon_Popup_Show_Rare_Text_Autofine_Content = {
    Text = "Auto Ascension"
  },
  PanelText_UI_Summon_Popup_Show_Rare_Text_C_Name_Content = {Text = "Awaken ×1"},
  PanelText_UI_Summon_Popup_Total_Show_Text_C_Share_Content = {
    Text = "First-Time Sharing:"
  },
  PanelText_UI_Summon_Popup_Total_Show_Text_Confirm_Name_Content = {Text = "Confirm"},
  PanelText_UI_Summon_Popup_Total_Show_Text_Name_Content = {Text = "Reawaken"},
  PanelText_UI_Summon_Popup_Total_Show_Text_ReSummon_Content = {
    Text = "Awaken 5 Times"
  },
  PanelText_UI_Summon_Popup_Total_Show_Text_Toggle_Content = {
    Text = "Fast Awaken"
  },
  PanelText_UI_Summon_Result_Panel_Text_C_Name_Content = {Text = "Awaken ×1"},
  PanelText_UI_Summon_Result_Panel_Text_C_Share_Content = {
    Text = "First-Time Sharing:"
  },
  PanelText_UI_Suummoon_Popup_AwakerWeapon_Text_C_Auto_Content = {
    Text = "Recommended WoD"
  },
  PanelText_UI_Suummoon_Popup_AwakerWeapon_Text_C_ChooseTitle_Content = {
    Text = "Select Awakener"
  },
  PanelText_UI_Suummoon_Popup_AwakerWeapon_Text_C_Choose_Content = {
    Text = "Select Awakener"
  },
  PanelText_UI_Suummoon_Popup_AwakerWeapon_Text_C_Confirm_Content = {
    Text = "Confirm Selection"
  },
  PanelText_UI_Suummoon_Popup_AwakerWeapon_Text_C_InfoTitle_Content = {
    Text = "Event Rules"
  },
  PanelText_UI_Suummoon_Popup_AwakerWeapon_Text_Title_Content = {
    Text = "Select Awakener"
  },
  PanelText_UI_Task_Item_Detail_List_New_Text_C_Get_Content = {Text = "Claim"},
  PanelText_UI_Task_Panel_Main_New_Text_C_Achievement_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Task_Panel_Main_New_Text_C_ActionReports_Content = {
    Text = "Mission Report"
  },
  PanelText_UI_Task_Panel_Main_New_Text_C_GrowUp_Content = {
    Text = "Growth Record"
  },
  PanelText_UI_Task_Panel_Main_New_Text_C_Master_Content = {
    Text = "Realm Mastery"
  },
  PanelText_UI_Task_Panel_Main_New_Text_C_Tips2_Content = {Text = "Empty List"},
  PanelText_UI_Task_Panel_Main_New_Text_C_Tips_Content = {
    Text = "Daily Quest Complete!"
  },
  PanelText_UI_Task_Panel_Main_New_Text_C_Title_Content = {
    Text = "No new tasks for now..."
  },
  PanelText_UI_Task_Popup_Career_Item_Text_C_Undone_Content = {
    Text = "In Progress"
  },
  PanelText_UI_Task_Popup_Career_Item_Text_Name_Content = {Text = "Claim"},
  PanelText_UI_Task_Popup_Career_New_Text_C_1_Content = {
    Text = "Current Progress"
  },
  PanelText_UI_Task_Popup_Career_New_Text_C_Chapter_Award_Content = {
    Text = "Completion Rewards"
  },
  PanelText_UI_Task_Popup_Career_Text_C_1_Content = {
    Text = "Current Progress"
  },
  PanelText_UI_Task_Popup_Career_Text_C_Award_Content = {
    Text = "Completion Rewards"
  },
  PanelText_UI_Task_Popup_Career_Text_C_Get_Content = {
    Text = "Rewards Claimed"
  },
  PanelText_UI_Task_Popup_Career_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Task_Popup_Challenge2_Text_C_Name_2_Content = {Text = "Claim All"},
  PanelText_UI_Task_Popup_Challenge2_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Task_Popup_Challenge2_Text_C_ReachName_Content = {
    Text = "All Achievements Completed"
  },
  PanelText_UI_Task_Popup_Challenge_Item1_Text_C_Info1_Content = {
    Text = "Raise your Keeper Level to earn more"
  },
  PanelText_UI_Task_Popup_Challenge_Item1_Text_C_Level1_Content = {Text = "Level"},
  PanelText_UI_Task_Popup_Challenge_Item1_Text_C_Undone1_Content = {
    Text = "In Progress"
  },
  PanelText_UI_Task_Popup_Challenge_Item1_Text_Name_Content = {Text = "Claim"},
  PanelText_UI_Task_Popup_Challenge_Item_Text_C_Info_Content = {
    Text = "Raise your Keeper Level to earn more"
  },
  PanelText_UI_Task_Popup_Challenge_Item_Text_C_Level_Content = {Text = "Level"},
  PanelText_UI_Task_Popup_Challenge_Item_Text_Name_Content = {Text = "Claim"},
  PanelText_UI_Task_Popup_Challenge_New_Text_C_1_Content = {Text = "NAME"},
  PanelText_UI_Task_Popup_Challenge_New_Text_C_2_Content = {Text = "ENTRY"},
  PanelText_UI_Task_Popup_Challenge_New_Text_C_3_Content = {Text = "LEVEL"},
  PanelText_UI_Task_Popup_Challenge_New_Text_C_Title_Content = {
    Text = "Achievements"
  },
  PanelText_UI_Task_Popup_Challenge_Text_C_1_Content = {Text = "NAME"},
  PanelText_UI_Task_Popup_Challenge_Text_C_2_Content = {Text = "ENTRY"},
  PanelText_UI_Task_Popup_Challenge_Text_C_3_Content = {Text = "LEVEL"},
  PanelText_UI_Task_Popup_Challenge_Text_C_OneKey_Content = {Text = "Claim All"},
  PanelText_UI_Task_Popup_Challenge_Text_C_Reach_Content = {
    Text = "May the Silver Key guide us."
  },
  PanelText_UI_Task_Popup_Challenge_Text_C_Title_Content = {
    Text = "Admission Certificate"
  },
  PanelText_UI_Task_Popup_Entrust_New_Text_C_Award_Content = {
    Text = "Quest Rewards"
  },
  PanelText_UI_Task_Popup_Entrust_New_Text_C_Special_Content = {Text = "Conditions"},
  PanelText_UI_Team_Item_Awaker_List_Text_C_Disable_Content = {Text = "Disable"},
  PanelText_UI_Team_Item_Awaker_List_Text_C_Help_Content = {
    Text = "Support Awakener"
  },
  PanelText_UI_Team_Item_MyTeam_List_Text_C_Current_Content = {
    Text = "Currently Deployed"
  },
  PanelText_UI_Team_Item_Secret_Contract_Text_C_Guide_Content = {
    Text = "Recommended"
  },
  PanelText_UI_Team_Item_Secret_Contract_Text_C_Hint_Content = {
    Text = "*Each Awakener can only equip one SSR Wheel of Destiny."
  },
  PanelText_UI_Team_Item_Secret_Contract_Text_C_Recommend_Content = {
    Text = "Recommended"
  },
  PanelText_UI_Team_Item_Secret_Contract_Text_C_Remove_Content = {
    Text = "Unequip All"
  },
  PanelText_UI_Team_Item_Secret_Contract_Text_C_Save_Content = {Text = "Save"},
  PanelText_UI_Team_Item_Secret_Contract_Text_C_Uninstall_Content = {
    Text = "Unequip All"
  },
  PanelText_UI_Team_Item_Secret_Props_Text_C_Foundation_Content = {Text = "Attributes"},
  PanelText_UI_Team_Item_Secret_Props_Text_C_Lv_Content = {
    Text = "Level Progress"
  },
  PanelText_UI_Team_Item_Secret_Props_Text_C_Refining_Content = {
    Text = "Transcription Progress"
  },
  PanelText_UI_Team_Item_Secret_Props_Text_C_Suit_Null_Content = {
    Text = "No active set effects"
  },
  PanelText_UI_Team_Item_Secret_Props_Text_C_Suit_Title_Content = {Text = "Set Effect"},
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Added_Content = {
    Text = "New Covenant Plan"
  },
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Name_2_Content = {Text = "Unequip"},
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Name_3_Content = {Text = "Enhance"},
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Name_4_Content = {Text = "Equip"},
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Null_Content = {
    Text = "No available Covenant for this part"
  },
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Quantity_Content = {Text = "Saved Sets"},
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Save_Content = {
    Text = "New Covenant Plan"
  },
  PanelText_UI_Team_Item_Suit_Bookmark_Text_C_Suit_Content = {
    Text = "No Plans Available"
  },
  PanelText_UI_Team_Item_Suit_Text_C_Current_Content = {Text = "Equipped"},
  PanelText_UI_Team_Item_Talent_Text_C_Activate_Content = {Text = "Activated"},
  PanelText_UI_Team_Item_Trinket_Detail_Text_C_Foundation_Content = {Text = "Attributes"},
  PanelText_UI_Team_Item_Trinket_Detail_Text_C_Lv_Content = {
    Text = "Level Progress"
  },
  PanelText_UI_Team_Item_Trinket_Detail_Text_C_Refining_Content = {
    Text = "Transcription Progress"
  },
  PanelText_UI_Team_Item_Trinket_Detail_Text_C_Suit_Null_Content = {
    Text = "No active set effects"
  },
  PanelText_UI_Team_Item_Trinket_Detail_Text_C_Suit_Title_Content = {Text = "Set Effect"},
  PanelText_UI_Team_Item_Weapon_Full_Text_C_Empty_Content = {
    Text = "Select a WoD from the left"
  },
  PanelText_UI_Team_Item_Weapon_Full_Text_C_Not_Content = {Text = "Inactive"},
  PanelText_UI_Team_Panel_Awaker_Main_Text_C_Zhanli_Content = {
    Text = "Combat Power"
  },
  PanelText_UI_Team_Panel_Edit_Suit_Text_C_Recommend_Content = {
    Text = "Recommended"
  },
  PanelText_UI_Team_Panel_Edit_Suit_Text_C_Remove_Content = {
    Text = "Unequip All"
  },
  PanelText_UI_Team_Panel_Edit_Suit_Text_C_Save_Content = {Text = "Save"},
  PanelText_UI_Team_Panel_Main_Popup_Team_Text_C_AffixInfo_Content = {Text = "Team Info"},
  PanelText_UI_Team_Panel_Main_Text_C_Activated_Content = {Text = "Activated"},
  PanelText_UI_Team_Panel_Main_Text_C_Boundaries_Tips_Content = {
    Text = "Recommended Realm"
  },
  PanelText_UI_Team_Panel_Main_Text_C_Cost_Tips_Content = {
    Text = "Requirements:"
  },
  PanelText_UI_Team_Panel_Main_Text_C_Empty_Content = {
    Text = "Posse Not Equipped"
  },
  PanelText_UI_Team_Panel_Main_Text_C_Frequency_Content = {Text = "Attempts"},
  PanelText_UI_Team_Panel_Main_Text_C_Hint_Content = {
    Text = "The Awakeners, Wheels of Destiny, and Posse used in this battle cannot be deployed in other Regional D-Effect Zones."
  },
  PanelText_UI_Team_Panel_Main_Text_C_Lives_Desc_Content = {
    Text = "Emergency Gnoses Left:"
  },
  PanelText_UI_Team_Panel_Main_Text_C_Name_1_Content = {Text = "Remove"},
  PanelText_UI_Team_Panel_Main_Text_C_Name_2_Content = {Text = "Support"},
  PanelText_UI_Team_Panel_Main_Text_C_NoActivate_Content = {Text = "Inactive"},
  PanelText_UI_Team_Panel_Main_Text_C_Prohibit_Content = {
    Text = "Posse disabled for this level"
  },
  PanelText_UI_Team_Panel_Main_Text_C_Skill_Content = {Text = "Posse"},
  PanelText_UI_Team_Panel_Main_Text_C_Tips2_Content = {Text = "Team Info"},
  PanelText_UI_Team_Panel_Main_Text_C_Tips_1_Content = {Text = "Support"},
  PanelText_UI_Team_Panel_Main_Text_C_Tips_2_Content = {Text = "Support"},
  PanelText_UI_Team_Panel_Main_Text_C_Tips_Content = {Text = "Remove"},
  PanelText_UI_Team_Panel_Main_Text_Career_Content = {
    Text = "Realm Requirement"
  },
  PanelText_UI_Team_Panel_Main_Text_Madness_Content = {
    Text = "Madness difficulty only grants First-Clear Rewards."
  },
  PanelText_UI_Team_Panel_Trinket_Equip_Text_C_Delete_Content = {Text = "Delete Set"},
  PanelText_UI_Team_Panel_Trinket_Equip_Text_C_Edit_Content = {Text = "Edit Set"},
  PanelText_UI_Team_Panel_Trinket_Equip_Text_C_Recommend_Content = {
    Text = "Recommended"
  },
  PanelText_UI_Team_Panel_Trinket_Equip_Text_C_Remove_Content = {
    Text = "Unequip All"
  },
  PanelText_UI_Team_Panel_Trinket_Equip_Text_C_Save_Content = {Text = "Save Plan"},
  PanelText_UI_Team_Panel_Weapon_Equip_Text_C_Guide_Content = {
    Text = "Recommended"
  },
  PanelText_UI_Team_Panel_Weapon_Equip_Text_C_Hint_Content = {
    Text = "*Each Awakener can only equip one SSR Wheel of Destiny."
  },
  PanelText_UI_Team_Panel_Weapon_Equip_Text_C_Name_2_Content = {Text = "Unequip"},
  PanelText_UI_Team_Panel_Weapon_Equip_Text_C_Name_3_Content = {Text = "Ascension"},
  PanelText_UI_Team_Panel_Weapon_Equip_Text_C_Null_Content = {
    Text = "No Wheel of Destiny Yet"
  },
  PanelText_UI_Team_Panel_Weapon_Equip_Text_C_Title_Content = {
    Text = "Select Wheel of Destiny"
  },
  PanelText_UI_Team_Panel_Weapon_Equip_Text_C_Uninstall_Content = {
    Text = "Unequip All"
  },
  PanelText_UI_Team_Popup_Awaker_List_Text_C_Attribute_Content = {Text = "Attributes"},
  PanelText_UI_Team_Popup_Awaker_List_Text_C_Name_Content = {
    Text = "Awakener Details"
  },
  PanelText_UI_Team_Popup_Awaker_List_Text_C_Title_Content = {
    Text = "Select Awakener"
  },
  PanelText_UI_Team_Popup_Awaker_List_Text_C_Xx_Content = {Text = "Details"},
  PanelText_UI_Team_Popup_MyTeam_Awaker_List_Text_C_Name_Content = {Text = "Select"},
  PanelText_UI_Team_Popup_Prop_Award_Text_C_Name01_Content = {Text = "Chaos"},
  PanelText_UI_Team_Popup_Prop_Award_Text_C_Name02_Content = {Text = "Aequor"},
  PanelText_UI_Team_Popup_Prop_Award_Text_C_Name03_Content = {Text = "Caro"},
  PanelText_UI_Team_Popup_Prop_Award_Text_C_Name04_Content = {Text = "Ultra"},
  PanelText_UI_Team_Popup_Prop_Award_Text_C_Title_Content = {
    Text = "Class Talent"
  },
  PanelText_UI_Vindicate_Popup_Reward_Text_C_Task_Content = {
    Text = "Upon first completion, you will obtain:"
  },
  PanelText_UI_Vindicate_Popup_Reward_Text_Title_Content = {
    Text = "First-Clear"
  },
  PanelText_UI_Vx_Battle_Chaowei_Butter_Text_C_ExtraTurn_Content = {Text = "Extra Turn"},
  PanelText_UI_Weapon_Item_Select_List_Text_C_1_Normal_Content = {
    Text = "Sort by Ascension"
  },
  PanelText_UI_Weapon_Item_Select_List_Text_C_1_Select_Content = {
    Text = "Sort by Ascension"
  },
  PanelText_UI_Weapon_Item_Select_List_Text_C_2_Normal_Content = {Text = "Order"},
  PanelText_UI_Weapon_Item_Select_List_Text_C_2_Select_Content = {Text = "Order"},
  PanelText_UI_Weapon_Item_Select_List_Text_No_Weapon2_Content = {
    Text = "No Wheel of Destiny available"
  },
  PanelText__Text_C_Dress_Content = {Text = "Switch"},
  PanelText__Text_C_Dressing_Content = {Text = "Equipped"},
  PanelText__Text_C_Empty_Content = {Text = "Removed"},
  PanelText__Text_C_Goto_Content = {Text = "Purchase"},
  PanelText__Text_C_Have_Content = {
    Text = "Already Owned"
  },
  PanelText__Text_C_Reveal_Content = {Text = "Hidden"}
})
return Text_PanelText
