...
function gs_1126:ctor()
end

function gs_1126:OnGridBattleStart(role)
end

function gs_1126:__OnGridSkillInit()
  self.cskill = self.cEffectGrid.battleSkill
  self.caster = self.cskill.maker
  self.buff_num = 0
end

function gs_1126:OnGridEnterRole(role)
...
function gs_1126:OnGridExitRole(role)
  if self.atk ~= nil then
    self.atk:Stop()
    self.atk = nil
  end
