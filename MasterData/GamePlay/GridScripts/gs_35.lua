local gs_35 = class("gs_35", LuaGridBase)
...
function gs_35:OnGridBattleStart(role)
  self:StartCreatGrid()
end

function gs_35:OnArriveAction(x, y)
  return LuaSkillCtrl:CallCreateEfcGrid(x, y, 35)
end

function gs_35:StartCreatGrid()
...
function gs_35:OnFindEmptyGridAndSetEfc()
...
function gs_35:OnGridEnterRole(role)
  local buffTier = role:GetBuffTier(self.config.buffId)
  if 0 < buffTier then
    return
  end
  LuaSkillCtrl:CallBuff(self, role, 1501, 1, nil, true)
end

function gs_35:OnGridBattleEnd(role)
  self.growGrid = nil
  base.OnGridBattleEnd(self, role)
end

function gs_35:LuaDispose()
  self.growGrid = nil
  ...
  if role ~= nil and not role.isDead then
    LuaSkillCtrl:DispelBuff(role, 1501, 1)
  end
