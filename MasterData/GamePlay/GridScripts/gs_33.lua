local gs_33 = class("gs_33", LuaGridBase)
...
function gs_33:OnGridBattleStart(role)
  self:StartCreatGrid()
end

function gs_33:__OnGridSkillInit()
  LuaGridBase.__OnGridSkillInit(self)
  self:StartCreatGrid()
end

function gs_33:OnArriveAction(x, y)
  return LuaSkillCtrl:CallCreateEfcGrid(x, y, 33)
end

function gs_33:StartCreatGrid()
  if self.caster == nil then
    return
  end
  self.growGrid = self.caster.recordTable.growGrid33
  if self.growGrid == nil then
    self.growGrid = {}
    self.caster.recordTable.growGrid33 = self.growGrid
...
function gs_33:OnFindEmptyGridAndSetEfc()
...
function gs_33:OnGridEnterRole(role)
...
function gs_33:OnGridBeCorvered(role)
  self.growGrid = nil
  ...
  if role ~= nil and not role.isDead then
    LuaSkillCtrl:DispelBuff(role, 1276, 1)
  end
end

function gs_33:OnGridUncorver(role)
  if role ~= nil then
    self:OnGridEnterRole(role)
  end
end

function gs_33:OnGridBattleEnd(role)
  self.growGrid = nil
  base.OnGridBattleEnd(self, role)
end

function gs_33:LuaDispose()
  self.growGrid = nil
  if self.timer ~= nil then
    self.timer:Stop()
    self.timer = nil
