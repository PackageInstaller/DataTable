...
bs_310602.config = {
  ...
  effectId_passive = 10491702,
  effectId_shield = 10491802,
  buffId = 104901,
  buffId_stun = 3006,
  buffId_live = 3009,
...
function bs_310602:ctor()
end

function bs_310602:InitSkill(isMidwaySkill)
...
function bs_310602:OnAfterBattleStart()
...
function bs_310602:OnRealSummonerCaster(role)
...
function bs_310602:OnAfterHurt(sender, target, skill, hurt, isMiss, isCrit, hurtType, isTriggerSet)
...
function bs_310602:OnSetDeadHurt(context)
  if context.target == self.caster and context.target:GetBuffTier(self.config.buffId) > 0 and 0 >= context.target:GetBuffTier(self.config.nanaka_buffId) then
...
function bs_310602:OnBreakShield(shieldType, sender, target)
...
function bs_310602:OnCasterDie()
