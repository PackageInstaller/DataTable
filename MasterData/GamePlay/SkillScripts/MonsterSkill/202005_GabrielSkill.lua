local bs_202005 = class("bs_202003", LuaSkillBase)
...
function bs_202005:ctor()
end

function bs_202005:InitSkill(isMidwaySkill)
  base.InitSkill(self, isMidwaySkill)
  self:AddBuffDieTrigger("bs_202003_1", 1, self.OnBuffDie, nil, nil, self.config.buffId_278)
end

function bs_202005:PlaySkill(data)
  ...
  local role, grid, role_pf
  if targetList.Count > 0 then
    for i = 0, targetList.Count - 1 do
      if targetList[i].targetRole.roleDataId == 116 then
        role_pf = targetList[i].targetRole
        role = nil
      else
    ...
    if role == nil and role_pf ~= nil then
      role = role_pf
    end
...
function bs_202005:OnAttackTrigger(target, arg5, grid)
...
function bs_202005:Onover(target)
...
function bs_202005:Onhurt(target, grid)
...
function bs_202005:OnBuffDie(buff, target, removeType)
  if removeType == eBuffRemoveType.Dispel then
    target.recordTable.endhurt = 1
  end
end

function bs_202005:OnBreakSkill(role)
...
function bs_202005:OnCasterDie()
  LuaSkillCtrl:CanclePreSetPos(self.caster)
