...
function bs_105707:InitSkill(isMidwaySkill)
...
function bs_105707:OnRoleDie(killer, role)
  ...
  elseif role.belongNum == self.caster.belongNum and role.roleType == eBattleRoleType.realSummoner and killer.belongNum == eBattleRoleBelong.player then
    ...
    if tar ~= nil then
      ...
      LuaSkillCtrl:CallEffect(tar, self.config.effectId_trail, self, nil, self.caster, nil, true)
      LuaSkillCtrl:CallEffect(tar, self.config.effectId_trail, self, nil, self.caster, nil, true)
    end
    local targetList = LuaSkillCtrl:GetSelectTeamRoles(eBattleRoleBelong.player)
    if targetList.Count > 0 then
...
function bs_105707:SkillEventFunc(effect, eventId, target)
...
function bs_105707:OnSyncAttrFromHost()
...
function bs_105707:OnCasterDie()
