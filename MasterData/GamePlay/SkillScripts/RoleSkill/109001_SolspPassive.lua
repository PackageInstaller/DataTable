...
function bs_109001:InitSkill(isMidwaySkill)
  ...
  self:AddAfterAddBuffTrigger("bs_109001_2", 1, self.OnAfterAddBuff, nil, nil, nil, eBattleRoleBelong.enemy, self.config.buffId)
  self.times = 0
...
function bs_109001:Dodamage(target)
  ...
  if now_maxHp >= origin_maxHp * self.arglist[2] // 1000 and 1 < now_maxHp then
...
function bs_109001:check_dead()
  ...
  if targetList.Count > 0 then
    for i = targetList.Count - 1, 0, -1 do
      if i < targetList.Count and 0 <= i and targetList[i] ~= nil and 0 < targetList[i].hp and targetList[i].roleType == eBattleRoleType.realSummoner and targetList[i].summoner:HasSummonerFeature(8) then
        num = num - 1
      end
    end
  end
