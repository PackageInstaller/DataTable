...
function bs_15074:ctor()
end

function bs_15074:InitSkill(isMidwaySkill)
...
function bs_15074:OnAfterHurt(sender, target, skill, hurt, isMiss, isCrit, hurtType, isTriggerSet)
...
function bs_15074:OnAfterBattleStart()
  local buffTier = LuaSkillCtrl:CallFormulaNumberWithSkill(self.config.buffTierFormula, self.caster, self.caster, self)
  LuaSkillCtrl:CallBuff(self, self.caster, self.config.buffId, buffTier)
end

function bs_15074:OnAfterHeal(sender, target, skill, heal, isStealHeal, isCrit, isTriggerSet)
