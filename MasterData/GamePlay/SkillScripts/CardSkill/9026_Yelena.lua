local bs_9026 = class("bs_9026", LuaSkillBase)
local base = LuaSkillBase
bs_9026.config = {}

function bs_9026:ctor()
end

function bs_9026:InitSkill(isMidwaySkill)
  base.InitSkill(self, isMidwaySkill)
  self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_9026", 1, self.OnAfterBattleStart)
end

function bs_9026:OnAfterBattleStart()
  local Start_tag, target_Role
  local targetlist = LuaSkillCtrl:GetSelectTeamRoles(eBattleRoleBelong.player)
  if targetlist ~= nil then
    for i = 0, targetlist.Count - 1 do
      if targetlist[i].roleDataId == 1063 then
