...
local bs_618 = class(" bs_618", bs_1)
local base = bs_1
bs_618.config = {
  effectId_pass = 60301,
  radius = 30,
...
bs_618.config = setmetatable(bs_618.config, {
  __index = base.config
})

function bs_618:ctor()
end

function bs_618:InitSkill(isMidwaySkill)
  base.InitSkill(self, isMidwaySkill)
  self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_618_1", 1, self.OnAfterBattleStart)
  self.hurtTimers = {}
end

function bs_618:OnAfterBattleStart()
...
function bs_618:OnCollisionEnter(collider, index, entity)
  if entity.belongNum ~= self.caster.belongNum and self.hurtTimers[entity] == nil then
    ...
    end, self, self.config.hurtTime)
  end
end

function bs_618:OnCollisionExit(collider, entity)
...
function bs_618:RealPlaySkill(target, data)
  return
end

function bs_618:OnCasterDie()
