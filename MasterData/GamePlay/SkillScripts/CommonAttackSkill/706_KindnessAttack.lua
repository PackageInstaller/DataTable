...
bs_706.config = setmetatable(bs_706.config, {
  __index = base.config
})

function bs_706:ctor()
end

function bs_706:InitSkill(isMidwaySkill)
  base.InitSkill(self, isMidwaySkill)
  self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_706", 1, self.OnAfterBattleStart)
end

function bs_706:OnAfterBattleStart()
  LuaSkillCtrl:CallEffect(self.caster, self.config.effectId_sign, self)
end

function bs_706:RealPlaySkill(target, data)
