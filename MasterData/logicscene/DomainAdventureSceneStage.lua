-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logicscene/scene/component/stage/DomainAdventureSceneStage.lua

module("logicscene.scene.component.stage.DomainAdventureSceneStage", package.seeall)

local DomainAdventureSceneStage = class("DomainAdventureSceneStage", SceneCityStage)

function DomainAdventureSceneStage:load(sceneId, loadListener, loadListenerObj)
...
function DomainAdventureSceneStage:_getBasicResourceList()
	...
	local needLoadRes = {}
	local preloadFlags = {}
	local needLoadResDic = {}

	for i = 1, #self._stageResUrls do
		if not SceneDatasCache.IsLoaded(self._stageResUrls[i]) then
			needLoadResDic[self._stageResUrls[i]] = true
		end
	end

	needLoadResDic[tilingSceneConfig.sky] = true

	for k, v in pairs(DomAdvEnum.ResourceList) do
		needLoadResDic[v] = true
	end

	local resourceDataList = DomainAdventureConfig.instance:getResourceDataList()

	for i, data in ipairs(resourceDataList) do
		if not string.nilorempty(data.path) then
			needLoadResDic[data.path] = true
		end
	end

	for path, _ in pairs(needLoadResDic) do
		table.insert(needLoadRes, path)
...
function DomainAdventureSceneStage:_preloadSceneResources(camSizeH, camSizeW, camDetSizeW, camDetSizeH)
	DomainAdventureSceneStage.super._preloadSceneResources(self, camSizeH, camSizeW, camDetSizeW, camDetSizeH)
...
function DomainAdventureSceneStage:unload()
	if self._skyGo then
	...
	DomainAdventureSceneStage.super.unload(self)
end

function DomainAdventureSceneStage:_modifyBirthPoints()
...
function DomainAdventureSceneStage:getBirthPos()
	local x = self._scene:getBornX()
	local y = self._scene:getBornZ()
...
function DomainAdventureSceneStage:_createWholeScele()
	SceneDatasCacheMgr.instance:getSceneData(self._stageResUrls[1])

	return DomainAdventureSceneStage.super._createWholeScele(self)
end

function DomainAdventureSceneStage:onEnterSceneFinished()
	self:_buildSky()
	DomainAdventureSceneStage.super.onEnterSceneFinished(self)
end

function DomainAdventureSceneStage:_buildSky()
	local tilingSceneConfig = TilingSceneMgr.instance:getTilingSceneConfig(self._sceneId)
...
return DomainAdventureSceneStage
