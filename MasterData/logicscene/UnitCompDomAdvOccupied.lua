-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logicscene/scene/unit/component/domainadventure/UnitCompDomAdvOccupied.lua

module("logicscene.scene.unit.component.domainadventure.UnitCompDomAdvOccupied", package.seeall)

local UnitCompDomAdvOccupied = class("UnitCompDomAdvOccupied", UnitComponentBase)

UnitCompDomAdvOccupied.occupiedResPath = "scene/season/board_saijiwanfa_occupied.prefab"
...
function UnitCompDomAdvOccupied:onInit()
	self._occupiedResObject = nil
end

function UnitCompDomAdvOccupied:onDestroy()
	self:_clearOccupiedResObj()

	self._unit = nil
end

function UnitCompDomAdvOccupied:onUnitVisible(go)
	self:updateOccupiedStatus()
end

function UnitCompDomAdvOccupied:onUnitInVisible(go)
	self:_clearOccupiedResObj()
end

function UnitCompDomAdvOccupied:updateOccupiedStatus()
	...
	if not self._occupiedResObject then
		self._occupiedResObject = self._unit.scene.resCache:newObject(UnitCompDomAdvOccupied.occupiedResPath)
...
function UnitCompDomAdvOccupied:onSeasonMinimapOpenedOrClosed(opened)
	self:updateOccupiedStatus()
end

function UnitCompDomAdvOccupied:prepareMiniMapReady()
	local gridId = SeasonPathFindingDataModel.instance:grid2Id(self._unit.row, self._unit.col)
	local isOccupied = SeasonModel.instance:isOccupied(gridId)
...
function UnitCompDomAdvOccupied:_onOccupiedResLoadedFinish()
	if not self._occupiedResObject then
		return
	end
...
function UnitCompDomAdvOccupied:_isEdgeVisible(edgeIndex)
	if edgeIndex > 6 then
		edgeIndex = 1
	end
	...
	local dirs = Directions[dirIndex]
...
function UnitCompDomAdvOccupied:_checkEdgesVisible(edgeIndex)
	local edge = self._edges[edgeIndex]
...
function UnitCompDomAdvOccupied:_clearOccupiedResObj()
...
function UnitCompDomAdvOccupied:_getEdgeVertices(edgeIndex)
	local nextIdx = edgeIndex + 1

	if nextIdx > #HexVertices then
		nextIdx = 1
	end

	return HexVertices[edgeIndex], HexVertices[nextIdx]
end

return UnitCompDomAdvOccupied
