-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logicscene/scene/component/season/SceneCompSeasonPathFinder.lua

module("logicscene.scene.component.season.SceneCompSeasonPathFinder", package.seeall)

local SceneCompSeasonPathFinder = class("SceneCompSeasonPathFinder", ScenePathFinderEx)

function SceneCompSeasonPathFinder:ctor(scene)
	self._scene = scene
end

function SceneCompSeasonPathFinder:onEnterSceneFinished(sceneId, bornX, bornZ)
	SceneCompSeasonPathFinder.super.onEnterSceneFinished(self, sceneId, bornX, bornZ)
	self:_onBossInfoUpdated()
end

function SceneCompSeasonPathFinder:getPath(unit, startX, startY, endX, endY, state)
...
function SceneCompSeasonPathFinder:getGridPath(gridStartX, gridStartY, gridEndX, gridEndY)
...
function SceneCompSeasonPathFinder:onEnterScene(sceneId, bornX, bornZ)
	SceneCompSeasonPathFinder.super.onEnterScene(self, sceneId, bornX, bornZ)
	...
	GlobalDispatcher:addListener(GlobalNotify.SeasonBossInfoUpdated, self._onBossInfoUpdated, self)
end

function SceneCompSeasonPathFinder:onExitScene()
	SceneCompSeasonPathFinder.super.onExitScene(self)
	...
	GlobalDispatcher:removeListener(GlobalNotify.SeasonBossInfoUpdated, self._onBossInfoUpdated, self)
end

function SceneCompSeasonPathFinder:_onFogStateUpdated(id)
...
function SceneCompSeasonPathFinder:_onOccupiedUpdated(id)
	return
end

function SceneCompSeasonPathFinder:_onBossInfoUpdated()
	local map = SeasonPVEBossController.instance:getBlockStates()

	for k, v in pairs(map) do
		local gridX, gridY = SeasonPathFindingDataModel.instance:id2Grid(checknumber(k))
...
function SceneCompSeasonPathFinder:getFindWayGridWeight(gridX, gridY)
	return 1
end

function SceneCompSeasonPathFinder:isWalkable(x, y)
...
function SceneCompSeasonPathFinder:isGridWalkable(gridX, gridY)
...
function SceneCompSeasonPathFinder:isGridNearBy(formGridX, formGridY, toGridX, toGridY)
...
function SceneCompSeasonPathFinder:findNeighbours(gridX, gridY)
	local node = self._scene.pathFindingInfo:getInternalPathFinder():getNode(gridX, gridY)
...
function SceneCompSeasonPathFinder:isInMoveRange(formGridX, formGridY, toGridX, toGridY, range)
	local fromNode = self._scene.pathFindingInfo:getInternalPathFinder():getNode(formGridX, formGridY)
	local toNode = self._scene.pathFindingInfo:getInternalPathFinder():getNode(toGridX, toGridY)
...
function SceneCompSeasonPathFinder:isPassable(formGridX, formGridY, toGridX, toGridY)
	local fromNode = self._scene.pathFindingInfo:getInternalPathFinder():getNode(formGridX, formGridY)
	local toNode = self._scene.pathFindingInfo:getInternalPathFinder():getNode(toGridX, toGridY)
...
function SceneCompSeasonPathFinder:isNeighbourAndPassable(formGridX, formGridY, toGridX, toGridY)
...
function SceneCompSeasonPathFinder:findRandomNotBerrierAround(x, y, checkAirWaill)
	if self._scene and self._scene.pathFindingInfo then
		if self._scene.pathFindingInfo:getInternalPathFinder():isConfigReady() then
...
function SceneCompSeasonPathFinder:checkGridAroundWalkableOne(gridX, gridY, cols, rows, list)
	local gridXT, gridYT
...
return SceneCompSeasonPathFinder
