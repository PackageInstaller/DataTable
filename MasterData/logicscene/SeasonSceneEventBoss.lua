-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logicscene/scene/unit/component/season/events/SeasonSceneEventBoss.lua

module("logicscene.scene.unit.component.season.events.SeasonSceneEventBoss", package.seeall)

local SeasonSceneEventBoss = class("SeasonSceneEventBoss", SeasonSceneEventBossDoppelganger)

function SeasonSceneEventBoss:_getModelResPath()
...
function SeasonSceneEventBoss:_loadModelByResPath(resPath)
	if not resPath then
		return
	end
...
function SeasonSceneEventBoss:_needShowModel()
	...
	local bossPhaseId = checknumber(pevCfg.bossDoppelgangerPhaseId)

	return bossMo.phaseId ~= bossPhaseId
end

function SeasonSceneEventBoss:handleBossPhaseChange(prePhaseId, curPhaseId)
...
function SeasonSceneEventBoss:_onModelLoadedFinish()
	SeasonSceneEventBoss.super.super._onModelLoadedFinish(self)
...
function SeasonSceneEventBoss:setPlayedBossAppear()
	self._preparePlayerBossAppear = false

	self._unit:setVisible(true)
end

return SeasonSceneEventBoss
