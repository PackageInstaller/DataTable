-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/commontabrank/view/freescufflle/FreeScufflleWealthChildRankView.lua

module("logic.extensions.commontabrank.view.freescufflle.FreeScufflleWealthChildRankView", package.seeall)

local FreeScufflleWealthChildRankView = class("FreeScufflleWealthChildRankView", ViewComponent)

function FreeScufflleWealthChildRankView:buildUI()
	FreeScufflleWealthChildRankView.super.buildUI(self)
...
function FreeScufflleWealthChildRankView:bindEvents()
	FreeScufflleWealthChildRankView.super.bindEvents(self)
end

function FreeScufflleWealthChildRankView:unbindEvents()
	FreeScufflleWealthChildRankView.super.unbindEvents(self)
end

function FreeScufflleWealthChildRankView:onEnter()
	FreeScufflleWealthChildRankView.super.onEnter(self)
	...
	self.addGEvent(self, GlobalNotify.HandlePM_FreeScuffleRankViewRes, self._handleMsgRes, self)
	FreeScuffleController.instance:sendPM_FreeScuffleRankViewReq(self._activityId, FreeScuffleEnum.RankId_Coin)
end

function FreeScufflleWealthChildRankView:onExit()
	FreeScufflleWealthChildRankView.super.onExit(self)
	self._rankScrollList:dispose()
	self._myPrizeScrollList:dispose()
end

function FreeScufflleWealthChildRankView:_handleMsgRes(msg)
	if msg.rankId ~= FreeScuffleEnum.RankId_Coin then
		return
	end

	self._rankList = msg.infoList
	self._myRank = msg.myRank
	self._myValue = checknumber(msg.myScore)

	self:_onUpdate()
end

function FreeScufflleWealthChildRankView:_getPrizesByRank(rank)
	local data = FreeScuffleConfig.instance:getCoinRankPrizeData(self._activityId, rank)
...
function FreeScufflleWealthChildRankView:_onUpdate()
	...
	self._txtMyValue.text = MmUtil.formatNumber(self._myValue, FreeScuffleEnum.Units_Coin, 1)
...
function FreeScufflleWealthChildRankView:_updateRankCell(view, cell, info, tag)
	...
	txtValue.text = MmUtil.formatNumber(value, FreeScuffleEnum.Units_Coin, 1)
...
function FreeScufflleWealthChildRankView:_clearRankCell(cell)
	local mainGo = cell.gameObject
	local headRoot = goutil.findChild(mainGo, "headRoot")
...
function FreeScufflleWealthChildRankView:_updateItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function FreeScufflleWealthChildRankView:_clearItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

function FreeScufflleWealthChildRankView:_updateMyPrizeItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function FreeScufflleWealthChildRankView:_clearMyPrizeItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

return FreeScufflleWealthChildRankView
