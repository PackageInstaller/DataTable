-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/commontabrank/view/magiccontest/MagicContestChildRankView.lua

module("logic.extensions.commontabrank.view.magiccontest.MagicContestChildRankView", package.seeall)

local MagicContestChildRankView = class("MagicContestChildRankView", ViewComponent)

function MagicContestChildRankView:buildUI()
	MagicContestChildRankView.super.buildUI(self)
...
function MagicContestChildRankView:bindEvents()
	MagicContestChildRankView.super.bindEvents(self)
end

function MagicContestChildRankView:unbindEvents()
	MagicContestChildRankView.super.unbindEvents(self)
end

function MagicContestChildRankView:onEnter()
	MagicContestChildRankView.super.onEnter(self)

	local params = self:getOpenParam() or {}

	self._activityId = checknumber(params[1])
	self._batType = params[2]
	...
	self.addGEvent(self, GlobalNotify.HandlePM_MagicContestGetRankInfoRes, self._handleMsgRes, self)
	MagicContestController.instance:sendPM_MagicContestGetRankInfoReq(self._activityId, self._batType)
end

function MagicContestChildRankView:onExit()
	MagicContestChildRankView.super.onExit(self)
	self._rankScrollList:dispose()
	self._myPrizeScrollList:dispose()
end

function MagicContestChildRankView:_handleMsgRes(msg)
	if msg.type ~= self._batType then
		return
	end
...
function MagicContestChildRankView:_getPrizesByRank(rank)
	local data = MagicContestConfig.instance:getRankPrizeDataByRank(self._activityId, self._batType, rank)
...
function MagicContestChildRankView:_onUpdate()
...
function MagicContestChildRankView:_updateRankCell(view, cell, info, tag)
...
function MagicContestChildRankView:_clearRankCell(cell)
	local mainGo = cell.gameObject
	local headRoot = goutil.findChild(mainGo, "headRoot")
...
function MagicContestChildRankView:_updateItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function MagicContestChildRankView:_clearItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

function MagicContestChildRankView:_updateMyPrizeItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function MagicContestChildRankView:_clearMyPrizeItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

return MagicContestChildRankView
