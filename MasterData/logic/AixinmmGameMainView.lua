-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/aixinmmgame/view/AixinmmGameMainView.lua

module("logic.extensions.aixinmmgame.view.AixinmmGameMainView", package.seeall)

local AixinmmGameMainView = class("AixinmmGameMainView", ViewComponent)
local LineEnum = {
	Down = 1,
	Up = 3,
	Mid = 2
...
function AixinmmGameMainView:ctor()
	AixinmmGameMainView.super.ctor(self)
end

function AixinmmGameMainView:buildUI()
	AixinmmGameMainView.super.buildUI(self)
	...
	self._fbbPlayerClass = GameUtil.AddLuaOnce(self._fbbPlayerGo, AixinmmGamePlayer)
	self._wyPlayerClass = GameUtil.AddLuaOnce(self._wyPlayerGo, AixinmmGamePetPlayer)
...
function AixinmmGameMainView:bindEvents()
	AixinmmGameMainView.super.bindEvents(self)
...
function AixinmmGameMainView:unbindEvents()
	AixinmmGameMainView.super.unbindEvents(self)
...
function AixinmmGameMainView:destroyUI()
	AixinmmGameMainView.super.destroyUI(self)
	self._moveItemPool:clear()
end

function AixinmmGameMainView:onEnter()
	AixinmmGameMainView.super.onEnter(self)

	self._activityId = self:getFirstParam()

	self._fbbPlayerClass:onEnter(self)
	self._wyPlayerClass:onEnter()
	AixinmmGameController.instance:startGame(self._activityId)
...
function AixinmmGameMainView:onEnterFinished()
	AixinmmGameMainView.super.onEnterFinished(self)
end

function AixinmmGameMainView:onExit()
	AixinmmGameMainView.super.onExit(self)
	self:_stop()
...
function AixinmmGameMainView:onExitFinished()
	AixinmmGameMainView.super.onExitFinished(self)
end

function AixinmmGameMainView:_onClickbtnClose()
	self:_pause(true)
...
function AixinmmGameMainView:_onClickbtnDown()
...
function AixinmmGameMainView:_onClickbtnUp()
	if not self._playCanMoveLine then
		return
	end
...
function AixinmmGameMainView:_onClickgamestarTip()
	goutil.setActive(self._gamestarTip.gameObject, false)
	self:_start()
end

function AixinmmGameMainView:_initMoveCells()
	self._moveItems = {}
	...
	local function createFunc()
		...
		return {
			...
			class = GameUtil.AddLuaOnce(go, AixinmmGameMoveItem)
		}
	end
...
function AixinmmGameMainView:_prepareData()
	self._sceneMoveInitSpeed = AixinmmGameConfig.instance:getDefineValue("SCENE_MOVE_SPEED", true)
	self._sceneMoveSpeed = self._sceneMoveInitSpeed
	self._sceneMoveSpeedMax = AixinmmGameConfig.instance:getDefineValue("SCENE_MOVE_SPEED_MAX", true)
	self._into_max_speed_need_time = AixinmmGameConfig.instance:getDefineValue("INTO_MAX_SPEED_TIME", true)
	self._moveDataList = AixinmmGameModel.instance:prepareGameDatas(self._activityId)
	self._battleSlotIndex = AixinmmGameConfig.instance:getDefineValue("BATTLE_SLOT", true)
	self._warinning_tip_slot = AixinmmGameConfig.instance:getDefineValue("WARINING_TIP_SLOT", true)
	self._wyHeadPosInitRatio = AixinmmGameConfig.instance:getDefineValue("WY_INIT_RATIO", true) / 100
	self._currPlayerLineType = LineEnum.Mid
	self._every_hit_reduce_hp_ratio = AixinmmGameConfig.instance:getDefineValue("EVERY_HIT_REDUCE_HP_RATIO", true) / 100
	self._every_hit_time = AixinmmGameConfig.instance:getDefineValue("EVERY_HIT_TIME", true)
	self._playTime = 0
	self._wy_playspeed = AixinmmGameConfig.instance:getDefineValue("WY_MOVE_SPEED", true)
	self._itemSpace = 150
...
function AixinmmGameMainView:_initMoveItems()
	local count = 15
...
function AixinmmGameMainView:_setPlayerPos()
	self._fbbPlayerClass:setAnchoredPos(155, (self._currPlayerLineType - 2) * self._lineSpace)
end

function AixinmmGameMainView:_setItemPos(item)
...
function AixinmmGameMainView:_start()
	self._isStopMove = false
	self._isPause = false
...
function AixinmmGameMainView:_pause(closeWinState)
	self._isPause = true
	self._closeWinState = closeWinState
end

function AixinmmGameMainView:_resume()
	self._isPause = false
	self._closeWinState = false
end

function AixinmmGameMainView:_stop()
...
function AixinmmGameMainView:_update()
...
function AixinmmGameMainView:_itemsMove()
...
function AixinmmGameMainView:_creatNextItem(item)
...
function AixinmmGameMainView:_delayCreatNextItem(hitItem)
...
function AixinmmGameMainView:_checkHasDelayAndDoCreatNextItem()
...
function AixinmmGameMainView:_isMaxSlot()
	return self._runLength >= self._battleSlotIndex * self._itemSpace
end

function AixinmmGameMainView:_isWarnningSlot()
	return self._runLength >= self._warinning_tip_slot * self._itemSpace
end

function AixinmmGameMainView:_setItemOnEnter(item)
...
function AixinmmGameMainView:_checkHit()
...
function AixinmmGameMainView:_JLBattle()
...
function AixinmmGameMainView:_delayDoHitFinish()
	self._playCanMoveLine = true
...
function AixinmmGameMainView:_calBattleData(targetCurPosX, targetHP)
...
function AixinmmGameMainView:_onBattle(hitCallBack, hitFinishCallBack)
	local tweenIndex = 0
...
function AixinmmGameMainView:_gameOver()
	self._fbbPlayerClass:exitBattle()
	self:_stop()
	AixinmmGameController.instance:settleGame(self._activityId, self._fbbPlayerClass:getHP())
...
function AixinmmGameMainView:_gotoWYBattle()
...
function AixinmmGameMainView:_playWarning()
	...
	local time = AixinmmGameConfig.instance:getDefineValue("WARNING_TIME", true)

	settimer(time, self._stopWarning, self, false)
end

function AixinmmGameMainView:_stopWarning()
	removetimer(self._stopWarning, self)
	goutil.setActive(self._warningTipGo, false)
end

function AixinmmGameMainView:_wyMoveToScene()
...
function AixinmmGameMainView:_fbbMoveToWy()
	for k, v in pairs(self._moveItems) do
		self._moveItemPool:returnObject(v)
	end
...
function AixinmmGameMainView:_onWYBattle()
	self:_stop()
...
function AixinmmGameMainView:_gotoEatScoreItem(hitItem)
	local duration = 0.2
...
function AixinmmGameMainView:_getCurrSceneMoveSpeed()
	return self._sceneMoveSpeed
end

function AixinmmGameMainView:_setCurrSceneMoveSpeed()
...
function AixinmmGameMainView:_bgMove()
...
function AixinmmGameMainView:_setFBBHeadPos(isMax)
	local ratio = 1
...
function AixinmmGameMainView:_setWYHeadPos(isMax)
	local ratio = 1
...
function AixinmmGameMainView:_endGameRes()
	return
end

function AixinmmGameMainView:_getOneHitReduceHp()
...
function AixinmmGameMainView:_retrurnOutHitItemToPool()
...
function AixinmmGameMainView:_loadRoles()
	if not self._fbbRoleGo then
		local go = self:getResInstance(AixinmmGameMainViewPresentor.FBBCharacterPath)
		...
		if go then
			...
			Framework.TransformUtil.SetLocalPos(tr, 0, -60, 0)
			Framework.TransformUtil.SetLocalScale(tr, scale, scale, scale)
			Framework.TransformUtil.SetLocalRotation(tr, 0, 0, 0)
		end
	end

	RoleObjectPool.instance:playAnimation(self._fbbRoleGo, "fly", true)
end

return AixinmmGameMainView
