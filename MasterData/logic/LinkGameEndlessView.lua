-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/linkgame/view/LinkGameEndlessView.lua

module("logic.extensions.linkgame.view.LinkGameEndlessView", package.seeall)

local LinkGameEndlessView = class("LinkGameEndlessView", LinkGameBaseView)

LinkGameEndlessView.hideTime = 0.1

function LinkGameEndlessView:ctor()
	LinkGameEndlessView.super.ctor(self)
end

function LinkGameEndlessView:bindEvents()
	LinkGameEndlessView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self._onClickClose, self)
	GameUtil.addClickHandler(self._btnRule, self._onClickRule, self)
	GameUtil.addClickHandler(self._btnStart, self._onClickStart, self)
end

function LinkGameEndlessView:unbindEvents()
	LinkGameEndlessView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
	GameUtil.rmClickHandler(self._btnRule)
	GameUtil.rmClickHandler(self._btnStart)
end

function LinkGameEndlessView:buildUI()
	LinkGameEndlessView.super.buildUI(self)
...
function LinkGameEndlessView:onEnter()
	self.addGEvent(self, GlobalNotify.PopupImageRuleClose, self._onCloseRuleView, self)

	local params = self:getOpenParam()

	self._gameId = checknumber((params or nil) and (params[1] or 1))
	self._ruleId = checknumber((params or nil) and (params[2] or 1))
	self._exInfo = params[3] or {}
	self._finishCallBack = self._exInfo.finishCallBack
	self._gameTime = 60
	self._pairs = 0

	LinkGameEndlessView.super.onEnter(self)
...
function LinkGameEndlessView:onExit()
	LinkGameEndlessView.super.onExit(self)
...
function LinkGameEndlessView:setMapData()
	self._miniGameCfg = LinkGameConfig.instance:getMiniGameCfg(self._gameId)
...
function LinkGameEndlessView:_readyStartGame()
	GameUtil.SetActive(self._btnStart, true)
end

function LinkGameEndlessView:_startGame()
	LinkGameEndlessView.super._startGame(self)
...
function LinkGameEndlessView:_onDeadGame()
...
function LinkGameEndlessView:_updateCell(cell, cellInfo)
...
function LinkGameEndlessView:_onSelectEnd()
	self._txtPairs.text = langPara("已消除对数：<color=#63EFF7FF>%d</color>", self:_getCurRemovePairs())

	LinkGameEndlessView.super._onSelectEnd(self)
end

function LinkGameEndlessView:_checkGame()
...
function LinkGameEndlessView:_getCurRemovePairs()
	return LinkGameModel.instance:getRemovePairs() + checknumber(self._pairs)
end

function LinkGameEndlessView:_showLinkEffect()
...
function LinkGameEndlessView:_hideLinkEffect()
	for i, cellInfo in ipairs(self._curPath) do
		if self._linkObject[cellInfo.row] and self._linkObject[cellInfo.row][cellInfo.col] and i > 1 and i < #self._curPath then
			...
			for j = 0, linkGo.transform.childCount - 1 do
				...
				for k = 0, stateGo.transform.childCount - 1 do
					...
					UnityTweens.UITweenFadeOut.StartTween(imageGO.gameObject, LinkGameEndlessView.hideTime)
				end
			end
		end
		...
		if self._mapObject[cellInfo.row] and self._mapObject[cellInfo.row][cellInfo.col] and (i == 1 or i == #self._curPath) then
			...
			UnityTweens.UITweenFadeOut.StartTween(bg, LinkGameEndlessView.hideTime)
			UnityTweens.UITweenFadeOut.StartTween(imgSelect, LinkGameEndlessView.hideTime)
			UnityTweens.UITweenFadeOut.StartTween(imgCon, LinkGameEndlessView.hideTime)
	...
	removetimer(self._onLinkEffectEnd, self)
	settimer(LinkGameEndlessView.hideTime + 0.01, self._onLinkEffectEnd, self, false)
end

function LinkGameEndlessView:setCellLinkState(cellInfo, isShow, linkState, param)
...
function LinkGameEndlessView:getCellSize()
	return {
		x = 104,
		y = 104
	}
...
function LinkGameEndlessView:getCellOffset()
	return {
		x = 4.4,
		y = 0
	}
...
function LinkGameEndlessView:_onLinkEffectEnd()
...
function LinkGameEndlessView:_endGame()
	LinkGameEndlessView.super._endGame(self)
...
function LinkGameEndlessView:_doFinfishCallBack(isPass, removePairs)
	local info = {}
...
function LinkGameEndlessView:_onClickClose()
	self:_pauseGame(self._gameTime)
...
function LinkGameEndlessView:_onClickRule()
	self:_pauseGame(self._gameTime)

	local ruleCfg = LinkGameConfig.instance:getRuleCfgs(self._ruleId or 1)
	local ruleList = {}
...
function LinkGameEndlessView:_onClickStart()
	self:_startGame()
end

function LinkGameEndlessView:_pauseGame(defautTime)
	self._isPauseGame = true
	self._saveTime = checknumber(self._targetTime) > 0 and self._targetTime - ServerTime.now() or defautTime
end

function LinkGameEndlessView:_recoverGame()
...
function LinkGameEndlessView:_onClock()
	...
	if ServerTime.now() > self._targetTime then
		...
		self._txtTime.text = langPara("%d", 0)
	else
		self._txtTime.text = langPara("%d", self._targetTime - ServerTime.now())
	end
end

function LinkGameEndlessView:_onCloseRuleView()
