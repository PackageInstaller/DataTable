-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/season/view/SeasonTotemPoleView.lua

module("logic.extensions.season.view.SeasonTotemPoleView", package.seeall)

local SeasonTotemPoleView = class("SeasonTotemPoleView", ViewComponent)

SeasonTotemPoleView.refreshEffect = "20211028/xunlianchang/fx_ui_xlc_shuaxin_da.prefab"

function SeasonTotemPoleView:ctor()
	SeasonTotemPoleView.super.ctor(self)
end

function SeasonTotemPoleView:unbindEvents()
	SeasonTotemPoleView.super.unbindEvents(self)
	...
	for i, item in ipairs(self._totemItems) do
		GameUtil.rmClickHandler(goutil.findChild(item, "btnActivation"))
	end
...
function SeasonTotemPoleView:bindEvents()
	SeasonTotemPoleView.super.bindEvents(self)
	...
	for i, item in ipairs(self._totemItems) do
		GameUtil.addClickHandler(goutil.findChild(item, "btnActivation"), GameUtil.handler(self._onClickTotem, self, i))
	end
...
function SeasonTotemPoleView:buildUI()
	SeasonTotemPoleView.super.buildUI(self)

	self._btnClose = self:getGo("btnClose")
	self._txtPower = self:getTxt("info/power/txtPower")
	self._txtLevel = self:getTxt("info/totempole/level/txtLevel")
	self._btnLevelUp = self:getGo("info/totempole/btnLevelUp")
	self._btnLevelUpRed = self:getGo("info/totempole/btnLevelUp/redpoint")
...
function SeasonTotemPoleView:onExit()
	SeasonTotemPoleView.super.onExit(self)
...
function SeasonTotemPoleView:onEnter()
	SeasonTotemPoleView.super.onEnter(self)
	...
	local barList = string.split(totemPolConfig.barList or "", "#")
	local btnList = {}
	...
	GameUtil.SetActive(self._btnTips, true)
end

function SeasonTotemPoleView:_onTotemInfoUpdated()
	self:_updateTabSelectStates()
	self:_updateCurTabDetail()
	self:_updateSummary()
end

function SeasonTotemPoleView:_updateTabSelectStates()
	for i, item in ipairs(self._raceFilterTabs) do
		GameUtil.SetActive(goutil.findChild(item, "select"), i == self._selectTabId)

		local boo = SeasonMainCampController.instance:checkTotemTabRed(i)

		GameUtil.SetActive(self._raceFilterTabsReds[i], boo)
	end

	GameUtil.setUIImageSpriteIdx(self._totempoleIcon, self._selectTabId - 1)
end

function SeasonTotemPoleView:_updateCurTabDetail()
	local model = SeasonMainCampModel.instance
	local totemInfo = model:getTotemPoleInfoByRaceType(self._userId, self._selectTabId)

	if totemInfo then
		for i, item in ipairs(self._totemItems) do
			local btnActivation = goutil.findChild(item, "btnActivation")
			...
			GameUtil.SetActive(btnActivation, totemInfo:isSmallTotemCanUpgrade(i))
			...
			if isGray and nextConfig then
				...
				GameUtil.SetActive(redpoint, isAllEnough)
			end
		end

		local isAct = totemInfo:isMainTotemCanUpgrade()

		GameUtil.SetActive(self._btnLevelUp, isAct)
		...
		if isAct then
			...
			if needCampLv <= SeasonMainCampModel.instance:getCampLv() then
				GameUtil.SetActive(self._btnLevelUpRed, isAllEnough)
			end
		end
...
function SeasonTotemPoleView:_playCellEff(go, path, loop)
	if not go then
		return
	end
...
function SeasonTotemPoleView:_removeCellEff(go)
	if not go then
		return
	end
...
function SeasonTotemPoleView:_onTotemPropertyEffectUpdate()
	self:_playCellEff(self._goEffect, SeasonTotemPoleView.refreshEffect, false)
end

function SeasonTotemPoleView:_updateSummary()
...
function SeasonTotemPoleView:_onClickTab(index)
...
function SeasonTotemPoleView:_onClickTotem(index)
	local totemInfo = SeasonMainCampModel.instance:getTotemPoleInfoByRaceType(self._userId, self._selectTabId)
...
function SeasonTotemPoleView:_onClickLevelUp()
	local totemInfo = SeasonMainCampModel.instance:getTotemPoleInfoByRaceType(self._userId, self._selectTabId)
...
function SeasonTotemPoleView:_onClickTips()
	ViewMgr.instance:open(ViewName.RulesView, "seasonTotem_rule")
end

function SeasonTotemPoleView:_onClickSkill(id)
