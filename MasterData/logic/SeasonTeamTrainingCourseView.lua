-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/season/view/teamcampshow/SeasonTeamTrainingCourseView.lua

module("logic.extensions.season.view.teamcampshow.SeasonTeamTrainingCourseView", package.seeall)

local SeasonTeamTrainingCourseView = class("SeasonTeamTrainingCourseView", ViewComponent)

SeasonTeamTrainingCourseView.propertyEffect = "20211028/xunlianchang/fx_ui_xlc_shuaxin_da.prefab"

function SeasonTeamTrainingCourseView:ctor()
	SeasonTeamTrainingCourseView.super.ctor(self)
end

function SeasonTeamTrainingCourseView:unbindEvents()
	SeasonTeamTrainingCourseView.super.unbindEvents(self)
...
function SeasonTeamTrainingCourseView:bindEvents()
	SeasonTeamTrainingCourseView.super.bindEvents(self)
...
function SeasonTeamTrainingCourseView:buildUI()
	SeasonTeamTrainingCourseView.super.buildUI(self)
...
function SeasonTeamTrainingCourseView:onExit()
	SeasonTeamTrainingCourseView.super.onExit(self)
	GlobalDispatcher:removeListener(GlobalNotify.SeasonFmtTeamInfo, self.onRefreshUI, self)
end

function SeasonTeamTrainingCourseView:onEnter()
	SeasonTeamTrainingCourseView.super.onEnter(self)
	...
	self.addGEvent(self, GlobalNotify.handlePM_SMGVETeammateAllTeamInfoRes, self.onRefreshUI, self)
	SeasonModePvePlusAgent.instance:sendPM_SMGVETeammateAllTeamInfoReq(self._teammateId)
	GameUtil.SetActive(self.btnGoto, false)
	GameUtil.SetActive(self.txtCost, false)
	GameUtil.SetActive(self.btnTip, false)
end

function SeasonTeamTrainingCourseView:_onPropertyEffect()
	self:_playCellEff(self._effect)
end

function SeasonTeamTrainingCourseView:_openGoldBarCon()
...
function SeasonTeamTrainingCourseView:onClickClose()
	SeasonFmtController.instance.curSelectTrainTeamId = nil

	self:close()
end

function SeasonTeamTrainingCourseView:onClickGoto()
	SeasonFmtController.instance:showFormationView(self.curSelectTeamId)
end

function SeasonTeamTrainingCourseView:onClickHelp()
	TipsFacade.instance:openRulesView("SeasonTeamTrainingCourseView")
end

function SeasonTeamTrainingCourseView:showFormation(teamMo)
	for i = 1, 9 do
		local raceId = checknumber(((teamMo and teamMo.posRaceIds or nil) and teamMo.posRaceIds)[i])
...
function SeasonTeamTrainingCourseView:onRefreshUI()
	self.curCost = ""
	self.txtCost.text = ""
	...
	local seasonId = SeasonModel.instance:getSeasonId()
	local campInfo = SeasonMainCampModel.instance:getMainCampInfoResOfOther()
	local campLv = campInfo.camplv
	local tgCfg = SeasonMainCampConfig.instance:getTrainingGroundConfig(seasonId)
	local teamPlanId = tgCfg.teamPlanId
	local teams = SeasonTeamsModel.instance:getTeammateAllTeamInfo() or {}
	...
	local attrs = {}
	local nextattrs = {}
	local teamMo = teams[self.curSelectTeamId]
	...
	for i, v in ipairs(self.tabList) do
		GameUtil.SetActive(self.tabRedList[i], false)
	end
end

function SeasonTeamTrainingCourseView:_playCellEff(go)
	if not go then
		return
	end
	...
	local resPath = SeasonTeamTrainingCourseView.propertyEffect
...
function SeasonTeamTrainingCourseView:_removeCellEff(go)
	if not go then
		return
	end
...
function SeasonTeamTrainingCourseView:showAttrList(attrs, nextattrs)
...
function SeasonTeamTrainingCourseView:getCompareAttrList(attrs, nextattrs)
	local showList = {}
...
function SeasonTeamTrainingCourseView:onTabClick(idx)
...
function SeasonTeamTrainingCourseView:onClickLvlup()
...
return SeasonTeamTrainingCourseView
