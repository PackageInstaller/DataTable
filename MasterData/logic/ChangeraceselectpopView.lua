-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/formation/view/ChangeraceselectpopView.lua

module("logic.extensions.formation.view.ChangeraceselectpopView", package.seeall)

local ChangeraceselectpopView = class("ChangeraceselectpopView", ViewComponent)

function ChangeraceselectpopView:buildUI()
	ChangeraceselectpopView.super.buildUI(self)
...
function ChangeraceselectpopView:bindEvents()
	ChangeraceselectpopView.super.bindEvents(self)
	self._closeButton:AddClickListener(self._onClickClose, self)
	self._btnCancel:AddClickListener(self._onClickClose, self)
	self._btnSure:AddClickListener(self._onClickSure, self)
end

function ChangeraceselectpopView:unbindEvents()
	ChangeraceselectpopView.super.unbindEvents(self)
	self._closeButton:RemoveClickListener()
	self._btnCancel:AddClickListener(self._onClickClose, self)
	self._btnSure:AddClickListener(self._onClickSure, self)
end

function ChangeraceselectpopView:onEnter()
	ChangeraceselectpopView.super.onEnter(self)
...
function ChangeraceselectpopView:onExit()
	ChangeraceselectpopView.super.onExit(self)
	self:_removeIcons()
end

function ChangeraceselectpopView:_onClickClose()
	self:close()
end

function ChangeraceselectpopView:_onClickSure()
	local petMo = self._petMoList[self._selectLeftIdx]

	if petMo then
		if self._selectPetId > 0 then
			if petMo.carriedPetId ~= self._selectPetId then
				PetAgent.instance:sendPM_CarriedLinkReq(petMo.petId, self._selectPetId)
			end
		elseif checknumber(petMo.carriedPetId) > 0 then
			PetAgent.instance:sendPM_CancelCariedLinkReq(petMo.petId, petMo.carriedPetId)
		end
	end
...
function ChangeraceselectpopView:_updateLeftCell(view, cell, data)
...
function ChangeraceselectpopView:_clearLeftCell(cell)
	local btnClick = Framework.ButtonAdapter.GetFrom(cell.gameObject, "btn")
	local node = goutil.findChild(cell, "node")
...
function ChangeraceselectpopView:_updateRightCell(view, cell, data)
	local node = goutil.findChild(cell, "node")
	local goSelect = goutil.findChild(cell, "select")
	local btnClick = Framework.ButtonAdapter.GetFrom(cell.gameObject, "btn")
...
function ChangeraceselectpopView:_clearRightCell(cell)
	local btnClick = Framework.ButtonAdapter.GetFrom(cell.gameObject, "btn")
	local node = goutil.findChild(cell, "node")
...
function ChangeraceselectpopView:_updateLeftList()
	self._leftScrollList:reloadData(self._petMoList)
	self:_updateRightList()
end

function ChangeraceselectpopView:_updateRightList()
	self._txtSkillName.text = "眷佑技"
	...
	if petMo then
		for i, mo in ipairs(self._allPetList) do
			if ContractConfig.instance:getCarriedByRaceId(petMo.raceId, mo.raceId) then
				table.insert(list, mo)
			end
		end

		self._selectPetId = petMo.carriedPetId
...
function ChangeraceselectpopView:_onClickLeftBtn(petMo, index)
...
function ChangeraceselectpopView:_onClickRightBtn(data)
	self._selectPetId = self._selectPetId == data.petId and 0 or data.petId

	self._rightScrollList:refresh()
end

function ChangeraceselectpopView:_updateRightSelect(newSelectIndex)
...
function ChangeraceselectpopView:_updateSkillShow(raceId)
	...
	if checknumber(raceId) > 0 then
		...
		if checknumber(petCfg.carriedSkillId) > 0 then
			MaterialMgr.setCell(MatType.Skill, petCfg.carriedSkillId, self._skillIcon)

			local cfg = BattleConfig.instance:getSkillCo(petCfg.carriedSkillId, raceId)
...
function ChangeraceselectpopView:_removeIcons()
	MaterialMgr.resetAll(self._skillIcon)
end

return ChangeraceselectpopView
