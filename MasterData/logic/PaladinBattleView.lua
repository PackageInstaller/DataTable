-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/moonking/view/PaladinBattleView.lua

module("logic.extensions.moonking.view.PaladinBattleView", package.seeall)

local PaladinBattleView = class("PaladinBattleView", ViewComponent)

function PaladinBattleView:ctor()
	PaladinBattleView.super.ctor(self)
...
function PaladinBattleView:bindEvents()
	PaladinBattleView.super.bindEvents(self)
	self._closeBtn:AddClickListener(self.close, self)
	self._tipsBtn:AddClickListener(function()
		UIStateManager.instance:push(ViewName.RulesView, "paladinbattle")
	end, self)
...
function PaladinBattleView:unbindEvents()
	PaladinBattleView.super.unbindEvents(self)
...
function PaladinBattleView:onExit()
	PaladinBattleView.super.onExit(self)
...
function PaladinBattleView:destroyUI()
	PaladinBattleView.super.destroyUI(self)
...
function PaladinBattleView:buildUI()
	PaladinBattleView.super.buildUI(self)
...
function PaladinBattleView:registCallbacks()
...
function PaladinBattleView:_MoshiInView()
...
function PaladinBattleView:_MoshiAtIndex(view, index)
	local cell
...
function PaladinBattleView:_OnMoshiCellUpdate(view, cell)
...
function PaladinBattleView:_RewardInView()
...
function PaladinBattleView:_RewardAtIndex(view, index)
	local cell
	local cell = view:DequeueCell()
...
function PaladinBattleView:_OnRewardCellUpdate(view, cell)
	local index = cell.index
	local data = self._curRewardDatas[index + 1]
...
function PaladinBattleView:onEnter()
	PaladinBattleView.super.onEnter(self)
	self:_startPlayVoice()
	...
	if curChallId <= 0 then
		curChallId = MoonKingModel.instance.pbChallendId
	end

	self._bubbleTxt.text = lang("text_tong_desc_29")

	MoonKingModel.instance:setMoonkingChooseBuff()

	local str = string.format("PaladinViewRed_%s_%s", curChallId, LoginModel.instance.userId)
	...
	local effPath = "20220729/shengqixiaoyan/fx_ui_xiaoyan_ksqs02.prefab"
...
function PaladinBattleView:_showActivityTimer()
...
function PaladinBattleView:_calculationSurplusTime()
...
function PaladinBattleView:_startPlayVoice()
	local plotId = 4290012
	local str = "PaladinBattleView_PlotId_" .. plotId

	if checknumber(GameUtil.getUserData(str)) == 1 then
		return
	end

	GlobalDispatcher:dispatch(GlobalNotify.PushStory, plotId)
	GameUtil.saveUserData(str, 1)
end

function PaladinBattleView:_updataBattleViewInfo(challId, isPrize)
	...
	local have = MaterialMgr.getMatCount(self._challCfg.daibiStr)
...
function PaladinBattleView:_updateMoshiCell(view, cell, data)
	local selectGo = goutil.findChild(cell, "selectGo")
	local scoreImaGo = goutil.findChild(cell, "scoreIma")
	...
	selectGo:SetActive(cell.data == self._curIndex)
...
function PaladinBattleView:_showChoiseStageInfo(index)
...
function PaladinBattleView:_updateRewardCell(view, cell, data)
	...
	if not data.isFull then
		GameUtil.SetActive(itemClickGo, false)

		return
	end

	GameUtil.SetActive(itemClickGo, true)
...
function PaladinBattleView:_updateShowProgressBar()
	if self._curRewardDatas then
		if not #self._curRewardDatas then
			local count = 0
...
function PaladinBattleView:_onClickNanduBtn()
