-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/jieshenevent/view/JieShenEventGameView.lua

module("logic.extensions.jieshenevent.view.JieShenEventGameView", package.seeall)

local JieShenEventGameView = class("JieShenEventGameView", ViewComponent)

function JieShenEventGameView:buildUI()
	JieShenEventGameView.super.buildUI(self)
...
function JieShenEventGameView:bindEvents()
	JieShenEventGameView.super.bindEvents(self)
...
function JieShenEventGameView:unbindEvents()
	JieShenEventGameView.super.unbindEvents(self)
...
function JieShenEventGameView:onEnter()
	JieShenEventGameView.super.onEnter(self)
	self.addGEvent(self, GlobalNotify.PM_JieShenEventFinishGameRes, self._onPassGame, self)
	...
	self._txtRule.text = self._ruleCfg and self._ruleCfg.rules or lang("缺失配置")
end

function JieShenEventGameView:onEnterFinished()
	JieShenEventGameView.super.onEnterFinished(self)
	...
	MaterialMgr.setCellByCfg(self._informerCfg.prize, self._itemShow)
end

function JieShenEventGameView:onExit()
	JieShenEventGameView.super.onExit(self)
...
function JieShenEventGameView:_onClickClose()
	if not self._isMoveing then
		self:close()
	end
end

function JieShenEventGameView:_onClickTip()
	UIStateManager.instance:push(ViewName.RulesView, self:_getRuleName())
end

function JieShenEventGameView:_onClickBeginGame()
	if self._isGameReady then
		self:_readyStartPipeGame()
	end
end

function JieShenEventGameView:_onClickReset()
	if self._isGameing and self._flagEnther then
		self:_resetGame()
	end
end

function JieShenEventGameView:_setCfgAndInfo()
	local params = self:getOpenParam()

	self._activityId = params[1]
	self._informerId = params[2]
	self._informerCfg = JieShenEventConfig.instance:getInformerCfg(self._activityId, self._informerId)
	self._timelineId = checknumber(self._informerCfg.param)
	self._stageCfg = JieShenEventConfig.instance:getStageCfg(self._informerCfg.stageId)
	self._curRoadIds = {}
	self._curMapInfos = {}
	self._pipeMapCfg = JieShenEventConfig.instance:getMapCfg(self._stageCfg.mapId)
	self._curPlayerPlace = nil

	local activityCfg = JieShenEventConfig.instance:getActivityCfg(self._activityId)

	self._ruleCfg = RulesConfig.instance:getRuleCo(activityCfg.gameRuleKey)
end

function JieShenEventGameView:_getRuleName()
	return "motherXiuChallenge_rule"
end

function JieShenEventGameView:_buildPipeMap()
...
function JieShenEventGameView:_resetGame()
...
function JieShenEventGameView:_resetChallenge()
	...
	for i = curPlaceId + 1, #self._curRoadIds do
		...
		if checkCell and self._nextPlayerPlace < 0 then
	...
	if self._nextPlayerPlace == self._curPlayerPlace then
		...
		if self._nextPlayerPlace == self._endIdRoad then
			-- block empty
		end
	elseif self.isTimeToMove then
	...
	self._txtTime.text = lang("连接起点与终点即可通关")
	self._txtCurMonsterNum.text = langPara("剩余敌阵：%s", curMonsterNum)

	goutil.setActive(self._txtCurMonsterNum.gameObject, false)
end

function JieShenEventGameView:_readyStartPipeGame()
...
function JieShenEventGameView:_startPipeGame()
	goutil.setActive(self._btnBeginGame, false)
...
function JieShenEventGameView:_overPipeGame()
	self:_removeAllTimer()
...
function JieShenEventGameView:_passPipeGame()
	self:_removeAllTimer()
	...
	JieShenEventController.instance:sendPassMiniGame(self._activityId, self._informerId, temMapInfos, self._curRoadIds)
end

function JieShenEventGameView:_onPassGame()
	self._curPlayerPlace = self._startIdRoad
	self.isTimeToMove = true

	self:_resetChallenge()
end

function JieShenEventGameView:_movePlayer()
...
function JieShenEventGameView:_onMovePlayer()
	if self._flagEnther then
		...
		if self._curStep > 0 then
			...
			settimer(0.6, self._onMovePlayer, self, false)

			local cell = self._pipeCellList[self._curRoadIds[self._moveStartId]]
			local tween = self._player.transform:DOMove(cell.go.transform.position, 0.5):SetEase(DG.Tweening.Ease.Linear)
...
function JieShenEventGameView:_onFinishMove()
	self._isMoveing = false

	local cell = self._pipeCellList[self._nextPlayerPlace]

	self._curPlayerPlace = self._nextPlayerPlace

	if cell.id == self._endIdRoad then
		self.isTimeToMove = false

		UIStateManager.instance:push(ViewName.JieShenEventTimelineTipsView, self._activityId, self._timelineId)
	else
		self.isTimeToMove = true
		self._flagEnther = true
...
function JieShenEventGameView:_roratePipeCell(cell)
	if cell then
...
function JieShenEventGameView:_onClickPipe(cell)
...
function JieShenEventGameView:_checkGamePass()
...
function JieShenEventGameView:_getNextPipeCell(cell, lastFaceType)
	if cell then
...
function JieShenEventGameView:_onReadyCountdown()
...
function JieShenEventGameView:_onGameCountDown()
