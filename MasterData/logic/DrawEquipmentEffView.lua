-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/drawequipments/view/DrawEquipmentEffView.lua

module("logic.extensions.equiplottery.view.DrawEquipmentEffView", package.seeall)

local DrawEquipmentEffView = class("DrawEquipmentEffView", ViewComponent)

function DrawEquipmentEffView:buildUI()
	DrawEquipmentEffView.super.buildUI(self)

	self._camera = self:getGo("Camera")
	self._btnPass = self:getBtn("rightTop/passNode")
end

function DrawEquipmentEffView:bindEvents()
	self._btnPass:AddClickListener(self._onClickPass, self)
end

function DrawEquipmentEffView:unbindEvents()
	self._btnPass:RemoveClickListener()
end

function DrawEquipmentEffView:onEnter()
	self._isEffLoaded = nil
	self._drawTimes = self:getFirstParam()
	self._effConfig = DrawEquipmentsController.drawEffectsConfig[self._drawTimes]
	...
	self._changeSetId = DrawEquipmentsController.instance:getDrawChangesetId()
	self._itemsData = DrawEquipmentsController.instance:getDrawItems()
	self._qualities = DrawEquipmentsController.instance:getDrawQualities()

	self._camera.transform:SetParent(nil)
	Framework.TransformUtil.SetPos(self._camera.transform, 0, 1, -10)
end

function DrawEquipmentEffView:onExit()
...
function DrawEquipmentEffView:_onDrawEffLoaded(eff)
	...
	for i = 1, #self._qualities do
		if self._equipNodes[i] then
			local qualityEffPath = DrawEquipmentsController.EquipEffsByQuality[self._qualities[i]]
...
function DrawEquipmentEffView:_playDrawSound()
...
function DrawEquipmentEffView:_stopDrawSound()
...
function DrawEquipmentEffView:_onEffectPlayFinished(eff)
...
function DrawEquipmentEffView:_onClickPass()
	self:_onEffectPlayFinished()
end

return DrawEquipmentEffView
