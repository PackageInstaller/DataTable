-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/drawandguess/view/DrawandguesscurteamView.lua

module("logic.extensions.drawandguess.view.DrawandguesscurteamView", package.seeall)

local DrawandguesscurteamView = class("DrawandguesscurteamView", ViewComponent)

function DrawandguesscurteamView:buildUI()
	DrawandguesscurteamView.super.buildUI(self)
...
function DrawandguesscurteamView:destroyUI()
	DrawandguesscurteamView.super.destroyUI(self)
...
function DrawandguesscurteamView:bindEvents()
	DrawandguesscurteamView.super.bindEvents(self)
...
function DrawandguesscurteamView:unbindEvents()
	DrawandguesscurteamView.super.unbindEvents(self)
...
function DrawandguesscurteamView:onEnter()
	DrawandguesscurteamView.super.onEnter(self)
	...
	self.addGEvent(self, GlobalNotify.FormTeamLeaveCurTeam, self._onExitCurTeam, self)
	self.addGEvent(self, GlobalNotify.FormTeamError, self._onError, self)
	self.addGEvent(self, GlobalNotify.FormTeamLeaveCurTeamChangeInfo, self._onInfoChange, self)
	self.addGEvent(self, GlobalNotify.FormTeamLeaveCurTeamReady, self._onTeammateReady, self)
	self.addGEvent(self, GlobalNotify.FormTeamGetReformTime, self._onGetRefeformTime, self)
	self.addGEvent(self, GlobalNotify.FormTeamChat, self._onSomeoneChat, self)
...
function DrawandguesscurteamView:onEnterFinished()
	DrawandguesscurteamView.super.onEnterFinished(self)
...
function DrawandguesscurteamView:onExit()
	DrawandguesscurteamView.super.onExit(self)
...
function DrawandguesscurteamView:_removeAllTimers()
...
function DrawandguesscurteamView:_refreshViewByInfo()
	for k, cell in ipairs(self._teammateList) do
		self:resetTeamCell(cell)
		goutil.setActive(cell.go, false)
...
function DrawandguesscurteamView:resetTeamCell(cell)
...
function DrawandguesscurteamView:_refreshSettingPart()
...
function DrawandguesscurteamView:_refreshTeammateList()
	for i = 1, self._teamCfg.maxCount do
...
function DrawandguesscurteamView:_onCountDown()
...
function DrawandguesscurteamView:_onClickExitTeam()
...
function DrawandguesscurteamView:_onClickReady()
...
function DrawandguesscurteamView:_onClickChat()
...
function DrawandguesscurteamView:_onClickAddition()
	FormTeamController.instance:getCurFetterInfo()
end

function DrawandguesscurteamView:_onClickInvite(id)
	if self._curInfo.isOnlyInvite then
		if self._curInfo:checkUserIsTeamLeader() then
			UIStateManager.instance:push(ViewName.DrawandguessinvitetypeView, self._activityId, self._teamCfg)
		else
...
function DrawandguesscurteamView:_onClickInfo(id)
...
function DrawandguesscurteamView:_onClickTip()
	UIStateManager.instance:open(ViewName.RulesView, "team_rule")
end

function DrawandguesscurteamView:_onError(status)
	return
end

function DrawandguesscurteamView:_onExitCurTeam()
	self:close()
end

function DrawandguesscurteamView:_formTeamJoinTeam(res)
	self:_removeAllTimers()
	self:_refreshViewByInfo()
...
function DrawandguesscurteamView:_onChat()
	return
end

function DrawandguesscurteamView:_onInfoChange()
	self._curInfo = FormTeamModel.instance:getCurTeamInfo()
...
function DrawandguesscurteamView:_onTeammateReady()
	self._curInfo = FormTeamModel.instance:getCurTeamInfo()
...
function DrawandguesscurteamView:_onGetRefeformTime()
	settimer(1, self._onCountDown, self)
	self:_onCountDown()
end

function DrawandguesscurteamView:_updateChatCell(view, goCell, data)
...
function DrawandguesscurteamView:_clearChatCell(goCell)
	local cell = {}
...
function DrawandguesscurteamView:_onSelectChat(data)
...
function DrawandguesscurteamView:_onSomeoneChat(userId, chatId)
	removetimer(self._onChatCountDown, self)
...
function DrawandguesscurteamView:_onChatCountDown()
...
function DrawandguesscurteamView:_endChatCD()
	self._isCurChatCD = false
end

function DrawandguesscurteamView:_onClickAutoReady()
...
function DrawandguesscurteamView:_setAutoReadyPart()
	local autoState = FormTeamModel.instance:getTeamAutoReadyState(self._teamCfg.groupId)
...
function DrawandguesscurteamView:_setAutoReadyState(isOn)
	removetimer(self._onAutoReadyCountDown, self)
...
function DrawandguesscurteamView:_onAutoReadyCountDown()
	self._timerAutoReady = self._timerAutoReady - 1
...
return DrawandguesscurteamView
