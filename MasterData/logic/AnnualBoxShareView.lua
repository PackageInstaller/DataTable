-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/annualbox/view/AnnualBoxShareView.lua

module("logic.extensions.annualbox.view.AnnualBoxShareView", package.seeall)

local AnnualBoxShareView = class("AnnualBoxShareView", ViewComponent)

function AnnualBoxShareView:ctor()
	AnnualBoxShareView.super.ctor(self)

	self._platformBtn = nil
end

function AnnualBoxShareView:bindEvents()
	AnnualBoxShareView.super.bindEvents(self)
...
function AnnualBoxShareView:unbindEvents()
	AnnualBoxShareView.super.unbindEvents(self)
...
function AnnualBoxShareView:onExit()
	AnnualBoxShareView.super.onExit(self)
...
function AnnualBoxShareView:destroyUI()
	AnnualBoxShareView.super.destroyUI(self)
end

function AnnualBoxShareView:buildUI()
	AnnualBoxShareView.super.buildUI(self)
...
function AnnualBoxShareView:onEnter()
	AnnualBoxShareView.super.onEnter(self)

	local params = self:getOpenParam()

	self._urlPath = params[1]

	GameUtil.setWidth(self._shareIma.gameObject, 984)
	GameUtil.setHeight(self._shareIma.gameObject, 544)
...
function AnnualBoxShareView:onEnterFinished()
	AnnualBoxShareView.super.onEnterFinished(self)
...
function AnnualBoxShareView:_onHeadCallback()
...
function AnnualBoxShareView:_onGetTexture2D(tex2d)
	if tex2d == nil then
		return
	end
...
function AnnualBoxShareView:_shareBtn(index)
	if string.nilorempty(self._savePath) then
		printError("sr---周年礼盒，分享图片出错！ AnnualBoxShareView:_shareBtn()")
		self:close()
...
function AnnualBoxShareView:_shareHander(statusCode, statusDesc)
	local btplatformId = BootstrapUtil.getPlatformId()
