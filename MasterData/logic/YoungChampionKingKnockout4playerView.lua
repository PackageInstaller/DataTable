-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/youngchampionking/view/YoungChampionKingKnockout4playerView.lua

module("logic.extensions.youngchampionking.view.YoungChampionKingKnockout4playerView", package.seeall)

local YoungChampionKingKnockout4playerView = class("YoungChampionKingKnockout4playerView", ViewComponent)

function YoungChampionKingKnockout4playerView:ctor()
	YoungChampionKingKnockout4playerView.super.ctor(self)
end

function YoungChampionKingKnockout4playerView:unbindEvents()
	YoungChampionKingKnockout4playerView.super.unbindEvents(self)
...
function YoungChampionKingKnockout4playerView:bindEvents()
	YoungChampionKingKnockout4playerView.super.bindEvents(self)
...
function YoungChampionKingKnockout4playerView:buildUI()
	YoungChampionKingKnockout4playerView.super.buildUI(self)
...
function YoungChampionKingKnockout4playerView:buildMatchCell(roundId, matchId)
	local cell = {}
...
function YoungChampionKingKnockout4playerView:getPlayerCell(pos)
	local cell = {}
...
function YoungChampionKingKnockout4playerView:getBtnCell(pos)
	local cell = {}
...
function YoungChampionKingKnockout4playerView:onExit()
	YoungChampionKingKnockout4playerView.super.onExit(self)
...
function YoungChampionKingKnockout4playerView:onEnter()
	YoungChampionKingKnockout4playerView.super.onEnter(self)
	...
	self:_updateUIByCfg()
	goutil.setActive(self._btnSwtich, self._enterRoundId == YoungChampionKingEnum.Common_Knockout_16To8_Id)
	self:_updateTimeTxt()
	RedPointController.instance:regRedPoint(self._redPointBtnGainCoin, RedPointModel.ID_YOUNG_CHAMPION_KING_GAIN_COIN)
end

function YoungChampionKingKnockout4playerView:_updateUIByCfg()
...
function YoungChampionKingKnockout4playerView:_updateTimeTxt()
	if self._enterRoundId == YoungChampionKingEnum.Common_Knockout_16To8_Id then
		self._startRoundId = YoungChampionKingEnum.Common_Knockout_16To8_Id
		self._endRoundId = YoungChampionKingEnum.Common_Knockout_8To4_Id
	else
		self._startRoundId = YoungChampionKingEnum.Common_Knockout_4To2_Id
		self._endRoundId = YoungChampionKingEnum.Common_Knockout_4To2_Id
	end
...
function YoungChampionKingKnockout4playerView:_updateTimer()
...
function YoungChampionKingKnockout4playerView:_handleGetRoomInfo()
	self._groupId = 1
	...
	if self._enterRoundId == YoungChampionKingEnum.Common_Knockout_16To8_Id then
		...
		if self._myInfo then
			...
			if matchRoundId == YoungChampionKingEnum.Common_Knockout_16To8_Id then
				self._groupId = math.ceil(matchId / 2)
			elseif matchRoundId == YoungChampionKingEnum.Common_Knockout_8To4_Id then
...
function YoungChampionKingKnockout4playerView:_onSwitchGroup(groupId)
	YoungChampionKingController.instance:sendPM_YCKGetKnockoutScheduleReq(self._activityId, groupId, self._enterRoundId)
end

function YoungChampionKingKnockout4playerView:_updateCell(view, goCell, data)
...
function YoungChampionKingKnockout4playerView:_clearCell(goCell)
	local cell = {}
...
function YoungChampionKingKnockout4playerView:_onClickVoteRecord()
	UIStateManager.instance:push(ViewName.YoungchampionkingsupportrecordView, self._activityId)
end

function YoungChampionKingKnockout4playerView:_onClickGainCoin()
	UIStateManager.instance:push(ViewName.YoungChampionKingGainCoinView)
end

function YoungChampionKingKnockout4playerView:_onClickBtnSwtich()
...
function YoungChampionKingKnockout4playerView:_onClickBtnVote(cell)
	UIStateManager.instance:push(ViewName.YoungchampionkingsupportView, self._activityId, cell.info, self._curRoundId, self._groupId)
end

function YoungChampionKingKnockout4playerView:_onClickBtnRecord(cell)
...
function YoungChampionKingKnockout4playerView:_onStepChange(newStepId)
	self:_onSwitchGroup(self._curGroupId)
end

function YoungChampionKingKnockout4playerView:_handleGetInfo(viewGroupId)
...
function YoungChampionKingKnockout4playerView:_handleGuess()
	self:_updateGroupMatch()
end

function YoungChampionKingKnockout4playerView:_updateGroupMatch()
	...
	for i, cell in ipairs(self._matchCellList) do
		...
		if self._enterRoundId == YoungChampionKingEnum.Common_Knockout_4To2_Id and cell.roundId == 2 then
			trueRoundId = trueRoundId + 1
		end
	...
	if self._enterRoundId == YoungChampionKingEnum.Common_Knockout_4To2_Id then
		goutil.setActive(self._playerCellEnd.tagWinLast, true)
		goutil.setActive(self._playerCellEnd.tagEmpty, false)
	end
end

function YoungChampionKingKnockout4playerView:_resetMatchCell(cell)
...
function YoungChampionKingKnockout4playerView:_setPlayerInfo(cell, info)
	if cell then
...
function YoungChampionKingKnockout4playerView:_setPlayerWinInfo(cell, isWin, roundId)
	if cell then
		if roundId == YoungChampionKingEnum.Common_Knockout_4To2_Id then
			goutil.setActive(cell.tagWin, isWin)
			goutil.setActive(cell.tagThirdPlaceMatch, not isWin)
		elseif roundId == YoungChampionKingEnum.Third_Place_Match_Round_Id then
			goutil.setActive(cell.tagChampionship, true)
		else
			goutil.setActive(cell.tagWin, isWin)
...
function YoungChampionKingKnockout4playerView:_updateMatchCell(cell)
	...
	if info.roundId == YoungChampionKingEnum.Final_Match_Round_Id then
		goutil.setActive(cell.playerCellA.tagChampionship, true)
		goutil.setActive(cell.playerCellB.tagChampionship, true)
	end
	...
	if not info.reports then
		...
		if info.roundId <= YoungChampionKingEnum.Common_Knockout_4To2_Id and self._curRoundId == info.roundId then
			local var_29_1 = self._curStepCfg and self._curStepCfg.canGuss
...
function YoungChampionKingKnockout4playerView:_getWinerInfo(info)
	if info then
		if not info.reports then
			local records = {}
...
return YoungChampionKingKnockout4playerView
