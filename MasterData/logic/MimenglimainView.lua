-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/mimengli/view/MimenglimainView.lua

module("logic.extensions.mimengli.view.MimenglimainView", package.seeall)

local MimenglimainView = class("MimenglimainView", ViewComponent)

function MimenglimainView:buildUI()
	MimenglimainView.super.buildUI(self)
...
function MimenglimainView:bindEvents()
	MimenglimainView.super.bindEvents(self)
...
function MimenglimainView:unbindEvents()
	MimenglimainView.super.unbindEvents(self)
...
function MimenglimainView:onExit()
	MimenglimainView.super.onExit(self)
...
function MimenglimainView:onEnter()
	MimenglimainView.super.onEnter(self)
	...
	self:_setActTime()
	self:_setRole()
	self.addGEvent(self, GlobalNotify.MiMengLi_Clg_Pet_Select_OK, self._onEnterLvlView, self)
	self.addGEvent(self, GlobalNotify.MiMengLi_Clg_Info_Update, self._updateUI, self)
	MiMengLiController.instance:sendGetInfo(self._activityId)
	self:_updateUI()
	MiMengLiController.instance:tryShowChangeSet()
end

function MimenglimainView:_getSkinId()
	return checknumber(self._actCfg.skinId)
end

function MimenglimainView:_upateRed()
	return
end

function MimenglimainView:_setActTime()
	self._txtTime.text = TimeGateController.instance:getActTimeShow(self._activityId)
end

function MimenglimainView:_setRole()
...
function MimenglimainView:_updateUI()
	self._info = MiMengLiModel.instance:getInfo(self._activityId) or {}

	GameUtil.SetActive(self._passClg1, false)
	self:_updateSuperBubble()
	self:_upateRed()
end

function MimenglimainView:_onClickBtnSkill()
...
function MimenglimainView:_onClickBtnIntroduce()
	PetbookController.instance:openPetinfoView(self:_getRaceId())
end

function MimenglimainView:_getRaceId()
	return PetSkinConfig.instance:getPetSkinRaceId(self:_getSkinId())
end

function MimenglimainView:_onClickBtnJump(key)
...
function MimenglimainView:_onClickBtnClg1()
	self._info = MiMengLiModel.instance:getInfo(self._activityId) or {}

	if not self._info.clgInfo then
		local clgInfo = {}
		local selectedPetHpInfo = clgInfo.selectedPetHpInfo

		if selectedPetHpInfo == nil or #selectedPetHpInfo == 0 then
			TipsFacade.instance:openTipWindowNoX(lang("tip"), self._actCfg.alertTips, function()
				UIStateManager.instance:push(ViewName.MimenglipetselectView, self._activityId)
			end)
		else
			self:_onEnterLvlView()
		end
	end
end

function MimenglimainView:_onEnterLvlView()
	UIStateManager.instance:push(ViewName.MimengliclglevelView, self._activityId)
end

function MimenglimainView:_onClickBtnClg2()
	return
end

function MimenglimainView:_onClickBtnTip()
	TipsFacade.instance:openRulesView("mimenglimainview_rule")
end

function MimenglimainView:_updateSuperBubble()
	MaterialMgr.resetAll(self._itemcellGo)
...
function MimenglimainView:_onClickbtnAttr()
	return
end

return MimenglimainView
