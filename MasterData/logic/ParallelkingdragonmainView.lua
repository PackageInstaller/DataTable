-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/parallelkingdragon/view/ParallelkingdragonmainView.lua

module("logic.extensions.parallelkingdragon.view.ParallelkingdragonmainView", package.seeall)

local ParallelkingdragonmainView = class("ParallelkingdragonmainView", ViewComponent)

function ParallelkingdragonmainView:buildUI()
	ParallelkingdragonmainView.super.buildUI(self)
	...
	self._jumpBtnCol_btnJump6 = goutil.findChild(self.mainGO, "jumpBtnCol/btnJump6")
	self._btnClg = goutil.findChild(self.mainGO, "btnClg")
	self._passGo = self:getGo("btnClg/passGo")
end

function ParallelkingdragonmainView:bindEvents()
	ParallelkingdragonmainView.super.bindEvents(self)
	...
	GameUtil.addClickHandler(self._jumpBtnCol_btnJump6, GameUtil.handler(self._onClickBtnJump, self, "jumpStr6"))
	GameUtil.addClickHandler(self._btnClg, self._onClickBtnClg, self)
end

function ParallelkingdragonmainView:unbindEvents()
	ParallelkingdragonmainView.super.unbindEvents(self)
	...
	GameUtil.rmClickHandler(self._jumpBtnCol_btnJump6)
	GameUtil.rmClickHandler(self._btnClg)
end

function ParallelkingdragonmainView:onEnter()
	ParallelkingdragonmainView.super.onEnter(self)
	self.addGEvent(self, GlobalNotify.ParallelKingDragonInfoRes, self._updateUI, self)

	self._activityType = GameEnum.ActivityType.Parallelkingdragon
	...
	self._actCfg = ParallelkingdragonConfig.instance:getActCfg(self._activityId)
	self._params = GameUtil.jsonToTable(self._actCfg.paramStr)

	self:_updateStaticUI()
	ParallelKingDragonAgent.instance:sendPM_ParallelKingDragonInfoReq(self._activityId)
end

function ParallelkingdragonmainView:onExit()
	ParallelkingdragonmainView.super.onExit(self)
...
function ParallelkingdragonmainView:_onClickBtnSkill()
...
function ParallelkingdragonmainView:_onClickBtnIntroduce()
	PetbookController.instance:openPetinfoView(self:_getRaceId())
end

function ParallelkingdragonmainView:_getRaceId()
	return PetSkinConfig.instance:getPetSkinRaceId(self:_getSkinId())
end

function ParallelkingdragonmainView:_getSkinId()
	return ParallelkingdragonConfig.instance:getSkinId(self._activityId)
end

function ParallelkingdragonmainView:_onClickBtnJump(key)
	local jumpToStr = self._params[key]

	GotoMgr.gotoByString(jumpToStr)
end

function ParallelkingdragonmainView:_onClickBtnClg()
	UIStateManager.instance:push(ViewName.ParallelkingdragonstageView, self._activityId)
end

function ParallelkingdragonmainView:_onClickBtnTip()
...
function ParallelkingdragonmainView:_updateUI()
	...
	local isFirstPass = self._info.maxPassStageId >= #stageCfgs

	goutil.setActive(self._passGo, isFirstPass)
end

function ParallelkingdragonmainView:_updateStaticUI()
...
function ParallelkingdragonmainView:_getActivityId()
...
return ParallelkingdragonmainView
