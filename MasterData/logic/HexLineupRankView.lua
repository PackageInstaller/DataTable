-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/hexlineup/view/HexLineupRankView.lua

module("logic.extensions.hexlineup.view.HexLineupRankView", package.seeall)

local HexLineupRankView = class("HexLineupRankView", ViewComponent)

function HexLineupRankView:ctor()
	HexLineupRankView.super.ctor(self)
end

function HexLineupRankView:buildUI()
	HexLineupRankView.super.buildUI(self)
...
function HexLineupRankView:bindEvents()
	HexLineupRankView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self.close, self)
end

function HexLineupRankView:unbindEvents()
	HexLineupRankView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
end

function HexLineupRankView:onEnter()
	HexLineupRankView.super.onEnter(self)
	...
	self.addGEvent(self, GlobalNotify.HandlePM_HexLineupRankViewRes, self._handleRankMsg, self)
	HexLineupController.instance:sendPM_HexLineupRankViewReq(self._activityId)
	self:_onUpdate()
end

function HexLineupRankView:onExit()
	HexLineupRankView.super.onExit(self)
	self:_onClear()
	self._myPrizeScrollerList:dispose()
end

function HexLineupRankView:_handleRankMsg(msg)
	if msg.activityId ~= self._activityId then
		return
	end

	self._rankInfoList = msg.rankInfoList or {}
...
function HexLineupRankView:_onUpdate()
...
function HexLineupRankView:_onClear()
	self._rankScrollerList:dispose()
end

function HexLineupRankView:_getPrizesByRank(rank)
	local prize
	local data = HexLineupConfig.instance:getEndlessRankPrizeDataByRank(self._activityId, rank)

	return (data or nil) and data.prize
end

function HexLineupRankView:_updateRankCell(view, cell, info, tag)
...
function HexLineupRankView:_clearRankCell(cell)
	local mainGo = cell.gameObject
	local head = goutil.findChild(mainGo, "head")
...
function HexLineupRankView:_updateItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject
...
function HexLineupRankView:_clearItemCell(cell)
	local mainGo = cell.gameObject
	local item = goutil.findChild(mainGo, "item")
...
function HexLineupRankView:_updateMyPrizeItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function HexLineupRankView:_clearMyPrizeItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

return HexLineupRankView
