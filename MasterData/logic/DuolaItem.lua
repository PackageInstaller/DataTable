-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/duolamengmeng/view/DuolaItem.lua

module("logic.extensions.duolamengmeng.view.DuolaItem", package.seeall)

local DuolaItem = class("DuolaItem")
...
function DuolaItem.GetGo(go)
	local component = LuaComponentContainer.Get(go, DuolaItem)

	component = component or LuaComponentContainer.Add(go, DuolaItem)

	return component
end

function DuolaItem:ctor(luaComponentContainer)
...
function DuolaItem:_rotateCircle()
...
function DuolaItem:getPos()
	return self._pos
end

function DuolaItem:checkVirusCollision(playerPos, playerRadius)
	for k, v in pairs(self._virusList) do
		local worldPos = v.go.transform.position
		local distance = DuolaMMcontroller.instance:calculateDistance(worldPos, playerPos) * 360
...
function DuolaItem:getDropId()
	return self._dropId
end

function DuolaItem:disappear()
...
function DuolaItem:buildVirus(num, type)
	for i = 1, num do
...
function DuolaItem:findSlot(type)
	local randomSlot = math.random(1, SLOT_NUM)
...
function DuolaItem:checkSlotIfEmpty(pos, slotSize)
	local isEmpty = true
