-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/xiaonuoassistant/view/cell/AssistCell14.lua

module("logic.extensions.xiaonuoassistant.view.cell.AssistCell14", package.seeall)

local AssistCell14 = class("AssistCell14", BaseAssistCell)

AssistCell14.Diamond = "105:2"

function AssistCell14:ctor(luaComponentContainer)
	AssistCell14.super.ctor(self, luaComponentContainer)
...
function AssistCell14:reset()
	AssistCell14.super.reset(self)
...
function AssistCell14:init(data)
	AssistCell14.super.init(self, data)
	...
	local usedTotalTimes, freeTimes = WTowerModel.instance:getTimeBoxTimes()

	self._totalTimes = VipModel.instance:getFreeSweepTimesOfWarriorTower() + VipModel.instance:getBuyChallengeTimesOfWarriorTower()
...
function AssistCell14:_onValueChanged(value)
	...
	local realSelectNum = Mathf.Min(value, self._leftTotalTimes)
	local needDiamondNum = self:_getCoinInfo(realSelectNum)
...
function AssistCell14:_updateCoins(costDiamondNum)
	XiaoNuoAssistCostModel.instance:setCoin(self._data:getFuncIndex(), AssistCell14.Diamond, costDiamondNum)

	local costTotalDimond = XiaoNuoAssistCostModel.instance:getTotalCoin(self._data:getFuncIndex(), AssistCell14.Diamond)
	...
	if costTotalDimond <= curTotalDiamond then
		if not XiaoNuoAssistantModel.CostColor then
			...
			self._txt4.text = string.format("<quad name=%s|icon size=25 width=1 offsetY=-5/> x <color=#%s>%d</color>", AssistCell14.Diamond, coinColorStr, costDiamondNum)
...
function AssistCell14:_getCoinInfo(selectTimes)
	return (self._data:_getCoinInfo(selectTimes))
end

function AssistCell14:_refreshCostShow(tabId)
...
function AssistCell14:_onHideDropdown()
	self:_hideDropdown(self._dropdown1.dropdown)
end

function AssistCell14:_onClickBtn1()
...
function AssistCell14:_setBtnIsOn1(isOn)
