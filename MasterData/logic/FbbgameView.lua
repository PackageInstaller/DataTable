-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/fbbchallenge/view/game/FbbgameView.lua

module("logic.extensions.fbbchallenge.view.game.FbbgameView", package.seeall)

local FbbgameView = class("FbbgameView", ViewComponent)
local LineEnum = {
	Down = 1,
	Up = 3,
	Mid = 2
...
function FbbgameView:ctor()
	FbbgameView.super.ctor(self)
end

function FbbgameView:buildUI()
	FbbgameView.super.buildUI(self)
	...
	self._fbbPlayerClass = GameUtil.AddLuaOnce(self._fbbPlayerGo, FbbPlayer)
	self._wyPlayerClass = GameUtil.AddLuaOnce(self._wyPlayerGo, WYPlayer)
...
function FbbgameView:bindEvents()
	FbbgameView.super.bindEvents(self)
...
function FbbgameView:unbindEvents()
	FbbgameView.super.unbindEvents(self)
...
function FbbgameView:destroyUI()
	FbbgameView.super.destroyUI(self)
	self._moveItemPool:clear()
end

function FbbgameView:onEnter()
	FbbgameView.super.onEnter(self)
	GlobalDispatcher:addListener(FbbchallengeController.PM_FengBaoBaoClgEndGameRes, self._endGameRes, self)
...
function FbbgameView:onEnterFinished()
	FbbgameView.super.onEnterFinished(self)
end

function FbbgameView:onExit()
	FbbgameView.super.onExit(self)
	self:_stop()
	GlobalDispatcher:removeListener(FbbchallengeController.PM_FengBaoBaoClgEndGameRes, self._endGameRes, self)
...
function FbbgameView:onExitFinished()
	FbbgameView.super.onExitFinished(self)
end

function FbbgameView:_onClickbtnClose()
	self:_pause(true)
...
function FbbgameView:_onClickbtnDown()
...
function FbbgameView:_onClickbtnUp()
	if not self._playCanMoveLine then
		return
	end
...
function FbbgameView:_onClickgamestarTip()
	goutil.setActive(self._gamestarTip.gameObject, false)
	self:_start()
end

function FbbgameView:_initMoveCells()
	self._moveItems = {}
	...
	local function createFunc()
		...
		return {
			...
			class = GameUtil.AddLuaOnce(go, FbbMoveItem)
		}
	end
...
function FbbgameView:_prepareData()
	self._sceneMoveInitSpeed = FbbchallengeConfig.instance:getDefineValue("SCENE_MOVE_SPEED", true)
	self._sceneMoveSpeed = self._sceneMoveInitSpeed
	self._sceneMoveSpeedMax = FbbchallengeConfig.instance:getDefineValue("SCENE_MOVE_SPEED_MAX", true)
	self._into_max_speed_need_time = FbbchallengeConfig.instance:getDefineValue("INTO_MAX_SPEED_TIME", true)
	self._moveDataList = FbbchallengeModel.instance:prepareGameDatas()
	self._battleSlotIndex = FbbchallengeConfig.instance:getDefineValue("BATTLE_SLOT", true)
	self._warinning_tip_slot = FbbchallengeConfig.instance:getDefineValue("WARINING_TIP_SLOT", true)
	self._wyHeadPosInitRatio = FbbchallengeConfig.instance:getDefineValue("WY_INIT_RATIO", true) / 100
	self._currPlayerLineType = LineEnum.Mid
	self._every_hit_reduce_hp_ratio = FbbchallengeConfig.instance:getDefineValue("EVERY_HIT_REDUCE_HP_RATIO", true) / 100
	self._every_hit_time = FbbchallengeConfig.instance:getDefineValue("EVERY_HIT_TIME", true)
	self._playTime = 0
	self._wy_playspeed = FbbchallengeConfig.instance:getDefineValue("WY_MOVE_SPEED", true)
	self._itemSpace = 150
...
function FbbgameView:_initMoveItems()
	local count = 15
...
function FbbgameView:_start()
	self._isStopMove = false
	self._isPause = false

	RoleObjectPool.instance:playAnimation(self._fbbRoleGo, "run1", true)
	settimer(0, self._update, self, true)
end

function FbbgameView:_pause(closeWinState)
	self._isPause = true
	self._closeWinState = closeWinState
end

function FbbgameView:_resume()
	self._isPause = false
	self._closeWinState = false
end

function FbbgameView:_stop()
...
function FbbgameView:_itemsMove()
...
function FbbgameView:_creatNextItem(item)
...
function FbbgameView:_delayCreatNextItem(hitItem)
...
function FbbgameView:_checkHasDelayAndDoCreatNextItem()
...
function FbbgameView:_checkHit()
...
function FbbgameView:_JLBattle()
...
function FbbgameView:_delayDoHitFinish()
	self._playCanMoveLine = true
...
function FbbgameView:_calBattleData(targetCurPosX, targetHP)
...
function FbbgameView:_onBattle(hitCallBack, hitFinishCallBack)
	local tweenIndex = 0
...
function FbbgameView:_gameOver()
	self._fbbPlayerClass:exitBattle()
	self:_stop()
	FbbchallengeController.instance:sendPM_FengBaoBaoClgEndGameReq(self._fbbPlayerClass:getHP())
	settimer(2, self.close, self, false)
	goutil.setActive(self._maskGo, true)
end

function FbbgameView:_gotoWYBattle()
...
function FbbgameView:_playWarning()
	...
	local time = FbbchallengeConfig.instance:getDefineValue("WARNING_TIME", true)

	settimer(time, self._stopWarning, self, false)
end

function FbbgameView:_stopWarning()
	removetimer(self._stopWarning, self)
	goutil.setActive(self._warningTipGo, false)
end

function FbbgameView:_wyMoveToScene()
...
function FbbgameView:_fbbMoveToWy()
	for k, v in pairs(self._moveItems) do
		self._moveItemPool:returnObject(v)
	end
...
function FbbgameView:_onWYBattle()
	self:_stop()
...
function FbbgameView:_gotoEatScoreItem(hitItem)
	local duration = 0.2
...
function FbbgameView:_getCurrSceneMoveSpeed()
	return self._sceneMoveSpeed
end

function FbbgameView:_setCurrSceneMoveSpeed()
...
function FbbgameView:_setFBBHeadPos(isMax)
	local ratio = 1
...
function FbbgameView:_setWYHeadPos(isMax)
	local ratio = 1
...
function FbbgameView:_endGameRes()
	return
end

function FbbgameView:_getOneHitReduceHp()
...
function FbbgameView:_retrurnOutHitItemToPool()
...
function FbbgameView:_loadRoles()
	if not self._fbbRoleGo then
		local go = self:getResInstance(FbbgameViewPresentor.FBBCharacterPath)
