-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/annualfuben/view/AnnualFubenBrokenRankView.lua

module("logic.extensions.annualfuben.view.AnnualFubenBrokenRankView", package.seeall)

local AnnualFubenBrokenRankView = class("AnnualFubenBrokenRankView", ViewComponent)

function AnnualFubenBrokenRankView:ctor()
	AnnualFubenBrokenRankView.super.ctor(self)
end

function AnnualFubenBrokenRankView:unbindEvents()
	AnnualFubenBrokenRankView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
end

function AnnualFubenBrokenRankView:bindEvents()
	AnnualFubenBrokenRankView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self.close, self)
end

function AnnualFubenBrokenRankView:buildUI()
	AnnualFubenBrokenRankView.super.buildUI(self)
...
function AnnualFubenBrokenRankView:onExit()
	AnnualFubenBrokenRankView.super.onExit(self)
...
function AnnualFubenBrokenRankView:onEnter()
	AnnualFubenBrokenRankView.super.onEnter(self)
	self.addGEvent(self, GlobalNotify.PM_AnnualFubenRankInfoRes, self._refreshView, self)

	self._itemTableListDir = {}
	self._activityId = checknumber(self:getFirstParam())

	AnnualFubenAgent.instance:sendPM_AnnualFubenStageRankInfoReq(self._activityId)
end

function AnnualFubenBrokenRankView:_refreshView()
	local rankInfo = AnnualFubenModel.instance:getTempRankInfo()
	...
	if not rankInfo.rankInfos then
		...
		local rankPrizeCfgs = AnnualFubenConfig.instance:getBrokenStageRankPrizeCfgs(self._activityId)
		...
		self._txtMyStep.text = AnnualFubenModel.instance:getBrokenStageId(self._activityId)
	end
end

function AnnualFubenBrokenRankView:_updateRankCell(view, cell, data, tag)
...
function AnnualFubenBrokenRankView:_clearRankCell(cell)
	local go = cell.gameObject
	local imgIcon = goutil.findChild(go, "imgHeadIcon")
...
function AnnualFubenBrokenRankView:_updateItemCell(view, cell, data, tag)
	local go = cell.gameObject
...
function AnnualFubenBrokenRankView:_clearItemCell(cell)
	local go = cell.gameObject

	MaterialMgr.resetAll(go)
end

return AnnualFubenBrokenRankView
