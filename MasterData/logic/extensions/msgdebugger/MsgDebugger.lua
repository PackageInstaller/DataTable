-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/msgdebugger/MsgDebugger.lua
...
function MsgDebugger:init()
	if not self._isInit then
		...
		GlobalDispatcher:addEventListener(MsgDebugger.EvtStartBlock, self._onStartBlockMsg, self)
		GlobalDispatcher:addEventListener(MsgDebugger.EvtCancelBlock, self._onCancelBlockMsg, self)
	end
end
...
function MsgDebugger:registerLocalNotify(name, handler, handlerObj, priority)
	GlobalDispatcher:addEventListener(name, handler, handlerObj)
end

function MsgDebugger:unregisterLocalNotify(name, handler, handlerObj)
	GlobalDispatcher:removeEventListener(name, handler, handlerObj)
end

function MsgDebugger:localNotify(name, message)
	GlobalDispatcher:dispatchEvent(name, message)
end
...
function MsgDebugger:startBlock(ext, cmd)
	if ext and cmd then
		local extCmdValue = ext * 10000 + cmd
		local pbUpStructName = NetConnMgr.instance:_getPbUpStructName(ext, cmd)

		self._toBlockExtCmdDict[extCmdValue] = pbUpStructName
	else
...
function MsgDebugger:clearCollect()
	MsgDebuggerCollectModel.instance:clearSentMsg()
	self:localNotify(MsgDebugger.EvtSend)
end
...
function MsgDebugger:_redefineNetConnMgr()
	...
	function NetConnMgr:sendMsg(msg, connType)
		...
		if extCmd then
			...
			local stackTrace = MsgDebuggerConfig.NeedPrintStackTrace and debug.traceback()

			msgMO:init(extCmd[1], extCmd[2], pbUpStructName, ServerTime.now(), msg, stackTrace, connType)
...
function MsgDebugger:_onSendMsg(msgMO)
	if msgMO.hasModifyValue then
		...
		paramStr = paramStr == "" and "no param" or paramStr
