-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/mainui/scene/component/MainSceneUnitFactory.lua

module("logic.extensions.mainui.scene.component.MainSceneUnitFactory", package.seeall)

local M = class("MainSceneUnitFactory", MainSceneHeroFactory)
...
function M:initCreateDefine()
	M.super.initCreateDefine(self)

	self._unitDefine[UnitTag.Player].creator = M._createUnitAdjutant
end
...
function M:removeUnitHeroByPoint(pointId)
	local unitTag = pointId == 0 and UnitTag.Player or UnitTag.Npc
...
function M:_createUnitAdjutant(unitTag, entityInfo, defineInfo)
	...
	if unit then
		...
		unit.xinmaoNode:setJudgeAirtightTime(true)
		...
		unit.collider:setColliderCenter(centerX, centerY, centerZ)
		unit.collider:setColliderSize(sizeX, sizeY, sizeZ)
	end
