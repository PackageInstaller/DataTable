-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/task/util/impl/tacit/TaskHeroKillEnemyId.lua

module("logic.extensions.task.util.impl.tacit.TaskHeroKillEnemyId", package.seeall)

local M = class("TaskHeroKillEnemyId")
...
function M:parseContent(code, desc, param, heroId)
	...
	local enmeyId = tonumber(paramArray[3])
	...
	local monsterName = ""
	local monsterCO = MonsterConfig.instance:getMonsterCO(enmeyId)

	if monsterCO then
		monsterName = monsterCO.name
	end

	tempStr = string.gsub(tempStr, "#2#", name)
	tempStr = string.gsub(tempStr, "#3#", monsterName)
