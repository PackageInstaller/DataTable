-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/treasureraider/controller/TRDiceHandler.lua

module("logic.extensions.treasureraider.controller.TRDiceHandler", package.seeall)

local TRDiceHandler = class("TRDiceHandler")
local FaceTo = {
	Vector3.New(0, 90, 0),
	Vector3.New(-90, 0, 0),
	Vector3.New(0, 180, 0),
...
function TRDiceHandler:ctor()
	return
end

function TRDiceHandler:initFromGo(mainGo, saiziCount)
	self._mainGo = mainGo
...
function TRDiceHandler:setVisible(value)
	GameUtil.SetActive(self._mainGo, value)
end

function TRDiceHandler:dispose()
	self._value = nil
...
function TRDiceHandler:play(params)
...
function TRDiceHandler:_getParam(key, defaultValue)
...
function TRDiceHandler:_callBack(func, thisArg, ...)
...
function TRDiceHandler:_doPlay()
...
function TRDiceHandler:_resetSaiZi(go)
...
function TRDiceHandler:_playDiceAni1()
...
function TRDiceHandler:_playDiceAni2()
	local goSaizi = self._currentGo
...
function TRDiceHandler:_endPlayDice()
	self:_removeTimer()
...
function TRDiceHandler:_setTweenBezierToLast()
	local goSaizi = self._currentGo
