-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/triplemacthesgame/view/TripleMacthesGameFramkView.lua

module("logic.extensions.triplemacthesgame.view.TripleMacthesGameFramkView", package.seeall)

local TripleMacthesGameFramkView = class("TripleMacthesGameFramkView", ViewComponent)

function TripleMacthesGameFramkView:buildUI()
	TripleMacthesGameFramkView.super.buildUI(self)
...
function TripleMacthesGameFramkView:unbindEvents()
	TripleMacthesGameFramkView.super.unbindEvents(self)
...
function TripleMacthesGameFramkView:bindEvents()
...
function TripleMacthesGameFramkView:onEnter()
	TripleMacthesGameFramkView.super.onEnter(self)
	...
	TripleMachesGameController.instance:sendPM_TripleMatchesGameStartGameReq(self._activityId)
end

function TripleMacthesGameFramkView:onExit()
	TripleMacthesGameFramkView.super.onExit(self)
	self._tableList:dispose()
end

function TripleMacthesGameFramkView:_refreshView()
...
function TripleMacthesGameFramkView:_updateCell(view, cell, data, tag)
...
function TripleMacthesGameFramkView:_clearCell(cell)
	local go = cell.gameObject
	local imageGo = goutil.findChild(go, "img")
...
function TripleMacthesGameFramkView:_removeCell()
	self:_refreshView()
end

function TripleMacthesGameFramkView:_stepOver()
	self:_refreshView()
...
function TripleMacthesGameFramkView:_comboFinish(combo)
	TripleMacthesModel.instance:setCombo(combo)
end

function TripleMacthesGameFramkView:_onClickClose()
	if TripleMachesGameController.instance:isCanDoAction() then
		TipsFacade.instance:openPopupWindow(lang("提示"), lang("退出后将直接结算，是否退出游戏"), function()
			...
			UIStateManager.instance:push(ViewName.TripleMacthesResultView)
		end)
	else
...
function TripleMacthesGameFramkView:_onClickTip()
	TipsFacade.instance:openRulesView("triple_macthes_game_play_rule")
end

function TripleMacthesGameFramkView:_onClickBulle()
	GameUtil.SetActive(self._bubble, true)
end

function TripleMacthesGameFramkView:_onClickBubbleInput()
	if not hover then
		GameUtil.SetActive(self._bubble, false)
	end
end

function TripleMacthesGameFramkView:_onClickPrize()
	UIStateManager.instance:push(ViewName.TripleMacthesPrizeView, self._activityId)
end

function TripleMacthesGameFramkView:_onClearSpecialEnd()
	TripleMacthesGameModel.instance:waitMoveRes()
	GameUtil.SetActive(self._PanelPrize, false)
	UIStateManager.instance:push(ViewName.TripleMacthesResultView)
end

function TripleMacthesGameFramkView:_PM_TripleMacthesGameStart()
	TripleMacthesGameModel.instance:getMoveRes()
	self:_refreshView()
end

function TripleMacthesGameFramkView:_PM_TripleMacthesGameBuyStep()
	TripleMacthesGameModel.instance:getMoveRes()
	self:_refreshView()
end

return TripleMacthesGameFramkView
