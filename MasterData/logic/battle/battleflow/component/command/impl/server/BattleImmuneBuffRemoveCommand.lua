-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/battle/battleflow/component/command/impl/server/BattleImmuneBuffRemoveCommand.lua

module("logic.battle.battleflow.component.command.impl.server.BattleImmuneBuffRemoveCommand", package.seeall)

local M = class("BattleImmuneBuffRemoveCommand", BattleCommandBase)
...
function M:execute(battleFlow, runningContext)
	...
	for i, buffCode in ipairs(self._immuneBuffList) do
		unit.property:removeImmuneBuff(buffCode)
		ImmuneBuffHandlerUtil.handleSpecialImmuneUpdate(unit, buffCode, false)
	end
...
function M:toString()
	return string.format("[BattleImmuneBuffRemoveCommand entityId = %s,immuneBuffList = %s]", self._entityId, #self._immuneBuffList)
end
