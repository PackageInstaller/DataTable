-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/divinekingnuoya/view/DivineKingNuoYaInfiniteFightChallengeView.lua

module("logic.extensions.divinekingnuoya.view.DivineKingNuoYaInfiniteFightChallengeView", package.seeall)

local DivineKingNuoYaInfiniteFightChallengeView = class("DivineKingNuoYaInfiniteFightChallengeView", ViewComponent)
local ROW = 4
local COL = 4
...
function DivineKingNuoYaInfiniteFightChallengeView:ctor()
	DivineKingNuoYaInfiniteFightChallengeView.super.ctor(self)
end

function DivineKingNuoYaInfiniteFightChallengeView:bindEvents()
	DivineKingNuoYaInfiniteFightChallengeView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self.close, self)
	GameUtil.addClickHandler(self._btnTip, self._onClickBtnTip, self)
end

function DivineKingNuoYaInfiniteFightChallengeView:unbindEvents()
	DivineKingNuoYaInfiniteFightChallengeView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
	GameUtil.rmClickHandler(self._btnTip)
end

function DivineKingNuoYaInfiniteFightChallengeView:buildUI()
	DivineKingNuoYaInfiniteFightChallengeView.super.buildUI(self)

	self._btnClose = self:getGo("leftTop/btnClose")
	self._btnTip = self:getGo("leftTop/btnTip")
	self._txtPlayRule = self:getTxt("rule/tip/scrView/Viewport/Content")
	self._txtPassGoal = self:getTxt("desc/txtPassGoal")
	self._txtBuffNum = self:getTxt("desc/txtNum")
	...
	for i = 1, ROW * COL do
		...
		cur.txtNum = self:getTxt("challenge/cell" .. i .. "/txtNum")
		cur.idx = i
		cur.changeModeDic = {}
...
function DivineKingNuoYaInfiniteFightChallengeView:onExit()
	DivineKingNuoYaInfiniteFightChallengeView.super.onExit(self)
...
function DivineKingNuoYaInfiniteFightChallengeView:onEnter()
	DivineKingNuoYaInfiniteFightChallengeView.super.onEnter(self)
...
function DivineKingNuoYaInfiniteFightChallengeView:_onSetUI()
	self._txtPlayRule.text = self._actCfg.ruleTip

	self:_setDragContainer()
end

function DivineKingNuoYaInfiniteFightChallengeView:_setDragContainer()
...
function DivineKingNuoYaInfiniteFightChallengeView:_initLineCells()
	self._lineItems = {}
...
function DivineKingNuoYaInfiniteFightChallengeView:_onUpdate()
	self:_updateData()
...
function DivineKingNuoYaInfiniteFightChallengeView:_updateData()
	self._info = DivineKingNuoYaModel.instance:getInfo(self._activityId)
	self._stageLists = self._info and self._info.stageLists
	self._forbidData = self._info and self._info.banItemPosIds
...
function DivineKingNuoYaInfiniteFightChallengeView:_updateUI()
	local buffProgress = DivineKingNuoYaController.instance:getBuffProgress(self._activityId)
	local mostHighHistoryValue = DivineKingNuoYaController.instance:getTotalBuffHistoryValue(self._activityId)
	...
	self._txtBuffNum.text = string.format("累计超杀：%s次", mostHighHistoryValue)
	self._txtPassGoal.text = self._actCfg.firstPassNeedPassStage > 0 and string.format("通关目标：累计通关%d个敌阵(%d/%d)", self._actCfg.firstPassNeedPassStage, self._info.historyMaxPassStageCount, self._actCfg.firstPassNeedPassStage) or string.format("当前累计通关：%d", self._info.historyMaxPassStageCount)
...
function DivineKingNuoYaInfiniteFightChallengeView:_setCellUI()
		...
		if self._stageLists[curCellIdx] then
			if not self._stageLists[curCellIdx].lineState then
				...
				if self._forbidData then
					...
					if self._stageLists[curCellIdx] then
						if not self._stageLists[curCellIdx].historyMaxVlaue then
							local historyMaxValue = 0

							curCell.txtNum.text = string.format("超杀：%d", historyMaxValue)
...
function DivineKingNuoYaInfiniteFightChallengeView:_setLineMapUI()
	self:_clearLineUI()
...
function DivineKingNuoYaInfiniteFightChallengeView:_setLineMapData()
	self:_clearLinesData()
...
function DivineKingNuoYaInfiniteFightChallengeView:_getConnectData(root, curCellIdx, changeName)
...
function DivineKingNuoYaInfiniteFightChallengeView:_isEdge(curCellIdx, changeName)
...
function DivineKingNuoYaInfiniteFightChallengeView:_inOneRow(curCellIdx, nextIdx)
...
function DivineKingNuoYaInfiniteFightChallengeView:_addDirctNode(root, showMode, changeName, cellNode)
...
function DivineKingNuoYaInfiniteFightChallengeView:_getDirctNodeLen(root, showMode, changeName)
...
function DivineKingNuoYaInfiniteFightChallengeView:_onClickBtnTip()
	local key = self._actCfg.ruleKeyChallenge

	TipsFacade.instance:openRulesView(key)
end

function DivineKingNuoYaInfiniteFightChallengeView:_onClickCell(cellIdx)
...
function DivineKingNuoYaInfiniteFightChallengeView:_clearLineUI()
...
function DivineKingNuoYaInfiniteFightChallengeView:_clearLinesData()
	self._showRootDic = {
		[ShowMode.Light] = {},
		[ShowMode.Dark] = {}
	}
...
function DivineKingNuoYaInfiniteFightChallengeView:_updateForbidCell(view, cell, data)
...
function DivineKingNuoYaInfiniteFightChallengeView:_clearForbidCell(cell)
...
function DivineKingNuoYaInfiniteFightChallengeView:_OnDragForbidBegin(cell, data)
	local imgBg = goutil.findChild(cell, "imgForbid")
...
function DivineKingNuoYaInfiniteFightChallengeView:_OnDragForbidMoved(go, data)
...
function DivineKingNuoYaInfiniteFightChallengeView:_OnDragForbidEnded(cell, data)
	local imgBg = goutil.findChild(cell, "imgForbid")
...
function DivineKingNuoYaInfiniteFightChallengeView:_onMoveIn(curCell, curCellIdx)
	if self._isDraging == true then
		self._touchingIdx = curCellIdx
	end
end

function DivineKingNuoYaInfiniteFightChallengeView:_onMoveOut(curCell)
	self._touchingIdx = nil
end

function DivineKingNuoYaInfiniteFightChallengeView:_onMoveInDragContainer()
	self._isTouchingContainer = true
end

function DivineKingNuoYaInfiniteFightChallengeView:_onMoveOutDragContainer()
	self._isTouchingContainer = false
end

return DivineKingNuoYaInfiniteFightChallengeView
