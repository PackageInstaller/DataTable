-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/chopking/model/ChopKingLoopClgFmtMo.lua

module("logic.extensions.chopking.model.ChopKingLoopClgFmtMo", package.seeall)

local ChopKingLoopClgFmtMo = class("ChopKingLoopClgFmtMo", BaseCustomFmtMo)

function ChopKingLoopClgFmtMo:initParams(activityId, loopId, stagId)
	self._activityId = activityId
	self._loopId = loopId
	self._stageId = stagId

	local cfgStage = ChopKingConfig.instance:getLoopClgStageCfg(self._activityId, self._stageId)
	...
	if loopId == 1 and stagId == 1 then
		self.nEnterShowRuleFlag = ICustomFmtMo.EEnterShowRuleFlag_UserSet
		self.strShowRuleFlagKey = string.format("ChopKingLoopClgFmtMo#%d", self._activityId)
...
function ChopKingLoopClgFmtMo:initPetList()
...
function ChopKingLoopClgFmtMo:initFightHandler()
	local function handler()
		UIJumper.instance:saveCurStack(ViewName.CustomMissionView, ViewName.CustomFormationView)

		local form = self:getCurSimpleForm()

		ChopKingAgent.instance:sendPM_ChopKingLoopClgFightReq(self._activityId, self._loopId, self._stageId, form)
	end

	self:setFightHandler(handler, nil)
end

function ChopKingLoopClgFmtMo:getMonsterConfigList()
	return self._creepsCfgs
end

function ChopKingLoopClgFmtMo:getFmtInfoConfig()
	return self._monsterCfg
end

function ChopKingLoopClgFmtMo:checkFormationEmptyAndPosNumLimit(posLimitCallback, hasOneKey, onekeyCallBack)
	...
	return ChopKingLoopClgFmtMo.super.checkFormationEmptyAndPosNumLimit(self, posLimitCallback, hasOneKey, onekeyCallBack)
end

local emptyTb = {}
local oneTb = {}

function ChopKingLoopClgFmtMo:getPsychicsPets()
...
function ChopKingLoopClgFmtMo:onFmoSetDown()
	ChopKingLoopClgFmtMo.super.onFmoSetDown(self)
