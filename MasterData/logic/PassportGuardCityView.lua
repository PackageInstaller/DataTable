-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/passport/view/guardcity/PassportGuardCityView.lua

module("logic.extensions.passport.view.guardcity.PassportGuardCityView", package.seeall)

local PassportGuardCityView = class("PassportGuardCityView", ViewComponent)

function PassportGuardCityView:ctor()
	PassportGuardCityView.super.ctor(self)
end

function PassportGuardCityView:bindEvents()
	PassportGuardCityView.super.bindEvents(self)
...
function PassportGuardCityView:unbindEvents()
	PassportGuardCityView.super.unbindEvents(self)
...
function PassportGuardCityView:buildUI()
	PassportGuardCityView.super.buildUI(self)
...
function PassportGuardCityView:onEnter()
	PassportGuardCityView.super.onEnter(self)

	self._passPortFuncId = 15
	...
	RedPointController.instance:regRedPoint(self._rdTask, RedPointModel.ID_PASSPORT_GUARDCITY)
...
function PassportGuardCityView:onExit()
	PassportGuardCityView.super.onExit(self)
...
function PassportGuardCityView:_refreshView()
...
function PassportGuardCityView:_refreshPayBtn()
...
function PassportGuardCityView:_refreshOneKey()
...
function PassportGuardCityView:_getCurDataList()
	local dataList = {}
...
function PassportGuardCityView:_updateCell(view, goCell, data, tag)
	if tag == 1 then
...
function PassportGuardCityView:_clearCell(goCell)
	local cell = {}
...
function PassportGuardCityView:_rmExtraClickHandler()
...
function PassportGuardCityView:_setRewardItem(item, reward, data, isReceive, isSuper)
...
function PassportGuardCityView:_setItemEffect(isShow, mainGo, effGo)
...
function PassportGuardCityView:_onClickHelp()
	ViewMgr.instance:open(ViewName.RulesView, self._curData.funcCfg.rule)
end

function PassportGuardCityView:_onClickOneKey()
...
function PassportGuardCityView:_onClickBuyPassport()
	...
	if not self._curData.hasPay then
		UIStateManager.instance:push(ViewName.PassportGuardCityBuyView, self._curData.actCfg, self._curData.funcCfg)
	else
		FloatWordMgr.instance:show("已经购买过进阶战令了")
	end
end

function PassportGuardCityView:_onClickGetExtra()
...
function PassportGuardCityView:_onClickRuleExtra()
	UIStateManager.instance:push(ViewName.RulesView, "zhanling_rule")
end

function PassportGuardCityView:_onClickBuyLevel()
	if self._curData then
...
function PassportGuardCityView:_onClickPetInfo()
	if self._skinId then
		PetbookController.instance:openPetinfoView(self._skinId)
	end
end

function PassportGuardCityView:_onClickSkill()
	if self._skinId then
		PetbookController.instance:previewBattle(self._skinId)
	end
end

function PassportGuardCityView:_onClickFind()
	UIStateManager.instance:push(ViewName.GuardCityMainView)
end

function PassportGuardCityView:getTagByData(data, idx)
	return data.isLast and 2 or 1
end

function PassportGuardCityView:getCellSize(view, idx)
