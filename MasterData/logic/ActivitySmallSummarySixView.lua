-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/operationsummary/view/smallsummary/ActivitySmallSummarySixView.lua

module("logic.extensions.operationsummary.view.smallsummary.ActivitySmallSummarySixView", package.seeall)

local ActivitySmallSummarySixView = class("ActivitySmallSummarySixView", ActivityGrandSummaryView)

function ActivitySmallSummarySixView:_getSumActivityId()
...
function ActivitySmallSummarySixView:buildUI()
	ActivitySmallSummarySixView.super.buildUI(self)

	self._btnShowCol = goutil.findChild(self.mainGO, "btnShowCol")
	self._tweenPool = {}
end

function ActivitySmallSummarySixView:onExit()
	ActivitySmallSummarySixView.super.onExit(self)
	self:clearShowAnim()
	self:clearBtnShowCol()
end

function ActivitySmallSummarySixView:onUpdateUI()
	ActivitySmallSummarySixView.super.onUpdateUI(self)
	self:updateBtnShowCol()
end

function ActivitySmallSummarySixView:CreatActivity()
	ActivitySmallSummarySixView.super.CreatActivity(self)
	...
	for _, btnShowKey in ipairs(btnShowKeyList) do
		...
		if not self._animClipsPool[btnShowKey] then
			...
			var_5_0.showType = self._animClipsPool[btnShowKey].showType or false
			var_5_0.isNeedAnim = self._animClipsPool[btnShowKey].isNeedAnim or false

			if self._animClipsPool[btnShowKey].showType then
				var_5_0.curIdx = #infoList or 1
			end

			self._animClipsPool[btnShowKey] = var_5_0
		end
	end
end

function ActivitySmallSummarySixView:getPoolKey(cellName)
	return cellName .. "_cell"
end

function ActivitySmallSummarySixView:updateActivity()
	ActivitySmallSummarySixView.super.updateActivity(self)
...
function ActivitySmallSummarySixView:updateBtnShowCol()
	local parent = self._btnShowCol.transform
...
function ActivitySmallSummarySixView:clearBtnShowCol()
	local parent = self._btnShowCol.transform
...
function ActivitySmallSummarySixView:onClickBtnShow(go)
...
function ActivitySmallSummarySixView:playeCellsShowAnim()
	removetimer(self.updateTime, self)
	settimer(0.1, self.updateTime, self, true)
end

function ActivitySmallSummarySixView:updateTime()
	local doNum = 0
...
function ActivitySmallSummarySixView:doScaleAnim(go, scale, callBack)
...
function ActivitySmallSummarySixView:clearAnimByGo(go)
	if self._tweenPool[go] then
		self._tweenPool[go]:Kill(true)
	end
end

function ActivitySmallSummarySixView:clearShowAnim()
	removetimer(self.updateTime, self)
...
return ActivitySmallSummarySixView
