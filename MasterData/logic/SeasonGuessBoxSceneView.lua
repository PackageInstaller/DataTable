-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/season/view/games/guessbox/SeasonGuessBoxSceneView.lua

module("logic.extensions.season.view.games.guessbox.SeasonGuessBoxSceneView", package.seeall)

local SeasonGuessBoxSceneView = class("SeasonGuessBoxSceneView", ViewComponent)

function SeasonGuessBoxSceneView:ctor()
	SeasonGuessBoxSceneView.super.ctor(self)
end

function SeasonGuessBoxSceneView:buildUI()
	SeasonGuessBoxSceneView.super.buildUI(self)
...
function SeasonGuessBoxSceneView:bindEvents()
	SeasonGuessBoxSceneView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self._onClickClose, self)
	GameUtil.addClickHandler(self._startMask, self._onClickStartMask, self)
end

function SeasonGuessBoxSceneView:unbindEvents()
	SeasonGuessBoxSceneView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
	GameUtil.rmClickHandler(self._startMask)
end

function SeasonGuessBoxSceneView:destroyUI()
	SeasonGuessBoxSceneView.super.destroyUI(self)
end

function SeasonGuessBoxSceneView:onEnter()
	SeasonGuessBoxSceneView.super.onEnter(self)
...
function SeasonGuessBoxSceneView:onExit()
	SeasonGuessBoxSceneView.super.onExit(self)
...
function SeasonGuessBoxSceneView:_openStartMask()
	UIStateManager.instance:open(ViewName.SuppressGameStartMask, self._readyStartGame, self)
end

function SeasonGuessBoxSceneView:_onReset()
...
function SeasonGuessBoxSceneView:_isCorrectBox(boxId)
	local flowerId = self._gbBoxCfg[boxId].flowerId

	return (self:_isCorrectFlower(flowerId))
end

function SeasonGuessBoxSceneView:_isCorrectFlower(flowerId)
	return flowerId == self._gbCfg.correctFlowerId
end

function SeasonGuessBoxSceneView:_setBoxCells()
	local childList = GameUtil.getChildren(self._boxView)
...
function SeasonGuessBoxSceneView:_onClickBoxCell(cell)
...
function SeasonGuessBoxSceneView:_onClickStartMask()
	self:_readyStartGame()
end

function SeasonGuessBoxSceneView:_readyStartGame()
	self._countDown = 2
...
function SeasonGuessBoxSceneView:_startGameCountDown()
...
function SeasonGuessBoxSceneView:_startGame()
	self:_startPackBoxesAnim()
end

function SeasonGuessBoxSceneView:_startPackBoxesAnim()
...
function SeasonGuessBoxSceneView:_openCorrectBoxAnim(boxId)
	local cell = self._boxCellList[boxId]
	local data = cell._data
...
function SeasonGuessBoxSceneView:_openWrongBoxAnim(boxId)
	local cell = self._boxCellList[boxId]
	local data = cell._data
	local flowerData = self._gbFlowerCfg[data.flowerId]
	local effGo = cell._mainGo
...
function SeasonGuessBoxSceneView:_openBoxEff(effGo, path)
	UIEffectManager.instance:stopEffect(self._openEff)
...
function SeasonGuessBoxSceneView:_openBoxAnim(go)
...
function SeasonGuessBoxSceneView:_startExchangeBoxAnim()
...
function SeasonGuessBoxSceneView:_startTimeLimitedSelection()
...
function SeasonGuessBoxSceneView:_endGameCountDown()
	if self._countDown <= 1 then
		removetimer(self._endGameCountDown, self)
		self:_endGame()
...
function SeasonGuessBoxSceneView:_endGame()
	removetimer(self._endGameCountDown, self)
	removetimer(self._endGame, self)
	GameUtil.SetActive(self._gameMask, true)
	GameUtil.SetActive(self._txtGameTimerGo, false)
...
function SeasonGuessBoxSceneView:_sendEndGameReq()
	local errorFlowers = {}
...
function SeasonGuessBoxSceneView:_showEndGameTitle(flowerIds)
...
function SeasonGuessBoxSceneView:_showResult(flowerIds)
	if #flowerIds ~= 2 then
	...
	for _, flowerId in ipairs(flowerIds) do
		...
		if not self:_isCorrectFlower(flowerId) then
			isWin = false
		end
	end
	...
	if self.seasonEvt then
		...
		self.seasonEvt:_doSendEndGame(flowerIds, "guessFlowerIds", obj)
	end

	self:close()
end

function SeasonGuessBoxSceneView:_onClickClose()
	removetimer(self._endGameCountDown, self)
	...
	if self._isGameStart then
		...
		TipsFacade.instance:openSeasonPopupWindow("提示", tipsContent, okFunc, cencelFunc, "确定", "取消")
	else
		self:close()
	end
end

return SeasonGuessBoxSceneView
