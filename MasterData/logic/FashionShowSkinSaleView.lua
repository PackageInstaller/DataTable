-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/dressactivityshop/view/FashionShowSkinSaleView.lua

module("logic.extensions.dressactivityshop.view.FashionShowSkinSaleView", package.seeall)

local FashionShowSkinSaleView = class("FashionShowSkinSaleView", ViewComponent)

function FashionShowSkinSaleView:ctor()
	FashionShowSkinSaleView.super.ctor(self)

	self.activityId = self:_getCurVersionInfo().activityId
	self.mainHudRedId = self:_getCurVersionInfo().redId
	self.cfg_single = DressActivityShopConfig.instance:getSingleCfgByActId(self.activityId)
	self.cfg_activity = ActivityDefineConfig.instance:getCfgById(GameEnum.ActivityType.DressActivityShop, self.activityId)

	if self.cfg_single == nil or #self.cfg_single == 0 then
		printError(string.format("缺少 Z-支付售卖活动, id = %s 的配置", self.activityId))
	end

	if self.cfg_activity == nil then
		printError(string.format("缺少 Y-运营活动时间(改)配置表, type = %s, id = %s 的配置", GameEnum.ActivityType.DressActivityShop, self.activityId))
	end
...
function FashionShowSkinSaleView:buildUI()
	FashionShowSkinSaleView.super.buildUI(self)
...
function FashionShowSkinSaleView:bindEvents()
	FashionShowSkinSaleView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self._onClickBtnClose, self)
end

function FashionShowSkinSaleView:unbindEvents()
	FashionShowSkinSaleView.super.unbindEvents(self)
...
function FashionShowSkinSaleView:destroyUI()
	FashionShowSkinSaleView.super.destroyUI(self)
	self:_clearModel()
end

function FashionShowSkinSaleView:onEnter()
	FashionShowSkinSaleView.super.onEnter(self)

	local key = PayTrussController.MAINHUDFIRSTCLICKREDPOINT .. self.activityId

	PayTrussController.instance:_hideRedIfOpen(key, self.mainHudRedId)
	self:_setPanelUI()
...
function FashionShowSkinSaleView:onExit()
	FashionShowSkinSaleView.super.onExit(self)
...
function FashionShowSkinSaleView:_onGetPayTrussInfo()
	PayTrussController.instance:_sendReqGetPayTrussInfo(self.activityId)
end

function FashionShowSkinSaleView:_buildCell(index, data)
	local cell = {}
	...
	cell._txtName = goutil.findChild(cell._mainGo, "txtName"):GetComponent("Text")
	cell._btnBuy = goutil.findChild(cell._mainGo, "btnBuy")
	cell._txtBtnBuy = goutil.findChild(cell._mainGo, "btnBuy/text"):GetComponent("Text")
	cell._itemcon = goutil.findChild(cell._mainGo, "itemcon")
	cell._txtLimit = goutil.findChild(cell._mainGo, "txtLimit"):GetComponent("Text")
...
function FashionShowSkinSaleView:_buildItem(index, cfgStr, father)
	local item = {}
...
function FashionShowSkinSaleView:_buildMod(index, data)
	local mod = {}
...
function FashionShowSkinSaleView:_setPanelUI()
	...
	local effPath = "20211231/daiji/fx_ui_fzsm_daiji.prefab"
...
function FashionShowSkinSaleView:_setModel()
	local attributes = {
		{
			camSize = 1.3,
			rawSize = {
...
function FashionShowSkinSaleView:_showModel(mod, attr)
...
function FashionShowSkinSaleView:_roleTrun(eventData, avatar)
...
function FashionShowSkinSaleView:_clearModel()
...
function FashionShowSkinSaleView:_setCellUI(cell)
	cell._txtName.text = cell.data.name or "未配置名称"

	local value = PayConfig.instance:getPayMoneyYuan(cell.data.payGoodsId)

	cell._txtBtnBuy.text = value .. "元"

	uGuiUtil.setGoGrayState(cell._btnBuy.gameObject, true)
...
function FashionShowSkinSaleView:_setItemUI(item)
	MaterialMgr.setCellByCfg(item.cfgStr, item._mainGo)
end

function FashionShowSkinSaleView:_setActivityTime()
...
function FashionShowSkinSaleView:_updateTimer()
	local leftTime = self.endTime - ServerTime.nowServerLook()
...
function FashionShowSkinSaleView:_onRefreshUI()
	local hasPurchasedIds = PayTrussModel.instance:_getPayTrussInfoFormModel()
	local cfg_line_coalition = DressActivityShopConfig.instance:getSuitCfgByActIdAndHasOwnTab(self.activityId, hasPurchasedIds)
	...
	if isCanBuy then
		local value = PayConfig.instance:getPayMoneyYuan(cfg_line_coalition.payGoodsId)

		self._btnBuyTxt.text = langPara("text_buy_all_commodity_tips_2", value)
	end
...
function FashionShowSkinSaleView:_updateCell(cell, hasPurchasedIds)
	for _, cell in ipairs(self.cellList) do
		local isHasBuy = table.indexof(hasPurchasedIds, cell.data.id) ~= false

		uGuiUtil.setGoGrayState(cell._btnBuy.gameObject, isHasBuy)
		goutil.setActive(cell._soldout, isHasBuy)
		goutil.setActive(cell._btnBuy.gameObject, not isHasBuy)

		if isHasBuy == true then
			cell._txtBtnBuy.text = lang("text_recall_desc_6")
		else
			local value = PayConfig.instance:getPayMoneyYuan(cell.data.payGoodsId)
...
function FashionShowSkinSaleView:_updateItem(item)
	return
end

function FashionShowSkinSaleView:_onClickBtnClose()
	self:close()
end

function FashionShowSkinSaleView:_onClickCellBtnBuy(cell)
	...
	if cfg_line_coalition == nil then
		value = PayConfig.instance:getPayMoneyYuan(cell.data.payGoodsId)
		tips = langPara("text_buy_commodity_tips", value, cell._txtName.text)
	else
		value = PayConfig.instance:getPayMoneyYuan(cfg_line_coalition.payGoodsId)
		tips = langPara("text_buy_all_commodity_tips", value)
	end

	local function okFunc()
		local para = {}

		para.id = cell.data.id

		local goodsId = cell.data.payGoodsId
	...
	local key = PayTrussController.FIRSTCLICKBTNBUYSHOWTIPS .. self.activityId
	local isHasClicked = PayTrussController.instance:_isHasClickedAnyGoods(key)

	if isHasClicked == true then
		okFunc()
	else
...
function FashionShowSkinSaleView:_onClickTotalBtnBuy()
	if self:_isInActivityTime() == false then
		return
	end
	...
	local goodsId = cfg_line_coalition.payGoodsId
	local subGoodsType = GameEnum.PaySubGoodsType.DRESSACTIVITY
	local subGoodsId = self.activityId
	local extParams = GameUtil.jsonToString(para)
...
function FashionShowSkinSaleView:_isInActivityTime()
	local leftTime = self.endTime - ServerTime.nowServerLook()
...
function FashionShowSkinSaleView:_getCurVersionInfo()
	curVersionInfo = PayTrussController.curVersionInfos[1]

	return curVersionInfo
end

return FashionShowSkinSaleView
