-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/pandoracopy/view/PandoraBattleView.lua

module("logic.extensions.pandoracopy.view.PandoraBattleView", package.seeall)

local PandoraBattleView = class("PandoraBattleView", ViewComponent)

function PandoraBattleView:ctor()
	PandoraBattleView.super.ctor(self)
...
function PandoraBattleView:bindEvents()
	PandoraBattleView.super.bindEvents(self)
	...
	self._btnEmpty:AddClickListener(self._onClickDaibi, self)
end

function PandoraBattleView:unbindEvents()
	PandoraBattleView.super.unbindEvents(self)
...
function PandoraBattleView:onExit()
	PandoraBattleView.super.onExit(self)
	...
	MaterialMgr.resetAll(self._scoreIconGo)
	MaterialMgr.resetAll(self._daibiIconGo)
end

function PandoraBattleView:destroyUI()
	PandoraBattleView.super.destroyUI(self)
...
function PandoraBattleView:buildUI()
	PandoraBattleView.super.buildUI(self)
	...
	self._btnEmpty = self:getBtn("allRewardGo/scoreGo/btnEmpty")
	self._goldBarCon = self:getGo("goldBarCon")

	self:registCallbacks()
end

function PandoraBattleView:registCallbacks()
...
function PandoraBattleView:_MoshiInView()
...
function PandoraBattleView:_MoshiAtIndex(view, index)
	local cell
...
function PandoraBattleView:_OnMoshiCellUpdate(view, cell)
...
function PandoraBattleView:_RewardInView()
...
function PandoraBattleView:_RewardAtIndex(view, index)
	local cell
	local cell = view:DequeueCell()
...
function PandoraBattleView:_OnRewardCellUpdate(view, cell)
	local index = cell.index
	local data = self._curRewardDatas[index + 1]
...
function PandoraBattleView:onEnter()
	PandoraBattleView.super.onEnter(self)
	...
	if curChallId <= 0 then
		curChallId = MoonKingModel.instance.pandoraChanllengeId
	end

	MoonKingModel.instance:setMoonkingChooseBuff()

	local str = string.format("PandoraViewRed_%s_%s", curChallId, LoginModel.instance.userId)
	...
	self:_setTopGoldBar(self._goldBarCon, self._challCfg.daibiStr)
...
function PandoraBattleView:_showActivityTimer()
...
function PandoraBattleView:_calculationSurplusTime()
...
function PandoraBattleView:_updataBattleViewInfo(challId, isPrize)
	...
	local matType, matId, matNum = MaterialMgr.getMatParams(self._challCfg.scoreStr)

	MaterialModel.instance:UpdateMaterial(matType, matId, self._moonkingInfo.totalScore)
...
function PandoraBattleView:_updateMoshiCell(view, cell, data)
...
function PandoraBattleView:_showChoiseStageInfo(index)
...
function PandoraBattleView:_updateRewardCell(view, cell, data)
...
function PandoraBattleView:_updateShowProgressBar()
	if self._curRewardDatas then
		if not #self._curRewardDatas then
			local count = 0
...
function PandoraBattleView:_onClickNanduBtn()
...
function PandoraBattleView:_onClickDaibi()
	local matStr = self._challCfg.scoreStr
	local matType, matId, matNum = MaterialMgr.getMatParams(matStr)

	CommonTipsMgr.instance:openMaterialTips(self._scoreIconGo, matType, matId)
end

function PandoraBattleView:_onClickBtnShowTip()
