-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/companionmall/view/CompanionMallAddressBookView.lua

module("logic.extensions.companionmall.view.CompanionMallAddressBookView", package.seeall)

local CompanionMallAddressBookView = class("CompanionMallAddressBookView", ViewComponent)
local ADD_NEW_ADDRESS = 1
local CHANGE_ADDRESS = 2

function CompanionMallAddressBookView:ctor()
	CompanionMallAddressBookView.super.ctor(self)
end

function CompanionMallAddressBookView:unbindEvents()
	CompanionMallAddressBookView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnAddAddress)
end

function CompanionMallAddressBookView:bindEvents()
	CompanionMallAddressBookView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnAddAddress, self._onClickBtnAddAddress, self)
	GameUtil.addClickHandler(self._btnSure, self._onClickSure, self)
end

function CompanionMallAddressBookView:buildUI()
	CompanionMallAddressBookView.super.buildUI(self)
...
function CompanionMallAddressBookView:onExit()
	CompanionMallAddressBookView.super.onExit(self)
...
function CompanionMallAddressBookView:onEnter()
	CompanionMallAddressBookView.super.onEnter(self)
	...
	self.addGEvent(self, GlobalNotify.PM_CompanionMallDeleteAddressRes, self._onUpdate, self)
...
function CompanionMallAddressBookView:_initBookCellUI()
...
function CompanionMallAddressBookView:_onUpdate()
	self:_initBookCellUI()
		...
		if infos then
			for i, info in pairs(infos) do
				...
				if addressBookInfo then
					...
					GameUtil.addClickHandler(btnSelect, GameUtil.handler(self._onClickBtnSelect, self, addressId))
...
function CompanionMallAddressBookView:_onProvinceScrCustomInputCallback(hover)
	if not hover then
...
function CompanionMallAddressBookView:_onCityScrCustomInputCallback(hover)
	if not hover then
...
function CompanionMallAddressBookView:_onDistrictScrCustomInputCallback(hover)
	if not hover then
...
function CompanionMallAddressBookView:_onClickBtnAddAddress()
	...
	UIStateManager.instance:push(ViewName.CompanionmalladdressView, self._activityId, -1, isReWrite)
end

function CompanionMallAddressBookView:_onClickBtnReWrite(addressId)
	local isReWrite = false

	UIStateManager.instance:push(ViewName.CompanionmalladdressView, self._activityId, addressId, isReWrite)
end

function CompanionMallAddressBookView:_onClickBtnDelete(addressId)
...
function CompanionMallAddressBookView:_onClickBtnSelect(addressId)
	return
end

return CompanionMallAddressBookView
