-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/material/proxy/nameframe/NameFrameCell.lua

module("logic.extensions.material.proxy.nameframe.NameFrameCell", package.seeall)

local NameFrameCell = class("NameFrameCell", MaterialCell)

function NameFrameCell:ctor(target)
	self.target = target
	self.topGo = goutil.findChild(self.target, "topGo")
	self._imgIcon = goutil.findChild(self.target, "ImgIcon")
end

function NameFrameCell:setCfgData(data)
	NameFrameCell.super.setCfgData(self, data)
...
function NameFrameCell:dispose()
	NameFrameCell.super.dispose(self)
...
function NameFrameCell:OnDestroy()
	NameFrameCell.super.OnDestroy(self)
...
function NameFrameCell:_OnClick()
