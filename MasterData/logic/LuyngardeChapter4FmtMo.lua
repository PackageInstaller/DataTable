-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/luyngarde/model/LuyngardeChapter4FmtMo.lua

module("logic.extensions.luyngarde.model.LuyngardeChapter4FmtMo", package.seeall)

local LuyngardeChapter4FmtMo = class("LuyngardeChapter4FmtMo", BaseCustomFmtMo)

function LuyngardeChapter4FmtMo:initParams(activityId)
	self.activityId = activityId

	local actCfg = LuyngardeConfig.instance:getActCfg(self.activityId)

	self.lockRackId = actCfg.lockRaceId
	self.creepId = actCfg.c4CreepsMasterId
	...
	self.chapterCfg = LuyngardeConfig.instance:getChapterCfg(self.activityId, 4)
...
function LuyngardeChapter4FmtMo:getMonsterConfigList()
	return self.masterCfg
end

function LuyngardeChapter4FmtMo:getFmtInfoConfig()
	return self.creeps
end

function LuyngardeChapter4FmtMo:initFightHandler()
	self:setFightHandler(function()
		UIJumper.instance:saveCurStack(ViewName.CustomMissionView, ViewName.CustomFormationView)
		LuyngardeController.instance:startChapter4Challenge(self.activityId, self:getCurSimpleForm())
	end, nil)
end

function LuyngardeChapter4FmtMo:updateData()
	self:tryInitPetList()
...
function LuyngardeChapter4FmtMo:_addSupportPet(petId)
...
function LuyngardeChapter4FmtMo:initPetList()
...
function LuyngardeChapter4FmtMo:refreshPetList(changePetMoMap)
	for petId, petMo in pairs(changePetMoMap) do
		self:updatePetMo(petMo)
	end
end

function LuyngardeChapter4FmtMo:checkFormationEmptyAndPosNumLimit(posLimitCallback, hasOneKey, onekeyCallBack)
	if self:_checkSupPetId() then
		return LuyngardeChapter4FmtMo.super.checkFormationEmptyAndPosNumLimit(self, posLimitCallback, hasOneKey, onekeyCallBack)
	else
...
function LuyngardeChapter4FmtMo:onFormationChangeFinish()
	self:setValidatorDescYesOrNo(self:_checkSupPetId())
end

function LuyngardeChapter4FmtMo:_checkSupPetId()
