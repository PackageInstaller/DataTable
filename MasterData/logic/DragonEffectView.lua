-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/lottery/view/DragonEffectView.lua

module("logic.extensions.lottery.view.DragonEffectView", package.seeall)

local DragonEffectView = class("DragonEffectView", ViewComponent)

function DragonEffectView:buildUI()
	DragonEffectView.super.buildUI(self)

	self._camera = self:getGo("Camera")
	self._btnPass = self:getBtn("rightTop/passNode")
end

function DragonEffectView:bindEvents()
	self._btnPass:AddClickListener(self._onClickPass, self)
end

function DragonEffectView:unbindEvents()
	self._btnPass:RemoveClickListener()
end

function DragonEffectView:onEnter()
	self._isEffLoaded = nil
	self._drawTimes = self:getFirstParam()
	self._effConfig = DragonController.drawEffectsConfig
	...
	self._changeSetId = DragonController.instance:getDrawChangesetId()
	self._itemsData = DragonController.instance:getLastDrawItems()
	self._qualities = DragonController.instance:getLastDrawQualities()

	self._camera.transform:SetParent(nil)
	Framework.TransformUtil.SetPos(self._camera.transform, 0, 1, -10)
end

function DragonEffectView:onExit()
...
function DragonEffectView:_onDrawEffLoaded(eff)
	...
	for i = 1, #self._qualities do
		if self._equipNodes[i] then
			local qualityEffPath = DragonController.EquipEffsByQuality[self._qualities[i]]
...
function DragonEffectView:_playDrawSound()
...
function DragonEffectView:_stopDrawSound()
...
function DragonEffectView:_onEffectPlayFinished(eff)
...
function DragonEffectView:_onClickPass()
	self:_onEffectPlayFinished()
end

return DragonEffectView
