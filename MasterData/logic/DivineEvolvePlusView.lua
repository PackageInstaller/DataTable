-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/divineevolveplus/view/DivineEvolvePlusView.lua

module("logic.extensions.divineevolveplus.view.DivineEvolvePlusView", package.seeall)

local DivineEvolvePlusView = class("DivineEvolvePlusView", ViewComponent)
local SKIP_KEY = "DivineEvolvePlusView_SKIP"
local State_Fold = 1
local State_UnFold = 2

function DivineEvolvePlusView:ctor()
	DivineEvolvePlusView.super.ctor(self)
end

function DivineEvolvePlusView:buildUI()
	DivineEvolvePlusView.super.buildUI(self)
	...
	self._imgSelect = self:getGo("left/btnSkip/imgSelect")
	self._btnSkip = self:getGo("left/btnSkip")
...
function DivineEvolvePlusView:bindEvents()
	DivineEvolvePlusView.super.bindEvents(self)
...
function DivineEvolvePlusView:unbindEvents()
	DivineEvolvePlusView.super.unbindEvents(self)
...
function DivineEvolvePlusView:onEnter()
	DivineEvolvePlusView.super.onEnter(self)
	GlobalDispatcher:addListener(GlobalNotify.DivineEvolvePlusRes, self._onDivineEvolveRes, self)
	GlobalDispatcher:addListener(GlobalNotify.DivineEvolvePlusAnimationEnd, self._onAnimationEnd, self)
...
function DivineEvolvePlusView:onExit()
	DivineEvolvePlusView.super.onExit(self)
	GlobalDispatcher:removeListener(GlobalNotify.DivineEvolvePlusRes, self._onDivineEvolveRes, self)
	GlobalDispatcher:removeListener(GlobalNotify.DivineEvolvePlusAnimationEnd, self._onAnimationEnd, self)
...
function DivineEvolvePlusView:_onClickClose()
	self:close()
end

function DivineEvolvePlusView:_onClickEvolve()
	if self._curPetInfo then
		self:_onClickPreEvolve()
	end
end

function DivineEvolvePlusView:_onClickTip()
	UIStateManager.instance:push(ViewName.RulesView, "divineevolveplusview")
end

function DivineEvolvePlusView:_onClickInfo()
	if self._curPetInfo then
...
function DivineEvolvePlusView:_onClickLook()
	if self._curPetInfo then
		UIStateManager.instance:push(ViewName.DivineEvolvePlusLookView, self._curPetInfo.cfg.raceId)
	end
end

function DivineEvolvePlusView:_onClickChange()
	if self._curPetInfo then
...
function DivineEvolvePlusView:_onClickShop()
...
function DivineEvolvePlusView:_onClickClearSearch()
	self._InputC_Search.text = ""
end

function DivineEvolvePlusView:_onClickPreEvolve()
	printInfo("test DivineEvolvePlusView:_onClickPreEvolve1", self._isExistUnique)

	if self._curEvolveConditionType == DivineEvolvePlusController.EvolveCondition.None then
		printInfo("test DivineEvolvePlusView:_onClickPreEvolve2", self._isExistUnique)
	...
	elseif self._curEvolveConditionType == DivineEvolvePlusController.EvolveCondition.UniqueItem then
		printInfo("test DivineEvolvePlusView:_onClickPreEvolve3", self._isExistUnique)

		if not self._isExistUnique then
			return
		end

		local cfg = EvolveConfig.instance:getDivineEvolvePlusCfg(self._curPetInfo.cfg.raceId, EvolveEnum.EvolveType.Divine)
		...
		if MaterialMgr.getMatEnough(cfg.goodsCost) then
			...
			local text = string.format("是否消耗<color=#EB4642> %s </color>*%d 和 %s *%d 进行神格突破？", name1, matNum, name2, matNum2)
		...
		printInfo("test DivineEvolvePlusView:_onClickPreEvolve4", self._isExistUnique)
		...
		if self._curSelectPetIds and self._curSelectPetIds[1] then
			...
			if petMo then
				local petBookLvNotMaxAddTip = "※神格突破需要<color=#EB4642>消耗1只精灵</color>，建议该精灵<color=#EB4642>图鉴满级后</color>再进行神格突破"
				local cfg = EvolveConfig.instance:getDivineEvolvePlusCfg(petMo.raceId, EvolveEnum.EvolveType.Divine)
				...
				elseif self._curPetInfo and self._curPetInfo.cfg and MaterialMgr.getMatEnough(self._curPetInfo.cfg.goodsCost) then
					...
					local function okFunc()
						DivineEvolvePlusController.instance:sendPM_DivineExchangeReq(petId, petMo.raceId, self._curPetInfo.cfg.newRaceId, true)
					end
...
function DivineEvolvePlusView:_onDivineEvolveRes(newPetMo, oldPetMo)
	printInfo("test 圣启进化成功！！！", oldPetMo, newPetMo)
	...
	if index == 0 then
		...
		TweenUtil.DoDelay(1.5, function()
			...
			DivineEvolvePlusController.instance:playEvolveAnimation(newPetMo, oldPetMo)
		end)
	else
...
function DivineEvolvePlusView:_onAnimationEnd(newPetMo, oldPetMo)
...
function DivineEvolvePlusView:_initGameObjects()
	goutil.setActive(self._maskGo, false)
...
function DivineEvolvePlusView:_loadEffect()
	self:_removeEffect()
...
function DivineEvolvePlusView:_removeEffect()
...
function DivineEvolvePlusView:_initPetList()
...
function DivineEvolvePlusView:updateListData(list)
	self._curViewDatas = list

	self._scrollList:reloadData(list)
end

function DivineEvolvePlusView:_refreshPetList(preEvolveRaceId)
	local selectIndex = 1
...
function DivineEvolvePlusView:_sortPetListByGetState(list)
...
function DivineEvolvePlusView:_isSelectPet(index, info, raceId)
	return (raceId == nil or raceId == 0) and index == 1 or raceId == ((info or nil) and (info.cfg.raceId or -1))
end

function DivineEvolvePlusView:_supperUpdateListData(list, raceId)
	raceId = checknumber(raceId)

	self:updateListData(list)
end

function DivineEvolvePlusView:_updateCell(view, cell, data)
...
function DivineEvolvePlusView:_clearTableview(cell)
	local icon = goutil.findChild(cell, "icon")
	local btnClick = Framework.ButtonAdapter.GetFrom(cell.gameObject, "btnClick")
...
function DivineEvolvePlusView:_onClickPetItem(petInfo)
...
function DivineEvolvePlusView:_refreshSelectState(petInfo)
	for i, v in ipairs(self._initList) do
...
function DivineEvolvePlusView:_getRaceId(petInfo)
	return (petInfo.isShowNew or nil) and (petInfo.cfg.newRaceId or petInfo.cfg.raceId)
end

function DivineEvolvePlusView:_refreshPetInfo(petInfo)
	if petInfo then
...
function DivineEvolvePlusView:_setPetBaseInfo(raceId)
...
function DivineEvolvePlusView:_showRoleModel(raceId)
...
function DivineEvolvePlusView:_resetRoleModel()
	self.loader = RoleObjectPool.instance:removeRole(self.loader)
end

function DivineEvolvePlusView:_setPreEvolveConditions(raceId)
	self._isExistUnique = false
	self._curEvolveConditionType = DivineEvolvePlusController.EvolveCondition.None
	...
	local cfg = EvolveConfig.instance:getDivineEvolvePlusCfg(raceId, EvolveEnum.EvolveType.Divine)
...
function DivineEvolvePlusView:_removePreEvolveConditions()
	for i, v in ipairs(self._preEvolveCells) do
...
function DivineEvolvePlusView:_resetPreEvolveCell(element, conditionStr, newRaceId)
...
function DivineEvolvePlusView:_setPreEvolveCell(index, element, conditionStr, desc, newRaceId)
	...
	element.btnAdd:AddClickListener(function()
		if self._curEvolveConditionType == DivineEvolvePlusController.EvolveCondition.Pet then
			...
			return
		elseif self._curEvolveConditionType == DivineEvolvePlusController.EvolveCondition.UniqueItem then
			FloatWordMgr.instance:show("已放入神曜材料，无需再次放入")
		...
		local function sortFunc(pets)
			...
			for k, v in ipairs(sortValue) do
				...
				if vArr[v + 1] ~= -1 then
					...
					if sortName ~= "petId" and table.indexof(names, sortName) then
						return
					end
				end
			end
...
function DivineEvolvePlusView:_setPreEvolveItem(element, goodsCost)
	MaterialMgr.resetAll(element.item)
...
function DivineEvolvePlusView:_resetPreEvolveUniqueItem(element, petCostItem)
...
function DivineEvolvePlusView:_setPreEvolveUniqueItem(element, petCostItem)
	MaterialMgr.resetAll(element.item)
...
function DivineEvolvePlusView:_getIconPath(initRare, raceType)
...
function DivineEvolvePlusView:_onUpdateEvolveCondition(index, petId)
...
function DivineEvolvePlusView:OnFilter(petMo, str)
	return FightingPowerFormula.instance:OnFilter(petMo, str)
end

function DivineEvolvePlusView:_updateTip1(active, newRaceId)
...
function DivineEvolvePlusView:_updateTip2(active, raceId)
...
function DivineEvolvePlusView:_lockUniqueItemCondition()
...
function DivineEvolvePlusView:_unlockUniqueItemCondition()
	local itemElement = self._preEvolveCells[3]
...
function DivineEvolvePlusView:_onSelectUniqueItemConditon(matNum)
...
function DivineEvolvePlusView:_lockEvolvePetCondition()
...
function DivineEvolvePlusView:_unlockEvolvePetCondition()
	local itemElement = self._preEvolveCells[1]
...
function DivineEvolvePlusView:_refreshFoldState(state, isRefreshList)
...
function DivineEvolvePlusView:_createFilterList()
...
function DivineEvolvePlusView:_getPosAndSizeDelta(go)
	local sizeDelta = {
		0,
		0
	}
...
function DivineEvolvePlusView:_updateFilterData(starList, jobList, rareList)
	self:_refreshFilterPetList()
end

function DivineEvolvePlusView:_refreshFilterPetList()
...
function DivineEvolvePlusView:_onFilter(petInfo)
...
function DivineEvolvePlusView:_isMatch(cfg, rare, skinId)
	return (PetFilterMgr.instance:checkIsMatch(PetFilterMgr.TYPE_DIVINE_EVOLVE, cfg, rare, skinId))
end

function DivineEvolvePlusView:_OnSearchTextChanged(text)
...
function DivineEvolvePlusView:_exchangeTimeOk(cfgExchange)
	return cfgExchange and (string.nilorempty(cfgExchange.beginTime) or ServerTime.now() > GameUtil.string2time(cfgExchange.beginTime))
end

return DivineEvolvePlusView
