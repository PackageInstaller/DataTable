-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/boccacciochapter/model/BoccaccioChapterC1FmtMo.lua

module("logic.extensions.boccacciochapter.model.BoccaccioChapterC1FmtMo", package.seeall)

local BoccaccioChapterC1FmtMo = class("BoccaccioChapterC1FmtMo", BaseCustomFmtMo)

function BoccaccioChapterC1FmtMo:onReset()
	BoccaccioChapterC1FmtMo.super.onReset(self)
end

function BoccaccioChapterC1FmtMo:updateCfg(activityId, stageId)
	self._activityId = activityId
	self._stageId = stageId
	self._chapterId = BoccaccioChapterEnum.ChapterId_1

	local stageData = BoccaccioChapterConfig.instance:getChapter1StageData(self._activityId, self._stageId)
...
function BoccaccioChapterC1FmtMo:initPetList()
...
function BoccaccioChapterC1FmtMo:initFightHandler()
	local function handler()
		UIJumper.instance:saveCurStack(ViewName.CustomMissionView, ViewName.CustomFormationView)

		local simpleForm = self:getCurSimpleForm()

		BoccaccioChapterController.instance:sendPM_BoccaccioChapter1ClgReq(self._activityId, self._stageId, simpleForm)
	end

	self:setFightHandler(handler, nil)
end

function BoccaccioChapterC1FmtMo:checkFormationEmptyAndPosNumLimit(posLimitCallback, hasOneKey, onekeyCallBack)
...
function BoccaccioChapterC1FmtMo:getMonsterConfigList()
	return self._creepsCfg
end

function BoccaccioChapterC1FmtMo:getFmtInfoConfig()
	return self._creepsTeamData
end

function BoccaccioChapterC1FmtMo:getFinalFmoList(posList, petList, needPets)
	local fixPosList = {}
	local fixPetMoList = {}
	local petMo = self:getPetMoById(self._mustDeployPetId)
...
return BoccaccioChapterC1FmtMo
