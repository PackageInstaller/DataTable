-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/duolammgame/view/DuoLaMMGameView.lua

module("logic.extensions.duolammgame.view.DuoLaMMGameView", package.seeall)

local DuoLaMMGameView = class("DuoLaMMGameView", ViewComponent)
...
function DuoLaMMGameView:ctor()
	DuoLaMMGameView.super.ctor(self)
end

function DuoLaMMGameView:buildUI()
	DuoLaMMGameView.super.buildUI(self)
	...
	self._ripiteEffect = nil
	self._prizeEffect = {}
end

function DuoLaMMGameView:bindEvents()
	DuoLaMMGameView.super.bindEvents(self)
	self._btnClose:AddClickListener(self._clickClose, self)
	self._bg:AddClickListener(self._clickBg, self)
end

function DuoLaMMGameView:unbindEvents()
	DuoLaMMGameView.super.unbindEvents(self)
	self._btnClose:RemoveClickListener()
	self._bg:RemoveClickListener()
end

function DuoLaMMGameView:onEnter()
	DuoLaMMGameView.super.onEnter(self)
	...
	if not self:_checkCurActCfg(params[1]) then
		TipsFacade.instance:openTipWindowNoX(lang("tip"), lang("缺游戏配置"), GameUtil.handler(self.close, self))

		return
	end

	local firstOpenKey = string.format("%s_%s", ViewName.DuoLaMMGameView, self._activityId)
	local isFirst = checknumber(GameUtil.getUserData(firstOpenKey)) <= 0

	if isFirst then
		UIStateManager.instance:push(ViewName.DuolaMMGameRuleView)
		GameUtil.saveUserData(firstOpenKey, 1)
	end

	self.addGEvent(self, GlobalNotify.DuoLaMMGameStartRes, self._start, self)
	self.addGEvent(self, GlobalNotify.DuoLaMMGameEndRes, self._handleGameOver, self)
	self.addGEvent(self, GlobalNotify.DuoLaMMGameError, self._handleGameError, self)
	self:_prepareData()
...
function DuoLaMMGameView:onExit()
	DuoLaMMGameView.super.onExit(self)
...
function DuoLaMMGameView:_showRiptiteEffect()
...
function DuoLaMMGameView:_checkCurActCfg(actId)
	self._activityId = checknumber(actId)

	if not ActivityDefineController.instance:isInActivityTimeById(GameEnum.ActivityType.DuoLaMMGame, self._activityId) then
		FloatWordMgr.instance:show("游戏未开放")

		return false
	end

	self._actCfg = DuoLaMMGameConfig.instance:getActCfg(self._activityId)
	self._prizeCfgs = DuoLaMMGameConfig.instance:getGamePieceCfgs(self._activityId)
	self._dropPlanCfg = DuoLaMMGameConfig.instance:getDropCfgs(self._actCfg.dropPlanId)
	self._rebornCfg = DuoLaMMGameConfig.instance:getBuyReliveTimesCfgs(self._activityId)
	self._gameCfg = DuoLaMMGameConfig.instance:getGameCfgs(self._actCfg.gamePlanId)

	if self._actCfg and self._prizeCfgs and self._dropPlanCfg and self._gameCfg then
		return true
	else
		return false
	end
...
function DuoLaMMGameView:_prepareData()
	self._prizeList = {}

	for k, v in ipairs(self._prizeCfgs) do
		local matCfg = MaterialMgr.getMatCfgByStr(v.prize)

		v.num = 0
		v.imgPath = GameUrl.getItemIconUrl(matCfg.icon)

		table.insert(self._prizeList, v)
	end

	if self._rebornCfg then
		self._rebornMaxTime = #self._rebornCfg or 0
	end
...
function DuoLaMMGameView:_refresh()
	self:_refreshCells()
	self._scrollList:reloadData(self._prizeList)
end

function DuoLaMMGameView:_refreshHeart()
	for i = 1, 3 do
		goutil.setActive(self._heart[i].go, i <= self._heartNum)
	end
end

function DuoLaMMGameView:_refreshPrize()
	self._scrollList:refresh(self._prizeList)
end

function DuoLaMMGameView:_updatePieceCell(view, cell, data)
...
function DuoLaMMGameView:_showJiaYiEffect(cell)
...
function DuoLaMMGameView:_clearPieceCell(cell)
	local icon = goutil.findChild(cell, "icon")
...
function DuoLaMMGameView:_refreshCells()
...
function DuoLaMMGameView:_clickBg()
...
function DuoLaMMGameView:_clickClose()
...
function DuoLaMMGameView:_addCell(index)
...
function DuoLaMMGameView:_clearCell(index)
...
function DuoLaMMGameView:_sendStartReq()
	DuoLaMMGameModel.instance:randomGameClientKey(self._activityId)

	local clientKey = DuoLaMMGameModel.instance:getGameClientKey(self._activityId)

	DuoLaMMGameController.instance:sendPM_DuoLaMMGameStartGameReq(self._activityId, clientKey)
end

function DuoLaMMGameView:_start()
...
function DuoLaMMGameView:_updateFrame()
...
function DuoLaMMGameView:_resetScroller()
	GameUtil.setLocalPos(self._scroller, 0, 0, 0)
end

function DuoLaMMGameView:_resetPlayer()
	self._player.pos = {
		x = START_POS_X,
		y = START_POS_Y
	}
...
function DuoLaMMGameView:_refreshCurCfg()
...
function DuoLaMMGameView:_getGameCfg(index)
	local gameCfg
...
function DuoLaMMGameView:_collide()
	self:_suspend()
...
function DuoLaMMGameView:_showHeartEffect()
...
function DuoLaMMGameView:_collideEffectEnd()
	if self._colorTween then
		self._colorTween:Kill(false)
...
function DuoLaMMGameView:_handleGameOver()
	removetimer(self._gameOver, self)
...
function DuoLaMMGameView:_gameOver()
	local ids = {}
	...
	local encryptedKey = DuoLaMMGameModel.instance:getEncryptedKey(self._activityId, ids)

	DuoLaMMGameController.instance:sendPM_DuoLaMMGameEndGameReq(self._activityId, ids, encryptedKey)
end

function DuoLaMMGameView:_gameOverTip()
	removetimer(self._updateFrame, self)
	settimer(1.6, self._gameOver, self, false)
	FloatWordMgr.instance:show("生命耗尽，游戏结束！")
end

function DuoLaMMGameView:_suspend()
	removetimer(self._updateFrame, self)
end

function DuoLaMMGameView:_continue()
	self:_updateFrame()
	settimer(GAME_UPDATETIME, self._updateFrame, self, true)
end

function DuoLaMMGameView:_sendReborn()
	self._rebornCallBack()
end

function DuoLaMMGameView:_handleReborn(status)
	if status == 0 then
		self:_gameReborn()
	else
...
function DuoLaMMGameView:_gameReborn()
...
function DuoLaMMGameView:_gameRebornEffectEnd()
...
function DuoLaMMGameView:_jump()
	...
	if nextCell then
		...
		local time = self:calculateDistance(nextPos, playerPos) / JUMP_SPEED
...
function DuoLaMMGameView:calculateDistance(pos1, pos2)
...
function DuoLaMMGameView:_jumpSuccess(comp)
	local dropId = comp:getDropId()
	local nextPos = comp:getPos()
...
function DuoLaMMGameView:_checkVirusCollision()
	local nextCell = self._cellList[self._index + 1]
...
function DuoLaMMGameView:_handleGameError(status)
	FloatWordMgr.instance:show("游戏数据返回错误" .. status)
	self:close()
end

return DuoLaMMGameView
