-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/yirenpozhen/view/HexagramGameView.lua

module("logic.extensions.yirenpozhen.view.HexagramGameView", package.seeall)

local HexagramGameView = class("HexagramGameView", ViewComponent)
...
function HexagramGameView:ctor()
	HexagramGameView.super.ctor(self)

	self._cells = {}
end

function HexagramGameView:buildUI()
	HexagramGameView.super.buildUI(self)
...
function HexagramGameView:bindEvents()
	HexagramGameView.super.bindEvents(self)
...
function HexagramGameView:unbindEvents()
	HexagramGameView.super.unbindEvents(self)
...
function HexagramGameView:destroyUI()
	HexagramGameView.super.destroyUI(self)
end

function HexagramGameView:onEnter()
	HexagramGameView.super.onEnter(self)

	local params = self:getFirstParam()

	self._changing = false
	self._stepNum = 0
	self._gameCfg = params.gameCfg
...
function HexagramGameView:onEnterFinished()
	HexagramGameView.super.onEnterFinished(self)
end

function HexagramGameView:onExit()
	HexagramGameView.super.onExit(self)
...
function HexagramGameView:onExitFinished()
	HexagramGameView.super.onExitFinished(self)
end

function HexagramGameView:_resetAll()
...
function HexagramGameView:_doResetAll()
...
function HexagramGameView:_generateAll()
	local layer = 1
...
function HexagramGameView:_clockWise()
...
function HexagramGameView:_anticlockWise()
	AudioPlayerEx.instance:playEffect(30106)
...
function HexagramGameView:_change()
	AudioPlayerEx.instance:playEffect(30106)
...
function HexagramGameView:_checkCompelete()
	for layer = 1, self._layerNum do
...
function HexagramGameView:_compelete()
	TipsFacade.instance:openPopupWindow("恭喜你成功通关", langPara("本次通过所用步数：%s\n你的最快通关步数：%s\n", self._stepNum, YirenPozhenModel.instance:getBestStepNum()), function()
		YirenPozhenController.instance:sendEndGame(1, self._gameId, self._stepNum)
		self:close()
	end, function()
		self:_resetAll()
	end, "退出", "重新开始", UnityEngine.TextAnchor.MiddleCenter)
end

function HexagramGameView:_refreshStep()
	self._txtStep.text = langPara("当前步数：%s", self._stepNum)
	self._txtRecord.text = langPara("我的记录：%s", YirenPozhenModel.instance:getBestStepNum())
end

function HexagramGameView:_clickHelp()
	local ruleCo = RulesConfig.instance:getRuleCo("HexagramGameRule")
...
function HexagramGameView:_failClose()
	YirenPozhenController.instance:sendEndGame(0, self._gameId, 9999)
	self:close()
end

return HexagramGameView
