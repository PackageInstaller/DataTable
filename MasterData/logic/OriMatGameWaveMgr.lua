-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/orimatgame/controller/OriMatGameWaveMgr.lua

module("logic.extensions.orimatgame.controller.OriMatGameWaveMgr", package.seeall)

local OriMatGameWaveMgr = class("OriMatGameWaveMgr")

function OriMatGameWaveMgr:initWave(monsterPlanIds, isLoopState)
	local waveList = {}
	local monsterIds = string.splitToNumber(monsterPlanIds, "#")

	for _, planId in ipairs(monsterIds) do
		local waveCfgs = OriMatGameConfig.instance:getMonsterWaveCfgs(planId) or {}
		...
		if not waveCfgs.waveGroupList then
			...
			for i, groupId in ipairs(monsterGroupList) do
				local groupCfg = OriMatGameConfig.instance:getMonsterGroupCfgs(groupId)
...
function OriMatGameWaveMgr:initFunc(checkAliveFunc, createMonsterFunc, newWaveFunc)
	self._checkAliveFunc = checkAliveFunc
	self._createMonsterFunc = createMonsterFunc
	self._newWaveFunc = newWaveFunc
end

function OriMatGameWaveMgr:_startNextWave(waveGroupObj)
...
function OriMatGameWaveMgr:_checkCurrMonsterAllDead(waveGroupObj, checkAliveFunc)
	...
	if cfg == nil then
		...
		if not GameUtil.callBack(checkAliveFunc, OriMatGameEnum.UnitType.Monster, waveGroupObj.monsterIdList) then
			return true
		end
	end
...
function OriMatGameWaveMgr:_createMonster(waveGroupObj, createMonsterFunc)
...
function OriMatGameWaveMgr:_updateWave(waveGroupObj, deltaTime, checkAliveFunc, createMonsterFunc)
...
function OriMatGameWaveMgr:updateWaveList(waveList, deltaTime)
...
function OriMatGameWaveMgr:checkIsGameOver(waveList)
	local isOver = true
...
function OriMatGameWaveMgr:getCurrMaxWaveCount(waveList)
	local count = 0
	local total = 0
...
OriMatGameWaveMgr.instance = OriMatGameWaveMgr.New()

return OriMatGameWaveMgr
