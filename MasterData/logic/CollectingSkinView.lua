-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/collectingskin/view/CollectingSkinView.lua

module("logic.extensions.collectingskin.view.CollectingSkinView", package.seeall)

local CollectingSkinView = class("CollectingSkinView", ViewComponent)

function CollectingSkinView:buildUI()
	CollectingSkinView.super.buildUI(self)
...
function CollectingSkinView:bindEvents()
	CollectingSkinView.super.bindEvents(self)
	self._btnClose:AddClickListener(self.close, self)
end

function CollectingSkinView:unbindEvents()
	CollectingSkinView.super.unbindEvents(self)
	self._btnClose:RemoveClickListener()
end

function CollectingSkinView:onEnter()
	CollectingSkinView.super.onEnter(self)
...
function CollectingSkinView:onExit()
	CollectingSkinView.super.onExit(self)
	self._tableviewSkin:dispose()
	self._tableviewProgress:dispose()
end

function CollectingSkinView:_refreshByCfgs()
...
function CollectingSkinView:_onGetInfo()
	self:_updateTableviewSkin()
	self:_onGainPrize()
end

function CollectingSkinView:_onGainPrize()
...
function CollectingSkinView:_updateTableviewSkin()
...
function CollectingSkinView:_updateSkinCell(view, cellGo, data)
	...
	if not string.nilorempty(data.bgPath) then
		local imgPath = GameUrl.getBigbgPngUrl(data.bgPath)
	...
	if not string.nilorempty(data.bgBlackPath) then
		local imgPathBlack = GameUrl.getBigbgPngUrl(data.bgBlackPath)
...
function CollectingSkinView:_clearSkinCell(cellGo)
	local cell = {}
...
function CollectingSkinView:_updateProgressCell(view, cellGo, data)
...
function CollectingSkinView:_clearProgressCell(cellGo)
	local cell = {}
...
function CollectingSkinView:_playCellEffect(effGo)
...
function CollectingSkinView:_clearCellEffect(effGo)
...
function CollectingSkinView:_onClickSkinBook(skinId, isPet)
	if skinId then
		if isPet == 1 then
...
function CollectingSkinView:_onClickGoto(skinId, isPet)
	if isPet == 1 then
