-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/divinenuoyachallenge/view/DivineNuoYaSelectView.lua

module("logic.extensions.divinenuoyachallenge.view.DivineNuoYaSelectView", package.seeall)

local DivineNuoYaSelectView = class("DivineNuoYaSelectView", TableViewComponent)

function DivineNuoYaSelectView:_getPath()
	return {
		cellPath = "tablecell",
		viewPath = "tableview"
	}
...
function DivineNuoYaSelectView:ctor()
	DivineNuoYaSelectView.super.ctor(self)
...
function DivineNuoYaSelectView:buildUI()
	DivineNuoYaSelectView.super.buildUI(self)
...
function DivineNuoYaSelectView:bindEvents()
	DivineNuoYaSelectView.super.bindEvents(self)
...
function DivineNuoYaSelectView:unbindEvents()
	DivineNuoYaSelectView.super.unbindEvents(self)
...
function DivineNuoYaSelectView:onEnter()
	DivineNuoYaSelectView.super.onEnter(self)
	GlobalDispatcher:addListener(GlobalNotify.DivineNuoYaClgSelectPetsRes, self._onSelectPetsRes, self)

	local params = self:getOpenParam()

	self._activityId = params[1]
	self._stageId = params[2]
	self._viewType = params[3]
	...
	if cfgStage then
		self._maxNum = cfgStage.selectPetCount or 0
	end

	if self._viewType == DivineNuoYaChallengeModel.SelectPet then
	...
	elseif self._viewType == DivineNuoYaChallengeModel.LookOverPet then
...
function DivineNuoYaSelectView:_updateGameObject(isShow)
...
function DivineNuoYaSelectView:onExit()
	DivineNuoYaSelectView.super.onExit(self)
	GlobalDispatcher:removeListener(GlobalNotify.DivineNuoYaClgSelectPetsRes, self._onSelectPetsRes, self)

	self._inputSearch.input.text = ""
end

function DivineNuoYaSelectView:_onSelectPetsRes()
	local activityId = self._activityId

	self:close()
	UIStateManager.instance:push(ViewName.DivineNuoYaExtremeClgView, activityId, self._stageId)
end

function DivineNuoYaSelectView:_onClickSure()
	...
	if count > 0 then
		...
		local function okFunc()
			...
			DivineNuoYaChallengeAgent.instance:sendPM_DivineNuoYaClgSelectPetsReq(self._activityId, petIds)
		end
...
function DivineNuoYaSelectView:_OnSearchTextChanged()
...
function DivineNuoYaSelectView:onClearSearchClick()
	self._inputSearch.input.text = ""
end

function DivineNuoYaSelectView:_getPosAndSizeDelta(go)
...
function DivineNuoYaSelectView:_onClickFilter()
...
function DivineNuoYaSelectView:_onClickSort()
...
function DivineNuoYaSelectView:SetNextSort(idx)
	...
	GameUtil.saveUserData(DivineNuoYaChallengeModel.SortKey, self._sortValue)
end

function DivineNuoYaSelectView:GetSortState()
	if self._sortValue == nil then
		self._sortValue = GameUtil.getUserData(DivineNuoYaChallengeModel.SortKey)
...
function DivineNuoYaSelectView:GetSortParms(checkIsInTeamFunc)
	local parm1 = {}
	local parm2 = {}
	local sortName
...
function DivineNuoYaSelectView:onRefreshPetSort(idx, onResetCallback)
...
function DivineNuoYaSelectView:updateFilterData(starList, jobList, rareList)
	self:_RefreshPetList()
end

function DivineNuoYaSelectView:_RefreshPetList()
	if self._viewType == DivineNuoYaChallengeModel.SelectPet then
		self._curViewDatas = {}
...
function DivineNuoYaSelectView:onFilter(pet)
...
function DivineNuoYaSelectView:_isMatch(cfg, rare, skinId)
	return PetFilterMgr.instance:checkIsMatch(PetFilterMgr.TYPE_PETVIEW, cfg, rare, skinId)
end

function DivineNuoYaSelectView:_updateCell(view, cell, data)
	...
	elseif self._viewType == DivineNuoYaChallengeModel.LookOverPet then
		...
		local value = data.leftHp == -1 and 100 or Mathf.Clamp01(data.leftHp / data.petMo._maxHp) * 100
...
function DivineNuoYaSelectView:_clearTableview(cell)
	local petIcon = goutil.findChild(cell, "petIcon")

	MaterialMgr.resetAll(petIcon)
end

function DivineNuoYaSelectView:_getPetIdsNum()
	local count = 0
...
function DivineNuoYaSelectView:_updateSelectNum()
	self._txtNum.text = string.format("最多可选择：%d/%d", self:_getPetIdsNum(), self._maxNum)

	self:_updateBtnSureState()
end

function DivineNuoYaSelectView:_updateLookOverPetList()
...
function DivineNuoYaSelectView:_updateBtnSureState()
	local count = self:_getPetIdsNum()

	self._txtSure.text = count == 0 and "一键选择" or "确 定"
end

function DivineNuoYaSelectView:_oneKeySelectPets()
