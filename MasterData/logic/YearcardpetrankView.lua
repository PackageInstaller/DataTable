-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/yearcard/view/yearcard2025/main/YearcardpetrankView.lua

module("logic.extensions.yearcard.view.yearcard2025.main.YearcardYearcardpetrankView", package.seeall)

local YearcardpetrankView = class("YearcardpetrankView", ViewComponent)
local TitlePosPlan = {
...
function YearcardpetrankView:_getPath()
	return {
		cellPath = "pagecell",
...
function YearcardpetrankView:_cellSize(view, index)
	return 132, 64
end

function YearcardpetrankView:ctor()
	YearcardpetrankView.super.ctor(self)

	self._filterType = PetFilterMgr.TYPE_PET_RANK
	self._cacheParams = {}
end

function YearcardpetrankView:unbindEvents()
	YearcardpetrankView.super.unbindEvents(self)
...
function YearcardpetrankView:bindEvents()
	YearcardpetrankView.super.bindEvents(self)
...
function YearcardpetrankView:buildUI()
	YearcardpetrankView.super.buildUI(self)
...
function YearcardpetrankView:onExit()
	YearcardpetrankView.super.onExit(self)
...
function YearcardpetrankView:onEnter()
	YearcardpetrankView.super.onEnter(self)

	self._bClickClose = false

	local params = self:getOpenParam()

	self._raceId = checknumber(params[2])
	self._bOnlyOnePet = checkbool(params[3])
...
function YearcardpetrankView:_playBgEffect()
	self:_removeBgEffect()
...
function YearcardpetrankView:_removeBgEffect()
...
function YearcardpetrankView:_invalidateConf()
...
function YearcardpetrankView:_tryInitSelectTab()
	local params = self:getOpenParam()

	self._tabId = MathUtil.clamp(checknumber(params[1]), 1, #self._tabConfs)
...
function YearcardpetrankView:_tryInitSelectPet()
	self:_onClickPet()
	self._petScrollList:reloadData(self._pets)
end

function YearcardpetrankView:_updateByFilterData()
...
function YearcardpetrankView:_updateFilterData()
	self:_updateByFilterData()
	self:_onClickPet()
end

function YearcardpetrankView:_queryRank()
...
function YearcardpetrankView:_onClickTabCell(data)
...
function YearcardpetrankView:_updatePetCell(view, cell, data, tag)
...
function YearcardpetrankView:_clearPetCell(cell)
	local petContainer = goutil.findChild(cell, "petContainer")
	local redPoint = goutil.findChild(cell, "dot")
...
function YearcardpetrankView:_updateRankCell(view, cell, data, tag)
	local tabConf = self._tabConfs[self._tabId]
...
function YearcardpetrankView:onHeadClick(userId, imgIcon)
	FriendController.instance:showInfoView(userId, imgIcon)
end

function YearcardpetrankView:_clearRankCell(cell)
	local imgIcon = goutil.findChild(cell, "head")

	HeadItemController.instance:resetHeadCell(imgIcon)
end

function YearcardpetrankView:_updateRank()
	local tabConf = self._tabConfs[self._tabId]
...
function YearcardpetrankView:_updateImpressNum()
	local tabConf = self._tabConfs[self._tabId]
	...
	if showPrizeConf and showPrizeConf.isImpressionSticker then
		...
		rankSize = math.min(rankSize, totalNum)
		self._txtImpressNum.text = rankSize .. "/" .. totalNum
	end
end

function YearcardpetrankView:_updateRankPrizeState()
	local tabConf = self._tabConfs[self._tabId]
	local _, myRank, bHasGainPrize, rankSize = PetRankModel.instance:getRankData(tabConf.rankType)
	local rankConf = HandbookConfig.instance:getPetRankConfByRaceId(self._raceId) or {}
	...
	bCanGainPrize = showPrizeConf and not bHasGainPrize and myRank > 0 and myRank >= showPrizeConf.rankRange[1] and myRank <= showPrizeConf.rankRange[2] or not bHasGainPrize and myRank > 0
	...
	if showImpressionStickerConf then
		goutil.setActive(self._canGet2, isOpen and bCanGainPrize and not bHasGainPrize)
	...
	goutil.setActive(self._receive, isOpen and bHasGainPrize)

	if isOpen then
		local prize = string.split(showPrizeConf.prize, "#")[1]

		MaterialMgr.setCellByCfg((myRank <= 0 and showPrizeConf.hideIfNotReach or nil) and HandbookConfig.instance:getPetRankClientCommon("default_reward_icon"), self._rewardItem)
...
function YearcardpetrankView:_updateRankEmpty()
...
function YearcardpetrankView:_updateMyRankTips()
	local tabConf = self._tabConfs[self._tabId]
	local tip = ""
	local statTime
	...
	goutil.setActive(self._btnGainPetGo, false)
	goutil.setActive(self._btnCultivateGo, false)
	...
	if not string.nilorempty(statTime) then
		...
		if startTimestamp > ServerTime.now() then
			...
			self._txtPowerBtnTips.text = string.format("%d.%d %d点开放", startDate.month, startDate.day, startDate.hour)
...
function YearcardpetrankView:_playEffect()
	local tabConf = self._tabConfs[self._tabId]
	local _, myRank, bHasGainPrize, rankSize = PetRankModel.instance:getRankData(tabConf.rankType)
	local rankConf = HandbookConfig.instance:getPetRankConfByRaceId(self._raceId) or {}
	local showPrizeConf = HandbookConfig.instance:getPetRankPrizeConf(rankConf.id, tabConf.rankType, myRank, rankSize)
...
function YearcardpetrankView:_clearEffect()
...
function YearcardpetrankView:_onClickPet(data)
	local tabConf = self._tabConfs[self._tabId]
...
function YearcardpetrankView:_onclickRule()
	UIStateManager.instance:open(ViewName.RulesView, "petrank_rule")
end

function YearcardpetrankView:_onClickCultivate()
	if self._raceId then
		local conf = HandbookConfig.instance:getPetRankConfByRaceId(self._raceId)
...
function YearcardpetrankView:_onClickGainPet()
	if self._raceId > 0 then
		MaterialMgr.openGetSource(MatType.PetPiece, self._raceId)
	end
end

function YearcardpetrankView:_onClickPowerCheck()
	local statTime
...
function YearcardpetrankView:_onClickFliter()
...
function YearcardpetrankView:_getPosAndSizeDelta(go)
...
function YearcardpetrankView:_onClickTip()
	local tabConf = self._tabConfs[self._tabId]
...
function YearcardpetrankView:_onClickReward()
	local tabConf = self._tabConfs[self._tabId]
	local _, myRank, bHasGainPrize, rankSize = PetRankModel.instance:getRankData(tabConf.rankType)
	local rankConf = HandbookConfig.instance:getPetRankConfByRaceId(self._raceId) or {}
	local showPrizeConf = HandbookConfig.instance:getPetRankPrizeConf(rankConf.id, tabConf.rankType, myRank, rankSize)
...
function YearcardpetrankView:_getTip()
	local tabConf = self._tabConfs[self._tabId]
	local _, myRank, bHasGainPrize, rankSize = PetRankModel.instance:getRankData(tabConf.rankType)
	local rankConf = HandbookConfig.instance:getPetRankConfByRaceId(self._raceId) or {}
	local showImpressionStickerConf = HandbookConfig.instance:getPetRankPrizeConf(rankConf.id, tabConf.rankType, myRank, rankSize, PetRankModel.RANK_PRIZE_IMPRESSION_STICKER_ID)
...
function YearcardpetrankView:_cache()
	self._cacheParams = {
		self._tabId,
		self._raceId,
		self._bOnlyOnePet
