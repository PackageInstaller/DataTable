-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/passport/view/guardcity/PassportGuardCityBuyView.lua

module("logic.extensions.passport.view.guardcity.PassportGuardCityBuyView", package.seeall)

local PassportGuardCityBuyView = class("PassportGuardCityBuyView", ViewComponent)

function PassportGuardCityBuyView:ctor()
	PassportGuardCityBuyView.super.ctor(self)
end

function PassportGuardCityBuyView:bindEvents()
	PassportGuardCityBuyView.super.bindEvents(self)
...
function PassportGuardCityBuyView:unbindEvents()
	PassportGuardCityBuyView.super.unbindEvents(self)
...
function PassportGuardCityBuyView:buildUI()
	PassportGuardCityBuyView.super.buildUI(self)
	...
	self._txtPrivilege = self:getTxt("BG/prizePrivilege/txtPrivilege")
	self._imgPrivilege = self:getGo("BG/prizePrivilege/imgPrivilege")
end

function PassportGuardCityBuyView:onEnter()
	PassportGuardCityBuyView.super.onEnter(self)
	...
	self._txtPrivilege.text = langPara("解锁驭灵战令后，每天额外增加%d次免费恢复疲劳次数。", GuardCityConfig.instance:getCommonValue("PRIVILEGE_FREE_TIMES", true))
...
function PassportGuardCityBuyView:onExit()
	PassportGuardCityBuyView.super.onExit(self)
	MaterialMgr.resetAll(self._imgPrivilege)
	self._tableviewNow:dispose()
end

function PassportGuardCityBuyView:_refreshTable()
	local dataListNow
...
function PassportGuardCityBuyView:_clearCell(cell)
	local point = goutil.findChild(cell, "point")

	MaterialMgr.resetAll(point)
end

function PassportGuardCityBuyView:_updateCell(view, cell, data)
	local point = goutil.findChild(cell, "point")
...
function PassportGuardCityBuyView:_onClickBtnBuy()
...
function PassportGuardCityBuyView:_onClickBtnGet()
...
return PassportGuardCityBuyView
