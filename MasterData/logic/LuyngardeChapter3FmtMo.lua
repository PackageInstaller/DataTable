-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/luyngarde/model/LuyngardeChapter3FmtMo.lua

module("logic.extensions.luyngarde.model.LuyngardeChapter3FmtMo", package.seeall)

local LuyngardeChapter3FmtMo = class("LuyngardeChapter3FmtMo", BaseCustomFmtMo)

function LuyngardeChapter3FmtMo:initParams(activityId, creepsMasterId, posId)
...
function LuyngardeChapter3FmtMo:getMonsterConfigList()
	return self.masterCfg
end

function LuyngardeChapter3FmtMo:getFmtInfoConfig()
	return self.creeps
end

function LuyngardeChapter3FmtMo:initFightHandler()
	self:setFightHandler(function()
		UIJumper.instance:saveCurStack(ViewName.CustomMissionView, ViewName.CustomFormationView)
		LuyngardeController.instance:chapter3SelfClg(self.activityId, self.curPosId, self:getCurSimpleForm())
	end, nil)
end

function LuyngardeChapter3FmtMo:updateData()
	self:tryInitPetList()
...
function LuyngardeChapter3FmtMo:_addSupportPet(petId)
...
function LuyngardeChapter3FmtMo:initPetList()
...
function LuyngardeChapter3FmtMo:refreshPetList(changePetMoMap)
	for petId, petMo in pairs(changePetMoMap) do
		self:updatePetMo(petMo)
	end
end

function LuyngardeChapter3FmtMo:checkFormationEmptyAndPosNumLimit(posLimitCallback, hasOneKey, onekeyCallBack)
	if self:_checkSupPetId() then
		return LuyngardeChapter3FmtMo.super.checkFormationEmptyAndPosNumLimit(self, posLimitCallback, hasOneKey, onekeyCallBack)
	else
...
function LuyngardeChapter3FmtMo:onFormationChangeFinish()
	self:setValidatorDescYesOrNo(self:_checkSupPetId())
end

function LuyngardeChapter3FmtMo:_checkSupPetId()
