-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/fantian/view/FantianlevelselectView.lua

module("logic.extensions.fantian.view.FantianlevelselectView", package.seeall)

local FantianlevelselectView = class("FantianlevelselectView", ViewComponent)

function FantianlevelselectView:ctor()
	FantianlevelselectView.super.ctor(self)
end

function FantianlevelselectView:unbindEvents()
	FantianlevelselectView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
	GameUtil.rmClickHandler(self._btnChallenge)
	GameUtil.rmClickHandler(self._btnTip)
end

function FantianlevelselectView:bindEvents()
	FantianlevelselectView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self.close, self)
	GameUtil.addClickHandler(self._btnChallenge, self._onClickChallenge, self)
	GameUtil.addClickHandler(self._btnTip, self._onClickBtnTip, self)
end

function FantianlevelselectView:buildUI()
	FantianlevelselectView.super.buildUI(self)
...
function FantianlevelselectView:onExit()
	FantianlevelselectView.super.onExit(self)
...
function FantianlevelselectView:onEnter()
	FantianlevelselectView.super.onEnter(self)
...
function FantianlevelselectView:_refreshUI()
...
function FantianlevelselectView:_upateStage(info)
	self._stageList = {}
	...
	for i = 1, 3 do
		...
		if not firstStageCfg or GameUtil.getTimePeriod(firstStageCfg.openTime, "") == GameUtil.inTimePeriod then
			...
			txtLock.text = string.format("%d.%d 开启", dateTime.month, dateTime.day)
...
function FantianlevelselectView:_onClickStage(index)
...
function FantianlevelselectView:_onClickLevel(index)
	...
	for i = 1, 3 do
		local cell = self:getGo("info/cellCon/cell_" .. i)
		...
		if i <= #list then
			...
			if hasGet then
				txtDesc.text = crCfg.dragonSoulDesc .. "<color=#20b376>（已获得）</color>"
			else
				isFinish = false
				txtDesc.text = crCfg.dragonSoulDesc .. "<color=#eb4642>（未获得）</color>"
			end
		end
	end
...
function FantianlevelselectView:_onClickBtnTip()
	TipsFacade.instance:openRulesView(self._actCfg.ruleKey)
end

function FantianlevelselectView:_onClickChallenge()
	local info = FanTianChallengeModel.instance:getInfo(self._activityId) or {}
	local curTierId = checknumber(info.curTierId)
...
function FantianlevelselectView:getItemDesc()
	return "龙魂"
end

function FantianlevelselectView:getResultViewName()
	return ViewName.FantianchallengeresultView
end

return FantianlevelselectView
