-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/anmoyangame/view/AnMoYanGameView.lua

module("logic.extensions.anmoyangame.view.AnMoYanGameView", package.seeall)

local AnMoYanGameView = class("AnMoYanGameView", ViewComponent)
...
function AnMoYanGameView:ctor()
	AnMoYanGameView.super.ctor(self)
end

function AnMoYanGameView:buildUI()
	AnMoYanGameView.super.buildUI(self)
...
function AnMoYanGameView:bindEvents()
	AnMoYanGameView.super.bindEvents(self)
	self._btnClose:AddClickListener(self._clickClose, self)
	self._bg:AddClickListener(self._clickBg, self)
end

function AnMoYanGameView:unbindEvents()
	AnMoYanGameView.super.unbindEvents(self)
	self._btnClose:RemoveClickListener()
	self._bg:RemoveClickListener()
end

function AnMoYanGameView:destroyUI()
	AnMoYanGameView.super.destroyUI(self)
end

function AnMoYanGameView:onEnter()
	AnMoYanGameView.super.onEnter(self)
	self.addGEvent(self, GlobalNotify.PM_DarkMoYanGameEndGameRes, self._getEndGameRes, self)
	...
	local prizeList = {}
	local prizeCfg = AnMoYanConfig.instance:getDropPieceCfg()

	for k, v in ipairs(prizeCfg) do
		local url = GameUrl.getIconFolderUrl("game/anmoyan", v.iconName)
	...
	self._params.prizeList = prizeList

	local gameId = AnMoYanConfig.instance:getGamePlanId(self._activityId)

	self._params.gameCfg = AnMoYanConfig.instance:getGameCfg(gameId)
	self._params.dropPlanCfg = AnMoYanConfig.instance:getDropPlanCfg()

	local planId = AnMoYanConfig.instance:getBuyTimePlanId(self._activityId)

	self._params.rebornCfg = AnMoYanConfig.instance:getDropBuyTimesCfgsByType(planId, 2)
	self._params.rebornCallBack = nil
	self._params.finishCallBack = nil

	GlobalDispatcher:addListener(GlobalNotify.PM_DarkMoYanGameStartGameRes, self._openStartMask, self)
	self:_prepareData()
...
function AnMoYanGameView:onEnterFinished()
	AnMoYanGameView.super.onEnterFinished(self)
end

function AnMoYanGameView:onExit()
	AnMoYanGameView.super.onExit(self)
	removetimer(self._updateFrame, self)
	removetimer(self._gameOver, self)
	removetimer(self._gameRebornEffectEnd, self)
	GlobalDispatcher:removeListener(GlobalNotify.PM_DarkMoYanGameStartGameRes, self._openStartMask, self)
...
function AnMoYanGameView:onExitFinished()
	AnMoYanGameView.super.onExitFinished(self)
end

function AnMoYanGameView:_openStartMask()
	UIStateManager.instance:open(ViewName.SuppressGameStartMask, self._start, self)
end

function AnMoYanGameView:_showRiptiteEffect()
	local path = "20220701/taolidiyu/fx_ui_taolidiyu.prefab"
...
function AnMoYanGameView:_prepareData()
...
function AnMoYanGameView:_refresh()
	self:_refreshCells()
	self._scrollList:reloadData(self._prizeList)
end

function AnMoYanGameView:_refreshHeart()
	for i = 1, 3 do
		goutil.setActive(self._heart[i].go, i <= self._heartNum)
	end
end

function AnMoYanGameView:_refreshPrize()
	self._scrollList:refresh(self._prizeList)
end

function AnMoYanGameView:_updatePieceCell(view, cell, data)
...
function AnMoYanGameView:_showJiaYiEffect(cell)
...
function AnMoYanGameView:_clearPieceCell(cell)
	local icon = goutil.findChild(cell, "icon")
...
function AnMoYanGameView:_refreshCells()
...
function AnMoYanGameView:_clickBg()
...
function AnMoYanGameView:_clickClose()
...
function AnMoYanGameView:_addCell(index)
...
function AnMoYanGameView:_clearCell(index)
...
function AnMoYanGameView:_start()
...
function AnMoYanGameView:_updateFrame()
...
function AnMoYanGameView:_resetScroller()
	GameUtil.setLocalPos(self._scroller, 0, 0, 0)
end

function AnMoYanGameView:_resetPlayer()
	self._player.pos = {
		x = START_POS_X,
		y = START_POS_Y
	}
...
function AnMoYanGameView:_refreshCurCfg()
...
function AnMoYanGameView:_getGameCfg(index)
	local gameCfg
...
function AnMoYanGameView:_collide()
	self:_suspend()
...
function AnMoYanGameView:_showHeartEffect()
...
function AnMoYanGameView:_collideEffectEnd()
	if self._colorTween then
		self._colorTween:Kill(false)
...
function AnMoYanGameView:_gameOver()
	removetimer(self._gameOver, self)

	self._state = GAME_STATE_END
	self._dropNum = 0

	for _, v in pairs(self._prizeList) do
		self._dropNum = self._dropNum + v.num
	end

	AnMoYanController.instance:sendPM_DarkMoYanGameEndGameReq(self._activityId, self._dropNum)
end

function AnMoYanGameView:_getEndGameRes()
	ViewMgr.instance:open(ViewName.AnMoYanGameResultView, self._dropNum, "本局积分", ViewName.AnMoYanGameView, ViewName.AnMoYanMainView)
end

function AnMoYanGameView:_gameOverTip()
	removetimer(self._updateFrame, self)
	settimer(1.6, self._gameOver, self, false)
	FloatWordMgr.instance:show("生命耗尽，游戏结束！")
end

function AnMoYanGameView:_suspend()
	removetimer(self._updateFrame, self)
end

function AnMoYanGameView:_continue()
	self:_updateFrame()
	settimer(GAME_UPDATETIME, self._updateFrame, self, true)
end

function AnMoYanGameView:_gameReborn()
...
function AnMoYanGameView:_gameRebornEffectEnd()
...
function AnMoYanGameView:_jump()
...
function AnMoYanGameView:_jumpSuccess(comp)
	local dropId = comp:getDropId()
	local nextPos = comp:getPos()
...
function AnMoYanGameView:_checkVirusCollision()
	local nextCell = self._cellList[self._index + 1]
