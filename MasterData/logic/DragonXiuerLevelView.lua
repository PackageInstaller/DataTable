-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/dragonxiuer/view/DragonXiuerLevelView.lua

module("logic.extensions.dragonxiuer.view.DragonXiuerLevelView", package.seeall)

local DragonXiuerLevelView = class("DragonXiuerLevelView", ViewComponent)

DragonXiuerLevelView.ConditionDesc = {
	ActiveCount = lang("存活精灵"),
	CircleCount = lang("击败回合数"),
	RoundCount = lang("出手次数")
}

function DragonXiuerLevelView:ctor()
	DragonXiuerLevelView.super.ctor(self)
end

function DragonXiuerLevelView:unbindEvents()
	DragonXiuerLevelView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
end

function DragonXiuerLevelView:bindEvents()
	DragonXiuerLevelView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self.close, self)
end

function DragonXiuerLevelView:buildUI()
	DragonXiuerLevelView.super.buildUI(self)
...
function DragonXiuerLevelView:onExit()
	DragonXiuerLevelView.super.onExit(self)
	self._groupTableList:dispose()
	self._listTableList:dispose()
end

function DragonXiuerLevelView:onEnter()
	DragonXiuerLevelView.super.onEnter(self)
...
function DragonXiuerLevelView:_refreshView()
...
function DragonXiuerLevelView:_updateGroupCell(view, cell, data, tag)
	...
	local selectChange = goutil.findChildComponent(go, "imgBg", "UIChangeGroup")
	...
	if data.stageId == self._selectGroup then
		selectChange:SetState(0)
	else
		selectChange:SetState(1)
	end
end

function DragonXiuerLevelView:_clearGroupCell(cell)
	local go = cell.gameObject
	local btnClick = goutil.findChild(go, "btnSelect")
	local prizeCon = goutil.findChild(go, "prize/con")
...
function DragonXiuerLevelView:_updateLevelCell(view, cell, data, tag)
	local go = cell.gameObject
	...
	txtCondition.text = langPara("<color=#FFFFFFFF>%s:</color>%d", DragonXiuerLevelView.ConditionDesc[groupCfg[self._selectGroup].scoreType], score)
...
function DragonXiuerLevelView:_clearLevelCell(cell)
	local go = cell.gameObject
	local con = goutil.findChild(go, "petCon")
	local btnChallenge = goutil.findChild(go, "clickArea")
	local btnReset = goutil.findChild(go, "btnReset")
...
function DragonXiuerLevelView:_onClickGroup(groupId)
	self._selectGroup = groupId

	self:_refreshView()
end

function DragonXiuerLevelView:_onClickLevel(levelId)
	local passStageId = DragonXiuerModel.instance:getPassStageId(self._actId)
...
function DragonXiuerLevelView:_onClickReset(levelId)
	local content = lang("是否确认重置该关卡的挑战状态")
