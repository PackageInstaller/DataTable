-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/yuhuichallenge/view/copy/NewerafantianchallengelevelView.lua

module("logic.extensions.yuhuichallenge.view.copy.NewerafantianchallengelevelView", package.seeall)

local NewerafantianchallengelevelView = class("NewerafantianchallengelevelView", ViewComponent)

function NewerafantianchallengelevelView:buildUI()
	NewerafantianchallengelevelView.super.buildUI(self)
...
function NewerafantianchallengelevelView:bindEvents()
	NewerafantianchallengelevelView.super.bindEvents(self)
...
function NewerafantianchallengelevelView:unbindEvents()
	NewerafantianchallengelevelView.super.unbindEvents(self)
...
function NewerafantianchallengelevelView:onEnter()
	NewerafantianchallengelevelView.super.onEnter(self)
...
function NewerafantianchallengelevelView:onExit()
	NewerafantianchallengelevelView.super.onExit(self)
...
function NewerafantianchallengelevelView:_onClickClose()
	self:close()
end

function NewerafantianchallengelevelView:_onClickTip()
	local actCfg = TimeLimitedConfig.instance:getCfgById(self._curChallengeId)

	UIStateManager.instance:push(ViewName.RulesView, actCfg.ruleKey)
end

function NewerafantianchallengelevelView:_onClickReset()
...
function NewerafantianchallengelevelView:_onClickLeft()
...
function NewerafantianchallengelevelView:_onClickRight()
	if self._isAllPass then
		FloatWordMgr.instance:show(lang("已全部通关"))
...
function NewerafantianchallengelevelView:_onError(status)
	return
end

function NewerafantianchallengelevelView:_refreshViewByCfg()
...
function NewerafantianchallengelevelView:_refreshViewByInfo()
	...
	if #self._stageCfgs < self._curStageId then
		if not #self._stageCfgs then
			...
			local cfg = YuHuiChallengeConfig.instance:getChallengeCfgById(self._curChallengeId)
			local stageCfgs = YuHuiChallengeConfig.instance:getStageCfgs(cfg.stagePlanId)
...
function NewerafantianchallengelevelView:_refreshBalancePart()
...
function NewerafantianchallengelevelView:_refreshLevelPart()
	if #self._stageCfgs < self._curStageId then
		if not #self._stageCfgs then
			local stageId = self._curStageId
...
return NewerafantianchallengelevelView
