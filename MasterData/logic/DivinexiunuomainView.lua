-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/divinexiunuo/view/DivinexiunuomainView.lua

module("logic.extensions.divinexiunuo.view.DivinexiunuomainView", package.seeall)

local DivinexiunuomainView = class("DivinexiunuomainView", ViewComponent)

function DivinexiunuomainView:buildUI()
	DivinexiunuomainView.super.buildUI(self)
...
function DivinexiunuomainView:bindEvents()
	DivinexiunuomainView.super.bindEvents(self)
...
function DivinexiunuomainView:unbindEvents()
	DivinexiunuomainView.super.unbindEvents(self)
...
function DivinexiunuomainView:onExit()
	DivinexiunuomainView.super.onExit(self)
...
function DivinexiunuomainView:onEnter()
	DivinexiunuomainView.super.onEnter(self)
	...
	self._actCfg = DivineXiuNuoConfig.instance:getActivityCfgById(self._activityId)

	self:_setActTime()
	self:_setRole()
	self.addGEvent(self, GlobalNotify.DIVINE_XIUNUO_INFO_UPDATE, self._updateUI, self)
	DivineXiuNuoController.instance:sendGetInfo(self._activityId)
	self:_updateUI()
end

function DivinexiunuomainView:_getSkinId()
	return checknumber(self._actCfg.skinId)
end

function DivinexiunuomainView:_upateRed()
	return
end

function DivinexiunuomainView:_setActTime()
	self._txtTime.text = TimeGateController.instance:getActTimeShow(self._activityId)
end

function DivinexiunuomainView:_setRole()
...
function DivinexiunuomainView:_updateUI()
	self._info = DivineXiuNuoModel.instance:getInfo(self._activityId) or {}

	local curPhaseId = checknumber(self._info.curPhaseId)
	local list = DivineXiuNuoConfig.instance:getPhaseListById(self._activityId) or {}

	GameUtil.SetActive(self._passClg1, curPhaseId > #list)
	self:_updateSuperBubble()
	self:_upateRed()
end

function DivinexiunuomainView:_onClickBtnSkill()
...
function DivinexiunuomainView:_onClickBtnIntroduce()
	PetbookController.instance:openPetinfoView(self:_getRaceId())
end

function DivinexiunuomainView:_getRaceId()
	return PetSkinConfig.instance:getPetSkinRaceId(self:_getSkinId())
end

function DivinexiunuomainView:_onClickBtnJump(key)
...
function DivinexiunuomainView:_onClickBtnClg1()
	local curPhaseId = checknumber(self._info.curPhaseId)
	local list = DivineXiuNuoConfig.instance:getPhaseListById(self._activityId) or {}
	...
	if curPhaseId > #list then
		...
		UIStateManager.instance:push(ViewName.DivinexiunuolevelView, self._activityId)
	end
end

function DivinexiunuomainView:_onClickBtnClg2()
	return
end

function DivinexiunuomainView:_onClickBtnTip()
	TipsFacade.instance:openRulesView("DivinexiunuomainView_rule")
end

function DivinexiunuomainView:_updateSuperBubble()
	MaterialMgr.resetAll(self._itemcellGo)
...
function DivinexiunuomainView:_onClickbtnAttr()
	return
end

return DivinexiunuomainView
