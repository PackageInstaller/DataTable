-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/divineevolveplus/view/DivineEvolvePlusCultivateView.lua

module("logic.extensions.divineevolveplus.view.DivineEvolvePlusCultivateView", package.seeall)

local DivineEvolvePlusCultivateView = class("DivineEvolvePlusCultivateView", ViewComponent)
local SHEN_WEI = "神格"
local CELL_MAX_NUM = 3

function DivineEvolvePlusCultivateView:ctor()
	DivineEvolvePlusCultivateView.super.ctor(self)
end

function DivineEvolvePlusCultivateView:unbindEvents()
	DivineEvolvePlusCultivateView.super.unbindEvents(self)
...
function DivineEvolvePlusCultivateView:bindEvents()
	DivineEvolvePlusCultivateView.super.bindEvents(self)
	...
	self._btnRule:AddClickListener(function()
		UIStateManager.instance:push(ViewName.RulesView, "shenyaoshengerule")
	end, self)
end

function DivineEvolvePlusCultivateView:buildUI()
	DivineEvolvePlusCultivateView.super.buildUI(self)
	...
	self._activated = self:getGo("activeState/activated")
...
function DivineEvolvePlusCultivateView:onExit()
	DivineEvolvePlusCultivateView.super.onExit(self)
...
function DivineEvolvePlusCultivateView:onEnter()
	DivineEvolvePlusCultivateView.super.onEnter(self)
...
function DivineEvolvePlusCultivateView:_refreshAll()
	self:_refreshData()
	self:_refresh()
end

function DivineEvolvePlusCultivateView:_refreshData()
...
function DivineEvolvePlusCultivateView:_refresh()
	...
	local programCfgList = DomainCultivateConfig.instance:getProgramCfgList(self._levelCfg[self._selectId].id, self._levelCfg[self._selectId].level)
	local isBuffCountEqualOne = #programCfgList == 1
	local consume
	local info = self._kamuiInfos[self._selectId]
	local isExistInfo = info ~= nil

	goutil.setActive(self._fazhen, isExistInfo or isBuffCountEqualOne)
	goutil.setActive(self._sleepState, not isExistInfo and not isBuffCountEqualOne)
	self._btnReset.gameObject:SetActive(isExistInfo and not isBuffCountEqualOne)
	self._btnActive.gameObject:SetActive(not isExistInfo)
	self._activated.gameObject:SetActive(isExistInfo)

	if isExistInfo then
		consume = self._levelCfg[self._selectId].resetCost

		local id = info.kamuiId
		...
		goutil.setActive(self._fazhen, true)
		goutil.setActive(self._sleepState, false)
	else
		...
		if self._selectId > #self._kamuiInfos + 1 then
			...
			goutil.setActive(self._matCost, false)
			goutil.setActive(self._fazhen, false)
			goutil.setActive(self._sleepState, true)
		else
			if isBuffCountEqualOne then
				local id = programCfgList[1].kamuiId
				local kamuiCfg = DomainCultivateConfig.instance:getDefineCfg(id)
			...
			goutil.setActive(self._fazhen, isBuffCountEqualOne)
			goutil.setActive(self._sleepState, not isBuffCountEqualOne)
...
function DivineEvolvePlusCultivateView:_reset()
	DomainCultivateController.instance:resetKamui(self._petId, self._selectId)
end

function DivineEvolvePlusCultivateView:_active()
	if BagModel.instance:getPet(self._petId) then
		DomainCultivateController.instance:activeKamuiReq(self._petId, self._selectId)
	else
		FloatWordMgr.instance:show(lang("非背包精灵"))
	end
end

function DivineEvolvePlusCultivateView:_buffView()
	UIStateManager.instance:open(ViewName.DomainCultivateDetailView, self._petId)
end

function DivineEvolvePlusCultivateView:_onClickSkill(i)
	self._selectId = i

	self:_refresh()
end

function DivineEvolvePlusCultivateView:_setCell(cell, i)
...
function DivineEvolvePlusCultivateView:_clearCell(cell)
	uGuiUtil.clearImage(cell.img)
	cell.btn:RemoveClickListener()
	goutil.destroy(cell.go)
end

function DivineEvolvePlusCultivateView:_activeRes()
	self:_refreshAll()
	...
	self._rightEff:setParent(self._rightEffect.transform)
	self._rightEff:setLocalPos(0, 0, 0)
	self._rightEff:setLocalEulerAngle(0, 0, 0)
	self._rightEff:setScale(1)

	self._rightEff.hideEffWhileNotOnTop = true
end

return DivineEvolvePlusCultivateView
