-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/gm/view/msgdebugger/MsgDebuggerParamMO.lua
...
function MsgDebuggerParamMO:isNilParam()
	local var_5_0 = self.parentParamMO

	if self.parentParamMO then
		var_5_0 = self.parentParamMO:isRepeated()
		var_5_0 = var_5_0 and (not self.parentParamMO.msgValue or self.parentParamMO.msgValue[self.id] == nil)
	end
...
function MsgDebuggerParamMO:modifyValue(value)
	...
	elseif self.paramTypeStr == MsgDebuggerConfig.ParamType.bool then
		self.msgValue = string.lower(value) == "true" or value == "1"
	else
		printError("暂时不支持的类型")
	end
...
function MsgDebuggerParamMO:setDefaultValue()
	...
	self.parentParamMO.msgValue = self.parentParamMO.msgValue or self.parentParamMO.parentParamMO.msgValue[self.parentParamMO.paramName]
