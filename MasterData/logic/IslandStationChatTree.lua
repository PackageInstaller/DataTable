-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/islandstation/data/IslandStationChatTree.lua

module("logic.extensions.islandstation.data.IslandStationChatTree", package.seeall)

local IslandStationChatTree = class("IslandStationChatTree")

function IslandStationChatTree:ctor(dialoguePlanId, dialogueId)
	self._root = nil
	self._lookUp = {}

	local cfg = IslandStationConfig.instance:getDialogueCfg(dialoguePlanId, dialogueId)
...
function IslandStationChatTree:dispose()
...
function IslandStationChatTree:findChatNode(chatId)
	return self._lookUp[chatId]
end

function IslandStationChatTree:getChatIdListFromRoot(chatId)
	local list = {}
...
function IslandStationChatTree:getChatIdListByAnswer(answerIds)
	local list = {}
...
function IslandStationChatTree:printChatTree()
	local output = {}
