-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/commontabrank/view/ultimatewar/UltimateWarTowerStageRankChildView.lua

module("logic.extensions.commontabrank.view.ultimatewar.UltimateWarTowerStageRankChildView", package.seeall)

local UltimateWarTowerStageRankChildView = class("UltimateWarTowerStageRankChildView", ViewComponent)

function UltimateWarTowerStageRankChildView:buildUI()
	UltimateWarTowerStageRankChildView.super.buildUI(self)
...
function UltimateWarTowerStageRankChildView:bindEvents()
	UltimateWarTowerStageRankChildView.super.bindEvents(self)
end

function UltimateWarTowerStageRankChildView:unbindEvents()
	UltimateWarTowerStageRankChildView.super.unbindEvents(self)
end

function UltimateWarTowerStageRankChildView:onEnter()
	UltimateWarTowerStageRankChildView.super.onEnter(self)

	local params = self:getOpenParam() or {}

	self._activityId = checknumber(params[1])
	self._rankType = UltimateWarEnum.RankType_TowerStage
...
function UltimateWarTowerStageRankChildView:onExit()
	UltimateWarTowerStageRankChildView.super.onExit(self)
	self._rankScrollList:dispose()
	self._myPrizeScrollList:dispose()
end

function UltimateWarTowerStageRankChildView:_handleMsgRes(msg)
...
function UltimateWarTowerStageRankChildView:_getPrizesByRank(rank)
	local data = UltimateWarConfig.instance:getSecondStepTowerRankPrizeData(self._activityId, rank)
...
function UltimateWarTowerStageRankChildView:_onUpdate()
...
function UltimateWarTowerStageRankChildView:_updateRankCell(view, cell, info, tag)
...
function UltimateWarTowerStageRankChildView:_clearRankCell(cell)
	local mainGo = cell.gameObject
	local headRoot = goutil.findChild(mainGo, "headRoot")
...
function UltimateWarTowerStageRankChildView:_updateItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function UltimateWarTowerStageRankChildView:_clearItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

function UltimateWarTowerStageRankChildView:_updateMyPrizeItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function UltimateWarTowerStageRankChildView:_clearMyPrizeItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

return UltimateWarTowerStageRankChildView
