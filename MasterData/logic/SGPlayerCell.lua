-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/anniversarycake/view/SGPlayerCell.lua

module("logic.extensions.anniversarycake.view.SGPlayerCell", package.seeall)

local SGPlayerCell = class("SGPlayerCell")

function SGPlayerCell:ctor(luaComponentContainer)
...
function SGPlayerCell:getGo()
	return self.mainGO
end

function SGPlayerCell:init(data, view, rectTrans, cakeMo)
	self._view = view
	...
	proxy:setCallBack(function()
		if self._cakeMo and self._cakeMo:isPlayerRewardCanGet(data.prizeId, data.needScore) then
			self._cakeMo:sendGainPersonalPrizeReq(MakeCakeModel.instance:getActivityId(), data.prizeId)
...
function SGPlayerCell:update()
	local isGeted = self._cakeMo:isPlayerRewardGeted(self._data.prizeId)
	local isCanGet = self._cakeMo:isPlayerRewardCanGet(self._data.prizeId, self._data.needScore)
...
function SGPlayerCell:clear()
	MaterialMgr.resetAll(self._item)
end

function SGPlayerCell:_playEffect()
	self:_clearEffect()
...
function SGPlayerCell:_clearEffect()
