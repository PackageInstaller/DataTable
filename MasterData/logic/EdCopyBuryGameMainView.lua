-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/eternaldragon/view/bury/EdCopyBuryGameMainView.lua

module("logic.extensions.eternaldragon.view.bury.EdCopyBuryGameMainView", package.seeall)

local EdCopyBuryGameMainView = class("EdCopyBuryGameMainView", ViewComponent)
local rightScorePer, wrongScorePer

function EdCopyBuryGameMainView:unbindEvents()
	EdCopyBuryGameMainView.super.unbindEvents(self)
	self._closeButton:RemoveClickListener()
end

function EdCopyBuryGameMainView:bindEvents()
	EdCopyBuryGameMainView.super.bindEvents(self)
	self._closeButton:AddClickListener(self._onClickClose, self)
end

function EdCopyBuryGameMainView:onExit()
	EdCopyBuryGameMainView.super.onExit(self)
	...
	GlobalDispatcher:removeListener(GlobalNotify.EternalDragonEndGameRes, self._onEndGameRes, self)
...
function EdCopyBuryGameMainView:buildUI()
	EdCopyBuryGameMainView.super.buildUI(self)
...
function EdCopyBuryGameMainView:onEnter()
	EdCopyBuryGameMainView.super.onEnter(self)

	self._activityId = 50003

	local params = self:getOpenParam() or {}

	self._edActivityId = checknumber(params[1])
	...
	GlobalDispatcher:addListener(GlobalNotify.EternalDragonEndGameRes, self._onEndGameRes, self)
...
function EdCopyBuryGameMainView:_onEggFinish(prizeNum)
...
function EdCopyBuryGameMainView:onEnterFinished()
	EdCopyBuryGameMainView.super.onEnterFinished(self)
	...
	if not isFirst then
		UIStateManager.instance:push(ViewName.BuryGameRuleView)
...
function EdCopyBuryGameMainView:_openStartMask()
	UIStateManager.instance:open(ViewName.SuppressGameStartMask, self._startCountDown, self)
end

function EdCopyBuryGameMainView:_onTicking()
...
function EdCopyBuryGameMainView:_removePit(posId)
...
function EdCopyBuryGameMainView:_onBuryPitSuccess(pitType, x, y, z, x1, y1, z1)
...
function EdCopyBuryGameMainView:_updateUI()
	self._txtCombo:SetText(self._comboNum)

	self._txtScore.text = self._curScore
end

function EdCopyBuryGameMainView:_createPit(data)
...
function EdCopyBuryGameMainView:_findFreePit()
...
function EdCopyBuryGameMainView:_findFreePosId()
	local t = {}
...
function EdCopyBuryGameMainView:_insertPitsToQueue()
	self._queue = {}
...
function EdCopyBuryGameMainView:_randomGetNum(weights)
...
function EdCopyBuryGameMainView:_startCountDown()
...
function EdCopyBuryGameMainView:_initGame()
...
function EdCopyBuryGameMainView:_startGameRes()
	self._dataReady = true
...
function EdCopyBuryGameMainView:_startGame()
	self._countDownReady = true
...
function EdCopyBuryGameMainView:_realStartGame()
	settimer(0, self._onTicking, self)
	self:_onTicking()
end

function EdCopyBuryGameMainView:_endGame()
	self._isInGame = false

	removetimer(self._onTicking, self)

	local isToOnEndGame = true

	if self._edActivityId > 0 then
		local data = EternalDragonConfig.instance:getEdActData(self._edActivityId)

		if data then
			::label_24_0::

			local var_24_0 = data.gameValidator

			if data.gameValidator then
				local needScore = data.gameValidator.score

				if not string.nilorempty(needScore) then
					isToOnEndGame = false

					local isPass = self._curScore >= checknumber(needScore)

					EternalDragonController.instance:sendPM_EternalDragonEndGameReq(self._edActivityId, isPass)
				end
			end
		end
	end

	if isToOnEndGame then
		self:_onEndGameRes()
	end
end

function EdCopyBuryGameMainView:_onClickClose()
...
function EdCopyBuryGameMainView:_calcuRightNum()
	local rightNum = 0
	local wrongNum = 0
...
function EdCopyBuryGameMainView:_pauseGame()
...
function EdCopyBuryGameMainView:_resumeGame()
...
function EdCopyBuryGameMainView:_checkEgg()
	if self._egged then
		return
	end
...
function EdCopyBuryGameMainView:_openEggView()
	self:_pauseGame()
...
function EdCopyBuryGameMainView:_onEndGameRes(status)
	self:close()
end

return EdCopyBuryGameMainView
