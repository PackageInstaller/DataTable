-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/suppressgame/view/game/duolamm/CopyDuolaGameView.lua

module("logic.extensions.suppressgame.view.game.duolamm.CopyDuolaGameView", package.seeall)

local CopyDuolaGameView = class("CopyDuolaGameView", ViewComponent)
...
function CopyDuolaGameView:ctor()
	CopyDuolaGameView.super.ctor(self)
end

function CopyDuolaGameView:buildUI()
	CopyDuolaGameView.super.buildUI(self)
...
function CopyDuolaGameView:bindEvents()
	CopyDuolaGameView.super.bindEvents(self)
	self._btnClose:AddClickListener(self._clickClose, self)
	self._bg:AddClickListener(self._clickBg, self)
end

function CopyDuolaGameView:unbindEvents()
	CopyDuolaGameView.super.unbindEvents(self)
	self._btnClose:RemoveClickListener()
	self._bg:RemoveClickListener()
end

function CopyDuolaGameView:destroyUI()
	CopyDuolaGameView.super.destroyUI(self)
end

function CopyDuolaGameView:onEnter()
	CopyDuolaGameView.super.onEnter(self)
	...
	self._params.gameCfg = DuolaMMconfig.instance:getGameCfg(gameId)
	self._params.dropPlanCfg = DuolaMMconfig.instance:getDropPlanCfg()

	local planId = checknumber(DuolaMMconfig.instance:getCommonCfg("GAME_BUY_TIMES_PLAN_ID"))

	self._params.rebornCfg = DuolaMMconfig.instance:getBuyTimesCfg(planId, 2)
	...
	local key = self._viewPresentor.viewName .. "firstRule"
...
function CopyDuolaGameView:onEnterFinished()
	CopyDuolaGameView.super.onEnterFinished(self)
end

function CopyDuolaGameView:onExit()
	CopyDuolaGameView.super.onExit(self)
...
function CopyDuolaGameView:onExitFinished()
	CopyDuolaGameView.super.onExitFinished(self)
end

function CopyDuolaGameView:_openStartMask()
	UIStateManager.instance:open(ViewName.SuppressGameStartMask, self._start, self)
end

function CopyDuolaGameView:_showRiptiteEffect()
...
function CopyDuolaGameView:_prepareData()
...
function CopyDuolaGameView:_refresh()
	self:_refreshCells()
	self._scrollList:reloadData(self._prizeList)
end

function CopyDuolaGameView:_refreshHeart()
	for i = 1, 3 do
		goutil.setActive(self._heart[i].go, i <= self._heartNum)
	end
end

function CopyDuolaGameView:_refreshPrize()
	self._scrollList:refresh(self._prizeList)
end

function CopyDuolaGameView:_updatePieceCell(view, cell, data)
...
function CopyDuolaGameView:_showJiaYiEffect(cell)
...
function CopyDuolaGameView:_clearPieceCell(cell)
	local icon = goutil.findChild(cell, "icon")
...
function CopyDuolaGameView:_refreshCells()
...
function CopyDuolaGameView:_clickBg()
...
function CopyDuolaGameView:_clickClose()
...
function CopyDuolaGameView:_addCell(index)
...
function CopyDuolaGameView:_clearCell(index)
...
function CopyDuolaGameView:_start()
...
function CopyDuolaGameView:_updateFrame()
...
function CopyDuolaGameView:_resetScroller()
	GameUtil.setLocalPos(self._scroller, 0, 0, 0)
end

function CopyDuolaGameView:_resetPlayer()
	self._player.pos = {
		x = START_POS_X,
		y = START_POS_Y
	}
...
function CopyDuolaGameView:_refreshCurCfg()
...
function CopyDuolaGameView:_getGameCfg(index)
	local gameCfg
...
function CopyDuolaGameView:_collide()
	self:_suspend()
...
function CopyDuolaGameView:_showHeartEffect()
...
function CopyDuolaGameView:_collideEffectEnd()
	if self._colorTween then
		self._colorTween:Kill(false)
...
function CopyDuolaGameView:_gameOver()
	removetimer(self._gameOver, self)
	...
	for _, v in pairs(self._prizeList) do
		dropNum = dropNum + v.num
	end

	SuppressGameController.instance:_endGameHandle(dropNum)
end

function CopyDuolaGameView:_gameOverTip()
	removetimer(self._updateFrame, self)
	settimer(1.6, self._gameOver, self, false)
	FloatWordMgr.instance:show("生命耗尽，游戏结束！")
end

function CopyDuolaGameView:_suspend()
	removetimer(self._updateFrame, self)
end

function CopyDuolaGameView:_continue()
	self:_updateFrame()
	settimer(GAME_UPDATETIME, self._updateFrame, self, true)
end

function CopyDuolaGameView:_gameReborn()
...
function CopyDuolaGameView:_gameRebornEffectEnd()
...
function CopyDuolaGameView:_jump()
...
function CopyDuolaGameView:_jumpSuccess(comp)
	local dropId = comp:getDropId()
	local nextPos = comp:getPos()
...
function CopyDuolaGameView:_checkVirusCollision()
	local nextCell = self._cellList[self._index + 1]
