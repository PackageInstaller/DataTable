-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/eternalcity/data/SimpleScrollerViewCtrl.lua

module("logic.extensions.eternalcity.data.SimpleScrollerViewCtrl", package.seeall)

local SimpleScrollerViewCtrl = class("SimpleScrollerViewCtrl")

function SimpleScrollerViewCtrl:ctor()
	return
end

function SimpleScrollerViewCtrl:onInit(scrollRect)
	...
	self:setRaycast(true)
end

function SimpleScrollerViewCtrl:onClear()
	self:stopFollow()
	self:stopViewTo()
...
function SimpleScrollerViewCtrl:onSet(ctX, ctY)
	local dx = 0
...
function SimpleScrollerViewCtrl:setNorPosition(targetPos)
...
function SimpleScrollerViewCtrl:viewTo(pos, startCallBack, finishCallBack)
...
function SimpleScrollerViewCtrl:stopViewTo()
...
function SimpleScrollerViewCtrl:startFollow(targetGo)
	self:setRaycast(false)
...
function SimpleScrollerViewCtrl:stopFollow()
	removetimer(self._tickingFollow, self)

	self._followTargetGo = nil

	self:setRaycast(true)
end

function SimpleScrollerViewCtrl:_tickingFollow()
...
function SimpleScrollerViewCtrl:setRaycast(value)
	if self._viewportRectTran then
		self._viewportRectTran.transform:GetComponent("EmptyRaycast").raycastTarget = value
	end
end

return SimpleScrollerViewCtrl
