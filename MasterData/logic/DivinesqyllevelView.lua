-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/xiayin/view/copy/DivinesqyllevelView.lua

module("logic.extensions.xiayin.view.copy.DivinesqyllevelView", package.seeall)

local DivinesqyllevelView = class("DivinesqyllevelView", ViewComponent)

function DivinesqyllevelView:buildUI()
	DivinesqyllevelView.super.buildUI(self)
...
function DivinesqyllevelView:bindEvents()
	DivinesqyllevelView.super.bindEvents(self)
...
function DivinesqyllevelView:unbindEvents()
	DivinesqyllevelView.super.unbindEvents(self)
...
function DivinesqyllevelView:onEnter()
	DivinesqyllevelView.super.onEnter(self)
...
function DivinesqyllevelView:onExit()
	DivinesqyllevelView.super.onExit(self)
...
function DivinesqyllevelView:_onClickChallenge()
	...
	XiaYinController.instance:openMissionView(self._activityId, self._curTier, creepsMasterId, self._selectLevelIndex, ViewName.DivinesqyltagView)
end

function DivinesqyllevelView:_onClickTab(tierId)
...
function DivinesqyllevelView:_onClickLevel(index)
...
function DivinesqyllevelView:_updateTabs()
...
function DivinesqyllevelView:_initLevels()
...
function DivinesqyllevelView:_updateLevels()
	for i, v in ipairs(self._levelList) do
		GameUtil.SetActive(v.selectGo, self._selectLevelIndex == i)
	end
end

function DivinesqyllevelView:_clearLevels()
	for i, v in ipairs(self._levelList) do
		MaterialMgr.clearIcon(v.node)
	end
end

function DivinesqyllevelView:_initEnemyFmtInfo()
	self._txtName.text = ""
	self._txtPass.text = ""
	self._txtRule.text = ""
...
function DivinesqyllevelView:_updateEnemyFmtInfo(index)
	self:_clearEnemyFmt()
...
function DivinesqyllevelView:_clearEnemyFmt()
