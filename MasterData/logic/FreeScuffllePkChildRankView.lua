-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/commontabrank/view/freescufflle/FreeScuffllePkChildRankView.lua

module("logic.extensions.commontabrank.view.freescufflle.FreeScuffllePkChildRankView", package.seeall)

local FreeScuffllePkChildRankView = class("FreeScuffllePkChildRankView", ViewComponent)

function FreeScuffllePkChildRankView:buildUI()
	FreeScuffllePkChildRankView.super.buildUI(self)
...
function FreeScuffllePkChildRankView:bindEvents()
	FreeScuffllePkChildRankView.super.bindEvents(self)
end

function FreeScuffllePkChildRankView:unbindEvents()
	FreeScuffllePkChildRankView.super.unbindEvents(self)
end

function FreeScuffllePkChildRankView:onEnter()
	FreeScuffllePkChildRankView.super.onEnter(self)
	...
	FreeScuffleController.instance:sendPM_FreeScuffleRankViewReq(self._activityId, FreeScuffleEnum.RankId_Pvp)
end

function FreeScuffllePkChildRankView:onExit()
	FreeScuffllePkChildRankView.super.onExit(self)
	self._rankScrollList:dispose()
	self._myPrizeScrollList:dispose()
end

function FreeScuffllePkChildRankView:_handleMsgRes(msg)
	if msg.rankId ~= FreeScuffleEnum.RankId_Pvp then
		return
	end
...
function FreeScuffllePkChildRankView:_getPrizesByRank(rank)
	local data = FreeScuffleConfig.instance:getPvpScoreRankPrizeData(self._activityId, rank)
...
function FreeScuffllePkChildRankView:_onUpdate()
...
function FreeScuffllePkChildRankView:_updateRankCell(view, cell, info, tag)
...
function FreeScuffllePkChildRankView:_clearRankCell(cell)
	local mainGo = cell.gameObject
	local headRoot = goutil.findChild(mainGo, "headRoot")
...
function FreeScuffllePkChildRankView:_updateItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function FreeScuffllePkChildRankView:_clearItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

function FreeScuffllePkChildRankView:_updateMyPrizeItemCell(view, cell, prizeStr, tag)
	local mainGo = cell.gameObject

	MaterialMgr.setCellByCfg(prizeStr, mainGo)
end

function FreeScuffllePkChildRankView:_clearMyPrizeItemCell(cell)
	local mainGo = cell.gameObject

	MaterialMgr.resetAll(mainGo)
end

return FreeScuffllePkChildRankView
