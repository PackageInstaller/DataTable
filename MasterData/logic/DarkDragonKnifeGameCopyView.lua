-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/darkdragonknife/view/DarkDragonKnifeGameCopyView.lua

module("logic.extensions.darkdragonknife.view.DarkDragonKnifeGameCopyView", package.seeall)

local DarkDragonKnifeGameCopyView = class("DarkDragonKnifeGameCopyView", ViewComponent)
...
function DarkDragonKnifeGameCopyView:ctor()
	DarkDragonKnifeGameCopyView.super.ctor(self)
end

function DarkDragonKnifeGameCopyView:buildUI()
	DarkDragonKnifeGameCopyView.super.buildUI(self)
...
function DarkDragonKnifeGameCopyView:bindEvents()
	DarkDragonKnifeGameCopyView.super.bindEvents(self)
...
function DarkDragonKnifeGameCopyView:unbindEvents()
	DarkDragonKnifeGameCopyView.super.unbindEvents(self)
...
function DarkDragonKnifeGameCopyView:destroyUI()
	DarkDragonKnifeGameCopyView.super.destroyUI(self)
	self._monsterPool:clear()
	self._bulletPool:clear()
end

function DarkDragonKnifeGameCopyView:onEnter()
	DarkDragonKnifeGameCopyView.super.onEnter(self)
	...
	self._scorePlan = checknumber(params[1])

	local propPlanId = checknumber(params[2])
	local gameId = checknumber(params[3])

	self._startCallBack = params[4]
	self._finishCallBack = params[5]
...
function DarkDragonKnifeGameCopyView:_onEffectLoaded(uiEffect)
...
function DarkDragonKnifeGameCopyView:onEnterFinished()
	DarkDragonKnifeGameCopyView.super.onEnterFinished(self)
end

function DarkDragonKnifeGameCopyView:onExit()
	DarkDragonKnifeGameCopyView.super.onExit(self)
...
function DarkDragonKnifeGameCopyView:onExitFinished()
	DarkDragonKnifeGameCopyView.super.onExitFinished(self)
end

function DarkDragonKnifeGameCopyView:_initDataAndUI()
...
function DarkDragonKnifeGameCopyView:_ActorPath()
	return {
		"lihui_spine/10139_arui_lihui/10139_arui_lihui-ui_p.prefab"
	}
end

function DarkDragonKnifeGameCopyView:_towerPath()
	return "fx_ui_longzun/xiaoyouxi/aruilong/aruilong-ui_p.prefab"
end

function DarkDragonKnifeGameCopyView:_resetAll()
...
function DarkDragonKnifeGameCopyView:_onJoystickUp()
	self._mainActor.velocity = Vector2.zero
end

function DarkDragonKnifeGameCopyView:_onJoystickDown()
	return
end

function DarkDragonKnifeGameCopyView:_onJoystickDrag(x, y)
	self._mainActor.velocity = self._mainActorVelocity * Vector2.New(x, y)
end

function DarkDragonKnifeGameCopyView:_onClickStartGame()
	if not self._isRunning then
		self:_startGame()
	end
end

function DarkDragonKnifeGameCopyView:_moveMainActor(deltaTime, velocity)
...
function DarkDragonKnifeGameCopyView:_moveContrary(deltaTime)
	self:_moveMainActorVelocity(deltaTime, self._tangentVector2)
end

function DarkDragonKnifeGameCopyView:_moveMainActorVelocity(deltaTime, velocity)
...
function DarkDragonKnifeGameCopyView:_startGame()
	GameUtil.callBack(self._startCallBack)
	self:_onStartGameRes()
end

function DarkDragonKnifeGameCopyView:_onStartGameRes()
...
function DarkDragonKnifeGameCopyView:_tick()
	if not self._isRunning then
		return
	end
...
function DarkDragonKnifeGameCopyView:_onSecond()
	self:_updateSiblingIndex()
end

function DarkDragonKnifeGameCopyView:_updateSiblingIndex()
	local t = {}
...
function DarkDragonKnifeGameCopyView:_checkCollider(deltaTime)
...
function DarkDragonKnifeGameCopyView:_updateMonster()
...
function DarkDragonKnifeGameCopyView:_bornMonster(type, gameCfg)
...
function DarkDragonKnifeGameCopyView:_bornBullet(params)
...
function DarkDragonKnifeGameCopyView:_bulletAttack(damageVal, mGo)
...
function DarkDragonKnifeGameCopyView:_playDeadEffect()
...
function DarkDragonKnifeGameCopyView:_stopGame()
	removetimer(self._stopGame, self)
	...
	local showTips = string.format("游戏时长：%ss,局内积分：%s", math.floor(self._tickedTime), totalScore)

	local function func()
		local info = {}

		info.isPass = true
		info.gameScore = totalScore

		GameUtil.callBack(self._finishCallBack, info)
		self:close()
	end

	TipsFacade.instance:openTipWindowNoX("游戏结束", showTips, func, "确认", UnityEngine.TextAnchor.MiddleCenter)
end

function DarkDragonKnifeGameCopyView:_damgeTower(hurt)
...
function DarkDragonKnifeGameCopyView:_refreshTowerBlood()
...
function DarkDragonKnifeGameCopyView:_addKillNum()
...
function DarkDragonKnifeGameCopyView:_refreshKillNum()
	self._txtKill.text = langPara("击杀数量：<color=#59E8FDFF>%s</color>", self._killNum)
end

function DarkDragonKnifeGameCopyView:_skillOpen()
...
function DarkDragonKnifeGameCopyView:_tickSkill(deltaTime)
...
function DarkDragonKnifeGameCopyView:_tickCombo(deltaTime)
...
function DarkDragonKnifeGameCopyView:_addCombo()
...
function DarkDragonKnifeGameCopyView:_refreshComboNum()
	self._txtCombo:SetText(self._comboNum)
end

function DarkDragonKnifeGameCopyView:_showMeanTimeKill(num)
	if num > 1 then
...
function DarkDragonKnifeGameCopyView:_playTowerIdleAnimation()
	local sg = self._sg
...
function DarkDragonKnifeGameCopyView:_playMonsterDeadEffect(pos)
...
return DarkDragonKnifeGameCopyView
