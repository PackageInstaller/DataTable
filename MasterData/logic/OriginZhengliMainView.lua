-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/originzhengli/view/OriginZhengliMainView.lua

module("logic.extensions.originzhengli.view.OriginZhengliMainView", package.seeall)

local OriginZhengliMainView = class("OriginZhengliMainView", ViewComponent)

function OriginZhengliMainView:ctor()
	OriginZhengliMainView.super.ctor(self)
end

function OriginZhengliMainView:unbindEvents()
	OriginZhengliMainView.super.unbindEvents(self)
...
function OriginZhengliMainView:bindEvents()
	OriginZhengliMainView.super.bindEvents(self)
...
function OriginZhengliMainView:buildUI()
	OriginZhengliMainView.super.buildUI(self)
...
function OriginZhengliMainView:onExit()
	OriginZhengliMainView.super.onExit(self)
...
function OriginZhengliMainView:onEnter()
	OriginZhengliMainView.super.onEnter(self)
...
function OriginZhengliMainView:_updateUIByCfg()
...
function OriginZhengliMainView:_updatePetInfo()
...
function OriginZhengliMainView:_updateUIByInfo()
	self._isPass = false

	local info = OriginZhengliModel.instance:getInfoByActId(self._activityId) or {}

	if info then
		self._isPass = info.isGainPrize or false
	end

	goutil.setActive(self._passTips, self._isPass)
end

function OriginZhengliMainView:_onClickBtnClg()
	UIStateManager.instance:push(ViewName.OriginZhengliStageView, self._activityId)
end

function OriginZhengliMainView:_onClickBtnTip()
	if not self._actCfg.ruleKey then
		TipsFacade.instance:openRulesView(self._actCfg.ruleKey)
	end
end

function OriginZhengliMainView:_onClickBtnJump(index)
...
function OriginZhengliMainView:_onClickBtnSkill()
...
function OriginZhengliMainView:_onClickBtnIntroduce()
	PetbookController.instance:openPetinfoView(self._curRaceId)
end

return OriginZhengliMainView
