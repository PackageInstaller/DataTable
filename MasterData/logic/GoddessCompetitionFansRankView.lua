-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/goddesscompetition/view/GoddessCompetitionFansRankView.lua

module("logic.extensions.goddesscompetition.view.GoddessCompetitionFansRankView", package.seeall)

local GoddessCompetitionFansRankView = class("GoddessCompetitionFansRankView", ViewComponent)
local FINAL_STEPID = 4

function GoddessCompetitionFansRankView:ctor()
	GoddessCompetitionFansRankView.super.ctor(self)
end

function GoddessCompetitionFansRankView:unbindEvents()
	GoddessCompetitionFansRankView.super.unbindEvents(self)
end

function GoddessCompetitionFansRankView:bindEvents()
	GoddessCompetitionFansRankView.super.bindEvents(self)
end

function GoddessCompetitionFansRankView:buildUI()
	GoddessCompetitionFansRankView.super.buildUI(self)
...
function GoddessCompetitionFansRankView:onExit()
	GoddessCompetitionFansRankView.super.onExit(self)
...
function GoddessCompetitionFansRankView:onEnter()
	GoddessCompetitionFansRankView.super.onEnter(self)
	...
	if info then
		if not info.faceId then
			...
			if faceId <= 0 then
				...
				faceId = goddessCfg and goddessCfg[1] and goddessCfg[1].goddessFaceId or 0
			end

			self:_onClickPet(faceId)
		end
	end
end

function GoddessCompetitionFansRankView:_refreshView()
	local rankInfo, myRank, myScore = GoddessCompetitionModel.instance:getRankInfo()

	rankInfo = rankInfo or {}

	self._rankTableList:reloadData(rankInfo or {})
	...
	self._txtMyScore.text = checknumber(myScore) > 0 and myScore or 0
end

function GoddessCompetitionFansRankView:_updateRankCell(view, cell, data, tag)
...
function GoddessCompetitionFansRankView:_clearRankCell(cell)
	local go = cell.gameObject

	HeadItemController.instance:resetHeadCell(go)
end

function GoddessCompetitionFansRankView:_updatePetCell(view, cell, data, tag)
	local go = cell.gameObject
...
function GoddessCompetitionFansRankView:_clearPetCell(cell)
	local go = cell.gameObject
	local con = goutil.findChild(go, "petContainer")
...
function GoddessCompetitionFansRankView:_onClickPet(faceId)
	if self._curFaceId ~= faceId then
		...
		if GameUtil.string2time(stepCfg.openTime) < ServerTime.now() then
			GoddessCompetitionAgent.instance:sendPM_GoddessCompetitionSuperFansRankViewReq(self._activityId, self._stepId, self._curFaceId)
		else
...
return GoddessCompetitionFansRankView
