-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/season/view/mainhud/SeasonChatTableView.lua

module("logic.extensions.chat.view.SeasonChatTableView", package.seeall)

local SeasonChatTableView = class("SeasonChatTableView", ChatTableView)

function SeasonChatTableView:ctor()
	SeasonChatTableView.super.ctor(self)

	self._viewName = ViewName.chat
end

function SeasonChatTableView:buildUI()
	SeasonChatTableView.super.buildUI(self)
...
function SeasonChatTableView:_onClickSkin()
	FloatWordMgr.instance:show(ConstString.NotRelease)
end

function SeasonChatTableView:_onClickMicrophone()
	FloatWordMgr.instance:show(ConstString.NotRelease)
end

function SeasonChatTableView:bindEventsNew()
	SeasonChatTableView.super.bindEventsNew(self)
...
function SeasonChatTableView:unbindEventsNew()
	SeasonChatTableView.super.unbindEventsNew(self)
...
function SeasonChatTableView:onEnter()
	SeasonChatTableView.super.onEnter(self)
	settimer(0.02, self._forceClickChannel, self, false)
end

function SeasonChatTableView:onExit()
	SeasonChatTableView.super.onExit(self)
	removetimer(self._forceClickChannel, self)
end

function SeasonChatTableView:_forceClickChannel()
	if checknumber(SeasonModel.instance:getTeammateId()) > 0 then
		self:onChannelChange(GameEnum.ChatChannel.Private, SeasonModel.instance:getTeammateId())
	else
		self:close()
	end
end

function SeasonChatTableView:checkChannelLimit()
	return true
end

function SeasonChatTableView:checkEnoughAndSendMsg(text, msgType)
...
function SeasonChatTableView:_onClickVoiceSetting()
...
function SeasonChatTableView:onChannelChange(channel, param)
	self:initChatRoom(channel, tostring(param))
	self._Nego_VoiceSetting:SetActive(channel ~= GameEnum.ChatChannel.System)
	self._Nego_Bottom:SetActive(channel ~= GameEnum.ChatChannel.System)
	goutil.setActive(self._imgGou, ChatSettingModel.instance:GetAutoVoice(channel))
end

function SeasonChatTableView:_createItemChatNormal(data, go, tag)
	local item = self._itemChatMap[go.gameObject]

	if not item then
		item = SeasonItemChatNormal.New(go.gameObject, tag)
