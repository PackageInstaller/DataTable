-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/qixilinkgame/view/QiXiLinkGameView.lua

module("logic.extensions.qixilinkgame.view.QiXiLinkGameView", package.seeall)

local QiXiLinkGameView = class("QiXiLinkGameView", LinkGameBaseView)

QiXiLinkGameView.hideTime = 0.1

function QiXiLinkGameView:ctor()
	QiXiLinkGameView.super.ctor(self)
end

function QiXiLinkGameView:bindEvents()
	QiXiLinkGameView.super.bindEvents(self)
	GameUtil.addClickHandler(self._btnClose, self._onClickClose, self)
	GameUtil.addClickHandler(self._btnRule, self._onClickRule, self)
	GameUtil.addClickHandler(self._btnStart, self._onClickStart, self)
end

function QiXiLinkGameView:unbindEvents()
	QiXiLinkGameView.super.unbindEvents(self)
	GameUtil.rmClickHandler(self._btnClose)
	GameUtil.rmClickHandler(self._btnRule)
	GameUtil.rmClickHandler(self._btnStart)
end

function QiXiLinkGameView:buildUI()
	QiXiLinkGameView.super.buildUI(self)
...
function QiXiLinkGameView:onEnter()
	QiXiLinkGameView.super.onEnter(self)
	...
	self._gameActCfg = QiXiLinkGameConfig.instance:getGameActCfg(self._gameId)
	...
	self._txtPairs.text = lang("当前已获积分：<color=#63EFF7FF>0</color>")
...
function QiXiLinkGameView:onExit()
	QiXiLinkGameView.super.onExit(self)
...
function QiXiLinkGameView:setMapData()
	local gameId = QiXiLinkGameController.instance:getTheGameId()

	self._miniGameCfg = QiXiLinkGameConfig.instance:getGameMiniGameCfgs(gameId)
	self._iconCfg = QiXiLinkGameConfig.instance:getGameIconCfgs(self._miniGameCfg.iconPlan)
...
function QiXiLinkGameView:_readyStartGame()
	GameUtil.SetActive(self._btnStart, true)
end

function QiXiLinkGameView:_startGame()
	QiXiLinkGameView.super._startGame(self)
	...
	self._txtPairs.text = lang("已获积分数：<color=#63EFF7FF>0</color>")
end

function QiXiLinkGameView:_onDeadGame()
	self:_pauseGame(self._gameTime)
	TipsFacade.instance:openTipWindowNoX(lang("提示"), lang("当前无操作空间，即将重新排列！"), function()
		LinkGameController.instance:randomCurMap()
		self:_genarateMap()
		self:_recoverGame()
...
function QiXiLinkGameView:_updateCell(cell, cellInfo)
...
function QiXiLinkGameView:_onSelectEnd()
	self._txtPairs.text = langPara("已获积分数：<color=#63EFF7FF>%d</color>", self:_getCurRemovePairs())

	QiXiLinkGameView.super._onSelectEnd(self)
end

function QiXiLinkGameView:_checkGame()
...
function QiXiLinkGameView:_getCurRemovePairs()
	local curScore = LinkGameModel.instance:getRemovePairs() + checknumber(self._pairs)

	curScore = math.min(curScore, self._gameActCfg.scoreLimit)

	return curScore
end

function QiXiLinkGameView:_showLinkEffect()
...
function QiXiLinkGameView:_hideLinkEffect()
	for i, cellInfo in ipairs(self._curPath) do
		if self._linkObject[cellInfo.row] and self._linkObject[cellInfo.row][cellInfo.col] and i > 1 and i < #self._curPath then
			...
			for j = 0, linkGo.transform.childCount - 1 do
				...
				for k = 0, stateGo.transform.childCount - 1 do
					...
					UnityTweens.UITweenFadeOut.StartTween(imageGO.gameObject, QiXiLinkGameView.hideTime)
				end
			end
		end
		...
		if self._mapObject[cellInfo.row] and self._mapObject[cellInfo.row][cellInfo.col] and (i == 1 or i == #self._curPath) then
			...
			UnityTweens.UITweenFadeOut.StartTween(bg, QiXiLinkGameView.hideTime)
			UnityTweens.UITweenFadeOut.StartTween(imgSelect, QiXiLinkGameView.hideTime)
			UnityTweens.UITweenFadeOut.StartTween(imgCon, QiXiLinkGameView.hideTime)
	...
	removetimer(self._onLinkEffectEnd, self)
	settimer(QiXiLinkGameView.hideTime + 0.01, self._onLinkEffectEnd, self, false)
end

function QiXiLinkGameView:setCellLinkState(cellInfo, isShow, linkState, param)
...
function QiXiLinkGameView:getCellSize()
	return {
		x = 104,
		y = 104
	}
...
function QiXiLinkGameView:getCellOffset()
	return {
		x = 4.4,
		y = 0
	}
...
function QiXiLinkGameView:_onLinkEffectEnd()
...
function QiXiLinkGameView:_endGame()
	QiXiLinkGameView.super._endGame(self)
	...
	if self._isOnTime == true then
		...
		TipsFacade.instance:openTipWindowNoX("游戏结束", desc, function()
			QiXiLinkGameController.instance:sendPM_LinkendlessGameEndGameReq(self._lastViewId, removePairs)
...
function QiXiLinkGameView:_doFinfishCallBack(isPass, removePairs)
	local info = {}
...
function QiXiLinkGameView:_onClickClose()
	self:_pauseGame(self._gameTime)
...
function QiXiLinkGameView:_onClickRule()
	self:_pauseGame(self._gameTime)
...
function QiXiLinkGameView:_onClickStart()
	self:_startGame()
end

function QiXiLinkGameView:_pauseGame(defautTime)
	self._isPauseGame = true
	self._saveTime = checknumber(self._targetTime) > 0 and self._targetTime - ServerTime.now() or defautTime
end

function QiXiLinkGameView:_recoverGame()
...
function QiXiLinkGameView:_onClock()
...
function QiXiLinkGameView:_onCloseRuleView()
