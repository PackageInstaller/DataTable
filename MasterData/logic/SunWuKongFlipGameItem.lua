-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/Scripts/Lua/logic/extensions/sunwukongchallenge/view/flipgame/SunWuKongFlipGameItem.lua

module("logic.extensions.sunwukongchallenge.view.flipgame.SunWuKongFlipGameItem", package.seeall)

local SunWuKongFlipGameItem = class("SunWuKongFlipGameItem")
local FlipGameState = {
	Default = 1,
	Erase = 3,
	Select = 2,
...
function SunWuKongFlipGameItem:ctor()
	self._id = 0
	self._curState = FlipGameState.Default
end

function SunWuKongFlipGameItem:buildUI(go)
	self._go = go
...
function SunWuKongFlipGameItem:init(id, index)
	self._id = id
	self._index = index
	self._txt.text = id
...
function SunWuKongFlipGameItem:reset()
	if self._btn then
		self._btn:RemoveClickListener()
	end
end

function SunWuKongFlipGameItem:_onClickBtn()
	if self._curState == FlipGameState.Default or self._curState == FlipGameState.UnSelect then
		printInfo("test _onClickBtn 默认状态")

		local isSuccess = SunWuKongChallengeFlipGameModel.instance:addSelectIndex(self._index, self._id)

		if isSuccess then
			self:setState(FlipGameState.Select)
		end

		GlobalDispatcher:dispatch(GlobalNotify.SunWuKongFlipGameSelect)
...
function SunWuKongFlipGameItem:setState(state)
...
function SunWuKongFlipGameItem:changeToUnSelectState()
	self:setState(FlipGameState.UnSelect)
end

function SunWuKongFlipGameItem:changeToEraseState()
	self:setState(FlipGameState.Erase)
	SunWuKongChallengeFlipGameModel.instance:addToEraseList(self._id)
end

function SunWuKongFlipGameItem:getPosition()
