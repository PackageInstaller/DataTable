...
function HomeBannerComp:_onTouchMove(arg_12_1)
	if self._startPosX and self._bannerNum > 0 then
		local var_12_0 = arg_12_1:getInput():getTouch():getLocationInView().x

		if math.abs(var_12_0 - self._startPosX) > 20 then
			local var_12_1 = self.m_selectIndexController:getSelectedIndex() + (var_12_0 > self._startPosX and -1 or 1)
