...
function NewSlgPvcRankRewardPop:updateReq()
	local var_6_0 = self:getController("bigTab"):getSelectedIndex() * 2 + self:getController("rightTab"):getSelectedIndex() + 1
	...
	if not self._rankInfo[var_6_0] then
		if var_6_0 == var_0_0.RANK_TYPE.PERSONAL_UNION and not g.core.model.User.allianceData:hasAlliance() then
			self:updateRankRewardView()
		...
		if var_6_0 == var_0_0.RANK_TYPE.PERSONAL_UNION and self._curCityCfg then
			var_6_1.city_id = self._curCityCfg.id or nil
		end

		g.core.network.GameNetProxy:send_C2S_NewSlg_GetRank(var_6_1)
	else
		self:updateRankRewardView()
	end
end
...
function NewSlgPvcRankRewardPop:updateRankRewardView()
	...
	local var_7_1 = var_7_0:getSelectedIndex() * 2 + self:getController("rightTab"):getSelectedIndex() + 1
	local var_7_2

	if var_7_0 == var_0_0.RANK_TYPE.PERSONAL_UNION then
