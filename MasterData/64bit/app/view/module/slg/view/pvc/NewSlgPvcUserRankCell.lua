local NewSlgPvcUserRankCell = class("NewSlgPvcUserRankCell", require("app.fairyGUI.newSlg.UI_NewSlgPvcUserRankCell"))

function NewSlgPvcUserRankCell:ctor()
	self._data = nil
	self._rewards = {}
...
function NewSlgPvcUserRankCell:_initCell()
	self.m_awardList:setVirtual(self)
	self.m_awardList:setItemRenderer(handler(self, self._onAwardItemRender))
end

function NewSlgPvcUserRankCell:updateRankCell(arg_3_1, arg_3_2, arg_3_3)
	self._data = arg_3_1
	...
	self.m_userIcon:updateAsUser(var_3_0)
	self.m_nameTxt:setText(var_3_0.name)
	self.m_levelTxt:setText(g.core.lang:get(428987, {
		level = var_3_0.level
	}))
	self.m_serverText:setText(g.core.platform.ServerListProxy:getServerBySid(var_3_0.sid).name)
	self.m_hasUserController:setSelectedIndex(1)
end

function NewSlgPvcUserRankCell:_updateReward(arg_4_1)
...
function NewSlgPvcUserRankCell:_onAwardItemRender(arg_5_1, arg_5_2)
	arg_5_2:updateIcon(self._rewards[arg_5_1 + 1])
end

function NewSlgPvcUserRankCell:_onIconClick()
