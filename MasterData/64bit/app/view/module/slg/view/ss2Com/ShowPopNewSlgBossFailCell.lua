local ShowPopNewSlgBossFailCell = class("ShowPopNewSlgBossFailCell")

function ShowPopNewSlgBossFailCell:ctor()
	...
	self._txtContribute = self:getChild("Txt_contribute")
	self._showBgCtrl = self:getController("show_bg")
	self._isSelfCtrl = self:getController("is_self")
end

function ShowPopNewSlgBossFailCell:updateCell(arg_2_1)
	if not arg_2_1 then
		return
	end
	...
	self._txtContribute:setText(arg_2_1.contribute_value or 0)
...
return ShowPopNewSlgBossFailCell
