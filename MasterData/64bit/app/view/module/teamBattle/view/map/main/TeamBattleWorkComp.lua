...
function TeamBattleWorkComp:_initComp()
	...
	local var_2_1 = var_2_0:getPlayerStruct():getWork()

	self.m_workList:setSelectedIndex(var_2_0)

	if var_2_1 == 0 then
		self:_onChangeWork()
	else
		self.m_workList:getChildAt((math.max(var_2_1 - 1, 0))):setCtrlState("button", {
			index = 1
		})
	end
end
