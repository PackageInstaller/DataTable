...
local CultivateWitchTaskLayer = class("CultivateWitchTaskLayer", require("app.fairyGUI.cultivate.UI_CultivateWitchTaskLayer"), function()
	return fgui.GComponent:create({
		pkgName = "cultivate",
		isFullScreen = true,
		pkgPath = "ui/cultivate/cultivate",
		resName = "CultivateWitchTaskLayer"
	}, ...)
end)

function CultivateWitchTaskLayer:ctor(arg_2_1)
...
function CultivateWitchTaskLayer:onLoad()
...
function CultivateWitchTaskLayer:_onCrossDayUpdate()
	if not self._actData:isInNormalOpenTime() then
		g.core.module.ModuleManager:switchModuleIntelligent(g.view.entrance.HOME)
...
function CultivateWitchTaskLayer:initTaskMainView()
	local var_5_0 = self._actData:getActivityCfg()
	local var_5_1 = g.core.config.cultivate_activity_display_info.get(var_5_0.id)

	self:addBg("bg/cultivate/" .. var_0_2:getResDirId(var_5_0.id) .. "/bg_mnzj_zjm.jpg")
	self.m_topBarComp:setResInfoById(var_5_1.interface_task)
	self.m_topBarComp:setTitle(var_5_1.module_task)
	self.m_titleTextIcon:setURL(var_0_3:getCultivateMulLangImage("txt_mnzj_renwu", var_0_2:getResDirId(var_5_0.id)))
...
function CultivateWitchTaskLayer:updatePassCardTime()
...
function CultivateWitchTaskLayer:_onClickTabItem()
	self._curSelIndex = self.m_tabList:getSelectedIndex()

	self:updateTaskList(true)
end

function CultivateWitchTaskLayer:_onCultivateActivityUpdateTask()
	self:updateTaskList()
end

function CultivateWitchTaskLayer:_onTabItemRenderer(arg_9_1, arg_9_2)
...
function CultivateWitchTaskLayer:updateTaskList(arg_10_1)
...
function CultivateWitchTaskLayer:_onTaskItemRenderer(arg_11_1, arg_11_2)
	arg_11_2:updateCultivateCell(self._taskList[arg_11_1 + 1], self._actId)
end

function CultivateWitchTaskLayer:_onCultivateActivityGetTaskReward(arg_12_1, arg_12_2, arg_12_3, arg_12_4)
	g.core.module.ModuleManager:awardSummary(arg_12_4.awards)
	self:updateTaskList()
end

return CultivateWitchTaskLayer
