...
function FurnitureMainLayer:_onTouchMove(arg_7_1)
	if not self._dragStart then
		...
		local var_7_1 = var_7_0:getTouch():getDelta()

		self.m_roomComp:setPosition(KsMath.rangeIn(var_7_0.x + var_7_1.x, 0, self._offXRange), (KsMath.rangeIn(self.m_roomComp:getPosition().y - var_7_1.y, 0, self._offYRange)))
	...
	elseif #self._dragFItemCompName > 0 and self._lastPos then
		local var_7_2 = arg_7_1:getInput():getTouch():getLocation()

		self._lastPos = var_7_2
...
function FurnitureMainLayer:receiveCompEvent(arg_14_1, arg_14_2)
	...
	elseif arg_14_1 == "CREATE_FURNITURE_OBJECT" then
		...
		if g.core.guide.GuideProxy:isGuideRunning() then
			...
			self:_setBottomFurnitureVisible(var_0_6:getFurnitureBasalTypeById(var_14_0) == var_0_2.FLOOR_LAYER)
		end
