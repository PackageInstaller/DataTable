...
local TowerSubStageLayer = class("TowerSubStageLayer", require("app.fairyGUI.tower.UI_TowerSubStageLayer"), function()
	return fgui.GComponent:create({
		resName = "TowerSubStageLayer",
		pkgName = "tower",
		isFullScreen = true,
...
function TowerSubStageLayer:ctor(arg_2_1)
	self._stageType = arg_2_1.stageType or var_0_2.STAGE_TYPE.JIAO_GUO
	self._maxPosX = 0
	self._minPosX = 0
...
function TowerSubStageLayer:onLoad()
...
function TowerSubStageLayer:initData()
...
function TowerSubStageLayer:checkPosCanMove()
...
function TowerSubStageLayer:getFinishPos(arg_6_1)
...
function TowerSubStageLayer:_initView()
	...
	if self._mapView == nil then
		self._mapView = fgui.UIPackage:createObject("tower", "TowerMapComp")
	...
	if self._subMapView == nil then
		self._subMapView = fgui.UIPackage:createObject("tower", "TowerMapComp")
...
function TowerSubStageLayer:_onCheckBtnClick()
	var_0_4:setIsSelectPopFormation((self.m_checkBtn:isSelected()))
end

function TowerSubStageLayer:_onOneKeyClick()
	if var_0_1:getLeftCount(var_0_3.PLAY_TYPE["TOWER_CHALLENGE_COUNT_" .. self._stageType]) <= 0 then
...
function TowerSubStageLayer:_onTouchMove(arg_12_1)
...
function TowerSubStageLayer:_onTouchEnd()
	self._isTouched = false
	self._isScroll = false
end

function TowerSubStageLayer:autoMove(arg_14_1)
...
function TowerSubStageLayer:_move(arg_15_1, arg_15_2)
...
function TowerSubStageLayer:updateImagePos(arg_16_1)
...
function TowerSubStageLayer:updateView(arg_17_1)
	if not self or not self._stageType then
		return
	end

	self.m_layerName:setText(g.core.lang:get(308001 + self._stageType))
...
function TowerSubStageLayer:updateImageNodeInScroll()
...
function TowerSubStageLayer:updateImageNode(arg_19_1)
...
function TowerSubStageLayer:_stopAutoMove()
...
function TowerSubStageLayer:_onRcvBattleBegin(arg_21_1, arg_21_2, arg_21_3)
...
function TowerSubStageLayer:_showAwardSummaryPop()
...
function TowerSubStageLayer:_checkIsShowPop()
...
function TowerSubStageLayer:_onRcvFastRolling(arg_24_1, arg_24_2, arg_24_3)
	...
	self:dispatchCompEvent("TowerSubStageLayer_update")
end

function TowerSubStageLayer:pushCapacityPop()
	local var_25_0 = {}
...
function TowerSubStageLayer:receiveCompEvent(arg_26_1, arg_26_2)
...
function TowerSubStageLayer:_onRankRewardBtnClick()
	var_0_5:pushPopup(require("app.view.module.tower.view.TowerRankRewardPop").new(self._stageType), {
		ignoreTouch = false,
		touchDisappear = false
	})
end

function TowerSubStageLayer:onUnload()
	self._isClose = true
