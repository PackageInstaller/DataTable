...
function DebugNavmesh2DComp:_onTouchEnd(arg_8_1)
	if self._isTouch then
		...
		local var_8_0 = self.m_meshRoot:globalToLocal((arg_8_1:getInput():getPosition()))

		var_8_0.y = -var_8_0.y

		if arg_8_1:getInput():getButton() == 0 then
			self._startPoint = Vector2.new(var_8_0.x, var_8_0.y)

			local var_8_1 = self:getInTriangles(self._startPoint)

			self._startNode = var_8_1
		...
		elseif arg_8_1:getInput():getButton() == 1 then
			self._endPoint = Vector2.new(var_8_0.x, var_8_0.y)

			local var_8_2 = self:getInTriangles(self._endPoint)

			self._endNode = var_8_2
		...
		if self._startNode and self._endNode then
			...
			local var_8_4, var_8_5 = self:aStart(self._startNode, self._endNode, self._startPoint, self._endPoint)

			if var_8_4 then
				self:funnelSearch2(var_8_4, var_8_5, self._startPoint, self._endPoint)

				for iter_8_4 = 2, #var_8_4 - 1 do
					var_8_4[iter_8_4]:setPath(true)
				end

				self:log("寻路耗时：" .. os.clock() - var_8_3)
			else
				self:showLines({})
				self:log("未找到路径,寻路耗时：" .. os.clock() - var_8_3)
			end
		end
