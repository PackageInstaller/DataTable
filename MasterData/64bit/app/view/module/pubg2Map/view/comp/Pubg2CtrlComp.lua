local Pubg2CtrlComp = class("Pubg2CtrlComp", require("app.fairyGUI.pubg.UI_Pubg2CtrlComp"))

function Pubg2CtrlComp:ctor()
...
function Pubg2CtrlComp:onLoad()
	self._inGlobalPos = self:localToGlobal(cc.p(0, 0))
end

function Pubg2CtrlComp:isTouching()
	return self._isTouch
end

function Pubg2CtrlComp:getAngle()
	if self._curCtrlPos.x * self._curCtrlPos.x + self._curCtrlPos.y * self._curCtrlPos.y <= 10 or self._curCtrlPos.y == 0 or self._curCtrlPos.x == 0 then
		return 0
	end

	return math.atan2(self._curCtrlPos.y, self._curCtrlPos.x) or 0
end

function Pubg2CtrlComp:_onTouchBegin(arg_5_1)
...
function Pubg2CtrlComp:_onTouchMove(arg_6_1)
...
function Pubg2CtrlComp:_onTouchEnd(arg_7_1)
	self._isTouch = false
	self._curCtrlPos = cc.p(0, 0)
