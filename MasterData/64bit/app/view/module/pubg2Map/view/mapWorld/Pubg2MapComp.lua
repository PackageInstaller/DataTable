...
function Pubg2MapComp:setStage(arg_2_1)
	...
	local var_2_0 = self:getFloorComp():getMyCompPos()

	self:getMapWorld():setFollowCenter(cc.p(var_2_0.x, -var_2_0.y))
end
...
function Pubg2MapComp:tickMapUpdate(arg_19_1)
	if self._stage then
		...
		if arg_19_1 then
			local var_19_0 = self:getFloorComp():getMyCompPos()

			self:getMapWorld():setFollowCenter(cc.p(var_19_0.x, -var_19_0.y))
		end
