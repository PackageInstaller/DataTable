...
function PreciousDevelopLayer:_onTouchMove(arg_15_1)
	...
	self.m_suitComp:setX(var_15_0.x + self._touchOffsetX)
...
function PreciousDevelopLayer:_onTouchEnd(arg_16_1)
	...
	local var_16_1 = var_16_0:getTouch():getLocation().x + self._touchOffsetX - self._suitCompX
	...
	if math.abs(var_16_0) >= var_0_5 and self._maxIdx > 1 then
		...
		self.m_suitComp:runAction((cc.Sequence:create(cc.Spawn:create(cc.EaseSineOut:create((cc.MoveBy:create(0.5, cc.p(-var_16_1, 0)))), (cc.FadeIn:create(0.3))), (cc.CallFunc:create(handler(self, function()
			self._canTouch = true
		end))))))
	end
