...
local BountyGiftCell = class("BountyGiftCell", require("app.fairyGUI.bounty.UI_BountyGiftCell"))

function BountyGiftCell:ctor()
	self._info = nil
	self._curInfo = nil
	self._isEnough = true
	self._canBuy = false
...
function BountyGiftCell:_onBtnClick()
	if self._canBuy then
...
function BountyGiftCell:updateCell(arg_6_1)
	if not arg_6_1 then
	...
	self.m_showLeftTimeController:setSelectedIndex(1)
end

function BountyGiftCell:_startDailyTimer()
	self._dailyTimer = self._dailyTimer or self:newSchedule(handler(self, self._onUpdateTime, 1))
end

function BountyGiftCell:_stopDailyTimer()
...
function BountyGiftCell:_onUpdateTime()
	if self.m_showRefreshTimeController:getSelectedIndex() == 1 then
		self:_onDailyTimeUpdate()
	end

	if self.m_showLeftTimeController:getSelectedIndex() == 1 then
		self:_onRefreshEndBuyTime()
	end
end

function BountyGiftCell:_onDailyTimeUpdate()
	local var_10_0 = self._dailyEndTime - g.core.common.ServerTime:getTime()

	if var_10_0 > 0 then
		self.m_refreshTimeTxt:setText(g.core.lang:get(420110, {
			time = g.core.common.ServerTime:getCountDownBySecond(var_10_0)
		}))
	else
		self.m_showRefreshTimeController:setSelectedIndex(0)
...
function BountyGiftCell:_onRefreshEndBuyTime()
	local var_11_0, var_11_1, var_11_2, var_11_3 = g.core.common.ServerTime:getLeftTimeParts(self._curInfo.endBuyTime)
	...
	elseif var_11_3 == 0 then
		self.m_showLeftTimeController:setSelectedIndex(0)
		self:dispatchCompEvent("GiftTimeOut_CheckRefresh")
		self:_stopDailyTimer()
	end
end

return BountyGiftCell
