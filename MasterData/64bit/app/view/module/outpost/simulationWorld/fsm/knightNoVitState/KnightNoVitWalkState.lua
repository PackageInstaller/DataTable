...
local KnightNoVitWalkState = class("KnightNoVitWalkState", require("app.view.module.outpost.simulationWorld.fsm.baseState.OutpostBaseState"))

function KnightNoVitWalkState:onCreate()
	self._stUUID = var_0_2.FsmState.WALK
end

function KnightNoVitWalkState:onInitState()
...
function KnightNoVitWalkState:doBeforeEntering()
	local var_3_0 = self._map:getGridByMapType(1)
	local var_3_1 = KsMath.mathCoordinateIndex(var_3_0[math.random(1, #var_3_0)])

	self._entity:setMovePos(cc.p(OutpostCommon.coordToPixel(var_3_1.x, var_3_1.y)))
...
function KnightNoVitWalkState:pathFind(arg_4_1)
	local var_4_0 = cc.p(self._entity:getPos())

	self._pathList = self._world:aStarPath(var_4_0, cc.p(self._entity.movePos), arg_4_1)
	...
	if arg_4_1 ~= nil then
		arg_4_1:runOnMainThread(function()
			...
			var_0_4:dispatchWorldEvent(OutpostEvent.KnightTalk, {
				talkType = var_0_2.TalkType.Talk16,
				entity = self._entity
			})
		end)
	else
...
function KnightNoVitWalkState:onTickUpdate(arg_6_1)
...
function KnightNoVitWalkState:doBeforeLeaving()
	self._entity:setMovePos(nil)
	self:stopTask()
end

return KnightNoVitWalkState
