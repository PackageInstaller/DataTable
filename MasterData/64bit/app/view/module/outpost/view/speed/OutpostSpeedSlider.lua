...
local OutpostSpeedSlider = class("OutpostSpeedSlider", require("app.fairyGUI.outpost.UI_OutpostSpeedSlider"))
local var_0_2 = 378

function OutpostSpeedSlider:ctor()
	self._lineList = {}
...
function OutpostSpeedSlider:_onTouchBegin(arg_2_1)
	arg_2_1:captureTouch()
end

function OutpostSpeedSlider:_onClickSpeedCheck()
	var_0_0:setSpeed(nil, (self.m_speedBtn:isSelected()))
end

function OutpostSpeedSlider:updateView()
	local var_4_0 = var_0_0:getMaxSpeed()

	self:_updateLine(var_4_0)
	self:setMin(0)
	self:setMax(var_4_0 - 1)

	local var_4_1 = var_0_0:getSpeed()

	self.m_speedText:setText(var_4_1)
	self:setValue(var_4_1 - 1)
end

function OutpostSpeedSlider:_updateLine(arg_5_1)
	for iter_5_0 = 1, math.max(#self._lineList, arg_5_1 - 2) do
		...
		if not self._lineList[iter_5_0] then
			...
			var_5_0:setURL("ui://outpost/pic_hxsz_wfjsjindu04")
...
function OutpostSpeedSlider:_onSpeedChanged()
...
function OutpostSpeedSlider:onSpeedChangedEnd()
	...
	local var_7_2 = var_0_0:getSpeed()

	var_0_0:setSpeed(var_7_1)
