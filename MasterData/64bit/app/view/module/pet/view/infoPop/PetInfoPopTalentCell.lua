...
local PetInfoPopTalentCell = class("PetInfoPopTalentCell", require("app.fairyGUI.pet.UI_PetInfoPopTalentCell"))

function PetInfoPopTalentCell:updateCell(arg_1_1)
...
function PetInfoPopTalentCell:_setTalent(arg_2_1)
	self.m_descText:setText("")

	local var_2_0 = g.core.config.talent_skill_info.get(arg_2_1.talentId)

	self.m_talentNameText:setText((var_0_0:get(201303, {
		name = var_2_0.name
	})))
	...
	if var_2_1 == "0" then
		var_2_1 = var_0_0:get(201011)
	elseif not self._isActive then
		self.m_talentUnLockTxt:setVisible(true)

		if var_2_0.open_type == 1 then
			self.m_talentUnLockTxt:setText(var_0_0:get(201300, {
				level = var_2_0.open_value
			}))
		elseif var_2_0.open_type == 2 then
			self.m_talentUnLockTxt:setText(var_0_0:get(201301, {
				level = var_2_0.open_value
			}))
		end
	end
...
function PetInfoPopTalentCell:_setPassive(arg_3_1)
	self.m_skillDescTxt:setText("")

	local var_3_0 = g.core.config.passive_skill_info.get(arg_3_1.skillId)

	self.m_passiveNameText:setText((var_0_0:get(201308, {
		name = var_3_0.name
	})))

	local var_3_1, var_3_2 = g.core.utils.String.formatPassiveSkillDesc(arg_3_1.skillId)

	self._isActive = arg_3_1.value >= arg_3_1.openValue
	self._openValue = arg_3_1.openValue

	self.m_skillUnLockTxt:setVisible(false)

	if var_3_1 == "0" then
		var_3_1 = var_0_0:get(201011)
	elseif not self._isActive then
		self.m_skillUnLockTxt:setVisible(true)

		if var_3_0.level == 1 then
			self.m_skillUnLockTxt:setText(var_0_0:get(201306, {
				level = arg_3_1.openValue
			}))
		else
			self.m_skillUnLockTxt:setText(var_0_0:get(201307, {
				level = arg_3_1.openValue
			}))
		end
	end

	self.m_skillDescTxt:setText(var_3_1)
	self.m_levelTxt:setText(var_0_0:get(201309, {
		level = var_3_0.level
	}))
end

function PetInfoPopTalentCell:refreshSize()
	local var_4_0 = self.m_styleController:getSelectedIndex()
	local var_4_1 = 90

	if var_4_0 == 0 then
		var_4_1 = self.m_descText:getPosition().y + self.m_descText:getHeight() + 6
	elseif var_4_0 == 1 then
		self.m_skillGroup:setPosition(cc.p(9, -1))

		var_4_1 = self.m_skillDescTxt:getPosition().y + self.m_skillDescTxt:getHeight() + 6
	else
		var_4_1 = self.m_skillDescTxt:getPosition().y + self.m_skillDescTxt:getHeight() + 6
	end

	var_4_1 = var_4_1 > 60 and var_4_1 or 60

	self:setHeight(var_4_1)
end

return PetInfoPopTalentCell
