local CompetitionRankAwardCell = class("CompetitionRankAwardCell", require("app.fairyGUI.competition.UI_CompetitionRankAwardCell"))

function CompetitionRankAwardCell:ctor()
	self.m_awardList:setVirtual()
	self.m_awardList:setItemRenderer(handler(self, self._onRenderAwardList))
end

function CompetitionRankAwardCell:updateCell(arg_2_1)
...
function CompetitionRankAwardCell:_onRenderAwardList(arg_3_1, arg_3_2)
	if self._tmpRankAwardArr[arg_3_1 + 1] then
		arg_3_2:updateIcon(self._tmpRankAwardArr[arg_3_1 + 1])
	end
end

return CompetitionRankAwardCell
