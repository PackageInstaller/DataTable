...
function FavoLayer:_onTouchMove(arg_21_1)
	if self._canTouchKnight and self._touchSkewPicPosX then
		local var_21_0 = arg_21_1:getInput():getTouch():getLocation()

		self.m_knightPlaceHolderComp:setX(var_21_0.x + self._touchSkewPicPosX)

		local var_21_1 = var_21_0.x + self._touchSkewPicPosX - self._picPosX

		var_21_1 = var_21_0.x + self._touchSkewPicPosX - self._picPosX > 0 and var_21_1 or -var_21_1

		self.m_knightPlaceHolderComp:setAlpha(1 - math.min(var_21_1, var_0_3) / var_0_3)
	end
end

function FavoLayer:_onTouchEnd(arg_22_1)
	if self._canTouchKnight and self._touchSkewPicPosX then
		local var_22_0 = arg_22_1:getInput()
		local var_22_1 = var_22_0:getTouch():getLocation().x + self._touchSkewPicPosX - self._picPosX
		...
		if math.abs(var_22_0) >= var_0_3 and self._selectedIndex then
			...
			self.m_knightPlaceHolderComp:runAction((cc.Sequence:create(cc.Spawn:create(cc.MoveBy:create(0.5, cc.p(-var_22_1, 0)), (cc.FadeIn:create(0.5))), cc.CallFunc:create(handler(self, function()
				self._canTouchKnight = true
			end)))))
		end
