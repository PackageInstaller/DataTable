...
function GuildWarFightMineCitySingleComp:onDragStart(arg_5_1)
	...
	if self._curDragComp:getUserData() then
		if self._curDragComp:getUserData():getUser() then
			...
			local var_5_0 = fgui.DragDropManager:getInstance()

			var_5_0:startDrag("", nil, (arg_5_1:getInput():getTouchId()))

			local var_5_1 = fgui.UIPackage:createObject("base_new", "BaseUserIcon")

			var_5_1:updateAsUser(self._curDragComp:getUserData():getUser())
			var_5_0:getAgent():displayObject():addChild(var_5_1:popDisplayObject())
