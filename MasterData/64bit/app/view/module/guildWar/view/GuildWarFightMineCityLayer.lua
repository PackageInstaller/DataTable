...
function GuildWarFightMineCityLayer:_onTouchMove(arg_8_1)
	...
	local var_8_1 = KsMath.rangeIn(self.m_contentComp:getX() + var_8_0:getTouch():getDelta().x, self._guildWarMinX, self._guildWarMaxX)

	self.m_contentComp:setX(var_8_0)

	if math.abs(var_8_1 - self._guildWarMinX) < 10 then
		self.m_arrowImg:setVisible(false)
	else
		self.m_arrowImg:setVisible(true)
	end

	if math.abs(var_8_1 - self._guildWarMaxX) < 10 then
		self.m_arrowImg2:setVisible(false)
	else
