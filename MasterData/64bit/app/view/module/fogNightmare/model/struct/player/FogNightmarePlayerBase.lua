...
function FogNightmarePlayerBase:_doBattleStart()
	...
	if self._isUsedUI then
		...
		local var_12_3 = g.core.model.User.fogNightmareData:getTowerData()
		local var_12_4 = var_12_3:getCurrentFloorNum()
		local var_12_5 = g.core.model.User.fogNightmareData:getTowerData():getFloorStruct(var_12_4):getSelectedStage()[1]
		...
		g.core.model.User.fogNightmareData:getAutoPlay():getCmdManager():pushCmd((FogNightmareCmdNet.new({
			...
			netParams = {
				floor = var_12_4,
				stage = var_12_5:getStageId(),
				wave = var_12_5.getTempSelectWave(var_12_3)
			},
...
function FogNightmarePlayerBase:_doWaveSelectAndPass()
	local var_30_0 = g.core.model.User.fogNightmareData:getTowerData()
	local var_30_1 = var_30_0:getCurFloorStruct():getSelectedStage()[1]
	local var_30_2
	local var_30_3 = 1

	for iter_30_0, iter_30_1 in ipairs((var_30_1.getAllWave(var_30_0))) do
	...
	if var_30_2 then
		...
		local var_30_4 = handler(self, self._monsterWavePass)

		var_30_1:setTempSelectWave(var_30_3)
		...
		local var_30_6 = FogNightmareCmdBase.new({
			execute = handler(self, self._doCheckNeedSelectKeepsake)
		})

		var_30_6:setQuickTrigger(true)
		g.core.model.User.fogNightmareData:getAutoPlay():getCmdManager():pushCmd(var_30_6)
	end
end
...
function FogNightmarePlayerBase:_cureWavePass(arg_32_1)
	...
	if self._isUsedUI then
		...
		local var_32_3 = g.core.model.User.fogNightmareData:getTowerData()
		local var_32_4 = var_32_3:getCurFloorStruct()

		g.core.model.User.fogNightmareData:getAutoPlay():getCmdManager():pushCmd((FogNightmareCmdNet.new({
			netEventKey = g.core.event.enum.EVENT_NET_S2C_FOG_RECOVER,
			netFunc = handler(g.core.network.GameNetProxy, g.core.network.GameNetProxy.send_C2S_Fog_Recover),
			netParams = {
				floor = var_32_4:getFloorId(),
				stage = var_32_4:getSelectedStage()[1].getStageId(var_32_3),
				wave = arg_32_1.wave:getWaveId()
			}
		})))
	end

	local var_32_5 = FogNightmareCmdBase.new({
		execute = handler(self, self._doEnterNextFloor)
	})

	var_32_5:setQuickTrigger(true)
	g.core.model.User.fogNightmareData:getAutoPlay():getCmdManager():pushCmd(var_32_5)
end
...
function FogNightmarePlayerBase:_revivalWavePass(arg_35_1)
	...
	if self._isUsedUI then
		...
		local var_35_3 = g.core.model.User.fogNightmareData:getTowerData()
		local var_35_4 = var_35_3:getCurFloorStruct()

		g.core.model.User.fogNightmareData:getAutoPlay():getCmdManager():pushCmd((FogNightmareCmdNet.new({
			netEventKey = g.core.event.enum.EVENT_NET_S2C_FOG_RECOVER,
			netFunc = handler(g.core.network.GameNetProxy, g.core.network.GameNetProxy.send_C2S_Fog_Recover),
			netParams = {
				floor = var_35_4:getFloorId(),
				stage = var_35_4:getSelectedStage()[1].getStageId(var_35_3),
				wave = arg_35_1.wave:getWaveId()
			}
		})))
	end

	local var_35_5 = FogNightmareCmdBase.new({
		execute = handler(self, self._doEnterNextFloor)
	})

	var_35_5:setQuickTrigger(true)
	g.core.model.User.fogNightmareData:getAutoPlay():getCmdManager():pushCmd(var_35_5)
end
...
function FogNightmarePlayerBase:_doCheckNeedSelectKeepsake()
	if not g.core.model.User.fogNightmareData:getTowerData():getCurFloorStruct():getSelectedStage()[1]:checkCanSelectRandomKeepsake() then
		if g.core.model.User.fogNightmareData:getTalentData():getRandSealData() then
			local var_48_0 = FogNightmareCmdBase.new({
				trigger = function()
			...
			var_48_0:setQuickTrigger(true)
			g.core.model.User.fogNightmareData:getAutoPlay():getCmdManager():pushCmd(var_48_0)
...
function FogNightmarePlayerBase:_selectBestKeepsake(arg_52_1)
	...
	for iter_52_0, iter_52_1 in ipairs(arg_52_1) do
		...
		local var_52_2 = g.core.model.User.fogNightmareData:getFormationData():getKnightByServerId(iter_52_1.knight_id):getDevelopStruct("keepsake")
		local var_52_3 = 0

		if var_52_2 then
			var_52_3 = var_52_2:getCfg().power
		end
