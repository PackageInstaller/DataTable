local KnightSkinBookSpineComp = class("KnightSkinBookSpineComp", require("app.fairyGUI.knightSkinBook.UI_KnightSkinBookSpineComp"))
local BattleKnight = require("app.view.battle.BattleKnight")

function KnightSkinBookSpineComp:ctor()
	self._knightPic = nil
	self._knightPos = cc.p(self.m_picComp:getWidth() / 2, 0)
end

function KnightSkinBookSpineComp:updateComp(arg_2_1, arg_2_2)
	if not arg_2_1 then
		return
	end

	local var_2_0 = g.core.common.Path:getKnightSkinBookSpineCompBg(arg_2_2)
...
return KnightSkinBookSpineComp
