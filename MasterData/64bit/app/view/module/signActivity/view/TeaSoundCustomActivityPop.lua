...
local TeaSoundCustomActivityPop = class("TeaSoundCustomActivityPop", require("app.fairyGUI.signActivity.UI_TeaSoundCustomActivityPop"), function()
	return fgui.GComponent:create({
		pkgName = "signActivity",
		pkgPath = "ui/signActivity/signActivity",
		resName = "TeaSoundCustomActivityPop"
	})
end)

function TeaSoundCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.TEASOUND_SIGN_ACTIVITY

	self:showAtCenter()
	self:setSize(display.width, display.height)
	self.m_roleLoader:setURL("pic/signActivity/bg_3signly_juese3.png")
...
function TeaSoundCustomActivityPop:onLoad()
...
function TeaSoundCustomActivityPop:_updateMainView()
...
function TeaSoundCustomActivityPop:_updateTimeText()
...
function TeaSoundCustomActivityPop:_checkAutoSignAward()
...
function TeaSoundCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function TeaSoundCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function TeaSoundCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function TeaSoundCustomActivityPop:homeLayerAutoPopup()
...
return TeaSoundCustomActivityPop
