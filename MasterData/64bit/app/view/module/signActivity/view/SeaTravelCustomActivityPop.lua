...
local SeaTravelCustomActivityPop = class("SeaTravelCustomActivityPop", require("app.fairyGUI.signActivity.UI_SeaTravelCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "SeaTravelCustomActivityPop",
		pkgName = "signActivity",
		pkgPath = "ui/signActivity/signActivity"
	})
end)

function SeaTravelCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.SEATRAVEL_SIGN_ACTIVITY
...
function SeaTravelCustomActivityPop:onLoad()
...
function SeaTravelCustomActivityPop:_updateMainView()
...
function SeaTravelCustomActivityPop:_updateTimeText()
...
function SeaTravelCustomActivityPop:_checkAutoSignAward()
...
function SeaTravelCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function SeaTravelCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function SeaTravelCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function SeaTravelCustomActivityPop:homeLayerAutoPopup()
...
return SeaTravelCustomActivityPop
