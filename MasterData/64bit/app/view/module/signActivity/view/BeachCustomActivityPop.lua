...
local BeachCustomActivityPop = class("BeachCustomActivityPop", require("app.fairyGUI.signActivity.UI_BeachCustomActivityPop"), function()
	return fgui.GComponent:create({
		resName = "BeachCustomActivityPop",
		pkgName = "signActivity",
		pkgPath = "ui/signActivity/signActivity"
	})
end)

function BeachCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.BEACH_SIGN_ACTIVITY
...
function BeachCustomActivityPop:onLoad()
...
function BeachCustomActivityPop:_updateMainView()
...
function BeachCustomActivityPop:_updateTimeText()
...
function BeachCustomActivityPop:_checkAutoSignAward()
...
function BeachCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function BeachCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function BeachCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function BeachCustomActivityPop:homeLayerAutoPopup()
