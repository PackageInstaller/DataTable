...
local BuildCustomActivityPop = class("BuildCustomActivityPop", require("app.fairyGUI.signActivity.UI_BuildCustomActivityPop"), function()
	return fgui.GComponent:create({
		pkgName = "signActivity",
		pkgPath = "ui/signActivity/signActivity",
		resName = "BuildCustomActivityPop"
	})
end)

function BuildCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.BUILD_SIGN_ACTIVITY

	self:showAtCenter()
	self:setSize(display.width, display.height)
	self.m_roleLoader:setURL("pic/signActivity/bg_3signly_juese2.png")
...
function BuildCustomActivityPop:onLoad()
...
function BuildCustomActivityPop:_updateMainView()
...
function BuildCustomActivityPop:_updateTimeText()
...
function BuildCustomActivityPop:_checkAutoSignAward()
...
function BuildCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function BuildCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function BuildCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function BuildCustomActivityPop:homeLayerAutoPopup()
