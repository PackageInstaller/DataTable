...
local SilentGuardianCustomActivityPop = class("SilentGuardianCustomActivityPop", require("app.fairyGUI.signActivity.UI_SilentGuardianCustomActivityPop"), function()
	return fgui.GComponent:create({
		pkgPath = "ui/signActivity/signActivity",
		resName = "SilentGuardianCustomActivityPop",
		pkgName = "signActivity"
	})
end)

function SilentGuardianCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.SILENT_SIGN_ACTIVITY

	self:showAtCenter()
	self:setSize(display.width, display.height)
	self.m_roleLoader:setURL("pic/signActivity/bg_3signly_juese7.png")
...
function SilentGuardianCustomActivityPop:onLoad()
...
function SilentGuardianCustomActivityPop:_updateMainView()
...
function SilentGuardianCustomActivityPop:_updateTimeText()
...
function SilentGuardianCustomActivityPop:_checkAutoSignAward()
...
function SilentGuardianCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function SilentGuardianCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function SilentGuardianCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function SilentGuardianCustomActivityPop:homeLayerAutoPopup()
...
return SilentGuardianCustomActivityPop
