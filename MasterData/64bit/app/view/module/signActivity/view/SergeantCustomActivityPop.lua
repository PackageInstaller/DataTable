...
local SergeantCustomActivityPop = class("SergeantCustomActivityPop", require("app.fairyGUI.signActivity.UI_SergeantCustomActivityPop"), function()
	return fgui.GComponent:create({
		pkgName = "signActivity",
		pkgPath = "ui/signActivity/signActivity",
		resName = "SergeantCustomActivityPop"
	})
end)

function SergeantCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.SERGEANT_SIGN_ACTIVITY

	self:showAtCenter()
	self:setSize(display.width, display.height)
	self.m_roleLoader:setURL("pic/signActivity/bg_3signly_juese6.png")
...
function SergeantCustomActivityPop:onLoad()
...
function SergeantCustomActivityPop:_updateMainView()
...
function SergeantCustomActivityPop:_updateTimeText()
...
function SergeantCustomActivityPop:_checkAutoSignAward()
...
function SergeantCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function SergeantCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function SergeantCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function SergeantCustomActivityPop:homeLayerAutoPopup()
...
return SergeantCustomActivityPop
