...
local MagpieCustomActivityPop = class("MagpieCustomActivityPop", require("app.fairyGUI.signActivity.UI_MagpieCustomActivityPop"), function()
	return fgui.GComponent:create({
		pkgPath = "ui/signActivity/signActivity",
		resName = "MagpieCustomActivityPop",
		pkgName = "signActivity"
	})
end)

function MagpieCustomActivityPop:ctor(arg_2_1)
	if arg_2_1 and arg_2_1.endcall then
		self._closeCallBack = arg_2_1.endcall
	end

	self._groupId = 0
	self._functionId = var_0_2.MAGPIE_SIGN_ACTIVITY
...
function MagpieCustomActivityPop:onLoad()
...
function MagpieCustomActivityPop:_updateMainView()
...
function MagpieCustomActivityPop:_updateTimeText()
...
function MagpieCustomActivityPop:_checkAutoSignAward()
...
function MagpieCustomActivityPop:_onS2CGMActivityLoginGetAward(arg_7_1, arg_7_2, arg_7_3)
...
function MagpieCustomActivityPop:_onClickClose()
	self.m_backTransition:play(handler(self, self._onClose))
end

function MagpieCustomActivityPop:_onClose()
	g.core.module.ModuleManager:onlyPopSelfByDisplay(self)
	self:homeLayerAutoPopup()
end

function MagpieCustomActivityPop:homeLayerAutoPopup()
...
return MagpieCustomActivityPop
