...
local BuffEntry = class("BuffEntry", require("app.view.battle.entry.Entry"))
local var_0_4 = BattleConst.TWEEN_TYPE

function BuffEntry:ctor(arg_1_1)
	self:setData(arg_1_1)
end

function BuffEntry:setData(arg_2_1)
	self._actor = arg_2_1.actor
	self._img = arg_2_1.buffIcon
	self._buffDesc = arg_2_1.buffDesc
	...
	BuffEntry.super.ctor(self)
end

function BuffEntry:initEntry()
	self.super.initEntry(self)
	self.addEntryToNewQueue(self, self, self.step)
end

function BuffEntry:_init()
...
function BuffEntry:updateSpine()
	...
	self._buffIconLabel:setString(self._buffDesc)
	self._buffIconSprite:setTexture(var_5_0)
end

function BuffEntry:step()
...
function BuffEntry:_createSpine()
	local var_9_1 = {}
	...
	var_9_1.listener = handler(self, self._onSpineEvent)
...
function BuffEntry:_createHandler(arg_10_1)
	...
	local var_10_4 = cc.Label:createWithTTF(self._buffDesc, "font/SourceHanSerifCN-Heavy-4.otf", 28)
...
function BuffEntry:_onSpineEvent(arg_11_1)
	if arg_11_1.type == BattleConst.SPINE_EVENT.COMPLETE then
		-- block empty
	end
end

function BuffEntry:isDone()
	return self._isDone
end

function BuffEntry:clearNode()
	if self._spineEffect ~= nil then
		self:_clearSpineEffect()
	end
end

function BuffEntry:_clearSpineEffect()
...
function BuffEntry:destroyEntry()
	BuffEntry.super.destroyEntry(self)
	...
	if not tolua.isnull(self._spineEffect) then
		if self._battleLayer and g.core.battle.BattleProxy:isInBattle() then
			...
			if not tolua.isnull(self._spineEffect) then
				self._battleLayer:getBattleEntryManager():saveToPool(var_0_4.BUFF_TWEEN, self)
			end
		else
			self:_clearSpineEffect()
		end
