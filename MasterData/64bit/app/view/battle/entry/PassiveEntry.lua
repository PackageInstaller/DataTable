...
local PassiveEntry = class("PassiveEntry", Entry)

function PassiveEntry:ctor(arg_1_1)
	...
	PassiveEntry.super.ctor(self)
end

function PassiveEntry:initEntry()
...
function PassiveEntry:_showUI()
	if g.core.battle.BattleProxy:getBattleLayer():isCurPaused() then
		return false
	end

	local var_3_0 = fgui.UIPackage:createObject("battle", "BattlePassiveCurtainComp")
...
function PassiveEntry:isDone()
	return self._isDone
end

function PassiveEntry:destroyEntry()
	PassiveEntry.super.destroyEntry(self)
