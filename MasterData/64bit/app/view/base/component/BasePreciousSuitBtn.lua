...
function BasePreciousSuitBtn:_onTouchBegin(arg_3_1)
	...
	local var_3_0 = self:_checkPosInComp(arg_3_1:getInput():getTouch():getLocation(), false)

	if var_3_0 and var_3_0:getId() > 0 then
		self._selectedComp = var_3_0
		self._moveComp = fgui.UIPackage:createObject("base_new", "BasePreciousSuitIcon")

		self._moveComp:becomeTo(var_3_0)
		self._moveComp:setTouchable(false)
		self._moveComp:setVisible(false)
		self:addChild(self._moveComp, 1)

		local var_3_1 = self.m_preciousSuitList:getPosition()
		local var_3_2 = self._selectedComp:getPosition()

		var_3_1.x = var_3_1.x + var_3_2.x
		var_3_1.y = var_3_1.y + var_3_2.y

		self._moveComp:setPosition(var_3_1)

		self._movePos = var_3_1
...
function BasePreciousSuitBtn:_onTouchEnd(arg_5_1)
	...
	if self._selectedComp then
		...
		if self._suitStartDragStationPos > 0 then
			...
			local var_5_0 = self:_checkPosInComp(arg_5_1:getInput():getTouch():getLocation(), true)

			if var_5_0 then
				local var_5_1 = var_5_0:getStationPos()
