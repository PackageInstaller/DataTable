...
local var_0_13 = {
	__index = function(self, arg_1_1)
		...
		if arg_1_1 == "toObject" then
			...
			assert(var_0_9[arg_1_1], "cannot find " .. arg_1_1 .. " in record_fog_idle_info")
...
function var_0_7._getData()
	if not var_0_7._originData then
		var_0_7._originData = loadCfg("core.config.cfg.fog_idle_info_data")
...
function var_0_7._getKey()
	if not var_0_8 then
		var_0_8 = loadCfg("core.config.cfg.fog_idle_info_key")
...
function var_0_7.checkDataClear()
	if var_0_7._lastUseTime > 0 and os.time() - var_0_7._lastUseTime > g.core.const.ConstMgr.BASE_CONST.CONFIG_CHECK_DATA_DELTA then
		...
		package.loaded["app.core.config.cfg.fog_idle_info_data"] = nil
		package.loaded["app.core.config.cfg.fog_idle_info_key"] = nil
	end
end
