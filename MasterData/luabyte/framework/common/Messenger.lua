...
local function AddListener(self, e_type, e_listener, ...)
	...
	for k, v in pairs(event) do
		if k == e_listener then
			Logger.LogError("Aready cotains listener : "..tostring(e_listener))
			return e_listener 
		end
	end
	...
	--保存调用堆栈
	if isDebug then 
		local debugTarget = event[e_listener]
		if debugTarget then 
			debugTarget.trace = e_type ..":".. debug.traceback()
		end
	end
	return e_listener 
end
...
local function Debug(self)
	...
	-- body
	for e_type, event in pairs(self.events) do
		-- body
		for e_listener, metaTarget in pairs(event) do
			-- body
			Logger.Log("未移除: " .. tostring(metaTarget.trace))
		end
	end
end
