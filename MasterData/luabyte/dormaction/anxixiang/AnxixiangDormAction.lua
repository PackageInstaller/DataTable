local AnxixiangDormAction = BaseClass("AnxixiangDormAction", require("DormAction.BaseDormAction"))
local M = AnxixiangDormAction
...
function M:DressUpCallBack(type)
    ...
    if type == DormConst.FurnitureType.GroupPhotoWall then
        local furnitureDatas = DormHelper.GetFurnitureDatasByAreaName("Photowall3")
        self:GroupPhotoWallCallBack(furnitureDatas)
    end
...
function M:MainCommandCallback(furnitureDatas)
    ...
    self.commandAction = require("DormAction.Anxixiang.AnxixiangMainAction").New(furnitureDatas)
end
...
function M:BedcommandCallback(furnitureDatas)
    ...
    self.commandAction = require("DormAction.Anxixiang.AnxixiangBedAction").New(furnitureDatas)
end
...
function M:SofacommandCallback(furnitureDatas)
    ...
    self.commandAction = require("DormAction.Anxixiang.AnxixiangSofaAction").New(furnitureDatas)
end
...
function M:ConsoleCallback(furnitureDatas)
    ...
    self.commandAction = require("DormAction.Anxixiang.AnxixiangConsoleAction").New(furnitureDatas)
end

--照片墙指令回调
function M:GroupPhotoWallCallBack(furnitureDatas)
    if self.commandAction then
        self.commandAction:Dispose()
        self.commandAction = nil
    end

    self.commandAction = require("DormAction.Anxixiang.AnxixiangGroupPhotoWallAction").New(furnitureDatas)
end
...
function M:RoleClickEventHandle(handler)
    ...
    if DormHelper.CheckGift(self.heroId, self.systemId) == false and self.state == DormConst.RoleState.Command then
...
function M:CheckFurnitureArea(Area)
    ...
    if Area.SimpleAreaName == DormConst.DormArea.Photowall3Area.areaName and
        DormHelper.GetCurrentWatchPointFurnitureName() ~= DormConst.FurnitureName.Photowall3 then
        self:CommanderMoveToFurniture(DormConst.FurnitureName.Photowall3)
        return
    end
end
...
function M:SwitchFurnitureAction(FurnitureName, data)
    ...
    if FurnitureName == DormConst.FurnitureName.Photowall3 then
        self:GroupPhotoWallCallBack(data)
        return
    end
end
