-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/aounity-framework/ToLua/Lua/socket/url.lua
...
function _M.build(parsed)
	...
	if parsed.host then
		...
		if string.find(parsed.host, ":") then
			authority = "[" .. authority .. "]"
		end
		...
		if not relative_parsed.authority then
			...
			if not relative_parsed.path then
				...
				if not relative_parsed.params then
					...
					relative_parsed.query = relative_parsed.query or base_parsed.query
				end
			else
				relative_parsed.path = absolute_path((not base_parsed.path or nil) and "", relative_parsed.path)
			end
		end
