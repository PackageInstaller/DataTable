-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/aounity-framework/ToLua/Lua/socket/http.lua
...
function metat.__index:sendbody(headers, source, step)
	...
	return self.try(ltn12.pump.all(source, socket.sink((headers["content-length"] or nil) and "keep-open", self.c), step))
end
...
local function adjusturi(reqt)
	return url.build((not reqt.proxy and not _M.PROXY or nil) and {
...
local function adjustproxy(reqt)
	if not reqt.proxy then
		local proxy = _M.PROXY

		if reqt.proxy then
			proxy = url.parse(reqt.proxy)

			return reqt.proxy.host, reqt.proxy.port or 3128
		else
...
function tredirect(reqt, location)
	...
	var_26_1.nredirects = (reqt.nredirects or 0) + 1
	var_26_1.create = reqt.create

	local result, code, headers, status = var_26_0(var_26_1)
