-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/aounity-framework/ToLua/Lua/UnityEngine/Profiler.lua
		...
		if top > 0 then
			...
			if ar then
				...
				if top < index then
					...
					if ar then
						func = ar.func
						index = findstack(ar.func) or index
					end
				end
		...
		if event == "call" then
			...
			elseif name then
				...
				if ar.short_src == "[C]" then
					...
					if not index then
						...
						elseif linedefined ~= -1 then
							...
							BeginSample(ar.short_src .. linedefined, func, cache)
						end
					else
				...
				elseif linedefined ~= -1 or method then
					...
					local name = short_src:match("([^/\\]+)%.%w+$") or short_src:match("([^/\\]+)$")
					local ffname = ffnames[name]

					name = ffname and ffname[linedefined] or name .. "." .. method
