-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/aounity-framework/ToLua/Lua/protobuf/protobuf.lua
...
local function _AttachFieldHelpers(message_meta, field_descriptor)
	...
	if field_descriptor.has_options then
		local is_packed = field_descriptor.GetOptions().packed
...
local function _InitMethod(message_meta)
	return function()
		...
		self._cname = message_meta._descriptor.name
...
local function _AddSetListenerMethod(message_meta)
	function message_meta._member:_SetListener(listener)
		self._listener = listener ~= nil and listener_mod.NullMessageListener() or listener
	end
end
...
local function _AddSerializePartialToStringMethod(message_descriptor, message_meta)
	...
	function message_meta._member:SerializePartialToIOString(iostring)
	...
	function message_meta._member:SerializePartialToString()
		local out = {}
...
local function _AddMergeFromStringMethod(message_descriptor, message_meta)
	...
	local function _internal_parse(self, buffer, pos, pend)
		...
		while pos ~= pend do
			...
			if decoders_by_tag[tag_bytes] == nil then
				...
				pos = decoders_by_tag[tag_bytes](buffer, new_pos, pend, self, field_dict)
			end
		end
...
local function _AddIsInitializedMethod(message_descriptor, message_meta)
	...
	function message_meta._member:FindInitializationErrors()
		...
		for field, value in message_meta._member.ListFields(self) do
			if field.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE then
				name = field.is_extension and string.format("(%s)", field.full_name) or field.name
