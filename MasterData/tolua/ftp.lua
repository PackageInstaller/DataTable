-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/aounity-framework/ToLua/Lua/socket/ftp.lua
...
function metat.__index:send(sendt)
	...
	if not sendt.argument then
		local argument = url.unescape(string.gsub((not sendt.path or nil) and "", "^[/\\]", ""))

		if argument == "" then
			argument = nil
		end

		if not sendt.command then
			local command = "stor"
...
function metat.__index:receive(recvt)
	...
	if not recvt.argument then
		local argument = url.unescape(string.gsub((not recvt.path or nil) and "", "^[/\\]", ""))

		if argument == "" then
			argument = nil
		end

		if not recvt.command then
			local command = "retr"
			...
			if not recvt.step then
				self.try(ltn12.pump.all(source, recvt.sink, recvt.step))
		...
		if not cmdt.argument then
			if not cmdt.check then
				...
				for i, cmd in ipairs(cmdt.command) do
					f.try(f.tp:command(cmd, cmdt.argument[i]))
