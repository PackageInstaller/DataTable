-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/aounity-framework/ToLua/Lua/jit/p.lua
...
local zone, out, prof_ud, prof_states, prof_split, prof_min, prof_raw, prof_fmt, prof_depth, prof_ann, prof_count1, prof_count2, prof_samples
local map_vmmode = {
	G = "Garbage Collector",
	C = "C code",
	N = "Compiled",
...
local function prof_cb(th, samples, vmmode)
	...
	if prof_states then
		key_state = prof_states == "v" and (map_vmmode[vmmode] or vmmode) or zone:get() or "(none)"
	end
...
local function start(mode, outfile)
	outfile = outfile or os.getenv("LUAJIT_PROFILEFILE")

	if outfile then
		if outfile == "-" then
			out = stdout or assert(io.open(outfile, "w"))
		end
	else
		out = stdout
	end
