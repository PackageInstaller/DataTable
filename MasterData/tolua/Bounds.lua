-- chunkname: @C:/GitLab-Runner/builds/sTUwNpCg/0/aqmobile/aqmobile-client/UnityProj/Assets/aounity-framework/ToLua/Lua/UnityEngine/Bounds.lua
...
function Bounds:IntersectRay(ray)
	...
	for i = 1, 3 do
		if (p[i] + extent[i]) * (1 / dir[i]) < (p[i] - extent[i]) * (1 / dir[i]) then
			if tmin < (p[i] + extent[i]) * (1 / dir[i]) then
				tmin = (p[i] + extent[i]) * (1 / dir[i])
			end

			if (p[i] - extent[i]) * (1 / dir[i]) < tmax then
				tmax = (p[i] - extent[i]) * (1 / dir[i])
			end
