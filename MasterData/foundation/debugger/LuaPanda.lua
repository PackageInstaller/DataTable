...
function this.testBreakpoint()
  if recordBreakPointPath and "" ~= recordBreakPointPath then
    do return end
    return this.breakpointTestInfo, nil, nil
  else
    local strTable = {}
...
Breakpoint: ]] .. recordBreakPointPath
  ...
  do return table.concat end
  return table.concat, strTable, NormalizedPath, 1, true
end
...
function this.getBaseInfo()
  ...
  do return table.concat end
  return table.concat, strTable, OSTypeErrTip, "\n", clibStr, math.ceil(forluaVer)
end
...
filepath: ]] .. key
  ...
  do return table.concat end
  return table.concat, strTable, consoleLogLevel, ", 过低的日志等级会降低调试速度，建议调整LuaPanda.lua文件头部consoleLogLevel=2", "\n找不到文件:" .. runSource .. ", 请检查路径是否正确。\n或者在VSCode文件" .. pathArray[#pathArray] .. "中打一个断点后，再执行一次doctor命令，查看路径分析结果。", runSource, ", 请检查路径是否正确。\n或者在VSCode文件", pathArray[#pathArray], "中打一个断点后，再执行一次doctor命令，查看路径分析结果。", #strTable + 1, [[

filepath: ]] .. key, key, 1, true
end
...
Use 'LuaPanda.getBreaks()' to watch.]]
  do return table.concat end
  return table.concat, strTable, pathErrTip, "\n", this.getPath(DebuggerFileName), "\n"
end
...
function this.revSubString(str, subStr, plain)
  ...
  do return string.sub, str, idx + 1 end
...
function this.dataProcess(dataStr)
  ...
  elseif dataTable.cmd == "initSuccess" then
    ...
    if "true" == tostring(dataTable.info.useCHook) then
      userSetUseClib = true
      if true == isUserSetClibPath then
        ...
        if nil ~= luapanda_chook then
          ...
          if not requireCLibSuccess and not this.tryRequireClib("libpdebug", x64Path) and not this.tryRequireClib("libpdebug", x86Path) then
            this.printToVSCode("Require clib failed, use Lua to continue debug, use LuaPanda.doctor() for more information.", 1)
          end
        end
      end
...
function this.getFilenameFromPath(path)
  ...
  do return string.match, path end
...
function this.getVariable(checkLayer, isFormatVariable, offset)
  ...
  repeat
    ...
    if "(*temporary)" ~= tostring(n) and tostring(n) ~= "(temporary)" then
      local var = {}
      var.name = n
      var.type = tostring(type(v))
      var.variablesReference = "0"
...
function tools.show(t, name, indent)
  ...
  local function basicSerialize(o)
    ...
    if type(o) == "function" then
      ...
      if info.what == "C" then
        do return string.format, "%q" end
        return string.format, "%q", so .. ", C function", ", C function", nil, nil, nil, nil, nil
      else
        do return string.format, "%q" end
        return string.format, "%q", so .. ", defined in (" .. info.linedefined .. "-" .. info.lastlinedefined .. ")" .. info.source, ", defined in (", info.linedefined, "-", info.lastlinedefined, ")", info.source
      end
    elseif type(o) == "number" or type(o) == "boolean" then
      return so
    else
      do return string.format, "%q" end
      return string.format, "%q", so, so .. ", defined in (" .. info.linedefined .. "-" .. info.lastlinedefined .. ")" .. info.source, ", defined in (", info.linedefined, "-", info.lastlinedefined, ")", info.source
    end
  end
...
function tools.createJson()
  ...
  function json.encode(v)
    ...
    if "number" == vtype or "boolean" == vtype then
      ...
      return tostring, v, "\"", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil
    end
    if "table" == vtype then
      local rval = {}
  ...
  function json.decode(s, startPos)
    ...
    if "{" == curChar then
      do return decode_scanObject, s end
      return decode_scanObject, s, startPos, "]", nil
    end
    if "[" == curChar then
      do return decode_scanArray, s end
      return decode_scanArray, s, startPos, "]", nil
    end
    if string.find("+-0123456789.e", curChar, 1, true) then
      do return decode_scanNumber, s end
      return decode_scanNumber, s, startPos, 1, true
    end
    if "\"" == curChar or "'" == curChar then
      do return decode_scanString, s end
      return decode_scanString, s, startPos, 1, true
    end
    if string.sub(s, startPos, startPos + 1) == "/*" then
      do return json.decode, s, decode_scanComment(s, startPos) end
      return json.decode, s, decode_scanComment(s, startPos)
    end
    do return decode_scanConstant, s end
    return decode_scanConstant, s, startPos, decode_scanComment(s, startPos)
  end
...
function tools.base64encode(data)
  return (data:gsub(".", function(x)
    ...
    do return base64CharTable.sub, base64CharTable, c + 1 end
    return base64CharTable.sub, base64CharTable, c + 1, c + 1, x:sub(i, i) == "1" and 2 ^ (6 - i) or 0, 2, i, i
  end) .. ({
    "",
    "==",
    "="
...
function tools.base64decode(data)
  ...
  return (data:gsub(".", function(x)
    ...
    return string.char, c, 1, nil, x:sub(i, i) == "1" and 2 ^ (8 - i) or 0, 2, i, i
  end))
end
