...
function sproto.new(bin)
  ...
  do return setmetatable, self end
...
function sproto:host(packagename)
  ...
  local obj = {
    __proto = self,
    __package = assert(core.querytype(self.__cobj, packagename), "type package not found:" .. packagename),
    __session = {}
  }
  do return setmetatable, obj end
  return setmetatable, obj, host_mt, packagename
end
...
function sproto:response_encode(protoname, tbl)
  ...
  if response then
    do return core.encode, response end
...
function sproto:response_decode(protoname, ...)
  ...
  local response = p.response
  if response then
    do return core.decode, response, ... end
...
function sproto:default(typename, type)
  if nil == type then
    do return core.default, querytype(self, typename) end
    return core.default, querytype(self, typename)
  else
    local p = queryproto(self, typename)
    if "REQUEST" == type then
      if p.request then
        do return core.default end
        return core.default, p.request, querytype(self, typename)
      end
    elseif "RESPONSE" == type then
      if p.response then
        do return core.default end
        return core.default, p.response, querytype(self, typename)
      end
    else
      error("Invalid type")
    end
...
local function gen_response(self, response, session)
  return function(args, ud)
    ...
    if response then
      ...
      do return core.pack end
      return core.pack, header .. content, content
    else
      do return core.pack end
      return core.pack, header, header .. content, content
    end
  end
end
