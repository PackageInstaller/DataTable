...
local function multipat(pat)
  do return Ct end
  return Ct, blank0 * (pat * blanks) ^ 0 * pat ^ 0 * blank0, blank0
end

local function namedpat(name, pat)
  do return Ct end
  return Ct, Cg(Cc(name), "type") * Cg(pat), Cg(pat), pat
end
...
local function checktype(types, ptype, t)
  ...
  if types[fullname] then
    ...
    ptype = ptype:match("(.+)%..+$")
    if ptype then
      do return checktype, types, ptype end
...
local function parser(text, filename)
  ...
  do return flattypename, check_protocol(adjust(r)) end
...
local function packfield(f)
  ...
  do return packbytes, table.concat(strtbl) end
...
local function packtype(name, t, alltypes)
  ...
  do return packbytes, table.concat(data) end
...
local function packproto(name, p, alltypes)
  ...
  do return packbytes, table.concat(tmp) end
...
local function packgroup(t, p)
  ...
  return table.concat, result, "\000\000", "\000\000", tt, tp, table.concat(tp)
end

local function encodeall(r)
  do return packgroup, r.type end
