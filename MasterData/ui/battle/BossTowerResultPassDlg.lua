-- chunkname: @E:/AntMan/AntMan_Android_pub1/Assets/Code/.LuaCode/UI\\Battle\\BossTowerResultPassDlg.lua
...
local strClassName = "BossTowerResultPassDlg"
local BossTowerResultPassDlg = Class(strClassName, UIControls.Window)

function BossTowerResultPassDlg:ctor()
	self:initUI()
...
function BossTowerResultPassDlg:initUI()
...
function BossTowerResultPassDlg:onShow(battleType, result, finish, itemBonus)
...
function BossTowerResultPassDlg:bgmOn()
	...
	if battleState and battleState.relicHero then
		...
		BossTowerResultPassDlg.super.bgmOn(self)
	end
end

function BossTowerResultPassDlg:setBossTowerData(...)
...
function BossTowerResultPassDlg:_getBossBattleResult(settle)
...
function BossTowerResultPassDlg:setWorldBossData(...)
	self.textHurt:setVisible(false)
	self.hurtTitle:setVisible(false)
...
function BossTowerResultPassDlg:_startStageBattle(battleInfo)
	local time = 0
...
function BossTowerResultPassDlg:onBtnRecord()
	UIManager.getUI("battleRecordDlg", true):onShow(self.isZombie or false)
end

function BossTowerResultPassDlg:onBtnAgain()
	...
	if battleState.battlePreInfo then
		CurAvatar:enterFormation(battleState.battleNo, BattleConst.BATTLE_TYPE_BOSSTOWER, battleState.battlePreInfo)
		self:setVisible(false)
	end
end

function BossTowerResultPassDlg:onBtnNext()
	UIManager.getUI("zombieResultAwardDlg", true):show(self.itemBonus, nil, self.bonus)
	self:setVisible(false)
end

function BossTowerResultPassDlg:playNextAni()
	self:playAni("ChangeBattleResultBossPerfect", nil)
end

function BossTowerResultPassDlg:destroy()
	BossTowerResultPassDlg.super.destroy(self)
end

return BossTowerResultPassDlg
