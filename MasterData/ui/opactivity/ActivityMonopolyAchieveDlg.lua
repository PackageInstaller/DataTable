-- chunkname: @E:/AntMan/AntMan_Android_pub1/Assets/Code/.LuaCode/UI\\OpActivity\\ActivityMonopolyAchieveDlg.lua
...
local ActivityMonopolyAchieveCell = Class("ActivityMonopolyAchieveCell", UIControls.Child)
local SOURCEIMG_PATH = "Atlas/ActivityAtlas/ActivityAtlas01"

function ActivityMonopolyAchieveCell:ctor()
...
function ActivityMonopolyAchieveCell:setData(achieveData, state, isNextToGet, needIconPath)
...
function ActivityMonopolyAchieveCell:_refrehState(needIconPath)
	local uiState = 0
...
local strClassName = "ActivityMonopolyAchieveDlg"
local ActivityMonopolyAchieveDlg = Class(strClassName, UIControls.Window)

MixinClass(ActivityMonopolyAchieveDlg, ActivityPanelMixin)

function ActivityMonopolyAchieveDlg:initUI()
	self.imgItem = UIControls.Image(self, "BgPanel/HavePanel/Icon")
	self.txtItemNum = UIControls.Label(self, "BgPanel/HavePanel/TextNum")
	self.textRound = UIControls.Label(self, "BgPanel/HavePanel/TextRoundNum")
	self.btnClose = UIControls.Button(self, "BgPanel/BtnClose")

	self.btnClose:addEventClick(self.onBtnClose)

	self.panelMaxRound = UIControls.Panel(self, "BgPanel/HavePanel/BgTips")
	self.cells = {}
	self.achieveData = {}
	...
	for index = 1, #self.achieveData do
		local newCell = ActivityMonopolyAchieveCell(self, "BgPanel/AchiPanel", "System/Activity/ActivityMonopoly/ActivityMonopolyAchiCell")
...
function ActivityMonopolyAchieveDlg:_setData()
	local svrData
	...
	local curRound = (svrData and svrData.round or 0) + 1

	self.textRound:setText(curRound)
	self.panelMaxRound:setVisible(self.maxRound and curRound >= self.maxRound)
...
function ActivityMonopolyAchieveDlg:refreshRealPanel()
	return
end

function ActivityMonopolyAchieveDlg:onBtnClose()
	self:setVisible(false)
end

return ActivityMonopolyAchieveDlg
