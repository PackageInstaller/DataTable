-- chunkname: @E:/AntMan/AntMan_Android_pub1/Assets/Code/.LuaCode/UI\\Welfare\\NewBeeChild.lua

local NewBeeChild = Class("NewBeeChild", UIControls.Child)
local CURRENCY = {
	[1] = "IconRMB"
}

function NewBeeChild:ctor()
...
function NewBeeChild:initData(data)
	return
end

function NewBeeChild:initData(data, svrData)
	self:setData(data[1], self.mParent.actObj)
end

function NewBeeChild:setData(data, preNestedId, actObj)
	self.actObj = actObj
	self.data = data
...
function NewBeeChild:onBtnClick()
