local UIActivityFlipCardTaskList = class("UIActivityFlipCardTaskList", UIBaseWindow)
local base = UIBaseWindow
local UINFlipCardTaskItem = require("Game.ActivityFlipCard.UI.UINFlipCardTaskItem")
local JumpManager = require("Game.Jump.JumpManager")

function UIActivityFlipCardTaskList:OnInit()
...
function UIActivityFlipCardTaskList:InitFlipCardTask(taskIdlist, allGetCallback, singleGetCallback)
  self._taskIdDic = {}
...
function UIActivityFlipCardTaskList:__Refill()
  self._taskDataList = {}
  local totalCount = 0
  local finishCount = 0
...
function UIActivityFlipCardTaskList:__RefillFromExternal()
  if not IsNull(self.transform) then
    self:__Refill()
  end
end

function UIActivityFlipCardTaskList:SetChristmasEnvTaskTitle(titleName)
  self.ui.tex_SkillName.text = titleName
end

function UIActivityFlipCardTaskList:__OnNewItem(go)
  local taskItem = UINFlipCardTaskItem.New()
  taskItem:Init(go)
  self._itemDic[go] = taskItem
end

function UIActivityFlipCardTaskList:__OnChangeItem(go, index)
  local taskItem = self._itemDic[go]
  local taskData = self._taskDataList[index + 1]
  taskItem:InitFlipCardTaskItem(taskData, self.__OnGetTaskDataCallback, self.__OnGotoTaskDataCallback)
  self._taskItemDic[taskData.id] = taskItem
end

function UIActivityFlipCardTaskList:__OnGetTaskData(taskData)
...
function UIActivityFlipCardTaskList:__OnGotoTaskData(taskData)
...
function UIActivityFlipCardTaskList:__TaskProcessUpdate(taskData)
  ...
  if item ~= nil and item:GetEnvTaskId() == taskData.id then
    item:RefreshFlipCardTaskItem()
  end
end

function UIActivityFlipCardTaskList:__TaskProcessDelete(taskID)
...
function UIActivityFlipCardTaskList:OnClickBack()
  UIUtil.OnClickBackByUiTab(self)
end

function UIActivityFlipCardTaskList:OnCloseTask()
  self:Delete()
end

function UIActivityFlipCardTaskList:OnDelete()
  self._itemDic = nil
...
return UIActivityFlipCardTaskList
