...
function UIDungeonStateInfo:OnInit()
  ...
  self.__onRareMoneyChange = BindCallback(self, self.RefreshRareMoney)
  ...
  MsgCenter:AddListener(eMsgEventId.OnRareMoneyChanged, self.__onRareMoneyChange)
  ...
  self:_InitChipListSeq()
end

function UIDungeonStateInfo:InitHeroAndChip(dynPlayer)
  self.heroList:InitHeroList(dynPlayer, self.resloader, self.chipList.selectHeroCallback, dynPlayer:GetEpBuffList())
...
function UIDungeonStateInfo:AddDropBuff(dropBuffId)
  self.buffList:AddDropBuff(dropBuffId)
end
    ...
    if dynplayer ~= nil then
      ...
      if theMoney ~= nil then
        ...
        if BattleUtil.IsInBrotatBattle() then
          money = money + dynplayer:GetCacheMoneyCount()
        end
      end
    end
  end
...
function UIDungeonStateInfo:SetRareMoneyActive(active)
  self.ui.obj_rareMoney:SetActive(active)
  if active then
    self:RefreshRareMoney()
  end
end
...
function UIDungeonStateInfo:RefreshRareMoney()
  local money = 0
  if self.dynplayer ~= nil then
    local theMoney = self.dynplayer:GetItemCount(ConstGlobalItem.RareMoney)
    if theMoney ~= nil then
      money = theMoney
    end
  end
  self.ui.tex_rareMoney.text = math.floor(money)
  local isShow = money ~= 0
  if self.ui.obj_rareMoney.activeSelf ~= isShow then
    self.ui.obj_rareMoney:SetActive(isShow)
  end
end
...
function UIDungeonStateInfo:__StartShowGetChipAni()
  ...
  self.chipList:ScrollChipItemInScreen(chipData, function(targetChipItem)
    if targetChipItem ~= nil then
      if self.isPlayPopTween == true then
        self.__tweenChipListPop:Complete(false)
      end
...
function UIDungeonStateInfo:StopAllGetChipAni()
  self.getChipAniCache = {}
  self.__startGetChipAni = false
  self.chipList:SetListBlocksRaycasts(true)
end
...
function UIDungeonStateInfo:_InitChipListSeq()
  self.__tweenChipListPop = self:__ChipListPop()
  self.__tweenChipListPush = self:__ChipListPush()
end

function UIDungeonStateInfo:__ChipListPop(seq)
  seq = seq or CS_DOTween:Sequence()
  local localPos = self.ui.top_info.transform.localPosition
  seq:AppendCallback(function()
    local battleWin = UIManager:GetWindow(UIWindowTypeID.Battle)
    if battleWin and battleWin.gameplayScore then
      battleWin.gameplayScore:PlaySeqUpToMid()
    end
  end):Append(self.ui.top_info.transform:DOLocalMove(Vector2.New(localPos.x, localPos.y + 160), 0)):Append(self.ui.top_info.transform:DOLocalMove(Vector2.New(localPos.x, localPos.y), 0.5)):SetAutoKill(false):SetUpdate(true):Pause():SetLink(self.ui.top_info)
  
  function seq.onComplete()
    self.buffList:SetBuffDetailFloatAlign(nil, VAType.up)
  end
  
  return seq
end

function UIDungeonStateInfo:__ChipListPush(seq)
  seq = seq or CS_DOTween:Sequence()
  local localPos = self.ui.top_info.transform.localPosition
  seq:AppendCallback(function()
    local battleWin = UIManager:GetWindow(UIWindowTypeID.Battle)
    if battleWin and battleWin.gameplayScore then
      battleWin.gameplayScore:PlaySeqMidToUp()
    end
  end):Append(self.ui.top_info.transform:DOLocalMove(Vector2.New(localPos.x, localPos.y), 0)):Append(self.ui.top_info.transform:DOLocalMove(Vector2.New(localPos.x, localPos.y + 160), 0.5)):SetAutoKill(false):SetUpdate(true):Pause():SetLink(self.ui.top_info)
  
  function seq.onComplete()
    self.buffList:SetBuffDetailFloatAlign(nil, VAType.down, 0, -5)
    self.isPlayPopTween = false
  end
  
  return seq
end

function UIDungeonStateInfo:PlayPopChipList()
  if self.__tweenChipListPop then
    self.__tweenChipListPop:Restart()
    self.isPlayPopTween = true
  end
end

function UIDungeonStateInfo:PlayPushChipList()
  if self.__tweenChipListPush then
    self.__tweenChipListPush:Restart()
  end
end
...
function UIDungeonStateInfo:OnDelete()
  ...
  MsgCenter:RemoveListener(eMsgEventId.OnRareMoneyChanged, self.__onRareMoneyChange)
  self.__tweenChipListPop:Kill()
  self.__tweenChipListPush:Kill()
  base.OnDelete(self)
end
