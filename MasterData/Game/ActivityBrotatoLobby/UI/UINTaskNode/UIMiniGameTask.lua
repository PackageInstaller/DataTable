local UISectorTask = require("Game.Sector.SectorTask.UISectorTask")
local UIMiniGameTask = class("UIMiniGameTask", UISectorTask)
local base = UISectorTask
local TaskEnum = require("Game.Task.TaskEnum")
local TinyGameEnum = require("Game.TinyGames.TinyGameEnum")
UIMiniGameTask.ETaskUIType = {Brotato = 1}

function UIMiniGameTask:OnBrotatoChange(brotatoData)
...
function UIMiniGameTask:AddTaskMsgListener()
...
function UIMiniGameTask:RemoveTaskMsgListener()
  base.RemoveTaskMsgListener(self)
  MsgCenter:RemoveListener(eMsgEventId.ActivityTinyGameBrotatoChange, self.__OnBrotatoChange)
end

function UIMiniGameTask:__RefreshAchivTaskUI()
  if self.taskUIType == self.ETaskUIType.Brotato then
    local taskCtrl = ControllerManager:GetController(ControllerTypeId.Task)
    local taskDataDic, _ = taskCtrl:GetDatas4Task(self.taskType)
...
function UIMiniGameTask:__RefillTaskList()
...
function UIMiniGameTask:OnClickPeriodItem(item, index)
  local data = self.sectorAchivDatas[index]
  if self.taskUIType == self.ETaskUIType.Brotato then
    if data.stateType == TaskEnum.eTaskState.Completed then
      local actFrameId = self.brotatoData:GetBrotatoActivityFrameId()
      if actFrameId ~= 0 then
        local miniGameNet = NetworkManager:GetNetwork(NetworkTypeID.TinyGame)
        miniGameNet:CS_TinyGame_Reward(actFrameId, index, TinyGameEnum.eType.brotato)
      end
    else
...
function UIMiniGameTask:InitBrotatoTask(brotatoData, closeCallback)
  self.taskUIType = self.ETaskUIType.Brotato
  self.brotatoData = brotatoData
  self.taskType = self.brotatoData:GetBrotatoTaskType()
