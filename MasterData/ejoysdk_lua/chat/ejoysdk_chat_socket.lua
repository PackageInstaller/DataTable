...
function M:get_server()
  return utils.deepcopy(self.server or {}), "TCP"
end
...
function M:connect()
  ...
  if _ejoysdk.os() == "ios" and e_lresolve and self.can_use_resolve then
    ...
    E.LOG.debug(TAG, "chat_connect: lsocket connect, addr:" .. tostring(self.server.addr) .. ", port:" .. tostring(self.server.port))
...
function M:read()
  ...
  while true do
    ...
    if succ then
      ...
      if nil == p then
        self:socket_error(err or "remote end closed the connection", error_code or CONSTANTS.CHAT_ERROR_CODES.CODE_REMOTE_END_CLOSED_CONNECTION)
        return
      end
...
function M:on_message(recv)
  ...
  local ret, code = self.handlers.on_message(pack)
  if ret then
    return ret, n
  else
    return ret, code or -1
  end
end
