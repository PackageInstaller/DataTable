...
local IP = require("ejoysdk_lua.album.ejoysdk_image_process")
...
local TICK_DURATION_ENTER_FROM_BACKGROUND_SEC = 10
...
function M:_init(handler, init_param)
  ...
  self.sync_top_session_cache = init_param.sync_top_session_cache or false
  ...
  self.reply_msg_limit = init_param.reply_msg_limit or 600
...
function M:_on_error(_error_msg, _error_code)
  ...
  self.rpc_process_time[rpc_process_type.init_connect] = nil
  local err_duration = os.time() - self.last_tick_time
  if err_duration >= TICK_DURATION_ENTER_FROM_BACKGROUND_SEC then
    E.LOG.debug(TAG, "on_error received and reset retry_connect_index, dur:" .. tostring(err_duration))
    self:reset_retry_connect_index()
  else
    E.LOG.debug(TAG, "on_error received and skip reset retry_connect_index, dur:" .. tostring(err_duration) .. ", retry_index:" .. tostring(self.retry_connect_index))
  end
...
function M:retry_connect(_code)
  ...
  local delay_time = 0 == self.retry_connect_index and 0.1 or self:retry_connect_delay_time(self.retry_connect_index)
    ...
    if 0 == ret.code and ret.sessions then
      EC.replace_latest_sessions_user_info_before_cb(ret.sessions, function(replaced_sessions)
        ...
        for _, session in pairs(replaced_sessions) do
          ...
          EC._fill_session_info_top(destination, session_info)
        ...
        E.LOG.debug(TAG, "get_latest_session, after, size:" .. tostring(ret.sessions and #ret.sessions or 0) .. ", destination:" .. tostring(destination))
...
function M:set_session_attr(session_attrs)
  local destination = get_destination(self.user_type)
  if session_attrs then
    for session_id, attrs in pairs(session_attrs) do
      local s = self.sessions[session_id]
      if s and type(attrs.top) == "number" then
        s.session_info = s.session_info or {}
        s.session_info.top = attrs.top
        E.LOG.debug(TAG, "set_session_attr changed, top:" .. tostring(attrs.top) .. ", for id:" .. tostring(session_id))
      end
    end
  end
  do return EC.set_session_attrs, destination end
  return EC.set_session_attrs, destination, session_attrs, pairs(session_attrs)
end
...
function M:_get_latest_session_cb(ret)
  ...
  if 0 == ret.code and ret.sessions then
    ...
    if self.sync_blank_group_sessions and self.sync_group_flag == false then
      for group_id, group in pairs(self.groups or {}) do
        if not adds[group_id] then
          ...
          local destination = get_destination(self.user_type)
          EC._fill_session_info_top(destination, session.session_info)
        end
      end
    ...
    E.LOG.debug(TAG, "_get_latest_session_cb, adds:" .. tostring(E.Utils.table_size(adds)) .. ", updates:" .. tostring(E.Utils.table_size(updates)))
    self:_info_chat_session_change(adds, updates, {})
    if self.sync_top_session_cache then
      E.LOG.debug(TAG, "begin sync top_session local cache")
      EC.sync_local_pending_session_attrs_to_server()
    end
  end
end
...
function M:resend(_send_id)
  ...
  if content and content.type == "res" and content.data and content.data.res_type == "image" then
    local mock_msg = self:_create_mock_msg(params)
    do return self._inner_send_real_image_msg, self end
    return self._inner_send_real_image_msg, self, mock_msg, nil, nil, nil, nil
  end
  do return self.send, self, "resend", params.session_id, content, params.at_list, params.quote_msg_ids end
  return self.send, self, "resend", params.session_id, content, params.at_list, params.quote_msg_ids, _send_id
end
...
function M:reply_msg(session_id, msg_id, reply_data)
  ...
  if current_time - self.last_time_reply_msg < self.reply_msg_limit then
    return nil
  end
  self.last_time_reply_msg = current_time
  local task_id = self:get_task_id()
  local destination = get_destination(self.user_type)
  local params = {}
  params.session_id = session_id
...
function M:_process_image_msg(msg, info_src)
  if not msg.content then
    return
  end
  local content_type = msg.content.type
  local content_data = msg.content.data or {}
  if "res" == content_type and content_data.res_type == "image" then
    local origin_image = (E_UTILS.get_media_base_url() or "") .. "/" .. content_data.res_key
    content_data.origin = origin_image
    local compress_params = content_data.extend_data and content_data.extend_data.compress_params
    if compress_params then
      content_data.thumbnail = compress_params.thumbnail and origin_image .. "?x-oss-process=image" .. compress_params.thumbnail or origin_image
      content_data.preview = compress_params.preview and origin_image .. "?x-oss-process=image" .. compress_params.preview or origin_image
    else
      content_data.thumbnail = origin_image
      content_data.preview = origin_image
    end
    if "info_msg" == info_src then
      local QL = require("ejoysdk_lua.ejoysdk_qualitylog")
      QL.commit_event(IP.image_process_event_name.chat_send_image, {
  ...
  elseif "arrival" == content_type then
    local res = content_data and content_data.res
...
function M:update_session_msgs(session_id, msgs, _in_src)
  ...
  local function exe_handler()
    ...
    self:_info_chat_msgs(session_id, msgs, _in_src)
...
function M:_create_system_session()
  ...
  session_info.is_ignore = chat_cache.is_ignore_session(session_info.id) or false
  ret.session_info = session_info
  ret.last_msgs = {}
  return ret
end
...
function M:_info_msg(_header, body)
  ...
  Chain.new():add(function(next_cb)
    ...
    for _, msg in pairs(body.msgs) do
      self:_process_image_msg(msg, "info_msg")
...
function M:_info_chat_msgs(session_id, msgs, info_src)
  local log_msgs = {}
  ...
  self.handler:info_chat_msgs(session_type, session_id, E_UTILS.deepcopy_only_once(msgs), session_from, info_src)
end
...
function M:_inner_send_real_image_msg(mock_msg, image_process_fail_cb)
  local task_id = self:get_task_id()
  if not mock_msg then
    return task_id
  end
  local mock_content_data = mock_msg.content and mock_msg.content.data or {}
  local image_info = {
    image_path = mock_content_data.thumbnail
  }
  local destination = get_destination(self.user_type)
  IP.upload_images({image_info}, "chat_image", function(succ, ...)
    if not succ then
      self:_update_mock_msg_send_fail(mock_msg)
      local code, msg = ...
      E.LOG.d(TAG, "[chat_image] upload_image fail, code: " .. tostring(code) .. ", msg: " .. msg)
      if image_process_fail_cb then
        image_process_fail_cb(...)
      end
      return
    end
    local upload_result = (...)
    local res_id = upload_result and upload_result.res_id
    local params = {}
    params.session_id = mock_msg.session_id
    params.send_id = mock_msg.send_id
    params.content = {
      type = "res",
      data = {
        res_type = "image",
        res_id = res_id,
        extend_data = {
          compress_params = mock_content_data.extend_data and mock_content_data.extend_data.compress_params,
          image_params = mock_content_data.extend_data and mock_content_data.extend_data.image_params
        }
      }
    }
    params.cmd = "send"
    ECS.rpc_call(destination, params, function(ret)
      ...
      self:_send_cb(task_id, "send_image", mock_msg, ret)
    end)
  end)
  return task_id
end

function M:_update_mock_msg_send_fail(mock_msg)
  if not mock_msg then
    return
  end
  mock_msg.reader_status = msg_util.msg_status().failed
  mock_msg.send_failed_code = -1
  mock_msg.is_local_msg = true
  E_UTILS.reset_deepcopy_only_once_record()
  self:update_session_msgs(mock_msg.session_id, {mock_msg})
end

function M:send_images(session_id, picker_params, _compress_params, image_process_fail_cb)
  IP.pick_images(picker_params or {}, function(pick_succ, ...)
    if not pick_succ then
      local code, msg = ...
      E.LOG.d(TAG, "[chat_image] select image failed, code: " .. tostring(code) .. ", msg: " .. msg)
      if image_process_fail_cb then
        image_process_fail_cb(...)
      end
      return
    end
    local mock_image_msgs = {}
    local images = (...)
    for i = 1, #images do
      local image = images[i]
      if image.size and image.size < 10485760 then
        local image_path = image.image_path
        local image_width = image.width
        local image_height = image.height
        _compress_params = _compress_params or {}
        local compress_params = {}
        compress_params.thumbnail = IP.get_image_adaptive_compress_params({
          size = _compress_params.thumbnail
        }, "thumbnail", {width = image_width, height = image_height})
        compress_params.preview = IP.get_image_adaptive_compress_params({
          quality = _compress_params.preview
        }, "preview")
        local params = {}
        params.session_id = session_id
        params.send_id = ECS.send_id()
        params.content = {
          ...
          data = {
            ...
            extend_data = {
              ...
              image_params = {width = image_width, height = image_height}
            }
          }
        }
        local mock_msg = self:_create_mock_msg(params)
        self.msg_send_cache[params.send_id] = params
        table.insert(mock_image_msgs, mock_msg)
      else
        image_process_fail_cb(CONSTANTS.CHAT_ERROR_CODES.CODE_CHAT_IMAGE_SIZE_TOO_LARGE, "image_" .. tostring(i) .. "'s size is too large")
        E.LOG.d(TAG, "[chat_image] image" .. tostring(i) .. " size is too large, size: " .. tostring(image.size or "nil"))
      end
    end
    self:update_session_msgs(session_id, mock_image_msgs)
    for i = 1, #mock_image_msgs do
      local mock_msg = mock_image_msgs[i]
      self:_inner_send_real_image_msg(mock_msg, image_process_fail_cb)
    end
  end)
end
