...
local volume_indication_interval_ms = 1000
local voice_channel_heartbeat_ms = 0
local last_self_volume_heartbeat_timestamp = 0
...
local function self_volume_heartbeat(self_speaker)
  if not (0 ~= voice_channel_heartbeat_ms and cur_channel) or mute_local_value then
    return
  end
  local self_volume = self_speaker and self_speaker.volume
  if not self_speaker or self_speaker.uid ~= "0" or not (self_volume > 0) then
    return
  end
  local current_timestamp = 1000 * E.time()
  if current_timestamp - last_self_volume_heartbeat_timestamp < voice_channel_heartbeat_ms then
    return
  end
  E.LOG.d(TAG, "start voice_channel_heartbeat, volume: " .. tostring(self_volume))
  local ECS = require("ejoysdk_lua.chat.ejoysdk_chat_server")
  ECS.voice_channel_heartbeat(cur_channel, self_volume, M.get_channel_voice_info(), function(ret)
    if 0 == ret.code then
      last_self_volume_heartbeat_timestamp = current_timestamp
      E.LOG.d(TAG, "report voice heartbeat succ, self.volume: " .. tostring(self_volume))
    else
      E.LOG.d(TAG, "report voice heartbeat failed, code: " .. tostring(ret.code) .. ", msg: " .. (ret.msg or "nil"))
    end
  end)
end
...
local voice_listener_wrapper = {
  [EVENT.ON_JOIN_CHANNEL_SUCC] = function(channel_id, uid, voice_user_id)
    ...
    local ECS = require("ejoysdk_lua.chat.ejoysdk_chat_server")
    ECS.get_agora_channel_token(channel_id, nil, function(ret)
      if 0 == ret.code then
        local cfg = ret.cfg or {}
        voice_channel_heartbeat_ms = cfg.voice_channel_heartbeat_ms or 0
        E.LOG.d(TAG, "get voice_channel_heartbeat_ms succ, value: " .. tostring(voice_channel_heartbeat_ms or "nil"))
        if 0 ~= voice_channel_heartbeat_ms then
          M.enable_volume_indication(volume_indication_interval_ms)
        end
      end
    end)
  end,
  ...
  [EVENT.ON_VOLUME_INDICATION] = function(...)
    local _, __, self_speaker = ...
    self_volume_heartbeat(self_speaker)
    callback(EVENT.ON_VOLUME_INDICATION, ...)
  end
}
local mt = {}
