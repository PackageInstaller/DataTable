...
function M.open_userinfo_completion_with_args(vendor_name, args, cb)
  local vendor = EV.get(vendor_name)
  if vendor and vendor.open_userinfo_completion_with_args then
    vendor.open_userinfo_completion_with_args(args, cb)
  end
end

function M.open_account_binding(vendor_name, args, cb)
  E.LOG.d(TAG, "open_account_binding start: " .. vendor_name)
  local vendor = EV.get(vendor_name)
  if vendor and vendor.open_account_binding then
    E.LOG.d(TAG, "open_account_binding")
    vendor.open_account_binding(args, cb)
  end
end

function M.get_account_binding_list(vendor_name, cb)
  E.LOG.d(TAG, "get_account_binding_list start: " .. vendor_name)
  local vendor = EV.get(vendor_name)
  if vendor and vendor.get_account_binding_list then
    E.LOG.d(TAG, "get_account_binding_list")
    vendor.get_account_binding_list(cb)
  end
end
