...
local function get_notification_meta_config()
  local UNI = require("ejoysdk_lua.vendors.unisdk")
  local sdk_infos = UNI.get_sdk_infos()
  local config
  if sdk_infos.ejoysdk then
    config = sdk_infos.ejoysdk.meta
  elseif sdk_infos.EJOYSDK then
    config = sdk_infos.EJOYSDK.meta
  end
  config = config or {}
  E.LOG.debug(TAG, "get_vendor_meta_config >>")
  E.log(config)
  local notification_meta = config.notification
  if notification_meta then
  end
  return notification_meta and notification_meta or {}
end

local notification_meta_cache = get_notification_meta_config()

function M.check_can_local_notification()
  if E.Sysinfo.os() == "windows" and E.is_support_function(E.NATIVE_SUPPORT_FUNCTION_NAMES.ADD_LOCAL_NOTIFICATION) then
    local result = E.sync_call("check_can_local_notification")
    return result
  else
    return false
  end
...
function M.add_local_notification_v2(params, cb)
  if E.Sysinfo.os() == "android" or E.Sysinfo.os() == "ios" then
    local notify_id = M.add_local_notification(params.title, params.content, params.calendar, params.ext, params.config)
    if cb then
      cb(true)
    end
    return notify_id
  elseif E.Sysinfo.os() == "windows" and E.is_support_function(E.NATIVE_SUPPORT_FUNCTION_NAMES.ADD_LOCAL_NOTIFICATION) then
    local windows_os_version = E.Sysinfo.os_version()
    local major_version = string.match(windows_os_version, "(%d+)%.(%d+)%.(%d+)") or 0
    if (tonumber(major_version) or 0) < 10 then
      E.LOG.debug(TAG, "add_local_notification_v2 windows_os_version is " .. tostring(windows_os_version))
      if cb then
        cb(false)
      end
      return nil, EC.BASE_API_COMMON_ERROR.CODE_NOT_SUPPORT, "not support"
    end
    local is_old_windows_process_cb = false
    local title = params.title or ""
    local content = params.content or ""
    local actions = params.actions or nil
    local expiration = params.expiration or 86400000
    local image_path = params.image_path or ""
    local game_name = notification_meta_cache.game_name
    local company_name = notification_meta_cache.company_name
    local sub_product = notification_meta_cache.sub_product
    local version_info = notification_meta_cache.version_info
    local clsid = notification_meta_cache.clsid
    local notify_id = E.sync_call("add_local_notification", JSON.encode({
      title = title,
      content = content,
      actions = actions,
      ...
      notify_app_info = {
        company_name = company_name or "LingXiGames",
        game_name = game_name or "EjoySDK",
        sub_product = sub_product or "",
        version_info = version_info or "",
        clsid = clsid or ""
      },
      use_old_windows_process_cb = is_old_windows_process_cb and 1 or 0
    }))
    E.LOG.debug(TAG, "add_local_progress_notification_custom notify_id: " .. tostring(notify_id))
    if notify_id and is_old_windows_process_cb then
      E.async_call("listen_local_notification", function(ret)
        if cb then
    ...
    E.LOG.debug(TAG, "not support local notification v2")
    if cb then
      cb(false)
    end
    return nil, EC.BASE_API_COMMON_ERROR.CODE_NOT_SUPPORT, "not support"
...
function M.remove_local_notification(notify_id)
  ...
  elseif E.Sysinfo.os() == "windows" then
    E.sync_call("remove_local_notification", JSON.encode({
      notify_id = tostring(notify_id)
    }))
  end
