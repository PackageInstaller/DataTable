...
local ASYNC_GET_WATERMARK_INFO = "ASYNC_GET_WATERMARK_INFO"
...
local function splitPathAndFilename(fullPath)
  local pattern = "(.-)([^\\/]-%.?([^%.\\/]*))$"
  local path, filename = string.match(fullPath, pattern)
  path = path or ""
  return path, filename
end

local function ensurePngExtension(filename)
...
function M.get_security_water_mark_info(param, cb)
  local os_config = E.CONFIG.get_config("os")
  if "windows" == os_config then
    local ejoysdk_cur_version = E.get_sdk_version_name("EJOYSDK")
    local need_min_version = "2.5.26"
    local ch_compare_result = version_check.compare_versions(ejoysdk_cur_version, need_min_version)
    E.LOG.debug(TAG, "ejoysdk_cur_version:" .. tostring(ejoysdk_cur_version) .. " need_min_version:" .. tostring(need_min_version) .. " ch_compare_result:" .. tostring(ch_compare_result))
  ...
  local new_params = {
    fastRun = "1",
    extra_params = get_extra_params(param)
  }
  update_data(new_params)
  UNI.cast(CHANNEL, CAST_UPDATE_DATA, new_params)
  UNI.async_call(CHANNEL, ASYNC_GET_WATERMARK_INFO, param, nil, function(succ, ...)
    if succ then
      local resp = (...)
      local waterMarkPath
      if "android" == os_config then
        waterMarkPath = resp.waterMarkPath
      else
        waterMarkPath = resp.path
      end
      if 1 == resp.error then
        local newWaterPath = ensurePngExtension(waterMarkPath)
        local ret, code, msg = E.File.copy_fullpath(waterMarkPath, newWaterPath)
        if ret then
          local path, filename = splitPathAndFilename(newWaterPath)
          resp.path = path
          resp.waterMarkPath = newWaterPath
          resp.filename = filename
          cb(1, resp)
        else
          E.LOG.warn(TAG, "rename failed, code:" .. tostring(code) .. ", msg:" .. tostring(msg))
          cb(code, resp)
        end
      else
        cb(1, resp)
