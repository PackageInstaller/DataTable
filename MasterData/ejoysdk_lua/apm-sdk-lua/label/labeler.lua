...
function M.set_account_info(login_status, account_info)
  if 1 == login_status then
    E.LOG.debug(LOGGER, "Got account info")
    E.LOG.debug(LOGGER, account_info)
    resource.account = account_info and account_info.uid
    return
  end
  if 2 == login_status then
    E.LOG.debug(LOGGER, "logout account")
    resource.account = nil
    return
  end
  E.LOG.error(LOGGER, "Got an invalid login_status" .. tostring(login_status))
end

function M.set_player_info(login_status, player_info)
  if 1 == login_status then
    E.LOG.debug(LOGGER, "Got player info")
    E.LOG.debug(LOGGER, player_info)
    resource.uid = player_info.player_id
    resource.game_server = player_info.server_id
    resource.is_login = "1"
    return
  ...
  if 2 == login_status then
    E.LOG.debug(LOGGER, "logout player")
    resource.uid = nil
    resource.is_login = "0"
    resource.game_server = nil
    resource.session_id = tostring(resource.utdid) .. "-" .. tostring(Time.now_ms())
