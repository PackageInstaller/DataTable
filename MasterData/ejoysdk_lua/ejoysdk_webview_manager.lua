...
local function show_webview_real(webview, injection, option)
  ...
  if _ejoysdk.os() == "ios" then
    option.custom_error_v2 = option.custom_error
    option.custom_error = false
  ...
  WE.commit_page_open(webview.url)
  if _ejoysdk.os() == "android" then
...
M._inject_data_local_path = inject_data_local_path
...
local function try_uri_convert(url)
  _ejoysdk.log("webview_open input url: " .. tostring(url))
  if type(url) ~= "string" then
    return url
  end
  if not url:lower():match("^file:") then
    _ejoysdk.log("webview_open non-file schema, skip convert: " .. tostring(url))
    return url
  end
  local E_Utils = require("ejoysdk_lua.ejoysdk_utils")
  local EjoyOfflineH5Utils = E_Utils.EjoyOfflineH5Utils
  local constants = require("ejoysdk_lua.ejoysdk_constants")
  local schema = constants.EJOY_LONG_PATH_FILE_SCHEMA
  local abs_path = EjoyOfflineH5Utils.convert_to_local_path(url)
  if not abs_path then
    _ejoysdk.log("webview_open file schema but no local path conversion, keep original: " .. tostring(url))
    return url
  end
  local normalized = EjoyOfflineH5Utils.parse_url_string(abs_path)
  if type(normalized) ~= "string" or "" == normalized then
    _ejoysdk.log("webview_open decode local path failed, keep original: " .. tostring(url))
    return url
  end
  _ejoysdk.log("webview_open normalized abs path:")
  _ejoysdk.log(normalized)
  _ejoysdk.log("webview_open abs path len = " .. tostring(#normalized))
  if #normalized <= 260 then
    _ejoysdk.log("webview_open short abs path, keep original")
    return url
  end
  local offline_url = url:gsub("^file:///", schema, 1)
  if offline_url == url then
    offline_url = url:gsub("^file://", schema, 1)
  end
  _ejoysdk.log("webview_open long abs path, convert to offline url")
  _ejoysdk.log("webview_open offline url: " .. offline_url)
  return offline_url
end
...
function M.add_webview(url, injection, option, on_js_callback, on_close_callback, tag)
  ...
  if _ejoysdk.os() == "windows" then
    url = try_uri_convert(url)
    _ejoysdk.log("webview_open call native final url: " .. tostring(url))
  end
  local webview = {
    url = url,
    injection = injection,
