...
local INIT_NAMESPACES = {
  ...
  APP_UPDATE = M.NAMESPACE.APP_UPDATE
}
...
M.GLOBAL_CONFIG = {GAME_ID = "10000330", PRODUCT_CODE = "p11555"}
M.GLOBAL_CONFIG_DEBUG = {GAME_ID = "10000330", PRODUCT_CODE = "p11556"}
M.global_pcode = M.GLOBAL_CONFIG.PRODUCT_CODE
M.SERVICE = {
...
function M.get_global_config(namespace)
  do return M.get_config, namespace end
  return M.get_config, namespace, M.global_pcode
end
...
local function get_request_params(product_code)
  local md5_utdid
  ...
  local game_id = E.get_pkg_info().game_id
  if product_code == M.global_pcode then
    game_id = M.GLOBAL_CONFIG.GAME_ID
  end
...
function M.env_info()
  if not env_info then
    ...
    if env_info.gmInfo and type(env_info.gmInfo) == "table" then
      ...
      env_info.gmInfo.app_update_ver = AU.get_app_update_current_version()
    end
    if env_info.devInfo then
...
local function get_configs_from_server(_namespaces, update_type, cb, product_code, disable_update_cache, options, ext)
  ...
  local req_params = get_request_params(req_product_code)
  ...
  if req_product_code == M.global_pcode then
    game_id = M.GLOBAL_CONFIG.GAME_ID
  end
  local public_params = {
...
function M.init()
  ...
  local _env = M.get_env()
  if _env == M.ENV.RELEASE then
    M.global_pcode = get_prefer_product_code(M.GLOBAL_CONFIG.PRODUCT_CODE)
  else
    M.global_pcode = get_prefer_product_code(M.GLOBAL_CONFIG_DEBUG.PRODUCT_CODE)
  end
  M._get_configs_from_server(INIT_NAMESPACES, function(...)
    E.LOG.debug(TAG, "get_configs_from_server, pcode: " .. M.global_pcode)
  end, M.global_pcode)
  loop_request()
  inited = true
end
