...
function M.set_domain(domain_name)
  ...
  if ".sialiagames.com.tw" == domain_name then
    domain_name = ".sialiagamesinc.com.tw"
  end
  domain = domain_name
...
M.PATH_MAPS_V2 = {
  ...
  vortex = "vortex",
  sns = "sns",
  ["campaign-center"] = "campaign_center"
}
...
function M.set_service_splice_version(_new_ss_verssion)
  if _service_splice_version ~= _new_ss_verssion then
    _ejoysdk.log("set_service_splice_version old:" .. tostring(_service_splice_version) .. ", new:" .. tostring(_new_ss_verssion))
    ...
    _ejoysdk.log("set_service_splice_version gangplank url after update service:" .. tostring(EJOYSDK_CONFIG.gangplank))
  end
end
...
function M.autoconfig(env, product)
  local product_changes = false
  if type(env) == "string" and type(product) == "string" then
    product_changes = EJOYSDK_CONFIG.product ~= product
    _ejoysdk.log("autoconfig product changes:" .. tostring(product_changes) .. ", old:" .. tostring(EJOYSDK_CONFIG.product) .. ", new:" .. tostring(product))
  ...
  init_server_domain()
  if product_changes then
    _ejoysdk.log("autoconfig product changes is true, begin reset ejoysdk_service url, splice_version:" .. tostring(_service_splice_version))
    M.set_service_splice_version(-1)
  else
    _ejoysdk.log("autoconfig product changes is false, skip reset ejoysdk_service url, splice_version:" .. tostring(_service_splice_version))
  end
  M.update_ejoysdk_service()
  _ejoysdk.log("autoconfig gangplank url after update service:" .. tostring(EJOYSDK_CONFIG.gangplank))
  if cdn_domain then
...
local function hook_config_value(key, value)
  ...
  if key == M.KEY.SERVER_DOMAIN and ".sialiagames.com.tw" == value then
    value = ".sialiagamesinc.com.tw"
  end
  if key == M.KEY.SERVER_DOMAIN and "sialiagames.com.tw" == value then
    value = "sialiagamesinc.com.tw"
  end
