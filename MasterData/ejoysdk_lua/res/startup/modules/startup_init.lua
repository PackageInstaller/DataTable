...
function M:_request_startup_configs(cb)
  ...
  for ns, res_map in pairs(all_ns_resources) do
    if startup_ns_map[ns] then
      for res_key, res_info in pairs(res_map) do
        local _real_res_key = GMR.get_real_res_key(PREDOWNLOAD_MGR.UTILS.get_real_res_key(res_key))
        local_res_params[ns] = local_res_params[ns] or {}
        local_res_params[ns][_real_res_key] = local_res_params[ns][_real_res_key] or {}
        local _res_params = local_res_params[ns][_real_res_key]
        _res_params.versions = _res_params.versions or {}
        local _using_res_info = res_info[RTM.NAMESPACE_RES_CONFIG_KEY.TYPE_USING_RES_INFO] or {}
        local _cur_res_type = _using_res_info[ER.RES_CONFIG_KEY.KEY_RES_TYPE]
        local _cur_res_ver = _using_res_info[ER.RES_CONFIG_KEY.KEY_VERSION]
        local _versions
        local _pkg_version = (game_startup_params or {})[ns] or {}[_real_res_key] or {}.pkg_res_version
        if PREDOWNLOAD_MGR.UTILS.is_predownload(res_key) then
          _res_params.predownload = _res_params.predownload or {}
          _res_params.predownload.versions = _res_params.predownload.versions or {}
          _versions = _res_params.predownload.versions
          local _default_versions = _res_params.versions
          UTILS.insert_array_notnull(_default_versions, _cur_res_ver)
        else
          _versions = _res_params.versions
          if not E.Utils.start_with(res_key, RTM.RES_GROUP_INFO_KEY.PREFIX) then
            _res_params.res_type = _cur_res_type
          end
          UTILS.insert_array_notnull(_versions, _pkg_version)
        end
        UTILS.insert_array_notnull(_versions, _cur_res_ver)
        local_res_params[ns][_real_res_key] = _res_params
      end
    else
  ...
  for ns, res_map in pairs(game_local_ns_res_params) do
    for res_key, res_info in pairs(res_map) do
      ...
      local preDownload = origin_res_params.predownload or {}
      local_res_params[ns][res_key] = res_info
      local versions = origin_res_params.versions or {}
      local predownload_versions = preDownload.versions or {}
      local game_res_version = res_info.version
      if game_res_version then
        UTILS.insert_array(versions, game_res_version)
        UTILS.insert_array(predownload_versions, game_res_version)
      end
      local startup_params = SC.get_startup_game_origin_params() or {}
      if startup_params.namespace == ns and startup_params.res_key == res_key then
        local startup_groups = startup_params.groups
        if startup_groups and next(startup_groups) then
          for _, group_info in ipairs(startup_groups) do
            if not UTILS.is_text_empty(group_info.version) then
              UTILS.insert_array(versions, group_info.version)
            elseif not UTILS.is_text_empty(group_info.pkg_res_version) then
              UTILS.insert_array(versions, group_info.pkg_res_version)
            end
          end
        end
      end
      local_res_params[ns][res_key].versions = versions
      local_res_params[ns][res_key].predownload = {
        versions = predownload_versions or {}
      }
    end
  end
      ...
      if cache_ns_infos[ns] then
        ...
        for rk, l_rk_obj in pairs(l_ns_obj) do
          ...
          for cache_rk, cache_rk_obj in pairs(cache_ns_data) do
            ...
            if using_res_type and cache_res_type and using_res_type == cache_res_type and rk ~= cache_rk and not PREDOWNLOAD_MGR.UTILS.is_predownload(cache_rk) and not GMR.is_group(cache_rk) then
...
local function _old_check_app_update(cb)
...
function M:_check_app_update(cb)
  AUM.set_game_listeners({
    on_game_exit = function()
      END.dispatch_app_update_game_exit_listener()
    end
  })
  AUM.run_main_update({need_show_install_guide = true}, function(is_success, ...)
    EL.LOG.debug(TAG, "check_app_res_update run_main_update is_success: " .. tostring(is_success))
    if is_success then
      local update_info = (...)
      if update_info and update_info.is_mandatory_force_update then
        E.LOG.debug(TAG, "run_main_update is_mandatory_force_update is true, wait for force update")
      else
        _old_check_app_update(cb)
      end
    else
      _old_check_app_update(cb)
    end
