...
local function stat(action, key, succ, params)
  ...
  E.LOG.debug(TAG, string.format("action=%s,key=%s,succ=%s", tostring(action), tostring(key), tostring(succ or "nil")))
...
function M.get_simple_res_state(res_state_info)
  ...
  local downloading_size = res_progress_info.downloading_size or 0
  local finish_count = res_progress_info.finish_count or 0
  if total_size >= downloading_size and total_count >= finish_count then
    simple_res_state_info.remain_size = total_size - downloading_size
    simple_res_state_info.remain_count = total_count - finish_count
  end
