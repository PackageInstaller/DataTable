...
function M.to_model_download_list_format(qz_file_list, folder, base_path, _dest_parent_path)
  ...
  for _, item in ipairs(qz_file_list) do
    ...
    if not EU.is_text_empty(item_from) then
      ...
      pending_index_list[down_item[RTM.FILE_LIST_ITEM_KEY.KEY_TO]] = down_item
    end
  end
  return pending_download_list, pending_index_list
end
...
function M.batch_move_file(move_map, cb)
  ...
  for _, target_file_path in pairs(move_map) do
    E.LOG.debug(TAG, "move target file path is " .. tostring(target_file_path))
...
function M.batch_copy_file(copy_mp, cb)
  E.File.batch_copy(copy_mp, function(copy_succ, ...)
    if copy_succ then
      cb(true)
    else
      local copy_err_code, copy_err_msg = ...
...
local function _batch_files_process(func, _dest_parent_path, map, info_map, cb)
  local LOCAL_FILE_LIST = require("ejoysdk_lua.res.model.res_src.ejoy_local_file_list")
  local current_file_list_info = LOCAL_FILE_LIST.static_get_file_list(_dest_parent_path, false)
  func(map, function(succ, ...)
    if succ then
      for _src, dst in pairs(map) do
        local _file_info = info_map[dst]
        current_file_list_info:add_file(_file_info.to, _file_info.md5, _file_info.size)
      end
      current_file_list_info:save()
      cb(true)
    else
      local code, msg, _ext = ...
...
function M.batch_move_res_file(_dest_parent_path, move_map, move_info_map, cb)
  _batch_files_process(M.batch_move_file, _dest_parent_path, move_map, move_info_map, cb)
end

function M.batch_copy_res_file(_dest_parent_path, copy_map, copy_info_map, cb)
  _batch_files_process(M.batch_copy_file, _dest_parent_path, copy_map, copy_info_map, cb)
end
...
function M.get_temp_dir_path(namespace, res_key, game_apply_dest_path, dirname)
  ...
  if PREDOWNLOAD_MGR.UTILS.is_predownload(res_key) then
    game_dest_parent_path = E.Path.parent_dir(game_dest_parent_path)
  end
...
function M.format_bytes(size)
  ...
  i = math.min(i, #sizes - 1)
  local formattedSize = size / 1024 ^ i
  if formattedSize >= 10 then
    formattedSize = string.format("%.0f", formattedSize)
  else
    formattedSize = string.format("%.1f", formattedSize)
  end
