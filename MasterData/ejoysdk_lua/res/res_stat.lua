...
local _IS_FIRST_STARTUP_UPDATE_KS = E.LazyKeyStore:New("EJOYSDK_IS_FIRST_STARTUP_UPDATE", false, true, false)
local _is_first_update_cache = _IS_FIRST_STARTUP_UPDATE_KS:get() == nil
local JF, APM
local M = {}
local TAG = "res_stat"
...
local function get_storage_size()
  do return tostring end
  return tostring, (E.Sysinfo.get_storage_info() or {}).internal_total_storage_size or 0
end

local function get_is_first_update()
  return _is_first_update_cache and "1" or "0"
end

local function mark_is_first_update_done()
  if _is_first_update_cache then
    _IS_FIRST_STARTUP_UPDATE_KS:set("1")
    _is_first_update_cache = false
  end
end
...
function M.stat_update_wait_confirm(namespace, res_key, update_ver, local_version, is_upload_now)
  ...
  params.storage_size = get_storage_size()
...
function M.stat_startup_init_begin()
  M.stat("startup_init_begin", "", nil, {
    storage_size = get_storage_size(),
    is_first_update = get_is_first_update()
  })
end
...
function M.stat_integrity_check_begin()
  M.stat("integrity_check_begin")
end

function M.stat_integrity_check_end(_succ, ...)
  if _succ then
    local _cost = (...)
    M.stat("integrity_check_end", "", true, {cost = _cost})
  else
    local code, msg = ...
    M.stat_err("integrity_check_end", "", code, msg)
  end
end
...
function M.get_res_stat_info(namespace, res_key, _local_version)
  ...
  if not local_version then
    local local_infos = SC.get_startup_local_res_infos_cache()
  ...
  local res_stat_info = {
    ...
    p2 = local_version,
    p3 = update_version,
    update_size = res_update_info.total_size or "0",
    is_first_update = get_is_first_update()
  }
