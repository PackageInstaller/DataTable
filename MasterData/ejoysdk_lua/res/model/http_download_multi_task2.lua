...
local _STOP_REASON = {
  ...
  NETWORK_STRATEGY_NOT_SATISFIED = "network_strategy_not_satisfied",
...
function M:_start_download_for_resume(cb)
  self:_check_all_file_state_if_need(function()
    ...
    self:_start_with_multi_request_adapter(unfinished_file_list, nil, function()
      if cb then
        cb(true)
      end
    end)
...
function M:_start_download_for_reset(cb)
  self:_check_all_file_state_if_need(function()
    ...
    self:_start_with_multi_request_adapter(unfinished_file_list, need_native_download_task_id_map, function()
      if cb then
        cb(true)
      end
    end)
...
function M:_setup_notification_present_progress_info(need_native_download_task_id_map)
  ...
  local _custom_config = self._data.notification_present_ext.custom_config or {}
  _custom_config.default_progress_style = self._data.default_progress_style or _custom_config.default_progress_style
  local _opts = {native_update_only = true, custom_config = _custom_config}
...
function M:_start_with_multi_request_adapter(unfinished_file_list, need_native_download_task_id_map, cb)
  ...
  self:update_download_config({
    ...
    max_band_width_kbytes_per_sec = self._data.max_band_width_kbytes_per_sec,
    default_progress_style = self._data.default_progress_style
  })
  ...
  self._data.download_multi_source:start(unfinished_file_list, params, cb)
end
...
function M:_inner_start_download(cb)
  self:_check_all_file_state_if_need(function(succ, ...)
    ...
    if succ then
      ...
      self:_start_with_multi_request_adapter(_unfinished_file_list, need_native_download_task_id_map, function()
        if cb then
          cb()
        end
      end)
...
function M:_init(_task_id, _file_list, _file_ext, _opts, params, _model_instance, _single_model)
  ...
  local fsm_callbacks = {
    ...
    ["on" .. _EVENTS.START_DOWNLOAD] = function(_fsm, from, ...)
      ...
      if from == _STATES.IDLE then
        ...
        return FSM.ASYNC
      elseif from == _STATES.STOPPED or from == _STATES.WAITTING then
    ...
    ["on" .. _EVENTS.RESUME_DOWNLOAD] = function(_fsm, from, ...)
      if from ~= _STATES.STOPPED and from ~= _STATES.WAITTING then
    ...
    ["on" .. _EVENTS.STOP_DOWNLOAD] = function(_fsm, from, ...)
      ...
      elseif from == _STATES.STOPPED or from == _STATES.COMPLETE then
        ...
        p_cb_wrapper(true)
        return FSM.SYNC
      elseif from == _STATES.WAITTING then
        self:log_debug("stop in waitting state, transition to stopped state")
        p_cb_wrapper(true)
        return FSM.SYNC, _STATES.STOPPED, stop_reason
      elseif from == _STATES.DOWNLOADING then
    ...
    ["on" .. _EVENTS.PAUSE_DOWNLOAD] = function(_fsm, from, ...)
      local p_cb = (...)
      
      local function p_cb_wrapper(p_succ, ...)
        if p_succ then
          self:log_debug("pause_download succ")
        else
          local p_code, p_msg = ...
          self:log_warn("pause_download failed, code:" .. tostring(p_code) .. ", msg:" .. tostring(p_msg))
        end
        if p_cb then
          p_cb(p_succ, ...)
        end
      ...
      if from ~= _STATES.DOWNLOADING then
        self:log_debug("pause download in not downloading state, return")
        p_cb_wrapper(true)
        return FSM.SYNC
      end
      self:_do_stop_download_task(function(succ, ...)
        if succ then
          self:_fsm_transition_to(from, _STATES.WAITTING, _STOP_REASON.WAIT_IN_QUEUE)
          p_cb_wrapper(true)
        else
          local cd_code, cd_msg = ...
          p_cb_wrapper(false, cd_code, cd_msg)
        end
        self:_fsm_on_async_event_finished(_EVENTS.PAUSE_DOWNLOAD)
      end, from)
      return FSM.ASYNC
    end,
    ...
    ["onenter" .. _STATES.STOPPED] = function(_fsm, from, ...)
      ...
      local current_download_state = self._data.download_state
      if 0 == current_download_state.err_code or stop_reason == _STOP_REASON.MANUAL_STOP then
    ...
    ["onenter" .. _STATES.WAITTING] = function(_fsm, from, ...)
      local stop_reason = (...)
      self:log_debug("onenter waitting, from:" .. tostring(from) .. ", stop_reason:" .. tostring(stop_reason))
      local current_download_state = self._data.download_state
      if stop_reason == _STOP_REASON.NETWORK_STRATEGY_NOT_SATISFIED then
        current_download_state.err_code = EC.DOWNLOAD_ERROR_CODES.DOWNLOAD_NETWORK_STRATEGY_NOT_SATISFIED
        current_download_state.err_msg = "network strategy not satisfied"
      else
        current_download_state.err_code = EC.DOWNLOAD_ERROR_CODES.DOWNLOAD_WAIT_IN_QUEUE
        current_download_state.err_msg = "wait in queue"
      end
      current_download_state.speed = 0
      local biz_name = self:model_task_name()
      ERU.reset_download_speed(biz_name)
      current_download_state.state = M.DOWNLOAD_STATE.WAITTING
      self:_notify_download_state_changed()
    end,
  ...
  self._data = {
    ...
    max_band_width_kbytes_per_sec = _opts and _opts.max_band_width_kbytes_per_sec or nil,
    ...
    default_progress_style = (_opts and _opts.notification_present_ext or {}).default_progress_style,
...
function M:update_download_config(config)
  config = config or {}
  self._data.max_retry_times = config.max_retry_times or self._data.max_retry_times
  self._data.max_band_width_kbytes_per_sec = config.max_band_width_kbytes_per_sec or self._data.max_band_width_kbytes_per_sec
  self._data.progress_notify_interval_millis = config.progress_notify_interval_millis or self._data.progress_notify_interval_millis
  self._data.default_progress_style = config.default_progress_style or self._data.default_progress_style
  self._data.model_instance:update_download_config(config)
end
...
function M:_is_stop_processing()
  local current_event = self:_fsm_current_event()
  return current_event == _EVENTS.STOP_DOWNLOAD or current_event == _EVENTS.PAUSE_DOWNLOAD
end
...
function M:stop_download_impl(cb)
  ...
  end, _STOP_REASON.MANUAL_STOP)
end

function M:pause_download_impl(cb, stop_reason)
  assert(self, "call stop_download missing ':' mark!")
  stop_reason = stop_reason or _STOP_REASON.WAIT_IN_QUEUE
  local current_proccessing_stop = self:_is_stop_processing()
  if current_proccessing_stop then
    return
  end
  self:log_debug("pause_download invoke received begin:" .. tostring(self._data.download_fsm.current) .. ", stop_reason:" .. tostring(stop_reason))
  self:_fsm_enqueue_event(_EVENTS.PAUSE_DOWNLOAD, function(succ, ...)
    if succ then
      self:log_debug("pause_download succ")
    else
      local code, msg = ...
      self:log_debug("pause_download failed, code:" .. tostring(code) .. ", msg:" .. tostring(msg))
    end
    if cb then
      cb(succ, ...)
    end
