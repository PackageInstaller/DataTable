...
function M:do_start(_cb)
  assert(false, "child class should implements this method")
end

function M:start(unfinished_file_list, params, cb)
...
local function _create_retry_task_id(task_id, multi_task_id)
  return "retry_" .. tostring(multi_task_id) .. "_" .. tostring(task_id)
end
...
local function remove_task_downloading_state(task_id, multi_task_id)
  ...
  if not multi_task_id then
    ...
    if multi_task_infos then
      ...
      local _retry_multi_task_id = _create_retry_task_id(task_id, multi_task_id)
      if multi_task_infos[_retry_multi_task_id] then
        multi_task_infos[_retry_multi_task_id] = nil
      end
...
function M:http_get(url, params, cb)
  ...
  local multi_task_id = self._data.multi_task:task_id_impl()
  local retry_task_id = _create_retry_task_id(task_id, multi_task_id)
