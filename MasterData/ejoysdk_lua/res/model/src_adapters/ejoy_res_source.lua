...
function M:_init(_model_name, _namespace, _res_key, _opts)
  ...
  self._data = {
    ...
    versions = {
      ...
      pkg_res_version = _opts.pkg_res_version
    }
  }
  self:update_config(_opts)
end
...
function M.static_remove_res_cache_files(namespace, res_key, version, ext)
  ...
  if E.Sysinfo.os() ~= "windows" or E.is_support_function(EC.NATIVE_SUPPORT_FUNCTION_NAMES.FILE_DIR_OPERATION) then
    ...
    E.LOG.debug(TAG, string.format("remove res cache files, result is %s, path is %s", tostring(rm_result), tostring(res_full_path)))
  else
...
function M.is_res_exists(res_location)
  ...
  E.LOG.debug(TAG, string.format("is_res_exists: %s, res_location:%s", tostring(exists), tostring(res_location)))
  return exists
end
...
local function load_local_file_list_content(namespace, res_key, version, local_version, ext)
  ...
  local _dest_path = get_res_version_dest_full_path(namespace, res_key, version, ext)
  local _file_list_cache = QZ_FILE_LIST_MGR.static_get_qz_file_list(_dest_path, FILE_NAME_LIST_JSON)
  local _file_list_path, _file_list_content = _file_list_cache:get_file_list_if_exists()
  if _file_list_content then
    FP.parse_file_list(namespace, res_key, local_version, version, _file_list_content)
    local _file_list_obj = FP.get_file_list(namespace, res_key, local_version, version)
    local _file_info_list_cache = QZ_FILE_LIST_MGR.static_get_qz_file_list(_dest_path, FILE_LIST_DESC_JSON)
    local _, _file_info_list_content = _file_info_list_cache:get_file_list_if_exists()
    if _file_info_list_content then
      return _file_list_obj, _file_info_list_content
    else
      EL.LOG.warn(TAG, "load_local_file_list_content not has list desc, so return nil and redownload")
      return nil
    end
  else
    EL.LOG.warn(TAG, "load_local_file_list_content, not exists:" .. tostring(_file_list_path))
    return nil
  end
end
...
local function _load_group_res_file_list_content(namespace, res_key, version, local_version, ext)
  ...
  local QZ_FILE_LIST_MGR = require("ejoysdk_lua.res.model.res_src.ejoy_qz_res_file_list_mgr")
  local _dest_path = M.static_get_index_file_cache_path(namespace, real_res_key, version, local_version, ext)
  local _file_list_cache = QZ_FILE_LIST_MGR.static_get_qz_file_list(_dest_path, FILE_NAME_LIST_JSON)
  local _file_list_path, _file_list_content = _file_list_cache:get_file_list_if_exists()
  if _file_list_content then
    FP.parse_file_list(namespace, res_key, local_version, version, _file_list_content)
  ...
  EL.LOG.warn(TAG, "_load_group_res_file_list_content, not exists: " .. tostring(_file_list_path))
  return nil
end
...
local function load_index_info_file_content(namespace, res_key, index_info_file_url, index_info_file_md5, target_version, local_version, file_info_list_cb, ext)
  ...
  local real_res_key = GRM.get_real_res_key(res_key)
  local QZ_FILE_LIST_MGR = require("ejoysdk_lua.res.model.res_src.ejoy_qz_res_file_list_mgr")
  local _dest_path = M.static_get_index_file_cache_path(namespace, real_res_key, target_version, local_version, ext)
  local _download_info = {url = index_info_file_url, md5 = index_info_file_md5}
  local _file_list_cache = QZ_FILE_LIST_MGR.static_get_qz_file_list(_dest_path, FILE_NAME_INFO_LIST_JSON)
  _file_list_cache:get_file_list(_download_info, function(succ, ...)
    if true == succ then
      local _is_from_download, _index_info_file_path, _index_info_file_content = ...
      file_info_list_cb(true, _index_info_file_content)
    else
...
function M:_load_index_file_content(index_file_url, index_file_md5, target_version, local_version, file_list_cb, is_total_file_list)
  ...
  if local_file_list_content then
    if type(list_desc_json) == "table" and list_desc_json.md5 then
      ...
      EL.LOG.debug(TAG, string.format("_load_local_file_list_content check md5, local:%s, new:%s, ver:%s, equals:%s", tostring(list_desc_json.md5), tostring(index_file_md5), tostring(target_version), tostring(md5_equals)))
      if md5_equals then
  ...
  local function index_file_handler(_index_file_path, index_file_content)
    FP.parse_file_list(namespace, res_key, local_version, target_version, index_file_content)
  ...
  local GRM = require("ejoysdk_lua.res.group_res_manager")
  local real_res_key
  if PREDOWNLOAD_MGR.UTILS.is_predownload(res_key) then
    real_res_key = PREDOWNLOAD_MGR.UTILS.get_real_res_key(res_key)
    real_res_key = GRM.get_real_res_key(real_res_key)
    real_res_key = PREDOWNLOAD_MGR.UTILS.get_task(real_res_key)
  else
    real_res_key = GRM.get_real_res_key(res_key)
  end
  local QZ_FILE_LIST_MGR = require("ejoysdk_lua.res.model.res_src.ejoy_qz_res_file_list_mgr")
  local _dest_path = M.static_get_index_file_cache_path(namespace, real_res_key, target_version, local_version, ext)
  local _download_info = {url = index_file_url, md5 = index_file_md5}
  local _file_list_cache = QZ_FILE_LIST_MGR.static_get_qz_file_list(_dest_path, FILE_NAME_LIST_JSON)
  _file_list_cache:get_file_list(_download_info, function(succ, ...)
    if true == succ then
      local _is_from_download, _index_file_path, _index_file_content = ...
      local file_list_desc_path = self:get_res_version_path(target_version) .. "/" .. FILE_LIST_DESC_JSON
      if not EHU.is_file_exists(file_list_desc_path) then
        local file_list_desc_info = {md5 = index_file_md5, type = "sdk"}
        local desc_info_str = CJSON.encode(file_list_desc_info)
        E.File.writefile_fullpath(file_list_desc_path, desc_info_str, false)
      end
      index_file_handler(_index_file_path, _index_file_content)
    else
      local err_code, err_msg = ...
...
function M:_process_pending_download_file_list(index_file_list, _bak_old_file_list, update_info, last_downloading_info, is_custom_file_list, custom_file_desc, cb)
  ...
  if is_custom_file_list then
    ...
    EL.LOG.debug(TAG, string.format("_process_pending_download_file_list, its custom list, total_size:%d, total_count:%d", total_size, total_count))
  else
  ...
  if 0 ~= EU.version_compare(last_downloading_version, update_res_version) then
    ...
    EL.LOG.debug(TAG, string.format("process_pending_download_file_list downloading_version smaller than update version, downloading ver:%s, update ver:%s", tostring(last_downloading_version), tostring(update_res_version)))
    ...
    if last_downloading_version and is_last_file_list_type_valid and is_new_file_list_type_valid then
      ...
      EL.LOG.debug(TAG, string.format("process_pending_download_file_list last_downloading_file_list_type:%s, update_file_list_type:%s ", tostring(last_downloading_file_list_type), tostring(update_file_list_type)))
      ...
      EL.LOG.debug(TAG, string.format("is_group_key:%s, res_key:%s", tostring(is_group_key), tostring(res_key)))
        ...
        for _src_path, _dst_path in pairs(reuse_res_path_map) do
          ...
          if item_fail_ret then
            ...
            if item_fail_ret.code == EC.EJOY_LIB_ERROR.FILE_NOT_EXISTS and is_downloading_file_exit then
              ...
              EL.LOG.debug(TAG, string.format("rename dl_bak dst_path: %s, ret:%s, code:%s", tostring(_dst_path .. HDT.DOWNLOADING_FILE_SUFFIX), tostring(ret), tostring(code)))
            end
            table.insert(pending_download_list, _new_item)
          else
            if EL.is_log_open() then
              EL.LOG.debug(TAG, string.format("same file list item:%s, reuse succ ", tostring(_src_path)))
            end
    ...
    local function _batch_rm_callback(_succ2, ...)
      ...
      if unknown_list_size > 0 then
        ...
        E.File.batch_info(new_to_check_info_rela_path_list, function(succ_data)
          ...
          EL.LOG.debug(TAG, string.format("batch_info cost:%d, check_size:%s, total:%d", _after - _before, tostring(need_check_size), unknown_list_size))
...
function M:update_config(config)
  ...
  if type(config[RTM.RES_UPDATES_OPTIONS.USING_PREDOWNLOAD]) == "boolean" then
    self._data.using_predownload = config[RTM.RES_UPDATES_OPTIONS.USING_PREDOWNLOAD]
  else
    config[RTM.RES_UPDATES_OPTIONS.USING_PREDOWNLOAD] = self._data.using_predownload
  end
...
function M:get_origin_file_list_for_groups(update_version, update_info, address_list, groups, cb)
  local function get_fun()
    ...
    E.LOG.debug(TAG, string.format("get_origin_file_list_for_groups, get group file list, version:%s, groups:%s, list size:%d, ext_info:%s", tostring(update_version), tostring(table.concat(groups, ", ")), #_list, tostring(ext_info)))
...
function M:_load_qz_file_list_with_type(update_info, specified_file_list_type, address_list, cb, is_total_file_list)
  ...
  if not _is_single_file_mode then
    ...
    EL.LOG.debug(TAG, string.format("has index file, begin load index file:%s, index_file_md5 is %s", tostring(index_file_url), tostring(index_file_md5)))
...
function M:check_update_file_list(last_downloading_info, address_list, update_info, file_list_cb)
  ...
  local res_info = {namespace = namespace, res_key = res_key}
  ...
  local function process_pending_download_list_fun(file_list, remote_folder, reuse_file_list, is_custom_file_list, custom_file_desc, _bak_old_file_list)
    self:_process_pending_download_file_list(file_list, _bak_old_file_list, update_info, last_downloading_info, is_custom_file_list, custom_file_desc, function(pending_file_list, downloading_size, total_size, finish_count, total_count)
      EL.LOG.debug(TAG, string.format("process_pending_download_file_list result, pending_file_list:%d, downloading_size: %d, total_size:%d", #pending_file_list, downloading_size, total_size))
      ...
      total_count = (total_count or 0) + #(reuse_file_list or {})
  ...
  EL.LOG.debug(TAG, string.format("check_has_request_file_list_handler:%s, ns:%s, rk:%s", tostring(need_custom_file_list), tostring(namespace), tostring(res_key)))
  if need_custom_file_list then
    local function req_custom_list_handler(origin_file_info, down_origin_file_info, down_patch_file_info, full_match_file_infos)
      ...
      local ext_info = {
        origin_file_list = origin_file_info.file_list or {},
        file_list = (down_patch_file_info or {}).file_list or {},
        down_origin_file_list = (down_origin_file_info or {}).file_list or {},
        reuse_file_lists = full_match_file_infos or {},
        namespace = namespace,
        res_key = res_key,
      ...
      if NUM.check_has_process_file_list_handler(namespace, res_key) then
        NUM.process_file_list(namespace, res_key, function(process_succ, ...)
          if process_succ then
            ...
            if patch_file_list and next(patch_file_list) ~= nil then
              update_info[RTM.UPDATE_INFO_KEY.FILE_LIST_TYPE] = RTM.FILE_LIST_TYPE.PATCH
              E.LOG.d(TAG, {last_downloading_info = last_downloading_info})
            ...
            local reuse_file_list = {}
            for _, _list in ipairs(ext_info.reuse_file_lists) do
              for _, file_info in pairs(_list.file_list or {}) do
                table.insert(reuse_file_list, file_info)
              end
            end
            process_pending_download_list_fun(file_list, temp_folder, reuse_file_list, true, {})
          else
            local code, msg = ...
        ...
        NUM.request_file_list(namespace, res_key, function(succ, ...)
          if succ then
            ...
            process_pending_download_list_fun(file_list, folder, nil, true, list_desc)
          else
            local code, msg = ...
            EL.LOG.warn(TAG, "request_file_list failed, code:" .. tostring(code) .. ", msg:" .. tostring(msg))
            file_list_cb(false, code, msg)
    ...
    local function res_reuse_check(origin_file_info, down_origin_file_info, down_patch_file_info, full_match_file_infos)
      local ER_UTILS = require("ejoysdk_lua.res.ejoysdk_res_utils")
      local _check_opts = SU.get_reuse_opts(namespace, res_key)
      full_match_file_infos = full_match_file_infos or {}
      local apply_dest_path = SU.get_apply_path(namespace, res_key)
      if UTIL.is_text_empty(apply_dest_path) then
        req_custom_list_handler(origin_file_info, down_origin_file_info, down_patch_file_info, full_match_file_infos)
      else
        local apply_patch_temp_dir = ER_UTILS.get_temp_dir_path(namespace, res_key, apply_dest_path)
        res_checker.reuse_check(apply_patch_temp_dir, origin_file_info, down_origin_file_info, down_patch_file_info, _check_opts, function(_down_origin_file_info, _down_patch_file_info, _match_file_info)
          local _full_match_file_info = {
            can_move = true,
            file_list = _match_file_info.file_list,
            src_path = apply_patch_temp_dir
          }
          table.insert(full_match_file_infos, _full_match_file_info)
          req_custom_list_handler(origin_file_info, _down_origin_file_info, _down_patch_file_info, full_match_file_infos)
        end)
      end
    end
    
    local function common_filter(origin_file_info, down_origin_file_info, down_patch_file_info, full_match_file_infos)
      if PREDOWNLOAD_MGR.UTILS.is_predownload(res_key) then
        req_custom_list_handler(origin_file_info, down_origin_file_info, down_patch_file_info, full_match_file_infos)
      else
        local GRM = require("ejoysdk_lua.res.group_res_manager")
        
        local function _hotfix_check(_origin_file_info, _origin_file_list, _down_origin_file_info, _down_patch_file_info, _full_match_file_info)
          local HOTFIX = require("ejoysdk_lua.res.hotfix.ejoysdk_res_hotfix")
          local _hotfix_mgr = HOTFIX.static_get_hotfix_mgr(namespace, res_key)
          local _addi_origin_file_list, _hotfix_down_origin_file_info, _hotfix_down_patch_file_info = _hotfix_mgr:hotfix_res_update(_origin_file_list, _down_origin_file_info, _down_patch_file_info, _full_match_file_info)
          UTIL.insert_arrays(_origin_file_info.file_list, _addi_origin_file_list)
          res_reuse_check(_origin_file_info, _hotfix_down_origin_file_info, _hotfix_down_patch_file_info, full_match_file_infos)
        end
        
        origin_file_info = origin_file_info or {}
        origin_file_info.file_list = origin_file_info.file_list or {}
        if GRM.is_group(res_key) then
          _hotfix_check(origin_file_info, origin_file_info.file_list, down_origin_file_info, down_patch_file_info, full_match_file_infos)
        else
          self:_load_qz_file_list_with_type(update_info, RTM.FILE_LIST_TYPE.ORIGIN, address_list, function(_succ, ...)
            local _origin_total_file_list, _bak_list, _ext_info = ...
            _hotfix_check(origin_file_info, _origin_total_file_list, down_origin_file_info, down_patch_file_info, full_match_file_infos)
          end, true)
        end
      end
    end
    
    local function filter_predownload(origin_file_info, down_origin_file_info, down_patch_file_info)
      local TAG__ = "demonk_source_filter"
      local is_predown_res_key = PREDOWNLOAD_MGR.UTILS.is_predownload(res_key)
      if not is_predown_res_key and not PREDOWNLOAD_MGR.UTILS.will_become_official(namespace, res_key) then
        common_filter(origin_file_info, down_origin_file_info, down_patch_file_info)
        return
      end
      local predown_using_res_info = PREDOWNLOAD_MGR.UTILS.get_using_info(namespace, res_key)
      local UIM = require("ejoysdk_lua.res.update_info_manager")
      local cur_predown_version = (predown_using_res_info or {}).version
      local cur_using_version = UIM.get_using_version(namespace, PREDOWNLOAD_MGR.UTILS.get_real_res_key(res_key), self._data.versions)
      E.LOG.d(TAG__, string.format("cur_predown_ver=%s,cur_using_version=%s", cur_predown_version, cur_using_version))
      if UTIL.is_text_empty(cur_predown_version) then
        cur_predown_version = cur_using_version
      end
      local copy_update_info = UTIL.deepcopy(update_info)
      local _pre_update_body = copy_update_info[RTM.UPDATE_INFO_KEY.UPDATE_BODY] or {}
      _pre_update_body[RTM.UPDATE_INFO_KEY.PREDOWNLOAD] = nil
      local patches = _pre_update_body[RTM.UPDATE_INFO_KEY.PATCHES]
      local PREDOWNLOAD_CHECK = require("ejoysdk_lua.res.predownload.ejoy_pre_res_check")
      local _check_func, _target_version
      if is_predown_res_key then
        _check_func = PREDOWNLOAD_CHECK.hotfix_pre_res_download_check
        _target_version = cur_using_version
      else
        _check_func = PREDOWNLOAD_CHECK.hotfix_res_download_check
        _target_version = cur_predown_version
      end
      local _patch_info
      if patches and next(patches) then
        for _, patch_info in ipairs(patches) do
          if patch_info.targetVersion == _target_version then
            E.LOG.d(TAG__, {
              target_patch_info = patch_info or "nil"
            })
            _patch_info = patch_info
            _pre_update_body.patch = patch_info
            break
          end
        end
      end
      E.LOG.d(TAG__, {update_body = copy_update_info})
      local _res_file_type = RTM.FILE_LIST_TYPE.PATCH
      if not _patch_info then
        _patch_info = {}
        _res_file_type = RTM.FILE_LIST_TYPE.ORIGIN
      end
      E.LOG.d(TAG__, "res_file_type:" .. tostring(_res_file_type))
      self:_load_qz_file_list_with_type(copy_update_info, _res_file_type, address_list, function(_load_file_list_succ, ...)
        if _load_file_list_succ then
          local _patch_file_list, _patch_bak_list, _patch_ext_info = ...
          _patch_ext_info = _patch_ext_info or {}
          local p_file_info = {
            from = _patch_info[RTM.UPDATE_INFO_KEY.TARGET_VERSION],
            folder = _patch_ext_info.folder,
            index_file_md5 = _patch_ext_info.index_file_md5,
            file_list = _patch_file_list
          }
          if type(p_file_info) == "table" then
            _check_func(res_info, origin_file_info, p_file_info, down_origin_file_info, down_patch_file_info, nil, function(__succ, ...)
              if __succ then
                local _full_match_file_info, _down_origin_file_info, _down_patch_file_info = ...
                local full_match_file_infos = {}
                local apply_dest_path = SU.get_apply_path(namespace, res_key)
                if _full_match_file_info and next(_full_match_file_info) then
                  local _predown_full_match_file_info = {
                    can_move = false,
                    file_list = _full_match_file_info.file_list,
                    src_path = E.Path.join(apply_dest_path, EC.RES_PREDOWNLOAD.FOLDER_NAME)
                  }
                  table.insert(full_match_file_infos, _predown_full_match_file_info)
                end
                common_filter(origin_file_info, _down_origin_file_info, _down_patch_file_info, full_match_file_infos)
              else
                local err_code, err_msg = ...
                file_list_cb(false, err_code, err_msg)
              end
    ...
    self:_load_qz_file_list_with_type(update_info, RTM.FILE_LIST_TYPE.ORIGIN, address_list, function(_succ, ...)
      if _succ then
        ...
        function traverse_patches(index)
          E.LOG.debug(string.format("traverse_patches begin, index:%s, patches size:%d", tostring(index), #patches))
          if index <= #patches then
            ...
            local _check_opts = SU.get_reuse_opts()
            res_checker.hotfix_res_download_check(res_info, origin_file_info, patches_file_info_list, removed_res_list, _check_opts, function(__succ, ...)
              if __succ then
                local down_origin, down_patch = ...
                filter_predownload(origin_file_info, down_origin, down_patch)
              else
                local err_code, err_msg = ...
                file_list_cb(false, err_code, err_msg)
              end
        ...
        if SU.can_reuse_res() and PREDOWNLOAD_MGR.UTILS.is_predownload(res_key) == false then
          traverse_patches(1)
        else
          self:_load_qz_file_list_with_type(update_info, nil, address_list, function(_load_file_list_succ, ...)
            if _load_file_list_succ then
              local _index_file_list, _bak_old_file_list, _ext_file_info = ...
              _ext_file_info = _ext_file_info or {}
              local _file_list_type = _ext_file_info.file_list_type
              local _folder = _ext_file_info.folder
              local _index_file_md5 = _ext_file_info.index_file_md5
              local _origin_file_info = {}
              local _patch_file_info = {}
              local _down_file_info = {
                folder = _folder,
                index_file_md5 = _index_file_md5,
                file_list = _index_file_list
              }
              if _file_list_type == RTM.FILE_LIST_TYPE.ORIGIN then
                _origin_file_info = _down_file_info
              else
                _patch_file_info = _down_file_info
              end
              filter_predownload(origin_file_info, _origin_file_info, _patch_file_info)
            else
              local err_code, err_msg = ...
              EL.LOG.warn(TAG, "request_file_list with failed, code:" .. tostring(err_code) .. ", msg:" .. tostring(err_msg))
              file_list_cb(false, err_code, err_msg)
    ...
    self:_load_qz_file_list_with_type(update_info, nil, address_list, function(_succ, ...)
      if _succ then
        ...
        if is_single_file_mode then
          ...
          process_pending_download_list_fun(index_file_list, folder, nil, false, nil, _bak_old_file_list)
        end
      else
        local code, msg = ...
        EL.LOG.warn(TAG, "download file list failed and return, code:" .. tostring(code) .. ", msg:" .. tostring(msg))
  ...
  local function _process_file_info_list(succ, ...)
    if succ then
      ...
      if qz_file_info_list and qz_file_info_list.group then
        local GRM = require("ejoysdk_lua.res.group_res_manager")
        local _res_key = PREDOWNLOAD_MGR.UTILS.get_real_res_key(res_key)
        local real_res_key = GRM.get_real_res_key(_res_key)
        E.LOG.debug(TAG, string.format("reskey=%s,real_reskdy=%s", _res_key, real_res_key))
        local group_name
        if real_res_key == _res_key then
          group_name = "default"
        else
          group_name = GRM.get_group_name_by_task_key(_res_key)
        end
...
function M:mark_update_res_invalid(namespace, res_key, version, ext)
  EL.LOG.debug(TAG, string.format("mark_update_res_invalid:%s, res_key:%s, version:%s", tostring(namespace), tostring(res_key), tostring(version)))
...
function M.static_remove_downloading_version(namespace, res_key, version, ext)
  ...
  if E.Sysinfo.os() ~= "windows" or E.is_support_function(EC.NATIVE_SUPPORT_FUNCTION_NAMES.FILE_DIR_OPERATION) then
    ...
    EL.LOG.debug(TAG, string.format("begin remove file list config:%s, result:%s", tostring(file_list_path), tostring(fr)))
    for d, _ in pairs(res_dirs) do
      local r = EHU.os_remove(d)
      EL.LOG.debug(TAG, string.format("rm dir:%s, result:%s", tostring(d), tostring(r)))
    end
  ...
  E.LOG.debug(TAG, string.format("static_remove_downloading_version, namespace:%s, res_key:%s, version:%s, result:%s", tostring(namespace), tostring(res_key), tostring(version), tostring(rm_result)))
  return rm_result
end
...
function M:write_to_res_version_file(version, content_str, file_name)
  ...
  E.LOG.d(TAG, "write_res_version:" .. file_list_desc_path)
