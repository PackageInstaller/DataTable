...
function M:_init(_model_name, _namespace, _res_key, _opts)
  ...
  self._data = {
    ...
    download_strategy_type = _opts.network_strategy or NS.STRATEGY.BOTH_MOBILE_WIFI_NETWORK
  }
...
function M:update_opts(opts)
  ...
  E.LOG.d("EjoyDownloadStrategyAgent", {update_opts = opts})
  local _new_strategy = opts[END.NAMESPACE_UPDATE_OPTIONS.DOWNLOAD_NETWORK_STRATEGY]
  if _new_strategy then
    self._data.download_strategy_type = _new_strategy
    if self.task_agent then
      self.task_agent:update_strategy(_new_strategy)
    end
  end
end
...
function M:_do_check_local_res_state()
  ...
  E.LOG.d(TAG, {
    check_local_State = self._data.local_res_downloading_info
  })
  self:update_cached_res_info()
  return true
end
...
function M:_on_download_progress_changed()
  ...
  for _, lis in ipairs(self._data.progress_listeners) do
    lis(self._data.namespace, self._data.res_key, self._data.current_down_progress_info, state_copy)
  end
end

function M:_on_download_state_changed()
  self:log_debug("_on_download_state_changed")
  local state = self._data.local_res_downloading_info[RTM.RES_STATE_INFO_KEY.STATE]
  local state_copy = utils.deepcopy(self._data.local_res_downloading_info)
  for _, lis in ipairs(self._data.state_listeners) do
    lis(self._data.namespace, self._data.res_key, state, state_copy, self._data.current_down_progress_info)
  end
end
...
function M:_do_start_download(res_file_list_info, last_finish_downloading_size, total_size, last_finish_count, file_count)
  ...
  local _opts = {
    ...
    notification_present_ext = {
      ...
      default_progress_style = self._data.opts.default_progress_style
    },
    ...
    priority = self._data.opts.priority,
    sub_priority = self._data.opts.sub_priority,
    max_band_width_kbytes_per_sec = self._data.opts.max_band_width_kbytes_per_sec,
    disable_param_if_range = self._data.opts.disable_param_if_range,
  ...
  if task then
    self:log_debug("progress_info, download_strategy_type:" .. tostring(self._data.download_strategy_type))
...
function M:_start_check_download(request_confirm_cb, confirm_cb)
  ...
  self._data.res_src_instance:check_update_file_list(last_downloading_info, self._data.download_address_list, self._data.res_update_info, function(succ, ...)
    ...
    if succ then
      ...
      request_confirm_cb(self._data.namespace, self._data.res_key, res_state_info, function(confirm_download)
...
function M:start_download(confirm_cb)
  ...
  local function __start_check_download()
    ...
    local HOTFIX_MGR = require("ejoysdk_lua.res.hotfix.ejoysdk_res_hotfix")
    ...
    local is_predownload = PREDOWNLOAD_MGR.UTILS.is_predownload(self._data.res_key)
    if not SU.can_reuse_res() and not is_predownload and not PREDOWNLOAD_MGR.UTILS.will_become_official(self._data.namespace, self._data.res_key) and not HOTFIX_MGR.static_get_hotfix_mgr(self._data.namespace, self._data.res_key):has_history() then
      END.dispatch_res_update_confirm(self._data.namespace, self._data.res_key, res_state_info, function(confirm_ret)
        if confirm_ret then
          self:log_debug("update version download confirmed:" .. tostring(cur_update_version))
          self:_start_check_download(function(_ns, _rk, _rsi, _confirm_cb)
            _confirm_cb(confirm_ret)
          end, confirm_cb)
        else
      ...
      self:_start_check_download(function(_ns, _rk, _rsi, _confirm_cb)
        self:log_debug("download confirm")
        END.dispatch_res_update_confirm(_ns, _rk, _rsi, _confirm_cb)
      end, confirm_cb)
    end
  end
...
function M:remove_download(cb)
  if not self._data.task_agent then
    self:log_warn("remove_download skip, for no current download task")
    cb(true)
    return
  end
  self._data.task_agent:remove_download(function(succ, ...)
    if succ then
      self:log_debug("remove downloading succ")
      local DANP = require("ejoysdk_lua.res.ui.download_android_notif_presenter")
      DANP.cancel_download_notification()
      local downloading_res_version = self._data.local_res_downloading_info and self._data.local_res_downloading_info[RTM.RES_STATE_INFO_KEY.VERSION_NAME]
      self:remove_res(downloading_res_version)
      self:log_debug("remove_download remove downloading res_version: " .. tostring(downloading_res_version))
      local EMM = require("ejoysdk_lua.res.model.ejoy_http_download_multi_model")
      local state_obj = {
        [EMM.STATE_KEY.STATE] = EMM.DOWNLOAD_STATE.IDLE,
        [EMM.STATE_KEY.ERR_CODE] = EC.DOWNLOAD_ERROR_CODES.CANCELLED,
        [EMM.STATE_KEY.ERR_MSG] = "remove download update"
      }
      self:_on_notify_res_state_changed(EMM.DOWNLOAD_STATE.IDLE, state_obj)
      self._data.task_agent = nil
    else
      self:log_debug("remove downloading failed")
    end
    cb(succ, ...)
  end)
end
...
function M:update_download_config(config)
  ...
  self._data.task_agent:update_config(config)
end
...
function M:download_res_files(file_path_list, opts, state_listener, progress_listener)
  ...
  if type(opts.priority) ~= "number" then
    opts.priority = self._data.opts.priority
  end
  if "number" ~= type(opts.sub_priority) then
    opts.sub_priority = self._data.opts.sub_priority
  end
  opts.notification_present_ext = opts.notification_present_ext or {}
  if type(opts.foreground_notification_custom_config) ~= "table" then
    opts.notification_present_ext.custom_config = self._data.foreground_notification_custom_config
  end
  if not EU.is_text_empty(opts.default_progress_style) then
    opts.notification_present_ext.default_progress_style = self._data.opts.default_progress_style
  end
  ...
  local function prepare_origin_list_handler(_file_list, _group_list)
    ...
    E.log(file_ext)
    E.log(opts)
    E.LOG.d("QZ_RES_HOTFIX", {pending_download_list = pending_download_list})
    local task, code, msg = multi_model:create_task(pending_download_list, file_ext, opts, function(_state, _state_obj)
      if _state == EMM.DOWNLOAD_STATE.COMPLETE then
        ...
        E.log(_state_obj)
        ext_info.download_info = {file_list = _file_list, group_list = _group_list}
    ...
    self._data.res_src_instance:get_origin_file_list_info(current_update_info_ver, self._data.res_update_info, self._data.download_address_list, function(_succ, ...)
      if _succ then
        ...
        for _, _path in ipairs(file_path_list) do
          ...
          utils.insert_array(_group_list, item_group)
        end
        prepare_origin_list_handler(file_list, _group_list)
      else
        local _code, _msg = ...
