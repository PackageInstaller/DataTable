...
local USING_PRE_RES_CONFIG = "pre_config"
...
M.INFO_TYPE_KEY = {
  ...
  TYPE_USING_PRE_RES_INFO = "using_pre_res_info",
...
M.RES_CONFIG_KEY = {
  ...
  KEY_PATCHES = "patches",
  KEY_PACK_KEY = "packKey",
  KEY_EXT = "ext",
  KEY_HANDLER = "handler",
  KEY_RAW_MAPPING_FILE = "rawMappingFile",
  KEY_RAW_MAPPING_FILE_MD5 = "rawMappingFileMd5",
  KEY_RAW_MAPPING_INDEX_FILE = "rawMappingIndexFile",
  KEY_RAW_MAPPING_INDEX_FILE_MD5 = "resMappingIndexFileMd5"
}
...
M.RES_UPDATES_OPTIONS = {
  ...
  USING_PREDOWNLOAD = "using_predownload"
}
...
function M.static_load_all_local_namespace_res()
  ...
  for ns, res_set in pairs(namespace_res_map) do
    ...
    for res_key, _ in pairs(res_set) do
      ...
      local using_ns_pre_res_key_cfg_path = ns_res_key_base_path .. USING_PRE_RES_CONFIG
      ...
      local using_pre_res = JSON.decode(E.File.readfile(using_ns_pre_res_key_cfg_path)) or {}
      ...
      res_data[M.NAMESPACE_RES_CONFIG_KEY.TYPE_USING_PRE_RES_INFO] = using_pre_res
      if dl_res then
      ...
      local pre_mgr = PREDOWNLOAD_MGR.static_get_mgr(ns, res_key)
      pre_mgr:local_info(using_pre_res)
    end
  end
...
local function _get_using_res_config_path(namespace, res_key, config)
  local ns_res_key_base_path = M.static_get_res_path(namespace, res_key)
  local using_ns_res_key_cfg_path = ns_res_key_base_path .. config
  return using_ns_res_key_cfg_path, ns_res_key_base_path
end

function M.static_get_using_res_config_path(namespace, res_key)
  do return _get_using_res_config_path, namespace, res_key end
  return _get_using_res_config_path, namespace, res_key, USING_RES_CONFIG
end

function M.static_get_using_res_predown_config_path(namespace, res_key)
  do return _get_using_res_config_path, namespace, res_key end
  return _get_using_res_config_path, namespace, res_key, USING_PRE_RES_CONFIG
end
...
function M.static_update_cached_res_info(namespace, res_key, using_res_info, _downloading_info, _progress_info)
  ...
  for ns, res_map in pairs(namespace_res_collection) do
    ...
    for rk, _ in pairs(res_map) do
      table.insert(res_list_str, rk)
    end
    res_list_str = table.concat(res_list_str, "\n")
...
function M.static_clear_cached_res_info(namespace, res_key)
  ;(namespace_res_collection or {})[namespace] or {}[res_key] = nil
  static_save_namespace_res_collection(namespace, res_key)
end
