...
function M:_init(task, strategy_name, state_listener)
  ...
  function self._data.strategy_state_change_listener(strategy_satisfied, _strategy_name)
    self:update_strategy(_strategy_name)
...
function M:update_strategy(strategy_name)
  ...
  if not strategy_processor then
    E.LOG.warn(TAG, "update_strategy skip, strategy_name not find strategy: " .. tostring(strategy_name))
    return
  end
...
function M:get_strategy()
  return self._data.strategy_name
end
...
function M:remove_download(cb)
  self:close(function(succ, ...)
    if true == succ then
      self._data.task:remove_download(cb)
    elseif cb then
      cb(succ, ...)
    end
  end)
...
function M:update_config(config)
  if self._data.task then
    self._data.task:update_download_config(config)
  end
end
