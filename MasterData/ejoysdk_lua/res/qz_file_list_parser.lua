...
local function parse_res_key(res_key)
  ...
  if true == is_predownload then
    _res_key = PREDOWNLOAD_MGR.UTILS.get_real_res_key(_res_key)
  end
  local group_res_key_prefix = RTM.RES_GROUP_INFO_KEY.PREFIX
  local is_group_key = false
  if E.Utils.start_with(_res_key, group_res_key_prefix) then
    is_group_key = true
    local items = E.Utils.split_string(_res_key, "-")
    local real_res_key = items[2]
    local res_prefix = group_res_key_prefix .. items[2] .. "-"
    local group_name = string.sub(_res_key, #res_prefix + 1)
    if true == is_predownload then
      real_res_key = PREDOWNLOAD_MGR.UTILS.get_task(real_res_key)
    end
    return is_predownload, is_group_key, real_res_key, group_name
  else
    if true == is_predownload then
      _res_key = PREDOWNLOAD_MGR.UTILS.get_task(_res_key)
    end
    return is_predownload, is_group_key, _res_key
  end
end
  ...
  if file_list_cache[cache_key] then
    ...
    for key, _ in pairs(file_list_cache) do
      if key == cache_key or E.Utils.start_with(key, group_cache_key_prefix) then
        ...
        table.insert(_remove_table_tmp, key)
      end
    end
    for _, key in ipairs(_remove_table_tmp) do
...
function M.parse_file_list(namespace, res_key, local_version, version, qz_file_list_content)
  ...
  local cache_key = get_res_cache_key(namespace, real_res_key, local_version, version)
  local qz_file_list = qz_file_list_content
...
function M.get_file_list(namespace, res_key, local_version, version, is_total)
  local _is_predownload, is_group_key, real_res_key, group_name = parse_res_key(res_key)
  local cache_key = get_res_cache_key(namespace, real_res_key, local_version, version)
  E.LOG.debug(TAG, "get file list, cache key is " .. tostring(cache_key) .. ",res_key:" .. tostring(res_key) .. ",real_res_key:" .. tostring(real_res_key) .. ", local_version:" .. tostring(local_version) .. ", version: " .. tostring(version) .. ", is_total: " .. tostring(is_total))
  if is_total then
    E.LOG.debug(TAG, "res_key is " .. tostring(real_res_key) .. ", get total file list")
    do return get_total_file_list end
    return get_total_file_list, cache_key, "res_key is " .. tostring(real_res_key) .. ", get total file list", tostring(real_res_key), ", get total file list", tostring(res_key), ",real_res_key:", tostring(real_res_key), ", local_version:", tostring(local_version), ", version: ", tostring(version), ", is_total: ", tostring(is_total), is_total
  end
  if is_group_key then
    E.LOG.debug(TAG, "is group_key, res_key is " .. tostring(real_res_key) .. ", group_name is " .. tostring(group_name))
    do return get_group_file_list, cache_key end
    return get_group_file_list, cache_key, group_name, tostring(real_res_key), ", group_name is ", tostring(group_name), group_name, tostring(real_res_key), ", local_version:", tostring(local_version), ", version: ", tostring(version), ", is_total: ", tostring(is_total), is_total
  else
    E.LOG.debug(TAG, "not group_key, res_key is " .. tostring(real_res_key) .. ", cache_key is " .. tostring(cache_key))
    do return get_default_file_list end
    return get_default_file_list, cache_key, "not group_key, res_key is " .. tostring(real_res_key) .. ", cache_key is " .. tostring(cache_key), tostring(real_res_key), ", cache_key is ", tostring(cache_key), cache_key, tostring(real_res_key), ", local_version:", tostring(local_version), ", version: ", tostring(version), ", is_total: ", tostring(is_total), is_total
  end
end
...
function M.get_file_list_cache(namespace, res_key, local_version, target_version, is_total, storage_type)
  ...
  if not qz_file_list_info then
    ...
    if E.File.exists_fullpath(index_info_file_cache_path) then
      ...
      list_content = CJSON.safe_decode(list_content)
...
function M.get_file_list_type(namespace, res_key, version)
  local _is_predownload, is_group_key, real_res_key, _group_name = parse_res_key(res_key)
