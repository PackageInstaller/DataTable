...
local DM = require("ejoysdk_lua.launcher.default_server_model")
local M = {}
M.SOURCE_TYPE = {
...
function M._update_alive_value(target_server_info, new_alive_server_info)
  fill_alive_value_v2({target_server_info}, {new_alive_server_info})
end
...
local function fetch_server_info(_params, _server_ids, _cb)
  ...
  E.LOG.debug(TAG, "server_list_ticket_detail_with_params_v2 >>>")
  E.log(server_list_params)
  launcher.server_list_ticket_detail_with_params_v2(function(succ1, ...)
    if succ1 then
      ...
      local alive_servers_fun = EG.alive_servers_with_params
      if _params.enable_request_alive_with_auth then
        local permit_info = EG.user_info().permit_info or {}
        local is_white = permit_info.is_white or false
        E.LOG.debug(TAG, "fetch_server_info enable_request_alive_with_auth is true, is_white:" .. tostring(is_white))
        if is_white then
          alive_servers_fun = EG.alive_servers_auth_with_params
        end
      end
      alive_servers_fun(function(succ2, ...)
        if succ2 then
          local alive_servers = (...)
          fill_alive_value_v2(server_list, alive_servers)
...
local function get_my_server_info_for_last_login(params, _cb)
  _ejoysdk.log("get_my_server_info_for_last_login last_server_source_type:" .. tostring(last_server_source_type) .. ", last_server_id:" .. tostring(last_server_id))
  ...
  local function real_get_last_login_server(final_last_login_server_id)
    E.LOG.debug(TAG, "get_my_server_info_for_last_login real_get_last_login_server final_last_login_server_id:" .. tostring(final_last_login_server_id))
    E.log(params)
    fetch_server_info(params, {final_last_login_server_id}, function(succ2, ...)
      ...
      E.LOG.debug(TAG, "get_my_server_info_for_last_login real_get_last_login_server fetch_server_info succ")
      E.log(ret)
      get_server_marks(params or {}, function(succ4, ...)
        ...
        if succ4 then
          ...
          E.LOG.debug(TAG, "get_my_server_info_for_last_login real_get_last_login_server get_server_marks succ")
          E.log(ret)
          callback(ret)
        else
  ...
  get_and_sort_last_login_player(function(succ, ...)
    if not succ then
      local code, msg = ...
      E.LOG.debug(TAG, "get_my_server_info_for_last_login failed, code:" .. tostring(code) .. ", msg:" .. tostring(msg))
      utils.safe_call_cb(_cb, false, ...)
      return
    end
    local last_login_player_server_id = (...)
    E.LOG.debug(TAG, "get_my_server_info_for_last_login get_and_sort_last_login_player last_login_player_server_id:" .. tostring(last_login_player_server_id))
    real_get_last_login_server(last_login_player_server_id)
  end)
end

function M.request_recommend_server_list(params, _cb)
  local function fetch_server_info_inner(recommend_server_ids, marks)
    fetch_server_info(params, recommend_server_ids, function(succ3, ...)
      ...
      if alive_list and next(alive_list) ~= nil then
        ...
        utils.safe_call_cb(_cb, true, alive_list, marks)
      elseif un_alive_list and next(un_alive_list) ~= nil then
        M._sort_server_info_list(un_alive_list)
        utils.safe_call_cb(_cb, true, un_alive_list, marks)
      else
        utils.safe_call_cb(_cb, false, CONSTANTS.GANGPLANK_ERROR_CODE.GANGPLANK_ERROR_TARGET_SERVER_NOT_FOUND, "target server not found")
      end
    end)
...
function M.request_server_info(server_id, params, _cb)
  fetch_server_info(params, {server_id}, function(succ, ...)
    if not succ then
      utils.safe_call_cb(_cb, false, ...)
      return
    end
    local server_info_list = (...)
    local ret = utils.safe_get_array_item(server_info_list, 1)
    E.LOG.debug(TAG, "request_server_info succ:")
    E.log(ret)
    if not ret then
      utils.safe_call_cb(_cb, false, CONSTANTS.GANGPLANK_ERROR_CODE.GANGPLANK_ERROR_TARGET_SERVER_NOT_FOUND, "target server not found")
      return
    ...
    get_server_marks(params or {}, function(succ4, ...)
      ...
      if succ4 then
        ...
        for id, mark in pairs(marks) do
          for _, m in ipairs(mark or {}) do
            if "recommend" == m then
              table.insert(recommend_server_ids, id)
              break
            end
          end
        end
...
local function get_my_server_info_for_recommend(params, _cb)
  ...
  M.request_recommend_server_list(params, function(succ, ...)
    if succ then
      local sorted_recommend_server_list, marks = ...
      local index = math.random(1, math.ceil(#sorted_recommend_server_list / 4))
      local ret = utils.safe_get_array_item(sorted_recommend_server_list, index)
      marks = marks or {}
      ...
      E.LOG.debug(TAG, "request_recommend_server succ, last_server_id:" .. tostring(last_server_id) .. ", last_server_source_type:" .. tostring(last_server_source_type) .. ", last_server_recommend_data_info:" .. tostring(last_server_recommend_data_info))
      utils.safe_call_cb(_cb, true, ret)
    else
      local code, msg = ...
...
function M.select_server_for_entergame(server_id)
  DM.select_server_for_entergame(server_id)
end

function M.select_player_for_entergame(player_id, server_id)
  DM.select_player_for_entergame(player_id, server_id)
end

function M.get_default_login_server(params, on_result, on_update)
  DM.get_default_login_server(params, on_result, on_update)
end

function M.get_select_server()
  do return end
  return DM.get_select_server, nil
end

function M.Test_reset_server_cache()
  server_marks_cache = nil
  recommend_servers_cache = nil
end
