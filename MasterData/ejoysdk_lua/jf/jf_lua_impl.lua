...
local function init_jf_file_cache()
  ...
  E.LOG.debug(TAG, "is_support_jf_file_cache: " .. tostring(is_support_jf_file_cache) .. ", use_jf_file_cache: " .. tostring(use_jf_file_cache))
  if is_support_jf_file_cache then
...
local function check_install_event(event_params)
  ...
  if event_name == JF_LUA_CONFIG.EVENT_NAMES.SDK_START_UP_SUCCESS then
    ...
    E.LOG.debug(TAG, "update_time is " .. update_time .. ", cache_update_time is " .. cache_update_time)
    if update_time ~= cache_update_time then
      ...
      if E.Sysinfo.os() == "harmonyos" then
        ...
        JF.commit_event_with_security_for_harmony(JF_LUA_CONFIG.EVENT_NAMES.SDK_INSTALL, event_params.opts, {is_upload_now = true})
      else
        local security = require("ejoysdk_lua.vendors.security")
        local mw = security.ejoysdk_get_mw()
        if mw then
          install_event.params.secInfo = mw
        end
        E.LOG.debug(TAG, "security.get_mw_sync for install event, commit event with secInfo")
