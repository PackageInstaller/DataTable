...
local function login_base(server, region, outsource, token, cb)
  ...
  local AU = require("ejoysdk_lua.res.app_update.app_update_manager")
  local params = {
  ...
  if AU.get_app_install_success() then
    params.current_version = AU.get_app_update_current_version()
  end
  E.log(params)
  local stat_params = {
    stack = E.HTTP.encode_uri(debug.traceback())
...
function M.open_userinfo_completion_with_args(vendor_name, args, cb)
  local user_info = M.user_info()
  if user_info and user_info.token and string.len(user_info.token) > 0 then
    local vendor_ret = CM.get_vendor(vendor_name, EV.ABILITY.ACCOUNT)
    CM.open_userinfo_completion_with_args(vendor_ret.vendor_name, args, cb)
  else
    E.LOG.error(TAG, "open userinfo completion page fail, not login yet")
  end
end

function M.open_account_binding(vendor_name, args, cb)
  local user_info = M.user_info()
  if user_info and user_info.token and string.len(user_info.token) > 0 then
    local vendor_ret = CM.get_vendor(vendor_name, EV.ABILITY.ACCOUNT)
    CM.open_account_binding(vendor_ret.vendor_name, args, cb)
  else
    cb(false, CONSTANTS.GANGPLANK_ERROR_CODE.GANGPLANK_ERROR_NEED_ACCOUNT_LOGIN, "not login")
    E.LOG.error(TAG, "open account binding page fail, not login yet")
  end
end

function M.get_account_binding_list(vendor_name, cb)
  local user_info = M.user_info()
  if user_info and user_info.token and string.len(user_info.token) > 0 then
    local vendor_ret = CM.get_vendor(vendor_name, EV.ABILITY.ACCOUNT)
    CM.get_account_binding_list(vendor_ret.vendor_name, cb)
  else
    cb(false, CONSTANTS.GANGPLANK_ERROR_CODE.GANGPLANK_ERROR_NEED_ACCOUNT_LOGIN, "not login")
    E.LOG.error(TAG, "get account binding list fail, not login yet")
  end
end
...
function M.set_player_info(player_info, player_info_type, cb)
  ...
  ET.publish(ET.gangplank.SET_PLAYER_INFO_INVOKED, clone_player_info)
end

function M.player_offline()
  has_set_player_info = false
  QL.commit_event(QL.EVENT_NAMES.SDK_PLAYER_OFFLINE_INVOKE, GDP.PLAYER_INFO.get_simple(), true)
  GDP.PLAYER_INFO.clear()
...
local function product_infos_base(channel, cb)
  ...
  local area = E.CONFIG.get_config("district")
  if is_oversea then
    local SC = require("ejoysdk_lua.user_center.system_config")
    if SC.get("pay_lock_area_switch") then
      E.LOG.debug(TAG, "system config pay_lock_area_switch is true")
      local OFFICIAL = require("ejoysdk_lua.vendors.official")
      local payArea = OFFICIAL.getLastLoginAccountPayArea()
      E.LOG.debug(TAG, {payArea = payArea})
      if not UTILS.STR.is_empty(payArea) then
        area = payArea
      end
    end
  end
...
function M.init(opts, listeners)
  ...
  local function publish_init_failed(status, message)
    E.LOG.error(TAG, "init failed, status:" .. tostring(status) .. ", msg:" .. tostring(message))
...
function M.open_shop(url, cb)
  ...
  if user_info and user_info.token and string.len(user_info.token) > 0 then
    USER_CENTER.get_login_auth_code(url, function(succ, ...)
      E.LOG.debug(TAG, "get_login_auth_code:" .. tostring(succ))
      E.log(...)
      if succ then
        local auth_code = (...)
        E.LOG.debug(TAG, "get_login_auth_code auth_code:" .. tostring(auth_code))
        local base, hash = url, ""
        local hash_pos = string.find(url, "#", 1, true)
        if hash_pos then
          base = string.sub(url, 1, hash_pos - 1)
          hash = string.sub(url, hash_pos)
        end
        local sep = string.find(base, "?", 1, true) and "&" or "?"
        local _url = base .. sep .. "game_auth_code=" .. auth_code .. hash
        E.LOG.debug(TAG, "get_login_auth_code url:" .. _url)
        if cb then
          cb(true, _url)
        end
        open_url(_url)
        ...
        E.LOG.error(TAG, "get_login_auth_code failed: " .. tostring(code) .. ", " .. msg)
        open_url(url)
        ESTAT.sdk_login_pfcomm({
          biz_mod = "ingame_login",
          biz_func = "open_shop",
          ...
          biz_st = "fail",
          biz_co = code or "400",
          biz_params = {msg = msg, url = url}
        })
      end
    end)
  else
    cb(false, CONSTANTS.GANGPLANK_ERROR_CODE.GANGPLANK_ERROR_NEED_ACCOUNT_LOGIN, "not login")
    E.LOG.error(TAG, "not login yet，open_shop with no auth code")
    open_url(url)
    ESTAT.sdk_login_pfcomm({
      biz_mod = "ingame_login",
      biz_func = "open_shop",
      ...
      biz_st = "fail",
      biz_co = "401",
      biz_params = {msg = "not login", url = url}
    })
  end
end
