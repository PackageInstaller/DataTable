...
local function convert_purchase_params(media, params)
  ...
  for origin_key, target_key in pairs(convert_purchase_keys) do
    ...
    if "product_price" == origin_key and type(params[origin_key]) == "string" then
      local number_price = tonumber(params[origin_key])
      if number_price then
        params[target_key] = number_price
      else
        E.LOG.error(TAG, "convert_purchase_params: invalid number string for product_price: " .. tostring(params[origin_key]))
