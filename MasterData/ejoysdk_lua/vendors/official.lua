...
local URL_CONFIG = {
  ...
  API = {
    ...
    social_account_binding = "m#/account/social?from=sdk",
...
function OFFICIAL.get_login_url()
  ...
  local airline_brand = URL_CONFIG.airline_info and URL_CONFIG.airline_info.brand
  local airline_node = URL_CONFIG.airline_info and URL_CONFIG.airline_info.node
  local append_params = "?"
  if string.find(url, "?") then
    append_params = "&"
  end
  if not airline_brand or "" == airline_brand then
    local CONFIG = require("ejoysdk_lua.ejoysdk_config")
    local meta_data = CONFIG.get_config("unisdk_meta")
    airline_brand = meta_data and meta_data.brand
  end
  append_params = append_params .. "airlineType=" .. (airline_brand or "")
  append_params = append_params .. "&" .. "serverNode=" .. (airline_node or "")
...
function OFFICIAL.getLastLoginAccountPayArea()
  if last_login_account_data then
    return last_login_account_data.pay_lock_area
  end
  return nil
end
...
function OFFICIAL.login(ext, cb)
  ...
  local function login_callback(result)
    ...
    if result and result:is_succ() then
      ...
      last_login_account_data = account_data
    end
...
local function open_webview_with_args(api, args, js_cb, close_cb, extra_data)
  local args_str = HTTP.urlencode(args)
  update_airline_url()
  local url = OFFICIAL.get_api_url(api)
  if args_str then
    url = url .. "&" .. args_str
...
function OFFICIAL.open_userinfo_completion_with_args(args, cb)
  local result = false
  open_webview_with_args("user_detail_completion", args, function(_args)
    if cb and _args.type == "update_user_info" then
      result = _args.result or false
    end
  end, function()
...
function OFFICIAL.open_account_binding(args, cb)
  local result = {}
  open_webview_with_args("social_account_binding", args, function(_args)
    if cb and _args.type == "update_account_binding" then
      result = _args.result or {}
      if result and result.succ then
        cb(true, result.thirdPartyList or {}, nil)
      else
        local code = result and result.code or CONSTANTS.OFFICIAL_ERR_CODES.CODE_BIND_FAIL
        local msg = result and result.msg or "Binding failed"
        cb(false, code, msg)
      end
    end
  end, function()
    E.LOG.debug(TAG, "social_account_binding closed")
  end)
end
...
function OFFICIAL.get_account_binding_list(cb)
  local user_info = USER.user_info() or {}
  E.LOG.debug(TAG, "start show user_info ")
  E.log(user_info)
  local Login = require("ejoysdk_lua.overseas.login")
  Login.available_login_list(function(...)
    E.LOG.debug(TAG, "start get_account_binding_list available_login_list ")
    E.log(...)
    local resp = (...)
    if resp and resp.succ then
      local available_login_list = resp.body
      local has_airline = false
      local temp_list = {}
      if available_login_list then
        for _, item in ipairs(available_login_list) do
          if item.type == "AIRLINE" then
            has_airline = true
          elseif item.type == "APPLE_LOGIN_H5" then
            item.type = "APPLE_LOGIN"
          end
          if item.type ~= "GOOGLE_PLAY" then
            table.insert(temp_list, item)
          end
        end
        available_login_list = temp_list
        if has_airline then
          table.insert(available_login_list, {type = "EMAIL"})
          table.insert(available_login_list, {type = "MOBILE"})
        end
        local params = {
          token = user_info.token
        }
        UP.post(UP.SERVICE.GET_ACCOUNT_USERINFO, params, function(succ, ...)
          E.LOG.debug(TAG, "get_account_binding_list get_account_userinfo: " .. tostring(succ))
          local thirdPartyList = {}
          if succ then
            local data = (...)
            E.log(data.thirdPartyList)
            if not data or not data.thirdPartyList then
              E.LOG.debug(TAG, "get_account_binding_list thirdPartyList is nil")
              cb(true, available_login_list)
              return
            end
            local bound_map = {}
            for _, item in ipairs(data.thirdPartyList) do
              local key = LoginTypeMap[item.thirdPartyType] or "UNKNOWN"
              if "UNKNOWN" ~= key then
                bound_map[key] = item
              end
            end
            for _, login in ipairs(available_login_list) do
              local t = login.type
              local entry = {type = t}
              local bound = bound_map[t]
              if bound then
                entry.thirdPartyUid = bound.thirdPartyUid
                entry.thirdPartyInfo = bound.thirdPartyInfo
              end
              if "AIRLINE" ~= t and "AGST" ~= t and "QR_LOGIN" ~= t then
                table.insert(thirdPartyList, entry)
              end
            end
            cb(true, thirdPartyList)
          else
            local code, msg = ...
            cb(false, code, msg)
          end
