...
function M.init(opt, cb)
  ...
  if jsonTable and next(jsonTable) and jsonTable.epicsandboxid and jsonTable.epicdeploymentid then
    sandboxIdLaunch = jsonTable.epicsandboxid
    deploymentIdLaunch = jsonTable.epicdeploymentid
  else
    E.LOG.debug(TAG, "epic launch args check failed, may has no epicsandboxid or epicdeploymentid")
    E.LOG.debug(TAG, {jsonTable = jsonTable})
    if cb then
      cb(false, CONSTANTS.CHANNEL_ERROR_CODE.CHANNEL_CHECK_ARGS_FAIL, "epic launch args check failed")
    end
    return
  end
  epic_init_params = {
