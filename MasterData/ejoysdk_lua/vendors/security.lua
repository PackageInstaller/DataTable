...
if ok and OUT_SE then
  function OUT_SE.ejoysdk_get_mw()
    if E.Sysinfo.os() == "harmonyos" then
      local jf = require("ejoysdk_lua.vendors.jf")
      
      E.LOG.debug("SECURITY", "get_mw_sync from jf cache")
      E.log(jf.secInfoCache)
      return jf.secInfoCache
    end
    local security = require("ejoysdk_lua.vendors.security")
    if security and type(security.get_mw_sync) == "function" then
      local success, result = pcall(security.get_mw_sync)
      if success then
        return result
      else
        E.LOG.warn("SECURITY", "Failed to call security.get_mw_sync: " .. tostring(result))
        return nil
      end
    else
      E.LOG.warn("SECURITY", "security.get_mw_sync is not available")
      return nil
    end
  end
  
  if OUT_SE.get_mw_async == nil then
    function OUT_SE.get_mw_async(cb)
      E.LOG.warn("SECURITY", "security.get_mw_async is not available, check game_security.lua version")
    end
  end
  return OUT_SE
end
...
function M.ejoysdk_get_mw()
  E.LOG.warn("SECURITY", "security.get_mw_sync is not available")
end

function M.get_mw_async()
  E.LOG.warn("SECURITY", "security.get_mw_async is not available")
end
