...
local function setupPayControlSwitch()
  ...
  if _ejoysdk.os() == "harmonyos" or _ejoysdk.os() == "windows" or _ejoysdk.os() == "weixin" or _ejoysdk.os() == "douyin" or is_airline_v2_login_and_go_ejoypay or is_cloudgame then
    ...
    if PC_MANAGER.check_is_pc_channel_pkg() then
      need_go_pay_prewrite_way = false
    else
...
function M.pay(product_id, _count, gp_order_id, _body)
  ...
  local function ds_create_order_fail_show_alert(code, message)
  ...
  if "windows" ~= os or is_windows_official_channel_id() then
    ...
    if need_go_pay_prewrite_way then
      ...
      AP.create_order(pay_params, channel_ext, function(succ, ...)
        ...
        if succ then
          ...
          ds_create_order_fail_show_alert(code, message)
        end
      end)
    else
      if "android" == os then
    ...
    if windows_channel_vendor and windows_channel_vendor.is_support_channel_sdk() then
      E.LOG.debug(TAG, "go windows channel sdk pay, first do create ds order")
      local account_pay_span = ATracer.start_pay_sub_span(ATracer.PAY_FLOWS_SPANS.ACCOUNT_PAY)
      ESTAT.create_order_report({
        biz_func = "sdk.create_ds_order",
        biz_act = "start",
      ...
      AP.create_order(pay_params, nil, function(succ, ...)
        local temp_data = (...)
        ESTAT.create_order_report({
          biz_func = "sdk.create_ds_order",
          biz_act = "finish",
        ...
        if succ then
          ...
          local ds_order_id = body_data.dsOrderId
          product.min_order_amt = body_data.minOrderAmount
          E.LOG.debug(TAG, "go to windows channel sdk  pay, ds_order_id >>" .. tostring(ds_order_id))
          E.LOG.debug(TAG, {body_data = body_data})
          ATracer.finish_pay_sub_span(account_pay_span, body_data)
          local attach_info_str = body_data.ex
          if body_data.ex and type(body_data.ex) == "table" then
          ...
          E.LOG.debug(TAG, "go windows channel sdk pay function")
          windows_channel_vendor.pay(gp_order_id, ds_order_id, product, attach_info_str)
        else
          local code, message = ...
          E.LOG.debug(TAG, "create ds order fail, code: " .. tostring(code) .. ", message: " .. tostring(message))
          local resp = {
    ...
    E.LOG.debug(TAG, "callback pay_listener false, do not support pay")
  ...
  E.HTTP.post(query_ipv4_url, query_ipv4_params, E.HTTP.CT_JSON, query_ipv4_body, function(resp)
    ...
    if resp.status == 200 and resp.body and 200 == resp.body.state then
      ...
      if E.Sysinfo.os() == "harmonyos" then
        JF.commit_event_with_security_for_harmony(SDK_START_UP_IPV4, ipv4_params, {is_upload_now = true})
      else
        if mw then
          ipv4_params.secInfo = mw
        end
        JF.commit_event(SDK_START_UP_IPV4, ipv4_params, {is_upload_now = true})
      end
    else
      ...
      if E.Sysinfo.os() == "harmonyos" then
        JF.commit_event_with_security_for_harmony(SDK_START_UP_IPV4, fail_params, {is_upload_now = true})
      else
        if mw then
          fail_params.secInfo = mw
        end
        JF.commit_event(SDK_START_UP_IPV4, fail_params)
      end
...
function M.init(opt, cb)
  ...
  if _ejoysdk.os() ~= "android" and check_if_support_vendor("AIRLINE_V2") then
    E.LOG.debug(TAG, "当前接入了灵犀2.0，且不是安卓系统，就不再需要初始化灵犀1.0插件了")
    if cb then
      cb(true)
    end
  else
    E.LOG.debug(TAG, "go init airline_v1")
