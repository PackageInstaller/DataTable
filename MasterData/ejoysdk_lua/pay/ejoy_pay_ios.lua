...
local last_pay_product_id, last_gp_order_id, last_pay_params
local is_retry_pay_by_sdk = false
...
local function ios_pay_result_handler(succ, _order_id, ext_params)
  ...
  if succ then
    ...
    if code == CONSTANTS.EJOYPAY_ERR_CODES.CODE_PAY_FAIL_PRODUCT_EMPTY and not is_retry_pay_by_sdk and last_pay_params then
      E.LOG.debug(TAG, "now need to retry pay with ios old function")
      last_pay_params.useDeprecatedMethod = true
      is_retry_pay_by_sdk = true
      UNI.pay(VENDOR_NAME, last_gp_order_id, last_pay_params)
      ESTAT.app_pay_report({
        biz_func = "sdk.apple_retry_pay",
        biz_act = "finish",
        biz_st = "succ",
        biz_co = "200",
        biz_params = {last_pay_params = last_pay_params, last_gp_order_id = last_gp_order_id}
      })
      return
    end
    local ret = UNI.sync_call(VENDOR_NAME, SYNC_GET_RECEIPT, {}, nil)
...
function M.pay(gp_order_id, _ds_order_id, product, pay_params)
  ...
  if appleProduct then
    ...
    pay_params.country_code = appleProduct.priceLocale and appleProduct.priceLocale.countryCode
  end
  ...
  PAYPROTOCOL.post(PAYPROTOCOL.SERVICE.PAY_PRE_CREATE_ORDER_IOS, pay_params, function(succ, ...)
    ...
    if succ then
      ...
      last_gp_order_id = gp_order_id
      last_pay_params = pay_params
