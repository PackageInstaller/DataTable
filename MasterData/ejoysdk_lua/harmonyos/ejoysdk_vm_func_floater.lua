...
local m_parameter_has_dragable = false
...
local function on_drag_move(x, y)
  if false == m_draggable and m_parameter_has_dragable then
    return
  end
...
local function import_webpage(params, cb)
  if m_webpage then
    cb(m_webpage)
    return
  end
      ...
      ns_module.setWebviewListener({
        onJsArgsEvent = function(_value)
          ...
          if value and value.args then
            E.LOG.debug(TAG, "onJsArgsEvent args>>")
            E.log(value.args)
            if value.args.type == "oauthUri" then
              ET.publish("logindone", value)
              CET.publish("logindone", value)
            else
              ET.publish("webview_jsargs", value)
              CET.publish("webview_jsargs", value)
            end
          end
        end,
...
local function floater_set_dragable(draggable, parameter_has_dragable)
  m_draggable = draggable
  m_parameter_has_dragable = parameter_has_dragable
  E.LOG.debug(TAG, "floater_set_dragable draggable:" .. tostring(draggable) .. ", parameter_has_dragable:" .. tostring(parameter_has_dragable))
end
...
local function floater_load_url(params)
  ...
  local parameter_has_dragable = false
  local drag_padding
  local debuggable = false
  if ctr_params then
    inject_js = ctr_params.injectJS or ""
    parameter_has_dragable = type(ctr_params.draggable) == "boolean"
    dragable = ctr_params.draggable or false
    injection = ctr_params.injection or {}
    options = ctr_params.options or {}
    drag_padding = ctr_params.drag_padding
    debuggable = ctr_params.debuggable or false
  end
  m_inject_js = inject_js
  m_url = params.url
  floater_set_dragable(dragable, parameter_has_dragable)
  ...
  local web_info = {
    EjoyWebViewPage_WebViewInfo = {
      ...
      draggable = dragable,
      parameterHasDraggable = parameter_has_dragable
    },
    EjoyWebViewPage_dragPaddingInfo = drag_padding
  }
  E.LOG.debug(TAG, "floater_load_url params:")
  E.log(params)
...
local function create_webpage_window(params, cb)
  E.LOG.debug(TAG, "create_webpage_window:")
  local create_params = params.params or {}
  E.log(params)
  ...
  m_float_browser_window:set_on_touch_outside_listener(function()
    if create_params.disable_outside_touch then
      E.LOG.debug(TAG, "window disable_outside_touch is true, begin hide")
      M.hide()
    else
      E.LOG.debug(TAG, "window disable_outside_touch is false, ignore touch outside event")
    end
  end)
end
...
function M.create(params, cb)
  ...
  import_webpage(params, function(webpage, ...)
    if webpage then
