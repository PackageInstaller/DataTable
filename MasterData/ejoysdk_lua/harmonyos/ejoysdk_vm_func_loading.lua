...
local function prepare_loading_window(cb, options, click_cb)
    ...
    prepare_loading_page(function(page_name, ...)
      if page_name then
        ...
        if options and options.title then
          local loading_info = {
            EjoyLoadingPage_EjoyLoadingInfo = {
              title = options.title or "",
              cancelButtonTitle = options.cancel_button_title or "取消",
              closeCallback = function(index)
                M.dismiss()
                if click_cb then
                  click_cb({value = index})
                end
              end
            }
          }
          local_storage = jf.webview():toLocalStorage(loading_info)
        end
        window.loadContentByName(page_name, local_storage, function(_err)
...
function M.show(options, click_cb)
