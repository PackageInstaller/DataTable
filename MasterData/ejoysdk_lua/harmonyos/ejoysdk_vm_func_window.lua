...
function M:_check_safe_frame_before_update(x, y, width, height)
  ...
  E.LOG.debug(TAG, "_check_safe_frame_before_update screen_width:" .. tostring(screen_width) .. ", screen_height:" .. tostring(screen_height) .. ", x:" .. tostring(x) .. ", y:" .. tostring(y) .. ", width" .. tostring(width) .. ", height:" .. tostring(height) .. ", max_width:" .. tostring(max_width) .. ", max_height:" .. tostring(max_height) .. ", ret_x:" .. tostring(ret_x) .. ", ret_y:" .. tostring(ret_y))
...
function M:update_size(width, height)
  ...
  E.LOG.debug(TAG, "update_size width:" .. tostring(_width) .. ", height:" .. tostring(_height) .. ", x:" .. tostring(_x) .. ", y:" .. tostring(_y))
...
function M:window_destory()
  ...
  if self._data.on_touch_outside_listener then
    E.LOG.debug(TAG, "window_destory, begin off touchOutside")
    self._data.window:off("touchOutside")
  end
...
function M:create_window(params, cb)
  ...
  window_stage.createSubWindow(window_name, function(err, sub_window)
    if err and err.code > 0 then
      ...
      if self._data.on_touch_outside_listener then
        E.LOG.debug(TAG, "has set on_touch_outside_listener, begin watch touchOutside")
        sub_window:on("touchOutside", self._data.on_touch_outside_listener)
      else
        E.LOG.debug(TAG, "NOT has on_touch_outside_listener, skip watch touchOutside")
      end
      ...
      local init_x = pos_obj and pos_obj.x or nil
      local init_y = pos_obj and pos_obj.y or nil
      local init_width = size_obj and size_obj.width or nil
      local init_height = size_obj and size_obj.height or nil
      init_x, init_y, init_width, init_height = self:_check_safe_frame_before_update(init_x, init_y, init_width, init_height)
      E.LOG.debug(TAG, "create_window init_x:" .. tostring(init_x) .. ", init_y:" .. tostring(init_y) .. ", init_width:" .. tostring(init_width) .. ", init_height:" .. tostring(init_height))
      if init_width and init_height then
        self:update_size(size_obj.width, size_obj.height)
      end
      if init_x and init_y then
        self:reposition(init_x, init_y)
      end
...
function M:show()
  if not self._data.window then
    E.LOG.warn(TAG, "window not exists, skip show2")
    return false
  end
...
function M:set_on_touch_outside_listener(listener)
  self._data.on_touch_outside_listener = listener
end
