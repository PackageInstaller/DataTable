...
function M.get_dict_base_url()
  local EGC = require("ejoysdk_lua.ejoysdk_gangplank_config")
  local base_url = "https://dict" .. EGC.server_domain_suffix()
  return base_url
end
...
function M.insert_array_notnull(tb, value)
  if nil ~= value then
    M.insert_array(tb, value)
  end
end

function M.insert_arrays(tb, new_tb)
  if type(tb) == "table" and type(new_tb) == "table" then
    for _, _item in ipairs(new_tb) do
      table.insert(tb, _item)
    end
  else
    return tb
  end
...
local EjoyOfflineH5Utils = {}
M.EjoyOfflineH5Utils = EjoyOfflineH5Utils

function EjoyOfflineH5Utils.parse_url_string(path)
  if type(path) ~= "string" then
    return path
  end
  local uri = require("ejoysdk_lua.libs.uri")
  path = uri.decode_percent_encoded_uri(path)
  path = path:gsub("\\", "/")
  return path
end

function EjoyOfflineH5Utils.convert_to_local_path(url)
  if type(url) ~= "string" then
    _ejoysdk.log(TAG .. "# " .. "webview_open convert_to_local_path skipped: url is not string")
    return nil
  end
  local lower = url:lower()
  if not lower:match("^file:") then
    _ejoysdk.log(TAG .. "# " .. "webview_open convert_to_local_path skipped: not file schema: " .. tostring(url))
    return nil
  end
  local main = url
  local hash_pos = main:find("#", 1, true)
  if hash_pos then
    main = main:sub(1, hash_pos - 1)
  end
  local query_pos = main:find("?", 1, true)
  if query_pos then
    main = main:sub(1, query_pos - 1)
  end
  local abs_path = main:match("^file:///(%a:[/\\].+)$")
  if abs_path then
    _ejoysdk.log(TAG .. "# " .. "webview_open convert_to_local_path matched 3-slash file url: " .. abs_path)
    return abs_path
  end
  abs_path = main:match("^file://(%a:[/\\].+)$")
  if abs_path then
    _ejoysdk.log(TAG .. "# " .. "webview_open convert_to_local_path matched 2-slash file url: " .. abs_path)
    return abs_path
  end
  _ejoysdk.log(TAG .. "# " .. "webview_open convert_to_local_path failed: " .. tostring(url))
  return nil
end
...
function M.get_file_name(file_full_path)
  ...
  local name = file_full_path:match("^.+[/\\](.+)$")
  return name or file_full_path
end
