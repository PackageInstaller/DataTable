...
function Awake()
    ...
    layout:Init("UIs/DungeonActivity17/DungeonDesertItem", LayoutCallBack, true)
...
function OnItemClickCB(item)
    ...
    if lastItemInfo then
        -- ShowInfo()
        -- ShowBGItem()
        -- MoveToTargetByIndex(lastItemInfo, 2)
...
function OnInit()
    UIUtil:AddTop2("DungeonDesert", topParent, OnClickBack, OnClickHome);
end
...
function CheckNew()
    if SectionNewUtil:IsNew("DungeonDesert") then
...
function ShowInfo(item)
    ...
    if itemInfo == nil then
        ResUtil:CreateUIGOAsync("DungeonActivity17/DungeonItemInfo", infoParent, function(go)
...
function OnLoadCallBack()
    ...
    if currItem then
        ...
        itemInfo.AddTeamReplace(currItem.GetType() == DungeonInfoType.Desert,OnBattleEnter,"Common/btn_18_05","334863")
    end
    SetInfoItemPos()
end
...
function SetInfoItemPos()
    if itemInfo then
        itemInfo.SetPanelPos("Title", 14, 408)
        itemInfo.SetPanelPos("Level", 14, 324)
        itemInfo.SetPanelPos("Target", 14, 188)
        itemInfo.SetPanelPos("Details", 14, -189)
        itemInfo.SetPanelPos("Button", 14, -323)
        if currItem then
            itemInfo.SetPanelPos("Output", 14, currItem.IsPlot() and -14 or -14)
            itemInfo.SetPanelPos("Plot", 14, currItem.IsSpecial() and 192 or 236)
        end
        itemInfo.SetPanelPos("PlotButton", 14, -325)
        itemInfo.SetPanelPos("Danger", 14, -15)
        itemInfo.SetItemPos("Double", -206, -428)
