...
local BevConditionCheckBuffStack = {}
local FH = require("Logic/HelpLogic")
local PB = require("Common/PbHelper")

function BevConditionCheckBuffStack.ctor(node)
  local n = BevNode.ctor(node)
  n.name = "BevConditionCheckBuffStack"
  ...
  n.stackCountExp = ""
  n.run = BevConditionCheckBuffStack.run
  return n
end

function BevConditionCheckBuffStack.new()
  return BevConditionCheckBuffStack.ctor({})
end

function BevConditionCheckBuffStack.run(node)
  ...
  local stackCount = node.eval(node.stackCountExp)
  local result = node.compareValue(node.compareOperation, foundBuff.stack, stackCount)
  if result then
...
return BevConditionCheckBuffStack
