...
function OnViewClosed(viewKey)
    if viewKey == "SpringFestival" then
        CSAPI.PlayBGM("SpringFes_Music_01", 50)
    end
end

function OnInit()
    UIUtil:AddTop2("SpringFestivalMenu", topParent, OnClickBack)
end
...
function OnOpen()
    lastBGM = CSAPI.PlayBGM("SpringFes_Music_01", 50)
    if data == nil then
...
function SetRewards()
    ...
    for i, v in ipairs(cfgRewards) do
        if v.rewards and v.isShow then
            cfgItem = Cfgs.ItemInfo:GetByID(cfgRewards[i].rewards[1][1])
            if cfgItem and cfgItem.icon then
                ResUtil.IconGoods:Load(this["icon" .. index].gameObject, cfgItem.icon)
                index = index + 1
            end
        end
    end
...
function OnClickRank()
    local sectionData = DungeonMgr:GetSectionData(21002)
...
function OnClickEnter()
    CSAPI.OpenView("SpringFestival", 1)
end
