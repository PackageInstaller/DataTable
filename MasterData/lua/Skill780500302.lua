...
---------------------------------------------
-- 技能基类
Skill780500302 = oo.class(SkillBase)
function Skill780500302:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill780500302:DoSkill(caster, target, data)
	-- 12006
	self.order = self.order + 1
	self:DamageLight(SkillEffect[12006], caster, target, data, 0.167,6)
end
-- 行动结束
function Skill780500302:OnActionOver(caster, target, data)
	-- 8200
	...
	-- 780500307
	self:AddBuff(SkillEffect[780500307], caster, target, data, 780500302)
	-- 8794
...
-- 伤害后
function Skill780500302:OnAfterHurt(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
...
-- 回合结束时
function Skill780500302:OnRoundOver(caster, target, data)
	-- 8795
	...
	if SkillJudger:Greater(self, caster, target, true,count9768/count818,count20/count49) then
		...
		if SkillJudger:Greater(self, caster, target, true,count970,2) then
			-- 780500331
			self:AddHp(SkillEffect[780500331], caster, self.card, data, math.max(math.floor(count9768/count818*count49-count20),1))
			-- 8462
			...
			-- 780500336
			self:AddHp(SkillEffect[780500336], self.card, caster, data, math.max(math.floor(count20/count49*count818-count9768),-math.floor((1+count970*(math.floor(count969/2+1.5)*0.05))*count62*count974*count975*count976*count977*(10+count973*10+count797*0.4*count796))))
			-- 8796
			local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
			-- 780500340
			...
			-- 780500348
			self:AddHp(SkillEffect[780500348], caster, self.card, data, math.max(math.floor(count9768/count818*count49-count20),1))
			-- 8462
			local count62 = SkillApi:GetAttr(self, caster, target,3,"attack")
			-- 8796
			local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
			...
			local count977 = SkillApi:GetAttr(self, caster, target,1,"bedamage2")
			-- 780500353
			self:AddHp(SkillEffect[780500353], self.card, caster, data, math.max(math.floor(count20/count49*count818-count9768),-math.floor(count62*count974*count975*count976*count977*(10+count973*10+count797*0.4*count796))))
			-- 8796
			local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
			-- 780500340
			self:DelBufferForce(SkillEffect[780500340], caster, self.card, data, 780500100+count796)
		...
		-- 780500326
		self:CallOwnerSkill(SkillEffect[780500326], caster, caster, data, 780500302)
		-- 8796
		local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
		-- 780500340
		self:DelBufferForce(SkillEffect[780500340], caster, self.card, data, 780500100+count796)
...
-- 回合开始时
function Skill780500302:OnRoundBegin(caster, target, data)
	-- 780500359
