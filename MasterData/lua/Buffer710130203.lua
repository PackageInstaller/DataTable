...
---------------------------------------------
-- 技能基类
Buffer710130203 = oo.class(BuffBase)
function Buffer710130203:Init(mgr, id, target, caster)
	BuffBase.Init(self, mgr, id, target, caster)
end
-- 行动开始
function Buffer710130203:OnActionBegin(caster, target)
	-- 8160
...
-- 伤害前
function Buffer710130203:OnBefourHurt(caster, target)
	-- 710130212
	self:tFunc_710130212_710130213(caster, target)
	self:tFunc_710130212_710130214(caster, target)
end
-- 创建时
function Buffer710130203:OnCreate(caster, target)
	-- 710130203
	self:AddAttrPercent(BufferEffect[710130203], self.caster, self.card, nil, "attack",0.15)
	-- 710130208
	self:AddAttrPercent(BufferEffect[710130208], self.caster, self.creater, nil, "attack",0.15)
end
function Buffer710130203:tFunc_710130212_710130213(caster, target)
	-- 8060
...
function Buffer710130203:tFunc_710130212_710130214(caster, target)
	-- 8160
	if SkillJudger:IsCasterBuff(self, self.caster, target, true) then
	else
		return
