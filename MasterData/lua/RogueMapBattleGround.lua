...
function Awake()
    ...
    -- local subMapGo = CSAPI.CreateGO("RogueMap/RogueMapSubMap",0,0,0,subMaps.transform)
    -- rmSubMap = ComUtil.GetLuaTable(subMapGo)
    -- rmSubMap.Init(this)
end
...
function OnViewOpened(viewKey)
    if viewKey ~= "RogueMapBattle" then
        viewList[viewKey] = 1
        SetCtrlState(99)
    end
end
...
function OnViewClosed(viewKey)
    local isCheck = viewList[viewKey] ~= nil
    viewList[viewKey] = nil
    if isCheck then
        local num = 0
...
function OnClickGrid(id)
    if isKeepingGrid then
        SetCtrlState(0) -- 恢复相机移动
        isKeepingGrid = false
        return
    end
    ...
    local mapState = mgr:GetMapState();
...
function OnDownGrid(id)
    ...
    gridKeepDownTime = CSAPI.GetTime()
    -- OnClickGridInfo(id);
end
function OnKeepGrid(id)
    ...
    if gridKeepDownTime then
        local currTime = CSAPI.GetTime();
        if currTime - gridKeepDownTime > 0.1 then
            isKeepingGrid = true
            SetCtrlState(99) -- 取消相机移动
            gridKeepDownTime = currTime
            OnClikcGridKeep(id)
        end
    end
end
...
-- 点击格子(滑动)
function OnClikcGridKeep(id)
    if keepGridId == id then
        return
    end
    keepGridId = id

    local state = mgr:CheckClickGridTime();
    if (not state) then
        return;
    end

    local doingData = mgr:GetCurrData();
    if (doingData) then
        -- LogError(doingData);
        return;
    end

    local character = mgr:GetCtrlCharacter();
    if (character) then
        ...
        if (grid and grid.IsReachable()) then
            mgr:UpdateMoveList(id,nil,true);
            -- else
...
function OnClickGridNormal(id)
    ...
    if (character) then
        ...
        local grid = GetGrid(id);
        if grid then
            if mgr:IsGridFastFight(id)then
                mgr:UpdateMistGridEvent(id)
            elseif grid.IsReachable() then
                mgr:UpdateMoveList(id);
            else
                mgr:ShowMoveTips()
                -- Tips.ShowTips(LanguageMgr:GetByID(1074)) -- StringConstant.battle_move_limit);
            end
...
function ApplyMove(character, targetId, isSlide)
    ...
    local path = mgr:FindPathNew(character, 100); -- 服务器给的移动位置，忽略步数限制
    if (path == null) then
        OnMoveCallBack();
    ...
    local delayTime = isSlide and 500 or 0;
    FuncUtil:Call(function()
        character.MoveTo(targetId, pathGos, OnMoveCallBack, nil, isSlide);
        if (isSlide) then
            ResUtil:CreateEffect("battle/G_03_DriftSand_02", 0, 0, 0, character.gameObject);
        end
    end, nil, delayTime);
...
-- 设置格子高亮范围
function SetTargetLightRange(targetId, customCost, customGridList)
    local character = mgr:GetCtrlCharacter()
    local height = character.GetJumpStep();
    local ignoreWaterCost = character.IsIgonreWaterCost();
    local isEnemy = character.GetType() ~= eDungeonCharType.MyCard;
    local passBans, targetBans = mgr:GetGridBans(character);
    local list = GetGridsInRange(targetId, passBans, targetBans, customCost, height, ignoreWaterCost);
    if customGridList then
        for _, gridId in ipairs(customGridList) do
            list[gridId] = 1
        end
    end
    SetGridLightStates(list, isEnemy, targetId);
end
-- 获取角色能去的范围
function GetLightRange(character, customCost)
    if (character) then
        ...
        for gridId, grid in pairs(list) do
            local gridProp = mgr:GetUnitOnGrid(gridId, eDungeonCharType.Prop);
...
function SetPoint(customGridList)
    if rmPoint == nil then
        local pointGo = CSAPI.CreateGO("RogueMap/RogueMapPoint", 0, 0, 0, points)
        rmPoint = ComUtil.GetLuaTable(pointGo)
        rmPoint.Init(this)
    end
    if rmPoint then
        rmPoint.Refresh(customGridList)
    end
end

function SetMist(gridId)
    if rmMist == nil then
        local mistGo = CSAPI.CreateGO("RogueMap/RogueMapMist", 0, 0, 0, mists)
        rmMist = ComUtil.GetLuaTable(mistGo)
        rmMist.Init(this)
    end
    if rmMist then
        rmMist.Refresh(gridId)
    end
end
