...
---------------------------------------------
-- 技能基类
Buffer55001272 = oo.class(BuffBase)
function Buffer55001272:Init(mgr, id, target, caster)
	BuffBase.Init(self, mgr, id, target, caster)
end
-- 移除buff时
function Buffer55001272:OnRemoveBuff(caster, target)
	-- 3405
...
-- 伤害前
function Buffer55001272:OnBefourHurt(caster, target)
	-- 8063
	...
	-- 100200311
	self:OwnerAddBuffCount(BufferEffect[100200311], self.caster, self.card, nil, 55001272,-1,10)
end
-- 创建时
function Buffer55001272:OnCreate(caster, target)
	-- 3404
	self:MissSurface(BufferEffect[3404], self.caster, self.card, nil, 10000)
end
-- 攻击结束
function Buffer55001272:OnAttackOver(caster, target)
	-- 8063
	if SkillJudger:CasterIsEnemy(self, self.caster, target, true) then
	else
		return
	...
	-- 100200308
	self:OwnerAddBuffCount(BufferEffect[100200308], self.caster, self.card, nil, 55001272,-1,10)
	-- 100200305
	self:SetValue(BufferEffect[100200305], self.caster, self.card, nil, "sh",0)
end
