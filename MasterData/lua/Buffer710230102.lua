...
---------------------------------------------
-- 技能基类
Buffer710230102 = oo.class(BuffBase)
function Buffer710230102:Init(mgr, id, target, caster)
	BuffBase.Init(self, mgr, id, target, caster)
end
-- 伤害前
function Buffer710230102:OnBefourHurt(caster, target)
	-- 8065
	...
	if self:Rand(c788*1000+5000) then
		self:AddTempAttr(BufferEffect[710230107], self.caster, self.caster, nil, "damage",0.05*self.nCount)
	end
end
-- 创建时
function Buffer710230102:OnCreate(caster, target)
	-- 710230102
	self:AddAttr(BufferEffect[710230102], self.caster, self.card, nil, "damage",0.05*self.nCount)
end
