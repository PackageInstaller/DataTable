...
local function OnCreate(self, layer)
	...
	self.OrderInLayer=layer.OrderInLayer
	...
	if not layer.Blur then
		self.unity_canvas.worldCamera = UIManager:GetInstance().UICamera
	else
		self.unity_canvas.worldCamera = UIManager:GetInstance().UIBlurCamera
	end
	...
	self.unity_canvas_scaler.matchWidthOrHeight = UIManager:GetInstance().MatchWidthOrHeight
...
local function PushWindowOrder(self)
	...
	--if self.top_window_order > self.min_window_order then return end
	self.top_window_order = self.top_window_order - UIManager:GetInstance().MaxOderPerWindow
end
