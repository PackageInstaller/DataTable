...
local function waitforunscaledseconds(seconds)
	assert(type(seconds) == "number" and seconds >= 0)
	local co = coroutine.running() or error ("coroutine.waitforunscaledseconds must be run in coroutine")
	local timer = TimerManager:GetInstance():GetCoTimer()
	local action = __GetAction(co, timer)
	
	timer:Init(seconds, __Action, action, true, false)
 	timer:Start()
	action_map[co] = action
 	return coroutine.yield()
end
...
coroutine.waitforunscaledseconds = waitforunscaledseconds
...
coroutine.stopwaiting = stopwaiting -- 禁止用这个
