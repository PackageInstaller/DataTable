...
function Awake()
    ...
    layout:Init("UIs/DungeonActivity16/DungeonChristmasItem", LayoutCallBack, true)
    ...
    if lastItemInfo then
        ...
        FuncUtil:Call(function()
            MoveToTargetByIndex(item.GetInfo(), 1, function()
                MoveToTargetByIndex(item.GetInfo(), 2,ShowRoleEnter)
                ShowInfo(item)
                ShowBGItem(curIndex)
            end, true)
...
function OnInit()
    UIUtil:AddTop2("DungeonChristmas", topParent, OnClickBack, OnClickHome);
end
...
function ShowInfo(item)
    ...
    if itemInfo == nil then
        ResUtil:CreateUIGOAsync("DungeonActivity16/DungeonItemInfo", infoParent, function(go)
...
function OnLoadCallBack()
    ...
    itemInfo.CallFunc("Double", "SetTextColor", "492722", "597546", "492722")
    ...
    if currItem then
        ...
        itemInfo.AddTeamReplace(currItem.GetType() == DungeonInfoType.Christmas,OnBattleEnter,"Common/btn_18_05","334863")
    end
    SetInfoItemPos()
end
...
function SetInfoItemPos()
    if itemInfo then
        itemInfo.SetPanelPos("Title", 14, 407)
        itemInfo.SetPanelPos("Level", 14, 325)
        itemInfo.SetPanelPos("Target", 14, 183)
        itemInfo.SetPanelPos("Details", 14, -188)
        itemInfo.SetPanelPos("Button", 14, -324)
        if currItem then
            itemInfo.SetPanelPos("Output", 14, currItem.IsPlot() and -20 or -20)
            itemInfo.SetPanelPos("Plot", 14, currItem.IsSpecial() and 183 or 183)
        end
        itemInfo.SetPanelPos("PlotButton", 14, -324)
        itemInfo.SetPanelPos("Danger", 14, -15)
...
function SetWidth(isSel)
    ...
    if isSel then
        CSAPI.SetRTSize(hsv.gameObject, -973, size[1])
        if #curDatas > 3 then
            local index = curIndex - 2
            index = curIndex == 1 and curIndex - 1 or index
            local x = index > 0 and -(20 + (404 + 84) * index) or 0
            local itemSize = CSAPI.GetRTSize(itemParent2.gameObject)
            x = x < -(itemSize[0] - (canvasSize[0] - 973)) and -(itemSize[0] - (canvasSize[0] - 973)) or x
            CSAPI.MoveTo(itemParent2, "UI_Local_Move", x, 0, 0, nil, 0.2)
        end
    else
        CSAPI.SetRTSize(hsv.gameObject, -188, size[1])
    end
end
...
-----------------------------------------------role-----------------------------------------------
local lastRoleIndex,curRoleIndex =-1,0
local lastPosX,curPosX=-1,0
local posList = {-10,-1031,-2120,-3233}
local rolePosList = {-2296,-1352,-457,445,1349,2275}

function CheckRoleIndex()
    curPosX = CSAPI.GetAnchor(bg)
    if curPosX == lastPosX then
        return 
    end
    lastPosX = curPosX
    curRoleIndex = GetRoleIndex()
    if curRoleIndex == lastRoleIndex then
        return 
    end
    lastRoleIndex = curRoleIndex
    SetRolePos()
end
...
function GetRoleIndex()
    ...
    for i, v in ipairs(posList) do
        if curPosX < v * (1 + offsetScale) then
            index = i + 1
        end
    end
    return index
...
function SetRolePos()
    CSAPI.SetAnchor(Character,rolePosList[curRoleIndex], -155)
end

function ShowRoleEnter()
    CheckRoleIndex()
    if not IsNil(roleAnim) then
        roleAnim:Play("Character_entry")
    end
end

function ShowRoleQuit()
    if not IsNil(roleAnim) then
        roleAnim:Play("Character_quit")
    end
end