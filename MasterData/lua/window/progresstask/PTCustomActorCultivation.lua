...
function OnEnable()
  ...
  RU.SetRedMark(string.format("ProgressTask/CustomActorCultivation%s/Login", acIndex), false)
...
function OnActivity(activity)
  ...
  if m_activity then
    ...
    REF.TextureActivityTitle1.UITexture.mainTexturePath = xlsxConfig.titleRes
  end
  this:Unbind("fci/custom-actor-cultivation/", OnPlan)
...
function SetNodeCultivation()
  ...
  REF.TextureDesc.UITexture.mainTexturePath = xlsxConfig.shortTitleRes
...
function SetNodeCG()
  ...
  REF.TextureActivityTitle2.UITexture.mainTexturePath = xlsxConfig.titleRes
end
...
function OnClickCG()
  local acIndex = this:GetData("CustomActorCultivation/ActivityIndex")
  DB:BroadcastGameEvent("SetViewInfo", "CustomActorCultivation" .. acIndex, nil)
