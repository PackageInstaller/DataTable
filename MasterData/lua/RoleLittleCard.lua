...
function RefreshByFormation()
    SetIcon(cardData:GetSmallImg())
    SetTag(cardData:GetData().tag)
    if elseData and elseData.showFakeLevel and elseData.showFakeLevel > cardData:GetLv() then
        SetLv(elseData.showFakeLevel,"ff0040")
    else
        SetLv(cardData:GetLv())
    end
    SetPro();
...
function SetFormation(_cid,_hideFormat)
    ...
    if index ~= -1 then
        ...
        if(teamType and (teamType==eTeamType.RogueS or teamType==eTeamType.PVP or teamType==eTeamType.PVPFriend or teamType==eTeamType.TowerDeep or teamType==eTeamType.PVP3 or teamType==eTeamType.PVPFriend3 or teamType==eTeamType.ChainFront or teamType==eTeamType.CrossBoss)) then 
