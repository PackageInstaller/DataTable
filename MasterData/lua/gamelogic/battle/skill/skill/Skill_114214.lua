---
--- 夺魂银丝
--- 对指定整排敌人造成${Params[1]}%物理伤害，${Params[2]}%的概率造成${Params[3]}回合的【拘束】和【标靶】，如果攻击前目标持有${Params[4]}个【中毒】，则改为${Params[5]}回合的【晕眩】和【防标靶】，获得${Params[6]}%额外倍率，并使目标的【中毒】立即结算一次伤害(${Params[7]} 0,1 开关)
---

local Skill_114214 = BaseClass("Skill_114214", Skill)
local base = Skill
...
local function OnUseActiveSkill(self, targetUnit)
    ...
    if BattleUtil.IsPctProbHit(self.Params[2]) then
        if not hasPoison then
            ...
            table.insert(result.Buffs, ActiveSkillBuff.New(BattleBuffType.Vulnerable, self.Params[3]))
        else
            table.insert(result.Buffs, ActiveSkillBuff.New(BattleBuffType.Stunning, self.Params[5]))
            table.insert(result.Buffs, ActiveSkillBuff.New(BattleBuffType.Vulnerable, self.Params[5]))
...
Skill_114214.__init = __init
Skill_114214.OnUseActiveSkill = OnUseActiveSkill
Skill_114214.OnAtkEnd = OnAtkEnd
