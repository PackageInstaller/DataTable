...
function Awake()
    ...
    layout:Init("UIs/DungeonActivity20/DungeonHolidayItem", LayoutCallBack, true)
...
function OnInit()
    UIUtil:AddTop2("DungeonHoliday", topParent, OnClickBack, OnClickHome);
end
...
function ShowInfo(item)
    ...
    if itemInfo == nil then
        ResUtil:CreateUIGOAsync("DungeonActivity20/DungeonItemInfo", infoParent, function(go)
...
function OnLoadCallBack()
    ...
    itemInfo.CallFunc("Double", "SetTextColor", "f8af56", "ebebeb", "f8af56")
    itemInfo.CallFunc("Danager", "SetColors", {{231, 228, 222, 255}, {126, 126, 126, 255}, {126, 126, 126, 128}})
    -- itemInfo.CallFunc("Output", "ShowOutput", true, "spec_04")
    itemInfo.CallFunc("Target", "SetGoal", "aaaaaa", "aaaaaa", "desire_01", "summer2_01")
    if currItem then
        itemInfo.CallFunc("Danger", "ShowDangeLevel", currItem.IsDanger(), currItem.GetCfgs(), currDanger)
        itemInfo.AddTeamReplace(currItem.GetType() == DungeonInfoType.Holiday,OnBattleEnter,"Common/btn_18_07","090909")
    end
    SetInfoItemPos()
end
...
function SetInfoItemPos()
    if itemInfo then
        ...
        if currItem then
            itemInfo.SetPanelPos("Output", 11, currItem.IsPlot() and 15 or -17)
            itemInfo.SetPanelPos("Plot", 11, currItem.IsSpecial() and 185 or 244)
        end
        itemInfo.SetPanelPos("PlotButton", 11, -339)
...
function SetWidth(isSel)
    ...
    if isSel then
        ...
        if #curDatas > 3 then
            ...
            local x = index > 0 and -(40 + (402 + 83) * index) or 0
