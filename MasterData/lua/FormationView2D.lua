...
function Init(data,_canDragLeave,_playTween,_clickID,_addtiveState,_infoNode,_isShowInfos,_showFakeLevel)    
	...
	if data ~= nil then
		...
		for k, v in pairs(teamData.data) do           
			...
			cardList[v.cid].InitData(v,isLeader,isMini,isMirror,isShowInfos,showFakeLevel);
...
function OnJoinDragBegin(eventData)
	...
	if eventData and eventData.card and CanEditTeam() then
		...
		-- 	joinModel=ComUtil.GetLuaTable(go);
			...
			joinModel.InitData(teamItemData,false,false,false,isShowInfos,showFakeLevel);
...
function SetInfoViewPos(teamItemData)
	for k,v in pairs(cardList) do
		v.InitData(v.data,false,isMini,isMirror,isShowInfos,showFakeLevel);
	end
	if teamItemData then
