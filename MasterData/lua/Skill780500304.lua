...
---------------------------------------------
-- 技能基类
Skill780500304 = oo.class(SkillBase)
function Skill780500304:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill780500304:DoSkill(caster, target, data)
	-- 12006
	self.order = self.order + 1
	self:DamageLight(SkillEffect[12006], caster, target, data, 0.167,6)
end
-- 行动结束
function Skill780500304:OnActionOver(caster, target, data)
	-- 8200
	...
	-- 780500309
	self:AddBuff(SkillEffect[780500309], caster, target, data, 780500304)
	-- 8794
...
-- 伤害后
function Skill780500304:OnAfterHurt(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
...
-- 回合结束时
function Skill780500304:OnRoundOver(caster, target, data)
	-- 8795
	...
	if SkillJudger:Greater(self, caster, target, true,count9768/count818,count20/count49) then
		...
		if SkillJudger:Greater(self, caster, target, true,count970,2) then
			-- 780500333
			self:AddHp(SkillEffect[780500333], caster, self.card, data, math.max(math.floor(count9768/count818*count49-count20),1))
			-- 8462
			...
			-- 780500338
			self:AddHp(SkillEffect[780500338], self.card, caster, data, math.max(math.floor(count20/count49*count818-count9768),-math.floor((1+count970*(math.floor(count969/2+1.5)*0.05))*count62*count974*count975*count976*count977*(15+count973*10+count797*0.4*count796))))
			-- 8796
			local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
			-- 780500340
			...
			-- 780500350
			self:AddHp(SkillEffect[780500350], caster, self.card, data, math.max(math.floor(count9768/count818*count49-count20),1))
			-- 8462
			local count62 = SkillApi:GetAttr(self, caster, target,3,"attack")
			-- 8796
			local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
			...
			local count977 = SkillApi:GetAttr(self, caster, target,1,"bedamage2")
			-- 780500355
			self:AddHp(SkillEffect[780500355], self.card, caster, data, math.max(math.floor(count20/count49*count818-count9768),-math.floor(count62*count974*count975*count976*count977*(15+count973*10+count797*0.4*count796))))
			-- 8796
			local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
			-- 780500340
			self:DelBufferForce(SkillEffect[780500340], caster, self.card, data, 780500100+count796)
		...
		-- 780500328
		self:CallOwnerSkill(SkillEffect[780500328], caster, caster, data, 780500304)
		-- 8796
		local count796 = SkillApi:SkillLevel(self, caster, target,3,7805001)
		-- 780500340
		self:DelBufferForce(SkillEffect[780500340], caster, self.card, data, 780500100+count796)
...
-- 回合开始时
function Skill780500304:OnRoundBegin(caster, target, data)
	-- 780500359
