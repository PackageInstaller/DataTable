...
function AnnouncementRedUtils.GetNoticePageRedData(page)
  ...
  local redMap = DataCenter.redPointData[RedTypeDefine.RedType.Announcement] or {}
  local newCount = redMap[SUB_RED_TYPE.PAGE] and redMap[SUB_RED_TYPE.PAGE][page]
...
function AnnouncementRedUtils.TestAddRed()
  local randomUid = math.random(1, 10)
  local map = AnnouncementModel.Instance:GetAnnMap()
  local uid
  for id, _ in pairs(map) do
    ...
    uid = id
    randomUid = randomUid - 1
  end
  local data = AnnouncementModel.Instance:GetAnnData(uid)
  if not data or not data.content then
    print("ann 未找到公告数据, uid:", uid)
    return
  end
  data.new = true
  AnnouncementRedUtils.AddRedDataByInfo(data)
  AnnouncementRedUtils.SetEntranceRed(true)
  RedDotController.Instance:RefreshAllRedDots()
  print("ann 已添加红点, uid:", uid, data.content)
end
