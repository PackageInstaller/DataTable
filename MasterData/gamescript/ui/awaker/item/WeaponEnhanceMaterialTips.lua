local WeaponEnhanceMaterialTips, Super = System.NewComponent("WeaponEnhanceMaterialTips")

function WeaponEnhanceMaterialTips:ctor(uiNode, mtrl, model)
  Super.ctor(self)
...
function WeaponEnhanceMaterialTips:OnBind(binder)
  ...
  if self.mtrl.isWeapon then
    local weaponDetailModel = binder:createModel(WeaponDetailModel, self.mtrl.item)
    self.ui.UI_Common_Btn_Lock:SetActive(false)
    ...
    binder:BindToRaw(function(_, enhanceLevel)
      local enhanceLevelLimit = self.mtrl.enhanceLevelLimit
    ...
    binder:BindToRaw(function(_, enhanceLevelLimit)
      local enhanceLevel = self.mtrl.enhanceLevel
      binder:SetText(self.ui.Text_Grade, string.format("Lv.%s/%s", enhanceLevel, enhanceLevelLimit))
    end, function()
    ...
    self.ui.Group_Weapon_Attrs:SetActive(false)
    self.ui.Group_Weapon:SetActive(false)
    binder:BindToText(self.ui.Text_Story, function()
      do return ItemDataUtils.GetItemDesc end
      return ItemDataUtils.GetItemDesc, mtrlTID
    end)
  end
end

function WeaponEnhanceMaterialTips:_SetAttr(binder, uiName, uiValue, attr)
...
function WeaponEnhanceMaterialTips:OnUnbind()
  Super.OnUnbind(self)
end

return WeaponEnhanceMaterialTips
