local ActivityBattleTrainTaskListComp, Super = System.NewClass("ActivityBattleTrainTaskListComp", ActivityTaskListComp)

function ActivityBattleTrainTaskListComp:_OnBindActivityPlot()
end

function ActivityBattleTrainTaskListComp:_InitUIResource(res)
  self.ui = UI_Events_BattleTeachingResource(res)
end

function ActivityBattleTrainTaskListComp:_OnBindEvent(binder)
  binder:BindEvent(EventMgr.Instance.OnActivityTaskComplete, System.fn(self, self._RefreshTaskTableView))
end

function ActivityBattleTrainTaskListComp:_OnBindTaskListView()
  ...
  self.taskTableView = self:CreateTableview(self.ui.TaskView, function()
    ...
    itemComps[gameObj] = self.binder:BindComponent(ActivityTaskBattleTrainItem(gameObj, taskData, self.activityData))
    return cell
  end, function()
    return itemW, itemH
  end, function()
