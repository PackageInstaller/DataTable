...
local function createAwakerPotencyModelState(awakerModel)
  local state = {}
...
local function createAwakerPotencyModellViews(model)
  local views = {}
...
local function createAwakerPotencyModellActions(data)
  local actions = {}
  ...
  function actions:SetSelectPotencyIndex(index)
    ...
    if index == data.selectPotencyIndex and 0 ~= index then
      data.selectPotencyIndex = -1
    end
...
local AwakerPotencyModel = Vue.model("AwakerPotencyModel", createAwakerPotencyModelState):views(createAwakerPotencyModellViews):actions(createAwakerPotencyModellActions):setup(onSetup)
return AwakerPotencyModel
