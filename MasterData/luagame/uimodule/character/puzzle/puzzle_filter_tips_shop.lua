...
local GRAPH_TEMP_KEY = "PUZZLE_FILTER_TIPS_TEMP_KEY_GRAPH_SHOP"
local ENTRY_TEMP_KEY = "PUZZLE_FILTER_TIPS_TEMP_KEY_ENTRY_SHOP"
...
function ui:ui_finish_load()
  ...
  self.v_uiobjects.Attr:SetActive(false)
...
function ui:ui_on_show(puzzle_goods_list)
  self.v_puzzle_goods_list = puzzle_goods_list
...
function ui:refresh_tog_by_mgr_data()
  self.v_new_filter_graph = PuzzleMgr.graph_map_filter_shop
  ...
  self.v_new_filter_entry = PuzzleMgr.entry_map_filter_shop
...
function ui:try_save_data()
  self:load_data()
  if not self:check_filter_suc() then
    Util.show_message_tip(2259)
    return
  end
  PuzzleMgr.graph_map_filter_shop = self.v_new_filter_graph
  PuzzleMgr.entry_map_filter_shop = self.v_new_filter_entry
  MsgGame:mq_publish2(Const.MSG_ON_PUZZLE_FILTER_UPDATE)
  self:ui_hide()
end

function ui:check_filter_suc()
  for _, v in ipairs(self.v_puzzle_goods_list) do
    if self:check_filter_pass_shop(v.Item) then
      return true
    end
  end
end

function ui:check_filter_pass_shop(puzzle_id)
  local cfg = ShareRes.get_buddy_puzzle_cfg(puzzle_id)
  local graph_pass = not self.v_new_filter_graph or self.v_new_filter_graph[cfg.GraphID]
  local entry_pass = false
  if graph_pass then
    entry_pass = not self.v_new_filter_entry or cfg.EntryId and self.v_new_filter_entry[cfg.EntryId]
  end
  return entry_pass
end
