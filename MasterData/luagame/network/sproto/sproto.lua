...
function sproto:pname2id(name)
  local v = queryproto(self, name)
  return v and v.tag
end
...
function sproto:exist_response(pname)
  local v = self.__pcache[pname]
  if not v then
    local _, _, resp = core.protocol(self.__cobj, pname)
    return resp
  else
    return v.response
  end
...
function host:attach(sp)
  return function(name, args, session, ud)
    ...
    if proto.request then
      ...
      return core.pack(header .. content), proto.tag
    else
      return core.pack(header), proto.tag
    end
  end
end
