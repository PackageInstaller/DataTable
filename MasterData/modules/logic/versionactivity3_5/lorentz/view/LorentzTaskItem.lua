-- chunkname: @modules/logic/versionactivity3_5/lorentz/view/LorentzTaskItem.lua

module("modules.logic.versionactivity3_5.lorentz.view.LorentzTaskItem", package.seeall)

local LorentzTaskItem = class("LorentzTaskItem", ListScrollCellExtend)

function LorentzTaskItem:onInitView()
...
function LorentzTaskItem:addEvents()
...
function LorentzTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function LorentzTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.LorentzTaskView)
	end
end

function LorentzTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("LorentzTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function LorentzTaskItem:_btngetallOnClick()
	LorentzController.instance:dispatchEvent(LorentzEvent.OneClickClaimReward, self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("LorentzTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function LorentzTaskItem:_delayFinish()
...
function LorentzTaskItem:_editableAddEvents()
	LorentzController.instance:registerCallback(LorentzEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LorentzTaskItem:_editableRemoveEvents()
	LorentzController.instance:unregisterCallback(LorentzEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LorentzTaskItem:_onOneClickClaimReward(actId)
...
function LorentzTaskItem:getAnimator()
	return self._animator
end

function LorentzTaskItem:onUpdateMO(mo)
	self._taskMo = mo

	local rankDiff = LorentzTaskListModel.instance:getRankDiff(mo)
...
function LorentzTaskItem:_moveByRankDiff(rankDiff)
...
function LorentzTaskItem:onSelect(isSelect)
	return
end

function LorentzTaskItem:_refreshUI()
...
function LorentzTaskItem:_onItemShow(cell_component, data, index)
...
function LorentzTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
