-- chunkname: @modules/logic/versionactivity3_5/lorentz/view/LorentzLevelItem.lua

module("modules.logic.versionactivity3_5.lorentz.view.LorentzLevelItem", package.seeall)

local LorentzLevelItem = class("LorentzLevelItem", LuaCompBase)

function LorentzLevelItem:init(go)
	self.viewGO = go
...
function LorentzLevelItem:_initNode(nodeGo, isSpecial)
...
function LorentzLevelItem:addEventListeners()
	self._btnclick:AddClickListener(self._btnOnClick, self)
end

function LorentzLevelItem:_btnOnClick()
	local isUnlock = LorentzModel.instance:isEpisodeUnlock(self.id)
...
function LorentzLevelItem:setParam(co, index, actId)
	self._config = co
	...
	if self._isStoryEpisode then
		self._node = self._typeNode[1] or self._typeNode[2]
	end
	...
	if not self._isStoryEpisode then
		local index = LorentzConfig.instance:getIndexInGameIdList(self.gameId)
...
function LorentzLevelItem:refreshUI()
	self._isunlock = LorentzModel.instance:isEpisodeUnlock(self.id)
	self._ispass = LorentzModel.instance:isEpisodePass(self.id)

	local isCurrent = self.id == LorentzModel.instance:getCurEpisode()
...
function LorentzLevelItem:_enterGame()
	if LorentzModel.instance:checkEpisodeFinishGame(self.id) and not LorentzModel.instance:isEpisodePass(self.id) then
		self:_onGameFinished()
	else
		LorentzGameController.instance:enterGame(self.id)
	end
end

function LorentzLevelItem:_onGameFinished()
	LorentzController.instance:_onGameFinished(self._actId, self.id)
end

function LorentzLevelItem:isUnlock()
	return self._isunlock
end

function LorentzLevelItem:playFinish()
	self._ispass = LorentzModel.instance:isEpisodePass(self.id)
...
function LorentzLevelItem:setFocusFlag(isFocus)
	gohelper.setActive(self._goCurrent, isFocus)
end

function LorentzLevelItem:playUnlock()
	self._isunlock = LorentzModel.instance:isEpisodeUnlock(self.id)
...
function LorentzLevelItem:playStarAnim()
	AudioMgr.instance:trigger(AudioEnum.RoleActivity.star_show)
end

function LorentzLevelItem:removeEventListeners()
	LorentzController.instance:unregisterCallback(LorentzEvent.JumpToEpisode, self._onJumpToEpisode, self)
	self._btnclick:RemoveClickListener()
end

return LorentzLevelItem
