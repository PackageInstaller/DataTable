-- chunkname: @modules/logic/versionactivity3_5/dungeon/view/VersionActivity3_5DungeonReportFullView.lua

module("modules.logic.versionactivity3_5.dungeon.view.VersionActivity3_5DungeonReportFullView", package.seeall)

local VersionActivity3_5DungeonReportFullView = class("VersionActivity3_5DungeonReportFullView", BaseView)

function VersionActivity3_5DungeonReportFullView:onInitView()
...
function VersionActivity3_5DungeonReportFullView:addEvents()
	return
end

function VersionActivity3_5DungeonReportFullView:removeEvents()
	return
end

function VersionActivity3_5DungeonReportFullView:_editableInitView()
...
function VersionActivity3_5DungeonReportFullView:onUpdateParam()
	return
end

function VersionActivity3_5DungeonReportFullView:onOpen()
	...
	AudioMgr.instance:trigger(AudioEnum3_5.VersionActivity3_5.ui_mln_details_open)
end

function VersionActivity3_5DungeonReportFullView:onOpenFinish()
	...
	UIBlockHelper.instance:startBlock("VersionActivity3_5DungeonReportFullView open", time)

	if not self._progressList[self._acceptedRewardNum] then
		if not self._progressList[self._curFinishRewardNum] then
			local to = 0

			self._tweenId = ZProj.TweenHelper.DOTweenFloat(self._progressList[self._acceptedRewardNum], to, time, self._onTweenFrame, nil, self, nil, EaseType.Linear)
		end
	end
end

function VersionActivity3_5DungeonReportFullView:_onTweenFrame(value)
	self._sliderprogress:SetValue(value)
end

function VersionActivity3_5DungeonReportFullView:_delayGetBonus()
	if self._curFinishRewardNum > self._acceptedRewardNum then
		Activity113Rpc.instance:sendGetAct113MilestoneBonusRequest(VersionActivity3_5Enum.ActivityId.Dungeon, self._getBonusCallback, self)
	end
end

function VersionActivity3_5DungeonReportFullView:_delayPlayUnlockAnim()
...
function VersionActivity3_5DungeonReportFullView:_getBonusCallback(cmd, resultCode, msg)
	if resultCode == 0 then
		...
		for index = self._initRewardNum + 1, self._acceptedRewardNum do
			local rewardConfig = VersionActivity3_5DungeonConfig.instance:getChapterReward(index)
...
function VersionActivity3_5DungeonReportFullView.indexOfByKey(array, value, key, begin)
...
function VersionActivity3_5DungeonReportFullView:_initItems()
	local sortList = {}
	...
	for i, v in ipairs(lua_v3a2_chapter_report.configList) do
		...
		if isFinish then
			...
			var_13_1[2] = tonumber(time) or 0

			table.insert(sortList, var_13_1)
		end
	end
	...
	for i, v in ipairs(lua_v3a2_chapter_report.configList) do
		...
		if isFinish then
			...
			local sortIndex = VersionActivity3_5DungeonReportFullView.indexOfByKey(sortList, v.element, 1) or 0
...
function VersionActivity3_5DungeonReportFullView:_initRewardItems()
	self._clickCacheList = self._clickCacheList or self:getUserDataTb_()
...
function VersionActivity3_5DungeonReportFullView:_onRewardItemShow(obj, data, index)
...
function VersionActivity3_5DungeonReportFullView:_addBtnClick(btnclick, data, index)
	...
	if not item then
		...
		btnclick.AddClickListener(btnclick, self._onBtnClick, self, btnclick)
	end
...
function VersionActivity3_5DungeonReportFullView:_onBtnClick(btnclick)
	return
end

function VersionActivity3_5DungeonReportFullView:onClose()
...
function VersionActivity3_5DungeonReportFullView:onDestroyView()
	return
end

return VersionActivity3_5DungeonReportFullView
