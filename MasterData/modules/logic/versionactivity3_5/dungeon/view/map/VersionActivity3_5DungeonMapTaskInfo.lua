-- chunkname: @modules/logic/versionactivity3_5/dungeon/view/map/VersionActivity3_5DungeonMapTaskInfo.lua

module("modules.logic.versionactivity3_5.dungeon.view.map.VersionActivity3_5DungeonMapTaskInfo", package.seeall)

local VersionActivity3_5DungeonMapTaskInfo = class("VersionActivity3_5DungeonMapTaskInfo", BaseView)

function VersionActivity3_5DungeonMapTaskInfo:onInitView()
...
function VersionActivity3_5DungeonMapTaskInfo:addEvents()
	self._btnclick:AddClickListener(self._btnclickOnClick, self)
end

function VersionActivity3_5DungeonMapTaskInfo:removeEvents()
	self._btnclick:RemoveClickListener()
end

function VersionActivity3_5DungeonMapTaskInfo:_btnclickOnClick()
	self._isUnFold = not self._isUnFold

	self:_updateFoldBtn()
end

function VersionActivity3_5DungeonMapTaskInfo:_updateFoldBtn()
...
function VersionActivity3_5DungeonMapTaskInfo:_editableInitView()
	self._itemList = self:getUserDataTb_()

	self.addEventCb(self, VersionActivityFixedDungeonController.instance, VersionActivity3_5DungeonEvent.V3a2TimelineChange, self._onTimelineChange, self)
end

function VersionActivity3_5DungeonMapTaskInfo:onUpdateParam()
	return
end

function VersionActivity3_5DungeonMapTaskInfo:_showTaskList(episodeId, skipShowTaskList)
...
function VersionActivity3_5DungeonMapTaskInfo:_onTimelineChange(showMap)
	local isHardMode = self.activityDungeonMo:isHardMode()
...
function VersionActivity3_5DungeonMapTaskInfo:_doShowAllTaskList()
	local list = VersionActivity3_5DungeonMapSceneElements.getAllElementCoList()
	local index = 0
...
function VersionActivity3_5DungeonMapTaskInfo:_doShowTaskList(episodeId)
...
function VersionActivity3_5DungeonMapTaskInfo:_getItem(index)
	local item = self._itemList[index]

	if not item then
		local go = gohelper.cloneInPlace(self._gotaskitem)

		item = MonoHelper.addLuaComOnceToGo(go, VersionActivity3_5DungeonMapTaskInfoItem)
...
function VersionActivity3_5DungeonMapTaskInfo:onOpen()
...
function VersionActivity3_5DungeonMapTaskInfo:showElements()
	self:_showTaskList(self.activityDungeonMo.episodeId)
end

function VersionActivity3_5DungeonMapTaskInfo:_onUpdateDungeonInfo()
	self:_showTaskList(self.activityDungeonMo.episodeId, true)
end

function VersionActivity3_5DungeonMapTaskInfo:_guideShowElementAnimFinish()
	self:_updateTaskList()
end

function VersionActivity3_5DungeonMapTaskInfo:_beginShowRewardView()
	self._showRewardView = true
end

function VersionActivity3_5DungeonMapTaskInfo:_endShowRewardView()
	self._showRewardView = false

	TaskDispatcher.runDelay(self._updateTaskList, self, DungeonEnum.RefreshTimeAfterShowReward)
end

function VersionActivity3_5DungeonMapTaskInfo:_updateTaskList()
	self:_showTaskList(self.activityDungeonMo.episodeId)
end

function VersionActivity3_5DungeonMapTaskInfo:_OnRemoveElement(id)
...
function VersionActivity3_5DungeonMapTaskInfo:onClose()
...
function VersionActivity3_5DungeonMapTaskInfo:onDestroyView()
	return
end

return VersionActivity3_5DungeonMapTaskInfo
