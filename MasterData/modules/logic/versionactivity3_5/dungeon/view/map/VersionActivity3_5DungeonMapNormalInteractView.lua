-- chunkname: @modules/logic/versionactivity3_5/dungeon/view/map/VersionActivity3_5DungeonMapNormalInteractView.lua

module("modules.logic.versionactivity3_5.dungeon.view.map.VersionActivity3_5DungeonMapNormalInteractView", package.seeall)

local VersionActivity3_5DungeonMapNormalInteractView = class("VersionActivity3_5DungeonMapNormalInteractView", BaseViewExtended)

function VersionActivity3_5DungeonMapNormalInteractView:onInitView(go)
...
function VersionActivity3_5DungeonMapNormalInteractView:initNoneContainer()
...
function VersionActivity3_5DungeonMapNormalInteractView:initFightContainer()
...
function VersionActivity3_5DungeonMapNormalInteractView:initDialogueContainer()
...
function VersionActivity3_5DungeonMapNormalInteractView:initTalkContainer()
...
function VersionActivity3_5DungeonMapNormalInteractView:addEvents()
	...
	self.addEventCb(self, VersionActivityFixedHelper.getVersionActivityDungeonController().instance, VersionActivityFixedDungeonEvent.OnClickElement, self.showInteractUI, self)
	self.addEventCb(self, ViewMgr.instance, ViewEvent.OnCloseViewFinish, self.onCloseViewFinishCall, self)
	self.addEventCb(self, DialogueController.instance, DialogueEvent.OnDialogueInfoChange, self.onDialogueInfoChange, self)
	self.addEventCb(self, JumpController.instance, JumpEvent.BeforeJump, self.beforeJump, self)
end

function VersionActivity3_5DungeonMapNormalInteractView:removeEvents()
	...
	self.removeEventCb(self, VersionActivityFixedHelper.getVersionActivityDungeonController().instance, VersionActivityFixedDungeonEvent.OnClickElement, self.showInteractUI, self)
	self.removeEventCb(self, ViewMgr.instance, ViewEvent.OnCloseViewFinish, self.onCloseViewFinishCall, self)
	self.removeEventCb(self, DialogueController.instance, DialogueEvent.OnDialogueInfoChange, self.onDialogueInfoChange, self)
	self.removeEventCb(self, JumpController.instance, JumpEvent.BeforeJump, self.beforeJump, self)
end

function VersionActivity3_5DungeonMapNormalInteractView:_editableInitView()
	self._handleTypeMap = {
...
function VersionActivity3_5DungeonMapNormalInteractView:showInteractUI(mapElement)
	if self._show then
		return
	end
...
function VersionActivity3_5DungeonMapNormalInteractView:show()
	if self._show then
		return
	end
...
function VersionActivity3_5DungeonMapNormalInteractView:hide()
...
function VersionActivity3_5DungeonMapNormalInteractView:refreshUI()
...
function VersionActivity3_5DungeonMapNormalInteractView:refreshRewards()
...
function VersionActivity3_5DungeonMapNormalInteractView:createRewardItem()
...
function VersionActivity3_5DungeonMapNormalInteractView:refreshNoneUI()
...
function VersionActivity3_5DungeonMapNormalInteractView:setFinishText()
	local finishText = self._config.finishText

	self._txtdesc.text = string.nilorempty(finishText) and self._config.desc or finishText
end

function VersionActivity3_5DungeonMapNormalInteractView:refreshFightUI()
...
function VersionActivity3_5DungeonMapNormalInteractView:refreshDialogueUI()
...
function VersionActivity3_5DungeonMapNormalInteractView:refreshTalkUI()
	self._sectionStack = {}
...
function VersionActivity3_5DungeonMapNormalInteractView:_playSection(sectionId, dialogIndex)
	self:_setSectionData(sectionId, dialogIndex)
	self:_playNextDialog()
end

function VersionActivity3_5DungeonMapNormalInteractView:_setSectionData(sectionId, dialogIndex)
...
function VersionActivity3_5DungeonMapNormalInteractView:_playNextDialog()
...
function VersionActivity3_5DungeonMapNormalInteractView:_showDialog(type, text, speaker, audio)
	...
	if hasSpeaker then
		self._txtchatname.text = speaker .. ":" or ""
	end
...
function VersionActivity3_5DungeonMapNormalInteractView:setIsChat(isChat)
	gohelper.setActive(self._godesc, not isChat)
	gohelper.setActive(self._gochat, isChat)
end

function VersionActivity3_5DungeonMapNormalInteractView:_addDialogOption(index, sectionId, text)
	if self._optionBtnList[index] then
		if not self._optionBtnList[index][1] then
			local optionGo = gohelper.cloneInPlace(self._gotalkitem)
...
function VersionActivity3_5DungeonMapNormalInteractView:_onOptionClick(param)
...
function VersionActivity3_5DungeonMapNormalInteractView:_refreshDialogBtnState(showOption, hasNext)
...
function VersionActivity3_5DungeonMapNormalInteractView:onClickRoot()
	self:hide()
...
function VersionActivity3_5DungeonMapNormalInteractView:_btncloseOnClick()
	self:hide()
...
function VersionActivity3_5DungeonMapNormalInteractView:_onClickNoneBtn()
	self:hide()
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_checkpoint_continuemesh)
	self:finishElement()
end

function VersionActivity3_5DungeonMapNormalInteractView:_onClickFightBtn()
	self:hide()
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_checkpoint_continuemesh)
...
function VersionActivity3_5DungeonMapNormalInteractView:_onClickEnterDialogueBtn()
	if self.isFinish and not self._isRecheck then
		self:hide()
		self:finishElement()
...
function VersionActivity3_5DungeonMapNormalInteractView:finishElement(dialogIds)
...
function VersionActivity3_5DungeonMapNormalInteractView:refreshElement()
	return
end

function VersionActivity3_5DungeonMapNormalInteractView:_btnnextOnClick()
	self:_playNextSectionOrDialog()
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_checkpoint_continuemesh)
end

function VersionActivity3_5DungeonMapNormalInteractView:_playNextSectionOrDialog()
	if #self._sectionList >= self._dialogIndex then
		self:_playNextDialog()
...
function VersionActivity3_5DungeonMapNormalInteractView:_btnfinishtalkOnClick()
	self:hide()
...
function VersionActivity3_5DungeonMapNormalInteractView:onDialogueInfoChange(dialogueId)
	if dialogueId == self.dialogueId then
		self:refreshDialogueUI()
	end
end

function VersionActivity3_5DungeonMapNormalInteractView:onCloseViewFinishCall(viewName)
	if viewName == ViewName.DialogueView and self.isFinish then
		self:refreshUI()
	end
end

function VersionActivity3_5DungeonMapNormalInteractView:beforeJump()
	self:hide()
end

function VersionActivity3_5DungeonMapNormalInteractView:onClose()
	return
end

function VersionActivity3_5DungeonMapNormalInteractView:onDestroyView()
	self.rootClick:RemoveClickListener()
end

return VersionActivity3_5DungeonMapNormalInteractView
