-- chunkname: @modules/logic/versionactivity1_7/enter/view/subview/V1a7_Season123EnterView.lua

module("modules.logic.versionactivity1_7.enter.view.subview.V1a7_Season123EnterView", package.seeall)

local V1a7_Season123EnterView = class("V1a7_Season123EnterView", BaseView)

function V1a7_Season123EnterView:onInitView()
...
function V1a7_Season123EnterView:addEvents()
	...
	self.addEventCb(self, CurrencyController.instance, CurrencyEvent.CurrencyChange, self.refreshStoreCoin, self)
	self.addEventCb(self, ActivityController.instance, ActivityEvent.RefreshActivityState, self.refreshUI, self)
end

function V1a7_Season123EnterView:removeEvents()
	self._btnnormal:RemoveClickListener()
	self._btnLocked:RemoveClickListener()
	self._btnstore:RemoveClickListener()
	self.removeEventCb(self, CurrencyController.instance, CurrencyEvent.CurrencyChange, self.refreshStoreCoin, self)
	self.removeEventCb(self, ActivityController.instance, ActivityEvent.RefreshActivityState, self.refreshUI, self)
end

function V1a7_Season123EnterView:_btnNormalOnClick()
...
function V1a7_Season123EnterView:_btnLockedOnClick()
...
function V1a7_Season123EnterView:_btnStoreOnClick()
	Season123Controller.instance:openSeasonStoreView(self.actId)
end

function V1a7_Season123EnterView:_editableInitView()
...
function V1a7_Season123EnterView:onUpdateParam()
	return
end

function V1a7_Season123EnterView:onOpen()
	self.animComp:playOpenAnim()

	self.actId = VersionActivity1_7Enum.ActivityId.Season

	RedDotController.instance:addRedDot(self._goreddot, RedDotEnum.DotNode.Season123Enter)
	self:refreshUI()
end

function V1a7_Season123EnterView:refreshUI()
...
function V1a7_Season123EnterView:refreshDesc()
...
function V1a7_Season123EnterView:refreshStoreCoin()
...
function V1a7_Season123EnterView:refreshEnterBtn()
	local status, toastId, toastParam = ActivityHelper.getActivityStatusAndToast(self.actId)
...
function V1a7_Season123EnterView:refreshRemainTime()
	self:refreshMainTime()
	self:refreshStoreTime()
end

function V1a7_Season123EnterView:refreshMainTime()
...
function V1a7_Season123EnterView:refreshStoreTime()
	...
	self._txtstoretime.text = status ~= ActivityEnum.ActivityStatus.Normal and toastId and (status == ActivityEnum.ActivityStatus.Expired and luaLang("ended") or luaLang("notOpen")) or actInfoMo:getRemainTimeStr2ByEndTime(true)
end

function V1a7_Season123EnterView:onClose()
	TaskDispatcher.cancelTask(self.refreshRemainTime, self)
end

function V1a7_Season123EnterView:onDestroyView()
	self.animComp:destroy()
end

return V1a7_Season123EnterView
