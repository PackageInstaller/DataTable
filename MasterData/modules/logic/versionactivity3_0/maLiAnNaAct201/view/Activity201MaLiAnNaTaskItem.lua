-- chunkname: @modules/logic/versionactivity3_0/maLiAnNaAct201/view/Activity201MaLiAnNaTaskItem.lua

module("modules.logic.versionactivity3_0.maLiAnNaAct201.view.Activity201MaLiAnNaTaskItem", package.seeall)

local Activity201MaLiAnNaTaskItem = class("Activity201MaLiAnNaTaskItem", ListScrollCellExtend)

function Activity201MaLiAnNaTaskItem:onInitView()
...
function Activity201MaLiAnNaTaskItem:addEvents()
...
function Activity201MaLiAnNaTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function Activity201MaLiAnNaTaskItem:_btnnotfinishbgOnClick()
	if self._act203TaskMO.config.jumpId > 0 then
		GameFacade.jump(self._act203TaskMO.config.jumpId)
	end
end

function Activity201MaLiAnNaTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._act203TaskMO.activityId)
	UIBlockHelper.instance:startBlock("Activity201MaLiAnNaTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function Activity201MaLiAnNaTaskItem:_btngetallOnClick()
	Activity201MaLiAnNaController.instance:dispatchEvent(Activity201MaLiAnNaEvent.OneClickClaimReward, self._act203TaskMO.activityId, true)
	UIBlockHelper.instance:startBlock("Activity201MaLiAnNaTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function Activity201MaLiAnNaTaskItem:_delayFinish()
	TaskRpc.instance:sendFinishTaskRequest(self._act203TaskMO.config.id)
end

function Activity201MaLiAnNaTaskItem:_delayFinishAll()
	TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Activity203)
end

function Activity201MaLiAnNaTaskItem:_editableInitView()
...
function Activity201MaLiAnNaTaskItem:_editableAddEvents()
	Activity201MaLiAnNaController.instance:registerCallback(Activity201MaLiAnNaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function Activity201MaLiAnNaTaskItem:_editableRemoveEvents()
	Activity201MaLiAnNaController.instance:unregisterCallback(Activity201MaLiAnNaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function Activity201MaLiAnNaTaskItem:_onOneClickClaimReward(actId, isAll)
	if self._act203TaskMO and self._act203TaskMO.activityId == actId and (self._act203TaskMO:alreadyGotReward() or self._act203TaskMO.id == -99999 and isAll) then
...
function Activity201MaLiAnNaTaskItem:getAnimator()
	return self._animator
end

function Activity201MaLiAnNaTaskItem:onUpdateMO(mo)
	self._act203TaskMO = mo

	local rankDiff = Activity201MaLiAnNaTaskListModel.instance:getRankDiff(mo)
...
function Activity201MaLiAnNaTaskItem:_moveByRankDiff(rankDiff)
...
function Activity201MaLiAnNaTaskItem:onSelect(isSelect)
	return
end

function Activity201MaLiAnNaTaskItem:_refreshUI()
	local atMO = self._act203TaskMO
...
function Activity201MaLiAnNaTaskItem:_onItemShow(cell_component, data, index)
...
function Activity201MaLiAnNaTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
...
return Activity201MaLiAnNaTaskItem
