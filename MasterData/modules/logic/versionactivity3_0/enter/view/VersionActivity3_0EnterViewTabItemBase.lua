-- chunkname: @modules/logic/versionactivity3_0/enter/view/VersionActivity3_0EnterViewTabItemBase.lua

module("modules.logic.versionactivity3_0.enter.view.VersionActivity3_0EnterViewTabItemBase", package.seeall)

local VersionActivity3_0EnterViewTabItemBase = class("VersionActivity3_0EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)

function VersionActivity3_0EnterViewTabItemBase:_editableInitView()
...
function VersionActivity3_0EnterViewTabItemBase:_getTagPath()
	return "#go_tag"
end

function VersionActivity3_0EnterViewTabItemBase:afterSetData()
	if self.actId then
	...
	if not self.activityCo then
		...
		logError("VersionActivity3_0EnterViewTabItemBase.afterSetData error, no act config, actId:%s", self.actId)
	...
	if not string.nilorempty(selectBg) then
		UISpriteSetMgr.instance:setV3a0MainActivitySprite(self.imageSelectTabIcon, selectBg)
	end

	local unselectBg = iconCoArr[2]

	if not string.nilorempty(unselectBg) then
		UISpriteSetMgr.instance:setV3a0MainActivitySprite(self.imageUnSelectTabIcon, unselectBg)
	end

	self.redDotIcon = RedDotController.instance:addRedDot(self.goRedDot, self.activityCo.redDotId, self.redDotUid)
end

function VersionActivity3_0EnterViewTabItemBase:childRefreshSelect()
...
function VersionActivity3_0EnterViewTabItemBase:childRefreshUI()
...
function VersionActivity3_0EnterViewTabItemBase:refreshTag()
	self:clearTag()
...
function VersionActivity3_0EnterViewTabItemBase:clearTag()
...
function VersionActivity3_0EnterViewTabItemBase:refreshNormalTag()
	...
	local isShowRemainTime = VersionActivity3_0EnterHelper.GetIsShowTabRemainTime(self.actId)

	if isShowRemainTime and actInfoMo then
		local offsetSecond = actInfoMo:getRealEndTimeStamp() - ServerTime.now()

		if offsetSecond > VersionActivity3_0Enum.MaxShowTimeOffset then
			return
		end
...
function VersionActivity3_0EnterViewTabItemBase:refreshNotUnlockTag()
	gohelper.setActive(self.goTagLock, false)
...
function VersionActivity3_0EnterViewTabItemBase:refreshLockTag()
	...
	if status == ActivityEnum.ActivityStatus.NotOpen then
		...
		self.txtLock.text = actInfoMo:getRemainTimeStr2ByOpenTime()
	else
		gohelper.setActive(self.goTagLock, false)
	end
end

function VersionActivity3_0EnterViewTabItemBase:isShowRedDot()
	return self.redDotIcon and self.redDotIcon.show
end

return VersionActivity3_0EnterViewTabItemBase
