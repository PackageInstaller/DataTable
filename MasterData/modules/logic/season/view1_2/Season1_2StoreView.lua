-- chunkname: @modules/logic/season/view1_2/Season1_2StoreView.lua

module("modules.logic.season.view1_2.Season1_2StoreView", package.seeall)

local Season1_2StoreView = class("Season1_2StoreView", BaseView)

function Season1_2StoreView:onInitView()
...
function Season1_2StoreView:addEvents()
	return
end

function Season1_2StoreView:removeEvents()
	return
end

function Season1_2StoreView:_editableInitView()
	self._simagebg:LoadImage(ResUrl.getV1A2SeasonIcon("full/shangcheng_bj.jpg"))
	gohelper.setActive(self._gostoreItem, false)

	self.storeItemList = self:getUserDataTb_()
end

function Season1_2StoreView:onUpdateParam()
	return
end

function Season1_2StoreView:onOpen()
	self.addEventCb(self, VersionActivityController.instance, VersionActivityEvent.OnGet107GoodsInfo, self._onGet107GoodsInfo, self)
	self.addEventCb(self, VersionActivityController.instance, VersionActivityEvent.OnBuy107GoodsSuccess, self._onBuyGoodsSuccess, self)
...
function Season1_2StoreView:_onGet107GoodsInfo(actId)
...
function Season1_2StoreView:_onBuyGoodsSuccess(actId)
	if actId ~= self.actId then
		return
	end
...
function Season1_2StoreView:updateView()
	self:refreshStoreContent()
end

function Season1_2StoreView:refreshStoreContent()
...
function Season1_2StoreView.sortGoods(goodCo1, goodCo2)
...
function Season1_2StoreView:getItemGo(index)
	local setting = self.viewContainer:getSetting()
	local resPath = setting.otherRes.itemPath

	return (self.viewContainer:getResInst(resPath, self._goContent, string.format("item%s", index)))
end

function Season1_2StoreView:refreshTime()
...
function Season1_2StoreView:onClose()
	TaskDispatcher.cancelTask(self.refreshTime, self)
	self.removeEventCb(self, VersionActivityController.instance, VersionActivityEvent.OnGet107GoodsInfo, self._onGet107GoodsInfo, self)
	self.removeEventCb(self, VersionActivityController.instance, VersionActivityEvent.OnBuy107GoodsSuccess, self._onBuyGoodsSuccess, self)
end

function Season1_2StoreView:onDestroyView()
