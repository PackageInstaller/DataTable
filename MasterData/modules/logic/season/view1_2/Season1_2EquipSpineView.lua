-- chunkname: @modules/logic/season/view1_2/Season1_2EquipSpineView.lua

module("modules.logic.season.view1_2.Season1_2EquipSpineView", package.seeall)

local Season1_2EquipSpineView = class("Season1_2EquipSpineView", BaseView)

function Season1_2EquipSpineView:onInitView()
...
function Season1_2EquipSpineView:addEvents()
	return
end

function Season1_2EquipSpineView:removeEvents()
	return
end

function Season1_2EquipSpineView:_editableInitView()
	self._uiSpine = GuiModelAgent.Create(self._gospine, true)

	self:createSpine()
end

function Season1_2EquipSpineView:onDestroyView()
...
function Season1_2EquipSpineView:onOpen()
	self:refreshHeroSkin()
end

function Season1_2EquipSpineView:onClose()
	if self._uiSpine then
		self._uiSpine:setModelVisible(false)
	end
end

function Season1_2EquipSpineView:createSpine(skinConfig)
	if skinConfig then
		self._uiSpine:useRT()
...
function Season1_2EquipSpineView:refreshHeroSkin()
...
function Season1_2EquipSpineView:onSpineLoaded()
	self._spineLoaded = true

	self._uiSpine:setUIMask(true)
end

return Season1_2EquipSpineView
