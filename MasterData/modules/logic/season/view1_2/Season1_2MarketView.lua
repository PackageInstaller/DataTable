...
function Season1_2MarketView:_setInfo()
	...
	if isNewStage then
		if not Season1_2MarketView.NewStageTagPos then
			local targetStageTagPos = Season1_2MarketView.NomalStageTagPos
...
function Season1_2MarketView:_showPartStageItem(stage)
	if stage < 7 then
		...
		for i = 1, 6 do
			...
			if i <= stage then
				self._partStageItems[i].canvasgroup.alpha = Season1_2MarketView.UnLockStageItemAlpha or Season1_2MarketView.LockStageItemAlpha
			end
		end
	else
		for i = 1, 7 do
