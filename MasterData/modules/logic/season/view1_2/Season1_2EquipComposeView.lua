-- chunkname: @modules/logic/season/view1_2/Season1_2EquipComposeView.lua

module("modules.logic.season.view1_2.Season1_2EquipComposeView", package.seeall)

local Season1_2EquipComposeView = class("Season1_2EquipComposeView", BaseView)

function Season1_2EquipComposeView:onInitView()
...
function Season1_2EquipComposeView:addEvents()
...
function Season1_2EquipComposeView:removeEvents()
	self._btncompose:RemoveClickListener()
	self._btndiscompose:RemoveClickListener()
end

Season1_2EquipComposeView.MaxUICount = 3

function Season1_2EquipComposeView:_editableInitView()
...
function Season1_2EquipComposeView:onDestroyView()
...
function Season1_2EquipComposeView:onOpen()
...
function Season1_2EquipComposeView:onClose()
	UIBlockMgrExtend.setNeedCircleMv(true)
	UIBlockMgr.instance:endBlock(Season1_2EquipComposeView.Compose_Anim_Block_Key)
	TaskDispatcher.cancelTask(self.onPlayComposeAnimOver, self)
	TaskDispatcher.cancelTask(self.delayRefreshView, self)
end

function Season1_2EquipComposeView:handleComposeSucc()
	return
end

function Season1_2EquipComposeView:handleComposeDataChanged()
	if self._delayRefreshUITime ~= nil then
		local curTime = Time.time

		if curTime - self._delayRefreshUITime < Season1_2EquipComposeView.DelayRefreshTime then
			return
		end
	end
...
function Season1_2EquipComposeView:refreshUI()
...
function Season1_2EquipComposeView:refreshHint()
...
function Season1_2EquipComposeView:refreshMatList()
	for i = 1, Season1_2EquipComposeView.MaxUICount do
		self:refreshMat(i)
	end
end

function Season1_2EquipComposeView:refreshButtons()
...
function Season1_2EquipComposeView:refreshMat(index)
...
function Season1_2EquipComposeView:getOrCreateMatItem(index)
...
function Season1_2EquipComposeView:checkCreateMatItemIcon(item, index)
	if not item.icon then
		local path = self.viewContainer:getSetting().otherRes[2]
		local go = self:getResInst(path, item.goIcon, "icon" .. tostring(index))

		item.icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season1_2CelebrityCardEquip)
...
function Season1_2EquipComposeView:onClickMatSlot(index)
	local itemUid = Activity104EquipItemComposeModel.instance.curSelectMap[index]
	local isEmpty = itemUid == Activity104EquipItemComposeModel.EmptyUid
...
function Season1_2EquipComposeView:_btncomposeOnClick()
...
function Season1_2EquipComposeView:_btndiscomposeOnClick()
	GameFacade.showToast(ToastEnum.ClickSeasonDiscompose)
end

Season1_2EquipComposeView.DelaySendTime = 1
Season1_2EquipComposeView.DelayRefreshTime = 1
Season1_2EquipComposeView.Compose_Anim_Block_Key = "Compose_Anim_Block"

function Season1_2EquipComposeView:playAnimBeforeSend()
	TaskDispatcher.cancelTask(self.onPlayComposeAnimOver, self)
	...
	UIBlockMgr.instance:startBlock(Season1_2EquipComposeView.Compose_Anim_Block_Key)
	TaskDispatcher.runDelay(self.onPlayComposeAnimOver, self, Season1_2EquipComposeView.DelaySendTime)
end

function Season1_2EquipComposeView:onPlayComposeAnimOver()
	UIBlockMgrExtend.setNeedCircleMv(true)
	UIBlockMgr.instance:endBlock(Season1_2EquipComposeView.Compose_Anim_Block_Key)
	...
	TaskDispatcher.runDelay(self.delayRefreshView, self, Season1_2EquipComposeView.DelayRefreshTime)
end

function Season1_2EquipComposeView:delayRefreshView()
	self:refreshUI()
end

return Season1_2EquipComposeView
