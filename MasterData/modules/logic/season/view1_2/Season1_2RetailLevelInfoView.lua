-- chunkname: @modules/logic/season/view1_2/Season1_2RetailLevelInfoView.lua

module("modules.logic.season.view1_2.Season1_2RetailLevelInfoView", package.seeall)

local Season1_2RetailLevelInfoView = class("Season1_2RetailLevelInfoView", BaseView)

function Season1_2RetailLevelInfoView:onInitView()
...
function Season1_2RetailLevelInfoView:addEvents()
...
function Season1_2RetailLevelInfoView:removeEvents()
...
function Season1_2RetailLevelInfoView:_onBattleReply(msg)
	Activity104Model.instance:onStartAct104BattleReply(msg)
end

function Season1_2RetailLevelInfoView:_btnclose1OnClick()
	self:closeThis()
end

function Season1_2RetailLevelInfoView:_btnclose2OnClick()
	self:closeThis()
end

function Season1_2RetailLevelInfoView:_btncloseOnClick()
	self:closeThis()
end

function Season1_2RetailLevelInfoView:_btnstartOnClick()
...
function Season1_2RetailLevelInfoView:_editableInitView()
	...
	self._simageleftbg:LoadImage(ResUrl.getV1A2SeasonIcon("msg_xia.png"))
	self._simagerightbg:LoadImage(ResUrl.getV1A2SeasonIcon("msg_shang.png"))
end

function Season1_2RetailLevelInfoView:onUpdateParam()
	return
end

function Season1_2RetailLevelInfoView:onOpen()
	self._cardItems = {}
...
function Season1_2RetailLevelInfoView:_setInfo()
	...
	for i = 1, #self.viewParam.retail.showActivity104EquipIds do
		...
		if not self._cardItems[i] then
			self._cardItems[i] = Season1_2CelebrityCardItem.New()
...
Season1_2RetailLevelInfoView.MaxStageCount = 7

function Season1_2RetailLevelInfoView:_refreshStateUI()
	self._stageItemsTab = self._stageItemsTab or self:getUserDataTb_()

	local stage = Activity104Model.instance:getAct104CurStage()

	for i = 1, Season1_2RetailLevelInfoView.MaxStageCount do
...
function Season1_2RetailLevelInfoView:onClose()
	Activity104Controller.instance:dispatchEvent(Activity104Event.SelectRetail)
end

function Season1_2RetailLevelInfoView:onDestroyView()
...
return Season1_2RetailLevelInfoView
