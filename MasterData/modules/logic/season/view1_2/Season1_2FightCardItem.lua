-- chunkname: @modules/logic/season/view1_2/Season1_2FightCardItem.lua

module("modules.logic.season.view1_2.Season1_2FightCardItem", package.seeall)

local Season1_2FightCardItem = class("Season1_2FightCardItem", UserDataDispose)

function Season1_2FightCardItem:ctor(go)
	self:__onInit()
...
Season1_2FightCardItem.MainRoleItemMinHeight = 390
Season1_2FightCardItem.NormalRoleItemMinHeight = 315
Season1_2FightCardItem.RoleCardPos = {
	Vector2(-16.5, 0.5),
	Vector2(-16.5, 0.5),
	Vector2(-16.5, -6.5),
	Vector2(-16.5, -17),
...
function Season1_2FightCardItem:setData(data)
	if not data then
	...
	if self.isMainRole then
		self.layoutElement.minHeight = Season1_2FightCardItem.MainRoleItemMinHeight or Season1_2FightCardItem.NormalRoleItemMinHeight
	end
...
function Season1_2FightCardItem:_setName()
	if self.isMainRole then
...
function Season1_2FightCardItem:_setHead()
...
function Season1_2FightCardItem:_setCard(equipId)
	if not self.cardItem then
		self.cardItem = Season1_2CelebrityCardItem.New()
	...
	local targetCardPos = Season1_2FightCardItem.RoleCardPos[tonumber(rare)] or Vector2(0, 0)

	recthelper.setAnchor(self.goCardPos.transform, targetCardPos.x, targetCardPos.y)
end

function Season1_2FightCardItem:_setDesc(equipId)
...
function Season1_2FightCardItem:createDescItem(index)
	local item = {}
...
function Season1_2FightCardItem:updateDescItem(item, skillStr)
...
function Season1_2FightCardItem:destroyDescItem(item)
	return
end

function Season1_2FightCardItem:destroy()
