-- chunkname: @modules/logic/season/view1_2/Season1_2TaskItem.lua

module("modules.logic.season.view1_2.Season1_2TaskItem", package.seeall)

local Season1_2TaskItem = class("Season1_2TaskItem", UserDataDispose)

Season1_2TaskItem.BlockKey = "Season1_2TaskItemAni"

function Season1_2TaskItem:ctor(go, parentScrollGO)
	self:__onInit()
...
function Season1_2TaskItem:addEventListeners()
...
function Season1_2TaskItem:initTotal()
	self._btnGetTotal = gohelper.findChildButtonWithAudio(self._goTotal, "#btn_getall")
end

function Season1_2TaskItem:onUpdateMO(data, open)
	if data then
...
function Season1_2TaskItem:hide()
	gohelper.setActive(self._viewGO, false)
end

function Season1_2TaskItem:refreshNormal(data)
	self.taskId = data.id
...
function Season1_2TaskItem:refreshReward(data)
...
function Season1_2TaskItem:createRewardItem(index)
...
function Season1_2TaskItem:refreshRewardItem(item, data)
	if not data then
	...
	if data.equipId then
		...
		if not item.equipIcon then
			item.equipIcon = Season1_2CelebrityCardItem.New()
...
function Season1_2TaskItem:destroyRewardItem(item)
...
function Season1_2TaskItem:refreshDesc(data)
	local config = data.config
...
function Season1_2TaskItem:refreshProgress(data)
	self._txtCurCount.text = data.progress
	self._txtMaxCount.text = data.config.maxProgress
end

function Season1_2TaskItem:refreshState(data)
...
function Season1_2TaskItem:onClickGoto()
...
function Season1_2TaskItem:onClickReceive()
	...
	UIBlockMgr.instance:startBlock(Season1_2TaskItem.BlockKey)
	TaskDispatcher.runDelay(self._onPlayActAniFinished, self, 0.76)
end

function Season1_2TaskItem:onClickGetTotal()
	TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Season)
end

function Season1_2TaskItem:_onPlayActAniFinished()
	UIBlockMgr.instance:endBlock(Season1_2TaskItem.BlockKey)
	TaskRpc.instance:sendFinishTaskRequest(self.taskId)
	self:hide()
end

function Season1_2TaskItem:destroy()
