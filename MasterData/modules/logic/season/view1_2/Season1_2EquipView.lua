-- chunkname: @modules/logic/season/view1_2/Season1_2EquipView.lua

module("modules.logic.season.view1_2.Season1_2EquipView", package.seeall)

local Season1_2EquipView = class("Season1_2EquipView", BaseView)

function Season1_2EquipView:onInitView()
...
function Season1_2EquipView:addEvents()
...
function Season1_2EquipView:removeEvents()
	self._btnequip:RemoveClickListener()
	self._btnopenhandbook:RemoveClickListener()
end

function Season1_2EquipView:_editableInitView()
...
function Season1_2EquipView:onDestroyView()
...
function Season1_2EquipView:onOpen()
	if not self.viewParam.pos then
		...
		if not self.viewParam.slot then
			if not self.viewParam.group then
				...
				self.addEventCb(self, Activity104EquipController.instance, Activity104EquipEvent.EquipUpdate, self.handleEquipUpdate, self)
				self.addEventCb(self, HeroGroupController.instance, HeroGroupEvent.OnSnapshotSaveSucc, self.handleSaveSucc, self)
				self.addEventCb(self, Activity104EquipController.instance, Activity104EquipEvent.EquipChangeCard, self.handleEquipCardChanged, self)
				self.addEventCb(self, Activity104EquipController.instance, Activity104EquipEvent.EquipChangeSlot, self.handleEquipSlotChanged, self)
				Activity104EquipController.instance:onOpenView(actId, group, self.viewParam.pos, self.viewParam.slot)
				self:refreshUI()
			end
		end
	end
...
function Season1_2EquipView:onClose()
	return
end

function Season1_2EquipView:refreshUI()
...
function Season1_2EquipView:handleEquipUpdate()
	self:refreshSlots()
	self:refreshDescGroup()
end

function Season1_2EquipView:refreshSlots()
...
function Season1_2EquipView:refreshSlot(slotIndex)
...
Season1_2EquipView.MaxUISlot = 2

function Season1_2EquipView:refreshDescGroup()
	for slot = 1, Season1_2EquipView.MaxUISlot do
...
function Season1_2EquipView:refreshDesc(descItem, itemUid, slotIndex)
	...
	if itemUid == Activity104EquipItemListModel.EmptyUid then
		...
		if itemMO then
			...
			if itemCO then
				...
				local colorNameStr
				local colorDescStr = SeasonEquipMetaUtils.getCareerColorDarkBg(itemMO.itemId)
...
function Season1_2EquipView:refreshProps(itemCfg, descItem, colorStr)
	local processedSet = {}
	local isDirty = false
...
function Season1_2EquipView:refreshSkills(itemCfg, descItem, colorStr)
...
function Season1_2EquipView:getOrCreatePropText(index, descItem)
...
function Season1_2EquipView:getOrCreateSkillText(index, descItem)
...
function Season1_2EquipView:getOrCreateSlot(slotIndex)
...
function Season1_2EquipView:getOrCreateSlotIcon(slotItem)
	...
	if not icon then
		...
		icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season1_2CelebrityCardEquip)
		slotItem.icon = icon
	end
...
function Season1_2EquipView:getOrCreateDesc(index)
...
function Season1_2EquipView:onClickSlot(slotIndex)
	if Activity104EquipItemListModel.instance.curSelectSlot ~= slotIndex then
...
function Season1_2EquipView:handleSaveSucc(snapshotId)
...
function Season1_2EquipView:handleSwitchAnimFrame()
	self:refreshSlots()
	self:refreshDescGroup()
end

function Season1_2EquipView.handleSlotSwitchAnimFrame(slotItem)
	local mainView = slotItem.mainView

	mainView:refreshDescGroup()
end

function Season1_2EquipView:handleEquipCardChanged(param)
...
function Season1_2EquipView:handleEquipSlotChanged()
	self._animator:Play("switch", 0, 0)
	self:refreshSlots()
end

function Season1_2EquipView:_btnequipOnClick()
	self._isManualSave = Activity104EquipController.instance:saveShowSlot()
end

function Season1_2EquipView:_btnhandbookOnClick()
	ViewMgr.instance:openView(ViewName.Season1_2EquipBookView, {
		actId = self.viewParam.actId
	})
end

return Season1_2EquipView
