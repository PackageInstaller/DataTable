-- chunkname: @modules/logic/season/view1_2/Season1_2CelebrityCardEquip.lua

module("modules.logic.season.view1_2.Season1_2CelebrityCardEquip", package.seeall)

local Season1_2CelebrityCardEquip = class("Season1_2CelebrityCardEquip", LuaCompBase)

function Season1_2CelebrityCardEquip:init(go)
	Season1_2CelebrityCardEquip.super.init(self, go)
...
Season1_2CelebrityCardEquip.MaxRare = 5

function Season1_2CelebrityCardEquip:_editableInitView()
	self._rareGoMap = {}

	for i = 1, Season1_2CelebrityCardEquip.MaxRare do
...
function Season1_2CelebrityCardEquip:onDestroy()
	self:disposeUI()
end

function Season1_2CelebrityCardEquip:checkInitBtnClick()
...
function Season1_2CelebrityCardEquip:checkInitLongPress()
...
function Season1_2CelebrityCardEquip:disposeUI()
...
function Season1_2CelebrityCardEquip:updateData(itemId)
	self.itemId = itemId

	self:refreshUI()
end

function Season1_2CelebrityCardEquip:createRareMap(go)
...
function Season1_2CelebrityCardEquip:refreshUI()
...
function Season1_2CelebrityCardEquip:refreshSelfChoice()
...
function Season1_2CelebrityCardEquip:refreshIcon()
	if self._curSelectedItem then
...
function Season1_2CelebrityCardEquip:refreshFlag()
	if not self.itemId then
		return
	end
...
function Season1_2CelebrityCardEquip:setFlagUIPos(targetPosX, targetPosY)
...
function Season1_2CelebrityCardEquip:setFlagUIScale(targetScale)
...
function Season1_2CelebrityCardEquip:handleIconLoaded()
	if not self._isDisposed then
		local item = self._curSelectedItem
...
function Season1_2CelebrityCardEquip:setColorDark(enable)
	self._darkMaskColor = enable and "#7b7b7b" or "#ffffff"

	self:refreshIcon()
end

function Season1_2CelebrityCardEquip:setShowTag(enable)
	self._showTag = enable

	self:refreshFlag()
end

function Season1_2CelebrityCardEquip:setShowProbability(enable)
	self._showProbability = enable

	self:refreshFlag()
end

function Season1_2CelebrityCardEquip:setShowNewFlag(enable)
	self._showNewFlag = enable

	self:refreshFlag()
end

function Season1_2CelebrityCardEquip:setShowNewFlag2(enable)
	self._showNewFlag2 = enable

	self:refreshFlag()
end

function Season1_2CelebrityCardEquip:setClickCall(clickCall, clickCallObj, param)
...
function Season1_2CelebrityCardEquip:setLongPressCall(longPressCall, longPressCallObj, param)
