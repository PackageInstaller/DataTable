-- chunkname: @modules/logic/season/view1_2/Season1_2HeroGroupListView.lua

module("modules.logic.season.view1_2.Season1_2HeroGroupListView", package.seeall)

local Season1_2HeroGroupListView = class("Season1_2HeroGroupListView", BaseView)

function Season1_2HeroGroupListView:onInitView()
...
function Season1_2HeroGroupListView:_editableInitView()
	...
	for i = 1, 4 do
		...
		local item = MonoHelper.addNoUpdateLuaComOnceToGo(cloneGO, Season1_2HeroGroupHeroItem, self)
...
function Season1_2HeroGroupListView:addEvents()
...
function Season1_2HeroGroupListView:removeEvents()
...
function Season1_2HeroGroupListView:onOpen()
	self._isOpen = true
...
function Season1_2HeroGroupListView:_playOpenAnimation()
...
function Season1_2HeroGroupListView:_checkRestrictHero()
...
function Season1_2HeroGroupListView:_removeRestrictHero()
...
function Season1_2HeroGroupListView:_onHeroGroupExit()
	...
	ViewMgr.instance:closeView(ViewName.Season1_2HeroGroupFightView, false, false)
end

function Season1_2HeroGroupListView:_closeTweenFinish(index)
...
function Season1_2HeroGroupListView:canDrag(param, isWrap)
...
function Season1_2HeroGroupListView:_onBeginDrag(param, pointerEventData)
...
function Season1_2HeroGroupListView:_onDrag(param, pointerEventData)
	if not self:canDrag(param) then
		return
	end
...
function Season1_2HeroGroupListView:_onEndDrag(param, pointerEventData)
	if not self:canDrag(param) then
		return
	end
	...
	self:_setHeroItemPos(heroItem, dragToIndex, true, function()
		...
		heroGroupMO.heroList[srcPos + 1] = heroGroupMO.heroList[targetPos + 1]
		heroGroupMO.heroList[targetPos + 1] = heroGroupMO.heroList[srcPos + 1]
...
function Season1_2HeroGroupListView:_setHeroItemPos(heroItem, index, tween, callback, callbackObj)
	local posTr = self.heroPosTrList[index]
...
function Season1_2HeroGroupListView:_tweenToPos(heroItem, anchorPos)
...
function Season1_2HeroGroupListView:_setDragEnabled(isEnabled)
	for i, drag in ipairs(self._heroItemDrag) do
		drag.enabled = isEnabled
	end
end

function Season1_2HeroGroupListView:_updateHeroList()
	local groupMO = HeroGroupModel.instance:getCurGroupMO()

	for i, heroItem in ipairs(self._heroItemList) do
		local mo = HeroSingleGroupMO.New()

		mo:init(i, groupMO.heroList[i])
		mo:setAid(groupMO.aidDict and groupMO.aidDict[i])
		heroItem:onUpdateMO(mo)
	end
end

function Season1_2HeroGroupListView:_onSnapshotSaveSucc()
	self:_updateHeroList()
...
function Season1_2HeroGroupListView:_calcIndex(position)
	for i = 1, 4 do
...
function Season1_2HeroGroupListView:onDestroyView()
...
function Season1_2HeroGroupListView:_onScreenSizeChange()
	for i = 1, ModuleEnum.MaxHeroCountInGroup do
		local heroItem = self._heroItemList[i]
...
function Season1_2HeroGroupListView:getHeroItemList()
	return self._heroItemList
end

return Season1_2HeroGroupListView
