-- chunkname: @modules/logic/season/enterview/SeasonEnterView.lua

module("modules.logic.season.enterview.SeasonEnterView", package.seeall)

local SeasonEnterView = class("SeasonEnterView", VersionActivityEnterBaseSubView)

function SeasonEnterView:onInitView()
...
function SeasonEnterView:addEvents()
...
function SeasonEnterView:removeEvents()
...
function SeasonEnterView:_editableInitView()
	...
	self.addEventCb(self, CurrencyController.instance, CurrencyEvent.CurrencyChange, self.refreshCurrency, self)
end

function SeasonEnterView:onOpen()
	SeasonEnterView.super.onOpen(self)
	self:refreshUI()
	self:refreshRemainTime()
end

function SeasonEnterView:onClose()
	SeasonEnterView.super.onClose(self)
end

function SeasonEnterView:_onRefreshRedDot()
	return
end

function SeasonEnterView:onUpdateParam()
	return
end

function SeasonEnterView:refreshEnterBtn()
	...
	local storeId = Activity104Enum.SeasonStore[self.actId]
	local storeStatus = ActivityHelper.getActivityStatus(storeId)

	gohelper.setActive(self._goStore, storeStatus == ActivityEnum.ActivityStatus.Normal)
end

function SeasonEnterView:refreshCurrency()
...
function SeasonEnterView:refreshUI()
	local actCo = ActivityConfig.instance:getActivityCo(self.actId)
...
function SeasonEnterView:onDestroyView()
...
function SeasonEnterView:everySecondCall()
	self:refreshRemainTime()
end

function SeasonEnterView:_onClickMainActivity()
	Activity104Controller.instance:openSeasonMainView()
end

function SeasonEnterView:_onClickStoreBtn()
	Activity104Controller.instance:openSeasonStoreView()
end

function SeasonEnterView:refreshRemainTime()
	self:refreshMainTime()
	self:refreshStoreTime()
end

function SeasonEnterView:refreshMainTime()
...
function SeasonEnterView:refreshStoreTime()
	...
	self._txtstoretime.text = status ~= ActivityEnum.ActivityStatus.Normal and status ~= ActivityEnum.ActivityStatus.NotUnlock and luaLang("turnback_end") or actInfoMo:getRemainTimeStr2ByEndTime(true)
end
