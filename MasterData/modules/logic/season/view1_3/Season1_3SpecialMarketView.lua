-- chunkname: @modules/logic/season/view1_3/Season1_3SpecialMarketView.lua

module("modules.logic.season.view1_3.Season1_3SpecialMarketView", package.seeall)

local Season1_3SpecialMarketView = class("Season1_3SpecialMarketView", BaseView)

function Season1_3SpecialMarketView:onInitView()
...
function Season1_3SpecialMarketView:addEvents()
...
function Season1_3SpecialMarketView:removeEvents()
...
function Season1_3SpecialMarketView:_onBattleReply(msg)
	Activity104Model.instance:onStartAct104BattleReply(msg)
end

function Season1_3SpecialMarketView:_btnlvstartOnClick()
	gohelper.setActive(self._goinfo, true)
	gohelper.setActive(self._golevel, false)
	self:_refreshInfo()
end

function Season1_3SpecialMarketView:_btnlastOnClick()
...
function Season1_3SpecialMarketView:_btnnextOnClick()
...
function Season1_3SpecialMarketView:_btnstartOnClick()
...
function Season1_3SpecialMarketView:_btnclickOnClick()
	return
end

function Season1_3SpecialMarketView:_editableInitView()
...
function Season1_3SpecialMarketView:onUpdateParam()
	return
end

function Season1_3SpecialMarketView:onOpen()
	gohelper.setActive(self._golevel, true)
	gohelper.setActive(self._goinfo, false)
	self.addEventCb(self, Activity104Controller.instance, Activity104Event.SwitchSpecialEpisode, self._onSwitchEpisode, self)
...
function Season1_3SpecialMarketView:onClose()
	self.removeEventCb(self, Activity104Controller.instance, Activity104Event.SwitchSpecialEpisode, self._onSwitchEpisode, self)
end

function Season1_3SpecialMarketView:_onSwitchEpisode(layer)
	self._layer = layer

	self:_refreshLevel()
end

function Season1_3SpecialMarketView:_refreshLevel()
	...
	for i = 1, maxSpecialLayer do
		if not self._showLvItems[i] then
			...
			self._showLvItems[i] = Season1_3SpecialMarketShowLevelItem.New()
	...
	self._simageleveldecorate:LoadImage(ResUrl.getV1A3SeasonIcon(string.format("icon/ty_chatu_%s.png", spEpisodeCo.icon)))
end

function Season1_3SpecialMarketView:_refreshInfo()
	local spEpisodeCo = SeasonConfig.instance:getSeasonSpecialCo(Activity104Model.instance:getCurSeasonId(), self._layer)
	...
	self._simagestageicon:LoadImage(ResUrl.getV1A3SeasonIcon(string.format("icon/a_chatu_%s.png", spEpisodeCo.icon)))
	...
	for i = 1, math.max(#self._rewardItems, #rewards) do
		if not self._rewardItems[i] then
			...
			self:refreshRewardItem(self._rewardItems[i], rewards[i])
		end
	end
...
function Season1_3SpecialMarketView:createRewardItem(index)
...
function Season1_3SpecialMarketView:refreshRewardItem(item, itemCo)
	if not itemCo then
...
function Season1_3SpecialMarketView:updateLeftDesc()
...
function Season1_3SpecialMarketView:createLeftDescItem(index)
	local item = self:getUserDataTb_()
...
function Season1_3SpecialMarketView:updateLeftDescItem(item, co)
	if not co then
		gohelper.setActive(item.go, false)
...
function Season1_3SpecialMarketView:moveToCurDesc()
...
function Season1_3SpecialMarketView:onDestroyView()
	TaskDispatcher.cancelTask(self.moveToCurDesc, self)
...
return Season1_3SpecialMarketView
