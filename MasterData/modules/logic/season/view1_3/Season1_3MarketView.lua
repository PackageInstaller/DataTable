-- chunkname: @modules/logic/season/view1_3/Season1_3MarketView.lua

module("modules.logic.season.view1_3.Season1_3MarketView", package.seeall)

local Season1_3MarketView = class("Season1_3MarketView", BaseView)

function Season1_3MarketView:onInitView()
...
function Season1_3MarketView:addEvents()
	...
	self.addEventCb(self, Activity104Controller.instance, Activity104Event.StartAct104BattleReply, self._onBattleReply, self)
end

function Season1_3MarketView:removeEvents()
	self._btnlast:RemoveClickListener()
	self._btnnext:RemoveClickListener()
	self._btnstart:RemoveClickListener()
	self._btnReplay:RemoveClickListener()
	self.removeEventCb(self, Activity104Controller.instance, Activity104Event.StartAct104BattleReply, self._onBattleReply, self)
end

function Season1_3MarketView:_onBattleReply(msg)
	Activity104Model.instance:onStartAct104BattleReply(msg)
end

function Season1_3MarketView:_btnlastOnClick()
...
function Season1_3MarketView:_btnnextOnClick()
...
function Season1_3MarketView:_delayShowInfo()
	self:_showInfo()
	self:_setStages()
	self:updateLeftDesc()
end

function Season1_3MarketView:_btnstartOnClick()
...
function Season1_3MarketView:_btnreplayOnClick()
	local actId = Activity104Model.instance:getCurSeasonId()
...
function Season1_3MarketView:_editableInitView()
...
function Season1_3MarketView:onUpdateParam()
	return
end

function Season1_3MarketView:onOpen()
	if self.viewParam then
		self._layer = self.viewParam.tarLayer or Activity104Model.instance:getAct104CurLayer()
	end
...
function Season1_3MarketView:resetData()
	self._showLvItems = {}
...
function Season1_3MarketView:_showInfoByOpen()
...
function Season1_3MarketView:_setLevels()
	...
	if self._layer <= 4 then
		for i = 1, 6 do
			...
			self._showLvItems[i] = Season1_3MarketShowLevelItem.New()
...
function Season1_3MarketView:_delayShowItem()
...
function Season1_3MarketView:_setStages()
	local episodeCo = SeasonConfig.instance:getSeasonEpisodeCo(Activity104Model.instance:getCurSeasonId(), self._layer)
	...
	self._simagedecorate:LoadImage(ResUrl.getV1A3SeasonIcon(string.format("icon/b_chatu_%s.png", episodeCo.stagePicture)))
end

function Season1_3MarketView:_showInfo()
	self:_setInfo()
	self:_setParts()
end

Season1_3MarketView.NomalStageTagPos = Vector2(46.3, 2.7)
Season1_3MarketView.NewStageTagPos = Vector2(4.37, 2.7)

function Season1_3MarketView:_setInfo()
	local episodeCo = SeasonConfig.instance:getSeasonEpisodeCo(Activity104Model.instance:getCurSeasonId(), self._layer)
	...
	if isNewStage then
		if not Season1_3MarketView.NewStageTagPos then
			local targetStageTagPos = Season1_3MarketView.NomalStageTagPos

			recthelper.setAnchor(self._gocenter.transform, targetStageTagPos.x, targetStageTagPos.y)
			self._simagestageicon:LoadImage(ResUrl.getV1A3SeasonIcon(string.format("icon/a_chatu_%s.png", episodeCo.stagePicture)))
			...
			for i = 2, math.max(#self._rewardItems - 1, #rewards) + 1 do
				if not self._rewardItems[i] then
					...
					self:refreshRewardItem(self._rewardItems[i], rewards[i - 1])
				end
			end
...
function Season1_3MarketView:refreshEquipCardItem()
	self._rewardItems[1] = self._rewardItems[1] or self:createRewardItem(1)
	...
	if episodeCo.firstPassEquipId > 0 then
		...
		if not self._rewardItems[1].itemIcon then
			self._rewardItems[1].itemIcon = Season1_3CelebrityCardItem.New()
...
function Season1_3MarketView:createRewardItem(index)
...
function Season1_3MarketView:refreshRewardItem(item, itemInfo)
	...
	item.itemIcon = item.itemIcon or IconMgr.instance:getCommonPropItemIcon(item.itemParent)
...
function Season1_3MarketView:_setParts()
	local episodeCo = SeasonConfig.instance:getSeasonEpisodeCo(Activity104Model.instance:getCurSeasonId(), self._layer)
...
Season1_3MarketView.UnLockStageItemAlpha = 1
Season1_3MarketView.LockStageItemAlpha = 0.3

function Season1_3MarketView:_showPartStageItem(stage)
	if stage < 7 then
		...
		for i = 1, 6 do
			...
			if i <= stage then
				self._partStageItems[i].canvasgroup.alpha = Season1_3MarketView.UnLockStageItemAlpha or Season1_3MarketView.LockStageItemAlpha
			end
		end
	else
		for i = 1, 7 do
...
function Season1_3MarketView:updateLeftDesc()
	self.descItems = self.descItems or {}
...
function Season1_3MarketView:createLeftDescItem(index)
	local item = self:getUserDataTb_()
...
function Season1_3MarketView:updateLeftDescItem(item, co)
	if not co then
		gohelper.setActive(item.go, false)
...
function Season1_3MarketView:moveToCurDesc()
...
function Season1_3MarketView:onClose()
	TaskDispatcher.cancelTask(self._delayShowInfo, self)
end

function Season1_3MarketView:onDestroyView()
	TaskDispatcher.cancelTask(self._showInfoByOpen, self)
	TaskDispatcher.cancelTask(self.moveToCurDesc, self)
