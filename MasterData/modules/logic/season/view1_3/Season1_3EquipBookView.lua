-- chunkname: @modules/logic/season/view1_3/Season1_3EquipBookView.lua

module("modules.logic.season.view1_3.Season1_3EquipBookView", package.seeall)

local Season1_3EquipBookView = class("Season1_3EquipBookView", BaseView)

function Season1_3EquipBookView:onInitView()
...
function Season1_3EquipBookView:addEvents()
	self._btnexchange:AddClickListener(self._btnexchangeOnClick, self)
end

function Season1_3EquipBookView:removeEvents()
	self._btnexchange:RemoveClickListener()
end

function Season1_3EquipBookView:_btnexchangeOnClick()
	local actId = Activity104Model.instance:getCurSeasonId()

	ViewMgr.instance:openView(ViewName.Season1_3EquipComposeView, {
		actId = actId
	})
end

function Season1_3EquipBookView:_editableInitView()
...
function Season1_3EquipBookView:onDestroyView()
...
function Season1_3EquipBookView:onOpen()
	local actId = Activity104Model.instance:getCurSeasonId()

	self.addEventCb(self, Activity104EquipBookController.instance, Activity104Event.OnBookUpdateNotify, self.refreshUI, self)
	self.addEventCb(self, Activity104EquipBookController.instance, Activity104Event.OnBookChangeSelectNotify, self.onChangeSelectCard, self)
	Activity104EquipBookController.instance:onOpenView(actId)
	self:refreshUI()
end

function Season1_3EquipBookView:onClose()
	return
end

function Season1_3EquipBookView:refreshUI()
	self:refreshDesc()
	self:refreshIcon()
end

function Season1_3EquipBookView:refreshDesc()
...
function Season1_3EquipBookView:refreshIcon()
...
function Season1_3EquipBookView:refreshProps(itemCfg)
	local processedSet = {}
	local isDirty = false
...
function Season1_3EquipBookView:refreshSkills(itemCfg)
...
function Season1_3EquipBookView:getOrCreatePropText(index)
...
function Season1_3EquipBookView:getOrCreateSkillText(index)
...
function Season1_3EquipBookView:checkCreateIcon()
	if not self._icon then
		local go = self._gocarditem

		self._icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season1_3CelebrityCardEquip)
	end
end

function Season1_3EquipBookView:onChangeSelectCard()
	self._animatorCard:Play("switch", 0, 0)
end

function Season1_3EquipBookView:onSwitchCardAnim()
	self:refreshUI()
end

return Season1_3EquipBookView
