-- chunkname: @modules/logic/season/view1_3/Season1_3SettlementHeroItem.lua

module("modules.logic.season.view1_3.Season1_3SettlementHeroItem", package.seeall)

local Season1_3SettlementHeroItem = class("Season1_3SettlementHeroItem", BaseViewExtended)

function Season1_3SettlementHeroItem:onInitView()
...
function Season1_3SettlementHeroItem:addEvents()
	return
end

function Season1_3SettlementHeroItem:removeEvents()
	return
end

function Season1_3SettlementHeroItem:onRefreshViewParam(is_replay, hero, equip, equip_104, replay_data, trail)
...
function Season1_3SettlementHeroItem:onOpen()
...
function Season1_3SettlementHeroItem:_showNormalData()
	if self._trail then
...
function Season1_3SettlementHeroItem:_showTrailHeroIcon(trail)
	if not trail then
		return
	end
...
function Season1_3SettlementHeroItem:_showHeroIcon(heroId, skinId)
...
function Season1_3SettlementHeroItem:_showEquipIcon(equip_id)
...
function Season1_3SettlementHeroItem:_showEquip104(index, uid, totalEquipCount, equipId)
	if equipId == 0 then
		return
	end
	...
	if totalEquipCount <= 1 then
		if not self._gosingle then
			...
			self:openSubView(Season1_3CelebrityCardGetItem, Season1_3CelebrityCardItem.AssetPath, parentTran, uid, nil, equipId)
		end
	end
end

function Season1_3SettlementHeroItem:_showReplayData()
...
function Season1_3SettlementHeroItem:_refreshEquipPos()
	return
end

function Season1_3SettlementHeroItem:onClose()
	return
end

function Season1_3SettlementHeroItem:onDestroyView()
	self._simageheroicon:UnLoadImage()
end

return Season1_3SettlementHeroItem
