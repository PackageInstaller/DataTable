-- chunkname: @modules/logic/season/view1_3/Season1_3TaskItem.lua

module("modules.logic.season.view1_3.Season1_3TaskItem", package.seeall)

local Season1_3TaskItem = class("Season1_3TaskItem", UserDataDispose)

Season1_3TaskItem.BlockKey = "Season1_3TaskItemAni"

function Season1_3TaskItem:ctor(go, parentScrollGO)
	self:__onInit()
...
function Season1_3TaskItem:addEventListeners()
...
function Season1_3TaskItem:initTotal()
	self._btnGetTotal = gohelper.findChildButtonWithAudio(self._goTotal, "#btn_getall")
end

function Season1_3TaskItem:onUpdateMO(data, open)
	if data then
...
function Season1_3TaskItem:hide()
	gohelper.setActive(self._viewGO, false)
end

function Season1_3TaskItem:refreshNormal(data)
	self.taskId = data.id
...
function Season1_3TaskItem:refreshReward(data)
...
function Season1_3TaskItem:createRewardItem(index)
...
function Season1_3TaskItem:refreshRewardItem(item, data)
	if not data then
	...
	if data.equipId then
		...
		if not item.equipIcon then
			item.equipIcon = Season1_3CelebrityCardItem.New()
...
function Season1_3TaskItem:destroyRewardItem(item)
...
function Season1_3TaskItem:refreshDesc(data)
	local config = data.config
...
function Season1_3TaskItem:refreshProgress(data)
	self._txtCurCount.text = data.progress
	self._txtMaxCount.text = data.config.maxProgress
end

function Season1_3TaskItem:refreshState(data)
...
function Season1_3TaskItem:onClickGoto()
...
function Season1_3TaskItem:onClickReceive()
	...
	UIBlockMgr.instance:startBlock(Season1_3TaskItem.BlockKey)
	TaskDispatcher.runDelay(self._onPlayActAniFinished, self, 0.76)
end

function Season1_3TaskItem:onClickGetTotal()
	TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Season)
end

function Season1_3TaskItem:_onPlayActAniFinished()
	UIBlockMgr.instance:endBlock(Season1_3TaskItem.BlockKey)
	TaskRpc.instance:sendFinishTaskRequest(self.taskId)
	self:hide()
end

function Season1_3TaskItem:destroy()
