-- chunkname: @modules/logic/season/view1_3/Season1_3CelebrityCardEquip.lua

module("modules.logic.season.view1_3.Season1_3CelebrityCardEquip", package.seeall)

local Season1_3CelebrityCardEquip = class("Season1_3CelebrityCardEquip", LuaCompBase)

function Season1_3CelebrityCardEquip:init(go)
	Season1_3CelebrityCardEquip.super.init(self, go)
...
Season1_3CelebrityCardEquip.MaxRare = 5

function Season1_3CelebrityCardEquip:_editableInitView()
	self._rareGoMap = {}

	for i = 1, Season1_3CelebrityCardEquip.MaxRare do
...
function Season1_3CelebrityCardEquip:onDestroy()
	self:disposeUI()
end

function Season1_3CelebrityCardEquip:checkInitBtnClick()
...
function Season1_3CelebrityCardEquip:checkInitLongPress()
...
function Season1_3CelebrityCardEquip:disposeUI()
...
function Season1_3CelebrityCardEquip:updateData(itemId)
	self.itemId = itemId

	self:refreshUI()
end

function Season1_3CelebrityCardEquip:createRareMap(go)
...
function Season1_3CelebrityCardEquip:refreshUI()
...
function Season1_3CelebrityCardEquip:refreshSelfChoice()
...
function Season1_3CelebrityCardEquip:refreshIcon()
	if self._curSelectedItem then
...
function Season1_3CelebrityCardEquip:refreshFlag()
	if not self.itemId then
		return
	end
...
function Season1_3CelebrityCardEquip:setFlagUIPos(targetPosX, targetPosY)
...
function Season1_3CelebrityCardEquip:setFlagUIScale(targetScale)
...
function Season1_3CelebrityCardEquip:handleIconLoaded()
	if not self._isDisposed then
		local item = self._curSelectedItem
...
function Season1_3CelebrityCardEquip:setColorDark(enable)
	self._darkMaskColor = enable and "#7b7b7b" or "#ffffff"

	self:refreshIcon()
end

function Season1_3CelebrityCardEquip:setShowTag(enable)
	self._showTag = enable

	self:refreshFlag()
end

function Season1_3CelebrityCardEquip:setShowProbability(enable)
	self._showProbability = enable

	self:refreshFlag()
end

function Season1_3CelebrityCardEquip:setShowNewFlag(enable)
	self._showNewFlag = enable

	self:refreshFlag()
end

function Season1_3CelebrityCardEquip:setShowNewFlag2(enable)
	self._showNewFlag2 = enable

	self:refreshFlag()
end

function Season1_3CelebrityCardEquip:setClickCall(clickCall, clickCallObj, param)
...
function Season1_3CelebrityCardEquip:setLongPressCall(longPressCall, longPressCallObj, param)
