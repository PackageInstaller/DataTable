-- chunkname: @modules/logic/season/view1_3/Season1_3SettlementView.lua

module("modules.logic.season.view1_3.Season1_3SettlementView", package.seeall)

local Season1_3SettlementView = class("Season1_3SettlementView", BaseViewExtended)

function Season1_3SettlementView:onInitView()
...
function Season1_3SettlementView:addEvents()
...
function Season1_3SettlementView:removeEvents()
...
function Season1_3SettlementView:_btnclosetipOnClick()
	self:_showAwardTip()
	gohelper.setActive(self._gofightsucc, false)
end

function Season1_3SettlementView:_btncloseawardOnClick()
...
function Season1_3SettlementView:_btnstatOnClick()
	ViewMgr.instance:openView(ViewName.FightStatView)
end

function Season1_3SettlementView:_showAwardTip()
...
function Season1_3SettlementView:_showSettlement()
...
function Season1_3SettlementView:_onRewardShow()
	self:_showSettlementReward()
end

function Season1_3SettlementView:_dailyShowHero()
	self._heroIndex = 0
...
function Season1_3SettlementView:_showHeroItem()
	self._heroIndex = self._heroIndex + 1

	gohelper.setActive(self._heroList[self._heroIndex].viewGO, true)
end

function Season1_3SettlementView:_showGetCardView()
	if self.equip_cards then
		self._showTipsFlow = FlowSequence.New()

		self._showTipsFlow:addWork(Season1_3SettlementTipsWork.New())
...
function Season1_3SettlementView:_onShowTipsFlowDone()
	self.equip_cards = nil
end

function Season1_3SettlementView:_checkNewRecord()
...
function Season1_3SettlementView:_onCoverDungeonRecordReply(isCover)
...
function Season1_3SettlementView:_detectCoverRecord()
...
function Season1_3SettlementView:_onBtnCoverRecordClick()
	DungeonRpc.instance:sendCoverDungeonRecordRequest(true)
end

function Season1_3SettlementView:_editableInitView()
...
function Season1_3SettlementView:_btncloseOnClick()
...
function Season1_3SettlementView:onOpen()
	self._startTime = Time.realtimeSinceStartup
	...
	if self._config then
		self._simagedecorate6:LoadImage(ResUrl.getV1A3SeasonIcon(string.format("icon/ty_chatu_%s.png", self._config.stagePicture)))
	end
	...
	if FightResultModel.instance.firstPass then
		local showProgress = self._episode_config.type == DungeonEnum.EpisodeType.Season
...
function Season1_3SettlementView:_showSettlementReward()
...
function Season1_3SettlementView:_setStages()
...
function Season1_3SettlementView:_onRewardItemShow(obj, data, index)
	local itemIconGO = gohelper.findChild(obj, "go_prop")

	if data.materilType == MaterialEnum.MaterialType.EquipCard then
		local cardItem = Season1_3CelebrityCardItem.New()
...
function Season1_3SettlementView:_onHeroItemLoaded()
	self._heroList = {}
		...
		if not battleCO.playerMax then
			...
			if not is_replay then
				...
				for i = 1, 4 do
					if trailDict[i] then
						...
						if trialCo then
							...
							if equip104_dic[hero_uid] then
								local equip_104 = equip104_dic[hero_uid].equipUid
								local heroItem = self:openSubView(Season1_3SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, nil, equip, equip_104, nil, trailDict[i])
						...
						var_26_3 = hero_uid ~= "0" and equip104_dic[hero_uid] and equip104_dic[hero_uid].equipUid or false
						...
						local heroItem = self:openSubView(Season1_3SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, hero_uid, equip, equip_104)
				...
				if equip104_dic[character_uid] then
					if equip104_dic[character_uid].equipUid then
						if equip104_dic[character_uid].equipUid and equip104_dic[character_uid].equipUid ~= "0" then
							gohelper.setActive(self._gosupercard, true)
							self:openSubView(Season1_3CelebrityCardGetItem, Season1_3CelebrityCardItem.AssetPath, self._gosupercard, equip104_dic[character_uid].equipUid, true)
						end
					end
				end
			else
...
function Season1_3SettlementView:_refreshHeroItemPos()
...
function Season1_3SettlementView:_onGetFightRecordGroupReply(fightGroupMO)
	for i = 1, 4 do
		...
		local heroItem = self:openSubView(Season1_3SettlementHeroItem, obj, nil, true, hero, equip, equip_104, replay_data)
	...
	if fightGroupMO.replay_activity104Equip_data[character_uid] then
		...
		self:openSubView(Season1_3CelebrityCardGetItem, Season1_3CelebrityCardItem.AssetPath, self._gosupercard, nil, nil, equipId)
	end
end

function Season1_3SettlementView:_showAwardTipsData()
...
function Season1_3SettlementView:_showPassData()
...
function Season1_3SettlementView:_releaseAwardImage()
...
function Season1_3SettlementView:_onAwardItemShow(obj, data, index)
	...
	if not itemCfg.rare then
		UISpriteSetMgr.instance:setSeasonSprite(image_bg, "img_pz_" .. itemCfg.rare, true)

		local image_circle = gohelper.findChildImage(obj, "image_circle")

		UISpriteSetMgr.instance:setSeasonSprite(image_circle, "bg_pinjidi_lanse_" .. itemCfg.rare)
...
function Season1_3SettlementView:_onAwardItemClick(data)
	MaterialTipController.instance:showMaterialInfo(data.materilType, data.materilId)
end

function Season1_3SettlementView:onClose()
...
return Season1_3SettlementView
