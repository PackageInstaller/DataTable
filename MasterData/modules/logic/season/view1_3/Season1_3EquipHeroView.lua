-- chunkname: @modules/logic/season/view1_3/Season1_3EquipHeroView.lua

module("modules.logic.season.view1_3.Season1_3EquipHeroView", package.seeall)

local Season1_3EquipHeroView = class("Season1_3EquipHeroView", BaseView)

function Season1_3EquipHeroView:onInitView()
...
function Season1_3EquipHeroView:addEvents()
...
function Season1_3EquipHeroView:onDestroyView()
...
function Season1_3EquipHeroView:onOpen()
	...
	if not self.viewParam.slot then
		if not self.viewParam.group then
			...
			Activity104EquipController.instance:onOpenView(actId, group, pos, self.viewParam.slot)
			self:refreshUI()
		end
	end
end

function Season1_3EquipHeroView:onClose()
	return
end

function Season1_3EquipHeroView:refreshUI()
...
function Season1_3EquipHeroView:handleEquipUpdate()
	self:refreshDescGroup()
	self:refreshSlots()
end

function Season1_3EquipHeroView:refreshSlots()
	for slotIndex = 1, Activity104EquipItemListModel.HeroMaxPos do
		self:refreshSlot(slotIndex)
	end
end

function Season1_3EquipHeroView:refreshSlot(slotIndex)
...
function Season1_3EquipHeroView:refreshDescGroup()
...
function Season1_3EquipHeroView:refreshDesc(descItem, itemUid)
	if itemUid == Activity104EquipItemListModel.EmptyUid then
...
function Season1_3EquipHeroView:refreshProps(itemCfg, descItem, colorStr)
	local processedSet = {}
	local isDirty = false
...
function Season1_3EquipHeroView:refreshSkills(itemCfg, descItem, colorStr)
...
function Season1_3EquipHeroView:getOrCreatePropText(index, descItem)
...
function Season1_3EquipHeroView:getOrCreateSkillText(index, descItem)
...
function Season1_3EquipHeroView:getOrCreateSlot(slotIndex)
...
function Season1_3EquipHeroView:getOrCreateSlotIcon(slotItem)
	...
	if not icon then
		...
		icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season1_3CelebrityCardEquip)
		slotItem.icon = icon
	end
...
function Season1_3EquipHeroView:getOrCreateDesc()
...
function Season1_3EquipHeroView:handleSaveSucc(snapshotId)
...
function Season1_3EquipHeroView:handleSwitchAnimFrame()
	logNormal("refresh by switch anim frame")
	self:refreshSlots()
	self:refreshDescGroup()
end

function Season1_3EquipHeroView:handleEquipCardChanged(param)
	self._animator:Play("switch", 0, 0)
end

function Season1_3EquipHeroView:_btnequipOnClick()
...
function Season1_3EquipHeroView:_btnhandbookOnClick()
	ViewMgr.instance:openView(ViewName.Season1_3EquipBookView, {
		actId = self.viewParam.actId
	})
end

return Season1_3EquipHeroView
