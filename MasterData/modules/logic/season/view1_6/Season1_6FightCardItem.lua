-- chunkname: @modules/logic/season/view1_6/Season1_6FightCardItem.lua

module("modules.logic.season.view1_6.Season1_6FightCardItem", package.seeall)

local Season1_6FightCardItem = class("Season1_6FightCardItem", UserDataDispose)

function Season1_6FightCardItem:ctor(go)
	self:__onInit()
...
Season1_6FightCardItem.MainRoleItemMinHeight = 390
Season1_6FightCardItem.NormalRoleItemMinHeight = 315
Season1_6FightCardItem.RoleCardPos = {
	Vector2(-16.5, 0.5),
	Vector2(-16.5, 0.5),
	Vector2(-16.5, -6.5),
	Vector2(-16.5, -17),
...
function Season1_6FightCardItem:setData(data)
	if not data then
	...
	if self.isMainRole then
		self.layoutElement.minHeight = Season1_6FightCardItem.MainRoleItemMinHeight or Season1_6FightCardItem.NormalRoleItemMinHeight
	end
...
function Season1_6FightCardItem:_setName()
	if self.isMainRole then
...
function Season1_6FightCardItem:_setHead()
...
function Season1_6FightCardItem:_setCard(equipId)
	if not self.cardItem then
		self.cardItem = Season1_6CelebrityCardItem.New()
	...
	local targetCardPos = Season1_6FightCardItem.RoleCardPos[tonumber(rare)] or Vector2(0, 0)

	recthelper.setAnchor(self.goCardPos.transform, targetCardPos.x, targetCardPos.y)
end

function Season1_6FightCardItem:_setDesc(equipId)
...
function Season1_6FightCardItem:createDescItem(index)
	local item = {}
...
function Season1_6FightCardItem:updateDescItem(item, skillStr)
...
function Season1_6FightCardItem:destroyDescItem(item)
	return
end

function Season1_6FightCardItem:destroy()
