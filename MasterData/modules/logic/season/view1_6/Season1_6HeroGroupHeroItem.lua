-- chunkname: @modules/logic/season/view1_6/Season1_6HeroGroupHeroItem.lua

module("modules.logic.season.view1_6.Season1_6HeroGroupHeroItem", package.seeall)

local Season1_6HeroGroupHeroItem = class("Season1_6HeroGroupHeroItem", LuaCompBase)

function Season1_6HeroGroupHeroItem:ctor(Season1_6HeroGroupListView)
	self._Season1_6HeroGroupListView = Season1_6HeroGroupListView
end

function Season1_6HeroGroupHeroItem:init(go)
	self.go = go
	...
	self._simagecharactericon = gohelper.findChild(go, "heroitemani/hero/charactericon")
	...
	self._cardItem1 = Season1_6HeroGroupCardItem.New(self._gocarditem1, self, {
		slot = 1
	})
	self._gocarditem2 = gohelper.findChild(go, "heroitemani/equipcard/cardlist/carditem2")
	self._cardItem2 = Season1_6HeroGroupCardItem.New(self._gocarditem2, self, {
		slot = 2
	})
	self._goherolvLayout = gohelper.findChild(go, "heroitemani/equipcard/vertical/layout")
	self._commonHeroCard = CommonHeroCard.create(self._simagecharactericon, self.__cname)

	self:_initData()
	self:_addEvents()
end

function Season1_6HeroGroupHeroItem:onUpdateMO(mo)
	self.mo = mo
	...
	if self._heroMO then
		if curHeroGroupMo.isReplay then
			if not curHeroGroupMo.replay_hero_data[self.mo.heroUid] then
				local replay_data

				if replay_data then
					local level, rank = HeroConfig.instance:getShowLevel(replay_data.level or self._heroMO.level)

					self._txtlvnum.text = level

					if replay_data then
						local skinConfig = FightConfig.instance:getSkinCO(replay_data.skin or self._heroMO.skin)
	...
	elseif self.trialCO then
		...
		self._commonHeroCard:onUpdateMO(self.trialCO.skin > 0 and SkinConfig.instance:getSkinCo(self.trialCO.skin) or SkinConfig.instance:getSkinCo(heroCo.skinId))
	...
	if not self._heroMO and not self._monsterCo then
		local isHeroShow = true

		if self.trialCO then
			isHeroShow = not self.isLock and not self.isRoleNumLock or false
		end
		...
		local isNoneShow = true
		local isNoneShow

		if self._heroMO or self._monsterCo or self.trialCO then
			isNoneShow = self.isLock or self.isAidLock or self.isRoleNumLock
		end
...
function Season1_6HeroGroupHeroItem:checkSubGoVisable()
	if self.isLock then
		return false
	end
...
function Season1_6HeroGroupHeroItem:_initData()
	self._firstEnter = true

	gohelper.setActive(self._goselectdebuff, false)
end

function Season1_6HeroGroupHeroItem:_addEvents()
	...
	self.addEventCb(self, Activity104EquipController.instance, Activity104EquipEvent.EquipUpdate, self._updateAct104Equips, self)
	self.addEventCb(self, Activity104Controller.instance, Activity104Event.SwitchSnapshotSubId, self._updateAct104Equips, self)
	self.addEventCb(self, HeroGroupController.instance, HeroGroupEvent.setHeroGroupEquipEffect, self.setHeroGroupEquipEffect, self)
	self.addEventCb(self, HeroGroupController.instance, HeroGroupEvent.PlayHeroGroupHeroEffect, self.playHeroGroupHeroEffect, self)
	self.addEventCb(self, HeroGroupController.instance, HeroGroupEvent.ChangeEquip, self._updateSeasonEquips, self)
	self.addEventCb(self, EquipController.instance, EquipEvent.onDeleteEquip, self._updateSeasonEquips, self)
	self.addEventCb(self, EquipController.instance, EquipEvent.onBreakSuccess, self._updateSeasonEquips, self)
	self.addEventCb(self, EquipController.instance, EquipEvent.onEquipStrengthenReply, self._updateSeasonEquips, self)
	self.addEventCb(self, EquipController.instance, EquipEvent.onEquipRefineReply, self._updateSeasonEquips, self)
	self.addEventCb(self, EquipController.instance, EquipEvent.onUpdateEquip, self._updateSeasonEquips, self)
end

function Season1_6HeroGroupHeroItem:_updateSeasonEquips()
	self:_updateEquips()
	self:_updateAct104Equips()
end

function Season1_6HeroGroupHeroItem:_onClickHeroItem()
...
function Season1_6HeroGroupHeroItem:_onClickThisDown()
	self:_setHeroItemPressState(true)
end

function Season1_6HeroGroupHeroItem:_onClickThisUp()
	self:_setHeroItemPressState(false)
end

function Season1_6HeroGroupHeroItem:_onClickEquip()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season1_6HeroGroupHeroItem:_updateEquips()
	self._equipType = -1
	...
	if self._heroMO then
		self.last_hero = self._heroMO.heroId or 0
	end

	self._firstEnter = false
end

Season1_6HeroGroupHeroItem.NonCardSlotBlackMaskHeight = 362
Season1_6HeroGroupHeroItem.HasCardSlotBlackMashHeight = 200
Season1_6HeroGroupHeroItem.SingleSlotCardItem1Pos = Vector2.New(-3.8, 24.9)
Season1_6HeroGroupHeroItem.TwoSlotCardItem1Pos = Vector2.New(-51.6, 28.2)

function Season1_6HeroGroupHeroItem:_updateAct104Equips()
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()
	local equipUid1, equipUid2, equipId1, equipId2
	...
	if curGroupMO.isReplay then
		...
		if self.trialCO and self.trialCO.act104EquipId1 > 0 then
			...
			equipUid1 = equips[1]
			equipId1 = Activity104Model.instance:getItemIdByUid(equips[1])
		end

		if self.trialCO and self.trialCO.act104EquipId2 > 0 then
			equipId2 = self.trialCO.act104EquipId2
		else
			equipUid2 = equips[2]
			equipId2 = Activity104Model.instance:getItemIdByUid(equips[2])
		end
	end

	self._cardItem1:setData(self.mo, equipId1, equipUid1, hasTrialEquip)
	self._cardItem2:setData(self.mo, equipId2, equipUid2, hasTrialEquip)

	self._hasUseSeasonEquipCard = self._cardItem1:hasUseSeasonEquipCard() or self._cardItem2:hasUseSeasonEquipCard()

	local hasShowCardSlot = true

	goto label_13_0

	::label_13_0::

	if hasShowCardSlot then
		if not Season1_6HeroGroupHeroItem.NonCardSlotBlackMaskHeight then
			local targetBlackMaskHeight = Season1_6HeroGroupHeroItem.HasCardSlotBlackMashHeight
			...
			if self._cardItem1.slotUnlock then
				if self._cardItem1.slotUnlock then
					if not Season1_6HeroGroupHeroItem.TwoSlotCardItem1Pos then
						local targetSlot1Pos = Season1_6HeroGroupHeroItem.SingleSlotCardItem1Pos
...
function Season1_6HeroGroupHeroItem:_equipIconAddDrag(go)
	if self._drag then
		return
	end
...
function Season1_6HeroGroupHeroItem:_onBeginDrag(equipTransform, pointerEventData)
...
function Season1_6HeroGroupHeroItem:_onDrag(param, pointerEventData)
	if UnityEngine.Input.touchCount > 1 then
		return
	end
...
function Season1_6HeroGroupHeroItem:_onEndDrag(equipTransform, pointerEventData)
	if UnityEngine.Input.touchCount > 1 then
		return
	end
	...
	if targetHeroItem then
		::label_17_0::

		local var_17_0 = targetHeroItem.trialCO

		if targetHeroItem.trialCO then
			local isTrialDefaultEquip = targetHeroItem.trialCO.equipId > 0
			...
			if not targetHeroItem or targetHeroItem == self or targetHeroItem.mo.aid or isTrialDefaultEquip then
				...
				self._setToPos(self, dragTransform, anchorPos, true, self.onDragEndFail, self)
			...
			self._setToPos(self, dragTransform, anchorPos, true, self.onDragEndSuccess, self, targetHeroItem)
		end
	end
end

function Season1_6HeroGroupHeroItem:topDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self.currentParent, dragTransform.gameObject)
end

function Season1_6HeroGroupHeroItem:resetDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self._equip.moveParent, dragTransform.gameObject)
end

function Season1_6HeroGroupHeroItem:getDragTransform()
	return self._equip.moveContainerTrs
end

function Season1_6HeroGroupHeroItem:getOrignDragPos()
	return self._equip.moveParent.transform.position
end

function Season1_6HeroGroupHeroItem:onDragEndFail()
...
function Season1_6HeroGroupHeroItem:onDragEndSuccess(targetHeroItem)
...
function Season1_6HeroGroupHeroItem:_playDragEndAudio(targetHeroItem)
	...
	if hasSkill then
		local showEffect = EquipHelper.detectEquipSkillSuited(targetHeroItem._heroMO and targetHeroItem._heroMO.heroId, self._equipMO.config.skillType, self._equipMO.refineLv)
...
function Season1_6HeroGroupHeroItem:_tweenToPos(transform, anchorPos)
...
function Season1_6HeroGroupHeroItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)
...
function Season1_6HeroGroupHeroItem:_moveToTarget(position)
	for i, v in ipairs(self._Season1_6HeroGroupListView.heroPosTrList) do
		if self._Season1_6HeroGroupListView._heroItemList[i] ~= self then
			...
			if math.abs(anchorPos.x) * 2 < recthelper.getWidth(posTr) and math.abs(anchorPos.y) * 2 < recthelper.getHeight(posTr) then
				return self._Season1_6HeroGroupListView._heroItemList[i]
			end
		end
	end
...
function Season1_6HeroGroupHeroItem:_setEquipDragEnabled(isEnabled)
	for i, heroItem in ipairs(self._Season1_6HeroGroupListView._heroItemList) do
...
function Season1_6HeroGroupHeroItem:playHeroGroupHeroEffect(state)
...
function Season1_6HeroGroupHeroItem:playRestrictAnimation(needRestrictHeroUidDict)
...
function Season1_6HeroGroupHeroItem:setGrayFactor(value)
...
function Season1_6HeroGroupHeroItem:showCounterSign()
	local career
	...
	if isRecommended or isCounter then
		if self._hasUseSeasonEquipCard then
			if not restrainTagPosUseSeasonCard then
				local targetPos = restrainTagPosUnUseSeasonCard
...
function Season1_6HeroGroupHeroItem:onItemBeginDrag(index)
...
function Season1_6HeroGroupHeroItem:onItemEndDrag(index, dragToIndex)
	ZProj.TweenHelper.DOScale(self.go.transform, 1, 1, 1, 0.2, nil, nil, nil, EaseType.Linear)
	self:_setHeroItemPressState(false)
end

function Season1_6HeroGroupHeroItem:onItemCompleteDrag(index, dragToIndex, complete)
...
function Season1_6HeroGroupHeroItem:hideDragEffect()
	gohelper.setActive(self._goselectdebuff, false)
end

function Season1_6HeroGroupHeroItem:setHeroGroupEquipEffect(show)
	self._canPlayEffect = show
end

function Season1_6HeroGroupHeroItem:setParent(transform)
	self.currentParent = transform

	gohelper.addChildPosStay(transform.gameObject, self._subGO)
end

function Season1_6HeroGroupHeroItem:flowOriginParent()
	return
end

function Season1_6HeroGroupHeroItem:flowCurrentParent()
	return
end

function Season1_6HeroGroupHeroItem:getHeroItemList()
	return self._Season1_6HeroGroupListView:getHeroItemList()
end

function Season1_6HeroGroupHeroItem:killEquipTweenId()
...
function Season1_6HeroGroupHeroItem:_removeEvents()
...
function Season1_6HeroGroupHeroItem:_setHeroItemPressState(press)
	...
	self._heroItemContainer.spines = self._heroGO:GetComponentsInChildren(GuiSpine.TypeSkeletonGraphic, true)
	...
	if self._imageAdd then
		if press then
			if not HeroGroupHeroItem.PressColor then
				self._imageAdd.color = Color.white
			end
		end
	end
end

function Season1_6HeroGroupHeroItem:_setUIPressState(graphicCompArr, isPress, oriColorMap)
...
function Season1_6HeroGroupHeroItem:onDestroy()
	self:_removeEvents()
	self:killEquipTweenId()
...
return Season1_6HeroGroupHeroItem
