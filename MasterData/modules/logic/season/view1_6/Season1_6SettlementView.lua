-- chunkname: @modules/logic/season/view1_6/Season1_6SettlementView.lua

module("modules.logic.season.view1_6.Season1_6SettlementView", package.seeall)

local Season1_6SettlementView = class("Season1_6SettlementView", BaseViewExtended)

function Season1_6SettlementView:onInitView()
...
function Season1_6SettlementView:addEvents()
...
function Season1_6SettlementView:removeEvents()
...
function Season1_6SettlementView:_btnclosetipOnClick()
	self:_showAwardTip()
	gohelper.setActive(self._gofightsucc, false)
end

function Season1_6SettlementView:_btncloseawardOnClick()
...
function Season1_6SettlementView:_btnstatOnClick()
	ViewMgr.instance:openView(ViewName.FightStatView)
end

function Season1_6SettlementView:_showAwardTip()
...
function Season1_6SettlementView:_showSettlement()
...
function Season1_6SettlementView:_onRewardShow()
	self:_showSettlementReward()
end

function Season1_6SettlementView:_dailyShowHero()
	self._heroIndex = 0
...
function Season1_6SettlementView:_showHeroItem()
	self._heroIndex = self._heroIndex + 1

	gohelper.setActive(self._heroList[self._heroIndex].viewGO, true)
end

function Season1_6SettlementView:_showGetCardView()
	if self.equip_cards then
		self._showTipsFlow = FlowSequence.New()

		self._showTipsFlow:addWork(Season1_6SettlementTipsWork.New())
...
function Season1_6SettlementView:_onShowTipsFlowDone()
	self.equip_cards = nil
end

function Season1_6SettlementView:_checkNewRecord()
...
function Season1_6SettlementView:_onCoverDungeonRecordReply(isCover)
...
function Season1_6SettlementView:_detectCoverRecord()
...
function Season1_6SettlementView:_onBtnCoverRecordClick()
	DungeonRpc.instance:sendCoverDungeonRecordRequest(true)
end

function Season1_6SettlementView:_editableInitView()
...
function Season1_6SettlementView:_btncloseOnClick()
...
function Season1_6SettlementView:onOpen()
	self._startTime = Time.realtimeSinceStartup
...
function Season1_6SettlementView:_showSettlementReward()
...
function Season1_6SettlementView:_setStages()
...
function Season1_6SettlementView:_onRewardItemShow(obj, data, index)
	local itemIconGO = gohelper.findChild(obj, "go_prop")

	if data.materilType == MaterialEnum.MaterialType.EquipCard then
		local cardItem = Season1_6CelebrityCardItem.New()
...
function Season1_6SettlementView:_onHeroItemLoaded()
	self._heroList = {}
					...
					if trailDict[i] then
						...
						if trialCo then
							...
							if equip104_dic[hero_uid] then
								...
								local heroItem = self:openSubView(Season1_6SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, nil, equip, equip_104, nil, trailDict[i])
						...
						local heroItem = self:openSubView(Season1_6SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, hero_uid, equip, equip_104)
				...
				if equip104_dic[character_uid] then
					if equip104_dic[character_uid].equipUid then
						if equip104_dic[character_uid].equipUid and equip104_dic[character_uid].equipUid ~= "0" then
							...
							self:openSubView(Season1_6CelebrityCardGetItem, Season1_6CelebrityCardItem.AssetPath, self._gosupercard, equip104_dic[character_uid].equipUid, true)
						end
					end
				end
			else
...
function Season1_6SettlementView:_refreshHeroItemPos()
...
function Season1_6SettlementView:_onGetFightRecordGroupReply(fightGroupMO)
	for i = 1, 4 do
		...
		local heroItem = self:openSubView(Season1_6SettlementHeroItem, obj, nil, true, hero, equip, equip_104, replay_data)
	...
	if fightGroupMO.replay_activity104Equip_data[character_uid] then
		...
		self:openSubView(Season1_6CelebrityCardGetItem, Season1_6CelebrityCardItem.AssetPath, self._gosupercard, nil, nil, equipId)
	end
end

function Season1_6SettlementView:_showAwardTipsData()
...
function Season1_6SettlementView:_showPassData()
...
function Season1_6SettlementView:_releaseAwardImage()
...
function Season1_6SettlementView:_onAwardItemShow(obj, data, index)
...
function Season1_6SettlementView:_onAwardItemClick(data)
	MaterialTipController.instance:showMaterialInfo(data.materilType, data.materilId)
end

function Season1_6SettlementView:onClose()
...
return Season1_6SettlementView
