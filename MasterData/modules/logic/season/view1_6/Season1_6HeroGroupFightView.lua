-- chunkname: @modules/logic/season/view1_6/Season1_6HeroGroupFightView.lua

module("modules.logic.season.view1_6.Season1_6HeroGroupFightView", package.seeall)

local Season1_6HeroGroupFightView = class("Season1_6HeroGroupFightView", BaseView)

function Season1_6HeroGroupFightView:onInitView()
	...
	self._simagereplayframe = gohelper.findChildSingleImage(self.viewGO, "#go_container/#go_replayready/frame3")
...
function Season1_6HeroGroupFightView:addEvents()
...
function Season1_6HeroGroupFightView:removeEvents()
...
function Season1_6HeroGroupFightView:_setTrialNumTips()
	local _, battleCO = Season1_6HeroGroupFightView._getEpisodeConfigAndBattleConfig()
	local tips = {}

	if battleCO and battleCO.trialLimit > 0 then
		tips[1] = battleCO.trialLimit >= 4 and luaLang("herogroup_trial_tip") or formatLuaLang("herogroup_trial_limit_tip", battleCO.trialLimit)
	end
...
function Season1_6HeroGroupFightView:_onTouch()
...
function Season1_6HeroGroupFightView:_setTrialTipsTxt(obj, data, index)
	gohelper.findChildTextMesh(obj, "desc").text = data
end

function Season1_6HeroGroupFightView:_switchTrialTips()
...
function Season1_6HeroGroupFightView:_btnrecommendOnClick()
...
function Season1_6HeroGroupFightView:_btnclothOnClock()
...
function Season1_6HeroGroupFightView:_btnrestrainOnClick()
	ViewMgr.instance:openView(ViewName.HeroGroupCareerTipView)
end

function Season1_6HeroGroupFightView:_btnstartseasonOnClick()
	self:_onClickStart()
end

function Season1_6HeroGroupFightView:_btnseasonreplayOnClick()
	self:_onClickReplay()
end

function Season1_6HeroGroupFightView:_btnseasonsupercardOnClick()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season1_6HeroGroupFightView:_groupDropValueChanged(value)
...
function Season1_6HeroGroupFightView:_editableInitView()
	self._simagerole:LoadImage(ResUrl.getSeasonIcon("img_vertin.png"))
	self._simagereplayframe:LoadImage(SeasonViewHelper.getSeasonIcon("replay_frame.png"))
	self:_initComponent()
	self:_initData()
	self:_addEvents()
end

function Season1_6HeroGroupFightView:onOpen()
...
function Season1_6HeroGroupFightView:_initComponent()
...
function Season1_6HeroGroupFightView:_initData()
	...
	NavigateMgr.instance:addEscape(ViewName.Season1_6HeroGroupFightView, self._onEscapeBtnClick, self)
...
function Season1_6HeroGroupFightView:_addEvents()
	...
	self.addEventCb(self, FightController.instance, FightEvent.RespBeginFightFail, self._respBeginFightFail, self)
	self.addEventCb(self, HeroGroupController.instance, HeroGroupEvent.PlayHeroGroupExitEffect, self._playHeroGroupExitEffect, self)
	self.addEventCb(self, HeroGroupController.instance, HeroGroupEvent.PlayCloseHeroGroupAnimation, self._playCloseHeroGroupAnimation, self)
	...
	self.addEventCb(self, Activity104Controller.instance, Activity104Event.SwitchSnapshotSubId, self._switchSnapshotSubId, self)
end

function Season1_6HeroGroupFightView:_checkReplay()
...
function Season1_6HeroGroupFightView:_receiveRecommend(cmd, resultCode, msg)
...
function Season1_6HeroGroupFightView:_switchSnapshotSubId()
	self:_refreshMainCard()
end

function Season1_6HeroGroupFightView:_refreshMainCard()
	...
	local unlock = Activity104Model.instance:isSeasonPosUnlock(nil, nil, mainRoleSlot, mainRolePos)
	...
	if unlock then
		...
		if curGroupMO then
			if curGroupMO.isReplay then
				if curGroupMO.replay_activity104Equip_data then
				...
				if battleCO and battleCO.trialMainAct104EuqipId > 0 then
					act104EquipId = battleCO.trialMainAct104EuqipId
				elseif curGroupMO.activity104Equips and curGroupMO.activity104Equips[mainRolePos] then
		...
		if act104EquipId ~= 0 then
			if not self._superCardItem then
				self._superCardItem = Season1_6CelebrityCardItem.New()
...
function Season1_6HeroGroupFightView:_checkDropArrow()
...
function Season1_6HeroGroupFightView:_onClickReplay()
...
function Season1_6HeroGroupFightView:_switchReplayGroup(lastRelayMode)
...
function Season1_6HeroGroupFightView:_haveRecord()
	if not OpenModel.instance:isFunctionUnlock(OpenEnum.UnlockFunc.FightReplay) then
		return false
	end
	...
	return userDungeonMO and userDungeonMO.hasRecord
end

function Season1_6HeroGroupFightView:_updateReplayHeroGroupList()
...
function Season1_6HeroGroupFightView:_switchReplayMul()
	self._txtreplaycount.text = self._replayMode and luaLang("multiple") .. self._multiplication or ""
end

function Season1_6HeroGroupFightView:_heroMoveForward(heroId)
	HeroGroupEditListModel.instance:setMoveHeroId(tonumber(heroId))
end

function Season1_6HeroGroupFightView:isReplayMode()
	return self._replayMode
end

function Season1_6HeroGroupFightView:_onCurrencyChange(changeIds)
...
function Season1_6HeroGroupFightView:onClose()
	AudioMgr.instance:trigger(AudioEnum.UI.Stop_HeroNormalVoc)
	self:_removeEvents()
...
function Season1_6HeroGroupFightView:_removeEvents()
...
function Season1_6HeroGroupFightView:_refreshReplay()
...
function Season1_6HeroGroupFightView:_refreshCloth()
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()
	...
	gohelper.setActive(self._btncloth.gameObject, Season1_6HeroGroupFightView.showCloth())
end

function Season1_6HeroGroupFightView:_checkEquipClothSkill()
...
function Season1_6HeroGroupFightView._getEpisodeConfigAndBattleConfig()
...
function Season1_6HeroGroupFightView.showCloth()
	...
	local episodeCO, battleCO = Season1_6HeroGroupFightView._getEpisodeConfigAndBattleConfig()
...
function Season1_6HeroGroupFightView:_onModifyHeroGroup()
	self:_refreshCloth()
	self:_refreshMainCard()
end

function Season1_6HeroGroupFightView:_onModifySnapshot()
	self:_refreshCloth()
	self:_refreshMainCard()
end

function Season1_6HeroGroupFightView:_onClickHeroGroupItem(id)
	TaskDispatcher.cancelTask(self.openHeroGroupEditView, self)
...
function Season1_6HeroGroupFightView:openHeroGroupEditView()
	ViewMgr.instance:openView(ViewName.HeroGroupEditView, self._param)
end

function Season1_6HeroGroupFightView:_checkFirstPosHasEquip()
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()
...
function Season1_6HeroGroupFightView:_showGuideDragEffect(param)
	if self._dragEffectLoader then
		self._dragEffectLoader:dispose()
...
function Season1_6HeroGroupFightView:_onClickStart()
...
function Season1_6HeroGroupFightView:_enterFight()
	if self._blockStart then
		return
	end
		...
		if result then
			...
			if self._replayMode then
				...
				local param = {
					...
					episodeId = fightParam.episodeId,
				...
				Activity104Rpc.instance:sendStartAct104BattleRequest(param, actId, layer, fightParam.episodeId)
			else
...
function Season1_6HeroGroupFightView:_onUseRecommendGroup()
	if self._replayMode then
		self._replayMode = false
		self._multiplication = 1
...
function Season1_6HeroGroupFightView:_refreshBtns()
	local showDrop = not self._replayMode and self:_noAidHero()

	gohelper.setActive(self._dropseasonherogroup.gameObject, showDrop)
end

function Season1_6HeroGroupFightView:_onGetFightRecordGroupReply(fightGroupMO)
	self.removeEventCb(self, FightController.instance, FightEvent.RespGetFightRecordGroupReply, self._onGetFightRecordGroupReply, self)
...
function Season1_6HeroGroupFightView:_updateReplayHeroGroup()
	HeroGroupModel.instance:setReplayParam(self._replayFightGroupMO)
	HeroGroupController.instance:dispatchEvent(HeroGroupEvent.SelectHeroGroup, self._replayFightGroupMO.id)
	self:_refreshCloth()
	self:_refreshMainCard()
...
function Season1_6HeroGroupFightView:_playHeroGroupExitEffect()
	self._anim:Play("close", 0, 0)
	self._btnContainAnim:Play("close", 0, 0)
end

function Season1_6HeroGroupFightView:_playCloseHeroGroupAnimation()
	self._anim:Play("close", 0, 0)
	self._btnContainAnim:Play("close", 0, 0)
...
function Season1_6HeroGroupFightView:_closeHeroContainAnim()
	if self._heroContainAnim then
		self._heroContainAnim.enabled = false
	end
end

function Season1_6HeroGroupFightView:onDestroyView()
...
function Season1_6HeroGroupFightView:_noAidHero()
...
return Season1_6HeroGroupFightView
