-- chunkname: @modules/logic/season/view1_6/Season1_6HeroGroupCardItem.lua

module("modules.logic.season.view1_6.Season1_6HeroGroupCardItem", package.seeall)

local Season1_6HeroGroupCardItem = class("Season1_6HeroGroupCardItem", UserDataDispose)

Season1_6HeroGroupCardItem.TweenDuration = 0.16
Season1_6HeroGroupCardItem.DragOffset = Vector2(0, 40)
Season1_6HeroGroupCardItem.ZeroPos = Vector2(-2.7, -5)
Season1_6HeroGroupCardItem.ZeroScale = 0.39

function Season1_6HeroGroupCardItem:ctor(go, parent, param)
	self:__onInit()
...
function Season1_6HeroGroupCardItem:init()
...
function Season1_6HeroGroupCardItem:AddDrag(go)
	if self._drag then
		return
	end
...
function Season1_6HeroGroupCardItem:setData(data, equipId, equipUid, hasTrialEquip)
...
function Season1_6HeroGroupCardItem:setActive(isActive)
	self.isActive = isActive
end

function Season1_6HeroGroupCardItem:updateView()
	...
	if self.posUnlock then
		...
		if equipId ~= 0 then
			if not self._seasonCardItem then
				self._seasonCardItem = Season1_6CelebrityCardItem.New()
...
function Season1_6HeroGroupCardItem:playEmptyUnlockAnim()
	local actId = Activity104Model.instance:getCurSeasonId()
	local pos = self.id - 1
...
function Season1_6HeroGroupCardItem:getEquipId(actId, groupId)
...
function Season1_6HeroGroupCardItem:hasUseSeasonEquipCard()
	return self._hasUseSeasonEquipCard
end

function Season1_6HeroGroupCardItem:_btnCardClick()
	if self.inDrag then
		return
	end
...
function Season1_6HeroGroupCardItem:canDrag()
...
function Season1_6HeroGroupCardItem:_onBeginDrag(dragTransform, pointerEventData)
	...
	self:_tweenToPos(self._trscard, anchorPos + Season1_6HeroGroupCardItem.DragOffset, true)

	local scale = Season1_6HeroGroupCardItem.ZeroScale * 1.7

	self.tweenId = ZProj.TweenHelper.DOScale(self._trscard, scale, scale, scale, Season1_6HeroGroupCardItem.TweenDuration)
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, 0, Season1_6HeroGroupCardItem.TweenDuration)
	self.inDrag = true
end

function Season1_6HeroGroupCardItem:_onDrag(dragTransform, pointerEventData)
	if not self:canDrag() then
		self.inDrag = false
	...
	self:_tweenToPos(self._trscard, anchorPos + Season1_6HeroGroupCardItem.DragOffset)

	self.inDrag = true
end

function Season1_6HeroGroupCardItem:_onEndDrag(dragTransform, pointerEventData)
	self.inDrag = false
	...
	local scale = Season1_6HeroGroupCardItem.ZeroScale * 1
	...
	if not targetItem or not targetItem:canExchange(self) then
		self._setToPos(self, self._trscard, Season1_6HeroGroupCardItem.ZeroPos, true, self._onDragFailTweenEnd, self)

		self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, self.orignRoteZ, Season1_6HeroGroupCardItem.TweenDuration)
	...
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, targetItem.orignRoteZ, Season1_6HeroGroupCardItem.TweenDuration)
end

function Season1_6HeroGroupCardItem:_tweenToPos(transform, anchorPos)
	...
	if math.abs(curAnchorX - anchorPos.x) > 10 or math.abs(curAnchorY - anchorPos.y) > 10 then
		self.posTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season1_6HeroGroupCardItem.TweenDuration)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
	end
end

function Season1_6HeroGroupCardItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)

	if tween then
		self.moveTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season1_6HeroGroupCardItem.TweenDuration, callback, callbackObj, param)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
...
function Season1_6HeroGroupCardItem:_moveToTarget(position)
...
function Season1_6HeroGroupCardItem:_setDragEnabled(isEnabled)
	local heroList = self.parent:getHeroItemList()
...
function Season1_6HeroGroupCardItem:setDragEnabled(isEnabled)
	if self._drag then
		self._drag.enabled = isEnabled
	end
end

function Season1_6HeroGroupCardItem:canExchange(targetItem)
	...
	for i = 1, maxCount do
		if i ~= srcSlot then
			...
			if tarCO and co and co.group == tarCO.group and tarPos ~= srcPos then
				GameFacade.showToast(Season1_6EquipItem.Toast_Same_Card)
...
function Season1_6HeroGroupCardItem:_onDragFailTweenEnd()
	self:_setDragEnabled(true)
	gohelper.setAsLastSibling(self.parent._cardItem1.go)
end

function Season1_6HeroGroupCardItem:_onDragSuccessTweenEnd(targetItem)
	self:killTweenId()
	self:_setToPos(self._trscard, Season1_6HeroGroupCardItem.ZeroPos)
	self:_setToPos(targetItem._trscard, Season1_6HeroGroupCardItem.ZeroPos)
...
function Season1_6HeroGroupCardItem:killTweenId()
...
function Season1_6HeroGroupCardItem:destory()
	if self._drag then
...
return Season1_6HeroGroupCardItem
