-- chunkname: @modules/logic/season/view1_4/Season1_4FightCardItem.lua

module("modules.logic.season.view1_4.Season1_4FightCardItem", package.seeall)

local Season1_4FightCardItem = class("Season1_4FightCardItem", UserDataDispose)

function Season1_4FightCardItem:ctor(go)
	self:__onInit()
...
Season1_4FightCardItem.MainRoleItemMinHeight = 390
Season1_4FightCardItem.NormalRoleItemMinHeight = 315
Season1_4FightCardItem.RoleCardPos = {
	Vector2(-16.5, 0.5),
	Vector2(-16.5, 0.5),
	Vector2(-16.5, -6.5),
	Vector2(-16.5, -17),
...
function Season1_4FightCardItem:setData(data)
	if not data then
	...
	if self.isMainRole then
		self.layoutElement.minHeight = Season1_4FightCardItem.MainRoleItemMinHeight or Season1_4FightCardItem.NormalRoleItemMinHeight
	end
...
function Season1_4FightCardItem:_setName()
	if self.isMainRole then
...
function Season1_4FightCardItem:_setHead()
...
function Season1_4FightCardItem:_setCard(equipId)
	if not self.cardItem then
		self.cardItem = Season1_4CelebrityCardItem.New()
	...
	local targetCardPos = Season1_4FightCardItem.RoleCardPos[tonumber(rare)] or Vector2(0, 0)

	recthelper.setAnchor(self.goCardPos.transform, targetCardPos.x, targetCardPos.y)
end

function Season1_4FightCardItem:_setDesc(equipId)
...
function Season1_4FightCardItem:createDescItem(index)
	local item = {}
...
function Season1_4FightCardItem:updateDescItem(item, skillStr)
...
function Season1_4FightCardItem:destroyDescItem(item)
	return
end

function Season1_4FightCardItem:destroy()
