-- chunkname: @modules/logic/season/view1_4/Season1_4HeroGroupHeroItem.lua

module("modules.logic.season.view1_4.Season1_4HeroGroupHeroItem", package.seeall)

local Season1_4HeroGroupHeroItem = class("Season1_4HeroGroupHeroItem", LuaCompBase)

function Season1_4HeroGroupHeroItem:ctor(Season1_4HeroGroupListView)
	self._Season1_4HeroGroupListView = Season1_4HeroGroupListView
end

function Season1_4HeroGroupHeroItem:init(go)
	self.go = go
	...
	self._cardItem1 = Season1_4HeroGroupCardItem.New(self._gocarditem1, self, {
		slot = 1
	})
	self._gocarditem2 = gohelper.findChild(go, "heroitemani/equipcard/cardlist/carditem2")
	self._cardItem2 = Season1_4HeroGroupCardItem.New(self._gocarditem2, self, {
		slot = 2
	})
...
function Season1_4HeroGroupHeroItem:onUpdateMO(mo)
	self.mo = mo
	...
	elseif self.trialCO then
		...
		self._simagecharactericon:LoadImage(ResUrl.getHeadIconMiddle((self.trialCO.skin > 0 and SkinConfig.instance:getSkinCo(self.trialCO.skin) or SkinConfig.instance:getSkinCo(heroCo.skinId)).retangleIcon))
...
function Season1_4HeroGroupHeroItem:checkSubGoVisable()
	if self.isLock then
		return false
	end
...
function Season1_4HeroGroupHeroItem:_initData()
	self._firstEnter = true

	gohelper.setActive(self._goselectdebuff, false)
end

function Season1_4HeroGroupHeroItem:_addEvents()
...
function Season1_4HeroGroupHeroItem:_updateSeasonEquips()
	self:_updateEquips()
	self:_updateAct104Equips()
end

function Season1_4HeroGroupHeroItem:_onClickHeroItem()
...
function Season1_4HeroGroupHeroItem:_onClickThisDown()
	self:_setHeroItemPressState(true)
end

function Season1_4HeroGroupHeroItem:_onClickThisUp()
	self:_setHeroItemPressState(false)
end

function Season1_4HeroGroupHeroItem:_onClickEquip()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season1_4HeroGroupHeroItem:_updateEquips()
	self._equipType = -1
...
Season1_4HeroGroupHeroItem.NonCardSlotBlackMaskHeight = 362
Season1_4HeroGroupHeroItem.HasCardSlotBlackMashHeight = 200
Season1_4HeroGroupHeroItem.SingleSlotCardItem1Pos = Vector2.New(-3.8, 24.9)
Season1_4HeroGroupHeroItem.TwoSlotCardItem1Pos = Vector2.New(-51.6, 28.2)

function Season1_4HeroGroupHeroItem:_updateAct104Equips()
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()
	...
	if hasShowCardSlot then
		if not Season1_4HeroGroupHeroItem.NonCardSlotBlackMaskHeight then
			local targetBlackMaskHeight = Season1_4HeroGroupHeroItem.HasCardSlotBlackMashHeight
			...
			if self._cardItem1.slotUnlock then
				if self._cardItem1.slotUnlock then
					if not Season1_4HeroGroupHeroItem.TwoSlotCardItem1Pos then
						local targetSlot1Pos = Season1_4HeroGroupHeroItem.SingleSlotCardItem1Pos
...
function Season1_4HeroGroupHeroItem:_equipIconAddDrag(go)
	if self._drag then
		return
	end
...
function Season1_4HeroGroupHeroItem:_onBeginDrag(equipTransform, pointerEventData)
...
function Season1_4HeroGroupHeroItem:_onDrag(param, pointerEventData)
	if UnityEngine.Input.touchCount > 1 then
		return
	end
...
function Season1_4HeroGroupHeroItem:_onEndDrag(equipTransform, pointerEventData)
	if UnityEngine.Input.touchCount > 1 then
		return
	end
...
function Season1_4HeroGroupHeroItem:topDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self.currentParent, dragTransform.gameObject)
end

function Season1_4HeroGroupHeroItem:resetDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self._equip.moveParent, dragTransform.gameObject)
end

function Season1_4HeroGroupHeroItem:getDragTransform()
	return self._equip.moveContainerTrs
end

function Season1_4HeroGroupHeroItem:getOrignDragPos()
	return self._equip.moveParent.transform.position
end

function Season1_4HeroGroupHeroItem:onDragEndFail()
...
function Season1_4HeroGroupHeroItem:onDragEndSuccess(targetHeroItem)
...
function Season1_4HeroGroupHeroItem:_playDragEndAudio(targetHeroItem)
...
function Season1_4HeroGroupHeroItem:_tweenToPos(transform, anchorPos)
...
function Season1_4HeroGroupHeroItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)
...
function Season1_4HeroGroupHeroItem:_moveToTarget(position)
	for i, v in ipairs(self._Season1_4HeroGroupListView.heroPosTrList) do
		if self._Season1_4HeroGroupListView._heroItemList[i] ~= self then
			...
			if math.abs(anchorPos.x) * 2 < recthelper.getWidth(posTr) and math.abs(anchorPos.y) * 2 < recthelper.getHeight(posTr) then
				return self._Season1_4HeroGroupListView._heroItemList[i]
			end
		end
	end
...
function Season1_4HeroGroupHeroItem:_setEquipDragEnabled(isEnabled)
	for i, heroItem in ipairs(self._Season1_4HeroGroupListView._heroItemList) do
...
function Season1_4HeroGroupHeroItem:playHeroGroupHeroEffect(state)
...
function Season1_4HeroGroupHeroItem:playRestrictAnimation(needRestrictHeroUidDict)
...
function Season1_4HeroGroupHeroItem:setGrayFactor(value)
...
function Season1_4HeroGroupHeroItem:showCounterSign()
	local career
...
function Season1_4HeroGroupHeroItem:onItemBeginDrag(index)
...
function Season1_4HeroGroupHeroItem:onItemEndDrag(index, dragToIndex)
	ZProj.TweenHelper.DOScale(self.go.transform, 1, 1, 1, 0.2, nil, nil, nil, EaseType.Linear)
	self:_setHeroItemPressState(false)
end

function Season1_4HeroGroupHeroItem:onItemCompleteDrag(index, dragToIndex, complete)
...
function Season1_4HeroGroupHeroItem:hideDragEffect()
	gohelper.setActive(self._goselectdebuff, false)
end

function Season1_4HeroGroupHeroItem:setHeroGroupEquipEffect(show)
	self._canPlayEffect = show
end

function Season1_4HeroGroupHeroItem:setParent(transform)
	self.currentParent = transform

	gohelper.addChildPosStay(transform.gameObject, self._subGO)
end

function Season1_4HeroGroupHeroItem:flowOriginParent()
	return
end

function Season1_4HeroGroupHeroItem:flowCurrentParent()
	return
end

function Season1_4HeroGroupHeroItem:getHeroItemList()
	return self._Season1_4HeroGroupListView:getHeroItemList()
end

function Season1_4HeroGroupHeroItem:killEquipTweenId()
...
function Season1_4HeroGroupHeroItem:_removeEvents()
...
function Season1_4HeroGroupHeroItem:_setHeroItemPressState(press)
...
function Season1_4HeroGroupHeroItem:_setUIPressState(graphicCompArr, isPress, oriColorMap)
...
function Season1_4HeroGroupHeroItem:onDestroy()
	self:_removeEvents()
	self:killEquipTweenId()
...
return Season1_4HeroGroupHeroItem
