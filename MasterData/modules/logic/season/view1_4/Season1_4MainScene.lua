-- chunkname: @modules/logic/season/view1_4/Season1_4MainScene.lua

module("modules.logic.season.view1_4.Season1_4MainScene", package.seeall)

local Season1_4MainScene = class("Season1_4MainScene", BaseView)

function Season1_4MainScene:onInitView()
	...
	self._listenerViews = {
		[ViewName.Season1_4MarketView] = 1,
		[ViewName.Season1_4SpecialMarketView] = 1
	}
	self.isFirst = true
...
function Season1_4MainScene:addEvents()
	return
end

function Season1_4MainScene:removeEvents()
	return
end

function Season1_4MainScene:_editableInitView()
...
function Season1_4MainScene:_addEvents()
...
function Season1_4MainScene:_removeEvents()
...
function Season1_4MainScene:initCamera(isRetail)
...
function Season1_4MainScene:resetCamera()
	self:doAudio(false)
end

function Season1_4MainScene:_onChangeCamera(isRetail)
	self:initCamera(isRetail)
end

function Season1_4MainScene:_showScenes(isRetail)
...
function Season1_4MainScene:showSceneAnim()
	local animationName, normalizedTime
...
function Season1_4MainScene:_enterRetail()
	self:_refreshRoles()
end

function Season1_4MainScene:_onRefreshRetail()
	self:_refreshRoles(true)
end

function Season1_4MainScene:_refreshRoles(isOpen)
...
function Season1_4MainScene:_battleSuccessRefreshRoles(retails)
	for i = 1, 6 do
...
function Season1_4MainScene:_onDelayRefreshRoles()
...
function Season1_4MainScene:_normalRefreshRoles(retails, noretail, isOpen)
	local t = {}
...
function Season1_4MainScene:_enterMarket()
	gohelper.setActive(self._gospotlight, false)
	self:_refreshRoles()
end

function Season1_4MainScene:_initSceneRootNode()
	local mainTrans = self._camera.transform.parent
	local sceneRoot = CameraMgr.instance:getSceneRoot()

	self._sceneRoot = UnityEngine.GameObject.New("Season1_4MainScene")
...
function Season1_4MainScene:_loadScene()
...
function Season1_4MainScene:_loadSceneFinish()
	...
	MainCameraMgr.instance:addView(ViewName.Season1_4MainView, self.autoInitMainViewCamera, self.resetCamera, self)
end

function Season1_4MainScene:autoInitMainViewCamera()
	self:initCamera(false)
end

function Season1_4MainScene:_initLevelupObjs()
	self._levelupObjs = {}
...
function Season1_4MainScene:showLevelObjs(stage)
	...
	local container = ViewMgr.instance:getContainer(ViewName.Season1_4MainView)
...
function Season1_4MainScene:_hideLevelObjs(isDefault)
...
function Season1_4MainScene:_showStage()
...
function Season1_4MainScene:_getStageSetting(stageId)
	return self._stageSetting[stageId] or self._stageSetting[6]
end

function Season1_4MainScene:_setGoSectionActive(index, active)
	gohelper.setActive(self:_getGoSection(index), active)
end

function Season1_4MainScene:_getGoSection(index)
	return self["_gosection" .. index]
end

function Season1_4MainScene:onUpdateParam()
	self:_refreshView()
end

function Season1_4MainScene:onOpen()
	self:_loadScene()
end

function Season1_4MainScene:_refreshView()
	self:_showStage()
end

function Season1_4MainScene:focusRole(index)
	if not index then
		self:cancelFocus()
...
function Season1_4MainScene:cancelFocus()
	self:killFocusTween()
...
function Season1_4MainScene:_onMoveCompleted()
	self:killFocusTween()
end

function Season1_4MainScene:_onSizeUpdate(val)
	self._camera.orthographicSize = val
end

function Season1_4MainScene:_onOpenFullViewFinish(viewName)
	if not string.find(viewName, "Season") then
		self:doAudio(false)
	end
end

function Season1_4MainScene:_onOpenViewFinish(viewName)
...
function Season1_4MainScene:_onCloseViewFinish(viewName)
	if not self._listenerViews[viewName] then
		return
	end
...
function Season1_4MainScene:onClose()
	self:_removeEvents()
end

function Season1_4MainScene:killFocusTween()
...
function Season1_4MainScene:doAudio(play)
...
function Season1_4MainScene:onDestroyView()
	self:killFocusTween()
	TaskDispatcher.cancelTask(self._refreshRoles, self)
	TaskDispatcher.cancelTask(self._hideLevelObjs, self)
