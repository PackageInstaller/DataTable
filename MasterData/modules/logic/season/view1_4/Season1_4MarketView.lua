...
function Season1_4MarketView:_setInfo()
	...
	if isNewStage then
		if not Season1_4MarketView.NewStageTagPos then
			local targetStageTagPos = Season1_4MarketView.NomalStageTagPos
...
function Season1_4MarketView:_showPartStageItem(stage)
	if stage < 7 then
		...
		for i = 1, 6 do
			...
			if i <= stage then
				self._partStageItems[i].canvasgroup.alpha = Season1_4MarketView.UnLockStageItemAlpha or Season1_4MarketView.LockStageItemAlpha
			end
		end
	else
		for i = 1, 7 do
