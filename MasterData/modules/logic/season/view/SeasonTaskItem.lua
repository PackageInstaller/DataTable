-- chunkname: @modules/logic/season/view/SeasonTaskItem.lua

module("modules.logic.season.view.SeasonTaskItem", package.seeall)

local SeasonTaskItem = class("SeasonTaskItem", UserDataDispose)

SeasonTaskItem.BlockKey = "SeasonTaskItemAni"

function SeasonTaskItem:ctor(go)
	self:__onInit()
...
function SeasonTaskItem:addEventListeners()
...
function SeasonTaskItem:initTotal()
	self._btnGetTotal = gohelper.findChildButtonWithAudio(self._goTotal, "#btn_getall")
end

function SeasonTaskItem:onUpdateMO(data, open)
	if data then
...
function SeasonTaskItem:hide()
	gohelper.setActive(self._viewGO, false)
end

function SeasonTaskItem:refreshNormal(data)
	self.taskId = data.id
...
function SeasonTaskItem:refreshReward(data)
...
function SeasonTaskItem:createRewardItem(index)
...
function SeasonTaskItem:refreshRewardItem(item, data)
	if not data then
	...
	if data.equipId then
		...
		if not item.equipIcon then
			item.equipIcon = SeasonCelebrityCardItem.New()
...
function SeasonTaskItem:destroyRewardItem(item)
...
function SeasonTaskItem:refreshDesc(data)
	local config = data.config
...
function SeasonTaskItem:refreshProgress(data)
	self._txtCurCount.text = data.progress
	self._txtMaxCount.text = data.config.maxProgress
end

function SeasonTaskItem:refreshState(data)
...
function SeasonTaskItem:onClickGoto()
...
function SeasonTaskItem:onClickReceive()
	...
	UIBlockMgr.instance:startBlock(SeasonTaskItem.BlockKey)
	TaskDispatcher.runDelay(self._onPlayActAniFinished, self, 0.76)
end

function SeasonTaskItem:onClickGetTotal()
	TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Season)
end

function SeasonTaskItem:_onPlayActAniFinished()
	UIBlockMgr.instance:endBlock(SeasonTaskItem.BlockKey)
	TaskRpc.instance:sendFinishTaskRequest(self.taskId)
	self:hide()
end

function SeasonTaskItem:destroy()
