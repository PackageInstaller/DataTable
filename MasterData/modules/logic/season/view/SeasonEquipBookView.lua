-- chunkname: @modules/logic/season/view/SeasonEquipBookView.lua

module("modules.logic.season.view.SeasonEquipBookView", package.seeall)

local SeasonEquipBookView = class("SeasonEquipBookView", BaseView)

function SeasonEquipBookView:onInitView()
...
function SeasonEquipBookView:addEvents()
	self._btnexchange:AddClickListener(self._btnexchangeOnClick, self)
end

function SeasonEquipBookView:removeEvents()
	self._btnexchange:RemoveClickListener()
end

function SeasonEquipBookView:_btnexchangeOnClick()
	ViewMgr.instance:openView(ViewName.SeasonEquipComposeView, {
		actId = self.viewParam.actId
	})
end

function SeasonEquipBookView:_editableInitView()
...
function SeasonEquipBookView:onDestroyView()
...
function SeasonEquipBookView:onOpen()
...
function SeasonEquipBookView:onClose()
	return
end

function SeasonEquipBookView:refreshUI()
	self:refreshDesc()
	self:refreshIcon()
end

function SeasonEquipBookView:refreshDesc()
...
function SeasonEquipBookView:refreshIcon()
...
function SeasonEquipBookView:refreshProps(itemCfg)
	local processedSet = {}
	local isDirty = false
...
function SeasonEquipBookView:refreshSkills(itemCfg)
...
function SeasonEquipBookView:getOrCreatePropText(index)
...
function SeasonEquipBookView:getOrCreateSkillText(index)
...
function SeasonEquipBookView:checkCreateIcon()
...
function SeasonEquipBookView:onChangeSelectCard()
	self._animatorCard:Play("switch", 0, 0)
end

function SeasonEquipBookView:onSwitchCardAnim()
	self:refreshUI()
end

return SeasonEquipBookView
