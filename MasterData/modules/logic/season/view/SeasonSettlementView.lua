-- chunkname: @modules/logic/season/view/SeasonSettlementView.lua

module("modules.logic.season.view.SeasonSettlementView", package.seeall)

local SeasonSettlementView = class("SeasonSettlementView", BaseViewExtended)

function SeasonSettlementView:onInitView()
...
function SeasonSettlementView:addEvents()
...
function SeasonSettlementView:removeEvents()
...
function SeasonSettlementView:_btnclosetipOnClick()
	self:_showAwardTip()
	gohelper.setActive(self._gofightsucc, false)
end

function SeasonSettlementView:_btncloseawardOnClick()
...
function SeasonSettlementView:_btnstatOnClick()
	ViewMgr.instance:openView(ViewName.FightStatView)
end

function SeasonSettlementView:_showAwardTip()
...
function SeasonSettlementView:_showSettlement()
...
function SeasonSettlementView:_onRewardShow()
	self:_showSettlementReward()
end

function SeasonSettlementView:_dailyShowHero()
	self._heroIndex = 0
...
function SeasonSettlementView:_onShowTipsFlowDone()
	self.equip_cards = nil
end

function SeasonSettlementView:_checkNewRecord()
...
function SeasonSettlementView:_onCoverDungeonRecordReply(isCover)
...
function SeasonSettlementView:_onBtnCoverRecordClick()
	DungeonRpc.instance:sendCoverDungeonRecordRequest(true)
end

function SeasonSettlementView:_editableInitView()
...
function SeasonSettlementView:_btncloseOnClick()
...
function SeasonSettlementView:onOpen()
	...
	self._config = SeasonConfig.instance:getSeasonEpisodeCo(ActivityEnum.Activity.Season, layer)

	if self._config then
		self._simagedecorate6:LoadImage(ResUrl.getSeasonMarketIcon(self._config.stagePicture))
	end
	...
	self:com_loadAsset("ui/viewres/season/seasonsettlementherogroupitem.prefab", self._onHeroItemLoaded)
...
function SeasonSettlementView:_showSettlementReward()
...
function SeasonSettlementView:_setStages()
	local stage = SeasonConfig.instance:getSeasonEpisodeCo(ActivityEnum.Activity.Season, Activity104Model.instance:getBattleFinishLayer()).stage
	...
	local color = stage == 7 and "#B83838" or "#FFFFFF"
...
function SeasonSettlementView:_onRewardItemShow(obj, data, index)
...
function SeasonSettlementView:_onHeroItemLoaded(loader)
	self._heroList = {}
	...
	for i = 1, 4 do
		local hero_item = gohelper.clone(tarPrefab, self["_gohero" .. i])
	...
	if not is_replay then
		...
		for i = 1, 4 do
			...
			var_26_1 = hero_uid ~= "0" and equip104_dic[hero_uid] and equip104_dic[hero_uid].equipUid or false
			...
			local heroItem = self:openSubView(SeasonSettlementHeroItem, self._hero_obj_list[i], nil, is_replay, hero_uid, equip, equip_104)
		...
		if equip104_dic[character_uid] then
			if equip104_dic[character_uid].equipUid then
				if equip104_dic[character_uid].equipUid and equip104_dic[character_uid].equipUid ~= "0" then
					...
					self:openSubView(SeasonCelebrityCardGetItem, "ui/viewres/season/seasoncelebritycarditem.prefab", self._gosupercard, equip104_dic[character_uid].equipUid, true)
				end
			end
		end
	else
...
function SeasonSettlementView:_refreshHeroItemPos()
...
function SeasonSettlementView:_onGetFightRecordGroupReply(fightGroupMO)
	for i = 1, 4 do
		...
		local heroItem = self:openSubView(SeasonSettlementHeroItem, obj, nil, true, hero, equip, equip_104)
	...
	if fightGroupMO.replay_activity104Equip_data[character_uid] then
		...
		self:openSubView(SeasonCelebrityCardGetItem, "ui/viewres/season/seasoncelebritycarditem.prefab", self._gosupercard, nil, nil, equipId)
	end
end

function SeasonSettlementView:_showAwardTipsData()
...
function SeasonSettlementView:_showPassData()
	...
	local last_config = SeasonConfig.instance:getSeasonEpisodeCo(ActivityEnum.Activity.Season, curLayer - 1)
	...
	local next_config = SeasonConfig.instance:getSeasonEpisodeCo(ActivityEnum.Activity.Season, curLayer + 1)
...
function SeasonSettlementView:_releaseAwardImage()
...
function SeasonSettlementView:_onAwardItemShow(obj, data, index)
...
function SeasonSettlementView:_onAwardItemClick(data)
	MaterialTipController.instance:showMaterialInfo(data.materilType, data.materilId)
end

function SeasonSettlementView:onClose()
