-- chunkname: @modules/logic/season/view3_0/Season3_0SettlementView.lua

module("modules.logic.season.view3_0.Season3_0SettlementView", package.seeall)

local Season3_0SettlementView = class("Season3_0SettlementView", BaseViewExtended)

function Season3_0SettlementView:onInitView()
...
function Season3_0SettlementView:addEvents()
...
function Season3_0SettlementView:removeEvents()
...
function Season3_0SettlementView:_btnclosetipOnClick()
	self:_showAwardTip()
	gohelper.setActive(self._gofightsucc, false)
end

function Season3_0SettlementView:_btncloseawardOnClick()
...
function Season3_0SettlementView:_btnstatOnClick()
	ViewMgr.instance:openView(ViewName.FightStatView)
end

function Season3_0SettlementView:_showAwardTip()
...
function Season3_0SettlementView:_showSettlement()
...
function Season3_0SettlementView:_onRewardShow()
	self:_showSettlementReward()
end

function Season3_0SettlementView:_dailyShowHero()
	self._heroIndex = 0
...
function Season3_0SettlementView:_showHeroItem()
	self._heroIndex = self._heroIndex + 1

	gohelper.setActive(self._heroList[self._heroIndex].viewGO, true)
end

function Season3_0SettlementView:_showGetCardView()
	if self.equip_cards then
		self._showTipsFlow = FlowSequence.New()

		self._showTipsFlow:addWork(Season3_0SettlementTipsWork.New())
...
function Season3_0SettlementView:_onShowTipsFlowDone()
	self.equip_cards = nil
end

function Season3_0SettlementView:_checkNewRecord()
...
function Season3_0SettlementView:_onCoverDungeonRecordReply(isCover)
...
function Season3_0SettlementView:_detectCoverRecord()
...
function Season3_0SettlementView:_onBtnCoverRecordClick()
	DungeonRpc.instance:sendCoverDungeonRecordRequest(true)
end

function Season3_0SettlementView:_editableInitView()
...
function Season3_0SettlementView:_btncloseOnClick()
...
function Season3_0SettlementView:onOpen()
	self._startTime = Time.realtimeSinceStartup
...
function Season3_0SettlementView:_showSettlementReward()
...
function Season3_0SettlementView:_setStages()
	...
	for i = 1, 7 do
		self:refreshStageGO(self["_gostageitem" .. i], i, (self._episode_config.type == DungeonEnum.EpisodeType.SeasonSpecial or nil) and maxStage - 1, maxStage)
	end
end

function Season3_0SettlementView:refreshStageGO(stageGO, stage, curStage, maxStage)
	if not stageGO then
		return
	end

	local isMaxStage = stage == maxStage
	local isNotShow = maxStage < stage or isMaxStage and curStage < stage

	if isNotShow then
		gohelper.setActive(stageGO, false)

		return
	end

	gohelper.setActive(stageGO, true)

	local showLight = stage <= curStage
	local lightGO = gohelper.findChild(stageGO, "light")
	local darkGO = gohelper.findChild(stageGO, "dark")

	gohelper.setActive(lightGO, showLight)
	gohelper.setActive(darkGO, not showLight)

	if showLight then
		local color = isMaxStage and "#B83838" or "#FFFFFF"
		local lightImage = gohelper.findChildImage(stageGO, "light")

		SLFramework.UGUI.GuiHelper.SetColor(lightImage, color)
	end
end

function Season3_0SettlementView:_onRewardItemShow(obj, data, index)
	local itemIconGO = gohelper.findChild(obj, "go_prop")

	if data.materilType == MaterialEnum.MaterialType.EquipCard then
		local cardItem = Season3_0CelebrityCardItem.New()
...
function Season3_0SettlementView:_onHeroItemLoaded()
	self._heroList = {}
				...
				for i = 1, 4 do
					if trailDict[i] then
						...
						if trialCo then
							...
							local heroItem = self:openSubView(Season3_0SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, nil, equip, nil, nil, trailDict[i])
						...
						var_27_2 = hero_uid ~= "0" and equip104_dic[hero_uid] and equip104_dic[hero_uid].equipUid or false
						...
						local heroItem = self:openSubView(Season3_0SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, hero_uid, equip, equip_104)
				...
				if equip104_dic[character_uid] then
					if equip104_dic[character_uid].equipUid then
						if equip104_dic[character_uid].equipUid and equip104_dic[character_uid].equipUid ~= "0" then
							...
							self:openSubView(Season3_0CelebrityCardGetItem, Season3_0CelebrityCardItem.AssetPath, self._gosupercard, equip104_dic[character_uid].equipUid, true)
						end
					end
				end
			else
...
function Season3_0SettlementView:_refreshHeroItemPos()
...
function Season3_0SettlementView:_onGetFightRecordGroupReply(fightGroupMO)
	for i = 1, 4 do
		...
		local heroItem = self:openSubView(Season3_0SettlementHeroItem, obj, nil, true, hero, equip, equip_104, replay_data)
	...
	if fightGroupMO.replay_activity104Equip_data[character_uid] then
		...
		self:openSubView(Season3_0CelebrityCardGetItem, Season3_0CelebrityCardItem.AssetPath, self._gosupercard, nil, nil, equipId)
	end
end

function Season3_0SettlementView:_showAwardTipsData()
...
function Season3_0SettlementView:_showPassData()
...
function Season3_0SettlementView:_releaseAwardImage()
...
function Season3_0SettlementView:_onAwardItemShow(obj, data, index)
...
function Season3_0SettlementView:_onAwardItemClick(data)
	MaterialTipController.instance:showMaterialInfo(data.materilType, data.materilId)
end

function Season3_0SettlementView:onClose()
...
return Season3_0SettlementView
