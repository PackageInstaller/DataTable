-- chunkname: @modules/logic/season/view3_0/Season3_0MarketView.lua

module("modules.logic.season.view3_0.Season3_0MarketView", package.seeall)

local Season3_0MarketView = class("Season3_0MarketView", BaseView)

function Season3_0MarketView:onInitView()
...
function Season3_0MarketView:addEvents()
...
function Season3_0MarketView:removeEvents()
...
function Season3_0MarketView:_btnlastOnClick()
...
function Season3_0MarketView:_btnnextOnClick()
...
function Season3_0MarketView:_delayShowInfo()
	self:_showInfo()
	self:_setStages()
	self:updateLeftDesc()
end

function Season3_0MarketView:_btnstartOnClick()
...
function Season3_0MarketView:_btnreplayOnClick()
	local actId = Activity104Model.instance:getCurSeasonId()
...
function Season3_0MarketView:_editableInitView()
...
function Season3_0MarketView:onUpdateParam()
	return
end

function Season3_0MarketView:onOpen()
...
function Season3_0MarketView:resetData()
	self._showLvItems = {}
...
function Season3_0MarketView:_showInfoByOpen()
...
function Season3_0MarketView:_setLevels()
	...
	if self._layer <= 4 then
		for i = 1, 6 do
			...
			self._showLvItems[i] = Season3_0MarketShowLevelItem.New()
...
function Season3_0MarketView:_delayShowItem()
...
function Season3_0MarketView:_setStages()
	local episodeCo = SeasonConfig.instance:getSeasonEpisodeCo(Activity104Model.instance:getCurSeasonId(), self._layer)
	local stage = episodeCo.stage
	local maxStage = Activity104Model.instance:getMaxStage()

	for i = 1, 7 do
		self:refreshLeftStage(self["_gostageinfoitem" .. i], i, stage, maxStage, "#3E3E3D")
		self:refreshLeftStage(self["_gostagelvitem" .. i], i, stage, maxStage, "#C1C1C2")
	end

	self._simagedecorate:LoadImage(SeasonViewHelper.getSeasonIcon(string.format("icon/b_chatu_%s.png", episodeCo.stagePicture)))
end

function Season3_0MarketView:refreshLeftStage(stageGO, stage, curStage, maxStage, normalColor)
	if gohelper.isNil(stageGO) then
		return
	end
	...
	local isLight = stage <= curStage

	gohelper.setActive(lightGO, isLight)
	gohelper.setActive(darkGO, not isLight)

	if isLight then
		local color = isMaxStage and "#B83838" or normalColor
...
function Season3_0MarketView:_showInfo()
	self:_setInfo()
	self:_setParts()
end

Season3_0MarketView.NomalStageTagPos = Vector2(46.3, 2.7)
Season3_0MarketView.NewStageTagPos = Vector2(4.37, 2.7)

function Season3_0MarketView:_setInfo()
	local episodeCo = SeasonConfig.instance:getSeasonEpisodeCo(Activity104Model.instance:getCurSeasonId(), self._layer)
	...
	if isNewStage then
		if not Season3_0MarketView.NewStageTagPos then
			local targetStageTagPos = Season3_0MarketView.NomalStageTagPos
...
function Season3_0MarketView:refreshEquipCardItem()
	...
	if episodeCo.firstPassEquipId > 0 then
		...
		if not self._rewardItems[1].itemIcon then
			self._rewardItems[1].itemIcon = Season3_0CelebrityCardItem.New()
...
function Season3_0MarketView:createRewardItem(index)
...
function Season3_0MarketView:refreshRewardItem(item, itemInfo)
...
function Season3_0MarketView:_setParts()
	local episodeCo = SeasonConfig.instance:getSeasonEpisodeCo(Activity104Model.instance:getCurSeasonId(), self._layer)
	...
	if self._layer < Activity104Model.instance:getAct104CurLayer() or self._layer >= Activity104Model.instance:getMaxLayer() then
		...
		if isNewStage then
			...
			self:_showPartStageItem(stage + 1, maxStage)
		end
...
Season3_0MarketView.UnLockStageItemAlpha = 1
Season3_0MarketView.LockStageItemAlpha = 0.3

function Season3_0MarketView:_showPartStageItem(nextStage, maxStage)
	for i = 1, 7 do
		...
		self:_refreshPartStage(self._partStageItems[i], nextStage, maxStage)
	end
end

function Season3_0MarketView:_refreshPartStage(item, nextStage, maxStage)
	local stage = item.index
	local isMaxStage = stage == maxStage
	local isNotShow = maxStage < stage or isMaxStage and nextStage < stage
	...
	gohelper.setActive(item.go, true)
	gohelper.setActive(item.next, stage == nextStage)
	gohelper.setActive(item.current, stage ~= nextStage)

	if stage <= nextStage then
		item.canvasgroup.alpha = Season3_0MarketView.UnLockStageItemAlpha or Season3_0MarketView.LockStageItemAlpha
	end
end

function Season3_0MarketView:updateLeftDesc()
...
function Season3_0MarketView:createLeftDescItem(index)
	local item = self:getUserDataTb_()
...
function Season3_0MarketView:updateLeftDescItem(item, co)
	if not co then
		gohelper.setActive(item.go, false)
...
function Season3_0MarketView:moveToCurDesc()
...
function Season3_0MarketView:onClose()
	TaskDispatcher.cancelTask(self._delayShowInfo, self)
end

function Season3_0MarketView:onDestroyView()
	TaskDispatcher.cancelTask(self._showInfoByOpen, self)
	TaskDispatcher.cancelTask(self.moveToCurDesc, self)
