-- chunkname: @modules/logic/season/view3_0/Season3_0HeroGroupCardItem.lua

module("modules.logic.season.view3_0.Season3_0HeroGroupCardItem", package.seeall)

local Season3_0HeroGroupCardItem = class("Season3_0HeroGroupCardItem", UserDataDispose)

Season3_0HeroGroupCardItem.TweenDuration = 0.16
Season3_0HeroGroupCardItem.DragOffset = Vector2(0, 40)
Season3_0HeroGroupCardItem.ZeroPos = Vector2(-2.7, -5)
Season3_0HeroGroupCardItem.ZeroScale = 0.39

function Season3_0HeroGroupCardItem:ctor(go, parent, param)
	self:__onInit()
...
function Season3_0HeroGroupCardItem:init()
...
function Season3_0HeroGroupCardItem:AddDrag(go)
	if self._drag then
		return
	end
...
function Season3_0HeroGroupCardItem:setData(data, equipId, equipUid, hasTrialEquip)
...
function Season3_0HeroGroupCardItem:setActive(isActive)
	self.isActive = isActive
end

function Season3_0HeroGroupCardItem:updateView()
	...
	if self.hasTrialEquip then
		local hasEquip = self:getEquipId(actId, groupId) ~= 0

		self.posUnlock = hasEquip
		self.slotUnlock = hasEquip
	end
	...
	if self.posUnlock then
		...
		if equipId ~= 0 then
			if not self._seasonCardItem then
				self._seasonCardItem = Season3_0CelebrityCardItem.New()
...
function Season3_0HeroGroupCardItem:playEmptyUnlockAnim()
	local actId = Activity104Model.instance:getCurSeasonId()
	local pos = self.id - 1
...
function Season3_0HeroGroupCardItem:getEquipId(actId, groupId)
...
function Season3_0HeroGroupCardItem:hasUseSeasonEquipCard()
	return self._hasUseSeasonEquipCard
end

function Season3_0HeroGroupCardItem:_btnCardClick()
	if self.inDrag then
		return
	end
	...
	if not unlock and not self.hasTrialEquip then
...
function Season3_0HeroGroupCardItem:canDrag()
...
function Season3_0HeroGroupCardItem:_onBeginDrag(dragTransform, pointerEventData)
	...
	self:_tweenToPos(self._trscard, anchorPos + Season3_0HeroGroupCardItem.DragOffset, true)

	local scale = Season3_0HeroGroupCardItem.ZeroScale * 1.7

	self.tweenId = ZProj.TweenHelper.DOScale(self._trscard, scale, scale, scale, Season3_0HeroGroupCardItem.TweenDuration)
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, 0, Season3_0HeroGroupCardItem.TweenDuration)
	self.inDrag = true
end

function Season3_0HeroGroupCardItem:_onDrag(dragTransform, pointerEventData)
	if not self:canDrag() then
		self.inDrag = false
	...
	self:_tweenToPos(self._trscard, anchorPos + Season3_0HeroGroupCardItem.DragOffset)

	self.inDrag = true
end

function Season3_0HeroGroupCardItem:_onEndDrag(dragTransform, pointerEventData)
	self.inDrag = false
	...
	local scale = Season3_0HeroGroupCardItem.ZeroScale * 1
	...
	if not targetItem or not targetItem:canExchange(self) then
		self._setToPos(self, self._trscard, Season3_0HeroGroupCardItem.ZeroPos, true, self._onDragFailTweenEnd, self)

		self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, self.orignRoteZ, Season3_0HeroGroupCardItem.TweenDuration)
	...
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, targetItem.orignRoteZ, Season3_0HeroGroupCardItem.TweenDuration)
end

function Season3_0HeroGroupCardItem:_tweenToPos(transform, anchorPos)
	...
	if math.abs(curAnchorX - anchorPos.x) > 10 or math.abs(curAnchorY - anchorPos.y) > 10 then
		self.posTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season3_0HeroGroupCardItem.TweenDuration)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
	end
end

function Season3_0HeroGroupCardItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)

	if tween then
		self.moveTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season3_0HeroGroupCardItem.TweenDuration, callback, callbackObj, param)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
...
function Season3_0HeroGroupCardItem:_moveToTarget(position)
...
function Season3_0HeroGroupCardItem:_setDragEnabled(isEnabled)
	local heroList = self.parent:getHeroItemList()
...
function Season3_0HeroGroupCardItem:setDragEnabled(isEnabled)
	if self._drag then
		self._drag.enabled = isEnabled
	end
end

function Season3_0HeroGroupCardItem:canExchange(targetItem)
	...
	for i = 1, maxCount do
		if i ~= srcSlot then
			local uid = groupMo:getAct104PosSlotEquip(srcPos, i)
			local id = Activity104Model.instance:getItemIdByUid(uid)
		...
		if i ~= tarSlot then
			local uid = groupMo:getAct104PosSlotEquip(tarPos, i)
			local id = Activity104Model.instance:getItemIdByUid(uid)
			local co = SeasonConfig.instance:getSeasonEquipCo(id)
...
function Season3_0HeroGroupCardItem:_onDragFailTweenEnd()
	self:_setDragEnabled(true)
	gohelper.setAsLastSibling(self.parent._cardItem1.go)
end

function Season3_0HeroGroupCardItem:_onDragSuccessTweenEnd(targetItem)
	self:killTweenId()
	self:_setToPos(self._trscard, Season3_0HeroGroupCardItem.ZeroPos)
	self:_setToPos(targetItem._trscard, Season3_0HeroGroupCardItem.ZeroPos)
...
function Season3_0HeroGroupCardItem:killTweenId()
...
function Season3_0HeroGroupCardItem:destory()
	if self._drag then
...
return Season3_0HeroGroupCardItem
