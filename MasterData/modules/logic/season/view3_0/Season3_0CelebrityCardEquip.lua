-- chunkname: @modules/logic/season/view3_0/Season3_0CelebrityCardEquip.lua

module("modules.logic.season.view3_0.Season3_0CelebrityCardEquip", package.seeall)

local Season3_0CelebrityCardEquip = class("Season3_0CelebrityCardEquip", LuaCompBase)

function Season3_0CelebrityCardEquip:init(go)
	Season3_0CelebrityCardEquip.super.init(self, go)
...
Season3_0CelebrityCardEquip.MaxRare = 5

function Season3_0CelebrityCardEquip:_editableInitView()
	self._rareGoMap = {}

	for i = 1, Season3_0CelebrityCardEquip.MaxRare do
...
function Season3_0CelebrityCardEquip:onDestroy()
	self:disposeUI()
end

function Season3_0CelebrityCardEquip:checkInitBtnClick()
...
function Season3_0CelebrityCardEquip:checkInitLongPress()
...
function Season3_0CelebrityCardEquip:disposeUI()
...
function Season3_0CelebrityCardEquip:updateData(itemId)
	self.itemId = itemId

	self:refreshUI()
end

function Season3_0CelebrityCardEquip:createRareMap(go)
...
function Season3_0CelebrityCardEquip:refreshUI()
...
function Season3_0CelebrityCardEquip:refreshSelfChoice()
...
function Season3_0CelebrityCardEquip:refreshIcon()
	if self._curSelectedItem then
...
function Season3_0CelebrityCardEquip:refreshFlag()
	if not self.itemId then
		return
	end
...
function Season3_0CelebrityCardEquip:setFlagUIPos(targetPosX, targetPosY)
...
function Season3_0CelebrityCardEquip:setFlagUIScale(targetScale)
...
function Season3_0CelebrityCardEquip:handleIconLoaded()
	if not self._isDisposed then
		local item = self._curSelectedItem
...
function Season3_0CelebrityCardEquip:setColorDark(enable)
	self._darkMaskColor = enable and "#7b7b7b" or "#ffffff"

	self:refreshIcon()
end

function Season3_0CelebrityCardEquip:setShowTag(enable)
	self._showTag = enable

	self:refreshFlag()
end

function Season3_0CelebrityCardEquip:setShowProbability(enable)
	self._showProbability = enable

	self:refreshFlag()
end

function Season3_0CelebrityCardEquip:setShowNewFlag(enable)
	self._showNewFlag = enable

	self:refreshFlag()
end

function Season3_0CelebrityCardEquip:setShowNewFlag2(enable)
	self._showNewFlag2 = enable

	self:refreshFlag()
end

function Season3_0CelebrityCardEquip:setClickCall(clickCall, clickCallObj, param)
...
function Season3_0CelebrityCardEquip:setLongPressCall(longPressCall, longPressCallObj, param)
