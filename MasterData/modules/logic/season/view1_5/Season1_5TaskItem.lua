-- chunkname: @modules/logic/season/view1_5/Season1_5TaskItem.lua

module("modules.logic.season.view1_5.Season1_5TaskItem", package.seeall)

local Season1_5TaskItem = class("Season1_5TaskItem", UserDataDispose)

Season1_5TaskItem.BlockKey = "Season1_5TaskItemAni"

function Season1_5TaskItem:ctor(go, parentScrollGO)
	self:__onInit()
...
function Season1_5TaskItem:addEventListeners()
...
function Season1_5TaskItem:initTotal()
	self._btnGetTotal = gohelper.findChildButtonWithAudio(self._goTotal, "#btn_getall")
end

function Season1_5TaskItem:onUpdateMO(data, open)
	if data then
...
function Season1_5TaskItem:hide()
	gohelper.setActive(self._viewGO, false)
end

function Season1_5TaskItem:refreshNormal(data)
	self.taskId = data.id
...
function Season1_5TaskItem:refreshReward(data)
...
function Season1_5TaskItem:createRewardItem(index)
...
function Season1_5TaskItem:refreshRewardItem(item, data)
	if not data then
	...
	if data.equipId then
		...
		if not item.equipIcon then
			item.equipIcon = Season1_5CelebrityCardItem.New()
...
function Season1_5TaskItem:destroyRewardItem(item)
...
function Season1_5TaskItem:refreshDesc(data)
	local config = data.config
...
function Season1_5TaskItem:refreshProgress(data)
	self._txtCurCount.text = data.progress
	self._txtMaxCount.text = data.config.maxProgress
end

function Season1_5TaskItem:refreshState(data)
...
function Season1_5TaskItem:onClickGoto()
...
function Season1_5TaskItem:onClickReceive()
	...
	UIBlockMgr.instance:startBlock(Season1_5TaskItem.BlockKey)
	TaskDispatcher.runDelay(self._onPlayActAniFinished, self, 0.76)
end

function Season1_5TaskItem:onClickGetTotal()
	TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Season)
end

function Season1_5TaskItem:_onPlayActAniFinished()
	UIBlockMgr.instance:endBlock(Season1_5TaskItem.BlockKey)
	TaskRpc.instance:sendFinishTaskRequest(self.taskId)
	self:hide()
end

function Season1_5TaskItem:destroy()
