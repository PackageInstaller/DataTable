-- chunkname: @modules/logic/season/view1_5/Season1_5EquipComposeView.lua

module("modules.logic.season.view1_5.Season1_5EquipComposeView", package.seeall)

local Season1_5EquipComposeView = class("Season1_5EquipComposeView", BaseView)

function Season1_5EquipComposeView:onInitView()
...
function Season1_5EquipComposeView:addEvents()
...
function Season1_5EquipComposeView:removeEvents()
	self._btncompose:RemoveClickListener()
	self._btndiscompose:RemoveClickListener()
end

Season1_5EquipComposeView.MaxUICount = 3

function Season1_5EquipComposeView:_editableInitView()
...
function Season1_5EquipComposeView:onDestroyView()
...
function Season1_5EquipComposeView:onOpen()
...
function Season1_5EquipComposeView:onClose()
	UIBlockMgrExtend.setNeedCircleMv(true)
	UIBlockMgr.instance:endBlock(Season1_5EquipComposeView.Compose_Anim_Block_Key)
	TaskDispatcher.cancelTask(self.onPlayComposeAnimOver, self)
	TaskDispatcher.cancelTask(self.delayRefreshView, self)
end

function Season1_5EquipComposeView:handleComposeSucc()
	return
end

function Season1_5EquipComposeView:handleComposeDataChanged()
	if self._delayRefreshUITime ~= nil then
		local curTime = Time.time

		if curTime - self._delayRefreshUITime < Season1_5EquipComposeView.DelayRefreshTime then
			return
		end
	end
...
function Season1_5EquipComposeView:refreshUI()
...
function Season1_5EquipComposeView:refreshHint()
...
function Season1_5EquipComposeView:refreshMatList()
	for i = 1, Season1_5EquipComposeView.MaxUICount do
		self:refreshMat(i)
	end
end

function Season1_5EquipComposeView:refreshButtons()
...
function Season1_5EquipComposeView:refreshMat(index)
...
function Season1_5EquipComposeView:getOrCreateMatItem(index)
...
function Season1_5EquipComposeView:checkCreateMatItemIcon(item, index)
	if not item.icon then
		local path = self.viewContainer:getSetting().otherRes[2]
		local go = self:getResInst(path, item.goIcon, "icon" .. tostring(index))

		item.icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season1_5CelebrityCardEquip)
...
function Season1_5EquipComposeView:onClickMatSlot(index)
	local itemUid = Activity104EquipItemComposeModel.instance.curSelectMap[index]
	local isEmpty = itemUid == Activity104EquipItemComposeModel.EmptyUid
...
function Season1_5EquipComposeView:_btncomposeOnClick()
...
function Season1_5EquipComposeView:_btndiscomposeOnClick()
	GameFacade.showToast(ToastEnum.ClickSeasonDiscompose)
end

Season1_5EquipComposeView.DelaySendTime = 1
Season1_5EquipComposeView.DelayRefreshTime = 1
Season1_5EquipComposeView.Compose_Anim_Block_Key = "Compose_Anim_Block"

function Season1_5EquipComposeView:playAnimBeforeSend()
	TaskDispatcher.cancelTask(self.onPlayComposeAnimOver, self)
	...
	UIBlockMgr.instance:startBlock(Season1_5EquipComposeView.Compose_Anim_Block_Key)
	TaskDispatcher.runDelay(self.onPlayComposeAnimOver, self, Season1_5EquipComposeView.DelaySendTime)
end

function Season1_5EquipComposeView:onPlayComposeAnimOver()
	UIBlockMgrExtend.setNeedCircleMv(true)
	UIBlockMgr.instance:endBlock(Season1_5EquipComposeView.Compose_Anim_Block_Key)
	...
	TaskDispatcher.runDelay(self.delayRefreshView, self, Season1_5EquipComposeView.DelayRefreshTime)
end

function Season1_5EquipComposeView:delayRefreshView()
	self:refreshUI()
end

return Season1_5EquipComposeView
