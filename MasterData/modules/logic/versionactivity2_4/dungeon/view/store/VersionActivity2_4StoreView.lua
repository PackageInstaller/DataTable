-- chunkname: @modules/logic/versionactivity2_4/dungeon/view/store/VersionActivity2_4StoreView.lua

module("modules.logic.versionactivity2_4.dungeon.view.store.VersionActivity2_4StoreView", package.seeall)

local VersionActivity2_4StoreView = class("VersionActivity2_4StoreView", BaseView)

function VersionActivity2_4StoreView:onInitView()
...
function VersionActivity2_4StoreView:addEvents()
...
function VersionActivity2_4StoreView:removeEvents()
...
function VersionActivity2_4StoreView:_onScrollValueChanged()
...
function VersionActivity2_4StoreView:refreshTime()
	local actInfoMo = ActivityModel.instance:getActivityInfo()[VersionActivity2_4Enum.ActivityId.DungeonStore]

	self._txttime.text = actInfoMo:getRemainTimeStr3(false, false)
end

function VersionActivity2_4StoreView:refreshStoreContent()
	if self.actId then
		...
		for i = 1, #self.actId do
			...
			if not self.storeItemList[i] then
				...
				storeItem = VersionActivity2_4StoreItem.New()
...
function VersionActivity2_4StoreView:scrollToFirstNoSellOutStore()
...
function VersionActivity2_4StoreView:getFirstNoSellOutGroup()
	local storeGroupDict = ActivityStoreConfig.instance:getActivityStoreGroupDict(self.actId)
...
function VersionActivity2_4StoreView:onClose()
	TaskDispatcher.cancelTask(self.refreshTime, self)
end

function VersionActivity2_4StoreView:onDestroyView()
	for _, storeItem in ipairs(self.storeItemList) do
		storeItem:onDestroy()
	end
end

return VersionActivity2_4StoreView
