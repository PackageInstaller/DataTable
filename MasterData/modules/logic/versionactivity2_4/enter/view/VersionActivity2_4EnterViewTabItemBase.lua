-- chunkname: @modules/logic/versionactivity2_4/enter/view/VersionActivity2_4EnterViewTabItemBase.lua

module("modules.logic.versionactivity2_4.enter.view.VersionActivity2_4EnterViewTabItemBase", package.seeall)

local VersionActivity2_4EnterViewTabItemBase = class("VersionActivity2_4EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)

function VersionActivity2_4EnterViewTabItemBase:_editableInitView()
...
function VersionActivity2_4EnterViewTabItemBase:_getTagPath()
	return "#txt_name/#go_tag"
end

function VersionActivity2_4EnterViewTabItemBase:afterSetData()
	if self.actId then
	...
	if not self.activityCo then
		...
		logError("VersionActivity2_4EnterViewTabItemBase.afterSetData error, no act config, actId:%s", self.actId)
	...
	if not string.nilorempty(selectBg) then
		UISpriteSetMgr.instance:setV2a4MainActivitySprite(self.imageSelectTabIcon, selectBg)
	end

	local unselectBg = iconCoArr[2]

	if not string.nilorempty(unselectBg) then
		UISpriteSetMgr.instance:setV2a4MainActivitySprite(self.imageUnSelectTabIcon, unselectBg)
	end

	self.redDotIcon = RedDotController.instance:addRedDot(self.goRedDot, self.activityCo.redDotId, self.redDotUid)
end

function VersionActivity2_4EnterViewTabItemBase:childRefreshSelect()
	gohelper.setActive(self.goSelected, self.isSelect)
	gohelper.setActive(self.goUnselected, not self.isSelect)
end

function VersionActivity2_4EnterViewTabItemBase:childRefreshUI()
...
function VersionActivity2_4EnterViewTabItemBase:refreshTag()
	self:clearTag()
...
function VersionActivity2_4EnterViewTabItemBase:clearTag()
...
function VersionActivity2_4EnterViewTabItemBase:refreshNormalTag()
	...
	local isShowRemainTime = VersionActivity2_4EnterHelper.GetIsShowTabRemainTime(self.actId)

	if isShowRemainTime and actInfoMo then
		local offsetSecond = actInfoMo:getRealEndTimeStamp() - ServerTime.now()

		if offsetSecond > VersionActivity2_4Enum.MaxShowTimeOffset then
			return
		end
...
function VersionActivity2_4EnterViewTabItemBase:refreshNotUnlockTag()
	gohelper.setActive(self.goTagLock, false)
...
function VersionActivity2_4EnterViewTabItemBase:refreshLockTag()
...
function VersionActivity2_4EnterViewTabItemBase:isShowRedDot()
	return self.redDotIcon and self.redDotIcon.show
end

return VersionActivity2_4EnterViewTabItemBase
