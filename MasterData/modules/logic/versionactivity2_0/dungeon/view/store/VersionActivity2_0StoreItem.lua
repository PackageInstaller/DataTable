-- chunkname: @modules/logic/versionactivity2_0/dungeon/view/store/VersionActivity2_0StoreItem.lua

module("modules.logic.versionactivity2_0.dungeon.view.store.VersionActivity2_0StoreItem", package.seeall)

local VersionActivity2_0StoreItem = class("VersionActivity2_0StoreItem", UserDataDispose)
...
function VersionActivity2_0StoreItem:onInitView(go)
	self:__onInit()
	...
	self.addEventCb(self, VersionActivityController.instance, VersionActivityEvent.OnBuy107GoodsSuccess, self.onBuyGoodsSuccess, self)
end

function VersionActivity2_0StoreItem:onBuyGoodsSuccess()
	self:sortGoodsCoList()
	self:refreshGoods()
end

function VersionActivity2_0StoreItem:updateInfo(groupId, groupGoodsCoList)
...
function VersionActivity2_0StoreItem:sortGoodsCoList()
	table.sort(self.groupGoodsCoList, sortGoods)
end

function VersionActivity2_0StoreItem:refreshTag()
	if self.gotag then
		return
	end
...
function VersionActivity2_0StoreItem:refreshTagClip(scrollStore)
	...
	self.tagCanvasGroup.alpha = Mathf.Clamp((CLIP_POS_Y - tagPosY.y) / (CLIP_POS_Y - START_FADE_POS_Y), 0, 1)
...
function VersionActivity2_0StoreItem:getHeight()
	return recthelper.getHeight(self.go.transform)
end

function VersionActivity2_0StoreItem:onDestroy()
...
return VersionActivity2_0StoreItem
