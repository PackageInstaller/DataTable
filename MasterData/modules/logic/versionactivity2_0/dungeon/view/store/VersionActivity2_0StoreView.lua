-- chunkname: @modules/logic/versionactivity2_0/dungeon/view/store/VersionActivity2_0StoreView.lua

module("modules.logic.versionactivity2_0.dungeon.view.store.VersionActivity2_0StoreView", package.seeall)

local VersionActivity2_0StoreView = class("VersionActivity2_0StoreView", BaseView)

function VersionActivity2_0StoreView:onInitView()
...
function VersionActivity2_0StoreView:addEvents()
...
function VersionActivity2_0StoreView:removeEvents()
...
function VersionActivity2_0StoreView:_onScrollValueChanged()
...
function VersionActivity2_0StoreView:refreshTime()
	local actInfoMo = ActivityModel.instance:getActivityInfo()[VersionActivity2_0Enum.ActivityId.DungeonStore]

	self._txttime.text = actInfoMo:getRemainTimeStr3(false, true)
end

function VersionActivity2_0StoreView:refreshStoreContent()
	if self.actId then
		...
		for i = 1, #self.actId do
			...
			if not self.storeItemList[i] then
				...
				storeItem = VersionActivity2_0StoreItem.New()
...
function VersionActivity2_0StoreView:scrollToFirstNoSellOutStore()
...
function VersionActivity2_0StoreView:getFirstNoSellOutGroup()
	local storeGroupDict = ActivityStoreConfig.instance:getActivityStoreGroupDict(self.actId)
...
function VersionActivity2_0StoreView:onClose()
	TaskDispatcher.cancelTask(self.refreshTime, self)
end

function VersionActivity2_0StoreView:onDestroyView()
	for _, storeItem in ipairs(self.storeItemList) do
		storeItem:onDestroy()
	end
end

return VersionActivity2_0StoreView
