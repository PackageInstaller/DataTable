-- chunkname: @modules/logic/versionactivity2_0/joe/view/ActJoeLevelView.lua

module("modules.logic.versionactivity2_0.joe.view.ActJoeLevelView", package.seeall)

local ActJoeLevelView = class("ActJoeLevelView", BaseView)

function ActJoeLevelView:onInitView()
...
function ActJoeLevelView:addEvents()
...
function ActJoeLevelView:removeEvents()
...
function ActJoeLevelView:_btnPlayBtnOnClick()
	if self.actConfig.storyId > 0 then
		StoryController.instance:playStory(self.actConfig.storyId)
	end
end

function ActJoeLevelView:_btnStoryOnClick(isOpen)
...
function ActJoeLevelView:_btnFightOnClick(isOpen)
...
function ActJoeLevelView:_btnTaskOnClick()
	ViewMgr.instance:openView(ViewName.ActJoeTaskView)
end

function ActJoeLevelView:_editableInitView()
...
function ActJoeLevelView:onOpen()
...
function ActJoeLevelView:_checkFirstEnter()
	if not self.storyItemList[2]:isUnlock() then
		local record = PlayerPrefsHelper.getNumber("ActJoeFirstEnter", 0)
...
function ActJoeLevelView:onClose()
...
function ActJoeLevelView:onDestroyView()
...
function ActJoeLevelView:OnStoryItemClick(index)
	self:_focusStoryItem(index, true)
end

function ActJoeLevelView:OnFightItemClick(_index)
...
function ActJoeLevelView:OnTabSwitch(needShowFight)
...
function ActJoeLevelView:_onStoryOpenEnd()
	self:_initPathStatus()
end

function ActJoeLevelView:_onFightOpenEnd()
	self:_playFightFinishAnim()
end

function ActJoeLevelView:_onGoStoryEnd()
	self:_initPathStatus()
end

function ActJoeLevelView:_onDragBegin()
	self._audioScroll:onDragBegin()
end

function ActJoeLevelView:_onDragEnd()
	self._audioScroll:onDragEnd()
end

function ActJoeLevelView:_onClickDown()
	self._audioScroll:onClickDown()
end

function ActJoeLevelView:_initStageItems()
	...
	for i = 1, #storyConfigList do
		...
		self.storyItemList[i] = MonoHelper.addNoUpdateLuaComOnceToGo(cloneGo, RoleActStoryItem, self)
	...
	for i = 1, #fightConfigList / 2 do
		...
		self.fightItemList[i] = MonoHelper.addNoUpdateLuaComOnceToGo(cloneGo, RoleActFightItem, self)
...
function ActJoeLevelView:_playFirstUnlock()
...
function ActJoeLevelView:_playStoryFinishAnim()
...
function ActJoeLevelView:_finishStoryEnd()
...
function ActJoeLevelView:_playPathAnim()
...
function ActJoeLevelView:_unlockStory()
	self.storyItemList[self.finishStoryIndex + 1]:playUnlock()
	TaskDispatcher.runDelay(self._unlockStoryEnd, self, 1.33)
end

function ActJoeLevelView:_unlockStoryEnd()
...
function ActJoeLevelView:_playFightFinishAnim()
...
function ActJoeLevelView:_starShowEnd()
...
function ActJoeLevelView:_unlockFightEnd()
...
function ActJoeLevelView:_delayOpenStory()
	self._anim:Play("openstory", 0, 0)
end

function ActJoeLevelView:_delayOpenFight()
	self._anim:Play("openfight", 0, 0)
end

function ActJoeLevelView:_showLeftTime()
	self._txtlimittime.text = ActivityHelper.getActivityRemainTimeStr(self.actId)
end

function ActJoeLevelView:_initPathStatus()
...
function ActJoeLevelView:_focusStoryItem(index, needPlay)
...
function ActJoeLevelView:_onFocusEnd(index)
	self.storyItemList[index]:playStory()
end

function ActJoeLevelView:_lockScreen(lock)
	if lock then
