-- chunkname: @modules/logic/survival/view/shelter/ShelterHeroGroupListView.lua

module("modules.logic.survival.view.shelter.ShelterHeroGroupListView", package.seeall)

local ShelterHeroGroupListView = class("ShelterHeroGroupListView", HeroGroupListView)
...
function ShelterHeroGroupListView:_editableInitView()
...
function ShelterHeroGroupListView:initHeroList()
...
function ShelterHeroGroupListView:_getHeroItemCls()
	return ShelterHeroGroupHeroItem
end

function ShelterHeroGroupListView:_calcIndex(position)
	for i = 1, self._roleNum do
		if self.heroPosTrList[i] then
			if gohelper.isMouseOverGo(self.heroPosTrList[i], position) then
				return i
			end
		end
	end
...
function ShelterHeroGroupListView:canDrag(param, isWrap)
...
function ShelterHeroGroupListView:_onEndDrag(param, pointerEventData)
...
function ShelterHeroGroupListView:_getOrCreateHeroGO(index)
...
function ShelterHeroGroupListView:_checkRestrictHero()
	...
	for i = 1, 5 do
		...
		if heroSingleGroupMO then
			...
			local heroHealthMo = heroMo and weekInfo:getHeroMo(heroMo.heroId)

			if HeroGroupModel.instance:isRestrict(heroSingleGroupMO.heroUid) or not heroMo or fight.usedHeroId[heroMo.heroId] or heroHealthMo.health <= 0 then
...
function ShelterHeroGroupListView:initHeroBgList()
...
function ShelterHeroGroupListView:_getOrCreateHeroBg(index)
...
function ShelterHeroGroupListView:_onScreenSizeChange()
...
function ShelterHeroGroupListView:onDestroyView()
	if self._heroItemList then
	...
	ShelterHeroGroupListView.super.onDestroyView(self)
end

return ShelterHeroGroupListView
