...
function SurvivalHeroGroupEditView:_btncharacterOnClick()
	if self._heroMO then
		local heroMoList = self._isShowQuickEdit and SurvivalHeroGroupQuickEditListModel.instance:getList() or SurvivalHeroGroupEditListModel.instance:getList()
		local newList = {}
...
function SurvivalHeroGroupEditView:_refreshAttribute()
	if self._heroMO then
		local mo = HeroGroupTrialModel.instance:getById(self._originalHeroUid)
		local trialEquipMo
		local attrDict = self._heroMO:getTotalBaseAttrDict(self._equips, nil, nil, not self._heroMO:isTrial(), (mo or nil) and mo.trialEquipMo, SurvivalBalanceHelper.getHeroBalanceInfo)
