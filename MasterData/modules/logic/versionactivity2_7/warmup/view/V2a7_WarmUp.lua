-- chunkname: @modules/logic/versionactivity2_7/warmup/view/V2a7_WarmUp.lua

module("modules.logic.versionactivity2_7.warmup.view.V2a7_WarmUp", package.seeall)

local V2a7_WarmUp = class("V2a7_WarmUp", BaseView)

function V2a7_WarmUp:onInitView()
...
function V2a7_WarmUp:addEvents()
...
function V2a7_WarmUp:removeEvents()
...
function V2a7_WarmUp:_btngetrewardOnClick()
...
function V2a7_WarmUp:_editableInitView()
...
function V2a7_WarmUp:onDataUpdateFirst()
	self:_refreshOnce()
	self:_refreshActive_btnplay()
end

function V2a7_WarmUp:onDataUpdate()
	self:_refresh()
end

function V2a7_WarmUp:onSwitchEpisode()
...
function V2a7_WarmUp:onUpdateActivity()
	self._descScrollRect:StopMovement()
	self:_setTaskContentToEnd()
	self:_refresh()
end

function V2a7_WarmUp:onUpdateParam()
	self:_refreshOnce()
	self:_refresh()
end

function V2a7_WarmUp:onOpen()
...
function V2a7_WarmUp:_refreshRightView()
...
function V2a7_WarmUp:_setActive_goWrongChannel(isBlockDesc)
...
function V2a7_WarmUp:_refreshData()
...
function V2a7_WarmUp:_refreshTabList()
	...
	for i = 1, maxEpisodeCount do
		...
		if i > #self._itemTabList then
			item = self:_create_V2a7_WarmUp_radiotaskitem(i)
...
function V2a7_WarmUp:_setSelectIndex(index, isFocus)
...
function V2a7_WarmUp:_taskScrollToIndex(index)
...
function V2a7_WarmUp:onClickTab(mo)
...
function V2a7_WarmUp:_refreshRewards()
	local co = self.viewContainer:getEpisodeConfigCur()
	...
	for i = 1, itemCount do
		...
		if i > #self._rewardItemList then
			item = self:_create_V2a7_WarmUp_rewarditem(i)
...
function V2a7_WarmUp:openDesc(cb, cbObj)
...
function V2a7_WarmUp:_tweenDescUpdateCb(value)
	local bottom = lerp(0, self._taskDescViewportHeight, value)

	self:_setMaskPaddingBottom(bottom)
end

function V2a7_WarmUp:_tweenDescEndCb(maskDurationTime, cb, cbObj)
...
function V2a7_WarmUp:_resetTaskContentPos()
	recthelper.setAnchorY(self._txtTaskContentTran, 0)
end

function V2a7_WarmUp:episode2Index(episodeId)
	return episodeId
end

function V2a7_WarmUp:index2EpisodeId(index)
...
function V2a7_WarmUp:_setMaskPaddingBottom(bottom)
	self._taskDescMask.padding = Vector4(0, bottom, 0, 0)
end

function V2a7_WarmUp:_autoSelectTab()
...
function V2a7_WarmUp:_create_V2a7_WarmUp_radiotaskitem(index)
	local go = gohelper.cloneInPlace(self._goradiotaskitem)
	local item = V2a7_WarmUp_radiotaskitem.New({
		parent = self,
...
function V2a7_WarmUp:_create_V2a7_WarmUp_rewarditem(index)
	local go = gohelper.cloneInPlace(self._gorewarditem)
	local item = V2a7_WarmUp_rewarditem.New({
		parent = self,
		baseViewContainer = self.viewContainer
	})
...
function V2a7_WarmUp:_resetTweenDescPos()
	GameUtil.onDestroyViewMember_TweenId(self, "_movetweenId")
	GameUtil.onDestroyViewMember_TweenId(self, "_tweenId")
...
function V2a7_WarmUp:_playAnim(name, cb, cbObj)
	self._animatorPlayer:Play(name, cb, cbObj)
end

function V2a7_WarmUp:tweenSwitch(cb, cbObj)
	self:_playAnim(UIAnimationName.Switch, cb, cbObj)
end

function V2a7_WarmUp:_onSwitch()
	local lastEpisodeId = self.viewContainer:getCurSelectedEpisode()
	local curEpisodeId
...
function V2a7_WarmUp:playRewardItemsHasGetAnim()
