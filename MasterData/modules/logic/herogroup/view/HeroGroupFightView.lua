...
function HeroGroupFightView:_editableInitView()
	...
	self.addEventCb(self, ActivityController.instance, ActivityEvent.RefreshDoubleDropInfo, self.refreshDropTips, self)
	self.addEventCb(self, Activity217Controller.instance, Activity217Event.OnInfoChanged, self.refreshDropTips, self)
...
function HeroGroupFightView:_refreshUI()
	...
	local var_40_0 = self._goReplayBtn

	gohelper.setActive(var_40_0, (not HeroGroupBalanceHelper.getIsBalanceMode() or nil) and self.episodeConfig and not (self.episodeConfig.canUseRecord ~= 1 or self._chapterConfig.type == DungeonEnum.ChapterType.WeekWalk))
end
...
function HeroGroupFightView:_refreshBtns(isCostPower)
	...
	if not self._enterAfterFreeLimit then
		...
		if isCostPower then
			...
			var_69_1 = self._replayMode and self._chapterConfig.type ~= DungeonEnum.ChapterType.Hard
		end

		gohelper.setActive(self._btnstartreplay.gameObject, var_69_1)
		...
		if isCostPower then
			...
			var_69_2 = self._replayMode and self._chapterConfig.type == DungeonEnum.ChapterType.Hard
		end

		gohelper.setActive(self._btnhardreplay.gameObject, var_69_2)
