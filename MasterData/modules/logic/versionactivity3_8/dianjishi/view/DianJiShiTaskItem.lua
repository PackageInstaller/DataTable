-- chunkname: @modules/logic/versionactivity3_8/dianjishi/view/DianJiShiTaskItem.lua

module("modules.logic.versionactivity3_8.dianjishi.view.DianJiShiTaskItem", package.seeall)

local DianJiShiTaskItem = class("DianJiShiTaskItem", ListScrollCellExtend)

function DianJiShiTaskItem:onInitView()
...
function DianJiShiTaskItem:addEvents()
...
function DianJiShiTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function DianJiShiTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.DianJiShiTaskView)
	end
end

function DianJiShiTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("DianJiShiTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function DianJiShiTaskItem:_btngetallOnClick()
	DianJiShiGameController.instance:dispatchEvent(DianJiShiGameEvent.OneClickClaimReward, self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("DianJiShiTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function DianJiShiTaskItem:_delayFinish()
...
function DianJiShiTaskItem:_editableAddEvents()
	DianJiShiGameController.instance:registerCallback(DianJiShiGameEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function DianJiShiTaskItem:_editableRemoveEvents()
	DianJiShiGameController.instance:unregisterCallback(DianJiShiGameEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function DianJiShiTaskItem:_onOneClickClaimReward(actId)
...
function DianJiShiTaskItem:getAnimator()
	return self._animator
end

function DianJiShiTaskItem:onUpdateMO(mo)
	self._taskMo = mo

	local rankDiff = DianJiShiTaskListModel.instance:getRankDiff(mo)
...
function DianJiShiTaskItem:_moveByRankDiff(rankDiff)
...
function DianJiShiTaskItem:onSelect(isSelect)
	return
end

function DianJiShiTaskItem:_refreshUI()
...
function DianJiShiTaskItem:_onItemShow(cell_component, data, index)
...
function DianJiShiTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
