-- chunkname: @modules/logic/versionactivity2_2/enter/view/VersionActivity2_2EnterBgmView.lua

module("modules.logic.versionactivity2_2.enter.view.VersionActivity2_2EnterBgmView", package.seeall)

local VersionActivity2_2EnterBgmView = class("VersionActivity2_2EnterBgmView", BaseView)

function VersionActivity2_2EnterBgmView:onInitView()
	if self._editableInitView then
		self:_editableInitView()
	end
end

function VersionActivity2_2EnterBgmView:addEvents()
...
function VersionActivity2_2EnterBgmView:removeEvents()
...
function VersionActivity2_2EnterBgmView:_editableInitView()
	self:initActHandle()
end

function VersionActivity2_2EnterBgmView:initActHandle()
	self.actHandleDict = self.actHandleDict or {
		[BossRushConfig.instance:getActivityId()] = self._bossrushBgmHandle,
		[VersionActivity2_2Enum.ActivityId.Lopera] = self._loperaBgmHandle
	}
end

function VersionActivity2_2EnterBgmView:onOpen()
	if not self.viewParam.activitySettingList then
		...
		self._isFirstOpenMainAct = actId == VersionActivity2_2Enum.ActivityId.Dungeon
...
function VersionActivity2_2EnterBgmView:onSelectActId(actId)
	self:modifyBgm(actId)
end

function VersionActivity2_2EnterBgmView:modifyBgm(actId)
	self._isMainAct = actId == VersionActivity2_2Enum.ActivityId.Dungeon
...
function VersionActivity2_2EnterBgmView:_doModifyBgm()
	if not self.actHandleDict[self._actId] then
		-- block empty
	end
end

function VersionActivity2_2EnterBgmView:defaultBgmHandle(actId)
	...
	AudioBgmManager.instance:modifyBgmAudioId(AudioBgmEnum.Layer.VersionActivity2_2Main, bgmId)
end

function VersionActivity2_2EnterBgmView:onClose()
	TaskDispatcher.cancelTask(self._doModifyBgm, self)
end

function VersionActivity2_2EnterBgmView:_bossrushBgmHandle(actId)
	self.playingActId = actId
...
function VersionActivity2_2EnterBgmView:_loperaBgmHandle(actId)
	self.playingActId = actId
...
function VersionActivity2_2EnterBgmView:_reactivityBgmHandle(actId)
	self.playingActId = actId
...
return VersionActivity2_2EnterBgmView
