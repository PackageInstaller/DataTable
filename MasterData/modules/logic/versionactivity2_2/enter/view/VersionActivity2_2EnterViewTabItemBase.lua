-- chunkname: @modules/logic/versionactivity2_2/enter/view/VersionActivity2_2EnterViewTabItemBase.lua

module("modules.logic.versionactivity2_2.enter.view.VersionActivity2_2EnterViewTabItemBase", package.seeall)

local VersionActivity2_2EnterViewTabItemBase = class("VersionActivity2_2EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)

function VersionActivity2_2EnterViewTabItemBase:_editableInitView()
...
function VersionActivity2_2EnterViewTabItemBase:_getTagPath()
	return "#txt_name/#go_tag"
end

function VersionActivity2_2EnterViewTabItemBase:afterSetData()
	if self.actId then
	...
	if not self.activityCo then
		...
		logError("VersionActivity2_2EnterViewTabItemBase.afterSetData error, no act config, actId:%s", self.actId)
	...
	if not string.nilorempty(selectBg) then
		UISpriteSetMgr.instance:setV2a2MainActivitySprite(self.imageSelectTabIcon, selectBg)
	end

	local unselectBg = iconCoArr[2]

	if not string.nilorempty(unselectBg) then
		UISpriteSetMgr.instance:setV2a2MainActivitySprite(self.imageUnSelectTabIcon, unselectBg)
	end

	self.redDotIcon = RedDotController.instance:addRedDot(self.goRedDot, self.activityCo.redDotId, self.redDotUid)
end

function VersionActivity2_2EnterViewTabItemBase:childRefreshSelect()
	gohelper.setActive(self.goSelected, self.isSelect)
	gohelper.setActive(self.goUnselected, not self.isSelect)
end

function VersionActivity2_2EnterViewTabItemBase:childRefreshUI()
...
function VersionActivity2_2EnterViewTabItemBase:refreshTag()
	self:clearTag()
...
function VersionActivity2_2EnterViewTabItemBase:clearTag()
...
function VersionActivity2_2EnterViewTabItemBase:refreshNormalTag()
	...
	local isShowRemainTime = VersionActivity2_2EnterHelper.GetIsShowTabRemainTime(self.actId)

	if isShowRemainTime and actInfoMo then
		local offsetSecond = actInfoMo:getRealEndTimeStamp() - ServerTime.now()

		if offsetSecond > VersionActivity2_2Enum.MaxShowTimeOffset then
			return
		end
...
function VersionActivity2_2EnterViewTabItemBase:refreshNotUnlockTag()
	gohelper.setActive(self.goTagLock, false)
...
function VersionActivity2_2EnterViewTabItemBase:refreshLockTag()
...
function VersionActivity2_2EnterViewTabItemBase:isShowRedDot()
	return self.redDotIcon and self.redDotIcon.show
end

return VersionActivity2_2EnterViewTabItemBase
