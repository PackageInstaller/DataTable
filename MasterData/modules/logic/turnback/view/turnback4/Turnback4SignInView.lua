-- chunkname: @modules/logic/turnback/view/turnback4/Turnback4SignInView.lua

module("modules.logic.turnback.view.turnback4.Turnback4SignInView", package.seeall)

local Turnback4SignInView = class("Turnback4SignInView", BaseView)

function Turnback4SignInView:onInitView()
...
function Turnback4SignInView:addEvents()
...
function Turnback4SignInView:removeEvents()
...
function Turnback4SignInView:_btnvideobtnOnClick()
	local TurnbackMo = TurnbackModel.instance:getCurTurnbackMo()

	if TurnbackMo then
		::label_4_0::

		local var_4_0 = TurnbackMo.config
...
function Turnback4SignInView:_btnvideoendbtnOnClick()
	local TurnbackMo = TurnbackModel.instance:getCurTurnbackMo()

	if TurnbackMo then
		::label_5_0::

		local var_5_0 = TurnbackMo.config

		if TurnbackMo.config then
			local storyId = TurnbackMo.config.endStory
...
function Turnback4SignInView:_editableInitView()
	self._signItems = {}
	self._turnbackId = TurnbackModel.instance:getCurTurnbackId()

	local info = TurnbackEnum.SignInSpecialDays[self._turnbackId]

	for day = 1, 7 do
		local item = self:getUserDataTb_()
		local parentGO = gohelper.findChild(self._gocontent, "pos" .. day)
		local isSpecial = info and info[day]

		item.go = isSpecial and gohelper.findChild(parentGO, "#go_item") or gohelper.clone(self._gocontentitem, parentGO, "day" .. day)
...
function Turnback4SignInView:_initItem()
	return
end

function Turnback4SignInView:onUpdateParam()
	return
end

function Turnback4SignInView:onOpen()
	self._turnbackId = TurnbackModel.instance:getCurTurnbackId()
...
function Turnback4SignInView:_playAnim()
	TaskDispatcher.cancelTask(self._playAnim, self)

	self._animator.speed = 1
end

function Turnback4SignInView:refreshItem()
	for day, item in ipairs(self._signItems) do
		item.cls:initItem(day)
	end
end

function Turnback4SignInView:_refreshUI()
	self:_checkShowSotry()
end

function Turnback4SignInView:_checkShowSotry()
	...
	if isAllReceive then
		...
		if config then
			...
			local canPlay = not StoryModel.instance:isStoryFinished(config.endStory)
...
function Turnback4SignInView:_onCloseViewFinish(viewName)
	if viewName == ViewName.Turnback3SignInRoleTalkView then
		self:_checkShowSotry()
	end
end

function Turnback4SignInView:onClose()
	TaskDispatcher.cancelTask(self._playAnim, self)
end

function Turnback4SignInView:onDestroyView()
	TaskDispatcher.cancelTask(self._playAnim, self)
end

return Turnback4SignInView
