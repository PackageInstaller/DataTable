-- chunkname: @modules/logic/versionactivity3_7/enter/view/VersionActivity3_7EnterViewTabItemBase.lua

module("modules.logic.versionactivity3_7.enter.view.VersionActivity3_7EnterViewTabItemBase", package.seeall)

local VersionActivity3_7EnterViewTabItemBase = class("VersionActivity3_7EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)

function VersionActivity3_7EnterViewTabItemBase:_editableInitView()
...
function VersionActivity3_7EnterViewTabItemBase:_getTagPath()
	return "#go_tag"
end

function VersionActivity3_7EnterViewTabItemBase:afterSetData()
	if self.actId then
	...
	if not self.activityCo then
		...
		logError("VersionActivity3_7EnterViewTabItemBase.afterSetData error, no act config, actId:%s", self.actId)
	...
	if not string.nilorempty(selectBg) then
		UISpriteSetMgr.instance:setV3a7MainActivitySprite(self.imageSelectTabIcon, selectBg)
	end

	local unselectBg = iconCoArr[2]

	if not string.nilorempty(unselectBg) then
		UISpriteSetMgr.instance:setV3a7MainActivitySprite(self.imageUnSelectTabIcon, unselectBg)
	end

	self.redDotIcon = RedDotController.instance:addRedDot(self.goRedDot, self.activityCo.redDotId, self.redDotUid)
end

function VersionActivity3_7EnterViewTabItemBase:childRefreshSelect()
...
function VersionActivity3_7EnterViewTabItemBase:childRefreshUI()
...
function VersionActivity3_7EnterViewTabItemBase:refreshTag()
	self:clearTag()
...
function VersionActivity3_7EnterViewTabItemBase:clearTag()
...
function VersionActivity3_7EnterViewTabItemBase:refreshNormalTag()
	...
	local isShowRemainTime = VersionActivity3_7EnterHelper.GetIsShowTabRemainTime(self.actId)

	if isShowRemainTime and actInfoMo then
		local offsetSecond = actInfoMo:getRealEndTimeStamp() - ServerTime.now()

		if offsetSecond > VersionActivity3_7Enum.MaxShowTimeOffset then
			return
		end
...
function VersionActivity3_7EnterViewTabItemBase:refreshNotUnlockTag()
	gohelper.setActive(self.goTagLock, false)
...
function VersionActivity3_7EnterViewTabItemBase:refreshLockTag()
...
function VersionActivity3_7EnterViewTabItemBase:isShowRedDot()
	return self.redDotIcon and self.redDotIcon.show
end

return VersionActivity3_7EnterViewTabItemBase
