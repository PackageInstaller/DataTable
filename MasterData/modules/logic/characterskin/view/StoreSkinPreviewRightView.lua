...
function StoreSkinPreviewRightView:_onViewDragEnd(param, pointerEventData)
	...
	if newSelectSkinIndex then
		...
		local showDynamicVertical = PlayerModel.instance:getMyUserId() % 2 == 0
...
function StoreSkinPreviewRightView:_onPayFinished()
	...
	if alreadyHas then
		...
		self._goodsPriceInfo = StoreHelper.getSkinGoodsPriceInfo(goodsConfig, (self._skinCo or nil) and self._skinCo.id)
