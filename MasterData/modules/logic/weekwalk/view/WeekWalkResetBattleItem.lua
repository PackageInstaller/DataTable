-- chunkname: @modules/logic/weekwalk/view/WeekWalkResetBattleItem.lua

module("modules.logic.weekwalk.view.WeekWalkResetBattleItem", package.seeall)

local WeekWalkResetBattleItem = class("WeekWalkResetBattleItem", ListScrollCellExtend)

function WeekWalkResetBattleItem:onInitView()
...
function WeekWalkResetBattleItem:addEvents()
	self._btnclick:AddClickListener(self._btnclickOnClick, self)
end

function WeekWalkResetBattleItem:removeEvents()
	self._btnclick:RemoveClickListener()
end

function WeekWalkResetBattleItem:_btnclickOnClick()
...
function WeekWalkResetBattleItem:setSelect(value)
	gohelper.setActive(self._goselected, value)
end

function WeekWalkResetBattleItem:ctor(param)
...
function WeekWalkResetBattleItem:getBattleInfo()
	return self._battleInfo
end

function WeekWalkResetBattleItem:getPrevBattleInfo()
	return self._battleInfos[self._index - 1]
end

function WeekWalkResetBattleItem:_editableInitView()
...
function WeekWalkResetBattleItem:_setStarStatus(value)
	local starNum = self._mapInfo:getStarNumConfig()
	...
	for i = 1, starNum do
		...
		gohelper.setActive(gohelper.findChild(star, "full"), i <= value)
	end
end

function WeekWalkResetBattleItem:_setNormalStatus()
	...
	self:_setStarStatus(self._battleInfo.star)
end

function WeekWalkResetBattleItem:_setFakedStatus(curFaked)
...
function WeekWalkResetBattleItem:setFakedReset(isFaked, curFaked)
	if isFaked then
...
function WeekWalkResetBattleItem:_updateFinishLine(isFinish)
...
function WeekWalkResetBattleItem:_initBattleStatus(prevFinish, isFinish)
...
function WeekWalkResetBattleItem:_getPrevFinish()
	local prevBattleInfo = self._battleInfos[self._index - 1]

	return not prevBattleInfo or prevBattleInfo.star > 0
end

function WeekWalkResetBattleItem:_editableAddEvents()
	return
end

function WeekWalkResetBattleItem:_editableRemoveEvents()
	return
end

function WeekWalkResetBattleItem:onDestroyView()
	return
end

return WeekWalkResetBattleItem
