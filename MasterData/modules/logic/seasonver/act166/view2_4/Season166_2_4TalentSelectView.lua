-- chunkname: @modules/logic/seasonver/act166/view2_4/Season166_2_4TalentSelectView.lua

module("modules.logic.seasonver.act166.view2_4.Season166_2_4TalentSelectView", package.seeall)

local Season166_2_4TalentSelectView = class("Season166_2_4TalentSelectView", BaseView)

function Season166_2_4TalentSelectView:onInitView()
...
function Season166_2_4TalentSelectView:addEvents()
	self.addEventCb(self, Season166Controller.instance, Season166Event.SetTalentSkill, self.OnSetTalentSkill, self)
end

function Season166_2_4TalentSelectView:removeEvents()
	return
end
...
function Season166_2_4TalentSelectView:_editableInitView()
...
function Season166_2_4TalentSelectView:onOpen()
...
function Season166_2_4TalentSelectView:onClose()
	self:saveUnlockState()
end

function Season166_2_4TalentSelectView:refreshTalentParam(talentInfo)
	self.talentLvl = talentInfo.level
	self.maxSlot = talentInfo.config.slot
end

function Season166_2_4TalentSelectView:_initLeftArea()
...
function Season166_2_4TalentSelectView:_initMiddleArea()
...
function Season166_2_4TalentSelectView:_initSkillItem()
	...
	for i = 1, 6 do
		...
		if skillId then
			...
			txtlocktips.text = GameUtil.getSubPlaceholderLuaLangTwoParam(luaLang("season166_talent_selectlock"), needStar, self.baseConfig.name)
			skillItem.anim = go:GetComponent(gohelper.Type_Animator)

			local btnClick = gohelper.findChildButtonWithAudio(go, "click")

			self.addClickCb(self, btnClick, self._clickItem, self, skillId)
...
function Season166_2_4TalentSelectView:_refreshSelectSkill(isAdd)
...
function Season166_2_4TalentSelectView:_refreshMiddlSlot()
...
function Season166_2_4TalentSelectView:_refreshSkillItemStatus()
	for id, skillItem in pairs(self.skillItemDic) do
		...
		skillItem.canvasGroup.alpha = (status == SkillItemStatus.Full or status == SkillItemStatus.Lock) and 0.5 or 1
		...
		if isUnlock then
			self.unlockStateTab[id] = Season166Enum.UnlockState or Season166Enum.LockState
		end
...
function Season166_2_4TalentSelectView:_clickItem(skillId)
	local unlockLvl = self:getSkillUnlockLvl(skillId)
...
function Season166_2_4TalentSelectView:OnSetTalentSkill(talentId, isAdd)
...
function Season166_2_4TalentSelectView:InferSkillStatus(skillId, unlockLvl)
...
function Season166_2_4TalentSelectView:getSkillUnlockLvl(skillId)
...
function Season166_2_4TalentSelectView:playUnlockEffect()
...
function Season166_2_4TalentSelectView:isTalentSelect(skillId)
	local curSelectSkills = self.talentInfo.skillIds

	return tabletool.indexOf(curSelectSkills, skillId)
end

function Season166_2_4TalentSelectView:saveUnlockState()
	local saveStrTab = {}
...
function Season166_2_4TalentSelectView:clcikHyperLink(effectId, clickPosition)
	CommonBuffTipController.instance:openCommonTipViewWithCustomPos(effectId, Vector2(-305, 30))
end

return Season166_2_4TalentSelectView
