-- chunkname: @modules/logic/seasonver/act166/view2_6/Season166_2_6InformationReportItem.lua

module("modules.logic.seasonver.act166.view2_6.Season166_2_6InformationReportItem", package.seeall)

local Season166_2_6InformationReportItem = class("Season166_2_6InformationReportItem", LuaCompBase)

function Season166_2_6InformationReportItem:init(go)
	self:__onInit()
...
function Season166_2_6InformationReportItem:addEventListeners()
	...
	Season166Controller.instance:registerCallback(Season166Event.ClickInfoReportItem, self.onClickInfoReportItem, self)
end

function Season166_2_6InformationReportItem:onClickInfoReportItem(param)
	if not param or param.infoId ~= self.infoId then
		return
	end

	self:refreshUnlockState(param.unlockState)
end

function Season166_2_6InformationReportItem:onClickItem()
...
function Season166_2_6InformationReportItem:refreshUI(config)
	self.config = config
	...
	if infoMo then
		self.unlockState = Season166Enum.UnlockState or Season166Enum.LockState
	end
...
function Season166_2_6InformationReportItem:playOpenAnim()
...
function Season166_2_6InformationReportItem:refreshSchdule(info)
	if not info then
...
function Season166_2_6InformationReportItem:_setImgValue(value)
...
function Season166_2_6InformationReportItem:createSchduleItem(index)
...
function Season166_2_6InformationReportItem:refreshSchduleItem(item, analyConfig, curStage)
...
function Season166_2_6InformationReportItem:refreshUnlockState(saveUnlockState)
	self.canShowNew = self.unlockState == Season166Enum.UnlockState and self.unlockState ~= saveUnlockState

	self:refreshReddot()
end

function Season166_2_6InformationReportItem:refreshUnlockAnimState(tab)
	if not tab[self.infoId] then
		if tab[self.infoId] == Season166Enum.LockState and self.unlockState == Season166Enum.UnlockState then
			local actInfo = Season166Model.instance:getActInfo(self.activityId)
			local infoMo = actInfo and actInfo:getInformationMO(self.infoId)
...
function Season166_2_6InformationReportItem:_playUnlockAnim()
...
function Season166_2_6InformationReportItem:onUnlockAnimPlayFinish()
	gohelper.setActive(self.goLockIcon, false)
end

function Season166_2_6InformationReportItem:refreshFinishAnimState(tab)
	if self.unlockState == Season166Enum.LockState then
		return
	end

	if not tab[self.infoId] then
		local state = Season166Enum.LockState
		local analys = Season166Config.instance:getSeasonInfoAnalys(self.activityId, self.infoId) or {}
		local actInfo = Season166Model.instance:getActInfo(self.activityId)
		local infoMo = actInfo and actInfo:getInformationMO(self.infoId)
...
function Season166_2_6InformationReportItem:onFinishAnimPlayFinish()
	return
end

function Season166_2_6InformationReportItem:refreshReddot()
...
function Season166_2_6InformationReportItem:checkReddotShow(redDotIcon)
...
function Season166_2_6InformationReportItem:removeEventListeners()
	...
	Season166Controller.instance:unregisterCallback(Season166Event.ClickInfoReportItem, self.onClickInfoReportItem, self)
end

function Season166_2_6InformationReportItem:onDestroy()
...
return Season166_2_6InformationReportItem
