-- chunkname: @modules/logic/seasonver/act166/view/Season166HeroGroupEditView.lua

module("modules.logic.seasonver.act166.view.Season166HeroGroupEditView", package.seeall)

local Season166HeroGroupEditView = class("Season166HeroGroupEditView", BaseView)

function Season166HeroGroupEditView:onInitView()
	...
	self._btnassistWithTalent = gohelper.findChildButtonWithAudio(self.viewGO, "characterinfo/#go_characterinfo/levelwithtalent/#btn_assistwithtalent")
...
function Season166HeroGroupEditView:addEvents()
	...
	self._btnassistWithTalent:AddClickListener(self._btncharacterOnClick, self)
...
function Season166HeroGroupEditView:removeEvents()
	...
	self._btnassistWithTalent:RemoveClickListener()
...
function Season166HeroGroupEditView:_btncloseFilterViewOnClick()
...
function Season166HeroGroupEditView:_btnclassifyOnClick()
	gohelper.setActive(self._gosearchfilter, true)
	self:_refreshFilterView()
end

function Season166HeroGroupEditView:_btnresetOnClick()
	for i = 1, 6 do
...
function Season166HeroGroupEditView:_btnokOnClick()
	gohelper.setActive(self._gosearchfilter, false)
...
function Season166HeroGroupEditView:_btnpassiveskillOnClick()
...
function Season166HeroGroupEditView:_btnconfirmOnClick()
	if self._isShowQuickEdit then
		self:_saveQuickGroupInfo()

		local isAssistHeroInTeam = Season166HeroSingleGroupModel.instance:isAssistHeroInTeam()

		if not isAssistHeroInTeam then
	...
	local singleGroupMO = Season166HeroSingleGroupModel.instance:getById(self._singleGroupMOId)
	...
	if self._heroMO then
		...
		local hasHero, hasHeroIndex = Season166HeroSingleGroupModel.instance:hasHeroUids(self._heroMO.uid, self._singleGroupMOId)

		if hasHero then
			Season166HeroSingleGroupModel.instance:removeFrom(hasHeroIndex)
			Season166HeroSingleGroupModel.instance:addTo(self._heroMO.uid, self._singleGroupMOId)
		...
		if Season166HeroSingleGroupModel.instance:isAidConflict(self._heroMO.heroId) then
		...
		if assistMO and singleGroupMO.heroUid == assistMO.heroUid then
			Season166HeroGroupModel.instance:cleanAssistData()
			HeroGroupController.instance:dispatchEvent(HeroGroupEvent.OnModifyHeroGroup)
		end

		Season166HeroSingleGroupModel.instance:removeFrom(self._singleGroupMOId)
		self:_saveCurGroupInfo()
		self:closeThis()
	end
end

function Season166HeroGroupEditView:_btncancelOnClick()
	self:closeThis()
end

function Season166HeroGroupEditView:_btncharacterOnClick()
	if self._heroMO then
		local heroMoList = {}

		heroMoList = self._isShowQuickEdit and Season166HeroGroupQuickEditModel.instance:getList() or Season166HeroGroupEditModel.instance:getList()

		if self._heroMO:isOtherPlayerHero() then
			local equipMO = Season166HeroGroupEditModel.instance:getEquipMOByHeroUid(self._heroMO.uid)
...
function Season166HeroGroupEditView:_btnattributeOnClick()
	if self._heroMO then
...
function Season166HeroGroupEditView:_btnexskillrankOnClick()
	local x, y = transformhelper.getLocalPos(self._goScrollContent.transform)
...
function Season166HeroGroupEditView:_btnlvrankOnClick()
	local x, y = transformhelper.getLocalPos(self._goScrollContent.transform)
...
function Season166HeroGroupEditView:_btnrarerankOnClick()
	local x, y = transformhelper.getLocalPos(self._goScrollContent.transform)
...
function Season166HeroGroupEditView:_btnquickeditOnClick()
	...
	if self._isShowQuickEdit then
		...
		local mo = Season166HeroGroupQuickEditModel.instance:getById(self._originalHeroUid)

		if mo then
			local index = Season166HeroGroupQuickEditModel.instance:getIndex(mo)

			Season166HeroGroupQuickEditModel.instance:selectCell(index, true)
		end
	else
		self:_saveQuickGroupInfo()
		self:_onHeroItemClick(nil)
		...
		local curHeroUid = Season166HeroSingleGroupModel.instance:getHeroUid(self._singleGroupMOId)

		if curHeroUid ~= "0" then
			local mo = Season166HeroGroupEditModel.instance:getById(curHeroUid)
			local index = Season166HeroGroupEditModel.instance:getIndex(mo)

			Season166HeroGroupEditModel.instance:selectCell(index, true)
		end
...
function Season166HeroGroupEditView:_attrBtnOnClick(i)
	self._selectAttrs[i] = not self._selectAttrs[i]

	self:_refreshFilterView()
end

function Season166HeroGroupEditView:_dmgBtnOnClick(i)
...
function Season166HeroGroupEditView:_locationBtnOnClick(i)
	self._selectLocations[i] = not self._selectLocations[i]

	self:_refreshFilterView()
end

function Season166HeroGroupEditView:_onHeroItemClick(heroMO)
	self._heroMO = heroMO

	self:_refreshCharacterInfo()
end

function Season166HeroGroupEditView:_refreshCharacterInfo()
	if self._heroMO then
...
function Season166HeroGroupEditView:_refreshMainInfo()
	if self._heroMO then
		...
		if (not OpenModel.instance:isFunctionUnlock(OpenEnum.UnlockFunc.Talent) or nil) and false then
		...
		gohelper.setActive(self._btnassistWithTalent.gameObject, not isOwnHero)
	end
end

function Season166HeroGroupEditView:_refreshAttribute()
	if self._heroMO then
...
function Season166HeroGroupEditView:_refreshPassiveSkill()
	if not self._heroMO then
		return
	end
...
function Season166HeroGroupEditView:_refreshSkill()
	self._skillContainer:onUpdateMO(self._heroMO and self._heroMO.heroId, false, self._heroMO)
end

function Season166HeroGroupEditView:_refreshBtnIcon()
...
function Season166HeroGroupEditView:_refreshFilterView()
	for i = 1, 2 do
...
function Season166HeroGroupEditView:_updateHeroList()
	local dmgs = {}
...
function Season166HeroGroupEditView:replaceSelectHeroDefaultEquip()
...
function Season166HeroGroupEditView:replaceHeroesDefaultEquip(heroUids)
	local heroGroupMo = Season166HeroGroupModel.instance:getCurGroupMO()
	local heroGroupEquipMoList = heroGroupMo.equips
	local heroMo
...
function Season166HeroGroupEditView:_saveCurGroupInfo()
...
function Season166HeroGroupEditView:_saveQuickGroupInfo()
	if Season166HeroGroupQuickEditModel.instance:getIsDirty() then
		local newHeroUids = Season166HeroGroupQuickEditModel.instance:getHeroUids()
		local heroGroupMO = Season166HeroGroupModel.instance:getCurGroupMO()

		self:replaceHeroesDefaultEquip(newHeroUids)

		for i = 1, Season166HeroGroupModel.instance:getBattleRoleNum() do
			local heroUid = newHeroUids[i]

			if heroUid ~= nil then
				Season166HeroSingleGroupModel.instance:addTo(heroUid, i)

				local singleGroupMO = Season166HeroSingleGroupModel.instance:getByIndex(i)
...
function Season166HeroGroupEditView:_onAttributeChanged(level, heroId)
	CharacterModel.instance:setFakeLevel(heroId, level)
end

function Season166HeroGroupEditView:_normalEditHasChange()
	return true
end

function Season166HeroGroupEditView:_refreshEditMode()
...
function Season166HeroGroupEditView:_refreshCurScrollBySort()
	if self._isShowQuickEdit then
		if Season166HeroGroupQuickEditModel.instance:getIsDirty() then
			self:_saveQuickGroupInfo()
		end
...
function Season166HeroGroupEditView:_onGroupModify()
	if self._isShowQuickEdit then
		Season166HeroGroupQuickEditModel.instance:copyQuickEditCardList()
	else
		local heroUid = Season166HeroSingleGroupModel.instance:getHeroUid(self._singleGroupMOId)

		if self._originalHeroUid ~= heroUid then
			self._originalHeroUid = heroUid

			Season166HeroGroupEditModel.instance:setParam(heroUid)
			self:_onHeroItemClick(nil)
			Season166HeroGroupEditModel.instance:cancelAllSelected()

			local mo = Season166HeroGroupEditModel.instance:getById(heroUid)
			local index = Season166HeroGroupEditModel.instance:getIndex(mo)
...
function Season166HeroGroupEditView:_editableInitView()
...
function Season166HeroGroupEditView:onOpen()
	...
	Season166HeroGroupEditModel.instance:setParam(self._originalHeroUid)

	self._heroMO = Season166HeroGroupEditModel.instance:copyCharacterCardList(true)
...
function Season166HeroGroupEditView:onClose()
...
function Season166HeroGroupEditView:_onOpenView(viewName)
...
function Season166HeroGroupEditView:_onCloseView(viewName)
...
function Season166HeroGroupEditView:_delyStopBgm()
	self._isStopBgm = false

	AudioMgr.instance:trigger(AudioEnum.Bgm.Pause_FightingMusic)
end

function Season166HeroGroupEditView:_showCharacterRankUpView(func)
	func()
end

function Season166HeroGroupEditView:onDestroyView()
...
function Season166HeroGroupEditView:_showRecommendCareer()
...
function Season166HeroGroupEditView:_onRecommendCareerItemShow(obj, data, index)
	local icon = gohelper.findChildImage(obj, "icon")

	UISpriteSetMgr.instance:setHeroGroupSprite(icon, "career_" .. data)
end

return Season166HeroGroupEditView
