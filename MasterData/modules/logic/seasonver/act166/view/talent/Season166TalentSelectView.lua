-- chunkname: @modules/logic/seasonver/act166/view/talent/Season166TalentSelectView.lua

module("modules.logic.seasonver.act166.view.talent.Season166TalentSelectView", package.seeall)

local Season166TalentSelectView = class("Season166TalentSelectView", BaseView)

function Season166TalentSelectView:onInitView()
...
function Season166TalentSelectView:_editableInitView()
...
function Season166TalentSelectView:onOpen()
	if self.viewParam and self.viewParam.talentId then
		...
		local basicSkillIds = string.splitToNumber(self.talentConfig.baseSkillIds, "#")
...
function Season166TalentSelectView:onClose()
	self:saveUnlockState()
end

function Season166TalentSelectView:refreshTalentParam(talentInfo)
	self.talentLvl = talentInfo.level
	self.maxSlot = talentInfo.config.slot
end

function Season166TalentSelectView:_initLeftArea()
...
function Season166TalentSelectView:_initMiddleArea()
...
function Season166TalentSelectView:_initSkillItem()
	...
	for _, styleCfg in ipairs(self.styleCfgDic) do
		...
		self.skillUnlockLvlDic[styleCfg.level] = skillIds
	end
...
function Season166TalentSelectView:_refreshSelectSkill(isAdd)
...
function Season166TalentSelectView:_refreshMiddlSlot()
...
function Season166TalentSelectView:_refreshSkillItemStatus()
...
function Season166TalentSelectView:_clickItem(skillId)
	local unlockLvl = self:getSkillUnlockLvl(skillId)
...
function Season166TalentSelectView:OnSetTalentSkill(talentId, isAdd)
...
function Season166TalentSelectView:InferSkillStatus(skillId, unlockLvl)
...
function Season166TalentSelectView:getSkillUnlockLvl(skillId)
...
function Season166TalentSelectView:playUnlockEffect()
...
function Season166TalentSelectView:isTalentSelect(skillId)
	local curSelectSkills = self.talentInfo.skillIds

	return tabletool.indexOf(curSelectSkills, skillId)
end

function Season166TalentSelectView:saveUnlockState()
	local saveStrTab = {}
...
function Season166TalentSelectView:clcikHyperLink(effectId, clickPosition)
	CommonBuffTipController.instance:openCommonTipViewWithCustomPos(effectId, Vector2(-305, 30))
end

return Season166TalentSelectView
