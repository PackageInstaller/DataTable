-- chunkname: @modules/logic/seasonver/act123/view1_9/Season123_1_9EquipBookView.lua

module("modules.logic.seasonver.act123.view1_9.Season123_1_9EquipBookView", package.seeall)

local Season123_1_9EquipBookView = class("Season123_1_9EquipBookView", BaseView)

function Season123_1_9EquipBookView:onInitView()
...
function Season123_1_9EquipBookView:addEvents()
	...
	self.addEventCb(self, Season123EquipBookController.instance, Season123Event.OnEquipBookItemChangeSelect, self.onChangeSelectCard, self)
	self.addEventCb(self, Season123EquipBookController.instance, Season123Event.OnRefleshEquipBookView, self.refreshEquipBook, self)
	self.addEventCb(self, Season123EquipBookController.instance, Season123Event.OnItemChange, self.refreshUI, self)
	self.addEventCb(self, CurrencyController.instance, CurrencyEvent.CurrencyChange, self.refreshUI, self)
	self.addEventCb(self, Season123EquipBookController.instance, Season123Event.OnDecomposeEffectPlay, self.doDecompose, self)
end

function Season123_1_9EquipBookView:removeEvents()
	...
	self.removeEventCb(self, Season123EquipBookController.instance, Season123Event.OnEquipBookItemChangeSelect, self.onChangeSelectCard, self)
	self.removeEventCb(self, Season123EquipBookController.instance, Season123Event.OnRefleshEquipBookView, self.refreshEquipBook, self)
	self.removeEventCb(self, Season123EquipBookController.instance, Season123Event.OnItemChange, self.refreshUI, self)
	self.removeEventCb(self, CurrencyController.instance, CurrencyEvent.CurrencyChange, self.refreshUI, self)
	self.removeEventCb(self, Season123EquipBookController.instance, Season123Event.OnDecomposeEffectPlay, self.doDecompose, self)
end

Season123_1_9EquipBookView.defaultDescColor = "#cac8c5"

function Season123_1_9EquipBookView:_btncomposeOnClick()
...
function Season123_1_9EquipBookView:sendComposeRequest()
...
function Season123_1_9EquipBookView:_btndecomposeOnClick()
	...
	ViewMgr.instance:openView(ViewName.Season123_1_9DecomposeView, param)
end

function Season123_1_9EquipBookView:_btnbatchDecomposeOnClick()
...
function Season123_1_9EquipBookView:_btnskipEffectOnClick()
...
function Season123_1_9EquipBookView:_editableInitView()
	...
	self.colorStr = Season123_1_9EquipBookView.defaultDescColor

	self:hideEffect()
	gohelper.setActive(self._btnskipEffect.gameObject, false)

	self._skillContainer = MonoHelper.addNoUpdateLuaComOnceToGo(self._gobuffContainer, Season123_1_9ShowSkillEffectTip)
end

function Season123_1_9EquipBookView:onOpen()
...
function Season123_1_9EquipBookView:onSwitchCardAnim()
	self:refreshUI()
end

function Season123_1_9EquipBookView:refreshEquipBook()
	self._scrollcardlist.verticalNormalizedPosition = 1

	self:refreshUI()
end

function Season123_1_9EquipBookView:refreshUI()
...
function Season123_1_9EquipBookView:refreshCardDesc()
...
function Season123_1_9EquipBookView:refreshCardItem(itemConfig)
...
function Season123_1_9EquipBookView:cardDescItemShow(obj, data, index)
	obj.name = "desc" .. index
	...
	txtDesc.text = self._skillContainer:addLink(data)

	fixTmpBreakLine:refreshTmpContent(txtDesc)
	SLFramework.UGUI.GuiHelper.SetColor(txtDesc, self.colorStr)
	self._skillContainer:addHyperLinkClick(txtDesc)
end

function Season123_1_9EquipBookView:refreshIcon()
	if not self._icon then
		local go = self._gocarditem

		self._icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season123_1_9CelebrityCardEquip)
	end
...
function Season123_1_9EquipBookView:refreshBtn()
	local equipItemCount = self:getItemCount()
...
function Season123_1_9EquipBookView:isMainCardOverCount()
	if self.itemId then
...
function Season123_1_9EquipBookView:canCompose()
	if not self.itemId then
		return false
	end
	...
	if currencyMO then
		if not currencyMO.quantity then
			local curCoinCount = 0

			return itemConfig and curCoinCount >= itemConfig.composeCost
		end
	end
end

function Season123_1_9EquipBookView:getItemCount()
	local itemMap = Season123Model.instance:getAllItemMo(self.actId)

	return GameUtil.getTabLen(itemMap)
end

function Season123_1_9EquipBookView:onChangeSelectCard()
	self._animatorCard:Play("switch", 0, 0)
	self:hideEffect()
end

function Season123_1_9EquipBookView:doDecompose(decomposeList)
...
function Season123_1_9EquipBookView:sendDecomposeEquipRequest()
...
function Season123_1_9EquipBookView:showDecomposeEffect()
...
function Season123_1_9EquipBookView:showComposeEffect()
...
function Season123_1_9EquipBookView:hideEffect()
	gohelper.setActive(self._godecomposeEffect, false)
	gohelper.setActive(self._gocomposeEffect, false)
end

function Season123_1_9EquipBookView:onClose()
	TaskDispatcher.cancelTask(self.sendComposeRequest, self)
	TaskDispatcher.cancelTask(self.sendDecomposeEquipRequest, self)
...
function Season123_1_9EquipBookView:onDestroyView()
...
return Season123_1_9EquipBookView
