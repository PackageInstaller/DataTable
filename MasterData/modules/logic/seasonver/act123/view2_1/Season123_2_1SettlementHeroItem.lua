-- chunkname: @modules/logic/seasonver/act123/view2_1/Season123_2_1SettlementHeroItem.lua

module("modules.logic.seasonver.act123.view2_1.Season123_2_1SettlementHeroItem", package.seeall)

local Season123_2_1SettlementHeroItem = class("Season123_2_1SettlementHeroItem", BaseViewExtended)

function Season123_2_1SettlementHeroItem:onInitView()
...
function Season123_2_1SettlementHeroItem:addEvents()
	return
end

function Season123_2_1SettlementHeroItem:removeEvents()
	return
end

function Season123_2_1SettlementHeroItem:onRefreshViewParam(is_replay, hero, equip, equip_104, replay_data, trail)
...
function Season123_2_1SettlementHeroItem:onOpen()
...
function Season123_2_1SettlementHeroItem:_showNormalData()
	if self._trail then
	...
	if self._equip then
		local equip_data = EquipModel.instance:getEquip(self._equip[1])
...
function Season123_2_1SettlementHeroItem:_showTrailHeroIcon(trail)
	if not trail then
		return
	end
	...
	local skinConfig = trailCo.skin > 0 and SkinConfig.instance:getSkinCo(trailCo.skin) or SkinConfig.instance:getSkinCo(heroCo.skinId)
...
function Season123_2_1SettlementHeroItem:_showHeroIcon(heroId, skinId)
...
function Season123_2_1SettlementHeroItem:checkAssist(heroUid, heroMO)
	if not heroMO then
...
function Season123_2_1SettlementHeroItem:_showEquipIcon(equip_id)
...
function Season123_2_1SettlementHeroItem:_showEquip123(index, uid, totalEquipCount, equipId)
	if equipId == 0 then
		return
	end

	if totalEquipCount <= 1 then
		if not self._gosingle then
			local parentTran = self["_gocard" .. index]

			self:openSubView(Season123_2_1CelebrityCardGetItem, Season123_2_1CelebrityCardItem.AssetPath, parentTran, uid, nil, equipId)
		end
	end
end

function Season123_2_1SettlementHeroItem:_showReplayData()
...
function Season123_2_1SettlementHeroItem:_refreshEquipPos()
	return
end

function Season123_2_1SettlementHeroItem:onClose()
	return
end

function Season123_2_1SettlementHeroItem:onDestroyView()
	self._simageheroicon:UnLoadImage()
end

return Season123_2_1SettlementHeroItem
