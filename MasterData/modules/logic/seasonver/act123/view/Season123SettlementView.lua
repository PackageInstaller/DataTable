-- chunkname: @modules/logic/seasonver/act123/view/Season123SettlementView.lua

module("modules.logic.seasonver.act123.view.Season123SettlementView", package.seeall)

local Season123SettlementView = class("Season123SettlementView", BaseViewExtended)

function Season123SettlementView:onInitView()
...
function Season123SettlementView:addEvents()
...
function Season123SettlementView:removeEvents()
...
function Season123SettlementView:_btnstatOnClick()
	ViewMgr.instance:openView(ViewName.FightStatView)
end

function Season123SettlementView:_showSettlement()
...
function Season123SettlementView:_onRewardShow()
	self:_showSettlementReward()
end

function Season123SettlementView:_dailyShowHero()
	self._heroIndex = 0
...
function Season123SettlementView:_showHeroItem()
	self._heroIndex = self._heroIndex + 1

	gohelper.setActive(self._heroList[self._heroIndex].viewGO, true)
end

function Season123SettlementView:_showGetCardView()
	if self.equip_cards then
		self._showTipsFlow = FlowSequence.New()

		self._showTipsFlow:addWork(Season123SettlementTipsWork.New())
...
function Season123SettlementView:_onShowTipsFlowDone()
	self.equip_cards = nil
end

function Season123SettlementView:_checkNewRecord()
...
function Season123SettlementView:_onCoverDungeonRecordReply(isCover)
...
function Season123SettlementView:_detectCoverRecord()
...
function Season123SettlementView:_onBtnCoverRecordClick()
	DungeonRpc.instance:sendCoverDungeonRecordRequest(true)
end

function Season123SettlementView:_detectAddFriend()
...
function Season123SettlementView:_onBtnConfirmFriendClick()
	if not FightResultModel.instance.assistUserId then
		local assistUserId = 0
...
function Season123SettlementView:_onAddAssistFriendReply()
	gohelper.setActive(self._gofriendpart, false)
end

function Season123SettlementView:_editableInitView()
...
function Season123SettlementView:_btncloseOnClick()
...
function Season123SettlementView:onOpen()
	self._startTime = Time.realtimeSinceStartup
...
function Season123SettlementView:_refreshRound()
	...
	if self._battleContext.stage and self._battleContext.layer then
		...
		if episodeMO and episodeMO.round then
			...
			self._totalTime.text = totalTimeStr
			self._totalTimeBlue.text = totalTimeStr
		end
	end
end

function Season123SettlementView:_showSettlementReward()
...
function Season123SettlementView:_onRewardItemShow(obj, data, index)
...
function Season123SettlementView:getCurEpisodeRewards()
...
function Season123SettlementView:_onHeroItemLoaded()
	self._heroList = {}
					...
					if trailDict[i] then
						...
						if trialCo then
							...
							if equip123_dic[hero_uid] then
								...
								local heroItem = self:openSubView(Season123SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, nil, equip, equip_104, nil, trailDict[i])
						...
						local heroItem = self:openSubView(Season123SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, hero_uid, equip, equip_104)
...
function Season123SettlementView:setMainPosCardItemByUid(index, equipUid)
	...
	if not seasonMO.unlockIndexSet[slotIndex] then
		...
		if equipUid and equipUid ~= "0" then
			...
			self:openSubView(Season123CelebrityCardGetItem, Season123CelebrityCardItem.AssetPath, goPos, equipUid, true)
		else
...
function Season123SettlementView:_refreshHeroItemPos()
...
function Season123SettlementView:_onGetFightRecordGroupReply(fightGroupMO)
	for i = 1, 4 do
		...
		local heroItem = self:openSubView(Season123SettlementHeroItem, obj, nil, true, hero, equip, equip_104, replay_data)
...
function Season123SettlementView:setMainPosCardItemById(index, mainEquip)
	local equipId
	...
	if not seasonMO.unlockIndexSet[slotIndex] then
		...
		if equipId and equipId ~= 0 then
			gohelper.setActive(goEmpty, false)
			gohelper.setActive(goPos, true)
			self:openSubView(Season123CelebrityCardGetItem, Season123CelebrityCardItem.AssetPath, goPos, nil, nil, equipId)
		else
			gohelper.setActive(goEmpty, true)
			gohelper.setActive(goPos, false)
		end
...
function Season123SettlementView:_delayClosetip()
	gohelper.setActive(self._gofightsucc, false)
	self:_showSettlement()
end

function Season123SettlementView:onClose()
...
return Season123SettlementView
