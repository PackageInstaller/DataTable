-- chunkname: @modules/logic/seasonver/act123/view/Season123EntryView.lua

module("modules.logic.seasonver.act123.view.Season123EntryView", package.seeall)

local Season123EntryView = class("Season123EntryView", BaseView)

function Season123EntryView:onInitView()
...
function Season123EntryView:addEvents()
...
function Season123EntryView:removeEvents()
...
function Season123EntryView:_editableInitView()
	self._entryList = {}
	self._centerItem = nil
end

function Season123EntryView:onDestroyView()
...
function Season123EntryView:onOpen()
...
function Season123EntryView:onClose()
...
function Season123EntryView:checkFirstOpenOverview()
...
function Season123EntryView:checkSceneIsLoaded()
	return
end

function Season123EntryView:refreshUI()
...
function Season123EntryView:refreshCenter()
	if self._centerItem then
...
function Season123EntryView:refreshPageBtn()
...
function Season123EntryView:refreshCardPackageUI()
...
function Season123EntryView:refreshUTTURetailTicket()
...
function Season123EntryView:refreshStoreCoin()
...
function Season123EntryView:refreshRemainTime()
	local actMO = ActivityModel.instance:getActMO(self.viewParam.actId)
...
function Season123EntryView:handleGetActInfo(actId)
	if self.viewParam.actId == actId then
		self:refreshUI()
	end
end

function Season123EntryView:handleOtherViewAutoOpened()
	self._anim:Play(Season123EntryView.Anim.Close)
end

function Season123EntryView:handleItemChanged()
	self:refreshUTTURetailTicket()
	self:refreshStoreCoin()
end

function Season123EntryView:handleSceneLoaded()
	self:checkSceneLoaded()
end

function Season123EntryView:handleStageFinishWithoutStory()
	if self._anim then
		self._anim:Play(Season123EntryView.Anim.Switch, 0, 0)
	end
end

function Season123EntryView:checkSceneLoaded()
	if self._isCenterInited then
		return
	end

	local sceneRoot = CameraMgr.instance:getSceneRoot()
	local sceneGO = gohelper.findChild(sceneRoot, "Season123EntryScene/scene")

	if sceneGO then
		gohelper.setActive(self._goentranceitem, true)

		self._centerItem = Season123EntryItem.New()
...
function Season123EntryView:_btnprevOnClick()
	UIBlockMgrExtend.instance:setNeedCircleMv(false)
	UIBlockMgr.instance:startBlock("switchStage")
	self._animExcessive:Play(Season123EntryView.Anim.Hard)
...
function Season123EntryView:_btnnextOnClick()
	UIBlockMgrExtend.instance:setNeedCircleMv(false)
	UIBlockMgr.instance:startBlock("switchStage")
	self._animExcessive:Play(Season123EntryView.Anim.Story)
	AudioMgr.instance:trigger(AudioEnum.UI.season123_stage_switch)
...
function Season123EntryView:_btnStoreOnClick()
	Season123Controller.instance:openSeasonStoreView(self.viewParam.actId)
end

function Season123EntryView:_btnStoryOnClick()
	Season123Controller.instance:openSeasonStoryView({
		actId = self.viewParam.actId
	})
end

function Season123EntryView:_btnEquipBookOnClick()
	Season123Controller.instance:openSeasonEquipBookView(self.viewParam.actId)
end

function Season123EntryView:_btnOverviewOnClick()
	Season123Controller.instance:openSeasonOverview({
		actId = self.viewParam.actId
	})
end

function Season123EntryView:_btnAchievementOnClick()
	Season123Controller.instance:openSeasonTaskView({
		actId = self.viewParam.actId
	})
end

function Season123EntryView:_btnCardPackageOnClick()
	Season123Controller.instance:openSeasonCardPackageView({
		actId = self.viewParam.actId
	})
end

function Season123EntryView:_btnRetailOnClick()
...
function Season123EntryView:_enterRetailView()
	Season123Controller.instance:openSeasonRetail({
		actId = self.viewParam.actId
	})
end

function Season123EntryView:_btnTrialOnClick()
	Season123EntryController.instance:enterTrailFightScene()
end

function Season123EntryView:onCloseView(viewName)
	if viewName == ViewName.Season123EpisodeListView then
		self._anim:Play(Season123EntryView.Anim.Switch1, 0, 0)
	end

	if viewName == ViewName.Season123RetailView then
		self._anim:Play(Season123EntryView.Anim.Switch, 0, 0)
	end

	if viewName == ViewName.Season123StoryPagePopView then
		self._anim:Play(Season123EntryView.Anim.Switch, 0, 0)
		TaskDispatcher.runDelay(self._autoClickNext, self, 0.5)
	end
end

function Season123EntryView:_switchStage()
	Season123EntryController.instance:switchStage(self.isNext)

	self.isNext = nil
end

function Season123EntryView:_endBlock()
	UIBlockMgr.instance:endBlock("switchStage")
	UIBlockMgrExtend.instance:setNeedCircleMv(true)
end

function Season123EntryView:initRedDot()
...
function Season123EntryView:checkHasReadUnlockStory()
	Season123Controller.instance:checkHasReadUnlockStory(self.viewParam.actId)
end

function Season123EntryView:_autoClickNext()
