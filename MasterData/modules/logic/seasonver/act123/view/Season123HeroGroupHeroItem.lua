-- chunkname: @modules/logic/seasonver/act123/view/Season123HeroGroupHeroItem.lua

module("modules.logic.seasonver.act123.view.Season123HeroGroupHeroItem", package.seeall)

local Season123HeroGroupHeroItem = class("Season123HeroGroupHeroItem", LuaCompBase)

function Season123HeroGroupHeroItem:ctor(SeasonHeroGroupListView)
	self._seasonHeroGroupListView = SeasonHeroGroupListView
end

function Season123HeroGroupHeroItem:init(go)
	self.go = go
	...
	self._cardItem1 = Season123HeroGroupCardItem.New(self._gocarditem1, self, {
		slot = 1
	})
...
function Season123HeroGroupHeroItem:onUpdateMO(mo, viewParam, replayHeroData)
	self.mo = mo
	...
	if not self._heroMO and not self._monsterCo and not self.trialCO then
		if replayHeroData then
			...
			local var_3_4 = replayHeroData.heroId
			...
			if not isNoData then
				...
				local var_3_5 = self.isLock

				if not self.isLock then
					var_3_5 = self.isAidLock
...
function Season123HeroGroupHeroItem:checkSubGoVisable()
	if self.isLock then
		return false
	end
...
function Season123HeroGroupHeroItem:_initData()
	self._firstEnter = true

	gohelper.setActive(self._goselectdebuff, false)
end

function Season123HeroGroupHeroItem:_addEvents()
...
function Season123HeroGroupHeroItem:_updateSeasonEquips()
	self:_updateEquips()
	self:_updateEquipCards()
	self:_updateAct123Hp()
end

function Season123HeroGroupHeroItem:_onClickHeroItem()
...
function Season123HeroGroupHeroItem:_onClickEquip()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123HeroGroupHeroItem:_updateEquips()
	self._equipType = -1
...
Season123HeroGroupHeroItem.NonCardSlotBlackMaskHeight = 362
Season123HeroGroupHeroItem.HasCardSlotBlackMashHeight = 200
Season123HeroGroupHeroItem.SingleSlotCardItem1Pos = Vector2.New(-3.8, 24.9)
Season123HeroGroupHeroItem.TwoSlotCardItem1Pos = Vector2.New(-51.6, 28.2)

function Season123HeroGroupHeroItem:_updateEquipCards()
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()
	...
	if hasShowCardSlot then
		if not Season123HeroGroupHeroItem.NonCardSlotBlackMaskHeight then
			local targetBlackMaskHeight = Season123HeroGroupHeroItem.HasCardSlotBlackMashHeight
			...
			local targetSlot1Pos = Season123HeroGroupHeroItem.SingleSlotCardItem1Pos
...
function Season123HeroGroupHeroItem:_updateAct123Hp()
	local context = Season123Model.instance:getBattleContext()
...
function Season123HeroGroupHeroItem:setHp(hpRate)
...
function Season123HeroGroupHeroItem:_equipIconAddDrag(go)
	if self._drag then
		return
	end
...
function Season123HeroGroupHeroItem:_onBeginDrag(equipTransform, pointerEventData)
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123HeroGroupHeroItem:_onDrag(param, pointerEventData)
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123HeroGroupHeroItem:_onEndDrag(equipTransform, pointerEventData)
	self:killEquipTweenId()
...
function Season123HeroGroupHeroItem:canMoveCardToPos(tarPos)
...
function Season123HeroGroupHeroItem:topDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self.currentParent, dragTransform.gameObject)
end

function Season123HeroGroupHeroItem:resetDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self._goequip, dragTransform.gameObject)
end

function Season123HeroGroupHeroItem:getDragTransform()
	return self._equip.moveContainerTrs
end

function Season123HeroGroupHeroItem:getOrignDragPos()
	return self._goequip.transform.position
end

function Season123HeroGroupHeroItem:onDragEndFail()
...
function Season123HeroGroupHeroItem:onDragEndSuccess(targetHeroItem)
...
function Season123HeroGroupHeroItem:_playDragEndAudio(targetHeroItem)
...
function Season123HeroGroupHeroItem:_tweenToPos(transform, anchorPos)
...
function Season123HeroGroupHeroItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)
...
function Season123HeroGroupHeroItem:_moveToTarget(position)
...
function Season123HeroGroupHeroItem:_setEquipDragEnabled(isEnabled)
...
function Season123HeroGroupHeroItem:playHeroGroupHeroEffect(state)
...
function Season123HeroGroupHeroItem:playRestrictAnimation(needRestrictHeroUidDict, deadHeroUidDict)
...
function Season123HeroGroupHeroItem:setGrayFactor(value)
...
function Season123HeroGroupHeroItem:showCounterSign()
	local career
...
function Season123HeroGroupHeroItem:onItemBeginDrag(index)
...
function Season123HeroGroupHeroItem:onItemEndDrag(index, dragToIndex)
	ZProj.TweenHelper.DOScale(self.go.transform, 1, 1, 1, 0.2, nil, nil, nil, EaseType.Linear)
end

function Season123HeroGroupHeroItem:onItemCompleteDrag(index, dragToIndex, complete)
...
function Season123HeroGroupHeroItem:hideDragEffect()
	gohelper.setActive(self._goselectdebuff, false)
end

function Season123HeroGroupHeroItem:setHeroGroupEquipEffect(show)
	self._canPlayEffect = show
end

function Season123HeroGroupHeroItem:setParent(transform)
	self.currentParent = transform

	gohelper.addChildPosStay(transform.gameObject, self._subGO)
end

function Season123HeroGroupHeroItem:flowOriginParent()
	return
end

function Season123HeroGroupHeroItem:flowCurrentParent()
	return
end

function Season123HeroGroupHeroItem:getHeroItemList()
	return self._seasonHeroGroupListView:getHeroItemList()
end

function Season123HeroGroupHeroItem:killEquipTweenId()
...
function Season123HeroGroupHeroItem:_removeEvents()
	self._btnclickitem:RemoveClickListener()
	self._btnclickequip:RemoveClickListener()
end

function Season123HeroGroupHeroItem:onDestroy()
	self:_removeEvents()
	self:killEquipTweenId()
...
return Season123HeroGroupHeroItem
