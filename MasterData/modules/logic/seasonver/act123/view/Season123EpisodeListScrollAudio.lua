-- chunkname: @modules/logic/seasonver/act123/view/Season123EpisodeListScrollAudio.lua

module("modules.logic.seasonver.act123.view.Season123EpisodeListScrollAudio", package.seeall)

local Season123EpisodeListScrollAudio = class("Season123EpisodeListScrollAudio", LuaCompBase)

function Season123EpisodeListScrollAudio:ctor(param)
	self.scroll = param
...
function Season123EpisodeListScrollAudio:onDragBegin()
	self._isDraging = true
...
function Season123EpisodeListScrollAudio:onDragEnd()
	self:onUpdate()

	self._isDraging = false
end

function Season123EpisodeListScrollAudio:onClickDown()
	AudioMgr.instance:trigger(AudioEnum.UI.UI_checkpoint_chain_start)
end

function Season123EpisodeListScrollAudio:onUpdate()
	if self.scroll and not self._isDisposed then
		local curY = recthelper.getAnchorY(self.scroll.content) * self._pixelOffsetMoveFactor
...
function Season123EpisodeListScrollAudio:_onValueChangeSampleOffset(y)
...
function Season123EpisodeListScrollAudio:_onUpdateRTPCSpeed(y)
...
function Season123EpisodeListScrollAudio:_onValueChangeForStartOrStop(y)
...
function Season123EpisodeListScrollAudio:_onValueChangeCheckReplay(y)
...
function Season123EpisodeListScrollAudio:_resetPlayingStatus(y)
	self._isScrollPlaying = false
	self._scrollingPlayY = y
end

function Season123EpisodeListScrollAudio:dispose()
	AudioMgr.instance:trigger(AudioEnum.UI.UI_checkpoint_chain_end)
...
function Season123EpisodeListScrollAudio:onDestroy()
	self.scroll = nil
end

return Season123EpisodeListScrollAudio
