-- chunkname: @modules/logic/seasonver/act123/view/Season123RecordWindowItem.lua

module("modules.logic.seasonver.act123.view.Season123RecordWindowItem", package.seeall)

local Season123RecordWindowItem = class("Season123RecordWindowItem", LuaCompBase)
local NotBestRecordMoveY = 6

function Season123RecordWindowItem:init(go)
	self.go = go
	...
	self._imgBestCircle = gohelper.findChildImage(self.go, "#go_normalrecord/totaltime/#go_bestcircle")
	self._goBlueTxtTime = gohelper.findChild(self.go, "#go_normalrecord/totaltime/#go_bestcircle/#txt_timeblue")
...
function Season123RecordWindowItem:addEventListeners()
	self._btndetails:AddClickListener(self._btndetailsOnClick, self)
end

function Season123RecordWindowItem:removeEventListeners()
	self._btndetails:RemoveClickListener()
end

function Season123RecordWindowItem:_btndetailsOnClick()
...
function Season123RecordWindowItem:onLoad(delayOpenAnimTime, isPlayOpen)
...
function Season123RecordWindowItem:_delayActive()
...
function Season123RecordWindowItem:onUpdateMO(mo)
	self.mo = mo
	...
	if not self.mo.round then
		...
		if not self.mo.heroList then
			...
			gohelper.setActive(self._goBlueTxtTime, isBest)

			self._imgBestCircle.enabled = isBest
...
function Season123RecordWindowItem:playAnimation(animName)
...
function Season123RecordWindowItem:_onHeroItemLoad(obj, heroData, index)
...
function Season123RecordWindowItem:onDestroy()
	TaskDispatcher.cancelTask(self._delayActive, self)

	self._isPlayOpen = false
end

return Season123RecordWindowItem
