-- chunkname: @modules/logic/seasonver/act123/view/Season123FightCardItem.lua

module("modules.logic.seasonver.act123.view.Season123FightCardItem", package.seeall)

local Season123FightCardItem = class("Season123FightCardItem", UserDataDispose)

function Season123FightCardItem:ctor(go)
	self:__onInit()
...
Season123FightCardItem.MainRoleItemMinHeight = 390
Season123FightCardItem.NormalRoleItemMinHeight = 315
Season123FightCardItem.RoleCardPos = {
	Vector2(-16.5, 0.5),
	Vector2(-16.5, 0.5),
	Vector2(-16.5, -6.5),
	Vector2(-16.5, -17),
...
function Season123FightCardItem:setData(data)
	if not data then
	...
	if self.isMainRole then
		self.layoutElement.minHeight = Season123FightCardItem.MainRoleItemMinHeight or Season123FightCardItem.NormalRoleItemMinHeight
	end
...
function Season123FightCardItem:_setName()
	if self.isMainRole then
...
function Season123FightCardItem:_setHead()
...
function Season123FightCardItem:getHeroMO()
...
function Season123FightCardItem:_setCard(equipId)
	if not self.cardItem then
		self.cardItem = Season123CelebrityCardItem.New()
	...
	local targetCardPos = Season123FightCardItem.RoleCardPos[tonumber(rare)] or Vector2(0, 0)

	recthelper.setAnchor(self.goCardPos.transform, targetCardPos.x, targetCardPos.y)
end

function Season123FightCardItem:_setDesc(equipId)
...
function Season123FightCardItem:createDescItem(index)
	local item = {}
...
function Season123FightCardItem:updateDescItem(item, skillStr)
...
function Season123FightCardItem:destroyDescItem(item)
	return
end

function Season123FightCardItem:destroy()
