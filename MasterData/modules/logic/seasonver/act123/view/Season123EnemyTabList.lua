-- chunkname: @modules/logic/seasonver/act123/view/Season123EnemyTabList.lua

module("modules.logic.seasonver.act123.view.Season123EnemyTabList", package.seeall)

local Season123EnemyTabList = class("Season123EnemyTabList", BaseView)

function Season123EnemyTabList:onInitView()
...
function Season123EnemyTabList:_editableInitView()
	self._simagebattlelistbg:LoadImage(ResUrl.getWeekWalkBg("bg_zuodi.png"))

	self._tabItems = {}
end

function Season123EnemyTabList:onDestroyView()
...
function Season123EnemyTabList:onOpen()
	self.addEventCb(self, Season123Controller.instance, Season123Event.EnemyDetailSwitchTab, self.refreshSelect, self)
	self:refreshUI()
end

function Season123EnemyTabList:refreshUI()
	self:refreshItems()
	self:refreshSelect()
end

function Season123EnemyTabList:refreshItems()
...
function Season123EnemyTabList:getOrCreateTabItem(index)
...
function Season123EnemyTabList:onClickTab(index)
	Season123EnemyController.instance:switchTab(index)
end

function Season123EnemyTabList:refreshSelect()
	local battleIds = Season123EnemyModel.instance:getBattleIds()
