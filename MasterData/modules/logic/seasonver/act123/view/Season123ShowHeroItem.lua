-- chunkname: @modules/logic/seasonver/act123/view/Season123ShowHeroItem.lua

module("modules.logic.seasonver.act123.view.Season123ShowHeroItem", package.seeall)

local Season123ShowHeroItem = class("Season123ShowHeroItem", UserDataDispose)

function Season123ShowHeroItem:ctor()
	self:__onInit()
end

function Season123ShowHeroItem:dispose()
	self:removeEvents()
	self:__onDispose()
end

function Season123ShowHeroItem:init(viewGO)
	self.viewGO = gohelper.findChild(viewGO, "root")

	self:initComponent()
end

Season123ShowHeroItem.exSkillFillAmount = {
	0.2,
	0.4,
	0.6,
	0.79,
...
Season123ShowHeroItem.MaxRare = 5

function Season123ShowHeroItem:initComponent()
	...
	local maxRank = HeroConfig.instance:getMaxRank(Season123ShowHeroItem.MaxRare)
...
function Season123ShowHeroItem:initData(index)
	self._index = index
...
function Season123ShowHeroItem:removeEvents()
	self._btnclick:RemoveClickListener()
end

function Season123ShowHeroItem:onClose()
...
function Season123ShowHeroItem:refreshUI()
	...
	if mo then
		...
		if not skinConfig then
			logError("Season123ShowHeroItem.refreshUI error, skinCfg is nil, id:" .. tostring(mo.skin))
...
function Season123ShowHeroItem:refreshRare(heroMO)
	for rare = 1, #self._rareList do
		gohelper.setActive(self._rareList[rare], rare <= heroMO.config.rare + 1)
	end
end

function Season123ShowHeroItem:refreshRank(heroMO)
...
function Season123ShowHeroItem:refreshExSkill(heroMO)
...
function Season123ShowHeroItem:refreshHp(mo)
...
function Season123ShowHeroItem:OpenAnim()
...
function Season123ShowHeroItem:playOpenAnim()
