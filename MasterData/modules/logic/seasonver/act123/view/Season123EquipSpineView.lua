-- chunkname: @modules/logic/seasonver/act123/view/Season123EquipSpineView.lua

module("modules.logic.seasonver.act123.view.Season123EquipSpineView", package.seeall)

local Season123EquipSpineView = class("Season123EquipSpineView", BaseView)

function Season123EquipSpineView:onInitView()
...
function Season123EquipSpineView:addEvents()
	return
end

function Season123EquipSpineView:removeEvents()
	return
end

function Season123EquipSpineView:_editableInitView()
	self._uiSpine = GuiModelAgent.Create(self._gospine, true)

	self:createSpine()
end

function Season123EquipSpineView:onDestroyView()
...
function Season123EquipSpineView:onOpen()
	self:refreshHeroSkin()
end

function Season123EquipSpineView:onClose()
	if self._uiSpine then
		self._uiSpine:setModelVisible(false)
	end
end

function Season123EquipSpineView:createSpine(skinConfig)
	if skinConfig then
		self._uiSpine:useRT()
...
function Season123EquipSpineView:refreshHeroSkin()
...
function Season123EquipSpineView:onSpineLoaded()
	self._spineLoaded = true

	self._uiSpine:setUIMask(true)
end

return Season123EquipSpineView
