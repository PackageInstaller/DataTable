...
function Season123EpisodeDetailView:_setResetInfo()
	...
	if needShowReset then
		...
		self._txtresettime.text = round and round > 0 and tostring(round) or "--"
	end
end
...
function Season123EpisodeDetailView:_setInfo()
	...
	if isNewStage then
		if not Season123EpisodeDetailView.NewStageTagPos then
			local targetStageTagPos = Season123EpisodeDetailView.NomalStageTagPos
...
function Season123EpisodeDetailView:_showPartStarGroupItem(starGroup)
	if starGroup < 7 then
		...
		for i = 1, 6 do
			...
			if i <= starGroup then
				self._partStageItems[i].canvasgroup.alpha = Season123EpisodeDetailView.UnLockStageItemAlpha or Season123EpisodeDetailView.LockStageItemAlpha
			end
		end
	else
		for i = 1, 7 do
