-- chunkname: @modules/logic/seasonver/act123/view/Season123CelebrityCardEquip.lua

module("modules.logic.seasonver.act123.view.Season123CelebrityCardEquip", package.seeall)

local Season123CelebrityCardEquip = class("Season123CelebrityCardEquip", LuaCompBase)

function Season123CelebrityCardEquip:init(go)
	Season123CelebrityCardEquip.super.init(self, go)
...
Season123CelebrityCardEquip.MaxRare = 6

function Season123CelebrityCardEquip:_editableInitView()
	self._rareGoMap = {}

	for i = 1, Season123CelebrityCardEquip.MaxRare do
...
function Season123CelebrityCardEquip:onDestroy()
	self:disposeUI()
end

function Season123CelebrityCardEquip:checkInitBtnClick()
...
function Season123CelebrityCardEquip:checkInitLongPress()
...
function Season123CelebrityCardEquip:disposeUI()
...
function Season123CelebrityCardEquip:updateData(itemId)
	self.itemId = itemId

	self:refreshUI()
end

function Season123CelebrityCardEquip:createRareMap(go)
...
function Season123CelebrityCardEquip:refreshUI()
...
function Season123CelebrityCardEquip:refreshSelfChoice()
...
function Season123CelebrityCardEquip:refreshIcon()
	if self._curSelectedItem then
...
function Season123CelebrityCardEquip:refreshFlag()
	if not self.itemId then
		return
	end

	if self._showProbability then
		local needShowType1 = self._cfg.isOptional ~= 1

		if self._showTag then
			local needShowType2 = self._cfg.isOptional == 1

			if self._showNewFlag2 then
				local needShowType3 = not needShowType2

				if self._showNewFlag then
					local needShowType4 = not needShowType2
...
function Season123CelebrityCardEquip:setFlagUIPos(targetPosX, targetPosY)
...
function Season123CelebrityCardEquip:setFlagUIScale(targetScale)
...
function Season123CelebrityCardEquip:handleIconLoaded()
	if not self._isDisposed then
		local item = self._curSelectedItem
...
function Season123CelebrityCardEquip:setColorDark(enable)
	self._darkMaskColor = enable and "#7b7b7b" or "#ffffff"

	self:refreshIcon()
end

function Season123CelebrityCardEquip:setShowTag(enable)
	self._showTag = enable

	self:refreshFlag()
end

function Season123CelebrityCardEquip:setShowProbability(enable)
	self._showProbability = enable

	self:refreshFlag()
end

function Season123CelebrityCardEquip:setShowNewFlag(enable)
	self._showNewFlag = enable

	self:refreshFlag()
end

function Season123CelebrityCardEquip:setShowNewFlag2(enable)
	self._showNewFlag2 = enable

	self:refreshFlag()
end

function Season123CelebrityCardEquip:setClickCall(clickCall, clickCallObj, param)
...
function Season123CelebrityCardEquip:setLongPressCall(longPressCall, longPressCallObj, param)
