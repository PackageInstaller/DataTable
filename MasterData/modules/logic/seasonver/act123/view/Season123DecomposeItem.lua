-- chunkname: @modules/logic/seasonver/act123/view/Season123DecomposeItem.lua

module("modules.logic.seasonver.act123.view.Season123DecomposeItem", package.seeall)

local Season123DecomposeItem = class("Season123DecomposeItem", ListScrollCellExtend)

function Season123DecomposeItem:init(go)
	Season123DecomposeItem.super.init(self, go)
...
function Season123DecomposeItem:addEventListeners()
...
function Season123DecomposeItem:removeEventListeners()
...
function Season123DecomposeItem:onUpdateMO(mo)
	self._mo = mo
...
Season123DecomposeItem.AnimRowCount = 4
Season123DecomposeItem.OpenAnimTime = 0.06
Season123DecomposeItem.OpenAnimStartTime = 0.05

function Season123DecomposeItem:refreshUI()
...
function Season123DecomposeItem:refreshEquipedHero(heroUid)
...
function Season123DecomposeItem:checkPlayAnim()
...
function Season123DecomposeItem:onDelayPlayOpen()
	TaskDispatcher.cancelTask(self.onDelayPlayOpen, self)
	self._animator:Play("open", 0, 0)
...
function Season123DecomposeItem:checkCreateIcon()
...
function Season123DecomposeItem:onClickSelf()
...
function Season123DecomposeItem:showDecomposeEffect()
...
function Season123DecomposeItem:hideDecomposeEffect()
	gohelper.setActive(self._goDecomposeEffect, false)
end

function Season123DecomposeItem:onDestroyView()
	if self.icon then
		self.icon:disposeUI()
	end
