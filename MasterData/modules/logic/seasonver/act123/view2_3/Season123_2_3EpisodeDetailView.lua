...
function Season123_2_3EpisodeDetailView:_setInfo()
	...
	if isNewStage then
		if not Season123_2_3EpisodeDetailView.NewStageTagPos then
			local targetStageTagPos = Season123_2_3EpisodeDetailView.NomalStageTagPos
...
function Season123_2_3EpisodeDetailView:_showPartStarGroupItem(starGroup)
	if starGroup < 7 then
		...
		for i = 1, 6 do
			...
			if i <= starGroup then
				self._partStageItems[i].canvasgroup.alpha = Season123_2_3EpisodeDetailView.UnLockStageItemAlpha or Season123_2_3EpisodeDetailView.LockStageItemAlpha
			end
		end
	else
		for i = 1, 7 do
