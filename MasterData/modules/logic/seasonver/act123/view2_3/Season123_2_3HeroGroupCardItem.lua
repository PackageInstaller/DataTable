-- chunkname: @modules/logic/seasonver/act123/view2_3/Season123_2_3HeroGroupCardItem.lua

module("modules.logic.seasonver.act123.view2_3.Season123_2_3HeroGroupCardItem", package.seeall)

local Season123_2_3HeroGroupCardItem = class("Season123_2_3HeroGroupCardItem", UserDataDispose)

Season123_2_3HeroGroupCardItem.TweenDuration = 0.16
Season123_2_3HeroGroupCardItem.DragOffset = Vector2(0, 40)
Season123_2_3HeroGroupCardItem.ZeroPos = Vector2(-2.7, -5)
Season123_2_3HeroGroupCardItem.ZeroScale = 0.39

function Season123_2_3HeroGroupCardItem:ctor(go, parent, param)
	self:__onInit()
...
function Season123_2_3HeroGroupCardItem:init()
	...
	self.addClickCb(self, self._btncardclick, self._btnCardClick, self)
...
function Season123_2_3HeroGroupCardItem:AddDrag(go)
	if self._drag then
		return
	end
...
function Season123_2_3HeroGroupCardItem:setData(data, layer, equipId, equipUid, hasTrialEquip)
...
function Season123_2_3HeroGroupCardItem:setActive(isActive)
	self.isActive = isActive
end

function Season123_2_3HeroGroupCardItem:updateView()
	...
	if self.posUnlock then
		...
		if equipId ~= nil and equipId ~= 0 then
			if not self._seasonCardItem then
				self._seasonCardItem = Season123_2_3CelebrityCardItem.New()
...
function Season123_2_3HeroGroupCardItem:playEmptyUnlockAnim()
	local pos = self.id - 1
	...
	self._animcardempty = self._animcardempty or self._gocardempty:GetComponent(typeof(UnityEngine.Animator))

	self._animcardempty:Play("lock")
	Season123HeroGroupModel.instance:saveGroupCardUnlockTweenPos(index)
end

function Season123_2_3HeroGroupCardItem:getEquipId(actId, groupId)
...
function Season123_2_3HeroGroupCardItem:hasUseSeasonEquipCard()
	return self._hasUseSeasonEquipCard
end

function Season123_2_3HeroGroupCardItem:_btnCardClick()
	if self.inDrag then
		return
	end
	...
	var_10_0.slot = self.slot or 1
	var_10_0.layer = self._layer
	var_10_0.stage = Season123HeroGroupModel.instance.stage

	local param = var_10_0

	Season123Controller.instance:openSeasonEquipView(param)
end

function Season123_2_3HeroGroupCardItem:canDrag()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return false
	end
...
function Season123_2_3HeroGroupCardItem:_onBeginDrag(dragTransform, pointerEventData)
	...
	self:_tweenToPos(self._trscard, anchorPos + Season123_2_3HeroGroupCardItem.DragOffset, true)

	local scale = Season123_2_3HeroGroupCardItem.ZeroScale * 1.7

	self.tweenId = ZProj.TweenHelper.DOScale(self._trscard, scale, scale, scale, Season123_2_3HeroGroupCardItem.TweenDuration)
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, 0, Season123_2_3HeroGroupCardItem.TweenDuration)
	self.inDrag = true
end

function Season123_2_3HeroGroupCardItem:_onDrag(dragTransform, pointerEventData)
	if not self:canDrag() then
		self.inDrag = false
	...
	self:_tweenToPos(self._trscard, anchorPos + Season123_2_3HeroGroupCardItem.DragOffset)

	self.inDrag = true
end

function Season123_2_3HeroGroupCardItem:_onEndDrag(dragTransform, pointerEventData)
	self.inDrag = false
	...
	local scale = Season123_2_3HeroGroupCardItem.ZeroScale * 1
	...
	if not targetItem or not targetItem:canExchange(self) then
		self._setToPos(self, self._trscard, Season123_2_3HeroGroupCardItem.ZeroPos, true, self._onDragFailTweenEnd, self)

		self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, self.orignRoteZ, Season123_2_3HeroGroupCardItem.TweenDuration)
	...
	self._setToPos(self, self._trscard, anchorPos, true, self._onDragSuccessTweenEnd, self, targetItem)

	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, targetItem.orignRoteZ, Season123_2_3HeroGroupCardItem.TweenDuration)
end

function Season123_2_3HeroGroupCardItem:_tweenToPos(transform, anchorPos)
	...
	if math.abs(curAnchorX - anchorPos.x) > 10 or math.abs(curAnchorY - anchorPos.y) > 10 then
		self.posTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season123_2_3HeroGroupCardItem.TweenDuration)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
	end
end

function Season123_2_3HeroGroupCardItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)

	if tween then
		self.moveTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season123_2_3HeroGroupCardItem.TweenDuration, callback, callbackObj, param)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
...
function Season123_2_3HeroGroupCardItem:_moveToTarget(position)
...
function Season123_2_3HeroGroupCardItem:_setDragEnabled(isEnabled)
	local heroList = self.parent:getHeroItemList()
...
function Season123_2_3HeroGroupCardItem:setDragEnabled(isEnabled)
	if self._drag then
		self._drag.enabled = isEnabled
	end
end

function Season123_2_3HeroGroupCardItem:canExchange(targetItem)
	...
	for i = 1, maxCount do
		if i ~= srcSlot then
			...
			if tarCO and co and co.group == tarCO.group and tarPos ~= srcPos then
				GameFacade.showToast(Season123_2_3EquipItem.Toast_Same_Card)
...
function Season123_2_3HeroGroupCardItem:canMoveToPos(tarPos)
...
function Season123_2_3HeroGroupCardItem:_onDragFailTweenEnd()
	self:_setDragEnabled(true)
	gohelper.setAsLastSibling(self.parent._cardItem1.go)
end

function Season123_2_3HeroGroupCardItem:_onDragSuccessTweenEnd(targetItem)
	self:killTweenId()
	self:_setToPos(self._trscard, Season123_2_3HeroGroupCardItem.ZeroPos)
	self:_setToPos(targetItem._trscard, Season123_2_3HeroGroupCardItem.ZeroPos)
...
function Season123_2_3HeroGroupCardItem:killTweenId()
...
function Season123_2_3HeroGroupCardItem:destory()
	if self._drag then
...
return Season123_2_3HeroGroupCardItem
