-- chunkname: @modules/logic/seasonver/act123/view2_3/Season123_2_3RecordWindowItem.lua

module("modules.logic.seasonver.act123.view2_3.Season123_2_3RecordWindowItem", package.seeall)

local Season123_2_3RecordWindowItem = class("Season123_2_3RecordWindowItem", LuaCompBase)
local NotBestRecordMoveY = 6

function Season123_2_3RecordWindowItem:init(go)
	self.go = go
...
function Season123_2_3RecordWindowItem:addEventListeners()
	self._btndetails:AddClickListener(self._btndetailsOnClick, self)
end

function Season123_2_3RecordWindowItem:removeEventListeners()
	self._btndetails:RemoveClickListener()
end

function Season123_2_3RecordWindowItem:_btndetailsOnClick()
...
function Season123_2_3RecordWindowItem:onLoad(delayOpenAnimTime, isPlayOpen)
...
function Season123_2_3RecordWindowItem:_delayActive()
	gohelper.setActive(self.go, true)
	self:playAnimation((self._isPlayOpen or nil) and (UIAnimationName.Open or UIAnimationName.Idle))

	self._isPlayOpen = false
end

function Season123_2_3RecordWindowItem:onUpdateMO(mo)
	self.mo = mo
	...
	if not self.mo.round then
		local isBest = self.mo.isBest

		if not self.mo.heroList then
			local heroList = {}

			self._txttime.text = self.mo.round
			self._txtBlueTxtTime.text = self.mo.round
			...
			if isBest then
				if not self._originalHeroListY then
					local heroListY = self._originalHeroListY + NotBestRecordMoveY
...
function Season123_2_3RecordWindowItem:playAnimation(animName)
...
function Season123_2_3RecordWindowItem:_onHeroItemLoad(obj, heroData, index)
	...
	if heroCfg and heroIcon then
		if not heroData.level then
			local level = 1

			if not heroData.skinId then
				local skinId = heroCfg.skinId
...
function Season123_2_3RecordWindowItem:onDestroy()
	TaskDispatcher.cancelTask(self._delayActive, self)

	self._isPlayOpen = false
end

return Season123_2_3RecordWindowItem
