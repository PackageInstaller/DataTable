-- chunkname: @modules/logic/seasonver/act123/view2_0/Season123_2_0StoreItem.lua

module("modules.logic.seasonver.act123.view2_0.Season123_2_0StoreItem", package.seeall)

local Season123_2_0StoreItem = class("Season123_2_0StoreItem", ListScrollCell)

function Season123_2_0StoreItem:init(go)
	self.go = go
...
function Season123_2_0StoreItem:onClick()
...
function Season123_2_0StoreItem:onUpdateMO(data)
	self.data = data
	...
	elseif productItemConfig.subType == ItemEnum.SubType.Portrait then
		...
		self.headIconItem = self.headIconItem or IconMgr.instance:getCommonHeadIcon(self.goIcon)
...
function Season123_2_0StoreItem:refreshRemainBuyCount()
	if self.data.maxBuyCount == 0 then
		...
		self.txtLimitBuy.text = GameUtil.getSubPlaceholderLuaLang(luaLang("store_buylimit_count"), {
			self.remainBuyCount
		})
...
function Season123_2_0StoreItem:refreshTag()
