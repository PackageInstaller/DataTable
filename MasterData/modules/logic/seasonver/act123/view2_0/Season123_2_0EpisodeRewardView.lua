-- chunkname: @modules/logic/seasonver/act123/view2_0/Season123_2_0EpisodeRewardView.lua

module("modules.logic.seasonver.act123.view2_0.Season123_2_0EpisodeRewardView", package.seeall)

local Season123_2_0EpisodeRewardView = class("Season123_2_0EpisodeRewardView", BaseView)

function Season123_2_0EpisodeRewardView:onInitView()
...
function Season123_2_0EpisodeRewardView:addEvents()
	self._btnClose:AddClickListener(self._btnCloseOnClick, self)
	self.addEventCb(self, TaskController.instance, TaskEvent.SuccessGetBonus, self.refreshUI, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.OpenEpisodeRewardView, self.refreshUI, self)
end

function Season123_2_0EpisodeRewardView:removeEvents()
	self._btnClose:RemoveClickListener()
	self.removeEventCb(self, TaskController.instance, TaskEvent.SuccessGetBonus, self.refreshUI, self)
	self.removeEventCb(self, Season123Controller.instance, Season123Event.OpenEpisodeRewardView, self.refreshUI, self)
end

function Season123_2_0EpisodeRewardView:_btnCloseOnClick()
	self:resetView()
end

function Season123_2_0EpisodeRewardView:_editableInitView()
...
function Season123_2_0EpisodeRewardView:onOpen()
	self.actId = self.viewParam.actId
	self.stage = self.viewParam.stage
	self.targetNumList = {}
end

function Season123_2_0EpisodeRewardView:refreshUI()
	...
	if self.stageMinRound == 0 or not self.stageIsPass then
		self.curProgress = self.defaultProgress or self.stageMinRound
	end
...
function Season123_2_0EpisodeRewardView:createAndRefreshRewardItem()
	gohelper.CreateObjList(self, self.rewardItemShow, self.itemList, self._gorewardContent, self._gorewardItem)
end

function Season123_2_0EpisodeRewardView:rewardItemShow(obj, data, index)
...
function Season123_2_0EpisodeRewardView:onItemGetClick()
...
function Season123_2_0EpisodeRewardView:refreshProgressBar()
	...
	processH = nowIndex == 0 and (firstNum <= self.curProgress and firstPartH / 2 or firstPartH / 2 + (firstNum - self.curProgress) / (firstPartH / 2)) or firstPartH + nowIndex * tagH + (nowIndex - 1) * normalPartH + ((nextIndexValue ~= nowIndexValue or nil) and (self.curProgress - nowIndexValue) / (nextIndexValue - nowIndexValue)) * normalPartH
	...
	recthelper.setWidth(self._gofill.transform, (nowIndex == targetCount or nil) and self.totalWidth)
end

function Season123_2_0EpisodeRewardView:initTargetNumList()
...
function Season123_2_0EpisodeRewardView:refreshScrollPos()
	local itemWidth = 240
	local itemSpaceH = 36
...
function Season123_2_0EpisodeRewardView:getCurCanGetIndex()
...
function Season123_2_0EpisodeRewardView:resetView()
	gohelper.setActive(self._goRwards, false)
end

function Season123_2_0EpisodeRewardView:onClose()
	return
end

function Season123_2_0EpisodeRewardView:onDestroyView()
...
return Season123_2_0EpisodeRewardView
