-- chunkname: @modules/logic/seasonver/act123/view2_0/Season123_2_0HeroGroupCardItem.lua

module("modules.logic.seasonver.act123.view2_0.Season123_2_0HeroGroupCardItem", package.seeall)

local Season123_2_0HeroGroupCardItem = class("Season123_2_0HeroGroupCardItem", UserDataDispose)

Season123_2_0HeroGroupCardItem.TweenDuration = 0.16
Season123_2_0HeroGroupCardItem.DragOffset = Vector2(0, 40)
Season123_2_0HeroGroupCardItem.ZeroPos = Vector2(-2.7, -5)
Season123_2_0HeroGroupCardItem.ZeroScale = 0.39

function Season123_2_0HeroGroupCardItem:ctor(go, parent, param)
	self:__onInit()
...
function Season123_2_0HeroGroupCardItem:init()
...
function Season123_2_0HeroGroupCardItem:AddDrag(go)
	if self._drag then
		return
	end
...
function Season123_2_0HeroGroupCardItem:setData(data, layer, equipId, equipUid, hasTrialEquip)
...
function Season123_2_0HeroGroupCardItem:setActive(isActive)
	self.isActive = isActive
end

function Season123_2_0HeroGroupCardItem:updateView()
	...
	if self.posUnlock then
		...
		if equipId ~= nil and equipId ~= 0 then
			if not self._seasonCardItem then
				self._seasonCardItem = Season123_2_0CelebrityCardItem.New()
...
function Season123_2_0HeroGroupCardItem:playEmptyUnlockAnim()
	local pos = self.id - 1
...
function Season123_2_0HeroGroupCardItem:getEquipId(actId, groupId)
...
function Season123_2_0HeroGroupCardItem:hasUseSeasonEquipCard()
	return self._hasUseSeasonEquipCard
end

function Season123_2_0HeroGroupCardItem:_btnCardClick()
	if self.inDrag then
		return
	end
...
function Season123_2_0HeroGroupCardItem:canDrag()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return false
	end
...
function Season123_2_0HeroGroupCardItem:_onBeginDrag(dragTransform, pointerEventData)
	...
	self:_tweenToPos(self._trscard, anchorPos + Season123_2_0HeroGroupCardItem.DragOffset, true)

	local scale = Season123_2_0HeroGroupCardItem.ZeroScale * 1.7

	self.tweenId = ZProj.TweenHelper.DOScale(self._trscard, scale, scale, scale, Season123_2_0HeroGroupCardItem.TweenDuration)
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, 0, Season123_2_0HeroGroupCardItem.TweenDuration)
	self.inDrag = true
end

function Season123_2_0HeroGroupCardItem:_onDrag(dragTransform, pointerEventData)
	if not self:canDrag() then
		self.inDrag = false
	...
	self:_tweenToPos(self._trscard, anchorPos + Season123_2_0HeroGroupCardItem.DragOffset)

	self.inDrag = true
end

function Season123_2_0HeroGroupCardItem:_onEndDrag(dragTransform, pointerEventData)
	self.inDrag = false
	...
	local scale = Season123_2_0HeroGroupCardItem.ZeroScale * 1
	...
	if not targetItem or not targetItem:canExchange(self) then
		self._setToPos(self, self._trscard, Season123_2_0HeroGroupCardItem.ZeroPos, true, self._onDragFailTweenEnd, self)

		self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, self.orignRoteZ, Season123_2_0HeroGroupCardItem.TweenDuration)
	...
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, targetItem.orignRoteZ, Season123_2_0HeroGroupCardItem.TweenDuration)
end

function Season123_2_0HeroGroupCardItem:_tweenToPos(transform, anchorPos)
	...
	if math.abs(curAnchorX - anchorPos.x) > 10 or math.abs(curAnchorY - anchorPos.y) > 10 then
		self.posTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season123_2_0HeroGroupCardItem.TweenDuration)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
	end
end

function Season123_2_0HeroGroupCardItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)

	if tween then
		self.moveTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season123_2_0HeroGroupCardItem.TweenDuration, callback, callbackObj, param)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
...
function Season123_2_0HeroGroupCardItem:_moveToTarget(position)
...
function Season123_2_0HeroGroupCardItem:_setDragEnabled(isEnabled)
	local heroList = self.parent:getHeroItemList()
...
function Season123_2_0HeroGroupCardItem:setDragEnabled(isEnabled)
	if self._drag then
		self._drag.enabled = isEnabled
	end
end

function Season123_2_0HeroGroupCardItem:canExchange(targetItem)
	...
	for i = 1, maxCount do
		if i ~= srcSlot then
			...
			if tarCO and co and co.group == tarCO.group and tarPos ~= srcPos then
				GameFacade.showToast(Season123_2_0EquipItem.Toast_Same_Card)
...
function Season123_2_0HeroGroupCardItem:canMoveToPos(tarPos)
...
function Season123_2_0HeroGroupCardItem:_onDragFailTweenEnd()
	self:_setDragEnabled(true)
	gohelper.setAsLastSibling(self.parent._cardItem1.go)
end

function Season123_2_0HeroGroupCardItem:_onDragSuccessTweenEnd(targetItem)
	self:killTweenId()
	self:_setToPos(self._trscard, Season123_2_0HeroGroupCardItem.ZeroPos)
	self:_setToPos(targetItem._trscard, Season123_2_0HeroGroupCardItem.ZeroPos)
...
function Season123_2_0HeroGroupCardItem:killTweenId()
...
function Season123_2_0HeroGroupCardItem:destory()
	if self._drag then
...
return Season123_2_0HeroGroupCardItem
