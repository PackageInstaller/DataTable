-- chunkname: @modules/logic/seasonver/act123/view2_0/Season123_2_0HeroGroupHeroItem.lua

module("modules.logic.seasonver.act123.view2_0.Season123_2_0HeroGroupHeroItem", package.seeall)

local Season123_2_0HeroGroupHeroItem = class("Season123_2_0HeroGroupHeroItem", LuaCompBase)

function Season123_2_0HeroGroupHeroItem:ctor(SeasonHeroGroupListView)
	self._seasonHeroGroupListView = SeasonHeroGroupListView
end

function Season123_2_0HeroGroupHeroItem:init(go)
	self.go = go
	...
	self._cardItem1 = Season123_2_0HeroGroupCardItem.New(self._gocarditem1, self, {
		slot = 1
	})
...
function Season123_2_0HeroGroupHeroItem:onUpdateMO(mo, viewParam, replayHeroData)
	self.mo = mo
		...
		if curHeroGroupMo.isReplay then
			if not curHeroGroupMo.replay_hero_data[self.mo.heroUid] then
				...
				if replay_data then
					...
					skinConfig = FightConfig.instance:getSkinCO((replay_data or nil) and (replay_data.skin or self._heroMO.skin))

					if self._heroMO then
						if not self._heroMO.config then
							local heroCfg = HeroConfig.instance:getHeroCO(replayHeroData.heroId)
	...
	elseif self.trialCO then
		...
		skinConfig = self.trialCO.skin > 0 and SkinConfig.instance:getSkinCo(self.trialCO.skin) or SkinConfig.instance:getSkinCo(heroCo.skinId)
	...
	if not self._heroMO and not self._monsterCo and not self.trialCO then
		if replayHeroData then
			::label_3_0::

			local var_3_3 = replayHeroData.heroId
			local isHeroShow = true

			if replayHeroData.heroId then
				if replayHeroData.heroId ~= 0 then
					if not self.isLock then
						isHeroShow = not self.isRoleNumLock
			...
			if not isNoData then
				...
				local var_3_4 = self.isLock

				if not self.isLock then
					var_3_4 = self.isAidLock

					if not self.isAidLock then
						local isNoneShow = self.isRoleNumLock
...
function Season123_2_0HeroGroupHeroItem:checkSubGoVisable()
	if self.isLock then
		return false
	end
...
function Season123_2_0HeroGroupHeroItem:_initData()
	self._firstEnter = true

	gohelper.setActive(self._goselectdebuff, false)
end

function Season123_2_0HeroGroupHeroItem:_addEvents()
	...
	self.addEventCb(self, Season123EquipController.instance, Season123EquipEvent.EquipUpdate, self._updateEquipCards, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.HeroGroupIndexChanged, self._updateSeasonEquips, self)
...
function Season123_2_0HeroGroupHeroItem:_updateSeasonEquips()
	self:_updateEquips()
	self:_updateEquipCards()
	self:_updateAct123Hp()
end

function Season123_2_0HeroGroupHeroItem:_onClickHeroItem()
...
function Season123_2_0HeroGroupHeroItem:_onClickEquip()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123_2_0HeroGroupHeroItem:_updateEquips()
	self._equipType = -1
...
Season123_2_0HeroGroupHeroItem.NonCardSlotBlackMaskHeight = 362
Season123_2_0HeroGroupHeroItem.HasCardSlotBlackMashHeight = 200
Season123_2_0HeroGroupHeroItem.SingleSlotCardItem1Pos = Vector2.New(-3.8, 24.9)
Season123_2_0HeroGroupHeroItem.TwoSlotCardItem1Pos = Vector2.New(-51.6, 28.2)

function Season123_2_0HeroGroupHeroItem:_updateEquipCards()
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()
	...
	if curGroupMO.isReplay then
		...
		if self.trialCO and self.trialCO.act104EquipId1 > 0 then
			...
			equipUid1 = equips[1]
			equipId1 = seasonMO:getItemIdByUid(equips[1])
		end
	...
	if hasShowCardSlot then
		if not Season123_2_0HeroGroupHeroItem.NonCardSlotBlackMaskHeight then
			local targetBlackMaskHeight = Season123_2_0HeroGroupHeroItem.HasCardSlotBlackMashHeight
			...
			local targetSlot1Pos = Season123_2_0HeroGroupHeroItem.SingleSlotCardItem1Pos
...
function Season123_2_0HeroGroupHeroItem:_updateAct123Hp()
	local context = Season123Model.instance:getBattleContext()
...
function Season123_2_0HeroGroupHeroItem:setHp(hpRate)
...
function Season123_2_0HeroGroupHeroItem:_equipIconAddDrag(go)
	if self._drag then
		return
	end
...
function Season123_2_0HeroGroupHeroItem:_onBeginDrag(equipTransform, pointerEventData)
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123_2_0HeroGroupHeroItem:_onDrag(param, pointerEventData)
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123_2_0HeroGroupHeroItem:_onEndDrag(equipTransform, pointerEventData)
	self:killEquipTweenId()
...
function Season123_2_0HeroGroupHeroItem:canMoveCardToPos(tarPos)
...
function Season123_2_0HeroGroupHeroItem:topDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self.currentParent, dragTransform.gameObject)
end

function Season123_2_0HeroGroupHeroItem:resetDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self._goequip, dragTransform.gameObject)
end

function Season123_2_0HeroGroupHeroItem:getDragTransform()
	return self._equip.moveContainerTrs
end

function Season123_2_0HeroGroupHeroItem:getOrignDragPos()
	return self._goequip.transform.position
end

function Season123_2_0HeroGroupHeroItem:onDragEndFail()
...
function Season123_2_0HeroGroupHeroItem:onDragEndSuccess(targetHeroItem)
	...
	if not HeroSingleGroupModel.instance:isTemp() then
		...
		if Season123HeroGroupModel.instance:isEpisodeSeason123() then
			...
			extraData.heroGroup = seasonMO:getCurHeroGroup()
...
function Season123_2_0HeroGroupHeroItem:_playDragEndAudio(targetHeroItem)
...
function Season123_2_0HeroGroupHeroItem:_tweenToPos(transform, anchorPos)
...
function Season123_2_0HeroGroupHeroItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)
...
function Season123_2_0HeroGroupHeroItem:_moveToTarget(position)
...
function Season123_2_0HeroGroupHeroItem:_setEquipDragEnabled(isEnabled)
...
function Season123_2_0HeroGroupHeroItem:playHeroGroupHeroEffect(state)
...
function Season123_2_0HeroGroupHeroItem:playRestrictAnimation(needRestrictHeroUidDict, deadHeroUidDict)
...
function Season123_2_0HeroGroupHeroItem:setGrayFactor(value)
	ZProj.UGUIHelper.SetGrayFactor(self._charactericon.gameObject, value)
end
...
function Season123_2_0HeroGroupHeroItem:showCounterSign()
	local career
...
function Season123_2_0HeroGroupHeroItem:onItemBeginDrag(index)
...
function Season123_2_0HeroGroupHeroItem:onItemEndDrag(index, dragToIndex)
	ZProj.TweenHelper.DOScale(self.go.transform, 1, 1, 1, 0.2, nil, nil, nil, EaseType.Linear)
end

function Season123_2_0HeroGroupHeroItem:onItemCompleteDrag(index, dragToIndex, complete)
...
function Season123_2_0HeroGroupHeroItem:hideDragEffect()
	gohelper.setActive(self._goselectdebuff, false)
end

function Season123_2_0HeroGroupHeroItem:setHeroGroupEquipEffect(show)
	self._canPlayEffect = show
end

function Season123_2_0HeroGroupHeroItem:setParent(transform)
	self.currentParent = transform

	gohelper.addChildPosStay(transform.gameObject, self._subGO)
end

function Season123_2_0HeroGroupHeroItem:flowOriginParent()
	return
end

function Season123_2_0HeroGroupHeroItem:flowCurrentParent()
	return
end

function Season123_2_0HeroGroupHeroItem:getHeroItemList()
	return self._seasonHeroGroupListView:getHeroItemList()
end

function Season123_2_0HeroGroupHeroItem:killEquipTweenId()
...
function Season123_2_0HeroGroupHeroItem:_removeEvents()
	self._btnclickitem:RemoveClickListener()
	self._btnclickequip:RemoveClickListener()
end

function Season123_2_0HeroGroupHeroItem:onDestroy()
	self:_removeEvents()
	self:killEquipTweenId()
...
return Season123_2_0HeroGroupHeroItem
