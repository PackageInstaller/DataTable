-- chunkname: @modules/logic/seasonver/act123/view1_8/Season123_1_8EntryView.lua

module("modules.logic.seasonver.act123.view1_8.Season123_1_8EntryView", package.seeall)

local Season123_1_8EntryView = class("Season123_1_8EntryView", BaseView)

function Season123_1_8EntryView:onInitView()
...
function Season123_1_8EntryView:addEvents()
...
function Season123_1_8EntryView:removeEvents()
...
function Season123_1_8EntryView:_editableInitView()
	self._entryList = {}
	self._centerItem = nil
end

function Season123_1_8EntryView:onDestroyView()
...
function Season123_1_8EntryView:onOpen()
	AudioMgr.instance:trigger(AudioEnum.UI.season123_entryview_open)

	local actId = self.viewParam.actId

	self.addEventCb(self, Season123Controller.instance, Season123Event.GetActInfo, self.handleGetActInfo, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.StageInfoChanged, self.refreshCenter, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.TaskUpdated, self.refreshCenter, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.StageFinishWithoutStory, self.handleStageFinishWithoutStory, self)
	self.addEventCb(self, Season123EntryController.instance, Season123Event.EntrySceneLoaded, self.handleSceneLoaded, self)
	self.addEventCb(self, Season123EntryController.instance, Season123Event.EntryStageChanged, self.refreshUI, self)
	self.addEventCb(self, BackpackController.instance, BackpackEvent.UpdateItemList, self.refreshCardPackageUI, self)
	self.addEventCb(self, CurrencyController.instance, CurrencyEvent.CurrencyChange, self.handleItemChanged, self)
	self.addEventCb(self, BackpackController.instance, BackpackEvent.UpdateItemList, self.handleItemChanged, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.OtherViewAutoOpened, self.handleOtherViewAutoOpened, self)
	self.addEventCb(self, ViewMgr.instance, ViewEvent.OnCloseView, self.onCloseView, self)
...
function Season123_1_8EntryView:onClose()
...
function Season123_1_8EntryView:checkFirstOpenOverview()
...
function Season123_1_8EntryView:checkSceneIsLoaded()
	return
end

function Season123_1_8EntryView:refreshUI()
...
function Season123_1_8EntryView:refreshCenter()
	if self._centerItem then
...
function Season123_1_8EntryView:refreshPageBtn()
...
function Season123_1_8EntryView:refreshCardPackageUI()
...
function Season123_1_8EntryView:refreshUTTURetailTicket()
	...
	if isFunctionOpen then
		...
		if curNum == 0 then
			if not ("<color=#CF4543>" .. curNum .. "</color>/" .. maxNum) then
				self._txtpropnum.text = curNum .. "/" .. maxNum
			end
		end
	end
end

function Season123_1_8EntryView:refreshStoreCoin()
...
function Season123_1_8EntryView:refreshRemainTime()
	local actMO = ActivityModel.instance:getActMO(self.viewParam.actId)
...
function Season123_1_8EntryView:handleGetActInfo(actId)
	if self.viewParam.actId == actId then
		self:refreshUI()
	end
end

function Season123_1_8EntryView:handleOtherViewAutoOpened()
	self._anim:Play(Season123_1_8EntryView.Anim.Close)
end

function Season123_1_8EntryView:handleItemChanged()
	self:refreshUTTURetailTicket()
	self:refreshStoreCoin()
end

function Season123_1_8EntryView:handleSceneLoaded()
	self:checkSceneLoaded()
end

function Season123_1_8EntryView:handleStageFinishWithoutStory()
	if self._anim then
		self._anim:Play(Season123_1_8EntryView.Anim.Switch, 0, 0)
	end
end

function Season123_1_8EntryView:checkSceneLoaded()
	if self._isCenterInited then
		return
	end

	local sceneRoot = CameraMgr.instance:getSceneRoot()
	local sceneGO = gohelper.findChild(sceneRoot, "Season123_1_8EntryScene/scene")

	if sceneGO then
		gohelper.setActive(self._goentranceitem, true)

		self._centerItem = Season123_1_8EntryItem.New()
...
function Season123_1_8EntryView:_btnprevOnClick()
	UIBlockMgrExtend.instance:setNeedCircleMv(false)
	UIBlockMgr.instance:startBlock("switchStage")
	self._animExcessive:Play(Season123_1_8EntryView.Anim.Hard)
...
function Season123_1_8EntryView:_btnnextOnClick()
	UIBlockMgrExtend.instance:setNeedCircleMv(false)
	UIBlockMgr.instance:startBlock("switchStage")
	self._animExcessive:Play(Season123_1_8EntryView.Anim.Story)
	AudioMgr.instance:trigger(AudioEnum.UI.season123_stage_switch)
...
function Season123_1_8EntryView:_btnStoreOnClick()
	Season123Controller.instance:openSeasonStoreView(self.viewParam.actId)
end

function Season123_1_8EntryView:_btnStoryOnClick()
	Season123Controller.instance:openSeasonStoryView({
		actId = self.viewParam.actId
	})
end

function Season123_1_8EntryView:_btnEquipBookOnClick()
	Season123Controller.instance:openSeasonEquipBookView(self.viewParam.actId)
end

function Season123_1_8EntryView:_btnOverviewOnClick()
	Season123Controller.instance:openSeasonOverview({
		actId = self.viewParam.actId
	})
end

function Season123_1_8EntryView:_btnAchievementOnClick()
	Season123Controller.instance:openSeasonTaskView({
		actId = self.viewParam.actId
	})
end

function Season123_1_8EntryView:_btnCardPackageOnClick()
	Season123Controller.instance:openSeasonCardPackageView({
		actId = self.viewParam.actId
	})
end

function Season123_1_8EntryView:_btnRetailOnClick()
...
function Season123_1_8EntryView:_enterRetailView()
	Season123Controller.instance:openSeasonRetail({
		actId = self.viewParam.actId
	})
end

function Season123_1_8EntryView:_btnTrialOnClick()
	Season123EntryController.instance:enterTrailFightScene()
end

function Season123_1_8EntryView:onCloseView(viewName)
	if viewName == ViewName.Season123_1_8EpisodeListView then
		self._anim:Play(Season123_1_8EntryView.Anim.Switch1, 0, 0)
	end

	if viewName == ViewName.Season123_1_8RetailView then
		self._anim:Play(Season123_1_8EntryView.Anim.Switch, 0, 0)
	end

	if viewName == ViewName.Season123_1_8StoryPagePopView then
		self._anim:Play(Season123_1_8EntryView.Anim.Switch, 0, 0)
		TaskDispatcher.runDelay(self._autoClickNext, self, 0.5)
	end
end

function Season123_1_8EntryView:_switchStage()
	Season123EntryController.instance:switchStage(self.isNext)

	self.isNext = nil
end

function Season123_1_8EntryView:_endBlock()
	UIBlockMgr.instance:endBlock("switchStage")
	UIBlockMgrExtend.instance:setNeedCircleMv(true)
end

function Season123_1_8EntryView:initRedDot()
...
function Season123_1_8EntryView:checkHasReadUnlockStory()
	Season123Controller.instance:checkHasReadUnlockStory(self.viewParam.actId)
end

function Season123_1_8EntryView:_autoClickNext()
