-- chunkname: @modules/logic/seasonver/act123/view1_8/Season123_1_8EquipHeroView.lua

module("modules.logic.seasonver.act123.view1_8.Season123_1_8EquipHeroView", package.seeall)

local Season123_1_8EquipHeroView = class("Season123_1_8EquipHeroView", BaseView)

function Season123_1_8EquipHeroView:onInitView()
...
function Season123_1_8EquipHeroView:addEvents()
...
function Season123_1_8EquipHeroView:onDestroyView()
...
function Season123_1_8EquipHeroView:onOpen()
	local actId = self.viewParam.actId

	if not self.viewParam.slot then
		if not self.viewParam.group then
			self.addEventCb(self, Season123EquipHeroController.instance, Season123EquipEvent.EquipUpdate, self.handleEquipUpdate, self)
			self.addEventCb(self, Season123EquipHeroController.instance, Season123EquipEvent.EquipChangeCard, self.handleEquipCardChanged, self)
			self.addEventCb(self, Season123EquipHeroController.instance, Season123EquipEvent.EquipChangeSlot, self.handleEquipSlotChanged, self)
			self.addEventCb(self, Season123EquipBookController.instance, Season123Event.OnItemChange, self.refreshUI, self)
			Season123EquipHeroController.instance:onOpenView(actId, self.viewParam.stage, self.viewParam.slot, self.handleSaveSucc, self, self.viewParam.equipUidList)
			self:refreshUI()
		end
	end
end

function Season123_1_8EquipHeroView:onClose()
	return
end

function Season123_1_8EquipHeroView:refreshUI()
...
function Season123_1_8EquipHeroView:handleEquipUpdate()
	self:refreshDescGroup()
	self:refreshSlots()
end

function Season123_1_8EquipHeroView:refreshSlots()
	for slotIndex = 1, Activity123Enum.MainCardNum do
		self:refreshSlot(slotIndex)
	end
end

function Season123_1_8EquipHeroView:refreshSlot(slotIndex)
...
Season123_1_8EquipHeroView.MaxUISlot = 2

function Season123_1_8EquipHeroView:refreshDescGroup()
	for slot = 1, Season123_1_8EquipHeroView.MaxUISlot do
...
function Season123_1_8EquipHeroView:refreshDesc(descItem, itemUid, slotIndex)
...
function Season123_1_8EquipHeroView:refreshProps(itemCfg, descItem, colorStr)
	local processedSet = {}
	local isDirty = false
...
function Season123_1_8EquipHeroView:refreshSkills(itemCfg, descItem, colorStr)
...
function Season123_1_8EquipHeroView:getOrCreatePropText(index, descItem)
...
function Season123_1_8EquipHeroView:getOrCreateSkillText(index, descItem)
...
function Season123_1_8EquipHeroView:getOrCreateSlot(slotIndex)
...
function Season123_1_8EquipHeroView:getOrCreateSlotIcon(slotItem)
	...
	if not icon then
		...
		icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season123_1_8CelebrityCardEquip)
		slotItem.icon = icon
	end
...
function Season123_1_8EquipHeroView:getOrCreateDesc(index)
...
function Season123_1_8EquipHeroView:onClickSlot(slotIndex)
	if Season123EquipHeroItemListModel.instance.curSelectSlot ~= slotIndex then
...
function Season123_1_8EquipHeroView:handleSwitchAnimFrame()
	self:refreshSlots()
	self:refreshDescGroup()
end

function Season123_1_8EquipHeroView.handleSlotSwitchAnimFrame(slotItem)
	local mainView = slotItem.mainView

	mainView:refreshDescGroup()
end

function Season123_1_8EquipHeroView:handleEquipCardChanged(param)
...
function Season123_1_8EquipHeroView:handleEquipSlotChanged()
	self._animator:Play("switch", 0, 0)
	self:refreshSlots()
end

function Season123_1_8EquipHeroView:handleSaveSucc()
...
function Season123_1_8EquipHeroView:_btnequipOnClick()
...
function Season123_1_8EquipHeroView:_btnhandbookOnClick()
	Season123Controller.instance:openSeasonEquipBookView(self.viewParam.actId)
end

return Season123_1_8EquipHeroView
