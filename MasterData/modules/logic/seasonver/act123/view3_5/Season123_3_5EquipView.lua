-- chunkname: @modules/logic/seasonver/act123/view3_5/Season123_3_5EquipView.lua

module("modules.logic.seasonver.act123.view3_5.Season123_3_5EquipView", package.seeall)

local Season123_3_5EquipView = class("Season123_3_5EquipView", BaseView)

function Season123_3_5EquipView:onInitView()
...
function Season123_3_5EquipView:addEvents()
...
function Season123_3_5EquipView:removeEvents()
	self._btnequip:RemoveClickListener()
	self._btnopenhandbook:RemoveClickListener()
end

function Season123_3_5EquipView:_editableInitView()
...
function Season123_3_5EquipView:onDestroyView()
...
function Season123_3_5EquipView:onOpen()
	if not self.viewParam.pos then
		local actId = self.viewParam.actId

		if not self.viewParam.slot then
			if not self.viewParam.group then
				local group = 1
				local layer = self.viewParam.layer
				local stage = self.viewParam.stage

				self.addEventCb(self, Season123EquipController.instance, Season123EquipEvent.EquipUpdate, self.handleEquipUpdate, self)
				self.addEventCb(self, HeroGroupController.instance, HeroGroupEvent.OnSnapshotSaveSucc, self.handleSaveSucc, self)
				self.addEventCb(self, Season123EquipController.instance, Season123EquipEvent.EquipChangeCard, self.handleEquipCardChanged, self)
				self.addEventCb(self, Season123EquipController.instance, Season123EquipEvent.EquipChangeSlot, self.handleEquipSlotChanged, self)
				Season123EquipController.instance:onOpenView(actId, group, stage, layer, self.viewParam.pos, self.viewParam.slot)
				self:refreshUI()
			end
		end
	end
...
function Season123_3_5EquipView:onClose()
	return
end

function Season123_3_5EquipView:refreshUI()
...
function Season123_3_5EquipView:handleEquipUpdate()
	self:refreshSlots()
	self:refreshDescGroup()
end

function Season123_3_5EquipView:refreshSlots()
...
function Season123_3_5EquipView:refreshSlot(slotIndex)
...
Season123_3_5EquipView.MaxUISlot = 2

function Season123_3_5EquipView:refreshDescGroup()
	for slot = 1, Season123_3_5EquipView.MaxUISlot do
...
function Season123_3_5EquipView:refreshDesc(descItem, itemUid, slotIndex)
	...
	if itemUid == Activity123Enum.EmptyUid then
		...
		if itemId then
			...
			if itemCO then
				...
				local isSelect = Season123EquipItemListModel.instance.curSelectSlot == slotIndex
				...
				descItem.canvasGroup.alpha = isSelect and 1 or 0.5
			else
...
function Season123_3_5EquipView:refreshProps(itemCfg, descItem, colorStr)
	local processedSet = {}
	local isDirty = false
...
function Season123_3_5EquipView:refreshSkills(itemCfg, descItem, colorStr)
...
function Season123_3_5EquipView:setSkillClickCallBack(effId, clickPosition)
...
function Season123_3_5EquipView:getOrCreatePropText(index, descItem)
...
function Season123_3_5EquipView:getOrCreateSkillText(index, descItem)
...
function Season123_3_5EquipView:getOrCreateSlot(slotIndex)
...
function Season123_3_5EquipView:getOrCreateSlotIcon(slotItem)
	...
	if not icon then
		...
		icon = MonoHelper.addNoUpdateLuaComOnceToGo(go, Season123_3_5CelebrityCardEquip)
		slotItem.icon = icon
	end
...
function Season123_3_5EquipView:getOrCreateDesc(index)
	...
	if not descItem then
		...
		descItem.goAttrDesc = gohelper.findChild(descItem.go, "desc/scroll_desc/Viewport/Content/attrlist/#go_attributeitem")
		descItem.goSkillDesc = gohelper.findChild(descItem.go, "desc/scroll_desc/Viewport/Content/skilldesc/#go_skilldescitem")
		descItem.goAttrParent = gohelper.findChild(descItem.go, "desc/scroll_desc/Viewport/Content/attrlist")
		descItem.propItems = {}
		descItem.skillItems = {}
		descItem.canvasGroup = gohelper.onceAddComponent(descItem.go, typeof(UnityEngine.CanvasGroup))
...
function Season123_3_5EquipView:onClickSlot(slotIndex)
...
function Season123_3_5EquipView:handleSaveSucc()
...
function Season123_3_5EquipView:handleSwitchAnimFrame()
	self:refreshSlots()
	self:refreshDescGroup()
end

function Season123_3_5EquipView.handleSlotSwitchAnimFrame(slotItem)
	local mainView = slotItem.mainView

	mainView:refreshDescGroup()
end

function Season123_3_5EquipView:handleEquipCardChanged(param)
...
function Season123_3_5EquipView:handleEquipSlotChanged()
	self._animator:Play("switch", 0, 0)
	self:refreshSlots()
end

function Season123_3_5EquipView:_btnequipOnClick()
...
function Season123_3_5EquipView:_btnhandbookOnClick()
	Season123Controller.instance:openSeasonEquipBookView(self.viewParam.actId)
end

return Season123_3_5EquipView
