-- chunkname: @modules/logic/seasonver/act123/view3_5/Season123_3_5TaskView.lua

module("modules.logic.seasonver.act123.view3_5.Season123_3_5TaskView", package.seeall)

local Season123_3_5TaskView = class("Season123_3_5TaskView", BaseView)

function Season123_3_5TaskView:onInitView()
...
function Season123_3_5TaskView:addEvents()
	...
	self.addEventCb(self, Season123Controller.instance, Season123Event.OnTaskRewardGetFinish, self._playGetRewardFinishAnim, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.TaskUpdated, self.refreshRedDot, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.clickTaskMapItem, self.moveScrollPos, self)
end

function Season123_3_5TaskView:removeEvents()
	self._btnreward:RemoveClickListener()
	self._btntask:RemoveClickListener()
	self._btnarrowdown:RemoveClickListener()
	self._btnarrowtop:RemoveClickListener()
	self.removeEventCb(self, Season123Controller.instance, Season123Event.OnTaskRewardGetFinish, self._playGetRewardFinishAnim, self)
	self.removeEventCb(self, Season123Controller.instance, Season123Event.TaskUpdated, self.refreshRedDot, self)
	self.addEventCb(self, Season123Controller.instance, Season123Event.clickTaskMapItem, self.moveScrollPos, self)
end

Season123_3_5TaskView.TaskMaskTime = 0.65
Season123_3_5TaskView.TaskGetAnimTime = 0.567
Season123_3_5TaskView.StageMapItemCount = 4
Season123_3_5TaskView.moveDistance = 50

function Season123_3_5TaskView:_btnrewardOnClick()
	self.curTaskType = Activity123Enum.TaskRewardViewType

	self:refreshUI()
end

function Season123_3_5TaskView:_btntaskOnClick()
	self.curTaskType = Activity123Enum.TaskNormalType

	self:refreshUI()
end

function Season123_3_5TaskView:_btnArrowDownOnClick()
	self.curStage = Mathf.Min(self.curStage + 1, Season123_3_5TaskView.StageMapItemCount)
...
function Season123_3_5TaskView:_btnArrowUpOnClick()
...
function Season123_3_5TaskView:_onScrollDragBegin(param, eventData)
	self.scrollStartPos = eventData.position
end

function Season123_3_5TaskView:_onScrollDragEnd(param, eventData)
	local moveOffset = eventData.position - self.scrollStartPos
	local canMove = Mathf.Abs(moveOffset.y) - Season123_3_5TaskView.moveDistance >= 0
...
function Season123_3_5TaskView:_editableInitView()
	...
	self._taskAnimRemoveItem:setMoveAnimationTime(Season123_3_5TaskView.TaskMaskTime - Season123_3_5TaskView.TaskGetAnimTime)
...
function Season123_3_5TaskView:onOpen()
	...
	if self.stageId ~= nil then
		Season123TaskModel.instance.curStage = self.stageId or Season123TaskModel.instance.curStage
	end

	if self.stageId ~= nil then
		self.curTaskType = Activity123Enum.TaskRewardViewType or self.curTaskType
	end

	self:createStageMapItem()
	self:refreshUI()
	self:refreshArrowRedDot()
	UIBlockMgr.instance:startBlock("playEnterSeason123_3_5TaskViewAnim")
	TaskDispatcher.runDelay(self.endEnterAnimBlock, self, 0.8)
	AudioMgr.instance:trigger(AudioEnum.Season123.play_ui_pkls_prop_get)
end

function Season123_3_5TaskView:refreshUI()
	self.curStage = Season123TaskModel.instance.curStage

	if not self.curTaskType then
		if not Season123TaskModel.instance.curTaskType then
			local curTaskType = self.curTaskType
...
function Season123_3_5TaskView:refreshRedDot()
...
function Season123_3_5TaskView:refreshArrowRedDot()
	...
	local downIndex = Mathf.Min(self.curStage + 1, Season123_3_5TaskView.StageMapItemCount)
...
function Season123_3_5TaskView:refreshArrowUI()
	gohelper.setActive(self._btnarrowdown.gameObject, self.curStage < Season123_3_5TaskView.StageMapItemCount)
	gohelper.setActive(self._btnarrowtop.gameObject, self.curStage > 1)
	self:refreshArrowRedDot()
end

function Season123_3_5TaskView:_playGetRewardFinishAnim(index)
	if index then
		self.removeIndexTab = {
			index
		}
	...
	TaskDispatcher.runDelay(self.delayPlayFinishAnim, self, Season123_3_5TaskView.TaskGetAnimTime)
end

function Season123_3_5TaskView:delayPlayFinishAnim()
	self._taskAnimRemoveItem:removeByIndexs(self.removeIndexTab)
end

function Season123_3_5TaskView:endEnterAnimBlock()
	UIBlockMgr.instance:endBlock("playEnterSeason123_3_5TaskViewAnim")
end

function Season123_3_5TaskView:createStageMapItem()
	for index = 1, Season123_3_5TaskView.StageMapItemCount do
		...
		if not stageMapItem then
			...
			stageMapItem.item = MonoHelper.addNoUpdateLuaComOnceToGo(stageMapItem.itemGO, Season123_3_5TaskMapItem, {
				stage = index,
				actId = self.actId
			})
...
function Season123_3_5TaskView:moveScrollPos()
	self.curStage = Season123TaskModel.instance.curStage
...
function Season123_3_5TaskView:onClose()
