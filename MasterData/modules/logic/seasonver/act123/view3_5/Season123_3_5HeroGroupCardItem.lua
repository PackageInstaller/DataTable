-- chunkname: @modules/logic/seasonver/act123/view3_5/Season123_3_5HeroGroupCardItem.lua

module("modules.logic.seasonver.act123.view3_5.Season123_3_5HeroGroupCardItem", package.seeall)

local Season123_3_5HeroGroupCardItem = class("Season123_3_5HeroGroupCardItem", UserDataDispose)

Season123_3_5HeroGroupCardItem.TweenDuration = 0.16
Season123_3_5HeroGroupCardItem.DragOffset = Vector2(0, 40)
Season123_3_5HeroGroupCardItem.ZeroPos = Vector2(-2.7, -5)
Season123_3_5HeroGroupCardItem.ZeroScale = 0.39

function Season123_3_5HeroGroupCardItem:ctor(go, parent, param)
	self:__onInit()
...
function Season123_3_5HeroGroupCardItem:init()
...
function Season123_3_5HeroGroupCardItem:AddDrag(go)
	if self._drag then
		return
	end
...
function Season123_3_5HeroGroupCardItem:setData(data, layer, equipId, equipUid, hasTrialEquip)
	...
	self:updateView()
	self:setActive(self.slotUnlock)
end

function Season123_3_5HeroGroupCardItem:setActive(isActive)
	self.isActive = isActive
end

function Season123_3_5HeroGroupCardItem:updateView()
	...
	if self.posUnlock then
		...
		if equipId ~= nil and equipId ~= 0 then
			if not self._seasonCardItem then
				self._seasonCardItem = Season123_3_5CelebrityCardItem.New()
...
function Season123_3_5HeroGroupCardItem:playEmptyUnlockAnim()
	local pos = self.id - 1
...
function Season123_3_5HeroGroupCardItem:getEquipId(actId, groupId)
	return self._equipId, self._equipUid
end

function Season123_3_5HeroGroupCardItem:hasUseSeasonEquipCard()
	return self._hasUseSeasonEquipCard
end

function Season123_3_5HeroGroupCardItem:_btnCardClick()
	if self.inDrag then
		return
	end
...
function Season123_3_5HeroGroupCardItem:canDrag()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return false
	end
...
function Season123_3_5HeroGroupCardItem:_onBeginDrag(dragTransform, pointerEventData)
	...
	self:_tweenToPos(self._trscard, anchorPos + Season123_3_5HeroGroupCardItem.DragOffset, true)

	local scale = Season123_3_5HeroGroupCardItem.ZeroScale * 1.7

	self.tweenId = ZProj.TweenHelper.DOScale(self._trscard, scale, scale, scale, Season123_3_5HeroGroupCardItem.TweenDuration)
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, 0, Season123_3_5HeroGroupCardItem.TweenDuration)
	self.inDrag = true
end

function Season123_3_5HeroGroupCardItem:_onDrag(dragTransform, pointerEventData)
	if not self:canDrag() then
		self.inDrag = false
	...
	self:_tweenToPos(self._trscard, anchorPos + Season123_3_5HeroGroupCardItem.DragOffset)

	self.inDrag = true
end

function Season123_3_5HeroGroupCardItem:_onEndDrag(dragTransform, pointerEventData)
	self.inDrag = false
	...
	local scale = Season123_3_5HeroGroupCardItem.ZeroScale * 1
	...
	if not targetItem or not targetItem:canExchange(self) then
		self._setToPos(self, self._trscard, Season123_3_5HeroGroupCardItem.ZeroPos, true, self._onDragFailTweenEnd, self)

		self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, self.orignRoteZ, Season123_3_5HeroGroupCardItem.TweenDuration)
	...
	self.rotaTweenId = ZProj.TweenHelper.DOLocalRotate(self._trscard, 0, 0, targetItem.orignRoteZ, Season123_3_5HeroGroupCardItem.TweenDuration)
end

function Season123_3_5HeroGroupCardItem:_tweenToPos(transform, anchorPos)
	...
	if math.abs(curAnchorX - anchorPos.x) > 10 or math.abs(curAnchorY - anchorPos.y) > 10 then
		self.posTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season123_3_5HeroGroupCardItem.TweenDuration)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
	end
end

function Season123_3_5HeroGroupCardItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)

	if tween then
		self.moveTweenId = ZProj.TweenHelper.DOAnchorPos(transform, anchorPos.x, anchorPos.y, Season123_3_5HeroGroupCardItem.TweenDuration, callback, callbackObj, param)
	else
		recthelper.setAnchor(transform, anchorPos.x, anchorPos.y)
...
function Season123_3_5HeroGroupCardItem:_moveToTarget(position)
	...
	if heroList then
		for index, heroItem in pairs(heroList) do
			...
			for i = 1, maxCount do
				local cardItem = heroItem.cardList[i]
...
function Season123_3_5HeroGroupCardItem:_setDragEnabled(isEnabled)
	local heroList = self.parent:getHeroItemList()

	for i, heroItem in ipairs(heroList) do
		for j = 1, 2 do
			local cardItem = heroItem.cardList[j]
...
function Season123_3_5HeroGroupCardItem:setDragEnabled(isEnabled)
	if self._drag then
		self._drag.enabled = isEnabled
	end
end

function Season123_3_5HeroGroupCardItem:canExchange(targetItem)
	...
	for i = 1, maxCount do
		if i ~= srcSlot then
			...
			if tarCO and co and co.group == tarCO.group and tarPos ~= srcPos then
				GameFacade.showToast(Season123_3_5EquipItem.Toast_Same_Card)
...
function Season123_3_5HeroGroupCardItem:canMoveToPos(tarPos)
...
function Season123_3_5HeroGroupCardItem:_onDragFailTweenEnd()
	self:_setDragEnabled(true)
end

function Season123_3_5HeroGroupCardItem:_onDragSuccessTweenEnd(targetItem)
	self:killTweenId()
	self:_setToPos(self._trscard, Season123_3_5HeroGroupCardItem.ZeroPos)
	self:_setToPos(targetItem._trscard, Season123_3_5HeroGroupCardItem.ZeroPos)
...
function Season123_3_5HeroGroupCardItem:killTweenId()
...
function Season123_3_5HeroGroupCardItem:destroy()
	if self._drag then
...
return Season123_3_5HeroGroupCardItem
