-- chunkname: @modules/logic/seasonver/act123/view3_5/Season123_3_5SettlementView.lua

module("modules.logic.seasonver.act123.view3_5.Season123_3_5SettlementView", package.seeall)

local Season123_3_5SettlementView = class("Season123_3_5SettlementView", BaseViewExtended)

function Season123_3_5SettlementView:onInitView()
	...
	self.goStarContent = gohelper.findChild(self.viewGO, "#go_settlement/left/#simage_circlebg/layout")
	self.goStar = gohelper.findChild(self.viewGO, "#go_settlement/left/#simage_circlebg/layout/go_star")

	gohelper.setActive(self.goStar, false)

	self.levelImgDict = {}
	self.levelImgDict.S = gohelper.findChild(self.viewGO, "#go_settlement/right/herocard/level/image_s")
	self.levelImgDict.A = gohelper.findChild(self.viewGO, "#go_settlement/right/herocard/level/image_a")
	self.levelImgDict.B = gohelper.findChild(self.viewGO, "#go_settlement/right/herocard/level/image_b")
	self.levelImgDict.C = gohelper.findChild(self.viewGO, "#go_settlement/right/herocard/level/image_c")
...
function Season123_3_5SettlementView:addEvents()
	self._btnclose:AddClickListener(self._btncloseOnClick, self)
	self._btnstat:AddClickListener(self._btnstatOnClick, self)
	self.addClickCb(self, self._btncoverrecord, self._onBtnCoverRecordClick, self)
	self.addClickCb(self, self._btnconfirmfriend, self._onBtnConfirmFriendClick, self)
	self.addEventCb(self, DungeonController.instance, DungeonEvent.OnCoverDungeonRecordReply, self._onCoverDungeonRecordReply, self)
...
function Season123_3_5SettlementView:removeEvents()
...
function Season123_3_5SettlementView:_btnstatOnClick()
	ViewMgr.instance:openView(ViewName.FightStatView)
end

function Season123_3_5SettlementView:_showSettlement()
...
function Season123_3_5SettlementView:_onRewardShow()
	self:_showSettlementReward()
end

function Season123_3_5SettlementView:_dailyShowHero()
	self._heroIndex = 0
...
function Season123_3_5SettlementView:_showHeroItem()
	self._heroIndex = self._heroIndex + 1

	gohelper.setActive(self._heroList[self._heroIndex].viewGO, true)
end

function Season123_3_5SettlementView:_showGetCardView()
	if self.equip_cards then
		self._showTipsFlow = FlowSequence.New()

		self._showTipsFlow:addWork(Season123_3_5SettlementTipsWork.New())
		self._showTipsFlow:registerDoneListener(self._onShowTipsFlowDone, self)

		local data = {}

		data.delayTime = 0
		data.equipList = self.equip_cards

		self._showTipsFlow:start(data)
	end
end

function Season123_3_5SettlementView:_onShowTipsFlowDone()
	self.equip_cards = nil
end

function Season123_3_5SettlementView:_checkNewRecord()
...
function Season123_3_5SettlementView:_onCoverDungeonRecordReply(isCover)
...
function Season123_3_5SettlementView:_detectCoverRecord()
	...
	if FightResultModel.instance.canUpdateDungeonRecord then
		...
		self._txtcurroundcount.color = FightResultModel.instance.newRecordRound >= FightResultModel.instance.oldRecordRound and GameUtil.parseColor("#272525") or GameUtil.parseColor("#AC5320")
	end
end

function Season123_3_5SettlementView:_onBtnCoverRecordClick()
	DungeonRpc.instance:sendCoverDungeonRecordRequest(true)
end

function Season123_3_5SettlementView:_detectAddFriend()
	if not FightResultModel.instance.assistUserId then
		local assistUserId = 0
		...
		if battleContext then
			if not battleContext.actId then
				local actId

				if battleContext then
					if not battleContext.stage then
						local stage
						local isShowAddFriend = not ((numAssistUserId ~= 0 or nil) and SocialModel.instance:isMyFriendByUserId(assistUserId)) and (actId and stage or nil) and Season123ProgressUtils.checkStageIsFinish(actId, stage)

						if isShowAddFriend then
							if not FightResultModel.instance.assistNickname then
								self._txtfriendtip.text = formatLuaLang("season123_add_assist_friend", FightResultModel.instance.assistNickname)
							end
						end
...
function Season123_3_5SettlementView:_onBtnConfirmFriendClick()
	if not FightResultModel.instance.assistUserId then
		local assistUserId = 0
...
function Season123_3_5SettlementView:_onAddAssistFriendReply()
	gohelper.setActive(self._gofriendpart, false)
end

function Season123_3_5SettlementView:_editableInitView()
...
function Season123_3_5SettlementView:_btncloseOnClick()
...
function Season123_3_5SettlementView:onOpen()
	self._startTime = Time.realtimeSinceStartup
	...
	if not self._battleContext.layer then
		self._config = Season123Config.instance:getSeasonEpisodeCo(Season123Model.instance:getCurSeasonId(), self._battleContext.stage, self._battleContext.layer)
		...
		Season123Controller.instance:checkAndHandleEffectEquip({
			...
			layer = self._battleContext.layer
		})
		...
		if FightResultModel.instance.firstPass then
			...
			local equipList = string.splitToNumber(episodeCo.gottenEquip, "#")

			for _, v in ipairs(equipList) do
				table.insert(equip_cards, v)
			end
		end
...
function Season123_3_5SettlementView:_refreshRound()
	...
	if self._battleContext.stage and self._battleContext.layer then
		...
		if episodeMO and episodeMO.round then
			self._txtindex.text = tostring(episodeMO.round)
		end
	end
end

function Season123_3_5SettlementView:_showSettlementReward()
...
function Season123_3_5SettlementView:_onRewardItemShow(obj, data, index)
	local itemIconGO = gohelper.findChild(obj, "go_prop")
	local receiveGO = gohelper.findChild(obj, "go_receive")

	if data.materilType == MaterialEnum.MaterialType.Season123EquipCard then
		local cardItem = Season123_3_5CelebrityCardItem.New()
...
function Season123_3_5SettlementView:getCurEpisodeRewards()
	local actId = self._battleContext.actId
	local stage = self._battleContext.stage
	local layer = self._battleContext.layer
	local episodeCo = Season123Config.instance:getSeasonEpisodeCo(actId, stage, layer)
...
function Season123_3_5SettlementView:_onHeroItemLoaded()
	self._heroList = {}
	...
	if battleCO then
		if not battleCO.playerMax then
			local playerMax = ModuleEnum.HeroCountInGroup
			...
			if not is_replay then
				...
				for i = 1, 4 do
					if trailDict[i] then
						...
						if trialCo then
							local hero_uid = HeroGroupHandler.getTrialHeroUID(trialCo.id, trialCo.trialTemplate)
							local equip = trailDict[i].equipUid

							if equip123_dic[hero_uid] then
								local equip_104 = equip123_dic[hero_uid].equipId
								local heroItem = self:openSubView(Season123_3_5SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, nil, equip, equip_104, nil, trailDict[i])
						...
						var_27_3 = hero_uid ~= "0" and equip123_dic[hero_uid] and equip123_dic[hero_uid].equipId or false

						local equip_104 = true

						goto label_27_1

						::label_27_1::

						local heroItem = self:openSubView(Season123_3_5SettlementHeroItem, self._hero_obj_list[i], nil, is_replay, hero_uid, equip, equip_104)
				...
				if equip123_dic[character_uid] then
					local mainCardIds = equip123_dic[character_uid].equipId

					if mainCardIds then
						self:setMainPosCardItem(1, mainCardIds[1])
						self:setMainPosCardItem(2, mainCardIds[2])
					end
				end
			else
...
function Season123_3_5SettlementView:setMainPosCardItem(index, equipId)
	...
	if not seasonMO.unlockIndexSet[slotIndex] then
		...
		if equipId and equipId ~= 0 then
			...
			self:openSubView(Season123_3_5CelebrityCardGetItem, Season123_3_5CelebrityCardItem.AssetPath, goPos, equipId, true)
		else
	...
	if index == 1 then
		self:refreshLevel(equipId)
	end
end

function Season123_3_5SettlementView:isItemID()
	return true
end

function Season123_3_5SettlementView:_refreshHeroItemPos()
...
function Season123_3_5SettlementView:_onGetFightRecordGroupReply(fightGroupMO)
	for i = 1, 4 do
		...
		local heroItem = self:openSubView(Season123_3_5SettlementHeroItem, obj, nil, true, hero, equip, equip_104, replay_data)
...
function Season123_3_5SettlementView:setMainPosCardItemById(index, mainEquip)
	local equipId
	...
	if not seasonMO.unlockIndexSet[slotIndex] then
		...
		if equipId and equipId ~= 0 then
			gohelper.setActive(goEmpty, false)
			gohelper.setActive(goPos, true)
			self:openSubView(Season123_3_5CelebrityCardGetItem, Season123_3_5CelebrityCardItem.AssetPath, goPos, nil, nil, equipId)
		else
			gohelper.setActive(goEmpty, true)
			gohelper.setActive(goPos, false)
		end
...
function Season123_3_5SettlementView:_delayClosetip()
	gohelper.setActive(self._gofightsucc, false)
	self:_showSettlement()
end

function Season123_3_5SettlementView:refreshStar()
	local dataList = {}
	local seasonMO = Season123Model.instance:getActInfo(self._battleContext.actId)
	local curStageMO = seasonMO:getStageMO(self._battleContext.stage)

	if curStageMO then
		local episodeMO = curStageMO:getEpisodeMo(self._battleContext.layer or 1)
		local totalStar = Season123Config.instance:getSeasonEpisodeTotalStar(self._episode_config.id)
		...
		if episodeMO then
			if not episodeMO.star then
				...
				for i = 1, totalStar do
					...
					table.insert(dataList, isFinish)
				end

				gohelper.CreateObjList(self, self._onStarItemShow, dataList, self.goStarContent, self.goStar, nil, nil, nil, 1)
				AudioMgr.instance:trigger(AudioEnum.Season123.play_fight_ui_uttu_pass_level)
			end
		end
	end
end

function Season123_3_5SettlementView:_onStarItemShow(obj, data, index)
	local isFinish = data
	local goLight = gohelper.findChild(obj, "light")

	gohelper.setActive(goLight, isFinish)
end

function Season123_3_5SettlementView:refreshLevel(equipId)
	local progressData = FightDataHelper.fieldMgr.progressDic:getDataByShowId(FightEnum.ProgressId.Progress_9)
	local cardConfig = Season123Config.instance:getSeasonEquipCo(equipId)

	if not cardConfig or not progressData or string.nilorempty(cardConfig.scoreStage) then
		for k, v in pairs(self.levelImgDict) do
	...
	local curScore = progressData.value
	local str = "C#0|" .. cardConfig.scoreStage
	...
	for _, v in ipairs(arr) do
		...
		tempTab.levelRoot = self.levelImgDict[tab[1]]
		tempTab.score = tonumber(tab[2])
		tempTab.level = tab[1]

		table.insert(list, tempTab)
	end
...
function Season123_3_5SettlementView:onClose()
...
return Season123_3_5SettlementView
