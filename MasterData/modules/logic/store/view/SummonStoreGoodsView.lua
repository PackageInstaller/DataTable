-- chunkname: @modules/logic/store/view/SummonStoreGoodsView.lua

module("modules.logic.store.view.SummonStoreGoodsView", package.seeall)

local SummonStoreGoodsView = class("SummonStoreGoodsView", BaseView)

function SummonStoreGoodsView:onInitView()
	...
	self._imagecosticon = gohelper.findChildImage(self.viewGO, "root/#go_buy/cost/#simage_costicon")
	self._txtoriginalCost = gohelper.findChildText(self.viewGO, "root/#go_buy/cost/#txt_originalCost")
	self._txtsalePrice = gohelper.findChildText(self.viewGO, "root/#go_buy/cost/#txt_originalCost/#txt_salePrice")
	...
	self._txtvalue = gohelper.findChildText(self.viewGO, "root/#go_buy/valuebg/#txt_value")
	self._btnmin = gohelper.findChildButtonWithAudio(self.viewGO, "root/#go_buy/#btn_min")
	self._btnsub = gohelper.findChildButtonWithAudio(self.viewGO, "root/#go_buy/#btn_sub")
	self._btnadd = gohelper.findChildButtonWithAudio(self.viewGO, "root/#go_buy/#btn_add")
	self._btnmax = gohelper.findChildButtonWithAudio(self.viewGO, "root/#go_buy/#btn_max")
	...
	self._inputvalue = gohelper.findChildTextMeshInputField(self.viewGO, "root/#go_buy/valuebg/#input_value")
...
function SummonStoreGoodsView:addEvents()
...
function SummonStoreGoodsView:removeEvents()
...
function SummonStoreGoodsView:_btnclickOnClick()
	MaterialTipController.instance:showMaterialInfo(self._itemType, self._itemId)
end

function SummonStoreGoodsView:_btnminOnClick()
	self._buyCount = 1
...
function SummonStoreGoodsView:_btnsubOnClick()
...
function SummonStoreGoodsView:_btnaddOnClick()
...
function SummonStoreGoodsView:_btnmaxOnClick()
...
function SummonStoreGoodsView:_btnCloseOnClick()
	self:closeThis()
end

function SummonStoreGoodsView:_onEndEdit(inputStr)
...
function SummonStoreGoodsView:_onValueChanged(inputStr)
	return
end

function SummonStoreGoodsView:_btnbuyOnClick()
	if (self._itemType == MaterialEnum.MaterialType.Hero or nil) and CharacterModel.instance:isHeroFullDuplicateCount(self._itemId) then
...
function SummonStoreGoodsView:_tryBuyGoods()
...
function SummonStoreGoodsView:_buyGoods()
	StoreController.instance:buyGoods(self._mo, self._buyCount, self._buyCallback, self)
end

function SummonStoreGoodsView:_buyCallback(cmd, resultCode, msg)
	if resultCode == 0 then
		self:closeThis()
	end
end

function SummonStoreGoodsView:_editableInitView()
...
function SummonStoreGoodsView:_refreshBuyCount()
	local costQuantity = self._mo:getCost(self._buyCount)

	self._txtsalePrice.text = costQuantity == 0 and luaLang("store_free") or tostring(costQuantity)
...
function SummonStoreGoodsView:ShowLockTips()
	...
	if episodeId == StoreEnum.Need4RDEpisodeId then
		...
		self._txtlocktips.text = string.format(luaLang((self._mo.isHardChapter or nil) and "dungeon_unlock_episode_hard"), string.format("%s-%s", lvlimitchapter, lvlimitepisode))
	end
end

function SummonStoreGoodsView:_refreshUI()
	...
	self._txtgoodsDesc.text = self._itemType == MaterialEnum.MaterialType.Hero and ItemModel.instance:getItemConfig(items[1], items[2]).desc2 or ItemModel.instance:getItemConfig(items[1], items[2]).desc
	...
	if string.nilorempty(cost) then
		...
		if not costs[self._mo.buyCount + 1] then
			...
			if costConfig ~= nil then
				self.viewContainer:setCurrencyType(self._costId, self._costType, id)
			else
				self.viewContainer:setCurrencyType(self._costId, self._costType)
			end
		end
	end
...
function SummonStoreGoodsView:_refreshGoUnique()
	gohelper.setActive(self._gounique, false)
end

function SummonStoreGoodsView:_refreshInclude()
	if not self:_isStoreItemUnlock() then
		return
	end
...
function SummonStoreGoodsView:_refreshGoods(goodsConfig)
...
function SummonStoreGoodsView:_refreshLimitTag()
	local items = string.splitToNumber(self.goodsConfig.product, "#")
	local itemType = items[1]
	local itemId = items[2]

	gohelper.setActive(self._goLimit, (itemType == MaterialEnum.MaterialType.Equip or nil) and EquipModel.instance:isLimit(itemId))
end

function SummonStoreGoodsView:_buyCountAddToast()
...
function SummonStoreGoodsView:_refreshMaxBuyCount()
...
function SummonStoreGoodsView:onOpen()
...
function SummonStoreGoodsView:_isStoreItemUnlock()
	local episodeId = StoreConfig.instance:getGoodsConfig(self._mo.goodsId).needEpisodeId
...
function SummonStoreGoodsView:onClose()
	local goodsConfig = StoreConfig.instance:getGoodsConfig(self._mo.goodsId)

	StoreController.instance:statCloseGoods(goodsConfig)
end

function SummonStoreGoodsView:onUpdateParam()
	self._mo = self.viewParam
...
function SummonStoreGoodsView:onDestroyView()
	self._simageleftbg:UnLoadImage()
	self._simagerightbg:UnLoadImage()
end

return SummonStoreGoodsView
