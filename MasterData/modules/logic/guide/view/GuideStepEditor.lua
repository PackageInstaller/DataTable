...
function GuideStepEditor:_btnusegoOnClick()
	...
	while t ~= nil do
		path = not path and t.name or t.name .. "/" .. path
		t = t.parent
	end
...
function GuideStepEditor:_btnswitchOnClick()
	...
	self._txtswitch.text = self._goallmodules.activeSelf and "点击隐藏" or "点击显示"
end
...
function GuideStepEditor:_setUIType(param, maskOn, x, y)
	...
	local arrowScaleNumStr = self._inputarrowScale:GetText()
	local arrowScaleNumStr

	if string.nilorempty(self._inputarrowScale:GetText()) then
		arrowScaleNumStr = 0
	end
