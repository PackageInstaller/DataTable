-- chunkname: @modules/logic/activitywelfare/subview/newinsight/VersionActivity3_9InsightShowTaskItem.lua

module("modules.logic.activitywelfare.subview.newinsight.VersionActivity3_9InsightShowTaskItem", package.seeall)

local VersionActivity3_9InsightShowTaskItem = class("VersionActivity3_9InsightShowTaskItem", LuaCompBase)

function VersionActivity3_9InsightShowTaskItem:init(go, index)
	self.go = go
	self._index = index
...
function VersionActivity3_9InsightShowTaskItem:_initItem()
	gohelper.setActive(self.go, false)

	self._rewardItems = self:getUserDataTb_()
	self._actId = ActivityEnum.Activity.V3a9_NewInsight
end

function VersionActivity3_9InsightShowTaskItem:addEvents()
...
function VersionActivity3_9InsightShowTaskItem:removeEvents()
	self._btngoto:RemoveClickListener()
	self._btncanget:RemoveClickListener()
	self._btnuse:RemoveClickListener()
end

function VersionActivity3_9InsightShowTaskItem:_btngotoOnClick()
	if self._config.jumpId > 0 then
		GameFacade.jump(self._config.jumpId)
	end
end

function VersionActivity3_9InsightShowTaskItem:_btncangetOnClick()
	TaskRpc.instance:sendFinishTaskRequest(self._config.id)
end

function VersionActivity3_9InsightShowTaskItem:_btnuseOnClick()
	local data = {}
...
function VersionActivity3_9InsightShowTaskItem:setTask(taskId)
	self._taskId = taskId

	self:refresh()
end

function VersionActivity3_9InsightShowTaskItem:refresh()
...
function VersionActivity3_9InsightShowTaskItem:_refreshBtns()
	...
	if self._taskMO.finishCount >= 1 then
		local hasUse = ActivityType172Model.instance:isTaskHasUsed(self._actId, self._taskId)
...
function VersionActivity3_9InsightShowTaskItem:_refreshTaskRewards()
	...
	for i = 1, #rewards do
		...
		self._rewardItems[i] = self._rewardItems[i] or IconMgr.instance:getCommonPropItemIcon(self._gorewards)
...
function VersionActivity3_9InsightShowTaskItem:destroy()
	self:removeEvents()
end

return VersionActivity3_9InsightShowTaskItem
