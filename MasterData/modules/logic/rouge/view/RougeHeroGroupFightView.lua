...
function RougeHeroGroupFightView:_groupDropValueChanged(value)
	local selectIndex = RougeHeroGroupModel.instance:getGroupTypeName() and value or value + 1
...
function RougeHeroGroupFightView:_refreshUI()
	...
	local var_37_0 = self._goReplayBtn

	gohelper.setActive(var_37_0, (not HeroGroupBalanceHelper.getIsBalanceMode() or nil) and self.episodeConfig and not (self.episodeConfig.canUseRecord ~= 1 or self._chapterConfig.type == DungeonEnum.ChapterType.WeekWalk))
end
...
function RougeHeroGroupFightView._getEpisodeConfigAndBattleConfig()
	...
	return episodeCO, RougeHeroGroupModel.instance.battleId and RougeHeroGroupModel.instance.battleId > 0 and lua_battle.configDict[RougeHeroGroupModel.instance.battleId] or DungeonConfig.instance:getBattleCo(RougeHeroGroupModel.instance.episodeId)
end
...
function RougeHeroGroupFightView:_refreshBtns(isCostPower)
	...
	if not self._enterAfterFreeLimit then
		...
		if isCostPower then
			...
			var_65_1 = self._replayMode and self._chapterConfig.type ~= DungeonEnum.ChapterType.Hard
		end

		gohelper.setActive(self._btnstartreplay.gameObject, var_65_1)
		...
		if isCostPower then
			...
			var_65_2 = self._replayMode and self._chapterConfig.type == DungeonEnum.ChapterType.Hard
		end

		gohelper.setActive(self._btnhardreplay.gameObject, var_65_2)
