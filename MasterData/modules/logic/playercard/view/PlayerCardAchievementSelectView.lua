...
function PlayerCardAchievementSelectView:onOpen()
	...
	if isGroup then
		if not self._focusTypes[4] then
			local focusType = self._focusTypes[1]

			PlayerCardAchievementSelectController.instance:onOpenView(focusType)
			self.addEventCb(self, PlayerCardAchievementSelectController.instance, AchievementEvent.SelectViewUpdated, self.refreshUI, self)
