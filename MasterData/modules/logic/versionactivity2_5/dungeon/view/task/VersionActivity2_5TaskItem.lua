-- chunkname: @modules/logic/versionactivity2_5/dungeon/view/task/VersionActivity2_5TaskItem.lua

module("modules.logic.versionactivity2_5.dungeon.view.task.VersionActivity2_5TaskItem", package.seeall)

local VersionActivity2_5TaskItem = class("VersionActivity2_5TaskItem", ListScrollCell)

function VersionActivity2_5TaskItem:init(go)
	self.viewGO = go
...
function VersionActivity2_5TaskItem:addEventListeners()
	...
	self.addEventCb(self, VersionActivity2_5DungeonController.instance, VersionActivity2_5DungeonEvent.OnClickAllTaskFinish, self._OnClickAllTaskFinish, self)
end

function VersionActivity2_5TaskItem:removeEventListeners()
	self.btnNotFinish:RemoveClickListener()
	self.btnFinish:RemoveClickListener()
	self.btnFinishAll:RemoveClickListener()
	self.removeEventCb(self, VersionActivity2_5DungeonController.instance, VersionActivity2_5DungeonEvent.OnClickAllTaskFinish, self._OnClickAllTaskFinish, self)
end

function VersionActivity2_5TaskItem:_btnNotFinishOnClick()
	if self.co.jumpId ~= 0 then
		...
		if GameFacade.jump(self.co.jumpId) then
			ViewMgr.instance:closeView(ViewName.VersionActivity2_5TaskView)
		end
	end
end

function VersionActivity2_5TaskItem:_btnFinishAllOnClick()
	VersionActivity2_5DungeonController.instance:dispatchEvent(VersionActivity2_5DungeonEvent.OnClickAllTaskFinish)
end

VersionActivity2_5TaskItem.FinishKey = "VersionActivity2_5TaskItem FinishKey"

function VersionActivity2_5TaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(VersionActivity2_5TaskItem.FinishKey)
...
function VersionActivity2_5TaskItem:_OnClickAllTaskFinish()
	if self.taskMo then
...
function VersionActivity2_5TaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivity2_5TaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivity2_5TaskItem.FinishKey)
	self.animatorPlayer:Play(UIAnimationName.Idle)

	if self.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, nil, nil, nil, VersionActivity2_5Enum.ActivityId.Dungeon)
	else
		TaskRpc.instance:sendFinishTaskRequest(self.co.id)
	end
end

function VersionActivity2_5TaskItem:_editableInitView()
	self.rewardItemList = {}
end

function VersionActivity2_5TaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivity2_5TaskItem:refreshNormalUI()
...
function VersionActivity2_5TaskItem:refreshRewardItems()
...
function VersionActivity2_5TaskItem:refreshGetAllUI()
	return
end

function VersionActivity2_5TaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivity2_5TaskItem:getAnimator()
	return self.animator
end

function VersionActivity2_5TaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return VersionActivity2_5TaskItem
