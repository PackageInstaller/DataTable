-- chunkname: @modules/logic/versionactivity2_5/dungeon/view/store/VersionActivity2_5StoreView.lua

module("modules.logic.versionactivity2_5.dungeon.view.store.VersionActivity2_5StoreView", package.seeall)

local VersionActivity2_5StoreView = class("VersionActivity2_5StoreView", BaseView)

function VersionActivity2_5StoreView:onInitView()
...
function VersionActivity2_5StoreView:addEvents()
...
function VersionActivity2_5StoreView:removeEvents()
	self._scrollstore:RemoveOnValueChanged()
	self.removeEventCb(self, JumpController.instance, JumpEvent.BeforeJump, self.closeThis, self)
end

function VersionActivity2_5StoreView:_onScrollValueChanged()
...
function VersionActivity2_5StoreView:refreshTime()
	local actInfoMo = ActivityModel.instance:getActivityInfo()[VersionActivity2_5Enum.ActivityId.DungeonStore]

	self._txttime.text = actInfoMo:getRemainTimeStr3(false, false)
end

function VersionActivity2_5StoreView:refreshStoreContent()
	if self.actId then
		...
		for i = 1, #self.actId do
			storeItem = self.storeItemList[i]

			if not self.storeItemList[i] then
				local storeItemGo = gohelper.cloneInPlace(self._gostoreItem)

				storeItem = VersionActivity2_5StoreItem.New()

				storeItem:onInitView(storeItemGo)
				table.insert(self.storeItemList, storeItem)
			end

			storeItem:updateInfo(i, self.actId[i])
		end
	end
end

function VersionActivity2_5StoreView:scrollToFirstNoSellOutStore()
...
function VersionActivity2_5StoreView:getFirstNoSellOutGroup()
	local storeGroupDict = ActivityStoreConfig.instance:getActivityStoreGroupDict(self.actId)
...
function VersionActivity2_5StoreView:onClose()
	TaskDispatcher.cancelTask(self.refreshTime, self)
end

function VersionActivity2_5StoreView:onDestroyView()
	for _, storeItem in ipairs(self.storeItemList) do
		storeItem:onDestroy()
	end
end

return VersionActivity2_5StoreView
