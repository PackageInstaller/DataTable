-- chunkname: @modules/logic/versionactivity2_5/dungeon/view/store/VersionActivity2_5StoreItem.lua

module("modules.logic.versionactivity2_5.dungeon.view.store.VersionActivity2_5StoreItem", package.seeall)

local VersionActivity2_5StoreItem = class("VersionActivity2_5StoreItem", UserDataDispose)
...
function VersionActivity2_5StoreItem:onInitView(go)
	self:__onInit()
...
function VersionActivity2_5StoreItem:onBuyGoodsSuccess()
	self:sortGoodsCoList()
	self:refreshGoods()
end

function VersionActivity2_5StoreItem:updateInfo(groupId, groupGoodsCoList)
...
function VersionActivity2_5StoreItem:sortGoodsCoList()
	table.sort(self.groupGoodsCoList, sortGoods)
end

function VersionActivity2_5StoreItem:refreshTag()
	if self.gotag then
		return
	end
...
function VersionActivity2_5StoreItem:refreshTagClip(scrollStore)
...
function VersionActivity2_5StoreItem:refreshGoods()
	local goodsItem
	...
	for index, goodsCo in ipairs(self.groupGoodsCoList) do
		...
		if not self.goodsItemList[index] then
			...
			goodsItem = VersionActivity2_5StoreGoodsItem.New()
...
function VersionActivity2_5StoreItem:getHeight()
	return recthelper.getHeight(self.go.transform)
end

function VersionActivity2_5StoreItem:onDestroy()
...
return VersionActivity2_5StoreItem
