-- chunkname: @modules/logic/versionactivity2_5/liangyue/view/LiangYueTaskItem.lua

module("modules.logic.versionactivity2_5.liangyue.view.LiangYueTaskItem", package.seeall)

local LiangYueTaskItem = class("LiangYueTaskItem", ListScrollCellExtend)

function LiangYueTaskItem:onInitView()
...
function LiangYueTaskItem:addEvents()
...
function LiangYueTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function LiangYueTaskItem:_btnnotfinishbgOnClick()
	if self._act184TaskMO.config.jumpId > 0 then
		GameFacade.jump(self._act184TaskMO.config.jumpId)
	end
end

function LiangYueTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._act184TaskMO.activityId)
	UIBlockHelper.instance:startBlock("LiangYueTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function LiangYueTaskItem:_btngetallOnClick()
	LiangYueController.instance:dispatchEvent(LiangYueEvent.OneClickClaimReward, self._act184TaskMO.activityId, true)
	UIBlockHelper.instance:startBlock("LiangYueTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function LiangYueTaskItem:_delayFinish()
	TaskRpc.instance:sendFinishTaskRequest(self._act184TaskMO.config.id)
end

function LiangYueTaskItem:_delayFinishAll()
	TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Activity184)
end

function LiangYueTaskItem:_editableInitView()
...
function LiangYueTaskItem:_editableAddEvents()
	LiangYueController.instance:registerCallback(LiangYueEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LiangYueTaskItem:_editableRemoveEvents()
	LiangYueController.instance:unregisterCallback(LiangYueEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LiangYueTaskItem:_onOneClickClaimReward(actId, isAll)
	if self._act184TaskMO and self._act184TaskMO.activityId == actId and (self._act184TaskMO:alreadyGotReward() or self._act184TaskMO.id == -99999 and isAll) then
...
function LiangYueTaskItem:getAnimator()
	return self._animator
end

function LiangYueTaskItem:onUpdateMO(mo)
	self._act184TaskMO = mo

	local rankDiff = LiangYueTaskListModel.instance:getRankDiff(mo)
...
function LiangYueTaskItem:_moveByRankDiff(rankDiff)
...
function LiangYueTaskItem:onSelect(isSelect)
	return
end

function LiangYueTaskItem:_refreshUI()
	local atMO = self._act184TaskMO
...
function LiangYueTaskItem:_onItemShow(cell_component, data, index)
...
function LiangYueTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
