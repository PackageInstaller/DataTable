-- chunkname: @modules/logic/versionactivity3_3/igor/view/IgorTaskItem.lua

module("modules.logic.versionactivity3_3.igor.view.IgorTaskItem", package.seeall)

local IgorTaskItem = class("IgorTaskItem", ListScrollCellExtend)

function IgorTaskItem:onInitView()
...
function IgorTaskItem:addEvents()
...
function IgorTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function IgorTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.IgorTaskView)
	end
end

function IgorTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("IgorTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function IgorTaskItem:_btngetallOnClick()
	BeiLiErController.instance:dispatchEvent(BeiLiErEvent.OneClickClaimReward, self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("IgorTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function IgorTaskItem:_delayFinish()
...
function IgorTaskItem:_editableAddEvents()
	BeiLiErController.instance:registerCallback(BeiLiErEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function IgorTaskItem:_editableRemoveEvents()
	BeiLiErController.instance:unregisterCallback(BeiLiErEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function IgorTaskItem:_onOneClickClaimReward(actId)
	if self._taskMo and self._taskMo.activityId == actId and self.taskMo:alreadyGotReward() then
...
function IgorTaskItem:getAnimator()
	return self._animator
end

function IgorTaskItem:onUpdateMO(mo)
	self._taskMo = mo

	local rankDiff = BeiLiErTaskListModel.instance:getRankDiff(mo)
...
function IgorTaskItem:_moveByRankDiff(rankDiff)
...
function IgorTaskItem:onSelect(isSelect)
	return
end

function IgorTaskItem:_refreshUI()
...
function IgorTaskItem:_onItemShow(cell_component, data, index)
...
function IgorTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
