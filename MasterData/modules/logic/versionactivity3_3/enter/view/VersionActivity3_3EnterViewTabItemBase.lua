-- chunkname: @modules/logic/versionactivity3_3/enter/view/VersionActivity3_3EnterViewTabItemBase.lua

module("modules.logic.versionactivity3_3.enter.view.VersionActivity3_3EnterViewTabItemBase", package.seeall)

local VersionActivity3_3EnterViewTabItemBase = class("VersionActivity3_3EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)

function VersionActivity3_3EnterViewTabItemBase:_editableInitView()
...
function VersionActivity3_3EnterViewTabItemBase:_getTagPath()
	return "#go_tag"
end

function VersionActivity3_3EnterViewTabItemBase:afterSetData()
	if self.actId then
	...
	if not self.activityCo then
		...
		logError("VersionActivity3_3EnterViewTabItemBase.afterSetData error, no act config, actId:%s", self.actId)
	...
	if not string.nilorempty(selectBg) then
		UISpriteSetMgr.instance:setV3a3MainActivitySprite(self.imageSelectTabIcon, selectBg)
	end

	local unselectBg = iconCoArr[2]

	if not string.nilorempty(unselectBg) then
		UISpriteSetMgr.instance:setV3a3MainActivitySprite(self.imageUnSelectTabIcon, unselectBg)
	end

	self.redDotIcon = RedDotController.instance:addRedDot(self.goRedDot, self.activityCo.redDotId, self.redDotUid)
end

function VersionActivity3_3EnterViewTabItemBase:childRefreshSelect()
...
function VersionActivity3_3EnterViewTabItemBase:childRefreshUI()
...
function VersionActivity3_3EnterViewTabItemBase:refreshTag()
	self:clearTag()
...
function VersionActivity3_3EnterViewTabItemBase:clearTag()
...
function VersionActivity3_3EnterViewTabItemBase:refreshNormalTag()
	...
	local isShowRemainTime = VersionActivity3_3EnterHelper.GetIsShowTabRemainTime(self.actId)

	if isShowRemainTime and actInfoMo then
		local offsetSecond = actInfoMo:getRealEndTimeStamp() - ServerTime.now()

		if offsetSecond > VersionActivity3_3Enum.MaxShowTimeOffset then
			return
		end
...
function VersionActivity3_3EnterViewTabItemBase:refreshNotUnlockTag()
	gohelper.setActive(self.goTagLock, false)
...
function VersionActivity3_3EnterViewTabItemBase:refreshLockTag()
...
function VersionActivity3_3EnterViewTabItemBase:isShowRedDot()
	return self.redDotIcon and self.redDotIcon.show
end

return VersionActivity3_3EnterViewTabItemBase
