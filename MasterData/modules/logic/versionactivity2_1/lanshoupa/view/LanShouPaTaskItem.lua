-- chunkname: @modules/logic/versionactivity2_1/lanshoupa/view/LanShouPaTaskItem.lua

module("modules.logic.versionactivity2_1.lanshoupa.view.LanShouPaTaskItem", package.seeall)

local LanShouPaTaskItem = class("LanShouPaTaskItem", ListScrollCellExtend)

function LanShouPaTaskItem:onInitView()
...
function LanShouPaTaskItem:addEvents()
...
function LanShouPaTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function LanShouPaTaskItem:_btnnotfinishbgOnClick()
	if self._act164TaskMO.config.jumpId > 0 then
		GameFacade.jump(self._act164TaskMO.config.jumpId)
	end
end

function LanShouPaTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._act164TaskMO.activityId)
	UIBlockHelper.instance:startBlock("LanShouPaTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function LanShouPaTaskItem:_btngetallOnClick()
	LanShouPaController.instance:dispatchEvent(LanShouPaEvent.OneClickClaimReward, self._act164TaskMO.activityId)
	UIBlockHelper.instance:startBlock("LanShouPaTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function LanShouPaTaskItem:_delayFinish()
	TaskRpc.instance:sendFinishTaskRequest(self._act164TaskMO.config.id)
end

function LanShouPaTaskItem:_delayFinishAll()
	TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.Activity164)
end

function LanShouPaTaskItem:_editableInitView()
	self._animator = self.viewGO:GetComponent(LanShouPaEnum.ComponentType.Animator)
...
function LanShouPaTaskItem:_editableAddEvents()
	LanShouPaController.instance:registerCallback(LanShouPaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LanShouPaTaskItem:_editableRemoveEvents()
	LanShouPaController.instance:unregisterCallback(LanShouPaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LanShouPaTaskItem:_onOneClickClaimReward(actId)
	if self._act164TaskMO and self._act164TaskMO.activityId == actId and self._act164TaskMO:alreadyGotReward() then
...
function LanShouPaTaskItem:getAnimator()
	return self._animator
end

function LanShouPaTaskItem:onUpdateMO(mo)
	self._act164TaskMO = mo

	local rankDiff = Activity164TaskListModel.instance:getRankDiff(mo)
...
function LanShouPaTaskItem:_moveByRankDiff(rankDiff)
	if rankDiff and rankDiff ~= 0 then
		...
		self._rankDiffMoveId = ZProj.TweenHelper.DOAnchorPosY(self.viewTrs, 0, LanShouPaEnum.AnimatorTime.TaskRewardMoveUp)
	end
end

function LanShouPaTaskItem:onSelect(isSelect)
	return
end

function LanShouPaTaskItem:_refreshUI()
	local atMO = self._act164TaskMO

	if not atMO then
		return
	end

	local isNormal = atMO.id ~= LanShouPaEnum.TaskMOAllFinishId
...
function LanShouPaTaskItem:_onItemShow(cell_component, data, index)
...
function LanShouPaTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
...
LanShouPaTaskItem.prefabPath = "ui/viewres/versionactivity_2_1/v2a1_tuesday/v2a1_tuesday_taskitem.prefab"

return LanShouPaTaskItem
