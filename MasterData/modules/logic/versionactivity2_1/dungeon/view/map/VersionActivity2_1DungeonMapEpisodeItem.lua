-- chunkname: @modules/logic/versionactivity2_1/dungeon/view/map/VersionActivity2_1DungeonMapEpisodeItem.lua

module("modules.logic.versionactivity2_1.dungeon.view.map.VersionActivity2_1DungeonMapEpisodeItem", package.seeall)

local VersionActivity2_1DungeonMapEpisodeItem = class("VersionActivity2_1DungeonMapEpisodeItem", BaseChildView)

function VersionActivity2_1DungeonMapEpisodeItem:onInitView()
...
function VersionActivity2_1DungeonMapEpisodeItem:addEvents()
...
function VersionActivity2_1DungeonMapEpisodeItem:removeEvents()
...
function VersionActivity2_1DungeonMapEpisodeItem:onClick()
	if self.isLock then
	...
	local viewContainer = ViewMgr.instance:getContainer(ViewName.VersionActivity2_1DungeonMapLevelView)
...
function VersionActivity2_1DungeonMapEpisodeItem:_editableInitView()
...
function VersionActivity2_1DungeonMapEpisodeItem:_showEye()
	...
	local isHard = self._config.chapterId == VersionActivity2_1DungeonEnum.DungeonChapterId.Hard

	gohelper.setActive(self._gonormaleye, not isHard)
	gohelper.setActive(self._gohardeye, isHard)
end

function VersionActivity2_1DungeonMapEpisodeItem:initViewParam()
	self._contentTransform = self.viewParam[1]
	self._layout = self.viewParam[2]
	self._mapSceneView = self.viewContainer.mapScene
end

function VersionActivity2_1DungeonMapEpisodeItem:onUpdateParam()
	self:initViewParam()
	self:_showAllElementTipView()
end

function VersionActivity2_1DungeonMapEpisodeItem:onOpen()
	self:initViewParam()
end

function VersionActivity2_1DungeonMapEpisodeItem:getMapAllElementList()
	return (VersionActivity2_1DungeonModel.instance:getElementCoList(self._map.id))
end

function VersionActivity2_1DungeonMapEpisodeItem:_showAllElementTipView()
	if not self._map or self._config.chapterId == VersionActivity2_1DungeonEnum.DungeonChapterId.Hard then
	...
	local finishElementCoList, mapAllElementList = VersionActivity2_1DungeonModel.instance:getElementCoListWithFinish(self._map.id)
	...
	if not mapAllElementList or #mapAllElementList < 1 then
		...
		for index, _ in ipairs(mapAllElementList) do
			...
			if not self.elementItemList[index] then
			...
			if self.pass then
				local isFinish = index <= finishCount
...
function VersionActivity2_1DungeonMapEpisodeItem:_hideAllElementTip()
	gohelper.setActive(self._gotipcontent, false)
end

function VersionActivity2_1DungeonMapEpisodeItem:refresh(config, dungeonMo)
	self._config = config
	...
	self._map = VersionActivity2_1DungeonConfig.instance:getEpisodeMapConfig(self._config.id)
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshUI()
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshLock()
	self.isLock = self:checkLock()

	gohelper.setActive(self.goLock, self:checkLock())
end

function VersionActivity2_1DungeonMapEpisodeItem:initElementIdList()
...
function VersionActivity2_1DungeonMapEpisodeItem:clearElementIdList()
	self.elementIdList = nil
end

function VersionActivity2_1DungeonMapEpisodeItem:checkLock()
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshStar()
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshFlag()
	gohelper.setActive(self._goflag, false)
end

function VersionActivity2_1DungeonMapEpisodeItem:refreshUnlockContent()
	if self.pass then
	...
	if openList and #openList > 0 then
		...
		if unLockContentList and #unLockContentList > 0 then
			self.txtlocktips.text = unLockContentList[1] or ""
		end
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshHardModeStar()
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshStoryModeStar()
	local starItem
	...
	for index, episodeCo in ipairs(episodeList) do
		...
		if not self.starItemList[index] then
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshEpisodeStar(starItem, episodeId)
...
function VersionActivity2_1DungeonMapEpisodeItem:refreshFocusStatus()
	gohelper.setActive(self.goSelected, self._config.id == self.activityDungeonMo.episodeId)
end

function VersionActivity2_1DungeonMapEpisodeItem:beginShowRewardView()
	self.beginReward = true
end

function VersionActivity2_1DungeonMapEpisodeItem:endShowRewardView()
...
function VersionActivity2_1DungeonMapEpisodeItem:calculatePosInContent()
	...
	self._maxWidth = math.max(halfWidth * 2, VersionActivity2_1DungeonEnum.EpisodeItemMinWidth) + 30
...
function VersionActivity2_1DungeonMapEpisodeItem:setImage(image, light, episodeId)
	local episodeConfig = DungeonConfig.instance:getEpisodeCO(episodeId)
	local starType = VersionActivity2_1DungeonEnum.EpisodeStarType[episodeConfig.chapterId]

	if light then
		if not starType.light then
			local lightStateIcon = starType.empty

			UISpriteSetMgr.instance:setV2a1DungeonSprite(image, lightStateIcon)
		end
	end
end

function VersionActivity2_1DungeonMapEpisodeItem:getMaxWidth()
	return self._maxWidth
end

function VersionActivity2_1DungeonMapEpisodeItem:updateSelectStatus(selectedItem, ignoreAni)
...
function VersionActivity2_1DungeonMapEpisodeItem:onRemoveElement(elementId)
...
function VersionActivity2_1DungeonMapEpisodeItem:playUnLockAnimation()
	local animator = self.goLock:GetComponent(typeof(UnityEngine.Animator))

	animator.enabled = true
end

function VersionActivity2_1DungeonMapEpisodeItem:playAnimation(animationName)
...
function VersionActivity2_1DungeonMapEpisodeItem:getEpisodeId()
	return self._config and self._config.id
end

function VersionActivity2_1DungeonMapEpisodeItem:createStarItem(goStar)
...
function VersionActivity2_1DungeonMapEpisodeItem:onClose()
	TaskDispatcher.cancelTask(self._hideAllElementTip, self)
end

function VersionActivity2_1DungeonMapEpisodeItem:onDestroyView()
	self.goClick:RemoveClickListener()
end

return VersionActivity2_1DungeonMapEpisodeItem
