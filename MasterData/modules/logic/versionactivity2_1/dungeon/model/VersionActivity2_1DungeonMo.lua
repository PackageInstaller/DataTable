-- chunkname: @modules/logic/versionactivity2_1/dungeon/model/VersionActivity2_1DungeonMo.lua

module("modules.logic.versionactivity2_1.dungeon.model.VersionActivity2_1DungeonMo", package.seeall)

local VersionActivity2_1DungeonMo = pureTable("VersionActivity2_1DungeonMo", VersionActivityDungeonBaseMo)

function VersionActivity2_1DungeonMo:updateEpisodeId(episodeId)
	local tmpEpisodeId
	...
	if episodeId then
		...
		if isPassAll then
			...
			for _, episodeCfg in ipairs(episodeList) do
				if (episodeCfg and DungeonModel.instance:getEpisodeInfo(episodeCfg.id) or nil) and self:checkEpisodeUnLock(episodeCfg) then
...
function VersionActivity2_1DungeonMo:checkEpisodeUnLock(episodeCo)
...
return VersionActivity2_1DungeonMo
