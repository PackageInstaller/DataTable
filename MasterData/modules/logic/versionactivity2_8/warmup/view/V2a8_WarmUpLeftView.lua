-- chunkname: @modules/logic/versionactivity2_8/warmup/view/V2a8_WarmUpLeftView.lua

module("modules.logic.versionactivity2_8.warmup.view.V2a8_WarmUpLeftView", package.seeall)

local V2a8_WarmUpLeftView = class("V2a8_WarmUpLeftView", BaseView)

function V2a8_WarmUpLeftView:onInitView()
	if self._editableInitView then
		self:_editableInitView()
	end
end

function V2a8_WarmUpLeftView:addEvents()
	self._itemClick:AddClickListener(self._onItemClick, self)
end

function V2a8_WarmUpLeftView:removeEvents()
...
function V2a8_WarmUpLeftView:ctor()
	self._draggedState = kFirstLocked
end

function V2a8_WarmUpLeftView:_editableInitView()
...
function V2a8_WarmUpLeftView:onOpen()
	self._animEvent:AddEventListener("play_ui_fuleyuan_yure_open", self._play_ui_fuleyuan_yure_open, self)
	self._animEvent:AddEventListener("play_ui_fuleyuan_yure_paper", self._play_ui_fuleyuan_yure_paper, self)
	self._animEvent:AddEventListener("play_ui_fuleyuan_yure_whoosh", self._play_ui_fuleyuan_yure_whoosh, self)
end

function V2a8_WarmUpLeftView:onDataUpdateFirst()
	...
	if isDone then
		self._draggedState = kFirstUnlocked or kFirstLocked
	end
end

function V2a8_WarmUpLeftView:onDataUpdate()
	self:_refresh()
end

function V2a8_WarmUpLeftView:onSwitchEpisode()
	local isDone = self:_checkIsDone()
...
function V2a8_WarmUpLeftView:_refresh()
	...
	if isDone then
		...
		elseif States.Clicked == state then
			...
			logError("[V2a8_WarmUpLeftView] invalid state: " .. tostring(state))
		end
	end
end

function V2a8_WarmUpLeftView:onClose()
	self._animEvent:RemoveAllEventListener()
end

function V2a8_WarmUpLeftView:onDestroyView()
	return
end

function V2a8_WarmUpLeftView:_playAnimIdle(cb, cbObj)
	self:_playAnim(UIAnimationName.Unopen, cb, cbObj)
end

function V2a8_WarmUpLeftView:_playAnimOpend(cb, cbObj)
	self:_playAnim(UIAnimationName.Open, cb, cbObj)
end

function V2a8_WarmUpLeftView:_playAnimClick(cb, cbObj)
	self:_playAnim(UIAnimationName.Click, cb, cbObj)
end

function V2a8_WarmUpLeftView:_playAnim(name, cb, cbObj)
	self._animatorPlayer:Play(name, cb or function()
		return
	end, cbObj)
end

function V2a8_WarmUpLeftView:_onItemClick()
	self:_setActive_drag(false)
...
local kBlock_Click = "V2a8_WarmUpLeftView:kBlock_Click"
local kTimeout = 9.99

function V2a8_WarmUpLeftView:_playAnimAfterClicked()
...
function V2a8_WarmUpLeftView:_play_ui_fuleyuan_yure_open()
	AudioMgr.instance:trigger(AudioEnum2_8.WarmUp.play_ui_fuleyuan_yure_open)
end

function V2a8_WarmUpLeftView:_play_ui_fuleyuan_yure_paper()
	AudioMgr.instance:trigger(AudioEnum2_8.WarmUp.play_ui_fuleyuan_yure_paper)
end

function V2a8_WarmUpLeftView:_play_ui_fuleyuan_yure_whoosh()
	AudioMgr.instance:trigger(AudioEnum2_8.WarmUp.play_ui_fuleyuan_yure_whoosh)
end

return V2a8_WarmUpLeftView
