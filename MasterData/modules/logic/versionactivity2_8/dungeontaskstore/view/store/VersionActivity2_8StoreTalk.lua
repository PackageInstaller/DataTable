-- chunkname: @modules/logic/versionactivity2_8/dungeontaskstore/view/store/VersionActivity2_8StoreTalk.lua

module("modules.logic.versionactivity2_8.dungeontaskstore.view.store.VersionActivity2_8StoreTalk", package.seeall)

local VersionActivity2_8StoreTalk = class("VersionActivity2_8StoreTalk", BaseView)

function VersionActivity2_8StoreTalk:onInitView()
...
function VersionActivity2_8StoreTalk:addEvents()
	return
end

function VersionActivity2_8StoreTalk:removeEvents()
	return
end

VersionActivity2_8StoreTalk.TalkMinTriggerDuration = 2
VersionActivity2_8StoreTalk.TalkStepShowDuration = 2
VersionActivity2_8StoreTalk.TalkTxtShowDuration = 6
VersionActivity2_8StoreTalk.TextSpawnInterval = 0.06
VersionActivity2_8StoreTalk.ChessJumpAnimDuration = 1.167
VersionActivity2_8StoreTalk.ChessOpenAnimDuration = 1.5
VersionActivity2_8StoreTalk.ScrollTalkMargin = 60
VersionActivity2_8StoreTalk.TextMaxHeight = 99999
VersionActivity2_8StoreTalk.TipHeight = 10
VersionActivity2_8StoreTalk.SplitChar = "|"

function VersionActivity2_8StoreTalk:onClickStage()
	AudioMgr.instance:trigger(AudioEnum2_8.VersionActivity2_8Store.play_ui_jinye_click_stage)
	self:triggerTalkByType(ActivityStoreConfig.BubbleTalkTriggerType.ClickStageArea)
end

function VersionActivity2_8StoreTalk:onClickText()
...
function VersionActivity2_8StoreTalk:_editableInitView()
	self._hasOpen = false
	self._scrollTalk.verticalNormalizedPosition = 1
	self.lastTriggerTime = -VersionActivity2_8StoreTalk.TalkMinTriggerDuration
	self.canTriggerList = {}
	self.tempTypeList = {}
	self.triggerStepList = {}
	self.actId = VersionActivity2_8DungeonTaskStoreController.instance:getModuleConfig().DungeonStore
	...
	self:initUpdateBeat()
	self.addEventCb(self, VersionActivityController.instance, VersionActivityEvent.OnBuy107GoodsSuccess, self.onBuyGoodsSuccess, self, LuaEventSystem.Low)
	self.addEventCb(self, ViewMgr.instance, ViewEvent.OnCloseViewFinish, self.onCloseViewFinish, self, LuaEventSystem.Low)
end

function VersionActivity2_8StoreTalk:initUpdateBeat()
...
function VersionActivity2_8StoreTalk:_onFrameLateUpdate()
	if self.closed then
		return
	end
	...
	local margin = VersionActivity2_8StoreTalk.ScrollTalkMargin
	local talkHeight = preferHeight > VersionActivity2_8StoreTalk.TextMaxHeight and VersionActivity2_8StoreTalk.TextMaxHeight + margin or preferHeight + margin
...
function VersionActivity2_8StoreTalk:_onFrame()
	...
	if currentTime - self.lastSpawnTime < VersionActivity2_8StoreTalk.TextSpawnInterval then
		return
	end
...
function VersionActivity2_8StoreTalk:_tickContent()
	if self.closed then
		return
	end
	...
	if self.currentCharIndex == self.contentLen + 1 then
		...
		if nextStepCo then
			...
			TaskDispatcher.runDelay(self.playNextStep, self, VersionActivity2_8StoreTalk.TalkStepShowDuration)
		else
			self:startWaitFinishTalk()
			TaskDispatcher.runDelay(self.onFinishTalk, self, VersionActivity2_8StoreTalk.TalkTxtShowDuration)
		end
	end
end

function VersionActivity2_8StoreTalk:startWaitFinishTalk()
	self.waitFinishTalk = true
end

function VersionActivity2_8StoreTalk:startWaitPlayNextStep()
	self.waitPlayNextStep = true

	gohelper.setActive(self._goArrowTip, true)
end

function VersionActivity2_8StoreTalk:onFinishTalk()
...
function VersionActivity2_8StoreTalk:playNextStep()
...
function VersionActivity2_8StoreTalk:onCloseViewFinish(viewName)
...
function VersionActivity2_8StoreTalk:onBuyGoodsSuccess(actId, goodsId)
...
function VersionActivity2_8StoreTalk:onOpen()
	self:initGroupCo()
	TaskDispatcher.runDelay(self._triggerEnterAudio, self, 0.5)
end

function VersionActivity2_8StoreTalk:_triggerEnterAudio()
	AudioMgr.instance:trigger(AudioEnum2_8.VersionActivity2_8Store.play_ui_jinye_chess_enter)
end

function VersionActivity2_8StoreTalk:onOpenFinish()
	if self.isFirstEnter then
		self:triggerTalkByType(ActivityStoreConfig.BubbleTalkTriggerType.FirstEnterActivityStore)

		local key = PlayerModel.instance:getPlayerPrefsKey(PlayerPrefsKey.Version2_8PlayedStoreGroupIdKey)
		local preValue = PlayerPrefsHelper.getString(key, "")
		local idList = string.splitToNumber(preValue, VersionActivity2_8StoreTalk.SplitChar)

		table.insert(idList, self.groupCo.groupId)
		PlayerPrefsHelper.setString(key, table.concat(idList, VersionActivity2_8StoreTalk.SplitChar))
	else
		self:triggerTalkByType(ActivityStoreConfig.BubbleTalkTriggerType.EnterActivityStore)
	end
end

function VersionActivity2_8StoreTalk:initGroupCo()
	local groupList = ActivityStoreConfig.instance:getUnlockGroupList(self.actId)
	local key = PlayerModel.instance:getPlayerPrefsKey(PlayerPrefsKey.Version2_8PlayedStoreGroupIdKey)
	local groupIdList = {}

	for _, v in ipairs(string.split(PlayerPrefsHelper.getString(key, ""), VersionActivity2_8StoreTalk.SplitChar)) do
...
function VersionActivity2_8StoreTalk:triggerTalkByTypeList(typeList, typeParam)
...
function VersionActivity2_8StoreTalk:triggerTalkByType(type, typeParam)
	tabletool.clear(self.canTriggerList)
	...
	if not self._hasOpen then
		...
		TaskDispatcher.runDelay(self._checkTriggerTalk, self, VersionActivity2_8StoreTalk.ChessOpenAnimDuration)
	else
		self:_checkTriggerTalk()
	end
end

function VersionActivity2_8StoreTalk:_checkTriggerTalk()
	self:_triggerTalk(self:getRandomTalkCo(self.canTriggerList))
end

function VersionActivity2_8StoreTalk:getRandomTalkCo(coList)
	local count = #coList
...
function VersionActivity2_8StoreTalk:_triggerTalk(talkCo)
	local currentTime = Time.time

	if currentTime - self.lastTriggerTime < VersionActivity2_8StoreTalk.TalkMinTriggerDuration then
		return
	end
	...
	self.lastSpawnTime = currentTime - VersionActivity2_8StoreTalk.TextSpawnInterval
...
function VersionActivity2_8StoreTalk:playStep(stepIndex)
	...
	TaskDispatcher.runRepeat(self.stopChessAnim, self, VersionActivity2_8StoreTalk.ChessJumpAnimDuration)
	...
	AudioMgr.instance:trigger(AudioEnum2_8.VersionActivity2_8Store.play_ui_jinye_chess_talk)
end

function VersionActivity2_8StoreTalk:buildStepCoList(talkCo)
	...
	if #self.triggerStepList > 1 then
		table.sort(self.triggerStepList, VersionActivity2_8StoreTalk.sortStep)
	end
end

function VersionActivity2_8StoreTalk:stopChessAnim()
	if self.waitPlayNextStep or self.waitFinishTalk then
...
function VersionActivity2_8StoreTalk:onClose()
	TaskDispatcher.cancelTask(self.stopChessAnim, self)
	TaskDispatcher.cancelTask(self.playNextStep, self)
	TaskDispatcher.cancelTask(self.onFinishTalk, self)
	AudioMgr.instance:trigger(AudioEnum2_8.VersionActivity2_8Store.stop_ui_jinye_chess_talk)
...
function VersionActivity2_8StoreTalk:onDestroyView()
	TaskDispatcher.cancelTask(self._checkTriggerTalk, self)
	self.stageClick:RemoveClickListener()
	self.talkTextClick:RemoveClickListener()
end

function VersionActivity2_8StoreTalk.sortStep(co1, co2)
	return co1.step < co2.step
end

return VersionActivity2_8StoreTalk
