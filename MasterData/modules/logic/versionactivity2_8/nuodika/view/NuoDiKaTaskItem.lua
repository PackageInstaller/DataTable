-- chunkname: @modules/logic/versionactivity2_8/nuodika/view/NuoDiKaTaskItem.lua

module("modules.logic.versionactivity2_8.nuodika.view.NuoDiKaTaskItem", package.seeall)

local NuoDiKaTaskItem = class("NuoDiKaTaskItem", ListScrollCellExtend)

function NuoDiKaTaskItem:onInitView()
...
function NuoDiKaTaskItem:addEvents()
...
function NuoDiKaTaskItem:_editableAddEvents()
	NuoDiKaController.instance:registerCallback(NuoDiKaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function NuoDiKaTaskItem:_editableRemoveEvents()
	NuoDiKaController.instance:unregisterCallback(NuoDiKaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function NuoDiKaTaskItem:_onOneClickClaimReward(actId)
...
function NuoDiKaTaskItem:getAnimator()
	return self._animator
end

function NuoDiKaTaskItem:onUpdateMO(mo)
	self._act180TaskMO = mo

	local rankDiff = NuoDiKaTaskListModel.instance:getRankDiff(mo)
...
function NuoDiKaTaskItem:_moveByRankDiff(rankDiff)
...
function NuoDiKaTaskItem:onSelect(isSelect)
	return
end

function NuoDiKaTaskItem:_refreshUI()
...
function NuoDiKaTaskItem:_onItemShow(cell_component, data, index)
...
function NuoDiKaTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
...
NuoDiKaTaskItem.prefabPath = "ui/viewres/versionactivity_2_8/v2a8_nuodika/v2a8_nuodika_taskitem.prefab"

return NuoDiKaTaskItem
