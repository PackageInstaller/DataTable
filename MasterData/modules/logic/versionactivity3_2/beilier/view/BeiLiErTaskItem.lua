-- chunkname: @modules/logic/versionactivity3_2/beilier/view/BeiLiErTaskItem.lua

module("modules.logic.versionactivity3_2.beilier.view.BeiLiErTaskItem", package.seeall)

local BeiLiErTaskItem = class("BeiLiErTaskItem", ListScrollCellExtend)

function BeiLiErTaskItem:onInitView()
...
function BeiLiErTaskItem:addEvents()
...
function BeiLiErTaskItem:removeEvents()
...
function BeiLiErTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("BeiLiErTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function BeiLiErTaskItem:_btngetallOnClick()
...
function BeiLiErTaskItem:_delayFinish()
...
function BeiLiErTaskItem:getAnimator()
	return self._animator
end

function BeiLiErTaskItem:onUpdateMO(mo)
	self._taskMo = mo
...
function BeiLiErTaskItem:_moveByRankDiff(rankDiff)
...
function BeiLiErTaskItem:onSelect(isSelect)
	return
end

function BeiLiErTaskItem:_refreshUI()
...
function BeiLiErTaskItem:_onItemShow(cell_component, data, index)
...
function BeiLiErTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
