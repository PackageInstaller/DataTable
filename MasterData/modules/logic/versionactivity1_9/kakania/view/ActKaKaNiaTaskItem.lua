-- chunkname: @modules/logic/versionactivity1_9/kakania/view/ActKaKaNiaTaskItem.lua

module("modules.logic.versionactivity1_9.kakania.view.ActKaKaNiaTaskItem", package.seeall)

local ActKaKaNiaTaskItem = class("ActKaKaNiaTaskItem", ListScrollCellExtend)

function ActKaKaNiaTaskItem:onInitView()
...
function ActKaKaNiaTaskItem:addEvents()
...
function ActKaKaNiaTaskItem:_btnnotfinishbgOnClick()
	...
	ViewMgr.instance:closeView(ViewName.ActKaKaNiaTaskView)
end

function ActKaKaNiaTaskItem:_btnfinishbgOnClick()
...
function ActKaKaNiaTaskItem:_btngetallOnClick()
	if RoleActivityController.instance:delayReward(RoleActivityEnum.AnimatorTime.TaskReward, self._actTaskMO) then
		RoleActivityController.instance:dispatchEvent(RoleActivityEvent.OneClickClaimReward)
	end
end

function ActKaKaNiaTaskItem:_editableInitView()
	self._animator = self.viewGO:GetComponent(typeof(UnityEngine.Animator))
	self.viewTrs = self.viewGO.transform
end

function ActKaKaNiaTaskItem:_editableAddEvents()
	self.addEventCb(self, RoleActivityController.instance, RoleActivityEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function ActKaKaNiaTaskItem:_editableRemoveEvents()
	self.removeEventCb(self, RoleActivityController.instance, RoleActivityEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function ActKaKaNiaTaskItem:_onOneClickClaimReward()
...
function ActKaKaNiaTaskItem:getAnimator()
	return self._animator
end

function ActKaKaNiaTaskItem:onUpdateMO(mo)
...
function ActKaKaNiaTaskItem:_moveByRankDiff(rankDiff)
...
function ActKaKaNiaTaskItem:_refreshUI()
...
function ActKaKaNiaTaskItem:_onItemShow(cell_component, data, index)
...
function ActKaKaNiaTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
