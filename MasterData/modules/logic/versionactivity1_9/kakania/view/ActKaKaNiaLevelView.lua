-- chunkname: @modules/logic/versionactivity1_9/kakania/view/ActKaKaNiaLevelView.lua

module("modules.logic.versionactivity1_9.kakania.view.ActKaKaNiaLevelView", package.seeall)

local ActKaKaNiaLevelView = class("ActKaKaNiaLevelView", BaseView)

function ActKaKaNiaLevelView:onInitView()
...
function ActKaKaNiaLevelView:addEvents()
...
function ActKaKaNiaLevelView:removeEvents()
...
function ActKaKaNiaLevelView:_btnPlayBtnOnClick()
	if self.actConfig.storyId > 0 then
		StoryController.instance:playStory(self.actConfig.storyId)
	end
end

function ActKaKaNiaLevelView:_btnStoryOnClick(isOpen)
...
function ActKaKaNiaLevelView:_btnFightOnClick(isOpen)
...
function ActKaKaNiaLevelView:_btnTaskOnClick()
	ViewMgr.instance:openView(ViewName.ActKaKaNiaTaskView)
end

function ActKaKaNiaLevelView:_editableInitView()
...
function ActKaKaNiaLevelView:onOpen()
	self.addEventCb(self, RoleActivityController.instance, RoleActivityEvent.StoryItemClick, self.OnStoryItemClick, self)
	self.addEventCb(self, RoleActivityController.instance, RoleActivityEvent.FightItemClick, self.OnFightItemClick, self)
	self.addEventCb(self, RoleActivityController.instance, RoleActivityEvent.TabSwitch, self.OnTabSwitch, self)
...
function ActKaKaNiaLevelView:_checkFirstEnter()
	if not self.storyItemList[2]:isUnlock() then
		local record = PlayerPrefsHelper.getNumber("ActKaKaNiaFirstEnter", 0)

		if record == 0 then
			PlayerPrefsHelper.setNumber("ActKaKaNiaFirstEnter", 1)
...
function ActKaKaNiaLevelView:onClose()
...
function ActKaKaNiaLevelView:onDestroyView()
...
function ActKaKaNiaLevelView:OnStoryItemClick(index)
	self:_focusStoryItem(index, true)
end

function ActKaKaNiaLevelView:OnFightItemClick(_index)
...
function ActKaKaNiaLevelView:OnTabSwitch(needShowFight)
...
function ActKaKaNiaLevelView:_onStoryOpenEnd()
	self:_initPathStatus()
end

function ActKaKaNiaLevelView:_onFightOpenEnd()
	self:_playFightFinishAnim()
end

function ActKaKaNiaLevelView:_onGoStoryEnd()
	self:_initPathStatus()
end

function ActKaKaNiaLevelView:_onDragBegin()
	self._audioScroll:onDragBegin()
end

function ActKaKaNiaLevelView:_onDragEnd()
	self._audioScroll:onDragEnd()
end

function ActKaKaNiaLevelView:_onClickDown()
	self._audioScroll:onClickDown()
end

function ActKaKaNiaLevelView:_initStageItems()
	...
	for i = 1, #storyConfigList do
		...
		self.storyItemList[i] = MonoHelper.addNoUpdateLuaComOnceToGo(cloneGo, KaKaNiaStoryItem, self)
	...
	for i = 1, #fightConfigList / 2 do
		...
		self.fightItemList[i] = MonoHelper.addNoUpdateLuaComOnceToGo(cloneGo, KaKaNiaFightItem, self)
...
function ActKaKaNiaLevelView:_playFirstUnlock()
...
function ActKaKaNiaLevelView:_playStoryFinishAnim()
...
function ActKaKaNiaLevelView:_finishStoryEnd()
...
function ActKaKaNiaLevelView:_playPathAnim()
...
function ActKaKaNiaLevelView:_unlockStory()
	self.storyItemList[self.finishStoryIndex + 1]:playUnlock()
	TaskDispatcher.runDelay(self._unlockStoryEnd, self, 1.33)
end

function ActKaKaNiaLevelView:_unlockStoryEnd()
...
function ActKaKaNiaLevelView:_playFightFinishAnim()
...
function ActKaKaNiaLevelView:_starShowEnd()
...
function ActKaKaNiaLevelView:_unlockFightEnd()
...
function ActKaKaNiaLevelView:_delayOpenStory()
	self._anim:Play("openstory", 0, 0)
end

function ActKaKaNiaLevelView:_delayOpenFight()
	self._anim:Play("openfight", 0, 0)
end

function ActKaKaNiaLevelView:_showLeftTime()
	self._txtlimittime.text = ActivityHelper.getActivityRemainTimeStr(self.actId)
end

function ActKaKaNiaLevelView:_initPathStatus()
...
function ActKaKaNiaLevelView:_focusStoryItem(index, needPlay)
...
function ActKaKaNiaLevelView:_onFocusEnd(index)
	self.storyItemList[index]:playStory()
end

function ActKaKaNiaLevelView:_lockScreen(lock)
	if lock then
		UIBlockMgrExtend.setNeedCircleMv(false)
		UIBlockMgr.instance:startBlock("KaKaNiaLock")
	else
		UIBlockMgr.instance:endBlock("KaKaNiaLock")
		UIBlockMgrExtend.setNeedCircleMv(true)
	end
end

return ActKaKaNiaLevelView
