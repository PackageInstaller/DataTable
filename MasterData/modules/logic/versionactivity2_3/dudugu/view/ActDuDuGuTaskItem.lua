-- chunkname: @modules/logic/versionactivity2_3/dudugu/view/ActDuDuGuTaskItem.lua

module("modules.logic.versionactivity2_3.dudugu.view.ActDuDuGuTaskItem", package.seeall)

local ActDuDuGuTaskItem = class("ActDuDuGuTaskItem", ListScrollCellExtend)

function ActDuDuGuTaskItem:onInitView()
...
function ActDuDuGuTaskItem:addEvents()
...
function ActDuDuGuTaskItem:_btnnotfinishbgOnClick()
	...
	ViewMgr.instance:closeView(ViewName.ActDuDuGuTaskView)
end

function ActDuDuGuTaskItem:_btnfinishbgOnClick()
...
function ActDuDuGuTaskItem:_btngetallOnClick()
	if RoleActivityController.instance:delayReward(RoleActivityEnum.AnimatorTime.TaskReward, self._actTaskMO) then
		RoleActivityController.instance:dispatchEvent(RoleActivityEvent.OneClickClaimReward)
	end
end

function ActDuDuGuTaskItem:_editableInitView()
	self._animator = self.viewGO:GetComponent(typeof(UnityEngine.Animator))
	self.viewTrs = self.viewGO.transform
end

function ActDuDuGuTaskItem:_editableAddEvents()
...
function ActDuDuGuTaskItem:_editableRemoveEvents()
...
function ActDuDuGuTaskItem:_onOneClickClaimReward()
...
function ActDuDuGuTaskItem:getAnimator()
	return self._animator
end

function ActDuDuGuTaskItem:onUpdateMO(mo)
...
function ActDuDuGuTaskItem:_moveByRankDiff(rankDiff)
...
function ActDuDuGuTaskItem:_refreshUI()
...
function ActDuDuGuTaskItem:_onItemShow(cell_component, data, index)
...
function ActDuDuGuTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
