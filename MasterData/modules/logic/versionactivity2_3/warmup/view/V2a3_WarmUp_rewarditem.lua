-- chunkname: @modules/logic/versionactivity2_3/warmup/view/V2a3_WarmUp_rewarditem.lua

module("modules.logic.versionactivity2_3.warmup.view.V2a3_WarmUp_rewarditem", package.seeall)

local V2a3_WarmUp_rewarditem = class("V2a3_WarmUp_rewarditem", RougeSimpleItemBase)

function V2a3_WarmUp_rewarditem:onInitView()
	if self._editableInitView then
		self:_editableInitView()
	end
end

function V2a3_WarmUp_rewarditem:addEvents()
	return
end

function V2a3_WarmUp_rewarditem:removeEvents()
	return
end

function V2a3_WarmUp_rewarditem:ctor(ctorParam)
	V2a3_WarmUp_rewarditem.super.ctor(self, ctorParam)
end

function V2a3_WarmUp_rewarditem:onDestroyView()
	V2a3_WarmUp_rewarditem.super.onDestroyView(self)
end

function V2a3_WarmUp_rewarditem:_editableInitView()
	V2a3_WarmUp_rewarditem.super._editableInitView(self)
...
function V2a3_WarmUp_rewarditem:playAnim_hasget(isIdle)
	if isIdle then
...
function V2a3_WarmUp_rewarditem:_set_Received()
	self._hasgetAnim:Play(kHasGetAnim, 0, 1)
end

function V2a3_WarmUp_rewarditem:_setActive_canget(isActive)
	gohelper.setActive(self._gocanget, isActive)
end

function V2a3_WarmUp_rewarditem:_setActive_hasget(isActive)
	gohelper.setActive(self._go_hasget, isActive)
end

return V2a3_WarmUp_rewarditem
