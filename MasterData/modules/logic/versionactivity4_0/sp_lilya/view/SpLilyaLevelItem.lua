-- chunkname: @modules/logic/versionactivity4_0/sp_lilya/view/SpLilyaLevelItem.lua

module("modules.logic.versionactivity4_0.sp_lilya.view.SpLilyaLevelItem", package.seeall)

local SpLilyaLevelItem = class("SpLilyaLevelItem", LuaCompBase)

function SpLilyaLevelItem:init(go)
	self.viewGO = go
...
function SpLilyaLevelItem:_initNode(nodeGo)
...
function SpLilyaLevelItem:addEventListeners()
	self._btnclick:AddClickListener(self._btnOnClick, self)
end

function SpLilyaLevelItem:removeEventListeners()
	self._btnclick:RemoveClickListener()
end

function SpLilyaLevelItem:_btnOnClick()
	SpLilyaController.instance:clickEpisodeLevel(self.id, self._index)
end

function SpLilyaLevelItem:setParam(co, index, actId)
	self._config = co
	self.id = co.episodeId
	self._actId = actId
	self._index = index
	self.gameId = self._config.gameId
	self.fightEpisodeId = self._config.fightEpisodeId
	self._isStoryEpisode = self.gameId == 0 and self.fightEpisodeId == 0
...
function SpLilyaLevelItem:isEpisodeUnlock()
	local mo = Activity220Model.instance:getById(self._actId)

	return mo:isEpisodeUnlock(self.id)
end

function SpLilyaLevelItem:isEpisodePass()
	local episodeInfo = Activity220Model.instance:getEpisodeInfo(self._actId, self.id)

	return episodeInfo and episodeInfo:isEpisodePass() or false
end

function SpLilyaLevelItem:getCurEpisode()
	local mo = Activity220Model.instance:getById(self._actId)

	return mo:getCurEpisode()
end

function SpLilyaLevelItem:refreshUI()
...
function SpLilyaLevelItem:playFinish()
	self._ispass = self:isEpisodePass()
...
function SpLilyaLevelItem:playUnlock()
	self._isunlock = self:isEpisodeUnlock()
...
function SpLilyaLevelItem:playStarAnim()
	AudioMgr.instance:trigger(AudioEnum.RoleActivity.star_show)
end

return SpLilyaLevelItem
