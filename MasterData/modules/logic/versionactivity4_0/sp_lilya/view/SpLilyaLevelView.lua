-- chunkname: @modules/logic/versionactivity4_0/sp_lilya/view/SpLilyaLevelView.lua

module("modules.logic.versionactivity4_0.sp_lilya.view.SpLilyaLevelView", package.seeall)

local SpLilyaLevelView = class("SpLilyaLevelView", BaseView)
local FocusDuration = 0.5

function SpLilyaLevelView:onInitView()
...
function SpLilyaLevelView:addEvents()
	self._btntask:AddClickListener(self._btntaskOnClick, self)
	self._scrollstory:AddOnValueChanged(self.onValueChanged, self)
	self.addEventCb(self, Activity220Controller.instance, Activity220Event.EpisodeFinished, self._onEpisodeFinished, self)
...
function SpLilyaLevelView:removeEvents()
	self._btntask:RemoveClickListener()
	self._scrollstory:RemoveOnValueChanged()
	self.removeEventCb(self, Activity220Controller.instance, Activity220Event.EpisodeFinished, self._onEpisodeFinished, self)
...
function SpLilyaLevelView:_btntaskOnClick()
	SpLilyaController.instance:openTaskView()
end

function SpLilyaLevelView:onValueChanged()
...
function SpLilyaLevelView:_onEpisodeFinished()
	...
	if newEpisode and newEpisode ~= 0 then
		...
		self._curEpisodeIndex = SpLilyaConfig.instance:getEpisodeIndex(maxUnlockEpisode)
...
function SpLilyaLevelView:_playStoryFinishAnim()
	local mo = Activity220Model.instance:getById(self.actId)
...
function SpLilyaLevelView:_finishStoryEnd()
...
function SpLilyaLevelView:_unlockStory()
	if self._finishEpisodeIndex then
		if not self._finishEpisodeIndex then
			return
		end

		self._finishEpisodeIndex:refreshUI()
		self._finishEpisodeIndex:playUnlock()
...
function SpLilyaLevelView:_unlockLvEnd()
	self._finishEpisodeIndex = nil
end

function SpLilyaLevelView:_onCloseView(viewName)
	if viewName == ViewName.SpLilyaTaskView then
		self:_refreshTask()
	end
end

function SpLilyaLevelView:_onScreenSizeChange()
...
function SpLilyaLevelView:_editableInitView()
	self:_initViewInfo()

	self.actId = SpLilyaModel.instance:getActId()
...
function SpLilyaLevelView:_initViewInfo()
...
function SpLilyaLevelView:onUpdateParam()
	if self._curEpisodeIndex then
		self:_focusStoryItem(self._curEpisodeIndex)
	end
end

function SpLilyaLevelView:onOpen()
...
function SpLilyaLevelView:_initLevelItems()
	...
	local episodeCos = SpLilyaConfig.instance:getEpisodeConfigList()
	local maxUnlockEpisode = Activity220Model.instance:getMaxUnlockEpisodeId(self.actId)
	...
	for i = 1, #episodeCos do
		...
		if nodeRoot then
			...
			self._episodeItems[i] = MonoHelper.addNoUpdateLuaComOnceToGo(cloneGo, SpLilyaLevelItem, self)

			self._episodeItems[i]:setParam(episodeCos[i], i, self.actId)
		else
			logError(string.format("SpLilyaLevelView:_initLevelItems error, no stage node, index:%s", i))
		end
	end
...
function SpLilyaLevelView:_focusStoryItem(index, needPlay)
...
function SpLilyaLevelView:_refreshTask()
...
function SpLilyaLevelView:_refreshLeftTime()
	self._txtLimitTime.text = ActivityHelper.getActivityRemainTimeStr(self.actId)
end

function SpLilyaLevelView:onClose()
	TaskDispatcher.cancelTask(self._refreshLeftTime, self)
...
function SpLilyaLevelView:onDestroyView()
	self._episodeItems = nil
end

return SpLilyaLevelView
