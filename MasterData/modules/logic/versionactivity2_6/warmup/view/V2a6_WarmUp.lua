-- chunkname: @modules/logic/versionactivity2_6/warmup/view/V2a6_WarmUp.lua

module("modules.logic.versionactivity2_6.warmup.view.V2a6_WarmUp", package.seeall)

local V2a6_WarmUp = class("V2a6_WarmUp", BaseView)

function V2a6_WarmUp:onInitView()
	self._simagefullbg = gohelper.findChildSingleImage(self.viewGO, "#simage_fullbg")
	self._btngoto = gohelper.findChildButtonWithAudio(self.viewGO, "Middle/#btn_goto")
...
function V2a6_WarmUp:addEvents()
...
function V2a6_WarmUp:removeEvents()
...
function V2a6_WarmUp:_btngetrewardOnClick()
...
function V2a6_WarmUp:_editableInitView()
...
function V2a6_WarmUp:onDataUpdateFirst()
	self:_refreshOnce()
end

function V2a6_WarmUp:onDataUpdate()
	self:_refresh()
end

function V2a6_WarmUp:onSwitchEpisode()
...
function V2a6_WarmUp:onUpdateParam()
	self:_refreshOnce()
	self:_refresh()
end

function V2a6_WarmUp:onOpen()
...
function V2a6_WarmUp:onDestroyView()
	return
end

function V2a6_WarmUp:_refreshOnce()
	self:_showDeadline()
	self:_refreshTabList()
	self:_autoSelectTab()
end

function V2a6_WarmUp:_refresh()
	self:_refreshData()
	self:_refreshTabList()
...
function V2a6_WarmUp:_refreshRightView()
...
function V2a6_WarmUp:_setActive_goWrongChannel(isBlockDesc)
	gohelper.setActive(self._goWrongChannel, isBlockDesc)
	self:_setActive_scroll_TaskDescGo(not isBlockDesc)
...
function V2a6_WarmUp:_refreshData()
...
function V2a6_WarmUp:_refreshTabList()
	...
	for i = 1, maxEpisodeCount do
		...
		if i > #self._itemTabList then
			item = self:_create_V2a6_WarmUp_radiotaskitem(i)
...
function V2a6_WarmUp:_setSelectIndex(index, isFocus)
...
function V2a6_WarmUp:_taskScrollToIndex(index)
...
function V2a6_WarmUp:onClickTab(mo)
...
function V2a6_WarmUp:_refreshRewards()
	local co = self.viewContainer:getEpisodeConfigCur()
	...
	for i = 1, itemCount do
		...
		if i > #self._rewardItemList then
			item = self:_create_V2a6_WarmUp_rewarditem(i)
...
function V2a6_WarmUp:openDesc(cb, cbObj)
	...
	self:_setActive_scroll_TaskDescGo(true)
...
function V2a6_WarmUp:_tweenDescUpdateCb(value)
	local bottom = lerp(0, self._taskDescViewportHeight, value)

	self:_setMaskPaddingBottom(bottom)
end

function V2a6_WarmUp:_tweenDescEndCb(maskDurationTime, cb, cbObj)
...
function V2a6_WarmUp:_resetTaskContentPos()
	recthelper.setAnchorY(self._txtTaskContentTran, 0)
end

function V2a6_WarmUp:episode2Index(episodeId)
	return episodeId
end

function V2a6_WarmUp:index2EpisodeId(index)
...
function V2a6_WarmUp:_setMaskPaddingBottom(bottom)
	self._taskDescMask.padding = Vector4(0, bottom, 0, 0)
end

function V2a6_WarmUp:_autoSelectTab()
...
function V2a6_WarmUp:_create_V2a6_WarmUp_radiotaskitem(index)
	local go = gohelper.cloneInPlace(self._goradiotaskitem)
	local item = V2a6_WarmUp_radiotaskitem.New({
		parent = self,
...
function V2a6_WarmUp:_create_V2a6_WarmUp_rewarditem(index)
	local go = gohelper.cloneInPlace(self._gorewarditem)
	local item = V2a6_WarmUp_rewarditem.New({
		parent = self,
		baseViewContainer = self.viewContainer
	})
...
function V2a6_WarmUp:_resetTweenDescPos()
	GameUtil.onDestroyViewMember_TweenId(self, "_movetweenId")
	GameUtil.onDestroyViewMember_TweenId(self, "_tweenId")
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_wulu_atticletter_write_stop)
	self:_resetTaskContentPos()
...
function V2a6_WarmUp:_playAnim(name, cb, cbObj)
	self._animatorPlayer:Play(name, cb, cbObj)
end

function V2a6_WarmUp:tweenSwitch(cb, cbObj)
	self:_playAnim(UIAnimationName.Switch, cb, cbObj)
end

function V2a6_WarmUp:_onSwitch()
	local lastEpisodeId = self.viewContainer:getCurSelectedEpisode()
	local curEpisodeId
...
function V2a6_WarmUp:playRewardItemsHasGetAnim()
...
function V2a6_WarmUp:setBlock_scroll(isBlock)
	self._scrollCanvasGroup.blocksRaycasts = not isBlock
end

function V2a6_WarmUp:_refreshActive_btngoto()
	...
	local isActive = ServerTime.now() >= TimeUtil.stringToTimestamp(offtime) and not string.nilorempty(self.viewContainer:getH5BaseUrl())

	gohelper.setActive(self._btngotoGo, isActive)
end

function V2a6_WarmUp:_refreshTxtTaskContentHeight()
	ZProj.UGUIHelper.RebuildLayout(self._txtTaskContentTran)

	self._descHeight = self._txtTaskContent.preferredHeight
end

function V2a6_WarmUp:_setActive_scroll_TaskDescGo(isActive)
	gohelper.setActive(self._scroll_TaskDescGo, isActive)
