-- chunkname: @modules/logic/versionactivity1_8/weila/view/ActWeilaTaskItem.lua

module("modules.logic.versionactivity1_8.weila.view.ActWeilaTaskItem", package.seeall)

local ActWeilaTaskItem = class("ActWeilaTaskItem", ListScrollCellExtend)

function ActWeilaTaskItem:onInitView()
...
function ActWeilaTaskItem:addEvents()
...
function ActWeilaTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function ActWeilaTaskItem:_btnnotfinishbgOnClick()
	ActWeilaController.instance:openLevelView({
		needShowFight = self._actTaskMO.config.listenerType ~= "StoryFinish"
	})
	ViewMgr.instance:closeView(ViewName.ActWeilaTaskView)
end

function ActWeilaTaskItem:_btnfinishbgOnClick()
	if ActWeilaController.instance:delayReward(ActWeilaEnum.AnimatorTime.TaskReward, self._actTaskMO) then
		self:_onOneClickClaimReward()
	end
end

function ActWeilaTaskItem:_btngetallOnClick()
	if ActWeilaController.instance:delayReward(ActWeilaEnum.AnimatorTime.TaskReward, self._actTaskMO) then
		ActWeilaController.instance:dispatchEvent(ActWeilaEvent.OneClickClaimReward)
	end
end

function ActWeilaTaskItem:_editableInitView()
	self._animator = self.viewGO:GetComponent(typeof(UnityEngine.Animator))
	self.viewTrs = self.viewGO.transform
end

function ActWeilaTaskItem:_editableAddEvents()
	self.addEventCb(self, ActWeilaController.instance, ActWeilaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function ActWeilaTaskItem:_editableRemoveEvents()
	self.removeEventCb(self, ActWeilaController.instance, ActWeilaEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function ActWeilaTaskItem:_onOneClickClaimReward()
...
function ActWeilaTaskItem:getAnimator()
	return self._animator
end

function ActWeilaTaskItem:onUpdateMO(mo)
	self._scrollreward.parentGameObject = self._view._csListScroll.gameObject
	self._actTaskMO = mo

	local rankDiff = ActWeilaTaskListModel.instance.instance:getRankDiff(mo)

	self:_refreshUI()
	self:_moveByRankDiff(rankDiff)
end

function ActWeilaTaskItem:_moveByRankDiff(rankDiff)
	if rankDiff and rankDiff ~= 0 then
		...
		self._rankDiffMoveId = ZProj.TweenHelper.DOAnchorPosY(self.viewTrs, 0, ActWeilaEnum.AnimatorTime.TaskRewardMoveUp)
	end
end

function ActWeilaTaskItem:_refreshUI()
...
function ActWeilaTaskItem:_onItemShow(cell_component, data, index)
...
function ActWeilaTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
