-- chunkname: @modules/logic/versionactivity1_8/dungeon/view/maplevel/VersionActivity1_8DungeonMapLevelView.lua

module("modules.logic.versionactivity1_8.dungeon.view.maplevel.VersionActivity1_8DungeonMapLevelView", package.seeall)

local VersionActivity1_8DungeonMapLevelView = class("VersionActivity1_8DungeonMapLevelView", BaseView)
local OPEN_ANIM_TIME = 0.4
local UNLOCK_ANIM_TIME = 2.7

function VersionActivity1_8DungeonMapLevelView:onInitView()
...
function VersionActivity1_8DungeonMapLevelView:addEvents()
...
function VersionActivity1_8DungeonMapLevelView:removeEvents()
	self.removeEventCb(self, CurrencyController.instance, CurrencyEvent.CurrencyChange, self._onCurrencyChange, self)
...
function VersionActivity1_8DungeonMapLevelView:_onCurrencyChange(changeIds)
...
function VersionActivity1_8DungeonMapLevelView:_btnleftarrowOnClick()
...
function VersionActivity1_8DungeonMapLevelView:_btnrightarrowOnClick()
	local isHardMode = self.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard
...
function VersionActivity1_8DungeonMapLevelView:refreshUIByMode(mode)
...
function VersionActivity1_8DungeonMapLevelView:_btnactivityrewardOnClick()
	DungeonController.instance:openDungeonRewardView(self.showEpisodeCo)
end

function VersionActivity1_8DungeonMapLevelView:_btnnormalStartOnClick()
...
function VersionActivity1_8DungeonMapLevelView:_btnhardStartOnClick()
	self:startBattle()
end

function VersionActivity1_8DungeonMapLevelView:startBattle()
	...
	if self.isSpecialEpisode then
		...
		local prefsKey = VersionActivity1_8DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode
		local strLastSelectMode = cjson.encode(self.lastEpisodeSelectModeDict)

		VersionActivity1_8DungeonController.instance:savePlayerPrefs(prefsKey, strLastSelectMode)
	end
...
function VersionActivity1_8DungeonMapLevelView:_playSkipMainStory()
	DungeonRpc.instance:sendStartDungeonRequest(self.showEpisodeCo.chapterId, self.showEpisodeCo.id)
	self:onStoryFinished()
end

function VersionActivity1_8DungeonMapLevelView:_playMainStory()
	DungeonRpc.instance:sendStartDungeonRequest(self.showEpisodeCo.chapterId, self.showEpisodeCo.id)
...
function VersionActivity1_8DungeonMapLevelView:onStoryFinished()
...
function VersionActivity1_8DungeonMapLevelView:_playStoryAndEnterFight(storyId)
...
function VersionActivity1_8DungeonMapLevelView:_enterFight()
	DungeonFightController.instance:enterFight(self.showEpisodeCo.chapterId, self.showEpisodeCo.id, 1)
end

function VersionActivity1_8DungeonMapLevelView:playAfterStory(storyId)
	local param = {}
...
function VersionActivity1_8DungeonMapLevelView:_btnlockStartOnClick()
	local preModeName = self:getPreModeName()

	GameFacade.showToast(ToastEnum.VersionActivityCanFight, preModeName)
end

function VersionActivity1_8DungeonMapLevelView:getPreModeName()
...
function VersionActivity1_8DungeonMapLevelView:_btnreplayStoryOnClick()
...
function VersionActivity1_8DungeonMapLevelView:_editableInitView()
	self.rewardItems = {}
...
function VersionActivity1_8DungeonMapLevelView:initLocalEpisodeMode()
	local unlockModePrefsKey = VersionActivity1_8DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode
	local strLastUnlockModeValue = VersionActivity1_8DungeonController.instance:getPlayerPrefs(unlockModePrefsKey, "")

	self.unlockedEpisodeModeDict = VersionActivity1_8DungeonController.instance:loadDictFromStr(strLastUnlockModeValue)

	local lastSelectModePrefsKey = VersionActivity1_8DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode
	local strLastSelectModeValue = VersionActivity1_8DungeonController.instance:getPlayerPrefs(lastSelectModePrefsKey, "")

	self.lastEpisodeSelectModeDict = VersionActivity1_8DungeonController.instance:loadDictFromStr(strLastSelectModeValue)
end

function VersionActivity1_8DungeonMapLevelView:onUpdateParam()
	self:onOpen()
	self.animator:Play(UIAnimationName.Open, 0, 0)
end

function VersionActivity1_8DungeonMapLevelView:onOpen()
...
function VersionActivity1_8DungeonMapLevelView:initViewParam()
	...
	self.index = VersionActivity1_8DungeonConfig.instance:getEpisodeIndex(self.originEpisodeId)
...
function VersionActivity1_8DungeonMapLevelView:initMode()
	...
	if self.isFromJump then
		...
		for i = #episodeList, 1, -1 do
			if DungeonModel.instance:hasPassLevelAndStory(episodeList[i].preEpisode) then
...
function VersionActivity1_8DungeonMapLevelView:checkNeedPlayModeUnLockAnimation()
...
function VersionActivity1_8DungeonMapLevelView:markSelectEpisode()
	if self.originEpisodeConfig.type == DungeonEnum.EpisodeType.Act1_8Dungeon then
...
function VersionActivity1_8DungeonMapLevelView:refreshStoryIdList()
	...
	if self.mode2EpisodeDict then
		if self.mode2EpisodeDict then
			checkStoryEpisodeCfg = self.mode2EpisodeDict
		end
...
function VersionActivity1_8DungeonMapLevelView:refreshBg()
...
function VersionActivity1_8DungeonMapLevelView:refreshUI()
...
function VersionActivity1_8DungeonMapLevelView:refreshModeCanFight()
...
function VersionActivity1_8DungeonMapLevelView:refreshEpisodeTextInfo()
	local chapterCo = DungeonConfig.instance:getChapterCO(self.showEpisodeCo.chapterId)
	local targetConfig = chapterCo.id == VersionActivity1_8DungeonEnum.DungeonChapterId.Story and self.showEpisodeCo or VersionActivity1_8DungeonConfig.instance:getStoryEpisodeCo(self.showEpisodeCo.id)
...
function VersionActivity1_8DungeonMapLevelView:buildEpisodeName(episodeCo)
	local name = episodeCo.name
	local firstName = GameUtil.utf8sub(name, 1, 1)
	local nameLen = GameUtil.utf8len(name)
	local txtColor = self.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard and "#cfccc9" or "#cfccc9"

	return self:buildColorText(string.format("<size=112>%s</size>%s", firstName, (nameLen > 1 or nil) and GameUtil.utf8sub(name, 2, nameLen - 1)), txtColor)
end

function VersionActivity1_8DungeonMapLevelView:buildColorText(text, color)
	return string.format("<color=%s>%s</color>", color, text)
end

function VersionActivity1_8DungeonMapLevelView:refreshStar()
	local normalEpisodeId = self.showEpisodeCo.id

	if normalEpisodeId then
		::label_37_0::

		local passStory = DungeonModel.instance:hasPassLevelAndStory(normalEpisodeId)
...
function VersionActivity1_8DungeonMapLevelView:setStarImage(image, light, episodeId)
...
function VersionActivity1_8DungeonMapLevelView:refreshMode()
...
function VersionActivity1_8DungeonMapLevelView:refreshArrow()
...
function VersionActivity1_8DungeonMapLevelView:refreshReward()
	local rewardList = {}
...
function VersionActivity1_8DungeonMapLevelView:refreshStartBtn()
	self:refreshCostPower()
	...
	if self.storyIdList then
		local hasStory = #self.storyIdList > 0

		if self.mode2EpisodeDict then
			if self.mode2EpisodeDict then
				if not self.mode2EpisodeDict.id then
					local checkPassEpisodeId = self.originEpisodeConfig.id
...
function VersionActivity1_8DungeonMapLevelView:refreshCostPower()
	local cost = 0
...
function VersionActivity1_8DungeonMapLevelView:refreshEye()
	...
	local isHard = self.originEpisodeConfig.chapterId == VersionActivity1_8DungeonEnum.DungeonChapterId.Hard

	gohelper.setActive(self._gonormaleye, not isHard)
	gohelper.setActive(self._gohardeye, isHard)
end

function VersionActivity1_8DungeonMapLevelView:playModeUnlockAnimation()
	if not self.needPlayUnlockModeAnimation then
		return
	end
	...
	UIBlockMgr.instance:startBlock(VersionActivity1_8DungeonEnum.BlockKey.MapLevelViewPlayUnlockAnim)
	TaskDispatcher.runDelay(self.onModeUnlockAnimationPlayDone, self, UNLOCK_ANIM_TIME)
end

function VersionActivity1_8DungeonMapLevelView:_playModeUnLockAnimation(animationName)
	self.lockTypeAnimator.enabled = true
...
function VersionActivity1_8DungeonMapLevelView:onModeUnlockAnimationPlayDone()
	self:_playModeUnLockAnimation(UIAnimationName.Idle)

	self.unlockedEpisodeModeDict[tostring(self.specialEpisodeId)] = self.mode

	local prefsKey = VersionActivity1_8DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode
	local strLastUnlockMode = cjson.encode(self.unlockedEpisodeModeDict)

	VersionActivity1_8DungeonController.instance:savePlayerPrefs(prefsKey, strLastUnlockMode)

	self.needPlayUnlockModeAnimation = false

	self:refreshMode()
	self:refreshStartBtn()
	UIBlockMgr.instance:endBlock(VersionActivity1_8DungeonEnum.BlockKey.MapLevelViewPlayUnlockAnim)
end

function VersionActivity1_8DungeonMapLevelView:onClose()
...
function VersionActivity1_8DungeonMapLevelView:onDestroyView()
	self.rewardItems = nil

	self._simagepower:UnLoadImage()
end

return VersionActivity1_8DungeonMapLevelView
