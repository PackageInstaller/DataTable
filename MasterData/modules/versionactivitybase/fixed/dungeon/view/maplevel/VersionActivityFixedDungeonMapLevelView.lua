...
function VersionActivityFixedDungeonMapLevelView:_isOpenPuzzleView(isReplay)
	...
	if storyId then
		...
		return (V3a5PuzzleController.instance:isOpenPuzzleView(storyId, true, co.id, afterStory))
	end
end
...
function VersionActivityFixedDungeonMapLevelView:refreshEpisodeTextInfo()
	...
	local targetConfig = chapterCo.id == VersionActivityFixedHelper.getVersionActivityDungeonEnum(self._bigVersion, self._smallVersion).DungeonChapterId.Story and self.showEpisodeCo or VersionActivityFixedDungeonConfig.instance:getStoryEpisodeCo(self.showEpisodeCo.id)
...
function VersionActivityFixedDungeonMapLevelView:refreshCostPower()
	...
	if cost <= CurrencyModel.instance:getPower() then
		...
		if not dungeonEnum.MapLevelCostPowerNormalColor then
			SLFramework.UGUI.GuiHelper.SetColor(self._txtusepowernormal, dungeonEnum.MapLevelCostPowerNormalColor)
