-- chunkname: @modules/versionactivitybase/fixed1/dungeon/view/VersionActivityFixedDungeonReportFullView1.lua

module("modules.versionactivitybase.fixed1.dungeon.view.VersionActivityFixedDungeonReportFullView1", package.seeall)

local VersionActivityFixedDungeonReportFullView1 = class("VersionActivityFixedDungeonReportFullView1", BaseView)

function VersionActivityFixedDungeonReportFullView1:onInitView()
...
function VersionActivityFixedDungeonReportFullView1:addEvents()
	return
end

function VersionActivityFixedDungeonReportFullView1:removeEvents()
	return
end

function VersionActivityFixedDungeonReportFullView1:_editableInitView()
...
function VersionActivityFixedDungeonReportFullView1:onUpdateParam()
	return
end

function VersionActivityFixedDungeonReportFullView1:onOpen()
	...
	AudioMgr.instance:trigger(VersionActityFixedAudioEnum1.CommonAudio.ui_mln_details_open)
end

function VersionActivityFixedDungeonReportFullView1:onOpenFinish()
	...
	UIBlockHelper.instance:startBlock("VersionActivityFixedDungeonReportFullView1 open", time)
...
function VersionActivityFixedDungeonReportFullView1:_onTweenFrame(value)
	self._sliderprogress:SetValue(value)
end

function VersionActivityFixedDungeonReportFullView1:_delayGetBonus()
	if self._curFinishRewardNum > self._acceptedRewardNum then
		...
		Activity113Rpc.instance:sendGetAct113MilestoneBonusRequest(enum.ActivityId.Dungeon, self._getBonusCallback, self)
	end
end

function VersionActivityFixedDungeonReportFullView1:_delayPlayUnlockAnim()
...
function VersionActivityFixedDungeonReportFullView1:_getBonusCallback(cmd, resultCode, msg)
	if resultCode == 0 then
		...
		for index = self._initRewardNum + 1, self._acceptedRewardNum do
			local rewardConfig = VersionActivityFixedDungeonConfig1.instance:getChapterReward(index)
...
function VersionActivityFixedDungeonReportFullView1.indexOfByKey(array, value, key, begin)
...
function VersionActivityFixedDungeonReportFullView1:_initItems()
	local sortList = {}
	...
	for i, v in ipairs(lua_v3a2_chapter_report.configList) do
		...
		if isFinish then
			...
			local sortIndex = VersionActivityFixedDungeonReportFullView1.indexOfByKey(sortList, v.element, 1) or 0
...
function VersionActivityFixedDungeonReportFullView1:_initRewardItems()
...
function VersionActivityFixedDungeonReportFullView1:_onRewardItemShow(obj, data, index)
...
function VersionActivityFixedDungeonReportFullView1:_addBtnClick(btnclick, data, index)
...
function VersionActivityFixedDungeonReportFullView1:_onBtnClick(btnclick)
	return
end

function VersionActivityFixedDungeonReportFullView1:onClose()
...
function VersionActivityFixedDungeonReportFullView1:onDestroyView()
	return
end

return VersionActivityFixedDungeonReportFullView1
