...
local function clean_string(s)
	...
	-- 保留：空格 + 英文大小写 + 数字 + 常见符号（# . _ - = 等）
	-- 删除：不可见控制符、UTF-8非法字节、二进制乱码
	local cleaned = s:gsub('[^%g \t]', '')
	return cleaned
end
-- 【核心函数】检查字符串是否为合法 UTF-8，返回 是否合法 + 错误信息
-- 能精准定位：非法字节位置、乱码片段、字节值，方便调试
local function check_utf8(str)
	...
	while i <= len do
		...
		elseif b >= 0xC2 and b <= 0xDF then
			if i + 1 > len then return false, "双字节不完整，位置："..i end
			local b2 = str:byte(i+1)
			if b2 < 0x80 or b2 > 0xBF then return false, "双字节非法，位置："..i end
			i = i + 2

			-- 三字节 (224~239开头)
		elseif b >= 0xE0 and b <= 0xEF then
			if i + 2 > len then return false, "三字节不完整，位置："..i end
			local b2 = str:byte(i+1)
			local b3 = str:byte(i+2)
			if b2 < 0x80 or b2 > 0xBF or b3 < 0x80 or b3 > 0xBF then
				return false, "三字节非法，位置："..i
			end
			i = i + 3

			-- 四字节 (240~244开头)
		elseif b >= 0xF0 and b <= 0xF4 then
			if i + 3 > len then return false, "四字节不完整，位置："..i end
			local b2 = str:byte(i+1)
			local b3 = str:byte(i+2)
			local b4 = str:byte(i+3)
			if b2 < 0x80 or b2 > 0xBF or b3 < 0x80 or b3 > 0xBF or b4 < 0x80 or b4 > 0xBF then
				return false, "四字节非法，位置："..i
			end
			i = i + 4

			-- 非法字节（真正触发你报错的根源）
		else
			return false, string.format("非法字节序列，位置=%d，字节值=0x%02X，片段=%s",
					i, b, str:sub(math.max(1, i-5), i+5)) -- 打印前后5个字符定位乱码
		end
	end

	return true, "UTF-8 合法"
end
...
local function CreateObjectReferenceInfoContainerFromFile(strFilePath)
	...
	local cCustomLogInfo = cContainer.m_cCustomLogInfo
...
local function CollectObjectReferenceInMemory(strName, cObject, cDumpInfoContainer)
	...
	local cCustomLogInfo = cDumpInfoContainer.m_cCustomLogInfo
	...
	if "table" == strType then
		...
		if cNameInfoContainer[cObject] then
			local sameInfo = "[same]["..cNameInfoContainer[cObject].."]["..strName.."]"
			-- print(sameInfo)
			table.insert(cCustomLogInfo, sameInfo)
			return
		end
...
local function OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject, cDumpInfoResultsBase, cDumpInfoResults)
	...
	local cCustomLogInfo = cDumpInfoResults.m_cCustomLogInfo
	...
	local strFileName = strExtraFileName
	if true then
		...
		DUtil.OutputOpenFile(strFileName)
	end
	...
	local cOutputer = function (strContent)
		...
		DUtil.OutputWriteFile(clean_string(strContent))
	end
	...
	if not cDumpInfoResultsBase or not cDumpInfoResults then
		local cFile = assert(io.open("temp12345.txt", "w"))
		io.output(cFile)
		for i, v in ipairs(cCustomLogInfo) do
			io.write(v.." \n")
		end
		io.close(cFile)
	end
