...
local wx_api = setmetatable({}, {
  __index = function(self, key)
    local _func = rawget(self, key)
    if not _func then
      _func = _ejoysdk.wx[key] or function()
...
local M = {}
local wx_fs = wx_api.getFileSystemManager()

function _ejoysdk.lread(filename)
  local ret, data = pcall(wx_fs.readFileSync, filename, "binary")
  return ret and data or nil
end

M.PLATFORM = {
  OS = "weixin",
  HTTP_UA = "EjoySDK-http-client/0.1 (Linux; Weixin)"
}
...
local function http_get_file(url, params, cb)
  ...
  if params.progress or params.finish_cb then
    ...
    function progress_cb(res)
      ...
      if 100 == progress and params.finish_cb then
        params.finish_cb()
        download_task.offProgressUpdate(progress_cb)
      end
    end
  ...
  if params.header_cb then
    local download_header_cb
    
    function download_header_cb(res)
      local resp_headers = HTTP.Header.New(res.header)
      params.header_cb(resp_headers)
      download_task.offHeadersReceived(download_header_cb)
    end
    
    download_task.onHeadersReceived(download_header_cb)
  end
end
...
local function http_post_data(url, params, content_type, body, cb)
  ...
  local req = {
    enableHttp2 = params.enable_http2 and true or false,
    url = url,
    method = "POST",
    header = headers,
    timeout = HTTP.wx_timeout,
...
local device_info = setmetatable({}, {
  __index = function(self, idx)
    if not rawget(self, "inited") then
      local info = _ejoysdk.wx.getDeviceInfo()
      rawset(self, "brand", info.brand)
      rawset(self, "model", info.model)
      rawset(self, "system", info.system)
      rawset(self, "platform", info.platform)
      rawset(self, "cpuType", info.cpuType)
      rawset(self, "memorySize", info.memorySize)
      rawset(self, "inited", true)
    end
    return rawget(self, idx) or ""
  end
})
local app_base_info = setmetatable({}, {
  __index = function(self, idx)
    if not rawget(self, "inited") then
      local info = _ejoysdk.wx.getAppBaseInfo()
      local wx_appid = info.host.appId
      if not wx_appid or "" == wx_appid then
        wx_appid = _ejoysdk.wx.getAccountInfoSync().miniProgram.appId
      end
      rawset(self, "WXSDKVersion", info.SDKVersion)
      rawset(self, "enableDebug", info.enableDebug)
      rawset(self, "appId", wx_appid)
      rawset(self, "language", info.language)
      rawset(self, "wx_version", info.version)
      rawset(self, "theme", info.theme)
      rawset(self, "fontSizeScaleFactor", info.fontSizeScaleFactor)
      rawset(self, "fontSizeSetting", info.fontSizeSetting)
      rawset(self, "inited", true)
    end
    return rawget(self, idx) or ""
  end
})
local account_info = setmetatable({}, {
  __index = function(self, idx)
    if not rawget(self, "inited") then
      local info = _ejoysdk.wx.getAccountInfoSync()
      rawset(self, "miniProgram", {
...
local window_info = setmetatable({}, {
  __index = function(self, idx)
    if "reset" == idx then
      return reset_metatable_value
    end
    if not rawget(self, "inited") then
      local info = _ejoysdk.wx.getWindowInfo()
      ...
      rawset(self, "statusBarHeight", info.statusBarHeight)
      local safe_area = info.safeArea
      if not safe_area or "" == safe_area then
        safe_area = {}
      end
      ...
      rawset(self, "screenTop", info.screenTop)
      rawset(self, "inited", true)
    end
    return rawget(self, idx) or ""
  end
...
function window_info:reset()
  _ejoysdk.log("trigger window_info reset")
...
local landscape_orientation

local function orientation_listener(res)
  landscape_orientation = res.value
  reset_window_info_listener()
end

wx_api.onDeviceOrientationChange(orientation_listener)
wx_api.onWindowResize(reset_window_info_listener)
...
function Sysinfo.cutout()
  ...
  local safe_inset
  local screen_width = window_info.screenWidth
  local screen_height = window_info.screenHeight
  if not window_info.safeArea.top or window_info.safeArea.top == "" then
    safe_inset = {
      top = 0,
      left = 0,
      bottom = 0,
    ...
    if screen_width < window_info.safeArea.width then
      _ejoysdk.log("window_info screen width height error, origin width: " .. tostring(screen_width) .. ", origin height: " .. tostring(screen_height))
      local temp_screen_width = screen_width
      screen_width = screen_height
      screen_height = temp_screen_width
      _ejoysdk.log("window_info screen width height error, final width: " .. tostring(screen_width) .. ", final height: " .. tostring(screen_height))
    end
    safe_inset = {
      top = math.floor(window_info.safeArea.top * window_info.pixelRatio),
      left = math.floor(window_info.safeArea.left * window_info.pixelRatio),
      bottom = math.floor((screen_height - window_info.safeArea.bottom) * window_info.pixelRatio),
      right = math.floor((screen_width - window_info.safeArea.right) * window_info.pixelRatio)
    }
  end
  if screen_width > screen_height and (safe_inset.left > safe_inset.right and "landscapeReverse" == landscape_orientation or safe_inset.left < safe_inset.right and "landscape" == landscape_orientation) then
    _ejoysdk.log("safe_inset reverse, origin safe_inset: ")
    _ejoysdk.js_log(safe_inset)
    _ejoysdk.log("safe_inset reverse, landscape_orientation: " .. tostring(landscape_orientation))
    local safe_inset_reverse = {
      top = safe_inset.bottom,
      bottom = safe_inset.top,
      left = safe_inset.right,
      right = safe_inset.left
    }
    safe_inset = safe_inset_reverse
  end
...
function Sysinfo.package_name()
  return "weixin_" .. tostring(app_base_info.appId)
end
...
function Sysinfo.os_version()
  return app_base_info.wx_version
end

function Sysinfo.os_sdk_version()
  return app_base_info.WXSDKVersion
end
...
local update_time_storage_key = app_base_info.appId .. "update_time"
...
function Sysinfo.app_version_name()
  return tostring(account_info.miniProgram.envVersion) .. "-" .. tostring(account_info.miniProgram.version)
end

function Sysinfo.get_cpu_model()
  return device_info.cpuType or ""
end
...
function Sysinfo.get_storage_info()
  return {internal_total_storage_size = 209715200}
end

function Sysinfo.screen()
  return {
    width = window_info.windowWidth,
    hight = window_info.windowHeight
  }
end
...
function Sysinfo.get_device_system()
  local system = device_info.system
  local brand = device_info.brand
  local os = "unknown"
...
function Sysinfo.memory_detail(cb)
  if device_info.memorySize then
    cb({
      total = device_info.memorySize * 1024 * 1024
    })
    return
  end
  cb({})
...
function Sysinfo.is_simulator()
  return device_info.brand == "devtools"
end

local sys_battery

local function get_sys_battery_async()
  wx_api.getBatteryInfo({
    success = function(res)
      sys_battery = {
        level = math.floor(res.level),
        scale = 100,
        status = res.isCharging and 2 or 1
      }
    end
  })
end

get_sys_battery_async()
...
local Timer = {}
M.Timer = Timer
local wx_set_timeout = _ejoysdk.setTimeout

function Timer.once(interval, callback)
  wx_set_timeout(callback, interval * 1000)
end
...
local user_dir_cache = _ejoysdk.wx.env.USER_DATA_PATH
local File = {}
M.File = File
...
function File.writefile_fullpath(filepath, filedata, append, is_b64)
  ...
  local fd = wx_fs.openSync({
    filePath = filepath,
    flag = append and "a+" or "w+"
  })
  js_log("文件创建成功")
  local ret = wx_fs.writeSync({fd = fd, data = filedata})
  wx_fs.close({fd = fd})
  return ret.bytesWritten and ret.bytesWritten == #filedata, filepath
end
...
function File.readfile(filename, from_bundle)
  ...
  if from_bundle then
    ...
    local ret, data = pcall(wx_fs.readFileSync, path, "utf-8")
    return ret and data or nil
  end
end
...
function File.readfile_fullpath(filename)
  ...
  local ret = wx_fs.readFileSync(filename, "utf-8")
  return ret
end

function File.process_exists(path)
  local ret, e = pcall(wx_fs.accessSync, path)
  js_log(ret, e)
  return ret
end
...
function File.zip(src_file_path, file_name, dst_path, cb)
...
function File.size(src_path, cb)
...
function File.process_make_dirs(dir)
  ...
  local ret, e = pcall(wx_fs.mkdirSync, dir, true)
  if not ret then
    js_log(e)
  end
  return ret
...
local function get_cache_from_file(key)
  local succ = pcall(wx_fs.accessSync, kv_cache_file_path)
  local val
  if not succ then
    return nil
  end
  local content = wx_fs.readFileSync(kv_cache_file_path, "utf8")
...
function KeyStore.get(key)
  local value = wx_api.getStorageSync(key)
  if "" == value then
    local res = wx_api.getStorageInfoSync()
...
local function set_cache_into_file(key, value)
  local succ = pcall(wx_fs.accessSync, kv_cache_dir)
  if not succ then
    wx_fs.mkdirSync(kv_cache_dir, true)
  end
  local is_file_exist = pcall(wx_fs.accessSync, kv_cache_file_path)
  local kv_store = {}
  ...
  wx_fs.writeFileSync(kv_cache_file_path, json_content, "utf8")
end
...
function KeyStore.set(key, value, _apply)
  ...
  local succ, err = pcall(wx_api.setStorageSync, key, value)
  if not succ then
    js_log("wx_api.setStorageSync failed: " .. tostring(err))
    set_cache_into_file(key, value)
  end
end
...
local function clear_cache_file()
  pcall(wx_fs.unlinkSync, kv_cache_file_path)
end

function KeyStore.clear()
  wx_api.clearStorageSync()
  clear_cache_file()
end
...
function Modal.open(title, option, cb)
  option = option or {}
  local open_success_cb = setmetatable({}, {
    __name = "js_function",
    __sync = true,
    __call = function(res)
...
function M.save_to_album(_path, _need_delete, _cb)
  _ejoysdk.wx.saveImageToPhotosAlbum({
    filePath = _path,
    complete = function(res)
      if _need_delete then
        js_log("delete file " .. _path)
        wx_fs.unlink({filePath = _path})
      end
      _cb(res)
    end
  })
...
function M.vibrate(milliseconds)
  if not milliseconds or -1 == milliseconds or milliseconds <= 15 then
    wx_api.vibrateShort({
      type = "heavy",
      success = function(res)
        _ejoysdk.log("weixin vibrateShort success")
      end,
      fail = function(res)
        _ejoysdk.log("weixin vibrateShort fail")
      end
    })
    return
  end
  wx_api.vibrateLong({
    success = function(res)
      _ejoysdk.log("weixin vibrateLong success")
    end,
    fail = function(res)
      _ejoysdk.log("weixin vibrateLong fail")
    end
  })
end
...
function M.set_app_orientation(orientation)
  ...
  _ejoysdk.wx.setDeviceOrientation({
    value = val,
    complete = function(res)
      js_log(res)
    end
...
function Permission.openSetting(options)
  options = options or {}
  _ejoysdk.wx.openSetting({
    withSubscriptions = options.withSubscriptions or false,
...
function M.open_privacy(params)
  _ejoysdk.js_log("open_privacy")
  _ejoysdk.wx.openPrivacyContract(params)
end

function M.require_privacy(params)
  _ejoysdk.js_log("require_privacy")
  _ejoysdk.wx.requirePrivacyAuthorize(params)
end
...
function M.subscribe_msg(options, cb)
  ...
  wx_api.requestSubscribeMessage({
    tmplIds = tmplIds,
    success = function(res)
...
function WebView.open(_url, _injection, _option, _on_js_callback, on_close_callback)
  _ejoysdk.log("wx can not open WebView, biz url:" .. tostring(_url))
...
function WebSocket:_init(params)
  self.params = params
  _ejoysdk.log("create websocket to " .. params.url)
  self.ws_task = _ejoysdk.wx.connectSocket({
    url = params.url,
  ...
  if params.onopen then
    self.ws_task:onOpen(params.onopen)
  end
...
function WebSocket:close()
  if self:get_state() == WebSocket.OPEN then
    ...
    self.ws_task:close({code = 1000, reason = "close"})
  else
