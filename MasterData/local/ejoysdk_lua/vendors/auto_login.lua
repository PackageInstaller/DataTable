...
local function try_pgs_autologin(cb)
  ...
  if EV.has_vendor(login_type) and not need_channel_login() then
    local pga_login = EV.get(login_type) or {}
    if pga_login.get_auth_code then
      pga_login.get_auth_code(function(succ, ...)
        if succ then
          AL.login(login_type, {login_from = "local", register_disabled = true}, cb)
        else
          cb(nil)
        end
      end)
...
local function try_auto_login(cb)
  ...
  if not auto_login_mark:get() then
    ...
    try_pgs_autologin(function(result)
      if result and result:is_succ() then
        cb(result)
      else
