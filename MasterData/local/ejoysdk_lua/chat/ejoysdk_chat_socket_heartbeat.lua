...
function M:_init(chat_socket, handlers, options)
...
function M:_heartbeat_action()
  ...
  function action()
    ...
    if not self.CHAT_SOCKET or self.CHAT_SOCKET:get_status() ~= self.CHAT_SOCKET.CHAT_CONNECTED then
      self.heartbeat_timer_running = false
      return -1
    end
