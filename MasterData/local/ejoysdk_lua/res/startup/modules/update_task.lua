...
function M:_confirm_download(update_ver, _checked_update_info, cb)
  ...
  function state_listener(namespace, rk, state, state_obj)
    EL.LOG.debug(TAG, "res update change, ns:" .. tostring(ns) .. ", res_key:" .. tostring(_res_key) .. "state=" .. tostring(state))
  ...
  function progress_listener(namespace, rk, progress_info)
...
function M:_init_task()
  ...
  local local_infos = SC.get_startup_local_res_infos() or {}
...
function M:cancel_update(cb)
  if self._data.update_state.state == _STATES.DOWNLOADING then
    ...
    cb(false, CC.RESOURCE_UPDATE_ERROR_CODES.RES_UPDATE_CANCEL_FAILED, "not in downloading")
  end
end
