...
function M.on_res_apply(engine_context, res_location, apply_dest_path, cb, ori_progress_cb)
  ...
  E.LOG.debug(TAG, "on_res_apply res_location is >> " .. tostring(res_location) .. ", apply_dest_path is >> " .. tostring(apply_dest_path))
  ...
  local function md5_check(md5_check_files, md5_check_target_info_map, md5_copy_map, md5_check_cb)
    ...
    function get_md5_fun()
      ...
      E.File.batch_md5(md5_check_file_paths, function(get_md5_succ, ...)
        if get_md5_succ then
          ...
          for key, value in pairs(md5_data) do
            ...
            if value ~= file_info.md5 then
              ...
              E.LOG.debug(TAG, "check patch file fail, file is " .. tostring(file_info.to) .. ", md5 is " .. tostring(value) .. ", expect md5 is " .. tostring(file_info.md5))
          ...
          E.LOG.debug(TAG, "batch_md5 fail, code is " .. tostring(md5_err_code) .. ", msg is " .. tostring(md5_err_msg))
          ...
          for key, get_md5_err_msg in pairs(md5_err_data) do
            ...
            table.insert(md5_error_files, file_info.to)
            E.LOG.debug("check patch file fail, file is " .. tostring(file_info.to) .. ", get md5 fail, msg is " .. tostring(get_md5_err_msg))
            if not md5_error_msg then
        ...
        E.LOG.debug(TAG, "md5 check last index is " .. tostring(md5_check_last_index) .. ", total count is " .. tostring(md5_check_total_count))
    ...
    if diff_file_path then
      ...
      E.patch_dir(patch_params, function(succ, ...)
        ...
        if succ then
          ...
          E.LOG.debug(TAG, "patch fail, error code is " .. tostring(patch_err_code) .. ", try download origin file")
              ...
              batch_delete_files(delete_files_array, function(batch_delete_succ, ...)
                ...
                update_using_file_list_cache(res_key, apply_dest_path, function()
                  ...
                  if remove_temp_dir_succ then
                    ...
                    E.LOG.debug(TAG, "remove temp dir fail, code is " .. tostring(remove_temp_err_code) .. ", msg is " .. tostring(remove_temp_err_msg))
                  end
                  cb(true, true)
                end)
              end)
