local var_0_0 = class("DrawCardGameManager.SettlementState", (import("...StateBase")))
...
local function var_0_2(arg_3_0, arg_3_1, arg_3_2)
	...
	local var_3_2 = string.format("%s_%d%02d", var_3_0, arg_3_0:IsLittleChara() and 1 or 2, var_3_1)

	arg_3_0:DoAction(var_3_2, arg_3_2)
	arg_3_0:PlayTalk(var_3_0, var_3_2)
end
...
local function var_0_3(arg_4_0, arg_4_1)
	return deferred.new(function(arg_5_0)
		...
		if var_5_1 then
			var_0_1(arg_4_0, var_5_2, var_5_3)
		else
			var_0_2(arg_4_0, arg_4_1, var_5_3)
		end
	end)
end
...
function var_0_0:Enter(arg_8_1, arg_8_2)
	local Include = require("manager.posterGirl.tgame.DrawCardGame.States.Include")

	var_0_0.super.Enter(self, arg_8_1)
	...
	deferred.all({
		...
		if var_11_0 then
			arg_8_1:GoToState(Include.ExitState, var_11_1)
		elseif arg_8_1.gameData.win == arg_8_1.totalRound - 1 and arg_8_1.gameData.matchPoint == nil then
			arg_8_1:GoToState(Include.MatchPointPerformanceState)
		else
			arg_8_1:GoToState(Include.DrawCardState)
		end
...
function var_0_0:Exit(arg_13_1)
	self:StopDragSoundFX()

	self.progress = 1
end

function var_0_0:Wait(arg_14_1, arg_14_2)
	if not arg_14_2 or arg_14_1.gameData.win <= 1 then
		return nil
	end

	if self.waitInput == nil then
		self.waitInput = deferred.new()
	end
...
local var_0_6 = 2.966667

function var_0_0:OnDragFinish(arg_15_1)
	local var_15_0 = manager.posterGirl.actor:GetHeroTimelineMgr():_GetTimeline().actionName_

	if var_0_5[var_15_0] then
		manager.posterGirl.actor:GetHeroTimelineMgr():JumpToTime(var_0_5[var_15_0], true)
		manager.posterGirl.actor:GetHeroTimelineMgr():JumpToTime(var_0_5[var_15_0] - var_0_6)
	end
	...
	if self.waitInput then
		self.waitInput:resolve()

		self.waitInput = nil
	end
end

var_0_0.DRAG_PLAY_SOUND_FX = 0.2
var_0_0.DRAG_STOP_SOUND_FX = 0.15

function var_0_0:OnDragInProgress(arg_16_1, arg_16_2)
	if manager.posterGirl.actor:GetHeroTimelineMgr():_GetTimeline().actionName_ ~= "game_chair_lose_102" then
		return
	end

	local var_16_0 = math.abs(arg_16_2 - self.progress) / Time.deltaTime

	if var_16_0 > var_0_0.DRAG_PLAY_SOUND_FX then
		self:StartDragSoundFX()
	elseif var_16_0 < var_0_0.DRAG_STOP_SOUND_FX then
		self:StopDragSoundFX()
	end

	self.progress = arg_16_2
end

function var_0_0:OnDragEnd(arg_17_1)
	self:StopDragSoundFX()
end

function var_0_0:StartDragSoundFX()
	if not self.playDragFx then
		manager.audio:PlayEffect("ui_scene_102003", "ui_scene_102003_chair_game_lose_102_loop")

		self.playDragFx = true
	end
end

function var_0_0:StopDragSoundFX()
	if self.playDragFx then
		manager.audio:PlayEffect("ui_scene_102003", "ui_scene_102003_chair_game_lose_102_loop_stop")

		self.playDragFx = false
	end
end

var_0_0.events = {
	ON_DRAG_FINISH = var_0_0.OnDragFinish,
	ON_DRAG_IN_PROGRESS = var_0_0.OnDragInProgress,
	ON_DRAG_END = var_0_0.OnDragEnd
}
