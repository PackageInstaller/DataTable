return {
	Enter = function(self, arg_1_1, arg_1_2)
		...
		if not var_1_0 or not var_1_0.isLoaded then
			self.operation_ = Asset.LoadLevelAsync("Levels/PinballGame", true)
	...
	UnLoadScene = function(self)
		...
		if var_3_0 and var_3_0.isLoaded == true then
			whereami = "home"
			self.exoperation_ = SceneManager.UnloadSceneAsync("PinballGame")

			if self.extimer_ then
				self.extimer_:Stop()

				self.extimer_ = nil
			end

			self.extimer_ = FrameTimer.New(handler(self, self.exProcess), 1, -1)

			self.extimer_:Start()
		end
	end,
	exProcess = function(self)
		if not self.exoperation_ or self.exoperation_.isDone then
	...
	CameraAdaptive = function(arg_8_0)
		UnityEngine.Camera.main.fieldOfView = Screen.width / Screen.height < 1.7 and 1.7 / (Screen.width / Screen.height) * CameraCfg.pinball.fieldOfView or CameraCfg.pinball.fieldOfView
	end,
	...
	UpgradeStageParamsFromCfg = function(self)
		local var_12_0 = PinballWaveCfg[self.stageId]
		local var_12_1

		if PinballAction.PlayingStageID == PinballData:GetChallengeStageID() then
			var_12_1 = PinballWaveCfg[PinballAction.ChallengeWaveList[table.indexof(PinballAction.ChallengeBoardList, self.stageId)]]
		end

		if not var_12_0 then
			if PinballAction.PlayingStageID == PinballData:GetChallengeStageID() then
				var_12_0 = PinballWaveCfg[PinballAction.ChallengeWaveList[table.indexof(PinballAction.ChallengeBoardList, self.stageId)]]
			end

			if not var_12_0 then
				Debug.LogError("弹珠玩法存在未配置的波次ID" .. self.stageId)

				return
			end
		end

		self.debateStageDamageTrigger = var_12_0.debate_start
		self.overflowDamageTransFac = var_12_0.damage_overflows
		self.baseSpareTimes = var_12_0.split_time
		self.debateRoundLimit = var_12_0.debate_wellen_time
		self.debateFinishCount = var_12_0.debate_quantity
		self.debateDescription = GetI18NText(var_12_0.debate_desc)
		self.debateSuccessDesc = GetI18NText(var_12_0.debate_success_desc)
		self.baseHighValueWordValue = var_12_0.text_fraction
		self.baseremainHp = var_12_0.brick_hp
		self.baseExplosionLength = var_12_0.explosion_scope
		self.baseExitPortalCD = var_12_0.portal_cd
		self.supplementTriggerNumber = var_12_0.Item_replenishment_time
		self.supplementMaxNumber = var_12_0.Item_replenishment_max
		self.bubbleIDList = var_12_0.chat_id
		...
		if PinballAction.PlayingStageID == PinballData:GetChallengeStageID() and var_12_1 and type(var_12_1.challenge_debate_wellen_id) == "table" and #var_12_1.challenge_debate_wellen_id > 0 then
			...
			self.debateStageDamageTrigger = var_12_1.debate_start
		else
			self.isBossStage = PinballData:IsBossWave(self.stageId)
			var_12_2 = var_12_0.debate_wellen_id
		end

		if var_12_2 and var_12_2 ~= 0 then
			self.debateStageData = Asset.Load("PinballGame/StageData/PinBallGameStage_" .. var_12_2)
			self.hasDebateStage = true
		end
	end,
	CheckBubbleTigger = function(arg_13_0, arg_13_1, arg_13_2)
		...
		if not PinballWaveCfg[arg_13_0] then
			if PinballAction.PlayingStageID == PinballData:GetChallengeStageID() then
		...
		for iter_13_0, iter_13_1 in ipairs(var_13_2.chat_id) do
			local var_13_3 = PinballBubbleCfg[iter_13_1]

			if PinballBubbleCfg[iter_13_1] and var_13_3.chat_type == arg_13_1 then
				var_13_0 = var_13_3.chat_type == 1 and arg_13_2 + 1 == var_13_3.params[1] and true or true
	...
	PlayTimelineWithCallBack = function(arg_14_0, arg_14_1)
		...
		if var_14_0 then
			...
			if var_14_1 and var_14_1.playableAsset then
				TimelineTools.PlayTimelineWithCallback(var_14_1, var_14_1.playableAsset, arg_14_1)
			end
		end
	end,
	...
	PlayBgChangeAniWithTarget = function(arg_16_0, arg_16_1)
		...
		if var_16_1 and arg_16_1 then
			...
			if var_16_0.childCount > 0 then
				...
				oldNext = var_16_2:Find("root/" .. "next"):GetComponent("SpriteRenderer")
				var_16_4 = var_16_2:Find("root/particle/next_particle_out")
			end

			local var_16_7 = GameObject.Instantiate(Asset.Load(var_16_1), var_16_0)

			var_16_7.name = "PinballHomeBg"

			local var_16_8 = var_16_7.transform:Find("root/particle/pre_particle_out")

			var_16_7.transform:Find("root/pre"):GetComponent("SpriteRenderer").sprite = oldNext.sprite

			var_16_4:SetParent(var_16_8.parent.transform)

			var_16_4.name = "pre_particle_out"

			var_16_4.gameObject:SetActive(true)

			var_16_4.localPosition = Vector3.New(var_16_8.localPosition.x, var_16_8.localPosition.y, var_16_8.localPosition.z)
			var_16_4.localScale = Vector3.New(var_16_8.localScale.x, var_16_8.localScale.y, var_16_8.localScale.z)

			GameObject.DestroyImmediate(var_16_8.gameObject)

			local var_16_9 = GameObject.Find("HomeBackParticlePos").transform
			local var_16_10 = var_16_7.transform:Find("root/particle")

			var_16_10.localPosition = Vector3.New(var_16_9.position.x, var_16_9.position.y, var_16_9.position.z)
			var_16_10.localScale = Vector3.New(var_16_9.localScale.x, var_16_9.localScale.y, var_16_9.localScale.z)
	...
	ShowBackGroundMask = function(arg_21_0)
		...
		if var_21_0 then
			var_21_0:GetComponent("SpriteRenderer").enabled = not not arg_21_0
		end
	end,
	GetOverFlowDamageScore = function()
		if GameSetting.pinball_game_overflow_damage_to_score.value then
			return GameSetting.pinball_game_overflow_damage_to_score.value
		end
