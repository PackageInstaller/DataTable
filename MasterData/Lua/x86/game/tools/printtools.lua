...
function watch(arg_4_0, arg_4_1, arg_4_2)
	...
	elseif type(arg_4_0) == "table" and type(arg_4_0.is_clone) == "boolean" then
		...
		var_4_0 = is_string(var_4_2) and (arg_4_0.destructed == true and string.format("object(%s<destructed>)", var_4_2) or string.format("object(%s)", var_4_2)) or string.format("object(%s)", save_string(arg_4_0.class_type))
	elseif type(arg_4_0) == "table" then
		if string.len(arg_4_1) > 20 then
			var_4_0 = "<table overflow>"
		else
			local var_4_3 = {
				string.format("<table>   size : %d\r\n%s{\r\n", sizeof(arg_4_0), arg_4_1)
			}
			local var_4_4 = 1
			...
			for iter_4_0, iter_4_1 in pairs(arg_4_0) do
				...
				if type(iter_4_0) == "string" and type(iter_4_1) == "table" then
					if string.len(iter_4_0) > 0 and string.sub(iter_4_0, 1, 1) == "_" then
						table.insert(var_4_3, string.format("%s\t%s: <table hide>,\r\n", arg_4_1, watch(iter_4_0, arg_4_1 .. "\t", arg_4_2 + 1)))
				...
				if var_4_5 then
					table.insert(var_4_3, string.format("%s\t%s:%s,\r\n", arg_4_1, watch(iter_4_0, arg_4_1 .. "\t", arg_4_2 + 1), watch(iter_4_1, arg_4_1 .. "\t", arg_4_2 + 1)))
				end

				var_4_4 = var_4_4 + 1

				if var_4_4 > 100 then
					table.insert(var_4_3, "... ...(table too long)")

					break
				end
			end

			table.insert(var_4_3, string.format("%s}", arg_4_1))

			var_4_0 = table.concat(var_4_3, "")
		end
	else
		var_4_0 = type(arg_4_0) == "function" and "<function>" or type(arg_4_0) == "string" and string.format("\"%s\"", string.gsub(arg_4_0, "\"", "\\\"")) or type(arg_4_0) == "number" and tostring(arg_4_0) or type(arg_4_0) == "boolean" and tostring(arg_4_0) or string.format("<%s type>", "unknow")
	end
