local var_0_0 = {
	GotoMainlineView = function()
		if getData("activity_hodur", "first_enter_home") ~= true then
			...
			manager.story:StartStoryById(GameSetting.hodur_home_story.value[1], function()
				JumpTools.OpenPageByJump("/hodurHomeView")
			end)
	...
	GoBattle = function(arg_4_0, arg_4_1, arg_4_2)
		...
		gameContext:Go("/sectionSelectHero", {
			...
			reserveParams = ReserveParams.New(ReserveTools.GetReserveType(BattleConst.STAGE_TYPE_NEW.HODUR_BATTLE, arg_4_0), ReserveTools.GetContID(BattleConst.STAGE_TYPE_NEW.HODUR_BATTLE, arg_4_0), nil, {
...
function var_0_0.GotoChallengeBattle(arg_5_0, arg_5_1, arg_5_2)
	...
	local var_5_0 = ActivityHodurChapterCfg[arg_5_0].stage_list[var_0_0.GetChallengeCurIndex()][1]
	local var_5_1 = {}

	for iter_5_0, iter_5_1 in ipairs(ActivityHodurChapterCfg[arg_5_0].stage_list) do
		table.insert(var_5_1, iter_5_1[1])
	end
	...
	gameContext:Go("/hodurChallengeSelectHeroBaseView", {
		...
		chapterID = arg_5_0,
		heroDataType = HeroConst.HERO_DATA_TYPE.HODURGAME,
		reserveParams = ReserveParams.New(ReserveTools.GetReserveType(BattleConst.STAGE_TYPE_NEW.HODUR_BATTLE, var_5_0), ReserveTools.GetContID(BattleConst.STAGE_TYPE_NEW.HODUR_BATTLE, var_5_0), nil, {
			stageType = BattleConst.STAGE_TYPE_NEW.HODUR_BATTLE,
			stageID = var_5_0,
...
function var_0_0.GetStagePass(arg_8_0, arg_8_1)
	...
	if table.indexof(var_8_0.cur_complete_stage_list, arg_8_0) then
		return true
	end
...
function var_0_0.GetStageUnlock(arg_9_0, arg_9_1)
	...
	if ActivityHodurCfg[arg_9_0].previous_stage == 0 then
		var_9_0 = true
	end

	if var_0_0.GetStagePass(ActivityHodurCfg[arg_9_0].previous_stage, arg_9_1) then
		var_9_0 = true
	end

	if ActivityHodurCfg[arg_9_0].event_lock == 0 then
		var_9_1 = true
	end

	if var_0_0.GetEventComplete(ActivityHodurCfg[arg_9_0].event_lock, arg_9_1) then
		var_9_1 = true
	end
...
function var_0_0.GetEventUnlock(arg_10_0, arg_10_1)
	if ActivityHodurEventCfg[arg_10_0].condition == 0 then
		return true
	end

	if var_0_0.GetStagePass(ActivityHodurEventCfg[arg_10_0].condition, arg_10_1) then
		return true
	end
...
function var_0_0.GetChapterState(arg_13_0)
	if var_0_0.IsChallenge(arg_13_0) then
		if ActivityHodurChapterCfg[arg_13_0].previous_chapter == 0 or var_0_0.GetChapterState(ActivityHodurChapterCfg[arg_13_0].previous_chapter) == HodurConst.COMMON_STATE.PASS then
			return HodurConst.COMMON_STATE.UNLOCK
		else
			return HodurConst.COMMON_STATE.LOCK
		end
	else
		local var_13_0 = HodurSystemData:GetMainlineChapterData(arg_13_0)
		...
		if ActivityHodurChapterCfg[arg_13_0].previous_chapter == 0 or var_0_0.GetChapterState(ActivityHodurChapterCfg[arg_13_0].previous_chapter) == HodurConst.COMMON_STATE.PASS then
			...
			local var_13_1, var_13_2 = var_13_0:GetPassNum()

			if var_13_1 >= #ActivityHodurChapterCfg[arg_13_0].stage_list then
...
function var_0_0.GetChallengeAffix()
	...
	for iter_24_0, iter_24_1 in ipairs((HodurSystemData:GetAffixCache())) do
...
function var_0_0.CheckSpecialAffix()
	for iter_25_0, iter_25_1 in ipairs((var_0_0.GetChallengeAffix())) do
...
function var_0_0.GetChallengeComplete()
	...
	return #(var_30_0.complete_stage_list or {}) >= #(ActivityHodurChapterCfg[var_30_0.chapter_id].stage_list or {})
end
...
function var_0_0.GetChapterIDByStageID(arg_33_0)
	for iter_33_0, iter_33_1 in ipairs(ActivityHodurChapterCfg.all) do
		for iter_33_2, iter_33_3 in ipairs(ActivityHodurChapterCfg[iter_33_1].stage_list) do
