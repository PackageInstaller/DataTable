local var_0_0 = {
	ChangeDormSpeicalFur = function(arg_1_0, arg_1_1)
		local var_1_0 = DormRoomTools:GetRoomSceneType((DormData:GetCurrectSceneID()))

		if BackHomeFurniture[arg_1_1].type == DormConst.FLOOR_TYPE_NUM then
			local var_1_1

			if BackHomeFurniture[arg_1_1].replace and #BackHomeFurniture[arg_1_1].replace > 0 then
				for iter_1_0 = 1, #BackHomeFurniture[arg_1_1].scene_id do
					if var_1_0 == BackHomeFurniture[arg_1_1].scene_id[iter_1_0] then
						var_1_1 = BackHomeFurniture[arg_1_1].replace[iter_1_0]

						break
					end
				end
			else
				var_1_1 = BackHomeFurniture[arg_1_1].model
			end

			if var_1_1 then
				DormLuaBridge.SetRoomThemeFloor(var_1_1, BackHomeFurniture[arg_1_1].extend)
			end
		elseif BackHomeFurniture[arg_1_1].type == DormConst.WALL_TYPE_NUM then
			DormLuaBridge.SetRoomThemeWall(BackHomeFurniture[arg_1_1].material, BackHomeFurniture[arg_1_1].mesh)
		elseif BackHomeFurniture[arg_1_1].type == DormConst.BACK_GROUND_TYPE then
			DormLuaBridge.SetRoomThemeBG(BackHomeFurniture[arg_1_1].extend, BackHomeFurniture[arg_1_1].material)
		end
	end,
	ChangeDormFloorOrWallData = function(self, arg_2_1)
		if arg_2_1.wallID then
			self:ChangeDormSpeicalFur(arg_2_1.wallID)
		end

		if arg_2_1.floorID then
			self:ChangeDormSpeicalFur(arg_2_1.floorID)
		end

		if arg_2_1.backGroundID then
			self:ChangeDormSpeicalFur(arg_2_1.backGroundID)
		end
	end
}
...
function var_0_0.AddSpecialFurInfo(arg_3_0, arg_3_1, arg_3_2, arg_3_3, arg_3_4)
	...
	if arg_3_4 then
		...
		arg_3_1.specialFur.floorID = arg_3_1.specialFur.floorID or var_0_0:GetOriFloorOrWall(arg_3_2, DormConst.FLOOR_TYPE_NUM)
		arg_3_1.specialFur.wallID = arg_3_1.specialFur.wallID or var_0_0:GetOriFloorOrWall(arg_3_2, DormConst.WALL_TYPE_NUM)
		arg_3_1.specialFur.backGroundID = arg_3_1.specialFur.backGroundID or var_0_0:GetOriFloorOrWall(arg_3_2, DormConst.BACK_GROUND_TYPE)
	end
end
