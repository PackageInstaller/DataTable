return {
	...
	GetHeroStandardID = function(arg_4_0)
		if not ActivitySkinTrialCfg[arg_4_0] then
			Debug.Log(string.format("<color=ff0000>ActivitySkinTrialCfg has no id(%s)</color>, return default standardID(%s)", arg_4_0, HeroStandardSystemCfg.all[1]))

			return HeroStandardSystemCfg.all[1]
		end

		local var_4_0 = BattleSkinTrialStageCfg[ActivitySkinTrialCfg[arg_4_0].stage_id]

		if not BattleSkinTrialStageCfg[ActivitySkinTrialCfg[arg_4_0].stage_id] then
			Debug.Log(string.format("<color=ff0000>BattleSkinTrialStageCfg has no id(%s)</color>, return default standardID(%s)", ActivitySkinTrialCfg[arg_4_0].stage_id, HeroStandardSystemCfg.all[1]))

			return HeroStandardSystemCfg.all[1]
		end

		return var_4_0.hero_list[1][2]
	end
}
