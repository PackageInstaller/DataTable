...
function var_0_0.IsCanSign()
	...
	for iter_1_0, iter_1_1 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.SEVEN_SIGN] or {}) do
...
function var_0_0.GetDailySignIndex()
	local var_2_0 = SignData:GetDailySignInfo()

	if not var_2_0 then
		return 0
	end

	if not table.keyof(var_2_0.days, (manager.time:GetDeltaToday())) then
		return table.nums(var_2_0.days) + 1
	end
...
function var_0_0.GetSevenDaySignIndex(arg_3_0)
	...
	local var_3_0 = SignData:GetSevenDaySignInfo(arg_3_0)

	if not var_3_0 or not ActivityData:GetActivityData(arg_3_0) then
		return 0
	end

	local var_3_1 = ActivityCumulativeSignCfg[arg_3_0]

	if ActivityCumulativeSignCfg[arg_3_0] == nil then
		return 0
	end

	if PlayerData:GetPlayerInfo().userLevel < var_3_1.limit_level then
		return 0
	end

	if var_3_0.signCount >= #var_3_1.config_list then
		return 0
	end

	if manager.time:GetTodayFreshTime() > var_3_0.lastSignTime then
		return var_3_0.signCount + 1
	end
...
function var_0_0.GetDailySignState(arg_4_0)
	...
	if arg_4_0 <= #var_4_1 then
		return ActivityConst.SIGN_STATE.GETTED
	end

	if var_4_2 < var_4_0 and #var_4_1 + 1 == arg_4_0 then
		return ActivityConst.SIGN_STATE.CAN_GET
	end

	if var_4_2 < var_4_0 and #var_4_1 + 2 == arg_4_0 or var_4_2 == var_4_0 and #var_4_1 + 1 == arg_4_0 then
