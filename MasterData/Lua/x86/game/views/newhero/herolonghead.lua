	...
	if self.heroDataType_ == HeroConst.HERO_DATA_TYPE.DEFAULT then
		if ArchiveData:GetArchive(HeroRecordCfg[HeroRecordCfg.get_id_list_by_hero_id[self.heroInfo_.id][1]].id).lv > 1 then
			...
			if arg_10_1 then
				...
				var_10_0 = ArchiveData:GetArchive(HeroRecordCfg[HeroRecordCfg.get_id_list_by_hero_id[self.heroInfo_.id][1]].id).lv > 1
			end
		end

		self.favorabilityController_:SetSelectedState(tostring(var_10_0))
	end
end
