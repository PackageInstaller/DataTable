...
function NewUserAndPlayerInfoView:RefreshUserData()
	...
	if not JumpTools.IsSystemLocked(ViewConst.SYSTEM_ID.DORM) then
		...
		elseif HeroRecordCfg.get_id_list_by_hero_id[DormitoryData:GetDormSceneData(self.dormID_).archiveIDList[1]][1] then
			local var_45_3 = GetI18NText(HeroRecordCfg[HeroRecordCfg.get_id_list_by_hero_id[DormitoryData:GetDormSceneData(self.dormID_).archiveIDList[1]][1]].name)

			if var_45_3 then
				self.dormName_ = string.format(GetTips("DORM_HERO_ROOM_NAME"), var_45_3)
			end
		end
	end
