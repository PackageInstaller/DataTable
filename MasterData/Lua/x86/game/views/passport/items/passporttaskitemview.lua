...
function PassportTaskItemView:AddUIListener()
	...
	self:AddBtnListener(self.getBtn_, nil, function()
		...
		if (var_12_1 and tonumber(var_12_1) > manager.time:GetServerTime() or nil) and false and (self.taskCfg_.type == 7 or self.taskCfg_.type == 8) then
			if PassportData:GetExpWeekly() >= GameSetting.battlepass_exp_limit_weekly.value[1] then
				ShowMessageBox({
