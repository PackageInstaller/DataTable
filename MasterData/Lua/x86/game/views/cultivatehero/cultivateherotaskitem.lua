...
function CultivateHeroTaskItem:RefreshDesc()
	...
	if HeroTools.GetHeroIsUnlock(CultivateHeroTaskCfg[self.taskID_].hero) then
		...
		elseif CultivateHeroData:GetHeroTaskInfoList(self.activityID_)[self.taskID_] and CultivateHeroData:GetHeroTaskInfoList(self.activityID_)[self.taskID_].progress then
			var_11_0 = CultivateHeroTaskCfg[self.taskID_].condition > CultivateHeroData:GetHeroTaskInfoList(self.activityID_)[self.taskID_].progress and CultivateHeroData:GetHeroTaskInfoList(self.activityID_)[self.taskID_].progress or CultivateHeroTaskCfg[self.taskID_].condition
		end
	end
