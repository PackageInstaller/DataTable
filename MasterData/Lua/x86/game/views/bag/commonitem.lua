local CommonItem = class("CommonItem", ReduxView)
...
function CommonItem:OnCtor(arg_1_1)
...
function CommonItem:Init()
	self:InitUI()
	self:AddUIListener()
	self:UpdateView()
end

function CommonItem:RefreshData(arg_3_1)
	self.info_ = arg_3_1
...
function CommonItem:InitUI()
	...
	self.ownedController_ = ControllerUtil.GetController(self.gameObject_.transform, "owned")
end

function CommonItem:AddUIListener()
...
function CommonItem:ResetTransform()
...
function CommonItem:SetScale(arg_11_1)
	self.transform_.localScale = arg_11_1
end

function CommonItem:SetSelectType(arg_12_1)
	self.manualSelectType_ = arg_12_1

	self.selectTypeController_:SetSelectedState(arg_12_1)
end

function CommonItem:UpdateType(arg_13_1)
...
function CommonItem:UpdateView()
...
function CommonItem:UpdateMaterialView()
	self:ShowLock(self.info_.locked == 1)

	if self.info_.num then
		self:SetBottomText(self.info_.num)
	end
end

function CommonItem:UpdateHeroPieceView()
	if self.info_.num then
		self:SetBottomText(self.info_.num)
	end
...
function CommonItem:UpdateWeaponServantView()
	if self.refineText_ then
		self.refineText_.text = not self.info_.isEquip and "" or GetTips("PROMOTE")
	end

	if self.info_.isAdd then
		self.servantStageText_.text = "<color=#FF9500>" .. tostring(self.info_.num) .. "</color>"
...
function CommonItem:UpdateEquipView()
	self:UpdateLevel(self.info_.level or self.info_.num or 1)
...
function CommonItem:UpdateHeroSkinView()
	self:ShowLock(self.info_.locked == 1)
	self:UpdateIcon()
	self:SetBottomText(self.info_.num or 1)
end

function CommonItem:UpdateIcon()
...
function CommonItem:ResizeIcon(arg_25_1)
...
function CommonItem:UpdateName()
	if ItemCfg[self.info_.id] then
		self.nameText_.text = ItemTools.getItemName(self.info_.id)
	end
end

function CommonItem:UpdateStar()
	local var_27_0 = 0
...
function CommonItem:ShowLock(arg_31_1)
	self.lockedController_:SetSelectedState(arg_31_1 and "true" or "false")
end

function CommonItem:ShowOwned(arg_32_1)
	if not self.ownedController_ then
		return
	end

	self.ownedController_:SetSelectedState(arg_32_1 or self.info_.owned and "true" or "false")
end

function CommonItem:ShowNew(arg_33_1)
	SetActive(self.new_, arg_33_1)
end

function CommonItem:ShowStar(arg_34_1)
	if arg_34_1 then
		self:UpdateStar()
	else
...
function CommonItem:SetBottomText(arg_35_1)
	self.num_.text = arg_35_1

	if self.boxImage_ ~= nil then
		self.boxImage_.enabled = #(arg_35_1 and tostring(arg_35_1) or "") > 0
	end
end

function CommonItem:SetBottomTextColor(arg_36_1)
	self.num_.color = arg_36_1
end

function CommonItem:SetIndex(arg_37_1)
	self.transform_:SetSiblingIndex(arg_37_1)
end

function CommonItem:ShowBlackBack(arg_38_1)
	return
end

function CommonItem:UpdateTime()
	local var_39_1 = ItemTools.GetItemExpiredTimeByInfo(self.info_)

	if var_39_1 == nil or var_39_1 == 0 then
...
function CommonItem:UpdateCamp(arg_40_1)
	if arg_40_1 == 0 then
		self.campController_:SetSelectedState("noCamp")
		SetActive(self.campPanelGo_, false)
	else
		self.campController_:SetSelectedState(tostring(arg_40_1))
...
function CommonItem:UpdateLevel(arg_41_1)
	self:SetBottomText(tostring(arg_41_1))
end

function CommonItem:ShowSelect(arg_42_1)
	type(arg_42_1)
	...
	if var_42_0 then
		...
		if ItemConst.ITEM_TYPE.EQUIP ~= ItemCfg[self.info_.id].type then
			self.selectNum_.text = "x" .. arg_42_1
		end
	else
...
function CommonItem:HideNum()
	self:SetBottomText("")
end

function CommonItem:Show(arg_44_1)
	SetActive(self.gameObject_, arg_44_1)
end

function CommonItem:ShowFloor(arg_45_1)
	self.floorController_:SetSelectedState(arg_45_1 or ItemConst.ITEM_FLOOR.LONG)
end

function CommonItem:SetGrey(arg_46_1)
	self.geryController_:SetSelectedState(arg_46_1)
end

function CommonItem:HideServantTxt()
	SetActive(self.severantGo_, false)
end

function CommonItem:RegistCallBack(arg_48_1)
	self.clickMode_ = CLICK_MODE.SINGLE
	self.clickFunc = arg_48_1

	SetActive(self.longClickBtn_, false)
end

function CommonItem:RegistLongCallBack(arg_49_1)
	self.clickMode_ = CLICK_MODE.LONG
	self.clickLongFunc = arg_49_1

	SetActive(self.longClickBtn_, true)
end

function CommonItem:RegistCutCallBack(arg_50_1)
	self.clickCutFunc = arg_50_1
end

function CommonItem:RegistGaryCallBack(arg_51_1)
	self.clickGaryFunc = arg_51_1
end

function CommonItem:GetItemInfo()
	return self.info_
end

function CommonItem:Dispose()
	CommonItem.super.Dispose(self)
