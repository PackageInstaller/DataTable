...
function var_0_0:Init(arg_1_1, arg_1_2)
	self.gameView_ = arg_1_1
	self.gameRootContainer_ = arg_1_2
	var_0_3 = var_0_3 or FrameTimer.New(handler(self, self.Update), 1, -1)
...
function var_0_0:RegisterEvents()
	manager.notify:RegistListener(GAME_FOCUS_CHANGE, handler(self, self.OnGameFocusChange))
end

function var_0_0:RemoveEvents()
	manager.notify:RemoveListener(GAME_FOCUS_CHANGE, handler(self, self.OnGameFocusChange))
end

function var_0_0:OnGameFocusChange(arg_5_1, arg_5_2)
	if arg_5_2 then
		self:Pause()
	end
end
...
function var_0_0:Continue()
	...
	if self.continueHandler_ then
		self.continueHandler_()
	end
...
function var_0_0:Update()
	...
	self.gameView_:Update(Time.deltaTime)

	if var_0_11 then
		return
	end

	self.gameView_:Update(var_19_0)
		...
		if var_0_15 ~= nil and #var_0_15 > 0 then
			for iter_19_2, iter_19_3 in ipairs(var_0_15) do
				if var_0_6 - var_19_1 <= 0 then
					...
					iter_19_3(var_0_5, 2, var_0_6 - var_19_1)
				end
			end
		end
	end

	if self.currentDisappearItem_ ~= nil then
		self.flySeconds_ = self.flySeconds_ + Time.deltaTime
		self.currentDisappearItem_.gameObject_.transform.localPosition = self.gameView_.gameObject_.transform:InverseTransformPoint({
			x = self.startPos_.x + self.flySeconds_ * (self.targetPos_.x - self.startPos_.x),
			y = self.startPos_.y + self.flySeconds_ * (self.targetPos_.y - self.startPos_.y),
			z = self.startPos_.z + self.flySeconds_ * (self.targetPos_.z - self.startPos_.z)
		})
		self.currentDisappearItem_.gameObject_.transform.localScale = Vector3(1 - 0.5 * self.flySeconds_, 1 - 0.5 * self.flySeconds_, 1 - 0.5 * self.flySeconds_)

		if self.flySeconds_ > 1 then
			self.flySeconds_ = 1

			self.gameView_:AppearBubble(self:GetTreasureLimitNum())

			if self.fishDisappearCallback_ ~= nil then
				self.fishDisappearCallback_(self.currentDisappearItem_)

				self.fishDisappearCallback_ = nil
				self.currentDisappearItem_ = nil
			end
		end
	end
end
...
function var_0_0:AddScore(arg_21_1)
	if var_0_5 == 1 then
		arg_21_1 = arg_21_1 * 2
	end
	...
	if self.onScoreAddHandler then
		self.onScoreAddHandler(arg_21_1)
	end

	if self.scoreChangeHandler_ ~= nil then
		self.scoreChangeHandler_()
	end
end

function var_0_0:PlayFishDisappear(arg_22_1, arg_22_2)
	if arg_22_1:GetData().fishId ~= 5 then
		arg_22_2(arg_22_1)
	else
		self.currentDisappearItem_ = arg_22_1
		arg_22_1.transform_.eulerAngles.z = 0
		arg_22_1.transform_.eulerAngles = arg_22_1.transform_.eulerAngles
		arg_22_1.transform_.pivot = Vector2(0.5, 0.5)

		self.currentDisappearItem_.gameObject_.transform:SetParent(self.gameView_.gameObject_.transform)

		self.startPos_ = self.currentDisappearItem_.gameObject_.transform:TransformPoint(0, 0, 0)
		self.targetPos_ = self.gameView_:GetBubbleTarget():TransformPoint(0, 0, 0)
		self.flySeconds_ = 0
		self.fishDisappearCallback_ = arg_22_2
	end
end

function var_0_0:ResetScore()
	var_0_4 = 0
...
function var_0_0:Reset()
	...
	self:RemoveEvents()
	self:RemoveAllFishes()
end

function var_0_0:Dispose()
	if self.currentDisappearItem_ then
		self.currentDisappearItem_:Dispose()
