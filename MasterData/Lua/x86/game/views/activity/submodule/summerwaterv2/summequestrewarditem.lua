local SummeQuestRewardItem = class("SummeQuestRewardItem", ReduxView)

function SummeQuestRewardItem:OnCtor(arg_1_1)
...
function SummeQuestRewardItem:Init()
	self:InitUI()
	self:AddUIListener()
	...
	self.greyController_ = self.showController_:GetController("grey")
	self.tagController_ = self.showController_:GetController("type")
	self.timeController_ = self.showController_:GetController("TimeTips")
end

function SummeQuestRewardItem:InitUI()
	self:BindCfgUI()
end

function SummeQuestRewardItem:AddUIListener()
	self:AddBtnListener(self.receiveBtn_, nil, function()
		TaskAction:SubmitTask(self.taskID_)
	end)
end

function SummeQuestRewardItem:IndexItem(arg_6_1, arg_6_2)
...
function SummeQuestRewardItem:SetData(arg_8_1)
...
function SummeQuestRewardItem:UpdateView()
	...
	self.greyController_:SetSelectedState(tostring(not var_9_0))

	if self.cfg_.type == TaskConst.TASK_TYPE.SUMMER_WATER_WEEKLY or self.cfg_.type == TaskConst.TASK_TYPE.SWIMSUIT_BATTLE_WEEKLY then
		self.tagController_:SetSelectedState("everyweek")
	elseif self.cfg_.type == TaskConst.TASK_TYPE.OSIRIS_TASK_DAILY then
		self.tagController_:SetSelectedState("everyday")
...
function SummeQuestRewardItem:AddTimer()
	if self.timer_ then
		self.timer_:Stop()
	...
	self.timeController_:SetSelectedState("close")
	...
	if var_10_0 then
		if var_10_0 == 0 then
			...
			self.timeController_:SetSelectedState("show")
...
function SummeQuestRewardItem:Dispose()
	...
	SummeQuestRewardItem.super.Dispose(self)
end

return SummeQuestRewardItem
