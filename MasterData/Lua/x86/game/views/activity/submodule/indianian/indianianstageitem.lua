...
function InidaNianStageItem:RefreshState()
	...
	local var_7_5 = IndiaNianData:GetCurrentHpByID(self.cfg_.id)

	self.selectController_:SetSelectedState(tostring(var_7_4 and not var_7_3 and not var_7_0))

	self.silderImg_.fillAmount = var_7_5 / self.cfg_.boss_hp
	self.percentText_.text = math.ceil(var_7_5 / self.cfg_.boss_hp * 100) .. "%"
end
