local ActivitySkinDrawRewardViewOld = class("ActivitySkinDrawRewardViewOld", ReduxView)

function ActivitySkinDrawRewardViewOld:UIName()
	return ActivitySkinDrawToolsOld.GetDrawRewardUIName(self.params_.mainActivityID)
end

function ActivitySkinDrawRewardViewOld:UIParent()
	return manager.ui.uiMain.transform
end

function ActivitySkinDrawRewardViewOld:Init()
	self:InitUI()
	self:AddUIListeners()
end

function ActivitySkinDrawRewardViewOld:InitUI()
	self:BindCfgUI()
...
function ActivitySkinDrawRewardViewOld:AddUIListeners()
...
function ActivitySkinDrawRewardViewOld:OnEnter()
	self.index_ = 0
...
function ActivitySkinDrawRewardViewOld:RefreshReplace()
	...
	for iter_9_0, iter_9_1 in ipairs(self.replaceList_) do
		...
		elseif ItemCfg[ActivityLimitedDrawPoolCfg[iter_9_1].reward[1][1]].type == ItemConst.ITEM_TYPE.HERO_SKIN then
...
function ActivitySkinDrawRewardViewOld:RefreshGoList()
	if self.activityID_ ~= self.params_.mainActivityID then
		self.typeGo_ = ActivitySkinDrawToolsOld.GetTypeGo(self.params_.mainActivityID)
...
function ActivitySkinDrawRewardViewOld:NextCode()
	...
	if self.index_ < #self.showList_ then
		...
		local var_11_3 = ActivityLimitedDrawPoolCfg[self.showList_[self.index_]].minimum_guarantee == 2 and 3 or 2

		if ItemCfg[ActivityLimitedDrawPoolCfg[self.showList_[self.index_]].reward[1][1]].type == ItemConst.ITEM_TYPE.FRAME then
			self:ShowReward(var_11_3, var_11_0, self.isReplaceFrame_)
		elseif ItemCfg[ActivityLimitedDrawPoolCfg[self.showList_[self.index_]].reward[1][1]].type == ItemConst.ITEM_TYPE.HERO_SKIN then
			self:ShowAni("special", var_11_0, self.isReplaceSkin_)
		elseif ItemCfg[ActivityLimitedDrawPoolCfg[self.showList_[self.index_]].reward[1][1]].type == ItemConst.ITEM_TYPE.SCENE then
			self.isScene_ = not self.isReplaceScene_

			self:ShowReward("special", var_11_0, self.isReplaceScene_)
		elseif ItemCfg[ActivityLimitedDrawPoolCfg[self.showList_[self.index_]].reward[1][1]].type == ItemConst.ITEM_TYPE.DYNAMIC_STICKER then
...
function ActivitySkinDrawRewardViewOld:ShowAni(arg_12_1, arg_12_2, arg_12_3)
	...
	self.typeItem_.movie = self.typeItem_.movie or ActivitySkinDrawMovieView.New((Object.Instantiate(Asset.Load(self.typeGo_.movie), self.content_)))
...
function ActivitySkinDrawRewardViewOld:ShowReward(arg_14_1, arg_14_2, arg_14_3)
...
function ActivitySkinDrawRewardViewOld:GotoResult()
...
function ActivitySkinDrawRewardViewOld:OnTop()
	manager.windowBar:HideBar()
end

function ActivitySkinDrawRewardViewOld:OnExit()
	SetActive(self.skipBtn_, true)
...
function ActivitySkinDrawRewardViewOld:Dispose()
...
return ActivitySkinDrawRewardViewOld
