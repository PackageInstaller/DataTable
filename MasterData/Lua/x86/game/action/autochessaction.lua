...
function var_0_0.StartNewGame(arg_12_0, arg_12_1)
	...
	manager.net:SendWithLoadingNew(89116, {
		game_type = arg_12_0,
		other_param = arg_12_1
	}, 89117, var_0_0.OnStartNewGameBack)
end

function var_0_0:OnStartNewGameBack(arg_13_1)
...
function var_0_0.LaunchRoundBattle(arg_14_0)
	manager.net:SendWithLoadingNew(89108, {
		game_type = arg_14_0
	}, 89109, var_0_0.OnLaunchRoundBattleBack)
end

function var_0_0:OnLaunchRoundBattleBack(arg_15_1)
	if isSuccess(self.result) then
		if arg_15_1.game_type == AutoChessConst.GAME_TYPE.PVE then
			saveData("auto_chess", "error", 0)
		end

		AutoChessData:InitBattleData(self.replay_info)
	else
		if arg_15_1.game_type == AutoChessConst.GAME_TYPE.PVE then
...
function var_0_0.SettleBattle(arg_17_0)
	manager.net:SendWithLoadingNew(89126, {
		game_type = arg_17_0
	}, 89127, var_0_0.OnSettleBattleBack)
end

function var_0_0:OnSettleBattleBack(arg_18_1)
	if isSuccess(self.result) then
...
function var_0_0:OnBuyChessBack(arg_20_1)
	if isSuccess(self.result) then
		manager.notify:Invoke("AUTO_CHESS_SHOP_BUY_CHESS", var_0_0.shopData, arg_20_1.to_index)

		if arg_20_1.shop_type == AutoChessConst.SHOP_TYPE.PROP and AutoChessItemCfg[var_0_0.shopData.chessId] and AutoChessTools.IsPrepareStatus() and not AutoChessData:GetIsInUseProp() then
			AutoChessData:SetIsInUseProp(true)

			local var_20_0 = AutoChessBattleRenderer.GetInstance():GetPrepareView()

			if var_20_0 then
				if AutoChessTools.IsTargetProp(AutoChessItemCfg[var_0_0.shopData.chessId].id) then
					local var_20_1 = var_20_0:GetChessItemByTargetIndex(arg_20_1.to_index)

					if var_20_1 then
						var_20_1:PlayPropAnimator(AutoChessItemCfg[var_0_0.shopData.chessId].icon)
					end
				else
					var_20_0:PlayPropAnimator(AutoChessItemCfg[var_0_0.shopData.chessId].icon)
				end
			end
		end
...
function var_0_0:OnSellChessBack(arg_22_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0:OnRefreshShopBack(arg_24_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0.LockShop(arg_25_0, arg_25_1, arg_25_2)
	manager.net:SendWithLoadingNew(89120, {
		game_type = arg_25_0,
		type = arg_25_1,
		info_list = arg_25_2
	}, 89121, var_0_0.OnLockShopBack)
end

function var_0_0:OnLockShopBack(arg_26_1)
	if isSuccess(self.result) then
...
function var_0_0:OnChangeChessTeamBack(arg_28_1)
	if isSuccess(self.result) then
		local var_28_0 = {}
		local var_28_1 = AutoChessData:GetPointChessData()

		for iter_28_0, iter_28_1 in ipairs(self.chess_list) do
...
function var_0_0:OnMergeChessBack(arg_30_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0:OnBueChessOneKeyBack(arg_32_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0:OnPlayerChessUpgradeBack(arg_34_1)
	if isSuccess(self.result) then
...
function var_0_0:OnSkipStageBack()
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0:OnLookEnemyInfoBack()
	if isSuccess(self.result) then
		AutoChessData:GetPrepareData(AutoChessConst.GAME_TYPE.PVE):InitEnemyData(self.enemy_auto_chess_board)
...
function var_0_0:OnCancelGameBack(arg_43_1, arg_43_2)
	if isSuccess(self.result) then
...
function var_0_0.OpenTeamnfoPopAfterFetch(arg_44_0, arg_44_1)
	if arg_44_1.user_id then
		var_0_0.FetchRankTeamInfo(arg_44_0, arg_44_1.user_id, function(arg_45_0)
			JumpTools.OpenPageByJump("autoChessRankTeamInfoPop", {
				rankData = arg_44_1,
				teamData = arg_45_0.record
			})
		end)
	end
end
...
function var_0_0:OnLookReplayBack(arg_50_1)
	if isSuccess(self.result) then
		AutoChessData:UpdateGameStatus({
...
function var_0_0:OnPrepareEndBack(arg_52_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0:OnExPortChessInfoBack(arg_54_1)
	if isSuccess(self.result) then
...
function var_0_0:OnSetGamePauseBack(arg_56_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
