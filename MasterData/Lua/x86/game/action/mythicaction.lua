...
function var_0_0:MythicStarBack(arg_14_1)
	if isSuccess(self.result) then
		if MythicCfg[MythicData:GetDifficulty()].difficulty == 1 then
			...
			sortMergeGetReward(var_14_0)
			MythicData:SetStarRewardState(arg_14_1.star)
			MythicAction:UpdateMythicRedTip()
...
function var_0_0:OnQueryChangeDifficulyuBack(arg_16_1)
	if isSuccess(self.result) then
...
function var_0_0.UpdateMythicRedTip(arg_17_0)
	if MythicData:GetDifficulty() ~= MythicData:GetFinalId() then
		...
		if var_17_0 and var_17_0 ~= 0 then
			for iter_17_0 = 1, MythicData:GetStarCount() do
