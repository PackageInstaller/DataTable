...
function var_0_0.PlayerUpgradeRedPoint()
	...
	for iter_4_0, iter_4_1 in ipairs(GameSetting.levelup_reward.value) do
		if var_4_0 < iter_4_1[1] then
			manager.redPoint:setTip(RedPointConst.NEWBIE_UPGRADE, 0)

			return
		end

		if not table.keyof(var_4_1, iter_4_1[1]) then
...
function var_0_0.InitRedPointConst(arg_13_0)
	...
	if var_13_1.noob_task_type ~= 0 then
		...
		for iter_13_1 = 1, #TaskTools:GetNoobPhaseTask(var_13_1.noob_task_type) do
			var_13_2[#var_13_2 + 1] = string.format("%s_%d", RedPointConst.NEWBIE_TASK, iter_13_1)
		end

		var_13_2[#var_13_2 + 1] = RedPointConst.NOOB_TASK_ACCUMULATE

		manager.redPoint:addGroup(RedPointConst.NEWBIE_TASK, var_13_2)
	end
end
...
function var_0_0.RefreshAccumulateRedPoint()
	...
	for iter_14_0, iter_14_1 in pairs((TaskTools:GetNoobCompletedList(NoobVersionCfg[ActivityNewbieTools.GetVersionID()].noob_task_type))) do
		var_14_0 = var_14_0 + iter_14_1
	end

	local var_14_1 = ActivityNoobData:GetAccumulateReceivedList() or {}

	for iter_14_2, iter_14_3 in ipairs((ActivityNewbieTools.GetNoobPorgressRewardList())) do
		if var_14_0 >= iter_14_3[1] and not var_14_1[iter_14_2] then
