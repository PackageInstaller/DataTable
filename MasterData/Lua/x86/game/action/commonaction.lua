...
function var_0_0.TryToUseItem(arg_1_0)
	...
	if var_1_0 then
		...
		}, 17013, var_0_0.OnTryToUseItem)
	else
		ShowTips("ERROR_ITEM_TIME_INVALID")
	end
end

function var_0_0:OnTryToUseItem(arg_2_1)
	manager.notify:CallUpdateFunc(ON_TRY_TO_USE_ITEM, self, arg_2_1)

	if isSuccess(self.result) then
		manager.notify:Invoke(USE_ITEM_SUCCESS)
	end
end
