...
function var_0_0:OnEnchantmentRefresh(arg_5_1)
	if isSuccess(self.result) then
		if arg_5_1.refresh_type == 1 then
			BattleEnchantmentData:EnchantmentFreeRefresh(self)
		elseif arg_5_1.refresh_type == 2 then
			BattleEnchantmentData:EnchantmentBuyRefresh(self)
		end

		manager.notify:CallUpdateFunc(ENCHANTMENT_REFRESH, self, arg_5_1)
	else
