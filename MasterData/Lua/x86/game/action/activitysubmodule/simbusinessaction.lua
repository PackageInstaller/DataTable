...
function var_0_0:OnUpgradeShelf(arg_14_1)
...
function var_0_0.BindRedPoint(arg_20_0)
	manager.redPoint:addGroup(ActivityTools.GetRedPointKey(SimBusinessData.activityID) .. SimBusinessData.activityID, {
	...
	for iter_20_0, iter_20_1 in ipairs(SimBusinessStageCfg.get_id_list_by_type[SimBusinessConst.LevelType.CHALLENGE]) do
		table.insert(var_20_0, SimBusinessTools.GetSpStageRedPoint(iter_20_1))
	end

	manager.redPoint:addGroup(RedPointConst.ACTIVITY_SIM_BUSINESS_SPECIAL_STAGE, var_20_0)

	local var_20_1 = {}

	for iter_20_2, iter_20_3 in ipairs(SimBusinessShelfCfg.all) do
		table.insert(var_20_1, string.format("%s_%s", RedPointConst.ACTIVITY_SIM_BUSINESS_SHELF_LEVEL_UP, iter_20_3))
	end

	manager.redPoint:addGroup(RedPointConst.ACTIVITY_SIM_BUSINESS_SHELF_LEVEL_UP, var_20_1)

	local var_20_2 = {}

	for iter_20_4, iter_20_5 in ipairs(SimBusinessGoodsCfg.all) do
		table.insert(var_20_2, string.format("%s_%s", RedPointConst.ACTIVITY_SIM_BUSINESS_UNLOCK_GOOD, iter_20_5))
	end

	manager.redPoint:addGroup(RedPointConst.ACTIVITY_SIM_BUSINESS_UNLOCK_GOOD, var_20_2)
end
