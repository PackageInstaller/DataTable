local var_0_0 = {
	InitRedPointKey = function(arg_1_0)
		...
		for iter_1_0, iter_1_1 in pairs(ActivityReforgeSkillCfg.all) do
			table.insert(var_1_0, RedPointConst.ACTIVITY_REFORGE_SKILL .. iter_1_1)
			manager.redPoint:setTip(RedPointConst.ACTIVITY_REFORGE_SKILL .. iter_1_1, 0)
		end

		manager.redPoint:addGroup(RedPointConst.ACTIVITY_REFORGE_SKILL .. arg_1_0, var_1_0)

		local var_1_1 = {}

		for iter_1_2, iter_1_3 in pairs(ActivityCfg[arg_1_0].sub_activity_list) do
			table.insert(var_1_1, RedPointConst.ACTIVITY_REFORGE_NEW_CHPATER .. iter_1_3)
		end

		manager.redPoint:addGroup(RedPointConst.ACTIVITY_REFORGE_NEW_CHPATER .. arg_1_0, var_1_1)
		manager.redPoint:addGroup(RedPointConst.ACTIVITY_REFORGE .. arg_1_0, {
			RedPointConst.ACTIVITY_REFORGE_TASK .. arg_1_0,
			RedPointConst.ACTIVITY_REFORGE_NEW_CHPATER .. arg_1_0
		})
	end
}
	...
	for iter_3_0, iter_3_1 in pairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.ACTIVITY_REFORGE]) do
		if ActivityData:GetActivityIsOpen(iter_3_1) then
			if #TaskTools:GetCanGetActivityTaskList(iter_3_1) > 0 then
				manager.redPoint:setTip(RedPointConst.ACTIVITY_REFORGE_TASK .. iter_3_1, 1)
			else
				manager.redPoint:setTip(RedPointConst.ACTIVITY_REFORGE_TASK .. iter_3_1, 0)
			end
		else
			manager.redPoint:setTip(RedPointConst.ACTIVITY_REFORGE_TASK .. iter_3_1, 0)
		end
...
function var_0_0.UpdateNewChapterRedPoint(arg_7_0)
	if ActivityData:GetActivityIsOpen(arg_7_0) then
		if ActivityReforgeData:GetChapterIsUnlock(arg_7_0) and ActivityReforgeData:GetChapterFinishLevelNum(arg_7_0) <= 0 then
			manager.redPoint:setTip(RedPointConst.ACTIVITY_REFORGE_NEW_CHPATER .. arg_7_0, 1)
		else
			manager.redPoint:setTip(RedPointConst.ACTIVITY_REFORGE_NEW_CHPATER .. arg_7_0, 0)
		end
	else
		manager.redPoint:setTip(RedPointConst.ACTIVITY_REFORGE_NEW_CHPATER .. arg_7_0, 0)
	end
...
function var_0_0.TeamLevelUp(arg_14_0, arg_14_1, arg_14_2, arg_14_3)
	if ActivityReforgeAction.CheckGold(ActivityReforgeTeamCfg[arg_14_2].cost[ActivityReforgeData:GetTeamInfo(arg_14_2).level], true) then
...
function var_0_0.BuySkill(arg_20_0, arg_20_1, arg_20_2, arg_20_3)
	if ActivityReforgeAction.CheckGold(ActivityReforgeSkillCfg[arg_20_2].cost, true) then
