	...
	if not ActivityAutoCookData.login_ then
		...
		if ActivityData:GetActivityIsOpen(ActivityConst.AUTO_COOK) then
			for iter_8_0, iter_8_1 in pairs(ActivityAutoCookStageCfg.all) do
				if (ActivityAutoCookStageCfg[iter_8_1].condition <= 0 or not not IsConditionAchieved(ActivityAutoCookStageCfg[iter_8_1].condition) and true) == true and ((ActivityAutoCookData:GetStageIsFinish(iter_8_1) or nil) and true) == false then
