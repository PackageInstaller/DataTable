...
function var_0_0:StartMatrixCallBack(arg_6_1)
	if isSuccess(self.result) then
		ActivityMatrixData:UpdateMatrixProcess(arg_6_1.activity_id, MatrixConst.STATE_TYPE.STARTED, self.progress)
...
function var_0_0:NextProgressCallBack(arg_8_1)
	if isSuccess(self.result) then
		ActivityMatrixData:UpdateMatrixProcess(arg_8_1.activity_id, MatrixConst.STATE_TYPE.STARTED, self.progress)

		local var_8_0 = ActivityMatrixData:GetGameState(arg_8_1.activity_id)

		if var_8_0 == MatrixConst.STATE_TYPE.STARTED then
			local var_8_1 = var_0_1:GetPhase()
			local var_8_2 = arg_8_1.params and arg_8_1.params[1]

			if MatrixConst.PHASE_TYPE.REWARD == var_8_1 then
				local var_8_3 = false

				if not var_8_2 then
					var_8_3 = true
					var_8_2 = var_0_1:GetGiveUpItemId() or 0
				end

				var_0_0.DoReward(var_8_2, var_8_3, arg_8_1.activity_id)
			elseif MatrixConst.PHASE_TYPE.ACTION == var_8_1 then
				if MatrixConst.EVENT_TYPE.INCIDENT == ActivityMatrixEventTemplateCfg[var_0_1:GetParams()[1]].event_type then
					var_0_0.DoEffect(MatrixChanceCfg[var_8_2].toll)
				end

				var_0_0.DoAfterEvnet(arg_8_1.activity_id, var_0_2)
			end

			if ActivityMatrixData:GetMatrixPhaseData(arg_8_1.activity_id):GetPhase() == 2 then
...
function var_0_0.DoEvent(arg_9_0)
	...
	if not (var_9_1 and ActivityMatrixEventTemplateCfg[var_9_1]) then
	...
	elseif MatrixConst.EVENT_TYPE.INCIDENT == var_9_2.event_type then
		...
		BattleController.GetInstance():LaunchBattle((BattleActivityMatrixTemplate.New(var_9_0:GetData(), arg_9_0)))
	end
end

function var_0_0.DoReward(arg_10_0, arg_10_1, arg_10_2)
	if not MatrixItemCfg[arg_10_0] then
		return
	end

	if MatrixConst.ITEM_TYPE.ITEM == MatrixItemCfg[arg_10_0].matrix_item_type then
		-- block empty
	elseif MatrixConst.ITEM_TYPE.EFFECT == MatrixItemCfg[arg_10_0].matrix_item_type then
		var_0_0.DoEffect(MatrixItemCfg[arg_10_0].params[1] or 0)
	end
end
...
function var_0_0:MatrixOverCallBack(arg_14_1)
	if isSuccess(self.result) then
		ActivityMatrixData:UpdateMatrixProcess(arg_14_1.activity_id, MatrixConst.STATE_TYPE.NOTSTARTED, self.progress)
		JumpTools.GoToSystem("/matrixBlank/activityMatrix", {
			main_matrix_activity_id = ActivityMatrixData:GetMainActivityId(arg_14_1.activity_id),
...
function var_0_0:MatrixGiveUpCallBack(arg_16_1)
	if isSuccess(self.result) then
		ActivityMatrixData:UpdateMatrixProcess(arg_16_1.activity_id, MatrixConst.STATE_TYPE.FAIL, self.progress)
...
function var_0_0:OnBuyShopItem(arg_18_1)
	if isSuccess(self.result) then
		ActivityMatrixData:UpdateMatrixProcess(arg_18_1.activity_id, MatrixConst.STATE_TYPE.STARTED, self.progress)
...
function var_0_0.QueryRefreshShopItem(arg_19_0)
	...
	local var_19_1 = ActivityMatrixData:GetShopFressRefreshTimes(arg_19_0)

	if var_19_0 < ActivityMatrixData:GetShoRefreshTimes(arg_19_0) then
		if var_19_1 <= var_19_0 then
			if ActivityMatrixData:GetMatrixCoint(arg_19_0) < GameSetting.matrix_shop_refresh_cost_item_list.value[math.min(var_19_0 - var_19_1 + 1, #GameSetting.matrix_shop_refresh_cost_item_list.value)][2] then
...
function var_0_0:OnRefrshShopItem(arg_20_1)
	if isSuccess(self.result) then
		ActivityMatrixData:UpdateMatrixProcess(arg_20_1.activity_id, MatrixConst.STATE_TYPE.STARTED, self.progress)
...
function var_0_0.CheckRewardRedPonit()
	for iter_22_0, iter_22_1 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.SINGLE_MATRIX] or {}) do
		if ActivityData:GetActivityIsOpen(iter_22_1) then
			local var_22_1 = false

			for iter_22_2, iter_22_3 in ipairs(ActivityCfg[iter_22_1].sub_activity_list) do
			...
			manager.redPoint:setTip(string.format("%s_%s", RedPointConst.ACTIVITY_MATRIX_REWARD, iter_22_1), var_22_1 and 1 or 0)
		else
...
function var_0_0.CheckReadRedPonit()
	for iter_23_0, iter_23_1 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.SINGLE_MATRIX] or {}) do
		local var_23_1 = false

		if ActivityData:GetActivityIsOpen(iter_23_1) then
			for iter_23_2, iter_23_3 in ipairs(ActivityCfg[iter_23_1].sub_activity_list) do
		...
		manager.redPoint:setTip(string.format("%s_%s", RedPointConst.ACTIVITY_MATRIX_READ, iter_23_1), var_23_1 and 1 or 0)
	end
end
...
function var_0_0.CheckSubReadRedPonit(arg_24_0)
	if not ActivityMatrixData:GetRead(arg_24_0) then
		...
		if var_24_0 and var_24_0.point == 0 and MatrixConst.STATE_TYPE.NOTSTARTED == var_24_0.game_state and ActivityMatrixData:GetIsClearance(ActivityMatrixCfg[arg_24_0].preconditions) and ActivityData:GetActivityIsOpen(arg_24_0) then
			return true
		end
	end
...
function var_0_0.SetActivityMtairxRead(arg_25_0, arg_25_1)
	if not ActivityMatrixData:GetIsClearance(ActivityMatrixCfg[arg_25_1].preconditions) or not ActivityData:GetActivityIsOpen(arg_25_1) then
		return
	end
	...
	for iter_25_0, iter_25_1 in ipairs(ActivityCfg[arg_25_0].sub_activity_list) do
	...
	manager.redPoint:setTip(string.format("%s_%s", RedPointConst.ACTIVITY_MATRIX_READ, arg_25_0), var_25_0 and 1 or 0)
end

function var_0_0.CheckClueRedPoint(arg_26_0)
	if ActivityData:GetActivityIsOpen(arg_26_0) then
		for iter_26_0, iter_26_1 in ipairs((ActivityMatrixData:GetMatrixClue(arg_26_0))) do
...
function var_0_0.GetSelectCamp()
	var_0_4 = var_0_4 or getData("AcvityMatrixClue", "selectCamp")
