...
function var_0_0:CurrencyInit(arg_4_1, arg_4_2)
	for iter_4_0, iter_4_1 in ipairs(arg_4_1) do
		if iter_4_1.time_valid and iter_4_1.time_valid ~= 0 then
			...
			if iter_4_1.id == CurrencyConst.CURRENCY_TYPE_VITALITY and iter_4_1.num <= GameLevelSetting[PlayerData:GetPlayerInfo().userLevel].fatigue_max - GameSetting.push_notification_fatigue_threshold.value[1] then
				self:SetFatigueCallFlag(1)
			end
		end
	end
...
function var_0_0.SetCurrencyValue(arg_6_0, arg_6_1, arg_6_2, arg_6_3)
	...
	if arg_6_3 > 0 then
		...
		if arg_6_1 == CurrencyConst.DORM_TASK_WEEKLY then
			var_6_0 = arg_6_2 - var_0_6[arg_6_1].num
		end
	...
	if arg_6_1 == CurrencyConst.DORM_TASK_WEEKLY then
		IdolTraineeData:UpdateQuestWeeklyData(var_6_0)
	end
...
function var_0_0.GetRechargeDiamond(arg_13_0)
	...
	return var_0_6[CurrencyConst.GetPlatformDiamondId()].num + var_0_6[CurrencyConst.CURRENCY_TYPE_RECHARGE_DIAMOND_FREE].num + ((AreaDifferenceCfg[GameToSDK.CURRENT_SERVER].currency_merge == 1 or SDKTools.GetIsOverSea() == false or nil) and var_0_6[CurrencyConst.GetOtherPlatformDiamondId()].num)
end

function var_0_0:GetCurrencyNum(arg_14_1)
	if arg_14_1 == CurrencyConst.CURRENCY_TYPE_RECHARGE_DIAMOND_FREE then
		return self:GetRechargeDiamond()
	end
