...
function var_0_0.SetDayListInfo(arg_2_0)
	...
	for iter_2_0 = 1, #ActivitySpringFestivalGive2100Cfg.all do
		table.insert(var_0_2, {
			day = iter_2_0,
			state = var_0_0:GetState(ActivitySpringFestivalGive2100Cfg.all[iter_2_0])
		})
	end
...
function var_0_0.InitRedPoint(arg_9_0)
	...
	for iter_9_0, iter_9_1 in ipairs(var_0_2) do
		if iter_9_1.state == "complete" then
			-- block empty
		elseif ActivityData:GetActivityIsOpen(ActivitySpringFestivalGive2100Cfg.all[iter_9_0]) and iter_9_1.state == "reward" and ActivityData:GetActivityData(ActivitySpringFestivalGive2100Cfg.all[iter_9_0]).startTime < manager.time:GetServerTime() then
			var_9_0 = var_9_0 + 1
		end
	end
