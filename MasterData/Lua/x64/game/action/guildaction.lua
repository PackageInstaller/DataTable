...
function var_0_0.GuildRequestJoin(arg_18_0, arg_18_1)
	manager.net:SendWithLoadingNew(31010, {
		...
		elseif arg_19_0.result == TipsCfg.get_id_list_by_define.ERROR_USER_JOIN_COOL_DOWN then
			if arg_19_0.remain_time > 0 then
				local var_19_0 = GetTips("CLUB_REQUEST_CD")
				local var_19_1

				if arg_19_0.remain_time < 60 then
					var_19_1 = GetTips("SECOND")
					var_19_1 = string.format("%s%s", arg_19_0.remain_time, var_19_1)
				elseif arg_19_0.remain_time < 3600 then
					var_19_1 = GetTips("MINUTE")
					var_19_1 = string.format("%s%s", math.floor(arg_19_0.remain_time / 60), var_19_1)
				else
					var_19_1 = GetTips("HOUR")
					var_19_1 = string.format("%s%s", math.floor(arg_19_0.remain_time / 3600), var_19_1)
				end

				ShowTips(string.format(var_19_0, var_19_1))
			end
		else
...
function var_0_0.RequestClubBossInfo(arg_63_0)
	manager.net:SendWithLoadingNew(33000, {
		club_id = arg_63_0
	}, 33001, var_0_0.OnRequestClubBossInfoCallback)
end
...
function var_0_0.GetAssistBriefInfo(arg_65_0)
	manager.net:SendWithLoadingNew(33002, {
		club_id = arg_65_0
	}, 33003, var_0_0.OnGetAssistBriefInfoCallback)
end

function var_0_0:OnGetAssistBriefInfoCallback(arg_66_1)
	if isSuccess(self.result) then
		GuildData:InitAssistInfoListFromServer(self.assist_info_list)
	else
...
function var_0_0.GuildBossChangeDifficulty(arg_67_0)
	manager.net:SendWithLoadingNew(33004, {
		difficulty = arg_67_0
	}, 33005, var_0_0.OnGuildBossChangeDifficultyCallback)
end

function var_0_0:OnGuildBossChangeDifficultyCallback(arg_68_1)
	if isSuccess(self.result) then
...
function var_0_0.GuildBossGetRankInfo(arg_69_0)
	manager.net:SendWithLoadingNew(33006, {
		club_id = arg_69_0
	}, 33007, var_0_0.OnGuildBossGetRankInfoCallback)
end

function var_0_0:OnGuildBossGetRankInfoCallback(arg_70_1)
	if isSuccess(self.result) then
		GuildData:InitGuildBossRankFromServer(self.info_list)
	else
		ShowTips(GetTips(self.result))
	end
end
...
function var_0_0.GetGuildBossReward(arg_71_0, arg_71_1, arg_71_2)
	manager.net:SendWithLoadingNew(33008, {
		...
		type = arg_71_1,
		award_id = arg_71_2
	}, 33009, var_0_0.OnGetGuildBossRewardCallback)
end

function var_0_0:OnGetGuildBossRewardCallback(arg_72_1)
	if isSuccess(self.result) then
		if arg_72_1.type == 1 then
			for iter_72_0, iter_72_1 in ipairs(ClubBossCfg[GuildData:GetBossDifficulty()].personal_reward) do
				if iter_72_1[1] == arg_72_1.award_id then
					getReward((mergeReward((formatRewardCfgList(iter_72_1[2])))))
		...
		elseif arg_72_1.type == 2 then
			for iter_72_2, iter_72_3 in ipairs(ClubBossCfg[GuildData:GetBossDifficulty()].reward) do
				if iter_72_3[1] == arg_72_1.award_id then
					getReward((mergeReward((formatRewardCfgList(iter_72_3[2])))))
...
function var_0_0.SetAssistHero(arg_73_0, arg_73_1, arg_73_2)
	local var_73_0 = clone(GuildData:GetCurrentBossAssistantHeroId() or {})
	...
	if arg_73_2 then
		...
		local var_73_1 = table.indexof(var_73_0, arg_73_1)

		if var_73_1 then
			table.remove(var_73_0, var_73_1)
		end
	end

	manager.net:SendWithLoadingNew(33010, {
		club_id = arg_73_0,
		hero_id_list = var_73_0
	}, 33011, var_0_0.OnSetAssistHeroCallback)
end

function var_0_0:OnSetAssistHeroCallback(arg_74_1)
	if isSuccess(self.result) then
...
function var_0_0.UpdateGuildPersonRedPoint()
	...
	for iter_77_0, iter_77_1 in ipairs(ClubBossCfg[GuildData:GetBossDifficulty()].personal_reward) do
		if not GuildData:HaveGetGuildBossPersonalReward(iter_77_1[1]) then
			var_77_0 = GuildData:GetBossPersonalScore() >= iter_77_1[1]

			break
		end

		var_77_0 = false and true

		break
	end

	manager.redPoint:setTip(RedPointConst.GUILD_BOSS_PERSONAL_BONUS, var_77_0 and 1 or 0)
end
...
function var_0_0.UpdateGuildCommonRedPoint()
	...
	for iter_78_0, iter_78_1 in ipairs(ClubBossCfg[GuildData:GetBossDifficulty()].reward) do
		var_78_0 = not GuildData:HaveGetGuildBossCommonReward(iter_78_1[1]) and ClubBossCfg[GuildData:GetBossDifficulty()].boss_hp * (100 - iter_78_1[1]) / 100 >= ClubBossCfg[GuildData:GetBossDifficulty()].boss_hp - GuildData:GetCurrentBossDamage()
	...
	manager.redPoint:setTip(RedPointConst.GUILD_BOSS_COMMON_BONUS, var_78_0 and 1 or 0)
end
