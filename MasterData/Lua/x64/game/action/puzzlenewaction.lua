...
manager.net:Bind(65301, function(arg_1_0)
	if ActivityTools.GetActivityType(arg_1_0.activity_id) ~= ActivityTemplateConst.PUZZLE_NEW then
		return
	end

	if not var_0_2[arg_1_0.activity_id] then
		var_0_2[arg_1_0.activity_id] = true

		PuzzleNewTools.InitConfig(arg_1_0.activity_id)
	end
	...
	PuzzleNewData:RefreshPlayRedPoint(arg_1_0.activity_id)
	PuzzleNewData:RefreshPieceRedPoint(arg_1_0.activity_id)
end)
...
function var_0_0.InitRedPoint(arg_8_0)
	...
	manager.redPoint:addGroup(nil, {
		(string.format("%s_%d", RedPointConst.PUZZLE_NEW_PIECE, arg_8_0))
	})

	var_8_2[#var_8_2 + 1] = string.format("%s_%d", RedPointConst.PUZZLE_NEW_PLAY, arg_8_0)

	if not PuzzleNewData:IsCompleted(arg_8_0) then
		for iter_8_0, iter_8_1 in ipairs(ActivityCfg[arg_8_0].sub_activity_list) do
			if ActivityCfg[iter_8_1].activity_template == ActivityTemplateConst.TASK then
				break
			end
		end

		var_8_2[#var_8_2 + 1] = string.format("%s_%d", RedPointConst.ACTIVITY_TASK, nil)
	end

	manager.redPoint:addGroup(string.format("%s_%d", RedPointConst.PUZZLE_NEW, arg_8_0), var_8_2)
end
