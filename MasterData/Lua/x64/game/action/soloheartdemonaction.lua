...
function var_0_0:UpdateRefreshRedPoint()
	if JumpTools.IsSystemLocked(ViewConst.SYSTEM_ID.SOLO_HEART_DEMON) then
		return
	end
	...
	saveData("SOLO_HEART_DEMON", "STAGE", tostring(self.challenge_stage))

	if getData("SOLO_HEART_DEMON", "STAGE_VIEW") ~= "1" and self.challenge_stage == 0 then
		manager.redPoint:setTip(RedPointConst.SOLO_HEART_DEMON_FIRST_VIEW, 1)

		return
	end

	if self.challenge_stage == 0 and var_6_0 ~= "0" then
		manager.redPoint:setTip(RedPointConst.SOLO_HEART_DEMON_FIRST_VIEW, 1)
		manager.redPoint:setTip(string.format("%s%s", ActivityTools.GetRedPointKey(var_0_1), var_0_1), 1)
