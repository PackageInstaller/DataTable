...
function var_0_0:OnSaveTeamCallback(arg_5_1)
...
function var_0_0:OnSaveBattleCallback(arg_7_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0:OnResetCallback(arg_11_1)
	if isSuccess(self.result) then
...
function var_0_0:OnSendDataToServerCallback(arg_13_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0:UpdateRefreshRedPoint()
	...
	saveData("MULT_HEART_DEMON", "STAGE", tostring(self.challenge_stage))

	if getData("MULT_HEART_DEMON", "STAGE_VIEW") ~= "1" and self.challenge_stage == 0 then
		manager.redPoint:setTip(RedPointConst.MULT_HEART_DEMON_FIRST_VIEW, 1)

		return
	end

	if self.challenge_stage == 0 and var_15_0 ~= "0" then
		manager.redPoint:setTip(RedPointConst.MULT_HEART_DEMON_FIRST_VIEW, 1)
		manager.redPoint:setTip(string.format("%s%s", ActivityTools.GetRedPointKey(var_0_1), var_0_1), 1)
...
function var_0_0.UpdateRewardRedPoint()
	...
	for iter_18_0, iter_18_1 in ipairs((TaskData2:GetTypedTaskSortListCommonFilter(TaskConst.TASK_TYPE.MULT_HEART_NORMAL_TASK))) do
		var_18_0[iter_18_1.id] = iter_18_1
	end

	for iter_18_2, iter_18_3 in pairs(var_18_0) do
		if TaskData2:GetTaskProgress(iter_18_2) >= AssignmentCfg[iter_18_2].need and not TaskData2:GetTaskComplete(iter_18_2) then
