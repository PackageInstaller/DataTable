...
function var_0_0:OnRequireVoteRoundInfo(arg_4_1)
	if isSuccess(self.result) then
		SwimsuitBattleData:UpdateVoteFromServer(self)
...
function var_0_0.VoteForHero(arg_5_0, arg_5_1, arg_5_2, arg_5_3)
	manager.net:SendWithLoadingNew(65506, {
		activity_id = arg_5_0,
		schedule_id = arg_5_1,
		competition_list = {
			{
	...
	}, 65507, var_0_0.OnVoteForHero)
end

function var_0_0:OnVoteForHero(arg_6_1)
	if isSuccess(self.result) then
		-- block empty
	else
		ShowTips(self.result)
...
function var_0_0.CheckSwimsuitBattleHeroStageOpen()
	for iter_9_0, iter_9_1 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.ACTIVITY_SWIMSUIT_BATTLE_HERO_STAGE] or {}) do
