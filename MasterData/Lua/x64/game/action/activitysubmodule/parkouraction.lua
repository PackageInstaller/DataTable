...
manager.net:Bind(11061, function(arg_2_0)
	...
	for iter_2_0, iter_2_1 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.PARKOUR] or {}) do
...
function var_0_0:OnParkourStarRewardCallBack(arg_4_1)
...
function var_0_0:UpdateCollect()
	local var_6_0 = ParkourData:GetParkourCollectList()[self.activity_id]

	if var_6_0 then
		for iter_6_0, iter_6_1 in ipairs(self.item_list) do
			local var_6_1 = {}
			...
			if #var_6_1 > 0 then
				local var_6_2 = ParkourData:GetCollectRedList(self.activity_id)

				table.insertto(var_6_2, var_6_1)
				saveData("Parkour", tostring(self.activity_id), var_6_2)
				manager.redPoint:setTip(string.format("%s_%s", RedPointConst.PARKOUR_COLLECT, self.activity_id), 1)
			end
		end
	end
end
...
function var_0_0.CheckPardourCollectRedPoint(arg_7_0)
	if ActivityCfg[arg_7_0] then
		local var_7_0 = ParkourData:GetCollectRedList(arg_7_0)

		for iter_7_0, iter_7_1 in pairs(ActivityCfg[arg_7_0].sub_activity_list) do
			if table.indexof(var_7_0, ActivityParkourCfg[iter_7_1].specail_item) then
...
function var_0_0.UpdateParkourRedPoint(arg_8_0, arg_8_1)
	for iter_8_0, iter_8_1 in pairs(ActivityCfg[arg_8_1].sub_activity_list) do
...
function var_0_0.CheckEntrustRewardRedPoint(arg_9_0)
	...
	if var_9_0 then
		local var_9_1 = #var_9_0.stars

		if table.indexof(var_9_0.stars, 4) then
			var_9_1 = var_9_1 - 1

			if not table.indexof(var_9_0.rewards, 4) then
...
function var_0_0.CheckPardourEntrustRedPoint(arg_10_0)
	for iter_10_0, iter_10_1 in pairs(ActivityCfg[arg_10_0].sub_activity_list) do
