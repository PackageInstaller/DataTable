...
function var_0_0.SetSelectSuit(arg_3_0, arg_3_1)
	manager.net:SendWithLoadingNew(76152, {
		activity_id = arg_3_0,
		up_select = arg_3_1
	}, 76153, var_0_0.OnSetSelectSuit)
end

function var_0_0:OnSetSelectSuit(arg_4_1)
...
function var_0_0.TryToSubmitScrollTask(arg_5_0, arg_5_1)
	...
	if ActivityTools.CheckActivityIsSuspended(arg_5_0) then
		do return end
	...
	var_5_0.times = math.floor(ActivityPtScrollData:GetClearTime(arg_5_0) / GameSetting.activity_pt_roulette_task_times.value[1])
	var_5_0.assignment_id_list = arg_5_1 or {}

	manager.net:SendWithLoadingNew(76154, var_5_0, 76155, var_0_0.OnTryToSubmitScrollTask)
end

function var_0_0:OnTryToSubmitScrollTask(arg_6_1)
	if isSuccess(self.result) then
		local var_6_0 = {}

		for iter_6_0, iter_6_1 in ipairs(self.reward_list) do
		...
		ActivityPtScrollData:ModifyClearTime(arg_6_1.activity_id, ActivityPtScrollData:GetClearTime(arg_6_1.activity_id) % GameSetting.activity_pt_roulette_task_times.value[1])

		for iter_6_2, iter_6_3 in ipairs(arg_6_1.assignment_id_list) do
			TaskData2:SubmitTask(iter_6_3)
			table.insertto(var_6_0, AssignmentCfg[iter_6_3].reward or {})
		end

		local var_6_2 = mergeReward((formatRewardCfgList(var_6_0)))

		if #var_6_2 > 0 then
			getReward(var_6_2)
		end

		TaskAction:OnTaskChange((TaskAction:PackTaskIdListToTaskList(arg_6_1.assignment_id_list)))
	else
		ShowTips(self.result)
	end
end
...
function var_0_0.RefreshRedPoint(arg_8_0)
	...
	manager.redPoint:setTip(RedPointConst.ACTIVITY_PT_SCROLL_OPEN .. "_" .. arg_8_0, (ActivityTools.IsUnlockActivity(arg_8_0) and not (getData(RedPointConst.ACTIVITY_PT_SCROLL_OPEN, tostring(arg_8_0)) or false) or nil) and true and 1 or 0)
end
