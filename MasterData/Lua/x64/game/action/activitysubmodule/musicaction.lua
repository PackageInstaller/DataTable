...
function var_0_0.InitMusicRedPoint(arg_2_0)
	...
	for iter_2_0, iter_2_1 in ipairs(ActivityCfg.get_id_list_by_activity_template[ActivityTemplateConst.ACTIVITY_MUSIC] or {}) do
		local var_2_0 = {}

		if ActivityData:GetActivityIsOpen(iter_2_1) then
			for iter_2_2, iter_2_3 in pairs(ActivityCfg[iter_2_1].sub_activity_list or {}) do
				table.insert(var_2_0, string.format("%s_%s", RedPointConst.MUSIC_OPEN, iter_2_3))
			end
		end

		local var_2_2 = string.format("%s_%s", RedPointConst.MUSIC_OPEN, iter_2_1)

		manager.redPoint:addGroup(var_2_2, var_2_0)
...
function var_0_0.QueryCompelet(arg_6_0, arg_6_1)
	...
	var_6_0 = MusicData:GetGameRecord(MusicConst.MusicNodeHitRating.Good) == 0 and var_6_1 == 0 and MusicConst.MusicFinishState.Perfect or var_6_1 == 0 and MusicConst.MusicFinishState.NoMistake or MusicConst.MusicFinishState.Finished
...
function var_0_0:OnCampeletCallBack(arg_7_1)
	if isSuccess(self.result) then
		local var_7_1 = MusicData:GetScore(arg_7_1.id)

		if var_7_1 <= arg_7_1.score or var_7_1 == 0 then
			MusicData:SetScore(arg_7_1.id, arg_7_1.score, arg_7_1.state)
			MusicAction.CheckRedPoint()
			var_0_0.OpenSettlement(ActivityMusicCfg[arg_7_1.id].activity_id, arg_7_1.score, arg_7_1.score, true)
		else
			var_0_0.OpenSettlement(ActivityMusicCfg[arg_7_1.id].activity_id, arg_7_1.score, var_7_1, false)
		end
	else
...
function var_0_0.OpenSettlement(arg_8_0, arg_8_1, arg_8_2, arg_8_3, arg_8_4)
	...
	gameContext:Go(MusicData:GetMusicViewPathList(arg_8_0).settlement, {
		cur = arg_8_1,
		max = arg_8_2,
		new = arg_8_3,
...
function var_0_0:OnRewardCallBack(arg_10_1)
	if isSuccess(self.result) then
		local var_10_1 = mergeReward2(self.reward_list or {})
...
function var_0_0.CheckTargetMusicRedPoint(arg_13_0)
	if ActivityData:GetActivityIsOpen(arg_13_0) and not MusicData:GetRead(arg_13_0) then
		...
		for iter_13_0, iter_13_1 in ipairs(ActivityMusicCfg.get_id_list_by_activity_id[arg_13_0]) do
...
function var_0_0.CheckOpenRedPoint(arg_14_0)
	...
	if ActivityCfg[arg_14_0] then
		var_14_0 = ActivityCfg[arg_14_0].sub_activity_list or {}
	...
	for iter_14_2, iter_14_3 in ipairs(var_14_0) do
		if ActivityCfg[iter_14_3] and ActivityCfg[iter_14_3].activity_template == ActivityTemplateConst.ACTIVITY_MUSIC_GAME then
...
function var_0_0.Play(arg_17_0)
	...
	if not ActivityMusicCfg[arg_17_0] then
...
function var_0_0.GoToMusicMain()
	...
	local var_18_0 = MusicData:GetMusicViewPathList(ActivityMusicCfg[MusicData:GetGameId()].activity_id)

	if ActivityTools.GetActivityTheme((MusicData:GetPageActivityID())) == ActivityConst.THEME.ACTIVITY_3_11 then
		LaunchQWorld(true)
	else
		LuaExchangeHelper.GoToMain()
		OpenPageUntilLoaded(var_18_0.musicMain, {
			activity_id = var_18_0.activityID
		})
	end
end
