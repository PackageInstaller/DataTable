...
function var_0_0.GetStageName(arg_1_0, arg_1_1)
	if BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_MATRIX == arg_1_0 then
		local var_1_0 = MatrixTierCfg[MatrixData:GetTierID()]

		return ""
	end

	local var_1_1 = var_0_0.GetStageCfg(arg_1_0, arg_1_1)

	if var_1_1 then
		return GetI18NText(var_1_1.name) or ""
	else
		return ""
	end
end
...
function var_0_0.GetChapterByStageId(arg_2_0, arg_2_1)
	for iter_2_0, iter_2_1 in pairs(ChapterCfg.get_id_list_by_type[arg_2_0]) do
		if table.keyof(ChapterCfg[iter_2_1].section_id_list, arg_2_1) then
			return iter_2_1
		end
	end
...
function var_0_0.GetChapterSectionIndex(arg_3_0, arg_3_1)
	if BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_EQUIP == arg_3_0 then
		return ChapterCfg[ChapterCfg.get_id_list_by_type[arg_3_0][1]].english, string.format("%d-%d", BattleInstance.GetEquipStageShowIndex(arg_3_1), (BattleInstance.GetHardLevel(arg_3_1)))
	elseif BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_ENCHANTMENT == arg_3_0 then
		return ChapterCfg[ChapterCfg.get_id_list_by_type[arg_3_0][1]].english, (BattleInstance.GetEquipStageShowIndex(arg_3_1))
	elseif BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_DAILY == arg_3_0 then
		for iter_3_0, iter_3_1 in pairs(ChapterCfg.get_id_list_by_type[arg_3_0]) do
			local var_3_0 = table.keyof(ChapterCfg[iter_3_1].section_id_list, arg_3_1)

			if var_3_0 then
				return ChapterCfg[iter_3_1].english, var_3_0
			end
		end
	elseif BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_PLOT == arg_3_0 then
		if BattleChapterStageCfg[arg_3_1].stage_num ~= "" then
			local var_3_1 = string.split(BattleChapterStageCfg[arg_3_1].stage_num, "-")
			local var_3_2 = ""

			for iter_3_2 = 1, #var_3_1 - 1 do
				var_3_2 = iter_3_2 == 1 and var_3_1[iter_3_2] or var_3_2 .. "-" .. var_3_1[iter_3_2]
			end

			return var_3_2, var_3_1[#var_3_1]
		end

		local var_3_3 = ChapterCfg[getChapterIDByStageID(arg_3_1)]

		return var_3_3.english, (table.keyof(var_3_3.section_id_list, arg_3_1))
	elseif BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_SUB_PLOT == arg_3_0 then
		for iter_3_3, iter_3_4 in pairs(ChapterCfg.get_id_list_by_type[arg_3_0]) do
			local var_3_4 = table.keyof(ChapterCfg[iter_3_4].section_id_list, arg_3_1)

			if var_3_4 then
				return ChapterCfg[iter_3_4].english, var_3_4
			end
		end
	end
...
function var_0_0.GetStageCfg(arg_5_0, arg_5_1)
	...
	elseif BattleConst.STAGE_TYPE_NEW.ACTIVITY_INDIA_NIAN == arg_5_0 then
		...
		var_5_0 = BattleConst.STAGE_TYPE_NEW.PUSH_BOX_BATTLE == arg_5_0 and BattleActivityCelebrationCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.CORE_VERIFICATION == arg_5_0 and BattleCoreVerificationCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.SP_HERO_CHALLENGE_BATTLE_3_1 == arg_5_0 and BattleVerthandiExclusiveCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_HERO_ENHANCE == arg_5_0 and BattleHeroEnhanceCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_REFORGE == arg_5_0 and BattleActivityReforgeCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_CATCH_DUCK == arg_5_0 and BattleCatchDuckStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_MONSTER_COSPLAY == arg_5_0 and BattleActivityMonsterCosplayCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_RHYTHM_GAME == arg_5_0 and BattleRhythmGameStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_WHACK_MOLE == arg_5_0 and BattleWhackMoleStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_SUMMER_WATER == arg_5_0 and BattleActivityWaterGunnerCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_SWIMSUIT_BATTLE == arg_5_0 and BattleSwimsuitCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.CHESS_BOARD == arg_5_0 and BattleChessBoardCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ADVANCE_MONSTER_TEST == arg_5_0 and BattleActivityAdvanceMonsterTestCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_SPKALI_CHALLENGE == arg_5_0 and BattleCoreKaliCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.CHALLENGE_ROGUE_TEAM == arg_5_0 and BattleRogueTeamStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ACTIVITY_DODGE_BARRAGE == arg_5_0 and BattleDodgeBarrageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.SEA_WARFARE == arg_5_0 and BattleSeaWarfareStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.SKULD == arg_5_0 and BattleSkuldStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.SAND_PLAY == arg_5_0 and BattleSandPlayStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.OSIRIS_PLAY_GAME == arg_5_0 and BattleOsirisGameStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.GOD_EATER_BATTLE == arg_5_0 and BattleGodEaterDailyMonsterCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.SUMMER_PUB == arg_5_0 and BattleSummerPubStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.CORE_VERIFICATION_CHALLENGE == arg_5_0 and BattleCoreVerificationClCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.CORE_VERIFICATION_CHALLENGE_MODE2 == arg_5_0 and BattleCoreVerificationCl2Cfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.CORE_VERIFICATION_CHALLENGE_MODE3 == arg_5_0 and BattleCoreVerificationCl3Cfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.CORE_VERIFICATION_CHALLENGE_MODE4 == arg_5_0 and BattleCoreVerificationCl4Cfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.ASH_BATTLE == arg_5_0 and BattleAshShootStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.HELLA_PINBALL_BATTLE == arg_5_0 and BattleHellaPinballStageCfg[arg_5_1] or BattleConst.STAGE_TYPE_NEW.HODUR_BATTLE == arg_5_0 and BattleHodurStageCfg[arg_5_1] or var_5_0
	end
...
function var_0_0.IsHeroClearAnyTrialStage(arg_6_0)
	if HeroCfg[arg_6_0].study_stage and HeroCfg[arg_6_0].study_stage[1] and BattleTeachData:GetHeroTeachInfo(arg_6_0, HeroCfg[arg_6_0].study_stage[1]) > 0 then
		return true
	end
...
function var_0_0.CheckPopTrailStage(arg_7_0, arg_7_1, arg_7_2)
	...
	if var_7_0[2] and BattleStageData:GetStageClearTimes(var_7_0[2]) > 0 or BattleStageTools.IsHeroClearAnyTrialStage(var_7_1) then
		return false
	end
	...
	if whereami == "battleResult" or whereami == "battle" then
		...
		elseif arg_7_0 == BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_SUB_PLOT then
			BattleInstance.GotoBattleReadyPage(var_7_5, arg_7_1, arg_7_0, function()
				...
				OpenPageUntilLoaded(ChapterTools.GetSubPlotUrl(var_7_5) .. "/subPlotSectionInfo", {
					chapterClientID = ChapterCfg[var_7_5].clientID,
					section = arg_7_1,
					chapterID = var_7_5,
					sectionType = arg_7_0
				})
...
function var_0_0.SetSectionTrailBattleWay(arg_12_0, arg_12_1)
	...
	if BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_PLOT == arg_12_0 then
		...
		if ChapterConst.ROLL_MAIN_CLINET[getChapterClientCfgByChapterID(var_12_0).id] then
	...
	elseif BattleConst.STAGE_TYPE_NEW.STAGE_TYPE_SUB_PLOT == arg_12_0 then
		...
		local var_12_3 = BattleFieldData:GetCacheChapter(var_12_2.id)

		if not table.keyof(var_12_2.chapter_list, var_12_3) then
			var_12_3 = var_12_2.chapter_list[1]

			BattleFieldData:SetCacheChapterClient(var_12_2.toggle, var_12_2.id)
			BattleFieldData:SetCacheChapter(var_12_2.id, var_12_3)
			BattleStageAction.ClickSubPlot(var_12_3)
		end

		HeroTrialData:SetBattleWay(nil, ChapterTools.GetSubPlotUrl(var_12_3, isEntrace) .. "/subPlotSectionInfo", {
			chapterClientID = var_12_2.id,
			section = arg_12_1,
			chapterID = var_12_3,
			sectionType = arg_12_0
		})
	...
	if arg_14_1 and arg_14_1 ~= 0 then
		...
		if var_14_0 and type(var_14_0.chip_list) == "table" then
			...
			for iter_14_0, iter_14_1 in ipairs(var_14_0.chip_list) do
				if ChipCfg[iter_14_1] then
					if ChipCfg[iter_14_1].role_type_id == ChipConst.ROLE_TYPE_CHAR_CHIP_BASE then
						var_14_3 = iter_14_1
					elseif ChipCfg[iter_14_1].role_type_id == ChipConst.ROLE_TYPE_CHAR_CHIP_EXTRA then
						table.insert(var_14_4, iter_14_1)
					elseif ChipCfg[iter_14_1].type_id == ChipConst.TYPE_MANAGER then
						var_14_1 = iter_14_1
					elseif ChipCfg[iter_14_1].type_id == ChipConst.TYPE_CHIP then
