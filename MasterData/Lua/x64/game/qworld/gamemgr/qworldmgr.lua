...
function var_0_0.GetActivityMap(arg_11_0, arg_11_1)
	if not var_0_2 then
		...
		for iter_11_0, iter_11_1 in ipairs(SandPlayMapCfg.all) do
			var_0_2[SandPlayMapCfg[iter_11_1].activity_id] = SandPlayMapCfg[iter_11_1].id
		end
	end
...
function var_0_0:GetMapId()
	return self:GetActivityMap(self:GetActivityId())
end
...
function var_0_0:GetMapHeroId()
	if var_0_1 then
		return self:GetQWorldEntityMgr():GetCurrentPlayerId()
	end

	return SandPlayMapCfg[self:GetMapId()].hero_id
end

function var_0_0:OnQWorldLoaded()
	...
	if var_15_0 then
		...
		if var_15_0.cacheTag and SandplayTagCfg[var_15_0.cacheTag] then
			...
			QWorldGoto:Goto(SandplayTagCfg[var_15_0.cacheTag].tag_behaviour, self.entityId, SandplayTagCfg[var_15_0.cacheTag].behaviour_parameters, SandplayTagCfg[var_15_0.cacheTag].passthrough_parameters, SandplayTagCfg[var_15_0.cacheTag].camera_parameters, {
				immediate = true
			})
...
function var_0_0.StartBlackFade(arg_23_0, arg_23_1, arg_23_2, arg_23_3, arg_23_4, arg_23_5)
	QWorldMgr:GetQWorldStoryMgr():GetBlackFade():StartFade({
		...
		done = (arg_23_5 or nil) and function()
			if arg_23_5 then
				arg_23_5()
			end
		end
...
function var_0_0:ActiveControl(arg_26_1)
	if var_0_1 then
		QWorldLuaBridge.CloseUI(not arg_26_1)
		self:ActivePlayerControl(arg_26_1)
	end
end

function var_0_0:ActivePlayerControlWithMain(arg_27_1)
	if var_0_1 and gameContext:GetLastOpenPage() == "qworldMainHome" then
		self:ActivePlayerControl(arg_27_1)
	end
end
...
function var_0_0.ActivePlayerControl(arg_28_0, arg_28_1)
	if var_0_1 then
		if arg_28_1 then
			if not QWorldMgr:GetQWorldPerformMgr():IsInPerform() then
				if not QWorldMgr:GetQWorldPerformMgr():IsInActivityPerform() then
...
function var_0_0:EnableCustomCamera(arg_32_1, arg_32_2, arg_32_3)
	if SandplayCameraGroupCfg[arg_32_1] then
		if SandplayCameraGroupCfg[arg_32_1].camera[arg_32_2 or 1] then
			self:EnableCamera(QWorldCameraFlag.CustomCamera, SandplayCameraGroupCfg[arg_32_1].camera[arg_32_2 or 1], arg_32_3)
		end
	end
end

function var_0_0:ExitCustomCameraByGroup(arg_33_1, arg_33_2)
	if SandplayCameraGroupCfg[arg_33_1] then
		if SandplayCameraGroupCfg[arg_33_1].camera[arg_33_2 or 1] then
			self:ExitCustomCamera(SandplayCameraGroupCfg[arg_33_1].camera[arg_33_2 or 1])
		end
	end
end
