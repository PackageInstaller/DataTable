...
function var_0_0:SetReminder(arg_2_1)
	...
	if arg_2_1 == TimerReminderConst.FATIGUE then
		...
		if var_2_0 <= var_2_1 then
			...
			var_2_3 = (var_2_0 - var_2_1 - 1) * var_2_4 + (CurrencyData:GetLastFatigueRecoverTime() == 0 and var_2_4 or var_2_4 - (manager.time:GetServerTime() - CurrencyData:GetLastFatigueRecoverTime()) % var_2_4) + 28800
		end

		local var_2_5 = manager.time:GetServerTime() + var_2_3
		local var_2_6 = string.split(manager.time:DescCTime(var_2_5, "!%Y %m %d %H %M %S"), " ")
		local var_2_7 = tonumber(var_2_6[4])

		if var_2_7 >= 0 and var_2_7 < 8 then
			var_2_5 = manager.time:Table2ServerTime({
				hour = 8,
				min = 0,
				sec = 0,
				year = tonumber(var_2_6[1]),
				month = tonumber(var_2_6[2]),
				day = tonumber(var_2_6[3])
			})
		end

		if var_0_1[arg_2_1].nextReminderTime == nil then
			var_0_1[arg_2_1].nextReminderTime = var_2_5
		else
			if var_0_1[arg_2_1].nextReminderTime and var_0_1[arg_2_1].nextReminderTime == var_2_5 then
				return
			end
			...
			if var_0_1[arg_2_1].lastReminderTime and var_2_5 <= var_0_1[arg_2_1].lastReminderTime + GameSetting.push_notification_fatigue_cooldown.value[1] * 60 then
		...
		local var_2_8 = PushNotificationCfg[PushNotificationCfg.get_id_list_by_type[arg_2_1][math.round(math.random(#PushNotificationCfg.get_id_list_by_type[arg_2_1]))]]

		if var_2_3 * 1000 >= 1000 then
			GameToSDK.SendMessage(string.format("{\"messageType\" : \"PushNotification\" , \"msgId\" : \"%d\" , \"title\" : \"%s\" , \"message\" : \"%s\" , \"time\" : \"%d\"}", arg_2_1, GetI18NText(var_2_8.title), GetI18NText(var_2_8.content), var_2_3 * 1000))
		end
