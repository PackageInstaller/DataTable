...
function var_0_0:constructImage()
	return {
		itemType = self.content_type,
		spriteUrl = self.image,
		text = self.text,
		event = {
			self.event_type,
			self.event_arg
...
function var_0_0:constructText()
	return {
		itemType = self.content_type,
		text = self.text
	}
...
function var_0_0:constructTitle()
	return {
		itemType = self.content_type,
		text = string.match(self.text, "<title>(%a+)</title>")
	}
end
...
function var_0_0:constructBulletin()
	local var_6_0 = {
		id = self.id,
		type = self.type,
		name = self.title,
		...
		index = self.index,
		star = self.star,
		content = var_0_0.constructSingleBulletin(self.content),
		i18n_info_name = {},
		i18n_info_content = {}
	}

	for iter_6_0, iter_6_1 in pairs(self.i18n_info) do
		if iter_6_1.type == var_0_8.TITLE then
			var_6_0.i18n_info_name[iter_6_1.language] = iter_6_1.string
		elseif iter_6_1.type == var_0_8.CONTENT then
			var_6_0.i18n_info_content[iter_6_1.language] = var_0_0.constructSingleBulletin(iter_6_1.content)
		end
	end
...
function var_0_0:insertBulletin()
	for iter_9_0, iter_9_1 in ipairs(var_0_2) do
		if iter_9_1.id == self.id then
			var_0_2[self.id] = var_0_0.constructBulletin(self)
...
function var_0_0:refreshBulletin()
	for iter_10_0, iter_10_1 in ipairs(self.announcement_list) do
...
function var_0_0:deleteBulletin()
	...
	manager.notify:Invoke(BULLETIN_LIST_UPDATE)
	manager.notify:Invoke(BULLETIN_DELETE, self.id)
end
...
function var_0_0.CheckDailyLoginPopBulletin()
	local var_23_0 = false

	for iter_23_0, iter_23_1 in pairs(var_0_2) do
		if iter_23_1.star == 1 and not getData("BulletinData", "TodayNeedPopSaveID_" .. iter_23_1.id) then
			table.insert(var_0_7, iter_23_1.id)

			if 0 < iter_23_1.startTime then
...
function var_0_0.SaveDailyLoginPopBulletin()
	for iter_24_0, iter_24_1 in ipairs(var_0_7) do
		saveData("BulletinData", "TodayNeedPopSaveID_" .. iter_24_1, true)
	end
