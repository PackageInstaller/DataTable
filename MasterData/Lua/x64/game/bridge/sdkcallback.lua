...
function OnGameSdkCallback(arg_1_0)
	local var_1_0 = cjson.decode(arg_1_0) or {}
	local var_1_1 = var_1_0.callbackType

	print("OnGameSdkCallback messageType", var_1_0.callbackType, var_1_0.code, table.toString(var_1_0))
	...
	if var_1_1 == "Login" then
		...
		_G.LOGIN_TYPE = tostring(var_1_0.channelLoginType) == "3" and 1 or 0
		_G.APP_ID = var_1_0.appId
		_G.YONGSHI_ID = var_1_0.id
		_G.B_SDK_ID = Application.identifier == "com.biligamekr.agonestore" and 8266 or GameToSDK.PLATFORM_ID == 0 and 8264 or GameToSDK.PLATFORM_ID == 1 and 8265 or 8264
	...
	elseif var_1_1 == "shareCallback" then
		manager.notify:Invoke(SHARE_CALLBACK, var_1_0.code)

		if var_1_0.code == 0 then
			ShowTips("SHARE_SUCCESS")
		elseif var_1_0.code == 2 then
			ShowTips("SHARE_CANCEL")
		else
			ShowTips((GetTips("SHARE_FAIL")))
		end
	elseif var_1_1 == "GetAgeTipCallback" then
		local var_1_8 = var_1_0.tip or ""

		if var_1_8 ~= nil and var_1_8 ~= "" then
			manager.notify:Invoke(AGE_TIP_CALLBACK, var_1_8)
		end
	...
	if var_1_2 then
		...
		if var_0_3 and var_0_3[var_1_1] and #var_0_3[var_1_1] > 0 then
			local var_1_9 = table.remove(var_0_3[var_1_1], 1)

			SendMessageToSDKWithCallBack(var_1_9.sendString, var_1_9.waitTag, var_1_9.callBack)
		end
	end
end
		...
		if not var_0_5 then
			...
			RegionServerMgr.instance:UpdateRegionServerInfo(_G.TMP_ACCOUNT_ID or "", function(arg_7_0, arg_7_1)
				...
				if arg_7_0 and not isNil(var_7_2) then
					...
					if not string.isNullOrEmpty(var_7_2.maintainReason) then
						print(string.format("平台下发维护信息 ：%s", var_7_2.maintainReason))

						local var_7_3, var_7_4 = pcall(cjson.decode, var_7_2.maintainReason)

						if var_7_3 then
							local var_7_5 = SettingData:GetCurrentLanguage()

							if var_7_5 == "tc" and var_7_4.tw then
								var_7_1 = var_7_4.tw
							elseif var_7_5 == "zh_cn" and var_7_4.cn then
								var_7_1 = var_7_4.cn
							elseif var_7_4[var_7_5] then
								var_7_1 = var_7_4[var_7_5]
							end
						else
							var_7_1 = var_7_2.maintainReason
						end
					end

					local var_7_6 = gameContext:GetOpenPageHandler("login")
...
function AntiAddiction(arg_16_0)
	ShowMessageBox({
		isTop = true,
		ButtonType = "SingleBtn",
		content = (arg_16_0.sdkTip ~= nil or nil) and (arg_16_0.sdkTip or GetTips("ANTIADDICTION_ONLINE_TIME_LIMIT")),
		OkCallback = function()
			ReconnectLogic.ReconnectError()
		end,
		CancelCallback = function()
...
function NeedGameUserInfo()
	...
	SendMessageToSDK(string.format("{\"messageType\" : \"SendGameUserInfo\", \"accountId\" : \"%s\",\"nickName\" : \"%s\" , \"uId\" : \"%d\", \"maxChapterLevel\" : \"%s\", \"createTime\" : \"%s\", \"userLv\" : \"%d\", \"maxChallengeLv\" : \"%s\", \"channel\" : \"%s\", \"rechargeNum\" : \"%s\", \"region\" : \"%s\", \"mixId\" : \"%s\", \"maxTower\" : \"%s\", \"serverId\" : \"%d\", \"currency\" : \"%d\" }", var_22_1, var_22_2, var_22_3, var_22_4, var_22_5, var_22_6, (BattleTowerStageCfg[var_22_7] or nil) and (BattleTowerStageCfg[var_22_7].name or ""), tostring(_G.CHANNEL_MASTER_ID), tostring(RechargeData:GetTotalRechargeNum()), "暂无区组", tostring(_G.YONGSHI_ID), tostring(TowerData:GetTowerMaxId()), _G.TMP_SERVER_ID, (CurrencyData:GetRechargeDiamond())))
...
function SetSDKUploadImageLimit(arg_28_0, arg_28_1)
	GameToSDK.SetImageMaxCount((string.format("{\"messageType\":\"SetImageMaxCount\",\"maxCount\":\"%d\",\"savePath\":\"%s\"}", arg_28_0, arg_28_1)))
end

function SDKUploadImage(arg_29_0, arg_29_1)
	GameToSDK.UploadImage((string.format("{\"messageType\":\"UploadImage\",\"imagePath\":\"%s\",\"moduleStr\":\"%s\"}", arg_29_1, arg_29_0)))
end

function SDKDownloadImage(arg_30_0, arg_30_1)
	GameToSDK.DownloadImage((string.format("{\"messageType\":\"DownloadImage\",\"path\":\"%s\",\"url\":\"%s\"}", arg_30_1, arg_30_0)))
end
