...
function var_0_0.InitConst()
	...
	for iter_1_0, iter_1_1 in ipairs(NoobVersionCfg.all) do
		if type(NoobVersionCfg[iter_1_1].noob_advance_task_type) == "table" then
			for iter_1_2, iter_1_3 in ipairs(NoobVersionCfg[iter_1_1].noob_advance_task_type) do
...
function var_0_0.GetSignCfg()
	return NoobVersionCfg[var_0_0.GetVersionID()].noob_sign
end

function var_0_0.GetNoobPorgressRewardList()
	return NoobVersionCfg[var_0_0.GetVersionID()].noob_task_progress_reward
end

function var_0_0.GetNoobTaskType()
	return NoobVersionCfg[var_0_0.GetVersionID()].noob_task_type
end
...
function var_0_0.CheckAdvanceTaskOpen()
	...
	if type(NoobVersionCfg[var_9_2].noob_advance_task_open) == "table" then
		for iter_9_0 = #NoobVersionCfg[var_9_2].noob_advance_task_open, 1, -1 do
			if var_0_0.CheckAdvanceTaskTypeOpen(NoobVersionCfg[var_9_2].noob_advance_task_type[iter_9_0]) then
				if iter_9_0 == #NoobVersionCfg[var_9_2].noob_advance_task_open and TaskTools:IsCompletedAllNoobAdvanceTask(NoobVersionCfg[var_9_2].noob_advance_task_type[iter_9_0]) then
					var_9_0 = false
				else
					var_9_0 = true
					var_9_1 = NoobVersionCfg[var_9_2].noob_advance_task_type[iter_9_0]
				end
...
function var_0_0.CheckAdvanceTaskTypeOpen(arg_10_0)
	...
	local var_10_2 = var_0_0.GetAdvanceTaskCfg(arg_10_0)
	local var_10_3 = NoobVersionCfg[var_10_1].noob_advance_task_open[var_10_2.index]

	if var_10_2.index == 1 then
		return var_10_3 <= var_10_0
	elseif TaskTools:IsCompletedAllNoobAdvanceTask(NoobVersionCfg[var_10_1].noob_advance_task_type[var_10_2.index - 1]) then
		return var_10_3 <= var_10_0
	end
end

function var_0_0.GotoNoobAdvanceTaskView()
	JumpTools.OpenPageByJump(var_0_0.GetVersionID() <= 2 and "/noobAdvanceTask" or "/noobAdvanceTaskNew", {
		isEnter = true
	})
end
...
function var_0_0.IsFinishAllActivity()
	local var_13_0 = ActivityNoobData:IsOpenCurVersion() and ActivityNoobData or ActivityNewbieData:IsOpenCurVersion() and false or nil

	return ActivityNewbieData:IsFinishAllActivity()
end
...
function var_0_0.RefreshFirstRechargeRedPoint()
	local var_23_0 = ActivityNoobData:IsOpenCurVersion() and ActivityNoobData or ActivityNewbieData:IsOpenCurVersion() and false or nil
	local var_23_1 = ActivityNewbieData:GetFirstRechargeStatus()
	local var_23_2 = 0

	if not var_0_0.GetRechargeIsOpen(1) then
		var_23_2 = 0
	elseif not var_0_0.GetSelectFirstRecharge(1) or var_23_1.firstGearStatus == 1 then
		var_23_2 = 1
	end

	manager.redPoint:setTip(RedPointConst.NOOB_FIRST_RECHARGE_1, var_23_2)

	local var_23_3 = 0

	if not var_0_0.GetRechargeIsOpen(1) then
		var_23_3 = 0
	elseif not var_0_0.GetSelectFirstRecharge(2) then
		var_23_3 = 1
	elseif not var_23_1.secondGearStatus then
		var_23_3 = 0
	elseif var_23_1.signTimes < #GameSetting.newbie_first_charge_reward_18.value[3] and var_23_1.lastSignTimestamp < manager.time:GetTodayFreshTime() then
		var_23_3 = 1
	end

	manager.redPoint:setTip(RedPointConst.NOOB_FIRST_RECHARGE_2, var_23_3)
end
...
function var_0_0.RefreshMonthlyCardRedPoint()
	local var_27_0 = ActivityNoobData:IsOpenCurVersion() and ActivityNoobData or ActivityNewbieData:IsOpenCurVersion() and false or nil
	local var_27_1 = ActivityNewbieData:GetFirstMonthlyCardStatus()
...
function var_0_0.RefreshBpRedPoint()
	local var_30_0 = ActivityNoobData:IsOpenCurVersion() and ActivityNoobData or ActivityNewbieData:IsOpenCurVersion() and false or nil
	local var_30_1 = ActivityNewbieData:GetBpRewardStatus()
	local var_30_2 = 0

	if not var_0_0.GetRechargeIsOpen(3) then
		var_30_2 = 0
	elseif not var_0_0.GetSelectBpRecharge() or var_30_1 == 1 then
		var_30_2 = 1
	end

	manager.redPoint:setTip(RedPointConst.NOOB_BP_RECHARGE, var_30_2)
end
...
function var_0_0.RefreshAdvanceTaskRedPoint()
	...
	if var_33_0 then
		...
		for iter_33_0, iter_33_1 in ipairs(NoobAdvanceTaskPhaseListCfg[NoobVersionCfg[var_33_2.versionID].noob_advance_task_phase[var_33_2.index]].phase_list) do
...
function var_0_0:RefreshNewBPRecharge()
	if self then
		self:SetSelectedState(var_0_0.GetBpNewRechargeState() and "true" or "false")
	end
end
