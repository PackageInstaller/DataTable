...
function var_0_0.GetNextChapter(arg_4_0, arg_4_1)
	if ChapterCfg[arg_4_0].section_id_list[#ChapterCfg[arg_4_0].section_id_list] == arg_4_1 then
		local var_4_0 = getChapterListByDifficulty(ChapterCfg[arg_4_0].type, ChapterCfg[arg_4_0].difficulty)
		local var_4_1 = table.keyof(var_4_0, arg_4_0)

		if var_4_1 < #var_4_0 then
			if ChapterTools.IsFinishPreChapter(var_4_0[var_4_1 + 1]) and ChapterTools.IsUnlockChapter(var_4_0[var_4_1 + 1]) then
				return var_4_0[var_4_1 + 1]
			end
		end
	end
...
function var_0_0.GetChapterNextStageID(arg_5_0, arg_5_1)
	if BattleChapterStageCfg[arg_5_1] then
		if BattleChapterStageCfg[arg_5_1].next_stage_id then
			if BattleChapterStageCfg[arg_5_1].next_stage_id <= -1 then
				return
			end

			if BattleChapterStageCfg[arg_5_1].next_stage_id ~= 0 then
				return BattleChapterStageCfg[arg_5_1].next_stage_id
			end
		end

		local var_5_0 = BattleChapterStageCfg[arg_5_1].next_unlock_id_list

		if BattleChapterStageCfg[arg_5_1].next_unlock_id_list and type(var_5_0) == "table" and #var_5_0 > 0 then
			if var_5_0[1] ~= 0 then
				return var_5_0[1]
			end
		end
	...
	local var_5_1 = ChapterCfg[arg_5_0].section_id_list
	local var_5_2 = table.keyof(ChapterCfg[arg_5_0].section_id_list, arg_5_1)

	if var_5_2 < #ChapterCfg[arg_5_0].section_id_list then
		if var_5_1[var_5_2 + 1] == GameSetting.travel_skuld_new_ending_stage_id.value[1] and BattleStageData:GetStageData()[var_5_1[var_5_2 + 1]].clear_times < 1 then
			return
		end

		return var_5_1[var_5_2 + 1]
	end
end

function var_0_0.GetActivityStoryNextStageID(arg_6_0, arg_6_1)
	local var_6_0 = ActivityData:GetActivityData(ActivityStoryChapterCfg[arg_6_0].activity_id)
	local var_6_1 = manager.time:GetServerTime()

	if var_6_1 < var_6_0.startTime or var_6_1 > var_6_0.stopTime then
		return
	end

	if ActivityStoryChapterCfg[arg_6_0].activity_id == ActivityConst.OSIRIS_STROY then
...
function var_0_0.GetOsirisNextStage(arg_7_0, arg_7_1, arg_7_2)
	if StoryStageActivityData:GetStageData(arg_7_1)[arg_7_2] then
		...
		if ((var_7_0 or nil) and ChessTools.GetChapterProgress(var_7_0)) < 100 then
			return
		end

		local var_7_2 = ActivityStoryChapterCfg[arg_7_1].stage_list
		local var_7_3 = table.keyof(ActivityStoryChapterCfg[arg_7_1].stage_list, arg_7_2)

		if var_7_3 < #ActivityStoryChapterCfg[arg_7_1].stage_list then
...
function var_0_0.NeedShowContinueBattleWindow(arg_9_0, arg_9_1)
	...
	if var_9_0.auto_next_stage_group <= 10 then
		return true
	elseif ItemTools.getItemNum(var_9_1[1].id) < var_9_1[1].num then
		return true
	end
...
function var_0_0.GetContinueBattleCost(arg_10_0, arg_10_1)
	...
	for iter_10_0, iter_10_1 in ipairs(var_10_0) do
		...
		if not var_10_3 or var_10_3.clear_times <= 0 then
	...
	return mergeReward((formatRewardCfgList(var_10_1))), #var_10_0
end
...
function var_0_0.GetContinueBattleStageList(arg_11_0, arg_11_1)
	...
	while var_11_0.auto_next_stage_group ~= 0 and var_11_0.auto_next_stage_group == var_11_0.auto_next_stage_group do
		table.insert(var_11_2, var_11_1)

		var_11_1 = var_0_0.GetChapterNextStageID(getChapterIDByStageID(var_11_1), var_11_1)
...
function var_0_0.HasStageCost(arg_12_0, arg_12_1)
	...
	if not var_12_1 or var_12_1.clear_times <= 0 then
		if type(var_12_0.extra_cost) == "table" and var_12_0.extra_cost[1] then
			...
			return ItemTools.getItemNum(CurrencyConst.CURRENCY_TYPE_VITALITY) >= var_12_0.cost
		end
	end
...
function var_0_0.IsLockStage(arg_13_0, arg_13_1)
	...
	if arg_13_1 then
		if not ActivityData:GetActivityIsOpen(ChapterCfg[arg_13_0].activity_id) then
			return PlayerData:GetPlayerInfo().userLevel < BattleStageTools.GetStageCfg(ChapterCfg[arg_13_0].type, arg_13_1).level
		end
	end
...
function var_0_0.GetStageArchiveID(arg_16_0)
	if StageArchiveCfg.get_id_list_by_unlock_by_stage_id[arg_16_0] then
		return StageArchiveCfg.get_id_list_by_unlock_by_stage_id[arg_16_0][1]
	else
		return 0
	end
end
...
function var_0_0.InitStageArchiveCfg()
	for iter_17_0, iter_17_1 in ipairs(StageArchivesCollectCfg.all) do
		for iter_17_2, iter_17_3 in ipairs(StageArchivesCollectCfg[iter_17_1].chapter_id) do
...
function var_0_0.GetUnlockStageArchiveList(arg_20_0)
	...
	for iter_20_0, iter_20_1 in ipairs((var_0_0.GetStageArchiveIDListByChapterID(arg_20_0))) do
...
function var_0_0.ReorderStageArchiveListByGroup(arg_22_0)
	...
	for iter_22_0, iter_22_1 in ipairs(arg_22_0) do
		var_22_0[StageArchivesCollectCfg[iter_22_1].tag_id] = var_22_0[StageArchivesCollectCfg[iter_22_1].tag_id] or {}

		table.insert(var_22_0[StageArchivesCollectCfg[iter_22_1].tag_id], iter_22_1)
	end
	...
	for iter_22_4, iter_22_5 in ipairs(var_22_1) do
		...
		var_22_2[iter_22_5] = var_22_0[iter_22_5]
	end
...
function var_0_0.OpenStageArchiveInfo(arg_25_0)
	if StageArchivesCollectCfg[arg_25_0].archive_type == StageConst.STAGE_ARCHIVE_TAG_TYPE.PICTURE_TEXT then
		JumpTools.OpenPageByJump("stageArchivePictureInfo", {
			archiveID = arg_25_0
		})
	elseif StageArchivesCollectCfg[arg_25_0].archive_type == StageConst.STAGE_ARCHIVE_TAG_TYPE.TEXT then
		JumpTools.OpenPageByJump("gameHelpLong", {
			title = StageArchivesCollectCfg[arg_25_0].name,
			content = formatText(StageArchivesCollectCfg[arg_25_0].desc)
		})
	elseif StageArchivesCollectCfg[arg_25_0].archive_type == StageConst.STAGE_ARCHIVE_TAG_TYPE.STORY then
		manager.story:StartStory(StageArchivesCollectCfg[arg_25_0].archive_parameter)
	elseif StageArchivesCollectCfg[arg_25_0].archive_type == StageConst.STAGE_ARCHIVE_TAG_TYPE.MOMO_TALK then
...
function var_0_0.IsHavedStageArchiveRedInChapter(arg_28_0)
	...
	for iter_28_0, iter_28_1 in ipairs((var_0_0.GetStageArchiveIDListByChapterID(arg_28_0))) do
		if var_0_0.IsStageArchiveUnLock(iter_28_1) then
			var_28_0 = var_0_0.IsHaveRedStageArchive(iter_28_1)
