local var_0_0 = {
	IsConditionAchieved = function(arg_1_0, arg_1_1)
		...
		if ConditionCfg[arg_1_0] == nil then
			return true, 0, 0
		end
	...
	GetGuildSpConditionDesc = function(arg_2_0)
		...
		if not ConditionCfg[arg_2_0] then
			return var_2_0
		end

		if ConditionCfg[arg_2_0].type == 9011 then
			var_2_0 = string.format(ConditionCfg[arg_2_0].desc, ConditionCfg[arg_2_0].params[1], ConditionCfg[arg_2_0].params[2], (GetTips(GuildActivitySPConst.NodeType[ConditionCfg[arg_2_0].params[3]])))
		elseif ConditionCfg[arg_2_0].type == 9012 then
			var_2_0 = string.format(ConditionCfg[arg_2_0].desc, ConditionCfg[arg_2_0].params[2])
		elseif ConditionCfg[arg_2_0].type == 9013 then
			var_2_0 = string.format(ConditionCfg[arg_2_0].desc, ConditionCfg[arg_2_0].params[1], ConditionCfg[arg_2_0].params[2])
		elseif ConditionCfg[arg_2_0].type == 9014 then
			var_2_0 = ConditionCfg[arg_2_0].desc
		elseif ConditionCfg[arg_2_0].type == 9015 then
			var_2_0 = string.format(ConditionCfg[arg_2_0].desc, ConditionCfg[arg_2_0].params[1])
		elseif ConditionCfg[arg_2_0].type == 9016 then
			var_2_0 = string.format(ConditionCfg[arg_2_0].desc, ConditionCfg[arg_2_0].params[1])
		elseif ConditionCfg[arg_2_0].type == 9017 then
			var_2_0 = string.format(ConditionCfg[arg_2_0].desc, ConditionCfg[arg_2_0].params[2])
		end
...
function var_0_0.GetCanPlatStory(arg_5_0, arg_5_1, arg_5_2)
	...
	if arg_5_2 == 1 then
		...
		elseif arg_5_1 == GuildActivitySPConst.StoryType.Condition then
			for iter_5_0, iter_5_1 in ipairs(GameSetting.activity_club_sp_middle_plot.value[1]) do
				if IsConditionAchieved(iter_5_1[1]) and not manager.story:IsStoryPlayed(iter_5_1[2]) and not var_0_1[iter_5_1[2]] then
					var_5_0 = iter_5_1[2]
	...
	elseif arg_5_2 == 2 then
		if arg_5_1 == GuildActivitySPConst.StoryType.Condition then
			for iter_5_2, iter_5_3 in ipairs(GameSetting.activity_club_sp_middle_plot.value[2]) do
				if IsConditionAchieved(iter_5_3[1]) and not manager.story:IsStoryPlayed(iter_5_3[2]) and not var_0_1[iter_5_3[2]] then
					var_5_0 = iter_5_3[2]
...
function var_0_0.GetSpActivityStage(arg_8_0)
	if ActivityData:GetActivityData((GuildActivitySPData:GetCurRunActivityID())).startTime + GameSetting.activity_club_sp_phase_two_open.value[1] * 24 * 3600 < manager.time:GetServerTime() then
...
function var_0_0.IsLastBossDefeated(arg_10_0, arg_10_1)
	if arg_10_1 == ActivityClubCfg.all[#ActivityClubCfg.all] and GuildActivityData:GetNodeData(arg_10_1).health <= 0 then
		return true
	end
