...
function var_0_0:PreDealWithAsset(arg_2_1, arg_2_2, arg_2_3)
	local var_2_0 = {}

	if #arg_2_1 > 0 then
		for iter_2_0 = #arg_2_1, arg_2_2 + 1, -1 do
			if arg_2_3 and ViewCfg[arg_2_1[iter_2_0]] and not ViewConst.IGNORE_UNLOAD[arg_2_1[iter_2_0]] then
	...
	for iter_2_1 = arg_2_2 + 1, #self do
		if arg_2_3 and ViewCfg[self[iter_2_1]] then
			table.insert(var_2_1, self[iter_2_1])
		elseif self[iter_2_1] == "home" then
			table.insert(var_2_1, self[iter_2_1])
		end
	end

	var_0_0.PreLoadAsset(var_2_1)
end
...
function var_0_0:PreLoadAsset()
	local var_4_0 = {}
	local var_4_1 = false

	for iter_4_0 = 1, #self do
		if ViewCfg[self[iter_4_0]] then
			for iter_4_1 = 1, #ViewCfg[self[iter_4_0]].need_atlas do
				var_4_0[ViewCfg[self[iter_4_0]].need_atlas[iter_4_1]] = 20
				var_4_1 = true
			end
		end
	end
	...
	for iter_4_2 = 1, #self do
		if self[iter_4_2] == "home" then
			local var_4_4 = {}

			for iter_4_3, iter_4_4 in pairs(SkinSceneActionCfg.all) do
				if HomeSceneSettingData:CanUseScene(SkinSceneActionCfg[iter_4_4].special_scene_id) then
					table.insert(var_4_4, "assetpend_dlc_" .. SkinSceneActionCfg[iter_4_4].skin_id)
				end
			end

			var_4_2 = {}

			for iter_4_5, iter_4_6 in ipairs(var_4_4) do
				if not manager.assetPend:CheckAssetPend(iter_4_6) then
					print("download dlc resource add : " .. iter_4_6)
					table.insert(var_4_2, iter_4_6)
				end
			end
...
function var_0_0.OnDownloadCallBack()
	...
	if var_6_0 then
		...
		print("download dlc resource process " .. tostring(var_6_1) .. "/" .. tostring((math.max(1, #var_0_0.downloadList))))
	end
end
...
function var_0_0.RefreshDownloadProcess()
	...
	LoadingUIManager.inst:UpdateLoadingInfo("", GetTips("DOWNLOADED") .. "(" .. tostring(var_8_0) .. "/" .. tostring(var_8_1) .. ")" .. manager.assetPend:GetDownloadProcessStr(), "")
end

function var_0_0:PreUnLoadAsset()
	local var_9_0 = {}

	for iter_9_0 = 1, #self do
		for iter_9_1 = 1, #ViewCfg[self[iter_9_0]].need_atlas do
			if not table.keyof(var_9_0, ViewCfg[self[iter_9_0]].need_atlas[iter_9_1]) then
				table.insert(var_9_0, ViewCfg[self[iter_9_0]].need_atlas[iter_9_1])
			end
		end
	end
