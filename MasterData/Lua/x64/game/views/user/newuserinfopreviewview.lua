...
function NewUserInfoPreviewView:RefreshUserData()
	...
	if not JumpTools.IsSystemLocked(ViewConst.SYSTEM_ID.DORM) then
		...
		if BackHomeCfg[self.dormID_].type == DormConst.BACKHOME_TYPE.PublicDorm then
			...
			local var_9_3 = HeroRecordCfg.get_id_list_by_hero_id[DormitoryData:GetDormSceneData(self.dormID_).archiveIDList[1]][1]

			if var_9_3 then
				if HeroRecordCfg[var_9_3].name then
					self.dormName_ = string.format(GetTips("DORM_HERO_ROOM_NAME"), HeroRecordCfg[var_9_3].name)
				end
			end
		end
	end
