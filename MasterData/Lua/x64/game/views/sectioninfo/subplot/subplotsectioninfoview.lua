...
function SubPlotSectionInfoView:RefreshData()
	...
	local var_6_2 = BattleStageData:GetStageData()[self.stageID_]

	self.isFirstClear_ = var_6_2 and var_6_2.clear_times <= 0 or true

	if self.isFirstClear_ then
		local var_6_3 = BattleStageTools.GetStageCfg(self.stageType_, self.stageID_)

		self.cost_ = type(var_6_3.extra_cost) == "table" and var_6_3.extra_cost[1] and (var_6_3.extra_cost[2] or 0) or var_6_3.cost or 0
	else
		self.cost_ = 0
	end
