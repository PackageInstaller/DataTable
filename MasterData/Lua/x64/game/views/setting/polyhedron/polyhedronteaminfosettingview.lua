local PolyhedronTeamInfoSettingView = class("PolyhedronTeamInfoSettingView", ReduxView)

function PolyhedronTeamInfoSettingView:SetActive(arg_1_1)
	if self.gameObject_ then
		SetActive(self.gameObject_, arg_1_1)
	end
end

function PolyhedronTeamInfoSettingView:Ctor(arg_2_1, arg_2_2, arg_2_3)
	self.hander_ = arg_2_1
	self.transform_ = arg_2_2.transform
	self.gameObject_ = arg_2_2
	self.stageData = arg_2_3

	manager.notify:RegistListener(COMBO_SKILL_SELECT, self.handler_)
	self:Init()
	self:RefreshSettingUI()
end

function PolyhedronTeamInfoSettingView:Init()
	self:InitUI()
	self:AddUIListener()
end

function PolyhedronTeamInfoSettingView:InitUI()
	self:BindCfgUI()

	self.fightHeroList = LuaList.New(handler(self, self.IndexFightHeroItem), self.m_fightHeroList, PolyhedronTeamInfoSettingHead)
	self.heroList = LuaList.New(handler(self, self.IndexHeroItem), self.m_heroList, PolyhedronTeamInfoSettingHead)
	self.comboController_ = self.m_comboSkillController_:GetController("name")
	self.campController_ = self.m_campAddController_:GetController("name")
end

function PolyhedronTeamInfoSettingView:AddUIListener()
	self:AddBtnListener(self.m_campAddBtn, nil, function()
		local var_6_0, var_6_1 = PolyhedronTools.GetRaceEffect(self.fightHeroArray)

		JumpTools.OpenPageByJump("sectionSelectRaceDescription", {
			raceID = var_6_0,
			sameCamp = var_6_1
		})
	end)
	self:AddBtnListener(self.m_comboSkillBtn, nil, function()
		JumpTools.OpenPageByJump("polyhedronComboSkillSelect", {
			stageID = 0,
			stageType = BattleConst.STAGE_TYPE_NEW.POLYHEDRON,
			activityID = PolyhedronData:GetActivityID(),
			heroList = self.fightHeroArray,
			trialList = {},
			comboSkillID = self:GetComboSkillID()
		})
	end)
end

function PolyhedronTeamInfoSettingView:GetComboSkillID()
	return BattleTeamData:GetComboInfo(BattleConst.STAGE_TYPE_NEW.POLYHEDRON, PolyhedronData:GetActivityID())
end

function PolyhedronTeamInfoSettingView:RefreshSettingUI()
	self.heroViewProxy_ = HeroViewDataProxy.New(HeroConst.HERO_DATA_TYPE.POLYHEDRON)
	self.polyhedronInfo = PolyhedronData:GetPolyhedronInfo()
	self.fightHeroArray = self.polyhedronInfo:GetFightHeroList()
	self.heroArray = {}

	for iter_9_0, iter_9_1 in ipairs((self.polyhedronInfo:GetHeroList())) do
		if not table.indexof(self.fightHeroArray, iter_9_1) then
			table.insert(self.heroArray, iter_9_1)
		end
	end

	self.fightHeroList:StartScroll(3)
	self.heroList:StartScroll(#self.heroArray)
	self.comboController_:SetSelectedIndex(self:GetComboSkillID() ~= 0 and 1 or 0)

	local var_9_0, var_9_1, var_9_2 = PolyhedronTools.GetRaceEffect(self.fightHeroArray)

	self.campController_:SetSelectedIndex(var_9_1 and 1 or 0)
end

function PolyhedronTeamInfoSettingView:IndexFightHeroItem(arg_10_1, arg_10_2)
	arg_10_2:SetFightData(self.polyhedronInfo, self.fightHeroArray[arg_10_1], self.heroViewProxy_)
	arg_10_2:RegistCallBack(function(arg_11_0)
		self:FightHeroClick(arg_11_0)
	end)
end

function PolyhedronTeamInfoSettingView:IndexHeroItem(arg_12_1, arg_12_2)
	arg_12_2:SetData(self.polyhedronInfo, self.heroArray[arg_12_1], self.heroViewProxy_)
	arg_12_2:RegistCallBack(function(arg_13_0)
		self:HeroClick(arg_13_0)
	end)
end

function PolyhedronTeamInfoSettingView:FightHeroClick(arg_14_1)
	if self.polyhedronInfo:GetLeaderHeroId() == arg_14_1 or arg_14_1 == 0 then
		return
	end

	local var_14_0 = clone(self.fightHeroArray)
	local var_14_1 = table.indexof(var_14_0, arg_14_1)

	if var_14_1 then
		table.remove(var_14_0, var_14_1)
		PolyhedronAction.QuerySwitchTeamHero(var_14_0)
	end
end

function PolyhedronTeamInfoSettingView:HeroClick(arg_15_1)
	if #self.fightHeroArray >= 3 then
		ShowTips("POLYHEDRON_TEAM_SET_FULL")

		return
	end

	if self.polyhedronInfo:GetHeroPolyData(arg_15_1):IsDead() then
		ShowTips("POLYHEDRON_TEAM_SET_DEAD")

		return
	end

	local var_15_0 = clone(self.fightHeroArray)

	table.insert(var_15_0, arg_15_1)
	PolyhedronAction.QuerySwitchTeamHero(var_15_0)
end

function PolyhedronTeamInfoSettingView:Dispose()
	PolyhedronTeamInfoSettingView.super.Dispose(self)

	self.transform_ = nil
	self.gameObject_ = nil

	ComboSkillData:SetNeedRefresh(true)

	if self.fightHeroList then
		self.fightHeroList:Dispose()

		self.fightHeroList = nil
	end

	if self.heroList then
		self.heroList:Dispose()

		self.heroList = nil
	end

	manager.notify:RemoveListener(COMBO_SKILL_SELECT, self.handler_)

	self.handler_ = nil
end

function PolyhedronTeamInfoSettingView:OnPolyhedronSwitchHero()
	self:RefreshSettingUI()
end

return PolyhedronTeamInfoSettingView
