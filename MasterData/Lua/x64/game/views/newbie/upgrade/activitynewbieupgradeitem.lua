...
function ActivityNewbieUpgradeView:Ctor(arg_1_1, arg_1_2)
...
function ActivityNewbieUpgradeView:AddListeners()
	self:AddBtnListener(self.button_, nil, function()
		local var_3_0 = GameSetting.levelup_reward.value[self.index_][1]

		if self.isFinish_ and not table.keyof(ActivityNewbieData:GetUpgradeFinishList(), GameSetting.levelup_reward.value[self.index_][1]) then
			ActivityNewbieAction.ReceiveUpgradeReward(GameSetting.levelup_reward.value[self.index_][1], function(arg_4_0)
				if isSuccess(arg_4_0.result) then
					getReward2(arg_4_0.reward_list)
					ActivityNewbieData:SetUpgradeFinish(var_3_0)
...
function ActivityNewbieUpgradeView:RefreshUI()
	...
	if self.isFinish_ then
		if table.keyof(ActivityNewbieData:GetUpgradeFinishList(), var_6_0) then
			self:ReceiveReward()
		else
...
function ActivityNewbieUpgradeView:ReceiveReward()
