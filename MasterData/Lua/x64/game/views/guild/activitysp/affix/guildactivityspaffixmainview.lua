local GuildActivitySPAffixMainView = class("GuildActivitySPAffixMainView", ReduxView)

function GuildActivitySPAffixMainView:UIName()
	return "UI/GuildActivitySPUI/GuildActivitySPtalentUI"
end

function GuildActivitySPAffixMainView:UIParent()
	return manager.ui.uiMain.transform
end

function GuildActivitySPAffixMainView:OnCtor()
	return
end

function GuildActivitySPAffixMainView:Init()
...
function GuildActivitySPAffixMainView:InitUI()
	self:BindCfgUI()
	...
	for iter_5_0 = 1, self.racePanelTrans_.childCount do
		self.raceItemList_[iter_5_0] = GuildActivitySPAffixRaceItem.New(self.racePanelTrans_:GetChild(iter_5_0 - 1))
	...
	for iter_5_1 = 1, self.affixPanelTrans_.childCount do
		self.affixItemList_[iter_5_1] = GuildActivitySPAffixItem.New(self.affixPanelTrans_:GetChild(iter_5_1 - 1))
	...
	for iter_5_2 = 1, self.affixInfoPanelTrans_.childCount do
		self.affixInfoItemList_[iter_5_2] = GuildActivitySPAffixInfoItem.New(self.affixInfoPanelTrans_:GetChild(iter_5_2 - 1))
	end
end

function GuildActivitySPAffixMainView:AddUIListener()
	self:AddBtnListener(self.upgradeBtn_, nil, function()
		...
		GuildActivitySPAction.UpgradeAffix(self.curAffixID_, function(arg_8_0, arg_8_1)
			if isSuccess(arg_8_0.result) then
				manager.notify:Invoke(MATERIAL_MODIFY, self.currencyID_)
				GuildActivitySPData:UpgradeAffix(self.activityID_, self.curAffixID_)
				...
				local var_8_0, var_8_1 = GuildActivitySPData:CheckRateUpgrade(self.activityID_)
...
function GuildActivitySPAffixMainView:AddEventListeners()
	return
end

function GuildActivitySPAffixMainView:OnEnter()
	self:InitBar()
...
function GuildActivitySPAffixMainView:InitBar()
...
function GuildActivitySPAffixMainView:OnTop()
	return
end

function GuildActivitySPAffixMainView:OnExit()
...
function GuildActivitySPAffixMainView:Dispose()
	...
	GuildActivitySPAffixMainView.super.Dispose(self)
end

function GuildActivitySPAffixMainView:RefreshUI()
	self:RefreshRaceItem()
end

function GuildActivitySPAffixMainView:RefreshRaceItem()
	...
	self.curRaceID_ = GuildActivitySPData:GetSelectRaceID(self.activityID_) or self.raceIDList_[1]

	self:OnSelectRaceItem(self.curRaceID_)
end

function GuildActivitySPAffixMainView:OnSelectRaceItem(arg_17_1)
	GuildActivitySPData:SetSelectRaceID(self.activityID_, arg_17_1)
...
function GuildActivitySPAffixMainView:RefreshAffixItem()
	self.affixIDList_ = TalentTreeCfg.get_id_list_by_activity_id_and_race[self.activityID_][self.curRaceID_]
	self.unlockAffixIDList_ = GuildActivitySPData:GetUnLockAffixList()
	...
	local var_18_3 = GuildActivitySPData:GetSelectAffixID(self.activityID_, self.curRaceID_)

	self.curAffixID_ = GuildActivitySPData:GetSelectAffixID(self.activityID_, self.curRaceID_) or self.affixIDList_[1]
...
function GuildActivitySPAffixMainView:OnSelectAffixItem(arg_19_1, arg_19_2)
	GuildActivitySPData:SetSelectAffixID(self.activityID_, self.curRaceID_, arg_19_1)
...
function GuildActivitySPAffixMainView:RefershAffixInfo()
	...
	self.titleText_.text = TalentTreeCfg[self.curAffixID_].name
	self.descText_.text = TalentTreeCfg[self.curAffixID_].desc
end

function GuildActivitySPAffixMainView:RefreshUpgradeBtn()
