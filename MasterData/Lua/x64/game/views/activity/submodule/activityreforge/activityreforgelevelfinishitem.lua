local ActivityReforgeLevelFinishItem = class("ActivityReforgeLevelFinishItem", ReduxView)

function ActivityReforgeLevelFinishItem:OnCtor(arg_1_1)
...
function ActivityReforgeLevelFinishItem:Init()
	self:InitUI()

	self.barGoList_ = {}
end

function ActivityReforgeLevelFinishItem:InitUI()
	self:BindCfgUI()
end

function ActivityReforgeLevelFinishItem:SetData(arg_4_1, arg_4_2, arg_4_3, arg_4_4, arg_4_5, arg_4_6)
	self.type_ = arg_4_1
...
function ActivityReforgeLevelFinishItem:RefreshUI()
	...
	for iter_5_0 = 1, self.barNum_ do
		...
		self.barGoList_[iter_5_0].transform.sizeDelta = Vector2.New(var_5_3, self.maxPalyerNum_ > 0 and self.allPlayerData_[iter_5_0] / self.maxPalyerNum_ * self.barPanelTrans_.rect.height or 0)
		self.barGoList_[iter_5_0].transform.anchoredPosition = Vector3.New((iter_5_0 - 1) * self.barGoList_[iter_5_0].transform.rect.width, 0, 0)
	end
	...
	for iter_5_1, iter_5_2 in pairs(self.barGoList_) do
		if iter_5_1 <= self.barNum_ then
			...
			self.barGoList_[iter_5_1].transform.localScale = Vector3.New(1, 1, 1)
		else
			SetActive(self.barGoList_[iter_5_1], false)
		end
	end

	local var_5_5 = self.curData_ / (self.barNum_ * self.rangeLength_)

	if self.curData_ / (self.barNum_ * self.rangeLength_) > 1 then
		var_5_5 = 1
	end

	self.bestLineTrans_.transform.anchoredPosition = Vector3.New(var_5_5 * self.barPanelTrans_.rect.width, 0, 0)
	self.curPosText_.text = string.format(GetTips("ACTIVITY_REFORGE_CUR_RANK"), math.ceil(var_5_5 * 100) .. "%")
end

function ActivityReforgeLevelFinishItem:Dispose()
	...
	ActivityReforgeLevelFinishItem.super.Dispose(self)
end

return ActivityReforgeLevelFinishItem
