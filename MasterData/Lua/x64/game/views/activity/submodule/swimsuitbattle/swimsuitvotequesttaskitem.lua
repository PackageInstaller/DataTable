local SwimsuitVoteQuestTaskItem = class("SwimsuitVoteQuestTaskItem", ReduxView)

function SwimsuitVoteQuestTaskItem:OnCtor(arg_1_1)
...
function SwimsuitVoteQuestTaskItem:Init()
	self:InitUI()
	self:AddUIListener()
...
function SwimsuitVoteQuestTaskItem:InitUI()
	self:BindCfgUI()
end

function SwimsuitVoteQuestTaskItem:AddUIListener()
	self:AddBtnListener(self.receiveBtn_, nil, function()
		TaskAction:SubmitTask(self.taskID_)
	end)
end

function SwimsuitVoteQuestTaskItem:IndexItem(arg_6_1, arg_6_2)
...
function SwimsuitVoteQuestTaskItem:SetData(arg_8_1, arg_8_2)
	...
	self:UpdateView(arg_8_2 < SwimsuitBattleData:GetCurVoteRound())
	self:AddTimer()
end

function SwimsuitVoteQuestTaskItem:UpdateView(arg_9_1)
	...
	if arg_9_1 then
		...
		elseif var_9_1 then
			...
			self.btnController_:SetSelectedState("over")
			self.greyController_:SetSelectedState("false")
		end
	elseif var_9_0 then
		self.btnController_:SetSelectedState("complete")
...
function SwimsuitVoteQuestTaskItem:AddTimer()
	if self.timer_ then
		self.timer_:Stop()
...
function SwimsuitVoteQuestTaskItem:Dispose()
	...
	SwimsuitVoteQuestTaskItem.super.Dispose(self)
end

return SwimsuitVoteQuestTaskItem
