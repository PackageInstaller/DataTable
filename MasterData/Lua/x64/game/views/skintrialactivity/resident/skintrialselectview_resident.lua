local SkinTrialSelectView_Resident = class("SkinTrialSelectView_Resident", ReduxView)

function SkinTrialSelectView_Resident:UIName()
	return SkinTrialTools.GetSelectViewUIName(self.params_.activityID)
end

function SkinTrialSelectView_Resident:UIParent()
	return manager.ui.uiMain.transform
end

function SkinTrialSelectView_Resident:Init()
	self:InitUI()
	self:AddListeners()
end

function SkinTrialSelectView_Resident:InitUI()
	self:BindCfgUI()
	self:InitStaticText()
	self:InitController()
	self:InitRewardItem()
...
function SkinTrialSelectView_Resident:InitStaticText()
...
function SkinTrialSelectView_Resident:InitController()
	self.rewardController_ = self.rewardControllerEx_:GetController("reward")
end

function SkinTrialSelectView_Resident:InitRewardItem()
...
function SkinTrialSelectView_Resident:OnEnter()
...
function SkinTrialSelectView_Resident:AddListeners()
...
function SkinTrialSelectView_Resident:OnBackBtnClick()
	SkinTrialData:SaveSkinTrialMainScrollPos(SkinTrialData:GetSkinTrialMainScrollPos("Global"), "Back")
	self:Back()
end

function SkinTrialSelectView_Resident:OnExit()
	manager.windowBar:HideBar()
	self:StopTimer()
	self:RemoveAllEventListener()
end

function SkinTrialSelectView_Resident:Dispose()
	...
	SkinTrialSelectView_Resident.super.Dispose(self)
end

function SkinTrialSelectView_Resident:RefreshUI()
	self:RefreshSkinInfo()
	self:RefreshRoleImage()
	self:RefreshReward()
end

function SkinTrialSelectView_Resident:RefreshSkinInfo()
...
function SkinTrialSelectView_Resident:RefreshRoleImage()
...
function SkinTrialSelectView_Resident:RefreshReward()
	self:RefreshRewardState()
	self:RefreshRewardItem()
end

function SkinTrialSelectView_Resident:RefreshRewardState()
...
function SkinTrialSelectView_Resident:RefreshRewardItem()
...
function SkinTrialSelectView_Resident:AddTimer()
...
function SkinTrialSelectView_Resident:StopTimer()
	if self.timer_ then
		self.timer_:Stop()
...
function SkinTrialSelectView_Resident:SaveRoute()
...
function SkinTrialSelectView_Resident:OnSkinTrialUpdate(arg_28_1)
	self:RefreshRewardState()
end

return SkinTrialSelectView_Resident
