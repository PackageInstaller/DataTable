	...
	for iter_2_0, iter_2_1 in ipairs(self.actionData.addBuffDataList) do
		...
		if iter_2_1.targetType == AutoChessConst.BUFF_TARGET_TYPE.CHESS then
			...
			if var_2_3 and var_2_3.keyword_type ~= AutoChessConst.BUFF_KEYWORD_TYPE.NONE then
				...
				if var_2_9 then
					local var_2_10 = AutoChessBattleRenderer.GetInstance():GetChessItemByUniqueId((var_2_9.sourceType == AutoChessConst.BUFF_TARGET_TYPE.CHESS or nil) and (var_2_9.sourceUid or iter_2_1.targetUid))
					local var_2_11 = var_2_10 and var_2_10:GetBuffKeywordConfig(var_2_3.keyword_type)

					if var_2_11 ~= nil then
						AutoChessRenderControllerFactory.GetInstance():GetRenderController(AutoChessConst.ACTION_RENDER_CONTROLLER_TYPE.ANIMATOR):PlayAnim(var_2_11.animator, var_2_11.useAnimName)

						if not string.isNullOrEmpty(var_2_11.useAnimSoundEffect) then
							manager.audio:PlayEffect("minigame_activity_4_5", var_2_11.useAnimSoundEffect, "")
						end
					end
				end
			end
		...
		if AutoChessTools.IsPrepareStatus() and not AutoChessData:GetIsInUseProp() and AutoChessItemCfg[iter_2_1.targetBuffData.buffId] and AutoChessItemCfg[iter_2_1.targetBuffData.buffId].icon then
			...
			if AutoChessTools.IsTargetProp(AutoChessItemCfg[iter_2_1.targetBuffData.buffId].id) then
				...
				local var_2_12 = AutoChessBattleRenderer.GetInstance():GetPrepareView()

				if var_2_12 then
					var_2_12:PlayPropAnimator(AutoChessItemCfg[iter_2_1.targetBuffData.buffId].icon)
				end
			end
		end
	end
