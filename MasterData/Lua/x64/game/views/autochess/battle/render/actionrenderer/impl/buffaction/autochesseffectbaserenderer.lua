...
function AutoChessEffectBaseRenderer:PlayBuffInvokeAnim()
	...
	local var_2_0 = AutoChessData:GetBuffData(self.invokeBuffUid).buffId

	self.invokeBuffChessItemView = AutoChessBattleRenderer.GetInstance():GetChessItemByUniqueId(self.targetUid)

	if not AutoChessBuffCfg[var_2_0] or not self.invokeBuffChessItemView then
		self:OnBuffInvokeAnimEnd()

		return
	end

	if AutoChessBuffCfg[var_2_0].action_type == AutoChessConst.SPECIAL_BUFF_TYPE.REMOVE_CHESS_BUFF_ACTION_TYPE then
		self:OnBuffInvokeAnimEnd()
	elseif AutoChessBuffCfg[var_2_0].keyword_type == AutoChessConst.BUFF_KEYWORD_TYPE.DEATH_WHISPE then
