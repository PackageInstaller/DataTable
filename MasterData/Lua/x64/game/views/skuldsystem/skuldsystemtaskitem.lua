local SkuldSystemTaskItem = class("SkuldSystemTaskItem", ReduxView)

function SkuldSystemTaskItem:OnCtor(arg_1_1)
	self.rewardItems_ = {}
	self.rewardItemGos_ = {}
...
function SkuldSystemTaskItem:RefreshTaskItem(arg_2_1)
	if not arg_2_1 then
		return
	end
...
function SkuldSystemTaskItem:OnExit()
	self.isShow_ = false
end

function SkuldSystemTaskItem:Dispose()
	...
	SkuldSystemTaskItem.super.Dispose(self)
end

function SkuldSystemTaskItem:InitUI()
	self:BindCfgUI()
...
function SkuldSystemTaskItem:AddListeners()
...
function SkuldSystemTaskItem:RemoveListeners()
	self.goBtn_.onClick:RemoveAllListeners()
	self.receiveBtn_.onClick:RemoveAllListeners()
end

function SkuldSystemTaskItem:RefreshUI()
...
function SkuldSystemTaskItem:RefreshProgress()
...
function SkuldSystemTaskItem:OnClickCommonItem(arg_12_1)
