local StrategyMatrixProcessShopItem = class("StrategyMatrixProcessShopItem", MatrixProcessShopItem)

function StrategyMatrixProcessShopItem:initUI()
	self:BindCfgUI()

	self.rewardItem = StrategyMatrixProcessRewardItem.New(self.m_reward)
	self.discountController = ControllerUtil.GetController(self.transform_, "discount")
end

function StrategyMatrixProcessShopItem:AddUIListener()
	self:AddBtnListener(self.m_priceBtn, nil, function()
		if StrategyMatrixData:GetMatrixCoint(self.matrix_activity_id) < self.priceNum then
...
function StrategyMatrixProcessShopItem:SetMatrixActivityId(arg_6_1)
	self.rewardItem:SetMatrixActivityId(arg_6_1)

	self.matrix_activity_id = arg_6_1
end

function StrategyMatrixProcessShopItem:OnRewardAction()
		...
		if self:GetAstrolabeMaxCount() <= #var_7_2:GetAstrolabeList() then
			JumpTools.OpenPageByJump("strategyMatrixProcessAstrolabeReplace", {
				...
				callback = function(arg_8_0)
					StrategyMatrixAction.BuyShopItem(self.matrix_activity_id, self.shopIndex, var_7_2:GetStandardId(), arg_8_0)
				end
			})
		else
			StrategyMatrixAction.BuyShopItem(self.matrix_activity_id, self.shopIndex, var_7_2:GetStandardId(), 0)
		end
		...
		if var_7_3 == 0 then
			...
			if var_7_5 ~= 0 then
				JumpTools.OpenPageByJump("strategyMatrixProcessWeaponReplace", {
					...
					weaponId = self.id,
					callback = function()
						StrategyMatrixAction.BuyShopItem(self.matrix_activity_id, self.shopIndex, var_7_4:GetStandardId(), var_7_5)
					end
				})
			else
				StrategyMatrixAction.BuyShopItem(self.matrix_activity_id, self.shopIndex, var_7_4:GetStandardId(), 0)
			end
		end
	else
		StrategyMatrixAction.BuyShopItem(self.matrix_activity_id, self.shopIndex, 0, 0)
	end
end

function StrategyMatrixProcessShopItem:GetHeroData(arg_10_1)
	return StrategyMatrixData:GetHeroDataNew(arg_10_1)
end

function StrategyMatrixProcessShopItem:GetAstrolabeMaxCount()
	return StrategyMatrixData:GetAstrolabeMaxCount(self.matrix_activity_id)
end

function StrategyMatrixProcessShopItem:GetSwitchItemIcon(arg_12_1)
	if arg_12_1 == 26 then
...
return StrategyMatrixProcessShopItem
