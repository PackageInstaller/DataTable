...
function var_0_0.ChangeCurSystem(arg_8_0, arg_8_1, arg_8_2, arg_8_3)
	...
	if arg_8_3 then
		...
		if var_0_1 ~= var_0_1 and var_0_1 ~= DormEnum.DormSystemType.Main and var_0_1 ~= DormEnum.DormSystemType.Illu then
			var_0_3[var_0_1]:ExitCurSystem()
		end
	...
	if arg_8_1 then
		...
		if BackHomeCfg[arg_8_1].type == DormConst.BACKHOME_TYPE.Canteen then
			var_0_1 = DormEnum.DormSystemType.Canteen
		elseif BackHomeCfg[arg_8_1].type == DormConst.BACKHOME_TYPE.PublicDorm then
			var_0_1 = DormEnum.DormSystemType.Dormitory
		elseif BackHomeCfg[arg_8_1].type == DormConst.BACKHOME_TYPE.PrivateDorm then
			var_0_1 = DormEnum.DormSystemType.Dormitory
		end
...
function var_0_0:GotoBackHomeScene(arg_9_1)
	if arg_9_1 then
		BackHomeAction:CheckToEnterDormScene(arg_9_1)
		self:ChangeCurSystem(arg_9_1)
	end
end

function var_0_0:GotoBackHomeCricketStage(arg_10_1, arg_10_2)
	if arg_10_1 == BackHomeCricketConst.ROOM_TYPE.TRAIN then
		DormMinigame.Launch("WT_RK", nil, "idolTraineeCamp")
		self:ChangeCurSystem(nil, true)
		DormData:SetCurrentScene(nil)
	...
	if arg_10_2 and var_0_1 == DormEnum.DormSystemType.CricketBattle then
		DormMinigame.Launch(IdolStageSceneCfg[(arg_10_1 == BackHomeCricketConst.ROOM_TYPE.PVP_STAGE and IdolTraineePvpBattleCfg or IdolTraineePveBattleCfg)[arg_10_2].scene_id].scene_name, DanceGameBridge)
	end
end

function var_0_0:GotoBackHomeIlluScene()
	DormIllu.Launch("X100_Illu")
	self:ChangeCurSystem(nil, false, true)
	DormData:SetCurrentScene(nil)
end
