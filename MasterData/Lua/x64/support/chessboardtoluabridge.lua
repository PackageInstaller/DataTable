function OnEnterChessBoardScene()
	...
	SceneManager.SetActiveScene((SceneManager.GetSceneByName(ChessBoardLevelCfg[var_1_0.mapId].scene_id)))
	...
	if manager.ChessBoardManager:GetHeroAttribute("CHESSBOARD_ATTRIBUTE_ID_FIRST_MAP") == 0 then
		local var_1_1 = ChessBoardLevelCfg[ChessBoardData:GetCurrentChessBoardMapID()].eventpool_id

		if var_1_1 > 0 and not table.isEmpty(ChessBoardEventPoolCfg[var_1_1].player_params) then
	...
	elseif manager.ChessBoardManager:GetHeroAttribute("CHESSBOARD_ATTRIBUTE_ID_FINISH") == 1 then
		...
		local var_1_2 = manager.ChessBoardManager:GetCurChanceInfo()

		if manager.ChessBoardManager:IsBattleBack() then
			manager.ChessBoardManager:BattleFinish()
		elseif var_1_2.chanceID > 0 then
			if var_1_2.isUnLock == 1 then
				ChessBoardTools.OpenChanceView(var_1_2.chanceID, true)
			else
