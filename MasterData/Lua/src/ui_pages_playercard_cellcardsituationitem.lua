local this = class("cellCardSituationItem", G_UIModuleBase)
...
function this:onCoverChanged(id, tex)
  if id == self.bind.id and tex and tex and self.bindComponents.rawImg then
...
function this:showImg()
  ...
  L_SituationManager:getCoverTex(self.bind.id, function(tex)
    if tex and self.bindComponents.rawImg then
      self.bindComponents.rawImg.texture = tex
