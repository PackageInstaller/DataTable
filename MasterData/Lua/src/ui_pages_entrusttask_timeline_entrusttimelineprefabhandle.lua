local this = class("entrustTimelinePrefabHandle", require("ui.manager.hero.timeline.timelinePrefabHandle"))
...
function this:loadTimelinePrefabAsync(timelineName, onLoaded)
  ...
  local function onResourceLoaded(id, prefab)
    if self.isDispose then
      if prefab then
        Unity.GameObject.Destroy(prefab)
      end
      return
    end
    prefab.transform.parent = self._root.transform
...
function this:refreshPosRot(prefab)
  if self.prefabPos then
    L_Vector3.setLocalPos(prefab.transform, L_Vector3.getTemp(self.prefabPos.x, self.prefabPos.y, self.prefabPos.z))
    self.prefabPos = nil
  else
    L_Vector3.setLocalPos(prefab.transform, L_Vector3.getTemp())
  end
...
function this:setCameraTimelineActive(isActive)
  local state = self._timelineStateMap[self._timelineName]
  if state then
    local cameraTrans = state.transform:Find("[assets]/cam_G")
    for i = 0, cameraTrans.childCount - 1 do
      local child = cameraTrans:GetChild(i).gameObject
      child:ActiveTrans(isActive)
    end
  end
end
...
function this:setTimelineTrans(pos, rot)
  local prefab = self._prefabMap[self._timelineName]
  L_Vector3.setLocalPos(prefab.transform, L_Vector3.getTemp(pos.x, pos.y, pos.z))
  L_Vector3.setRot(prefab.transform, L_Vector3.getTemp(rot.x, rot.y, rot.z))
end
