local module = class("cellGoodSpecialItem", G_UIModuleBase)
...
function module:refresh()
  ...
  elseif self.itemType == L_Const.resType.pet or self.itemType == L_Const.resType.petCustomized then
    self.bind.go_rect_pet = true
    self.bind.img_pet = _goodsTpl:getIcon(self.goodsCfg)
  elseif self.itemType == L_Const.resType.soulEssence then
    self.bind.go_item = true
    local tpl = _soulessenceTpl:getTplById(self.itemId)
    self.bind.itemIcon = _soulessenceTpl:getAvatarTexture(tpl, 1)
  else
    self.bind.go_item = true
