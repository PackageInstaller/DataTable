local this = class("moduleKiBoTask", G_UIModuleBase)
...
function this.bind()
  return {
    ...
    active_notFinish = true,
    txt_finishCondition = "",
    noviceTaskList_mobile = {
      moduleName = "pages/activityTask/cellKiBoTaskItem"
    },
    noviceTaskList_pc = {
      moduleName = "pages/activityTask/cellKiBoTaskItem"
    },
...
function this:open()
  ...
  if L_DeviceTpl:getIsMobile() then
    self.bindComponents.mobile_layout.padding.left = 0
    self.bindComponents.mobile_layout.spacing = -15
  end
...
function this:refreshModule()
  ...
  self.bindComponents.ani_descPart:Play("anim_KT_change_right")
  self.parent.bindComponents.ani_pageNoviceTask:Stop()
  if L_DeviceTpl:getIsPc() then
    self.parent.bindComponents.ani_pageNoviceTask:Play("anim_KT_in")
  else
    self.parent.bindComponents.ani_pageNoviceTask:Play("anim_KT_in_mobile")
  end
end
...
function this:refreshNoviceTaskGroupContent(isFromLeftRightChange)
  ...
  if self.data.currentTaskGroupId == 201 then
  ...
  self.isUnlock = true
  if isTaskGroupComplete and self.data.currentTaskGroupId + 1 <= self.data.maxTaskGroupId then
    local nextGroupID = self.data.currentTaskGroupId + 1
    local lockIndex = 1
    local results = {}
    self.isUnlock, results, lockIndex = L_ConditionManager:isComplete(_noviceTaskGroupTpl:getTaskGroupUnLockCondition(nextGroupID))
    self.bind.txt_finishCondition = _noviceTaskGroupTpl:getTaskGroupLockTips(nextGroupID, lockIndex)
  end
  self.bind.active_canFinishCondition = false
  if not self.isUnlock and L_NoviceTaskStore:getTaskGroupIfFinishedById(self.data.currentTaskGroupId) == true then
    self.bind.go_btnNoviceTaskGroupTitleRight = false
    self.bind.active_canFinishCondition = true
  end
  ...
  self.bind.active_notFinish = self.isUnlock and (not isTaskGroupComplete or L_NoviceTaskStore:getTaskGroupIfFinishedById(self.data.currentTaskGroupId) == true)
  ...
  self.bind.txt_canFinish = isTaskGroupComplete and L_NoviceTaskStore:getTaskGroupIfFinishedById(self.data.currentTaskGroupId) == true and L_WordsTpl:getValue("ui_novicetask_alreadyreceived") or L_WordsTpl:getValue("puzzle_interact_2")
  local base_data = {}
...
function this:playListRefreshVFX()
  ...
  if self._waitForRewardClose_groupReward then
    if self.data.currentTaskGroupId < L_NoviceTaskStore:getMaxTaskGroupId(self.data.currTypeTaskId) then
      if self.bind.currTypeTaskId ~= 1 and not self.isUnlock then
        self:refreshNoviceTaskAllImmediately()
        return
      end
      self:switchNoviceTaskGroup(SwitchNoviceTaskGroupDirection.TURN_RIGHT)
      ...
      L_FlyMsgManager:showNormalMsgByKey("notice_novicetask_unlock", {
        [0] = noviceTaskGroupTpl:getTaskGroupTiltleById(self.data.currentTaskGroupId)
      })
    else
      self:refreshNoviceTaskAllImmediately()
    end
...
function this:switchNoviceTaskGroup(switchDirection)
  if switchDirection == SwitchNoviceTaskGroupDirection.TURN_LEFT then
    ...
    self.bindComponents.ani_descPart:Stop()
    if L_DeviceTpl:getIsPc() then
      self.bindComponents.ani_descPart:Play("anim_KT_change_left")
    else
      self.bindComponents.ani_descPart:Play("anim_KT_change_left_mobile")
    end
  else
    self.data.currentTaskGroupId = self.data.currentTaskGroupId + 1
    self.bindComponents.ani_descPart:Stop()
    if L_DeviceTpl:getIsPc() then
      ...
      self.bindComponents.ani_descPart:Play("anim_KT_change_right_mobile")
    end
  end
  ...
  self.parent.bindComponents.ani_pageNoviceTask:Stop()
  if L_DeviceTpl:getIsPc() then
    self.parent.bindComponents.ani_pageNoviceTask:Play("anim_KT_change")
  else
    self.parent.bindComponents.ani_pageNoviceTask:Play("anim_KT_change_mobile")
  end
end
...
function this:playListCellInVFX(delayTime, intervalTime)
  ...
  if L_DeviceTpl:getIsPc() then
    for i, v in pairs(self.modules.noviceTaskList_pc) do
      local cell = v
      cell:playCellOutVFX()
      L_TimerManager:stopTimer(self, string.format("VFX_showKiBoTaskContentCell_%d", i))
    end
  ...
  L_TimerManager:newOrResetTimer(self, "VFX_showNoviceTaskContent", function()
    if L_DeviceTpl:getIsPc() then
      for i, v in pairs(self.modules.noviceTaskList_pc) do
        local index = i
        L_TimerManager:newOrResetTimer(self, string.format("VFX_showNoviceTaskContentCell_%d", index), function()
          local cell = v
