  ...
  if rawget(val, 1) ~= nil or next(val) == nil then
    ...
    for k in pairs(val) do
      if type(k) ~= "number" then
        errorf("invalid table: mixed or invalid key types")
      end
      n = n + 1
    end
    if n ~= #val then
      errorf("invalid table: sparse array")
    end
...
local function encode_number(val)
  if val ~= val or val <= -math.huge or val >= math.huge then
    errorf("unexpected number value '" .. tostring(val) .. "'")
  end
  return tostring(val)
end
...
local function decode_error(str, idx, msg)
  ...
  errorf(string.format("%s at line %d col %d", msg, line_count, col_count))
end
...
local function codepoint_to_utf8(n)
  ...
  errorf(string.format("invalid unicode codepoint '%x'", n))
end
...
function json.decode(str)
  if type(str) ~= "string" or str == "" then
    errorf("expected argument of type string, got " .. type(str))
    return nil
  end
