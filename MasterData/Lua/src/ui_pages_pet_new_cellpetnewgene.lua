local module = class("cellPetNewGene", G_UIModuleBase)

function module:ctor(...)
  module.super.ctor(self, ...)
  self.playPreviewFxOnOpen = false
  self.data = {
    id = nil,
    guid = nil,
    rarity = {
...
function module.bind()
  return {
    ...
    active_InnateLock = false,
    active_previewFx = false,
    anim_root = ""
  }
end
...
function module:created(bindData, emitMethods, parent)
  self.playPreviewFxOnOpen = bindData and bindData.playPreviewFx == true
  module.super.created(self, bindData, emitMethods, parent)
end

function module:open()
  if not self.playPreviewFxOnOpen then
    return
  end
  self.playPreviewFxOnOpen = false
  self:playPreviewFx()
...
function module:refreshData(data)
  ...
  self.bind.img_Delete = data.isRemoved == true or data.img_Delete == true
  self:refreshInfo()
end

function module:refreshDataWithSkillId(skillId, isRemoved)
  ...
  self.bind.img_Delete = isRemoved == true
  self:refreshInfo()
end
...
function module:playPreviewFx()
  self.bind.active_previewFx = true
  self.bind.anim_root = "anim_petfeed_attshow"
end

function module:close()
  self.playPreviewFxOnOpen = false
  self.bind.active_previewFx = false
end

function module:refreshInfo()
  local isPreviewNewGene = self.bind.isPreviewNewGene == true
  if self.bind.rectPos then
    self.bindComponents.geneTrans.localPosition = self.bind.rectPos
  end
  self.bind.geneNameText = self.bind.skillName
  ...
  if not math.isEmpty(self.bind.geneId) then
    ...
    if petItem then
      ...
      if not self.bind.hideNewTag then
        isNewTag = isPreviewNewGene
  ...
  local isInnateGene = L_PetStore:isInnateGene(self.bind.genePos, self.bind.geneId)
  self.bind.active_isTalent = isInnateGene and not self.bind.isKiboArena
  self.bind.active_InnateLock = isInnateGene
