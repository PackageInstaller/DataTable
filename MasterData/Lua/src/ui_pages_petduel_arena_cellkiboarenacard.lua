local this = class("cellKiboArenaCard", G_UIModuleBase)
...
function this:open()
  ...
  self.updateTimer = self.updateTimer or Timer.repeated(0.2, self.updateHandler, self, self.gameObject)
end
...
function this:refreshPlayerPetInfo()
  local id = self.bind.petInfo.guid
...
function this:tick()
  ...
  local entityH = C_KiboDuelSimulatorManager.instance:GetEntityByUid(guid)
  if entityH and entityH:IsValid() then
    local entity = entityH.entity
    local maxHp = math.ceil(entity.data.aliveProperty.maxHp:ToFloat())
    local hp = math.ceil(entity.data.aliveProperty.hp:ToFloat())
    self.bind.fill_hp = hp / maxHp
    self.bind.txt_hp = tostring(hp) .. "/" .. tostring(maxHp)
    local maxSp = math.ceil(entity.data.aliveProperty.maxSp:ToFloat())
    local sp = math.ceil(entity.data.aliveProperty.sp:ToFloat())
    self.bind.fill_mp = sp / maxSp
    self.bind.txt_mp = tostring(sp) .. "/" .. tostring(maxSp)
  end
end
