local this = class("modulePetInfoCollectInfo", G_UIModuleBase)
...
function this:refreshView()
  ...
  self.bind.satietyActive = self.bind.petIconActive
...
function this:refreshSatiety()
  local currentSatiety = L_SatietyStore:getPetSatiety(self._selectGuid)
  local maxSatiety = L_SatietyStore:getPetMaxSatiety(self._selectGuid)
  local color = L_Const.colorHtml.yellow016
  if currentSatiety == 0 then
    color = L_Const.colorHtml.red002
  end
  self.bind.satietyCurScore = L_GameUtil.fillColor(string.format("%d", currentSatiety), color)
  self.bind.satietyTotalScore = string.format("/%d", maxSatiety)
end
