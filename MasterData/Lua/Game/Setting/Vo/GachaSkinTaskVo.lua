---
--- Generated by EmmyLua(https://github.com/EmmyLua)
--- Created by yunchuan.
--- DateTime: 2024/3/5 14:09
---

local SettingRefBase = require('Game.Setting.SettingRefBase')
local base = SettingRefBase
local ParseUtils = CS.ParseUtils

---@class GachaSkinTaskVo
local GachaSkinTaskVo = Class("GachaSkinTaskVo", SettingRefBase)

function GachaSkinTaskVo:__init()
    self.id = nil              --任务ID
    self.desc = nil              --任务描述
    self.activityId = nil              --活动id
    self.taskContentType = nil              --任务目标类型
    self.taskContentId = nil              --任务目标
    self.taskContenNum = nil              --任务值
    self.rewardId = nil              --奖励道具id
    self.rewardNum = nil              --奖励道具数量
    self.openType = nil              --跳转目标
    self.openParameter = nil              --跳转参数
    self.status = nil              --状态
end

function GachaSkinTaskVo:FillVo(row)
    base.FillVo(self,row)
    self.id = parse_number(row, "id")
    self.desc = ParseUtils.ParseLocalizeText(row, "desc")
    self.activityId = parse_number(row, "activityId")
    self.taskContentType = parse_number(row, "taskContentType")
    self.taskContentId = parse_number(row, "taskContentId")
    self.taskContenNum = parse_number(row, "taskContenNum")
    self.rewardId = parse_number(row, "rewardId")
    self.rewardNum = parse_number(row, "rewardNum")
    self.openType = parse_string(row, "openType")
    self.openParameter = parse_string(row, "openParameter")
    self.status = parse_number(row, "status")
end

function GachaSkinTaskVo:__delete()
    self.id = nil
    self.desc = nil
    self.activityId = nil
    self.taskContentType = nil
    self.taskContentId = nil
    self.taskContenNum = nil
    self.rewardId = nil
    self.rewardNum = nil
    self.openType = nil
    self.openParameter = nil
    self.status = nil
end

return GachaSkinTaskVo