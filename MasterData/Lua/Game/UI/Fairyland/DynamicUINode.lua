...
function DynamicUINode:InitNode(parentRectTransform, circleRadius, deadZoneWidth, maxDistance, ...)
    ...
    self.enableScale = checkNumber(self.maxDistance) > 0
    self.camera = self:GetCamera()
    self:DoInit(...)
    self.rootCanvasCoordinateMatrix = DreamSpaceUtils.RefreshRootCanvasCoordinateMatrix( self.parentRectTransform );
end
...
function DynamicUINode:IsInScreen()
    ...
    if self.enableScale then
        scale = (self.maxDistance - distance) / self.maxDistance
    end

    return true, pos, scale
end

function DynamicUINode:UpdateUIPositionAndScale()
    local scale = 1
    self.isInScreen, self.rectTransform.position, scale = self:IsInScreen()
    ...
    if self.enableScale and self.isInScreen then
