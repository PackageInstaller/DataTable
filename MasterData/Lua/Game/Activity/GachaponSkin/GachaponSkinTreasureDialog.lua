...
---@class GachaponSkinTreasureDialog
---@field Env                           	GachaponSkinTreasureDialog
...
local GachaponSkinTreasureDialog = Class('GachaponSkinTreasureDialog')
...
function GachaponSkinTreasureDialog:__init()
	self.oneSkinData = nil
	self.activity = nil
end


function GachaponSkinTreasureDialog:__delete()
	self.oneSkinData = nil
	self.activity = nil
	GameUtils.RemoveRedPointCallBack(Constants.RedPointConst.MainCardSkinTreasureTask)
...
function GachaponSkinTreasureDialog:Awake()
	...
	GameUtils.BindRedPoint(Constants.RedPointConst.MainCardSkinTreasureTask,self.BtnTask , true)
	...
	SetButtonAction(self.BtnMember.gameObject, function()
		if isNotNull(self.oneSkinData) then
			UIModule.OpenDialog({id = Constants.UITypeIds.ActivityGachaponRuleDialog, parameters = {
				activityUuid = self.oneSkinData.id, ruleId = self.activity.ruleId,type = self.activity.type
			} })
		end
	end)
...
function GachaponSkinTreasureDialog:OnTaskDialogClose(data)
	if checkInt(data.activityUuid) ~= checkInt(self.activity.id) then
		return
	end
	local redNum = data.redNum
	local detail = self.oneSkinData
	GameUtils.SetRedPointNum(Constants.RedPointConst.MainCardSkinTreasureTask, redNum)
	if redNum <= 0 then
...
function GachaponSkinTreasureDialog:Start()

end

function GachaponSkinTreasureDialog:RefreshData(data)
	self.activity = data
	GameUtils.Request(Interfaces.ActivitySkinTreasureExhibition , nil,
			function(request, response)
				...
				if self.oneSkinData.taskRedDotHint >= 0 then
					GameUtils.SetRedPointNum(Constants.RedPointConst.MainCardSkinTreasureTask , 1)
					GameUtils.InvokeRedPointCallBack(Constants.RedPointConst.MainCardSkinTreasureTask)
				end
...
function GachaponSkinTreasureDialog:InitRewardList()
...
function GachaponSkinTreasureDialog:AddLuaBehavious()
	for i =1 , 10 do
...
function GachaponSkinTreasureDialog:CheckGoodsEnough()
...
function GachaponSkinTreasureDialog:UpdateAllItems()
	for i =1 , 10 do
		self:UpdateGoodItemIndex(i)
	end
end
-- 抽皮肤
function GachaponSkinTreasureDialog:DrawSkinClick()
	local callFunc = function()
		GameUtils.Request(Interfaces.ActivitySkinTreasureCall , nil ,function(request, response)
			if checkNumber(response.errCode) ~= 0 then return end
...
function GachaponSkinTreasureDialog:PlayNormalTimeLine(name , func)
...
function GachaponSkinTreasureDialog:PlaySpecialTimeLine(name, skinId , func)
	---@type Engine.UI.UILuaDialog
	local luaDialog = UIModule.CurrentScreen:GetDialog(Constants.UITypeIds.ActivityGachaponSkinDialog)
	---@type ActivityGachaponSkinDialog
	local Env = luaDialog.Env
...
function GachaponSkinTreasureDialog:GetLoadModelParentGo()
...
function GachaponSkinTreasureDialog:GetGameObjectByName(name)
...
function GachaponSkinTreasureDialog:DelLoadModelParentGo()
	local KCookie = CS.Engine.Lib.KCookie
	local MainSceneID = KCookie.Get(Constants.UICacheDataKeys.MainSceneID)
	local sceneInstance   = SceneManagement:GetInstance():GetSceneInstance(MainSceneID)
	if not isNull(sceneInstance) then
...
-- 是否抽完
function GachaponSkinTreasureDialog:IsAllCompete()
	local oneActivityData = self.oneSkinData
	if isNull(oneActivityData.callRewards) then
		oneActivityData.callRewards = {}
	end
...
function GachaponSkinTreasureDialog:UpddateButton()
	local isAllCompleted = self:IsAllCompete()
	if isAllCompleted then
...
---获取消耗道具
function GachaponSkinTreasureDialog:GetConsumeGoodsData()
	local oneActivityData = self.oneSkinData
	if isNull(oneActivityData.callRewards) then
		oneActivityData.callRewards = {}
	end
...
function GachaponSkinTreasureDialog:UpdateGoodItemIndex(index)
...
--- 判断是否拥有该角色
---@return boolean
function GachaponSkinTreasureDialog:IsHaveCard()
	---@type ActivityCardSkinComponent
	local ActivityCardSkinComponent =  GameUtils.GetEntityCompByType(ECSComponentType.Constants.ActivityCardSkinComponent)
	local activityData = ActivityCardSkinComponent:GetActivityDataById(self.oneSkinData.id)
...
function GachaponSkinTreasureDialog:GetOneGoodsDataByIndex(index)
	local oneActivityData = self.oneSkinData
	local rewardList =  oneActivityData.rewardList
	return rewardList[index]
end
--- 根据rewardId 获取index
function GachaponSkinTreasureDialog:GetIndexByRewardId(rewardId)
	local oneActivityData = self.oneSkinData
	local rewardList =  oneActivityData.rewardList
	local index = 1
	for kk, oneRewards in pairs(rewardList) do
...
---播放获得动画
function GachaponSkinTreasureDialog:PlayGettingIndex(index)
	---@type ActivityGachaponSkinNode
	local env = CfUtils.GetLuaScr(self["GoodItem" .. index].gameObject ,ActivityGachaponSkinNodePath)
	env:PlayGetting()
end
...
function GachaponSkinTreasureDialog:OnDestroy()
	self.controller = nil -- luaBehaviour 中 CSharp 对象
	self:DelLoadModelParentGo()
	self:Delete()
end

return GachaponSkinTreasureDialog
