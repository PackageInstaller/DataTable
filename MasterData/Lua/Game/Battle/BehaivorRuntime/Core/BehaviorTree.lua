...
function behaviorTree:load(jsonData, names)
	...
	for i,v in pairs(data.nodes) do
		...
		local ls = string.slice(spec.name, '_')
		local nodeName = ls[1]
		local funcName = ls[2]

		if names[nodeName] then
			Cls = names[nodeName]
		elseif b3[nodeName] then
			Cls = b3[nodeName]
		else
			print("Error : BehaviorTree.load : Invalid node name + " .. nodeName .. ".")
		end
		...
		if not node.properties["funcName"] then
			node.properties["funcName"] = funcName
		end
		nodes[id] = node
	end
