...
local track_back =  function (msg)
    local trackIndex  = 2  -- 1是本方法，没打印必要，所以从2开始
    local threadIndex = trackIndex
    local threadLoops = 20
    local localLoops  = 20
    local stackLines  = {}
    local wrapChasNum = 98

    while threadIndex <= threadLoops do
        local threadInfo = debug.getinfo(threadIndex, 'l')
        if not threadInfo then break end
        local lastIndex   = 1
        local localIndex  = 1
        local localValues = {''}
        while localIndex <= localLoops do
            local localName, localValue = debug.getlocal(threadIndex, localIndex)
            if not localName then break end
            local valueText = string.format('%s = %s', tostring(localName), tostring(localValue))
            if string.len(localValues[lastIndex]) + string.len(valueText) > wrapChasNum then
                lastIndex = lastIndex + 1
                localValues[lastIndex] = valueText
            else
                if localValues[lastIndex] == '' then
                    localValues[lastIndex] = valueText
                else
                    localValues[lastIndex] = string.format('%s, %s', localValues[lastIndex], valueText)
                end
            end
            localIndex = localIndex + 1
        end
        stackLines[threadIndex] = string.format('{\n\t\t%s\n\t}', table.concat(localValues, ', \n\t\t'))
        threadIndex = threadIndex + 1
    end
    local HttpModule = CS.Engine.Http.HttpModule
    local pid =  tostring(HttpModule.RequestBodyMeta.PlayerId)
    local uid =  tostring(HttpModule.RequestBodyMeta.UserId)
    local timestamp = os.date('%Y-%m-%d %H:%M:%S', os.time())
    local findCount = 0
    local traceback = string.gsub(debug.traceback(tostring(msg), trackIndex), '\n', function(s)
        findCount = findCount + 1
        -- 1 is hand, skip hand
        if findCount == 1 then
            return string.format('%s[LUA_ERROR] [%s] [uid=%s, pid=%s] ', s, timestamp, uid, pid)
        else
            return '\n\t' .. (stackLines[trackIndex+(findCount-5)] or '') .. s
        end
    end)
    -- print crash
    local crashList = {
    ...
    local crashLog = table.concat(crashList, '\n')
    return crashLog
end
    ...
    local generator = setmetatable({
        w_func = function()
            ...
            if server == 'Debug' or server == 'beta' or server == 'oversea' then
                local status,err = xpcall(func,track_back, unpack(params))
                if status == false then
                    CS.UnityEngine.Debug.LogError(tostring(err))
                end
            else
                local status,err = xpcall(func,debug.traceback, unpack(params))
                if status == false then
                    CS.UnityEngine.Debug.LogError(tostring(err))
                end
            end
