-- @FileName:   BuildStageMainUI.lua
-- @Description:   大西瓜关卡界面
-- @Copyright:   (LY) 2023 雷焰网络
-- 活动关卡主界面，按区域展示关卡节点、解锁状态、红点、任务与排行榜入口

module('build.BuildStageMainUI', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("build/BuildStageMainUI.prefab")
...
function ctor(self)
    ...
    self:setBg("stage_01.jpg",true,"build")
    self:setUICode(LinkCode.Build)
    local name = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Build):getName()
    self:setTxtTitle(name)
end
...
function configUI(self)
    ...
    self.mTabGroup.gameObject:SetActive(false)
    ...
    --self.mImgBg:setSprite(UrlManager:getSprite("build/bg_01.jpg"))

end

function initViewText(self)
    self.mTxtTask.text = _TT(151210)
    self.mTxtRank.text = _TT(151211)
end

--打开规则说明界面
function onClickFuncTipsHandler(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BUILD_TIPS_VIEW)
...
function onClickTask(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BUILD_TASK_PANEL)
end

function onClickRank(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BUILD_RANK_PANEL)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_BUILD_STAGE_PANEL,self.showPanel,self)
    self:showPanel()
end
...
function showPanel(self)
    ...
    if area_id then
        local areaConfig = build.BuildManager:getAreaConfig(area_id)
        ...
        if lastArea > 0 then
            if not build.BuildManager:getAreaPassState(lastArea) then
                area_id = 1
            end
        end
    else
        local areaConfigDic = build.BuildManager:getAreaConfigDic()
        for i = 1, #areaConfigDic do
            local timeOpen = areaConfigDic[i]:isOpen()
            local lastOpen = true
            if i > 1 then
                lastOpen = build.BuildManager:getAreaPassState(areaConfigDic[i - 1].tid)
            end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_BUILD_STAGE_PANEL,self.showPanel,self)
...
function close(self)
    GameDispatcher:dispatchEvent(EventName.CLOSE_BUILD_DUP_PANEL)
...
function addAllEventListener(self)
    ...
    GameDispatcher:addEventListener(EventName.BUILD_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:addEventListener(EventName.UPDATE_BUILD_STAGE_PANEL, self.onDataRefresh, self)
end
...
function removeAllEventListener(self)
    ...
    GameDispatcher:removeEventListener(EventName.BUILD_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:removeEventListener(EventName.UPDATE_BUILD_STAGE_PANEL, self.onDataRefresh, self)
    

end

function onClickReward(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BUILD_STAR_REWARD_PANEL)
end
...
function creatTabList(self)
    ...
    self:clearTabItem()
    for i = 1, #areaConfigDic do
        local tabItem = SimpleInsItem:create(self.mItemTab, self.mTabGroup, "build_stageMainUI_tabItem")
...
function onSelectMapTab(self, area_id)
    ...
    self.mImgBg:SetImg(UrlManager:getBgPath("build/stage_0".. (self.m_select_areaId)..".jpg"))
    --self:setBg("stage_0".. (self.m_select_areaId)..".jpg",true,"build")
    ...
    for i = 1, length do
        ...
        local dupItem = SimpleInsItem:create(self.mDupItem, self.list, "build_stageMainUI_dupItem")
        self.m_dupItemList[dup_id] = dupItem

        local configVo = build.BuildManager:getDupConfig(dup_id)
        ...
        --local star_count = build.BuildManager:getDupPassStar(dup_id)
        for i = 1, 3 do
            dupItem:getChildGO("mStar_" .. i):SetActive(false)
        end

        local isPass = build.BuildManager:getDupPassState(dup_id)
        ...
        if configVo.pre_id ~= 0 then
            lastPass = build.BuildManager:getDupPassState(configVo.pre_id)
        end
        ...
        dupItem:addUIEvent("mRayPoint", function ()
            ...
            StorageUtil:saveBool1(gstor.BUILD_DUPNEWOPENSTR .. dup_id, true)
...
function onSelectDupItem(self, dupConfigVo)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_BUILD_DUP_PANEL)
    GameDispatcher:dispatchEvent(EventName.OPEN_BUILD_DUP_PANEL, dupConfigVo)
end
...
function refreshAreaRedPoint(self)
    local areaConfig = build.BuildManager:getAreaConfigDic()
    for areaId, areaConfigVo in pairs(areaConfig) do
        local tabItem = self.m_tabItemList[areaId]
        if tabItem then
            if build.BuildManager:getAreaShowRed(areaConfigVo) then
...
function refreshDupRedPoint(self)
    for i = 1, #self.m_areaConfig.stage_list do
        ...
        if dupItem then
            if build.BuildManager:getDupShowRed(dup_id) then
                RedPointManager:add(dupItem:getChildTrans("mRayPoint"), nil, 30,30)
            else
...
function refreshStarAwardRedPoint(self)
    if build.BuildManager:getTaskRed() then
        RedPointManager:add(self.mBtnTask.transform, nil, 30,30)
    else
        RedPointManager:remove(self.mBtnTask.transform)
    end
    --self.ImEffect:SetActive(build.BuildManager:getStarAwardRedState())
end
