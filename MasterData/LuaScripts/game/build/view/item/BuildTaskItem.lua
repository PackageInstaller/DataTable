--[[ 
-----------------------------------------------------
@filename       : BuildTaskItem
@Description    : 活动玩法任务Item
@date           : 2023/5/24 15:00
@Author         : Tonn
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
module("build.BuildTaskItem ", Class.impl("lib.component.BaseItemRender"))

-- 任务列表项，展示任务描述、奖励、进度、领取按钮和已领取状态
...
function setData(self, param)
    ...
    if self.data then
        ...
        local received = build.BuildManager:checkTaskAwardReceived(self.data.id)
        self:getChildGO("mGroupReceived"):SetActive(received)

        local taskMsgVo =  build.BuildManager:getTaskMsgVo(self.data.id)
