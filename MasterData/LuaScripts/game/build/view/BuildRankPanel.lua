--[[ 
-----------------------------------------------------
@filename       : BuildRankPanel
@Description    : 大西瓜积分排行榜
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------
]]
module("build.BuildRankPanel", Class.impl(View))

-- 建造小游戏排行榜界面，请求并展示排行榜列表、玩家自身排名和头像信息
UIRes = UrlManager:getUIPrefabPath("build/BuildRankPanel.prefab")
...
function configUI(self)
    ...
    self.mScroller:SetItemRender(build.BuildRankItem)
...
function active(self)
    super.active(self)
    GameDispatcher:dispatchEvent(EventName.REQ_RANK_DATA, { type = rank.RankConst.BUILD })
...
function updateView(self)
    local rankData = rank.RankManager:getRankInfoVo(rank.RankConst.BUILD)
