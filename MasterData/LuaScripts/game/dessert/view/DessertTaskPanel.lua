--[[
-----------------------------------------------------
@filename       : DessertTaskPanel
@Description    : 甜品大作战限时任务面板
@date           : 2023/5/24 15:00
@Author         : Tonn
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
--
module("dessert.DessertTaskPanel", Class.impl(View))
UIRes = UrlManager:getUIPrefabPath("dessert/DessertTaskPanel.prefab")   
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg", false, "Dessert")
...
function configUI(self)
    ...
    self.mScroller:SetItemRender(dessert.DessertTaskItem)
...
function addAllUIEvent(self)
    self:addUIEvent(self:getChildGO("mBtnGetAll"), function()
        dessert.DessertManager:receiveAllTaskAward()
    end)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.DESSERT_TASK_UPDATE, self.setScrollerData, self)
    self:updateTime()
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.DESSERT_TASK_UPDATE, self.setScrollerData, self)
    --MoneyManager:setMoneyTidList()
...
function setScrollerData(self)
    if next(self.mTaskList) == nil then
        for _, vo in pairs(dessert.DessertManager:getTaskConfig()) do
            table.insert(self.mTaskList, vo)
        end
    end

    local msgList = dessert.DessertManager:getDessertTaskMasData()

    if msgList and next(msgList) then
        local listHelper = {}
        for _, msgVo in pairs(msgList) do
            if msgVo.state == 2 then
                local configVo = dessert.DessertManager:getTaskConfigVo(msgVo.id)
...
function updateView(self)
    ...
    self.mAutoReceive:SetActive(dessert.DessertManager:checkTaskAwardCanReceive())
...
function updateTime(self)
    local mainActivityVo = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Dessert)
