-- @FileName:   DessertSettlePanel.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2023-07-21 10:59:29
-- @Copyright:   (LY) 2023 雷焰网络
module('dessert.DessertSettlePanel', Class.impl(View))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("dessert/DessertSettlePanel.prefab")
...
function active(self, args)
    ...
    self.m_DupConfigVo = dessert.DessertManager:getDupConfig(self.m_DupId)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_DESSERT_PASS_DUP, self.updateView, self)
    if self.isPass then
        GameDispatcher:dispatchEvent(EventName.REQ_DESSERT_PASS_DUP, {
            dupId = self.m_DupId,
            point = self.m_socre
        })
    end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_DESSERT_PASS_DUP, self.updateView, self)
...
function onExit(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_DESSERT_GAME_PANEL)
end
...
function onNextDup(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_DESSERT_GAME_PANEL)
        ...
        if self.isPass then
            local next_dupId = dessert.DessertManager:getNextDupId(self.m_DupId)
            local nextDupConfig = dessert.DessertManager:getDupConfig(next_dupId)
            GameDispatcher:dispatchEvent(EventName.OPEN_DESSERT_GAME_PANEL, {
...
function updateView(self)
    ...
    if next_dupId and self.isPass then
        local next_configVo = dessert.DessertManager:getDupConfig(next_dupId)
