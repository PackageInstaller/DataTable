...
function listNotification(self)
    ...
    GameDispatcher:addEventListener(EventName.REQ_BIND_PLAYER, self.onReqBindPlayerHandler, self)
    GameDispatcher:addEventListener(EventName.REQ_JOINT_BIND_AGREE, self.onReqAgreePlayerHandler, self)
    GameDispatcher:addEventListener(EventName.REQ_JOINT_BIND_REJECT, self.onReqRejectPlayerHandler, self)
    GameDispatcher:addEventListener(EventName.REQ_JOINT_OPS_APPLY_LIST, self.onReqJointOpsApplyListHandler, self)
    GameDispatcher:addEventListener(EventName.REQ_JOINT_TASK_RECEIVE, self.onReqJointTaskHandler, self)
    
    GameDispatcher:addEventListener(EventName.OPEN_JOINT_BIND_TIPS, self.onOpenJointBindTipsHandler, self)
    GameDispatcher:addEventListener(EventName.OPEN_JOINT_OPS_APPLY_LIST, self.onOpenJointBindApplyListViewHandler, self)
    GameDispatcher:addEventListener(EventName.CLOSE_JOINT_BIND_APPLY_LIST, self.onCloseJointBindApplyListViewHandler, self)
...
function registerMsgHandler(self)
    return {
        ...
        --联结作战面板信息
        SC_JOINT_OPS_PANEL = self.onJointOpsPanelHandler,
        -- 联结作战联结申请结果
        SC_JOINT_OPS_APPLY_RESULT = self.onJointOpsResultHandler,
        -- 联结作战同意结果
        SC_JOINT_OPS_AGREE_RESULT = self.onJointOpsAgreeResultHandler,
        -- 联结作战拒绝结果
        SC_JOINT_OPS_REJECT_RESULT = self.onJointOpsRejectResultHandler,
        -- 联结作战申请列表
        SC_JOINT_OPS_APPLY_LIST = self.onJointOpsApplyListHandler,
        -- 联结作战任务面板信息
        SC_JOINT_OPS_TASK_PANEL = self.onJointTaskPanelHandler,
        -- 联结作战任务领取
        SC_JOINT_OPS_TASK_RECEIVE = self.onJointTaskReceiveHandler,
        -- 联结作战任务更新
        SC_JOINT_OPS_TASK_UPDATE = self.onJointTaskUpdateHandler,
...
--- *c2s* 联结作战申请 19902
function onReqBindPlayerHandler(self,args)
    SOCKET_SEND(Protocol.CS_JOINT_OPS_APPLY, {show_code = args.linkCode}, Protocol.SC_JOINT_OPS_APPLY_RESULT)
end

--- *c2s* 同意联结作战 19904
function onReqAgreePlayerHandler(self,args)
    SOCKET_SEND(Protocol.CS_JOINT_OPS_AGREE, {player_id = args.playerId}, Protocol.SC_JOINT_OPS_AGREE_RESULT)
end

--- *c2s* 拒绝联结作战 19906
function onReqRejectPlayerHandler(self,args)
    SOCKET_SEND(Protocol.CS_JOINT_OPS_REJECT, {player_id = args.playerId}, Protocol.SC_JOINT_OPS_REJECT_RESULT)
end
--- *c2s* 请求联结作战申请列表 19908
function onReqJointOpsApplyListHandler(self,args)
    SOCKET_SEND(Protocol.CS_JOINT_OPS_APPLY_LIST, {}, Protocol.SC_JOINT_OPS_APPLY_LIST)
end

--- *c2s* 任务领取奖励 19911
function onReqJointTaskHandler(self,args)
    SOCKET_SEND(Protocol.CS_JOINT_OPS_TASK_RECEIVE, {task_id_list = args.taskIdList}, Protocol.SC_JOINT_OPS_TASK_RECEIVE)
end
...
function onJointOpsPanelHandler(self,msg)
    activity.ActitvityExtraManager:parseBindMsg(msg)
end

function onJointOpsResultHandler(self,msg)
    activity.ActitvityExtraManager:parseBindResult(msg)
end

function onJointOpsAgreeResultHandler(self,msg)
    activity.ActitvityExtraManager:parseBindAgreeResult(msg)
end

function onJointOpsRejectResultHandler(self,msg)
    activity.ActitvityExtraManager:parseBindRejectResult(msg)
end

function onJointOpsApplyListHandler(self,msg)
    activity.ActitvityExtraManager:parseBindApplyList(msg)
end

function onJointTaskPanelHandler(self,msg)
    activity.ActitvityExtraManager:parseJointTaskPanel(msg)
end

function onJointTaskReceiveHandler(self,msg)
    activity.ActitvityExtraManager:parseJointTaskReceive(msg)
end

function onJointTaskUpdateHandler(self,msg)
    activity.ActitvityExtraManager:parseJointTaskUpdate(msg)
end
----------------------------------------------------------
----------------------------------------------------------
...
function onOpenJointBindTipsHandler(self,args)
    if self.mActivityJointBindTips == nil then
        self.mActivityJointBindTips = activity.ActivityBindTabTipsView.new()
        self.mActivityJointBindTips:addEventListener(View.EVENT_VIEW_DESTROY, self.onDestroyActivityJointBindTips, self)
    end
    self.mActivityJointBindTips:open(args)
end

function onDestroyActivityJointBindTips(self)
    self.mActivityJointBindTips:removeEventListener(View.EVENT_VIEW_DESTROY, self.onDestroyActivityJointBindTips, self)
    self.mActivityJointBindTips = nil
end

function onOpenJointBindApplyListViewHandler(self,args)
    if self.mActivityJointBindApplyListView == nil then
        self.mActivityJointBindApplyListView = activity.ActivityBindApplyListView.new()
        self.mActivityJointBindApplyListView:addEventListener(View.EVENT_VIEW_DESTROY, self.onDestroyActivityJointBindApplyListView, self)
    end
    self.mActivityJointBindApplyListView:open(args)
    
end

function onDestroyActivityJointBindApplyListView(self)
    self.mActivityJointBindApplyListView:removeEventListener(View.EVENT_VIEW_DESTROY, self.onDestroyActivityJointBindApplyListView, self)
    self.mActivityJointBindApplyListView = nil
end

function onCloseJointBindApplyListViewHandler(self)
    if self.mActivityJointBindApplyListView then
        self.mActivityJointBindApplyListView:close()
    end
end
