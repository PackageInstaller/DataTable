module("formation.FormationVisionController", Class.impl("game.formation.normal.controller.FormationController"))

---------------------------------------------------------------固定需要-----------------------------------------------------------------
function getManager(self)
    return formation.FormationVisionManager
end
...
function __openFormationPanel(self)
    if self.m_heroFormationPanel == nil then
        self.m_heroFormationPanel = formation.FormationVisionPanel.new()
...
-- 异象残境为多队伍编队，使用专属选择面板以显示“队伍X已上阵”并禁止跨队重复上阵
function __onOpenFormationHeroSelectPanelHandler(self, args)
    if self.mFormationHeroSelectPanel == nil then
        self.mFormationHeroSelectPanel = formation.FormationVisionHeroSelectPanel.new()
