module("braceletBuild.BraceletStrengthenSucPanel", Class.impl(View))
...
function setData(self, oldEquipVo, curEquipVo, isBreakUp)
    ...
    local oldAttrList, oldAttrDic = oldEquipVo:getTotalAttr()
    local curAttrList, curAttrDic = curEquipVo:getTotalAttr()
    for i = 1, #oldAttrList do
        local oldKeyValueVo = oldAttrList[i]
        if oldKeyValueVo.value ~= curAttrDic[oldKeyValueVo.key] then
            local item = SimpleInsItem:create(self.ItemAttr, self.mScrollBreakUpAttr, "BraceletStrengthenSucPanelItemAttr")
            ...
            item:getChildGO("mImgChange"):SetActive(oldKeyValueVo.value ~= curAttrDic[oldKeyValueVo.key])
        end
    end
end
