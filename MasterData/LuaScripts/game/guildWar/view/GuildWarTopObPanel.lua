...
module("guild.GuildWarTopObPanel", Class.impl(View))
-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("guildWar/GuildWarTopObPanel.prefab")
...
function updateInfo(self)
    ...
    -- end
    local day,guildInfo1,guildInfo2 = guildWar.GuildWarManager:getGuildWarTopObData()

    local url = guild.GuildManager:getIconDataById(guildInfo1.icon).icon
    self.mImgGuildIcon:SetImg(UrlManager:getIconPath(url), false)
    self.mTxtGuildName.text = guildInfo1.name
    self.mTxtGuildLv.text = _TT(1361) .. guildInfo1.lv

    self.membersList = guildWar.GuildWarManager:getObInfoAllMembers(1) -- guild.GuildManager:getGuildInfo().members
    ...
    local canFightMembers = guildInfo1.members
    ...
    --local enemyGuildPanel = guildWar.GuildWarManager:getObInfoAllMembers(2)
    ...
    --if enemyGuildPanel ~= nil and (self.state == guildWar.GuildWarState.GuildWarStart or self.state == guildWar.GuildWarState.GuildWarTopStart) then
        local url = guild.GuildManager:getIconDataById(guildInfo2.icon).icon
        self.mImgGuildIconEnemy:SetImg(UrlManager:getIconPath(url), false)

        self.mTxtGuildNameEnemy.text = guildInfo2.name
        self.mTxtGuildLvEnemy.text = _TT(1361) .. guildInfo2.lv

        local members = guildWar.GuildWarManager:getObInfoAllMembers(2)

        local hasBuildList, hasCount, allScore, remBuild, allBuild = self:getSimOptCount(members)
        local canFightMembers = guildInfo2.members
        local allCount = #canFightMembers * sysParam.SysParamManager:getValue(SysParamType.GUILDWAR_COUNT)
