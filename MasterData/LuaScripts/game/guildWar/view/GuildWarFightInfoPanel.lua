--[[
-----------------------------------------------------
@filename       : GuildWarFightInfoPanel
@Description    : 战斗准备界面
@copyright      : (LY) 2020 雷焰网络
-----------------------------------------------------
]] module("guildWar.GuildWarFightInfoPanel", Class.impl(View))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("guildWar/GuildWarFightInfoPanel.prefab")
...
function onBtnFormationClick(self)
    ...
    self:close()
    formation.openFormation(formation.TYPE.GUILDWARATK, nil, self.defIndex, nil)
end
...
function onBtnFightClick(self)
    ...
    local listAtt = formation.FormationGuildWarAtkManager:getSelectFormationHeroList(23000 + self.defIndex)
    local listDef = formation.FormationGuildWarDefManager:getSelectFormationHeroList(22000 + self.defIndex)

    if #listAtt == 0 then
        gs.Message.Show("没有配置进攻队列")
        return
    end
...
function showTeam(self)
    ...
    local list = formation.FormationGuildWarAtkManager:getSelectFormationHeroList(23000 + self.defIndex)
    for i = 1, 5 do
    ...
    self.enemyHeroList = {}
    for i = 1, #self.mEnemyFormation do
        if self.mEnemyFormation[i].team_id == self.defIndex then
            self.enemyHeroList = self.mEnemyFormation[i].hero_list
        end
    end
    ...
    for i = 1, 5 do
        ...
        local isNull = self.enemyHeroList[i] == nil
