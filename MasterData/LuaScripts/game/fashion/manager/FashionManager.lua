...
-- 通过模型id获取战员tid
function getFashionVoByModelId(self, modelId)
    for k, heroDic in pairs(self:getFashionConfigDic(fashion.Type.CLOTHES)) do
        for i, fashionVo in ipairs(heroDic) do
            if fashionVo.model == modelId then
                return fashionVo
            end
        end
    end
...
function getHeroFashionColorUnLock(self, heroTid, fashionId, colorId)
    local msgVo = self:getHeroFashionColor(heroTid, fashionId)
    return table.indexof01(msgVo.colorList, colorId) > 0
end
-- 战员皮肤部位解锁
...
function parseAllHeroFashionColorInfo(self, msg)
    self.mFashionColorUnlockList = {}
    for i = 1, #msg.hero_fashion_color_list do
        table.insert(self.mFashionColorUnlockList, msg.hero_fashion_color_list[i])
    end
end

function getAllHeroFashionColorInfo(self, tid, fashionId, colorId)
    for i = 1, #self.mFashionColorUnlockList do
        if self.mFashionColorUnlockList[i].hero_tid == tid and self.mFashionColorUnlockList[i].fashion_id == fashionId and self.mFashionColorUnlockList[i].color_id == colorId then
            return true
        end
    end
    return false
...
function updateAllHeroFashionColorInfo(self, tid, fashionId, colorId, fashionCoinNum)
    local info = {}
    info.hero_tid = tid
    info.fashion_id = fashionId
    info.color_id = colorId
    if fashionCoinNum <= 0 then
        table.insert(self.mFashionColorUnlockList, info)
    end
end

function getAllHeroFashion(self)
    return self.m_heroFashionDic
end
