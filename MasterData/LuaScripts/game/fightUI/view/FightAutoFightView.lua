...
function initData(self, rootGo)
    ...
    self.mAoyiOn = SimpleInsItem:create2(self:getChildGO("mGroupAoyiOn"))
    self.mAoyiOff = SimpleInsItem:create2(self:getChildGO("mGroupAoyiOff"))
    ...
    local function onAoyiChangeOn()
        self:setAoyiState(0,true)
    end
    local function onAoyiChangeOff()
        self:setAoyiState(1,true)
    end
    ...
    self.mAoyi1 = fightUI.FightSkillItem.new()
    self.mAoyi2 = fightUI.FightSkillItem.new()
    ...
    if fightSkinId == role.DefFightUIId  then
        ...
        self.mAoyi1:setup(UrlManager:getUIPrefabPath("fight/FightSkillItem.prefab"))
        self.mAoyi2:setup(UrlManager:getUIPrefabPath("fight/FightSkillItem.prefab"))
    else
        self.mSkill1:setup(UrlManager:getUIPrefabPath("fight_"..fightSkinId.."/FightSkillItem.prefab"))
        self.mSkill2:setup(UrlManager:getUIPrefabPath("fight_"..fightSkinId.."/FightSkillItem.prefab"))
        self.mAoyi1:setup(UrlManager:getUIPrefabPath("fight_"..fightSkinId.."/FightSkillItem.prefab"))
        self.mAoyi2:setup(UrlManager:getUIPrefabPath("fight_"..fightSkinId.."/FightSkillItem.prefab"))
    end
    ...
    --self.mAoyi1:setup(UrlManager:getUIPrefabPath("fight/FightSkillItem.prefab"))
    self.mAoyi1:addOnParent(self:getChildTrans("mGroupAoyiNode1"))
    self.mAoyi1:setSkillCall(onAoyiChangeOn)
    self.mAoyi1:getSoulBgTrans().gameObject:SetActive(false)
    self.mAoyi1:setEffGroupActive(false)


    --self.mAoyi2:setup(UrlManager:getUIPrefabPath("fight/FightSkillItem.prefab"))
    self.mAoyi2:addOnParent(self:getChildTrans("mGroupAoyiNode2"))
    self.mAoyi2:setSkillCall(onAoyiChangeOff)
    self.mAoyi2:getSoulBgTrans().gameObject:SetActive(false)
    self.mAoyi2:setEffGroupActive(false)
...
function updateSkill(self, liveId, roleItem)
    ...
    local aoyiIndex = self:getAoyiStateIndex(liveId)
    self:setAoyiState(aoyiIndex, false)
    ...
    if skillDatas then
        for _, skillTmpVo in ipairs(skillDatas) do
            ...
            if posIdx == 3 then
                self.mAoyi1:setHeroID(heroId)
                self.mAoyi1:setSkillID(skillRo:getRefID(), self.mCurLiveId)
                self.mAoyi1:setIcon(UrlManager:getIconPath("skill/" .. skillRo:getIcon()))
                self.mAoyi1:setState(fightUI.FightSkillItemState.NORMAL)
                self.mAoyi1:setSoul(skillTmpVo:skillCost())

                self.mAoyi2:setHeroID(heroId)
                self.mAoyi2:setSkillID(skillRo:getRefID(), self.mCurLiveId)
                self.mAoyi2:setIcon(UrlManager:getIconPath("skill/" .. skillRo:getIcon()))
                self.mAoyi2:setState(fightUI.FightSkillItemState.NORMAL)
                self.mAoyi2:setSoul(skillTmpVo:skillCost())
            end
        end
    end
end

function setAoyiState(self,index ,isChange)
    if isChange then
        self:onCloseSelectSkill()
        if self.mAoyiSelectIndex == index then
            return
        end
        local type = 0
        for k, v in pairs(fight.FightDef.AUTO_AOYI_RULE) do
            if v == index then
                type = k
                break
            end
        end
        fight.FightManager:reqAoyiStateChange(self.mCurLiveId, type)

        if self.mCurRoleItem then
            if index == 0 then
                self.mCurRoleItem:getChildGO("mIconAoyiAuto"):SetActive(false)
            else
                self.mCurRoleItem:getChildGO("mIconAoyiAuto"):SetActive(true)
            end
            self.mCurRoleItem:getChildGO("mIconAoyiAuto"):GetComponent(ty.AutoRefImage):SetImg(
                UrlManager:getFightUIPath(index == 0 and "fight_aoyi_on.png" or "fight_aoyi_off.png"), true
            )
        end
    end
    self.mAoyiSelectIndex = index
    self.mAoyiOn:getChildGO("mImgSelected"):SetActive(index == 0)
    self.mAoyiOff:getChildGO("mImgSelected"):SetActive(index == 1)
end
    ...
    for i = 1, 5 do
        ...
        if attList and i <= #attList then
            ...
            if liveVo:getRaceType() == 0 then
                ...
                local aoyiIndex = self:getAoyiStateIndex(liveVo.id)
                if aoyiIndex == 0 then
                    item:getChildGO("mIconAoyiAuto"):SetActive(false)
                else
                    item:getChildGO("mIconAoyiAuto"):GetComponent(ty.AutoRefImage):SetImg(
                        UrlManager:getFightUIPath("fight_aoyi_off.png"), true
                    )
                    item:getChildGO("mIconAoyiAuto"):SetActive(true)
                end
                -- item:getChildGO("mIconAoyiAuto"):GetComponent(ty.AutoRefImage):SetImg(
                --     UrlManager:getFightUIPath(aoyiIndex == 0 and "fight_aoyi_on.png" or "fight_aoyi_off.png"), true
                -- )
...
function getAoyiStateIndex(self,liveId)
    local curHeroData = fight.FightManager:getHero(liveId)
    local autoMsgIndex = curHeroData.auto_aoyi_rule
    local itemIndex = fight.FightDef.AUTO_AOYI_RULE[autoMsgIndex]
    return itemIndex
end
