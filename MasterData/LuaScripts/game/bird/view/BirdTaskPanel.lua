--[[
-----------------------------------------------------
@filename       : BirdTaskPanel
@Description    : 大西瓜任务界面
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
--
module("bird.BirdTaskPanel", Class.impl(View))
UIRes = UrlManager:getUIPrefabPath("bird/BirdTaskPanel.prefab")
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg", false, "bird")
...
function configUI(self)
    ...
    self.mScroller:SetItemRender(bird.BirdTaskItem)
...
function addAllUIEvent(self)
    self:addUIEvent(self:getChildGO("mBtnGetAll"), function()
        bird.BirdManager:receiveAllTaskAward()
    end)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.BIRD_TASK_UPDATE, self.setScrollerData, self)
    self:updateTime()
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.BIRD_TASK_UPDATE, self.setScrollerData, self)
    --MoneyManager:setMoneyTidList()
...
function setScrollerData(self)
    if next(self.mTaskList) == nil then
        for _, vo in pairs(bird.BirdManager:getTaskConfig()) do
            table.insert(self.mTaskList, vo)
        end
    end

    local msgList = bird.BirdManager:getBirdTaskMasData()

    if msgList and next(msgList) then
        local listHelper = {}
        for _, msgVo in pairs(msgList) do
            if msgVo.state == 2 then
                local configVo = bird.BirdManager:getTaskConfigVo(msgVo.id)
...
function updateView(self)
    ...
    self.mAutoReceive:SetActive(bird.BirdManager:checkTaskAwardCanReceive())
...
function updateTime(self)
    local mainActivityVo = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Bird)
