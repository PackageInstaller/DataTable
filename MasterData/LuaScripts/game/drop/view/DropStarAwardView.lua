-- @FileName:   DropStarAwardView.lua
-- @Description:   大西瓜奖励界面
-- @Copyright:   (LY) 2024 锚点降临
-- 星级累计奖励界面，按可领取、未领取和已领取状态排序展示奖励

module('drop.DropStarAwardView', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("drop/DropStarAwardView.prefab")
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(drop.DropStarAwardItem)
...
function active(self, map_id)
    super.active(self)
    GameDispatcher:addEventListener(EventName.UPDATE_DROP_STAGE_PANEL, self.udpateView, self)
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_DROP_STAGE_PANEL, self.udpateView, self)
end
...
function udpateView(self)
    local starCount = drop.DropManager:getPassAllStarCount()
    local rewardList =  drop.DropManager:getStarRewardConfig()

    self.canGetList = {}
    local data = {}
    for i = 1, #rewardList do
        local isGet =  drop.DropManager:getAwardState(rewardList[i].id)
    ...
    local mainActivityVo = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Drop)
...
function onGetAll(self)
    -- logAll("请求全部领取")
    GameDispatcher:dispatchEvent(EventName.REQ_DROP_RECEIVE_STAR, self.canGetList)
end
