--[[
-----------------------------------------------------
@filename       : DropTaskPanel
@Description    : 大西瓜任务界面
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
--
module("drop.DropTaskPanel", Class.impl(View))
-- 建造小游戏任务界面，展示任务倒计时、任务列表、进度排序和一键领取入口
UIRes = UrlManager:getUIPrefabPath("drop/DropTaskPanel.prefab")
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg", false, "drop")
...
function configUI(self)
    ...
    self.mScroller:SetItemRender(drop.DropTaskItem)
...
function addAllUIEvent(self)
    self:addUIEvent(self:getChildGO("mBtnGetAll"), function()
        drop.DropManager:receiveAllTaskAward()
    end)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.DROP_TASK_UPDATE, self.setScrollerData, self)
    self:updateTime()
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.DROP_TASK_UPDATE, self.setScrollerData, self)
    --MoneyManager:setMoneyTidList()
...
function setScrollerData(self)
    if next(self.mTaskList) == nil then
        for _, vo in pairs(drop.DropManager:getTaskConfig()) do
            table.insert(self.mTaskList, vo)
        end
    end

    local msgList = drop.DropManager:getDropTaskMasData()

    if msgList and next(msgList) then
        local listHelper = {}
        for _, msgVo in pairs(msgList) do
            if msgVo.state == 2 then
                local configVo = drop.DropManager:getTaskConfigVo(msgVo.id)
...
function updateView(self)
    ...
    self.mAutoReceive:SetActive(drop.DropManager:checkTaskAwardCanReceive())
...
-- 每秒刷新活动剩余时间文本，使用主活动管理器中的 Drop 活动时间
