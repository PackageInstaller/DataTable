module("drop.DropGameWorld", Class.impl(Manager))
...
function registerToTheWorld(self, key, level, obj, colType, selfGroup, colGroup, motion, scale, enterCallback,
    ...
    local worldVo = LuaPoolMgr:poolGet(drop.DropWorldVo)
...
function onUpdate(self)
    ...
    if self.datas[drop.Id.MoveWorldId] == nil then
        return
    end 
    self.colliderDic = {}
    if self.datas[drop.Id.MoveWorldId].motion then
        self.datas[drop.Id.MoveWorldId]:updateWordlInfo()
    end
    for key, worldVo in pairs(self.datas) do
        if key ~= drop.Id.MoveWorldId then
                --if worldVo.motion then
                    worldVo:updateWordlInfo()
                --end
        
                local result = self.datas[drop.Id.MoveWorldId]:compare(worldVo)
      
              
                if result then
                    if self.colliderDic[drop.Id.MoveWorldId] == nil then
                        self.colliderDic[drop.Id.MoveWorldId] = {self.datas[drop.Id.MoveWorldId]}
                    end
                    table.insert(self.colliderDic[drop.Id.MoveWorldId], worldVo)
                end
        end
    end
    ...
    --     if self.worldData[i].colGroup == drop.DropGroup.None then
    --         break
    --     end
