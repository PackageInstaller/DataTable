module("split.SplitGameWorld", Class.impl(Manager))
...
function registerToTheWorld(self, key, level, obj, colType, selfGroup, colGroup, motion, scale, enterCallback,
    ...
    local worldVo = LuaPoolMgr:poolGet(split.SplitWorldVo)
...
function onUpdate(self)
    ...
    self.colliderDic = {}
    for i = 1, #self.worldData do
        if self.worldData[i].colGroup == split.SplitGroup.None then
            break
        end
