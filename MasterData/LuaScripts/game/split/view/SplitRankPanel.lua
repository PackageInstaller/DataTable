--[[ 
-----------------------------------------------------
@filename       : SplitRankPanel
@Description    : 大西瓜积分排行榜
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------
]]
module("split.SplitRankPanel", Class.impl(View))

UIRes = UrlManager:getUIPrefabPath("split/SplitRankPanel.prefab")
...
function configUI(self)
    ...
    self.mScroller:SetItemRender(split.SplitRankItem)
...
function active(self)
    super.active(self)
    GameDispatcher:dispatchEvent(EventName.REQ_RANK_DATA, { type = rank.RankConst.SPLIT })
...
function updateView(self)
    local rankData = rank.RankManager:getRankInfoVo(rank.RankConst.SPLIT)
