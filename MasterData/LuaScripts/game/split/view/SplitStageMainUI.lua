-- @FileName:   SplitStageMainUI.lua
-- @Description:   大西瓜关卡界面
-- @Copyright:   (LY) 2023 雷焰网络

module('split.SplitStageMainUI', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("split/SplitStageMainUI.prefab")
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg",true,"split")

   local name = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Split):getName()
...
function onClickFuncTipsHandler(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_SPLIT_TIPS_VIEW)
...
function onClickTask(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_SPLIT_TASK_PANEL)
end

function onClickRank(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_SPLIT_RANK_PANEL)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_SPLIT_STAGE_PANEL,self.showPanel,self)
    self:showPanel()
end
...
function showPanel(self)
    ...
    if StorageUtil:getBool1(prefixVersion .. "splitTips") == false then
        self:onClickFuncTipsHandler()
        StorageUtil:saveBool1(prefixVersion .. "splitTips", true)
    end
    ...
    if area_id then
        local areaConfig = split.SplitManager:getAreaConfig(area_id)
        ...
        if lastArea > 0 then
            if not split.SplitManager:getAreaPassState(lastArea) then
                area_id = 1
            end
        end
    else
        local areaConfigDic = split.SplitManager:getAreaConfigDic()
        for i = 1, #areaConfigDic do
            local timeOpen = areaConfigDic[i]:isOpen()
            local lastOpen = true
            if i > 1 then
                lastOpen = split.SplitManager:getAreaPassState(areaConfigDic[i - 1].tid)
            end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_SPLIT_STAGE_PANEL,self.showPanel,self)
...
function close(self)
    GameDispatcher:dispatchEvent(EventName.CLOSE_SPLIT_DUP_PANEL)
...
function addAllEventListener(self)
    ...
    GameDispatcher:addEventListener(EventName.SPLIT_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:addEventListener(EventName.UPDATE_SPLIT_STAGE_PANEL, self.onDataRefresh, self)
end
...
function removeAllEventListener(self)
    ...
    GameDispatcher:removeEventListener(EventName.SPLIT_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:removeEventListener(EventName.UPDATE_SPLIT_STAGE_PANEL, self.onDataRefresh, self)
    

end

function onClickReward(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_SPLIT_STAR_REWARD_PANEL)
end
...
function onSelectMapTab(self, area_id)
    ...
    for i = 1, length do
        ...
        local configVo = split.SplitManager:getDupConfig(dup_id)
        ...
        --local star_count = split.SplitManager:getDupPassStar(dup_id)
        for i = 1, 3 do
            dupItem:getChildGO("mStar_" .. i):SetActive(false)
        end

        local isPass = split.SplitManager:getDupPassState(dup_id)
        ...
        if configVo.pre_id ~= 0 then
            lastPass = split.SplitManager:getDupPassState(configVo.pre_id)
        end
        ...
        dupItem:addUIEvent("mRayPoint", function ()
            ...
            StorageUtil:saveBool1(gstor.SPLIT_DUPNEWOPENSTR .. dup_id, true)
...
function onSelectDupItem(self, dupConfigVo)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_SPLIT_DUP_PANEL)
    GameDispatcher:dispatchEvent(EventName.OPEN_SPLIT_DUP_PANEL, dupConfigVo)
end
...
function refreshAreaRedPoint(self)
    local areaConfig = split.SplitManager:getAreaConfigDic()
    for areaId, areaConfigVo in pairs(areaConfig) do
        local tabItem = self.m_tabItemList[areaId]
        if tabItem then
            if split.SplitManager:getAreaShowRed(areaConfigVo) then
...
function refreshDupRedPoint(self)
    for i = 1, #self.m_areaConfig.stage_list do
        ...
        if dupItem then
            if split.SplitManager:getDupShowRed(dup_id) then
                RedPointManager:add(dupItem:getChildTrans("mRayPoint"), nil, 116,18)
            else
...
function refreshStarAwardRedPoint(self)
    if split.SplitManager:getTaskRed() then
        RedPointManager:add(self.mBtnTask.transform, nil, 30,30)
    else
        RedPointManager:remove(self.mBtnTask.transform)
    end
    --self.ImEffect:SetActive(split.SplitManager:getStarAwardRedState())
end
