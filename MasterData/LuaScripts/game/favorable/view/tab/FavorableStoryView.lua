--[[ 
-----------------------------------------------------
@filename       : FavorableStoryView
@Description    : 战员档案-剧情
@copyright      : (LY) 2021 雷焰网络
-----------------------------------------------------
]]
module('game.favorable.view.tab.FavorableStoryView', Class.impl(TabSubView))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("favorable/tab/FavorableStoryView.prefab")
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_HERO_STORY_REWARD, self.updateView, self)
end
...
function deActive(self)
    ...
    GameDispatcher:removeEventListener(EventName.UPDATE_HERO_STORY_REWARD, self.updateView, self)
end
...
function updateView(self)
    ...
    local dic = favorable.FavorableManager:getStoryList(self.mHeroTid).favorableData
    local list = {}
    ...
    table.sort(list, function(a, b) return a.favorableLevel < b.favorableLevel end) 
    for i, v in ipairs(list) do
        v.tweenId = i
        local item = favorable.FavorableStoryItem:poolGet()
