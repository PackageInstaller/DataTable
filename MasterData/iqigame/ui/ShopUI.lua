...
local RecommendItem = require("IQIGame.UI.Shop.RecommendItem")
local LongTermItem = require("IQIGame.UI.Shop.LongTermItem")
...
function ShopUI:InitAllTags()
	self.firstTapItemList = {}
	self.secondaryTapItemList = {}
	self.recommendFirstTagItem = FirstTapItem.New(self.recommendTag.gameObject, true)

	self.recommendFirstTagItem:Refresh()
	self.recommendFirstTagItem:SetIndex(1)

	self.longTimeFirstTagItem = FirstTapItem.New(self.longTimeTag.gameObject, true)

	self.longTimeFirstTagItem:Refresh()
	self.longTimeFirstTagItem:SetIndex(2)
end

function ShopUI:InitMembers()
...
function ShopUI:InitScrollData()
	self.CellScroll = {}
	self.CellScroll = {
...
function ShopUI:InitComponent()
	...
	self.longTermItemView = LongTermItem.New(self.longTermItem, self)
	self.recommendItemView = RecommendItem.New(self.recommendItem, self)
	self.moneyCell = CurrencyCell.New(self.topCurrencyRoot)
	self.commonNpcTalk = CommonNpcTalkView.New(self.CommonTalk)
end

function ShopUI:InitDelegate()
...
function ShopUI:IsManualShowOnOpen(userData)
	return false
end

function ShopUI:GetBGM(userData)
	return 11000071
end
...
function ShopUI:OnOpen(userData)
	...
	AssetUtil.LoadImage(self, ShopUIApi:GetShopRoleHeadImagePath(PlayerModule.GetSex()), self.HeadImg:GetComponent("Image"))
...
function ShopUI:OnDestroy()
	...
	self.recommendFirstTagItem:Dispose()
	self.longTimeFirstTagItem:Dispose()
	self.longTermItemView:Dispose()
	self.recommendItemView:Dispose()
...
function ShopUI:OnSoulStoneRenderGridCell(gridCellData)
...
function ShopUI:OnGiftBagRenderGridCell(gridCellData)
	local data = ShopModule.shopDataSortList[self.curChooseTypeId][gridCellData.index + 1]
	local insID = gridCellData.gameObject:GetInstanceID()
...
function ShopUI:OnRolePerformanceGridCell(gridCellData)
	local data = ShopModule.shopDataSortList[self.curChooseTypeId][gridCellData.index + 1]
	local insID = gridCellData.gameObject:GetInstanceID()
...
function ShopUI:OnRechargeGiftRenderGridCell(gridCellData)
	local data = ShopModule.shopDataSortList[self.curChooseTypeId][gridCellData.index + 1]
	local insID = gridCellData.gameObject:GetInstanceID()
...
function ShopUI:RefreshFirstTags()
	...
	local firstTapData = ShopModule.GetShopFirstTapData(self.shopEnterType, Constant.ShopOpenType.PayShop)
...
function ShopUI:RefreshSecondaryTags(shopType)
...
function ShopUI:OnFirstTagClick(firstTapItem)
	if self.currentFirstTap then
	...
	if self.recommendFirstTagItem == firstTapItem then
		self:OnRecommendTagClick()

		return
	end

	if self.longTimeFirstTagItem == firstTapItem then
		self:OnLongTimeTagClick()
...
function ShopUI:OnSecondaryTagClick(secondaryTapItem)
	if self.currentSecondaryTag then
...
function ShopUI:OnRecommendSecondaryTagClick(secondaryTapItem)
	if self.currentSecondaryTag then
		self.currentSecondaryTag:SetSelect(false)
	end
...
function ShopUI:OnRecommendTagClick()
	self:CheckSecondaryTagWithShopShowType(Constant.ShopShowType.Recommend)

	local recommendData = ShopModule.GetRecommendShopDataByConfig()

	self.recommendItemView:RefreshAll(recommendData)

	self.curChooseType = Constant.ShopType.Recommend.Type

	self:ShowRecommondScroll()
	self.moneyCell:RefreshItem(Constant.TopMoneyType.ShopUIRecommend)
	LuaUtility.SetGameObjectShow(self.EmptyParent, #recommendData == 0)
end

function ShopUI:OnLongTimeTagClick()
	self:CheckSecondaryTagWithShopShowType(Constant.ShopShowType.LongTrem)

	local recommendData = ShopModule.GetMonthCardShopData()

	self.longTermItemView:RefreshAll(recommendData, true)

	self.curChooseType = Constant.ShopType.LongTime.Type

	self:ShowLongTermScroll()
	self.moneyCell:RefreshItem(Constant.TopMoneyType.ShopUIRecommend)
	LuaUtility.SetGameObjectShow(self.EmptyParent, #recommendData == 0)
end

function ShopUI:RefreshShopForType(isDynamic)
	if self.curChooseType == Constant.ShopType.Recommend.Type then
		-- block empty
	elseif self.curChooseType == Constant.ShopType.LongTime.Type then
		self:OnLongTimeTagClick()
	else
...
function ShopUI:ScrollChange(shopCfg)
	self:HideAllScroll()
...
function ShopUI:CheckSecondaryTagWithShopShowType(showType)
	LuaUtility.SetGameObjectShow(self.secondaryTagScrollRoot, showType == Constant.ShopShowType.Normal)
end

function ShopUI:OnNotifyChangeShopItems()
	self:RefreshShopForType(true)
	self:RefreshRedDot()
	self.commonNpcTalk:Refresh(Constant.NPCTalkGroupType.Shop, Constant.NPCTriggerType.BuyOnShop, 0)
end

function ShopUI:JumpShop(args)
	...
	elseif self.shopEnterType == Constant.ShopEnterType.Normal then
...
function ShopUI:CheckInitializationTap(userData)
...
function ShopUI:GetFirstTabItemBy(shopType)
	if shopType == Constant.ShopType.MonthCard.Type then
		return self.longTimeFirstTagItem
	end
