-- chunkname: @IQIGame\\UI\\StoryUI.lua

local StoryUI = Base:Extend("StoryUI", "IQIGame.Onigao.UI.StoryUI", {})
...
function StoryUI:InitComponent()
	...
	LuaUtility.SetRollingNumberValue(self.Com_RollingNumber, 0, false)
...
function StoryUI:OnAddListeners()
...
function StoryUI:OnRemoveListeners()
...
function StoryUI:SetButtonSkipShow(show)
	return
end

function StoryUI:OnClickButtonSkip()
	StoryModule.StartSkip()
end

function StoryUI:GetPreloadAssetPaths()
	return nil
end

function StoryUI:GetOpenPreloadAssetPaths(userData)
	return nil
end

function StoryUI:IsManualShowOnOpen(userData)
	return false
end

function StoryUI:GetBGM(userData)
	return nil
end

function StoryUI:OnOpen(userData)
...
function StoryUI:OnClose(userData)
	self.PicturePanel:Close()
	self.MoviePanel:Close()
	self.PuzzleSelectPanel:Close()
	self.DialogueSelectPanel:Close()
...
function StoryUI:OnPause()
	return
end

function StoryUI:OnResume()
	return
end

function StoryUI:OnCover()
	return
end

function StoryUI:OnReveal()
	return
end

function StoryUI:OnRefocus(userData)
	return
end

function StoryUI:OnUpdate(elapseSeconds, realElapseSeconds)
	return
end

function StoryUI:OnDepthChanged(uiGroupDepth, depthInUIGroup)
	return
end

function StoryUI:OnLoadSucceed(assetName, asset, duration, userData)
	return
end

function StoryUI:OnLoadFailed(assetName, status, errorMessage, userData)
	return
end
...
function StoryUI:OnButtonAutoDialogueClick()
...
function StoryUI:OnButtonPuzzleClick()
	UIModule.Open(Constant.UIControllerName.PuzzleUI, Constant.UILayer.UI)
end

function StoryUI:OnStoryUIRefreshEvent(clueID, needTip, clueGetType)
	self:Refresh()
...
function StoryUI:SetButtonPuzzleShow(isShow)
...
function StoryUI:RefreshAutoSpeakShow()
...
function StoryUI:ShowNPCName(NPCList)
	if not NPCList then
		return
	end
...
function StoryUI:HideNPCName()
	for i = 1, #self.NPCNameList do
		self.NPCNameList[i]:Show(false)
	end
end

function StoryUI:SetJoyStickSpeed(speed)
	self.storyJoystickView:OnSetMoveSpeed(speed)
end

function StoryUI:PlayMovie(resourceID, PlayMovieTask)
	self.MoviePanel:Open()
	self.MoviePanel:PlayMovie(resourceID, PlayMovieTask)
end

function StoryUI:ShowPicture(resourceID, posParam, ShowPictureTask)
	self.PicturePanel:Open()
	self.PicturePanel:Refresh(resourceID, posParam, ShowPictureTask)
end

function StoryUI:ClosePicture()
	self.PicturePanel:Close()
end

function StoryUI:HideReturnButton()
	LuaUtility.SetGameObjectShow(self.returnButton, false)
end

function StoryUI:ShowReturnButton()
...
function StoryUI:ChangeTimeScale(timeScale)
	if timeScale then
...
function StoryUI:SetTopRootShow(show)
	LuaUtility.SetGameObjectShow(self.TopRoot, show)
end

function StoryUI:OnGMStartFightClick()
	BehaviorTreeUtility.DebugStartFight()
end

function StoryUI:SetSurveyTargetShow(show)
...
function StoryUI:SetSurveyTargetText(CfgTextID)
...
function StoryUI:SetSurveyTargetProgress(numProgress, playAnimate)
	LuaUtility.SetRollingNumberValue(self.Com_RollingNumber, numProgress, playAnimate)
end

function StoryUI:RefreshSurveyedEffect()
...
function StoryUI:OnButtonDialogueReviewClick()
	UIModule.Open(Constant.UIControllerName.DialogueReviewUI, Constant.UILayer.UI)
	StoryUIApi:PlayCommonButtonSound()
end

function StoryUI:OnButtonChatClick()
...
function StoryUI:OnToggleRegularClick(isOn)
...
function StoryUI:SetRegularShow(show)
...
function StoryUI:RegularToggleEnabled(enabled)
	self.toggleRegular.isOn = enabled
end

function StoryUI:SetReturnButtonShow(show)
...
function StoryUI:SetButtonCollectionShow(show)
...
function StoryUI:SetAutoDialogueShow(show)
...
function StoryUI:SetStoryDescription(isShow, timeID, mainSiteID, branchSiteID, duration, SetStorySiteShow)
	if not isShow then
		LuaUtility.SetCanvasGroupShowWithGameObject(self.DescriptionPanel, false)
...
function StoryUI:EnterStoryMod()
	...
	LuaUtility.SetAnchoredPositionWithRectTransform(self.AutoDialogueRoot:GetComponent("RectTransform"), -155, -53.5)
end

function StoryUI:EnterMazeMod()
	self:_LoadMazeUI()
	LuaUtility.SetCanvasGroupShowWithGameObject(self.objectSurveyTarget, false)
	...
	LuaUtility.SetCanvasGroupShowWithGameObject(self.buttonPuzzle, false)
	LuaUtility.SetAnchoredPositionWithRectTransform(self.AutoDialogueRoot:GetComponent("RectTransform"), -355, -62)
end

function StoryUI:_LoadMazeUI()
	if self.StoryMazePanel then
		self.StoryMazePanel:Refresh()

		return
	end

	local path = "Assets/03_Prefabs/UI/_ImagePuzzleUI/storyMaze/StoryMaze.prefab"

	AssetUtil.LoadAsset(self, path, self._OnMazeUILoadSuccess)
end

function StoryUI:_OnMazeUILoadSuccess(assetName, asset, duration, userData)
	local o = GameObject.Instantiate(asset, self.MazeRoot.transform)
	local StoryMazePanel = require("IQIGame.UI.Story.Maze.StoryMazePanel")

	self.StoryMazePanel = StoryMazePanel.New(o, self)

	self.StoryMazePanel:Refresh()
end

function StoryUI:RestoreUI()
...
function StoryUI:CheckGMHideUI()
...
function StoryUI:OnClickGMButtonHideDialogue()
...
function StoryUI:SetStoryUIShow(show)
	if show then
...
function StoryUI:SetUIStateOnImagePuzzleOpen()
	LuaUtility.SetCanvasGroupShowWithGameObject(self.Com_ReturnBtn, false)
	LuaUtility.SetCanvasGroupShowWithGameObject(self.buttonCollectionRoot, false)
...
function StoryUI:SetUIStateOnImagePuzzleClose()
	local uiState = StoryModule.GetUiStateDic()
...
function StoryUI:GetCanvas()
	return self.gameObject:GetComponent("Canvas")
end
