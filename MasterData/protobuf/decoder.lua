-- chunkname: @protobuf/decoder.lua
		...
		elseif is_repeated then
			...
			return function(buffer, pos, pend, message, field_dict)
				...
				while true do
					...
					if sub(buffer, new_pos + 1, new_pos + tag_len) ~= tag_bytes or pend <= new_pos then
...
function StringDecoder(field_number, is_repeated, is_packed, key, new_default)
	...
	if is_repeated then
		...
		return function(buffer, pos, pend, message, field_dict)
			...
			while true do
				local size, new_pos
				local var_12_0, var_12_1 = DecodeVarint(buffer, pos)

				pos = var_12_1

				if pend < var_12_1 + var_12_0 then
					error("Truncated string.")
				end

				value:append(sub(buffer, pos + 1, var_12_1 + var_12_0))

				pos = var_12_1 + var_12_0 + tag_len

				if sub(buffer, var_12_1 + var_12_0 + 1, var_12_1 + var_12_0 + tag_len) ~= tag_bytes or var_12_1 + var_12_0 == pend then
		...
		return function(buffer, pos, pend, message, field_dict)
			local size, new_pos
			local var_13_0, var_13_1 = DecodeVarint(buffer, pos)

			pos = var_13_1

			if pend < var_13_1 + var_13_0 then
				error("Truncated string.")
			end

			field_dict[key] = sub(buffer, pos + 1, var_13_1 + var_13_0)
...
function BytesDecoder(field_number, is_repeated, is_packed, key, new_default)
	...
	if is_repeated then
		...
		return function(buffer, pos, pend, message, field_dict)
			...
			while true do
				local size, new_pos
				local var_15_0, var_15_1 = DecodeVarint(buffer, pos)

				pos = var_15_1

				if pend < var_15_1 + var_15_0 then
					error("Truncated string.")
				end

				value:append(sub(buffer, pos + 1, var_15_1 + var_15_0))

				pos = var_15_1 + var_15_0 + tag_len

				if sub(buffer, var_15_1 + var_15_0 + 1, var_15_1 + var_15_0 + tag_len) ~= tag_bytes or var_15_1 + var_15_0 == pend then
		...
		return function(buffer, pos, pend, message, field_dict)
			local size, new_pos
			local var_16_0, var_16_1 = DecodeVarint(buffer, pos)

			pos = var_16_1

			if pend < var_16_1 + var_16_0 then
				error("Truncated string.")
			end

			field_dict[key] = sub(buffer, pos + 1, var_16_1 + var_16_0)
...
function MessageDecoder(field_number, is_repeated, is_packed, key, new_default)
	...
	if is_repeated then
		...
		return function(buffer, pos, pend, message, field_dict)
			...
			while true do
				local size, new_pos
				local var_18_0, var_18_1 = DecodeVarint(buffer, pos)

				pos = var_18_1
				new_pos = var_18_1 + var_18_0

				if pend < var_18_1 + var_18_0 then
					error("Truncated message.")
				end
				...
				if sub(buffer, new_pos + 1, new_pos + tag_len) ~= tag_bytes or new_pos == pend then
					return new_pos
				end
			end
		end
		...
		return function(buffer, pos, pend, message, field_dict)
			...
			local size, new_pos
			local var_19_0, var_19_1 = DecodeVarint(buffer, pos)

			pos = var_19_1

			if pend < var_19_1 + var_19_0 then
				error("Truncated message.")
			end

			if value:_InternalParse(buffer, pos, var_19_1 + var_19_0) ~= var_19_1 + var_19_0 then
				error("Unexpected end-group tag.")
			end
...
function _SkipLengthDelimited(buffer, pos, pend)
	local size
	local var_22_0, var_22_1 = _DecodeVarint(buffer, pos)

	pos = var_22_1
	pos = var_22_1 + var_22_0

	if pend < var_22_1 + var_22_0 then
		error("Truncated message.")
	end
