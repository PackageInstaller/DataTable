...
local ChapterBattleCsv = require("csvdata.activity_108_battle")
...
local ActPveBattleLayer = class("ActPveBattleLayer", BaseBattleLayer)
local uiData = {
...
function ActPveBattleLayer:ctor(params)
  params = params or {}
  local battleId = params.battleId
  self.leftInfo = params.leftInfo
  self.format = params.format
  self.carbonData = ChapterBattleCsv[battleId]
...
function ActPveBattleLayer:__init()
...
function ActPveBattleLayer:sendGameEndRequest(starNum)
  ...
  local function endBattle()
    ...
    function sendGameInfo()
      ...
      game:sendData(actionCodes.Activity_act108EndBattleRpc, MsgPack.pack({
        chapterId = self.carbonData.type,
        isWin = 0 < starNum,
        key = game.role.activityStatus[tostring(108)].key or ""
      }))
      ...
      game:addResponseHandler(actionCodes.Activity_act108EndBattleRpc, function(event)
        ...
        msg.herosInfo = {}
        msg.starNum = starNum
        msg.dropItems = msg.rewards
        msg.enemyDead = self.enemyDead
        msg.levelId = self.carbonData.type
        msg.bossDegree = self.carbonData.degree
        msg.carbonType = BattleType.ActCarbon
...
function ActPveBattleLayer:initLeftInfo()
  local heroData = {}
...
function ActPveBattleLayer:loadTMXMapData()
...
function ActPveBattleLayer:enterNextPhase()
...
function ActPveBattleLayer:loadPhaseEnemys()
...
function ActPveBattleLayer:onPlayerDead(player)
...
function ActPveBattleLayer:step(dt)
  self:baseStep(dt)
...
function ActPveBattleLayer:showView()
