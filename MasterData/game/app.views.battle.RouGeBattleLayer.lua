...
local ChapterBattleCsv = require("csvdata.rouge.rouge_battle")
...
local RouGeBattleLayer = class("RouGeBattleLayer", BaseBattleLayer)
local uiData = {
...
function RouGeBattleLayer:ctor(params)
  params = params or {}
  self.floorId = params.floorId
  local enemyId = params.enemyId
  self.leftInfo = params.leftInfo
  self.format = params.format
  self.skills = params.skills
  self.buffs = params.buffs
  self.carbonData = ChapterBattleCsv[self.floorId][enemyId]
  ...
  self.killTime = globalCsv.RouGeBattleLimitTime
  self.lastKillTime = 0
  self.currentPhase = 1
...
function RouGeBattleLayer:__init()
...
function RouGeBattleLayer:sendGameEndRequest(starNum)
  local function endBattle()
    ...
    function sendGameInfo()
      ...
      game:sendData(actionCodes.Activity_endRougeRpc, MsgPack.pack({
        isWin = 0 < starNum,
        level = self.floorId
      }))
      UIHelper.showWaiting()
      game:addResponseHandler(actionCodes.Activity_endRougeRpc, function(event)
        ...
        msg.carbonType = BattleType.RouGe
        msg.format = self.format
        msg.herosHurtValue = herosHurtValue
        game:createView("rouge.RouGeBattleResultLayer", msg)
      end)
    end
...
function RouGeBattleLayer:initLeftInfo()
  local heroData = {}
...
function RouGeBattleLayer:loadTMXMapData()
...
function RouGeBattleLayer:enterNextPhase()
...
function RouGeBattleLayer:loadPhaseEnemys()
...
function RouGeBattleLayer:onPlayerDead(player)
...
function RouGeBattleLayer:step(dt)
  self:baseStep(dt)
  self:updateTime(dt)
...
function RouGeBattleLayer:updateTime(dt)
...
function RouGeBattleLayer:showView()
