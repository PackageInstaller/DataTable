local taskCsv = require("csvdata.rouge.rouge_pass_task")
local levelCsv = require("csvdata.rouge.rouge_pass_level")
local rewardCsv = require("csvdata.rouge.rouge_pass_reward")
...
local passRes = "ui/pass/pass3/"
local activityType = "93"
local RouGeAwardLayer = {}
RouGeAwardLayer.mainLayer = nil

function RouGeAwardLayer:initView(mainLayer, mainNode)
  RouGeAwardLayer.mainLayer = mainLayer
  self.mainNode = RouGeAwardLayer.mainLayer.rougeNode
  self:init()
end

function RouGeAwardLayer:init()
  RouGeAwardLayer.mainLayer.bg:setImage(passRes .. "bg.png")
  ...
  }):pos(240, 463):addTo(self.mainNode)
  ...
  self.timeLabel = display.newTTFLabel({
    text = "本期剩余时间：" .. timeStr,
    color = UIHelper.hex2rgb("#c6ceef"),
    size = 18,
    align = ui.TEXT_ALIGNMENT_CENTER
  }):center(dateBg):addTo(dateBg)
  local barBg = display.newSprite(passRes .. "bar_bg.png"):pos(560, 447):addTo(self.mainNode)
  ...
  getExpBtn:pos(887, 460):addTo(self.mainNode)
  ...
  getExpBtn:setCallback(function()
    ...
    list:size(cc.size(600, 250))
    list:setInnerContainerSize(cc.size(600, 250))
    list:setClippingEnabled(true)
    list:setBounceEnabled(true)
    list:anch(0, 0):pos(52, 40):addTo(bg)
    local rows = math.ceil(#taskCsv / 2)
    for row = 1, rows do
      local viewNode = ccui.Widget:create()
      viewNode:setContentSize(cc.size(590, 88)):addTo(list)
      for col = 1, 2 do
        local xPos = (col - 1) * 295
        local data = taskCsv[(row - 1) * 2 + col]
        if not data then
          break
        end
        local cell = display.newSprite(passRes .. "cell.png"):anch(0, 0.5):pos(xPos, 40):addTo(viewNode)
        display.newTTFLabel({
          text = data.desc,
          size = 21
        }):enableOutline(UIHelper.hex2rgb("#373e58"), 2):anch(0, 0.5):pos(18, 63):addTo(cell)
        display.newTTFLabel({
          text = string.format("获得赛季经验值x%s", data.gift),
          size = 17,
          color = UIHelper.hex2rgb("#727894")
        }):pos(168, 23):addTo(cell)
      end
    end
  ...
  local rechargeData = RechargeCsv[globalCsv.RouGeRechargeId]
  ...
  if self.curStatus.status ~= 1 then
    self.notbuy = display.newSprite(passRes .. "lock_1.png"):pos(70, 30):scale(0.82):addTo(cell_title)
  end
  ...
  self:initList()
  UIHelper.newImageView(passRes .. "bg_r.png"):anch(0, 0):pos(985, 72):addTo(self.mainNode)
...
function RouGeAwardLayer:initList(notAni)
  ...
  for idx, set in ipairs(sortData) do
    ...
    display.newTTFLabel({
      text = idx,
      size = 28,
      color = UIHelper.hex2rgb("#CCBBAE")
    }):anch(0.5, 0.5):pos(65, 335):addTo(cell)
    local yPos = 274
...
function RouGeAwardLayer:sendBuyMsg(rechargeData, _callBack)
...
function RouGeAwardLayer:showBuyPop()
...
function RouGeAwardLayer:getCanGetReward()
  local reward = {}
  for _, data in pairs(rewardCsv) do
...
function RouGeAwardLayer:getAllGetReward()
  local reward = {}
  for _, data in pairs(rewardCsv) do
    if data.need == 1 then
      local items = data.reward:toTableArray()
...
function RouGeAwardLayer:showCanGetAni(parent)
...
function RouGeAwardLayer:checkCanGet(setData)
