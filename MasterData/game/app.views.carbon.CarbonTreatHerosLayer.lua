...
local CarbonTreatHerosLayer = class("CarbonTreatHerosLayer", UIBase)

function CarbonTreatHerosLayer:ctor(params)
  UIBase.ctor(self, uiData, params)
end

function CarbonTreatHerosLayer:init(params)
  self.index = params.index
  self.condition = (params.condition or ""):toNumMap()
  self.heroInfo = {}
  self.tempHpPercent = false
  if params.battleType == BattleType.Paradise then
    self.tempHpPercent = true
    self.heroInfo = game.role.paradiseBattle.heroList[tostring(hero.id)]
  end
...
function CarbonTreatHerosLayer:initHeroInfo()
  ...
  for slot = 1, 5 do
    ...
    if self.heroList[slot] then
      ...
      local hp = self.tempHpPercent and (self.heroInfo[tostring(hero.id)] or 0) or attrValues.hp * hero.hpPercent / 1000
  ...
  display.newTTFLabel({
    text = string.format("血量增加%d%%", self.condition[2] or 0),
    size = 22,
    color = UIHelper.hex2rgb("#703803")
  }):anch(0, 0.5):pos(430, -90):addTo(self.hero)
end

function CarbonTreatHerosLayer:initCost()
  ...
  for i = 1, #self.chooseHero do
    ...
    self.costCounts[3] = self.costCounts[3] + math.ceil(hero.unit.material2 * evolutionCsv[hero.quality].materialCarry * (self.condition[1] or 0) * 12 / 7)
    self.costCounts[4] = self.costCounts[4] + math.ceil(hero.unit.material2 * evolutionCsv[hero.quality].materialCarry * (self.condition[1] or 0) * 12 / 7)
    self.costCounts[5] = self.costCounts[5] + (hero.hpPercent == 0 and 2 or hero.hpPercent < 300 and 1 or 0)
  end
  local start = 3
...
function CarbonTreatHerosLayer:showData()
  self:initCost()
end

function CarbonTreatHerosLayer:checkCostEnough()
