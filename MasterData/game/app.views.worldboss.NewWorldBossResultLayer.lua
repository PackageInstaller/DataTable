...
local NewWorldBossResultLayer = class("NewWorldBossResultLayer", UIBase)

function NewWorldBossResultLayer:ctor(params)
  UIBase.ctor(self, uiData, params)
end

function NewWorldBossResultLayer:init(params)
  self.dalteX = 0
  ...
  self.actions = {}
  self.bossIdx = params.bossIdx
  self.formatInfo = game.role.activityStatus["107"][tostring(self.bossIdx)].formation or {
    list = {},
    pos = {}
  }
...
function NewWorldBossResultLayer:showData(params)
  self:hide()
  self.heros = {}
...
function NewWorldBossResultLayer:initDelay()
  self:show()
...
function NewWorldBossResultLayer:initBaseInfo()
  local yPos = 310
...
function NewWorldBossResultLayer:initRoleInfo()
...
function NewWorldBossResultLayer:initPanelInfo()
  ...
  delay = delay + dtime
  if self.carbonType == BattleType.NewWorldboss then
    display.newTTFLabel({
      text = "造成的总伤害:" .. (self.hurt or 0),
      size = 20,
      color = UIHelper.hex2rgb("#2ee009")
    }):anch(0, 0.5):pos(205, 150):addTo(self.panel)
  else
    local desc = {
      "B级:战胜所有敌人",
      "A级:本场战斗队伍总生命损失小于60%",
...
function NewWorldBossResultLayer:initHeroInfo()
  local formation = self.formatInfo
...
function NewWorldBossResultLayer:showItems()
...
function NewWorldBossResultLayer:nextCallBack()
  game:enterScene("MainScene", {layer = "union"})
end

function NewWorldBossResultLayer:resetUI()
  local xPos = {
    0,
    250,
    1136 + self.dalteX
...
function NewWorldBossResultLayer:boss_CreateHeroHead(params)
...
function NewWorldBossResultLayer:createHeroHead(params)
  local hero = params.hero
  local unit = UnitCsv[hero.type]
  local attrs = hero:getTotalAttrValues()
  local info = params.info or {
...
function NewWorldBossResultLayer:barRunAnimation(params)
...
function NewWorldBossResultLayer:autoBattleNext()
...
return NewWorldBossResultLayer
