...
local ParadisePveBattleLayer = class("ParadisePveBattleLayer", BaseBattleLayer)
local uiData = {
...
function ParadisePveBattleLayer:ctor(params)
  params = params or {}
...
function ParadisePveBattleLayer:__init()
  ...
  if 0 > game.role.paradiseBattlePass:getv(self.carbonId, -1) and 0 < self.monsterData.story1 then
...
function ParadisePveBattleLayer:sendGameEndRequest(starNum)
  local function endBattle()
    ...
    function sendGameInfo()
      ...
      game:sendData(actionCodes.Paradise_endGameRpc, MsgPack.pack({
      ...
      game:addResponseHandler(actionCodes.Paradise_endGameRpc, function(event)
        ...
        msg.carbonType = BattleType.Paradise
  ...
  if game.role.paradiseBattlePass:getv(self.carbonId, -1) < 0 and 0 < self.monsterData.story2 then
    game:createView("talk.PlotTalkLayer", {
...
function ParadisePveBattleLayer:initLeftInfo()
  local heroData = {}
...
function ParadisePveBattleLayer:loadTMXMapData()
...
function ParadisePveBattleLayer:enterNextPhase()
...
function ParadisePveBattleLayer:loadPhaseEnemys()
...
function ParadisePveBattleLayer:onPlayerDead(player)
...
function ParadisePveBattleLayer:step(dt)
  self:baseStep(dt)
...
function ParadisePveBattleLayer:showView()
...
function ParadisePveBattleLayer:checkGuide()
  if game.role.majorGuideStep == 11 then
