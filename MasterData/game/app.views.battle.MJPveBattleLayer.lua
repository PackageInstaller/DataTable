...
local ChapterBattleCsv = require("csvdata.mijing_battle")
...
local MJPveBattleLayer = class("MJPveBattleLayer", BaseBattleLayer)
local uiData = {
...
function MJPveBattleLayer:ctor(params)
  params = params or {}
...
function MJPveBattleLayer:__init()
...
function MJPveBattleLayer:sendGameEndRequest(starNum)
  local function endBattle()
    ...
    function sendGameInfo()
      ...
      game:sendData(actionCodes.Carbon_bossEndGameRpc, MsgPack.pack({
        passTime = self.battleLogic:getTime(),
        starNum = starNum,
        check = check,
        key = (json.decode(game.role.bossCarbonInfo or "[]") or {}).key,
        bossType = self.carbonData.type,
        bossDegree = self.carbonData.degree,
        checkEnemy = self:getEnemyBattleValue(self.enemys)
      }))
      UIHelper.showWaiting()
      game:addResponseHandler(actionCodes.Carbon_bossEndGameRpc, function(event)
...
function MJPveBattleLayer:initLeftInfo()
  local heroData = {}
...
function MJPveBattleLayer:loadTMXMapData()
...
function MJPveBattleLayer:enterNextPhase()
...
function MJPveBattleLayer:loadPhaseEnemys()
...
function MJPveBattleLayer:onPlayerDead(player)
...
function MJPveBattleLayer:step(dt)
  self:baseStep(dt)
...
function MJPveBattleLayer:showView()
