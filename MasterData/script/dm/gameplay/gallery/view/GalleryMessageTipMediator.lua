-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/gallery/view/GalleryMessageTipMediator.lua

GalleryMessageTipMediator = class("GalleryMessageTipMediator", DmPopupViewMediator, _M)

GalleryMessageTipMediator:has("_developSystem", {
	is = "r"
}):injectWith("DevelopSystem")

local kBtnHandlers = {}

function GalleryMessageTipMediator:initialize()
	super.initialize(self)
end

function GalleryMessageTipMediator:dispose()
	self._viewClose = true

	super.dispose(self)
end

function GalleryMessageTipMediator:onRemove()
	super.onRemove(self)
end

function GalleryMessageTipMediator:onRegister()
...
function GalleryMessageTipMediator:enterWithData(data)
	return
end

function GalleryMessageTipMediator:onShowContent(event)
...
local Tsoul_LevelMax = ConfigReader:requireRecordById("ConfigValue", "Info_LevelMax").content

function GalleryMessageTipMediator:setUi(data)
	local showData = data.info
	local tsoulId = showData.id
	local tsoulConfig = ConfigReader:getRecordById("InfoBase", tsoulId)
	...
	local maxLevel = Tsoul_LevelMax[tostring(tsoulConfig.Rareity)]

	titlePanel:getChildByName("Text_name"):setString(Strings:get(tsoulConfig.Name))
	titlePanel:getChildByName("textlv"):setString(Strings:get("CUSTOM_FIGHT_LEVEL") .. maxLevel)
	...
	local attrNodeClone = nodeAttr:getChildByFullName("Node_1")
	local normalSkillId = tsoulConfig.Skill1
	local skillConfig = ConfigReader:getRecordById("SkillAttrEffect", normalSkillId)
	local kAttrType = skillConfig.AttrType
	local listView = nodeAttr:getChildByFullName("listView")

	listView:removeAllChildren()

	for i = 1, #kAttrType do
		local attrNode = attrNodeClone:clone()
		...
		img:loadTexture(AttrTypeImage[kAttrType[i]], ccui.TextureResType.plistType)

		local attrNum = SkillAttribute:getAddNumByConfig(skillConfig, i, maxLevel)

		text:setString(getAttrNameByType(kAttrType[i]) .. " :   " .. attrNum)
		listView:pushBackCustomItem(attrNode)
	end

	local nodeAdd = self._main:getChildByName("nodeSkill")
	local listView = nodeAdd:getChildByFullName("listView")
	local normalSkillId = tsoulConfig.Skill2
	local skillConfig = ConfigReader:getRecordById("SkillAttrEffect", normalSkillId)
	...
	local des = Strings:get(skillConfig.EffectDesc, {
	...
	local width = listView:getContentSize().width
	local des = Strings:get(tsoulConfig.ConditionDesc)
	local label = ccui.RichText:createWithXML(des, {
...
function GalleryMessageTipMediator:adjustPos(icon, direction)
