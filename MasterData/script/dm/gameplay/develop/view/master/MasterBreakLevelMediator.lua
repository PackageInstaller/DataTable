-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/develop/view/master/MasterBreakLevelMediator.lua

MasterBreakLevelMediator = class("MasterBreakLevelMediator", DmPopupViewMediator, _M)

MasterBreakLevelMediator:has("_developSystem", {
	is = "r"
}):injectWith("DevelopSystem")

local kNum = 6
local kHeroStiveId = "IR_HeroStive"
local HeroStar_StiveChange = ConfigReader:getDataByNameIdAndKey("ConfigValue", "MasterStar_StiveChange", "content")
...
function MasterBreakLevelMediator:initialize()
	super.initialize(self)
end

function MasterBreakLevelMediator:dispose()
	self:closeAllScheduler()
	super.dispose(self)
end

function MasterBreakLevelMediator:onRegister()
	...
	self._bgWidget = bindWidget(self, "main.bgNode", PopupNormalWidget, {
		...
		title = Strings:get("Master_Star_UI_Preview_SC"),
		title1 = Strings:get("Master_Star_UI_Preview_EN"),
		bgSize = {
			width = 837,
			height = 574
		}
...
function MasterBreakLevelMediator:enterWithData(data)
	self:initData(data)
	self:initView()
	self:initTableView()
	self:refreshView()
...
function MasterBreakLevelMediator:initData(data)
	self._callback = data.callback
	self._heroId = data.masterId
...
function MasterBreakLevelMediator:initView()
	...
	local icon = ccui.ImageView:create("asset/items/IR_MasterStive.png")

	icon:addTo(node):setScale(0.28)
	...
	text:setString(Strings:get("Master_Star_UI_Remind6"))
	...
	local str = Strings:get("Master_Star_UI_Energy_Tip", {
...
function MasterBreakLevelMediator:initTableView()
...
function MasterBreakLevelMediator:createTeamCell(cell, index)
	...
	if self._stiveItem and index == 1 then
		...
		index = self._stiveItem and index - 1 or index
...
function MasterBreakLevelMediator:getCellNum()
	local num = 0
	local index = 0
...
function MasterBreakLevelMediator:refreshView()
...
function MasterBreakLevelMediator:onClickQuick()
	self:resetExp()
...
function MasterBreakLevelMediator:getQuickItems()
	...
	if selectData.canUseOwn == "1" and self._selfItem then
		...
		if self._consumeTemp[itemId] then
			if not self._consumeTemp[itemId].eatCount then
				local eatCount = 0
...
function MasterBreakLevelMediator:onClickSure()
	if self._curExp < self._needNum then
		self:dispatch(ShowTipEvent({
	...
	local function callback()
		self._masterSystem:setMasterBreakUpItem(self._curExp, itemsTemp)
...
function MasterBreakLevelMediator:onClickRule()
	if not self._ruleNode:isVisible() then
		self._ruleNode:setVisible(true)
	end
end

function MasterBreakLevelMediator:onOkClicked()
	self._masterSystem:resetMasterBreakUpItem()
...
function MasterBreakLevelMediator:onClickSelect()
...
function MasterBreakLevelMediator:refreshProgrView(nowExp)
...
function MasterBreakLevelMediator:onEatItemClicked(sender, eventType, isAdd)
	if eventType == ccui.TouchEventType.began then
		self._selectItemId = sender.data.itemId
		self._touchAddCount = 0
		self._touchNode = sender
		self._canTouch = isAdd and self:checkBeginEatCount(sender) or self:checkBeginMinusCount(sender)
...
function MasterBreakLevelMediator:getCostNumByItemCount(count, sender)
...
function MasterBreakLevelMediator:getSpeedByItemCount(count, sender)
	local itemId = sender.data.itemId
	local rarity = sender.data.rarity
	local data = ConfigReader:getRecordById("ExpConsume", tostring(itemId or "200004"))
...
function MasterBreakLevelMediator:checkBeginEatCount(sender)
...
function MasterBreakLevelMediator:checkBeginMinusCount(sender)
	local data = sender.data
...
function MasterBreakLevelMediator:canEatItem(itemNode, hasTip)
	...
	if self._addExp >= self._needNum then
		if hasTip then
			local tip = Strings:get("Hero_Star_UI_Remind1")
...
function MasterBreakLevelMediator:canMinusItem(itemNode, hasTip)
	local data = itemNode.data

	if data.eatCount <= 0 and (not hasTip or true) then
		return false
	end
...
function MasterBreakLevelMediator:touchBegin(sender)
	if self._isAdd then
...
function MasterBreakLevelMediator:createLongAddSch(sender)
	self:closeSleepScheduler()
...
function MasterBreakLevelMediator:touchEnd(sender)
	local data = sender.data
...
function MasterBreakLevelMediator:resetExp()
	self._eatExp = 0
	self._curExp = 0
...
function MasterBreakLevelMediator:touchSleepEnd(sender)
	self:closeSleepScheduler()
...
function MasterBreakLevelMediator:longAdd(sender)
...
function MasterBreakLevelMediator:eatOneItem(sender)
	local isAdd = self._isAdd

	if not (isAdd and self:canEatItem(sender, true) or self:canMinusItem(sender, true)) then
		self:closeLongAddScheduler()
...
function MasterBreakLevelMediator:refreshEatNode(sender)
	local data = sender.data
...
function MasterBreakLevelMediator:progrShow(sender)
	if self._isAdd then
...
function MasterBreakLevelMediator:checkCanEatItem(sender)
...
function MasterBreakLevelMediator:checkCanMinusItem(sender)
	local data = sender.data
...
function MasterBreakLevelMediator:refreshEatOnceExp(nextExp)
...
function MasterBreakLevelMediator:refreshMinusOnceExp(nextExp)
...
function MasterBreakLevelMediator:progrNormalShow(closeCheck, sender)
...
function MasterBreakLevelMediator:progrNormalMinusShow(closeCheck, sender)
	local nextExp = self._needNum
...
function MasterBreakLevelMediator:createProgrScheduler(sender)
	self:closeProgrScheduler()
...
function MasterBreakLevelMediator:closeProgrScheduler()
...
function MasterBreakLevelMediator:createSleepScheduler(sender)
	self:closeSleepScheduler()
...
function MasterBreakLevelMediator:closeSleepScheduler()
...
function MasterBreakLevelMediator:createLongAddScheduler(time, sender)
	self:closeLongAddScheduler()
...
function MasterBreakLevelMediator:closeLongAddScheduler()
...
function MasterBreakLevelMediator:closeAllScheduler()
	self:closeLongAddScheduler()
	self:closeSleepScheduler()
	self:closeProgrScheduler()
end
