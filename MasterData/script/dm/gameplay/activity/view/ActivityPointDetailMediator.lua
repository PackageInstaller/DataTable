-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/activity/view/ActivityPointDetailMediator.lua
...
function ActivityPointDetailMediator:refreshTeamView()
	...
	local masterIcon = IconFactory:createRoleIconSpriteNew({
		...
		id = (self:getOwnMasterId(self._pointId) or nil) and self:getOwnMasterRoleModel(self:getOwnMasterId(self._pointId))
	})
...
function ActivityPointDetailMediator:onClickChallenge()
	...
	if not pass then
		...
		var_34_0.tip = tips or Strings:find("ACTIVITY_ENERGY_NOT_ENOUGH")

		self:dispatch(ShowTipEvent(var_34_0))
...
function ActivityPointDetailMediator:onChallenge()
	...
	self._activitySystem:requestDoChildActivity(activityId, subActivityId, {
		...
		if rsdata.resCode == GS_SUCCESS then
			if self._point:isBoss() then
				self._activitySystem:enterActstageBattle(rsdata.data, activityId, subActivityId, true)
			else
...
function ActivityPointDetailMediator:reachBattleCondition()
	...
	if config then
		...
		containPower = func ~= "getItemCount" and self._bagSystem[func](self._bagSystem, itemId) or self._bagSystem:getItemCount(itemId)
...
function ActivityPointDetailMediator:onRequsetSwip(times)
	...
	param.type = litTypeMap[self._parent._stageType]
	param.doActivityType = 107
	param.mapId = self._mapId
	param.pointId = self._pointId
	param.times = tostring(times)
