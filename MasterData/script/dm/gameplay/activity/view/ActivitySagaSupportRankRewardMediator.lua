-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/activity/view/ActivitySagaSupportRankRewardMediator.lua
...
function ActivitySagaSupportRankRewardMediator:getSubsboard(subIds)
	...
	var_6_0.rankEnd = self:getRankSystem():getRequestRankCountPerTime()

	local sendData = var_6_0
...
function ActivitySagaSupportRankRewardMediator:touchForTableView(curTableView, idx)
	...
	if (kMinRefreshHeight < offsetY and viewHeight < contentHeight and #self["tableViewData" .. idx] < self._rankSystem:getMaxRank() or nil) and true then
