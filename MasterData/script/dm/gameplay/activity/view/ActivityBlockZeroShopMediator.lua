-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/activity/view/ActivityBlockZeroShopMediator.lua

ActivityBlockZeroShopMediator = class("ActivityBlockZeroShopMediator", DmAreaViewMediator, _M)

ActivityBlockZeroShopMediator:has("_activitySystem", {
	is = "r"
}):injectWith("ActivitySystem")
ActivityBlockZeroShopMediator:has("_developSystem", {
	is = "r"
}):injectWith("DevelopSystem")
ActivityBlockZeroShopMediator:has("_surfaceSystem", {
	is = "r"
}):injectWith("SurfaceSystem")
ActivityBlockZeroShopMediator:has("_systemKeeper", {
	is = "rw"
}):injectWith("SystemKeeper")
ActivityBlockZeroShopMediator:has("_shopSystem", {
	is = "r"
}):injectWith("ShopSystem")

local kBtnHandlers = {}
local kNums = 4
local kCellHeight = 365
local kCellWidth = 832

function ActivityBlockZeroShopMediator:initialize()
	super.initialize(self)
end

function ActivityBlockZeroShopMediator:dispose()
	super.dispose(self)
end

function ActivityBlockZeroShopMediator:onRegister()
	super.onRegister(self)

	self._heroSystem = self._developSystem:getHeroSystem()

	self:mapButtonHandlersClick(kBtnHandlers)
	self.mapEventListener(self, self:getEventDispatcher(), EVT_PASSPORT_REFRESH, self, self.refreshView)
end

function ActivityBlockZeroShopMediator:setupTopInfoWidget()
...
function ActivityBlockZeroShopMediator:resumeWithData()
	self:initData()
	self:initView()
...
function ActivityBlockZeroShopMediator:refreshView()
	self:initData()
	self:refreshScrollView()
	self:setTalkView()
	self:setTimeUi()
...
function ActivityBlockZeroShopMediator:enterWithData(data)
	self._data = data
...
function ActivityBlockZeroShopMediator:refreshTableView()
	if self._rewardView then
		self._rewardView:reloadData()
	end
end

function ActivityBlockZeroShopMediator:initData()
...
function ActivityBlockZeroShopMediator:initView()
	...
	self._cellHeight = self._cellHeight + 25
	self._layoutClone = self:getView():getChildByFullName("layoutClone")

	self._layoutClone:setVisible(false)
...
function ActivityBlockZeroShopMediator:setTalkView()
	self._showTalk = false

	self:onClickTalkRole()
end

function ActivityBlockZeroShopMediator:onClickTalkRole(sender)
...
function ActivityBlockZeroShopMediator:playAnimation(sender)
...
function ActivityBlockZeroShopMediator:refreshScrollView()
	...
	self._scrollView:setScrollBarOpacity(0)

	self._cells = {}

	for i = 1, length do
		local layout = self._layoutClone:clone()
...
function ActivityBlockZeroShopMediator:createCell(cell, index)
	for i = 1, kNums do
		...
		if itemData then
			...
			clonePanel:setPosition(20 + (i - 1) * (self._cellWidth + 10), 0)
			clonePanel:setTag(i)
...
function ActivityBlockZeroShopMediator:setInfo(clonePanel, itemData, itemIndex)
	...
	local duihuanText = clonePanel:getChildByFullName("cell.info_panel.duihuan_text")
	...
	if not rewards[1] then
		...
		if rewards and rewards[1] then
			...
			rewardIcon:setPosition(cc.p(8, 47))
		...
		curTime:setPositionX(duihuanText:getPositionX() + duihuanText:getContentSize().width - 7)
		...
		moneyIcon:setPositionX(moneyLayout:getContentSize().width / 2 - (moneyIcon:getContentSize().width + money:getContentSize().width) / 2 + 6)
		money:setPositionX(moneyIcon:getPositionX() + moneyIcon:getContentSize().width)
		moneyLayout:setPositionY(14)
		money:setPositionY(18)
...
function ActivityBlockZeroShopMediator:onClickGet(sender, eventType, itemData)
...
function ActivityBlockZeroShopMediator:setTimeUi()
...
function ActivityBlockZeroShopMediator:onClickBack(sender, eventType)
	self:dismiss()
end
