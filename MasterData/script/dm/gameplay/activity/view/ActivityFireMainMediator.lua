-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/activity/view/ActivityFireMainMediator.lua

ActivityFireMainMediator = class("ActivityFireMainMediator", DmAreaViewMediator, _M)

ActivityFireMainMediator:has("_activitySystem", {
	is = "r"
}):injectWith("ActivitySystem")
ActivityFireMainMediator:has("_developSystem", {
	is = "r"
}):injectWith("DevelopSystem")
ActivityFireMainMediator:has("_surfaceSystem", {
	is = "r"
...
function ActivityFireMainMediator:initialize()
...
function ActivityFireMainMediator:dispose()
	self:stopTimer()
	super.dispose(self)
end

function ActivityFireMainMediator:stopTimer()
...
function ActivityFireMainMediator:onRegister()
	super.onRegister(self)

	self._heroSystem = self._developSystem:getHeroSystem()

	self:setupTopInfoWidget(kBtnHandlers)
	self.mapEventListener(self, self:getEventDispatcher(), EVT_PLAYER_SYNCHRONIZED, self, self.refreshView)
...
function ActivityFireMainMediator:setTimerString(timeStr, timeTip, key)
	if not self._richTextRemainTime["timeText" .. key] then
		local remainLabel = self._stagePanel:getChildByFullName("Panel_" .. key .. ".Text_time")

		remainLabel:setString("")

		local timeText_Rish = ccui.RichText:createWithXML("", {})

		timeText_Rish:setAnchorPoint(remainLabel:getAnchorPoint())
		timeText_Rish:setPosition(cc.p(remainLabel:getPosition()))
		timeText_Rish:addTo(remainLabel:getParent())

		self._richTextRemainTime["timeText" .. key] = timeText_Rish
	end
	...
	self._richTextRemainTime["timeText" .. key]:setString(txt)
end

function ActivityFireMainMediator:setupTopInfoWidget()
...
function ActivityFireMainMediator:updateInfoWidget()
...
function ActivityFireMainMediator:initStageName()
...
function ActivityFireMainMediator:enterWithData(data)
...
function ActivityFireMainMediator:resumeWithData()
	self:doReset()
end

function ActivityFireMainMediator:initData()
...
function ActivityFireMainMediator:initView()
	self:initInfo()
	self:initStageName()
	self:initTimer()
	self:initRoleTimer()
...
function ActivityFireMainMediator:playBackgroundMusic()
	local bgm = self._activity:getBgm()

	AudioEngine:getInstance():playBackgroundMusic(bgm)
end

function ActivityFireMainMediator:initInfo()
	...
	for index, mapId in pairs(KMapIds) do
		for i = 1, 3 do
			...
			if redPoint then
				redPoint:setVisible(self._blockActivity[mapId] and self._blockActivity[mapId]:hasRedPointByType((tag == 3 or nil) and StageType.kHard))
			end
		end
	end
...
function ActivityFireMainMediator:getStatus(mapId, tag)
	if tag == nil then
		tag = 1
	end
	...
	local startTime = self._blockActivity[mapId]:getStartTime() / 1000

	if tag == 2 then
		startTime = self._blockActivity[mapId]:getMapIdStartTimeByStageType(StageType.kElite)
	end

	if tag == 3 then
		startTime = self._blockActivity[mapId]:getMapIdStartTimeByStageType(StageType.kHard)
	end
	...
	if tag == 2 then
		local pass = act:getStageByStageType(StageType.kElite):isUnlock()

		if not pass then
			return {
				tip = "ActivityBlock_UI_26",
				status = MAPOPENSTATUS.KNoOpenNotAllPass
			}
		end
	end
	...
	if tag == 3 then
		local pass = act:getStageByStageType(StageType.kHard):isUnlock()
...
function ActivityFireMainMediator:initTimer()
	...
	for index, mapId in pairs(KMapIds) do
		if self._blockActivity[mapId] then
			...
			local startTime_N = self._blockActivity[mapId]:getStartTime() / 1000
			local startTime_E = self._blockActivity[mapId]:getMapIdStartTimeByStageType(StageType.kElite)
			local startTime_H = self._blockActivity[mapId]:getMapIdStartTimeByStageType(StageType.kHard)
			...
			if remoteTimestamp < endTime then
				...
				local function checkTimeFunc()
					...
					if remainTime <= 0 then
						...
						self:setTimerString("", Strings:get("ActivityBlock_UI_8"), "1")
						self:setTimerString("", Strings:get("ActivityBlock_UI_8"), "2")
						self:setTimerString("", Strings:get("ActivityBlock_UI_8"), "3")

						return
					end

					if startTime_N < remoteTimestamp then
						local str = self:getFormatTimeStr(remainTime)

						self:setTimerString(str, Strings:get("ActivityBlock_UI_24", {
							time = ""
						}), "1")
					else
						local str = self:getFormatTimeStr(startTime_N - remoteTimestamp)

						self:setTimerString(str, Strings:get("ActivityBlock_UI_25", {
							time = ""
						}), "1")
					end

					if startTime_E < remoteTimestamp then
						local str = self:getFormatTimeStr(remainTime)
						...
						local str = self:getFormatTimeStr(startTime_E - remoteTimestamp)
					...
					if startTime_H < remoteTimestamp then
						local str = self:getFormatTimeStr(remainTime)
						...
						local str = self:getFormatTimeStr(startTime_H - remoteTimestamp)
			...
			if status.status == MAPOPENSTATUS.KEnd then
				self:setTimerString("", Strings:get("ActivityBlock_UI_8"), "1")
				self:setTimerString("", Strings:get("ActivityBlock_UI_8"), "2")
				self:setTimerString("", Strings:get("ActivityBlock_UI_8"), "3")
			else
				self:setTimerString("", "", "1")
				self:setTimerString("", "", "2")
				self:setTimerString("", "", "3")
			end
		end
	end
end

function ActivityFireMainMediator:initRoleTimer()
	if self._roleTimer then
		self._roleTimer:stop()
	end
...
function ActivityFireMainMediator:updateRolePanel()
	...
	if self._roleUrl then
		local showSurface = self._roleUrl ~= ""
...
function ActivityFireMainMediator:doReset()
	self:stopTimer()
...
function ActivityFireMainMediator:refreshView()
	self:doReset()
end

function ActivityFireMainMediator:onClickBack(sender, eventType)
	self:dismiss()
end

function ActivityFireMainMediator:onClickStage(sender)
	...
	local stageType = StageType.kNormal
	...
	if st.status == MAPOPENSTATUS.KNoOpenTime then
		...
		self:dispatch(ShowTipEvent({
			tip = (stageType == StageType.kHard or nil) and Strings:get("HMap_TimeLimit_Tips", {
				time = times
			})
		}))
	...
	if st.status == MAPOPENSTATUS.KNoOpenNotAllPass then
		local tipStr = Strings:get(self._activityConfig.SubActivityPassPointTip, {
			title = title
		})

		self:dispatch(ShowTipEvent({
			tip = (st.tip or nil) and Strings:get(st.tip)
		}))
	...
	self._activitySystem:enterBlockMap(self._activityId, mapId, stageType, true)
end

function ActivityFireMainMediator:onClickAttrAdd()
...
function ActivityFireMainMediator:onClickRule()
...
function ActivityFireMainMediator:onClickTask()
...
function ActivityFireMainMediator:onClickTeam()
	local start = false
	local over = true
...
function ActivityFireMainMediator:onClickSurface()
...
function ActivityFireMainMediator:onClickLeft()
...
function ActivityFireMainMediator:onClickRight()
	if not self._canChangeHero then
		return
	end
...
function ActivityFireMainMediator:playStageAnim()
...
function ActivityFireMainMediator:onClickRolePanel()
	local model = self._roles[self._roleIndex].model
	local type_ = self._roles[self._roleIndex].type
	local modelId, heroId
...
function ActivityFireMainMediator:getFormatTimeStr(remainTime)
	local str = ""
