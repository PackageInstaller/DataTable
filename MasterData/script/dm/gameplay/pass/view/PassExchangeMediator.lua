-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/pass/view/PassExchangeMediator.lua

PassExchangeMediator = class("PassExchangeMediator", DmPopupViewMediator, _M)

PassExchangeMediator:has("_developSystem", {
	is = "r"
}):injectWith("DevelopSystem")
PassExchangeMediator:has("_passSystem", {
	is = "r"
}):injectWith("PassSystem")
PassExchangeMediator:has("_activitySystem", {
	is = "r"
...
function PassExchangeMediator:initialize()
	super.initialize(self)
end

function PassExchangeMediator:dispose()
	self:stopEffect()
	super.dispose(self)
...
function PassExchangeMediator:onRegister()
	super.onRegister(self)
	self:mapButtonHandlersClick(kBtnHandlers)
	self.mapEventListener(self, self:getEventDispatcher(), EVT_PASSPORT_REFRESH, self, self.doPassPortRefresh)
end

function PassExchangeMediator:removeTableView()
...
function PassExchangeMediator:stopEffect()
	if self._passSpineTalkVoice ~= nil then
		self._passSpineTalkVoice:stopEffect()
	end
end

function PassExchangeMediator:setTabIndex(index)
	self._tabIndex = index
end

function PassExchangeMediator:setupView(parent)
	self._parent = parent
...
function PassExchangeMediator:doPassPortRefresh()
	self:refreshData()
	self:refreshView()
end

function PassExchangeMediator:initData()
	self._showPassEnter, self._showPass, self._showPassShop = self._passSystem:checkShowPassAndPassShop()
	self._exchangeList = self._passSystem:getPassListModel():getPassExchangeList()
end

function PassExchangeMediator:initWidget()
	...
	self._passSpineTalkVoice:addSpine(self._roleNode, 1)
	self:updateRemainTime()
end

function PassExchangeMediator:initTableView()
	local viewSize = self._viewPanel:getContentSize()
	local width, height = viewSize.width, self._cloneCell:getContentSize().height
...
function PassExchangeMediator:createCell(cell, index)
...
function PassExchangeMediator:refreshData(fromParent)
	self._exchangeList = self._passSystem:getPassListModel():getPassExchangeList()
	self._showPassEnter, self._showPass, self._showPassShop = self._passSystem:checkShowPassAndPassShop()
end

function PassExchangeMediator:refreshView(hasAnim)
...
function PassExchangeMediator:onClickGet(sender, eventType, onePassExchange)
	...
	elseif eventType == ccui.TouchEventType.ended and self._isReturn then
		...
		local subActivityId = self._passSystem:getSubActivityID()
...
function PassExchangeMediator:setTalkView()
	if self._passSpineTalkVoice ~= nil then
...
function PassExchangeMediator:playAnimation()
	local desStr = ""
...
function PassExchangeMediator:runStartAction()
...
function PassExchangeMediator:runListAnim()
	self:getView():stopAllActions()
...
function PassExchangeMediator:updateRemainTime()
...
function PassExchangeMediator:refreshRemainTime(remainTime)
	local str = ""
	...
	if getCurrentLanguage() ~= GameLanguageType.CN then
		local image = self._timeNode:getChildByFullName("Image_14")

		image:setContentSize(cc.size(240, 45))
	end
	...
	contentText:setPosition(cc.p(-70, 0))
	contentText:renderContent(180, 0, true)
end
