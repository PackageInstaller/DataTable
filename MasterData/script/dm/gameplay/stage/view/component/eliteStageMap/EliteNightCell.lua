-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/stage/view/component/eliteStageMap/EliteNightCell.lua

EliteNightCell = class("EliteNightCell", ChapterBase)

local kBtnHandlers = {}

function EliteNightCell:initialize(data)
...
function EliteNightCell:update(stageType, redPointTab)
	...
	self:initChapterPanel(self._chapter1, "E16", callFunc)
	...
	self:initChapterPanel(self._chapter2, "E17", callFunc)
...
function EliteNightCell:onScroll(offX)
	return
end

function EliteNightCell:initView()
	self:initWidget()
	self:initAnim()
end

function EliteNightCell:initWidget()
...
function EliteNightCell:initAnim()
