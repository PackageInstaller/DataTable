-- chunkname: @/tmp/or_script/lua_compile/dm/gameplay/stage/view/component/eliteStageMap/EliteTenCell.lua

EliteTenCell = class("EliteTenCell", ChapterBase)

local kBtnHandlers = {}
local onPlayAnim

function EliteTenCell:initialize(data)
...
function EliteTenCell:update(stageType, redPointTab)
	self._stageType = stageType

	local function callFunc()
		local contentData = {}

		contentData.stageType = self._stageType
		contentData.tabOffX = self._mediator._tableView:getContentOffset().x

		self._mediator:getStageSystem():setContentData(contentData)
	end

	self:initChapterPanel(self._chapter1, "E18", callFunc)
	self._chapter1:setPosition(cc.p(self._chapter1RowX, self._chapter1RowY))
	self._chapter1.redPoint:setVisible(self:checkRedPoint(redPointTab, 18))
	self:initChapterPanel(self._chapter2, "E19", callFunc)
	self._chapter2:setPosition(cc.p(self._chapter2RowX, self._chapter2RowY))
	self._chapter2.redPoint:setVisible(self:checkRedPoint(redPointTab, 19))
end

function EliteTenCell:initView()
	self:initWidget()
	self:initAnim()
end

function EliteTenCell:initWidget()
...
function EliteTenCell:onScroll(offX)
	return
end

function EliteTenCell:initAnim()
	local mc = cc.MovieClip:create("shibajiuchang_shibajiuchang")
	local imageBgPanel = self._main:getChildByName("BG")

	mc:setScale(0.75)
	mc:addTo(imageBgPanel)
	mc:setPosition(cc.p(1115, 330))
