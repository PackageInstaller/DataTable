-- chunkname: @/tmp/or_script/lua_compile/foundation/crypto/md5.lua
...
if bit ~= nil then
	...
	function bit_or(m, n)
		...
		for i = 1, #tbl_m do
			tbl[i] = tbl_m[i] == 0 and tbl_n[i] == 0 and 0 or 1
		end
	...
	function bit_and(m, n)
		...
		for i = 1, #tbl_m do
			tbl[i] = (tbl_m[i] == 0 or tbl_n[i] == 0) and 0 or 1
		end
	...
	function bit_xor(m, n)
		...
		for i = 1, #tbl_m do
			tbl[i] = tbl_m[i] ~= tbl_n[i] and 1 or 0
		end
...
local function z(f, a, b, c, d, x, s, ac)
	a = bit_and(a + f(b, c, d) + x + ac, 4294967295)
