-- chunkname: @/tmp/or_script/lua_compile/cocos/framework/display.lua

local display = {}
local director = cc.Director:getInstance()
local view = director:getOpenGLView()

assert(view ~= nil, "OpenGLView is uninitialized!")
...
local function setDesignResolution(r, framesize)
	if r.autoscale == "FILL_ALL" then
		...
		if r.autoscale == "FIXED_WIDTH" then
			...
			view:setDesignResolutionSize(framesize.width / scaleX, framesize.height / scaleX, cc.ResolutionPolicy.NO_BORDER)
		elseif r.autoscale == "FIXED_HEIGHT" then
			view:setDesignResolutionSize(framesize.width / scaleY, framesize.height / scaleY, cc.ResolutionPolicy.NO_BORDER)
...
if type(CC_DESIGN_RESOLUTION) == "table" then
	...
	for key, value in pairs(CC_DESIGN_RESOLUTION) do
		temp[key] = value
	end

	display.setAutoScale(temp)
end
...
function display.newScene(name, params)
	...
	local scene = not params.physics and cc.Scene:create() or cc.Scene:createWithPhysics()
...
function display.wrapScene(scene, transition, time, more)
	...
	if display.SCENE_TRANSITIONS[key] then
		...
		time = time or 0.2
		more = more or t[2]
		scene = more ~= nil and cls:create(time, scene, more) or cls:create(time, scene)
	else
...
function display.newLayer(...)
	...
	elseif c == 2 then
		...
		local p2 = params[2]
		local var_10_0 = assert
		local var_10_1 = type(p2) == "table" and (p2.width or p2.r) or false

		var_10_0((false or nil) and true, "display.newLayer() - invalid paramerter 2")

		layer = p2.r and cc.LayerGradient:create(color1, cc.convertColor(p2, "4b")) or cc.LayerColor:create(color1, p2.width, p2.height)
	elseif c == 3 then
		local color1 = cc.convertColor(params[1], "4b")
		local p2 = params[2]
		local p2type = type(p2)

		layer = p2type == "table" and cc.LayerGradient:create(color1, cc.convertColor(p2, "4b"), params[3]) or cc.LayerColor:create(color1, p2, params[3])
	end
...
function display.newSprite(source, x, y, params)
	...
	while true do
		...
		if sourceType == "string" then
			...
			sprite = not scale9 and spriteClass:create(source) or spriteClass:create(source, params.rect, params.capInsets)
		...
		if sourceType == "cc.Texture2D" then
