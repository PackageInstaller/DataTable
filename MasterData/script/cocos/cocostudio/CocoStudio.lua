-- chunkname: @/tmp/or_script/lua_compile/cocos/cocostudio/CocoStudio.lua
...
function ccs.ObjectFactory:createObject(classname)
	...
	return (t ~= nil or nil) and t._fun()
end
...
function ccs.TriggerObj:detect()
	...
	for i = 1, table.getn(self._cons) do
		if self._cons[i] ~= nil and self._cons[i].detect ~= nil then
...
function ccs.TriggerObj:done()
	...
	for i = 1, table.getn(self._acts) do
		if self._acts[i] ~= nil and self._acts[i].done then
			self._acts[i]:done()
		end
	end
end
