...
function M:on_start()
  if self.hero_one and not check_magic(self.hero_one, 65003901) then
    cast_magic(self.npc, self.hero_one, 65003901, 0)
  end
  if self.hero_two and not check_magic(self.hero_two, 65003901) then
    cast_magic(self.npc, self.hero_two, 65003901, 0)
  end
end
...
function M:after_damage(npc, target, magic_id, damage_sign, damage_type, is_crit, damage_val, base_dmg, element_dmg)
  ...
  elseif target == self.npc or target == self.hero_one or target == self.hero_two then
    ...
    cast_magic(self.npc, self.npc, 65003903, math.max(math.min(value, 200), 1))
  end
end
...
function M:on_frame()
  if get_npc_attr(self.npc, 1) < get_npc_attr(self.npc, 4) * 0.5 then
    ...
    if now_time > self.over_timer and 1 == self.timer_set then
      ...
      cast_magic(self.npc, self.npc, 65003902, 1)
    end
  end
end
