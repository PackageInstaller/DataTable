...
local rampMgr = import("character.base.base_rampageManager")
...
local per1, per2 = tool:randByTime(0.1, 0.025)

function M:_init(npc)
end

function M:on_start()
  self.level = get_ability_level(60022) or 1
  if self.level > 1 then
    per1, per2 = tool:randByTime(0.1, 0.05)
  end
...
function M:after_damage_target(target, magic_id, damage_sign, damage_type, is_crit, damage_val, base_dmg, element_dmg, skill_id, missile)
  ...
  if rampMgr:isOn() and per2 and get_magic_num(self.npc, 61710011) < 4 then
