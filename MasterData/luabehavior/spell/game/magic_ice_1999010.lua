...
function M:_init(npc)
  ...
  cast_magic(npc, npc, 1999012, 0)
  if not check_magic(npc, Const.MONSTER_BE_HIT.SLOMO_OTHERS) then
    ...
    cast_missile(npc, npc, nil, nil, 10019900301, 0)
  end
end

function M:creat_magic()
  if 0 == self.control then
    if not check_magic(self.target, 60510081) then
      cast_magic(self.npc, self.npc, 1999011, 1)
      self.control = 1
    else
      self.control = 1
    end
