...
function M:on_enter()
  ...
  self:set_sceneobj_visible("sc0102 (3)", false)
  self:cast_magic(Global.hero, Global.hero, 4032098, 0)
  if not self.is_add_npc then
    self:add_npc_by_pos_key(1, 300100202, "A1", 0, 0, "A4", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100302, "A4", 0, 0, "A1", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100202, "A2", 0, 0, "A5", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100202, "A3", 0, 0, "A6", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100302, "A5", 0, 0, "A2", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100302, "A6", 0, 0, "A3", 0, 0, 4, 1)
    self:add_functional_npc(2, "C1", "C1", {npc_id = 102150})
  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and not self:is_pass_room() then
        ...
        self:open_all_transmit()
        self:set_scene_camera(109)
        self:timer_call_out(nil, function()
          self:set_scene_tips(100111, true)
        end, 0.8)
      end
    end
  end
...
function M:on_leave()
  self:set_sceneobj_visible("sc0102 (3)", true)
end
