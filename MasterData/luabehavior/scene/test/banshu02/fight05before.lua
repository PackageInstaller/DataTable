...
function M:on_enter()
  if not self.is_accept_task then
    self:accept_task_group(9025)
    self.is_accept_task = true
  end
  self:skip_finish_anim(true)
  self:set_mapview_transmit(false)
  self:add_npc_by_pos_key(1, 300100202, "A3", 0, 0, "M5", 0, 0, 4, 1)
  self:add_npc_by_pos_key(1, 300100102, "A1", 0, 0, "A1", 0, 0, 4, 1)
  self:add_npc_by_pos_key(1, 300100302, "A2", 0, 0, "A2", 0, 0, 4, 1)
  self:cast_magic(Global.hero, Global.hero, 4032098, 0)
  local x, y = self:get_scene_map_pos_postion("tp1")
