...
function M:on_enter()
  ...
  if not self.is_add_npc then
    ...
    self:play_timeline("timeline1_4", 2, 1, nil, false)
    self.is_add_npc = true
  end
  self:set_sceneobj_visible("Fx_henji", true)
...
function M:on_story_begin(story_id, step_id, param)
  if 1011002 == story_id and 8 == step_id then
    self:cast_magic(Global.hero, Global.hero, 4032100, 0)
  end
end

function M:on_story_end(story_id, step)
  if 1011002 == story_id then
    self:fight_end(true)
    self:open_all_transmit()
    self:complete_task(900101)
  end
end
...
function M:on_func_npc_interact_end(func_npc_id)
  if 100104 == func_npc_id then
    ...
    self:play_story(1011002)
  end
  if 100105 == func_npc_id then
    self:play_story(1011007)
  end
  if 100106 == func_npc_id then
    self:play_story(1011008)
  end
end

function M:on_timeline_end(timeline_name)
  self:set_scene_tips(100113, true)
end
...
function M:on_enter_area(npc, area)
  ...
  if area.Key == "M0" then
    self:remove_area("M0", nil)
    self:set_scene_tips(100107, true)
    self:timer_call_out(nil, function()
      self:set_scene_tips(100108, true)
    end, 3)
  end
