...
function M:on_npc_show_end(id, type)
  ...
  self.monster1 = self:add_npc_by_pos_key(1, 201021, "center", 0, 0, "center", 0, 0, 4, self.normal_level)
end

function M:on_npc_dead(npc)
  if self.challenge_max_time - self.m_oSceneCounter:GetTimeCounter(40001):GetTime() > 0 then
    self.challenge_is_win = true
    self:set_scene_tips(40011, true)
    self:remove_functional_npc(40001)
    self:add_functional_npc(4, "center", "center", {npc_id = 40021})
    self:open_all_transmit()
  end
