...
function M:on_enter()
  ...
  if not self.is_paly_timeline then
    ...
    self:play_timeline("timeline3_1", 0, 0)
...
function M:on_npc_dead(npc)
  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and 3 == self.add_monster_tag and not self:is_pass_room() then
        self:fight_end(true)
...
function M:on_timeline_end(timeline_name)
  ...
  self:accept_task_group(1003)
  self:set_scene_tips(100116, true)
end
