...
function M:on_npc_dead(npc)
  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and 2 == self.m2 and not self:is_pass_room() then
        self:fight_end(true)
...
function M:on_enter_area(npc, area)
  ...
  if area.Key == "M1" and 0 == self.m2 then
    self:remove_area("M1")
    self:set_sceneobj_visible("scene_timeline1_3_qiao1", true)
    self.m2 = 1
  end
  if area.Key == "M2" and 1 == self.m2 then
    self:remove_area("M2")
    self:add_monster2()
    self.m2 = 2
  end
