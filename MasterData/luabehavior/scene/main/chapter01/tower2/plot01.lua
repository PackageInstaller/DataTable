...
function M:on_enter()
  ...
  if not self.is_add_npc then
    self:play_timeline("timeline_sc0102_enter2", 0, 1.5)
    self:cast_magic(Global.hero, Global.hero, 4010212, 0)
    self:add_functional_npc(2, "tp3", "tp3", {npc_id = 102149})
...
function M:on_timeline_end(timeline_name)
  if "timeline_sc0102_enter2" == timeline_name then
    self:cast_magic(Global.hero, Global.hero, 4010213, 0)
    self:play_story(1012001)
    self:fight_end(true)
    self:open_all_transmit()
  end
end
...
function M:on_story_end(story_id)
  if not self.is_accept_task and 1012001 == story_id then
    self:accept_task_group(1002)
