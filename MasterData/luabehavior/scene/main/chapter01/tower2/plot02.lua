  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and not self:is_pass_room() then
        ...
        self.is_fight_end = true
        if self.is_fight_end and self.is_paly_timeline then
          self:open_all_transmit()
        end
...
function M:on_save()
  local data = {
    ...
    is_fight_end = self.is_fight_end
  }
  return data
end
...
function M:on_load(data)
  ...
  self.is_fight_end = data.is_fight_end
end
