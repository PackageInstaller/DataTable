...
function M:on_enter()
  ...
  self:set_route_status(4, 5, false)
  self:set_route_status(13, 5, true)
  if not self.is_add_npc then
    self:add_functional_npc(4, "NPC", "tp4", {npc_id = 100103})
    self:play_timeline("timeline1_5", 0, 1.5)
...
function M:on_story_end()
  self:fight_end(true)
  self:set_scene_camera(102)
  self:set_sceneobj_visible("block1", true)
  self:set_sceneobj_visible("block1", true)
  self:open_all_transmit()
end
...
function M:on_timeline_end(timeline_name)
  self:timer_call_out(nil, function()
    self:set_scene_tips(100106, true)
  end, 1)
end
...
function M:on_func_npc_interact_end(func_npc_id)
  ...
  self:remove_functional_npc(100103)
end
