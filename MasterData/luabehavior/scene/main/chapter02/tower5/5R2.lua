  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and not self:is_pass_room() and 1 == self.wave then
        self.wave = 2
        self:add_monster2()
        self:set_scene_tips(1021042, 1, {})
        self:timer_call_out("AC", function()
          self:set_scene_tips(1021043, 1, {})
          Log.Info(Global.hero, "AAAAAAA")
        end, 4.1)
        self:timer_call_out("CC", function()
          self:set_scene_tips(1021044, 1, {})
          Log.Info(Global.hero, "BBBBBBBB")
        end, 8.1)
      end
      if 0 == #self.monster_list and not self:is_pass_room() then
        self:end_perform(self.room_type)
        self:play_story(1025002)
      end
    end
  end
end
...
function M:on_story_end(story_id)
  if 1025002 == story_id then
    self:play_story(100102072)
  end
  if 100102072 == story_id then
    self:fight_end(true)
    self:open_all_transmit()
  end
end
