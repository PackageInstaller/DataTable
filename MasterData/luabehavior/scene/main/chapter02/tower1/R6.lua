...
function M:on_enter()
  ...
  self:set_route_status(22, 23, true)
  self:set_route_status(22, 31, false)
  self:enable_god_skill(false)
  if 0 == self.a then
    self:timer_call_out(nil, function()
      self:play_story(1021002)
    end, 1)
  end
...
function M:on_story_end()
  if 0 == self.b then
    self:add_functional_npc(4, "bornpoint", "tp3", {npc_id = 102102})
...
function M:on_npc_show_end(func_npc_id)
  self:add_monster()
  self:timer_call_out(nil, function()
    self:start_texture_guide(10212)
  end, 1)
end
