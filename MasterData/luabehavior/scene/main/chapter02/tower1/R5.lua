...
function M:on_enter()
  self:set_route_status(31, 22, true)
  self:set_route_status(30, 31, false)
  if self:is_pass_room() then
    ...
    self:set_scene_camera(102)
    self:add_monster()
    self:set_scene_tips(1021009, true, 1)
    self:timer_call_out(nil, function()
      self:set_scene_tips(1021010, true, 1)
    end, 4.1)
  end
end
...
function M:add_monster()
  ...
  for _, data in ipairs(lMonster["RANDOW_VAL_" .. list_id]) do
    local key = data[10]
    local magic_id = data[11]
    local lv = self.monster_lv[key]
    local args = {
      _tunpack(data, 1, 10)
    }
    local monster = self:add_npc_by_pos_key(_tunpack(args))
    if 3001036 == magic_id then
      self:cast_magic(monster, monster, 3001036013, 0)
    end
...
function M:on_npc_dead(npc)
  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 1 == #self.monster_list and not self:is_pass_room() and 1 == self.wave then
        self.wave = 2
        self:add_monster2()
      end
