...
function M:after_damage(npc, target, magic_id, damage_sign, damage_type, is_crit, damage_val, base_dmg, element_dmg)
  if target == self.npc and damage_val < 0 then
    local hero_target = Global.hero:get_target()
    if not self.npc:get_ignore_lock() and (not (Global.hero and hero_target) or self.npc:get_uuid() ~= hero_target:get_uuid()) then
      return
    end
...
function M:set_npc_not_beattack_collid_enabled(is_enable)
  if self.npc:is_cant_be_hit() and not is_enable then
    return
  end
