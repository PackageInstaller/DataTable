...
function monster:_init(npc)
  ...
  self.born_point_amount = 18
  self.born_points = {}
  self.born_points_used = {}
  self.last_create_time = 0
  self.init_over = false
  self.enemy_list = {}
  self.current_room_id = get_cur_room_id()
  self.current_value = 0
  self.value_to_success = 0
  self.need_judge_success = false
  if self.current_room_id == 800201 then
    self.enemy_id_group = {
      [1] = {
        id = 3002047,
        weight = 100,
  ...
  elseif self.current_room_id == 800202 then
    self.enemy_id_group = {
      [1] = {
        id = 3002047,
        weight = 100,
  ...
  elseif self.current_room_id == 800203 then
    self.enemy_id_group = {
      [1] = {
        id = 3002047,
        weight = 100,
  ...
  elseif self.current_room_id == 800204 then
    self.enemy_id_group = {
      [1] = {
        id = 3002047,
        weight = 100,
  ...
  elseif self.current_room_id == 800205 then
    self.enemy_id_group = {
      [1] = {
        id = 3002047,
        weight = 100,
    ...
    self.need_judge_success = true
    self.value_to_success = 100
  elseif self.current_room_id == 800206 then
    self.enemy_id_group = {
      [1] = {
        id = 3002047,
        weight = 100,
...
function monster:create_enemy()
  ...
  local enemy = add_npc_by_pos_key(2, enemy_id, self.born_points[randomPos].pos, 0, 0, self.born_points[randomPos].pos, 0, 0, 2, 1)
  for i = 1, #self.enemy_id_group do
...
function monster:on_npc_hp_zero(npc)
  ...
  if self.need_judge_success == false then
    return
  end
  if npc.id == self.enemy_id_group[1].id then
    self.current_value = self.current_value + 10
    print(self.current_value)
  elseif npc.id == self.enemy_id_group[2].id then
    self.current_value = self.current_value + 20
    print(self.current_value)
  elseif npc.id == self.enemy_id_group[3].id then
    self.current_value = self.current_value + 20
  end
  if self.current_value >= self.value_to_success then
