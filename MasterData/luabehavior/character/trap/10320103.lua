...
function M:on_frame()
  if get_sync_var("change") then
    set_sync_var("change", false)
    self:clear_timer()
    self:start_switch_scene_effect(self.switch_effect_cfg1_1, false)
  end
  if get_sync_var("change1") then
    set_sync_var("change1", false)
    self:clear_timer()
    self:start_switch_scene_effect(self.switch_effect_cfg3_3, false)
  end
end
