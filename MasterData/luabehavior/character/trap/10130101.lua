...
function M:on_skill_hit(npc, skill_id, hit_target, hit_type, missile_cfg, missile)
  if hit_target == self.npc and missile_cfg.Id == 30010390201 then
    set_sceneobj_visible_save_state("scene_timeline_diaodeng", true, true)
    set_sync_var("diaodeng", true)
...
function M:on_frame()
  if check_magic(self.npc, Const.LOCK_ON_TAG) and get_god_skill_level() >= 2 and not self.tips and not get_sync_var("tips_close") then
