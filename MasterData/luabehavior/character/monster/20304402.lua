...
function monster._init(monster, npc)
  ...
  do
    ...
    _ENV.Show_B(_ENV.RunTurn(_ENV.RunStart(_ENV.RunLoop(_ENV.RunHit))))
    _ENV.RunHit(_ENV.RunHitDrift, _ENV.RunHitDriftLeft, _ENV.RunHitDriftRight, _ENV.RunHitStopLeft, _ENV.RunHitStopRight)
    _ENV.RunHitDriftRight(_ENV.Show_B)
    _ENV.RunHitDriftLeft(_ENV.Show_B)
  end
  do
  ...
  do
    ...
    function skills.RunLoop.onBegin(RunLoop)
      ...
      RunLoop.base.onBegin(RunLoop)
    end
    
    function skills.RunHitDrift.isReady(RunHitDrift)
    ...
    function skills.RunHitDrift.onEnd(RunHitDrift)
      monster:transState(monster.states.wander, true)
    end
    
    function skills.RunHitDriftLeft.isReady(RunHitDriftLeft)
    ...
    function skills.RunHitDriftRight.isReady(RunHitDriftRight)
    ...
    function skills.RunHitDriftLeft.onEnd(RunHitDriftLeft)
      RunHitDriftLeft.base.onEnd(RunHitDriftLeft)
      monster:transState(monster.states.Show_B, true)
    end
    
    function skills.RunHitDriftRight.onEnd(RunHitDriftRight)
      RunHitDriftRight.base.onEnd(RunHitDriftRight)
      monster:transState(monster.states.Show_B, true)
    end
    
    function skills.RunHitStopLeft.isReady(RunHitStopLeft)
    ...
    function skills.RunHitStopRight.isReady(RunHitStopRight)
...
function monster.on_self_missile_end_pos(monster, pos_x, pos_z, missile_cfg, missile)
  if missile_cfg.Id == 203044010304 then
...
function monster.on_missile_collide_obstacle(monster, owner, obstacle_type, missile_cfg, missile)
  ...
  abort_skill(monster.npc, false)
  if monster.StunSwitch == true then
    monster.StunSwitch = false
    cast_magic(monster.npc, monster.npc, 1999220)
  end
end
