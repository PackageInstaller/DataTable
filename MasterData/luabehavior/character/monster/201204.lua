...
function M:_init()
  ...
  self.Skill01 = {
    ...
    1.5,
    3,
    get_skill_cfg(20120401).CastTime,
    get_skill_cfg(20120401).AfterTime
  }
...
function M:on_skill_hit(npc, skill_id, hit_target, hit_type, missile_cfg, missile)
  ...
  if missile_cfg.Id == 2012050101 and self.AllowCastSkill == true then
    if self.ArriveTargetPos1 == false then
      self.AllowCastSkill = false
      cast_skill(self.npc, nil, 20120401)
    end
...
function M:CheckCurrentPos()
  if check_npc_distance_to_pos(self.npc, self.TargetPos1.x, self.TargetPos1.z, 1, false) == true then
    self.AllowRun = true
    self.ArriveTargetPos1 = true
  end
  if true == check_npc_distance_to_pos(self.npc, self.TargetPos2.x, self.TargetPos2.z, 1, false) then
    self.AllowRun = true
    self.ArriveTargetPos2 = true
  end
  if true == check_npc_distance_to_pos(self.npc, self.TargetPos3.x, self.TargetPos3.z, 1, false) then
...
function M:CheckPlayerDistance()
  ...
  if DistanceToPlayer <= 4 and check_magic(self.npc, 20120403) == false then
    cast_magic(self.npc, self.npc, 20120403, 1)
  end
  if DistanceToPlayer > 10 and check_magic(self.npc, 20120403) == true then
    cast_magic(self.npc, self.npc, 20120405, 1)
    abort_magic_by_id(self.npc, 20120403, 1)
  end
  if DistanceToPlayer >= 10 and true == self.DistanceMoreThan10 and DistanceToPlayer <= 10 then
