...
function M:_init()
  ...
  self.skill01 = {
    ...
    0,
    2,
    get_skill_cfg(20103901).CastTime,
    get_skill_cfg(20103901).AfterTime,
    0
  }
...
function M:wander_main_logic()
  ...
  if not check_npc_distance(self.npc, self.target, self.wander[1], true) and now_time >= self.refresh_timer then
    self:move_approch_wander()
    self.refresh_timer = now_time + 0.33
  end
