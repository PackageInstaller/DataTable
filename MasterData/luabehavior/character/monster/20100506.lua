...
function monster._init(monster, npc)
  ...
  listen_missile_evt(MISSILE_EVT_TYPE.BOUND_CATCH_NPC, npc, 201005060101, monster.on_water_catch_npc, monster)
  do
...
function monster.on_self_missile_begin_pos(monster, pos_x, pos_z, missile_cfg, missile)
  if missile_cfg.Id == 201005060101 then
    monster.tool.blueWaters = monsterTool.blueWaters or {}
    missile.movetoNum = 0
    missile.drinkNum = 0
    missile.drinkDis = 5
    missile.drinkNum_Max = 4
...
function monster.on_remove(monster)
  unlisten_missile_evt(MISSILE_EVT_TYPE.BOUND_CATCH_NPC, monster.npc, 201005060101, monster)
end
