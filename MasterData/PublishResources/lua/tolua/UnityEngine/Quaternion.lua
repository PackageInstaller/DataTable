...
function Quaternion:MulVec2(point)
  local result = self:MulVec3(Vector3(point.x, point.y, 0))
...
function Quaternion.__mul(lhs, rhs)
  ...
  elseif Vector2 == getmetatable(rhs) then
    return lhs:MulVec2(rhs)
  end
end
...
function Quaternion.__eq(lhs, rhs)
  if lhs._className ~= rhs._className then
    return false
  end
