...
function SkillScopeCalculator_DoubleCrossBeBlocked:CalcRange(scopeType, scopeParam, centerPos, bodyArea, casterDir, nTargetType, casterPos)
  ...
  local centerIncluded = false
  if type(scopeParam) ~= "number" then
    size = scopeParam[1]
    if scopeParam[2] and scopeParam[2] == 1 then
      centerIncluded = true
    end
  end
  ...
  if centerIncluded then
    self:_InsertTargetGrid(cross_area, centerPos, wholeArea)
  end
