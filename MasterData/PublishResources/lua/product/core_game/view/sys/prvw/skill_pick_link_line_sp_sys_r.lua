_class("SkillPickUpLinkLineSPSystem_Render", ReactiveSystem)
SkillPickUpLinkLineSPSystem_Render = SkillPickUpLinkLineSPSystem_Render

function SkillPickUpLinkLineSPSystem_Render:Constructor(world)
  self._world = world
  self._pickUpType = nil
  self._pickUpNum = 0
end

function SkillPickUpLinkLineSPSystem_Render:GetTrigger(world)
...
function SkillPickUpLinkLineSPSystem_Render:Filter(entity)
  local pickUpTargetCmpt = entity:PickUpTarget()
  local skillHandleType = pickUpTargetCmpt:GetPickUpTargetType()
  if skillHandleType == SkillPickUpType.LinkLineSP then
    return true
  end
  return false
end

function SkillPickUpLinkLineSPSystem_Render:ExecuteEntities(entities)
...
function SkillPickUpLinkLineSPSystem_Render:DoLinkLine(entity)
  ...
  self._pickUpNum = utilDataSvc:GetPet1702361LinkLineStep(petEntity, activeSkillID)
  local canLinkMonster = 0
...
function SkillPickUpLinkLineSPSystem_Render:DoPickUp(entity)
  local previewEntity = self._world:GetPreviewEntity()
  if not previewEntity then
    return
  end
  ...
  local utilDataSvc = self._world:GetService("UtilData")
  local skillConfigData = configService:GetSkillConfigData(activeSkillID, petEntity)
  local legendPower = utilDataSvc:GetPetLegendPowerAttr(petEntity)
  local pickUpPerPower = tonumber(skillConfigData._pickUpParam[1]) or 1
  self._pickUpNum = utilDataSvc:GetPet1702361LinkLineStep(petEntity, activeSkillID)
  local previewPickUpComponent = petEntity:PreviewPickUpComponent()
  previewPickUpComponent:ClearGridPos()
...
function SkillPickUpLinkLineSPSystem_Render:UpdateUI(previewPickUpComponent, isLinkMonster)
