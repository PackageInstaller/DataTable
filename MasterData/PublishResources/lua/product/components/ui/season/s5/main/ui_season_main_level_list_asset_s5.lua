_class("UISeasonMainLevelListAssetS5", UICustomWidget)
UISeasonMainLevelListAssetS5 = UISeasonMainLevelListAssetS5

function UISeasonMainLevelListAssetS5:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonMainLevelListAssetS5:InitWidget()
  self.icon = self:GetUIComponent("Image", "icon")
  self.count = self:GetUIComponent("UILocalizationText", "count")
end

function UISeasonMainLevelListAssetS5:SetData(id, count)
  local zeros
  if 9999 < count then
