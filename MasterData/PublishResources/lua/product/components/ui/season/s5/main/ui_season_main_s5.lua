_class("UISeasonMainS5", UIController)
UISeasonMainS5 = UISeasonMainS5

function UISeasonMainS5:LoadDataOnEnter(TT, res)
  res:SetSucc(true)
...
function UISeasonMainS5:OnShow(uiParams)
  ...
  self._topBtn:SetData(function()
    self._animation:Play("uieff_UISeasonMainS5_out")
    self:Lock("UISeasonMainS5:Back")
    GameGlobal.Timer():AddEvent(300, function()
      self:UnLock("UISeasonMainS5:Back")
      if self.view then
        self._active = false
        if self._component:ComponentIsOpen() then
          self._uiModule:ExitSeasonTo(UIStateType.UIS5Main)
        else
  ...
  self._ovalArea = self.ovalAreaPool:SpawnObject("UISeasonMainOvalAreaS5")
  ...
  self._pointCount:SetText(string.format("<color=#ece3d6>%s</color>/%s", cur, ceil))
  ...
  self:AttachEvents()
  self._levelList = self.levelListPool:SpawnObject("UISeasonMainLevelListS5")
  ...
  self:_ResetQuestRed()
  self._modeSwitch = self._modeSwitchLoader:SpawnObject("UISeasonModeSwitchS5")
  ...
  if not self._uiModule:IsBackTrack() then
    self:Lock("PlayS5EnterAnim")
    self._animation:Play("uieff_UISeasonMainS5_in")
    self._levelList:PlayEnterAnim()
    GameGlobal.Timer():AddEvent(1000, function()
      self:UnLock("PlayS5EnterAnim")
      if self._active then
        self._seasonAnim:Play("uieff_UISeasonDailyS5_in")
      end
    end)
    self._levelList:PlayEnterAnim()
    self._animation:Play("uieff_UISeasonMainS5_in")
...
function UISeasonMainS5:RefreshAfterCloseOnceMission()
  self:_RefreshThirdBtnVisible()
end

function UISeasonMainS5:AttachEvents()
...
function UISeasonMainS5:DisableOpenList(bDisable, reason)
...
function UISeasonMainS5:IsDisableOpenList()
...
function UISeasonMainS5:OnAfterUILayerChanged()
  self:_TryShowOpenList()
end

function UISeasonMainS5:_TryShowOpenList()
...
function UISeasonMainS5:ShowOpenList()
...
function UISeasonMainS5:CheckShowExpressBubbleOnEnter()
...
function UISeasonMainS5:OnUIGetItemCloseInQuest(type)
  self:RefreshBuffArea()
end

function UISeasonMainS5:OnUpdate(dtMS)
...
function UISeasonMainS5:OnHide()
  self._active = false
...
function UISeasonMainS5:InitWidget()
  ...
  self._btnBg_3Go = self:GetGameObject("btn_bg_3")
  self._btnBg_2Go = self:GetGameObject("btn_bg_2")
end

function UISeasonMainS5:_SetTaskListNavi()
...
function UISeasonMainS5:_SetTaskListBtn()
...
function UISeasonMainS5:_RefreshThirdBtnVisible()
  if self._uiModule:IsBackTrack() then
    self._thirdBtnGo:SetActive(false)
    self._btnBg_3Go:SetActive(false)
    self._btnBg_2Go:SetActive(false)
    return
  end
  ...
  self._btnBg_3Go:SetActive(bShow)
  self._btnBg_2Go:SetActive(not bShow)
  local bNew = onceMissionData:HasEntryNew()
  self._thierdBtnNew:SetActive(bNew)
end

function UISeasonMainS5:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffMainAreaS5")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonMainS5:InitFinalPlotEnterArea()
...
function UISeasonMainS5:SwitchTestFuncEntry(show)
  if self._testFunc ~= nil then
    self._testFunc:Switch(show)
  end
end

function UISeasonMainS5:SetShow(show)
  self.root:SetActive(show)
  self.showUI:SetActive(not show)
end

function UISeasonMainS5:PlayerIconOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS5:ArchieveBtnOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS5:CollectionBtnOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
...
function UISeasonMainS5:TopTipOnClick(go)
  if self._seasonManager:LockUI() then
    return
  end
  self:_TryStopPlayer()
  self:_HideActionPointTip()
  self:ShowDialog("UISeasonActionPointTipS5", self._actionPointCpt, go.transform.anchoredPosition, function()
    self:_RefreshActionPointTip()
  end)
end

function UISeasonMainS5:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonMainS5:LeaveToBattle()
  self._active = false
  self._uiModule:ExitSeasonGame()
end

function UISeasonMainS5:LeaveToMain()
  self._active = false
end

function UISeasonMainS5:ShowUIOnClick()
  self:SetShow(true)
end

function UISeasonMainS5:AfterPlayEnterVideo(played)
...
function UISeasonMainS5:AfterPlayEnterStory(played)
  if played then
...
function UISeasonMainS5:ShowSerialRewards()
  self._isWaitShowSerialRewards = true
end

function UISeasonMainS5:CheckShowSerialRewards()
...
function UISeasonMainS5:CheckShowCollectionRewardOnEnter()
...
function UISeasonMainS5:CalcBuffLevelOnEnter()
  local seasonModule = self:GetModule(SeasonModule)
  ...
  elseif curLevel ~= oldLevel then
    ...
    self:ShowDialog("UISeasonBuffLevelUpS5", oldLevel, curLevel, componentID, function()
...
function UISeasonMainS5:CheckShowCollectionComposePlotOnEnter()
...
function UISeasonMainS5:OnCollectionComposePlotEnd(completedQuest)
  if not self.questCmpt then
...
function UISeasonMainS5:_OnGetCollection()
  self:_ResetCollectionInfo()
  self:_TryShowOpenList()
end

function UISeasonMainS5:_ResetCollectionInfo()
...
function UISeasonMainS5:_OnActionPointChanged()
  local cur, ceil = self._actionPointCpt:GetItemCount()
  self._pointCount:SetText(string.format("<color=#ece3d6>%s</color>/%s", cur, ceil))
end

function UISeasonMainS5:_OnAutoFightSweepFinished()
  self:_OnActionPointChanged()
  self:_ResetCollectionInfo()
  self:_ResetQuestRed()
end

function UISeasonMainS5:_TryStopPlayer()
  self._seasonManager:SeasonPlayerManager():GetPlayer():Stop(false)
end

function UISeasonMainS5:_PlayEnterAnim()
...
function UISeasonMainS5:_OnSeasonDailyResetSucc()
  if self._seasonDaily then
    self._seasonDaily:SetData()
  end
end

function UISeasonMainS5:_OnDiffChanged(curDiff)
...
function UISeasonMainS5:_PlayBtmEft(isIn)
  if isIn then
    self:_PlayBtmEftIn()
  else
    self:_PlayBtmEftOut()
...
function UISeasonMainS5:_PlayBtmEftIn()
end

function UISeasonMainS5:_PlayBtmEftOut()
end

function UISeasonMainS5:IsPlayAnimation()
  if self._animation then
    local name = "uieff_UISeasonMainS5_in"
...
function UISeasonMainS5:_OnModeChanged(mode)
...
function UISeasonMainS5:_TriggerGuide()
  self:_CheckGuide()
  self:_SetBackpackInfo()
end

function UISeasonMainS5:_CheckGuide()
  if self:_CheckModeValid(SeasonMapMode.Mode2) then
    GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UISeasonMainS5)
  else
    GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UISeasonMainS5)
  end
end

function UISeasonMainS5:_CheckModeValid(mode)
...
function UISeasonMainS5:_OnSeasonStoryChanged(isOpen)
  self:SetShow(not isOpen)
end

function UISeasonMainS5:_CheckSeasonBuildOpen()
...
function UISeasonMainS5:_SetBackTrackUI()
  if self._uiModule:IsBackTrack() then
    ...
    self:GetGameObject("bg2"):SetActive(false)
    self:GetGameObject("bg3"):SetActive(false)
...
function UISeasonMainS5:BackSeasonBtnOnClick(go)
...
function UISeasonMainS5:_IsBackpackOpen()
  if self._uiModule:IsBackTrack() then
    return false
  end
...
function UISeasonMainS5:_SetBackpackInfo()
  local loginModule = self:GetModule(LoginModule)
  local svrTimeModule = self:GetModule(SvrTimeModule)
  local curTime = svrTimeModule:GetServerTime() * 0.001
...
function UISeasonMainS5:BackpackBtnOnClick(go)
...
function UISeasonMainS5:ThirdBtnOnClick(go)
  local onceMissionData = self._seasonModule:GetOnceMissionData()
  local bShow = onceMissionData:AnyComponentOpening()
  if not bShow then
    ToastManager.ShowToast(StringTable.Get("str_activity_error_109"))
  ...
  local mode = self._seasonManager:SeasonMapManager():Mode()
  if mode == 1 then
    local title = StringTable.Get("str_season_s5_branch_change_mode_title")
    local content = StringTable.Get("str_season_s5_branch_change_mode_message")
    self:ShowDialog("UISeasonConfirm2", title, content, function()
      self._modeSwitch:Switch2Modle2(function()
        self:_MoveToEvnentPointById(8005302)
      end)
    end)
    return
  end
...
function UISeasonMainS5:_MoveToEvnentPointById(eventId)
...
function UISeasonMainS5:_OnSeasonBackpackClose()
  self:_SetBackpackInfo()
end

function UISeasonMainS5:_HideActionPointTip()
...
function UISeasonMainS5:_RefreshActionPointTip()
  ...
  if self.view then
    ...
    self._apCountdownTip:SetText(StringTable.Get("str_season_s5_ticket_refresh_time", t))
  end
