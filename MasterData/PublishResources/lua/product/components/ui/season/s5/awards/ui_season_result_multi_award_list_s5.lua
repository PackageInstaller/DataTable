_class("UISeasonResultMultiAwardListS5", UICustomWidget)
UISeasonResultMultiAwardListS5 = UISeasonResultMultiAwardListS5

function UISeasonResultMultiAwardListS5:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonResultMultiAwardListS5:InitWidget()
  self._seasonAwardGen = self:GetUIComponent("UISelectObjectPath", "Content")
  self._sr = self:GetUIComponent("ScrollRect", "ScrollView")
end

function UISeasonResultMultiAwardListS5:OnHide()
end

function UISeasonResultMultiAwardListS5:SetData(matchRes, seasonMissionInfo)
  ...
  self._seasonAwardGen:SpawnObjects("UISeasonResultSingleAwardListS5", count)
...
function UISeasonResultMultiAwardListS5:ProcessCollectionItem(awardList, collectionList)
  local retList = {}
...
function UISeasonResultMultiAwardListS5:ResetScrollPos()
  self._sr.horizontalNormalizedPosition = 0
end

function UISeasonResultMultiAwardListS5:_GetItemCount(vecItem)
  local nItemCount = 0
  if vecItem then
    for i = 1, #vecItem do
...
function UISeasonResultMultiAwardListS5:BattleNormalResultSortAsset(assets)
...
function UISeasonResultMultiAwardListS5:GetPassAward()
  local awardHeadType, cfgId
  return UICommonHelper:GetInstance():GetPassAward(awardHeadType, cfgId)
end

function UISeasonResultMultiAwardListS5:HasItem(dataList, itemId)
  local isNormal = 0
  if dataList then
