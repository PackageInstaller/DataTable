...
_class("UISeasonResultItemS5", UICustomWidget)
UISeasonResultItemS5 = UISeasonResultItemS5

function UISeasonResultItemS5:Constructor()
...
function UISeasonResultItemS5:OnShow(uiParams)
  self._transform = self:GetGameObject().transform
  self._nodes = {}
  self._nodes[UIItemNode.Normal] = UIItemNormalNodeForSeasonS5:New(self:GetUIComponent("UIView", "g_normal"), self)
...
function UISeasonResultItemS5:OnHide()
...
function UISeasonResultItemS5:PlayAni(aniName)
  self._anim:Play(aniName)
end

function UISeasonResultItemS5:SetForm(type, scale, dontPlayAni)
...
function UISeasonResultItemS5:EnableNode(nodeId, isEnable)
...
function UISeasonResultItemS5:SetData(params)
...
function UISeasonResultItemS5:ShowNodes(uiItemNode, enable)
...
function UISeasonResultItemS5:SetScale(scale)
  scale = scale or 1
...
function UISeasonResultItemS5:Select(select, noAnim)
...
function UISeasonResultItemS5:SetClickCallBack(callBack, param)
...
function UISeasonResultItemS5:BtnOnClick(go)
  if self._clickCallBack then
    self._clickCallBack(go)
  end
end

function UISeasonResultItemS5:SetBtnImage(bImage)
end

function UISeasonResultItemS5:SetLongPressCallBack(longPressCallBack, longPressUpCallBack, pressTime, update)
...
function UISeasonResultItemS5:SetReduceLongPressCallBack(sec)
  if self._nodes[UIItemNode.Reduce] then
    self._nodes[UIItemNode.Reduce]:SetReduceLongPressCallBack(sec)
  end
end

function UISeasonResultItemS5:InitLongPress()
...
function UISeasonResultItemS5:LongEvent()
...
function UISeasonResultItemS5:LongEventUpLv(gradeLv)
  local cfg = Cfg.cfg_up_lv_long_press[gradeLv]
  if not cfg then
    Log.fatal("###[UISeasonResultItemS5] LongEventUpLv cfg is nil ! grade:", gradeLv)
  end
  local arr = cfg.Value
  local datas = {}
  for i = 1, #arr do
...
function UISeasonResultItemS5:Calculate(count)
...
function UISeasonResultItemS5:GetBtn()
  return self._btnGO
end

function UISeasonResultItemS5:SetToggleGroup(group)
...
function UISeasonResultItemS5:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  end
end

function UISeasonResultItemS5:SetToggleValue(isOn)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleValue(isOn)
  end
end

function UISeasonResultItemS5:GetItemData()
  return self._uiItemData
end

function UISeasonResultItemS5:ClearItemData()
  self._uiItemData = nil
end

function UISeasonResultItemS5:SetBtnImageByName(imgName, atlasName)
end

_class("UIItemNormalNodeForSeasonS5", UIItemNormalNode)
UIItemNormalNodeForSeasonS5 = UIItemNormalNodeForSeasonS5

function UIItemNormalNodeForSeasonS5:SetQuality(quality)
  if quality <= 0 then
  ...
  if qualityName ~= "" then
    ...
    local atlas = self._owner:GetAsset("UIS5Scene.spriteatlas", LoadType.SpriteAtlas)
...
function UIItemNormalNodeForSeasonS5:SetIconOffset(itemId)
...
function UIItemNormalNodeForSeasonS5:SetOffset(showText1)
end
