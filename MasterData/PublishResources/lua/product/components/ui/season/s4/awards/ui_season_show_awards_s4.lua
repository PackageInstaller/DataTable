_class("UISeasonShowAwardsS4", UIController)
UISeasonShowAwardsS4 = UISeasonShowAwardsS4

function UISeasonShowAwardsS4:GetComponents()
...
function UISeasonShowAwardsS4:OnShow(uiParams)
  ...
  if uiParams[1] then
    if table.count(uiParams[1]) == 0 then
      Log.fatal("###[UISeasonShowAwardsS4] table.count(uiParams[1]) == 0 !")
    end
  else
    Log.fatal("###[UISeasonShowAwardsS4] uiParams[1] is nil !")
  end
  if uiParams[3] then
    itemlist = uiParams[1]
  else
...
function UISeasonShowAwardsS4:CreateData(itemlist)
...
function UISeasonShowAwardsS4:DoAnimation()
  self._canvasGroup = self:GetUIComponent("CanvasGroup", "canvasGroup")
  self._canvasGroup.alpha = 0
  self._tweener = self._canvasGroup:DOFade(1, 0.02)
end

function UISeasonShowAwardsS4:ClosePanel()
  self:CloseDialog()
end

function UISeasonShowAwardsS4:OnHide()
  if self._closeCallback then
    self._closeCallback()
  end
end

function UISeasonShowAwardsS4:_InitListView(scrollView, index)
  if index < 0 then
    return nil
  end
  local count
  ...
  if item.IsInitHandlerCalled == false then
    ...
    rowPool:SpawnObjects("UISeasonShowAwardsItemS4", count)
  end
...
function UISeasonShowAwardsS4:_ShowItem(giftItem, index, tweenIdx)
  local beforeTime = 0
...
function UISeasonShowAwardsS4:OnItemSelect(id, count, go)
  GameGlobal.EventDispatcher():Dispatch(GameEventType.ShowItemTips, id, self._trans.transform.position)
end

function UISeasonShowAwardsS4:NextOnClick(go)
...
function UISeasonShowAwardsS4:CalcPage()
...
function UISeasonShowAwardsS4:_GetNextPageIndex()
...
function UISeasonShowAwardsS4:_GetCurPageFirstIndex()
  return self._curPageFirstIndex
end

function UISeasonShowAwardsS4:_GetItemDataByIndex(index)
...
function UISeasonShowAwardsS4:_FormatItemCount(itemCount)
  return HelperProxy:GetInstance():FormatItemCount(itemCount)
end

function UISeasonShowAwardsS4:ShowTips(itemId, pos)
