_class("UISeasonS4MusicTab", UICustomWidget)
UISeasonS4MusicTab = UISeasonS4MusicTab

function UISeasonS4MusicTab:OnShow(uiParams)
  self:InitWidget()
...
function UISeasonS4MusicTab:OnHide()
...
function UISeasonS4MusicTab:InitWidget()
...
function UISeasonS4MusicTab:SetData(data)
  self._collageData = data
  self._musicCount = self._collageData:GetMusicCount()
  self._items = self.content:SpawnObjects("UISeasonS4CollageMusicItem", self._musicCount)
...
function UISeasonS4MusicTab:SetShow(show)
...
function UISeasonS4MusicTab:_OnSelect(data, isInit)
...
function UISeasonS4MusicTab:PlayOnClick(go)
...
function UISeasonS4MusicTab:PauseOnClick(go)
  if not self._curIdx then
    return
  end
  if self._pause then
...
function UISeasonS4MusicTab:LastOnClick(go)
  if not self._curIdx then
    return
  end
...
function UISeasonS4MusicTab:NextOnClick(go)
  if not self._curIdx then
    return
  end
  if self._unlockCount == 1 then
...
function UISeasonS4MusicTab:_RefreshPlayBar()
  if self._curIdx then
...
function UISeasonS4MusicTab:_ResetProgress()
  self.progress.fillAmount = 0
  self.time:SetText("")
end

function UISeasonS4MusicTab:_PlayingTick()
  if self._pause then
    return
  end
  local data = self._collageData:GetMusicByIndex(self._curIdx)
...
function UISeasonS4MusicTab:_SnapTo(index, animate)
...
function UISeasonS4MusicTab:PlayExitAnim()
end
