_class("UISeasonStageAwardItemS4", UICustomWidget)
UISeasonStageAwardItemS4 = UISeasonStageAwardItemS4

function UISeasonStageAwardItemS4:OnShow()
  self._trans = self:GetGameObject()
  self:GetComponents()
end

function UISeasonStageAwardItemS4:GetComponents()
  self._trans = self:GetGameObject()
...
function UISeasonStageAwardItemS4:Flush(v, levelDiff)
  if not v then
    return
  end
  self._v = v
  ...
  elseif awardType == StageAwardType.HasGen then
    ...
    textColor = Color(0.424, 0.325, 0.267, 1)
  end
  if levelDiff then
...
function UISeasonStageAwardItemS4:SetIcon(name, itemId)
...
function UISeasonStageAwardItemS4:ShowIcon(show, itemId)
  self._iconGO:SetActive(show)
  self:SetIconOffset(itemId)
end

function UISeasonStageAwardItemS4:SetIconOffset(itemId)
...
function UISeasonStageAwardItemS4:_IsPet(id)
  local cfg = Cfg.cfg_pet({ID = id})
  return cfg and true or false
end

function UISeasonStageAwardItemS4:SetQuality(quality)
  if quality <= 0 then
    quality = 1
  end
...
function UISeasonStageAwardItemS4:ShowQuality(show)
  self._qualityGO:SetActive(show)
end

function UISeasonStageAwardItemS4:SetText1(text, showNumber)
  if text ~= nil then
    local show = false
...
function UISeasonStageAwardItemS4:ShowText1(show)
  self._text1GO:SetActive(show)
  self:SetOffset(show)
end

function UISeasonStageAwardItemS4:SetOffset(showText1)
end

function UISeasonStageAwardItemS4:SetAwardText(awardText, color)
  self._awardTxt:SetText(awardText)
  self._awardTxt.color = color
end

function UISeasonStageAwardItemS4:SetTypeBg(typeBgName)
end

function UISeasonStageAwardItemS4:SetTypeMainBg(typeBgName)
  self._typeMainBgImg.sprite = self.atlas:GetSprite(typeBgName)
end

function UISeasonStageAwardItemS4:SetTopAward()
  self:SetTopText(self._topTextStr)
end

function UISeasonStageAwardItemS4:SetTopText(text)
...
function UISeasonStageAwardItemS4:ShowTopText(show)
  self._topGo:SetActive(show)
end

function UISeasonStageAwardItemS4:SetAct()
...
function UISeasonStageAwardItemS4:ShowText(show)
  self._actGo:SetActive(show)
end

function UISeasonStageAwardItemS4:BtnOnClick(go)
...
function UISeasonStageAwardItemS4:PlayAnim(totalDelay)
...
function UISeasonStageAwardItemS4:_PlayInAnim()
  if self._anim then
...
function UISeasonStageAwardItemS4:OnHide()
  if self._animTimer then
    GameGlobal.Timer():CancelEvent(self._animTimer)
    self._animTimer = nil
  end
...
function UISeasonStageAwardItemS4:SetWaitAnim()
  if self._rootGo then
