_class("UIS2ExchangeConfirm", UIController)
UIS2ExchangeConfirm = UIS2ExchangeConfirm

function UIS2ExchangeConfirm:LoadDataOnEnter(TT, res, uiParams)
  ...
  if self._info == nil then
    Log.error("UIS2ExchangeConfirm:LoadDataOnEnter() param = nil")
    res:SetSucc(false)
    return
  end
end

function UIS2ExchangeConfirm:OnShow(uiParams)
...
function UIS2ExchangeConfirm:OnHide()
  self:_DetachEvents()
end

function UIS2ExchangeConfirm:_Refresh()
  self:_SetBuyCount(self._buyCount)
  self:_SetCoinCount(self._buyCount)
end

function UIS2ExchangeConfirm:_UpdateBuyCount(newCount)
  local min, max = self:_ConstMin(), self:_CalcMaxCount()
  self._buyCount = Mathf.Clamp(newCount, min, max)
  self:_Refresh()
end

function UIS2ExchangeConfirm:_SetState(state)
...
function UIS2ExchangeConfirm:_SetItem(itemId, itemCount)
...
function UIS2ExchangeConfirm:_SetCoin()
...
function UIS2ExchangeConfirm:_SetCoinCount(buyCount)
...
function UIS2ExchangeConfirm:_ConstMin()
  return 1
end

function UIS2ExchangeConfirm:_ConstMax()
  local constMax = 99
  local max = self._component:GetCanExchangeCount(self._info, constMax)
  return Mathf.Max(max, 1)
end

function UIS2ExchangeConfirm:_CalcMinCount()
  local min, max = self:_ConstMin(), self:_ConstMax()
  return min
end

function UIS2ExchangeConfirm:_CalcMaxCount()
  local min, max = self:_ConstMin(), self:_ConstMax()
...
function UIS2ExchangeConfirm:_CalcHaveCount()
  local itemId = self._info.m_cost_item_id
...
function UIS2ExchangeConfirm:_CalcTotalPrice(buyCount)
  local price = self._info.m_cost_count
  local total = price * buyCount
  return total
end

function UIS2ExchangeConfirm:_CheckCoinEnough(buyCount)
  local haveCount = self:_CalcHaveCount()
  local total = self:_CalcTotalPrice(buyCount)
  return haveCount >= total
end

function UIS2ExchangeConfirm:MinBtnOnClick()
  local newCount = self:_CalcMinCount()
  self:_UpdateBuyCount(newCount)
end

function UIS2ExchangeConfirm:SubBtnOnClick()
  local newCount = self._buyCount - 1
  self:_UpdateBuyCount(newCount)
end

function UIS2ExchangeConfirm:AddBtnOnClick()
  local newCount = self._buyCount + 1
  self:_UpdateBuyCount(newCount)
end

function UIS2ExchangeConfirm:MaxBtnOnClick()
  local newCount = self:_CalcMaxCount()
  self:_UpdateBuyCount(newCount)
end

function UIS2ExchangeConfirm:BuyBtnOnClick()
...
function UIS2ExchangeConfirm:_OnReceiveRewards(res, rewards)
...
function UIS2ExchangeConfirm:PetDetailBtnOnClick(go)
  self:ShowDialog("UIShopPetDetailController", self.goodData:GetItemId())
end

function UIS2ExchangeConfirm:CloseBtnOnClick(go)
  self:_CloseDialogWithAnim()
end

function UIS2ExchangeConfirm:_AttachEvents()
  self:AttachEvent(GameEventType.ItemCountChanged, self._Refresh)
end

function UIS2ExchangeConfirm:_DetachEvents()
  self:DetachEvent(GameEventType.ItemCountChanged, self._Refresh)
end

function UIS2ExchangeConfirm:_CloseDialogWithAnim(callback)
  self:CloseDialog()
end
