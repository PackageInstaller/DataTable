_class("UIWidgetSeasonResultRewardS2", UICustomWidget)
UIWidgetSeasonResultRewardS2 = UIWidgetSeasonResultRewardS2

function UIWidgetSeasonResultRewardS2:OnShow()
  self._trans = self:GetGameObject()
  self._itemID = nil
  local sop = self:GetUIComponent("UISelectObjectPath", "uiitem")
  self.uiItem = sop:SpawnObject("UISeasonResultItemS2")
...
function UIWidgetSeasonResultRewardS2:SetShotEffStartTime()
  self.effStartTime = 750
end

function UIWidgetSeasonResultRewardS2:Init(count, iconID, threeStar, returnPrism, firstPass, extAward, doubleExtAward, activityAward, returnHelpAward)
...
function UIWidgetSeasonResultRewardS2:GetNormalTxt()
...
function UIWidgetSeasonResultRewardS2:HasItem(dataList)
...
function UIWidgetSeasonResultRewardS2:BGOnClick(go)
  GameGlobal.GameRecorder():RecordAction(GameRecordAction.UIInput, {
    ui = "UIWidgetSeasonResultRewardS2",
    input = "BGOnClick",
    args = {}
  })
...
function UIWidgetSeasonResultRewardS2:SetLevelDiff(levelDiff)
  local typeMainBg = "exp_s2_map_sedi2"
  if levelDiff then
    if levelDiff == UISeasonLevelDiff.Normal then
      typeMainBg = "exp_s2_map_sedi2"
    elseif levelDiff == UISeasonLevelDiff.Hard then
      typeMainBg = "exp_s2_map_sedi3"
    end
  end
  local atlasName = "UIS2Stage.spriteatlas"
