_class("UISeasonStageMultiAwardListS2", UICustomWidget)
UISeasonStageMultiAwardListS2 = UISeasonStageMultiAwardListS2

function UISeasonStageMultiAwardListS2:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonStageMultiAwardListS2:InitWidget()
  self._seasonAwardGen = self:GetUIComponent("UISelectObjectPath", "Content")
  self._sr = self:GetUIComponent("ScrollRect", "ScrollView")
end

function UISeasonStageMultiAwardListS2:OnHide()
end

function UISeasonStageMultiAwardListS2:SetData(multiAwardList)
  self._multiAwardList = multiAwardList
  local count = #self._multiAwardList
  self._seasonAwardGen:SpawnObjects("UISeasonStageSingleAwardListS2", count)
...
function UISeasonStageMultiAwardListS2:ResetScrollPos()
  self._sr.horizontalNormalizedPosition = 0
end

function UISeasonStageMultiAwardListS2:PlayAnim(totalAnimDelay)
  if self._list then
...
function UISeasonStageMultiAwardListS2:SetWaitAnim()
  if self._list then
    for index, singleList in ipairs(self._list) do
