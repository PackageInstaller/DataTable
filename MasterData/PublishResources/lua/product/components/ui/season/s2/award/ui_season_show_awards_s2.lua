_class("UISeasonShowAwardsS2", UIController)
UISeasonShowAwardsS2 = UISeasonShowAwardsS2

function UISeasonShowAwardsS2:GetComponents()
...
function UISeasonShowAwardsS2:OnShow(uiParams)
  ...
  if uiParams[1] then
    if table.count(uiParams[1]) == 0 then
      Log.fatal("###[UISeasonShowAwardsS2] table.count(uiParams[1]) == 0 !")
    end
  else
    Log.fatal("###[UISeasonShowAwardsS2] uiParams[1] is nil !")
  end
  if uiParams[3] then
    itemlist = uiParams[1]
  else
...
function UISeasonShowAwardsS2:CreateData(itemlist)
...
function UISeasonShowAwardsS2:DoAnimation()
  self._canvasGroup = self:GetUIComponent("CanvasGroup", "canvasGroup")
  self._canvasGroup.alpha = 0
  self._tweener = self._canvasGroup:DOFade(1, 0.02)
end

function UISeasonShowAwardsS2:ClosePanel()
  self:CloseDialog()
end

function UISeasonShowAwardsS2:OnHide()
  if self._closeCallback then
    self._closeCallback()
  end
end

function UISeasonShowAwardsS2:_InitListView(scrollView, index)
  if index < 0 then
    return nil
  end
  local count
  ...
  if item.IsInitHandlerCalled == false then
    ...
    rowPool:SpawnObjects("UISeasonShowAwardsItemS2", count)
  end
...
function UISeasonShowAwardsS2:_ShowItem(giftItem, index, tweenIdx)
  local beforeTime = 0
...
function UISeasonShowAwardsS2:OnItemSelect(id, count, go)
  GameGlobal.EventDispatcher():Dispatch(GameEventType.ShowItemTips, id, self._trans.transform.position)
end

function UISeasonShowAwardsS2:NextOnClick(go)
...
function UISeasonShowAwardsS2:CalcPage()
...
function UISeasonShowAwardsS2:_GetNextPageIndex()
...
function UISeasonShowAwardsS2:_GetCurPageFirstIndex()
  return self._curPageFirstIndex
end

function UISeasonShowAwardsS2:_GetItemDataByIndex(index)
...
function UISeasonShowAwardsS2:_FormatItemCount(itemCount)
  return HelperProxy:GetInstance():FormatItemCount(itemCount)
end

function UISeasonShowAwardsS2:ShowTips(itemId, pos)
