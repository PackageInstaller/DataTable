_class("UIS2ExchangeCell", UICustomWidget)
UIS2ExchangeCell = UIS2ExchangeCell

function UIS2ExchangeCell:SetData(index, info, seasonId, component, tipsCallback)
  self._index = index
  self._info = info
...
function UIS2ExchangeCell:PlayAnimationInSequence(index)
  local tb = {
    {
...
function UIS2ExchangeCell:_GetType()
...
function UIS2ExchangeCell:_SetBg(type)
  local tb = {
    [1] = "exp_s2_shop_kuang01",
    [2] = "exp_s2_shop_kuang02",
    [3] = "exp_s2_shop_kuang03"
  }
  local atlasName = "UIS2Exchange.spriteatlas"
  local spriteName = tb[type]
  UIWidgetHelper.SetImageSprite(self, "_bg", atlasName, spriteName)
end

function UIS2ExchangeCell:_SetRemain()
  local constHide = 0
...
function UIS2ExchangeCell:_SetDiscount()
...
function UIS2ExchangeCell:_SetItem(type)
...
function UIS2ExchangeCell:_SetCoin()
...
function UIS2ExchangeCell:_SetSoldout()
  local isSoldout = self._component:IsExchangeItemSoldout(self._info)
  self:GetGameObject("_soldout"):SetActive(isSoldout)
end

function UIS2ExchangeCell:BtnOnClick()
  Log.info("UIS2ExchangeCell:BtnOnClick index = ", self._index)
  local isSoldout = self._component:IsExchangeItemSoldout(self._info)
  if isSoldout then
    ToastManager.ShowToast(StringTable.Get("str_activity_common_shop_sold_out_msg"))
  else
    self:ShowDialog("UIS2ExchangeConfirm", self._seasonId, self._component, self._info)
  end
end
