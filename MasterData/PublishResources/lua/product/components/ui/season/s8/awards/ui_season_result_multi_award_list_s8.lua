_class("UISeasonResultMultiAwardListS8", UICustomWidget)
UISeasonResultMultiAwardListS8 = UISeasonResultMultiAwardListS8

function UISeasonResultMultiAwardListS8:OnShow(uiParams)
  self:InitWidget()
end

function UISeasonResultMultiAwardListS8:InitWidget()
  self._seasonAwardGen = self:GetUIComponent("UISelectObjectPath", "Content")
  self._sr = self:GetUIComponent("ScrollRect", "ScrollView")
end

function UISeasonResultMultiAwardListS8:OnHide()
end

function UISeasonResultMultiAwardListS8:SetData(matchRes, seasonMissionInfo)
  ...
  self._seasonAwardGen:SpawnObjects("UISeasonResultSingleAwardListS8", count)
...
function UISeasonResultMultiAwardListS8:ProcessCollectionItem(awardList, collectionList)
  local retList = {}
...
function UISeasonResultMultiAwardListS8:ResetScrollPos()
  self._sr.horizontalNormalizedPosition = 0
end

function UISeasonResultMultiAwardListS8:_GetItemCount(vecItem)
  local nItemCount = 0
  if vecItem then
    for i = 1, #vecItem do
...
function UISeasonResultMultiAwardListS8:BattleNormalResultSortAsset(assets)
...
function UISeasonResultMultiAwardListS8:GetPassAward()
  local awardHeadType, cfgId
  return UICommonHelper:GetInstance():GetPassAward(awardHeadType, cfgId)
end

function UISeasonResultMultiAwardListS8:HasItem(dataList, itemId)
  local isNormal = 0
  if dataList then
