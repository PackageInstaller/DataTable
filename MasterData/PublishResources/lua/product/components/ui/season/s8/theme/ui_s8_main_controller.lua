_class("UIS8MainController", UIController)
UIS8MainController = UIS8MainController

function UIS8MainController:Constructor()
  self._isReview = false
  self._canShare = self:GetModule(ShareModule):CanShare()
  self._svrTimeModule = GameGlobal.GetModule(SvrTimeModule)
end

function UIS8MainController:_SetRemainingTime(widgetName, extraId, descId, endTime)
...
function UIS8MainController:_SetCommonTopButton()
...
function UIS8MainController:_Back()
  if self:Manager():CurUIStateType() == UIStateType.UIS8Main then
...
function UIS8MainController:_PlayAnim(idx, callback)
  local tb = {
    ["in"] = {
      animName = "uieffanim_UIS8MainController_in",
      duration = 1800
    },
    out = {
      animName = "uieffanim_UIS8MainController_out",
      duration = 500
    },
    hide = {
      animName = "uieffanim_UIS8MainController_hide",
      duration = 500
    },
    show = {
      animName = "uieffanim_UIS8MainController_show",
      duration = 500
    },
    title = {
      animName = "UIS8MainController_Season_loop"
    }
  }
  UIWidgetHelper.PlayAnimation(self, "_anim", tb[idx].animName, tb[idx].duration, callback)
end

function UIS8MainController:_CheckGuide()
  GameGlobal.EventDispatcher():Dispatch(GameEventType.GuideOpenUI, GuideOpenUI.UIS8MainController)
end

function UIS8MainController:LoadDataOnEnter(TT, res, uiParams)
...
function UIS8MainController:OnShow(uiParams)
  ...
  local obj = UIWidgetHelper.SpawnObject(self, "Title", "UIS8Title")
  self:_Refresh(true)
  self:_AttachEvents()
end

function UIS8MainController:OnHide()
  self:_DetachEvents()
end

function UIS8MainController:_Refresh(first)
...
function UIS8MainController:_CheckOnceMissionOpen()
...
function UIS8MainController:_CalcTime()
  ...
  if curTime < time_action then
    extra, time = "str_season_s8_main_time_cd_action", time_action
  elseif curTime < time_exchange then
    extra, time = "str_season_s8_main_time_cd_exchange", time_exchange
  end
  self:_SetRemainingTime("_time", extra, nil, time)
end

function UIS8MainController:_SetBtn_Collage(first)
  local obj = UIWidgetHelper.SpawnObject(self, "CollageBtn", "UIS8CollageBtn")
  obj:SetData(self._seasonId, first)
end

function UIS8MainController:_SetBtn_Exchange(first)
  local obj = UIWidgetHelper.SpawnObject(self, "ExchangeBtn", "UIS8ExchangeBtn")
  obj:SetData(self._seasonId, self._component_exchange, first)
end

function UIS8MainController:_SetBtn_Medal(first)
  local obj = UIWidgetHelper.SpawnObject(self, "MedalBtn", "UIS8MedalBtn")
  obj:SetData(self._seasonId, first)
end

function UIS8MainController:_SetBtn_Quest(first)
  local obj = UIWidgetHelper.SpawnObject(self, "QuestBtn", "UIS8QuestBtn")
  obj:SetData(self._seasonId, self._component_quest, first)
end

function UIS8MainController:_SetBtn_Go(first)
  self.mazeBtnGo:SetActive(self:_CheckMazeOpen())
  self.onceMissionBtnGo:SetActive(self:_CheckOnceMissionOpen())
  local obj = UIWidgetHelper.SpawnObject(self, "GoBtn", "UIS8GoBtn")
  obj:SetData(self._seasonId, self._component_action, first)
end

function UIS8MainController:_SetBtn_SeasonMaze(first)
...
function UIS8MainController:_Shot(callback)
...
function UIS8MainController:_AfterShot()
...
function UIS8MainController:ShowBtnOnClick(go)
  self:_ShowUI()
end

function UIS8MainController:StoryBtnOnClick(go)
end

function UIS8MainController:IntroBtnOnClick(go)
  UISeasonHelper.ShowSeasonHelperBook(UISeasonHelperTabIndex.S1Main)
end

function UIS8MainController:MazeBtnOnClick(go)
