_class("UISeasonShowAwardsS8", UIController)
UISeasonShowAwardsS8 = UISeasonShowAwardsS8

function UISeasonShowAwardsS8:GetComponents()
...
function UISeasonShowAwardsS8:OnShow(uiParams)
  ...
  if uiParams[1] then
    if table.count(uiParams[1]) == 0 then
      Log.fatal("###[UISeasonShowAwardsS8] table.count(uiParams[1]) == 0 !")
    end
  else
    Log.fatal("###[UISeasonShowAwardsS8] uiParams[1] is nil !")
  end
  if uiParams[3] then
    itemlist = uiParams[1]
  else
...
function UISeasonShowAwardsS8:CreateData(itemlist)
...
function UISeasonShowAwardsS8:DoAnimation()
  self._canvasGroup = self:GetUIComponent("CanvasGroup", "canvasGroup")
  self._canvasGroup.alpha = 0
  self._tweener = self._canvasGroup:DOFade(1, 0.02)
end

function UISeasonShowAwardsS8:ClosePanel()
  self:CloseDialog()
end

function UISeasonShowAwardsS8:OnHide()
  if self._closeCallback then
    self._closeCallback()
  end
end

function UISeasonShowAwardsS8:_InitListView(scrollView, index)
  if index < 0 then
    return nil
  end
  local count
  ...
  if item.IsInitHandlerCalled == false then
    ...
    rowPool:SpawnObjects("UISeasonShowAwardsItemS8", count)
  end
...
function UISeasonShowAwardsS8:_ShowItem(giftItem, index, tweenIdx)
  local beforeTime = 0
...
function UISeasonShowAwardsS8:OnItemSelect(id, count, go)
  GameGlobal.EventDispatcher():Dispatch(GameEventType.ShowItemTips, id, self._trans.transform.position)
end

function UISeasonShowAwardsS8:NextOnClick(go)
...
function UISeasonShowAwardsS8:CalcPage()
...
function UISeasonShowAwardsS8:_GetNextPageIndex()
...
function UISeasonShowAwardsS8:_GetCurPageFirstIndex()
  return self._curPageFirstIndex
end

function UISeasonShowAwardsS8:_GetItemDataByIndex(index)
...
function UISeasonShowAwardsS8:_FormatItemCount(itemCount)
  return HelperProxy:GetInstance():FormatItemCount(itemCount)
end

function UISeasonShowAwardsS8:ShowTips(itemId, pos)
