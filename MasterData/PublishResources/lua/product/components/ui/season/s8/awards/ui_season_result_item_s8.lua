...
_class("UISeasonResultItemS8", UICustomWidget)
UISeasonResultItemS8 = UISeasonResultItemS8

function UISeasonResultItemS8:Constructor()
...
function UISeasonResultItemS8:OnShow(uiParams)
  self._transform = self:GetGameObject().transform
  self._nodes = {}
  self._nodes[UIItemNode.Normal] = UIItemNormalNodeForSeasonS8:New(self:GetUIComponent("UIView", "g_normal"), self)
...
function UISeasonResultItemS8:OnHide()
...
function UISeasonResultItemS8:PlayAni(aniName)
  self._anim:Play(aniName)
end

function UISeasonResultItemS8:SetForm(type, scale, dontPlayAni)
...
function UISeasonResultItemS8:EnableNode(nodeId, isEnable)
...
function UISeasonResultItemS8:SetData(params)
...
function UISeasonResultItemS8:ShowNodes(uiItemNode, enable)
...
function UISeasonResultItemS8:SetScale(scale)
  scale = scale or 1
...
function UISeasonResultItemS8:Select(select, noAnim)
...
function UISeasonResultItemS8:SetClickCallBack(callBack, param)
...
function UISeasonResultItemS8:BtnOnClick(go)
  if self._clickCallBack then
    self._clickCallBack(go)
  end
end

function UISeasonResultItemS8:SetBtnImage(bImage)
end

function UISeasonResultItemS8:SetLongPressCallBack(longPressCallBack, longPressUpCallBack, pressTime, update)
...
function UISeasonResultItemS8:SetReduceLongPressCallBack(sec)
  if self._nodes[UIItemNode.Reduce] then
    self._nodes[UIItemNode.Reduce]:SetReduceLongPressCallBack(sec)
  end
end

function UISeasonResultItemS8:InitLongPress()
...
function UISeasonResultItemS8:LongEvent()
...
function UISeasonResultItemS8:LongEventUpLv(gradeLv)
  local cfg = Cfg.cfg_up_lv_long_press[gradeLv]
  if not cfg then
    Log.fatal("###[UISeasonResultItemS8] LongEventUpLv cfg is nil ! grade:", gradeLv)
  end
  local arr = cfg.Value
  local datas = {}
  for i = 1, #arr do
...
function UISeasonResultItemS8:Calculate(count)
...
function UISeasonResultItemS8:GetBtn()
  return self._btnGO
end

function UISeasonResultItemS8:SetToggleGroup(group)
...
function UISeasonResultItemS8:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleOnValueChangedCallBack(onValueChangedCallBack)
  end
end

function UISeasonResultItemS8:SetToggleValue(isOn)
  if self._nodes[UIItemNode.Toggle] then
    self._nodes[UIItemNode.Toggle]:SetToggleValue(isOn)
  end
end

function UISeasonResultItemS8:GetItemData()
  return self._uiItemData
end

function UISeasonResultItemS8:ClearItemData()
  self._uiItemData = nil
end

function UISeasonResultItemS8:SetBtnImageByName(imgName, atlasName)
end

_class("UIItemNormalNodeForSeasonS8", UIItemNormalNode)
UIItemNormalNodeForSeasonS8 = UIItemNormalNodeForSeasonS8

function UIItemNormalNodeForSeasonS8:SetQuality(quality)
  if quality <= 0 then
  ...
  if qualityName ~= "" then
    ...
    local atlas = self._owner:GetAsset("UIS7Scene.spriteatlas", LoadType.SpriteAtlas)
...
function UIItemNormalNodeForSeasonS8:SetIconOffset(itemId)
...
function UIItemNormalNodeForSeasonS8:SetOffset(showText1)
end
