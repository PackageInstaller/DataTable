_class("UIWidgetSeasonResultRewardS6", UICustomWidget)
UIWidgetSeasonResultRewardS6 = UIWidgetSeasonResultRewardS6

function UIWidgetSeasonResultRewardS6:OnShow()
  self._trans = self:GetGameObject()
  self._itemID = nil
  local sop = self:GetUIComponent("UISelectObjectPath", "uiitem")
  self.uiItem = sop:SpawnObject("UISeasonResultItemS6")
...
function UIWidgetSeasonResultRewardS6:SetShotEffStartTime()
  self.effStartTime = 750
end

function UIWidgetSeasonResultRewardS6:Init(count, iconID, threeStar, returnPrism, firstPass, extAward, doubleExtAward, activityAward, returnHelpAward)
...
function UIWidgetSeasonResultRewardS6:GetNormalTxt()
...
function UIWidgetSeasonResultRewardS6:HasItem(dataList)
...
function UIWidgetSeasonResultRewardS6:BGOnClick(go)
  GameGlobal.GameRecorder():RecordAction(GameRecordAction.UIInput, {
    ui = "UIWidgetSeasonResultRewardS6",
    input = "BGOnClick",
    args = {}
  })
...
function UIWidgetSeasonResultRewardS6:SetLevelDiff(levelDiff)
  local typeMainBg = "exp_s6_map_daoju_di01"
  if levelDiff then
    if levelDiff == UISeasonLevelDiff.Normal then
      typeMainBg = "exp_s6map_daoju_di01"
    elseif levelDiff == UISeasonLevelDiff.Hard then
      typeMainBg = "exp_s6_map_daoju_di02"
    end
  end
  local atlasName = "UIS6Scene.spriteatlas"
