_class("UIS6GoBtn", UICustomWidget)
UIS6GoBtn = UIS6GoBtn

function UIS6GoBtn:OnShow()
  self._constBtnName = GameGlobal.GetUIModule(SeasonModule):GetGoBtnKey()
end

function UIS6GoBtn:OnHide()
end

function UIS6GoBtn:SetData(seasonId, component, playAnim)
  ...
  if playAnim then
    UIWidgetHelper.PlayAnimationInSequence(self, "_anim", "_anim", "uieffanim_UIS6_GoBtn_in", 0, 667)
  end
end

function UIS6GoBtn:_Refresh(component)
...
function UIS6GoBtn:_SetState(state)
  self:GetGameObject("_lock"):SetActive(state == 2)
end

function UIS6GoBtn:_CalcNew()
  ...
  return isOpen and (isNew or talentNew or TradeNew or OnceMissonNew)
end

function UIS6GoBtn:_CalcRed()
  local isOpen = self._component and self._component:ComponentIsOpen()
  if not isOpen then
    return isOpen
  end
  local lastTime = UISeasonLocalDBHelper.SeasonBtn_Get(self._constBtnName, "Red")
  Log.debug("UIS6GoBtn:_CalcRed() lastTime = ", lastTime)
