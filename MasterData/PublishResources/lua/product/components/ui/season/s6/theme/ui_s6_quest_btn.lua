_class("UIS6QuestBtn", UICustomWidget)
UIS6QuestBtn = UIS6QuestBtn

function UIS6QuestBtn:OnShow()
  self._constBtnName = self:GetName()
end

function UIS6QuestBtn:OnHide()
end

function UIS6QuestBtn:SetData(seasonId, component, playAnim)
  ...
  self:_CheckPoint()
  if playAnim then
    UIWidgetHelper.PlayAnimationInSequence(self, "_anim", "_anim", "uieffanim_UIS6_QuestBtn_in", 0, 333)
  end
end

function UIS6QuestBtn:_SetState(state)
  self:GetGameObject("_over"):SetActive(state == 2)
end

function UIS6QuestBtn:_SetNum(cur, total)
  local show = total ~= 0
...
function UIS6QuestBtn:_SetFin(cur, total)
  local show = cur == total and total ~= 0
  self:GetGameObject("_fin"):SetActive(show)
end

function UIS6QuestBtn:_Calc(component)
...
function UIS6QuestBtn:_CalcNew()
