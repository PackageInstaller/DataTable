_class("UISeasonOnceMissionTab", UICustomWidget)
UISeasonOnceMissionTab = UISeasonOnceMissionTab

function UISeasonOnceMissionTab:OnShow(uiParams)
  self:InitWidget()
  self:Deselect()
end

function UISeasonOnceMissionTab:OnHide()
  self:CancelTimer()
end

function UISeasonOnceMissionTab:InitWidget()
...
function UISeasonOnceMissionTab:SetData(onceMissionData, idx, cpt, onSelect)
  self._index = idx
...
function UISeasonOnceMissionTab:RefreshState()
  self:CancelTimer()
...
function UISeasonOnceMissionTab:RefreshNew()
  local bNew = self._onceMissionData:HasNewByComp(self._cpt)
  self.new:SetActive(bNew)
end

function UISeasonOnceMissionTab:Deselect()
  self:_PlayAnim("down")
end

function UISeasonOnceMissionTab:Select()
  self:_PlayAnim("up")
end

function UISeasonOnceMissionTab:RootOnClick(go)
  local state = self._onceMissionData:GetCompState(self._cpt)
  if state == SeasonOnceMissionData.ComState_Closed then
...
function UISeasonOnceMissionTab:ReadNew()
  local bNew = self._onceMissionData:HasNewByComp(self._cpt)
  if bNew then
...
function UISeasonOnceMissionTab:CancelTimer()
...
function UISeasonOnceMissionTab:RequestOnceMissionData()
  self:StartTask(function(TT)
    self:Lock("UISeasonOnceMissionTab:CancelTimer")
    self._onceMissionData:ForceLoadData(TT)
    self:RefreshState()
    self:UnLock("UISeasonOnceMissionTab:CancelTimer")
  end, self)
end

function UISeasonOnceMissionTab:RefreshTime()
...
function UISeasonOnceMissionTab:_PlayAnim(idx, callback)
  local tb = {
    up = {
      animName = "uieff_UISeasonOnceMissionTab_up"
    },
    down = {
      animName = "uieff_UISeasonOnceMissionTab_down"
    }
  }
  if tb[idx] ~= nil then
