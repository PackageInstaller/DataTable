_class("UISeasonShowAwardsS3", UIController)
UISeasonShowAwardsS3 = UISeasonShowAwardsS3

function UISeasonShowAwardsS3:GetComponents()
...
function UISeasonShowAwardsS3:OnShow(uiParams)
  ...
  if uiParams[1] then
    if table.count(uiParams[1]) == 0 then
      Log.fatal("###[UISeasonShowAwardsS3] table.count(uiParams[1]) == 0 !")
    end
  else
    Log.fatal("###[UISeasonShowAwardsS3] uiParams[1] is nil !")
  end
  if uiParams[3] then
    itemlist = uiParams[1]
  else
...
function UISeasonShowAwardsS3:CreateData(itemlist)
...
function UISeasonShowAwardsS3:DoAnimation()
  self._canvasGroup = self:GetUIComponent("CanvasGroup", "canvasGroup")
  self._canvasGroup.alpha = 0
  self._tweener = self._canvasGroup:DOFade(1, 0.02)
end

function UISeasonShowAwardsS3:ClosePanel()
  self:CloseDialog()
end

function UISeasonShowAwardsS3:OnHide()
  if self._closeCallback then
    self._closeCallback()
  end
end

function UISeasonShowAwardsS3:_InitListView(scrollView, index)
  if index < 0 then
    return nil
  end
  local count
  ...
  if item.IsInitHandlerCalled == false then
    ...
    rowPool:SpawnObjects("UISeasonShowAwardsItemS3", count)
  end
...
function UISeasonShowAwardsS3:_ShowItem(giftItem, index, tweenIdx)
  local beforeTime = 0
...
function UISeasonShowAwardsS3:OnItemSelect(id, count, go)
  GameGlobal.EventDispatcher():Dispatch(GameEventType.ShowItemTips, id, self._trans.transform.position)
end

function UISeasonShowAwardsS3:NextOnClick(go)
...
function UISeasonShowAwardsS3:CalcPage()
...
function UISeasonShowAwardsS3:_GetNextPageIndex()
...
function UISeasonShowAwardsS3:_GetCurPageFirstIndex()
  return self._curPageFirstIndex
end

function UISeasonShowAwardsS3:_GetItemDataByIndex(index)
...
function UISeasonShowAwardsS3:_FormatItemCount(itemCount)
  return HelperProxy:GetInstance():FormatItemCount(itemCount)
end

function UISeasonShowAwardsS3:ShowTips(itemId, pos)
