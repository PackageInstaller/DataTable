_class("UISeasonLevelStageS3", UIController)
UISeasonLevelStageS3 = UISeasonLevelStageS3

function UISeasonLevelStageS3:_GetComponents()
...
function UISeasonLevelStageS3:OnShow(uiParams)
...
function UISeasonLevelStageS3:LoadDataOnEnter(TT, res)
end

function UISeasonLevelStageS3.GetAutoOpenState(matchType, stageId)
...
function UISeasonLevelStageS3.SetAutoOpenState(matchType, stageId, isOpen)
  local roleModule = GameGlobal.GetModule(RoleModule)
  local pstId = roleModule:GetPstId()
  local playerPrefsKey = pstId .. "AutoOpenState" .. matchType
  if stageId then
...
function UISeasonLevelStageS3:_CanAutoFight()
  local tipsStr = ""
...
function UISeasonLevelStageS3:_CanShowAutoFight()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS3:InitAutoBtnState()
...
function UISeasonLevelStageS3:_CheckShowWordBuff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
  if not missionCfg then
    return false
  end
...
function UISeasonLevelStageS3:AutoFightBtnOnClick()
  ...
  if self._autoBtnEnable then
    ...
    self:ShowDialog("UISerialAutoFightOptionNew", MatchType.MT_Season, id, power, nil, unlock, nil, campParams, nil, function()
...
function UISeasonLevelStageS3:OnHide()
...
function UISeasonLevelStageS3:HasPassThreeStar(missionCfg)
...
function UISeasonLevelStageS3:GetSortedArr(awardType, cfg, stageAwardType)
...
function UISeasonLevelStageS3:ProcessAward(missionCfg, noNormal)
  local missionId = missionCfg.ID
  local awards = {}
...
function UISeasonLevelStageS3:RefreshAwardsShowState()
  self._noDiffAwardGo = self:GetGameObject("NoDiffAward")
  self._seasonAwardGo = self:GetGameObject("SeasonAward")
  self._seasonNormalAwardGo = self:GetGameObject("MultiAwardGroup1")
  self._seasonHardAwardGo = self:GetGameObject("MultiAwardGroup2")
...
function UISeasonLevelStageS3:RefreshWordsArea()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS3:InitWords()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS3:_InitWords(sop, missionId)
...
function UISeasonLevelStageS3:_GetWordDesc(levelId, wordId)
...
function UISeasonLevelStageS3:RefreshBtnUnknown()
  local show = self:_CheckShowWordBuff()
  self.btnWord:SetActive(show)
end

function UISeasonLevelStageS3:InitForMultiDiff()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS3:RefreshPanelShow()
...
function UISeasonLevelStageS3:RefreshPanelBg()
  local bgName = "exp_s3_map_gq_di07"
  if self._missionDiff == UISeasonLevelDiff.Normal then
    if self._curPanel == UISeasonLevelStagePanelMode.AwardsInfo then
      bgName = "exp_s3_map_gq_di07"
    else
      bgName = "exp_s3_map_gq_di07"
    end
  elseif self._curPanel == UISeasonLevelStagePanelMode.AwardsInfo then
    bgName = "exp_s3_map_gq_di08"
  else
    bgName = "exp_s3_map_gq_di08"
  end
...
function UISeasonLevelStageS3:RefreshPanelText()
  if self._missionDiff == UISeasonLevelDiff.Normal then
    if self._curPanel == UISeasonLevelStagePanelMode.AwardsInfo then
...
function UISeasonLevelStageS3:RefreshDiffArea()
  if self._missionDiff == UISeasonLevelDiff.Normal then
    self._normalBtnText.color = Color(0.878, 0.961, 1, 1)
    self._hardBtnText.color = Color(0.455, 0.518, 0.651, 1)
    self._normalMask:SetActive(false)
    self._hardMask:SetActive(true)
  elseif self._missionDiff == UISeasonLevelDiff.Hard then
    self._normalBtnText.color = Color(0.455, 0.518, 0.651, 1)
    self._hardBtnText.color = Color(0.878, 0.961, 1, 1)
    self._hardMask:SetActive(false)
    self._normalMask:SetActive(true)
  end
end

function UISeasonLevelStageS3:InitAllAwards()
  if self._hasMultiDiff then
    ...
    self._normalMultiAwardList = normalAwardGen:SpawnObject("UISeasonStageMultiAwardListS3")
    ...
    self._hardMultiAwardList = hardAwardGen:SpawnObject("UISeasonStageMultiAwardListS3")
...
function UISeasonLevelStageS3:PlayAnimAwardList(totalAnimDelay)
...
function UISeasonLevelStageS3:_ProcessLevelDiffAwards(missionId, levelDiff, noNormal)
  if missionId then
...
function UISeasonLevelStageS3:Init()
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS3:UpdateCondition(three_star_condition, conditions)
...
function UISeasonLevelStageS3:BtnFightOnClick(go)
  if not self._seasonModule:CheckSeasonAndMissionCoseAndJump() then
    return
  end
...
function UISeasonLevelStageS3:IsPowerEnough()
...
function UISeasonLevelStageS3:ShowTips(itemId, pos)
  self._tips:SetData(itemId, pos)
end

function UISeasonLevelStageS3:BgOnClick()
  self:Close()
end

function UISeasonLevelStageS3:ThreeStarTipsBtnOnClick(go)
  self:ShowDialog("UIThreeStarTips")
end

function UISeasonLevelStageS3:BtnUnknownOnClick(go)
  local buffData = {}
  buffData.name = ""
  buffData.des = ""
...
function UISeasonLevelStageS3:_SetStoryBtn()
...
function UISeasonLevelStageS3:BtnPlotOnClick()
  local storyList = DiscoveryStoryList:New()
  storyList:Init(self._missionID)
...
function UISeasonLevelStageS3:SliderHardOnClick()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS3:SliderNormalOnClick()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
      return
    end
...
function UISeasonLevelStageS3:_SwitchDiff()
  if self._hasMultiDiff then
    if self._missionDiff == UISeasonLevelDiff.Normal then
...
function UISeasonLevelStageS3:DiffInfoBtnOnClick(go)
  self:ShowDialog("UISeasonStageDiffTips")
end

function UISeasonLevelStageS3:RefreshPowerTips()
  ...
  local cur, ceil = self._pointComp:GetItemCount()
  contentText:SetText(string.format("<color=#37d2b0>%s</color>/%s", cur, ceil))
end

function UISeasonLevelStageS3:RefreshPowerCost()
  local needPowerText = self._needPower
  if self._powerID == RoleAssetID.RoleAssetPhyPoint then
  else
    if not self._pointComp then
...
function UISeasonLevelStageS3:InitChapterName()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS3:_InitChapterName(text, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
...
function UISeasonLevelStageS3:RefreshChapter()
  if self._hasMultiDiff then
...
function UISeasonLevelStageS3:RefreshThreeStarArea(playAnim, animDelay)
  local missionCfg = Cfg.cfg_season_mission[self._missionID]
...
function UISeasonLevelStageS3:PlayAnimThreeStarArea(totalAnimDelay)
...
function UISeasonLevelStageS3:InitEnemys()
  if self._hasMultiDiff then
    local normalMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
    local hardMissionId = self._diffMissonIdMap[UISeasonLevelDiff.Hard]
...
function UISeasonLevelStageS3:_InitEnemy(sop, missionId)
  local missionCfg = Cfg.cfg_season_mission[missionId]
  local progress = self._curLevelProgress
...
function UISeasonLevelStageS3:RefreshEnemyArea()
...
function UISeasonLevelStageS3:RefreshRecommendLv()
...
function UISeasonLevelStageS3:RefreshAreasTitleStyle()
end

function UISeasonLevelStageS3:InitBuffLevelArea()
  self._buffLevelArea = self.buffLevelGen:SpawnObject("UISeasonBuffStageAreaS3")
  if self._buffLevelArea then
    self._buffLevelArea:SetData(self._seasonObj)
  end
end

function UISeasonLevelStageS3:RefreshBuffArea()
  if self._buffLevelArea then
    self._buffLevelArea:RefreshInfo()
  end
end

function UISeasonLevelStageS3:Close()
  self:_CloseDialogWithAnim()
end

function UISeasonLevelStageS3:_CloseDialogWithAnim(callback)
...
function UISeasonLevelStageS3:_Resume()
...
function UISeasonLevelStageS3:BattleInfoPanelBtnOnClick()
...
function UISeasonLevelStageS3:AwardsPanelBtnOnClick()
  self._curPanel = UISeasonLevelStagePanelMode.AwardsInfo
  self:RefreshPanelShow()
  self:RefreshPanelBg()
  self:RefreshPanelText()
...
function UISeasonLevelStageS3:PowerTopTipsOnClick(go)
  self:ShowDialog("UISeasonActionPointTipS3", self._pointComp, go.transform.anchoredPosition)
end

function UISeasonLevelStageS3:_Countdown()
...
function UISeasonLevelStageS3:_ReqFlushPower(TT)
...
function UISeasonLevelStageS3:OnAfterUILayerChanged()
...
function UISeasonLevelStageS3:ShowMainUI(show)
...
function UISeasonLevelStageS3:RecordOnClick(go)
  self:ShowDialog("UIActivityLevelRecordController", self._component, self._missionID)
end

function UISeasonLevelStageS3:_TryFixStory(TT)
  local normalMissionID = self._diffMissonIdMap[UISeasonLevelDiff.Normal]
...
function UISeasonLevelStageS3:_ReqFinishStory(TT, id, type)
