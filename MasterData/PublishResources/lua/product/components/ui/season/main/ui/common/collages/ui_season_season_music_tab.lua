_class("UISeasonSeasonMusicTab", UICustomWidget)
UISeasonSeasonMusicTab = UISeasonSeasonMusicTab

function UISeasonSeasonMusicTab:OnShow(uiParams)
  self:InitWidget()
...
function UISeasonSeasonMusicTab:OnHide()
...
function UISeasonSeasonMusicTab:InitWidget()
...
function UISeasonSeasonMusicTab:SetData(data)
  self._collageData = data
  self._musicCount = self._collageData:GetMusicCount()
  self._items = self.content:SpawnObjects("UISeasonSeasonCollageMusicItem", self._musicCount)
...
function UISeasonSeasonMusicTab:SetShow(show)
...
function UISeasonSeasonMusicTab:_OnSelect(data, isInit)
...
function UISeasonSeasonMusicTab:PlayOnClick(go)
...
function UISeasonSeasonMusicTab:PauseOnClick(go)
  if not self._curIdx then
    return
  end
  if self._pause then
...
function UISeasonSeasonMusicTab:LastOnClick(go)
  if not self._curIdx then
    return
  end
...
function UISeasonSeasonMusicTab:NextOnClick(go)
  if not self._curIdx then
    return
  end
  if self._unlockCount == 1 then
...
function UISeasonSeasonMusicTab:_RefreshPlayBar()
  if self._curIdx then
...
function UISeasonSeasonMusicTab:_ResetProgress()
  self.progress.fillAmount = 0
  self.time:SetText("")
end

function UISeasonSeasonMusicTab:_PlayingTick()
  if self._pause then
    return
  end
  local data = self._collageData:GetMusicByIndex(self._curIdx)
...
function UISeasonSeasonMusicTab:_SnapTo(index, animate)
...
function UISeasonSeasonMusicTab:PlayExitAnim()
end
