_class("UIN18LineMissionController_Review", UIController)
UIN18LineMissionController_Review = UIN18LineMissionController_Review

function UIN18LineMissionController_Review:InitWidget()
  local backBtns = self:GetUIComponent("UISelectObjectPath", "_backBtns")
  self._backBtns = backBtns:SpawnObject("UICommonTopButton")
  self._backBtns:SetData(function()
    local campaignModule = GameGlobal.GetModule(CampaignModule)
    campaignModule:CampaignSwitchState(true, UIStateType.CAMPAIGN_TYPE_REVIEW_N18, UIStateType.UIMain, nil, self._campaign._id)
  end)
...
function UIN18LineMissionController_Review:LoadDataOnEnter(TT, res, uiParams)
  self._campaignType = ECampaignType.CAMPAIGN_TYPE_REVIEW_N18
  self._componentId_LineMission = ECampaignReviewN18ComponentID.ECAMPAIGN_REVIEW_ReviewN18_LINE_MISSION
...
function UIN18LineMissionController_Review:OnShow(uiParams)
  self._isOpen = true
...
function UIN18LineMissionController_Review:OnHide()
  self.SLeval = nil
  self.NodeCfg = nil
  self._isOpen = false
  ...
  UIN18LineMissionController_Review.super:Dispose()
  self._scroller:Dispose()
  self:DetachEvents()
end

function UIN18LineMissionController_Review:_Refresh()
  self:FlushNodes()
end

function UIN18LineMissionController_Review:FlushNodes()
...
function UIN18LineMissionController_Review:_OnNodeClick(stageId, isStory, worldPos)
  if isStory then
...
function UIN18LineMissionController_Review:_EnterStage(stageId, worldPos)
  local missionCfg = Cfg.cfg_campaign_mission[stageId]
...
function UIN18LineMissionController_Review:PlotEndCallback(stageId)
...
function UIN18LineMissionController_Review:_CheckSerialAutoFightShow(stageType, stageId)
...
function UIN18LineMissionController_Review:ShowSerialRewards()
  self:ShowDialog("UISerialAutoFightInfo", OpenUISerialFightInfoState.Finished)
end

function UIN18LineMissionController_Review:AttachEvents()
  self:AttachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
  self:AttachEvent(GameEventType.OnUIGetItemCloseInQuest, self.OnUIGetItemCloseInQuest)
end

function UIN18LineMissionController_Review:DetachEvents()
  self:DetachEvent(GameEventType.ActivityCloseEvent, self._CheckActivityClose)
  self:DetachEvent(GameEventType.OnUIGetItemCloseInQuest, self.OnUIGetItemCloseInQuest)
end

function UIN18LineMissionController_Review:_CheckActivityClose(id)
  if self._campaign and self._campaign._id == id then
    self:SwitchState(UIStateType.UIMain)
  end
end

function UIN18LineMissionController_Review:OnUIGetItemCloseInQuest(type)
