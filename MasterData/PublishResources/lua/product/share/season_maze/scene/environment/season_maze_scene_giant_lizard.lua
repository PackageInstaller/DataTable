_class("SeasonMazeSceneGiantLizard", SeasonMazeSceneEnvironmentBase)
SeasonMazeSceneGiantLizard = SeasonMazeSceneGiantLizard

function SeasonMazeSceneGiantLizard:Constructor(sceneRoot)
  self._transform = sceneRoot.transform:Find(SeasonSceneLayer.Building .. "/zone2/S1_pfb_daxiyi")
  if not self._transform then
    Log.error("SeasonMazeSceneGiantLizard S1_pfb_daxiyi not exist.")
    return
  end
  ...
  if not self._effect then
    Log.error("SeasonMazeSceneGiantLizard eff_Scene_daxiyi_penqi not exist.")
  end
  ...
  self._seasonMazeManager = GameGlobal.GetUIModule(SeasonMazeModule):SeasonMazeManager()
  self:RandomAnimation()
end

function SeasonMazeSceneGiantLizard:Update(deltaTime)
  ...
  self._seasonMazeManager:SeasonMazeAudioManager():GetSeasonMazeAudio():SetLizardVolume()
end

function SeasonMazeSceneGiantLizard:Dispose()
end

function SeasonMazeSceneGiantLizard:RandomAnimation()
  if not self._curAnim or self._curAnim.name ~= self._randomAnims[1].name then
  ...
  if self._seasonMazeManager:SeasonMazeAudioManager():GetSeasonMazeAudio() then
    self._seasonMazeManager:SeasonMazeAudioManager():GetSeasonMazeAudio():PlayLizardAudio(self._curAnim.name)
  end
