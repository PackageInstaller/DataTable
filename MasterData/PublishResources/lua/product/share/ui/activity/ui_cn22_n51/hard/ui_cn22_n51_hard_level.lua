_class("UICN22N51HardLevel", UIController)
UICN22N51HardLevel = UICN22N51HardLevel

function UICN22N51HardLevel:Constructor()
  self._isReview = false
end

function UICN22N51HardLevel:LoadDataOnEnter(TT, res, uiParams)
  local campaignModule = GameGlobal.GetModule(CampaignModule)
  self._campaignType = UICN22N51Helper.GetCampaignType(self._isReview)
  self._componentId_BlackMission = UICN22N51Helper.GetComponentId("black", self._isReview)
...
function UICN22N51HardLevel:OnShow(uiParams)
...
function UICN22N51HardLevel:_InitLevelData()
...
function UICN22N51HardLevel:_SortCfg(cfgs)
...
function UICN22N51HardLevel:_Refresh(playAnim)
  for i = 1, 6 do
...
function UICN22N51HardLevel:SetTimeTex()
...
function UICN22N51HardLevel:_SetRemainingTime(widgetName, descId, endTime)
...
function UICN22N51HardLevel:InitCommonTopButton()
  self.topButtonWidget = self.topbuttons:SpawnObject("UINewCommonTopButton")
  self.topButtonWidget:SetData(function()
    local campaignModule = GameGlobal.GetModule(CampaignModule)
    campaignModule:CampaignSwitchState(true, UIStateType.UICN22N51MainController, UIStateType.UIMain, nil, self._campaign._id)
  end, nil, function()
    self:SwitchState(UIStateType.UIMain)
  end)
end

function UICN22N51HardLevel:OnHide()
...
function UICN22N51HardLevel:ClearTeam()
  self:_InitLevelData()
end

function UICN22N51HardLevel:OnComponentClose(componentCfgId)
end

function UICN22N51HardLevel:_Back()
...
function UICN22N51HardLevel:InitWidget()
  self.topbuttons = self:GetUIComponent("UISelectObjectPath", "topbuttons")
  self._levels = {}
  for i = 1, 6 do
    self._levels[i] = UICN22N51HardLevelItem:New(self:GetUIComponent("UIView", "Level" .. i))
  end
...
function UICN22N51HardLevel:_EnterLevel(idx)
...
function UICN22N51HardLevel:Press1OnClick()
  self:_EnterLevel(1)
end

function UICN22N51HardLevel:Press2OnClick()
  self:_EnterLevel(2)
end

function UICN22N51HardLevel:Press3OnClick()
  self:_EnterLevel(3)
end

function UICN22N51HardLevel:Press4OnClick()
  self:_EnterLevel(4)
end

function UICN22N51HardLevel:Press5OnClick()
  self:_EnterLevel(5)
end

function UICN22N51HardLevel:Press6OnClick()
  self:_EnterLevel(6)
end

function UICN22N51HardLevel:InfoBtnOnClick()
  local endTime = self._blackHardCompInfo.m_close_time
  local svrTimeModule = self:GetModule(SvrTimeModule)
  local curTime = math.floor(svrTimeModule:GetServerTime() * 0.001)
  local remainTime = endTime - curTime
  ...
  if remainTime <= 0 then
    local campaignModule = GameGlobal.GetModule(CampaignModule)
    campaignModule:CampaignSwitchState(true, UIStateType.UICN22N51HardLevel, UIStateType.UIMain, nil, self._campaign._id)
    return
  end
  self:ShowDialog("UIIntroLoader", "UICN22N51HardLevel")
end

function UICN22N51HardLevel:_SpawnObject(widgetName, className)
