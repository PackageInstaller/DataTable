...
_class("UICNUIActivityCN21N50IntroType1Widget", UIIntroType1Widget)
UICNUIActivityCN21N50IntroType1Widget = UICNUIActivityCN21N50IntroType1Widget

function UICNUIActivityCN21N50IntroType1Widget:_PlayAnimation(animName, duration, callback)
...
function UICNUIActivityCN21N50IntroType1Widget:CloseDialog()
  self._uiView:CloseDialogWithAnimation()
end

function UICNUIActivityCN21N50IntroType1Widget:SetData(uiView, cfgId)
  self._uiView = uiView
  self._cfg = Cfg.cfg_intro_loader_type_1[cfgId]
  if not self._cfg then
    Log.exception("UICNUIActivityCN21N50IntroType1Widget:SetData() cfg_intro_loader_type_1[", cfgId, "] error")
    self:CloseDialog()
    return
  end
  self:_SetTitle()
...
function UICNUIActivityCN21N50IntroType1Widget:_SetTitle()
  local id = self._cfg.Title
  if not string.isnullorempty(id) then
    local txt = StringTable.Get(id)
    UIWidgetHelper.SetLocalizedTMPText(self, "_title", txt)
  end
end

function UICNUIActivityCN21N50IntroType1Widget:_SetSubTitle()
...
function UICNUIActivityCN21N50IntroType1Widget:_SetIntro()
  ...
  if not string.isnullorempty(id) then
    ...
    for i, ui in ipairs(uis) do
      ...
      ui:SetDataWithHeadTmp(head, body)
    end
  end
end

function UICNUIActivityCN21N50IntroType1Widget:_CalcLen(keyHead)
  local n = 0
  while true do
    n = n + 1
...
function UICNUIActivityCN21N50IntroType1Widget:CloseBtnOnClick(go)
