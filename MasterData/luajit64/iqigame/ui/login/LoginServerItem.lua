...
function m:SetData(serverData)
	...
	UGUIUtil.SetText(self.NameText, LoginUIApi:GetString("goSelectServerName", serverData.serverName, self.State))

	for i = 1, 4 do
		self["StateImage" .. i]:SetActive(i == self.State)
	end

	UGUIUtil.SetText(self.StateText, LoginUIApi:GetString("ServerStateText", self.State))
	self.MaintainBg:SetActive(self.State == 1)
	self.NormalBg:SetActive(self.State ~= 1)
	UGUIUtil.SetTextInChildren(self.RecommendNode, LoginUIApi:GetString("ServerRecommendNodeText"))
	UGUIUtil.SetTextInChildren(self.NewNode, LoginUIApi:GetString("ServerNewNodeText"))
