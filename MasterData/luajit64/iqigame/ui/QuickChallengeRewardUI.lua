-- chunkname: @IQIGame\\UI\\QuickChallengeRewardUI.lua

local QuickChallengeRewardUI = {
	closeTime = 0,
	CompanionUnitCells = {}
}

QuickChallengeRewardUI = Base:Extend("QuickChallengeRewardUI", "IQIGame.Onigao.UI.QuickChallengeRewardUI", QuickChallengeRewardUI)

local BattleUnitSettlementCell = require("IQIGame.UI.Battle.BattleUnitSettlementCell")

function QuickChallengeRewardUI:OnInit()
...
function QuickChallengeRewardUI:GetPreloadAssetPaths()
	return nil
end

function QuickChallengeRewardUI:GetOpenPreloadAssetPaths(userData)
	return nil
end

function QuickChallengeRewardUI:IsManualShowOnOpen(userData)
	return false
end

function QuickChallengeRewardUI:GetBGM(userData)
	return nil
end

function QuickChallengeRewardUI:OnOpen(userData)
...
function QuickChallengeRewardUI:OnClose(userData)
	self.closeTime = 0
	self.rewardData = nil
...
function QuickChallengeRewardUI:OnAddListeners()
...
function QuickChallengeRewardUI:OnRemoveListeners()
...
function QuickChallengeRewardUI:OnPause()
	return
end

function QuickChallengeRewardUI:OnResume()
	return
end

function QuickChallengeRewardUI:OnCover()
	return
end

function QuickChallengeRewardUI:OnReveal()
	return
end

function QuickChallengeRewardUI:OnRefocus(userData)
	return
end

function QuickChallengeRewardUI:OnUpdate(elapseSeconds, realElapseSeconds)
...
function QuickChallengeRewardUI:OnDepthChanged(uiGroupDepth, depthInUIGroup)
	return
end

function QuickChallengeRewardUI:OnLoadSucceed(assetName, asset, duration, userData)
	return
end

function QuickChallengeRewardUI:OnLoadFailed(assetName, status, errorMessage, userData)
	return
end

function QuickChallengeRewardUI:OnDestroy()
...
function QuickChallengeRewardUI:OnClickConfirmBtn()
	if self.rewardData.onBtnCloseCallBack then
		self.rewardData.onBtnCloseCallBack()
	end

	UIModule.Close(Constant.UIControllerName.QuickChallengeRewardUI)
...
function QuickChallengeRewardUI:UpdateView()
...
function QuickChallengeRewardUI:ShowHurtRank(battleOverCommandData, battleAreaCid)
	local isShow = false
...
function QuickChallengeRewardUI:OnClickJumpBtn()
...
function QuickChallengeRewardUI:StopTimer()
