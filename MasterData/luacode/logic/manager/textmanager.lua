...
local _fucntionOriginalTableName = {
  [1] = "word.cwordgift",
  [2] = "word.cwordfriend",
  [3] = "word.cworddialogue",
  [4] = "word.cwordskill",
  [6] = "word.cwordrole",
  [7] = "word.cwordclienttip",
  [8] = "word.cwordclientstring",
  [9] = "word.cwordyard",
  [10] = "word.cwordnpc",
  [11] = "word.cwordtask",
  [12] = "word.cwordshop",
  [13] = "word.cwordhead",
  [14] = "word.cwordhandbook",
  [15] = "word.cwordbattle",
  [16] = "word.cwordequip",
  [17] = "word.cwordscene",
  [18] = "word.cwordtopup",
  [19] = "word.cworditem",
  [20] = "word.cworddungeonselect",
  [21] = "word.cwordguild",
  [22] = "word.cwordbuff",
  [23] = "word.cenemyinfo",
  [24] = "word.cwordguide"
}
local _regionNameTable = {
  [1] = "_ch",
  [2] = "_en",
  [3] = "_kr",
  [4] = "_jp"
...
function TextManager.GetText(textID, ...)
  if textID <= 0 then
    return ""
  end
  ...
  for i, v in ipairs(table.pack(...)) do
    retValue = string.gsub(retValue, "%$parameter" .. tostring(i) .. "%$", v)
  end
  return retValue
end

function TextManager.StartCheck()
...
function TextManager.CheckConfig(configTableName, textFieldName, idFieldName)
  local errorData = {}
  ...
  for i, id in ipairs(ids) do
    ...
    if type(record[textFieldName]) == "string" then
      if record[textFieldName] ~= TextManager.GetText(record[idFieldName]) and not errorData[TextManager.GetText(record[idFieldName])] then
        key = record[idFieldName]
        if key ~= 0 then
          errorData[record[idFieldName]] = TextManager.GetText(record[idFieldName])
        end
      end
    elseif type(record[textFieldName]) == "table" then
...
function TextManager.CheckChangeIdConfig(configTableName, textFieldName, oldTableName, newTableName)
  local errorData = {}
  local configTable = BeanManager.GetTableByName(configTableName)
  local ids = configTable:GetAllIds()
...
function TextManager.CheckNewIdConfig(configTableName, oldFieldName, newFieldName, oldTableName, newTableName)
  local errorData = {}
  local configTable = BeanManager.GetTableByName(configTableName)
  local ids = configTable:GetAllIds()
  local newTable = BeanManager.GetTableByName(newTableName)
