...
local sitLimit = 6
local g = -9.8
local TrainCharacter = strictclass("TrainCharacter")

function TrainCharacter:Ctor(roleId, isAttacker)
...
function TrainCharacter:Destroy()
  self._task = nil
...
function TrainCharacter:IsAttacker()
  return self._isAttacker
end

function TrainCharacter:GetId()
  return self._roleId
end

function TrainCharacter:Clone()
  return clone(self)
end

function TrainCharacter:CreatePlayer(pointObject)
  ...
  local prefab = self._role:GetCabinPrefab()
  self._gameObject = YardSceneHelper.CreatePlayer(prefab.assetBundleName, prefab.prefabName, pointObject)
  self._popPoint = TransformStaticFunctions.GetChild(self._gameObject, "Popposition")
  ...
  EventTriggerHelper.AddTriggerEnterEventListener(self._gameObject, self.OnTriggerEnterEvent, self)
  EventTriggerHelper.AddTriggerExitEventListener(self._gameObject, self.OnTriggerExitEvent, self)
  ...
  if self._isAttacker then
    AnimationHelper.SetAnimatorInteger2(self._gameObject, "isAttacker", 1)
    AnimationHelper.SetAnimatorInteger2(self._gameObject, "isAttacking", 1)
    local proportion = self._role:GetProportion()
    local total = proportion.Hit + proportion.Rest
    self._gameTimerID = GameTimer.AddTask(3, 3, function()
      local num = math.random(1, total)
      if num <= proportion.Hit then
        AnimationHelper.SetAnimatorInteger2(self._gameObject, "isAttacking", 1)
      else
        AnimationHelper.SetAnimatorInteger2(self._gameObject, "isAttacking", 0)
      end
    end)
  end
end

function TrainCharacter:InitZPosition(z)
...
function TrainCharacter:SetMoveRange(pointObject)
...
function TrainCharacter:GetMoveRange()
  return self._moveRange
end

function TrainCharacter:LoadBehavior()
...
function TrainCharacter:BeginDragYardCharacter()
  self._dragTag = true
  self._startFreeFall = nil
  self._pausePlay = true
  AnimationHelper.SetAnimatorInteger2(self._gameObject, "isPick", 1)
  if self._uiCell then
    self._uiCell._rootWindow:SetActive(false)
  end
end

function TrainCharacter:EndDragYardCharacter()
  self._dragTag = false
  self._startFreeFall = 0
end

function TrainCharacter:SetClickParam(intData)
...
function TrainCharacter:SetDeltaMovePosition(x, y, z)
  if self._gameObject then
    RigidbodyStaticFunctions.MoveDeltaPosition(self._gameObject, x, y, z)
  end
end

function TrainCharacter:SetMovePositionSpeed(x, y, z)
  if self._gameObject then
    RigidbodyStaticFunctions.SetMovePositionSpeed(self._gameObject, x, y, z)
  end
end

function TrainCharacter:GetPosition()
  if self._gameObject then
    local x, y, z = TransformStaticFunctions.GetPosition(self._gameObject)
    return {
      x = x,
...
function TrainCharacter:IsState(type)
  return type == self._type
end

function TrainCharacter:IsArriveBorder()
  return self._arriveBorderDirection ~= 0
end

function TrainCharacter:Walk(same, minTime, maxTime)
  self:WalkOrStand(1, same * self._arriveBorderDirection, minTime, maxTime)
end

function TrainCharacter:Stand(same, minTime, maxTime)
  self:WalkOrStand(0, same * self._direction, minTime, maxTime)
end

function TrainCharacter:WalkOrStand(type, direction, minTime, maxTime)
  self._type = type
...
function TrainCharacter:OnUpdate(notification)
...
function TrainCharacter:OnTriggerEnterEvent(colliderObj, curPosition)
  if colliderObj == self._leftColliderObject or colliderObj == self._rightColliderObject then
    if colliderObj == self._leftColliderObject then
...
function TrainCharacter:OnTriggerExitEvent(colliderObj, curPosition)
  if colliderObj == self._leftColliderObject or colliderObj == self._rightColliderObject then
    self._arriveBorderDirection = 0
  end
end

function TrainCharacter:DragYardCharacter(x, y, z)
  local deltaX = 0
  local deltaY = 0
  ...
  if self._startFreeFall and self._posRef.y <= self._moveRange.minY then
    ...
    if self._uiCell then
      self._uiCell._rootWindow:SetActive(true)
    end
  end
...
function TrainCharacter:ClickCharacter()
  if self._isAttacker then
    return
  end
...
function TrainCharacter:OnClickAnimationEnd()
...
function TrainCharacter:SetUiCell(cell)
  self._uiCell = cell
end
