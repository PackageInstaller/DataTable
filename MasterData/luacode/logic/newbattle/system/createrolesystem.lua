...
local CreateRoleComponent, TypeComponent, HpComponent, AttackComponent, BehaviorComponent, PhysicsComponent, PositionComponent, DefenseComponent, AnimationEventComponent, BuffComponent, SummonDataComponent, SkillComponent, DeadDisappearCountComponent

function CreateRoleSystem:Ctor(...)
...
function CreateRoleSystem:Destroy()
  CreateRoleSystem.super.Destroy(self)
end

function CreateRoleSystem:Update()
  for _, entity in ipairs(self._entitys) do
    ...
    for _, infoList in ipairs(createRoleComponent._playerInfoList) do
      ...
      config = config or self._dataUtility.cMonsterCfgTable[playerInfo.id]
      if config then
        ...
        component._alternateFighter = infoList.alternateFighterBean or {}
        local BodyId = box2d.newBody(self._world._physicsWorld, 2, x, y)
        box2d.bodySetUserData(self._world._physicsWorld, BodyId, {
        ...
        if playerInfo.baseskill ~= 0 then
          ...
          if not hasSaveSkillInSkillComponentSkillList then
            table.insert(component._skillList, {
              skillId = playerInfo.baseskill,
              cdTime = fixedpoint(cd),
              currentTime = config.silentFraps
            })
          end
        end
        behavior._data = {
          RandomFunction = self._world._randomPcg
        }
        ...
        if 1 < hpStageNum or config.bossOrNot == 1 then
          ...
          for k, _ in ipairs(bosshpstage) do
            ...
            table.insert(self._world._bossHpStage, {
              totalHpNum = revertHpValue + hpdelta,
              num = revertHpValue,
              stage = false
            })
              ...
              if record and (record.target == TypeComponent.CampType.Left or record.target == TypeComponent.CampType.Both) and record.time == -1 and self._buffUtility.CanAddGlobalBuffBuff(entity, v.buffId, self._world) then
                if v.roleLimit and 0 < string.len(v.roleLimit) then
                  ...
                  for _, roleid in ipairs(roleids) do
                    if playerInfo.id == tonumber(roleid) then
                      isFindeRoleId = true
                      break
                    end
                  end
        ...
        elseif infoList.camp == TypeComponent.CampType.Right then
...
function CreateRoleSystem:ConfigLeftPlayer(entity, playerInfo, config)
  ...
  for _, v in ipairs(playerInfo.equipskilllist) do
    ...
    if v == 20039 then
      ...
      if not hasOccur then
        table.insert(self._world._leftPlayerPowerIncreaseSpeedList, {
          entityId = entity._entityId,
          speed = fixedpoint(self._dataUtility.cBattleConstCfgTable[67].attr) / 1000
        })
      end
    else
      self._buffUtility.SetBuffListToOwnCampEntityBySkillID(entity, entity, v, self._world)
      ...
      if playerInfo.id <= 5000 then
        ...
        if not hasSaveSkillInSkillComponentSkillList then
          table.insert(component._skillList, {
            skillId = v,
            cdTime = fixedpoint(cd),
            currentTime = self._dataUtility.cSkillCostTable[v].CDfraps,
            activeSkillIndex = k
          })
        end
      else
...
function CreateRoleSystem:ConfigRightPlayer(entity, playerInfo, config)
    ...
    if v ~= 0 then
      ...
      if not cRecord then
        ...
        if playerInfo.id <= 5000 then
          ...
          elseif playerInfo.key == LineUpStation.MIDDLE_ROW then
            delayTime = tonumber(self._dataUtility.cBattleConstCfgTable[122].attr)
          elseif playerInfo.key == LineUpStation.BACK_ROW then
