      ...
      if self._utility.IsRoleDead(entity) then
        ...
        if skillInfo then
          ...
          for _, skillStruct in ipairs(skillList) do
            if skillStruct.buff:GetBuffSkillTriggerType() == 1 then
              if self._utility.IsRoleFirstSkill(entity, beforeRateOrChangeOriginalSkillId) then
            ...
            elseif skillStruct.buff:GetBuffSkillTriggerType() == 2 then
              if self._utility.IsRoleSecondSkill(entity, beforeRateOrChangeOriginalSkillId) then
                local random = self._world._randomPcg(1, 100)
                local effectTime = skillStruct.buff:GetLinkSkillEffectTime()
                local totalEffectTime = skillStruct.buff:GetLinkSkillEffectTotalTime()
                local triggerLinkSkillFlag = random <= skillStruct.buff:GetBUffTriggerRate()
            ...
            elseif not self._utility.IsAttackSkill(beforeRateOrChangeOriginalSkillId) then
              local random = self._world._randomPcg(1, 100)
              local effectTime = skillStruct.buff:GetLinkSkillEffectTime()
              local totalEffectTime = skillStruct.buff:GetLinkSkillEffectTotalTime()
              local triggerLinkSkillFlag = random <= skillStruct.buff:GetBUffTriggerRate()
          ...
          elseif self._utility.IsYuZiZhaoHuan(currentEntitySkillID) then
            ...
            if component then
              ...
              if lengs == 3 then
                ...
                local summonEntity = self._world:CreateLeftPlayerSummon({
                  name = "yuzi",
                  monsterid = monsterid,
                  buffdata = buffdata,
                  skillid = currentEntitySkillID,
            ...
            if (currentEntitySkillID == 201024 or 201066 == currentEntitySkillID or currentEntitySkillID == 201278) and self._animationUtility.IsNeedResetEntityAnimationEventComponentIsInChangePosition(entity) and hittedEntityList[1] and not self._utility.IsRoleDead(hittedEntityList[1]) then
