...
local function begin_recover(self)
...
function Skate:BuildStateEffects()
...
function Skate:AddWaypoints(points)
  self._waypoints = points
end

function Skate:SetRecoverArea(area)
  self._recoverArea = area
end

function Skate:OnEnterArea(builder)
  if builder == self._builder and self._state == 0 then
    self:ToNextState():Run()
  end
end

function Skate:OnLeaveArea(builder)
