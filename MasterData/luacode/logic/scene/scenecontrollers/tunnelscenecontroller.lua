...
local function SetUVSpeed(self, isMaxSpeed)
...
local function SetActorSpeed(self, x, y, speed)
...
local function SetTunnelScene(self, direction, length)
  local targetTunnelSceneRotationY, targetActorPosX, targetActorPosZ
...
local function HandleTimeLineStop(self, notification)
  local userInfo = notification.userInfo
...
local function UpdateMove(self, deltaTime, unscaleDeltaTime)
  if self._isStartMove == false then
    return
  end
  self._moveTimer = self._moveTimer + deltaTime
  if self._moveTimer >= self._moveTotalTime then
...
function TunnelSceneController:Init()
...
function TunnelSceneController:OnEnable()
  LuaAudioManager.PlayBGM(61)
end

function TunnelSceneController:OnTriggerBehaviourPlay(doubleValue, longValue, stringValue, objectValue)
...
function TunnelSceneController:OnDestroy()
...
function TunnelSceneController:Update(deltaTime, unscaleDeltaTime)
...
function TunnelSceneController:BSC_UpdateBattleLogicOneStep()
  ...
  elseif self._buff20014MusicState == "down" then
...
function TunnelSceneController:OnSRoleCreate(protocol)
...
function TunnelSceneController:OnSRoleDead(protocol)
...
function TunnelSceneController:OnSSkillStart(skillID)
...
function TunnelSceneController:OnSSkillEnd(skillID)
...
function TunnelSceneController:BSC_BattleEnd()
...
function TunnelSceneController:OnSBattleStart(protocol)
