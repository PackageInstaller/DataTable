...
local SmallBattleDialog = class("SmallBattleDialog", Dialog)
SmallBattleDialog.AssetBundleName = "ui/layouts.battlepassnew"
SmallBattleDialog.AssetName = "BattlePassNewForBattle"
local charNumber = 3
local ResultType = {Success = 1, Failed = 0}
local RightEnemyIndex = 0
SmallBattleDialog.TargetType = {KillBoss = 1}

function SmallBattleDialog:Ctor(...)
  SmallBattleDialog.super.Ctor(self, ...)
  self._groupName = "Modal"
end

function SmallBattleDialog:OnCreate()
...
function SmallBattleDialog:OnDestroy()
...
function SmallBattleDialog:SetData(target, serverResult, data, charlist, score)
  self._target = target
  if self._target == SmallBattleDialog.TargetType.KillBoss then
    LogInfoFormat("SmallBattleDialog", "BattlettleType -- %s; BattleId -- %s", "KillBoss", data.id)
    self._score = score
  end
  ...
  for _, c in ipairs(self._charList) do
    table.insert(self._leftQueue, {
      attack = c.attack,
      blood = c.blood,
      battlesort = c.battlesort,
      roleId = c.roleId
    })
  end
  if #self._leftQueue <= 0 then
    LogError("SmallBattleDialog", "left has no character")
    self:RealQuit()
    return
  end
  table.sort(self._leftQueue, function(a, b)
    return a.battlesort < b.battlesort
  end)
  for i = 1, charNumber do
    self._char[i].panel:SetActive(false)
  end
  local npcShapeRecorder, role
  local i = charNumber
  if #self._leftQueue == 1 and self._target == SmallBattleDialog.TargetType.KillBoss then
    i = math.ceil(charNumber / 2)
  end
  for _, c in ipairs(self._leftQueue) do
    if self._char[i] then
      ...
      LogErrorFormat("SmallBattleDialog", "index %s char not on UI", i)
    end
    i = i - 1
  end
  self._rightQueue = {
    attack = self._floorData.enemyATK,
    blood = self._floorData.enemyHP
  }
  ...
  if self._result == serverResult then
    ...
    LogErrorFormat("SmallBattleDialog", "serverResult %s clientResult %s. operation failure", serverResult, self._result)
    self:RealQuit()
  end
end

function SmallBattleDialog:OnAnimationStateExit(handle, stateName, normalizedTime)
...
function SmallBattleDialog:OnPrefabModelAnimationStateExit(handle, stateName, normalizedTime)
...
function SmallBattleDialog:OnInjuryNumAnimationStateExit(index, handle, stateName, normalizedTime)
...
function SmallBattleDialog:OnSkipBtnClicked()
  self:Quit()
end

function SmallBattleDialog:Act()
...
function SmallBattleDialog:MakeBattleOpList()
  self._opList = {}
  local removeList = {}
  local leftV
  while true do
    for _, v in ipairs(self._leftQueue) do
      ...
      table.insert(self._opList, {
        index = v.index,
        damage = v.attack,
        target = RightEnemyIndex,
        blood = self._rightQueue.blood
      })
    ...
    table.insert(self._opList, {
      index = RightEnemyIndex,
      damage = self._rightQueue.attack,
      target = leftV.index,
      blood = leftV.blood
    })
...
function SmallBattleDialog:Quit()
  if self._result == ResultType.Failed then
    DialogManager.CreateSingletonDialog("smallbattle.smallbattlefaileddialog")
  elseif self._result == ResultType.Success then
    DialogManager.CreateSingletonDialog("smallbattle.smallbattlewindialog"):SetData(self._score)
  else
    self:RealQuit()
  end
end

function SmallBattleDialog:RealQuit()
  LuaNotificationCenter.PostNotification(Common.n_AprilFoolsClientBattleProcessOver, self, nil)
  self:Destroy()
end

function SmallBattleDialog:OnDialogWillDestroy(notification)
  if notification.userInfo._dialogName == "smallbattle.smallbattlefaileddialog" or notification.userInfo._dialogName == "smallbattle.smallbattlewindialog" then
    self:RealQuit()
  end
end

return SmallBattleDialog
