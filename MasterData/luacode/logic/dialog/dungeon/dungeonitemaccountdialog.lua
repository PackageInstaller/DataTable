...
local DungeonItemAccountDialog = class("DungeonItemAccountDialog", Dialog)
DungeonItemAccountDialog.AssetBundleName = "ui/layouts.dungeon"
DungeonItemAccountDialog.AssetName = "DungeonItemAccount"

function DungeonItemAccountDialog:Ctor(...)
  DungeonItemAccountDialog.super.Ctor(self, ...)
...
function DungeonItemAccountDialog:OnCreate()
  self._width, self._height = self:GetRootWindow():GetRectSize()
end

function DungeonItemAccountDialog:OnDestroy()
...
function DungeonItemAccountDialog:SetData(protocol)
  local itemAccounts = {}
  for i, v in ipairs(protocol.items) do
    ...
    if item then
      table.insert(itemAccounts, {
        item = item,
        num = v.number
      })
    else
      LogErrorFormat("DungeonItemAccountDialog", "bagtype %s item gain %s itemtype %s protocol.id %s", v.bagtype, v.gain, v.itemtype, v.id)
    end
  end
  table.sort(itemAccounts, function(a, b)
    return a.item:GetRank() < b.item:GetRank()
  end)
...
function DungeonItemAccountDialog:ProcessData()
  local itemCell = {}
...
function DungeonItemAccountDialog:OnRefresh()
  if #self._cellInfo == 0 then
    if #self._dataList == 0 then
      self:Destroy()
    end
...
return DungeonItemAccountDialog
