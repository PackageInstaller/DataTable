...
function MailCell:OnDestroy()
end

function MailCell:RefreshCell(data)
  ...
  self._getAttaImage:SetActive(data.status ~= 0 and 0 < #data.attachmentList)
  self._attaImage:SetActive(data.status == 0 and 0 < #data.attachmentList)
...
function MailCell:OnMouseClicked()
  self._selectBack:SetActive(true)
  local IsRefresh = false
  if self._cellData.status == 0 and 0 >= #self._cellData.attachmentList then
    IsRefresh = true
  end
  ...
  if self._cellData.status == 0 and 0 >= #self._cellData.attachmentList and self._cellData.id ~= 2456 and self._cellData.id ~= DataCommon.ReturnBattlePass_MailId then
...
function MailCell:OnEvent(eventName, arg)
  if eventName == "ChangedSelected" then
    local tag = self._cellData.uniqueId == arg
