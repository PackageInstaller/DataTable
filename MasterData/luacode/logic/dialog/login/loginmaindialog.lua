...
function LoginMainDialog:OnCreate()
...
function LoginMainDialog:OnDestroy()
  UIBackManager.SetUIBackShow(false)
  LuaNotificationCenter.RemoveObserver(self)
end

function LoginMainDialog:Init(isTest)
  self._isTest = isTest
  ...
  if isTest then
    ...
    SdkManager.Login(JSON.encode({
      domain = client_server_list[1].sdkDomain,
      port = client_server_list[1].sdkPort
    }))
  end
end

function LoginMainDialog:OnServerCellClicked(data)
  ...
  self._ip_quest_port = ipServerPortList[ipIndex]
  SdkManager.Login(JSON.encode({
    domain = data.sdkDomain,
    port = data.sdkPort
  }))
end

function LoginMainDialog:OnTapClicked(args)
  if self._login then
    LuaNetManager.ConnectToServer(self._ip, self._port, self._token, "", self._plat)
  elseif self._failToLinkSDK then
    self._login = false
    self._failToLinkSDK = false
    self._failToLinkIP = false
    if self._tapClickHandler ~= -1 then
  ...
  elseif self._failToLinkIP then
    self._failToLinkIP = false
    if self._tapClickHandler ~= -1 then
      self:GetRootWindow():Unsubscribe_PointerClickEvent(self._tapClickHandler)
      self._tapClickHandler = -1
...
function LoginMainDialog:OnLoginOutBtnClicked()
  CS.PixelNeko.P1.PlatformManager.Logout("")
end

function LoginMainDialog:OnLoginResult(notification)
  ...
  if resultJson then
    ...
    elseif resultJson.result == "false" and resultJson.selfDefineErrorType == LoginMainDialog.ErrorType.LinkFailed then
...
function LoginMainDialog:OnLogoutResult(notification)
  local resultJson = JSON.decode(notification.userInfo)
  if resultJson then
    if resultJson.result == "true" then
      self._loginOutBtn:SetActive(false)
    elseif resultJson.result ~= "false" or resultJson.selfDefineErrorType == LoginMainDialog.ErrorType.LogoutFailed then
    end
  end
end

function LoginMainDialog:OnMainConnectException(notification)
  NekoData.BehaviorManager.BM_Message:AddMessageTip(notification.userInfo:GetException())
end

function LoginMainDialog:OnIPServerResponse(notification)
  local resultJson = JSON.decode(notification.userInfo)
  if resultJson.result == "true" then
...
function LoginMainDialog:OnFailedNoticeShow()
  if self._tapClickHandler ~= -1 then
    self:GetRootWindow():Unsubscribe_PointerClickEvent(self._tapClickHandler)
    self._tapClickHandler = -1
  end
  ...
  local startTimeTable = {
    year = tonumber(startTime[1]),
    month = tonumber(startTime[2]),
    day = tonumber(startTime[3]),
    hour = tonumber(startTime[4]) - 8,
    min = tonumber(startTime[5]),
    sec = tonumber(startTime[6])
  }
  local endTimeTable = {
    year = tonumber(endTime[1]),
    month = tonumber(endTime[2]),
    day = tonumber(endTime[3]),
    hour = tonumber(endTime[4]) - 8,
    min = tonumber(endTime[5]),
    sec = tonumber(endTime[6])
  }
...
function LoginMainDialog:OnNoticeBtnClicked()
...
function LoginMainDialog:OnIPServerLinkFailed(notification)
...
function LoginMainDialog:OnSdkUrlWrong()
...
function LoginMainDialog:OnChangeServerBtnClicked()
  self:Init(self._isTest)
end
