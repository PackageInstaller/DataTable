...
function BreakEquipTabCell:Ctor(...)
...
function BreakEquipTabCell:OnCreate()
...
function BreakEquipTabCell:OnDestroy()
...
function BreakEquipTabCell:OnHidden()
  if self._effectHandler then
    self._effect:ReleaseEffect(self._effectHandler)
    self._effectHandler = nil
  end
...
function BreakEquipTabCell:OnRefresh(notification)
...
function BreakEquipTabCell:RefreshTabCell()
end

function BreakEquipTabCell:Init(equipKey)
  ...
  local currentStageUnlockCount, nextStageUnlockIndex, nextStageUnlockWord, currentBreakCfgRecorder
    ...
    for key, value in pairs(randomWordList) do
      if value.index == nextStageUnlockIndex then
        ...
        if nextStageUnlockWord.kind == 1 then
          ...
          break
        elseif nextStageUnlockWord.kind == 2 then
          self._lock:SetActive(true)
...
function BreakEquipTabCell:RefreshItemFrame(items)
  self._selectedItem = {}
...
function BreakEquipTabCell:OnStartBtn()
...
function BreakEquipTabCell:OnGreyBtn()
  NekoData.BehaviorManager.BM_Message:AddMessageTip(TextManager.GetText(701159))
end

function BreakEquipTabCell:SendCBreakThroughEquip()
...
function BreakEquipTabCell:OnMaterialsIcon(index)
...
function BreakEquipTabCell:OnCellLongPress()
...
function BreakEquipTabCell:NumberOfCell(frame)
...
function BreakEquipTabCell:CellAtIndex(frame, index)
  if frame == self._attrFrame then
...
function BreakEquipTabCell:DataAtIndex(frame, index)
  if frame == self._attrFrame then
