...
function GachaResultDialog:Ctor(...)
...
function GachaResultDialog:OnCreate()
  ...
  self._width, self._height = self._equipPanel:GetRectSize()
  self._anchoredx, self._anchoredy = self._equipPanel:GetAnchoredPosition()
  ...
  self._shareAward = {
    icon = self:GetChild("ShareBtn/Item"),
    num = self:GetChild("ShareBtn/Num"),
    back = self:GetChild("ShareBtn/TextBack"),
    txt = self:GetChild("ShareBtn/Text")
  }
...
function GachaResultDialog:OnDestroy()
  ...
  if self._otherItemsFromMail then
    ...
    if dialog then
      dialog:LoadData({
        items = self._otherItemsFromMail
      })
    end
  end
...
function GachaResultDialog:RevertCanShare()
  self._sharing = false
end

function GachaResultDialog:SetDrawCardType(type)
  self._drawCardType = type
  if not self._drawCardType or self._drawCardType == DataCommon.DrawCardType.Equip then
  end
end

function GachaResultDialog:SetOtherWayGetCard(otherWayGet)
  if otherWayGet then
...
function GachaResultDialog:SetTenDrawGuide()
  self._tenDrawGuide = true
  self._tenDrawGuide_DetailBtn:SetActive(true)
end

local function ChangeCellPosition(self)
...
local function ShowMoneyPanel(self)
  local dialog = DialogManager.GetDialog("gacha.gachamaindialog")
  if dialog then
...
function GachaResultDialog:SetData(cardList, otherItemsFromMail)
  if self._drawCardType and self._drawCardType == DataCommon.DrawCardType.Equip then
    ...
    for i, v in ipairs(cardList) do
      ...
      elseif itemtype == ItemTypeEnum.BASEITEM then
        local item = Item.Create(id)
        if item:IsUniqueEquip() then
          itemInfo.item = UniqueEquip.Create(id)
          itemInfo.type = "UniqueEquip"
    ...
    if cardList and 0 < #cardList then
      if 1 < #cardList then
        if self._frameOne then
          self._frameOne:Destroy()
          self._frameOne = nil
...
function GachaResultDialog:RefreshShare(state)
...
function GachaResultDialog:OnAgainBtnClicked()
  local dialog = DialogManager.GetDialog("gacha.gachamaindialog")
  if dialog and dialog:GetSelectedPool() then
    ...
    elseif NekoData.BehaviorManager.BM_Gacha:GetDrawTotalTimes() - NekoData.BehaviorManager.BM_Gacha:GetDrawTimes() > 0 then
...
local function SureTenDrawGuide(self)
...
function GachaResultDialog:OnExitBtnClicked()
  if self._tenDrawGuide then
...
function GachaResultDialog:OnExitGuideBtnClicked()
  DialogManager.DestroySingletonDialog("gacha.gachamoviedialog")
  local convertItems = NekoData.BehaviorManager.BM_Gacha:GetConvertItems()
  if convertItems and #convertItems ~= 0 then
    local dialog = DialogManager.CreateSingletonDialog("bag.itemaccountdialog")
...
function GachaResultDialog:OnShareClick()
...
function GachaResultDialog:CellAtIndex(frame, index)
  if frame == self._moneyFrame then
...
function GachaResultDialog:DataAtIndex(frame, index)
  if frame == self._moneyFrame then
  ...
  elseif frame == self._frameTen then
    return {
      self._roleList[2 * index - 1],
      self._roleList[2 * index]
    }
  elseif frame == self._equipFrame then
...
function GachaResultDialog:CheckRoleInfo(keyTag)
  LogInfo("展示角色详情界面")
  local index
  local roleIdList = {}
  local cfgIdList = {}
  ...
  DialogManager.CreateSingletonDialog("mainline.bossrush.checkotherroleinfodialog"):Init({
    ...
    cfgIdList = cfgIdList,
    index = index
  }, CheckOtherRoleInfoDialog.ShowType.BaseLevelInfo, CheckOtherRoleInfoDialog.TagType.GachaResult)
end
