...
function SpringFestivalTeamEditDialog:Ctor(...)
...
function SpringFestivalTeamEditDialog:OnCreate()
...
function SpringFestivalTeamEditDialog:OnDestroy()
  LuaNotificationCenter.RemoveObserver(self)
  UIBackManager.SetUIBackShow(false)
end

function SpringFestivalTeamEditDialog:SetCopyInfo(copyType, id, additionalData)
...
function SpringFestivalTeamEditDialog:DestroyModal(dialog)
end

function SpringFestivalTeamEditDialog:OnRefreshSpirit(notification)
  self._spirit = notification.userInfo.spirit
end

function SpringFestivalTeamEditDialog:OnRefreshCurrency()
end

function SpringFestivalTeamEditDialog:RefreshFashion()
...
function SpringFestivalTeamEditDialog:CheckTeamRationalForBeginTask()
...
function SpringFestivalTeamEditDialog:OnRefreshTeam(notification)
  self:LoadLocalData()
...
function SpringFestivalTeamEditDialog:OnRefresh(notification)
  self:LoadLocalData()
  self:Refresh(self._teamId)
end

function SpringFestivalTeamEditDialog:LoadLocalData()
...
function SpringFestivalTeamEditDialog:Refresh(teamId)
...
function SpringFestivalTeamEditDialog:JudgeIfNeedAutoTeamFunction()
...
function SpringFestivalTeamEditDialog:CheckIfTeamRational()
  local isMainLine = CDungeonSelectMainlineTable:GetRecorder(self._copyID)
...
function SpringFestivalTeamEditDialog:RefreshTeamElementInfo()
...
function SpringFestivalTeamEditDialog:OnEnterDungeon()
...
function SpringFestivalTeamEditDialog:OnBtn1Clicked()
...
function SpringFestivalTeamEditDialog:OnBtn4Clicked()
...
function SpringFestivalTeamEditDialog:BeginTaskBtnClicked()
  local tag = false
...
function SpringFestivalTeamEditDialog:OnTeamClicked(index)
  for i, v in ipairs(self._teamBtn) do
    if v.editBtn then
      v.editBtn:SetActive(false)
    end
...
function SpringFestivalTeamEditDialog:OnTeamNameEditClicked()
...
function SpringFestivalTeamEditDialog:SetTestBattleInfo(testBattleInfo)
  self._testBattleInfo = testBattleInfo
  NekoData.DataManager.DM_Team:ClearSupportRole()
end

function SpringFestivalTeamEditDialog:ReturnRoleList()
  local roleList = {}
  for _, v in ipairs(self._curTeamRoles) do
    if v.roleKey ~= 0 then
...
function SpringFestivalTeamEditDialog:OnBackBtnClicked()
  self:Destroy()
end

function SpringFestivalTeamEditDialog:OnMenuBtnClicked()
  DialogManager.CreateSingletonDialog("fastmenu.fastmenudialog")
end

function SpringFestivalTeamEditDialog:OnCharPosLockClicked(posIndex)
  NekoData.BehaviorManager.BM_Message:SendMessageById(100244)
end

function SpringFestivalTeamEditDialog:OnCharPosClicked(posIndex)
...
function SpringFestivalTeamEditDialog:OnCharCellBeginDrag(screenPosition, data)
  self._draging = true
...
function SpringFestivalTeamEditDialog:OnCharCellDrag(screenPosition, data)
...
function SpringFestivalTeamEditDialog:OnCharCellEndDrag(screenPosition, data)
  self._draging = false
...
function SpringFestivalTeamEditDialog:OnSwitchPositionBtnClicked()
  if self._inSwitchPositionState then
...
function SpringFestivalTeamEditDialog:OnAutoTeamBtnClicked()
...
function SpringFestivalTeamEditDialog:OnElementBtnClicked()
...
function SpringFestivalTeamEditDialog:GetSupportVocationTable()
...
function SpringFestivalTeamEditDialog:UnreasonableLineupTip(id, func)
