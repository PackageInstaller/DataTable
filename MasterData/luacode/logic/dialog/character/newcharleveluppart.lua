...
function CharLevelUpPart.OnCreate(delegate)
  self = delegate
...
function CharLevelUpPart.OnDestroy()
  self = nil
  LuaNotificationCenter.RemoveObserver(CharLevelUpPart)
end

function CharLevelUpPart.RefreshLevelUpInfo(notPlayTask)
  ...
  elseif self._lv >= LongPressLevelUpLevelLimit then
    self._clickLevelUpBtnCount = nil
  elseif not self._clickLevelUpBtnCount then
    self._clickLevelUpBtnCount = 0
  end
  ...
  if self._data:GetLevel() == self._data:GetLevelMaxByBreakRecord(roleBreakRecord) then
    ...
    if 0 < maxExp - self._data:GetExp() then
  ...
  if not self._lvUpper then
    ...
    local properties = self._data:GetCurLvBasePro()
    table.insert(self._levelup_task1, Tween.new(Time, {
...
function CharLevelUpPart.PlayEffect()
...
function CharLevelUpPart:OnEffectEnd(effectHandler, window)
  if self._effectHandlers[effectHandler] then
    self._effectHandlers[effectHandler] = nil
  end
end

function CharLevelUpPart.PlayLongPressEffect()
...
function CharLevelUpPart.ReleaseLongPressEffect()
  if self._effectHandler2 and self._effectHandler2 ~= 0 then
    self._levelUpLongPressEffect:ReleaseEffect(self._effectHandler2)
    self._effectHandler2 = 0
  end
...
function CharLevelUpPart.RefreshExp(levelandexp)
  self._mana = NekoData.BehaviorManager.BM_Currency:GetMoney()
  local lv = self._lv
...
function CharLevelUpPart.OnLevelUpBtnPointerDown()
...
function CharLevelUpPart.OnLevelUpBtnClick()
  ...
  if self._lastExp ~= self._exp and not self._sendProtocolTag and not self._progressTask then
    ...
    local breakLevel = self._data:GetLevelMaxByBreakRecord(roleBreakRecord)
    if not breakLevel then
      return
    end
    if self._lv == breakLevel then
      if levelMax > self._lv then
    ...
    elseif self._lv >= self._data:GetMaxLevel() then
...
function CharLevelUpPart.OnLevelUpBtnBeginLongPress()
...
function CharLevelUpPart.OnLevelUpBtnLongPress()
...
function CharLevelUpPart.OnLevelUpBtnEndLongPress()
...
function CharLevelUpPart.OnLevelUpBtnCancelLongPress()
  if self._longPressLevelUp then
    NekoData.BehaviorManager.BM_Voice:Play(self._data:GetId(), NekoData.BehaviorManager.BM_Voice.VoiceType.LevelUp, nil)
    self._longPressLevelUp = false
  end
...
function CharLevelUpPart.OnUpdate(a, notification)
  if self._levelup_task1 and #self._levelup_task1 ~= 0 then
  ...
  if self._progressTask then
    ...
    if self._progressTask:update(notification.userInfo.deltaTime) then
      ...
      str = string.gsub(str, "%$parameter2%$", tostring(maxExp))
      self._levelup_expTxt:SetText(str)
      if 0 < maxExp - math.ceil(self._progressTask.subject.exp) then
...
function CharLevelUpPart.OnQuickLevelUpBtnClick()
  local roleBreakRecord = self._data:GetBreakRecords()[self._breakLv]
  ...
  if self._lv == levelMax then
    ...
    if maxLevel <= self._lv then
      ...
      if record.clientBreakLevel == self._breakLv then
        ...
        if needMana > NekoData.BehaviorManager.BM_Currency:GetCurrencyNum(DataCommon.ManaID) then
          enough = false
        end
      else
