...
function CharBreakPart.OnCreate(delegate)
  self = delegate
...
function CharBreakPart.OnDestroy()
  self = nil
  LuaNotificationCenter.RemoveObserver(CharBreakPart)
end

function CharBreakPart.RefreshBreakInfo()
  ...
  self._break_showSuccessData = {
    role = data,
    curLv = data:GetLevel(),
    curMaxLv = data:GetMaxLevelIgnorePlayerLv(),
    curBreakLv = data:GetBreakLv(),
    nextMaxLv = data:GetNextBreakMaxLevel(),
    nextBreakLv = data:GetNextBreakLv(),
    maxBreakLv = data:GetMaxBreakLv(),
    gainSkin = 0,
    curHp = properties[AttrTypeEnum.MAX_HP],
    curAttack = properties[AttrTypeEnum.ATTACK],
    curDefend = properties[AttrTypeEnum.DEFEND],
    curMagicDefend = properties[AttrTypeEnum.MAGIC_DEFEND]
  }
  ...
  self._break_levelLeftBack:SetActive(0 < currentBreakLv)
  self._break_levelLeft:SetActive(0 < currentBreakLv)
  if 0 < currentBreakLv then
  ...
  if breakLvRange ~= nil and 4 <= #breakLvRange then
  ...
  if self._break_showSuccessData.nextBreakLv and self._break_showSuccessData.curBreakLv < self._break_showSuccessData.nextBreakLv then
    ...
    if self._break_showSuccessData.curLv >= self._break_showSuccessData.curMaxLv then
  ...
  self._break_breakBtn:SetInteractable(data:GetBreakSoul() <= self._mana and self._break_showSuccessData.curLv == self._break_showSuccessData.curMaxLv and tag and self._break_showSuccessData.nextBreakLv and self._break_showSuccessData.curBreakLv < self._break_showSuccessData.nextBreakLv)
end

function CharBreakPart.OnBreakBtnClick()
...
function CharBreakPart.OnRoleInfoChange(nouse, notification)
  if notification.userInfo.name == "sroleupdatebreaklv" then
    ...
    if dialog then
      dialog:SetData({
        role = self._data,
        effect = 1022,
        showSuccessData = self._break_showSuccessData,
        type = SHOWTALKTYPE.Break
      })
    end
