...
function CharEvolvePart.OnCreate(delegate)
  self = delegate
  ...
  self._evolve_item1_x, self._evolve_item1_ox, self._evolve_item1_y, self._evolve_item1_oy = self._evolve_item1:GetPosition()
  self._evolve_item1Back_x, self._evolve_item1Back_ox, self._evolve_item1Back_y, self._evolve_item1Back_oy = self._evolve_item1Back:GetPosition()
  self._evolve_item1Name_x, self._evolve_item1Name_ox, self._evolve_item1Name_y, self._evolve_item1Name_oy = self._evolve_item1Name:GetPosition()
  LuaNotificationCenter.AddObserver(CharEvolvePart, CharEvolvePart.OnRoleInfoChange, Common.n_RoleInfoChange, nil)
end

function CharEvolvePart.OnDestroy()
...
function CharEvolvePart.RefreshEvolveInfo()
  ...
  self._evolve_showSuccessData = {
    curLv = curLv,
    maxLv = maxLv,
    id = self._data:GetId(),
    changeProName = changeProName,
    addProOriginal = addProOriginal,
    newProValue = newProValue
  }
  local str = ""
  for i = 1, maxLv do
    if i <= curLv then
  ...
  if self._data:GetIsLeader() and count == 0 then
    ...
    if record then
      self._specialItem = {
        item = Item.Create(record.showItemID)
      }
  ...
  if self._specialItem.num then
    if self._specialItem.num >= self._specialNeedNum then
  ...
  if self._data:GetIsLeader() then
    ...
    if self._normalItem.num >= self._normalNeedNum then
  ...
  elseif self._evolve_selectTag == EvolveTag.Normal then
...
function CharEvolvePart.ClearSelectTag()
  self._evolve_selectTag = nil
end

function CharEvolvePart.OnEvolveBtnClick()
...
function CharEvolvePart.SelectSpecialItem()
  self._evolve_selectTag = EvolveTag.Special
...
function CharEvolvePart.UnSelectSpecialItem()
  self._evolve_selectTag = nil
  self._evolve_item1Select:SetActive(false)
  self._evolve_item1Back:SetActive(true)
  self._evolve_evolveBtn:SetInteractable(false)
...
function CharEvolvePart.SelectNormalItem()
...
function CharEvolvePart.UnSelectNormalItem()
  self._evolve_selectTag = nil
  self._evolve_item2Select:SetActive(false)
  self._evolve_item2Back:SetActive(true)
  self._evolve_evolveBtn:SetInteractable(false)
...
function CharEvolvePart.OnRoleInfoChange(nouse, notification)
  if notification.userInfo.name == "sevolution" then
    ...
    if dialog then
      local data = {
        role = self._data,
        effect = 1008,
        showSuccessData = self._evolve_showSuccessData,
        type = SHOWTALKTYPE.Evolve
      }
      dialog:SetData(data)
    end
  end
...
function CharEvolvePart.OnSpecialItemIconClick()
...
function CharEvolvePart.OnNormalItemIconClick()
...
function CharEvolvePart.OnEvolveSuccessEffectEnd(ins, handler, stateName, normalizedTime)
