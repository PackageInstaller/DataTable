...
function GuildBossRankDialog:Ctor(...)
...
function GuildBossRankDialog:OnCreate()
...
function GuildBossRankDialog:OnDestroy()
...
function GuildBossRankDialog:InitBossList()
  self._bossList = {}
  local guilddata = {}
  guilddata.bossid = 0
...
function GuildBossRankDialog:RefreshRankList(protocol)
...
function GuildBossRankDialog:RefreshWithCache()
  self._rankList = self._cacheRankData[self._selectBossId] or {}
...
function GuildBossRankDialog:RefreshPersonalRank()
...
function GuildBossRankDialog:OnTeamClicked()
...
function GuildBossRankDialog:DoReq()
...
function GuildBossRankDialog:StartTimer()
  self._timer = GameTimer.AddTask(5, -1, function(self)
    self._timer = nil
  end, self)
end

function GuildBossRankDialog:OnSelectBossCell(bossCellData)
...
function GuildBossRankDialog:NumberOfCell(helper)
  if helper == self._bossHelper then
    return #self._bossList
  end
end

function GuildBossRankDialog:CellAtIndex(helper, index)
  if helper == self._bossHelper then
    return "guildboss.guildbossrankbosscell"
  end
end

function GuildBossRankDialog:DataAtIndex(helper, index)
  if helper == self._bossHelper then
    return self._bossList[index]
  end
end

function GuildBossRankDialog:OnCurPosChange(helper, proportion)
  if helper == self._bossHelper then
...
function GuildBossRankDialog:OnBackBtnClicked()
  self:Destroy()
end

function GuildBossRankDialog:OnMenuBtnClicked()
