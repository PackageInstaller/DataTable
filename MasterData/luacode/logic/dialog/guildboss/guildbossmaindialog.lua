...
function GuildBossMainDialog:Ctor(...)
...
function GuildBossMainDialog:OnCreate()
  ...
  for i = 1, 4 do
    self._dot[i] = {
      num = self:GetChild("Progress/Dot" .. i .. "/Num"),
      addnum = self:GetChild("Progress/Dot" .. i .. "/NumAdd"),
      dotGrey = self:GetChild("Progress/Dot" .. i .. "/DotGrey"),
      dot = self:GetChild("Progress/Dot" .. i .. "/Dot")
    }
  end
...
function GuildBossMainDialog:OnBackBtnClicked()
  self:Destroy()
end

function GuildBossMainDialog:OnDestroy()
  self._frame:Destroy()
  LuaNotificationCenter.RemoveObserver(self)
end

function GuildBossMainDialog:OnMenuBtnClicked()
  DialogManager.CreateSingletonDialog("fastmenu.fastmenudialog")
end

function GuildBossMainDialog:Init()
  ...
  if timestamp * 1000 > self._data.endtime then
...
function GuildBossMainDialog:SetData()
end

function GuildBossMainDialog:NumberOfCell(helper)
...
function GuildBossMainDialog:CellAtIndex(helper, index)
  if helper == self._mainScrollViewHelper then
...
function GuildBossMainDialog:DataAtIndex(helper, index)
  if helper == self._mainScrollViewHelper then
...
function GuildBossMainDialog:OnRankBtnClicked()
  local csend = LuaNetManager.CreateProtocol("protocol.party.boss.crequestpartybossrank")
  csend.bossid = 0
  csend:Send()
end

function GuildBossMainDialog:OnTipsBtnClicked()
