...
local MessageCell = class("MessageCell", Dialog)
MessageCell.AssetBundleName = "ui/layouts.setting"
MessageCell.AssetName = "SettingMessage"

function MessageCell:Ctor(...)
  MessageCell.super.Ctor(self, ...)
end

function MessageCell:OnCreate()
  ...
  self._dispatchSwitchBtn = self:GetChild("DispatchBattle/SwitchBtn")
  self._dispatchSwitchBtn:Subscribe_PointerClickEvent(self.OnDispatchBtnClicked, self)
  self._dispatchSwitchBtn:SetSelected(not NekoData.BehaviorManager.BM_Game:GetIfNeedOfflineSweepNotification())
  self._dispatchSwitchBtn_Txt = self:GetChild("DispatchBattle/Txt")
  self._dispatchSwitchBtn_Txt:SetText(TextManager.GetText(CNotice:GetRecorder(4).titleID))
end

function MessageCell:OnDestroy()
end

function MessageCell:RefreshCell()
end
...
local function SchedulingEveryDayLoginWorker()
  ...
  if timeTable.hour < 4 then
    ...
    LogInfo("MessageCell", "CancelSchedulingWorkerByType('everydaylogin')")
    ...
    LogInfoFormat("MessageCell", "open the notification : scheduling everyday login worker messageid %s type %s title %s content %s time %s", spiritNoticeRecorder.id, "everydaylogin", TextManager.GetText(spiritNoticeRecorder.titleID), TextManager.GetText(spiritNoticeRecorder.detailsID), lastTime)
...
function MessageCell:OnEveryDayLoginBtnClicked()
...
local function SchedulingSpiritFullWorker()
  ...
  if 0 < lastTime then
    ...
    LogInfoFormat("MessageCell", "open the notification : scheduling spirit full worker messageid %s type %s title %s content %s time %s", spiritNoticeRecorder.id, "spirit", TextManager.GetText(spiritNoticeRecorder.titleID), TextManager.GetText(spiritNoticeRecorder.detailsID), lastTime)
...
function MessageCell:OnSpiritBtnClicked()
  ...
  if self._spiritBtn:IsSelected() then
    ...
    LogInfo("MessageCell", "CancelSchedulingWorkerByType('spirit')")
    SchedulingPushStaticFunctions.CancelSchedulingWorkerByType("spirit")
  end
  self._spiritBtn:SetSelected(value == 1)
  NekoData.DataManager.DM_Game:SetLocalSystemConfig({
...
local function SchedulingActivityExploreWorker(slots)
  ...
  for i, slot in ipairs(slots) do
    ...
    if canTimerWorker then
      LogInfoFormat("MessageCell", "open the notification : scheduling activity explore worker messageid %s type %s title %s content %s time %s", spiritNoticeRecorder.id, "slot" .. tostring(slot.slotId), TextManager.GetText(spiritNoticeRecorder.titleID), TextManager.GetText(spiritNoticeRecorder.detailsID), slot.finishLeftTime / 1000)
...
function MessageCell:OnActivityExploreBtnClicked()
  ...
  if self._activityExploreBtn:IsSelected() then
    ...
    for i, slot in ipairs(slots) do
      LogInfo("MessageCell", "CancelSchedulingWorkerByType('slot' .. tostring(slot.slotId))")
...
function MessageCell:OnDispatchBtnClicked()
  local key = NekoData.DataManager.DM_Game.LocalSystemConfigType.OfflineSweep
  local value = 1
  if self._dispatchSwitchBtn:IsSelected() then
    if not NekoData.BehaviorManager.BM_Game:GetIfHaveAskForNotificationPermission() then
      if not SchedulingPushStaticFunctions.AreNotificationsEnabled() then
        NekoData.BehaviorManager.BM_Message:AddSecondConfirmDialog(109, nil, function()
          local value = 0
          ...
          SchedulingPushStaticFunctions.OpenTheNotificationSettingUI()
          SchedulingEveryDayLoginWorker()
          self._dispatchSwitchBtn:SetSelected(value == 1)
          NekoData.DataManager.DM_Game:SetLocalSystemConfig({
            [key] = value
          })
        end, {}, nil, {})
    ...
    if value == 0 then
      NekoData.DataManager.DM_OfflineSweep:CheckClientPush()
    end
  else
    LogInfo("MessageCell", "CancelSchedulingWorkerByType('offlinesweep')")
    SchedulingPushStaticFunctions.CancelSchedulingWorkerByType("offlinesweep")
  end
  self._dispatchSwitchBtn:SetSelected(value == 1)
  NekoData.DataManager.DM_Game:SetLocalSystemConfig({
    [key] = value
