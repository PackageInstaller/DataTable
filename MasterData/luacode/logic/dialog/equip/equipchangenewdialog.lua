...
function EquipChangeNewDialog:Ctor(...)
...
function EquipChangeNewDialog:OnCreate()
  ...
  self._x_select, self._ox_select, self._y_select, self._oy_select = self._select_field:GetSize()
  self._rootX_select, self._rootOX_select, self._rooty_select, self._rootOY_select = self._select_panel:GetSize()
  self._pos_x_select, self._pos_offset_x_select, self._pos_y_select, self._pos_offset_y_select = self._select_panel:GetPosition()
  ...
  self._x_current, self._ox_current, self._y_current, self._oy_current = self._current_field:GetSize()
  self._rootX_current, self._rootOX_current, self._rooty_current, self._rootOY_current = self._current_panel:GetSize()
  self._pos_x_current, self._pos_offset_x_current, self._pos_y_current, self._pos_offset_y_current = self._current_panel:GetPosition()
...
function EquipChangeNewDialog:OnDestroy()
...
function EquipChangeNewDialog:CheckCachedFM()
...
function EquipChangeNewDialog:OnRefresh(notification)
...
function EquipChangeNewDialog:OnDecomposeEquips(notification)
...
function EquipChangeNewDialog:Init(roleKey, equipType, equipKey, isPreset)
...
function EquipChangeNewDialog:InitRightPanel()
...
function EquipChangeNewDialog:RefreshRightPanel()
  local key
  if self._currentEquipKey then
    self._currentEquip = NekoData.BehaviorManager.BM_BagInfo:GetEquipItem(self._currentEquipKey)
    if self._currentEquip then
  ...
  key = key or self._selectEquipKey
  self:SetSelectedEquip(key)
end

function EquipChangeNewDialog:SetSelectedEquip(selectEquipKey)
...
function EquipChangeNewDialog:SetPropContrast()
  ...
  setmetatable(currentRoleProp, {
    __index = function()
      return 0
    end
  })
  setmetatable(previewRoleProp, {
    __index = function()
      return 0
    end
  })
  ...
  if roleVocationId == 5 then
    ...
    local currentDamageIncrease, previewDamageIncrease
  ...
  if 0 < table.nums(gemsAttr) then
    ...
    if 0 < gemSuitID then
      ...
      table.insert(self._currentDownDataList, suitNameData)
      ...
      table.insert(self._currentDownDataList, suitEffectData)
    end
  end
...
function EquipChangeNewDialog:RefreshLeftPanel()
...
function EquipChangeNewDialog:GetNumByEquipId(id)
  local count = 0
...
function EquipChangeNewDialog:ScreeningAndSorting()
...
function EquipChangeNewDialog:OnFilterBtnClick()
...
function EquipChangeNewDialog:OnLevelBtnClick()
  if not self._filterDlg then
    LogError("[Dialog]: ", "Common filter dlg create error")
    return
  end
...
function EquipChangeNewDialog:OnSortBtnClick()
...
function EquipChangeNewDialog:SetSortCriteriaNameId(nameId)
...
function EquipChangeNewDialog:SetScreeningConditions(screeningConditions)
  self._screeningConditions = screeningConditions
  self:ScreeningAndSorting()
end

function EquipChangeNewDialog:OnLockBtnClick()
...
function EquipChangeNewDialog:OnCurrentLockBtnClick()
...
function EquipChangeNewDialog:OnMenuBtnClicked()
  DialogManager.CreateSingletonDialog("fastmenu.fastmenudialog")
end

function EquipChangeNewDialog:OnEquipLockStateChanged(notification)
  for i, v in ipairs(self._listByConditions) do
...
function EquipChangeNewDialog:OnEquipChecked(notification)
...
function EquipChangeNewDialog:OnChangeBtnClicked()
  local equip = NekoData.BehaviorManager.BM_BagInfo:GetEquipItem(self._selectEquipKey)
  local roleKey = equip:GetRoleKey()
...
function EquipChangeNewDialog:SendCchangeequipment()
  if self._isPreset then
  ...
  elseif self._receiveSChangeEquip then
...
function EquipChangeNewDialog:OnUnEquipBtnClicked()
  if self._isPreset then
    if self._currentEquipKey ~= 0 then
      local cchangeEquipment = LuaNetManager.CreateProtocol("protocol.item.cchangepresetequip")
      if cchangeEquipment then
...
function EquipChangeNewDialog:OnRoleUpdateProperties(notification)
...
function EquipChangeNewDialog:OnEquipChanged(notification)
  self._role = NekoData.BehaviorManager.BM_AllRoles:GetRole(self._role:GetRoleId())
  if self._equipType == EquipTypeEnum.WEAPON then
    self._currentEquipKey = self._role:GetWeapon()
  elseif self._equipType == EquipTypeEnum.ARMOR then
    self._currentEquipKey = self._role:GetArmor()
  elseif self._equipType == EquipTypeEnum.JEWELRY then
...
function EquipChangeNewDialog:OnEquipPresetChanged(notification)
  self._role = NekoData.BehaviorManager.BM_AllRoles:GetRole(self._role:GetRoleId())
  if self._equipType == EquipTypeEnum.WEAPON then
...
function EquipChangeNewDialog:OnResolveBtnClick()
  if DialogManager.GetDialog("equip.equipchangenewdialog") then
    DialogManager.CreateSingletonDialog("equip.equipresolvedialog")
  end
end

function EquipChangeNewDialog:GetEquipType()
  return self._equipType
end

function EquipChangeNewDialog:NumberOfCell(frame)
  if frame == self._frame then
    return #self._listByConditions
  elseif frame == self._select_frame then
    return #self._downDataList
  elseif frame == self._current_frame then
    return #self._currentDownDataList
  elseif frame == self._propContrastFrame then
    return #self._propContrastData
  end
end

function EquipChangeNewDialog:CellAtIndex(frame, index)
  if frame == self._frame then
  ...
  elseif frame == self._current_frame then
    if self._currentDownDataList[index].tag == "BaseProp" then
      return "equip.equipchangebasepropcell"
    elseif self._currentDownDataList[index].tag == "RandomProp" then
      return "equip.equiptiprandompropcell"
    elseif self._currentDownDataList[index].tag == "GemProp" then
      return "equip.equiplistgempropcell"
    elseif self._currentDownDataList[index].tag == "GemSuit" then
      return "equip.equiptipsgemsuitcell"
    elseif self._currentDownDataList[index].tag == "SuitInfo" then
      return "equip.equiptipssuitinfocell"
    elseif self._currentDownDataList[index].tag == "SuitSkill" then
      return "equip.equiptipssuitskillcell"
    elseif self._currentDownDataList[index].tag == "Describe" then
      return "equip.equiptipdescribecell"
    elseif self._currentDownDataList[index].tag == "Title1" then
      return "equip.equippoptitlecell1"
    elseif self._currentDownDataList[index].tag == "Title2" then
      return "equip.equippoptitlecell2"
    end
  elseif frame == self._propContrastFrame then
...
function EquipChangeNewDialog:DataAtIndex(frame, index)
  if frame == self._frame then
...
function EquipChangeNewDialog:ShouldLengthChange()
  return true
end

function EquipChangeNewDialog:GetRoleEquipSuitMapExceptCurrentType()
  return self._roleEquipSuitMap
end

function EquipChangeNewDialog:OnBackClick()
...
function EquipChangeNewDialog:OnForgeBtnClick()
  DialogManager.CreateSingletonDialog("equipforge.equipforgedialog"):Init(self._selectEquipKey)
end

function EquipChangeNewDialog:OnBackBtnClicked()
  self:Destroy()
end
