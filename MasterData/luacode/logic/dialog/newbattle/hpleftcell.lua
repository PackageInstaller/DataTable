...
function HPLeftCell:OnDestroy()
  self._buffFrame:Destroy()
end

function HPLeftCell:OnUpdate(battler, deltaTime)
  if battler then
    ...
    if 0 < hp and not self._showTag then
      self:Show(true)
    end
...
function HPLeftCell:SetShowTag(state)
  self._showTag = state
end

function HPLeftCell:SetData(battler, delegate)
    ...
    if isNeedShowBreakLv then
      ...
      for i = 1, #allIds do
        ...
        if not roleRecord then
          ...
          fatherBattler = fatherBattler or NekoData.BehaviorManager.BM_Battle:GetBattlerSummonByEntityId(fatherEntityId)
        ...
        if roleRecord and recorder.breakType == roleRecord.breakType then
          ...
          if rolerecord then
            ...
            role:SetUniqueEquipAttrs({
              specialWeaponLevel = battler:GetExclusiveLevel(),
            ...
            if levelNum > recorder.levelmin and levelNum <= levelmax then
...
function HPLeftCell:SetPosition(x, xoffect, y, yoffect)
  self:GetRootWindow():SetPosition(x, xoffect, y, yoffect)
end

function HPLeftCell:Show(show, notDestroy)
  if show then
    self._showTag = true
    self._needDelete = false
...
function HPLeftCell:OnAnimationEnd()
...
function HPLeftCell:RefreshBuffs(info)
  local buff, index
  ...
  if buff then
    ...
    if info.counttime == 0 then
      ...
      elseif buffConfigRecord.stacktimes == 0 then
        local OverRideType = {
          Cover = 0,
          RespectivelyShow = 1,
          Overlay = 2,
  ...
  elseif info.counttime ~= 0 then
    ...
    if buffConfigRecord then
      ...
      if imageRecord then
        ...
        self._buffFrame:InsertCellsAtIndex({
          #self._buffs
        })
        local totalLength = self._buffFrame:GetTotalLength()
        self._usingBuffPanel:SetHeight(0, totalLength)
...
function HPLeftCell:BeginShowSkillLoading(skillid, type, lasttime, skillattr)
...
function HPLeftCell:ClearSkillLoading(skillid)
...
function HPLeftCell:RefreshInvincibleState(state)
  ...
  if 0 >= self._invincibleStateCount then
    ...
    local record = CImagePathTable:GetRecorder(12123) or DataCommon.DefaultImageAsset
    self._greenProgressBar:SetSprite(record.assetBundle, record.assetName)
  else
    self._unBreakEffect:SetActive(self._hpCellShowState)
    local record = CImagePathTable:GetRecorder(12122) or DataCommon.DefaultImageAsset
    self._greenProgressBar:SetSprite(record.assetBundle, record.assetName)
  end
end

function HPLeftCell:RefreshShield(protocol)
...
function HPLeftCell:RefreshSecondHurtNumShowState(state)
...
function HPLeftCell:RefreshSecondHurtNum(num, totalNum)
  self._secondHurtNum:SetText(num)
  self._secondAllHurtNum:SetText(totalNum)
end

function HPLeftCell:RefreshName()
...
function HPLeftCell:GetCellIndex(buffId)
  for i, v in ipairs(self._buffs) do
    if v.id == buffId then
      return i
    end
...
function HPLeftCell:NumberOfCell(frame)
  if frame == self._buffFrame then
    return #self._buffs
  end
end

function HPLeftCell:CellAtIndex(frame, index)
  if frame == self._buffFrame then
    return "newbattle.buffcell"
  end
end

function HPLeftCell:DataAtIndex(frame, index)
  if frame == self._buffFrame then
