...
local ShopRecommendPageDetailCell1 = class("ShopRecommendPageDetailCell1", Dialog)
ShopRecommendPageDetailCell1.AssetBundleName = "ui/layouts.baseshop"
ShopRecommendPageDetailCell1.AssetName = "RecommendShopPage1"

function ShopRecommendPageDetailCell1:Ctor(...)
  ShopRecommendPageDetailCell1.super.Ctor(self, ...)
  self._itemsData1 = {}
  self._itemsData2 = {}
end

function ShopRecommendPageDetailCell1:OnCreate()
...
function ShopRecommendPageDetailCell1:OnDestroy()
  self._itemsFrame1:Destroy()
  self._itemsFrame2:Destroy()
end

function ShopRecommendPageDetailCell1:RefreshCell(data)
  if data then
    for _, value in pairs(data) do
      if value.goodType == 13 or value.goodType == 15 or value.goodType == 17 then
        self._data = value
        break
      end
    end
  ...
  if self._data then
    self._price:SetText(self._data.discountPrice)
    if self._data.discount == -1 or self._data.discount == 10 then
      ...
      self._price2:SetText(self._data.price)
...
function ShopRecommendPageDetailCell1:OnClick()
  self._delegate._delegate:SetJumpMonthCardFlag(1)
  NekoData.BehaviorManager.BM_Shop:TryOpenShopWithID(50)
end

function ShopRecommendPageDetailCell1:NumberOfCell(frame)
...
function ShopRecommendPageDetailCell1:CellAtIndex(frame, index)
  return "shop.recommendshoppageitemcell"
end

function ShopRecommendPageDetailCell1:DataAtIndex(frame, index)
  if frame == self._itemsFrame1 then
...
function ShopRecommendPageDetailCell1:PlayShowAnimation()
  self._animationPanel:PlayAnimation("RecommendShopPage1")
end

return ShopRecommendPageDetailCell1
