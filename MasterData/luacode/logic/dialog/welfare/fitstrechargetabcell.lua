...
function FitstRechargeTabCell:Ctor(...)
  FitstRechargeTabCell.super.Ctor(self, ...)
end

function FitstRechargeTabCell:OnCreate()
...
function FitstRechargeTabCell:OnDestroy()
  LuaNotificationCenter.RemoveObserver(self)
end

function FitstRechargeTabCell:RefreshTabCell()
...
function FitstRechargeTabCell:SetButtonState()
  local state = NekoData.BehaviorManager.BM_Welfare:GetFirstRechargeGiftState()
  self._frame1GetBtn:SetActive(state.statue == 1 and state.giftsState[1] and state.giftsState[1] == 0)
  self._frame1GetBtnGrey:SetActive(state.statue == 0 or not state.giftsState[1])
  self._frame1FinishBtn:SetActive(state.statue == 1 and state.giftsState[1] and state.giftsState[1] == 1)
  self._frame2GetBtn:SetActive(state.statue == 1 and state.giftsState[2] and state.giftsState[2] == 0)
  self._frame2GetBtnGrey:SetActive(state.statue == 0 or not state.giftsState[2])
  self._frame2FinishBtn:SetActive(state.statue == 1 and state.giftsState[2] and state.giftsState[2] == 1)
end

function FitstRechargeTabCell:ModelClick()
  local animator = AnimatorStaticFunctions.Get(self._model._uiObject)
  animator:SetTrigger("onClick")
end

function FitstRechargeTabCell:IButtonClick()
  DialogManager.CreateSingletonDialog("welfare.firtstrechargetip")
end

function FitstRechargeTabCell:ToShop()
...
function FitstRechargeTabCell:GetFirstRechargeGift(id)
  local csend = LuaNetManager.CreateProtocol("protocol.shop.cgetfirstrechargegift")
  csend.gainGiftID = id
  csend:Send()
end

function FitstRechargeTabCell:ShowMessage(id)
  if id == 1 then
...
function FitstRechargeTabCell:OnCellClick(index)
