...
function ReturnBattlePassTaskCell:Ctor(...)
  ReturnBattlePassTaskCell.super.Ctor(self, ...)
end

function ReturnBattlePassTaskCell:OnCreate()
  self._lv = self:GetChild("LVNum")
  self._lvUnlock = self:GetChild("LevelBack")
  self._lvLock = self:GetChild("LevelBackLock")
  self._normal = {
    item = self:GetChild("Normal/ItemCell"),
    get = self:GetChild("Normal/Get"),
    lock = self:GetChild("Normal/Lock"),
    selected = self:GetChild("Normal/Select"),
    reddot = self:GetChild("Normal/RedDot"),
    effect = self:GetChild("Normal/Effect")
  }
  self._normal.reddot:SetActive(false)
  self._advance = {
    item = self:GetChild("Advance/ItemCell"),
    char = self:GetChild("Advance/CharCell"),
    get = self:GetChild("Advance/Get"),
    lock = self:GetChild("Advance/Lock"),
    selected = self:GetChild("Advance/Select"),
    reddot = self:GetChild("Advance/RedDot"),
    effect = self:GetChild("Advance/Effect")
  }
...
function ReturnBattlePassTaskCell:OnDestroy()
end

function ReturnBattlePassTaskCell:SetItem(obj, data)
  if not obj then
    return
  end
...
function ReturnBattlePassTaskCell:SetCharInfo(obj, data, type)
  if not obj then
    return
  end
...
function ReturnBattlePassTaskCell:OnCellClicked(kind)
  if kind == 1 then
    local common = self._data.common
    if common.state ~= Collection.RECEIVE and common.itemId == DataCommon.ReturnBattlePass_RoleSelectItemId then
      DialogManager.CreateSingletonDialog("welfare.returnwelfare.roleopstiondialog"):SetData({
        CollectionType = 1,
        level = common.level or 1,
        option = 0
      }, common.state == Collection.UN_RECEIVE)
    elseif common.state == Collection.UN_RECEIVE then
  ...
  elseif kind == 2 or kind == 3 then
    ...
    if high.state ~= Collection.RECEIVE and high.itemId == DataCommon.ReturnBattlePass_RoleSelectItemId then
      DialogManager.CreateSingletonDialog("welfare.returnwelfare.roleopstiondialog"):SetData({
        CollectionType = 2,
        level = high.level or 1,
        option = 0
      }, high.state == Collection.UN_RECEIVE)
    elseif high.state == Collection.UN_RECEIVE then
...
function ReturnBattlePassTaskCell:RefreshCell(data)
  self._data = data
...
function ReturnBattlePassTaskCell:OnEvent(eventName, arg)
