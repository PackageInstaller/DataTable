...
local OffLinePVPEnemyLineCell = class("OffLinePVPEnemyLineCell", Dialog)
OffLinePVPEnemyLineCell.AssetBundleName = "ui/widgets"
OffLinePVPEnemyLineCell.AssetName = "CharacterCellLarge"
...
function OffLinePVPEnemyLineCell:Ctor(...)
  OffLinePVPEnemyLineCell.super.Ctor(self, ...)
  self._handler = 0
end

function OffLinePVPEnemyLineCell:OnCreate()
...
function OffLinePVPEnemyLineCell:OnDestroy()
end

function OffLinePVPEnemyLineCell:RefreshCell(data)
  if data.nodata then
  ...
  if data.roleId == 0 then
    ...
    if self._handler then
      self:GetRootWindow():Unsubscribe_PointerClickEvent(self._handler)
      self._handler = nil
    end
    return
  end
...
function OffLinePVPEnemyLineCell:OnCellClicked()
  ...
  for index, v in ipairs(roleList) do
    ...
    if v.id == self._cellData.roleId then
      curIdx = idx
    end
  end
  local dlg = DialogManager.CreateSingletonDialog("mainline.bossrush.checkotherroleinfodialog")
  if dlg then
    dlg:Init({index = curIdx, roleList = roleList}, CheckOtherRoleInfoDialog.ShowType.PVP)
  end
end

return OffLinePVPEnemyLineCell
