...
function NewMainlineFloorWorldRewardDialog:Ctor(...)
  NewMainlineFloorWorldRewardDialog.super.Ctor(self, ...)
  self._groupName = "SecondConfirm"
end

function NewMainlineFloorWorldRewardDialog:OnCreate()
...
function NewMainlineFloorWorldRewardDialog:OnDestroy()
  LuaNotificationCenter.RemoveObserver(self)
  self._frame:Destroy()
  DialogManager.DestroySingletonDialog("bag.itemtipsdialog")
end

function NewMainlineFloorWorldRewardDialog:SetData(data)
  self._data = data
  ...
  for i, id in ipairs(record.worldrewardItems) do
    ...
    table.insert(self._item, {
      item = item,
      num = record.worldrewardItemNums[i]
    })
  end
...
function NewMainlineFloorWorldRewardDialog:OnGetBtnClicked()
  if self._data.clearZones == self._data.totalZones then
...
function NewMainlineFloorWorldRewardDialog:NumberOfCell(frame)
  return #self._item
end

function NewMainlineFloorWorldRewardDialog:CellAtIndex(frame)
  return "mainline.mainline.newmainlinefloorworldrewardcell"
end

function NewMainlineFloorWorldRewardDialog:DataAtIndex(frame, index)
  return self._item[index]
end

function NewMainlineFloorWorldRewardDialog:OnReceive()
  self._haveGottenTxt:SetActive(true)
  self._getBtn:SetActive(false)
  self:Destroy()
end

function NewMainlineFloorWorldRewardDialog:OnCloseBtnClicked(args)
  if not UIManager.RectangleContainsScreenPoint(self._backPanel._uiObject, args.position.x, args.position.y) then
    self:OnBackBtnClicked()
  end
end

function NewMainlineFloorWorldRewardDialog:OnBackBtnClicked()
  self:Destroy()
end
