...
function MainLineFloorDialog:DestroyNodeDialog()
...
function MainLineFloorDialog:BuildNodeData()
...
local function SortItemByPinji(items, get, first, normal, chance)
  ...
  table.sort(item, function(a, b)
    if a.item:GetPinJiID() == b.item:GetPinJiID() then
...
function MainLineFloorDialog:FireEvent(id)
  for _, node in ipairs(self._nodeData) do
...
function MainLineFloorDialog:StartEffectFireEvent()
  for _, node in ipairs(self._nodeData) do
...
function MainLineFloorDialog:OnCellClicked(select)
  ...
  for _, v in ipairs(self._floorData) do
    if select == v.id then
      ...
      self._recommendedRolesBtn:SetActive(0 < #recommendIds or 0 < #monsterIds)
      self._recommendedRolesBtnBack:SetActive(0 < #recommendIds or 0 < #monsterIds)
...
function MainLineFloorDialog:OnGoBtnClicked()
  ...
  if self._floorDetail.unlockLv > NekoData.BehaviorManager.BM_Game:GetMyRoleInfo().userlevel then
    NekoData.BehaviorManager.BM_Message:SendMessageById(100232, {
      self._floorDetail.unlockLv
    })
  elseif self._floorDetail.spirit > self._spirit then
...
function MainLineFloorDialog:OnActivityUpdate()
...
function MainLineFloorDialog:SetActivityPower(floor, spirit)
  if NekoData.BehaviorManager.BM_Activity:HasActivity(1) then
...
function MainLineFloorDialog:OnBackBtnClicked()
...
function MainLineFloorDialog:OnExitStageAnimationStateExit(handler, stateName, normalizedTime)
  if stateName == "DungeonSelectHide" then
    self:Destroy()
  end
end

function MainLineFloorDialog:OnSwitchStageAnimationStateExit(handler, stateName, normalizedTime)
  if stateName == "DungeonSelectHide" then
    local dialog = DialogManager.GetDialog("mainline.mainline.mainlineworlddialog")
    if dialog then
...
function MainLineFloorDialog:OnMenuBtnClicked()
...
function MainLineFloorDialog:OnWorldExploreBoxClicked()
...
function MainLineFloorDialog:OnDetailBoxClicked()
...
function MainLineFloorDialog:SetLeftWorldInfo()
...
function MainLineFloorDialog:OnLastWorldBtnClicked()
  self._swichID = self._lastWorldId
  self._animator:Play("DungeonSelectHide")
end

function MainLineFloorDialog:OnNextWorldBtnClicked()
  self._swichID = self._nextWorldId
  self._animator:Play("DungeonSelectHide")
end

function MainLineFloorDialog:OnBeginDrag(args)
  self._dragSpeed = 0
  self._dragDelta = 0
  self._slideTime = 0.5
...
function MainLineFloorDialog:OnDrag(args)
...
function MainLineFloorDialog:OnEndDrag(args)
  self._dragDelta = 0
...
function MainLineFloorDialog:UpdateView(deltaTime)
...
function MainLineFloorDialog:OnLateUpdate(notification)
...
function MainLineFloorDialog:OnReceiveDungeonPointAward(notification)
  for _, v in ipairs(self._floorData) do
...
function MainLineFloorDialog:OnReceiveDungeonWorldAward(notification)
...
function MainLineFloorDialog:OnAutoGoBtnClicked()
...
function MainLineFloorDialog:OnWorldBtnDrag(args)
  if args.delta.x ~= 0 then
    self._direction = args.delta.x
  end
end

function MainLineFloorDialog:OnWorldBtnEndDrag(args)
  if self._direction then
    if self._direction > 0 then
      if self._lastWorldId then
        self:OnLastWorldBtnClicked()
      end
    elseif self._direction < 0 and self._nextWorldId then
...
function MainLineFloorDialog:OnAutoBtnClickAreaClicked()
...
function MainLineFloorDialog:OnRecommendedRolesBtnClicked()
...
function MainLineFloorDialog:HaveSpiritItemSoonExpire()
...
function MainLineFloorDialog:SetRedDot()
  self._vitRedDot:SetActive(self:HaveSpiritItemSoonExpire())
end

function MainLineFloorDialog:HandleSpriteEvidence()
