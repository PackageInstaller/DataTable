...
function BranchLineCell:Ctor(...)
  BranchLineCell.super.Ctor(self, ...)
  self._data = {}
end

function BranchLineCell:OnCreate()
...
function BranchLineCell:OnDestroy()
  LuaNotificationCenter.RemoveObserver(self)
end

function BranchLineCell:RefreshCell(data)
  self._data = data
  ...
  self._done:SetActive(data.passed and not data.reset)
...
function BranchLineCell:OnCellClicked(index)
  ...
  if self._data.unlock then
    ...
    if dialog then
      dialog:Init({
        chapterId = self._data.id
      })
    end
  end
end

function BranchLineCell:UnLockBranchLineWorld()
  if not self._data.unlock then
    if self._unlockByPerDungeon then
      ...
      local parameter = {
        itemName,
        TextManager.GetText(recorder.chapternumnametxt),
        TextManager.GetText(recorder.sidestorytitletxt)
      }
...
function BranchLineCell:UnLockYesFunction()
