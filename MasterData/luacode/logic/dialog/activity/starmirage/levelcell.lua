...
LevelCell.AssetBundleName = "ui/layouts.activitystar"
LevelCell.AssetName = "ActivityStarCell"
...
function LevelCell:SetData(delegate, data)
  ...
  if self._bm:IsPlayGuideClickEffect() and self._levelId == 10000 then
    if not self._handler then
      self._handler = NekoData.BehaviorManager.BM_Game:CreateGuideClickEffectDialog(self._rootWindow, 1148)
    end
...
function LevelCell:PlayAnimation()
  ...
  self._rootWindow:PlayAnimation("ActivityStarCell" .. nodeStr)
end
