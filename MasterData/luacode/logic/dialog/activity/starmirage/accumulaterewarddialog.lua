...
AccumulateRewardDialog.AssetBundleName = "ui/layouts.activitystar"
AccumulateRewardDialog.AssetName = "ActivityStarAccount"

function AccumulateRewardDialog:Ctor(...)
  AccumulateRewardDialog.super.Ctor(self, ...)
  self._groupName = "SecondConfirm"
  self._mainRewardItemList = {}
  self._recorder = CEventCollectBonus:GetRecorder(DataCommon.AccumulateRewards.StarMirage)
...
function AccumulateRewardDialog:OnCreate()
  ...
  self:GetChild("BackImage/Tips"):SetText(TextManager.GetText(1900580))
  ...
  self._timeTxt:SetText(string.gsub(TextManager.GetText(1900581), "%$parameter1%$", self._recorder.endtime))
  self:Init()
...
local function RefreshProgressShow(self)
  ...
  str = string.gsub(str, "%$parameter1%$", NekoData.BehaviorManager.BM_Activity:GetManager(DataCommon.StarMirageManagerID):GetAccumulatePointsById(record.collecttype))
...
function AccumulateRewardDialog:OnSRefreshActivityRewards(notification)
  if notification.userInfo.activityID == DataCommon.Activities.StarMirage then
    ...
    self._dataList = NekoData.BehaviorManager.BM_Activity:GetManager(DataCommon.StarMirageManagerID):GetAccumulateRewardList()
...
function AccumulateRewardDialog:CellAtIndex(frame, index)
  return "activity.starmirage.accumulaterewardcell"
end
