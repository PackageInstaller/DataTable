...
function SelectLevelMainDialog:Ctor(...)
...
function SelectLevelMainDialog:OnCreate()
...
function SelectLevelMainDialog:OnDestroy()
...
function SelectLevelMainDialog:Init()
...
function SelectLevelMainDialog:RefreshCurrencyPanel()
...
function SelectLevelMainDialog:RefreshData()
...
function SelectLevelMainDialog:OnRefreshActivity(notification)
...
function SelectLevelMainDialog:OnRefreshLevelList(notification)
  if notification.userInfo.activityId == DataCommon.Activities.StarMirage then
    self:Init()
    RefreshSwitchTypeBtnShow(self)
...
function SelectLevelMainDialog:OnEnterMianCity()
  self:RefreshData()
  LuaAudioManager.StopBGM(136)
  if self._tabType == TabTypeEnum.Easy then
...
function SelectLevelMainDialog:SetTabType(type)
  if self._tabType ~= type then
    ...
    if type == TabTypeEnum.Easy then
      ...
      LuaAudioManager.SetCategoryVolume(CategoryMap[1253], 0)
    elseif type == TabTypeEnum.Hard then
      LuaAudioManager.SetCategoryVolume(CategoryMap[1252], 0)
...
function SelectLevelMainDialog:OnSwitchTypeBtnClicked(type)
...
function SelectLevelMainDialog:NumberOfCell(frame, index)
  if frame == self._currencyFrame then
    return #self._currencyList
  end
end

function SelectLevelMainDialog:CellAtIndex(frame, index)
  if frame == self._currencyFrame then
  ...
  elseif frame == self._tabFrame then
    ...
    elseif index == TabTypeEnum.Hard then
      return "activity.starmirage.hardtabcell"
    end
  end
end

function SelectLevelMainDialog:DataAtIndex(frame, index)
  if frame == self._currencyFrame then
    return self._currencyList[index]
  end
end

function SelectLevelMainDialog:OnRootWindowClicked(args)
  local tabCell = self._tabFrame:GetCellAtIndex(self._tabType)
...
function SelectLevelMainDialog:OnBackBtnClicked()
  self:Destroy()
end

function SelectLevelMainDialog:OnMenuBtnClicked()
