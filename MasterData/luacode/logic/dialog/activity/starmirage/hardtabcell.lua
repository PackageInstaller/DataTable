...
function HardTabCell:OnDestroy()
...
function HardTabCell:OnRefreshSpirit(notification)
...
function HardTabCell:OnNPCChatEnd(notification)
...
function HardTabCell:RefreshActivityRewards()
...
function HardTabCell:RefreshByProtocol(data)
...
function HardTabCell:RefreshTabCell(data, notSelectLevel, refresh)
...
function HardTabCell:SelectLevelById(levelId)
  if levelId then
...
function HardTabCell:OnUpdate(notification)
...
function HardTabCell:OnAutoBtnClickAreaClicked()
...
function HardTabCell:OnGoBtnClicked()
  local temp = self._levelNodeData[self._selectLevelId]
  local record = temp.record
  local data = temp.data
  if record.spirit > self._spirit then
...
function HardTabCell:ResourceSelectDetailShow()
...
function HardTabCell:OnStateExit(handler, stateName, normalizedTime)
...
function HardTabCell:NumberOfCell(frame, index)
  if frame == self._detail_entryFrame then
    return #self._entryList
  elseif frame == self._roleAddPanel_frame then
...
function HardTabCell:CellAtIndex(frame, index)
  if frame == self._detail_entryFrame then
...
function HardTabCell:DataAtIndex(frame, index)
  if frame == self._detail_entryFrame then
