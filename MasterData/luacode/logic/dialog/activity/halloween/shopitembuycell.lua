...
ShopItemBuyCell.AssetBundleName = "ui/layouts.activityhalloween"
ShopItemBuyCell.AssetName = "HalloweenBuyCell"
...
function ShopItemBuyCell:OnCellClicked()
  ...
  elseif self._item:GetItemTypeId() == 91 then
    local cfg = CSkinItem:GetRecorder(self._item:GetID())
    local dlg = DialogManager.CreateSingletonDialog("activity.halloween.fashionbuydialog")
    dlg:SetData({
      itemId = cfg.Skinid
    })
