...
local ShopIdMap = {
  [TypeEnum.Type.Easy] = TypeEnum.Shop.Easy,
  [TypeEnum.Type.Hard] = TypeEnum.Shop.Hard
}
...
function LevelDialog:RefreshFunctionBtnShow()
  ...
  self._levelDetailPanel_dispatchBtn:SetInteractable(levelInfo and levelInfo.firstGet)
...
function LevelDialog:OnDispatchBtnClicked()
  ...
  local levelInfo = self._bm:GetLevelInfoById(self._selectLevelId)
  if levelInfo.costSpirit > NekoData.BehaviorManager.BM_Currency:GetCurrencyNum(DataCommon.SpiritID) then
...
function LevelDialog:OnDispatchBtnClickAreaClicked()
  NekoData.BehaviorManager.BM_Message:SendMessageById(100473)
end

function LevelDialog:OnEntryIBtnClicked()
...
function LevelDialog:OnAccumulateRewardBtnClicked()
...
function LevelDialog:NumberOfCell(frame, index)
  ...
  elseif frame == self._levelDetailPanel_entryPanel_cellFrame then
