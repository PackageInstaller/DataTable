...
local ShopType = {
  [ShopTypeMap.Easy] = DataCommon.StarMirage1[DataCommon.Activities.StarMirageCopy].Type.Easy,
  [ShopTypeMap.Hard] = DataCommon.StarMirage1[DataCommon.Activities.StarMirageCopy].Type.Hard
}
local ShopMainDialog = class("ShopMainDialog", Dialog)
ShopMainDialog.AssetBundleName = "ui/layouts.activitystar2"
ShopMainDialog.AssetName = "ActivityStar2Shop"
...
function ShopMainDialog:Ctor(...)
  ...
  self._init = false
  self._coinType = {}
  local activityID = DataCommon.Activities.StarMirageCopy
  ...
  for i = 1, #allIds do
    ...
    if record.eventid == activityID then
      self._coinType[record.diffculty] = record.currency
    end
  end
end
...
function ShopMainDialog:OnCreate()
  ...
  self._timeTile = self:GetChild("UI/ShopArea/Refresh/Txt")
  self._timeTile:SetText(NekoData.BehaviorManager.BM_Message:GetString(2129))
...
function ShopMainDialog:RefreshCurrencyPanel()
  ...
  table.insert(self._currencyList, self._coinType[ShopType[self._selectShopId]])
...
local function RefreshByTab(self)
  ...
  if self._selectShopId == ShopTypeMap.Easy then
    self._switchBtns[ShopTypeMap.Easy]:SetActive(false)
    self._switchBtns[ShopTypeMap.Hard]:SetActive(true)
  elseif self._selectShopId == ShopTypeMap.Hard then
    self._switchBtns[ShopTypeMap.Easy]:SetActive(true)
    self._switchBtns[ShopTypeMap.Hard]:SetActive(false)
  end
  if self._selectShopId == ShopTypeMap.Easy then
  ...
  self._timeTxt:SetText(NekoData.BehaviorManager.BM_Message:GetString(2119, record.CloseTime))
...
function ShopMainDialog:OnBuyShopGood(notification)
  ...
  for k, v in pairs(ShopTypeMap) do
    if v == shopId then
...
function ShopMainDialog:CellAtIndex(frame, index)
  ...
  elseif frame == self._tabFrame then
    return "activity.starmirage1.shoptabcell"
  end
end
