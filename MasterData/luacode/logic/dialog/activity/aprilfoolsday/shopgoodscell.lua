...
ShopGoodsCell.AssetBundleName = "ui/layouts.activityaprilfools"
ShopGoodsCell.AssetName = "ActivityAprilFoolsShopCell"
local SkinTypeId = 91
local CellState = {
  G = 1,
  R = 2,
...
function ShopGoodsCell:OnCellClicked()
  if self._bm:IsShopOpen() then
    ...
    if self._data.isrmt == 0 then
      DialogManager.CreateSingletonDialog("activity.aprilfoolsday.shopbuydialogfree"):SetData(self._data.iteminfo, self._shopId)
    else
      local isSkin = false
      for _, id in ipairs(self._data.iteminfo.itemId) do
        local item = Item.Create(id)
      ...
      if isSkin then
        DialogManager.CreateSingletonDialog("activity.aprilfoolsday.shopbuydialogskin"):SetData(self._data.iteminfo, self._shopId)
      else
        DialogManager.CreateSingletonDialog("activity.aprilfoolsday.shopbuydialog6"):SetData(self._data.iteminfo, self._shopId)
      end
    end
  else
