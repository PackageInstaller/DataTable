...
function Sheet3:SetData(shopID)
...
function Sheet3:RefreshShopInfo()
  ...
  local freeGoodsData, notFreeGoodsData, skinGoodsData
  if PrintTable then
  ...
  for _, goods in pairs(goodsData) do
    if goods.discountPrice == 0 then
      freeGoodsData = goods
    elseif type(goods.itemId) == "table" then
      local isSkin = false
...
function Sheet3:OnTipsBtnClicked()
