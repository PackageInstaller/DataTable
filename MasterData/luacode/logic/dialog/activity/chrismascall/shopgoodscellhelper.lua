...
function ShopGoodsCellHelper:Ctor(rootElement, indexNum)
...
function ShopGoodsCellHelper:RefreshCell(data, shopID)
  self._data = data
  self._shopID = shopID
...
function ShopGoodsCellHelper:OnBuyClicked()
  if self._data.goodRemain ~= 0 then
    if self._indexNum == 3 then
      DialogManager.CreateSingletonDialog("activity.chrismascall.skinbuydialog"):SetData(self._data, self._shopID)
    else
      DialogManager.CreateSingletonDialog("activity.chrismascall.itembuydialog"):SetData(self._data, self._shopID)
    end
  end
end
