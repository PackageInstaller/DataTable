...
local CValentineShopCurrency = BeanManager.GetTableByName("recharge.cvalentineshopcurrency")
local CValentineShopChocolate = BeanManager.GetTableByName("recharge.cvalentineshopchocolate")
local ShopTypeMap = DataCommon.Lover.Shop
local ShopItemCell = class("ShopItemCell", Dialog)
ShopItemCell.AssetBundleName = "ui/layouts.activityvalentines"
ShopItemCell.AssetName = "ActivityValentinesShopCell"

local function isLock(self)
  local recoder
  local curNum = 0
  self._nameStr = ""
  if self._data.moneyType == DataCommon.LoveTownCurrency then
    local currencyItem = Item.Create(DataCommon.LoveTownCurrency)
    self._nameStr = currencyItem:GetName()
  elseif self._data.moneyType == DataCommon.LoveChocolate then
    local currencyItem = Item.Create(DataCommon.LoveChocolate)
    self._nameStr = currencyItem:GetName()
  end
  return false
end
...
function ShopItemCell:OnCellClicked()
  ...
  if self._delegate._delegate._selectShopId == ShopTypeMap.Chocolate and NekoData.BehaviorManager.BM_Shop:GetShopItemCanNotUseWithShopType(ShopTypeMap.Chocolate) then
    return
  end
