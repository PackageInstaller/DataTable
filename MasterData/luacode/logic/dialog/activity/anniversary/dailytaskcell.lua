...
function DailyTaskCell:RefreshCell(data)
  local levelRecord = CAnniversarylevel:GetRecorder(data.record.levelid)
  local image = CImagePathTable:GetRecorder(levelRecord.levelpicselect) or DataCommon.DefaultImageAsset
  self._levelIcon:SetSprite(image.assetBundle, image.assetName)
  self._detailTxt:SetText(NekoData.BehaviorManager.BM_Message:GetString(2009, {
    TextManager.GetText(levelRecord.levelname)
  }))
  ...
  if data.taskStatus == TaskStatus.COMMITED or data.taskStatus == TaskStatus.FINISHED then
    ...
    self._progress:SetText(NekoData.BehaviorManager.BM_Message:GetString(2011, {
      data.record.battlecount,
      data.record.battlecount
    }))
  else
    self._progress:SetActive(false)
    self._progress_notFull:SetActive(true)
    self._progress_notFull:SetText(NekoData.BehaviorManager.BM_Message:GetString(2010, {
      data.taskCurProgress,
      data.record.battlecount
    }))
  end
end

function DailyTaskCell:OnGetBtnClick()
...
function DailyTaskCell:OnItemCellClicked()
