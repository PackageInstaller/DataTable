local ChristmasPart2CommonBattleChangeTeamRoleCell = class("ChristmasPart2CommonBattleChangeTeamRoleCell", Dialog)
ChristmasPart2CommonBattleChangeTeamRoleCell.AssetBundleName = "ui/layouts.activitychristmas"
ChristmasPart2CommonBattleChangeTeamRoleCell.AssetName = "ActivityChristmasChangeNewCell"

function ChristmasPart2CommonBattleChangeTeamRoleCell:Ctor(...)
  ChristmasPart2CommonBattleChangeTeamRoleCell.super.Ctor(self, ...)
  self._sameTag = false
end

function ChristmasPart2CommonBattleChangeTeamRoleCell:OnCreate()
  ...
  self._current = self:GetChild("LineBack/Num1")
  self._totle = self:GetChild("LineBack/Num2")
  self._progress = self:GetChild("LineBack/Line")
  self._isJobLimit = false
  self:GetRootWindow():Subscribe_PointerClickEvent(self.OnCellClicked, self)
end

function ChristmasPart2CommonBattleChangeTeamRoleCell:OnDestroy()
  LuaNotificationCenter.RemoveObserver(self)
end

function ChristmasPart2CommonBattleChangeTeamRoleCell:RefreshCell(data)
  if type(data) == "table" then
    ...
    if self._index then
      if self._delegate._curRoleKey == self._role:GetRoleId() then
        ...
        self._cur:SetActive(false)
        if self._role:IsInAlternativeTeams(self._delegate._curTeam.id) then
          self._txt:SetActive(false)
    ...
    if not value then
      ...
      self._starMirageUpPanel_txt:SetText(TextManager.GetText(1900578))
    end
  end
  ...
  self._totle:SetText(totle)
  local currentStatus = NekoData.BehaviorManager.BM_Activity:GetManager(DataCommon.ChristmasActivityManagerID):GetRolesStatusByRoleID(self._role:GetRoleId())
  if currentStatus < 1 then
    self._hasRoleTired = true
  end
  self._current:SetText(currentStatus)
  self._progress:SetFillAmount(currentStatus / tonumber(totle))
end

function ChristmasPart2CommonBattleChangeTeamRoleCell:IsSameWithTeamRoles()
...
function ChristmasPart2CommonBattleChangeTeamRoleCell:OnCellClicked()
...
function ChristmasPart2CommonBattleChangeTeamRoleCell:OnEvent(eventName, arg)
...
return ChristmasPart2CommonBattleChangeTeamRoleCell
