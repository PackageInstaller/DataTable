...
local function RefreshPersonalRank(self, data)
  if data then
    ...
    if data.rank ~= BossChallenge.NOT_Challenge then
      ...
      self._challengePanel_difficultyTxt:SetText(NekoData.BehaviorManager.BM_Game:GetBossRushDifficultyColorStr(data.bossPanelData.times, "Christmas"))
    end
  else
...
function RankTabCell:OnTeamClicked()
  DialogManager.CreateSingletonDialog("activity.christmas.rankteaminfodialog"):InitDataInfo(self._data.personalRankData, self._delegate._tabType)
end
...
function RankTabCell:CellAtIndex(frame, index)
  return "activity.christmas.rankcell"
end
