...
function ItemBuyDialog:OnBuyBtnClicked()
  if self._data.discount == -1 or self._data.discount == 10 and self._data.discountPrice == 0 then
    self._bm:KillFreeRedDot()
    LuaNotificationCenter.PostNotification(Common.n_RefreshActivityShopRedDot, nil, {
      activityId = DataCommon.Activities.DoubleEleven
    })
  end
...
function ItemBuyDialog:CellAtIndex(frame, index)
  return "activity.swimsuitechoes.shopitembuycell"
end
