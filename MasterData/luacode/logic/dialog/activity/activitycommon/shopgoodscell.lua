...
ShopGoodsCell.AssetBundleName = "ui/layouts.activitycommon"
ShopGoodsCell.AssetName = "ActivityCommon2ShopCell"
local SkinTypeId = 91
...
function ShopGoodsCell:OnCreate()
  ...
  self._priceTextDelete = self:GetChild("CellBack/Price/TextDelete")
  self._priceTextDeleteLine = self:GetChild("CellBack/Price/TextDelete/Line")
...
function ShopGoodsCell:RefreshCell(data)
  ...
  self._priceNum:SetText(tostring(SdkManager.GetPrice(self._data.iteminfo.discountPrice)))
  if self._data.iteminfo.discountPrice == self._data.iteminfo.price then
    ...
    self._priceTextDelete:SetText(tostring(SdkManager.GetPrice(self._data.iteminfo.price)))
  end
  if self._data.iteminfo.discountPrice == 0 then
    self._priceNum:SetText(TextManager.GetText(CStringRes:GetRecorder(1848).msgTextID))
    self._priceImg:SetActive(false)
    ...
    if self._data.isrmt == 0 then
      ...
      if isSkin and skinIDCfg then
        ...
        if NekoData.BehaviorManager.BM_SkinList:IsUnLockSkin(skinItem:GetSkinId()) then
          NekoData.BehaviorManager.BM_Message:SendMessageById(100488)
          return
        end
...
function ShopGoodsCell:OnFashionBtnClicked()
  DialogManager.CreateSingletonDialog("activity.activitycommon.skinlistdialog"):SetData(self._skinTimeList)
end
