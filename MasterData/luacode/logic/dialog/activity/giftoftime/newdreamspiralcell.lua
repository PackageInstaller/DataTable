...
local NewDreamSpiralCell = class("NewDreamSpiralCell", Dialog)
NewDreamSpiralCell.AssetBundleName = "ui/layouts.battlepassnew"
NewDreamSpiralCell.AssetName = "BattlePassCellNew"

function NewDreamSpiralCell:Ctor(...)
  NewDreamSpiralCell.super.Ctor(self, ...)
end

function NewDreamSpiralCell:OnCreate()
  ...
  self._normal.selected:SetActive(false)
  self._advance.selected:SetActive(false)
  ...
  self._process = self:GetChild("LineBack/Line")
  self._exclusiveEquipFrame = self:GetChild("Panel/Advance/ItemCell/_BackGround/ExclusiveEquipFrame")
  self._charPhoto = self:GetChild("Panel/Advance/ItemCell/ExclusiveEquip/CharPhoto")
  self._charBack = self:GetChild("Panel/Advance/ItemCell/ExclusiveEquip/CharBack")
end

function NewDreamSpiralCell:OnDestroy()
end

function NewDreamSpiralCell:SetItem(obj, data)
  if not obj then
    return
  end
...
function NewDreamSpiralCell:SetCharInfo(obj, data, type)
  if not obj then
    return
  end
...
local function GetReward(rewardType, lv)
  local cmd = LuaNetManager.CreateProtocol("protocol.activity.clittlebattlepassreceivecollection")
  cmd.CollectionType = rewardType
  cmd.level = lv or 1
  cmd:Send()
end

function NewDreamSpiralCell:OnCellClicked(kind)
  if kind == 1 then
...
function NewDreamSpiralCell:RefreshCell(data)
  self._data = data
  ...
  if item:IsUniqueEquip() then
    ...
    self._exclusiveEquipFrame:SetActive(true)
    local imageRecord = item:GetPinJiImage()
    self._exclusiveEquipFrame:SetSprite(imageRecord.assetBundle, imageRecord.assetName)
    local uniqueEquipItem = UniqueEquip.Create(high.itemId)
    local roleId = uniqueEquipItem:GetRoleId()
    local role = Role.Create(roleId, nil, true)
    ...
    self._exclusiveEquipFrame:SetActive(false)
  end
  if itemType == 27 then
  ...
  local isNextLevel = self._delegate._info.dreamLevel + 1 == data.level
  if isNextLevel then
    local chipInfo = self._delegate._info.chipInfo
    if chipInfo then
      self._process:SetFillAmount(chipInfo.has / chipInfo.max)
    end
  elseif common.state == Collection.LOCK then
    self._process:SetFillAmount(0)
  else
    self._process:SetFillAmount(1)
  end
end

function NewDreamSpiralCell:OnEvent(eventName, arg)
  if eventName == "selectNormal" then
  elseif eventName == "selectHigh" then
  end
end

return NewDreamSpiralCell
