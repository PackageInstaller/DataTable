...
DreamSpiralUnlockDialog.AssetBundleName = "ui/layouts.battlepassnew"
DreamSpiralUnlockDialog.AssetName = "BattlePassNewBuy"

function DreamSpiralUnlockDialog:Ctor(...)
...
function DreamSpiralUnlockDialog:OnCreate()
  ...
  self.tipTxt = self:GetChild("Panel/Txt")
  self._buyBtn_Text = self:GetChild("Panel/BuyBtn/Text")
  self.tipTxt:SetText(NekoData.BehaviorManager.BM_Message:GetString(1946))
  self._buyBtn_Text:SetText(NekoData.BehaviorManager.BM_Message:GetString(1971))
  ...
  LuaNotificationCenter.AddObserver(self, self.OnBackBtnClicked, Common.n_NewDreamSpiralRefresh, nil)
end

function DreamSpiralUnlockDialog:OnBackBtnClicked()
  self:Destroy()
end

function DreamSpiralUnlockDialog:OnDestroy()
...
function DreamSpiralUnlockDialog:OnBuyBtnClicked()
  local hasDiamond = NekoData.BehaviorManager.BM_Currency:GetDiamond()
  if hasDiamond >= self._info.chargeInfo.price then
    local cmd = LuaNetManager.CreateProtocol("protocol.activity.clittlebattlepassunlockhigh")
    if cmd then
      cmd:Send()
    end
  else
    NekoData.BehaviorManager.BM_Message:AddSecondConfirmDialog(15, nil, function()
      ...
      DialogManager.DestroySingletonDialog("activity.giftoftime.newdreamspiralunlockdialog")
    end, {}, nil, {})
  end
end

function DreamSpiralUnlockDialog:NumberOfCell(frame)
  return #self._items
end

function DreamSpiralUnlockDialog:CellAtIndex(frame, index)
  return "shop.dreamspiralitemcell"
end

function DreamSpiralUnlockDialog:DataAtIndex(frame, index)
