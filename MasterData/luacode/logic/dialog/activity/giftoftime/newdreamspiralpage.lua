...
function NewDreamSpiralPage:Ctor(...)
...
function NewDreamSpiralPage:OnCreate()
  ...
  self._advancelAward = {
    quality = self:GetChild("Advance/Back"),
    image = self:GetChild("Advance/Image"),
    name = self:GetChild("Advance/Text")
  }
...
function NewDreamSpiralPage:OnDestroy()
...
function NewDreamSpiralPage:UpdateLeftTime()
...
function NewDreamSpiralPage:Refresh()
  ...
  if chipInfo then
    self._expNum:SetText(NekoData.BehaviorManager.BM_Message:GetString(1939, {
      chipInfo.has,
      chipInfo.max
    }))
  else
...
function NewDreamSpiralPage:NumberOfCell(frame)
  if frame == self._cellFrame then
    return #self._data
  end
end

function NewDreamSpiralPage:CellAtIndex(frame, index)
  if frame == self._cellFrame then
    return "activity.giftoftime.newdreamspiralcell"
  end
end

function NewDreamSpiralPage:DataAtIndex(frame, index)
  if frame == self._cellFrame then
    return self._data[index]
  end
end

function NewDreamSpiralPage:OnUnlockBtnClicked()
...
function NewDreamSpiralPage:OnGetExpBtnClicked()
  self._delegate:OnCutBtn2Clicked()
end

function NewDreamSpiralPage:OnGetAllBtnClicked()
...
function NewDreamSpiralPage:OnTipBtnClicked()
