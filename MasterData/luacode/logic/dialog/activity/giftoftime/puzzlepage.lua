...
function PuzzlePage:Ctor(...)
  PuzzlePage.super.Ctor(self, ...)
  self._puzzleData = {}
  self._rewardData = {}
end

function PuzzlePage:OnCreate()
...
function PuzzlePage:SetStaticRes()
...
function PuzzlePage:RefreshData()
...
function PuzzlePage:RefreshMainData()
  ...
  local imageRecord = CImagePathTable:GetRecorder(jigSawRecord.jigsawRes) or DataCommon.DefaultImageAsset
  ...
  for index, value in ipairs(jigSawRecord.moneyNum) do
    self._puzzleData[index] = {
      moneyNum = value,
      isOpen = self._bm:GetUnlockedPuzzle()[index]
    }
  end
  for index, value in ipairs(jigSawRewaredRecord.itemID) do
    self._rewardData[index] = {
      itemID = value,
      itemNum = jigSawRewaredRecord.itemNum[index],
      status = self._bm:GetAwardStatus()[index]
    }
  end
...
function PuzzlePage:RefreshCurrencyData()
  ...
  self._currency2Num:SetText(NekoData.BehaviorManager.BM_Message:GetString(1960, {
    NumberManager.GetShowNumber(self._haveCurrencyNum2)
  }))
end

function PuzzlePage:RefreshTaskReddot()
...
function PuzzlePage:OnCurrency2IconClicked()
