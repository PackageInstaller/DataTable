...
local ChildDialogs = {
  ...
  "activity.springfestival.bossdetailsdialog"
}
...
function BossMainDialog:Ctor(...)
  BossMainDialog.super.Ctor(self, ...)
  self._groupName = "Modal"
end

function BossMainDialog:OnCreate()
...
function BossMainDialog:OnDestroy()
  LuaNotificationCenter.RemoveObserver(self)
  UIBackManager.SetUIBackShow(false)
end

function BossMainDialog:SetData()
  self:RefreshSpringSpirit()
end

function BossMainDialog:OnBackBtnClicked()
  self:Destroy()
end

function BossMainDialog:OnMenuBtnClicked()
  DialogManager.CreateSingletonDialog("fastmenu.fastmenudialog")
end

function BossMainDialog:SetBossInfo()
  ...
  for k, v in pairs(self._bossInfo.bossList) do
    if v.state >= SpringBossState.DEFEAT then
  ...
  for i, bossUi in ipairs(self._bossUI) do
    ...
    if bossUi._lockText then
      bossUi._lockText:SetActive(data.state == SpringBossState.UNLOCK)
    end
    if bossUi._lockImg then
      bossUi._lockImg:SetActive(data.state == SpringBossState.UNLOCK)
    end
    ...
    if bossUi._line then
      bossUi._line:SetActive(data.state >= SpringBossState.DEFEAT)
    end
...
function BossMainDialog:GetRemainTimeStr(endTime)
...
function BossMainDialog:OnBossClick(i)
  ...
  elseif self._bossInfo.bossList[i].state == SpringBossState.DEFEAT then
...
function BossMainDialog:RefreshSpringSpirit()
...
function BossMainDialog:OpenRankDialog(data)
...
function BossMainDialog:OnInstructBtnClicked()
...
function BossMainDialog:AddNewModal(dialog)
...
function BossMainDialog:OnSSpringBossState()
  self._bossInfo = self._bm:GetBossInfo()
  self._totleScore:SetText(self._bossInfo.score)
  local rankNum
  if self._bossInfo.rank == -1 then
...
function BossMainDialog:SReceiveSpringBossReward(data)
...
function BossMainDialog:OnAnimationEnd(handler, stateName, normalizedTime)
...
function BossMainDialog:ShowSpringFestivalFireworks()
  if self._defeatCount == #self._bossUI then
    local flg = CS.UnityEngine.PlayerPrefs.GetInt("SpringFestivalFireworksReleased")
    if not flg or flg ~= 2 then
      self._effect:SetActive(true)
