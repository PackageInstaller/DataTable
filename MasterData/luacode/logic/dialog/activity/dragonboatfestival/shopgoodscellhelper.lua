...
function ShopGoodsCellHelper:Ctor(rootElement)
  ...
  self._redDot = self._rootElement:FindChild("RedDot")
...
function ShopGoodsCellHelper:OnDestroy()
  LuaNotificationCenter.RemoveObserver(self)
end

function ShopGoodsCellHelper:RefreshCell(data, shopID, indexNum)
  self._data = data
  self._shopID = shopID
  ...
  self._priceOnSaleRmbImage:SetActive(not (self._data.moneyType > 35000))
  self._priceOnSaleCurrencyImage:SetActive(self._data.moneyType > 35000)
  self._priceNormalRmbImag:SetActive(not (self._data.moneyType > 35000))
...
function ShopGoodsCellHelper:OnBuyClicked()
  if not self._bm:GetShopIsOpen() then
    NekoData.BehaviorManager.BM_Message:SendMessageById(100469)
    return
  end
  if self._data.goodRemain ~= 0 then
    DialogManager.CreateSingletonDialog("activity.dragonboatfestival.itembuydialog"):SetData(self._data, self._shopID)
  end
end
