...
function ThemeDetailDialog:Ctor(...)
...
function ThemeDetailDialog:OnCreate()
...
function ThemeDetailDialog:OnDestroy()
  self._chooseFrame:Destroy()
  self._furnitureFrame:Destroy()
  LuaNotificationCenter.RemoveObserver(self)
end

function ThemeDetailDialog:OnRefreshGoodInfo()
...
function ThemeDetailDialog:OnGoodsChecked(notification)
...
function ThemeDetailDialog:SetData(themeId)
...
function ThemeDetailDialog:RefreshSortPanel()
...
function ThemeDetailDialog:RefreshLeftThemeDetail()
...
function ThemeDetailDialog:RefreshFurnitureList(bySearch)
...
function ThemeDetailDialog:OnLeftArrowClick()
...
function ThemeDetailDialog:OnRightArrowClick()
  local dialog = DialogManager.GetDialog("courtcabin.shop.furniturestoredialog")
  if dialog and dialog._selectTabId == 0 then
    local themeTabCell = dialog._tabFrame:GetCellAtIndex(dialog._selectTabId)
...
function ThemeDetailDialog:OnMouseClick(args)
...
function ThemeDetailDialog:SetSortId(sortId)
...
function ThemeDetailDialog:OnBackBtnClick()
  self:Destroy()
end

function ThemeDetailDialog:OnCurPosChange(frame, proportion)
...
function ThemeDetailDialog:NumberOfCell(frame)
  if frame == self._furnitureFrame then
...
function ThemeDetailDialog:CellAtIndex(frame, index)
  if frame == self._furnitureFrame then
...
function ThemeDetailDialog:DataAtIndex(frame, index)
  if frame == self._furnitureFrame then
...
function ThemeDetailDialog:AddNewModal(dialog)
end
