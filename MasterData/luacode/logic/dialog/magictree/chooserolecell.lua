...
function ChooseRoleCell:Ctor(...)
  ChooseRoleCell.super.Ctor(self, ...)
end

function ChooseRoleCell:OnCreate()
...
function ChooseRoleCell:OnDestroy()
end

function ChooseRoleCell:RefreshCell(data)
  self._data = data
...
function ChooseRoleCell:RefreshEnergy()
  local imgRecord
  local restEnergy = 0
...
function ChooseRoleCell:RefreshWorkingState()
  ...
  local buildingId = self._data:GetBuildingId()
  local tag = buildingId and buildingId ~= DataCommon.Alchemy and buildingId ~= DataCommon.Cabin
  self._grey:SetActive(index or buildingId)
  self._canNotSelect = not index and tag and not initIndex
  local buildingId = self._data:GetBuildingId()
  if buildingId then
    self._working:SetActive(true)
    local courtYardRecord = CCourtYardNameCfg:GetRecorder(buildingId)
    local imageRecord = CImagePathTable:GetRecorder(courtYardRecord.yardsheltericon) or DataCommon.DefaultImageAsset
    ...
    if index then
      ...
      elseif buildingId == DataCommon.Cabin then
        ...
        str = NekoData.BehaviorManager.BM_Message:GetString(1324, {
          TextManager.GetText(CDormFloorCfg:GetRecorder(roomId).stayingfloor)
        })
...
function ChooseRoleCell:OnCellClicked()
  if not self._canNotSelect then
    self._delegate:SelectRoles(self._data:GetRoleId())
  end
end

function ChooseRoleCell:OnEvent(eventName, arg)
