...
function ChangeSupportRoleDialog:Ctor(...)
...
function ChangeSupportRoleDialog:OnCreate()
...
function ChangeSupportRoleDialog:OnDestroy()
...
function ChangeSupportRoleDialog:RefreshDownPanel()
...
function ChangeSupportRoleDialog:ShowInfo()
...
function ChangeSupportRoleDialog:OnUniqueEquipLevelUp(notification)
...
function ChangeSupportRoleDialog:SetSelectedRole(key)
...
function ChangeSupportRoleDialog:NumberOfCell(frame)
  if frame == self._upFrame then
    return #self._listByConditions
  elseif self._selectedRole then
    return 1
  else
    return 0
  end
...
function ChangeSupportRoleDialog:CellAtIndex(frame)
  return "assistbattle.changesupportrolecell"
end

function ChangeSupportRoleDialog:DataAtIndex(frame, index)
  if frame == self._upFrame then
...
function ChangeSupportRoleDialog:CheckConditionNum()
  local count = 0
...
function ChangeSupportRoleDialog:ScreeningAndSorting()
  ...
  table.sort(self._listByConditions, function(a, b)
    ...
    if a_skill2Unlock and not b_skill2Unlock then
      return true
    elseif not a_skill2Unlock and b_skill2Unlock then
      return false
    end
    local _a, _b
...
function ChangeSupportRoleDialog:OnFilterBtnClick()
...
function ChangeSupportRoleDialog:OnLevelBtnClick()
  if not self._filterDlg then
    return
  end
...
function ChangeSupportRoleDialog:OnSortBtnClick()
...
function ChangeSupportRoleDialog:SetSortCriteriaNameId(nameId)
...
function ChangeSupportRoleDialog:SetScreeningConditions(screeningConditions)
  self._screeningConditions = screeningConditions
  self:ScreeningAndSorting()
end

function ChangeSupportRoleDialog:OnConfirmBtnClicked()
  local supportRole = NekoData.BehaviorManager.BM_Friends:GetSupportRole()
  if not (supportRole or self._selectedRole) or supportRole and self._selectedRole and self._selectedRole:GetRoleId() == supportRole:GetRoleId() then
    self:Destroy()
  else
...
function ChangeSupportRoleDialog:OnBackBtnClicked()
  local supportRole = NekoData.BehaviorManager.BM_Friends:GetSupportRole()
  if not (self._selectedRole or supportRole) or self._selectedRole and supportRole and self._selectedRole:GetRoleId() == supportRole:GetRoleId() then
    self:Destroy()
  else
...
function ChangeSupportRoleDialog:OnMenuBtnClicked()
  DialogManager.CreateSingletonDialog("fastmenu.fastmenudialog")
end

function ChangeSupportRoleDialog:SendCUpdateUISortType()
