...
function HandBookRoleDetailInfoDialog:Ctor(...)
  HandBookRoleDetailInfoDialog.super.Ctor(self, ...)
  self._groupName = "Modal"
end

function HandBookRoleDetailInfoDialog:OnCreate()
...
function HandBookRoleDetailInfoDialog:OnDestroy()
...
function HandBookRoleDetailInfoDialog:OnRefreshRoleGood(notification)
...
function HandBookRoleDetailInfoDialog:OnRefreshReceiveAward(notification)
  local protocol = notification.userInfo
  local roleId = self._role:GetId()
...
function HandBookRoleDetailInfoDialog:OnRefreshUniqueEquipPanel()
  self._uniqueEquipBtn:SetActive(self._role:GetUniqueEquipId())
end

function HandBookRoleDetailInfoDialog:AddLevelUpEffect()
  self._levelUpEffect:AddEffectSync(EffectUtil.GetAssetBundleNameAndAssetName(1115))
end

function HandBookRoleDetailInfoDialog:RefreshPanel(roleID)
...
function HandBookRoleDetailInfoDialog:Refresh(roleID, npcID)
...
function HandBookRoleDetailInfoDialog:RefreshTabsRedDot()
...
function HandBookRoleDetailInfoDialog:OnTabBtnClick(index)
...
function HandBookRoleDetailInfoDialog:DidToPage(helper, index)
...
function HandBookRoleDetailInfoDialog:CellAtIndex(helper, index)
...
function HandBookRoleDetailInfoDialog:SetVoiceAndLines2(roleID, fashionID, tableKey, index, contentId, animation, playHandBook)
...
function HandBookRoleDetailInfoDialog:SetVoiceIdNil(auto)
  if self._voiceId then
    if not auto then
      NekoData.BehaviorManager.BM_Voice:StopVoice(self._voiceId)
    end
...
function HandBookRoleDetailInfoDialog:OnAnimationStateExit(handle, stateName, normalizedTime)
  if stateName == "CharDataMainDialogHide" then
    self:SetVoiceIdNil(true)
  end
end

function HandBookRoleDetailInfoDialog:OnCuePlayEnd(notification)
  if self._voiceId then
...
function HandBookRoleDetailInfoDialog:OnLiHuiClicked(onInitial)
...
function HandBookRoleDetailInfoDialog:OnLive2DPointerDown(posName, roleID, fashionID, touchRecordList)
  if self._voiceId then
    return
  end
  local touchRecord
...
function HandBookRoleDetailInfoDialog:CacheGiveItemId(itemId)
  self._giveItemId = itemId
end

function HandBookRoleDetailInfoDialog:RefreshByRoleId(roleID)
...
function HandBookRoleDetailInfoDialog:OnLeftArrowClick()
...
function HandBookRoleDetailInfoDialog:OnRightArrowClick()
  if not self._roleListInit then
    self:InitRoleList()
  end
  if self._canChangeRole then
...
function HandBookRoleDetailInfoDialog:InitRoleList()
...
function HandBookRoleDetailInfoDialog:OnRoleChangeStateExit(arg1, arg2)
  if arg2 == "CharChangeRole" then
    self._canChangeRole = true
  end
end

function HandBookRoleDetailInfoDialog:OnUniqueEquipBtnClicked()
...
function HandBookRoleDetailInfoDialog:OnBackBtnClicked()
  self:Destroy()
end

function HandBookRoleDetailInfoDialog:OnMenuBtnClicked()
  DialogManager.CreateSingletonDialog("fastmenu.fastmenudialog")
end

function HandBookRoleDetailInfoDialog:OnCutoverBtnClick()
