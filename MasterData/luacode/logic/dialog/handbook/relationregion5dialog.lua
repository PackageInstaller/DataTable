...
local RelationRegion5Dialog = class("RelationRegion5Dialog", Dialog)
RelationRegion5Dialog.AssetBundleName = "ui/layouts.tujiannew"
RelationRegion5Dialog.AssetName = "RelationRegion5"

function RelationRegion5Dialog:Ctor(...)
  RelationRegion5Dialog.super.Ctor(self, ...)
...
function RelationRegion5Dialog:OnCreate()
...
function RelationRegion5Dialog:OnDestroy()
  self._deleget:OnEnable()
  self._handBookSceneController:SetCameraAnimatorState(0)
end

function RelationRegion5Dialog:Init(deleget, dialogIndex)
  self._deleget = deleget
  self._dialogIndex = dialogIndex
end

function RelationRegion5Dialog:OnBtnClicked(index)
...
function RelationRegion5Dialog:OnAnimationStateExit(handler, stateName, normalizedTime)
...
function RelationRegion5Dialog:OnEnable()
  self._animator:SetInteger("state", 1)
end

function RelationRegion5Dialog:ToBookList()
  self._deleget:ToBookList()
  self:Destroy()
end

function RelationRegion5Dialog:OnBackBtnClicked()
  self:Destroy()
end

return RelationRegion5Dialog
