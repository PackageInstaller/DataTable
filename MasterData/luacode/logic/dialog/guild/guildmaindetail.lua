...
function GuildMainDetail:OnCreate()
...
function GuildMainDetail:OnDestroy()
...
function GuildMainDetail:OnGotoGuildBossJump()
  self:OnActivityBtnClicked()
  self:OnGuildActivityGuildBossClicked()
end

function GuildMainDetail:Init()
  ...
  local waitForUpdata = guildInfo.partyExp >= guildInfo.partyExpMax and maxLevel ~= guildInfo.partyLv
  ...
  self._missionFinishImage:SetActive(100 <= daliyTaskProgress)
  self._jumpMissionBtn:SetActive(daliyTaskProgress < 100)
  self._introInputText:SetInteractable(false)
  local intro
  if guildInfo.partyDeclaration and 0 < #guildInfo.partyDeclaration then
  ...
  if NekoData.BehaviorManager.BM_Guild:GetGuildBossRed().flag and 0 < NekoData.BehaviorManager.BM_Guild:GetGuildBossRed().flag then
  ...
  if NekoData.DataManager.DM_Guild:IsGotoGuildBossJump() then
    NekoData.DataManager.DM_Guild:ClearGotoGuildBossJump()
    self:OnGotoGuildBossJump()
  end
end

function GuildMainDetail:RefreshActiveRedIcon()
  local isActivityRedDot = NekoData.BehaviorManager.BM_Guild:HasActivityRedDotOn()
...
function GuildMainDetail:RefreshGuildExp()
  local guildInfo = NekoData.BehaviorManager.BM_Guild:GetGuildInfo()
  self._levelUpTime = guildInfo.levelUpTime // 1000
  self._exp:SetText(guildInfo.partyExp .. "/" .. guildInfo.partyExpMax)
  local maxLevel = NekoData.BehaviorManager.BM_Guild:GetGuildMaxLevel()
...
function GuildMainDetail:CountDown()
...
function GuildMainDetail:NumberOfCell(frame)
...
function GuildMainDetail:CellAtIndex(frame, index)
  if frame == self._chatFrame then
...
function GuildMainDetail:DataAtIndex(frame, index)
  if frame == self._chatFrame then
...
function GuildMainDetail:OnBackBtnClicked()
  self:Destroy()
end

function GuildMainDetail:OnMenuBtnClicked()
  DialogManager.CreateSingletonDialog("fastmenu.fastmenudialog")
end

function GuildMainDetail:OnCopyIDBtnClicked()
  self:GetRootWindow():CopyToClipBoard(tostring(self._guildId))
  NekoData.BehaviorManager.BM_Message:SendMessageById(100079)
end

function GuildMainDetail:OnIntroChangeBtnClicked()
...
function GuildMainDetail:OnSModifyPartyDeclaration()
  SetInputModel(self, false)
  local guildInfo = NekoData.BehaviorManager.BM_Guild:GetGuildInfo()
  local intro
...
function GuildMainDetail:OnIntroChangeSubmitClicked()
...
function GuildMainDetail:OnIntroChangeCancelClicked()
  SetInputModel(self, false)
  local guildInfo = NekoData.BehaviorManager.BM_Guild:GetGuildInfo()
  local intro
  if guildInfo.partyDeclaration and #guildInfo.partyDeclaration > 0 then
...
function GuildMainDetail:OnIntroTextInputValueChanged()
...
function GuildMainDetail:OnMemberBtnClick()
...
function GuildMainDetail:OnCheckBtnClick()
...
function GuildMainDetail:OnSNotifyPosition(notification)
...
function GuildMainDetail:OnAddNewMsg(notification)
...
function GuildMainDetail:OnLevelIBtnClick()
  DialogManager.CreateSingletonDialog("guild.guildupgradetips")
end

function GuildMainDetail:OnGuildIBtnClick()
  DialogManager.CreateSingletonDialog("guild.guildtips")
end

function GuildMainDetail:OnCurPosChange(frame, proportion)
  if frame == self._chatFrame then
...
function GuildMainDetail:OnSendBtnClicked()
...
function GuildMainDetail:OnChatTextInputValueChanged()
...
function GuildMainDetail:OnNewMessageTipClicked()
  self._chatFrame:MoveToBottom()
end

function GuildMainDetail:OnEmojiBtnClicked()
  if self._emojiFrame then
    self._emojiFrame:Destroy()
...
function GuildMainDetail:OnDialogClicked(args)
...
function GuildMainDetail:HideEmojiFrame()
  self._emojiFrame:Destroy()
  self._emojiFrame = nil
  self._emojiArea:SetActive(false)
end

function GuildMainDetail:OnLogoClicked()
end

function GuildMainDetail:ShouldLengthChange(frame, index)
  return true
end

function GuildMainDetail:OnShieldFunction()
...
function GuildMainDetail:AddNewModal(dialog)
...
function GuildMainDetail:OnChatBtnClicked()
...
function GuildMainDetail:OnActivityBtnClicked()
  if not self._activityInit then
    self._activityInit = true
    local recorder = CGuildActiveOpen:GetRecorder(1)
    local imageRecord = CImagePath:GetRecorder(recorder.ActiveImage) or DataCommon.DefaultImageAsset
...
function GuildMainDetail:OnShopBtnClicked()
...
function GuildMainDetail:OnjumpMissionBtnClick()
...
function GuildMainDetail:OnEmojiTypeBtnClicked(_type)
