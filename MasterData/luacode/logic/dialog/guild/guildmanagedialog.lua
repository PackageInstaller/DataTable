...
function GuildManageDialog:Ctor(...)
  GuildManageDialog.super.Ctor(self, ...)
  self._groupName = "SecondConfirm"
end

function GuildManageDialog:OnCreate()
...
function GuildManageDialog:OnDestroy()
...
function GuildManageDialog:Init(data)
...
function GuildManageDialog:SetBlackListBtnState()
  local str
...
function GuildManageDialog:ShowJobCount()
...
function GuildManageDialog:OnSCheckPositionRemain(notification)
...
function GuildManageDialog:OnSNotifyPosition(notification)
  local data = notification.userInfo
...
function GuildManageDialog:OnTurnPresidentBtnClick()
  ...
  NekoData.BehaviorManager.BM_Message:AddSecondConfirmDialog(107, {
    self._nameStr
  }, function()
...
function GuildManageDialog:OnAppointVicePresidentBtnClick()
...
function GuildManageDialog:OnAppointEliteBtnClick()
...
function GuildManageDialog:OnAppointCommonMemberBtnClick()
  if self._job == JobEnum.VICE_PRESIDENT then
    local csend = LuaNetManager.CreateProtocol("protocol.party.cappointvicepresident")
    if csend then
      local operate = 0
  ...
  elseif self._job == JobEnum.CREAM then
    local csend = LuaNetManager.CreateProtocol("protocol.party.cappointcream")
    if csend then
      local operate = 0
      csend.partyId = NekoData.BehaviorManager.BM_Guild:GetGuildID()
  ...
  elseif self._job == JobEnum.MEMBER then
    NekoData.BehaviorManager.BM_Message:SendMessageById(100299, nil)
    return
  elseif self._job == JobEnum.PRESIDENT then
    NekoData.BehaviorManager.BM_Message:SendMessageById(100287, nil)
    return
  end
end

function GuildManageDialog:OnRemoveMemberBtnClick()
...
function GuildManageDialog:OnDialogClicked(args)
  if self._manageBack:IsActive() then
    if not CS.PixelNeko.UI.UIManager.RectangleContainsScreenPoint(self._manageBack._uiObject, args.userInfo.eventData.position.x, args.userInfo.eventData.position.y) and not CS.PixelNeko.UI.UIManager.RectangleContainsScreenPoint(self._back._uiObject, args.userInfo.eventData.position.x, args.userInfo.eventData.position.y) then
      self:OnBackBtnClicked()
    end
  elseif not CS.PixelNeko.UI.UIManager.RectangleContainsScreenPoint(self._back._uiObject, args.userInfo.eventData.position.x, args.userInfo.eventData.position.y) then
    self:OnBackBtnClicked()
  end
end

function GuildManageDialog:OnBackBtnClicked()
  self:Destroy()
end

function GuildManageDialog:OnPhotoClicked()
...
function GuildManageDialog:OnAddFriendClicked()
  NekoData.BehaviorManager.BM_Friends:RequesAddFriend(self._userId)
end

function GuildManageDialog:OnBlacklistBtnClicked()
...
function GuildManageDialog:OnSRefreshOtherUserIdentity(notification)
  if notification.userInfo.userid == self._userId then
    self._isBlackList = notification.userInfo.newIdentity == OtherUserInfo.BLACK_LIST
    self:SetBlackListBtnState()
  end
end

function GuildManageDialog:OnReportClicked()
  local data = {
    reportid = self._userId,
    nickname = self._nameStr
  }
