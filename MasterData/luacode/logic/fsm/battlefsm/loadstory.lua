local State = {}
local chatdialogId, battleType, battleController
...
local TypeEnum = {
  ...
  [CBattleStartProtocol.ANNIVERSARY] = {
  ...
  [CBattleStartProtocol.STARRY_MIRROR] = {
    bm = NekoData.BehaviorManager.BM_StarMirageCopy,
    levelTable = BeanManager.GetTableByName("dungeonselect.cstarrymirrorlevel"),
    type = DataCommon.StarMirage1[DataCommon.Activities.StarMirageCopy].Type,
    dialogType = 3
  },
  [CBattleStartProtocol.SUMMER_ECHO] = {
    bm = NekoData.BehaviorManager.BM_Activity:GetManager(DataCommon.SummerActivityEchoesManagerID),
    levelTable = BeanManager.GetTableByName("activity.csrfunction"),
    dialogType = 9
  }
}
local ActivityIdType = {
  [CBattleStartProtocol.STARRY] = DataCommon.Activities.StarMirage,
  [CBattleStartProtocol.ANNIVERSARY] = DataCommon.Activities.Anniversary,
  [CBattleStartProtocol.STARRY_MIRROR] = DataCommon.Activities.StarMirageCopy
}
  ...
  if type then
    ...
    if chatdialogId and result == 1 then
      ...
      if battleType == CBattleStartProtocol.SUMMER_ECHO then
        ...
        elseif record.difficulty == type.type.Hard then
...
function State.OnNPCChatEnd(observer, notification)
