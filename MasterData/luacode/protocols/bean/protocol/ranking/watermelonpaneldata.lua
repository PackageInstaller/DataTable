...
local WatermelonPanelData = dataclass("WatermelonPanelData")
WatermelonPanelData.damage = 0
WatermelonPanelData.power = 0

function WatermelonPanelData:Ctor()
  self.roleIdList = {}
  self.roleLvList = {}
  self.roleBreakList = {}
  self.roleSkinList = {}
...
function WatermelonPanelData:Marshal(buffer)
  if not ProtocolBufferStaticFunctions.WriteInt32(buffer, self.damage) then
    return false
  end
...
function WatermelonPanelData:Unmarshal(buffer)
  local ret = true
  ret, self.damage = ProtocolBufferStaticFunctions.ReadInt32(buffer)
  if not ret then
    return ret
  end
