...
local function loadLocalServerlist(callback)
  local l, file
  ...
  if isTestPack then
    ...
    elseif SdkManager.GetChannel() == "quka" or SdkManager.GetChannel() == "qukb" then
      ...
      if channel == "114" then
        filename = "serverlist_official.json"
      else
        filename = "serverlist_channel.json"
      end
    else
  ...
  if l then
    if SdkManager.IsOverseas then
      CS.PixelNeko.NetManager.HttpGetRequestAsync(l.downloadurl, function(result)
        if not result then
