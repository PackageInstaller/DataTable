...
function HorizontalGridFrameImpl:Ctor(interface, container, delegate, rowNums, canSlide, viewportName, downToTop)
...
function HorizontalGridFrameImpl:Destroy()
...
function HorizontalGridFrameImpl:GetTotalLength()
  return self._totalLength
end

function HorizontalGridFrameImpl:GetCurrentPosition()
  return self._currentPosition
end

function HorizontalGridFrameImpl:ReloadAllCell()
  for i, logicCell in ipairs(self._logicCells) do
    if logicCell._cell then
      self:RecycleCell(logicCell)
    end
...
function HorizontalGridFrameImpl:MoveToLeft(isAnimate)
  self._needUpdate = true
  if isAnimate then
    local first = {
...
function HorizontalGridFrameImpl:MoveToRight(isAnimate)
  self._needUpdate = true
...
function HorizontalGridFrameImpl:MoveLeftToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveRightIndexTask = nil
...
function HorizontalGridFrameImpl:MoveRightToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveLeftIndexTask = nil
  local width, height = self._viewport:GetRectSize()
...
function HorizontalGridFrameImpl:MoveToAssignedPos(pos, isAnimate)
  self._needUpdate = true
...
function HorizontalGridFrameImpl:ReloadCellsAtIndex(indexList, isAnimate)
...
function HorizontalGridFrameImpl:RemoveCellsAtIndex(indexList, isAnimate)
  if type(indexList) ~= "table" then
    LogError("HorizontalGridFrameImpl", "please input a table value")
    return
  end
  ...
  for _, index in ipairs(indexList) do
    ...
    for i = index + 1, cellNums do
      ...
      elseif self._logicCells[i]._row == self._rowNums - 1 then
        self._logicCells[i]._col = self._logicCells[i]._col - 1
...
function HorizontalGridFrameImpl:InsertCellsAtIndex(indexList, isAnimate)
...
function HorizontalGridFrameImpl:FireEvent(eventName, ...)
  for i, logicCell in ipairs(self._logicCells) do
...
function HorizontalGridFrameImpl:FireIndexCellEvent(eventName, index, ...)
  local logicCell = self._logicCells[index]
...
function HorizontalGridFrameImpl:SetMargin(leftValue, rightValue)
...
function HorizontalGridFrameImpl:OnBeginDrag(args)
  self._dragDelta = 0
...
function HorizontalGridFrameImpl:OnDrag(args)
...
function HorizontalGridFrameImpl:OnEndDrag(args)
  self._dragDelta = 0
...
function HorizontalGridFrameImpl:OnPointerDown(deltaTime)
  self._dragDelta = 0
...
function HorizontalGridFrameImpl:OnPointerUp(deltaTime)
  self._sprintclickdown = false
  self:UpdateView(0)
end

function HorizontalGridFrameImpl:OnLateUpdate(notification)
...
function HorizontalGridFrameImpl:UpdateView(deltaTime)
  if self._needUpdate then
    local currentPosition = self._currentPosition
    local width, height = self._viewport:GetRectSize()
    ...
    for i, logicCell in ipairs(self._logicCells) do
      if logicCell._posX + self._cellSizeX >= currentPosition - self._leftMargin and logicCell._posX <= currentPosition + width + self._rightMargin then
...
function HorizontalGridFrameImpl:GetCellDialog(logicCell)
  if logicCell._cell then
    return
  end
...
function HorizontalGridFrameImpl:RecycleCell(logicCell)
  if logicCell._cell then
    if not self._recycleCells[logicCell._dialogName] then
      self._recycleCells[logicCell._dialogName] = {}
    end
...
function HorizontalGridFrameImpl:SendMessageToInstance(name, args)
...
function HorizontalGridFrameImpl:SetSlide(slide, showSoftMask)
  if slide then
...
function HorizontalGridFrameImpl:SetSoftMaskActive(showSoftMask)
  CS.PixelNeko.Lua.SoftMaskStaticFunctions.SetSoftMaskActive(self._viewport._uiObject, showSoftMask)
end

function HorizontalGridFrameImpl:RefreshUIParticleClipper()
  self._refreshUIParticleClipper = true
  CS.PixelNeko.Render.ShaderUtility.UIParticleClipper.RefreshUIParticleClipper(self._viewport:GetUIObject())
end

function HorizontalGridFrameImpl:ClearAllPositionChangeTask()
  self._slideInertiaTask = nil
  self._sprintTask = nil
  self._moveLeftTask = nil
  self._moveRightTask = nil
