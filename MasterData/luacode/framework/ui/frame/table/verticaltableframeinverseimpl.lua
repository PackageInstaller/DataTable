...
local VerticalTableFrameInverseImpl = class("VerticalTableFrameInverseImpl")

function VerticalTableFrameInverseImpl:Ctor(interface, container, delegate, canSlide, autoAlign, viewportName)
  ...
  if CS.PixelNeko.Lua.UI.FrameStaticFunctions.GetFrameChildCount(container._uiObject) > 0 then
    LogErrorFormat("VerticalTableFrameInverseImpl", "非法操作: container [%s] 已经创建过frame Viewport", container._uiObject)
  end
...
function VerticalTableFrameInverseImpl:Destroy()
...
function VerticalTableFrameInverseImpl:ReloadAllCell()
  for i, logicCell in ipairs(self._logicCells) do
    if logicCell._cell then
      self:RecycleCell(logicCell)
    end
...
function VerticalTableFrameInverseImpl:MoveToTop(isAnimate)
  self._needUpdate = true
...
function VerticalTableFrameInverseImpl:MoveToBottom(isAnimate)
  self._needUpdate = true
  if isAnimate then
    local first = {
      pos = self._currentPosition
...
function VerticalTableFrameInverseImpl:MoveDownToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveTopIndexTask = nil
  ...
  if destination < 0 - self._downMargin then
    destination = 0 - self._downMargin
  end
...
function VerticalTableFrameInverseImpl:MoveTopToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveDownIndexTask = nil
  local width, height = self._viewport:GetRectSize()
...
function VerticalTableFrameInverseImpl:MoveToAssignedPos(pos, isAnimate)
  self._needUpdate = true
...
function VerticalTableFrameInverseImpl:GetTotalLength()
  return self._totalLength
end

function VerticalTableFrameInverseImpl:GetCurrentPosition()
  return self._currentPosition
end

function VerticalTableFrameInverseImpl:ReloadCellsAtIndex(indexList, isAnimate)
  ...
  for _, index in pairs(indexList) do
    if not index or index > cellNums then
      LogErrorFormat("VerticalTableFrameInverseImpl", "the index %d is out of range", index)
      return
    end
    ...
    local width, height = self._viewport:GetRectSize()
    local topDelta = self._totalLength - height - self._currentPosition
    local totalHasChange = false
    if delta > topDelta or height > self._totalLength then
...
function VerticalTableFrameInverseImpl:RemoveCellsAtIndex(indexList, isAnimate)
  if type(indexList) ~= "table" then
    LogError("VerticalTableFrameInverseImpl", "please input a table value")
    return
  end
  ...
  for _, index in pairs(indexList) do
    ...
    local totalHasChange = false
    if currentPosition < pos + gap and pos < currentPosition + height then
      if isAnimate then
        self._removeTask = {}
        for i = 1, index - 1 do
...
function VerticalTableFrameInverseImpl:InsertCellsAtIndex(indexList, isAnimate, dontMoveCurrentPosition)
...
function VerticalTableFrameInverseImpl:FireEvent(eventName, ...)
  for i, logicCell in ipairs(self._logicCells) do
...
function VerticalTableFrameInverseImpl:GetTopIndex()
  return self._topIndex
end

function VerticalTableFrameInverseImpl:GetDownIndex()
  return self._downIndex
end

function VerticalTableFrameInverseImpl:SetMargin(upValue, downValue)
...
function VerticalTableFrameInverseImpl:OnBeginDrag(args)
  self._dragDelta = 0
...
function VerticalTableFrameInverseImpl:OnDrag(args)
...
function VerticalTableFrameInverseImpl:OnEndDrag(args)
...
function VerticalTableFrameInverseImpl:OnPointerDown(args)
  self._dragDelta = 0
...
function VerticalTableFrameInverseImpl:OnPointerUp(args)
  self._sprintclickdown = false
  self:UpdateView(0)
end

function VerticalTableFrameInverseImpl:OnLateUpdate(notification)
...
function VerticalTableFrameInverseImpl:UpdateView(deltaTime)
  if self._needUpdate then
    local currentPosition = self._currentPosition
    local width, height = self._viewport:GetRectSize()
    if height < self._totalLength + self._upMargin then
    ...
    for i, logicCell in ipairs(self._logicCells) do
      if height < self._totalLength + self._upMargin then
        self:SetCellVisible(i, logicCell, currentPosition + height + self._upMargin, currentPosition - self._downMargin)
      else
        self:SetCellVisible(i, logicCell, height - self._upMargin, self._downMargin)
      end
    end
    self._cellX = -1
    for i, logicCell in ipairs(self._logicCells) do
      if logicCell._visible then
        ...
        logicCell._cell:GetRootWindow():SetPosition(0, 0, 0, logicCell._pos)
    ...
    if height < self._totalLength + self._upMargin then
      self._viewcontainer:SetPosition(0, (viewportX - self._cellX) / 2, 0, -currentPosition)
    else
      self._viewcontainer:SetPosition(0, (viewportX - self._cellX) / 2, 0, height - self._totalLength - currentPosition - self._upMargin)
    end
    ...
    if not self._sprintTask and not self._slideInertiaTask and #self._insertTask == 0 and #self._removeTask == 0 and not self._removeCurTask and not self._removeTolTask and not self._moveTopTask and not self._moveBottomTask and not self._moveDownIndexTask and not self._moveTopIndexTask and not self._alignTask and not self._moveToAssignedPosTask then
      self._needUpdate = false
    end
...
function VerticalTableFrameInverseImpl:SetCellVisible(i, logicCell, up, down)
  if down < logicCell._pos + logicCell._size and up > logicCell._pos then
    if self._flag then
      self._flag = false
      self._topIndex = i
    end
...
function VerticalTableFrameInverseImpl:GetCellDialog(logicCell)
  if logicCell._cell then
    return
  end
...
function VerticalTableFrameInverseImpl:RecycleCell(logicCell)
  if logicCell._cell then
    if not self._recycleCells[logicCell._dialogName] then
      self._recycleCells[logicCell._dialogName] = {}
    end
...
function VerticalTableFrameInverseImpl:SetSlide(slide, showSoftMask)
  if slide then
...
function VerticalTableFrameInverseImpl:SetSoftMaskActive(showSoftMask)
  CS.PixelNeko.Lua.SoftMaskStaticFunctions.SetSoftMaskActive(self._viewport._uiObject, showSoftMask)
end

function VerticalTableFrameInverseImpl:RefreshUIParticleClipper()
  self._refreshUIParticleClipper = true
  CS.PixelNeko.Render.ShaderUtility.UIParticleClipper.RefreshUIParticleClipper(self._viewport:GetUIObject())
end

function VerticalTableFrameInverseImpl:ClearAllPositionChangeTask()
  self._slideInertiaTask = nil
  self._sprintTask = nil
  self._moveTopTask = nil
  self._moveBottomTask = nil
...
return VerticalTableFrameInverseImpl
