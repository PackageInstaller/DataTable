...
function VerticalTableFrameImpl:Ctor(interface, container, delegate, canSlide, autoAlign, viewportName)
...
function VerticalTableFrameImpl:Destroy()
...
function VerticalTableFrameImpl:ReloadAllCell()
  for i, logicCell in ipairs(self._logicCells) do
    if logicCell._cell then
      self:RecycleCell(logicCell)
    end
...
function VerticalTableFrameImpl:MoveToTop(isAnimate)
  self._needUpdate = true
  if isAnimate then
    local first = {
...
function VerticalTableFrameImpl:MoveToBottom(isAnimate)
  self._needUpdate = true
...
function VerticalTableFrameImpl:MoveTopToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveDownIndexTask = nil
...
function VerticalTableFrameImpl:MoveDownToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveTopIndexTask = nil
  local width, height = self._viewport:GetRectSize()
...
function VerticalTableFrameImpl:MoveToAssignedPos(pos, isAnimate)
  self._needUpdate = true
...
function VerticalTableFrameImpl:GetTotalLength()
  return self._totalLength
end

function VerticalTableFrameImpl:GetCurrentPosition()
  return self._currentPosition
end

function VerticalTableFrameImpl:ReloadCellsAtIndex(indexList, isAnimate)
  ...
  for _, index in pairs(indexList) do
    ...
    local width, height = self._viewport:GetRectSize()
    if height < self._totalLength + self._downMargin and self._currentPosition > self._totalLength - height + self._downMargin then
      self._currentPosition = self._totalLength - height + self._downMargin
    elseif height > self._totalLength then
      self._currentPosition = 0 - self._upMargin
    end
    self._needUpdate = true
  end
...
function VerticalTableFrameImpl:RemoveCellsAtIndex(indexList, isAnimate)
  if type(indexList) ~= "table" then
    LogError("VerticalTableFrameImpl", "please input a table value")
    return
  end
  ...
  for _, index in ipairs(indexList) do
    ...
    if height < self._totalLength and self._currentPosition > self._totalLength - height then
...
function VerticalTableFrameImpl:InsertCellsAtIndex(indexList, isAnimate)
...
function VerticalTableFrameImpl:GetTopIndex()
  return self._topIndex
end

function VerticalTableFrameImpl:GetDownIndex()
  return self._downIndex
end

function VerticalTableFrameImpl:SetMargin(upValue, downValue)
...
function VerticalTableFrameImpl:OnBeginDrag(args)
  self._dragDelta = 0
...
function VerticalTableFrameImpl:OnDrag(args)
...
function VerticalTableFrameImpl:OnEndDrag(args)
    ...
    if self._autoAlign then
      ...
      elseif 0 < dragDelta then
        ...
        elseif half > self._currentPosition + height then
          ...
          local cellNums = #self._logicCells
          if cellNums >= self._downIndex + 1 then
...
function VerticalTableFrameImpl:OnPointerDown(args)
  self._dragDelta = 0
...
function VerticalTableFrameImpl:OnPointerUp(args)
  self._sprintclickdown = false
  self:UpdateView(0)
end

function VerticalTableFrameImpl:OnLateUpdate(notification)
...
function VerticalTableFrameImpl:UpdateView(deltaTime)
  if self._needUpdate then
    local currentPosition = self._currentPosition
    local width, height = self._viewport:GetRectSize()
    ...
    local flag = true
    for i, logicCell in ipairs(self._logicCells) do
      if logicCell._pos + logicCell._size > currentPosition - self._upMargin and logicCell._pos < currentPosition + height + self._downMargin then
        if flag then
          flag = false
          self._downIndex = i
        end
...
function VerticalTableFrameImpl:GetCellDialog(logicCell)
  if logicCell._cell then
    return
  end
...
function VerticalTableFrameImpl:RecycleCell(logicCell)
  if logicCell._cell then
    if not self._recycleCells[logicCell._dialogName] then
      self._recycleCells[logicCell._dialogName] = {}
    end
...
function VerticalTableFrameImpl:SetSlide(slide, showSoftMask)
  if slide then
...
function VerticalTableFrameImpl:SetSoftMaskActive(showSoftMask)
  CS.PixelNeko.Lua.SoftMaskStaticFunctions.SetSoftMaskActive(self._viewport._uiObject, showSoftMask)
end

function VerticalTableFrameImpl:FireEvent(eventName, ...)
  for i, logicCell in ipairs(self._logicCells) do
...
function VerticalTableFrameImpl:RefreshUIParticleClipper()
  self._refreshUIParticleClipper = true
  CS.PixelNeko.Render.ShaderUtility.UIParticleClipper.RefreshUIParticleClipper(self._viewport:GetUIObject())
end

function VerticalTableFrameImpl:ClearAllPositionChangeTask()
  self._slideInertiaTask = nil
  self._sprintTask = nil
  self._moveTopTask = nil
  self._moveBottomTask = nil
