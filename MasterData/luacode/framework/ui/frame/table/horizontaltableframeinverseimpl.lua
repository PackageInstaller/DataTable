...
function HorizontalTableFrameInverseImpl:Ctor(interface, container, delegate, canSlide, autoAlign, viewportName)
...
function HorizontalTableFrameInverseImpl:Destroy()
...
function HorizontalTableFrameInverseImpl:ReloadAllCell()
  for i, logicCell in ipairs(self._logicCells) do
    if logicCell._cell then
      self:RecycleCell(logicCell)
    end
...
function HorizontalTableFrameInverseImpl:MoveToRight(isAnimate)
  self._needUpdate = true
...
function HorizontalTableFrameInverseImpl:MoveToLeft(isAnimate)
  self._needUpdate = true
  if isAnimate then
    local first = {
      pos = self._currentPosition
...
function HorizontalTableFrameInverseImpl:MoveRightToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveLeftIndexTask = nil
  self._slideInertiaTask = nil
...
function HorizontalTableFrameInverseImpl:MoveIndexToCentreInTime(desIndex, time)
  self._needUpdate = true
  self._moveRightIndexTask = nil
  self._slideInertiaTask = nil
...
function HorizontalTableFrameInverseImpl:MoveLeftToIndex(desIndex, isAnimate)
  self._needUpdate = true
  self._moveRightIndexTask = nil
  self._slideInertiaTask = nil
  if self._moveLeftIndexTask then
...
function HorizontalTableFrameInverseImpl:MoveToAssignedPos(pos, isAnimate)
  self._needUpdate = true
...
function HorizontalTableFrameInverseImpl:GetTotalLength()
  return self._totalLength
end

function HorizontalTableFrameInverseImpl:GetCurrentPosition()
  return self._currentPosition
end

function HorizontalTableFrameInverseImpl:ReloadCellsAtIndex(indexList, isAnimate)
  ...
  for _, index in pairs(indexList) do
    ...
    if delta > topDelta or width > self._totalLength then
    ...
    if curValue ~= latestValue then
      ...
      if width < self._totalLength + self._rightMargin and self._currentPosition > self._totalLength + self._rightMargin - width then
...
function HorizontalTableFrameInverseImpl:RemoveCellsAtIndex(indexList, isAnimate)
  if type(indexList) ~= "table" then
    LogError("HorizontalTableFrameInverseImpl", "please input a table value")
    return
  end
  ...
  for _, index in pairs(indexList) do
    ...
    if currentPosition < pos + gap and pos < currentPosition + width then
      if isAnimate then
        ...
        if gap > delta or width > self._totalLength then
...
function HorizontalTableFrameInverseImpl:InsertCellsAtIndex(indexList, isAnimate, dontMoveCurrentPosition)
...
function HorizontalTableFrameInverseImpl:FireEvent(eventName, ...)
  for i, logicCell in ipairs(self._logicCells) do
...
function HorizontalTableFrameInverseImpl:GetLeftIndex()
  return self._leftIndex
end

function HorizontalTableFrameInverseImpl:GetRightIndex()
  return self._rightIndex
end

function HorizontalTableFrameInverseImpl:SetMargin(leftValue, rightValue)
...
function HorizontalTableFrameInverseImpl:OnBeginDrag(args)
  self._dragDelta = 0
...
function HorizontalTableFrameInverseImpl:OnDrag(args)
...
function HorizontalTableFrameInverseImpl:OnEndDrag(args)
...
function HorizontalTableFrameInverseImpl:OnPointerDown(args)
  self._dragDelta = 0
...
function HorizontalTableFrameInverseImpl:OnPointerUp(args)
  self._sprintclickdown = false
  self:UpdateView(0)
end

function HorizontalTableFrameInverseImpl:OnLateUpdate(notification)
...
function HorizontalTableFrameInverseImpl:UpdateView(deltaTime)
  if self._needUpdate then
    local currentPosition = self._currentPosition
    local width, height = self._viewport:GetRectSize()
...
function HorizontalTableFrameInverseImpl:SetCellVisible(i, logicCell, up, down)
...
function HorizontalTableFrameInverseImpl:GetCellDialog(logicCell)
  if logicCell._cell then
    return
  end
...
function HorizontalTableFrameInverseImpl:RecycleCell(logicCell)
  if logicCell._cell then
    if not self._recycleCells[logicCell._dialogName] then
      self._recycleCells[logicCell._dialogName] = {}
    end
...
function HorizontalTableFrameInverseImpl:SetSlide(slide, showSoftMask)
  if slide then
...
function HorizontalTableFrameInverseImpl:SetSoftMaskActive(showSoftMask)
  CS.PixelNeko.Lua.SoftMaskStaticFunctions.SetSoftMaskActive(self._viewport._uiObject, showSoftMask)
end

function HorizontalTableFrameInverseImpl:RefreshUIParticleClipper()
  self._refreshUIParticleClipper = true
  CS.PixelNeko.Render.ShaderUtility.UIParticleClipper.RefreshUIParticleClipper(self._viewport:GetUIObject())
end

function HorizontalTableFrameInverseImpl:ClearAllPositionChangeTask()
  self._slideInertiaTask = nil
  self._sprintTask = nil
  self._moveLeftTask = nil
  self._moveRightTask = nil
