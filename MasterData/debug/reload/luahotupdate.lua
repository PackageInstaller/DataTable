-- chunkname: @E:/AntMan/AntMan_Android_pub1/Assets/Code/.LuaCode/Debug\\Reload\\luahotupdate.lua
...
function HU.InitFileMap(RootPath)
	for _, rootpath in pairs(RootPath) do
		...
		files = Framework.Tools.FileUtils.GetLuaFiles(rootpath)

		local len = files.Length

		for i = 0, len - 1 do
			local line = tostring(files[i])

			if line ~= nil then
				if HU.FileMap[line] == nil then
					HU.FileMap[line] = {}
				end
...
function HU.ReplaceOld(OldObject, NewObject, LuaPath, From, Deepth)
	if type(OldObject) == type(NewObject) then
		if type(NewObject) == "table" then
			HU.UpdateAllFunction(OldObject, NewObject, LuaPath, From, "", LuaPath)
...
function HU.HotUpdateCode(LuaPath, SysPath)
	...
	if OldObject ~= nil then
		...
		if Success then
			...
			HU.UpdateAllFunction(HU.ENV, HU.FakeENV, " ENV ", "Main", "", "ENV")
...
function HU.UpdateAllFunction(OldTable, NewTable, Name, From, Deepth, LuaPath)
	...
	if tostring(OldTable) == nil or tostring(NewTable) == nil then
		return
	end
	...
	for ElementName, Element in pairs(NewTable) do
		...
		if type(Element) == type(OldElement) then
			...
			elseif type(Element) == "table" then
				HU.UpdateAllFunction(OldElement, Element, ElementName, "HU.UpdateAllFunction", Deepth .. "    ", LuaPath)
			else
		...
		elseif OldElement == nil then
			...
			print("zh----- reload时增加了数据", ElementName, LuaPath)
		end
	end

	if LuaPath and string.find(LuaPath, "ClientData") then
		for ElementName, Element in pairs(OldTable) do
			local NewElement = NewTable[ElementName]

			if NewElement == nil then
				print("zh----- reload时删除了数据", ElementName, LuaPath)

				OldTable[ElementName] = nil
			end
		end
	end
	...
	if type(OldMeta) == "table" and type(NewMeta) == "table" then
		HU.UpdateAllFunction(OldMeta, NewMeta, Name .. "'s Meta", "HU.UpdateAllFunction", Deepth .. "    ", LuaPath)
	end
end
