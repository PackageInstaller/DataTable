module('fashionPermit.FashionPermitTaskPanel', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("fashionPermit/FashionPermitTaskPanel.prefab")
...
function configUI(self)
    ...
    self.mTaskScroller:SetItemRender(fashionPermit.FashionPermitTaskItem)
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_FASHIONPERMIT_TASK_PANEL,self.showPanel,self)
    self:showPanel()
end
...
function deActive(self)
    super.deActive(self)
    MoneyManager:setMoneyTidList({})
    GameDispatcher:removeEventListener(EventName.UPDATE_FASHIONPERMIT_TASK_PANEL,self.showPanel,self)
end
...
function showPanel(self)
    ...
    local canGet = fashionPermit.FashionPermitManager:canGetTask()
...
--    local taskList = fashionPermit.FashionPermitManager:getFashionPermitTaskData()
    GameDispatcher:dispatchEvent(EventName.REQ_GAIN_FASHION_PERMIT_TASK,{taskId = 0})
end
