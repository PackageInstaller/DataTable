module("arenaEntrance.ArenaHellAwardItem", Class.impl("lib.component.BaseItemRender"))
...
function setData(self, param)
    ...
    self:recoverAllGrid()
    local list = {}
    if self.data.type == arenaEntrance.ArenaEntaceAwadType.SEASON_AWARD then
        list = self.data:getAwardlist()
    elseif self.data.type == arenaEntrance.ArenaEntaceAwadType.WEEK_AWARD then
        list = self.data:getWeekAwardlist()
    else
    ...
    self.mGroupRank:SetActive(self.data.type ~= arenaEntrance.ArenaEntaceAwadType.SEASON_RANK)
    self.mGroupAward:SetActive(self.data.type == arenaEntrance.ArenaEntaceAwadType.SEASON_RANK)
    self.mTxtRank.gameObject:SetActive(true)
    self.mTxtInterval.gameObject:SetActive(self.data.type == arenaEntrance.ArenaEntaceAwadType.SEASON_RANK)
    if self.data.type == arenaEntrance.ArenaEntaceAwadType.SEASON_RANK then
