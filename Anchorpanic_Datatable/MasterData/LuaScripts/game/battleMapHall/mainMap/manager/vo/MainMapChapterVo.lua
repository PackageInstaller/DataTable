module("battleMap.MainMapChapterVo", Class.impl())
--[[ 
    主线关卡章节数据
]]
function ctor(self)
...
function parseData(self, cusId, cusData)
    ...
    -- table.sort(self.m_stageIdList, self.__sortStageIdList)
end
...
function getStageVoList(self, style)
    local styleType = style and style or battleMap.MainMapStyleType.Easy
    local stageVoList = {}
    for i = 1, #self.m_stageIdList do
        local stageId = self.m_stageIdList[i]
        local stageVo = battleMap.MainMapManager:getStageVo(stageId)
...
function getEndStageId(self, style)
...
function getStagePro(self, style)
    ...
    local passNum = 0
    for i = 1, totalNum do
        if (battleMap.MainMapManager:isStagePass(list[i].stageId)) then
            passNum = passNum + 1
        end
    end
