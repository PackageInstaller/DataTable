--[[ 
-----------------------------------------------------
@filename       : CycleCollectionTargetPanel
@Description    : 收藏品目标界面
@copyright      : (LY) 2020 雷焰网络
-----------------------------------------------------
]]
module("cycle.CycleCollectionTargetPanel", Class.impl(View))

UIRes = UrlManager:getUIPrefabPath("cycle/CycleCollectionTargetPanel.prefab")
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(cycle.CycleCollectionTargetItem)
end

function active(self, args)
    super.active(self)
    GameDispatcher:addEventListener(EventName.UPDATE_CYCLE_COLLECTION_AWARD_PANEL,self.updateView,self)
...
function deActive(self)
    super.deActive(self)
    self:removeAllDelay()
    GameDispatcher:removeEventListener(EventName.UPDATE_CYCLE_COLLECTION_AWARD_PANEL,self.updateView,self)
