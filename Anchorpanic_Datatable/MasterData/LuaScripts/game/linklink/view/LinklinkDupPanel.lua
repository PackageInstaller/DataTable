...
module('game.linklink.view.LinklinkDupPanel', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("linklink/LinklinkDupPanel.prefab")
...
function updateView(self, args, noShow)
    ...
    local starCount = linklink.LinklinkManager:getDupPassStar(self.m_DupConfigVo.tid)
    self:clearItem()
    self.mStarItemList = {}
    for i = 1, #self.m_DupConfigVo.time_limit do
        local starItem = SimpleInsItem:create(self.item_Score, self.listGroup, "LinklinkDupPanel_StarItem")
        self.mStarItemList[i] = starItem

        local isMeet = starCount >= i
        ...
        starItem:getChildGO("mTextDesc"):GetComponent(ty.Text).text = string.format("<color=#%s>%s</color>", color, _TT(139002, param))
    end
...
function onClickClose(self)
    ...
    GameDispatcher:dispatchEvent(EventName.ONCLOSE_LINKLINK_DUPPANEL)
end

function onFight(self)
    local activity_id = activity.ActivityId.Linklink
