--[[ 
-----------------------------------------------------
@filename       : CycleCollectionTargetItem
@Description    : 收藏品目标Item
@copyright      : (LY) 2020 雷焰网络
-----------------------------------------------------
]]
module("cycle.CycleCollectionTargetItem", Class.impl("lib.component.BaseItemRender"))
...
function setData(self, param)
    ...
    self.mImPro.fillAmount = count / self.mData.num
        if count >= self.mData.num then
            self.mTextPro.text = count .. "/" .. self.mData.num
        else
            self.mTextPro.text = "<color=#D53529>" .. count .. "</color>/" .. self.mData.num
        end
    ...
    self.mTextName.text = _TT(77852,self.mData.num)
...
function __onClickRecHandler(self)
    if self.mData.state == task.AwardRecState.CAN_REC then
        GameDispatcher:dispatchEvent(EventName.REQ_CYCLE_COLLECTION_AWARD,{id = self.mData.id})
    end
end
