...
function refreshSwitchCv(self, isByAutoRefresh)
    self:destroyCvDropDown()
    ...
    if isOpen then
        ...
        self.dropDown_switchCv:setOptionClickCallBack(function (index)
            ...
            if(not isByAutoRefresh)then
                if(curSelectIdx ~= index)then
                    web.RunHarmoniousStep2()
                    web.CheckHarmoniousResult()
                end
            else
                isByAutoRefresh = nil
            end
