...
module('ghost.GhostSettlePanel', Class.impl(View))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("ghost/GhostSettlePanel.prefab")
...
function active(self, args)
    ...
    self.m_DupConfigVo = ghost.GhostManager:getDupConfig(self.m_DupId)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_GHOST_PASS_DUP, self.updateView, self)
    if self.isPass then
        GameDispatcher:dispatchEvent(EventName.REQ_GHOST_PASS_DUP, {
            dupId = self.m_DupId,
            point = self.m_socre
        })
    end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_GHOST_PASS_DUP, self.updateView, self)
...
function onExit(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_GHOST_GAME_PANEL)
end
...
function onNextDup(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_GHOST_GAME_PANEL)
        ...
        if self.isPass then
            local next_dupId = ghost.GhostManager:getNextDupId(self.m_DupId)
            local nextDupConfig = ghost.GhostManager:getDupConfig(next_dupId)
            GameDispatcher:dispatchEvent(EventName.OPEN_GHOST_GAME_PANEL, {
                dupId = nextDupConfig.id
            })
            cusLog("2333333333333")
        else
            GameDispatcher:dispatchEvent(EventName.OPEN_GHOST_GAME_PANEL, {
...
function updateView(self)
    ...
    if next_dupId and self.isPass then
        local next_configVo = ghost.GhostManager:getDupConfig(next_dupId)
