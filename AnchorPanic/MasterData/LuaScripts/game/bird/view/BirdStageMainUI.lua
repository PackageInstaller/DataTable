-- @FileName:   BirdStageMainUI.lua
-- @Description:   大西瓜关卡界面
-- @Copyright:   (LY) 2023 雷焰网络

module('bird.BirdStageMainUI', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("bird/BirdStageMainUI.prefab")
...
function ctor(self)
    ...
    self:setBg("bird_bg.jpg",true,"bird")

    local name = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Bird):getName()
...
function onClickFuncTipsHandler(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BIRD_TIPS_VIEW)
...
function onClickTask(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BIRD_TASK_PANEL)
end

function onClickRank(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BIRD_RANK_PANEL)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_BIRD_STAGE_PANEL,self.showPanel,self)
    self:showPanel()
end
...
function showPanel(self)
    ...
    if area_id then
        local areaConfig = bird.BirdManager:getAreaConfig(area_id)
        ...
        if lastArea > 0 then
            if not bird.BirdManager:getAreaPassState(lastArea) then
                area_id = 1
            end
        end
    else
        local areaConfigDic = bird.BirdManager:getAreaConfigDic()
        for i = 1, #areaConfigDic do
            local timeOpen = areaConfigDic[i]:isOpen()
            local lastOpen = true
            if i > 1 then
                lastOpen = bird.BirdManager:getAreaPassState(areaConfigDic[i - 1].tid)
            end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_BIRD_STAGE_PANEL,self.showPanel,self)
...
function close(self)
    GameDispatcher:dispatchEvent(EventName.CLOSE_BIRD_DUP_PANEL)
...
function addAllEventListener(self)
    ...
    GameDispatcher:addEventListener(EventName.BIRD_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:addEventListener(EventName.UPDATE_BIRD_STAGE_PANEL, self.onDataRefresh, self)
end
...
function removeAllEventListener(self)
    ...
    GameDispatcher:removeEventListener(EventName.BIRD_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:removeEventListener(EventName.UPDATE_BIRD_STAGE_PANEL, self.onDataRefresh, self)
    

end

function onClickReward(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_BIRD_STAR_REWARD_PANEL)
end
...
function onSelectMapTab(self, area_id)
    ...
    for i = 1, length do
        ...
        local configVo = bird.BirdManager:getDupConfig(dup_id)
        ...
        --local star_count = bird.BirdManager:getDupPassStar(dup_id)
        for i = 1, 3 do
            dupItem:getChildGO("mStar_" .. i):SetActive(false)
        end

        local isPass = bird.BirdManager:getDupPassState(dup_id)
        ...
        if configVo.pre_id ~= 0 then
            lastPass = bird.BirdManager:getDupPassState(configVo.pre_id)
        end
        ...
        dupItem:addUIEvent("mRayPoint", function ()
            ...
            StorageUtil:saveBool1(gstor.BIRD_DUPNEWOPENSTR .. dup_id, true)
...
function onSelectDupItem(self, dupConfigVo)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_BIRD_DUP_PANEL)
    GameDispatcher:dispatchEvent(EventName.OPEN_BIRD_DUP_PANEL, dupConfigVo)
end
...
function refreshAreaRedPoint(self)
    local areaConfig = bird.BirdManager:getAreaConfigDic()
    for areaId, areaConfigVo in pairs(areaConfig) do
        local tabItem = self.m_tabItemList[areaId]
        if tabItem then
            if bird.BirdManager:getAreaShowRed(areaConfigVo) then
...
function refreshDupRedPoint(self)
    for i = 1, #self.m_areaConfig.stage_list do
        ...
        if dupItem then
            if bird.BirdManager:getDupShowRed(dup_id) then
                RedPointManager:add(dupItem:getChildTrans("mRayPoint"), nil, 110,18)
            else
