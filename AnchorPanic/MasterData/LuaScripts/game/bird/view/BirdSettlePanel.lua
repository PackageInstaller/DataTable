-- @FileName:   BirdSettlePanel.lua
-- @Description:   大西瓜结算界面
-- @Copyright:   (LY) 2023 雷焰网络
module('bird.BirdSettlePanel', Class.impl(View))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("bird/BirdSettlePanel.prefab")
...
function active(self, args)
    ...
    self.m_DupConfigVo = bird.BirdManager:getDupConfig(self.m_DupId)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_BIRD_PASS_DUP, self.updateView, self)
    if self.isPass then
        GameDispatcher:dispatchEvent(EventName.REQ_BIRD_PASS_DUP, {
            dupId = self.m_DupId,
            point = self.m_socre
        })
    end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_BIRD_PASS_DUP, self.updateView, self)
...
function onExit(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_BIRD_GAME_PANEL)
end
...
function onNextDup(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_BIRD_GAME_PANEL)
        ...
        if self.isPass then
            local next_dupId = bird.BirdManager:getNextDupId(self.m_DupId)
            local nextDupConfig = bird.BirdManager:getDupConfig(next_dupId)
...
function updateView(self)
    ...
    if next_dupId and self.isPass then
        local next_configVo = bird.BirdManager:getDupConfig(next_dupId)
