...
module("noviceActivity.NoviceActivityAccRechargeTwoItem", Class.impl('lib.component.BaseItemRender'))
--构造函数
function ctor(self)
    super.ctor(self)
    self.mAwardItemList = {}
...
function updateDataView(self)
    self:recoverAllGrid()
    local curNum= Celebration.CelebrationManager:getRechargeNumTwo()<self.data:getMaxNum() and Celebration.CelebrationManager:getRechargeNumTwo() or self.data:getMaxNum()
...
function onClickGetHandler(self)
    GameDispatcher:dispatchEvent(EventName.REQ_NOVICE_ACTIVITY_RECHARGE_RECEIVE_TWO, self.data.index) 
end
