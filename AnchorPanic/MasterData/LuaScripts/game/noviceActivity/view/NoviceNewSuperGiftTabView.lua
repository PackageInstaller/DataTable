--[[ SetActive
-----------------------------------------------------
@filename       : NoviceNewSuperGiftTabView
@Description      二周年超级礼物
@Author         : sxt
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------
]] module('noviceActivity.NoviceNewSuperGiftTabView', Class.impl(TabSubView))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("noviceActivity/NoviceNewSuperGiftTabView.prefab")
...
function onClickBuy(self)
    if self.isBuy then
        ...
        recharge.sendRecharge(recharge.RechargeType.GIFT_DIRECT_BUY, nil, recharge.rechargeDirectId.newSuperGift)
    end
...
function showPanel(self, isInit)
    ...
    self:clearItemList()
    self.isBuy = recharge.RechargeManager:getIsBuyNewSuperGift()
    ...
    for i = 1, #propsList, 1 do
        ...
        item:getChildGO("mImgIcon"):GetComponent(ty.AutoRefImage):SetImg(UrlManager:getPropsIconUrl(propsList[i].tid),
            false)

        item:getChildGO("mTxtName"):GetComponent(ty.Text).text = vo.name
        item:getChildGO("mTxtNum"):GetComponent(ty.Text).text = _TT(151018, num)
...
function updateTime(self)
    ...
    if self.rechargeVo and self.rechargeVo.end_time - clientTime > 0 then
