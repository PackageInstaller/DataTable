...
module("guildWar.GuildWarAwardTopTabView", Class.impl(TabSubView))

UIRes = UrlManager:getUIPrefabPath("guildWar/tab/GuildWarTopAwardTabView.prefab")
...
function configUI(self)
    ...
    self.mScrollReward:SetItemRender(guildWar.GuildWarTopAwardItem)
...
function updateRewardView(self)
    ...
    local seasonVo = guildWar.GuildWarManager:getGuildWarTopAwardData()
    local maxId = guildWar.GuildWarManager:getGuildWarTopAwardMaxId()
    local seasonId = guildWar.GuildWarManager:getGuildWarTopSeasonId()

    if seasonId > maxId then
        seasonId = maxId
    end

    local rewardList = {}
    for k, v in pairs(seasonVo) do
        if k == seasonId then
            rewardList = v:getSeasonData()
        end
    end
