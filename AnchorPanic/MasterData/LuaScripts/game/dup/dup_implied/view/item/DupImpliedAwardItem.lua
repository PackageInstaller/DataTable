module("dup.DupImpliedAwardItem", Class.impl("lib.component.BaseItemRender"))

function onInit(self, go)
    super.onInit(self, go)

	self.m_textRank = self:getChildGO("TextRank"):GetComponent(ty.Text)
    self.m_mask = self:getChildGO("ImgMask")
    self.m_curr = self:getChildGO("ImgCurr")
    self.m_groupAward = self:getChildGO("GroupItem").transform
    self.m_groupAward2 = self:getChildGO("GroupItem2").transform
