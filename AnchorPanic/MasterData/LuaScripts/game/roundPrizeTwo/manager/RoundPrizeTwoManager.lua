...
module('game.roundPrizeTwo.manager.RoundPrizeTwoManager', Class.impl(Manager))
...
function parseProbabilityConfigData(self)
    ...
    local baseData = RefMgr:getData("new_year_turntable_data2")
    for key, data in pairs(baseData) do
        local vo = roundPrizeTwo.RoundPrizeProbabilityTwoConfigVo.new()
...
function parseRewardConfigData(self)
    ...
    local baseData = RefMgr:getData("new_year_turntable_acc_data2")
    for key, data in pairs(baseData) do
