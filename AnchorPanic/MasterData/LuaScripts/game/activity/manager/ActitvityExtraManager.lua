...
function getIsLimitShopTypeParam(self, type, param)
    local list = {}
    if type == activity.LimitShopActivityType.View and
        funcopen.FuncOpenManager:isOpen(funcopen.FuncOpenConst.FUNC_ID_SHOP_LIMIT_GIFT, false) then
...
function parsenLimitShopGiftMsg(self, msg)
    if msg then
        ...
        if msg.is_new_unlock == 1 and newType then
            GameDispatcher:dispatchEvent(EventName.OPEN_LIMIT_SHOP_BUY_PANEL, {
                type = newType
            })
        end
...
function sendReceiveRequest(self, id)
    if id ~= nil then
        if self:getConfigVoById(id) ~= nil then
            ...
            if state == 1 then
                SOCKET_SEND(Protocol.CS_GAIN_CONCERN_GIFT, {
                    id = id
                })
            elseif state == 2 then
        ...
        if self:getConfigVoById(id) ~= nil then
            ...
            if self.subscribeMsgDic[id] then
                if self.subscribeMsgDic[id] == 0 then
                    SOCKET_SEND(Protocol.CS_CONCERN_GIFT_CAN_GAIN, {
                        id = id
                    })
                else
...
function getSelectGiftIsRedState(self)
    -- 每日奖励可领取
    local isCanGetEveryDayRewardState =
        not activity.ActivityManager:checkIsGotDailyReward(activity.ActivityId.SelectBuy)
...
function getBindIsRedState(self)
    if self.jointOptPlayer == nil then
        return false
    end
    if self:getBindApplyIsRedState() or self:getBindTaskIsRedState() then
        return true
    end
    return false
end

function getBindApplyIsRedState(self)
    if self.jointOptPlayer.player_id == "0" and #self:getBindApplyList() > 0 then
        return true
    end
    return false
end

function getBindTaskIsRedState(self)
    if self.type ~= 0 then
        if self.jointOptPlayer.player_id ~= "0" then
            for i = 1, #self.mJointTaskMsgList do
                if self.mJointTaskMsgList[i].state == 0 then
                    return true
                end
            end
            return false
...
function parseBindMsg(self, msg)
    self.showCode = msg.show_code
    self.type = msg.type -- 0-普通玩家,1-回归玩家,2-活跃玩家
    self.jointOptPlayer = msg.joint_ops_player -- 联结的玩家信息
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
    GameDispatcher:dispatchEvent(EventName.UPDATE_JOINT_BIND_INFO)
end

function getJointPlayerType(self)
    return self.type
end

function getJointPlayerInfo(self)
    return self.jointOptPlayer
end

function getJointShowCode(self)
    return self.showCode
end

function parseBindResult(self, msg)
    if msg.result == 1 then
        gs.Message.Show(_TT(41790))
    else
        if msg.result == 2 then
            gs.Message.Show(_TT(41791)) -- "已在其他服务器"+
        elseif msg.result == 3 then
            gs.Message.Show(_TT(41792))
        elseif msg.result == 4 then
            gs.Message.Show(_TT(90102))
        elseif msg.result == 5 then
            gs.Message.Show(_TT(90103))
        elseif msg.result == 6 then
            gs.Message.Show(_TT(90104))
        end
    end
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
    GameDispatcher:dispatchEvent(EventName.UPDATE_JOINT_BIND_INFO)
...
function setLastJointPlayerId(self, playerId)
    self.lastJointPlayerId = playerId
end

function parseBindAgreeResult(self, msg)
    if msg.result == 1 then
        gs.Message.Show(_TT(41794))
        GameDispatcher:dispatchEvent(EventName.CLOSE_JOINT_BIND_APPLY_LIST)
    elseif msg.result == 2 then
        gs.Message.Show(_TT(41791))
    elseif msg.result == 3 then
        gs.Message.Show(_TT(41792))
    elseif msg.result == 4 then
        gs.Message.Show(_TT(90102))
    ...
    elseif msg.result == 6 then
        gs.Message.Show(_TT(90104))
        -- for i = 1, #self.mApplyList, 1 do
        --     if self.mApplyList[i].player_id == self.lastJointPlayerId then
        --         table.remove(self.mApplyList, i)
        --     end
        -- end
    end
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
    GameDispatcher:dispatchEvent(EventName.UPDATE_JOINT_BIND_APPLY_LIST)
    GameDispatcher:dispatchEvent(EventName.UPDATE_JOINT_BIND_INFO)
end

function parseBindRejectResult(self, msg)
    if msg.result == 1 then
        gs.Message.Show(_TT(41795))
    elseif msg.result == 2 then
        gs.Message.Show(_TT(41791))
    elseif msg.result == 0 then
        gs.Message.Show(_TT(41796))
        return
    end
    -- for i = 1, #self.mApplyList, 1 do
    --     if self.mApplyList[i].player_id == self.lastJointPlayerId then
...
function parseBindApplyList(self, msg)
    self.mApplyList = msg.joint_ops_apply_list
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
    GameDispatcher:dispatchEvent(EventName.UPDATE_JOINT_BIND_INFO)
    --
end

function getBindApplyList(self)
    return self.mApplyList and self.mApplyList or {}
end

function parseJointTaskPanel(self, msg)
    self.mJointTaskMsgList = msg.task_list
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
    GameDispatcher:dispatchEvent(EventName.UPDATE_JOINT_BIND_TASK)
end

function parseJointTaskReceive(self, msg)
    if msg.result == 1 then
        for i = 1, #self.mJointTaskMsgList do
            if table.indexof01(msg.task_id_list, self.mJointTaskMsgList[i].id) > 0 then
                self.mJointTaskMsgList[i].state = 2
            end
        end
    end
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
...
function parseJointTaskUpdate(self, msg)
    for i = 1, #self.mJointTaskMsgList, 1 do
        if self.mJointTaskMsgList[i].id == msg.task_info.id then
            self.mJointTaskMsgList[i] = msg.task_info
            break
        end
    end
    GameDispatcher:dispatchEvent(EventName.UPDATE_ACTIVITY_RED)
...
function getTaskMsgDataById(self, id)
    for i = 1, #self.mJointTaskMsgList, 1 do
        if self.mJointTaskMsgList[i].id == id then
            return self.mJointTaskMsgList[i]
        end
    end
    return nil
end

function parseBindData(self)
    self.mBindData = {}
    local baseData = RefMgr:getData("joint_ops_task_data")
    for key, data in pairs(baseData) do
        local vo = LuaPoolMgr:poolGet(activity.ActivityBindDataVo)
        vo:parseData(key, data)
        table.insert(self.mBindData, vo)
    end
    -- table.sort(self.mBindData, function(vo1, vo2)
    --     return vo1.id < vo2.id
    -- end)
end

function getBindData(self)
    if self.mBindData == nil then
        self:parseBindData()
    end

    local taskList = {}

    for i = 1, #self.mBindData do
        if self.type == self.mBindData[i].taskParty then
            self.mBindData[i].msgData = self:getTaskMsgDataById(self.mBindData[i].id)
            table.insert(taskList, self.mBindData[i])
        end
    end
    table.sort(taskList, function(vo1, vo2)
        if vo1.msgData.state ~= vo2.msgData.state then
            return vo1.msgData.state < vo2.msgData.state
        else
            return vo1.id < vo2.id
        end
    end)
