module('game.activity.view.ActivityBindTabTipsView', Class.impl(View))

UIRes = UrlManager:getUIPrefabPath('purchase/ActivityBindTabTipsView.prefab')
...
function initViewText(self)
    self.m_textTitle.text = _TT(41783)--"请输入个性签名"
    self.m_defaultText.text = _TT(151013)--"请输入"
...
function active(self)
    ...
    self.mTxtTip.text = type == 1 and _TT(41784) or _TT(41785)
end
...
function __onClickConfirmHandler(self)
    ...
    --     return
    -- end
    GameDispatcher:dispatchEvent(EventName.REQ_BIND_PLAYER, { linkCode = content })
    self:onClose()
    --self.m_input.text = ""
    --self:onClose()
end
