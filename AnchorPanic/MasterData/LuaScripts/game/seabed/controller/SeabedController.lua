...
function listNotification(self)
    ...
    GameDispatcher:addEventListener(EventName.OPEN_SEABED_COLLECTION_AWARD_PANEL,self.onOpenSeabedCollectionAwardPanel,self)
...
function onUpdateSeabedRed(self,args)
    ...
    mainui.MainUIManager:setRedFlag(funcopen.FuncOpenConst.FUNC_ID_ADVENTURE,isFlag, funcopen.FuncOpenConst.FUNC_ID_SEADED)    
end
...
function onReqAbandonSeabed(self,args)
    SOCKET_SEND(Protocol.CS_SEABED_SETTLE, {
    },Protocol.SC_SEABED_SETTLE_INFO)
end
...
function onReqSeabedShopQuit(self,args)
    SOCKET_SEND(Protocol.CS_SEABED_LEAVE_SHOP, {
    },Protocol.SC_SEABED_UPDATE_CELL_INFO)
end

function onReqSeabedPostarBuff(self,args)
    SOCKET_SEND(Protocol.CS_SEABED_POSTWAR_REFRESH_BUFF, {
    },Protocol.SC_SEABED_UPDATE_CELL_INFO)
end
...
function onReqSeabedRankList(self,args)
    SOCKET_SEND(Protocol.CS_SEABED_RANK_PANEL, {
    },Protocol.SC_SEABED_RANK_PANEL)
end
...
function onOpenSeabedHeroSelectPanel(self,args)
    if self.mSeabedHeroSelectPanel == nil then
        local isEnd = seabed.SeabedManager:getOpenIsPass()
        self.mSeabedHeroSelectPanel = isEnd and seabed.SeabedHeroSelectPanel_end.new() or seabed.SeabedHeroSelectPanel.new()
...
function onOpenSeabedBattleBuffPanel(self,args)
    if self.mSeabedBattleBuffPanel == nil then
        local isEnd = seabed.SeabedManager:getOpenIsPass()
        self.mSeabedBattleBuffPanel = isEnd and seabed.SeabedBattleBuffPanel_end.new() or seabed.SeabedBattleBuffPanel.new()  
...
function onOpenSeabedBuffChangePanel(self,args)
    if self.mSeabedBuffChangePanel == nil then
        local isEnd = seabed.SeabedManager:getOpenIsPass()
        self.mSeabedBuffChangePanel = isEnd and seabed.SeabedBuffChangePanel_end.new() or seabed.SeabedBuffChangePanel.new()
...
function onOpenSeabedCollectionAwardPanel(self,args)
    if self.mSeabedCollectionAwardPanel == nil then
        ...
        self.mSeabedCollectionAwardPanel:addEventListener(View.EVENT_VIEW_DESTROY,self.onDestorySeabedCollectionAwardPanel,self)
    end
    self.mSeabedCollectionAwardPanel:open(args)
end

function onDestorySeabedCollectionAwardPanel(self)
    self.mSeabedCollectionAwardPanel:removeEventListener(View.EVENT_VIEW_DESTROY,self.onDestorySeabedCollectionAwardPanel,self)
