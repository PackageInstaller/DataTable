...
function setRecycleData(self,msg)
    self.recycleData = {}
    for i = 1, #msg.info_list, 1 do
        table.insert(self.recycleData, {tid = msg.info_list[i].item_tid, num = msg.info_list[i].item_num})
    end

    GameDispatcher:dispatchEvent(EventName.UPDATE_RECOVERY_PANEL)
end

function getRecycleData(self)
    return self.recycleData
end

function getRecyleDataNum(self)
    if self.recycleData then
        return #self.recycleData
    end
    return 0
end
