...
-----------------------------------------------------
]] module('game.favorable.view.item.FavorableStoryItem', Class.impl(BaseReuseItem))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("favorable/item/FavorableStoryItem.prefab")
...
function configUI(self)
    ...
    self.mBtnClickStory = self:getChildGO("mBtnClickStory")

end

function onClickStory(self)
    if self.isLock then
        gs.Message.Show(_TT(41757))
        return
    end

    if self:getIsGain() == false then
        GameDispatcher:dispatchEvent(EventName.REQ_HERO_STORY_REWARD,{tid = self.heroTid,lv = self.data.favorableLevel})
        return
    end
    --self.heroVo
    UIFactory:alertStoryMessge(_TT(41756,self.heroVo.name), self.heroVo.name,_TT(self.data.storyTitle),
        true, function()
            storyTalk.StoryTalkManager:setCurStoryID(self.data.storyId)
...
function addAllUIEvent(self)
    ...
    self:addUIEvent(self.mBtnClickStory, self.onClickStory)
end
...
function onRewardClick(self)
    GameDispatcher:dispatchEvent(EventName.REQ_HERO_STORY_REWARD, {
        tid = self.heroTid,
        lv = self.data.favorableLevel
    })
end
...
function setData(self, cusParent, cusData, cusHeroId, cusHeroTid)
    ...
    self.mTxtTitle.text = _TT(cusData.storyTitle)
    if self.propsGrid ~= nil then
        self.propsGrid:poolRecover()
        self.propsGrid = nil
    end

    -- if #cusData.storyReward> 1 then
    self.propsGrid = PropsGrid:create(self.mGroupAward, cusData.storyReward[1], 1)
    ...
    if (self.heroVo) then
        ...
        self:setLock(cusData.favorableLevel > self.favorableLevel)
    else
        self:setLock(true)
    end
...
function setSelect(self, isSelect)
    ...
    if self.isSelect then
        GameDispatcher:dispatchEvent(EventName.OPEN_HERO_DETAIL_DES_TIPS, {
            title = self.data:getTitle(),
            des = self.data:getDesc(),
            finishCall = function()
...
function setLock(self, isLock)
    ...
    if isLock then
        self.mTxtLock.text = _TT(41719, self.data.favorableLevel)
    else
    ...
    local curStr = string.gsub(_TT(self.data.storyDes), "\n", "　")
...
function getIsGain(self)
    return favorable.FavorableManager:getHeroStoryRewardGeted(self.heroTid, self.data.favorableLevel)
end
