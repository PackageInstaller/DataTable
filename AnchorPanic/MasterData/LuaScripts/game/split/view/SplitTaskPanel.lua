--[[
-----------------------------------------------------
@filename       : SplitTaskPanel
@Description    : 大西瓜任务界面
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
--
module("split.SplitTaskPanel", Class.impl(View))
UIRes = UrlManager:getUIPrefabPath("split/SplitTaskPanel.prefab")
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg", false, "split")
...
function configUI(self)
    ...
    self.mScroller:SetItemRender(split.SplitTaskItem)
...
function addAllUIEvent(self)
    self:addUIEvent(self:getChildGO("mBtnGetAll"), function()
        split.SplitManager:receiveAllTaskAward()
    end)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.SPLIT_TASK_UPDATE, self.setScrollerData, self)
    self:updateTime()
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.SPLIT_TASK_UPDATE, self.setScrollerData, self)
    --MoneyManager:setMoneyTidList()
...
function getMsgDataState(self,msgList,id)
...
function setScrollerData(self)
    if next(self.mTaskList) == nil then
        for _, vo in pairs(split.SplitManager:getTaskConfig()) do
            table.insert(self.mTaskList, vo)
        end
    end

    local msgList = split.SplitManager:getSplitTaskMasData()

    if msgList and next(msgList) then
        local listHelper = {}
        for _, msgVo in pairs(msgList) do
            if msgVo.state == 2 then
                local configVo = split.SplitManager:getTaskConfigVo(msgVo.id)
    ...
    table.sort(self.mTaskList, function (vo1, vo2)
        local vo1State = self:getMsgDataState(msgList,vo1.id)
        local vo2State = self:getMsgDataState(msgList,vo2.id)
...
function updateView(self)
    ...
    self.mAutoReceive:SetActive(split.SplitManager:checkTaskAwardCanReceive())
...
function updateTime(self)
    local mainActivityVo = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Split)
