...
module('dessert.DessertStarAwardView', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("dessert/DessertStarAwardView.prefab")

destroyTime = 0 -- 自动销毁时?1默认 0即时销毁999不销毁
panelType = 2 -- 窗口类型 1 全屏 2 弹窗 -1无底图弹窗
isShowBlackBg = 0 --是否显示全屏纯黑防穿帮底图
isBlur = 0 --是否开启模糊背景（?弹窗面板有效，默认开启，0关关闭
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(dessert.DessertStarAwardItem)
...
function active(self, map_id)
    super.active(self)
    GameDispatcher:addEventListener(EventName.UPDATE_DESSERT_STAGE_PANEL, self.udpateView, self)
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_DESSERT_STAGE_PANEL, self.udpateView, self)
end
...
function udpateView(self)
    local starCount = dessert.DessertManager:getPassAllStarCount()
    local rewardList =  dessert.DessertManager:getStarRewardConfig()

    self.canGetList = {}
    local data = {}
    for i = 1, #rewardList do
        local isGet =  dessert.DessertManager:getAwardState(rewardList[i].id)
...
function onGetAll(self)
    -- logAll("请求全部领取")
    GameDispatcher:dispatchEvent(EventName.REQ_DESSERT_RECEIVE_STAR, self.canGetList)
end
