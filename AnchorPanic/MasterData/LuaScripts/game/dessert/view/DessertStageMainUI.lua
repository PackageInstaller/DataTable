-- @FileName:   DessertStageMainUI.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2023-07-21 10:59:29
-- @Copyright:   (LY) 2023 雷焰网络

module('dessert.DessertStageMainUI', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("dessert/DessertStageMainUI.prefab")

destroyTime = 0 -- 自动销毁时?1默认 0即时销毁999不销毁
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg",true,"dessert")

    --self:setTxtTitle(_TT(151221))
    self:setUICode(LinkCode.Dessert)
    local name = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Dessert):getName()
...
function onClickFuncTipsHandler(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DESSERT_TIPS_VIEW)
...
function onClickTask(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DESSERT_TASK_PANEL)
end

function onClickRank(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DESSERT_RANK_PANEL)
end
...
function active(self, args)
    ...
    --GameDispatcher:addEventListener(EventName.UPDATE_DESSART_STAGE_PANEL,self.showPanel,self)
    self:showPanel()
end
...
function showPanel(self)
    ...
    if area_id then
        local areaConfig = dessert.DessertManager:getAreaConfig(area_id)
        ...
        if lastArea > 0 then
            if not dessert.DessertManager:getAreaPassState(lastArea) then
                area_id = 1
            end
        end
    else
        local areaConfigDic = dessert.DessertManager:getAreaConfigDic()
        for i = 1, #areaConfigDic do
            local timeOpen = areaConfigDic[i]:isOpen()
            local lastOpen = true
            if i > 1 then
                lastOpen = dessert.DessertManager:getAreaPassState(areaConfigDic[i - 1].tid)
            end
...
function deActive(self)
    super.deActive(self)
    --GameDispatcher:removeEventListener(EventName.UPDATE_DESSART_STAGE_PANEL,self.showPanel,self)
...
function close(self)
    GameDispatcher:dispatchEvent(EventName.CLOSE_DESSERT_DUP_PANEL)
...
function addAllEventListener(self)
    ...
    GameDispatcher:addEventListener(EventName.DESSERT_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:addEventListener(EventName.UPDATE_DESSERT_STAGE_PANEL, self.onDataRefresh, self)
end
...
function removeAllEventListener(self)
    ...
    GameDispatcher:removeEventListener(EventName.DESSERT_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:removeEventListener(EventName.UPDATE_DESSERT_STAGE_PANEL, self.onDataRefresh, self)
    

end

function onClickReward(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DESSERT_STAR_REWARD_PANEL)
end
...
function onSelectMapTab(self, area_id)
    ...
    for i = 1, length do
        ...
        local configVo = dessert.DessertManager:getDupConfig(dup_id)
        ...
        --local star_count = dessert.DessertManager:getDupPassStar(dup_id)
        for i = 1, 3 do
            dupItem:getChildGO("mStar_" .. i):SetActive(false)
        end

        local isPass = dessert.DessertManager:getDupPassState(dup_id)
        ...
        if configVo.pre_id ~= 0 then
            lastPass = dessert.DessertManager:getDupPassState(configVo.pre_id)
        end
        ...
        dupItem:addUIEvent("mRayPoint", function ()
            ...
            StorageUtil:saveBool1(gstor.DESSERT_DUPNEWOPENSTR .. dup_id, true)
...
function onSelectDupItem(self, dupConfigVo)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_DESSERT_DUP_PANEL)
    GameDispatcher:dispatchEvent(EventName.OPEN_DESSERT_DUP_PANEL, dupConfigVo)
end
...
function refreshAreaRedPoint(self)
    local areaConfig = dessert.DessertManager:getAreaConfigDic()
    for areaId, areaConfigVo in pairs(areaConfig) do
        local tabItem = self.m_tabItemList[areaId]
        if tabItem then
            if dessert.DessertManager:getAreaShowRed(areaConfigVo) then
...
function refreshDupRedPoint(self)
    for i = 1, #self.m_areaConfig.stage_list do
        ...
        if dupItem then
            if dessert.DessertManager:getDupShowRed(dup_id) then
...
function refreshStarAwardRedPoint(self)
    if dessert.DessertManager:getTaskRed() then
        RedPointManager:add(self.mBtnTask.transform, nil, 30,30)
    else
        RedPointManager:remove(self.mBtnTask.transform)
    end
    --self.ImEffect:SetActive(dessert.DessertManager:getStarAwardRedState())
end
