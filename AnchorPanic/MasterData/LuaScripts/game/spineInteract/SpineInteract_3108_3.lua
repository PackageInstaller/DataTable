...
function setup(self, go, modelId)
    ...
    GameDispatcher:addEventListener(EventName.HIDE_MAINUI_EVENT, self.showGuideEffect, self)
end
...
-- 添加引导特效
function showGuideEffect(self, isSetup)
    local showTime = isSetup and 6 or 2
    local isFirst = role.RoleManager:getMainUISpineIsFirstShow(self.modelId)
    if isSetup and isFirst == 1 then
        return
    end
    ...
    self.effTimerSn = LoopManager:addTimer(showTime, 1, self, self.removeGuideEffect)
end
...
function destroy(self)
    GameDispatcher:removeEventListener(EventName.HIDE_MAINUI_EVENT, self.showGuideEffect, self)
