-- @FileName:   DropStageMainUI.lua
-- @Description:   大西瓜关卡界面
-- @Copyright:   (LY) 2023 雷焰网络
-- 活动关卡主界面，按区域展示关卡节点、解锁状态、红点、任务与排行榜入口

module('drop.DropStageMainUI', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("drop/DropStageMainUI.prefab")
...
function ctor(self)
    ...
    self:setBg("stage_01.jpg",true,"drop")
    self:setUICode(LinkCode.Drop)
    local name = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Drop):getName()
...
function configUI(self)
    ...
    --self.mImgBg:setSprite(UrlManager:getSprite("drop/bg_01.jpg"))
...
function onClickFuncTipsHandler(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DROP_TIPS_VIEW)
...
function onClickTask(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DROP_TASK_PANEL)
end

function onClickRank(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DROP_RANK_PANEL)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_DROP_STAGE_PANEL,self.showPanel,self)
    self:showPanel()
end
...
function showPanel(self)
    ...
    if area_id then
        local areaConfig = drop.DropManager:getAreaConfig(area_id)
        ...
        if lastArea > 0 then
            if not drop.DropManager:getAreaPassState(lastArea) then
                area_id = 1
            end
        end
    else
        local areaConfigDic = drop.DropManager:getAreaConfigDic()
        for i = 1, #areaConfigDic do
            local timeOpen = areaConfigDic[i]:isOpen()
            local lastOpen = true
            if i > 1 then
                lastOpen = drop.DropManager:getAreaPassState(areaConfigDic[i - 1].tid)
            end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_DROP_STAGE_PANEL,self.showPanel,self)
    ...
    --GameDispatcher:dispatchEvent(EventName.CLOSE_DROP_DUP_PANEL)
end
...
function addAllEventListener(self)
    ...
    GameDispatcher:addEventListener(EventName.DROP_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:addEventListener(EventName.UPDATE_DROP_STAGE_PANEL, self.onDataRefresh, self)
end
...
function removeAllEventListener(self)
    ...
    GameDispatcher:removeEventListener(EventName.DROP_TASK_UPDATE, self.onDataRefresh, self)
    GameDispatcher:removeEventListener(EventName.UPDATE_DROP_STAGE_PANEL, self.onDataRefresh, self)
    

end

function onClickReward(self)
    GameDispatcher:dispatchEvent(EventName.OPEN_DROP_STAR_REWARD_PANEL)
end
...
function creatTabList(self)
    ...
    self:clearTabItem()
    for i = 1, #areaConfigDic do
        local tabItem = SimpleInsItem:create(self.mItemTab, self.mTabGroup, "drop_stageMainUI_tabItem")
...
function onSelectMapTab(self, area_id)
    ...
    self.mImgBg:SetImg(UrlManager:getBgPath("drop/stage_0".. (self.m_select_areaId)..".jpg"))
    --self:setBg("stage_0".. (self.m_select_areaId)..".jpg",true,"drop")
    ...
    for i = 1, length do
        ...
        local dupItem = SimpleInsItem:create(self.mDupItem, self.list, "drop_stageMainUI_dupItem")
        self.m_dupItemList[dup_id] = dupItem

        local configVo = drop.DropManager:getDupConfig(dup_id)
        ...
        --local star_count = drop.DropManager:getDupPassStar(dup_id)
        for i = 1, 3 do
            dupItem:getChildGO("mStar_" .. i):SetActive(false)
        end

        local isPass = drop.DropManager:getDupPassState(dup_id)
        ...
        if configVo.pre_id ~= 0 then
            lastPass = drop.DropManager:getDupPassState(configVo.pre_id)
        end
        ...
        dupItem:addUIEvent("mRayPoint", function ()
            ...
            StorageUtil:saveBool1(gstor.DROP_DUPNEWOPENSTR .. dup_id, true)
    ...
    --gs.LayoutRedroper.ForceRedropLayoutImmediate(self.list)
...
function onSelectDupItem(self, dupConfigVo)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_DROP_DUP_PANEL)
    GameDispatcher:dispatchEvent(EventName.OPEN_DROP_DUP_PANEL, dupConfigVo)
end
...
function refreshAreaRedPoint(self)
    local areaConfig = drop.DropManager:getAreaConfigDic()
    for areaId, areaConfigVo in pairs(areaConfig) do
        local tabItem = self.m_tabItemList[areaId]
        if tabItem then
            if drop.DropManager:getAreaShowRed(areaConfigVo) then
...
function refreshDupRedPoint(self)
    for i = 1, #self.m_areaConfig.stage_list do
        ...
        if dupItem then
            if drop.DropManager:getDupShowRed(dup_id) then
