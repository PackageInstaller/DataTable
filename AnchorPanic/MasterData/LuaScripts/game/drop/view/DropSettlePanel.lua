-- @FileName:   DropSettlePanel.lua
-- @Description:   大西瓜结算界面
-- @Copyright:   (LY) 2023 雷焰网络
-- 玩法结算弹窗，展示胜负和首通奖励，并根据通关状态进入下一关或重玩
module('drop.DropSettlePanel', Class.impl(View))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("drop/DropSettlePanel.prefab")
...
function active(self, args)
    ...
    self.m_DupConfigVo = drop.DropManager:getDupConfig(self.m_DupId)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_DROP_PASS_DUP, self.updateView, self)
    if self.isPass then
        GameDispatcher:dispatchEvent(EventName.REQ_DROP_PASS_DUP, {
            dupId = self.m_DupId,
            point = self.m_socre
        })
    end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_DROP_PASS_DUP, self.updateView, self)
...
function onExit(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_DROP_GAME_PANEL)
end
...
function onNextDup(self)
    ...
    if self.isPass then
        local next_dupId = drop.DropManager:getNextDupId(self.m_DupId)
        local nextDupConfig = drop.DropManager:getDupConfig(next_dupId)
...
function updateView(self)
    ...
    if next_dupId and self.isPass then
        local next_configVo = drop.DropManager:getDupConfig(next_dupId)
