...
function ctor(self)
    ...
    local correctCall = function(self, webData, jsonObj)
        ...
        web.CheckServerHotUpdateSuperState(nil, function()
            web.CheckLocalHotUpdateSuperState(nil, function() 
                self:start()
            end)
        end)
    end
    local errorCall = function(self, errorData, jsonObj)
        correctCall(self, errorData, nil)
    end
    -- 直接把首次预热请求后再显示游戏了
    ...
    local function correctCall(self, webData, jsonObj)
        ...
        if (jsonObj == nil) then
            ...
            if (subCode == web.RECOMMEND_SUB_CODE.NORMAL) then
                ...
                print("WebController", string.format(_TT(10000280)--[["网络异常请重试！提示码：%s，%s"]], web.TIP_CODE.CDN_ERROR, subCode))
                UIFactory:alertOK0(_TT(10000235)--[["网络提示"]], string.format(_TT(10000280)--[["网络异常请重试！提示码：%s，%s"]], web.TIP_CODE.CDN_ERROR, subCode),
                function()
                    WebInterfaceUtil:postAsyncLoop(url, parasmDic, correctCall, function(self, errorData, jsonObj) correctCall(self, errorData, nil) end, self, tryCount)
                end)
            end
    ...
    local function correctCall(self, webData, jsonObj)
        ...
        if (jsonObj == nil) then
            ...
            if (subCode == web.RECOMMEND_SUB_CODE.NORMAL) then
                ...
                if (loginCallBack) then
                    loginCallBack(false)
                    print("WebController", string.format(_TT(10000280)--[["网络异常请重试！提示码：%s，%s"]], web.TIP_CODE.LOGIN_RECOMMEND_ERROR, subCode))
                    UIFactory:alertOK0(_TT(10000235)--[["网络提示"]], string.format(_TT(10000280)--[["网络异常请重试！提示码：%s，%s"]], web.TIP_CODE.LOGIN_RECOMMEND_ERROR, subCode), 
                    function()
                        WebInterfaceUtil:postAsyncLoop(url, parasmDic, correctCall, function(self, errorData, jsonObj) correctCall(self, errorData, nil) end, self, tryCount) 
                    end)
                else
                    print("WebController", string.format(_TT(10000280)--[["网络异常请重试！提示码：%s，%s"]], web.TIP_CODE.RECOMMEND_ERROR, subCode))
                    UIFactory:alertOK0(_TT(10000235)--[["网络提示"]], string.format(_TT(10000280)--[["网络异常请重试！提示码：%s，%s"]], web.TIP_CODE.RECOMMEND_ERROR, subCode), 
                    function()
                        WebInterfaceUtil:postAsyncLoop(url, parasmDic, correctCall, function(self, errorData, jsonObj) correctCall(self, errorData, nil) end, self, tryCount) 
                    end)
                end
...
function reqRecharge(self, itemId, money, moneyType, itemTitle, itemName, itemDes, successFun, failFun)
    if(sdk.ChannelData:isChannel(sdk.ChannelData.WINDOWS_ZHTW))then
        sdk.SdkManager:pay("", "", "", 0, "", "", "", false)
    else
                ...
                self.sdkRechargeCallBack = function(self, rechargeData)
                    ...
                    if(rechargeData ~= nil and rechargeData.isSuccess ~= nil and tonumber(rechargeData.isSuccess) == 1)then
                        if(successFun)then
                            ...
                            -- 飞行quick诞诞游那边无法给支付结果，飞行说统一这类不提示充值结果
                            if(not sdk.ChannelData:isChannelDDY())then
                                gs.Message.Show(_TT(10000281)--[["充值成功"]])
                            end
                        end
                        logInfo("充值成功", "SdkManager")
                        sdk.SdkManager:foreignNotifyRechargeSuc(itemId)
                    else
                        if(failFun)then
                            failFun()
                        else
