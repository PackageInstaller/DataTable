module("build.BuildGameWorld", Class.impl(Manager))

-- 建造小游戏简易碰撞世界，维护注册物体并在每帧计算主运动物体与场景物体的碰撞关系
-- 构造函数
function ctor(self)
    super.ctor(self)
    self:__initData()
...
function registerToTheWorld(self, key, level, obj, colType, selfGroup, colGroup, motion, scale, enterCallback,
    ...
    local worldVo = LuaPoolMgr:poolGet(build.BuildWorldVo)
...
function onUpdate(self)
    ...
    if self.datas[build.Id.MoveWorldId] == nil then
        return
    end 
    self.colliderDic = {}
    if self.datas[build.Id.MoveWorldId].motion then
        self.datas[build.Id.MoveWorldId]:updateWordlInfo()
    end
    for key, worldVo in pairs(self.datas) do
        if key ~= build.Id.MoveWorldId then
                --if worldVo.motion then
                    worldVo:updateWordlInfo()
                --end
        
                local result = self.datas[build.Id.MoveWorldId]:compare(worldVo)
      
              
                if result then
                    if self.colliderDic[build.Id.MoveWorldId] == nil then
                        self.colliderDic[build.Id.MoveWorldId] = {self.datas[build.Id.MoveWorldId]}
                    end
                    table.insert(self.colliderDic[build.Id.MoveWorldId], worldVo)
                end
        end
    end
    ...
    --     if self.worldData[i].colGroup == build.BuildGroup.None then
    --         break
    --     end
