-- @FileName:   BuildSettlePanel.lua
-- @Description:   大西瓜结算界面
-- @Copyright:   (LY) 2023 雷焰网络
-- 玩法结算弹窗，展示胜负和首通奖励，并根据通关状态进入下一关或重玩
module('build.BuildSettlePanel', Class.impl(View))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("build/BuildSettlePanel.prefab")
...
function active(self, args)
    ...
    self.m_DupConfigVo = build.BuildManager:getDupConfig(self.m_DupId)

    self.mTxtWin.text = self.isWin and "游戏胜利" or "游戏失败"
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_BUILD_PASS_DUP, self.updateView, self)
    if self.isPass then
        GameDispatcher:dispatchEvent(EventName.REQ_BUILD_PASS_DUP, {
            dupId = self.m_DupId,
            point = self.m_socre
        })
    end
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_BUILD_PASS_DUP, self.updateView, self)
...
function onExit(self)
    ...
    GameDispatcher:dispatchEvent(EventName.CLOSE_BUILD_GAME_PANEL)
end
...
function onNextDup(self)
    ...
    if self.isPass then
        local next_dupId = build.BuildManager:getNextDupId(self.m_DupId)
        local nextDupConfig = build.BuildManager:getDupConfig(next_dupId)
...
function updateView(self)
    ...
    if next_dupId and self.isPass then
        local next_configVo = build.BuildManager:getDupConfig(next_dupId)
