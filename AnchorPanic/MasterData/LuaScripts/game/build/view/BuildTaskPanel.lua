--[[
-----------------------------------------------------
@filename       : BuildTaskPanel
@Description    : 大西瓜任务界面
@copyright      : (LY) 2023 雷焰网络
-----------------------------------------------------]]
--
module("build.BuildTaskPanel", Class.impl(View))
-- 建造小游戏任务界面，展示任务倒计时、任务列表、进度排序和一键领取入口
UIRes = UrlManager:getUIPrefabPath("build/BuildTaskPanel.prefab")
...
function ctor(self)
    ...
    self:setBg("bg_01.jpg", false, "build")
...
function configUI(self)
    ...
    self.mScroller:SetItemRender(build.BuildTaskItem)
...
function addAllUIEvent(self)
    self:addUIEvent(self:getChildGO("mBtnGetAll"), function()
        build.BuildManager:receiveAllTaskAward()
    end)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.BUILD_TASK_UPDATE, self.setScrollerData, self)
    self:updateTime()
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.BUILD_TASK_UPDATE, self.setScrollerData, self)
    --MoneyManager:setMoneyTidList()
...
function getMegDataState(self,msgList,id)
...
function setScrollerData(self)
    if next(self.mTaskList) == nil then
        for _, vo in pairs(build.BuildManager:getTaskConfig()) do
            table.insert(self.mTaskList, vo)
        end
    end

    local msgList = build.BuildManager:getBuildTaskMasData()

    if msgList and next(msgList) then
        local listHelper = {}
        for _, msgVo in pairs(msgList) do
            if msgVo.state == 2 then
                local configVo = build.BuildManager:getTaskConfigVo(msgVo.id)
    ...
    table.sort(self.mTaskList, function (vo1, vo2)
        local vo1State = self:getMegDataState(msgList, vo1.id) 
        local vo2State = self:getMegDataState(msgList, vo2.id) 
...
function updateView(self)
    ...
    self.mAutoReceive:SetActive(build.BuildManager:checkTaskAwardCanReceive())
...
-- 每秒刷新活动剩余时间文本，使用主活动管理器中的 Build 活动时间
function updateTime(self)
    local mainActivityVo = mainActivity.MainActivityManager:getMainActivityVoById(activity.ActivityId.Build)
