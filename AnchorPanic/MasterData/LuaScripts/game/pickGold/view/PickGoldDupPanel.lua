-- @FileName:   PickGoldDupPanel.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2023-07-21 10:59:29
-- @Copyright:   (LY) 2023 雷焰网络

module('game.pickGold.view.PickGoldDupPanel', Class.impl(View))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("pickGold/PickGoldDupPanel.prefab")
...
function updateView(self, args, noShow)
    ...
    self.mTextCurScore.text = _TT(151204) .. pickGold.PickGoldManager:getPassStageScore(self.m_DupConfigVo.id)

    local isGet = pickGold.PickGoldManager:isPassDup(self.m_DupConfigVo.id)
...
function onClickClose(self)
    ...
    GameDispatcher:dispatchEvent(EventName.ONCLOSE_PICKGOLD_DUPPANEL)
end

function onFight(self)
    local activity_id = activity.ActivityId.PickGold
