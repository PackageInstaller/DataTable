--[[ 
-----------------------------------------------------
@filename       : HeroLvlTargetPanel
@Description    : 战员等级目标界面
@date           : 2022-2-18 13:44:00
@Author         : Zzz
@copyright      : (LY) 2020 雷焰网络
-----------------------------------------------------
]]
module("hero.HeroLvlTargetPanel", Class.impl(View))

UIRes = UrlManager:getUIPrefabPath("hero/HeroLvlTargetPanel.prefab")
...
function configUI(self)
    ...
    self.m_scroller:SetItemRender(hero.HeroLvlTargetItem)
end
...
function onHeroLvlTargetUpdateHandler(self, args)
    ...
    if (not args or heroVo.tid == args.heroTid) then
        self:updateView()
    end
end
