...
function addAllEventListener(self)
    GameDispatcher:addEventListener(EventName.CIRUIT_CLOSEDUPINFOPANEL, self.onDupInfoPanelClose, self)
    GameDispatcher:addEventListener(EventName.CIRUIT_OPENSCENEUI, self.close, self)

end

function removeAllEventListener(self)
    GameDispatcher:removeEventListener(EventName.CIRUIT_CLOSEDUPINFOPANEL, self.onDupInfoPanelClose, self)
...
function creatTabList(self)
    ...
    for i = 1, #areaConfigDic do
        ...
        table.insert(self.m_tabItemList, tabItem)
...
function onSelectMapTab(self, area_id)
    ...
    for i = 1, length do
        ...
        table.insert(self.m_dupItemList, dupItem)

        local configVo = ciruit.CiruitManager:getDupConfig(self.m_areaConfig.stage_list[i])
