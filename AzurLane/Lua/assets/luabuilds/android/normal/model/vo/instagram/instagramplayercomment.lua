class = var_0_10000

local var_0_0 = "InstagramPlayerComment"

import = var_0_10002

local var_0_1 = var_0_10000(var_0_0, var_0_10002(".InstagramComment"))

function var_0_1.Ctor(arg_1_0, arg_1_1, arg_1_2, arg_1_3, arg_1_4)
	var_0_1.super.Ctor(arg_1_0, arg_1_1, arg_1_2, arg_1_3, arg_1_4)

	local var_1_0 = arg_1_0.level + 1

	type = var_6

	if var_6(arg_1_1.npc_reply) == "table" then
		ipairs = var_6

		for iter_1_0, iter_1_1 in var_6(arg_1_1.npc_reply) do
			InstagramNpcComment = var_1_10011
			var_1_10011 = var_1_10011.New(arg_1_0.allReply[iter_1_1], arg_1_2, var_1_0, arg_1_0)
			table = var_12

			var_12.insert(arg_1_0.replyList, var_1_10011)
		end
	elseif arg_1_1.npc_reply ~= 0 then
		InstagramNpcComment = var_6

		local var_1_1 = var_6.New(arg_1_0.allReply[arg_1_1.npc_reply], arg_1_2, var_1_0, arg_1_0)

		table = var_7

		var_7.insert(arg_1_0.replyList, var_1_1)
	end

	return
end

function var_0_1.GetName(arg_2_0)
	getProxy = var_1_10001
	PlayerProxy = var_1_10002

	local var_2_0 = var_1_10001(var_1_10002)

	return var_1.getData(var_2_0).name
end

function var_0_1.GetPainting(arg_3_0)
	return "ui/InstagramUI_atlas", "txdi_3"
end

function var_0_1.GetType(arg_4_0)
	Instagram = var_1_10001

	return var_1_10001.TYPE_PLAYER_COMMENT
end

return var_0_1
