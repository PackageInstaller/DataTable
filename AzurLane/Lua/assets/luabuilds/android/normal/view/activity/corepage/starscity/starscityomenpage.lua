class = var_0_10000

local var_0_0 = "StarsCityOmenPage"

import = var_0_10002

local var_0_1 = var_0_10000(var_0_0, var_0_10002("view.activity.CorePage.CoreLoginTemplatePage"))

function var_0_1.OnInit(arg_1_0)
	local var_1_0 = arg_1_0._tf

	arg_1_0.bg = var_1.Find(var_1_0, "bg")

	local var_1_1 = arg_1_0.bg

	arg_1_0.dayTF = var_1.Find(var_1_1, "total_progress/day")

	local var_1_2 = arg_1_0.bg

	arg_1_0.maxDayTF = var_1.Find(var_1_2, "total_progress/max_day")

	local var_1_3 = arg_1_0.bg

	arg_1_0.item = var_1.Find(var_1_3, "item")

	local var_1_4 = arg_1_0.bg

	arg_1_0.items = var_1.Find(var_1_4, "items")
	UIItemList = var_1
	arg_1_0.uilist = var_1.New(arg_1_0.items, arg_1_0.item)

	local var_1_5 = arg_1_0.bg

	arg_1_0.btnDetail = var_1.Find(var_1_5, "btn_detail")

	local var_1_6 = arg_1_0.btnDetail

	arg_1_0.txtDetail = var_1.Find(var_1_6, "detail")

	local var_1_7 = arg_1_0.bg

	arg_1_0.ruleTxt = var_1.Find(var_1_7, "rule_get")

	local var_1_8 = arg_1_0.bg

	arg_1_0.btnStory = var_1.Find(var_1_8, "btn_story")
	arg_1_0.scenario = arg_1_0:GetOutPostScenarioPage().New(arg_1_0._tf, arg_1_0.event)

	local var_1_9 = arg_1_0.scenario

	var_1.SetCoreStoryPage(var_1_9, arg_1_0)

	local var_1_10 = arg_1_0.scenario

	var_1.RegisterView(var_1_10, arg_1_0.coreActivityUI)

	StarsCityOmenTaskWindow = var_1
	arg_1_0.taskWindow = var_1.New(arg_1_0._tf, arg_1_0.event)
	setActive = var_1

	var_1(arg_1_0.item, false)

	local var_1_11 = arg_1_0.bg

	arg_1_0.progressLabel = var_1.Find(var_1_11, "total_progress/label")
	setText = var_1

	local var_1_12 = arg_1_0.progressLabel

	i18n = var_3

	var_1(var_1_12, var_3("Outpost_20250904_Progress"))

	setText = var_1

	local var_1_13 = arg_1_0.txtDetail

	i18n = var_3

	var_1(var_1_13, var_3("Outpost_20260514_Detail"))

	setText = var_1

	local var_1_14 = arg_1_0.ruleTxt

	i18n = var_3

	var_1(var_1_14, var_3("Outpost_20260806_rule"))

	return
end

function var_0_1.GetOutPostScenarioPage(arg_2_0)
	OutPostScenarioPage_260806 = var_1_10001

	return var_1_10001
end

function var_0_1.OnDataSetting(arg_3_0)
	arg_3_0.nday = 0
	getProxy = var_1
	TaskProxy = var_1_10002
	arg_3_0.taskProxy = var_1(var_1_10002)

	local var_3_0 = arg_3_0.activity

	arg_3_0.taskGroup = var_1.getConfig(var_3_0, "config_client").unlock_task
	updateActivityTaskStatus = var_1

	return var_1(arg_3_0.activity)
end

function var_0_1.OnFirstFlush(arg_4_0)
	var_0_1.super.OnFirstFlush(arg_4_0)

	setActive = var_1

	var_1(arg_4_0.maxTF, #arg_4_0.taskGroup)

	onButton = var_1

	local var_4_0 = arg_4_0
	local var_4_1 = arg_4_0.btnDetail

	local function var_4_2()
		local var_5_0 = arg_4_0.taskWindow

		var_0.ExecuteAction(var_5_0, "Show", arg_4_0.activity)

		return
	end

	SFX_PANEL = var_1_10005

	var_1(var_4_0, var_4_1, var_4_2, var_1_10005)

	onButton = var_1

	local var_4_3 = arg_4_0
	local var_4_4 = arg_4_0.btnStory

	local function var_4_5()
		if arg_4_0.scenario then
			local var_6_0 = arg_4_0.scenario

			var_0.Load(var_6_0)

			local var_6_1 = arg_4_0.scenario

			var_0.SetActivity(var_6_1, arg_4_0.activity)

			local var_6_2 = arg_4_0.scenario

			var_0.UpdateStoryTask(var_6_2)

			local var_6_3 = arg_4_0.scenario

			var_0.ActionInvoke(var_6_3, "UpdateView")

			local var_6_4 = arg_4_0

			var_0.ShowScenarioLayer(var_6_4, true)
		end

		return
	end

	SFX_PANEL = var_1_10005

	var_1(var_4_3, var_4_4, var_4_5, var_1_10005)

	return
end

function var_0_1.OnShowFlush(arg_7_0)
	var_0_1.super.OnShowFlush(arg_7_0)

	return
end

function var_0_1.ShowScenarioLayer(arg_8_0, arg_8_1)
	if arg_8_1 then
		local var_8_0 = arg_8_0.coreActivityUI

		var_2.ActiveScenarioLayer(var_8_0, true)

		if arg_8_0.scenario then
			local var_8_1 = arg_8_0.scenario

			var_2.ActionInvoke(var_8_1, "Show")
		end
	else
		if arg_8_0.scenario then
			local var_8_2 = arg_8_0.scenario

			var_2.Hide(var_8_2)
		end

		local var_8_3 = arg_8_0.coreActivityUI

		var_2.ActiveScenarioLayer(var_8_3, false)
	end

	return
end

function var_0_1.IsShowingPopWindow(arg_9_0)
	if arg_9_0.scenario then
		local var_9_0 = arg_9_0.scenario

		return var_1.isShowing(var_9_0)
	end

	return
end

function var_0_1.ClosePopWindow(arg_10_0)
	if arg_10_0.scenario then
		local var_10_0 = arg_10_0.scenario

		var_1.Hide(var_10_0)
		arg_10_0:ShowScenarioLayer(false)
	end

	return
end

function var_0_1.OnUpdateFlush(arg_11_0)
	local var_11_0 = #arg_11_0.taskGroup

	arg_11_0.nday = arg_11_0:getTaskIdx(arg_11_0.activity)

	arg_11_0:PlayStory()

	if arg_11_0.dayTF then
		setText = var_2

		var_2(arg_11_0.dayTF, "DAY " .. arg_11_0.nday)

		setText = var_2

		var_2(arg_11_0.maxDayTF, "/" .. var_11_0)
	end

	local var_11_1 = arg_11_0.uilist

	var_2.align(var_11_1, #arg_11_0.taskGroup[arg_11_0.nday])

	local var_11_2 = arg_11_0.taskWindow

	if var_2.isShowing(var_11_2) then
		local var_11_3 = arg_11_0.taskWindow

		var_2.ExecuteAction(var_11_3, "Show", arg_11_0.activity)
	end

	return
end

function var_0_1.UpdateTask(arg_12_0, arg_12_1, arg_12_2)
	local var_12_0 = arg_12_1 + 1
	local var_12_1 = arg_12_2:Find("item")
	local var_12_2 = arg_12_0.taskGroup[arg_12_0.nday][var_12_0]
	local var_12_3 = arg_12_0.taskProxy
	local var_12_4

	if not var_6.getTaskById(var_12_3, var_12_2) then
		var_12_3 = arg_12_0.taskProxy
		var_12_4 = var_6.getFinishTaskById(var_12_3, var_12_2)
	end

	assert = var_12_3

	var_12_3(var_12_4, "without this task by id: " .. var_12_2)

	Drop = var_12_3

	local var_12_5 = var_12_3.Create(var_12_4:getConfig("award_display")[1])

	updateDrop = var_8

	var_8(var_12_1, var_12_5)

	onButton = var_8

	local var_12_6 = arg_12_0
	local var_12_7 = var_12_1

	local function var_12_8()
		local var_13_0 = arg_12_0
		local var_13_1 = var_0.emit

		BaseUI = var_2_10002

		var_13_1(var_13_0, var_2_10002.ON_DROP, var_12_5)

		return
	end

	SFX_PANEL = var_1_10012

	var_8(var_12_6, var_12_7, var_12_8, var_1_10012)

	local var_12_9 = var_12_4:getProgress()
	local var_12_10 = var_12_4
	local var_12_11 = var_12_4.getConfig(var_12_10, "target_num")

	setScrollText = var_12_10

	var_12_10(arg_12_2:Find("mask/description"), var_12_4:getConfig("desc"))

	local var_12_12, var_12_13 = arg_12_0:GetProgressColor()

	if var_12_12 then
		setColorStr = var_12

		local var_12_14

		if not var_12(var_12_9, var_12_12) then
			::label_12_0::

			var_12_14 = var_12_9
		end

		if var_12_13 then
			setColorStr = var_12

			local var_12_15

			if not var_12("/" .. var_12_11, var_12_13) then
				::label_12_1::

				var_12_15 = "/" .. var_12_11
			end

			setText = var_12

			var_12(arg_12_2:Find("progressText"), var_12_14 .. var_12_15)

			setSlider = var_12

			var_12(arg_12_2:Find("progress"), 0, var_12_11, var_12_9)

			local var_12_16 = arg_12_2:Find("go_btn")
			local var_12_17 = arg_12_2:Find("get_btn")
			local var_12_18 = arg_12_2:Find("got_btn")

			arg_12_0:SetBtnLocal(arg_12_2)

			local var_12_19 = var_12_4
			local var_12_20 = var_12_4.getTaskStatus(var_12_19)

			setActive = var_12_19

			var_12_19(var_12_16, var_12_20 == 0)

			setActive = var_12_19

			var_12_19(var_12_17, var_12_20 == 1)

			setActive = var_12_19

			var_12_19(var_12_18, var_12_20 == 2)

			onButton = var_12_19

			local var_12_21 = arg_12_0
			local var_12_22 = var_12_16

			local function var_12_23()
				local var_14_0 = arg_12_0
				local var_14_1 = var_0.emit

				ActivityMediator = var_2_10002

				var_14_1(var_14_0, var_2_10002.ON_TASK_GO, var_12_4)

				return
			end

			SFX_PANEL = var_1_10020

			var_12_19(var_12_21, var_12_22, var_12_23, var_1_10020)

			onButton = var_12_19

			local var_12_24 = arg_12_0
			local var_12_25 = var_12_17

			local function var_12_26()
				local var_15_0 = {}
				local var_15_1 = var_12_4
				local var_15_2 = var_1.getConfig(var_15_1, "award_display")

				getProxy = var_15_1
				PlayerProxy = var_3

				local var_15_3 = var_15_1(var_3)
				local var_15_4 = var_2.getRawData(var_15_3)

				pg = var_15_3

				local var_15_5 = var_15_3.gameset.urpt_chapter_max.description[1]

				LOCK_UR_SHIP = var_2_10005

				if var_2_10005 then
					var_2_10005 = 0
				else
					getProxy = var_2_10005
					BagProxy = var_2_10006
					var_2_10006 = var_2_10005(var_2_10006)
					var_2_10005 = var_2_10005.GetLimitCntById(var_2_10006, var_15_5)
				end

				Task = var_2_10006

				local var_15_6, var_15_7 = var_2_10006.StaticJudgeOverflow(var_15_4.gold, var_15_4.oil, var_2_10005, true, true, var_15_2)

				if var_15_6 then
					table = var_8

					var_8.insert(var_15_0, function(arg_16_0)
						pg = var_3_10001

						local var_16_0 = var_3_10001.MsgboxMgr.GetInstance()
						local var_16_1 = var_1.ShowMsgBox
						local var_16_2 = {}

						MSGBOX_TYPE_ITEM_BOX = var_3_10004
						var_16_2.type = var_3_10004
						i18n = var_3_10004
						var_16_2.content = var_3_10004("award_max_warning")
						var_16_2.items = var_15_7
						var_16_2.onYes = arg_16_0

						var_16_1(var_16_0, var_16_2)

						return
					end)
				end

				seriesAsync = var_8

				var_8(var_15_0, function()
					local var_17_0 = arg_12_0
					local var_17_1 = var_0.emit

					ActivityMediator = var_3_10002

					var_17_1(var_17_0, var_3_10002.ON_TASK_SUBMIT, var_12_4)

					return
				end)

				return
			end

			SFX_PANEL = var_1_10020

			var_12_19(var_12_24, var_12_25, var_12_26, var_1_10020)

			return
		end
	end
end

function var_0_1.getTaskIdx(arg_18_0, arg_18_1)
	local var_18_0 = 1
	local var_18_1 = arg_18_1:getNDay()
	local var_18_2 = #arg_18_0.taskGroup

	math = var_1_10005

	local var_18_3 = var_1_10005.min(var_18_1, var_18_2)
	local var_18_4 = true

	for iter_18_0 = 1, var_18_3 do
		if not var_18_4 then
			break
		end

		var_18_0 = iter_18_0

		if iter_18_0 < var_18_3 then
			ipairs = var_1_10011

			for iter_18_1, iter_18_2 in var_1_10011(arg_18_0.taskGroup[iter_18_0]) do
				if not arg_18_0:isTaskFinished(iter_18_2) then
					var_18_4 = false

					break
				end
			end
		end
	end

	math = var_7

	return var_7.min(var_18_0, var_18_2)
end

function var_0_1.isTaskFinished(arg_19_0, arg_19_1)
	if not arg_19_0.taskProxy then
		getProxy = var_2
		TaskProxy = var_1_10003
		arg_19_0.taskProxy = var_2(var_1_10003)
	end

	local var_19_0 = arg_19_0.taskProxy
	local var_19_2

	if not var_2.getTaskById(var_19_0, arg_19_1) then
		local var_19_1 = arg_19_0.taskProxy

		var_19_2 = var_2.getFinishTaskById(var_19_1, arg_19_1)
	end

	return var_19_2 and var_19_2:getTaskStatus() == 2
end

function var_0_1.GetProgressColor(arg_20_0)
	return "#FFFFFF", "#C3C3C3"
end

function var_0_1.SetBtnLocal(arg_21_0, arg_21_1)
	local var_21_0 = arg_21_1:Find("get_btn")
	local var_21_1 = arg_21_1:Find("go_btn")
	local var_21_2 = arg_21_1
	local var_21_3 = arg_21_1.Find(var_21_2, "got_btn")

	setText = var_21_2

	local var_21_4 = var_21_0
	local var_21_5 = var_21_0.Find(var_21_4, "Text")

	i18n = var_21_4

	var_21_2(var_21_5, var_21_4("LiquorFloorTaskUI_get"))

	setText = var_21_2

	local var_21_6 = var_21_1
	local var_21_7 = var_21_1.Find(var_21_6, "Text")

	i18n = var_21_6

	var_21_2(var_21_7, var_21_6("LiquorFloorTaskUI_go"))

	setText = var_21_2

	local var_21_8 = var_21_3
	local var_21_9 = var_21_3.Find(var_21_8, "Text")

	i18n = var_21_8

	var_21_2(var_21_9, var_21_8("LiquorFloorTaskUI_got"))

	return
end

function var_0_1.OnHideFlush(arg_22_0)
	local var_22_0 = arg_22_0.taskWindow

	if var_1.isShowing(var_22_0) then
		local var_22_1 = arg_22_0.taskWindow

		var_1.Hide(var_22_1)
	end

	return
end

function var_0_1.OnDestroy(arg_23_0)
	if arg_23_0.taskWindow then
		local var_23_0 = arg_23_0.taskWindow

		var_1.Hide(var_23_0)

		local var_23_1 = arg_23_0.taskWindow

		var_1.Destroy(var_23_1)

		arg_23_0.taskWindow = nil
	end

	if arg_23_0.scenario then
		local var_23_2 = arg_23_0.scenario

		if var_1.isShowing(var_23_2) then
			local var_23_3 = arg_23_0.scenario

			var_1.Hide(var_23_3)
		end

		local var_23_4 = arg_23_0.scenario

		var_1.Destroy(var_23_4)
	end

	return
end

return var_0_1
