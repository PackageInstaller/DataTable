...
local function OnCreate(self)
	...
	-- 初始化其它基本信息  and #self.__var_arg > 0
...
-- 激活、反激活
local MoveNameTable = {} --需要移动动画的界面
local PopNameTable = {} --需要弹窗动画的弹窗

local delayTime = 0.25;

local function SetPopWindowScale(self,transform)
	if table.any(PopNameTable,function(v) return self.__name == v  end) then
		UIPublic.SetPopWindowScale(transform);
	end
end

local function SetUIMove(self,rectTransform)
	if  table.any(MoveNameTable,function(v) return self.__name == v  end)
			and not table.any(PopNameTable,function(v) return self.__name == v  end)  then
		rectTransform.localPosition = Vector2.New(1920,0);
		DoTweenManager.DOAnchorPos3D(rectTransform,Vector2.New(0,0),delayTime,nil);
	else
		SetPopWindowScale(self,rectTransform.transform);
	end
end
...
local function GetActive(self)
	return not IsNull(self.gameObject) and self.gameObject.activeSelf
end
