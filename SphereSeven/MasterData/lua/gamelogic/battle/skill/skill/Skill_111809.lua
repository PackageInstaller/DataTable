---
--- 炎爪
--- 对血量最高的前排敌人造成&{Params[1]}%物理伤害，解除自身一个弱化状态并击退其&{Params[2]}%的行动值。当并非在战斗中第一次使用，且攻击前自身的异常状态数量不多于&{Params[3]}个时，对同一列后排造成等量伤害。
---

local Skill_111809 = BaseClass("Skill_111809", Skill)
local base = Skill

local function __init(self)
    self.Data.UseTimes = 0
    self.Data.AtkBehind = false
end
...
local function OnAtkStart(self)
    ...
    self.Data.AtkBehind = self.__sbc:BuffTypeCount(true) <= self.Params[3]
...
local function AfterOneTargetAllHit(self, targetUnit, activeSkillResult)
    ...
    if self.__bc.CurActSkill ~= self or self.Data.AtkBehind then
        return
    end
...
Skill_111809.__init = __init
Skill_111809.OnAtkStart = OnAtkStart
Skill_111809.OnUseActiveSkill = OnUseActiveSkill
Skill_111809.AfterOneTargetAllHit = AfterOneTargetAllHit
