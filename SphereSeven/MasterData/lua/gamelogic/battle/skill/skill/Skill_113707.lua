---
--- 闪电吐息
--- 对全体敌人造成&{Params[1]}%法术伤害，攻击后赋予自身&{Params[2]}回合【攻击力提升】，【物防提升】。当攻击前自身处于强化状态不小于&{Params[3]}个时，获得&{Params[4]}%额外倍率，並擊退全體敵人&{Params[5]}%的行動值
---

local Skill_113707 = BaseClass("Skill_113707", Skill)
local base = Skill

Skill_113707.SkillLightningBreath = true
...
local function __init(self)
    ...
    self.ExtraActValueReduce = self.Params[5]
end
...
local function OnUseActiveSkill(self, targetUnit)
    ...
    if buffCount >= self.ExtraDamageMultiplierBuffCount then
        ...
        result.ActChange = result.ActChange - self.ExtraActValueReduce
    end
...
Skill_113707.__init = __init
Skill_113707.OnAtkStart = OnAtkStart
Skill_113707.BeforeSkillEffect = BeforeSkillEffect
Skill_113707.OnUseActiveSkill = OnUseActiveSkill
Skill_113707.OnAtkEnd = OnAtkEnd
