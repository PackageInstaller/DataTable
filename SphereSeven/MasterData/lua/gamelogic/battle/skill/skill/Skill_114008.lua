...
---  *怒火：计数用特殊状态。
---  免疫【沉默】，【遗忘】，【嘲讽】。
---  处于【晕眩】，【睡眠】，【冻结】，【石化】，【麻痹】状态时，速度降低${Params[3]}%，不会无法行动。"

local Skill_114008 = BaseClass("Skill_114008", Skill)
local base = Skill
...
local function OnBuffAdd(self, buff)
    ...
    if self.Data.reduceSpeed == 1 then
        self.__snc:Dec(NumericType.SpeedPct, self.Params[3])
    end
end
...
local function OnBuffRemove(self, buff)
    ...
    if self.Data.reduceSpeed == 0 then
        self.__snc:Inc(NumericType.SpeedPct, self.Params[3])
    end
end

Skill_114008.__init = __init
Skill_114008.OnPassiveSkillEnabled = OnPassiveSkillEnabled
Skill_114008.OnPassiveSkillDisabled = OnPassiveSkillDisabled
Skill_114008.OnBattleStart = OnBattleStart
Skill_114008.AfterHurt = AfterHurt
Skill_114008.OnUnitSelfTurnStart = OnUnitSelfTurnStart
Skill_114008.OnUnitSelfTurnEnd = OnUnitSelfTurnEnd
Skill_114008.OnBuffAdd = OnBuffAdd
Skill_114008.OnBuffRemove = OnBuffRemove
