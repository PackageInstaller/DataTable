---
--- 永不停歇
--- "每次受到攻击伤害时，获得${Params[1]}层【怒火】。当【怒火】达到${Params[2]}层时，立即进行一次反击并清除${Params[3]}层怒火。当自己在任意回合中没有受到伤害时，在该回合结束时减少${Params[4]}层【怒火】。
--- 每有一层怒火，攻击力+${Params[5]}%。
---  *怒火：计数用特殊状态。
---  免疫【沉默】，【遗忘】，【嘲讽】。
---  处于【晕眩】，【睡眠】，【冻结】，【石化】，【麻痹】状态时，速度降低${Params[6]}%，不会无法行动。"

local Skill_114016 = BaseClass("Skill_114016", Skill)
local base = Skill
...
local function OnUnitSelfTurnEnd(self)
    ...
    if not self.Data.thisTurnBeHurt then
        ...
        if buff ~= nil then
            self.__sbc:RemoveBuff(BattleBuffType.Anger, self.SkillId, self.Params[4])
        end
    end
end
...
local function OnBuffAdd(self, buff)
    ...
    if buff.BuffId == BattleBuffType.Anger then
        self.__snc:Inc(NumericType.AtkPct, self.Params[5] * buff.NumericVal)
        return
    end
...
local function OnBuffRemove(self, buff)
    ...
    if buff.BuffId == BattleBuffType.Anger then
        self.__snc:Dec(NumericType.AtkPct, self.Params[5] * buff.NumericVal)
        return
    end
...
local function OnBuffMultiply(self, buff, val, data)
    ...
    if val > 0 then
        self.__snc:Inc(NumericType.AtkPct, self.Params[5] * val)
    else
        self.__snc:Dec(NumericType.AtkPct, self.Params[5] * val)
    end
end

Skill_114016.__init = __init
Skill_114016.OnPassiveSkillEnabled = OnPassiveSkillEnabled
Skill_114016.OnPassiveSkillDisabled = OnPassiveSkillDisabled
Skill_114016.OnBattleStart = OnBattleStart
Skill_114016.AfterHurt = AfterHurt
Skill_114016.OnUnitSelfTurnStart = OnUnitSelfTurnStart
Skill_114016.OnUnitSelfTurnEnd = OnUnitSelfTurnEnd
Skill_114016.OnBuffAdd = OnBuffAdd
Skill_114016.OnBuffRemove = OnBuffRemove
Skill_114016.OnBuffMultiply = OnBuffMultiply
