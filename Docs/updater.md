...
# 指定包/分组
python AetherGazer.py assets --output ./AetherGazer --only newbie,advance,permanent
python AetherGazer.py assets --output ./AetherGazer --only assetpend_common_chapter_2

# 下载 + MD5 校验
python AetherGazer.py assets --output ./AetherGazer --verify

# 强制重新全量
python AetherGazer.py assets --output ./AetherGazer --force-full

# 只要数据表资产 + Lua 并解析到 MasterData/（不下载整包资源）
python AetherGazer.py masterdata

# 资产 + 数据表一次跑完
python AetherGazer.py all --output ./AetherGazer
```
...
```bash
# 中日一起（默认）
python AetherGazer.py assets --output ./AetherGazer --only voice
# 只中文
python AetherGazer.py assets --output ./AetherGazer --only voice --voice-lang zh
# 只日文
python AetherGazer.py assets --output ./AetherGazer --only voice --voice-lang ja
```
