...
local CATCA_CAT_AWARD_ITEM_TEMP_KEY = "CATCA_CAT_AWARD_ITEM_TEMP_KEY"

function ui:ui_finish_load()
  self:set_button("BtnJump", function()
    UIMgr:get_ui("catch_cat_stage_panel"):ui_show(self.v_activity_id)
  end)
  self.v_desc_txt = self.v_uiobjects.ActivityDesc:GetComponent(TextMeshProType)
  self:register_exist_auto_template(CATCA_CAT_AWARD_ITEM_TEMP_KEY, self.v_uiobjects.ItemObjCom1, self.v_uiobjects.Content)
end

function ui:ui_on_show(activity_id, direct_jump)
  self.v_activity_id = activity_id
  self.v_activity_cfg = ShareRes.get_activity_cfg(activity_id)
  local task_cfg = ShareRes.create("activity.kitten_escape_task")[activity_id]
  if not task_cfg then
    Log.Error("围住小猫任务表无对应id配置：", activity_id)
    return
  end
  self.v_award_group = task_cfg.AwardGroupId
  self.v_desc_txt.text = task_cfg.Desc
  self:update_info()
  ...
  RedPointMgr:bind_redpoint(self, self.v_uiobjects.RedPoint, RedEnum.CATCA_CAT_ACT_ENTRY)
  if direct_jump then
    UIMgr:get_ui("catch_cat_stage_panel"):ui_show(self.v_activity_id)
  end
end
...
function ui:refresh_award()
  ...
  self:give_back_auto_cache(CATCA_CAT_AWARD_ITEM_TEMP_KEY)
  self:clear_wrap_award()
  self.v_item_list = {}
  local awards = {}
  ShareRes.get_item_obj_use_award_list(self.v_award_group, awards)
  for _, award_data in ipairs(awards) do
    local item_obj = self:get_auto_cache(CATCA_CAT_AWARD_ITEM_TEMP_KEY)
