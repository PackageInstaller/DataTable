...
local TEMPLATE_KEY_ATTR = "CHAR_PUZZLE_TEMPLATE_KEY_ATTR_PLAYER"
local TEMPLATE_KEY_EFFECT = "CHAR_PUZZLE_TEMPLATE_KEY_EFFECT_PLAYER"
local ImageType = TypeUnityUIImage
local ButtonType = TypeUnityUIButton
...
function ui:on_point_click_at_map()
  ...
  local node_data = closest_node_idx and self.v_map_node_idx_to_puzzle_item_data[closest_node_idx]
  if node_data then
    Util.show_puzzle_tip(-1, {
...
function ui:refresh_map_nodes_occupy()
  ...
  for idx, node_data in ipairs(puzzle_place_infos) do
    ...
    for _, relative_hex_pos in ipairs(graph_hex_pos_list) do
      ...
      hex_to_puzzle_id[real_hex_x][real_hex_y] = node_data
    end
  end
  self.v_map_node_idx_to_puzzle_item_data = {}
  for idx, node_hex_pos in ipairs(self.v_map_hex_pos_list) do
    local node_data = hex_to_puzzle_id[node_hex_pos[1]] and hex_to_puzzle_id[node_hex_pos[1]][node_hex_pos[2]]
    if node_data then
      self.v_map_node_idx_to_puzzle_item_data[idx] = node_data
    end
  end
end
