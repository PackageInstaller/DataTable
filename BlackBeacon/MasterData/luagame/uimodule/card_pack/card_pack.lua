...
local CARD_PACK_ITEM_KEY = "CARD_PACK_ITEM_KEY"
...
function ui:_on_item_update(msg)
  if msg.mm_x == TreasureChestCfg.TREASURE_CHEST_TYPE then
    self:_refresh()
  end
end
...
function ui:_refresh()
  self.v_data_list = TreasureChestMgr:get_treasure_chest_list_ex()
...
function ui:_refresh_data()
  ...
  self:give_back_auto_cache(CARD_PACK_ITEM_KEY)
  local empty_count = 0
  if self.v_data_list and next(self.v_data_list) ~= nil then
    ...
    for i = 1, 3 do
      ...
      local obj = self:get_auto_cache(CARD_PACK_ITEM_KEY)
...
function ui:_init_view()
  ...
  self:register_exist_auto_template(CARD_PACK_ITEM_KEY, self.v_uiobjects.CardPileTem, self.v_uiobjects.CardPile)
