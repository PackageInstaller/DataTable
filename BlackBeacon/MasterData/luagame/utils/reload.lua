...
do
  ...
  local dummy_cache, dummy_module_cache
...
local function enum_object(value)
  ...
  local function iterate(value)
    if sandbox.isdummy(value) then
      if print then
        print("ENUM", value, table.concat(path, "."))
      end
      table.insert(all, {
        value,
        table.unpack(path)
...
local function match_objects(objects, old_module, map, globals)
  ...
  for _, item in ipairs(objects) do
    ...
    if sandbox.isdummy(obj) then
      ...
      map[obj] = old_one
      if print then
        print("MATCH", old_one, table.unpack(item, 2))
      end
    end
  end
end
    ...
    if data.old_module then
      ...
      for new_one, old_one in pairs(map) do
        if type(new_one) == "table" and old_one then
          ...
          for k, v in pairs(new_one) do
            if type(v) ~= "table" or getmetatable(v) ~= nil or nil == old_one[k] then
              old_one[k] = v
            end
          end
        end
      ...
      for _, item in ipairs(data.objects) do
        ...
        if not sandbox.isdummy(v) and not map[v] then
          ...
          if print then
            print("MOVE", mod_name, table.concat(item, ".", 2), ok)
          end
        end
      end
    else
...
local function slove_globals(all)
  ...
  for mod_name, data in pairs(all) do
    for gk, item in pairs(data.globals) do
      ...
      if invalid then
        if print then
          print("GLOBAL INVALID", path)
        end
        data.globals[gk] = nil
      elseif not unsloved then
        i = i + 1
        if print then
          print("GLOBAL", path, value)
        end
