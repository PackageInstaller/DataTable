...
for _, lib in pairs({
  ...
  for k, v in pairs(t) do
    if "function" == type(v) and not pcall(string.dump, v) then
      c_functions[v] = lib .. k
    end
  end
end
...
local function DataDumper(value, varname, fastmode, ident, postprocess)
  ...
  if fastmode then
    ...
    function fcts.table(value, ident, path)
    ...
    fcts["function"] = function(value, ident, path)
      ...
      elseif debug == nil or nil == debug.getupvalue(value, 1) then
        return string_format("loadstring(%q)", string_dump(value))
      end
