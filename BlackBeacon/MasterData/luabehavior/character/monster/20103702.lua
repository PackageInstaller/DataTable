...
function M:skill_main_logic()
  ...
  if 0 == self.power_2 and 1 == CommonMonster.get_value("二阶段狂暴", 1) then
    cast_magic(self.npc, self.npc, 20104101001, 0)
    cast_magic(self.npc, self.npc, 20104101002, 0)
    self.power_2 = 1
  end
  if is_cast_skill_time(self.npc) then
    return
  end
  if not check_npc_distance(self.npc, self.target, self.skill01[4], false) and check_npc_distance(self.npc, self.target, self.skill01[5], false) and now_time >= self.skill01[3] and 1 ~= CommonMonster.get_value("二阶段狂暴", 1) then
  ...
  elseif not check_npc_distance(self.npc, self.target, self.skill02[4], false) and check_npc_distance(self.npc, self.target, self.skill02[5], false) and now_time >= self.skill02[3] and 1 == CommonMonster.get_value("二阶段狂暴", 1) then
    ...
    self.skill02[3] = now_time + self.skill02[2] + CommonMonster.set_value("群组攻击影响时间", 2, self.skill02[2] + 1)
    self.control = 99
  elseif not check_npc_distance(self.npc, self.target, self.skill03[4], false) and check_npc_distance(self.npc, self.target, self.skill03[5], false) and now_time >= self.skill03[3] and 1 == CommonMonster.get_value("二阶段狂暴", 1) then
    ...
    self.skill03[3] = now_time + self.skill03[2] + CommonMonster.set_value("群组攻击影响时间", 2, self.skill03[2] + 1)
    self.control = 99
  end
end
