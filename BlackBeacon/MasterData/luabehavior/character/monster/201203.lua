...
function M:on_before_npc_hp_zero(npc)
  ...
  if npc == self.npc then
    local near_npc = search_npc(self.npc, 6, 5.1, nil, false, true)
    if near_npc then
    ...
    if get_sync_var("qian_night") or get_sync_var("qian_dark") then
      create_scene_effect("Fx_jingcu_break_sound_5m", npc:get_pos_vec3(), 2)
      remove_scene_effect("1")
    end
    create_scene_effect("FX_SC05_jingcusi_1", npc:get_pos_vec3(), 2)
