...
function monster._init(monster, npc)
    ...
    function Call_explode.start(skill)
      ...
      local function CallSon1(self)
        if is_cast_skill_time(monster.npc, 2010130106, 1) then
          Call_explode.npc1 = add_npc2(2, 20101304, p1.x, p1.z, 0, 0, 2, self.level)
      ...
      local function CallSon2(self)
        if is_cast_skill_time(monster.npc, 2010130106, 1) then
          Call_explode.npc2 = add_npc2(2, 20101304, p2.x, p2.z, 0, 0, 2, self.level)
          skill.monster.son_num = skill.monster.son_num + 1
        end
        table.insert(skill.monster.son_table, Call_explode.npc1)
        table.insert(skill.monster.son_table, Call_explode.npc2)
      end
    ...
    function Call_Charge.start(skill)
      ...
      local function CallSon1(self)
        if is_cast_skill_time(monster.npc, 2010130113, 1) then
          ...
          Call_Charge.npc1 = add_npc2(2, 20101305, p1.x, p1.z, 0, 0, 2, self.level)
        end
      end
      ...
      local function CallSon2(self)
        if is_cast_skill_time(monster.npc, 2010130113, 1) then
          skill.monster.son_num2 = skill.monster.son_num2 + 1
          Call_Charge.npc2 = add_npc2(2, 20101305, p2.x, p2.z, 0, 0, 2, self.level)
        end
        table.insert(skill.monster.son_table, Call_Charge.npc1)
        table.insert(skill.monster.son_table, Call_Charge.npc2)
      end
  ...
  do
    ...
    function Shield_Start.start(skill)
      ...
      self.shield_npc = add_npc2(2, 201059, get_npc_pos(self.npc).x, get_npc_pos(self.npc).z, 0, 0, 2, self.level)
