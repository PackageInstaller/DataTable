...
function M:_init()
  ...
  self.skill01 = {
    ...
    0,
    2,
    get_skill_cfg(20103101).CastTime,
    get_skill_cfg(20103101).AfterTime,
    self.skill02
  }
  self.wander = {1.5, 0.5}
  self.wander_timer = 3
...
function M:on_frame()
  ...
  local now_time = get_npc_time(self.npc)
  if self.dead_state == true and now_time >= self.dead_state_timer then
...
function M:on_npc_hp_zero(npc)
  if npc == self.npc then
    local random_pos_x = math.random(0, 1)
    local random_pos_z = math.random(0, 1)
    local now_pos = get_npc_pos(self.npc)
    add_npc2(2, 201032, now_pos.x, now_pos.z, 0, 0, 2, 1)
    add_npc2(2, 201032, now_pos.x, now_pos.z, 0, 0, 2, 1)
    add_npc2(2, 201032, now_pos.x, now_pos.z, 0, 0, 2, 1)
    self.dead_state = true
    self.dead_state_timer = get_npc_time(self.npc) + 0.2
  end
