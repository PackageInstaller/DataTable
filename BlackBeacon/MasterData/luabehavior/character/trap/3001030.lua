...
function M:_init()
  ...
  self.skill01 = {
    ...
    0,
    0,
    get_skill_cfg(300103001).CastTime,
    get_skill_cfg(300103001).AfterTime
  }
  self.pos = {
    [1] = {x = 0, z = 0}
  }
...
function M:on_npc_hp_zero(npc)
  ...
  if get_sync_var("shot_score") and not self.koufen then
    set_sync_var("shot_score", get_sync_var("shot_score") + 5)
  end
end
...
function M:cast_skill(skill_instant_ID)
  ...
  if 300103001 == skill_instant_ID[1] then
...
function M:wander_main_logic()
  ...
  if self.walk_set == self.trapid then
    self.pos[1].x, self.pos[1].z = get_scene_map_pos_postion("go" .. self.trapid)
    switch_move_type(self.npc, 0)
...
function M:on_skill_hit(npc, skill_id, hit_target, hit_type, missile_cfg, missile)
  ...
  if missile_cfg.Id == 31010030401 then
    cast_magic(self.npc, self.npc, 4031023, 10)
  end
end
...
function M:on_skill_end(npc, skill_id)
  ...
  if 300103001 == skill_id then
    if get_sync_var("shot_score") and get_sync_var("shot_score") - 10 >= 0 then
      set_sync_var("shot_score", get_sync_var("shot_score") - 10)
    elseif get_sync_var("shot_score") and get_sync_var("shot_score") - 10 < 0 then
