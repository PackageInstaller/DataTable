...
function M:on_enter()
  ...
  if not self.is_add_npc then
    self:add_functional_npc(2, "NPC", "tp3", {npc_id = 100102})
    self.is_add_npc = true
  end
  self:actvie_obstacle("AW1", "wall", true)
  if self:is_pass_room() then
    self:set_scene_camera(101)
    self:open_all_transmit()
  else
    self:timer_call_out(nil, function()
      self:play_timeline("timeline_3", 2, 0.5)
    end, 1)
  end
end

function M:on_timeline_end(timeline_name)
  if "timeline_3" == timeline_name then
    self:add_monster()
  end
end

function M:on_func_npc_interact_end(func_npc_id)
  if 100102 == func_npc_id then
    self.open = true
    if 0 == #self.monster_list then
      self:timer_call_out(nil, function()
        self:fight_end(true)
        self:trigger_fight_story(100126)
        self:open_all_transmit()
      end, 1.3)
    end
  end
