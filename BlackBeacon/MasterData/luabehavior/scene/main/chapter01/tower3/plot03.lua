...
function M:on_enter()
  ...
  if self.is_interact and not self:is_pass_room() then
    self:add_npc_by_pos_key(1, 300100206, "A1", 0, 0, "tp1", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100206, "A3", 0, 0, "tp1", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100206, "A2", 0, 0, "tp1", 0, 0, 4, 1)
    self:add_monster()
  end
  local x, y = self:get_scene_map_pos_postion("center")
  self:lookat_position(Global.hero, x, y, true)
  if 0 == self.first_enter then
    self.book = self:add_functional_npc(4, "NPC", "center", {npc_id = 101304})
    self:add_functional_npc(2, "NPC1", "center", {npc_id = 101305})
    self:add_functional_npc(2, "NPC2", "center", {npc_id = 101306})
  end
...
function M:on_story_end(story_id)
  if 1014001 == story_id then
    self:play_story(100101061)
  end
  if 100101061 == story_id then
    self:add_monster()
    self:add_npc_by_pos_key(1, 300100206, "A1", 0, 0, "tp1", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100206, "A3", 0, 0, "tp1", 0, 0, 4, 1)
    self:add_npc_by_pos_key(1, 300100206, "A2", 0, 0, "tp1", 0, 0, 4, 1)
