...
function M:on_story_end(story_id)
  if 1011003 == story_id then
    self:start_texture_guide(10113)
    self.guide = true
  end
end
...
function M:on_enter_area(npc, area)
  ...
  if area.Key == "guide01" and self.guide then
    self:start_guide(2023)
  end
  if not self.is_paly_timeline and area.Key == "story03" and self:is_pass_room() then
    self:remove_area("story03", "story")
    self:clear_input()
