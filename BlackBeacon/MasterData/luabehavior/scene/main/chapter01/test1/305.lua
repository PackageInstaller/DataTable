...
function M:on_enter()
  ...
  self:set_route_status(13, 14, false)
  self:set_route_status(15, 14, true)
  self:set_sceneobj_visible("T3001002_2", false)
  self:set_sceneobj_visible("T3001002_1", false)
  ...
  if not self.is_add_npc then
    self:add_functional_npc(2, "NPC", "tp4", {npc_id = 100101})
...
function M:on_npc_dead(npc)
  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and 2 == self.wave and not self:is_pass_room() then
        self:fight_end(true)
...
function M:on_enter_area(npc, area)
  ...
  if area.Key == "guide01" and not self.is_paly_timeline then
    self:add_monster2()
    self.wave = 2
    self:remove_area("guide01")
...
function M:on_leave()
  self:set_sceneobj_visible("T3001002_2", true)
  self:set_sceneobj_visible("T3001002_1", true)
end
