  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and not self:is_pass_room() then
        ...
        self:open_all_transmit()
        self:set_scene_camera(103)
        self.book = self:add_functional_npc(2, "center", "tp3", {npc_id = 101301})
      end
    end
  end
end
...
function M:on_func_npc_interact_end(func_npc_id)
  Log.Error(Global.hero, "0101")
  if func_npc_id == self.book then
    Log.Error(Global.hero, "0202")
    self:trigger_fight_story(10130001)
    self:accept_task_group(10132)
  end
end
