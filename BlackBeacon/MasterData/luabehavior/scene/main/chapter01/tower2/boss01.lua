...
function M:on_enter()
  ...
  if not self.is_add_npc then
    self:play_timeline("timeline2_1", 0, 2, nil, false)
    self.is_add_npc = true
    self:timer_call_out("timeline2_1", function()
      self:play_story(1012006)
    end, 5.5)
  else
    self:add_monster()
  end
  ...
  for i = count, 1, -1 do
    ...
    if monster == npc then
      ...
      if 0 == #self.monster_list and not self:is_pass_room() then
        ...
        self:complete_task(100205)
        self:timer_call_out(nil, function()
          self:play_story(1012007)
        end, 1)
      end
    end
  end
...
function M:on_timeline_end(timeline_name)
  if not self:is_pass_room() and "timeline2_1" == timeline_name then
    self:timer_call_out_remove("timeline2_1")
  end
end

function M:on_story_end(story_id, step)
  if 1012006 == story_id then
    self:add_monster()
    self:stop_timeline("timeline2_1")
    self:complete_task(100204)
  else
    self:fight_end(true)
  end
end
