...
function M:add_monster4()
  ...
  for index = 1, 3 do
    self:cast_magic(self.mm10, self.mm10, cast_magic_id[index], 1)
    self:cast_magic(self.mm11, self.mm11, cast_magic_id[index], 1)
    self:cast_magic(self.mm12, self.mm12, cast_magic_id[index], 1)
  end
end

function M:add_monster5()
  local list_id = 5
  for _, data in ipairs(lMonster["RANDOW_VAL_" .. list_id]) do
    local key = data[10]
    local args = {
  ...
  for index = 1, 3 do
    self:cast_magic(self.mm13, self.mm13, cast_magic_id[index], 1)
    self:cast_magic(self.mm14, self.mm14, cast_magic_id[index], 1)
  end
end
...
function M:on_npc_dead(npc)
  ...
  elseif 0 == remain_mon2 and 0 == remain_mon3 and not self:is_pass_room() and 3 == self.wave then
    self.wave = 4
    self:add_monster4()
  elseif 0 == remain_mon2 and not self:is_pass_room() and 4 == self.wave then
    self.wave = 5
    self:add_monster5()
  elseif 0 == remain_mon2 and 0 == remain_mon3 and not self:is_pass_room() and 5 == self.wave then
    self:fight_end(true)
