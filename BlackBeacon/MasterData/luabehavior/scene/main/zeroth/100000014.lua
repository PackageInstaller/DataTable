...
function M:on_enter()
  if self:is_pass_room() then
    ...
    self:set_scene_camera(10000002)
    local x, y = self:get_scene_map_pos_postion("center")
    self:lookat_position(Global.hero, x, y, true)
    self.monster1 = self:add_npc_by_pos_key(1, 201046, "M1", 0, 0, "tp3", 0, 0, 4, self.normal_level)
    self:cast_magic(Global.hero, Global.hero, 4010175, 1)
    self:timer_call_out(nil, function()
      self:set_sync_var("story1304", true)
    end, 1)
  end
end
...
function M:on_frame()
  if 0 == self.stageshow and self:check_magic(Global.hero, 201046010) then
    self.stageshow = 1
    self:set_ui_object_visible("fight", "UpBg", true)
...
function M:on_click_fight_story(tips_id, param)
  if 1403 == tips_id then
  end
end
