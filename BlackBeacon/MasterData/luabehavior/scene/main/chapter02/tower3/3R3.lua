...
function M:on_enter()
  self:set_route_status(8, 11, true)
  self:set_route_status(8, 6, false)
  self:add_functional_npc(4, "center", "center", {npc_id = 102105})
...
function M:on_story_end()
  self.car = self:add_npc_by_pos_key(2, 300103801, "center", 0, 0, "C2", 0, 0, 4, 1)
  self:complete_task(102103)
  self:accept_task_group(1021013)
  self:add_monster()
end

function M:on_enter_area(npc, area)
  Log.Info(Global.hero, area.Key, "4444444444")
  if area.Key == "C2" and npc == self.car then
    self:fight_end(true)
    Log.Info(Global.hero, "22222222222")
    self:open_all_transmit()
    self.wave = 0
    self:set_scene_tips(1021041, true, 3)
    self:cast_magic(self.car, self.car, 3001038005, 0)
    self:timer_call_out(nil, function()
      self:remove_npc(self.car)
    end, 0.7)
  end
