...
function M:on_npc_dead(npc)
  local remain_mon = self:get_npc_count_by_kind(2)
  if 0 == remain_mon and not self:is_pass_room() then
    self:end_perform(self.room_type)
    self:timer_call_out(nil, function()
      self:play_story(100000001)
    end, 2)
    self:open_all_transmit()
