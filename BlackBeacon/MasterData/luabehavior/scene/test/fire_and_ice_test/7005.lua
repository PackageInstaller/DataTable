...
function M:on_enter()
  ...
  elseif not self:is_pass_room() then
    ...
    self:lookat_position(Global.hero, x, y, true)
    self:actvie_obstacle("AW1", "wall", true)
    self.monster1 = self:add_npc_by_pos_key(1, 201035, "M1", 0, 0, "bornpoint", 0, 0, 4, self.normal_level)
    ...
    self.monster5 = self:add_npc_by_pos_key(1, 201034, "M5", 0, 0, "bornpoint", 0, 0, 4, self.normal_level)
  end
end
...
function M:on_npc_dead(npc)
  if not self:check_activate(self.monster1) and not self:check_activate(self.monster2) and not self:check_activate(self.monster3) and not self:check_activate(self.monster4) and not self:check_activate(self.monster5) then
    self:fight_end(true)
