local Base = require("manager.scene.custom_scene_logic_runner")
local M = Util.create_child_mt(Base)
local _tunpack = table.unpack
local lMonster = {}
lMonster.RANDOW_VAL_101 = {
  [1] = {
    1,
    201010,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201010,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201010,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201010,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_102 = {
  [1] = {
    1,
    201016,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201009,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201016,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201009,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_201 = {
  [1] = {
    1,
    201016,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201009,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201016,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201009,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_202 = {
  [1] = {
    1,
    201010,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201010,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201010,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201010,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_401 = {
  [1] = {
    1,
    201009,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201009,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201014,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201014,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_402 = {
  [1] = {
    1,
    201014,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201014,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201009,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201009,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_501 = {
  [1] = {
    1,
    201033,
    "M5",
    0,
    0,
    "M5",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201033,
    "M6",
    0,
    0,
    "M6",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201009,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201009,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_502 = {
  [1] = {
    1,
    201016,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201016,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201009,
    "M5",
    0,
    0,
    "M5",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201009,
    "M6",
    0,
    0,
    "M6",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_701 = {
  [1] = {
    1,
    201027,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201027,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201028,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201028,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_702 = {
  [1] = {
    1,
    201027,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201027,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201017,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201017,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_801 = {
  [1] = {
    1,
    201016,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201016,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201027,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201027,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_802 = {
  [1] = {
    1,
    201033,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201027,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201033,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201027,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_901 = {
  [1] = {
    1,
    201027,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201027,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201012,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201012,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}
lMonster.RANDOW_VAL_902 = {
  [1] = {
    1,
    201027,
    "M1",
    0,
    0,
    "M1",
    -1,
    -1,
    4,
    "monster1"
  },
  [2] = {
    1,
    201027,
    "M2",
    0,
    0,
    "M2",
    -1,
    -1,
    4,
    "monster1"
  },
  [3] = {
    1,
    201029,
    "M3",
    0,
    0,
    "M3",
    -1,
    -1,
    4,
    "monster1"
  },
  [4] = {
    1,
    201029,
    "M4",
    0,
    0,
    "M4",
    -1,
    -1,
    4,
    "monster1"
  }
}

function M:_init(...)
  Base._init(self, ...)
  self.room_num, self.floor_num = self:get_tower_floor_info()
  self.room_type = self:get_room_type()
  self.normal_level = self:get_tower_monster_level("MonLevel1")
  self.rare_level = self:get_tower_monster_level("MonLevel2")
  self.boss_level = self:get_tower_monster_level("MonLevel3")
  self.monster_lv = {
    monster1 = self.normal_level,
    monster2 = self.rare_level,
    monster3 = self.boss_level
  }
  self.monster_list = {}
  self.random_id = math.random(1, 2)
end

function M:on_enter()
  local x, y = self:get_scene_map_pos_postion("center")
  self:lookat_position(Global.hero, x, y, true)
  if self:is_pass_room() then
    self:actvie_obstacle("AW1", "wall", true)
    self:set_scene_camera(9901)
    self:open_all_transmit()
  else
    self:actvie_obstacle("AW1", "wall", true)
    self:set_scene_camera(9902)
    self:add_monster()
  end
end

function M:add_monster()
  local list_id = self.floor_num * 100 + self.random_id
  for _, data in ipairs(lMonster["RANDOW_VAL_" .. list_id]) do
    local key = data[10]
    local lv = self.monster_lv[key]
    local args = {
      _tunpack(data, 1, 9)
    }
    args[10] = lv
    local monster = self:add_npc_by_pos_key(_tunpack(args))
    self.monster_list[#self.monster_list + 1] = monster
  end
end

function M:on_npc_dead(npc)
  local count = #self.monster_list
  for i = count, 1, -1 do
    local monster = self.monster_list[i]
    if monster == npc then
      table.remove(self.monster_list, i)
      if 0 == #self.monster_list and not self:is_pass_room() then
        self:fight_end(true)
        self:end_perform(self.room_type)
        self:timer_call_out("camera1", function()
          self:start_texture_guide(1002)
        end, 0.5)
        self:open_all_transmit()
        if not self.is_add_npc then
          self:add_functional_npc(4, "center", "tp3", {npc_id = 50005})
          self:add_functional_npc(2, "tp2", "tp3", {npc_id = 21002})
          self.is_add_npc = true
        end
      end
    end
  end
end

function M:on_npc_show_start(id, type)
  if 50005 == id then
    self:accept_task_group(21111)
    self:accept_task_group(21112)
    return
  end
end

function M:on_task_finish(taskid)
  Log.Info(Global.hero, "finishtaskprint")
  if 211011 == taskid then
    Log.Info(Global.hero, "finishtaskprint211011")
  end
  if 211012 == taskid then
    Log.Info(Global.hero, "finishtaskprint211011")
  end
end

function M:on_frame()
end

function M:on_guide_finished(guide_id)
  if 2015 == guide_id then
    self:set_camera_focus_pos(true)
    self:set_scene_camera(9901)
  end
end

function M:on_texture_guide_finish(group_id)
  if 1002 == group_id then
    self:set_scene_camera(9901)
    Log.Info(Global.hero, "test_texture_guide")
  end
end

function M:on_enter_area(npc, area)
  if not npc:is_hero() then
    return
  end
  self:judge_all_tp(area)
end

function M:on_leave_area(npc, area)
end

function M:on_save()
  local data = {
    is_add_npc = self.is_add_npc
  }
  return data
end

function M:on_load(data)
  self.is_add_npc = data.is_add_npc
end

return M
