...
function M:on_enter_area(npc, area)
  ...
  if area.Key == "restore_hp" and self:is_pass_room() then
    self:enter_special_area(AREA_TYPE.restore_hp)
  end
end

function M:on_leave_area(npc, area)
  if area.Key == "restore_hp" then
    self:leave_special_area(AREA_TYPE.restore_hp)
  end
end
