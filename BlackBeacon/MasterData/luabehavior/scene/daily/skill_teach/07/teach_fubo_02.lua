...
function M:on_enter()
  ...
  self:timer_call_out(nil, function()
    self:cast_magic(Global.hero, Global.hero, 4032072, 3)
  end, 1)
end
...
function M:on_frame()
  ...
  if 1 == self.combo_guide and 92 == self:get_sync_var("combo_guide") then
    self.combo_guide = 92
    self:abort_magic_by_id(self.monster2, 4032903)
    self:abort_magic_by_id(self.monster3, 4032903)
...
function M:add_monster()
  if 0 == self.combo_guide then
    self.monster2 = self:add_npc_by_pos_key(1, 20106001, "M22", 0, 0, "tp3", 0, 0, 4, 99)
    self.monster3 = self:add_npc_by_pos_key(1, 20106001, "M23", 0, 0, "tp3", 0, 0, 4, 99)
    self:cast_magic(self.monster2, self.monster2, 4032901, 1)
    self:cast_magic(self.monster2, self.monster2, 4032903, 1)
    self:cast_magic(self.monster3, self.monster3, 4032901, 1)
    self:cast_magic(self.monster3, self.monster3, 4032903, 1)
