...
function M:on_enter()
  ...
  self:timer_call_out(nil, function()
    self:cast_magic(Global.hero, Global.hero, 4032071, 3)
  end, 1)
end
...
function M:on_frame()
  ...
  if 1 == self.combo_guide and 30 == self:get_sync_var("combo_guide") then
    self.combo_guide = 30
    self:timer_call_out(nil, function()
  ...
  if 32 == self.combo_guide and 99 == self:get_sync_var("combo_guide") then
    self.combo_guide = 99
...
function M:add_monster()
  ...
  if 30 == self.combo_guide then
    self.monster2 = self:add_npc_by_pos_key(1, 201021, "center", 0, 0, "tp3", 0, 0, 4, 2)
