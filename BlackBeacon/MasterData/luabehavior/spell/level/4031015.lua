...
function M:on_start()
  ...
  if self.god and get_npc_attr(self.god, 107) <= 50 then
    self.hide_skill6 = true
    set_sync_var("hide_skill6", 1)
  else
    set_sync_var("hide_skill6", 2)
  end
end
...
function M:on_frame()
  ...
  if self.god and not self.hide_skill6 then
    if 0 == self.magic_set and self.strength < 30 then
      set_skill_btn_icon(61, "TPS_gray")
    elseif 0 == self.magic_set and self.strength > 30 then
      set_skill_btn_icon(61, "UI_H1001005_exskill")
    elseif 1 == self.magic_set then
      set_skill_btn_icon(61, "UI_H1001005_exskill")
    end
    if 0 == self.magic_set and self.strength < self.strengthMax and get_npc_time(self.npc) > self.strengthTime + 0.3 then
      cast_magic(self.npc, self.god, 4031018, 1)
      self.strengthTime = get_npc_time(self.npc)
      self:set_charging()
    end
    if 1 == self.magic_set and self.strength > 0 and get_npc_time(self.npc) > self.strengthTime + 0.3 then
      cast_magic(self.npc, self.god, 4031017, 1)
      self.strengthTime = get_npc_time(self.npc)
    end
    if 1 == self.magic_set and 0 == self.strength then
      self:cast_skill2()
    end
...
function M:on_room_change(npc)
  if check_magic(npc, 4031015) then
    cast_magic(self.npc, self.npc, 4031016, 1)
  end
end
