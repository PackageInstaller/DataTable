...
function M:battle_solo()
  if 1 == self.check_counting then
    cast_magic(self.npc, self.npc, 650018011, 0)
    if self.hero_one then
      cast_magic(self.npc, self.hero_one, 650018011, 0)
    end
    if self.hero_two then
      cast_magic(self.npc, self.hero_two, 650018011, 0)
    end
  else
    abort_magic_by_id(self.npc, 650018011)
    if self.hero_one then
      abort_magic_by_id(self.hero_one, 650018011)
    end
    if self.hero_two then
      abort_magic_by_id(self.hero_two, 650018011)
    end
  end
end
