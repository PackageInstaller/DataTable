...
function M:on_start()
  ...
  if 0 ~= self.ball then
    if self.npc1 then
      cast_magic(self.npc1, self.npc1, 60740201, self.ball)
    end
    if self.npc2 then
      cast_magic(self.npc2, self.npc2, 60740201, self.ball)
    end
    if self.npc3 then
      cast_magic(self.npc3, self.npc3, 60740201, self.ball)
    end
  end
end
...
function M:on_ball_level_change(red, blue, yellow)
  ...
  self.ball = ball2
  if 0 == self.ball then
    abort_magic_by_id(self.npc1, 60740201, 1)
    if self.npc2 then
      abort_magic_by_id(self.npc2, 60740201, 1)
    end
    if self.npc3 then
      abort_magic_by_id(self.npc3, 60740201, 1)
    end
    return
  end
  if self.npc1 then
