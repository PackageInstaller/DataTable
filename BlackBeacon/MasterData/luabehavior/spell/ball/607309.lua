...
function M:on_start()
  ...
  if self.npc1 then
    cast_magic(self.npc1, self.npc1, 60730901, 1)
    self.npc1_get = 1
  end
  if self.npc2 then
    cast_magic(self.npc2, self.npc2, 60730901, 1)
    self.npc2_get = 1
  end
  if self.npc3 then
    cast_magic(self.npc3, self.npc3, 60730901, 1)
    self.npc3_get = 1
  end
end

function M:on_round_finished(npc1, npc2, npc3, round)
  self.count = self.count + 1
  if self.npc1 then
    cast_magic(self.npc1, self.npc1, 60730902, 1)
    if get_npc_attr(self.npc1, 1) > get_npc_attr(self.npc1, 4) then
      cast_magic(self.npc1, self.npc1, 60730703, 0)
    end
  end
  if self.npc2 then
    cast_magic(self.npc2, self.npc2, 60730902, 1)
    if get_npc_attr(self.npc2, 1) > get_npc_attr(self.npc2, 4) then
      cast_magic(self.npc2, self.npc2, 60730703, 0)
    end
  end
  if self.npc3 then
    cast_magic(self.npc3, self.npc3, 60730902, 1)
