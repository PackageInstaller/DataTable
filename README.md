## 个人收集的一些游戏的数据表，有时候也会做全资产下载，脚本问题请开Issue，其它问题转Discussions

|        名字        |       游戏       |               说明               |
| :----------------: | :--------------: | :------------------------------: |
|    AlchemyStars    |       白鸡       |            国际服 Lua            |
|    AnchorPanic    |     锚点降临     |             国服 Lua             |
|   AngelicaAsterR   |     当归紫苑     |  Dmm Json，带数据表自动检查更新  |
|      EverSoul      |   永远之魂(误)   | 国际服 + 日服 Json，自动检查更新 |
|   GirlsCreationR   |       草画       |  Dmm Json，带全资产自动检查更新  |
|     LostSword     |       丢剑       | 韩服 Json，带数据表自动检查更新 |
| MirrenStarLegends |     碳猫语录     |             国服 Csv             |
|     MonmusuTDx     |      草正太      |  Dmm Json，带全资产自动检查更新  |
| MuvLuvGirlsGardenX |       抹布       |             Dmm Json             |
|   OtogiFrontierR   |       村人       |  Dmm Json，带全资产自动检查更新  |
|    RevivedWitch    |    复苏的魔女    |             国服 Lua             |
|    SonOfShenyin    |     神隐之子     |             国服 Lua             |
|    SoulOfDragon    |     龙魂旅人     |           国服 Sqlite           |
|      SoulTide      |     灵魂潮汐     |           国服 LuaJIT           |
|    SphereSeven    |      七星传      |            Qoo服 Lua            |
|    WitchSpring    | 第五部啥时候出？ |   国际服 Xml (单机游戏，好玩)   |
|       Embers       |       辉烬       |            国服 Json            |
|   StarResonance   |     我去星批     |            国服 Json            |
|  ManasisRefranin  |     麻辣鸡丝     |            国服 Json            |
|    ModelWarGirl    |    高能手办团    |           国服 LuaJIT           |
|     OZRewrite     |    OZRewrite    |        国际服 CSV + Json        |
|    BlackBeacon    |     黑色信标     |            国服 Lua            |
|   ErrorGameReset   |   404错误游戏   |            台服 Json            |
|    ReverseBlue    |      反向蓝      |            国服 Json            |
|    GoddessOrder    |   最速关服二游   |           国际服 Json           |
|   Echonox 24/36   |     时序残响     |           国服 LuaJIT           |
|     StarSavior     |  你也是救援者？  |           国际服 Json           |
|       IMYSR       |    爱丽丝圣礼    |             Dmm Json             |
|    Techcronossx    |     铁扣连接     |             Dmm Json             |
|   TaimaninSquad   |   对魔忍特遣队   |           国际服 Json           |
|    SakunaHinuka    |     天穗手游     |            日服 Json            |
|  RlyehShoujotaiX  |    邪神酱(误)    |             Dmm Json             |
|  KalpaOfUniverse  |      天地劫      |            国服 Json            |
|      DotAbyss      |       鸡渊       |             Dmm Json             |
|    CounterSide    |      未来战      |            国际服 Lua            |
|  TrickcalChibiGo  |    坨坨可爱捏    |            韩服 Json            |
| MillenniumWarAigis |     千年战争     |             Dmm Json             |
|  OshiroProjectRE  | 我去没r18(御城) |             Dmm Json             |
|    MementoMori    |      妹妹头      |           国际服 Json           |
|  CuisineDimension  |     料理次元     |             国服 Lua             |
|   FalloutTamira   |   异尘：达米拉   |          国服Lua + Json          |
|     MAkeDrama     |    MAkeDrama    |            韩服 Json            |
