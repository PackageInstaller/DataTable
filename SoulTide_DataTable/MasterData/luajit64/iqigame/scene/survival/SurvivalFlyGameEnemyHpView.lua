-- chunkname: @IQIGame\\Scene\\Survival\\SurvivalFlyGameEnemyHpView.lua
...
function m:SetData(enemyData)
	...
	self.totalHp = self.enemyData:GetChallengeEnemyAttr(FlyGameConstant.Attribute.FlyGame_Shield_Value)
	self.currentHp = self.totalHp

	local isBoss = self.enemyData:GetConfigData().Type == FlyGameConstant.MonsterType.MonsterType_Boss
