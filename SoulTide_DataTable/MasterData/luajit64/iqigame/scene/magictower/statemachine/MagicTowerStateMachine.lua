-- chunkname: @IQIGame\\Scene\\MagicTower\\StateMachine\\MagicTowerStateMachine.lua
...
function m:ActionRemoveElement(param)
	MagicTowerModule.MagicTowerScene:RemoveElement(self.controller)
end
...
function m:ConditionIsReadyToStart(param)
	...
	return MagicTowerModule.MagicTowerScene.IsReadyToStart == sign
end
...
function m:ConditionIsFight(param)
	...
	return self.controller:IsInState(MagicTowerStateEnum.Fight) == sign
end

function m:ConditionIsGather(param)
	local sign = param:getParamAsBoolean(0)

	return self.controller:IsInState(MagicTowerStateEnum.Gather) == sign
end
