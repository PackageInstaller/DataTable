-- chunkname: @IQIGame\\Module\\ActivePanda\\PandaBattleModule.lua
...
function m:OnUIOpen(sender, args)
	...
	if args.UIName == cfgSceneListData.LoadingUI then
		...
		if self.State == 1 then
			self:SetPandaSceneActive(false)
		else
...
function m:SetPandaSceneActive(value)
	if ActivePandaModule.pandaScene then
		ActivePandaModule.pandaScene:SetActive(value)
	end
end
