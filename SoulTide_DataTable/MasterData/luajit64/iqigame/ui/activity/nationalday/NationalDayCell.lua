-- chunkname: @IQIGame\\UI\\Activity\\NationalDay\\NationalDayCell.lua

local NationalDayCell = {
	stateParents = {},
	bgPool = {},
	effectIds = {}
}

function NationalDayCell.New(go)
	local o = Clone(NationalDayCell)
...
function NationalDayCell:Initialize(go)
	...
	for i = 1, 3 do
		...
		transState:Find("Text_01"):GetComponent("Text").text = ActivityUIApi:GetString("nationalDay_stateDesc", i)
...
function NationalDayCell:Refresh(taskData, index, curIndex)
...
function NationalDayCell:OnHide()
	self:StopFxEffect()
	self:RemoveListener()
end

function NationalDayCell:OnDestroy()
...
function NationalDayCell:RefreshMisc()
	self.tfDay.text = ActivityUIApi:GetString("nationalDay_dayDesc", self.index, self.state, self.index == self.specialDay)
...
function NationalDayCell:PlayFxEffect(id, goParent)
	self.effectIds[id] = GameEntry.Effect:PlayUIMountPointEffect(id, 50000, 0, goParent, 0)
end

function NationalDayCell:StopFxEffect()
