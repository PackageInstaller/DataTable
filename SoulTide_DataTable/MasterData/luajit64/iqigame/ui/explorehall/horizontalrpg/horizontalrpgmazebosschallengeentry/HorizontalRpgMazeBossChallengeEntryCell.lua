-- chunkname: @IQIGame\\UI\\ExploreHall\\HorizontalRPG\\HorizontalRPGMazeBossChallengeEntry\\HorizontalRpgMazeBossChallengeEntryCell.lua
...
function m:UpdateView()
	self.cfgRPGMazeBossData = CfgHorizontalRPGBossTable[self.RPGMazeBossCid]
	...
	self.isUnlock = HorizontalRPGModule.IsUnlockBoos(self.RPGMazeBossCid)

	self.Lock:SetActive(not self.isUnlock)
	self.ImgBoss:SetActive(self.isUnlock)
	self.UnlockNode:SetActive(self.isUnlock)
	self.LockNode:SetActive(not self.isUnlock)

	local path = UIGlobalApi.GetImagePath(self.cfgRPGMazeBossData.BossBg)
...
function m:UpdateSpineSortingOrder()
	if self.SpineRenderers ~= nil then
		...
		for i = 0, self.SpineRenderers.Length - 1 do
			self.SpineRenderers[i].sortingOrder = canvas.sortingOrder + 1
		end
	end
end
