-- chunkname: @IQIGame\\UI\\ExploreHall\\EndlessPlus\\EndlessPlusExBossRankCell.lua
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = EndlessPlusExBossDetailUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = EndlessPlusExBossDetailUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = EndlessPlusExBossDetailUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetGoalieData(rankingPOD, ranking, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = EndlessPlusExBossDetailUIApi:GetString("ChallengeRankStr", ranking, false, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = EndlessPlusExBossDetailUIApi:GetString("ChallengeRankStr", ranking, false, rankType)

	UGUIUtil.SetText(self.PlayerNameText, rankingPOD.name)
	UGUIUtil.SetText(self.Value1Text, tostring(rankingPOD.value))

	if ranking > 0 then
		local path = EndlessPlusExBossDetailUIApi:GetString("MazeBossRankIconPath", ranking, false, rankType)
...
function m:SetMyData(ranking, hurt, customData, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = EndlessPlusExBossDetailUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = EndlessPlusExBossDetailUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, PlayerModule.PlayerInfo.baseInfo.pName)
	UGUIUtil.SetText(self.Value1Text, EndlessPlusExBossDetailUIApi:GetString("ChallengeRankHurt", hurt, true))

	if ranking > 0 then
		local path = EndlessPlusExBossDetailUIApi:GetString("MazeBossRankIconPath", ranking, true, rankType)
...
function m:OnClickBtnSelf()
	if self.showFormationUI then
		CenterRankModule.GetRankUserData(Constant.RankingType.EndlessPlusEXBossRank, false, self.index, function(userData)
			UIModule.Open(Constant.UIControllerName.EndlessPlusExBossFormationUI, Constant.UILayer.UI, {
				userData = userData,
				rankingPOD = self.rankingPOD
			})
		end)
	end
end
