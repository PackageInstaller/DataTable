-- chunkname: @IQIGame\\UI\\ExploreHall\\AmusementPark\\AmusementParkBoss\\AmusementParkBossRankCell.lua
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = AmusementParkBossUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = AmusementParkBossUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = AmusementParkBossUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetGoalieData(rankingPOD, ranking, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = AmusementParkBossUIApi:GetString("ChallengeRankStr", ranking, false, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = AmusementParkBossUIApi:GetString("ChallengeRankStr", ranking, false, rankType)

	UGUIUtil.SetText(self.PlayerNameText, rankingPOD.name)
	UGUIUtil.SetText(self.Value1Text, tostring(rankingPOD.value))

	if ranking > 0 then
		local path = AmusementParkBossUIApi:GetString("BossRankIconPath", ranking, false, rankType)
...
function m:SetMyData(ranking, hurt, customData, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = AmusementParkBossUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = AmusementParkBossUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, AmusementParkBossUIApi:GetString("PlayerNameText", PlayerModule.PlayerInfo.baseInfo.pName))
	UGUIUtil.SetText(self.Value1Text, AmusementParkBossUIApi:GetString("ChallengeRankHurt", hurt, true))

	if ranking > 0 then
		local path = AmusementParkBossUIApi:GetString("BossRankIconPath", ranking, true, rankType)
...
function m:OnClickBtnSelf()
	if self.showFormationUI then
		CenterRankModule.GetRankUserData(Constant.RankingType.AmusementParkExBossRank, false, self.index, function(userData)
