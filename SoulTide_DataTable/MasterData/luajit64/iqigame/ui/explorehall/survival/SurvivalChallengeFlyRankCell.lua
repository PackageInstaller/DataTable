-- chunkname: @IQIGame\\UI\\ExploreHall\\Survival\\SurvivalChallengeFlyRankCell.lua
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = SurvivalChallengeFlyRankUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = SurvivalChallengeFlyRankUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = SurvivalChallengeFlyRankUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetMyData(ranking, hurt, customData, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = SurvivalChallengeFlyRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = SurvivalChallengeFlyRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, SurvivalChallengeFlyRankUIApi:GetString("TextMyName", PlayerModule.PlayerInfo.baseInfo.pName))
	UGUIUtil.SetText(self.Value1Text, SurvivalChallengeFlyRankUIApi:GetString("ChallengeRankHurt", hurt, true))

	if ranking > 0 then
		local path = SurvivalChallengeFlyRankUIApi:GetString("BossRankIconPath", ranking, true, rankType)
