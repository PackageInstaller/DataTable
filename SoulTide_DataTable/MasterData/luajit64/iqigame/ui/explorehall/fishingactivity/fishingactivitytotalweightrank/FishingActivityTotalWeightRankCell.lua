-- chunkname: @IQIGame\\UI\\ExploreHall\\FishingActivity\\FishingActivityTotalWeightRank\\FishingActivityTotalWeightRankCell.lua
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetGoalieData(rankingPOD, ranking, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankStr", ranking, false, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankStr", ranking, false, rankType)

	UGUIUtil.SetText(self.PlayerNameText, rankingPOD.name)
	UGUIUtil.SetText(self.Value1Text, tostring(rankingPOD.value))

	if ranking > 0 then
		local path = FishingActivityTotalWeightRankUIApi:GetString("BossRankIconPath", ranking, false, rankType)
...
function m:SetMyData(ranking, value, customData, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, FishingActivityTotalWeightRankUIApi:GetString("PlayerNameText", PlayerModule.PlayerInfo.baseInfo.pName))
	UGUIUtil.SetText(self.Value1Text, FishingActivityTotalWeightRankUIApi:GetString("ChallengeRankHurt", value, true))

	if ranking > 0 then
		local path = FishingActivityTotalWeightRankUIApi:GetString("BossRankIconPath", ranking, true, rankType)
