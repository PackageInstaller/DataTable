-- chunkname: @IQIGame\\UI\\ExploreHall\\FlightChallenge\\FlightGameRankCell.lua
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = FlightGameBossRankUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = FlightGameBossRankUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = FlightGameBossRankUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetMyData(ranking, hurt, customData, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = FlightGameBossRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = FlightGameBossRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, FlightGameBossRankUIApi:GetString("TextMyName", PlayerModule.PlayerInfo.baseInfo.pName))
	UGUIUtil.SetText(self.Value1Text, FlightGameBossRankUIApi:GetString("ChallengeRankHurt", hurt, true))

	if ranking > 0 then
		local path = FlightGameBossRankUIApi:GetString("BossRankIconPath", ranking, true, rankType)
