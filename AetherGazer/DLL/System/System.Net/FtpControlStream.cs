...
internal class FtpControlStream : CommandStream
{
	...
	protected override PipelineInstruction PipelineCallback(PipelineEntry entry, ResponseDescription response, bool timeout, ref Stream stream)
	{
		...
		{
			PipelineInstruction result = QueueOrCreateDataConection(entry, response, timeout, ref stream, out var isSocketReady);
			if (!isSocketReady)
			{
				return result;
			}
	...
	{
		//IL_004d: Field data (rva=0x760) could not be found in any section!
		//IL_004d: Field data (rva=0x760) could not be found in any section!
		...
		if (num2 > num)
		{
			...
			text = text2.TrimEnd(array);
			string text3 = text.Replace("%", "%25");
			...
			if (!Uri.TryCreate(uri, text3, out var result))
			{
	...
	{
		//IL_000d: Field data (rva=0x8a0) could not be found in any section!
		//IL_000d: Field data (rva=0x8a0) could not be found in any section!
		char[] array = new char[4];
		RuntimeHelpers.InitializeArray(array, (RuntimeFieldHandle)/*OpCode not supported: LdMemberToken*/);
		string[] array2 = responseString.Split(array);
		if (array2.Length <= 7)
		{
			throw new FormatException(global::SR.Format("The response string '{0}' has invalid format.", responseString));
		}
		int num = array2.Length - 1;
		if (!char.IsNumber(array2[num], 0))
		{
			num--;
		}
		return Convert.ToByte(array2[num--], NumberFormatInfo.InvariantInfo) | (Convert.ToByte(array2[num--], NumberFormatInfo.InvariantInfo) << 8);
	}
