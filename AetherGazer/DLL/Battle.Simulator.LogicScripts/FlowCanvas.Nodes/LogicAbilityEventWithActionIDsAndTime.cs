...
[Name("技能执行事件(ID组(后三位)&时间(毫秒))(逻辑)", 0)]
[Category("Logic/Abilities")]
[Description("技能Update时调用的事件")]
public class LogicAbilityEventWithActionIDsAndTime : LogicAbilityEventBase
{
	[Name("ID组(后三位)", 0)]
	public List<int> m_actionIDs = new List<int>();

	private ValueInput<int> m_executeTimeInput;
	...
	{
		m_executeTimeInput = AddValueInput<int>("时间(毫秒)", "executeTime");
	}
	...
	protected override bool ConditionFunc(SimEntity simEntity, int abilityID, int logicTime, int actionID)
	{
		...
		return logicTime == m_executeTimeInput.GetValue();
	}
}
