...
[Name("增加战斗评分(int)(逻辑)", 0)]
[Category("Logic/Attribute")]
[Description("增加战斗评分,包含了增益部分")]
public class AddCombatScoreWithIntType : FlowControlNode
{
	private ValueInput<int> m_entityIDInput;

	private ValueInput<int> m_sourceIDInput;

	private ValueInput<int> m_addIntTypeInput;
	...
	protected override void RegisterPorts()
	{
		...
		m_addIntTypeInput = AddValueInput<int>("改变类型", "addType");
		...
		{
			if (!AddCombatScoreBase.AddCombatScore(base.mSimContext, m_entityIDInput.GetValue(), m_addValueInput.GetValue(), m_addIntTypeInput.GetValue(), m_sourceIDInput.GetValue(), m_activeStateInput.GetValue()))
			{
				m_error.Call(f);
			}
			else
