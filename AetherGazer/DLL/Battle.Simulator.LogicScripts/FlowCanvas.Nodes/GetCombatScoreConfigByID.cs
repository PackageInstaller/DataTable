...
[Name("得到战斗评分配置(配置ID)", 0)]
[Category("Logic/Config")]
[Description("如题")]
public class GetCombatScoreConfigByID : FlowControlNode
{
	private ValueInput<int> _EnLevelInput;
	...
	{
		_EnLevelInput = AddValueInput<int>("配置ID", "enLevel");
		_Out = AddFlowOutput("Out", "output");
		_Error = AddFlowOutput("Error", "error");
		AddFlowInput("In", "intput", delegate(Flow f)
		{
			_ScoreInfo = null;
			if (!ConfigHelper.GetInstance().TryGetConfig<CombatScore>(_EnLevelInput.GetValue(), out _ScoreInfo))
			{
				_Error.Call(f);
			}
			else
