...
[Name("Buff事件(类型枚举)(逻辑)", 0)]
[Category("Logic/事件")]
[Description("类型(处理逻辑)ID的枚举类型对应的是 Battle 目录下的 e_BuffClass")]
public class BuffEvent2 : EventNode
{
	...
	private ValueInput<BuffClass> mWantedBuffInput;
	...
	{
		if (buffClass == (int)mWantedBuffInput.GetValue())
		{
			mOwnerID = ownerID;
	...
	protected override void RegisterPorts()
	{
		...
		AddValueOutput("参数1", "buff参数1", (ValueHandler<int>)(() => mBuffParam1));
		AddValueOutput("参数2", "buff参数2", (ValueHandler<int>)(() => mBuffParam2));
		AddValueOutput("参数3", "buff参数3", (ValueHandler<int>)(() => mBuffParam3));
		AddValueOutput("参数4", "buff参数4", (ValueHandler<int>)(() => mBuffParam4));
		AddValueOutput("是否超时移除", () => mIsTimeout);
		mWantedBuffInput = AddValueInput<BuffClass>("类型(处理逻辑)ID", "期望buff类型");
	}
}
