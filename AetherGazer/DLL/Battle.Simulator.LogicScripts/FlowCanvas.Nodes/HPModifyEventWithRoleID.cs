...
[Name("血量变化事件(实体类型ID)(逻辑)", 0)]
[Category("Logic/事件")]
[Description("该事件发生在血量变化之前，此时血量=旧血量\n变化量为正则加血,为负则扣血\n新旧血量小于0时会返回0，且新血量=旧血量+变化量，故新血量可能不准确\n必须传入实体类型ID")]
public class HPModifyEventWithRoleID : EventNode
{
	...
	public void HPModifyCallback(int entityID, long oldHP, long newHP, long amount, int casterID, int actionID, int thrownID, DamageType damageType)
	{
		...
		if (entityWithEntityID != null && entityWithEntityID.hasEntityConfig && entityWithEntityID.entityConfig.mId == _roleIDInput.value)
		{
			_entityID = entityID;
			_oldHP = oldHP;
			_newHP = newHP;
