...
[Name("[庚辰]得到离目标距离最近的浮空实体ID(范围内)", 0)]
[Category("Logic/角色定制/庚辰")]
[Description("得到距离最近的浮空实体ID")]
public class GetNeastEnityOnFloatStatusInRange : FlowControlNodeOfLogic
{
	...
	protected override void RegisterPorts()
	{
		...
		ValueInput<int> rangeInput = AddValueInput<int>("范围(毫米)", "range").SetDefaultAndSerializedValue(int.MaxValue);
		...
		AddFlowInput("In", "input", delegate(Flow f)
		{
			...
			else
			{
				...
				long num = (long)rangeInput.value * (long)rangeInput.value;
