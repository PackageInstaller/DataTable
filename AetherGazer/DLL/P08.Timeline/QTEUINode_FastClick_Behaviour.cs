...
public class QTEUINode_FastClick_Behaviour : QTEUINodeBehaviour
{
	...
	public int falloutInterval;

	public AnimationCurve growCurve;

	public AnimationCurve falloutCurve;

	public List<OnceClickActiveEvent> clickEvent;
	...
	public override void RegisterUIEvent()
	{
		...
		P08StoryQTEFastClicks component = _ui.GetComponent<P08StoryQTEFastClicks>();
		component.Init(timeLimit, clickInterval, falloutInterval, growCurve, falloutCurve, delegate
		{
			OnSuccess();
		}, delegate
		{
		...
		component.RegistProgreeUpdateEvent(OnProgress);
	}
	...
	private void OnClicked(int clickNum)
	{
		...
		{
			OnceClickActiveEvent onceClickActiveEvent = clickEvent[i];
			if (!QTEUINode.VerifyCompare(clickNum, onceClickActiveEvent.clickNum, onceClickActiveEvent.compareType) || string.IsNullOrEmpty(onceClickActiveEvent.goName) || !(_pd != null))
			{
				continue;
			}
			Transform transform = _pd.transform.Find(onceClickActiveEvent.goName);
			if (!(transform != null))
			{
				continue;
			}
			PlayableDirector component = transform.gameObject.GetComponent<PlayableDirector>();
			if (onceClickActiveEvent.resetActive)
			{
