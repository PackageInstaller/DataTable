...
public class Effect_distortion_blend : MonoBehaviour, IMaterialModifier
{
	private class ShaderID
	{
		...
		public static readonly int _PowerX = Shader.PropertyToID("_PowerX");

		public static readonly int _PowerY = Shader.PropertyToID("_PowerY");
	...
	[Range(0f, 1f)]
	public float _PowerX;

	[Range(0f, 1f)]
	public float _PowerY;
	...
	internal void UpdateMaterial()
	{
		if (!(_material == null))
		{
			...
			_material.SetFloat(ShaderID._PowerX, _PowerX);
			_material.SetFloat(ShaderID._PowerY, _PowerY);
