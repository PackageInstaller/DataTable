-- chunkname: @IQIGame\\Module\\MainCity\\MainCityBattleModule.lua
...
function m:OnBattleInitCommand(sender, args)
	local cameraGO = UnityEngine.GameObject.FindGameObjectWithTag("MainCamera")

	self.MainCamera = cameraGO:GetComponent("Camera")
...
function m:OnUIOpen(sender, args)
	...
	if args.UIName == cfgSceneListData.LoadingUI then
		...
		if self.State == 1 then
			self:SetMainCitySceneActive(false)
		else
...
function m:QuitBattle(onComplete)
	self.OnQuitBattleCompleteCallback = onComplete
...
function m:OnBattleClear()
	self:SetMainCitySceneActive(true)
	self:Clear()
	...
	if self.OnQuitBattleCompleteCallback ~= nil then
		self.OnQuitBattleCompleteCallback()

		self.OnQuitBattleCompleteCallback = nil
	end
end

function m:SetMainCitySceneActive(value)
	if self.MainCamera == nil then
		logError("MainCityBattleModule.SetMainCitySceneActive: self.MainCamera is nil. State: " .. tostring(self.State))
	end
