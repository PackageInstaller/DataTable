-- chunkname: @IQIGame\\Module\\Guild\\GuildBattleModule.lua
...
function m:OnUIOpen(sender, args)
	...
	if args.UIName == cfgSceneListData.LoadingUI then
		...
		if self.State == 1 then
			self:SetGuildSceneActive(false)
		else
...
function m:SetGuildSceneActive(value)
	if GuildModule.guildScene then
		GuildModule.guildScene:SetActive(value)
	end
end
