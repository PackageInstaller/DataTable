-- chunkname: @IQIGame\\Module\\RPGMission\\RPGMissionModule.lua

RPGMissionModule = {}

local TaskUIData = require("IQIGame.Module.Task.TaskUIData")

function RPGMissionModule.GetTaskDatum()
	local ret = {}
	local processing = TaskModule.GetTaskProcessing(TaskModule.TaskType.RPG_TASK)
	local finished = TaskModule.GetTaskFinished(TaskModule.TaskType.RPG_TASK)
...
function RPGMissionModule.CheckRedPoint()
	local tasks = TaskModule.GetTaskProcessing(TaskModule.TaskType.RPG_TASK)
