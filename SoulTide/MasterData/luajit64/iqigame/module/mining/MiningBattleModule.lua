-- chunkname: @IQIGame\\Module\\Mining\\MiningBattleModule.lua
...
function m:OnUIOpen(sender, args)
	...
	if args.UIName == cfgSceneListData.LoadingUI then
		...
		if self.State == 1 then
			self:SetMiningSceneActive(false)
		else
...
function m:OnBattleClear()
	self:SetMiningSceneActive(true)
	self:Clear()
...
function m:SetMiningSceneActive(value)
	if MiningModule.miningScene then
		MiningModule.miningScene:SetActive(value)
	end
end
