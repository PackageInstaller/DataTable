-- chunkname: @IQIGame\\UI\\DualTeamExplore\\DualTeamBossChallenge\\DualTeamBossRankCell.lua
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetGoalieData(rankingPOD, ranking, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankStr", ranking, false, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankStr", ranking, false, rankType)

	UGUIUtil.SetText(self.PlayerNameText, rankingPOD.name)
	UGUIUtil.SetText(self.Value1Text, tostring(rankingPOD.value))

	if ranking > 0 then
		local path = DualTeamExploreExBossChallengeUIApi:GetString("MazeBossRankIconPath", ranking, false, rankType)
...
function m:SetMyData(ranking, hurt, customData, rankType)
	...
	self.BgMine:SetActive(false)
	self.BgRankingGoalie:SetActive(false)
	UGUIUtil.SetText(self.RankingText, DualTeamExploreExBossChallengeUIApi:GetString("RankingText", ranking))
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, PlayerModule.PlayerInfo.baseInfo.pName)
	UGUIUtil.SetText(self.Value1Text, DualTeamExploreExBossChallengeUIApi:GetString("ChallengeRankHurt", hurt, true))

	if ranking > 0 then
		local path = DualTeamExploreExBossChallengeUIApi:GetString("MazeBossRankIconPath", ranking, true, rankType)
...
function m:OnClickBtnSelf()
	if self.showFormationUI then
		CenterRankModule.GetRankUserData(Constant.RankingType.DualTeamExploreExBossRank, false, self.index, function(userData)
			UIModule.Open(Constant.UIControllerName.DualTeamExploreExBossFormationUI, Constant.UILayer.UI, {
				userData,
				self.rankingPOD
			})
		end)
