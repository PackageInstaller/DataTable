-- chunkname: @IQIGame\\UI\\RpgMaze\\MazeEXBossRankCell.lua
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = MazeBossChallengeDetailUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = MazeBossChallengeDetailUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = MazeBossChallengeDetailUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetGoalieData(rankingPOD, ranking, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = MazeBossChallengeDetailUIApi:GetString("ChallengeRankStr", ranking, false, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = MazeBossChallengeDetailUIApi:GetString("ChallengeRankStr", ranking, false, rankType)

	UGUIUtil.SetText(self.PlayerNameText, rankingPOD.name)
	UGUIUtil.SetText(self.Value1Text, tostring(rankingPOD.value))

	if ranking > 0 then
		local path = MazeBossChallengeDetailUIApi:GetString("MazeBossRankIconPath", ranking, false, rankType)
...
function m:SetMyData(ranking, hurt, customData, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = MazeBossChallengeDetailUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = MazeBossChallengeDetailUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, PlayerModule.PlayerInfo.baseInfo.pName)
	UGUIUtil.SetText(self.Value1Text, MazeBossChallengeDetailUIApi:GetString("ChallengeRankHurt", hurt, true))

	if ranking > 0 then
		local path = MazeBossChallengeDetailUIApi:GetString("MazeBossRankIconPath", ranking, true, rankType)
...
function m:OnClickBtnSelf()
	if self.showFormationUI then
		CenterRankModule.GetRankUserData(Constant.RankingType.MazeEXBossRank, false, self.index, function(userData)
