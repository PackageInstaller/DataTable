-- chunkname: @IQIGame\\UI\\Activity\\DollMission\\DollMissionTaskCell.lua

local DollMissionTaskCell = {
	cellDoing = {
		itemCellPool = {}
	},
	cellReward = {
...
function DollMissionTaskCell.New(view)
	local obj = Clone(DollMissionTaskCell)
...
function DollMissionTaskCell:Init(view)
	self.View = view
	...
	self.cellDoing.goBtnJump.transform:Find("Text"):GetComponent("Text").text = ActivityNewDollMissionUIApi:GetString("btnGetTxt")
	...
	self.cellReward.goBtnGet.transform:Find("Text"):GetComponent("Text").text = ActivityNewDollMissionUIApi:GetString("btnGetTxt")
	...
	self.cellOver.goSign:GetComponent("Text").text = ActivityNewDollMissionUIApi:GetString("finishSign")

	self:AddListener()
end

function DollMissionTaskCell:Refresh(taskData)
...
function DollMissionTaskCell:OnHide()
	return
end

function DollMissionTaskCell:OnDestroy()
...
function DollMissionTaskCell:AddListener()
...
function DollMissionTaskCell:RemoveListener()
...
function DollMissionTaskCell:OnClickBtnJump()
	JumpModule.Jump(self.cfgTaskData.Source)
end

function DollMissionTaskCell:OnClickBtnGet()
	TaskModule.SendCommitTaskMsg(self.cfgTaskData.Id)
end

function DollMissionTaskCell:RefreshState(state)
...
function DollMissionTaskCell:RefreshTemplate(template, state)
	template.tfTargetDesc.text = ActivityNewDollMissionUIApi:GetString("targetDesc", self.cfgTaskData.Description, self.taskData.CurrentNum, self.taskData.TargetNum)
	template.tfProgress.text = ActivityNewDollMissionUIApi:GetString("targetProgressDesc", self.taskData.CurrentNum, self.taskData.TargetNum)
