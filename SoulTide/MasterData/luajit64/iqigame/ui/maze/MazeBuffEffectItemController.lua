-- chunkname: @IQIGame\\UI\\Maze\\MazeBuffEffectItemController.lua
...
function m:SetData(noticeData)
	...
	local cfgBuffData = CfgBuffTable[noticeData.cid]

	self.BuffView:SetActive(cfgBuffData.DebuffType == 1)
	self.DebuffView:SetActive(cfgBuffData.DebuffType == 2)
	AssetUtil.LoadImage(self, UIGlobalApi.GetIconPath(cfgBuffData.Icon), self.Icon:GetComponent("Image"))

	local descText

	if noticeData.subType == Constant.Maze.BuffNoticeTypeTrigger then
		descText = cfgBuffData.BuffTriggerDescribe
	else
		descText = MazeMainUIApi:GetString("UpdateBuffNotice", noticeData.subType == Constant.Maze.BuffNoticeTypeAdd, cfgBuffData.DebuffType == 2)
	end

	if cfgBuffData.DebuffType == 1 then
		UGUIUtil.SetText(self.BuffNameText, cfgBuffData.Name)
		UGUIUtil.SetText(self.BuffDescText, descText)
	else
		UGUIUtil.SetText(self.DebuffNameText, cfgBuffData.Name)
		UGUIUtil.SetText(self.DebuffDescText, descText)
	end
end
