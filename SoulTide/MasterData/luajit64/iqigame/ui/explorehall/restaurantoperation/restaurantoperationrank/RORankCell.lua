-- chunkname: @IQIGame\\UI\\ExploreHall\\RestaurantOperation\\RestaurantOperationRank\\RORankCell.lua
...
function m:SetData(index, rankingPOD)
	...
	UGUIUtil.SetText(self.TextOperationPlayer, tab[3])

	local value4 = tab[4] or 0

	self.Value1Text:GetComponent("Text").text = value4
...
function m:SetEmpty(index, rankType)
	...
	self.TextIndex:GetComponent("Text").text = RestaurantOperationRankUIApi:GetString("ChallengeRankStr", tostring(index), false, rankType)
	self.TextName:GetComponent("Text").text = RestaurantOperationRankUIApi:GetString("ChallengeRankStr", "--", false, rankType)
	self.TextHurt:GetComponent("Text").text = RestaurantOperationRankUIApi:GetString("ChallengeRankHurt", "--", false)
end
...
function m:SetGoalieData(rankingPOD, ranking, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = RestaurantOperationRankUIApi:GetString("ChallengeRankStr", ranking, false, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = RestaurantOperationRankUIApi:GetString("ChallengeRankStr", ranking, false, rankType)

	UGUIUtil.SetText(self.PlayerNameText, rankingPOD.name)
	UGUIUtil.SetText(self.Value1Text, tostring(rankingPOD.value))

	if ranking > 0 then
		local path = RestaurantOperationRankUIApi:GetString("BossRankIconPath", ranking, false, rankType)
...
function m:SetMyData(ranking, hurt, customData, rankType)
	...
	if rankType == 0 then
		...
		self.RankingText:GetComponent("Text").text = RestaurantOperationRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)
	end

	self.RankingText1:SetActive(ranking <= 0)

	self.RankingText1:GetComponent("Text").text = RestaurantOperationRankUIApi:GetString("ChallengeRankStr", ranking, true, rankType)

	UGUIUtil.SetText(self.PlayerNameText, RestaurantOperationRankUIApi:GetString("PlayerNameText", PlayerModule.PlayerInfo.baseInfo.pName))
	...
	UGUIUtil.SetText(self.Value1Text, RestaurantOperationRankUIApi:GetString("ChallengeRankHurt", value4, true))

	if ranking > 0 then
		local path = RestaurantOperationRankUIApi:GetString("BossRankIconPath", ranking, true, rankType)
