-- chunkname: @IQIGame\\UI\\ExploreHall\\AmusementPark\\AmusementParkBoss\\AmusementParkBossRankRewardCell.lua
...
function m:Update(index)
	...
	UGUIUtil.SetText(self.RankingText, AmusementParkBossRankRewardUIApi:GetString("TextRewardIndex", self.cfgReward.RankLowerLimit, self.cfgReward.RankUpperLimit, self.cfgReward.RankType))

	if not isShowImgNum then
		local path = AmusementParkBossRankRewardUIApi:GetString("RankRewardIconPath", self.cfgReward.RankLowerLimit, self.cfgReward.RankUpperLimit, self.cfgReward.RankType)
