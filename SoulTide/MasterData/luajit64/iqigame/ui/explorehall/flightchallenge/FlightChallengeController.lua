-- chunkname: @IQIGame\\UI\\ExploreHall\\FlightChallenge\\FlightChallengeController.lua
...
function m:OnUpdateDailyDupEvent()
	if FlightChallengeModule.IsShow() and self.View then
		self:UpdateView()
	end
end
...
function m:UpdateView()
	if FlightChallengeModule.dailyDupPOD then
		local cfgDailyDupData = CfgDailyDupTable[FlightChallengeModule.dailyDupPOD.common.cid]
		local currentServerTime = PlayerModule.GetServerTime()
		local openDate = FlightChallengeModule.dailyDupPOD.common.openDate
		local accessEndTime = openDate + cfgDailyDupData.AccessibleTime
		local activityEndTime = openDate + cfgDailyDupData.Duration
		local startTimeText, endTimeText = ExploreHallUIApi:GetString("FlightChallengeLimitTimeTexts", currentServerTime, openDate, accessEndTime, activityEndTime)
...
function m:RefreshUpGradeRed()
	local top = FlightChallengeModule.CheckRedPoint()
...
function m:Show()
	...
	self.cfgControlData = CfgFlightChallengeControlTable[FlightChallengeModule.GlobalCid]

	self.Host.UIController:ChangeBGM(self.cfgControlData.UIBGM)
end

function m:OnUpGradeBtn()
	UIModule.Open(Constant.UIControllerName.FlightGameTeamUI, Constant.UILayer.UI)
end

function m:OnFlightGameBtn()
	UIModule.Open(Constant.UIControllerName.FlightGameFormationUI, Constant.UILayer.UI)
end

function m:OnBossChallengeBtn()
	UIModule.Open(Constant.UIControllerName.FlightGameBossDetailUI, Constant.UILayer.UI)
end
...
function m:OnShopBtn()
	MallModule.OpenActivityMall(Constant.Mall_EntranceType.FlightChallengeShop)
end
