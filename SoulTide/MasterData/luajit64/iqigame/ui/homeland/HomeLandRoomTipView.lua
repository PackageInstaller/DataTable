-- chunkname: @IQIGame\\UI\\HomeLand\\HomeLandRoomTipView.lua

local HomeLandRoomTipView = {
	tipIcons = {},
	funObjTransforms = {}
}
local HomeLandCastleTipIcon = require("IQIGame.UI.HomeLand.HomeLandCastleTipIcon")

function HomeLandRoomTipView.__New(ui, parent)
	local o = Clone(HomeLandRoomTipView)
...
function HomeLandRoomTipView:InitView(ui, rootParent)
	self.rootUI = ui
...
function HomeLandRoomTipView:AddEventListener()
...
function HomeLandRoomTipView:RemoveEventListener()
...
function HomeLandRoomTipView:OnDragIng(sender, args)
...
function HomeLandRoomTipView:OnLookObjEventArgs(sender, args)
...
function HomeLandRoomTipView:UpDataView(roomPOD)
...
function HomeLandRoomTipView:ConvertPosToUI(pos)
	local v_v3 = pos
...
function HomeLandRoomTipView:PlayDecorateEffect(decorateCid)
...
function HomeLandRoomTipView:UpdatePlantEvent(buildingCid)
	self:UpdateTipEvent(buildingCid)
end

function HomeLandRoomTipView:UpdateCookingEvent(buildingCid)
	self:UpdateTipEvent(buildingCid)
end

function HomeLandRoomTipView:UpdateManufactureResultEvent(buildingCid)
	self:UpdateTipEvent(buildingCid)
end

function HomeLandRoomTipView:UpdateBuildWork(buildingCid)
	self:UpdateTipEvent(buildingCid)
end

function HomeLandRoomTipView:UpdateTipEvent(buildingCid)
...
function HomeLandRoomTipView:Dispose()
	for i, v in ipairs(self.tipIcons) do
		v:Dispose()
	end
...
function HomeLandRoomTipView:Open()
	self.rootUI:SetActive(true)
	self:AddEventListener()
end

function HomeLandRoomTipView:Close()
	self.rootUI:SetActive(false)
	self:RemoveEventListener()
end

return HomeLandRoomTipView
