-- chunkname: @IQIGame\\UIExternalApi\\SurvivalChallengeFlyRankUIApi.lua

SurvivalChallengeFlyRankUIApi = BaseLangApi:Extend()

function SurvivalChallengeFlyRankUIApi:Init()
...
function SurvivalChallengeFlyRankUIApi:GetTextMyName(name)
	return self.TextColor[31] .. name .. self.TextColor[0]
end

function SurvivalChallengeFlyRankUIApi:GetBossRankIconPath(rank, mySel, type)
	if type == 1 then
		if rank < 11 then
...
function SurvivalChallengeFlyRankUIApi:GetChallengeRankHurt(hurt, mySelf)
	local str = hurt
...
function SurvivalChallengeFlyRankUIApi:GetChallengeRankStr(rank, mySelf, type)
	local str = ""
...
function SurvivalChallengeFlyRankUIApi:GetRankingLabels()
	return {
...
function SurvivalChallengeFlyRankUIApi:GetNoneRankText()
	return self:GetCfgText(1213068)
end

SurvivalChallengeFlyRankUIApi:Init()
