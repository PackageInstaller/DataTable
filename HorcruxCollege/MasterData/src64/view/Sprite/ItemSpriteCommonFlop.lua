...
ItemSpriteCommonFlop = class("ItemSpriteCommonFlop", function()
	return cc.Node:create()
end)

local var_0_6 = require("data.item_data")
local var_0_7 = "CommonFlopLayer"
local var_0_8 = {
...
function ItemSpriteCommonFlop.create(arg_3_0, arg_3_1, arg_3_2)
	local var_3_0 = ItemSpriteCommonFlop.new()
...
function ItemSpriteCommonFlop:init(arg_4_1, arg_4_2)
	self.activityId = arg_4_1
	var_0_7 = self.activityId == 730 and "CommonFlopLayer" or "Activities/activity" .. self.activityId .. "/flop"
	self.order = arg_4_2
	self.backImage = ccui.ImageView:create(var_0_7 .. "/card_back.png", var_0_5)
	...
	self:registerScriptHandler(function(arg_5_0)
		if arg_5_0 == "exit" then
			activity_manager:releaseEventListenerByName("ItemSpriteCommonFlop_" .. self.order)
		end
	end)
end

function ItemSpriteCommonFlop:initTouchEvent()
	local function var_6_0(arg_7_0, arg_7_1)
		...
		if self.itemid then
			if var_0_6[self.itemid].bag_item_type == kITEM_HORCRUX then
				var_0_10(self.itemid)
			else
		...
		activity_manager:setActivityCommonFlopResult(self.activityId, self.order)
	end
...
function ItemSpriteCommonFlop:registerActivityEventListener()
	activity_manager:registerEventListener("ItemSpriteCommonFlop_" .. self.order, activity_manager.activityEventId.ACTIVITY_COMMONFLOP_CONF_UPDATE, function(arg_9_0)
	...
	activity_manager:registerEventListener("ItemSpriteCommonFlop_" .. self.order, activity_manager.activityEventId.ACTIVITY_COMMONFLOP_SET_RESULT, function(arg_10_0)
...
function ItemSpriteCommonFlop:update(arg_11_1)
	...
	if var_0_6[self.itemid].bag_item_type == kITEM_COMPONENT then
		...
		self.itemIcon:setVisible(true)
		self.itemIcon:loadTexture("equipment/" .. var_0_6[self.itemid].image_id .. ".png")
	end
	...
	if var_0_6[self.itemid].bag_item_type == kITEM_HORCRUX and arg_11_1.item_attr then
	...
	self.animation:setVisible(arg_11_1.isChampion)
	self.animation:setScale(var_0_9)
	self.fontImage:setScale(var_0_9)
	self.itemIcon:setScale(self.itemIcon:getScale() * var_0_9)
end

function ItemSpriteCommonFlop:playTurnAni(arg_13_1)
	self.backImage:runAction(cc.Sequence:create(cc.OrbitCamera:create(0.1, 1, 0, 0, 90, 0, 0), cc.Hide:create(), cc.CallFunc:create(function()
		self.fontImage:runAction(cc.Sequence:create(cc.Show:create(), cc.OrbitCamera:create(0.1, 1, 0, 270, 90, 0, 0), cc.CallFunc:create(function()
			arg_13_1.drawConf.isChampion = arg_13_1.isChampion
...
function ItemSpriteCommonFlop:reset()
