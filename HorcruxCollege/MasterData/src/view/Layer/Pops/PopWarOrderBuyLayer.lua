PopWarOrderBuyLayer = class("PopWarOrderBuyLayer", function()
	return PopBaseLayer:create()
end)

function PopWarOrderBuyLayer.create(arg_2_0, arg_2_1)
	local var_2_0 = PopWarOrderBuyLayer.new()
...
function PopWarOrderBuyLayer:init(arg_3_1)
	print("open poplayer : PopWarOrderBuyLayer")
	...
	self:registerScriptHandler(function(arg_4_0)
		if arg_4_0 == "exit" then
			activity_manager:releaseEventListenerByName("PopWarOrderBuyLayer")
		end
	end)
end

function PopWarOrderBuyLayer.registerActivityEventListener(arg_5_0)
	activity_manager:registerEventListener("PopWarOrderBuyLayer", activity_manager.activityEventId.BATTLE_PASS_VIP_UPDATE, function(arg_6_0)
		arg_5_0:updateGotoBtn()
	end)
end

function PopWarOrderBuyLayer:initUI()
...
function PopWarOrderBuyLayer:initExitTouchEvent()
...
function PopWarOrderBuyLayer:updateGotoBtn()
	...
	self.btngoto:loadTextures(self.imagePath .. "/btn_level_up_vip.png", nil, self.imagePath .. "/btn_level_up_vip.png", var_0_5)
	...
	self.btngoto:setVisible(not activity_manager:isWarOrderSVIP(self.activityId))
end

function PopWarOrderBuyLayer:initSliber()
	self.slider = self.panelBg:getChildByName("slider")

	self.slider:setPercent(0)

	self.upLevel = 1
	self.maxupLevel = activity_manager:getWarOrderMaxLevel(self.activityId) - activity_manager:getCurWarOrderLevel(self.activityId)
...
function PopWarOrderBuyLayer:initBtns()
	self.panelBg:getChildByName("btn_buy"):addTouchEventListener(function(arg_16_0, arg_16_1)
		if arg_16_1 ~= ccui.TouchEventType.ended then
			return
		end

		if not activity_manager:canWarOrderLevelUp(self.activityId, self.upLevel) then
			global_ShowBlockWords(L_BUY_BATTLEPASS_LEVEL[3])

			return
		end

		if playermodel.diamond < activity_manager:getWarOrderLevelUpCost(self.activityId, self.upLevel) then
		...
		activity_manager:buyWarOrderActivityLevel(self.activityId, self.upLevel, 0, activity_manager:getWarOrderLevelUpCost(self.activityId, self.upLevel))
		self:exit()
	end)
	self.panelBg:getChildByName("btn_add"):addTouchEventListener(function(arg_17_0, arg_17_1)
		...
		print(self.upLevel, self.maxupLevel, activity_manager:canWarOrderLevelUp(self.activityId, self.upLevel + 1))

		if self.upLevel >= self.maxupLevel then
			return
		end

		if not activity_manager:canWarOrderLevelUp(self.activityId, self.upLevel + 1) then
			return
		end
...
function PopWarOrderBuyLayer:updateGiftView()
	self.dataList = activity_manager:getWarOrderLevelupCanGetDrop(self.activityId, self.upLevel)

	local var_19_0 = activity_manager:getCurWarOrderLevel(self.activityId) - 1
	...
	if not self.costNum then
		local var_19_1 = ccui.ImageView:create("equipment/1000000.png")

		var_19_1:setPosition(270, 190)
		var_19_1:setScale(0.4)
		self.rootLayer:addChild(var_19_1)

		self.costNum = cc.Label:createWithTTF(activity_manager:getWarOrderLevelUpCost(self.activityId, self.upLevel), FONT_NAME, 28)

		self.costNum:setPosition(340, 190)
		self.costNum:setColor(cc.c3b(252, 237, 242))
		self.rootLayer:addChild(self.costNum)
	else
		self.costNum:setString(activity_manager:getWarOrderLevelUpCost(self.activityId, self.upLevel))
	end
		...
		if not var_20_0 then
			...
			for iter_20_0 = 1, var_0_4 do
				if self.dataList[arg_20_1 + 1][iter_20_0] then
					...
					var_20_2:setPosition(cc.p(110 + 210 * (iter_20_0 - 1), var_0_3 / 2))
			...
			var_20_3:setPosition(320, var_0_3 - 20)
			var_20_3:setName("lv_label")
			var_20_0:addChild(var_20_3)
		else
			for iter_20_1 = 1, var_0_4 do
				...
				if self.dataList[arg_20_1 + 1][iter_20_1] then
					if not var_20_5 then
						...
						var_20_5:setPosition(cc.p(110 + 210 * (iter_20_1 - 1), var_0_3 / 2))
...
function PopWarOrderBuyLayer.createSprite(arg_23_0, arg_23_1)
	local var_23_0 = ItemPurchaseSprite:createPurchaseItem(arg_23_1.dropid, arg_23_1.dropNum)

	var_23_0:setScale(0.8)
	var_23_0:getChildByName("wordBg"):loadTexture("public/box/word_pink_bg.png", var_0_5)
	var_23_0:getChildByName("numLabel"):setString("等级" .. arg_23_1.level)

	return var_23_0
end

function PopWarOrderBuyLayer.updateSprite(arg_24_0, arg_24_1, arg_24_2)
