ActivitySignLayer_5_1 = class("ActivitySignLayer_5_1", function()
...
local var_0_3 = "Activity_DaKaActivity/time_bg.png"
local activity_manager = require("controller.activity_manager")
local var_0_5
local var_0_6 = config._DEBUG and 0 or 1

function ActivitySignLayer_5_1.create(arg_2_0, arg_2_1)
	local var_2_0 = ActivitySignLayer_5_1.new()

	require("controller.texture_manager"):loadPopLayerTextures({
		"Activity_DaKaActivity"
	})
	var_2_0:init(arg_2_1)
	var_2_0:setName("ActivitySignLayer_5_1")
	activity_manager:fireEvent(activity_manager.activityEventId.POP_LAYER_OPEMED)

	return var_2_0
end

function ActivitySignLayer_5_1:init(arg_3_1)
	var_0_5 = true
	arg_3_1.initParam = arg_3_1.initParam or {}
	self.signTime = arg_3_1.signTime
	self.signData = arg_3_1.signData
	self.signTag = arg_3_1.signTag
	self.callback = arg_3_1.initParam.callback or arg_3_1.callback
	self.otherData = arg_3_1.otherData
	self.activitySingCallback = arg_3_1.activitySingCallback
	self.rootLayer = ccs.GUIReader:getInstance():widgetFromJsonFile(config._DEBUG and "ActivityDaKaSignLayer.json" or "ActivityDaKaSignLayer.ExportJson")
	...
	for iter_3_0, iter_3_1 in pairs((require("data.activity_sign_data"))) do
		if iter_3_1.sign_id == "5-1" then
			var_3_0 = var_3_0 + 1
		end
	end

	if var_3_0 <= self.signTime then
		self.label_sure:setString(L_ACTIVITY_SIGN51_END)
	...
	elseif self:isCheckSignIn() then
		if self.signTag then
			...
			self.label_sure:setString(L_ACTIVITY_SIGN51_FINISH)
			self.button_sure:setPressedActionEnabled(true)
			self.button_sure:setBright(false)
		else
			self.label_sure:setString(L_ACTIVITY_SIGN51_BTN)
	...
	self.button_sure:addTouchEventListener(function(arg_5_0, arg_5_1)
		...
		if self.signTime > var_3_0 then
			global_ShowBlockWords(L_ACTIVITY_SIGN51_CLOSE, nil, nil, 2)
	...
	self:registerScriptHandler(function(arg_7_0)
		if arg_7_0 == "exit" then
			activity_manager:releaseEventListenerByName("ActivitySignLayer_5_1")
...
function ActivitySignLayer_5_1.registerActivityEventListener(arg_8_0)
	activity_manager:registerEventListener("ActivitySignLayer_5_1", activity_manager.activityEventId.EXIT_FORM_PURCHASE_LAYER, function(arg_9_0)
		arg_8_0:runAction(cc.Sequence:create(cc.DelayTime:create(1), cc.CallFunc:create(function()
			arg_8_0:exitCallback()
		end)))
	end)
end

function ActivitySignLayer_5_1:isCheckSignIn()
	local var_11_0 = false

	if not self.signData or self.signData == "0" then
		return var_11_0
	else
		local var_11_1 = string_split(self.signData, "-")
		local var_11_2 = os.date("*t", math.floor((time_check_manager:getCurTime())))

		var_11_0 = tonumber(var_11_1[1]) == var_11_2.year and tonumber(var_11_1[2]) == var_11_2.month and tonumber(var_11_1[3]) == var_11_2.day
	end
...
local function var_0_7(arg_12_0)
	...
	if config._DEBUG then
		var_12_0 = cc.Sprite:create("Activity_DaKaActivity/item_bg.png") or cc.Sprite:createWithSpriteFrameName("Activity_DaKaActivity/item_bg.png")
	end

	var_12_0:setAnchorPoint(cc.p(0.5, 0.5))
	var_12_0:setScale(arg_12_0:getContentSize().width * 0.95 / var_12_0:getContentSize().width)
	var_12_0:setPosition(cc.p(arg_12_0:getContentSize().width / 2, arg_12_0:getContentSize().height / 2 + 2))
	arg_12_0:addChild(var_12_0)
end

function ActivitySignLayer_5_1:createSignBg()
	self.sign_bg:loadTexture("mainScenebg/activity/" .. self.otherData.path .. "/new_gongce.png")
	self.sign_bg:setPosition(cc.p(278, 530))
end

function ActivitySignLayer_5_1:createLimitTime()
	local var_14_0

	if config._DEBUG then
		var_14_0 = cc.Scale9Sprite:create(var_0_3) or cc.Scale9Sprite:createWithSpriteFrameName(var_0_3)
	end

	var_14_0:setAnchorPoint(cc.p(0, 1))
	var_14_0:setPosition(cc.p(-5, 555))
	var_14_0:setContentSize(cc.size(150, 20))
	self.sign_bg:addChild(var_14_0)

	local var_14_1 = cc.Label:createWithTTF("", FONT_DES, 20)

	var_14_1:setAnchorPoint(cc.p(0.5, 0.5))
	var_14_1:setPosition(cc.p(70, 545))
	self.sign_bg:addChild(var_14_1)

	local var_14_2 = math.ceil((global_get_time_by_date(self.otherData.endTime) - os.time()) / 86400)

	if var_14_2 >= 0 then
		var_14_1:setString(string.format(L_ACTIVITY_REST_TIME, var_14_2))
	else
		var_14_1:setString(L_ACTIVITY_END)
	end
end

function ActivitySignLayer_5_1:createAwards()
	...
	while self.otherData.drops[var_15_2] do
		...
		var_15_3:setPosition(cc.p(10 + (var_15_2 - 1) * var_15_0, 80))
		...
		local var_15_6 = ItemSprite:createNewWithItemId(var_15_4, var_15_5, nil, nil, nil, 1)

		var_15_6:setAnchorPoint(cc.p(0.5, 0.5))
		var_15_6:setPosition(cc.p(var_15_3:getContentSize().width / 2, var_15_3:getContentSize().height / 2 + 2))
...
function ActivitySignLayer_5_1:getAwards()
...
function ActivitySignLayer_5_1:activitySign()
...
function ActivitySignLayer_5_1.exitCallback(arg_24_0, arg_24_1)
	LayerManager:removePopLayer()
end

function ActivitySignLayer_5_1:showAwardsDetails(arg_25_1, arg_25_2)
...
function ActivitySignLayer_5_1.fullScreen(arg_26_0, arg_26_1)
...
function ActivitySignLayer_5_1.exit(arg_27_0)
	LayerManager:removePopLayer()
end
