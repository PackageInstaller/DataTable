...
local function minValue(array)
    if array and #array > 0 then
        local valueTable = {}
        for i = 1, #array do
            table.insert(valueTable, array[i])
        end
...
local function csToLuaTable(cs_array)
    if cs_array and cs_array.Length > 0 then
        local array = {}
        for i = 0, cs_array.Length - 1 do
            table.insert(array, cs_array[i])
        end
        return array
    end
    return nil
...
local function csLstToLuaTable(cs_array)
    local array = {}
    for key, value in pairs(cs_array) do
...
local function isNullOrEmpty(array)
...
local function isNullOrEmptyCSharp(array)
...
local function intercept(array, startwith, endwith)
    local tb = nil
    if not table.isNullOrEmpty(array) then
        ...
        tb = {}
        if endwith > #array then
            endwith = #array
        end
        for i = startwith, endwith do
            table.insert(tb, array[i])
        end
    end
    return tb
end
...
local function arrayCopy(array,toArray)
    if array == nil then 
        return nil
    end
    toArray = toArray or {}
...
table.csToLuaTable = csToLuaTable
table.csLstToLuaTable = csLstToLuaTable
table.isNullOrEmpty = isNullOrEmpty
table.isNullOrEmptyCSharp = isNullOrEmptyCSharp
table.intercept = intercept
table.arrayContains = arrayContains
table.arrayIndexof = arrayIndexof
table.arrayCopy = arrayCopy
