...
local function RecordComponent(self, name, component_class, component)
    ...
    assert(self.components[name][component_class] == nil, "Aready exist component_class : " .. tostring(component_class.__cname) .. ",name : " .. tostring(name))
...
local function OnDestroy(self)
    self:Walk(function(component)
        ...
        if component.holder == self then
            component:Delete()
        elseif component.view == component.holder then --表示view引用的组件，或者组件引用和的组件
                -- body
            component:Delete()
        end
    end)
