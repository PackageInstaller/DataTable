local AnxixiangMainAction = BaseClass("AnxixiangMainAction")
local M = AnxixiangMainAction
...
function M:RoleClickEventHandle(handler)
    if DormHelper.CheckGift(self.dormAction.heroId, self.systemId) == false then
        return
    end
