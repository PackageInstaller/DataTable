local M = BaseClass("LotteryTypeItem2", require("UI.Ctrl.Lottery.BaseLotteryItem"))
...
function M:Refresh(drawData)
    local ssrLogoPath = "UI/SpritePics/common/ui_common_rank_4_small";
    self._view.SsrLogo:SetPic(ssrLogoPath)

    M.super.Refresh(self, drawData)
    local needTimes = 10
    local restTimes = 10
    if drawData ~= nil then
        if self._view.TipsBg ~= nil then
            self._view.TipsBg:SetActive(not drawData.GuaranteeFlag or drawData.GuaranteeFlag == 0)
        end
