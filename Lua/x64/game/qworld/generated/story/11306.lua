BaseStoryGraph = import("game.qworld.story.Graph.BaseStoryGraph")

local var_0_0 = class("11306", BaseStoryGraph)

function var_0_0.Ctor(arg_1_0)
	var_0_0.super.Ctor(arg_1_0)
	arg_1_0:RegisterFuncMap()
end

function var_0_0.Identify(arg_2_0)
	return "11306"
end

function var_0_0.ConfigPath(arg_3_0)
	return "QWWorld/StoryGraph/11306"
end

function var_0_0.StartStory(arg_4_0)
	arg_4_0:BlackFadeDialogueNode_1()
end

function var_0_0.BlackFadeDialogueNode_1(arg_5_0)
	arg_5_0:HideTalkView()
	arg_5_0:OnBlackFade(false, 0, 0.2, 0.5, 0.5, function()
		arg_5_0:HideAllDynamicAction(true, true, 4081, {})
		arg_5_0:EnableCameraByTagAction("1", 1, 0)
		arg_5_0:ContinueBlackFade()
		arg_5_0:OnNext(function()
			arg_5_0:ClearOnNext()
			arg_5_0:DialogueNode_2()
		end)
	end, function()
		arg_5_0:Next()
	end)
end

function var_0_0.DialogueNode_2(arg_9_0)
	arg_9_0:ShowTalkView()
	arg_9_0:SetTalkContent(11306001, "", 9073)
	arg_9_0:SetNeedSkip(false, {
		Invoke = function()
			arg_9_0:Skip("End")
		end
	})
	arg_9_0:SetNeedClick(true, {
		Invoke = function()
			arg_9_0:Next()
		end
	})
	arg_9_0:OnNext(function(arg_12_0)
		arg_9_0:ClearOnNext()
		arg_9_0:DialogueNode_3()
	end)
end

function var_0_0.DialogueNode_3(arg_13_0)
	arg_13_0:ShowTalkView()
	arg_13_0:SetTalkContent(11306002, "", 9073)
	arg_13_0:SetNeedSkip(false, {
		Invoke = function()
			arg_13_0:Skip("End")
		end
	})
	arg_13_0:SetNeedClick(true, {
		Invoke = function()
			arg_13_0:Next()
		end
	})
	arg_13_0:OnNext(function(arg_16_0)
		arg_13_0:ClearOnNext()
		arg_13_0:DialogueNode_4()
	end)
end

function var_0_0.DialogueNode_4(arg_17_0)
	arg_17_0:InstEffectAction({
		pointTag = "1",
		destroyMode = "DontDestroy",
		createMode = "Point",
		path = "QWWorld/Effect/fx_daxiao_0801_QW_small.prefab"
	})
	arg_17_0:ShowTalkView()
	arg_17_0:SetTalkContent(11306003, "", 0)
	arg_17_0:SetNeedSkip(false, {
		Invoke = function()
			arg_17_0:Skip("End")
		end
	})
	arg_17_0:SetNeedClick(true, {
		Invoke = function()
			arg_17_0:Next()
		end
	})
	arg_17_0:OnNext(function(arg_20_0)
		arg_17_0:ClearOnNext()
		arg_17_0:DialogueNode_5()
	end)
end

function var_0_0.DialogueNode_5(arg_21_0)
	arg_21_0:ShowTalkView()
	arg_21_0:SetTalkContent(11306004, "", 9073)
	arg_21_0:SetNeedSkip(false, {
		Invoke = function()
			arg_21_0:Skip("End")
		end
	})
	arg_21_0:SetNeedClick(true, {
		Invoke = function()
			arg_21_0:Next()
		end
	})
	arg_21_0:OnNext(function(arg_24_0)
		arg_21_0:ClearOnNext()
		arg_21_0:DialogueNode_6()
	end)
end

function var_0_0.DialogueNode_6(arg_25_0)
	arg_25_0:ShowTalkView()
	arg_25_0:SetTalkContent(11306005, "", 0)
	arg_25_0:SetNeedSkip(false, {
		Invoke = function()
			arg_25_0:Skip("End")
		end
	})
	arg_25_0:SetNeedClick(true, {
		Invoke = function()
			arg_25_0:Next()
		end
	})
	arg_25_0:OnNext(function(arg_28_0)
		arg_25_0:ClearOnNext()
		arg_25_0:BlackFadeDialogueNode_7()
	end)
end

function var_0_0.BlackFadeDialogueNode_7(arg_29_0)
	arg_29_0:HideTalkView()
	arg_29_0:OnBlackFade(false, 0, 0.2, 0.5, 0.5, function()
		arg_29_0:HideAllDynamicAction(false, false, 4095, {})
		arg_29_0:EnableCameraByTagAction("", 1, 0)
		arg_29_0:ContinueBlackFade()
		arg_29_0:OnNext(function()
			arg_29_0:ClearOnNext()
			arg_29_0:CloseStoryEventNode_8()
		end)
	end, function()
		arg_29_0:Next()
	end)
end

function var_0_0.CloseStoryEventNode_8(arg_33_0)
	arg_33_0:EnableCameraCutAction(false)
	arg_33_0:EndStory()
end

function var_0_0.RegisterFuncMap(arg_34_0)
	arg_34_0:RegisterTalkFunction("End", arg_34_0.CloseStoryEventNode_8)
end

return var_0_0
