ys = ys or {}

local var_0_0 = {}

ys.Battle.BattleAttr = var_0_0
var_0_0.AttrListInheritance = {
	"level",
	"formulaLevel",
	"repressReduce",
	"cannonPower",
	"torpedoPower",
	"antiAirPower",
	"airPower",
	"antiSubPower",
	"fleetGS",
	"loadSpeed",
	"attackRating",
	"dodgeRate",
	"velocity",
	"luck",
	"cri",
	"criDamage",
	"criDamageResist",
	"hiveExtraHP",
	"GCT",
	"bulletSpeedRatio",
	"torpedoSpeedExtra",
	"damageRatioBullet",
	"damageEnhanceProjectile",
	"healingEnhancement",
	"injureRatio",
	"injureRatioByCannon",
	"injureRatioByBulletTorpedo",
	"injureRatioByAir",
	"damageRatioByCannon",
	"damageRatioByBulletTorpedo",
	"damageRatioByAir",
	"damagePreventRantTorpedo",
	"accuracyRateExtra",
	"dodgeRateExtra",
	"perfectDodge",
	"immuneDirectHit",
	"chargeBulletAccuracy",
	"dropBombAccuracy",
	"aircraftBooster",
	"manualEnhancement",
	"initialEnhancement",
	"worldBuffResistance",
	"airResistPierceActive",
	"airResistPierce"
}

function var_0_0.InsertInheritedAttr(arg_1_0)
	for iter_1_0, iter_1_1 in pairs(arg_1_0) do
		var_0_0.AttrListInheritance[#var_0_0.AttrListInheritance + 1] = iter_1_1
	end

	return
end

var_0_0.InsertInheritedAttr(ys.Battle.BattleConfig.AMMO_DAMAGE_ENHANCE)
var_0_0.InsertInheritedAttr(ys.Battle.BattleConfig.AMMO_DAMAGE_REDUCE)
var_0_0.InsertInheritedAttr(ys.Battle.BattleConfig.DAMAGE_AMMO_TO_ARMOR_RATE_ENHANCE)
var_0_0.InsertInheritedAttr(ys.Battle.BattleConfig.DAMAGE_TO_ARMOR_RATE_ENHANCE)
var_0_0.InsertInheritedAttr(ys.Battle.BattleConfig.SHIP_TYPE_ACCURACY_ENHANCE)

var_0_0.TAG_EHC_KEY = "DMG_TAG_EHC_"
var_0_0.FROM_TAG_EHC_KEY = "DMG_FROM_TAG_"
var_0_0.TAG_CRI_EHC_KEY = "CRI_TAG_EHC_"
var_0_0.TAG_CRIDMG_EHC_KEY = "CRIDMG_TAG_EHC_"
var_0_0.ATTACK_ATTR_TYPE = {
	[ys.Battle.BattleConst.WeaponDamageAttr.CANNON] = "cannonPower",
	[ys.Battle.BattleConst.WeaponDamageAttr.TORPEDO] = "torpedoPower",
	[ys.Battle.BattleConst.WeaponDamageAttr.ANTI_AIR] = "antiAirPower",
	[ys.Battle.BattleConst.WeaponDamageAttr.AIR] = "airPower",
	[ys.Battle.BattleConst.WeaponDamageAttr.ANIT_SUB] = "antiSubPower"
}

function var_0_0:GetAtkAttrByType(arg_2_1)
	return math.max(self[var_0_0.ATTACK_ATTR_TYPE[arg_2_1]], 0)
end

function var_0_0.SetAttr(arg_3_0, arg_3_1)
	arg_3_0._attr = setmetatable({}, {
		__index = arg_3_1
	})

	return
end

function var_0_0:GetAttr()
	return self._attr
end

function var_0_0:SetBaseAttr()
	self._baseAttr = Clone(self._attr)

	return
end

function var_0_0:IsInvincible()
	return self._attr.isInvincible and self._attr.isInvincible > 0
end

function var_0_0:AppendInvincible()
	self._attr.isInvincible = (self._attr.isInvincible or 0) + 1

	return
end

function var_0_0:AddImmuneAreaLimit(arg_8_1)
	self._attr.immuneAreaLimit = (self._attr.immuneAreaLimit or 0) + arg_8_1

	self._move:ImmuneAreaLimit((self._attr.immuneAreaLimit or 0) + arg_8_1 > 0)

	return
end

function var_0_0:AddImmuneMaxAreaLimit(arg_9_1)
	self._attr.immuneMaxAreaLimit = (self._attr.immuneMaxAreaLimit or 0) + arg_9_1

	self._move:ImmuneMaxAreaLimit((self._attr.immuneMaxAreaLimit or 0) + arg_9_1 > 0)

	return
end

function var_0_0:IsImmuneAreaLimit()
	return self._attr.immuneAreaLimit and self._attr.immuneAreaLimit > 0
end

function var_0_0:IsImmuneMaxAreaLimit()
	return self._attr.immuneMaxAreaLimit and self._attr.immuneMaxAreaLimit > 0
end

function var_0_0:IsVisitable()
	return not self._attr.isUnVisitable or self._attr.isUnVisitable <= 0
end

function var_0_0:UnVisitable()
	self._attr.isUnVisitable = (self._attr.isUnVisitable or 0) + 1

	return
end

function var_0_0:Visitable()
	self._attr.isUnVisitable = (self._attr.isUnVisitable or 0) - 1

	return
end

function var_0_0:IsSpirit()
	return self._attr.isSpirit and self._attr.isSpirit > 0
end

function var_0_0:Spirit()
	self._attr.isSpirit = (self._attr.isSpirit or 0) + 1

	return
end

function var_0_0:Entity()
	self._attr.isSpirit = (self._attr.isSpirit or 0) - 1

	return
end

function var_0_0:IsStun()
	return self._attr.isStun and self._attr.isStun > 0
end

function var_0_0:Stun()
	self._attr.isStun = (self._attr.isStun or 0) + 1

	return
end

function var_0_0:CancelStun()
	self._attr.isStun = (self._attr.isStun or 0) - 1

	return
end

function var_0_0:IsCloak()
	return (self._attr.isCloak or 0) == 1
end

function var_0_0:Cloak()
	self._attr.isCloak = 1
	self._attr.airResistPierceActive = 1

	return
end

function var_0_0:Uncloak()
	self._attr.isCloak = 0
	self._attr.airResistPierceActive = 0

	return
end

function var_0_0:IsLockAimBias()
	return (self._attr.lockAimBias or 0) >= 1
end

function var_0_0:IsUnitCldImmune()
	return (self._attr.unitCldImmune or 0) >= 1
end

function var_0_0:UnitCldImmune()
	self._attr.unitCldImmune = (self._attr.unitCldImmune or 0) + 1

	return
end

function var_0_0:UnitCldEnable()
	self._attr.unitCldImmune = (self._attr.unitCldImmune or 0) - 1

	return
end

function var_0_0.GetCurrentTargetSelect(arg_28_0)
	local var_28_0

	for iter_28_0, iter_28_1 in ipairs((var_0_0.GetCurrent(arg_28_0, "TargetChoise"))) do
		if not var_28_0 or ys.Battle.BattleConfig.TARGET_SELECT_PRIORITY[iter_28_1] > ys.Battle.BattleConfig.TARGET_SELECT_PRIORITY[var_28_0] then
			var_28_0 = iter_28_1
		end
	end

	return var_28_0
end

function var_0_0.AddTargetSelect(arg_29_0, arg_29_1)
	table.insert(var_0_0.GetCurrent(arg_29_0, "TargetChoise"), arg_29_1)

	return
end

function var_0_0.RemoveTargetSelect(arg_30_0, arg_30_1)
	local var_30_0 = var_0_0.GetCurrent(arg_30_0, "TargetChoise")

	for iter_30_0, iter_30_1 in ipairs(var_30_0) do
		if iter_30_1 == arg_30_1 then
			table.remove(var_30_0, iter_30_0)

			break
		end
	end

	return
end

function var_0_0.GetCurrentGuardianID(arg_31_0)
	local var_31_0 = var_0_0.GetCurrent(arg_31_0, "guardian")

	if #var_31_0 == 0 then
		return nil
	else
		return var_31_0[#var_31_0]
	end

	return
end

function var_0_0.AddGuardianID(arg_32_0, arg_32_1)
	local var_32_0 = var_0_0.GetCurrent(arg_32_0, "guardian")

	if not table.contains(var_32_0, arg_32_1) then
		table.insert(var_32_0, arg_32_1)
	end

	return
end

function var_0_0.RemoveGuardianID(arg_33_0, arg_33_1)
	local var_33_0 = var_0_0.GetCurrent(arg_33_0, "guardian")

	for iter_33_0, iter_33_1 in ipairs(var_33_0) do
		if iter_33_1 == arg_33_1 then
			table.remove(var_33_0, iter_33_0)

			return
		end
	end

	return
end

function var_0_0:SetPlayerAttrFromOutBattle(arg_34_1, arg_34_2)
	local var_34_0 = self._attr or {}

	self._attr = var_34_0
	var_34_0.id = arg_34_1.id
	var_34_0.battleUID = self:GetUniqueID()
	var_34_0.level = arg_34_1.level
	var_34_0.formulaLevel = arg_34_1.level
	var_34_0.maxHP = arg_34_1.durability
	var_34_0.HPRate = 1
	var_34_0.DMGRate = 0
	var_34_0.cannonPower = arg_34_1.cannon
	var_34_0.torpedoPower = arg_34_1.torpedo
	var_34_0.antiAirPower = arg_34_1.antiaircraft
	var_34_0.antiSubPower = arg_34_1.antisub or 0

	if arg_34_2 then
		var_34_0.baseAntiSubPower = arg_34_2.antisub or arg_34_1.antisub
	end

	var_34_0.airPower = arg_34_1.air
	var_34_0.loadSpeed = arg_34_1.reload
	var_34_0.armorType = arg_34_1.armorType
	var_34_0.attackRating = arg_34_1.hit
	var_34_0.dodgeRate = arg_34_1.dodge
	var_34_0.velocity = ys.Battle.BattleFormulas.ConvertShipSpeed(arg_34_1.speed)
	var_34_0.baseVelocity = var_34_0.velocity
	var_34_0.luck = arg_34_1.luck
	var_34_0.repressReduce = arg_34_1.repressReduce or 1
	var_34_0.oxyMax = arg_34_1.oxy_max
	var_34_0.oxyCost = arg_34_1.oxy_cost
	var_34_0.oxyRecovery = arg_34_1.oxy_recovery
	var_34_0.oxyRecoverySurface = arg_34_1.oxy_recovery_surface
	var_34_0.oxyRecoveryBench = arg_34_1.oxy_recovery_bench
	var_34_0.oxyAtkDuration = arg_34_1.attack_duration
	var_34_0.raidDist = arg_34_1.raid_distance
	var_34_0.sonarRange = arg_34_1.sonarRange or 0

	if arg_34_2 then
		var_34_0.cloakExposeBase = arg_34_2.dodge + ys.Battle.BattleConfig.CLOAK_EXPOSE_CONST or 0
	end

	var_34_0.cloakExposeExtra = 0
	var_34_0.cloakRestore = var_34_0.cloakExposeBase + var_34_0.cloakExposeExtra + ys.Battle.BattleConfig.CLOAK_BASE_RESTORE_DELTA
	var_34_0.cloakRecovery = ys.Battle.BattleConfig.CLOAK_RECOVERY
	var_34_0.cloakStrikeAdditive = ys.Battle.BattleConfig.CLOAK_STRIKE_ADDITIVE
	var_34_0.cloakBombardAdditive = ys.Battle.BattleConfig.CLOAK_STRIKE_ADDITIVE
	var_34_0.airResistPierce = ys.Battle.BattleConfig.BASE_ARP
	var_34_0.aimBias = 0
	var_34_0.aimBiasDecaySpeed = 0
	var_34_0.aimBiasDecaySpeedRatio = 0
	var_34_0.aimBiasExtraACC = 0
	var_34_0.baseScale = arg_34_1.scale / 50
	var_34_0.modelScale = var_34_0.baseScale
	var_34_0.healingRate = 1
	var_34_0.DMG_TAG_EHC_N_99 = arg_34_1[AttributeType.AntiSiren] or 0
	var_34_0.comboTag = "combo_" .. var_34_0.battleUID
	var_34_0.labelTag = {}
	var_34_0.barrageCounterMod = 1
	var_34_0.TargetChoise = {}
	var_34_0.guardian = {}
	var_34_0.injureRatioKamikazeAir = 1
	var_34_0.injureRatioKamikazeShip = 1
	var_34_0.nationality = self:GetTemplate().nationality

	var_0_0.SetBaseAttr(self)

	return
end

function var_0_0.AttrFixer(arg_35_0, arg_35_1)
	if arg_35_0 == SYSTEM_SCENARIO then
		arg_35_1.repressReduce = ys.Battle.BattleDataProxy.GetInstance():GetRepressReduce()
	elseif arg_35_0 ~= SYSTEM_DUEL then
		if arg_35_0 == SYSTEM_SHAM then
			local var_35_0, var_35_1 = ys.Battle.BattleDataFunction:GetPlayerUnitDurabilityExtraAddition(arg_35_1.level)

			arg_35_1.durability = arg_35_1.durability * var_35_0 + var_35_1
		end
	end

	return
end

function var_0_0.InitDOTAttr(arg_36_0, arg_36_1)
	for iter_36_0, iter_36_1 in ipairs(ys.Battle.BattleConfig.DOT_CONFIG) do
		for iter_36_2, iter_36_3 in pairs(iter_36_1) do
			arg_36_0[iter_36_3] = iter_36_2 == "hit" and (arg_36_1[iter_36_3] or ys.Battle.BattleConfig.DOT_CONFIG_DEFAULT[iter_36_2]) or ys.Battle.BattleConfig.DOT_CONFIG_DEFAULT[iter_36_2]
		end
	end

	return
end

function var_0_0:SetEnemyAttr(arg_37_1)
	local var_37_0 = self:GetLevel()
	local var_37_1 = self._attr or {}

	self._attr = var_37_1
	var_37_1.battleUID = self:GetUniqueID()
	var_37_1.level = var_37_0
	var_37_1.formulaLevel = var_37_0
	var_37_1.maxHP = math.ceil(self._tmpData.durability + self._tmpData.durability_growth * ((var_37_0 - 1) / 1000))
	var_37_1.HPRate = 1
	var_37_1.DMGRate = 0
	var_37_1.cannonPower = self._tmpData.cannon + self._tmpData.cannon_growth * ((var_37_0 - 1) / 1000)
	var_37_1.torpedoPower = self._tmpData.torpedo + self._tmpData.torpedo_growth * ((var_37_0 - 1) / 1000)
	var_37_1.antiAirPower = self._tmpData.antiaircraft + self._tmpData.antiaircraft_growth * ((var_37_0 - 1) / 1000)
	var_37_1.airPower = self._tmpData.air + self._tmpData.air_growth * ((var_37_0 - 1) / 1000)
	var_37_1.antiSubPower = self._tmpData.antisub + self._tmpData.antisub_growth * ((var_37_0 - 1) / 1000)
	var_37_1.loadSpeed = self._tmpData.reload + self._tmpData.reload_growth * ((var_37_0 - 1) / 1000)
	var_37_1.armorType = self._tmpData.armor_type
	var_37_1.attackRating = self._tmpData.hit + self._tmpData.hit_growth * ((var_37_0 - 1) / 1000)
	var_37_1.dodgeRate = self._tmpData.dodge + self._tmpData.dodge_growth * ((var_37_0 - 1) / 1000)
	var_37_1.velocity = ys.Battle.BattleFormulas.ConvertShipSpeed(self._tmpData.speed + self._tmpData.speed_growth * ((var_37_0 - 1) / 1000))
	var_37_1.baseVelocity = var_37_1.velocity
	var_37_1.luck = self._tmpData.luck + self._tmpData.luck_growth * ((var_37_0 - 1) / 1000)
	var_37_1.bulletSpeedRatio = 0
	var_37_1.id = "enemy_" .. tostring(self._tmpData.id)
	var_37_1.nationality = self._tmpData.nationality
	var_37_1.repressReduce = 1
	var_37_1.healingRate = 1
	var_37_1.comboTag = "combo_" .. var_37_1.battleUID
	var_37_1.baseScale = self._tmpData.scale / 50
	var_37_1.modelScale = var_37_1.baseScale
	var_37_1.labelTag = {}
	var_37_1.TargetChoise = {}
	var_37_1.guardian = {}

	var_0_0.SetBaseAttr(self)

	return
end

function var_0_0:SetEnemyWorldEnhance()
	self._attr.maxHP = self._attr.maxHP * ys.Battle.BattleFormulas.WorldEnemyAttrEnhance(self._tmpData.world_enhancement[1], self._attr.level)
	self._attr.cannonPower = self._attr.cannonPower * ys.Battle.BattleFormulas.WorldEnemyAttrEnhance(self._tmpData.world_enhancement[2], self._attr.level)
	self._attr.torpedoPower = self._attr.torpedoPower * ys.Battle.BattleFormulas.WorldEnemyAttrEnhance(self._tmpData.world_enhancement[3], self._attr.level)
	self._attr.antiAirPower = self._attr.antiAirPower * ys.Battle.BattleFormulas.WorldEnemyAttrEnhance(self._tmpData.world_enhancement[4], self._attr.level)
	self._attr.airPower = self._attr.airPower * ys.Battle.BattleFormulas.WorldEnemyAttrEnhance(self._tmpData.world_enhancement[5], self._attr.level)
	self._attr.attackRating = self._attr.attackRating * ys.Battle.BattleFormulas.WorldEnemyAttrEnhance(self._tmpData.world_enhancement[6], self._attr.level)
	self._attr.dodgeRate = self._attr.dodgeRate * ys.Battle.BattleFormulas.WorldEnemyAttrEnhance(self._tmpData.world_enhancement[7], self._attr.level)

	local var_38_0 = ys.Battle.BattleDataProxy.GetInstance():GetInitData()
	local var_38_1, var_38_2, var_38_3 = ys.Battle.BattleFormulas.WorldMapRewardAttrEnhance(var_38_0.EnemyMapRewards, var_38_0.FleetMapRewards)

	self._attr.cannonPower = self._attr.cannonPower * (1 + var_38_1)
	self._attr.torpedoPower = self._attr.torpedoPower * (1 + var_38_1)
	self._attr.airPower = self._attr.airPower * (1 + var_38_1)
	self._attr.antiAirPower = self._attr.antiAirPower * (1 + var_38_1)
	self._attr.antiSubPower = self._attr.antiSubPower * (1 + var_38_1)
	self._attr.maxHP = math.ceil(self._attr.maxHP * (1 + var_38_2))
	self._attr.worldBuffResistance = var_38_3

	var_0_0.SetBaseAttr(self)

	return
end

function var_0_0:SetMinionAttr(arg_39_1)
	local var_39_0 = self:GetMaster()
	local var_39_1 = var_0_0.GetAttr(var_39_0)
	local var_39_2 = self._tmpData
	local var_39_3 = self._attr or {}

	self._attr = var_39_3
	var_39_3.battleUID = self:GetUniqueID()

	for iter_39_0, iter_39_1 in ipairs(var_0_0.AttrListInheritance) do
		var_39_3[iter_39_1] = var_39_1[iter_39_1]
	end

	for iter_39_2, iter_39_3 in pairs(var_39_1) do
		if string.find(iter_39_2, var_0_0.TAG_EHC_KEY) then
			var_39_3[iter_39_2] = iter_39_3
		end
	end

	for iter_39_4, iter_39_5 in pairs(var_39_1) do
		if string.find(iter_39_4, var_0_0.TAG_CRI_EHC_KEY) then
			var_39_3[iter_39_4] = iter_39_5
		end
	end

	var_39_3.id = var_39_1.id
	var_39_3.level = var_39_1.level
	var_39_3.formulaLevel = var_39_1.level
	var_39_3.baseScale = self._tmpData.scale / 50
	var_39_3.modelScale = var_39_3.baseScale

	local function var_39_4(arg_40_0, arg_40_1)
		var_39_3[arg_40_1] = var_39_2[arg_40_0 .. "_growth"] == 0 and var_39_2[arg_40_0] or var_39_2[arg_40_0 .. "_growth"] == -1 and (arg_40_0 == "durability" and var_39_0:GetCurrentHP() or var_39_1[arg_40_1]) or var_39_1[arg_40_1] * var_39_2[arg_40_0 .. "_growth"] * 0.0001

		return
	end

	var_39_3.HPRate = 1
	var_39_3.DMGRate = 0

	var_39_4("durability", "maxHP")
	var_39_4("cannon", "cannonPower")
	var_39_4("torpedo", "torpedoPower")
	var_39_4("antiaircraft", "antiAirPower")
	var_39_4("air", "airPower")
	var_39_4("antisub", "antiSubPower")
	var_39_4("reload", "loadSpeed")
	var_39_4("hit", "attackRating")
	var_39_4("dodge", "dodgeRate")
	var_39_4("luck", "luck")

	var_39_3.armorType = self._tmpData.armor_type

	var_39_4("speed", "velocity")

	var_39_3.velocity = ys.Battle.BattleFormulas.ConvertShipSpeed(var_39_3.velocity)
	var_39_3.baseVelocity = var_39_3.velocity
	var_39_3.bulletSpeedRatio = 0
	var_39_3.repressReduce = 1
	var_39_3.healingRate = 1
	var_39_3.comboTag = "combo_" .. var_39_3.battleUID
	var_39_3.labelTag = {}
	var_39_3.TargetChoise = {}
	var_39_3.guardian = {}
	var_39_3.nationality = self._tmpData.nationality

	var_0_0.SetBaseAttr(self)

	return
end

function var_0_0:IsWorldMapRewardAttrWarning(arg_41_1)
	for iter_41_0 = 1, 3 do
		if arg_41_1[iter_41_0] / ((self[iter_41_0] ~= 0 or nil) and (self[iter_41_0] or 1)) < pg.gameset.world_mapbuff_tips.key_value / 16 then
			return true
		end
	end

	return false
end

function var_0_0.MonsterAttrFixer(arg_42_0, arg_42_1)
	if arg_42_0 == SYSTEM_SCENARIO then
		local var_42_0 = ys.Battle.BattleDataProxy.GetInstance()

		var_0_0.SetCurrent(arg_42_1, "formulaLevel", math.max(1, var_0_0.GetCurrent(arg_42_1, "level") - (var_42_0:IsCompletelyRepress() and var_42_0:GetRepressLevel() or 0)))
	elseif arg_42_0 == SYSTEM_WORLD then
		var_0_0.SetEnemyWorldEnhance(arg_42_1)
	end

	return
end

function var_0_0:SetAircraftAttFromMother(arg_43_1)
	local var_43_0 = self._attr or {}

	self._attr = var_43_0
	var_43_0.battleUID = self:GetUniqueID()
	var_43_0.hostUID = arg_43_1:GetUniqueID()

	if not type(arg_43_1._attr.id) == "string" or string.find(arg_43_1._attr.id, "enemy_") == nil then
		var_43_0.id = arg_43_1._attr.id
	end

	local var_43_1 = var_0_0.GetAttr(arg_43_1)

	for iter_43_0, iter_43_1 in ipairs(var_0_0.AttrListInheritance) do
		var_43_0[iter_43_1] = var_43_1[iter_43_1]
	end

	for iter_43_2, iter_43_3 in pairs(var_43_1) do
		if string.find(iter_43_2, var_0_0.TAG_EHC_KEY) then
			var_43_0[iter_43_2] = iter_43_3
		end
	end

	for iter_43_4, iter_43_5 in pairs(var_43_1) do
		if string.find(iter_43_4, var_0_0.TAG_CRI_EHC_KEY) then
			var_43_0[iter_43_4] = iter_43_5
		end
	end

	var_43_0.armorType = 0
	var_43_0.velocity = var_0_0.GetCurrent(arg_43_1, "baseVelocity")
	var_43_0.labelTag = {}
	var_43_0.TargetChoise = {}
	var_43_0.guardian = {}
	var_43_0.comboTag = "combo_" .. var_43_0.hostUID

	return
end

function var_0_0:SetAircraftAttFromTemp()
	self._attr = self._attr or {}
	self._attr.velocity = self._attr.velocity or ys.Battle.BattleFormulas.ConvertAircraftSpeed(self._tmpData.speed)
	self._attr.maxHP = self._attr.maxHP or self._tmpData.max_hp + self._tmpData.hp_growth / 1000 * ((self._attr.level or 1) - 1) + var_0_0.GetCurrent(self, "hiveExtraHP")
	self._attr.crashDMG = self._tmpData.crash_DMG
	self._attr.dodge = self._tmpData.dodge
	self._attr.dodgeLimit = self._tmpData.dodge_limit

	return
end

function var_0_0:SetAirFighterAttr(arg_45_1)
	local var_45_0 = self._attr or {}

	self._attr = var_45_0

	local var_45_1 = ys.Battle.BattleDataProxy.GetInstance()
	local var_45_2 = var_45_1:GetDungeonLevel()

	var_45_0.battleUID = self:GetUniqueID()
	var_45_0.hostUID = 0
	var_45_0.id = 0
	var_45_0.level = var_45_2
	var_45_0.formulaLevel = var_45_2

	if var_45_1:IsCompletelyRepress() then
		var_45_0.formulaLevel = math.max(var_45_0.formulaLevel - 10, 1)
	end

	local var_45_3 = (var_45_2 - 1) / 1000

	var_45_0.maxHP = math.floor(arg_45_1.max_hp + arg_45_1.hp_growth * ((var_45_2 - 1) / 1000))
	var_45_0.attackRating = arg_45_1.accuracy + arg_45_1.ACC_growth * var_45_3
	var_45_0.dodge = arg_45_1.dodge
	var_45_0.dodgeLimit = arg_45_1.dodge_limit
	var_45_0.cannonPower = arg_45_1.attack_power + arg_45_1.AP_growth * var_45_3
	var_45_0.torpedoPower = arg_45_1.attack_power + arg_45_1.AP_growth * var_45_3
	var_45_0.antiAirPower = arg_45_1.attack_power + arg_45_1.AP_growth * var_45_3
	var_45_0.antiSubPower = arg_45_1.attack_power + arg_45_1.AP_growth * var_45_3
	var_45_0.airPower = arg_45_1.attack_power + arg_45_1.AP_growth * var_45_3
	var_45_0.loadSpeed = 0
	var_45_0.armorType = 1
	var_45_0.dodgeRate = 0
	var_45_0.luck = 50
	var_45_0.velocity = ys.Battle.BattleFormulas.ConvertAircraftSpeed(arg_45_1.speed)
	var_45_0.repressReduce = 1
	var_45_0.TargetChoise = {}
	var_45_0.guardian = {}
	var_45_0.crashDMG = arg_45_1.crash_DMG

	return
end

function var_0_0:SetFusionAttrFromElement(arg_46_1, arg_46_2, arg_46_3)
	local var_46_0 = var_0_0.GetAttr(arg_46_1)
	local var_46_1 = self._attr or {}

	self._attr = var_46_1
	var_46_1.id = var_46_0.id
	var_46_1.level = var_46_0.level
	var_46_1.formulaLevel = var_46_0.level
	var_46_1.battleUID = self:GetUniqueID()

	for iter_46_0, iter_46_1 in ipairs(var_0_0.AttrListInheritance) do
		var_46_1[iter_46_1] = var_46_0[iter_46_1]
	end

	for iter_46_2, iter_46_3 in pairs(var_46_0) do
		if string.find(iter_46_2, var_0_0.TAG_EHC_KEY) then
			var_46_1[iter_46_2] = iter_46_3
		end
	end

	for iter_46_4, iter_46_5 in pairs(var_46_0) do
		if string.find(iter_46_4, var_0_0.TAG_CRI_EHC_KEY) then
			var_46_1[iter_46_4] = iter_46_5
		end
	end

	local var_46_2 = arg_46_1:GetHP()

	for iter_46_6, iter_46_7 in ipairs(arg_46_2) do
		var_46_2 = var_46_2 + iter_46_7:GetHP()
	end

	var_46_1.maxHP = var_46_2
	var_46_1.hpProvideRate = {}
	var_46_1.hpProvideRate[var_0_0.GetCurrent(arg_46_1, "id")] = arg_46_1:GetHP() / var_46_2

	for iter_46_8, iter_46_9 in ipairs(arg_46_2) do
		var_46_1.hpProvideRate[var_0_0.GetCurrent(iter_46_9, "id")] = iter_46_9:GetHP() / var_46_2
	end

	local function var_46_3(arg_47_0)
		var_46_1[arg_47_0] = var_0_0.GetCurrent(arg_46_1, arg_47_0) * (arg_46_3[arg_47_0] or 1)

		return
	end

	var_46_3("cannonPower")
	var_46_3("torpedoPower")
	var_46_3("antiAirPower")
	var_46_3("antiSubPower")
	var_46_3("baseAntiSubPower")
	var_46_3("airPower")
	var_46_3("loadSpeed")
	var_46_3("attackRating")
	var_46_3("dodgeRate")
	var_46_3("luck")
	var_46_3("velocity")
	var_46_3("baseVelocity")

	var_46_1.armorType = var_0_0.GetCurrent(arg_46_1, "armorType")
	var_46_1.aimBias = 0
	var_46_1.aimBiasDecaySpeed = 0
	var_46_1.aimBiasDecaySpeedRatio = 0
	var_46_1.aimBiasExtraACC = 0
	var_46_1.healingRate = 1
	var_46_1.comboTag = "combo_" .. var_46_1.battleUID
	var_46_1.labelTag = {}
	var_46_1.barrageCounterMod = 1
	var_46_1.TargetChoise = {}
	var_46_1.guardian = {}

	var_0_0.SetBaseAttr(self)

	return
end

function var_0_0:FlashByBuff(arg_48_1, arg_48_2)
	self._attr[arg_48_1] = arg_48_2 + (self._baseAttr[arg_48_1] or 0)

	if string.find(arg_48_1, var_0_0.FROM_TAG_EHC_KEY) then
		local var_48_1 = 0

		for iter_48_0, iter_48_1 in pairs(self._attr) do
			if string.find(iter_48_0, var_0_0.FROM_TAG_EHC_KEY) and iter_48_1 ~= 0 then
				var_48_1 = 1

				break
			end
		end

		var_0_0.SetCurrent(self, var_0_0.FROM_TAG_EHC_KEY, var_48_1)
	end

	return
end

function var_0_0:FlashVelocity(arg_49_1, arg_49_2)
	var_0_0.SetCurrent(self, "velocity", (Mathf.Clamp(self._baseAttr.velocity * arg_49_1 + arg_49_2, var_0_0.GetBase(self, "velocity") * 0.2, var_0_0.GetBase(self, "velocity") * 1.8)))

	return
end

function var_0_0:HasSonar()
	return ys.Battle.BattleConfig.VAN_SONAR_PROPERTY[self:GetTemplate().type] ~= nil
end

function var_0_0:SetCurrent(arg_51_1, arg_51_2)
	self._attr[arg_51_1] = arg_51_2

	return
end

function var_0_0.GetCurrent(arg_52_0, arg_52_1)
	return var_0_0._attrFunc[AttributeType.IsPrimalBattleAttr(arg_52_1) or false](arg_52_0, arg_52_1)
end

function var_0_0:_getPrimalAttr(arg_53_1)
	return math.max(self._attr[arg_53_1], 0)
end

function var_0_0:_getSecondaryAttr(arg_54_1)
	return self._attr[arg_54_1] or 0
end

var_0_0._attrFunc = {
	[true] = var_0_0._getPrimalAttr,
	[false] = var_0_0._getSecondaryAttr
}

function var_0_0:GetBase(arg_55_1)
	return self._baseAttr[arg_55_1] or 0
end

function var_0_0:GetCurrentTags()
	return self._attr.labelTag or {}
end

function var_0_0:Increase(arg_57_1, arg_57_2)
	if arg_57_2 then
		self._attr[arg_57_1] = (self._attr[arg_57_1] or 0) + arg_57_2
	end

	return
end

function var_0_0:RatioIncrease(arg_58_1, arg_58_2)
	if arg_58_2 then
		self._attr[arg_58_1] = self._attr[arg_58_1] + self._baseAttr[arg_58_1] * arg_58_2 / 16
	end

	return
end

function var_0_0:GetTagAttr(arg_59_1, arg_59_2)
	local var_59_0 = {}

	for iter_59_0, iter_59_1 in ipairs((arg_59_1:GetLabelTag())) do
		var_59_0[var_0_0.TAG_EHC_KEY .. iter_59_1] = true
	end

	local var_59_1 = 1

	for iter_59_2, iter_59_3 in pairs(var_59_0) do
		local var_59_2 = var_0_0.GetCurrent(self, iter_59_2)

		if var_59_2 ~= 0 then
			if arg_59_2 then
				var_59_2 = ys.Battle.BattleDataFunction.GetLimitAttributeRange(iter_59_2, var_59_2)
			end

			var_59_1 = var_59_1 * (1 + var_59_2)
		end
	end

	if var_0_0.GetCurrent(arg_59_1, var_0_0.FROM_TAG_EHC_KEY) > 0 then
		local var_59_3 = var_0_0.FROM_TAG_EHC_KEY .. self:GetWeaponTempData().attack_attribute .. "_"

		for iter_59_4, iter_59_5 in pairs((var_0_0.GetCurrentTags(self))) do
			if iter_59_5 > 0 then
				local var_59_4 = var_0_0.GetCurrent(arg_59_1, var_59_3 .. iter_59_4)

				if var_59_4 ~= 0 then
					var_59_1 = var_59_1 * (1 + var_59_4)
				end
			end
		end
	end

	return var_59_1
end

function var_0_0.GetTagAttrCri(arg_60_0, arg_60_1)
	local var_60_0 = {}

	for iter_60_0, iter_60_1 in ipairs((arg_60_1:GetLabelTag())) do
		var_60_0[var_0_0.TAG_CRI_EHC_KEY .. iter_60_1] = true
	end

	local var_60_1 = 0

	for iter_60_2, iter_60_3 in pairs(var_60_0) do
		local var_60_2 = var_0_0.GetCurrent(arg_60_0, iter_60_2)

		if var_60_2 ~= 0 then
			var_60_1 = var_60_1 + var_60_2
		end
	end

	return var_60_1
end

function var_0_0.GetTagAttrCriDmg(arg_61_0, arg_61_1)
	local var_61_0 = {}

	for iter_61_0, iter_61_1 in ipairs((arg_61_1:GetLabelTag())) do
		var_61_0[var_0_0.TAG_CRIDMG_EHC_KEY .. iter_61_1] = true
	end

	local var_61_1 = 0

	for iter_61_2, iter_61_3 in pairs(var_61_0) do
		local var_61_2 = var_0_0.GetCurrent(arg_61_0, iter_61_2)

		if var_61_2 ~= 0 then
			var_61_1 = var_61_1 + var_61_2
		end
	end

	return var_61_1
end

return
