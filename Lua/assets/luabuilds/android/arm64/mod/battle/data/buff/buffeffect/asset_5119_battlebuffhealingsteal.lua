...
function ys.Battle.BattleBuffHealingSteal:SetArgs(arg_2_1, arg_2_2)
	self._stealRate = self._tempData.arg_list.stealingRate or 1
	self._absorbRate = self._tempData.arg_list.arsorbRate or 1

	return
end

function ys.Battle.BattleBuffHealingSteal:onTakeHealing(arg_3_1, arg_3_2, arg_3_3)
	local var_3_0 = arg_3_2:GetCaster()

	if var_3_0 and var_3_0:IsAlive() and var_3_0 ~= arg_3_1 then
		local var_3_1 = math.ceil(arg_3_3.damage * self._stealRate)

		arg_3_3.damage = arg_3_3.damage - var_3_1

		var_3_0:UpdateHP(math.ceil(var_3_0:GetAttrByName("healingRate") * (var_3_1 * self._absorbRate)), {
			isMiss = false,
			isCri = false,
			isShare = false,
			isHeal = true
