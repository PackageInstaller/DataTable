...
function ys.Battle.BattleStoryWave:SetWaveData(arg_2_1)
...
function ys.Battle.BattleStoryWave:DoWave()
	...
	if var_0_0.Battle.BattleDataProxy.GetInstance():GetInitData().battleType == SYSTEM_SCENARIO then
		...
		if self._param.progress then
			if not var_3_2 then
				var_3_0 = false
			elseif math.min(var_3_2.progress + var_3_2:getConfig("progress_boss"), 100) < self._param.progress then
				var_3_0 = false
			end
		end
	...
	if var_3_0 then
		...
		ChapterOpCommand.PlayChapterStory(self._storyID, function(arg_4_0, arg_4_1)
			if arg_4_0 then
				self:doFail(arg_4_1)
			else
				self:doPass(arg_4_1)
