...
function ys.Battle.BattleGravitationBulletFactory:onBulletHitFunc(arg_3_1, arg_3_2)
	...
	local var_3_3 = self:GetBulletData()
	...
	var_3_2:SpawnLastingColumnArea(var_3_3:GetEffectField(), var_3_3:GetIFF(), pg.Tool.FilterY(var_3_6), var_3_1.range, var_3_1.time, function(arg_4_0)
		...
		return
	end, false, self:GetFXID(), function(arg_6_0)
				...
				if not var_6_1 then
					...
					if var_6_0:IsAlive() then
						if var_3_7.knockBack ~= false then
							var_6_0:SetUncontrollableSpeed(pg.Tool.FilterY(var_6_0:GetPosition() - var_3_6), 1, 0.2, 6)
						end
		...
		local var_6_3, var_6_4 = var_0_1.GetFXPool():GetFX(self:GetMissFXID())

		pg.EffectMgr.GetInstance():PlayBattleEffect(var_6_3, var_6_4:Add(var_3_6), true)
		var_3_2:RemoveBulletUnit(var_3_3:GetUniqueID())

		return
	end, true):SetDiveFilter(var_3_5)
...
function ys.Battle.BattleGravitationBulletFactory:MakeModel(arg_8_1, arg_8_2)
	local var_8_0 = arg_8_1:GetBulletData()
	local var_8_1 = var_8_0:GetTemplate()

	if not self:GetBulletPool():InstBullet(arg_8_1:GetModleID(), function(arg_9_0)
		arg_8_1:AddModel(arg_9_0)

		return
	end) then
		arg_8_1:AddTempModel(self:GetTempGOPool():GetObject())
	end

	arg_8_1:SetSpawn(arg_8_2)
	arg_8_1:SetFXFunc(self.onBulletHitFunc, self.onBulletMissFunc)
	self:GetSceneMediator():AddBullet(arg_8_1)

	if var_8_0:GetIFF() ~= self:GetDataProxy():GetFriendlyCode() and var_8_1.alert_fx ~= "" then
		arg_8_1:MakeAlert(self:GetFXPool():GetFX(var_8_1.alert_fx))
	end
