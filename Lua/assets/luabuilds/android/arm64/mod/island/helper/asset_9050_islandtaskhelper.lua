local IslandTaskHelper = class("IslandTaskHelper")

function IslandTaskHelper:GetRuntimeData(arg_1_1)
	...
	return switch(self, {
		...
		[IslandTaskTargetType.SHIP_DRESS] = function()
			return (var_1_0 == 0 or nil) and (var_1_2:GetDiffDressCnt() or var_1_2:GetDiffDressCntByType(var_1_0))
		end,
		[IslandTaskTargetType.SHIP_SKIN] = function()
			return (var_1_0 == 0 or nil) and (var_1_2:GetAllSkinCnt() or #var_1_2:GetOwnSkinListByShipId(var_1_0))
		end,
		...
		[IslandTaskTargetType.GAME_MAX_SCORE] = function()
			...
			return (var_25_0 or nil) and (var_25_0.data2 or 0)
		end,
		[IslandTaskTargetType.GAME_CUR_SCORE] = function()
			...
			return (var_26_0 or nil) and (var_26_0.data1 or 0)
		end
	}, function()
		assert(false, "not exist runtime type: " .. self)

		return
	end)
end

function IslandTaskHelper:UpdateRuntimeTaskByTargetType()
	local var_28_0 = getProxy(IslandProxy):GetIsland():GetTaskAgency()

	for iter_28_0, iter_28_1 in pairs(var_28_0:GetTasks()) do
		if iter_28_1:ExistTargetType(self) then
...
function IslandTaskHelper:UpdateClientTaskProgress(arg_29_1)
	for iter_29_0, iter_29_1 in ipairs((getProxy(IslandProxy):GetIsland():GetTaskAgency():GetDiffTargetIdsByTypeAndParam(self, arg_29_1))) do
	...
	seriesAsync({
		...
		function(arg_32_0)
			...
			for iter_32_0, iter_32_1 in pairs(getProxy(IslandProxy):GetIsland():GetTaskAgency():GetCanSubmitTasks()) do
				if iter_32_1:CheckSubmitOnApproach(self) then
		...
		IslandTaskHelper.UpdateClientTaskProgress(IslandTaskTargetType.APPROACH, self)
...
function IslandTaskHelper:OnActionEnd()
	IslandTaskHelper.UpdateClientTaskProgress(IslandTaskTargetType.ACTION_END, self)
	IslandTaskHelper.UpdateClientTaskProgress(IslandTaskTargetType.ACTION_END, 0)

	return
end

function IslandTaskHelper:OnSubmitTask()
	...
	if underscore.any(self, function(arg_37_0)
		return pg.island_task[arg_37_0].type == IslandTaskType.DAILY and pg.island_task[arg_37_0].count_offset == 1
	end) then
...
function IslandTaskHelper:_GetTaskAcceptStoryId()
	return pg.NewStoryMgr.GetInstance():StoryName2StoryId(pg.island_task[self].rec_perform)
end

function IslandTaskHelper:_GetTaskTargetLinkStoryIds()
	if pg.island_task_target[self].type ~= IslandTaskTargetType.INTERACTION then
		return nil
	end

	local var_39_0 = pg.island_interaction[pg.island_task_target[self].target_param[1]]

	if pg.island_interaction[pg.island_task_target[self].target_param[1]].type == IslandInteractionUntil.TYPE_STORY then
		local var_39_1 = pg.NewStoryMgr.GetInstance():StoryName2StoryId(var_39_0.param)

		return var_39_1 and {
			var_39_1
		} or nil
	elseif var_39_0.type == IslandInteractionUntil.TYPE_PERFORMANCE then
...
function IslandTaskHelper:_GetTaskSubmitStoryIds()
	if not pg.island_task[self].com_perform[1] then
		return nil
	end

	local var_40_0 = pg.island_task[self].com_perform[2]

	if pg.island_task[self].com_perform[1] == 1 then
		local var_40_1 = pg.NewStoryMgr.GetInstance():StoryName2StoryId(var_40_0)

		return var_40_1 and {
			var_40_1
		} or nil
	elseif pg.island_task[self].com_perform[1] == 2 then
...
function IslandTaskHelper:_GetTaskLinkStoryIds()
	local var_41_0 = {}
	local var_41_1 = IslandTaskHelper._GetTaskAcceptStoryId(self.id)

	if var_41_1 then
		table.insert(var_41_0, var_41_1)
	end

	for iter_41_0, iter_41_1 in ipairs(self:GetTargetList()) do
		if iter_41_1:IsFinish() then
			local var_41_2 = IslandTaskHelper._GetTaskTargetLinkStoryIds(iter_41_1.id)
...
function IslandTaskHelper:_GetFinishTaskLinkStoryIds()
	local var_42_0 = {}
	local var_42_1 = IslandTaskHelper._GetTaskAcceptStoryId(self)

	if var_42_1 then
		table.insert(var_42_0, var_42_1)
	end

	for iter_42_0, iter_42_1 in ipairs(pg.island_task[self].target_id) do
		local var_42_2 = IslandTaskHelper._GetTaskTargetLinkStoryIds(iter_42_1)
	...
	local var_42_3 = IslandTaskHelper._GetTaskSubmitStoryIds(self)
...
function IslandTaskHelper:FixTaskLinksStory()
	local var_43_0 = getProxy(IslandProxy):GetIsland():GetTaskAgency()
	local var_43_1 = {}

	for iter_43_0, iter_43_1 in pairs(var_43_0:GetTasks()) do
		table.insertto(var_43_1, IslandTaskHelper._GetTaskLinkStoryIds(iter_43_1))
	end

	for iter_43_2, iter_43_3 in ipairs(var_43_0:GetFinishedIds()) do
		table.insertto(var_43_1, IslandTaskHelper._GetFinishTaskLinkStoryIds(iter_43_3))
	end
	...
	for iter_43_4 = 3110000, 3119999 do
		...
		if var_43_2 then
			if pg.NewStoryMgr.GetInstance():GetPlayedFlag(iter_43_4) then
				for iter_43_5, iter_43_6 in ipairs((IslandPerformancePerformer.GetStoryNameList(var_43_2))) do
