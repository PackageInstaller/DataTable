local IslandSubmitOrderCommand = class("IslandSubmitOrderCommand", pm.SimpleCommand)

function IslandSubmitOrderCommand:execute(arg_1_1)
	...
	if var_1_0:GetOrder():IsUrgency() then
		self:HandleUrgencyOrder(var_1_0)
	elseif var_1_0:GetOrder():IsFirm() then
		self:HandleFirmOrder(var_1_0)
	else
		self:HandleCommonOrder(var_1_0)
	end

	return
end

function IslandSubmitOrderCommand:HandleUrgencyOrder(arg_2_1)
	...
	pg.ConnectionMgr.GetInstance():Send(21405, {
		...
		if arg_3_0.result == 0 then
			...
			self:sendNotification(GAME.ISLAND_SUBMIT_ORDER_DONE, {
				dropData = self:HandleDrops(arg_2_1),
				slotId = arg_2_1.id
			})
...
function IslandSubmitOrderCommand:HandleCommonOrder(arg_4_1)
	...
	pg.ConnectionMgr.GetInstance():Send(21401, {
		...
		if arg_5_0.result == 0 then
			local var_5_0 = self:HandleDrops(arg_4_1)

			self:HandleConsume(arg_4_1)
...
function IslandSubmitOrderCommand:HandleFirmOrder(arg_6_1)
	...
	pg.ConnectionMgr.GetInstance():Send(21414, {
		...
		if arg_7_0.result == 0 then
			self:HandleConsume(arg_6_1)
			self:HandleFirmActivityOrder(arg_6_1)
			...
			self:sendNotification(GAME.ISLAND_SUBMIT_ORDER_DONE, {
				dropData = self:HandleDrops(arg_6_1),
				slotId = arg_6_1.id
			})
...
function IslandSubmitOrderCommand:HandleDrops(arg_8_1)
...
function IslandSubmitOrderCommand:HandleConsume(arg_9_1)
	for iter_9_0, iter_9_1 in ipairs((arg_9_1:GetOrder():GetConsume())) do
		self:sendNotification(GAME.CONSUME_ITEM, iter_9_1)
	end

	return
end

function IslandSubmitOrderCommand:HandleFirmActivityOrder(arg_10_1)
...
return IslandSubmitOrderCommand
