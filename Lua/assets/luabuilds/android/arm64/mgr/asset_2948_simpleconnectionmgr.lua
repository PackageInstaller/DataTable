...
function pg.SimpleConnectionMgr:Connect(arg_1_1, arg_1_2, arg_1_3, arg_1_4)
	var_0_1.stopTimer()
	...
	var_0_1.timer = Timer.New(function()
		if not var_0_5 then
			...
			var_0_1.stopTimer()
			self.onDisconnected(false, DISCONNECT_TIME_OUT)
...
function pg.SimpleConnectionMgr:onData()
	if var_0_6[self.cmd] then
		local var_6_0 = var_0_0.Packer.GetInstance():Unpack(self.cmd, self:getLuaStringBuffer())

		for iter_6_0, iter_6_1 in ipairs(var_0_6[self.cmd]) do
			iter_6_1(var_6_0)
		end
	end
...
function pg.SimpleConnectionMgr:Reconnect(arg_14_1)
	self:Disconnect()
