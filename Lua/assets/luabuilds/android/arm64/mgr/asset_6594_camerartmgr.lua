...
function pg.CameraRTMgr:Create(arg_4_1)
	...
	setParent(var_4_2, self.mainTransform, false)
	...
	self.uiList["CameraRTUI" .. arg_4_1.id] = var_4_2

	return var_4_2
end

function pg.CameraRTMgr:ShowOrHide(arg_5_1, arg_5_2)
	if not self.uiList["CameraRTUI" .. arg_5_1] then
		warning("不存在CameraRTUI id=" .. arg_5_1)

		return
	end

	setActive(self.uiList["CameraRTUI" .. arg_5_1]:GetComponent("CameraRTUI").RenderCamera, arg_5_2)
	setActive(self.uiList["CameraRTUI" .. arg_5_1], arg_5_2)

	return
end

function pg.CameraRTMgr:Destroy(arg_6_1)
	if not self.uiList["CameraRTUI" .. arg_6_1] then
		warning("不存在CameraRTUI id=" .. arg_6_1)

		return
	end

	setActive(self.uiList["CameraRTUI" .. arg_6_1]:GetComponent("CameraRTUI").RenderCamera, false)
	Destroy(self.uiList["CameraRTUI" .. arg_6_1])

	self.uiList["CameraRTUI" .. arg_6_1] = nil
