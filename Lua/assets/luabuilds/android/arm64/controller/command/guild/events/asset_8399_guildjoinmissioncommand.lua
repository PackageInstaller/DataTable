local GuildJoinMissionCommand = class("GuildJoinMissionCommand", import(".GuildEventBaseCommand"))

function GuildJoinMissionCommand:execute(arg_1_1)
	...
	if not self:CanFormationMission(var_1_0.id) then
		return
	end
	...
	pg.ConnectionMgr.GetInstance():Send(61007, {
		...
		if arg_2_0.result == 0 then
			...
			self:sendNotification(GAME.GUILD_JOIN_MISSION_DONE, {
				id = var_1_1
			})
...
return GuildJoinMissionCommand
