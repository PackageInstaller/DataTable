local GetGuildShopCommand = class("GetGuildShopCommand", pm.SimpleCommand)

function GetGuildShopCommand:execute(arg_1_1)
	...
	if var_1_1 == GuildConst.MANUAL_REFRESH then
		if var_1_3:getData():getResource(PlayerConst.ResGuildCoin) < var_1_4:getGuildShop():GetResetConsume() then
	...
	pg.ConnectionMgr.GetInstance():Send(1, {
		...
		if arg_2_0.result == 0 then
			...
			self:sendNotification(GAME.GET_GUILD_SHOP_DONE)
		else
			if var_1_2 then
				var_1_2()
			end
