local ChargeSuccessCommand = class("ChargeSuccessCommand", pm.SimpleCommand)

function ChargeSuccessCommand:execute(arg_1_1)
	...
	if _.is_empty(var_1_5:getFirstChargeList() or {}) then
	...
	if var_1_1:firstPayDouble() then
		...
		if not table.contains(var_1_8, var_1_0.shopId) then
			table.insert(var_1_8, var_1_0.shopId)
		end

		var_1_5:setFirstChargeList(var_1_8)
	end

	local var_1_9 = getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_CHARGEAWARD)

	if var_1_9 and var_1_9.data1 == 0 then
		var_1_9.data1 = 1
	end

	local var_1_10, var_1_11 = getProxy(ShopsProxy):IsSkinTypeCharge(var_1_0.shopId)

	if var_1_10 then
		getProxy(ShipSkinProxy):addSkin(ShipSkin.New({
			id = var_1_11
		}))
		self:sendNotification(GAME.SKIN_BY_CHARGE_DONE, {
			skinId = var_1_11,
			id = var_1_0.shopId
		})
	end
