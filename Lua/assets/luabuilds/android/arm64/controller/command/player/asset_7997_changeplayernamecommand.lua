local ChangePlayerNameCommand = class("ChangePlayerNameCommand", pm.SimpleCommand)

function ChangePlayerNameCommand:execute(arg_1_1)
	...
	if var_1_0.name == var_1_3.name then
		pg.TipsMgr.GetInstance():ShowTips(i18n("same_player_name_tip"))

		return
	end

	if not nameValidityCheck(var_1_0.name, 4, 14, {
		"spece_illegal_tip",
	...
	if var_1_1 == 1 then
		self:ModifyNameByItem(var_1_3, var_1_0.name, var_1_2)
	elseif var_1_1 == 2 then
		self:ForceModifyName(var_1_3, var_1_0.name, var_1_2)
	end

	return
end

function ChangePlayerNameCommand:ModifyNameByItem(arg_2_1, arg_2_2, arg_2_3)
	...
	elseif var_2_3[1] == DROP_TYPE_ITEM then
		local var_2_5 = getProxy(BagProxy):getItemById(var_2_3[2])

		if not var_2_5 or var_2_5.count < var_2_3[3] then
	...
	local var_2_6 = pg.gameset.player_name_cold_time.key_value
	...
	local function var_2_7()
		pg.ConnectionMgr.GetInstance():Send(11007, {
			...
			if arg_4_0.result == 0 then
				...
				arg_2_1:updateModifyNameColdTime(pg.TimeMgr.GetInstance():GetServerTime() + var_2_6)
				var_2_2:updatePlayer(arg_2_1)
				self:sendNotification(GAME.CONSUME_ITEM, Drop.Create(var_2_3))

				if arg_2_3 then
					arg_2_3()
				end

				self:sendNotification(GAME.CHANGE_PLAYER_NAME_DONE)
	...
	pg.MsgboxMgr.GetInstance():ShowMsgBox({
		content = i18n("player_name_change_warning", var_2_4.count, var_2_4:getConfig("name"), arg_2_2),
		onYes = function()
			var_2_7()
...
function ChangePlayerNameCommand:ForceModifyName(arg_6_1, arg_6_2, arg_6_3)
...
return ChangePlayerNameCommand
