local ChargeConfirmCommand = class("ChargeConfirmCommand", pm.SimpleCommand)

function ChargeConfirmCommand:execute(arg_1_1)
	...
	pg.ConnectionMgr.GetInstance():Send(11504, {
		...
		elseif arg_2_0.result == 4 then
			self:sendNotification(GAME.CHARGE_CONFIRM_FAILED, {
				payId = var_1_1,
				bsId = var_1_2
			})
		else
