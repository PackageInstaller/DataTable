local UserRegisterCommand = class("UserRegisterCommand", pm.SimpleCommand)

function UserRegisterCommand:execute(arg_1_1)
	...
	pg.ConnectionMgr.GetInstance():Connect(NetConst.GATEWAY_HOST, NetConst.GATEWAY_PORT, function()
		pg.ConnectionMgr.GetInstance():Send(10001, {
			...
			if arg_3_0.result == 0 then
				self.facade:sendNotification(GAME.USER_REGISTER_SUCCESS, var_1_0)
			else
				self.facade:sendNotification(GAME.USER_REGISTER_FAILED, arg_3_0.result)
			end
