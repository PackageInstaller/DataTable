local UpgradeSpWeaponCommand = class("UpgradeSpWeaponCommand", pm.SimpleCommand)

function UpgradeSpWeaponCommand:execute(arg_1_1)
	...
	seriesAsync({
		function(arg_2_0)
			...
			while true do
				...
				if var_2_3 > 0 and var_2_8:GetRarity() > var_2_5:GetRarity() then
					break
				end

				if var_1_10 < var_1_12 then
					break
				end
			...
			if var_2_11.gold < var_1_13 then
			...
			if not _.all(var_1_14, function(arg_5_0)
				return arg_5_0.count <= ((var_2_10[arg_5_0.id] or nil) and (var_2_10[arg_5_0.id].count or 0))
			end) then
			...
			if not _.all(var_1_3, function(arg_6_0)
				return arg_6_0.count <= ((var_2_10[arg_6_0.id] or nil) and (var_2_10[arg_6_0.id].count or 0))
			end) then
				pg.TipsMgr.GetInstance():ShowTips(i18n("spweapon_tip_materal_no_enough"))
			...
			if var_2_9 > 0 and var_2_2 > 0 then
		...
		function(arg_8_0)
			pg.ConnectionMgr.GetInstance():Send(14203, {
				ship_id = var_1_2,
				spweapon_id = var_1_1,
				item_id_list = _.reduce(var_1_3, {}, function(arg_9_0, arg_9_1)
				...
				spweapon_id_list = _.map(var_1_4, function(arg_10_0)
				...
				if arg_11_0.result == 0 then
					...
					self:sendNotification(GAME.UPGRADE_SPWEAPON_DONE, var_11_2)
				else
