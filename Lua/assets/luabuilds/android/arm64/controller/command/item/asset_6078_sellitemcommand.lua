local SellItemCommand = class("SellItemCommand", pm.SimpleCommand)

function SellItemCommand:execute(arg_1_1)
	...
	for iter_1_0, iter_1_1 in pairs(var_1_0.items) do
		if var_1_3:getItemCountById(iter_1_1.id) < iter_1_1.count then
			pg.TipsMgr.GetInstance():ShowTips(i18n("common_no_x", var_1_3:RawGetItemById(iter_1_1.id):getConfig("name")))
	...
	pg.ConnectionMgr.GetInstance():Send(15008, {
		...
		if arg_2_0.result == 0 then
			...
			for iter_2_2, iter_2_3 in pairs((var_1_3:GetSellingPrice(var_1_1))) do
				local var_2_1 = Drop.Create(iter_2_3)

				if var_2_1.count > 0 then
					addPlayerOwn(var_2_1)
					table.insert(var_2_0, var_2_1)
				end
			end

			self:sendNotification(GAME.SELL_ITEM_DONE, {
				awards = var_2_0
			})
