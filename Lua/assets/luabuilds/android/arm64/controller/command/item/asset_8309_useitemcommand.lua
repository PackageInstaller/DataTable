local UseItemCommand = class("UseItemCommand", pm.SimpleCommand)

function UseItemCommand:execute(arg_1_1)
	...
	if not UseItemCommand.Check(var_1_5, var_1_2) then
		return
	end
	...
	elseif var_1_6 == ItemUsage.SKIN_SHOP_DISCOUNT or var_1_6 == ItemUsage.USAGE_SHOP_DISCOUNT then
		...
		if var_1_10 > 0 and getProxy(PlayerProxy):getRawData():getResource(var_1_11) < var_1_10 then
	...
	pg.ConnectionMgr.GetInstance():Send(15002, {
		...
		if arg_2_0.result == 0 then
			...
			if var_1_6 == ItemUsage.FOOD then
				self:sendNotification(GAME.ADD_FOOD, {
					id = var_1_1,
					count = var_1_2
				})
			...
			if var_1_1 >= QRJ_ITEM_ID_RANGE[1] and var_1_1 <= QRJ_ITEM_ID_RANGE[2] then
...
function UseItemCommand:Check(arg_4_1)
	local var_4_0, var_4_1 = CheckOverflow((GetItemsOverflowDic((self:GetOverflowCheckItems(arg_4_1)))))
