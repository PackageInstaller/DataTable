local StartCampTecCommand = class("StartCampTecCommand", pm.SimpleCommand)

function StartCampTecCommand:execute(arg_1_1)
	...
	if var_1_3 then
		pg.TipsMgr.GetInstance():ShowTips(i18n("technology_uplevel_error_studying", pg.fleet_tech_group[var_1_4:getStudyingTecItem()].name))

		return
	end

	pg.MsgboxMgr.GetInstance():ShowMsgBox({
		content = i18n("technology_uplevel_error_no_res", pg.fleet_tech_template[var_1_0.levelID].cost, var_1_3:DescCDTime(pg.fleet_tech_template[var_1_0.levelID].time), math.fmod(var_1_0.levelID, 1000) - 1, math.fmod(var_1_0.levelID, 1000)),
		onYes = function()
			...
			pg.ConnectionMgr.GetInstance():Send(1, {
				...
				if arg_3_0.result == 0 then
					...
					var_1_4:setTimer()
					self:sendNotification(TechnologyConst.START_TEC_BTN_SUCCESS, var_1_1)
