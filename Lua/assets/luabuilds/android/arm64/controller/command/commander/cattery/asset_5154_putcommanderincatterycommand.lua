local PutCommanderInCatteryCommand = class("PutCommanderInCatteryCommand", pm.SimpleCommand)

function PutCommanderInCatteryCommand:execute(arg_1_1)
	...
	pg.ConnectionMgr.GetInstance():Send(25030, {
		...
		if arg_2_0.result == 0 then
			if var_1_3 then
				self:UpdateCommanderLevelAndExp(var_1_8:GetCommanderId(), arg_2_0)
			...
			self:sendNotification(GAME.PUT_COMMANDER_IN_CATTERY_DONE, {
				id = var_1_8.id
			})
		else
...
function PutCommanderInCatteryCommand:UpdateCommanderLevelAndExp(arg_3_1, arg_3_2)
...
return PutCommanderInCatteryCommand
