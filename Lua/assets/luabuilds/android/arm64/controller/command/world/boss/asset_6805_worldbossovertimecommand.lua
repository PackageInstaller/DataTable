local WorldBossOverTimeCommand = class("WorldBossOverTimeCommand", pm.SimpleCommand)

function WorldBossOverTimeCommand:execute(arg_1_1)
	...
	if var_1_2 and var_1_2:IsExpired() then
		...
		self:sendNotification(GAME.WORLD_SELF_BOSS_OVERTIME_DONE)
	end

	return
end

return WorldBossOverTimeCommand
