local NewEducateScheduleCommand = class("NewEducateScheduleCommand", pm.SimpleCommand)

function NewEducateScheduleCommand:execute(arg_1_1)
	...
	pg.ConnectionMgr.GetInstance():Send(29040, {
		...
		if arg_2_0.result == 0 then
			...
			self:TrackPlan(var_2_0, var_1_2, var_2_3)
			...
			for iter_2_0, iter_2_1 in ipairs(var_1_2) do
				getProxy(NewEducateProxy):Costs((NewEducatePlan.New(iter_2_1.value):GetCostWithBenefit(var_2_4)))
			end

			self:sendNotification(GAME.NEW_EDUCATE_SCHEDULE_DONE, {
				...
				callback = function()
					if #var_2_3 > 0 then
						if not var_1_3 then
							...
							self:sendNotification(GAME.NEW_EDUCATE_SCHEDULE_SKIP, {
								id = var_1_1
							})
						end
					else
...
function NewEducateScheduleCommand:TrackPlan(arg_4_1, arg_4_2, arg_4_3)
...
return NewEducateScheduleCommand
