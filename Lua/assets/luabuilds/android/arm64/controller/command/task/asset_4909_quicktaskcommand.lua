local QuickTaskCommand = class("QuickTaskCommand", pm.SimpleCommand)

function QuickTaskCommand:execute(arg_1_1)
...
function QuickTaskCommand:OnQuickTaskComplete(arg_7_1, arg_7_2)
	...
	if self.result == 0 then
		...
		pg.m02:sendNotification(GAME.SUBMIT_TASK_DONE, PlayerConst.addTranDrop(self.award_list, {
			taskId = arg_7_1.id
		}), {
			arg_7_1.id
		})
		...
		if var_7_1 and not var_7_1:isEnd() then
			if table.contains(var_7_1:getConfig("config_data")[1] or {}, arg_7_1.id) then
		...
		pg.TipsMgr.GetInstance():ShowTips(errorTip("task_submitTask", self.result))
...
function QuickTaskCommand:AddGuildLivness()
	if self:IsGuildAddLivnessType() then
		...
		if var_8_1 and self:isGuildTask() then
