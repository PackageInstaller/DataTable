local SubmitTaskCommand = class("SubmitTaskCommand", pm.SimpleCommand)

function SubmitTaskCommand:GetSubmitActivityTask()
	return GAME.SUBMIT_ACTIVITY_TASK
end

function SubmitTaskCommand:GetSubmitTaskDone()
	return GAME.SUBMIT_TASK_DONE
end

function SubmitTaskCommand:GetSubmitTaskAwardDone()
	return GAME.SUBMIT_TASK_AWARD_DOWN
end

function SubmitTaskCommand:execute(arg_4_1)
	...
	if var_4_7:isActivityTask() then
		pg.m02:sendNotification(self:GetSubmitActivityTask(), {
	...
	if var_4_7:IsOverflowShipExpItem() and not self:InTaskScene() then
	...
	seriesAsync(var_4_8, function()
		pg.ConnectionMgr.GetInstance():Send(20005, {
			...
			if arg_9_0.result == 0 then
				...
				SubmitTaskCommand.OnSubmitSuccess(var_4_7, var_4_1)
				pg.m02:sendNotification(self:GetSubmitTaskDone(), var_9_0, {
					var_4_7.id
				})
				pg.m02:sendNotification(self:GetSubmitTaskAwardDone(), {
					awards = var_9_0
				}, {
					var_4_7.id
				})
...
function SubmitTaskCommand:OnSubmitSuccess(arg_10_1)
	SubmitTaskCommand.CheckTaskSub(self)
	SubmitTaskCommand.AddGuildLivness(self)
	SubmitTaskCommand.CheckTaskType(self)
	SubmitTaskCommand.UpdateActivity(self)
...
function SubmitTaskCommand:CheckTaskSub()
	if self:getConfig("sub_type") == TASK_SUB_TYPE_GIVE_ITEM then
		getProxy(BagProxy):removeItemById(tonumber((tonumber(self:getConfig("target_id")))), tonumber((self:getConfig("target_num"))))
	elseif self:getConfig("sub_type") == TASK_SUB_TYPE_GIVE_VIRTUAL_ITEM then
		getProxy(ActivityProxy):removeVitemById(tonumber(self:getConfig("target_id")), (self:getConfig("target_num")))
	elseif self:getConfig("sub_type") == TASK_SUB_TYPE_PLAYER_RES then
		local var_11_0 = getProxy(PlayerProxy)
		local var_11_1 = var_11_0:getData()

		var_11_1:consume({
			[id2res((tonumber(self:getConfig("target_id"))))] = self:getConfig("target_num")
		})
...
function SubmitTaskCommand:CheckTaskType()
	local var_12_0 = self:getConfig("type")
	...
	elseif var_12_0 == 8 then
		...
		getProxy(TaskProxy):updateTask(self)
	else
		getProxy(TaskProxy):removeTask(self)
	end

	return
end

function SubmitTaskCommand:AddGuildLivness()
	if self:IsGuildAddLivnessType() then
		local var_13_0 = getProxy(GuildProxy)
		local var_13_1 = var_13_0:getData()
		local var_13_2 = 0
		local var_13_3 = false

		if var_13_1 and self:isGuildTask() then
			...
			var_13_3 = true
		elseif self:IsRoutineType() then
			var_13_2 = pg.guildset.new_daily_task_guild_active.key_value
		elseif self:IsWeeklyType() then
...
function SubmitTaskCommand:UpdateActivity()
	...
	if var_14_1 and not var_14_1:isEnd() then
		if table.contains(var_14_1:getConfig("config_data")[1] or {}, self.id) then
...
function SubmitTaskCommand:InTaskScene()
