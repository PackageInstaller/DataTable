local UnlockCryptolaliaCommand = class("UnlockCryptolaliaCommand", pm.SimpleCommand)

function UnlockCryptolaliaCommand:execute(arg_1_1)
	...
	pg.ConnectionMgr.GetInstance():Send(16205, {
		...
		if arg_2_0.ret == 0 then
			...
			self:sendNotification(GAME.UNLOCK_CRYPTOLALIA_DONE, {
				id = var_1_1
			})
		else
...
return UnlockCryptolaliaCommand
