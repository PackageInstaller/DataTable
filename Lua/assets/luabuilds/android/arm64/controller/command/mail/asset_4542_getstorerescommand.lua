local GetStoreResCommand = class("GetStoreResCommand", pm.SimpleCommand)

function GetStoreResCommand:execute(arg_1_1)
	...
	pg.ConnectionMgr.GetInstance():Send(30012, {
		...
		if arg_6_0.result == 0 then
			...
			self:sendNotification(GAME.GET_STORE_RES_DONE)
		else
