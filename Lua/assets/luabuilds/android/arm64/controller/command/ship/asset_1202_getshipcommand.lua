local GetShipCommand = class("GetShipCommand", pm.SimpleCommand)

function GetShipCommand:execute(arg_1_1)
	...
	table.insert(var_1_6, function(arg_3_0)
		...
		pg.ConnectionMgr.GetInstance():Send(12025, {
			...
			for iter_8_0, iter_8_1 in ipairs(arg_8_0.ship_list) do
				...
				if var_8_1:isMetaShip() and not var_8_1.virgin and Player.isMetaShipNeedToTrans(var_8_1.configId) then
					...
					var_1_4:addShip(var_8_1)
				end
			end

			if #var_8_0 > 0 then
				var_1_2:setBuildShipState()
				self:sendNotification(GAME.GET_SHIP_DONE, {
					ships = var_8_0,
					type = var_7_0
				})
			end
