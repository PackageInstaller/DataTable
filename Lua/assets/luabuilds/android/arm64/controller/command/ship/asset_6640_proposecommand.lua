local ProposeCommand = class("ProposeCommand", pm.SimpleCommand)

function ProposeCommand:execute(arg_1_1)
	...
	if var_1_2:getProposeType() == "imas" then
		local var_1_4 = ITEM_ID_FOR_PROPOSE_IMAS or ITEM_ID_FOR_PROPOSE
	...
	pg.ConnectionMgr.GetInstance():Send(12032, {
		...
		if arg_2_0.result == 0 then
			...
			if not var_1_2:IsLocked() then
				...
				self:sendNotification(GAME.UPDATE_LOCK_DONE, var_1_2)
			else
				var_1_1:updateShip(var_1_2)
			end
			...
			self:sendNotification(GAME.PROPOSE_SHIP_DONE, {
				ship = var_1_2
			})
		else
