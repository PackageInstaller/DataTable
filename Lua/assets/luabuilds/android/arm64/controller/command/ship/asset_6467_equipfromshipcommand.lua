local EquipFromShipCommand = class("EquipFromShipCommand", pm.SimpleCommand)

function EquipFromShipCommand:execute(arg_1_1)
	...
	if var_1_8:getEquip(var_1_0.pos) then
		if var_1_7:getCapacity() >= getProxy(PlayerProxy):getData():getMaxEquipmentBag() then
	...
	seriesAsync(var_1_13, function(arg_5_0, arg_5_1, arg_5_2, arg_5_3)
		...
		pg.ConnectionMgr.GetInstance():Send(12006, {
			...
			if arg_6_0.result == 0 then
				...
				self:sendNotification(GAME.EQUIP_TO_SHIP_DONE, arg_5_0)
