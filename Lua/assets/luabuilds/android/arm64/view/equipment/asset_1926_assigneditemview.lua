local AssignedItemView = class("AssignedItemView", import("..base.BaseSubView"))

function AssignedItemView:getUIName()
	return "StoreHouseItemAssignedView"
end

function AssignedItemView:OnInit()
...
function AssignedItemView:Show()
...
function AssignedItemView:Hide()
...
function AssignedItemView:updateValue()
	...
	self.ulist:each(function(arg_15_0, arg_15_1)
		...
		setText(arg_15_1:Find("item/icon_bg/count"), self.count * self.displayDrops[arg_15_0 + 1].count)

		local var_15_0 = self:isOverLimit(arg_15_0 + 1, self.count)

		setActive(arg_15_1:Find("block_mask"), var_15_0)

		if var_15_0 and self.selectedIndex == arg_15_0 + 1 then
...
function AssignedItemView:isOverLimit(arg_16_1, arg_16_2)
	local var_16_0 = self.displayDrops[arg_16_1]
	local var_16_1 = underscore.detect(self.itemVO:getConfig("limit"), function(arg_17_0)
	...
	if not var_16_2 then
		...
		return var_16_2 < var_16_0:getOwnedCount() + var_16_0.count * self.count
	end
...
local function var_0_1(arg_18_0)
	return getProxy(CollectionProxy):getShipGroup(pg.ship_data_template[arg_18_0].group_type) ~= nil
end

function AssignedItemView:update(arg_19_1)
	self.count = 1
	self.selectedIndex = nil
	self.selectedItem = nil
	self.itemVO = arg_19_1
	self.displayDrops = underscore.map(arg_19_1:getConfig("display_icon"), function(arg_20_0)
		return Drop.Create(arg_20_0)
	end)

	local var_19_0 = arg_19_1:getConfig("time_limit") == 1

	self.ulist:make(function(arg_21_0, arg_21_1, arg_21_2)
		arg_21_1 = arg_21_1 + 1

		if arg_21_0 == UIItemList.EventUpdate then
			local var_21_0 = self.displayDrops[arg_21_1]

			updateDrop(arg_21_2:Find("item"), self.displayDrops[arg_21_1])
			onToggle(self, arg_21_2, function(arg_22_0)
				if arg_22_0 then
					self.selectedIndex = arg_21_1
					self.selectedItem = arg_21_2
				elseif self.selectedIndex == arg_21_1 then
					self.selectedIndex = nil
					self.selectedItem = nil
				end
			...
			onButton(self, arg_21_2:Find("block_mask"), function()
				pg.TipsMgr.GetInstance():ShowTips(i18n("item_assigned_type_limit_error"))

				return
			end, SFX_CANCEL)

			if not self.selectedItem and not self:isOverLimit(arg_21_1, self.count) then
				self.selectedItem = arg_21_2
			end
		end
	...
	UpdateOwnDisplay(self.itemTF:Find("left/own"), var_19_1)

	if underscore.any(self.displayDrops, function(arg_24_0)
		...
		RegisterDetailButton(self, self.itemTF:Find("left/detail"), var_19_1)
	else
		removeOnButton(self.itemTF:Find("left/detail"))
	end

	setText(self.nameTF, arg_19_1:getConfig("name"))
	setText(self.descTF, arg_19_1:getConfig("display"))

	return
end

function AssignedItemView:OnDestroy()
