local NewBattleResultBackSceneHandler = class("NewBattleResultBackSceneHandler", pm.Mediator)

function NewBattleResultBackSceneHandler:Ctor(arg_1_1)
	NewBattleResultBackSceneHandler.super.Ctor(self)
	pg.m02:registerMediator(self)

	self.contextData = arg_1_1

	return
end

function NewBattleResultBackSceneHandler:Execute()
	if self.contextData.system == SYSTEM_DUEL then
		self:ExitDuelSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_ACT_BOSS then
		self:ExitActBossSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_ROUTINE or self.contextData.system == SYSTEM_SUB_ROUTINE then
		self:ExitRoutineSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_SCENARIO then
		self:ExitScenarioSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_CHALLENGE then
		self:ExitChallengeSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_HP_SHARE_ACT_BOSS or self.contextData.system == SYSTEM_BOSS_EXPERIMENT or self.contextData.system == SYSTEM_ACT_BOSS_SP then
		self:ExitShareBossSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_WORLD_BOSS then
		self:ExitWorldBossSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_WORLD then
		self:ExitWorldSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_BOSS_RUSH or self.contextData.system == SYSTEM_BOSS_RUSH_EX or self.contextData.system == SYSTEM_BOSS_RUSH_COLLABRATE then
		if self:CheckBossRushSystem(self.contextData) then
			self:ResultRushBossSystem(self.contextData)
		end
	elseif self.contextData.system == SYSTEM_LIMIT_CHALLENGE then
		self:ExitLimitChallengeSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_BOSS_SINGLE then
		self:ExitBossSingleSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_BOSS_SINGLE_VARIABLE then
		self:ExitBossSingleVariableSystem(self.contextData)
	elseif self.contextData.system == SYSTEM_REWARD_PERFORM then
		self:ExitRewardPerform(self.contextData)
	else
		self:ExitCommonSystem(self.contextData)
	end
...
function NewBattleResultBackSceneHandler:ExitDuelSystem(arg_3_1)
...
function NewBattleResultBackSceneHandler:ExitActBossSystem(arg_4_1)
	...
	if getProxy(ContextProxy):getCurrentContext():getContextByMediator(ContinuousOperationMediator) then
		self:CheckActBossSystem(arg_4_1)
	else
		pg.m02:sendNotification(GAME.GO_BACK)
	end
...
function NewBattleResultBackSceneHandler:ExitRoutineSystem(arg_5_1)
...
function NewBattleResultBackSceneHandler:ExitScenarioSystem(arg_6_1)
	...
	if arg_6_1.score > ys.Battle.BattleConst.BattleScore.C then
		self:ShowExtraChapterActSocre(arg_6_1)
	end
...
function NewBattleResultBackSceneHandler:ExitChallengeSystem(arg_7_1)
	getProxy(ChallengeProxy):WriteBackOnExitBattleResult(self.contextData.score, self.contextData.mode)
...
function NewBattleResultBackSceneHandler:ExitShareBossSystem(arg_8_1)
...
function NewBattleResultBackSceneHandler:ExitWorldBossSystem(arg_9_1)
...
function NewBattleResultBackSceneHandler:ExitWorldSystem(arg_10_1)
...
function NewBattleResultBackSceneHandler:ResultRushBossSystem(arg_11_1)
...
function NewBattleResultBackSceneHandler:ExitRushBossSystem(arg_13_1, arg_13_2)
...
function NewBattleResultBackSceneHandler:ExitLimitChallengeSystem(arg_14_1)
...
function NewBattleResultBackSceneHandler:ExitBossSingleSystem(arg_15_1)
	...
	if getProxy(ContextProxy):getCurrentContext():getContextByMediator(BossSingleContinuousOperationMediator) then
		self:CheckBossSingleSystem(arg_15_1)
	else
		pg.m02:sendNotification(GAME.GO_BACK)
	end
...
function NewBattleResultBackSceneHandler:ExitBossSingleVariableSystem(arg_16_1)
	...
	if getProxy(ContextProxy):getCurrentContext():getContextByMediator(BossSingleContinuousOperationMediator) then
		self:CheckBossSingleSystem(arg_16_1)
	else
		pg.m02:sendNotification(GAME.GO_BACK)
	end
...
function NewBattleResultBackSceneHandler:ExitRewardPerform(arg_17_1)
...
function NewBattleResultBackSceneHandler:ExitCommonSystem(arg_18_1)
...
function NewBattleResultBackSceneHandler:ShowExtraChapterActSocre(arg_20_1)
	local var_20_9000
	...
	for iter_20_0, iter_20_1 in ipairs((var_0_1())) do
		if iter_20_1:getConfig("config_data")[1] == arg_20_1.stageId and var_20_0:IsEXChapter() then
			local var_20_1 = arg_20_1.prefabFleet

			if not arg_20_1.prefabFleet then
				var_20_1 = arg_20_1.oldMainShips

				local var_20_3 = math.floor(arg_20_1.statistics._totalTime)
			end

			local var_20_4, var_20_5 = ActivityLevelConst.getExtraChapterSocre(arg_20_1.stageId, var_20_9000, ActivityLevelConst.getShipsPower(var_20_1), iter_20_1)
			local var_20_6 = var_20_5 < var_20_4 and i18n("extra_chapter_record_updated") or i18n("extra_chapter_record_not_updated")

			if var_20_5 < var_20_4 then
				iter_20_1.data1 = var_20_4
			...
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				hideNo = true,
				content = i18n("extra_chapter_socre_tip", var_20_4, var_20_5, var_20_6)
			})
		end
	end
...
local function var_0_2(arg_21_0)
	...
	local function var_21_5(arg_22_0, arg_22_1)
		...
		var_21_3 = var_21_3 + var_22_0

		return
	end

	var_21_5(var_21_2[arg_21_0.mainFleetId], (var_21_1 or nil) and (var_21_4[1] or 0))
	var_21_5(var_21_2[arg_21_0.mainFleetId + 10], (var_21_1 or nil) and (var_21_4[2] or 0))
...
local function var_0_3(arg_23_0, arg_23_1)
	...
	if var_23_0 then
		var_23_1 = var_23_0.data.autoFlag or nil
	end
...
local function var_0_4(arg_25_0, arg_25_1)
	...
	if var_25_0 then
		var_25_1 = var_25_0.data.autoFlag or nil
	end
...
function NewBattleResultBackSceneHandler:CheckActBossSystem(arg_30_1)
	...
	local var_30_0 = _.map(_.values(getProxy(FleetProxy):getActivityFleets()[arg_30_1.actId][arg_30_1.mainFleetId].ships), function(arg_31_0)
	...
	if #var_30_0 > 0 then
		var_0_3(arg_30_1, i18n("multiple_sorties_stop_reason2", Fleet.DEFAULT_NAME_BOSS_ACT[arg_30_1.mainFleetId], table.concat(_.map(var_30_0, function(arg_32_0)
	...
	local var_30_1 = getProxy(ContextProxy)
	local var_30_2 = getProxy(ContextProxy):getCurrentContext():getContextByMediator(ContinuousOperationMediator)

	if var_30_2 and not var_30_2.data.autoFlag then
		var_0_3(arg_30_1)
...
function NewBattleResultBackSceneHandler:ContinuousBossRush(arg_33_1, arg_33_2, arg_33_3, arg_33_4, arg_33_5, arg_33_6)
...
function NewBattleResultBackSceneHandler:CheckBossRushSystem(arg_34_1)
	...
	local var_34_7 = not var_34_1 or var_34_4 > #var_34_5 or not (not var_34_6 or var_34_6.data.autoFlag)

	self.contextData.isAutoFight = not var_34_6 or var_34_6.data.autoFlag

	if not var_34_7 then
		self:ContinuousBossRush(arg_34_1.system, var_34_2, var_34_4, var_34_5, arg_34_1.continuousBattleTimes, arg_34_1.totalBattleTimes)
	end
...
local function var_0_6(arg_35_0)
	...
	local var_35_1 = var_35_0:getActivityById(arg_35_0.actId)
	...
	local function var_35_5(arg_36_0, arg_36_1)
	...
	var_35_5(var_35_3[arg_35_0.mainFleetId], var_35_0[1] or 0)
	var_35_5(var_35_3[arg_35_0.mainFleetId + ((var_35_1:getConfig("type") == ActivityConst.ACTIVITY_TYPE_BOSSSINGLE_VARIABLE or nil) and (Fleet.MEGA_SUBMARINE_FLEET_OFFSET or 10))], var_35_2[2] or 0)

	return var_35_4
end

function NewBattleResultBackSceneHandler:CheckBossSingleSystem(arg_37_1)
	...
	local var_37_0 = _.map(_.values(getProxy(FleetProxy):getActivityFleets()[arg_37_1.actId][arg_37_1.mainFleetId].ships), function(arg_38_0)
	...
	if #var_37_0 > 0 then
		var_0_4(arg_37_1, i18n("multiple_sorties_stop_reason2", Fleet.DEFAULT_NAME_BOSS_ACT[arg_37_1.mainFleetId], table.concat(_.map(var_37_0, function(arg_39_0)
	...
	local var_37_1 = getProxy(ContextProxy)
	local var_37_2 = getProxy(ContextProxy):getCurrentContext():getContextByMediator(BossSingleContinuousOperationMediator)

	if var_37_2 and not var_37_2.data.autoFlag then
		var_0_4(arg_37_1)
...
local function var_0_7(arg_40_0, arg_40_1)
	...
	if var_40_0:GetStageBonus(arg_40_1) == 0 then
		if getProxy(SettingsProxy):isTipActBossExchangeTicket() == 1 and getProxy(PlayerProxy):getRawData():getResource(pg.activity_event_worldboss[var_40_0:getConfig("config_id")].ticket) > 0 then
			return true
		end
	end
...
function NewBattleResultBackSceneHandler:listNotificationInterests()
	return {
...
function NewBattleResultBackSceneHandler:addSubLayers(arg_44_1, arg_44_2, arg_44_3)
...
function NewBattleResultBackSceneHandler:Dispose()
	pg.m02:removeMediator(self.__cname)

	return
end

return NewBattleResultBackSceneHandler
