local CommanderFormationPage = class("CommanderFormationPage", import("...base.BaseSubView"))

function CommanderFormationPage:getUIName()
	return "CommanderFormationUI"
end

function CommanderFormationPage:OnInit()
	setActive(self.samllTF, true)

	self.pos1 = self.samllTF:Find("commander1")
	self.pos2 = self.samllTF:Find("commander2")

	setActive(self.descPanel, false)

	self.descFrameTF = self.descPanel:Find("frame")
	self.descPos1 = self.descFrameTF:Find("commander1/frame/info")
	self.descPos2 = self.descFrameTF:Find("commander2/frame/info")
	self.skillTFPos1 = self.descFrameTF:Find("commander1/skill_info")
	self.skillTFPos2 = self.descFrameTF:Find("commander2/skill_info")
	self.abilitysTF = UIItemList.New(self.descFrameTF:Find("atttr_panel/abilitys/mask/content"), self.descFrameTF:Find("atttr_panel/abilitys/mask/content/attr"))
	self.talentsTF = UIItemList.New(self.descFrameTF:Find("atttr_panel/talents/mask/content"), self.descFrameTF:Find("atttr_panel/talents/mask/content/attr"))
	self.abilityArr = self.descPanel:Find("frame/atttr_panel/abilitys/arr")
	self.talentsArr = self.descPanel:Find("frame/atttr_panel/talents/arr")
	self.restAllBtn = self.descFrameTF:Find("rest_all")
	self.quickBtn = self.descFrameTF:Find("quick_btn")
	self.recordCommanders = {
		self.recordPanel:Find("current/commanders/commander1/frame/info"),
		self.recordPanel:Find("current/commanders/commander2/frame/info")
	}
	self.reocrdSkills = {
		self.recordPanel:Find("current/commanders/commander1/skill_info"),
		self.recordPanel:Find("current/commanders/commander2/skill_info")
	}
	self.recordList = UIItemList.New(self.recordPanel:Find("record/content"), self.recordPanel:Find("record/content/commanders"))

	onButton(self, self.samllTF, function()
		self:openDescPanel()

		return
	end, SFX_PANEL)
	onButton(self, self.quickBtn, function()
		self:OpenRecordPanel()

		return
	end, SFX_PANEL)
	onButton(self, self._tf:Find("bg"), function()
		if isActive(self.recordPanel) then
			self:CloseRecordPanel()
		elseif isActive(self.descPanel) then
			self:closeDescPanel()
		end

		return
	end, SFX_PANEL)
	onButton(self, self.restAllBtn, function()
		self:emit(FormationMediator.COMMANDER_FORMATION_OP, {
			FleetType = LevelUIConst.FLEET_TYPE_SELECT,
			data = {
				type = LevelUIConst.COMMANDER_OP_REST_ALL
			},
			fleetId = self.fleet.id
		})

		return
	end, SFX_PANEL)
	setText(self.descPanel:Find("frame/atttr_panel/abilitys/title/Text"), i18n("commander_subtile_ablity"))
	setText(self.descPanel:Find("frame/atttr_panel/talents/title/Text"), i18n("commander_subtile_talent"))
	setText(self.recordPanel:Find("current/title/Text"), i18n("commander_formation_prefab_fleet"))

	return
end

function CommanderFormationPage:Update(arg_7_1, arg_7_2)
	self.fleet = arg_7_1
	self.prefabFleets = arg_7_2

	local var_7_0 = self.fleet:getCommanders()

	for iter_7_0 = 1, CommanderConst.MAX_FORMATION_POS do
		assert(self["pos" .. iter_7_0], "pos tf can not nil")
		self:updateCommander(self["pos" .. iter_7_0], iter_7_0, var_7_0[iter_7_0])
	end

	self:updateDesc()
	self:updateRecordPanel()

	return
end

function CommanderFormationPage:openDescPanel(arg_8_1)
	local var_8_0 = arg_8_1 or 0.2

	if LeanTween.isTweening(go(self.samllTF)) or LeanTween.isTweening(go(self.descFrameTF)) then
		return
	end

	setAnchoredPosition(self.samllTF, {
		x = 0
	})
	LeanTween.moveX(self.samllTF, 800, var_8_0):setOnComplete(System.Action(function()
		setActive(self.descPanel, true)
		setActive(self.descBg, true)
		pg.UIMgr.GetInstance():OverlayPanel(self._tf)
		setAnchoredPosition(self.descFrameTF, {
			x = 800
		})
		LeanTween.moveX(self.descFrameTF, 0, var_8_0)

		return
	end))

	self.contextData.inDescPage = true

	return
end

function CommanderFormationPage:closeDescPanel(arg_10_1)
	local var_10_0 = arg_10_1 or 0.2

	if LeanTween.isTweening(go(self.samllTF)) or LeanTween.isTweening(go(self.descFrameTF)) then
		return
	end

	setAnchoredPosition(self.descFrameTF, {
		x = 0
	})
	LeanTween.moveX(self.descFrameTF, 800, var_10_0):setOnComplete(System.Action(function()
		setActive(self.descPanel, false)
		setActive(self.descBg, false)
		pg.UIMgr.GetInstance():UnOverlayPanel(self._tf, self._parentTf)
		setAnchoredPosition(self.samllTF, {
			x = 800
		})
		LeanTween.moveX(self.samllTF, 0, var_10_0)

		return
	end))

	self.contextData.inDescPage = false

	return
end

function CommanderFormationPage:updateDesc()
	local var_12_0 = self.fleet:getCommanders()

	for iter_12_0 = 1, CommanderConst.MAX_FORMATION_POS do
		assert(self["pos" .. iter_12_0], "pos tf can not nil")
		self:updateCommander(self["descPos" .. iter_12_0], iter_12_0, var_12_0[iter_12_0], true)
		self:updateSkillTF(var_12_0[iter_12_0], self["skillTFPos" .. iter_12_0])
	end

	self:updateAdditions()

	return
end

function CommanderFormationPage:updateAdditions()
	local var_13_0 = _.values(self.fleet:getCommandersTalentDesc())
	local var_13_1, var_13_2 = self.fleet:getCommandersAddition()

	self.abilitysTF:make(function(arg_14_0, arg_14_1, arg_14_2)
		if arg_14_0 == UIItemList.EventUpdate then
			setText(arg_14_2:Find("name"), AttributeType.Type2Name(var_13_1[arg_14_1 + 1].attrName))
			setText(arg_14_2:Find("Text"), ("+" .. math.floor(var_13_1[arg_14_1 + 1].value * 1000) / 1000) .. "%")
			GetImageSpriteFromAtlasAsync("attricon", var_13_1[arg_14_1 + 1].attrName, arg_14_2:Find("icon"), false)
			setImageAlpha(arg_14_2:Find("bg"), arg_14_1 % 2)
		end

		return
	end)
	self.abilitysTF:align(#var_13_1)
	setActive(self.abilityArr, #var_13_1 > 4)
	self.talentsTF:make(function(arg_15_0, arg_15_1, arg_15_2)
		if arg_15_0 == UIItemList.EventUpdate then
			local var_15_0 = var_13_0[arg_15_1 + 1]

			setScrollText(findTF(arg_15_2, "name_mask/name"), var_13_0[arg_15_1 + 1].name)
			setText(arg_15_2:Find("Text"), (var_15_0.value > 0 and "+" or "") .. var_15_0.value .. (var_15_0.type == CommanderConst.TALENT_ADDITION_RATIO and "%" or ""))
			setImageAlpha(arg_15_2:Find("bg"), arg_15_1 % 2)
		end

		return
	end)
	self.talentsTF:align(#var_13_0)
	setActive(self.talentsArr, #var_13_0 > 4)
	Canvas.ForceUpdateCanvases()

	return
end

function CommanderFormationPage:updateSkillTF(arg_16_1, arg_16_2)
	setActive(arg_16_2, arg_16_1)

	if arg_16_1 then
		local var_16_0 = arg_16_1:getSkills()[1]

		GetImageSpriteFromAtlasAsync("CommanderSkillIcon/" .. var_16_0:getConfig("icon"), "", arg_16_2:Find("icon"))
		setText(arg_16_2:Find("level"), "Lv." .. var_16_0:getLevel())
		onButton(self, arg_16_2, function()
			self:emit(FormationMediator.ON_CMD_SKILL, var_16_0)

			return
		end, SFX_PANEL)
	else
		removeOnButton(arg_16_2)
	end

	return
end

function CommanderFormationPage:updateCommander(arg_18_1, arg_18_2, arg_18_3, arg_18_4)
	local var_18_0 = arg_18_1:Find("add")
	local var_18_1 = arg_18_1:Find("info")

	if arg_18_3 then
		GetImageSpriteFromAtlasAsync("CommanderHrz/" .. arg_18_3:getPainting(), "", (arg_18_1:Find("info/mask/icon")))

		local var_18_2 = arg_18_1:Find("info/name")

		if var_18_2 then
			setText(var_18_2, arg_18_3:getName())
		end

		setImageSprite(arg_18_1:Find("info/frame"), GetSpriteFromAtlas("weaponframes", "commander_" .. Commander.rarity2Frame(arg_18_3:getRarity())))
	end

	if arg_18_4 then
		onButton(self, var_18_1, function()
			self:emit(FormationMediator.ON_SELECT_COMMANDER, arg_18_2, self.fleet.id)

			return
		end, SFX_PANEL)
		onButton(self, var_18_0, function()
			self:emit(FormationMediator.ON_SELECT_COMMANDER, arg_18_2, self.fleet.id)

			return
		end, SFX_PANEL)
	end

	setActive(var_18_0, not arg_18_3)
	setActive(var_18_1, arg_18_3)

	return
end

function CommanderFormationPage:OpenRecordPanel()
	setActive(self.descFrameTF, false)
	setActive(self.recordPanel, true)

	return
end

function CommanderFormationPage:updateRecordPanel()
	local var_22_0 = self.fleet:getCommanders()

	for iter_22_0, iter_22_1 in ipairs(self.recordCommanders) do
		self:updateCommander(iter_22_1, iter_22_0, var_22_0[iter_22_0])
		self:updateSkillTF(var_22_0[iter_22_0], self.reocrdSkills[iter_22_0])
	end

	self.recordList:make(function(arg_23_0, arg_23_1, arg_23_2)
		if arg_23_0 == UIItemList.EventUpdate then
			self:UpdatePrefabFleet(self.prefabFleets[arg_23_1 + 1], arg_23_2, var_22_0)
		end

		return
	end)
	self.recordList:align(#self.prefabFleets)

	return
end

function CommanderFormationPage:UpdatePrefabFleet(arg_24_1, arg_24_2, arg_24_3)
	local var_24_0 = arg_24_2:Find("fleet_name")
	local var_24_1 = arg_24_1:getName()

	onInputEndEdit(self, var_24_0, function()
		self:emit(FormationMediator.COMMANDER_FORMATION_OP, {
			FleetType = LevelUIConst.FLEET_TYPE_SELECT,
			data = {
				type = LevelUIConst.COMMANDER_OP_RENAME,
				id = arg_24_1.id,
				str = getInputText(var_24_0),
				onFailed = function()
					setInputText(var_24_0, var_24_1)

					return
				end
			},
			fleetId = self.fleet.id
		})

		return
	end)
	setInputText(var_24_0, var_24_1)
	onButton(self, arg_24_2:Find("use_btn"), function()
		self:emit(FormationMediator.COMMANDER_FORMATION_OP, {
			FleetType = LevelUIConst.FLEET_TYPE_SELECT,
			data = {
				type = LevelUIConst.COMMANDER_OP_USE_PREFAB,
				id = arg_24_1.id
			},
			fleetId = self.fleet.id
		})
		self:CloseRecordPanel()

		return
	end, SFX_PANEL)
	onButton(self, arg_24_2:Find("record_btn"), function()
		self:emit(FormationMediator.COMMANDER_FORMATION_OP, {
			FleetType = LevelUIConst.FLEET_TYPE_SELECT,
			data = {
				type = LevelUIConst.COMMANDER_OP_RECORD_PREFAB,
				id = arg_24_1.id
			},
			fleetId = self.fleet.id
		})

		return
	end, SFX_PANEL)

	local var_24_2 = {
		arg_24_2:Find("commander1/skill_info"),
		arg_24_2:Find("commander2/skill_info")
	}

	for iter_24_0, iter_24_1 in ipairs({
		arg_24_2:Find("commander1/frame/info"),
		arg_24_2:Find("commander2/frame/info")
	}) do
		local var_24_3 = arg_24_1:getCommanderByPos(iter_24_0)

		self:updateCommander(iter_24_1, iter_24_0, var_24_3)
		self:updateSkillTF(var_24_3, var_24_2[iter_24_0])
	end

	return
end

function CommanderFormationPage:CloseRecordPanel()
	setActive(self.descFrameTF, true)
	setActive(self.recordPanel, false)

	return
end

function CommanderFormationPage:OnDestroy()
	if self:isShowing() then
		LeanTween.cancel(go(self.samllTF))
		LeanTween.cancel(go(self.descFrameTF))

		if isActive(self.descPanel) then
			pg.UIMgr.GetInstance():UnOverlayPanel(self._tf, self._parentTf)
		end
	end

	return
end

return CommanderFormationPage
