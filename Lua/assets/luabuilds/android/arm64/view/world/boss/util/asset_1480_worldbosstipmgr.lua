...
function pg.WorldBossTipMgr:Show(arg_3_1)
	if var_0_1 then
		local function var_3_0()
			if self:IsEnable(arg_3_1:GetType()) then
	...
	if var_0_0 and self:IsEnableNotify(arg_3_1:GetType()) then
		...
		local var_3_5 = getProxy(PlayerProxy)
		local var_3_6 = {
			id = 4,
			timestamp = pg.TimeMgr.GetInstance().GetServerTime(var_3_5),
			args = {
				isDeath = false,
				supportType = var_3_4,
				playerName = var_3_1,
			...
			player = arg_3_1:GetPlayer() or var_3_5:getData(),
			uniqueId = arg_3_1.id .. "_" .. arg_3_1.lastTime
		}

		if var_3_3 == ChatConst.ChannelGuild then
			self:AddGuildMsg(var_3_3, var_3_6)
		else
			getProxy(ChatProxy):addNewMsg(ChatMsg.New(var_3_3, var_3_6))
		end
	end
...
function pg.WorldBossTipMgr:IsEnable(arg_7_1)
	return self:IsEnableNotify(arg_7_1) and (function()
...
function pg.WorldBossTipMgr:Start()
	if #self.list > 0 then
		self:AddTimer()
	end
...
function pg.WorldBossTipMgr:AddTimer()
	self:RemoveTimer()
	setActive(self.tipTF, true)
	self.scrollText:SetText(self:BuildClickableTxt(self.list[1]))
	LeanTween.value(go(self.tipTF), 1, 0, 1):setOnUpdate(System.Action_float(function(arg_13_0)
		self.tipTFCG.alpha = arg_13_0
...
function pg.WorldBossTipMgr.OnClick(arg_17_0, arg_17_1, arg_17_2, arg_17_3, arg_17_4)
	...
	if var_17_1.isSetup then
		local var_17_2 = var_17_1:GetBossById(tonumber(arg_17_2))

		if not var_17_2 or var_17_2:isDeath() then
			local var_17_3 = getProxy(ChatProxy)
			local var_17_4

			if var_17_2 then
				var_17_4 = var_17_2.lastTime or "0"
			end

			for iter_17_0, iter_17_1 in ipairs((var_17_3:GetMessagesByUniqueId(tonumber(arg_17_2) .. "_" .. var_17_4))) do
				iter_17_1.args.isDeath = true

				var_17_3:UpdateMsg(iter_17_1)
			end

			local var_17_5 = getProxy(GuildProxy)

			for iter_17_2, iter_17_3 in ipairs((var_17_5:GetMessagesByUniqueId(tonumber(arg_17_2) .. "_" .. var_17_4))) do
				iter_17_3.args.isDeath = true

				var_17_5:UpdateMsg(iter_17_3)
			end
...
function pg.WorldBossTipMgr:RemoveTimer()
	if LeanTween.isTweening(go(self.tipTF)) then
		LeanTween.cancel(go(self.tipTF))
	end
