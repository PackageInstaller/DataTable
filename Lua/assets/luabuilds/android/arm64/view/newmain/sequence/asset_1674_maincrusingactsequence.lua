local MainCrusingActSequence = class("MainCrusingActSequence")
local var_0_1 = false

function MainCrusingActSequence:Execute(arg_1_1)
	...
	if var_1_0 and not var_1_0:isEnd() then
		table.insert(var_1_1, function(arg_2_0)
			self:CheckCrusingAct(var_1_0, arg_2_0)
...
function MainCrusingActSequence:ShowWindow(arg_7_1)
	local var_7_0 = {}

	var_7_0.parentContext = getProxy(ContextProxy):getCurrentContext()
	var_7_0.context = Context.New({
	...
	pg.m02:sendNotification(GAME.LOAD_LAYERS, var_7_0)

	return
end

function MainCrusingActSequence:CheckCrusingAct(arg_8_1, arg_8_2)
	...
	if #var_8_1 > 0 and PlayerPrefs.GetInt(string.format("crusing_%d_last_time", arg_8_1.id), 3) > math.floor(var_8_0 / 0) then
		...
		self:ShowMsg(var_8_1, var_8_0, arg_8_2)
	else
		arg_8_2()
	end
...
function MainCrusingActSequence:ShowMsg(arg_9_1, arg_9_2, arg_9_3)
	if arg_9_2 < 0 then
		pg.MsgboxMgr.GetInstance():ShowMsgBox({
			hideNo = true,
