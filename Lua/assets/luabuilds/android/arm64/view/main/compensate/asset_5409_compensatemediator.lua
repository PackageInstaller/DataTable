local CompensateMediator = class("CompensateMediator", import("view.base.ContextMediator"))

CompensateMediator.ON_GET_REWARD = "CompensateMediator.ON_GET_REWARD"

function CompensateMediator:register()
	local var_1_0 = getProxy(CompensateProxy)

	self:bind(CompensateMediator.ON_GET_REWARD, function(arg_2_0, arg_2_1)
		self:sendNotification(GAME.GET_COMPENSATE_REWARD, arg_2_1)
...
function CompensateMediator:initNotificationHandleDic()
	self.handleDic = {
...
function CompensateMediator:ShowAndCheckDrops(arg_7_1)
	if not arg_7_1 then
		return
	end
	...
	if #arg_7_1 > 0 then
		table.insert(var_7_0, function(arg_8_0)
			self.viewComponent:emit(BaseUI.ON_ACHIEVE, arg_7_1, arg_8_0)
