local BossRushBattleResultMediator = class("BossRushBattleResultMediator", import("view.base.ContextMediator"))

BossRushBattleResultMediator.ON_SETTLE = "BossRushBattleResultMediator:ON_SETTLE"
BossRushBattleResultMediator.BEGIN_STAGE = "BossRushBattleResultMediator:BEGIN_STAGE"

function BossRushBattleResultMediator:register()
	self:bind(BossRushBattleResultMediator.ON_SETTLE, function()
		if not self.contextData.win or self.contextData.system == SYSTEM_BOSS_RUSH_EX then
	...
	self:bind(BossRushBattleResultMediator.BEGIN_STAGE, function(arg_4_0)
...
function BossRushBattleResultMediator:listNotificationInterests()
	return {
...
function BossRushBattleResultMediator:handleNotification(arg_6_1)
	...
	elseif var_6_0 == GAME.BOSSRUSH_TRACE_DONE then
		self.viewComponent:emit(BossRushBattleResultMediator.BEGIN_STAGE)
...
function BossRushBattleResultMediator:ShowTotalAward(arg_13_1)
	local var_13_0 = getProxy(ActivityProxy):getActivityById(self.contextData.actId)
	local var_13_1, var_13_2 = getProxy(ContextProxy):getContextByMediator(_G[var_13_0 and var_13_0:getConfig("config_client").mediator or "BossRushKurskMediator"])
...
function BossRushBattleResultMediator:remove()
	return
end

return BossRushBattleResultMediator
