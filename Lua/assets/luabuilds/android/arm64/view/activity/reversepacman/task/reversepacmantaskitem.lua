local ReversePacmanTaskItem = class("ReversePacmanTaskItem", import("view.base.BasePanel"))

function ReversePacmanTaskItem:Ctor(arg_1_1, arg_1_2)
	self._go = arg_1_1.gameObject

	ReversePacmanTaskItem.super.Ctor(self, self._go)
...
function ReversePacmanTaskItem:Init()
	onButton(self, self.uiGoBtn, function()
		self:emit(ReversePacmanTaskMediator.ON_TASK_GO, self.taskVO)

		return
	end, SFX_PANEL)
	onButton(self, self.uiGetBtn, function()
		self:emit(ReversePacmanTaskMediator.ON_TASK_SUBMIT, self.taskVO)
...
function ReversePacmanTaskItem:didEnter()
	return
end

function ReversePacmanTaskItem:willExit()
	self:detach()

	return
end

function ReversePacmanTaskItem:SetData(arg_9_1)
	self.taskVO = arg_9_1
