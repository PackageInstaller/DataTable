local SixthAnniversaryIslandShopWindowLayer = class("SixthAnniversaryIslandShopWindowLayer", import("..base.BaseUI"))

function SixthAnniversaryIslandShopWindowLayer:getUIName()
	return "SixthAnniversaryIslandGoodsWindow"
end

function SixthAnniversaryIslandShopWindowLayer:setGoods(arg_2_1)
	self.goods = arg_2_1
	self.singleCost = arg_2_1:getConfig("resource_num")
	self.max = math.floor(Drop.New({
		type = arg_2_1:getConfig("resource_category"),
		id = arg_2_1:getConfig("resource_type")
	}):getOwnedCount() / self.singleCost)
...
function SixthAnniversaryIslandShopWindowLayer:init()
	...
	onButton(self, self._tf:Find("content/bottom/btn_confirm"), function()
		...
		self:emit(SixthAnniversaryIslandShopWindowMediator.SHOPPING_CONFIRM, self.count)
...
function SixthAnniversaryIslandShopWindowLayer:updateCount(arg_12_1)
...
function SixthAnniversaryIslandShopWindowLayer:didEnter()
	local var_13_0 = {
...
function SixthAnniversaryIslandShopWindowLayer:willExit()
	pg.UIMgr.GetInstance():UnOverlayPanel(self._tf)

	return
end

return SixthAnniversaryIslandShopWindowLayer
