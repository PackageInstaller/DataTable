local CoreInviterPage = class("CoreInviterPage")

CoreInviterPage.REFRESH_TIME = 1800

function CoreInviterPage:Ctor(arg_1_1, arg_1_2)
	pg.DelegateInfo.New(self)

	self._event = arg_1_2
	self._go = arg_1_1
	self._tf = tf(arg_1_1)
	self.ptTxt = self._tf:Find("bg1/pt_panel/slider/Text"):GetComponent(typeof(Text))
	self.phaseTotalTxt = self._tf:Find("bg1/pt_panel/total_progress"):GetComponent(typeof(Text))
	self.phaseTxt = self._tf:Find("bg1/pt_panel/progress"):GetComponent(typeof(Text))
	self.progress = self._tf:Find("bg1/pt_panel/slider")
	self.getBtn = self._tf:Find("bg1/pt_panel/get")
	self.awardTF = self._tf:Find("bg1/pt_panel/item")
	self.awardOverView = self._tf:Find("bg1/pt_panel/award_overview")
	self.bg = self._tf:Find("bg"):GetComponent(typeof(Image))
	self.returnerList = UIItemList.New(self._tf:Find("bg1/returners/content"), self._tf:Find("bg1/returners/content/tpl"))
	self.help = self._tf:Find("bg1/help")
	self.pushBtn = self._tf:Find("bg1/push_btn")
	self.pushedBtn = self._tf:Find("bg1/pushed_btn")
	self.pushDisBtn = self._tf:Find("bg1/push_btn_dis")
	self.codeTxt = self._tf:Find("bg1/code"):GetComponent(typeof(Text))
	self.taskLockPanel = self._tf:Find("bg1/task_lock_panel")

	setText(self.taskLockPanel:Find("Text"), i18n("word_sell_lock"))

	self.taskPanel = self._tf:Find("bg1/task_panel")
	self.taskItemTF = self._tf:Find("bg1/task_panel/item")
	self.taskProgress = self._tf:Find("bg1/task_panel/progress")
	self.taskDesc = self._tf:Find("bg1/task_panel/desc")
	self.taskGoBtn = self._tf:Find("bg1/task_panel/go")
	self.taskGotBtn = self._tf:Find("bg1/task_panel/got")
	self.taskGetBtn = self._tf:Find("bg1/task_panel/get")
	self.taskProgressTxt = self._tf:Find("bg1/task_panel/p"):GetComponent(typeof(Text))

	setText(self._tf:Find("bg1/pt_panel/title"), i18n("activity_return_reward_pt"))
	self:Init()

	return
end

function CoreInviterPage:Init()
...
function CoreInviterPage:Update(arg_7_1)
	...
	if not ActivityMainScene.FetchReturnersTime or var_7_0 >= ActivityMainScene.FetchReturnersTime then
		ActivityMainScene.FetchReturnersTime = var_7_0 + CoreInviterPage.REFRESH_TIME
...
function CoreInviterPage:getTotalPt(arg_8_1)
	local var_8_0 = 0
...
function CoreInviterPage:UpdateData()
...
function CoreInviterPage:UpdateUI()
...
function CoreInviterPage:UpdateTaskTF(arg_14_1)
...
function CoreInviterPage:UpdateReturners()
...
function CoreInviterPage:Dispose()
