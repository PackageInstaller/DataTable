local CoreActivityAprilFoolDiscoveryPage = class("CoreActivityAprilFoolDiscoveryPage", import("view.activity.CorePage.CoreActivityPage"))

function CoreActivityAprilFoolDiscoveryPage:OnInit()
...
function CoreActivityAprilFoolDiscoveryPage:OnDataSetting()
...
function CoreActivityAprilFoolDiscoveryPage:OnFirstFlush()
...
function CoreActivityAprilFoolDiscoveryPage:OnUpdateFlush()
	CoreActivityAprilFoolDiscoveryPage.super.OnUpdateFlush(self)
	...
	if (self.activity.data1 > 0 and "activity_bg_aprilfool_final" or "activity_bg_aprilfool_discovery") ~= self.bgName then
		setImageSprite(self.bg, LoadSprite("ui/AprilFoolDiscoveryPage_atlas", var_7_1))
	...
	for iter_7_0, iter_7_1 in ipairs(self.items) do
		...
		self.loader:GetSprite("UI/AprilFoolDiscoveryPage_atlas", var_0_1[var_7_4], iter_7_1:Find("state"))
...
function CoreActivityAprilFoolDiscoveryPage:UpdateSelection()
...
function CoreActivityAprilFoolDiscoveryPage:CreateCDTimer()
...
function CoreActivityAprilFoolDiscoveryPage:RemoveCDTimer()
	if self.CDTimer then
		self.CDTimer:Stop()
...
function CoreActivityAprilFoolDiscoveryPage:OnDestroy()
	self.loader:Clear()
	self:RemoveCDTimer()
	CoreActivityAprilFoolDiscoveryPage.super.OnDestroy(self)

	return
end

return CoreActivityAprilFoolDiscoveryPage
