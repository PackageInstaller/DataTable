local WillOfDeathPage = class("WillOfDeathPage", import("..CoreActivityPage"))

function WillOfDeathPage:OnInit()
	self.bg = self._tf:Find("AD")
	self.rtTask = self.bg:Find("task")
	self.slider = self.rtTask:Find("slider")
	self.step = self.rtTask:Find("step")
	self.progress = self.rtTask:Find("progress")
	self.displayBtn = self.rtTask:Find("display_btn")
	self.awardTF = self.rtTask:Find("award")
	self.battleBtn = self.rtTask:Find("battle_btn")
	self.getBtn = self.rtTask:Find("get_btn")
	self.gotBtn = self.rtTask:Find("got_btn")

	return
end

function WillOfDeathPage:OnDataSetting()
	if self.ptData then
...
function WillOfDeathPage:OnFirstFlush()
	setText(self.rtTask:Find("taskDesc"), i18n("brs_dmact_tip"))
...
function WillOfDeathPage:OnUpdateFlush()
	...
	if self.step then
		...
		setText(self.step, string.format("<size=52>%s</size><size=32><color=#ffffff33>/%s</color></size>", var_9_2, var_9_3))
	end

	local var_9_5, var_9_6, var_9_7 = self.ptData:GetResProgress()

	setText(self.progress, string.format("%s<color=#ffffff33>/%s</color>", var_9_7 >= 1 and setColorStr(var_9_5, COLOR_GREEN) or var_9_5, var_9_6))
...
function WillOfDeathPage:OnDestroy()
	return
end

function WillOfDeathPage:GetWorldPtData(arg_12_1)
