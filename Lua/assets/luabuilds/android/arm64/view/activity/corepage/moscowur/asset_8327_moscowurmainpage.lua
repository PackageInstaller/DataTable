local MoscowURMainPage = class("MoscowURMainPage", import("view.activity.CorePage.Helena.HelenaMainPage"))

function MoscowURMainPage:OnFirstFlush()
	MoscowURMainPage.super.OnFirstFlush(self)
...
function MoscowURMainPage:updateUI()
	if self.shop_id then
		local var_3_0, var_3_1 = self.timeMgr:inTime(pg.shop_template[self.shop_id].time)
		local var_3_2

		if var_3_1 then
			var_3_2 = MoscowURMainPage:skinCommdityTimeStamps((self.timeMgr:Table2ServerTime(var_3_1)))
		end
	...
	if var_3_4 then
		var_3_5 = MoscowURMainPage:skinCommdityTimeStamps((self.timeMgr:Table2ServerTime(var_3_4)))
	end
