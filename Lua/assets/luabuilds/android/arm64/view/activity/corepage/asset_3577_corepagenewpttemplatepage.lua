local CorePageNewPtTemplatePage = class("CorePageNewPtTemplatePage", import("view.activity.CorePage.CoreActivityPage"))

CorePageNewPtTemplatePage.OFFSET = 0.00042
CorePageNewPtTemplatePage.SHOW_COUNT = 8
CorePageNewPtTemplatePage.AWARD_OFFSET = 1e-05

function CorePageNewPtTemplatePage:OnInit()
...
function CorePageNewPtTemplatePage:OnDataSetting()
	if self.ptData then
...
function CorePageNewPtTemplatePage:OnFirstFlush()
...
function CorePageNewPtTemplatePage:BuildPhaseAwardScrollPos()
	self.impTotalPos = self.scrollCom:HeadIndexToValue(#self.awardList - CorePageNewPtTemplatePage.SHOW_COUNT) - self.scrollCom:HeadIndexToValue(0)
	self.importantPos = {}
	...
	for iter_10_0, iter_10_1 in pairs(self.awardList) do
		if iter_10_1.isImportant then
			table.insert(self.importantPos, {
				index = iter_10_0,
				pos = self.scrollCom:HeadIndexToValue(iter_10_0 - CorePageNewPtTemplatePage.SHOW_COUNT) / self.impTotalPos
			})
		end
	end
...
function CorePageNewPtTemplatePage:UpdateNextAward(arg_11_1)
	arg_11_1 = math.min(arg_11_1, 1)

	for iter_11_0, iter_11_1 in pairs(self.importantPos) do
		if arg_11_1 + CorePageNewPtTemplatePage.AWARD_OFFSET < iter_11_1.pos then
...
function CorePageNewPtTemplatePage:UpdateAward(arg_12_1, arg_12_2)
...
function CorePageNewPtTemplatePage:OnUpdateFlush()
	if isActive(self._tf) then
		self.scrollCom:ScrollTo(math.clamp(self.scrollCom:HeadIndexToValue(self.ptData:GetLevel()) / self.impTotalPos + CorePageNewPtTemplatePage.OFFSET * self.ptData:GetLevel(), 0, 1), true)
	end
...
function CorePageNewPtTemplatePage:GetWorldPtData(arg_15_1)
...
return CorePageNewPtTemplatePage
