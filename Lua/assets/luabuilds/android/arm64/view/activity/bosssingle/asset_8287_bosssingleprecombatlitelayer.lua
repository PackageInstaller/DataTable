local BossSinglePreCombatLiteLayer = class("BossSinglePreCombatLiteLayer", import(".BossSinglePreCombatLayer"))
local FormationUI = import("view.ship.FormationUI")
local var_0_2 = {
	[99] = true
}

function BossSinglePreCombatLiteLayer:getUIName()
	return "BossSinglePreCombatUI"
end

function BossSinglePreCombatLiteLayer:ResUISettings()
	return {
		anim = true,
		order = 5,
...
function BossSinglePreCombatLiteLayer:init()
	self:CommonInit()
...
function BossSinglePreCombatLiteLayer:CommonInit()
	self.eventTriggers = {}
	...
	setActive(self._checkBtn, false)
	setActive(self._continuousBtn, false)

	return
end

function BossSinglePreCombatLiteLayer:Register()
...
function BossSinglePreCombatLiteLayer:SetPlayerInfo(arg_19_1)
	return
end

function BossSinglePreCombatLiteLayer:SetSubFlag(arg_20_1)
	self._subUseable = arg_20_1 or false

	return
end

function BossSinglePreCombatLiteLayer:SetShips(arg_21_1)
...
function BossSinglePreCombatLiteLayer:SetStageID(arg_22_1)
		...
		for iter_22_1, iter_22_2 in ipairs(var_22_0) do
			...
			onButton(self, var_22_4, function()
				...
				if var_23_0 and var_0_2[var_23_0.type] then
					...
					self:emit(BossSinglePreCombatLiteLayer.ON_DROP_LIST, {
						item2Row = true,
						itemList = var_23_1,
						content = var_23_0.display
					})
				else
					self:emit(BossSinglePreCombatLiteLayer.ON_DROP, var_22_5)
				end
...
function BossSinglePreCombatLiteLayer:SetFleets(arg_25_1)
	self._fleetVOs = {}
...
function BossSinglePreCombatLiteLayer:SetCurrentFleet(arg_28_1)
...
function BossSinglePreCombatLiteLayer:CheckLegalFleet()
...
function BossSinglePreCombatLiteLayer:UpdateFleetView(arg_30_1)
...
function BossSinglePreCombatLiteLayer:updateFleetBg()
...
function BossSinglePreCombatLiteLayer:resetFormationComponent()
...
function BossSinglePreCombatLiteLayer:uiStartAnimating()
...
function BossSinglePreCombatLiteLayer:uiExitAnimating()
...
function BossSinglePreCombatLiteLayer:quickExitFunc()
	if self._currentForm == PreCombatLayer.FORM_EDIT then
		self:emit(BossSinglePreCombatMediator.ON_ABORT_EDIT)
	end

	BossSinglePreCombatLiteLayer.super.quickExitFunc(self)

	return
end

function BossSinglePreCombatLiteLayer:didEnter()
...
function BossSinglePreCombatLiteLayer:displayFleetInfo()
	local var_54_0 = self._currentFleetVO:getFleetType()

	setActive(self._vanguardGS.parent, var_54_0 == FleetType.Normal)
	setActive(self._mainGS.parent, var_54_0 == FleetType.Normal)

	local var_54_1 = math.floor(self._currentFleetVO:GetGearScoreSum(TeamType.Vanguard))
	local var_54_2 = math.floor(self._currentFleetVO:GetGearScoreSum(TeamType.Main))

	setActive(self._subGS.parent, var_54_0 == FleetType.Submarine)

	local var_54_3 = math.floor(self._currentFleetVO:GetGearScoreSum(TeamType.Submarine))
	local var_54_4 = self._currentFleetVO:GetCostSum()
	local var_54_5 = self.contextData.system
	local var_54_6 = pg.battle_cost_template[self.contextData.system].oil_cost == 0 and 0 or var_54_4.oil

	setActive(self._costContainer, true)
	FormationUI.tweenNumText(self._costText, var_54_6)
	FormationUI.tweenNumText(self._vanguardGS, var_54_1)
	FormationUI.tweenNumText(self._mainGS, var_54_2)
	FormationUI.tweenNumText(self._subGS, var_54_3)
	setText(self._fleetNameText, self._currentFleetVO:GetName())
	setText(self._fleetNumText, self._currentFleetVO.id)

	if self.contextData.costLimit then
		...
		for iter_54_0, iter_54_1 in ipairs({
			...
			local var_54_13 = self.contextData.costLimit[iter_54_0 == 1 and 1 or 2]
		...
		FormationUI.tweenNumText(self._costText, var_54_9)
	end

	setActive(self._costTip, isCostOver)
...
function BossSinglePreCombatLiteLayer:SetFleetStepper()
...
function BossSinglePreCombatLiteLayer:onBackPressed()
...
function BossSinglePreCombatLiteLayer:willExit()
...
return BossSinglePreCombatLiteLayer
