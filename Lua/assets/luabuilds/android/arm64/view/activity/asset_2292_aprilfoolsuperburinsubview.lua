local AprilFoolSuperBurinSubView = class("AprilFoolSuperBurinSubView", import(".AprilFoolBulinSubView"))

function AprilFoolSuperBurinSubView:getUIName()
	return "AprilFoolSuperBurinSubView"
end

function AprilFoolSuperBurinSubView:OnInit()
	local var_2_0 = getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_PUZZLA)

	if not var_2_0 or var_2_0:isEnd() then
		self:Destroy()

		return
	end

	local var_2_1 = pg.activity_event_picturepuzzle[var_2_0.id]

	assert(pg.activity_event_picturepuzzle[var_2_0.id], "Can't Find activity_event_picturepuzzle 's ID : " .. var_2_0.id)

	self.bulin = self._tf:Find("bulin")

	onButton(self, self.bulin, function()
		local var_3_0 = self.pieceId

		pg.m02:sendNotification(GAME.PUZZLE_PIECE_OP, {
			actId = var_2_0.id,
			id = self.pieceId,
			cmd = PuzzleActivity.CMD_ACTIVATE,
			callback = function()
				seriesAsync({
					function(arg_5_0)
						local var_5_0 = var_2_1.awards[table.indexof(var_2_1.pickup_picturepuzzle, var_3_0)]

						assert(var_5_0, "Cant Find Award of PieceID " .. var_3_0)
						self:emit(BaseUI.ON_ACHIEVE, {
							{
								type = var_5_0[1],
								id = var_5_0[2],
								count = var_5_0[3]
							}
						}, arg_5_0)

						return
					end,
					function(arg_6_0)
						local var_6_0 = getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_PUZZLA)

						if #table.mergeArray(var_6_0.data1_list, var_6_0.data2_list, true) < #var_2_1.pickup_picturepuzzle + #var_2_1.drop_picturepuzzle then
							return arg_6_0()
						end

						pg.NewStoryMgr.GetInstance():Play(var_6_0:getConfig("config_client").comStory, arg_6_0)

						return
					end,
					function()
						self:Destroy()

						return
					end
				})

				return
			end
		})

		return
	end)

	return
end

return AprilFoolSuperBurinSubView
