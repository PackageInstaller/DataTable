local ColoringScene = class("ColoringScene", import("view.base.BaseUI"))
local var_0_1 = 387
local var_0_2 = 467
local var_0_3 = 812.5
local var_0_4 = 1200
...
function ColoringScene:getUIName()
	...
	assert(false, "Not Set PixelDraw Activity config_client ID: " .. ((var_1_0 or nil) and (var_1_0.id or 0)))

	return
end

function ColoringScene:setActivity(arg_2_1)
	self.activity = arg_2_1

	return
end

function ColoringScene:setColorItems(arg_3_1)
	self.colorItems = arg_3_1

	return
end

function ColoringScene:setColorGroups(arg_4_1)
	self.colorGroups = arg_4_1

	return
end

function ColoringScene:init()
...
function ColoringScene:DidMediatorRegisterDone()
...
function ColoringScene:didEnter()
	onButton(self, self.btnBack, function()
		...
		self:emit(ColoringScene.ON_BACK, nil, 0.3)
...
function ColoringScene:uiStartAnimating()
		...
		pg.MsgboxMgr.GetInstance():ShowMsgBox({
			...
			onYes = function()
				if self.colorGroups[self.selectedIndex]:canBeCustomised() then
					self:emit(ColoringMediator.EVENT_COLORING_CLEAR, {
...
function ColoringScene:initInteractive()
			...
			if self.colorGroups[self.selectedIndex]:getState() == ColorGroup.StateColoring and not self.colorGroups[self.selectedIndex]:canBeCustomised() then
				...
				if var_22_0 ~= 0 then
					...
					if var_22_1 and #var_22_1 > 0 then
						self:emit(ColoringMediator.EVENT_COLORING_CELL, {
							activityId = self.activity.id,
							id = self.colorGroups[self.selectedIndex].id,
							cells = var_22_1
						})
...
function ColoringScene:SelectColoBar(arg_24_1)
...
function ColoringScene:updatePage()
...
function ColoringScene:updateSelectedColoring()
...
function ColoringScene:updateCells()
...
function ColoringScene:updateCell(arg_33_1, arg_33_2)
...
function ColoringScene:calcCellSize()
...
function ColoringScene:updateLines()
...
function ColoringScene:searchColoringCells(arg_36_1, arg_36_2, arg_36_3, arg_36_4)
	local var_36_0 = {
		row = arg_36_2,
		column = arg_36_3,
		color = arg_36_4
...
function ColoringScene:SearchValidDiagonalColoringCells(arg_40_1, arg_40_2, arg_40_3)
	assert(arg_40_1)
...
function ColoringScene:TryPlayStory()
...
function ColoringScene:onBackPressed()
...
function ColoringScene:willExit()
