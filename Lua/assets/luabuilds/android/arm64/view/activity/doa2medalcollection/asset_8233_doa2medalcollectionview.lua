local Doa2MedalCollectionView = class("Doa2MedalCollectionView", import("view.base.BaseUI"))

Doa2MedalCollectionView.FADE_OUT_TIME = 1
Doa2MedalCollectionView.PAGE_NUM = 9
Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE = 2

local var_0_1 = "ui/doa2medalcollectionui_atlas"

function Doa2MedalCollectionView:getUIName()
	return "Doa2MedalCollectionUI"
end

function Doa2MedalCollectionView:init()
	self:initData()
	self:findUI()
	self:addListener()
...
function Doa2MedalCollectionView:didEnter()
	self:checkAward()

	if self.activeIDList then
...
function Doa2MedalCollectionView:willExit()
...
function Doa2MedalCollectionView:getBGM()
	return "doa-az-story-1"
end

function Doa2MedalCollectionView:initData()
	...
	for iter_6_0 = 1, Doa2MedalCollectionView.PAGE_NUM do
		self.pageIDList[iter_6_0] = {}

		for iter_6_1 = 1, Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE do
			self.pageIDList[iter_6_0][iter_6_1] = self.allIDList[(iter_6_0 - 1) * Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE + iter_6_1]
		end
	end

	if self.activityData then
		self.activatableIDList = self.activityData.data1_list or {}
	end

	self.activeIDList = self.activityData and self.activityData.data2_list
	self.curPage = nil
	self.newMedalID = nil
...
function Doa2MedalCollectionView:findUI()
	...
	setActive(self.tplButtom, false)

	self.imgGot = var_7_0:Find("ProgressImg/got")
	self.switchBtnList = {}
	self.medalTfList = {}

	for iter_7_0 = 1, Doa2MedalCollectionView.PAGE_NUM do
		...
		for iter_7_1 = 1, Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE do
			local var_7_3 = findTF(self._tf, "MedalContainer/medal" .. (iter_7_0 - 1) * Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE + iter_7_1)
...
function Doa2MedalCollectionView:addListener()
...
function Doa2MedalCollectionView:UpdateActivity(arg_15_1)
	self:checkAward()

	return
end

function Doa2MedalCollectionView:updateMedalContainerView(arg_16_1, arg_16_2)
	if arg_16_2 then
	...
	for iter_16_0 = 1, #self.medalTfList do
		if (arg_16_1 - 1) * Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE < iter_16_0 and iter_16_0 <= (arg_16_1 - 1) * Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE + Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE then
	...
	if self.pageIDList then
		for iter_16_1, iter_16_2 in ipairs(self.pageIDList[arg_16_1]) do
			self:updateMedalView(self.pageIDList[arg_16_1], iter_16_2)
		end
	end
...
function Doa2MedalCollectionView:updateMedalView(arg_21_1, arg_21_2)
	...
	local var_21_3 = self.medalTfList[(self.curPage - 1) * Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE + table.indexof(arg_21_1, arg_21_2, 1)]
...
function Doa2MedalCollectionView:updateSwitchBtnTF()
	setText(self.leftPage, (self.curPage - 1) * Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE + 1)
	setText(self.rightPage, (self.curPage - 1) * Doa2MedalCollectionView.MEDAL_NUM_PER_PAGE + 2)
...
function Doa2MedalCollectionView:updateAfterSubmit(arg_24_1)
	self.activityProxy = getProxy(ActivityProxy)
	self.activityData = self.activityProxy:getActivityById(ActivityConst.DOA2_MEDAL_ACT_ID)
...
function Doa2MedalCollectionView:caculateActivatable(arg_25_1)
	local var_25_0 = 0

	if not self.pageIDList then
		return var_25_0
	end
...
function Doa2MedalCollectionView:checkAward()
	if not self.activeIDList then
		return
	end

	setActive(self.imgGot, #self.activeIDList == #self.allIDList and self.activityData.data1 == 1)

	if #self.activeIDList == #self.allIDList and self.activityData.data1 ~= 1 and not self.awardFlag then
...
return Doa2MedalCollectionView
