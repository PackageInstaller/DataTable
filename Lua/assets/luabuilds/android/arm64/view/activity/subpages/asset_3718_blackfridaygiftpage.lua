local BlackFridayGiftPage = class("BlackFridayGiftPage", import("...base.BaseActivityPage"))

BlackFridayGiftPage.DAY_COLOR = {
	"110C08",
	"C8A471"
}

function BlackFridayGiftPage:OnInit()
	self.rtGift = self._tf:Find("AD/gift")
	self.rtFreeGift = self._tf:Find("AD/gift_free")

	local var_1_0 = self._tf:Find("AD/days")

	self.uiList = UIItemList.New(var_1_0, var_1_0:Find("day"))
	...
	self.uiList:make(function(arg_2_0, arg_2_1, arg_2_2)
		...
		if arg_2_0 == UIItemList.EventUpdate then
			...
			setTextColor(arg_2_2:Find("Text"), Color.NewHex(self.DAY_COLOR[2]))
			setActive(arg_2_2:Find("lock"), arg_2_1 > self.nday)
			setActive(arg_2_2:Find("tip"), arg_2_1 <= self.nday and self.freeGifts[arg_2_1]:canPurchase())
			onToggle(self, arg_2_2, function(arg_3_0)
				if arg_3_0 then
					self.index = arg_2_1

					self:ShowGifts(arg_2_1)
				end

				setTextColor(arg_2_2:Find("Text"), Color.NewHex(self.DAY_COLOR[arg_3_0 and 1 or 2]))
...
function BlackFridayGiftPage:OnDataSetting()
	if not self.idLists then
		self.idLists = self.activity:getConfig("config_client").gifts

		assert(#self.idLists[1] == #self.idLists[2])
	end

	self.nday = math.min(#self.idLists[1], self.activity:getNDay())

	local var_4_0 = getProxy(ShopsProxy)

	self.gifts = underscore.map(self.idLists[1], function(arg_5_0)
		return var_4_0:GetGiftCommodity(arg_5_0, Goods.TYPE_CHARGE)
	end)
	self.freeGifts = underscore.map(self.idLists[2], function(arg_6_0)
...
function BlackFridayGiftPage:OnUpdateFlush()
	self.uiList:align(#self.idLists[1])

	if not self.index then
		self.index = self.nday

		while self.index > 0 and not self.gifts[self.index]:canPurchase() and not self.freeGifts[self.index]:canPurchase() do
			self.index = self.index - 1
		end

		self.index = (self.index - 1) % self.nday + 1

		triggerToggle(self.uiList.container:GetChild(self.index - 1), true)
	else
		self:ShowGifts(self.index)
	end

	return
end

function BlackFridayGiftPage:ShowGifts(arg_8_1)
	self:UpdateCard(self.rtGift, self.gifts[self.index])
	self:UpdateCard(self.rtFreeGift, self.freeGifts[self.index])
...
function BlackFridayGiftPage:UpdateCard(arg_10_1, arg_10_2)
	local var_10_0
	...
	UIItemList.StaticAlign(arg_10_1:Find("awards"), arg_10_1:Find("awards/award"), #var_10_0.items, function(arg_13_0, arg_13_1, arg_13_2)
		if arg_13_0 == UIItemList.EventUpdate then
			...
			onButton(self, arg_13_2, function()
				self:emit(BaseUI.ON_DROP, var_13_0)
...
function BlackFridayGiftPage:OnCharge(arg_16_1)
	if arg_16_1:isChargeType() then
		...
		function var_16_0.onYes()
			if ChargeConst.isNeedSetBirth() then
				self:emit(ActivityMediator.OPEN_CHARGE_BIRTHDAY)
			else
				self:emit(ActivityMediator.CHARGE, arg_16_1.id)
			end

			return
		end

		self:emit(ActivityMediator.OPEN_CHARGE_ITEM_PANEL, var_16_0)
	else
		local var_16_1 = {}
		...
		self:emit(ActivityMediator.OPEN_CHARGE_ITEM_PANEL, {
			isLocalPrice = false,
			isChargeType = false,
			isMonthCard = false,
			commodity = arg_16_1,
			...
			onYes = function()
				pg.MsgboxMgr.GetInstance():ShowMsgBox({
					...
					onYes = function()
						self:emit(ActivityMediator.BUY_ITEM, arg_16_1.id, 1)
...
function BlackFridayGiftPage:OnDestroy()
	return
end

return BlackFridayGiftPage
