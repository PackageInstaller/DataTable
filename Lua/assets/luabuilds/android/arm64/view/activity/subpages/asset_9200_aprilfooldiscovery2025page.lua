local AprilFoolDiscovery2025Page = class("AprilFoolDiscovery2025Page", import(".AprilFoolDiscoveryRePage"))
local var_0_1 = "burinteam"

function AprilFoolDiscovery2025Page:OnInit()
	...
	for iter_1_0 = 1, #var_0_1 do
		self.loader:GetSprite("ui/activityuipage/AprilFoolDiscovery2025Page_atlas", string.sub(var_0_1, iter_1_0, iter_1_0), self.items[iter_1_0]:Find("Character"))
	end
...
function AprilFoolDiscovery2025Page:OnDataSetting()
	local var_2_0 = AprilFoolDiscovery2025Page.super.OnDataSetting(self)
...
function AprilFoolDiscovery2025Page:OnFirstFlush()
	...
	local var_4_1 = self.activity:getConfig("config_client").guideName

	self:AddFunc(function(arg_8_0)
...
function AprilFoolDiscovery2025Page:OnUpdateFlush()
	local var_9_1 = #self.activity.data2_list == #self.keyList
	local var_9_2 = self.activity.data2_list

	for iter_9_0, iter_9_1 in ipairs(self.items) do
		local var_9_3 = self.keyList[iter_9_0]
		local var_9_4 = table.contains(var_9_2, self.keyList[iter_9_0]) and 3 or table.contains(self.activity.data3_list, self.keyList[iter_9_0]) and 2 or 1
	...
	SetActive(self.battle_btn, not var_9_1)
	SetActive(self.btnBattle, var_9_1)
	self:UpdateSelection()
	...
	if #table.mergeArray(self.activity.data1_list, self.activity.data2_list, true) >= #pg.activity_event_picturepuzzle[self.activity.id].pickup_picturepuzzle + #pg.activity_event_picturepuzzle[self.activity.id].drop_picturepuzzle then
		local var_9_5 = self.activity:getConfig("config_client").comStory

		self:AddFunc(function(arg_12_0)
...
function AprilFoolDiscovery2025Page:UpdateSelection()
	setText(self.tip, table.contains(self.activity.data3_list, self.keyList[self.selectIndex]) and i18n("SuperBulin2_tip" .. self.selectIndex) or "")
	self:CreateCDTimer()

	return
end

return AprilFoolDiscovery2025Page
