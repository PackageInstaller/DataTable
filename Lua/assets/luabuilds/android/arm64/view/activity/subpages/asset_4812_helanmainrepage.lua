local HeLanMainRePage = class("HeLanMainRePage", import("...base.BaseActivityPage"))
local var_0_1 = 71132
local var_0_3 = 5901

function HeLanMainRePage:OnInit()
	HeLanMainRePage.super.OnInit(self)
...
function HeLanMainRePage:OnDataSetting()
	self.timeMgr = pg.TimeMgr.GetInstance()

	return
end

function HeLanMainRePage:OnFirstFlush()
...
function HeLanMainRePage:OnUpdateFlush()
	self:updateUI()

	return
end

function HeLanMainRePage:updateUI()
	local var_7_1, var_7_2 = self.timeMgr:inTime(pg.shop_template[var_0_1].time)
	local var_7_3

	if var_7_2 then
		var_7_3 = HeLanMainRePage:skinCommdityTimeStamps((self.timeMgr:Table2ServerTime(var_7_2)))
	end
	...
	if var_7_5 then
		var_7_6 = HeLanMainRePage:skinCommdityTimeStamps((self.timeMgr:Table2ServerTime(var_7_5)))
	end
	...
	self.btnFuncList = {
		...
		fight = function(arg_12_0)
			onButton(self, arg_12_0, function()
				if pg.activity_template[self.activity.id].config_client.fightLinkActID then
					self:emit(ActivityMediator.SKIP_ACTIVITY_MAP, pg.activity_template[self.activity.id].config_client.fightLinkActID)
				else
...
function HeLanMainRePage:skinCommdityTimeStamps(arg_14_1)
