local ToLovePtPage = class("ToLovePtPage", import("view.base.BaseActivityPage"))

ToLovePtPage.OFFSET = 0.00042
ToLovePtPage.SHOW_COUNT = 8

function ToLovePtPage:OnInit()
	self.bg = self._tf:Find("AD")
	self.getBtn = self.bg:Find("get_btn")
	self.getBtnGray = self.getBtn:Find("gray")
...
function ToLovePtPage:OnDataSetting()
	if self.ptData then
...
function ToLovePtPage:OnFirstFlush()
...
function ToLovePtPage:BuildPhaseAwardScrollPos()
	self.impTotalPos = self.scrollCom:HeadIndexToValue(#self.awardList - ToLovePtPage.SHOW_COUNT) - self.scrollCom:HeadIndexToValue(0)
	self.importantPos = {}
	...
	for iter_10_0, iter_10_1 in pairs(self.awardList) do
		if iter_10_1.isImportant then
			table.insert(self.importantPos, {
				index = iter_10_0,
				pos = self.scrollCom:HeadIndexToValue(iter_10_0 - ToLovePtPage.SHOW_COUNT) / self.impTotalPos
			})
		end
	end
...
function ToLovePtPage:UpdateNextAward(arg_11_1)
...
function ToLovePtPage:UpdateAward(arg_12_1, arg_12_2)
...
function ToLovePtPage:OnUpdateFlush()
	if isActive(self._tf) then
		self.scrollCom:ScrollTo(math.clamp(self.scrollCom:HeadIndexToValue(self.ptData:GetLevel()) / self.impTotalPos + ToLovePtPage.OFFSET * self.ptData:GetLevel(), 0, 1), true)
	end
...
function ToLovePtPage:GetWorldPtData(arg_15_1)
