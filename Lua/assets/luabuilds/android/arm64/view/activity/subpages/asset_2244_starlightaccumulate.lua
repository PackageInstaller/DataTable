local StarlightAccumulate = class("StarlightAccumulate", import("view.base.BaseActivityPage"))

StarlightAccumulate.OFFSET = 0.00042
StarlightAccumulate.SHOW_COUNT = 8

function StarlightAccumulate:OnInit()
...
function StarlightAccumulate:OnDataSetting()
	if self.ptData then
...
function StarlightAccumulate:OnFirstFlush()
...
function StarlightAccumulate:BuildPhaseAwardScrollPos()
	self.impTotalPos = self.scrollCom:HeadIndexToValue(#self.awardList - StarlightAccumulate.SHOW_COUNT) - self.scrollCom:HeadIndexToValue(0)
	self.importantPos = {}
	...
	for iter_10_0, iter_10_1 in pairs(self.awardList) do
		if iter_10_1.isImportant then
			table.insert(self.importantPos, {
				index = iter_10_0,
				pos = self.scrollCom:HeadIndexToValue(iter_10_0 - StarlightAccumulate.SHOW_COUNT) / self.impTotalPos
			})
		end
	end
...
function StarlightAccumulate:UpdateNextAward(arg_11_1)
...
function StarlightAccumulate:UpdateAward(arg_12_1, arg_12_2)
...
function StarlightAccumulate:OnUpdateFlush()
	if isActive(self._tf) then
		self.scrollCom:ScrollTo(math.clamp(self.scrollCom:HeadIndexToValue(self.ptData:GetLevel()) / self.impTotalPos + StarlightAccumulate.OFFSET * self.ptData:GetLevel(), 0, 1), true)
	end
...
function StarlightAccumulate:GetWorldPtData(arg_15_1)
