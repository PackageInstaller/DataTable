local DormSignTwoPage = class("DormSignTwoPage", import("view.base.BaseActivityPage"))

function DormSignTwoPage:getUIName()
	return "DormSignTwoPage"
end

function DormSignTwoPage:OnInit()
...
function DormSignTwoPage:OnDataSetting()
...
function DormSignTwoPage:UpdateTaskData()
...
function DormSignTwoPage:OnFirstFlush()
	self.uilist:make(function(arg_8_0, arg_8_1, arg_8_2)
		...
		elseif arg_8_0 == UIItemList.EventUpdate then
			...
			setActive(arg_8_2:Find("get_bg"), self.taskDic[self.taskGroup[arg_8_1 + 1]] and self.taskDic[self.taskGroup[arg_8_1 + 1]]:getTaskStatus() == 1 and not self.taskDic[self.taskGroup[arg_8_1 + 1]]:isOver())
		end
	...
	PlayerPrefs.SetString("DormSignTwoPage", DormSignTwoPage.GetDate())

	return
end

function DormSignTwoPage:OnUpdateFlush()
	self:UpdateTaskData()
...
function DormSignTwoPage.GetDate()
	return pg.TimeMgr.GetInstance():STimeDescC(pg.TimeMgr.GetInstance():GetServerTime(), "%Y/%m/%d")
end

function DormSignTwoPage.IsShowRed()
	return DormSignTwoPage.IsShowGoRed() or DormSignTwoPage.IsShowAwardRed()
end

function DormSignTwoPage.IsShowGoRed()
	return PlayerPrefs.GetString("DormSignTwoPage", "") ~= DormSignTwoPage.GetDate()
end

function DormSignTwoPage.IsShowAwardRed()
