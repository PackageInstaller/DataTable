local ThirdAnniversaryJPPage = class("ThirdAnniversaryJPPage", import("...base.BaseActivityPage"))

function ThirdAnniversaryJPPage:OnInit()
	self:findUI()
	self:initData()
...
function ThirdAnniversaryJPPage:findUI()
	...
	for iter_2_1 = 5, 2 * (self.btnContainer.childCount / 2) - 1 do
...
function ThirdAnniversaryJPPage:initData()
	self.paintCount = 18
	self.curPaintIndex = 1
	self.paintSwitchTime = 1
	self.paintStaticTime = 3.5
	self.paintStaticCountValue = 0
	self.paintPathPrefix = "thirdanniversaryjppage/"
	self.paintNamePrefix = "paint"
...
function ThirdAnniversaryJPPage:switchNextPaint()
...
function ThirdAnniversaryJPPage:OnFirstFlush()
	self:initPaint()
...
function ThirdAnniversaryJPPage:initPaint()
...
function ThirdAnniversaryJPPage:initBtnList(arg_9_1)
	...
	onButton(self, arg_9_1[3], function()
		self:emit(ActivityMediator.SELECT_ACTIVITY, getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_RETURN_AWARD).id)
	...
	onButton(self, arg_9_1[5], function()
		self:emit(ActivityMediator.SELECT_ACTIVITY, pg.activity_const.JIUJIU_ADVENTURE_ID.act_id)
...
function ThirdAnniversaryJPPage:initTimer()
	local var_15_0 = 0.016666666666666666

	self.paintStaticCountValue = 0
	self.frameTimer = Timer.New(function()
		self.paintStaticCountValue = self.paintStaticCountValue + var_15_0
	...
	self.frameTimer2 = Timer.New(function()
		self.btnContainer.anchoredPosition = Vector3((self.startAnchoredPosX - (self.btnContainer.anchoredPosition.x - self.btnSpeed * var_15_0) >= self.moveLength or nil) and self.btnContainer.anchoredPosition.x + self.moveLength, 0, 0)
...
function ThirdAnniversaryJPPage:OnDestroy()
