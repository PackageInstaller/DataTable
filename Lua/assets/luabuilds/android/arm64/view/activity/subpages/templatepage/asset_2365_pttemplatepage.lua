local PtTemplatePage = class("PtTemplatePage", import("view.base.BaseActivityPage"))

function PtTemplatePage:OnInit()
...
function PtTemplatePage:OnDataSetting()
	if self.ptData then
...
function PtTemplatePage:OnFirstFlush()
	...
	onButton(self, self.battleBtn, function()
		local var_5_0
		local var_5_1
		local var_5_2 = self.activity:getConfig("config_client")
		...
		if var_5_2.fightLinkActID then
			if getProxy(ActivityProxy):getActivityById(var_5_3) then
				self:emit(ActivityMediator.SKIP_ACTIVITY_MAP, var_5_3)
...
function PtTemplatePage:OnUpdateFlush()
...
function PtTemplatePage:OnDestroy()
	return
end

function PtTemplatePage:GetWorldPtData(arg_12_1)
