local LittleOuGenRePage = class("LittleOuGenRePage", import(".TemplatePage.PtTemplatePage"))

function LittleOuGenRePage:OnInit()
	LittleOuGenRePage.super.OnInit(self)
...
function LittleOuGenRePage:OnUpdateFlush()
	LittleOuGenRePage.super.OnUpdateFlush(self)
...
function LittleOuGenRePage:updataTask()
...
function LittleOuGenRePage:OnFirstFlush()
	LittleOuGenRePage.super.OnFirstFlush(self)
...
function LittleOuGenRePage:initTask()
	...
	for iter_9_0 = 1, #var_9_0 do
		...
		if pg.task_data_template[var_9_0[iter_9_0]].type ~= Task.TYPE_ACTIVITY then
			if var_9_1 == Task.TYPE_ACTIVITY_BRANCH then
				self:insertTaskToGroup(var_9_0[iter_9_0], pg.task_data_template[var_9_0[iter_9_0]], (self:getTaskGroup(var_9_1, pg.task_data_template[var_9_0[iter_9_0]].sub_type)))
			end
		end
	end
...
function LittleOuGenRePage:updateTaskUI()
...
function LittleOuGenRePage:updateTaskList(arg_11_1, arg_11_2, arg_11_3, arg_11_4)
	...
	elseif var_11_0 <= var_11_1 then
		...
		if not self.nextTickFlag then
			onNextTick(function()
				triggerButton(findTF(var_11_8, "get"))

				self.nextTickFlag = false

				return
			end)

			self.nextTickFlag = true
		end
	else
		setActive(findTF(var_11_8, "go"), true)
		onButton(self, findTF(var_11_8, "go"), function()
			local var_15_0 = self.taskProxy:getTaskById(arg_11_3.id)
...
function LittleOuGenRePage:changeGroupOpening(arg_17_1)
...
function LittleOuGenRePage:getTaskTfFromPool()
...
function LittleOuGenRePage:getTaskGroup(arg_19_1, arg_19_2)
...
function LittleOuGenRePage:insertTaskToGroup(arg_20_1, arg_20_2, arg_20_3)
...
function LittleOuGenRePage:sortTaskGroups()
...
function LittleOuGenRePage:OnDestroy()
	if LeanTween.isTweening(go(self.slider)) then
		LeanTween.cancel(go(self.slider))
	end
