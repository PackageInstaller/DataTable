local RaphaelSPPtPage = class("RaphaelSPPtPage", import("view.base.BaseActivityPage"))

RaphaelSPPtPage.OFFSET = 0.00042
RaphaelSPPtPage.SHOW_COUNT = 8

function RaphaelSPPtPage:OnInit()
...
function RaphaelSPPtPage:OnDataSetting()
	if self.ptData then
...
function RaphaelSPPtPage:OnFirstFlush()
...
function RaphaelSPPtPage:BuildPhaseAwardScrollPos()
	self.impTotalPos = self.scrollCom:HeadIndexToValue(#self.awardList - RaphaelSPPtPage.SHOW_COUNT) - self.scrollCom:HeadIndexToValue(0)
	self.importantPos = {}
	...
	for iter_10_0, iter_10_1 in pairs(self.awardList) do
		if iter_10_1.isImportant then
			table.insert(self.importantPos, {
				index = iter_10_0,
				pos = self.scrollCom:HeadIndexToValue(iter_10_0 - RaphaelSPPtPage.SHOW_COUNT) / self.impTotalPos
			})
		end
	end
...
function RaphaelSPPtPage:UpdateNextAward(arg_11_1)
...
function RaphaelSPPtPage:UpdateAward(arg_12_1, arg_12_2)
...
function RaphaelSPPtPage:OnUpdateFlush()
	if isActive(self._tf) then
		self.scrollCom:ScrollTo(math.clamp(self.scrollCom:HeadIndexToValue(self.ptData:GetLevel()) / self.impTotalPos + RaphaelSPPtPage.OFFSET * self.ptData:GetLevel(), 0, 1), true)
	end
...
function RaphaelSPPtPage:GetWorldPtData(arg_15_1)
