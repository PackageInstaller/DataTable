local FireworkPanelLayer = class("FireworkPanelLayer", import("view.base.BaseUI"))

function FireworkPanelLayer:getUIName()
	return "FireworkPanelUI"
end

function FireworkPanelLayer:init()
...
function FireworkPanelLayer:initData()
...
function FireworkPanelLayer:getLocalData()
	local var_4_0 = {}

	for iter_4_0 = 1, #self.allIds do
		local var_4_1 = PlayerPrefs.GetInt("fireworks_" .. self.playerId .. "_pos_" .. iter_4_0)
...
function FireworkPanelLayer:setLocalData()
	for iter_5_0 = 1, #self.allIds do
		PlayerPrefs.SetInt("fireworks_" .. self.playerId .. "_pos_" .. iter_5_0, self.orderIds[iter_5_0] or 0)
	end

	return
end

function FireworkPanelLayer:didEnter()
	onButton(self, self._tf:Find("main/mask"), function()
		self:emit(FireworkPanelLayer.ON_CLOSE)
	...
	onButton(self, self.fireBtn, function()
		if #self.orderIds > 0 then
			self:emit(FireworkPanelMediator.LET_OFF_FIREWORKS, self.orderIds)
		end
...
function FireworkPanelLayer:initLeft()
...
function FireworkPanelLayer:initRight()
...
function FireworkPanelLayer:onLeftClick(arg_16_1, arg_16_2)
	if arg_16_2 then
...
function FireworkPanelLayer:onRightClick(arg_17_1)
...
function FireworkPanelLayer:willExit()
	pg.UIMgr.GetInstance():UnOverlayPanel(self._tf)

	return
end

return FireworkPanelLayer
