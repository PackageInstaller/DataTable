...
function RopingCowGameView:initData()
	self.storylist = pg.mini_game[self:GetMGData().id].simple_config_data.story

	local var_40_0 = pg.mini_game[self:GetMGData().id].simple_config_data.stroy_act

	self.storyFinal = pg.activity_template[var_40_0].config_client.story
	self.storyBefore = pg.activity_template[var_40_0].config_client.beforestory
...
function RopingCowGameView:initUI()
	...
	onButton(self, findTF(self.menuUI, "btnStart"), function()
		if self:getGameTimes() > 0 then
			self:checkOverflow(pg.mini_game[self:GetMGData().id].simple_config_data.drop[self:getGameUsedTimes() + 1], function()
	...
	local var_42_1 = pg.mini_game[self:GetMGData().id].simple_config_data.drop
	...
	for iter_42_0 = 1, #var_42_1 do
		...
		setParent(var_42_2, findTF(self.menuUI, "battList/Viewport/Content"))
		...
		updateDrop(var_42_3, var_42_4)
		onButton(self, var_42_3, function()
			self:emit(BaseUI.ON_DROP, var_42_4)

			return
		end, SFX_PANEL)
		table.insert(self.dropItems, var_42_3)
		setActive(var_42_2, true)
		table.insert(self.battleItems, var_42_2)
	end
...
function RopingCowGameView:initGameUI()
	...
	self.captureButton:AddPointDownFunc(function(arg_60_0, arg_60_1)
	...
	for iter_57_0 = 1, var_0_12 do
		table.insert(var_57_0, (findTF(self.sceneTf, "cow" .. iter_57_0)))
	end

	self.sceneScoreTf = findTF(self.sceneTf, "score")
	self._playerController = var_0_33(findTF(self.sceneTf, "player"), self)
	self._cowController = var_0_32(var_57_0, findTF(self.sceneTf, "cowContainer"), self)
...
function RopingCowGameView:updateMenuUI()
	local var_63_0 = self:getGameUsedTimes()
	local var_63_1 = self:getGameTimes()

	for iter_63_0 = 1, #self.battleItems do
		setActive(findTF(self.battleItems[iter_63_0], "state_open"), false)
		setActive(findTF(self.battleItems[iter_63_0], "state_closed"), false)
		setActive(findTF(self.battleItems[iter_63_0], "state_clear"), false)
		setActive(findTF(self.battleItems[iter_63_0], "state_current"), false)

		if iter_63_0 <= var_63_0 then
			SetParent(self.dropItems[iter_63_0], findTF(self.battleItems[iter_63_0], "state_clear/icon"))
			setActive(self.dropItems[iter_63_0], true)
			setActive(findTF(self.battleItems[iter_63_0], "state_clear"), true)
		elseif iter_63_0 == var_63_0 + 1 and var_63_1 >= 1 then
			setActive(findTF(self.battleItems[iter_63_0], "state_current"), true)
			SetParent(self.dropItems[iter_63_0], findTF(self.battleItems[iter_63_0], "state_current/icon"))
			setActive(self.dropItems[iter_63_0], true)
		elseif var_63_0 < iter_63_0 and iter_63_0 <= var_63_0 + var_63_1 then
			setActive(findTF(self.battleItems[iter_63_0], "state_open"), true)
			SetParent(self.dropItems[iter_63_0], findTF(self.battleItems[iter_63_0], "state_open/icon"))
			setActive(self.dropItems[iter_63_0], true)
		else
			setActive(findTF(self.battleItems[iter_63_0], "state_closed"), true)
			setActive(self.dropItems[iter_63_0], false)
		end
	end

	self.totalTimes = self:getGameTotalTime()

	local var_63_2 = 1 - (self:getGameUsedTimes() - 3 < 0 and 0 or self:getGameUsedTimes() - 3) / (self.totalTimes - 4)

	if var_63_2 > 1 then
		var_63_2 = 1
	end

	scrollTo(self.battleScrollRect, 0, var_63_2)
	setActive(findTF(self.menuUI, "btnStart/tip"), var_63_1 > 0)
	self:CheckGet()
...
function RopingCowGameView:changeSpeed(arg_73_1)
	return
end
...
function RopingCowGameView:updateGameUI()
	...
	if self._playerController then
		local var_78_0 = self._playerController:getThrowTime()

		self.captureCdMaskImg.fillAmount = var_78_0 and var_78_0 > 0 and var_78_0 / var_0_10 or 0
	end

	return
end

function RopingCowGameView:addScore(arg_79_1)
	self.scoreNum = self.scoreNum + arg_79_1
	...
	for iter_79_0 = 0, self.sceneScoreTf.childCount - 1 do
		local var_79_0 = self.sceneScoreTf:GetChild(iter_79_0)
...
function RopingCowGameView:showSettlement()
	...
	local var_82_0 = self:GetMGData():GetRuntimeData("elements")
	local var_82_1 = self.scoreNum
	local var_82_2

	if var_82_0 and #var_82_0 > 0 then
		var_82_2 = var_82_0[1] or 0
	end

	setActive(findTF(self.settlementUI, "ad/new"), var_82_2 < var_82_1)
	...
	setText(findTF(self.settlementUI, "ad/highText"), var_82_2)
	setText(findTF(self.settlementUI, "ad/currentText"), var_82_1)

	if self:getGameTimes() and self:getGameTimes() > 0 then
		local var_82_3 = self:getGameUsedTimes() + 1
		local var_82_4 = pg.NewStoryMgr.GetInstance()

		if self.storylist[var_82_3] then
			local var_82_5 = self.storylist[var_82_3][1] or nil
			...
			if var_82_5 and not var_82_4:IsPlayed(var_82_5) then
				...
				var_82_4:Play(var_82_5, function()
					...
					if not var_82_4:IsPlayed(self.storyFinal[1][1]) then
						for iter_83_0, iter_83_1 in ipairs(self.storyBefore) do
					...
					if var_83_0 then
						var_82_4:Play(self.storyFinal[1][1], function()
							self:SendSuccess(0)
...
function RopingCowGameView:checkOverflow(arg_87_1, arg_87_2)
	local var_87_0 = getProxy(PlayerProxy):getRawData()
	local var_87_1, var_87_2 = Task.StaticJudgeOverflow(var_87_0.gold, var_87_0.oil, LOCK_UR_SHIP and 0 or getProxy(BagProxy):GetLimitCntById(pg.gameset.urpt_chapter_max.description[1]), true, true, {
		arg_87_1
	})
