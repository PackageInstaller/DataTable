...
function var_0_0:GetEightDirMark()
	local var_29_0 = {}
	...
	for iter_29_0, iter_29_1 in ipairs({
		...
		var_29_0[iter_29_0] = iter_29_1 > 0 and var_0_1[iter_29_0][1] or iter_29_1 < 0 and var_0_1[iter_29_0][2] or ""
	end

	return var_29_0[1] .. var_29_0[2]
end

function var_0_0:GetFourDirMark()
	local var_30_0 = {}

	for iter_30_0, iter_30_1 in ipairs(self.y * self.y < self.x * self.x and {
		0,
		self.x
	} or {
		self.y,
		...
		var_30_0[iter_30_0] = iter_30_1 > 0 and var_0_1[iter_30_0][1] or iter_30_1 < 0 and var_0_1[iter_30_0][2] or ""
	end

	return var_30_0[1] .. var_30_0[2]
end

function var_0_0:GetDestroyPoint()
	local var_31_0 = 0

	if self.class == TargetItem then
		switch(self.type, {
			bomb = function()
				var_31_0 = 50
	...
	elseif isa(self, TargetObject) then
		switch(self.class, {
	...
	elseif isa(self, MoveEnemy) then
		switch(self.class, {
...
function var_0_0:ReSetDir()
	if self.x ~= 0 and self.y ~= 0 then
		local var_53_0 = math.abs(self.x)
		local var_53_1 = math.abs(self.y)

		if var_53_1 < var_53_0 then
			self.x = math.ceil(var_53_0) * math.sign(self.x)
			self.y = 0
		else
			self.x = 0
			self.y = math.ceil(var_53_1) * math.sign(self.y)
		end
	end
