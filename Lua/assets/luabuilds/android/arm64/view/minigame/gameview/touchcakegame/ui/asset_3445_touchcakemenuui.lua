local TouchCakeMenuUI = class("TouchCakeMenuUI")
local var_0_1
local var_0_2

function TouchCakeMenuUI:Ctor(arg_1_1, arg_1_2)
	self._tf = arg_1_1
	...
	GetComponent(findTF(self.menuUI, "lastTimes/img"), typeof(Image)):SetNativeSize()
	...
	onButton(self._event, findTF(self.menuUI, "btnBack"), function()
		self._event:emit(var_0_2.CLOSE_GAME)

		return
	end, SFX_CANCEL)
	onButton(self._event, findTF(self.menuUI, "btnRule"), function()
		self._event:emit(var_0_2.SHOW_RULE, true)

		return
	end, SFX_CANCEL)
	onButton(self._event, self.imgHelp, function()
		self._event:emit(var_0_2.SHOW_RULE, false)
	...
	onButton(self._event, self.btnStart, function()
		self._event:emit(var_0_2.READY_START)

		return
	end, SFX_CANCEL)
	onButton(self._event, self.btnRank, function()
		self._event:emit(var_0_2.SHOW_RANK)

		return
	end, SFX_CANCEL)
	onButton(self._event, self.btnHome, function()
		self._event:emit(var_0_2.ON_HOME)
...
function TouchCakeMenuUI:show(arg_11_1)
	setActive(self.menuUI, arg_11_1)

	return
end

function TouchCakeMenuUI:setGameRoomUI(arg_12_1)
	if arg_12_1 then
...
function TouchCakeMenuUI:CheckGet()
...
function TouchCakeMenuUI:getGameTimes(arg_15_1)
	return arg_15_1.count
end

function TouchCakeMenuUI:getGameUsedTimes(arg_16_1)
	return arg_16_1.usedtime
end

function TouchCakeMenuUI:getUltimate(arg_17_1)
	return arg_17_1.ultimate
end

return TouchCakeMenuUI
