local CatchTreasureGameView = class("CatchTreasureGameView", import("..BaseMiniGameView"))
...
function CatchTreasureGameView:getUIName()
	return "CatchTreasureGameUI"
end

function CatchTreasureGameView:getBGM()
	return var_0_1
end

function CatchTreasureGameView:didEnter()
	self:initEvent()
	self:initData()
	self:initUI()
	self:initGameUI()
...
function CatchTreasureGameView:initEvent()
...
function CatchTreasureGameView:initData()
...
function CatchTreasureGameView:initUI()
	...
	self.titleDesc = findTF(self.menuUI, "desc")

	GetComponent(self.titleDesc, typeof(Image)):SetNativeSize()
	...
	onButton(self, findTF(self.menuUI, "home"), function()
		self:emit(BaseUI.ON_HOME)

		return
	end, SFX_CANCEL)

	local var_70_0 = findTF(self.menuUI, "tplBattleItem")
	...
	for iter_70_0 = 1, 7 do
		...
		setParent(var_70_2, findTF(self.menuUI, "battList/Viewport/Content"))
		...
		local var_70_4 = {
			type = self.dropData[iter_70_0][1],
			id = self.dropData[iter_70_0][2],
			count = self.dropData[iter_70_0][3]
		}

		updateDrop(var_70_3, var_70_4)
		onButton(self._event, var_70_3, function()
			self:emit(BaseUI.ON_DROP, var_70_4)

			return
		end, SFX_PANEL)
		table.insert(self.dropItems, var_70_3)
		setActive(var_70_2, true)
		table.insert(self.battleItems, var_70_2)
	end
...
function CatchTreasureGameView:initGameUI()
	...
	self.dragDelegate:AddPointDownFunc(function(arg_88_0, arg_88_1)
...
function CatchTreasureGameView:Update()
	self:AddDebugInput()

	return
end

function CatchTreasureGameView:AddDebugInput()
...
function CatchTreasureGameView:updateMenuUI()
	local var_91_0 = self:getGameUsedTimes()
	local var_91_1 = self:getGameTimes()

	for iter_91_0 = 1, 7 do
		setActive(findTF(self.battleItems[iter_91_0], "state_open"), false)
		setActive(findTF(self.battleItems[iter_91_0], "state_closed"), false)
		setActive(findTF(self.battleItems[iter_91_0], "state_clear"), false)
		setActive(findTF(self.battleItems[iter_91_0], "state_current"), false)

		if iter_91_0 <= var_91_0 then
			SetParent(self.dropItems[iter_91_0], findTF(self.battleItems[iter_91_0], "state_clear/icon"))
			setActive(self.dropItems[iter_91_0], true)
			setActive(findTF(self.battleItems[iter_91_0], "state_clear"), true)
		elseif iter_91_0 == var_91_0 + 1 and var_91_1 >= 1 then
			setActive(findTF(self.battleItems[iter_91_0], "state_current"), true)
			SetParent(self.dropItems[iter_91_0], findTF(self.battleItems[iter_91_0], "state_current/icon"))
			setActive(self.dropItems[iter_91_0], true)
		elseif var_91_0 < iter_91_0 and iter_91_0 <= var_91_0 + var_91_1 then
			setActive(findTF(self.battleItems[iter_91_0], "state_open"), true)
			SetParent(self.dropItems[iter_91_0], findTF(self.battleItems[iter_91_0], "state_open/icon"))
			setActive(self.dropItems[iter_91_0], true)
		else
			setActive(findTF(self.battleItems[iter_91_0], "state_closed"), true)
			SetParent(self.dropItems[iter_91_0], findTF(self.battleItems[iter_91_0], "state_closed/icon"))
			setActive(self.dropItems[iter_91_0], true)
		end
	end
	...
	local var_91_2 = 1 - (self:getGameUsedTimes() - 3 < 0 and 0 or self:getGameUsedTimes() - 3) / (self.totalTimes - 4)

	if var_91_2 > 1 then
		var_91_2 = 1
	end

	scrollTo(self.battleScrollRect, 0, var_91_2)
	setActive(findTF(self.menuUI, "btnStart/tip"), var_91_1 > 0)
	self:CheckGet()

	local var_91_3 = self:GetMGData():GetRuntimeData("elements")

	setText(findTF(self.menuUI, "high"), (var_91_3 and #var_91_3 > 0 or nil) and (var_91_3[1] or 0))

	return
end

function CatchTreasureGameView:CheckGet()
...
function CatchTreasureGameView:openMenuUI()
...
function CatchTreasureGameView:clearUI()
...
function CatchTreasureGameView:readyStart()
...
function CatchTreasureGameView:getGameTimes()
	return self:GetMGHubData().count
end

function CatchTreasureGameView:getGameUsedTimes()
	return self:GetMGHubData().usedtime
end

function CatchTreasureGameView:getUltimate()
	return self:GetMGHubData().ultimate
end

function CatchTreasureGameView:getGameTotalTime()
	return (self:GetMGHubData():getConfig("reward_need"))
end

function CatchTreasureGameView:gameStart()
...
function CatchTreasureGameView:transformColor(arg_101_1)
...
function CatchTreasureGameView:addScore(arg_102_1, arg_102_2)
	...
	local var_102_0 = findTF(self.sceneScoreTf, "img")
	local var_102_1 = GetComponent(var_102_0, typeof(Text))

	if arg_102_1 then
		local var_102_2

		for iter_102_0 = 1, #var_0_40 do
			if arg_102_1 and arg_102_1 >= var_0_40[iter_102_0].score then
				var_102_2 = var_0_40[iter_102_0].font

				break
			end
		end

		local var_102_4 = self:transformColor("6f1807")

		self.scoreNum = self.scoreNum + arg_102_1

		setText(var_102_0, (arg_102_1 >= 0 and "+" or "") .. arg_102_1)

		var_102_1.fontSize = var_102_2 or 40
	...
	if arg_102_2 then
		...
		setTextColor(var_102_0, (self:transformColor("66f2fb")))

		local var_102_5

		if self.gameTime > 0 then
			self.gameTime = self.gameTime + arg_102_2
			var_102_5 = setText
		end
...
function CatchTreasureGameView:onTimer()
	self:gameStep()

	return
end

function CatchTreasureGameView:gameStep()
...
function CatchTreasureGameView:timerStart()
...
function CatchTreasureGameView:timerStop()
...
function CatchTreasureGameView:updateGameUI()
...
function CatchTreasureGameView:onGameOver(arg_108_1)
	...
	self:emit(BaseMiniGameMediator.GAME_FINISH_TRACKING, {
		game_id = self:GetMGData().id,
		hub_id = self:GetMGHubData().id,
...
function CatchTreasureGameView:showSettlement()
	setActive(self.settlementUI, true)
	GetComponent(findTF(self.settlementUI, "ad"), typeof(Animator)):Play("settlement", -1, 0)

	local var_110_0 = self:GetMGData():GetRuntimeData("elements")
	local var_110_1 = self.scoreNum
	local var_110_2

	if var_110_0 and #var_110_0 > 0 then
		var_110_2 = var_110_0[1] or 0
	end

	setActive(findTF(self.settlementUI, "ad/new"), var_110_2 < var_110_1)
	...
	setText(findTF(self.settlementUI, "ad/highText"), var_110_2)
	setText(findTF(self.settlementUI, "ad/currentText"), var_110_1)
...
function CatchTreasureGameView:resumeGame()
	self.gameStop = false
...
function CatchTreasureGameView:stopGame()
	self.gameStop = true
...
function CatchTreasureGameView:onBackPressed()
	if not self.gameStartFlag then
		self:emit(CatchTreasureGameView.ON_BACK_PRESSED)
	else
		if self.settlementFlag then
			return
		end
...
function CatchTreasureGameView:willExit()
	if self.handle then
