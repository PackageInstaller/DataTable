local GameRoomBoatAdView = class("GameRoomBoatAdView", import("..BaseMiniGameView"))
local BoatAdGameVo = import("view.miniGame.gameView.BoatAdGame.BoatAdGameVo")

function GameRoomBoatAdView:getUIName()
	return BoatAdGameVo.game_room_ui
end

function GameRoomBoatAdView:getBGM()
	return BoatAdGameVo.menu_bgm
end

function GameRoomBoatAdView:didEnter()
	self:initData()
	self:initEvent()
	self:initUI()
	self:checkGet()
...
function GameRoomBoatAdView:checkGet()
...
function GameRoomBoatAdView:initData()
...
function GameRoomBoatAdView:initEvent()
...
function GameRoomBoatAdView:initUI()
...
function GameRoomBoatAdView:Update()
...
function GameRoomBoatAdView:readyStart()
...
function GameRoomBoatAdView:gameStart()
...
function GameRoomBoatAdView:changeSpeed(arg_29_1)
	return
end

function GameRoomBoatAdView:onTimer()
	self:gameStep()

	return
end

function GameRoomBoatAdView:gameStep()
...
function GameRoomBoatAdView:timerStart()
...
function GameRoomBoatAdView:timerResume()
	if not self.timer.running then
		self.realTimeStartUp = Time.realtimeSinceStartup
...
function GameRoomBoatAdView:timerStop()
...
function GameRoomBoatAdView:stepRunTimeData()
...
function GameRoomBoatAdView:addScore(arg_36_1)
	BoatAdGameVo.scoreNum = BoatAdGameVo.scoreNum + arg_36_1

	return
end

function GameRoomBoatAdView:onGameOver(arg_37_1)
...
function GameRoomBoatAdView:OnApplicationPaused()
...
function GameRoomBoatAdView:clearController()
	self.gameScene:clear()

	return
end

function GameRoomBoatAdView:pauseGame()
	self.gameStop = true
...
function GameRoomBoatAdView:resumeGame()
	self.gameStop = false
...
function GameRoomBoatAdView:onBackPressed()
	if self.readyStartFlag then
		return
	end

	if not self.gameStartFlag then
		self:emit(GameRoomBoatAdView.ON_BACK_PRESSED)
...
function GameRoomBoatAdView:OnSendMiniGameOPDone(arg_44_1)
	return
end

function GameRoomBoatAdView:willExit()
	if self.handle then
