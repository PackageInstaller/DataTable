local GameRoomPipeView = class("GameRoomPipeView", import("..BaseMiniGameView"))
local PipeGameVo = import("view.miniGame.gameView.PipeGame.PipeGameVo")

function GameRoomPipeView:getUIName()
	return PipeGameVo.game_room_ui
end

function GameRoomPipeView:getBGM()
	return PipeGameVo.menu_bgm
end

function GameRoomPipeView:didEnter()
	self:initData()
	self:initEvent()
	self:initUI()
...
function GameRoomPipeView:initData()
...
function GameRoomPipeView:initEvent()
	...
	self:bind(PipeGameEvent.SUBMIT_GAME_SUCCESS, function(arg_20_0, arg_20_1, arg_20_2)
		self:SendSuccess(PipeGameVo.scoreNum)
...
function GameRoomPipeView:initUI()
	...
	self.menuUI:update(self:GetMGHubData())
	self.menuUI:setGameRoomUI(self:getGameRoomData() and true or false)
	self.menuUI:show(true)
...
function GameRoomPipeView:changeBgm(arg_23_1)
	local var_23_0
...
function GameRoomPipeView:UpdateBeat()
...
function GameRoomPipeView:readyStart()
...
function GameRoomPipeView:gameStart()
...
function GameRoomPipeView:changeSpeed(arg_27_1)
	return
end

function GameRoomPipeView:onTimer()
	self:gameStep()

	return
end

function GameRoomPipeView:gameStep()
...
function GameRoomPipeView:timerStart()
...
function GameRoomPipeView:timerResume()
	if not self.timer.running then
		self.timer:Start()
	end
...
function GameRoomPipeView:timerStop()
...
function GameRoomPipeView:stepRunTimeData()
...
function GameRoomPipeView:addScore(arg_34_1)
	PipeGameVo.scoreNum = PipeGameVo.scoreNum + arg_34_1

	return
end

function GameRoomPipeView:onGameOver()
...
function GameRoomPipeView:OnApplicationPaused()
...
function GameRoomPipeView:clearController()
	self.gameScene:clear()

	return
end

function GameRoomPipeView:pauseGame()
	self.gameStop = true
...
function GameRoomPipeView:resumeGame()
	self.gameStop = false
...
function GameRoomPipeView:getRankData()
	pg.m02:sendNotification(GAME.MINI_GAME_FRIEND_RANK, {
		id = PipeGameVo.game_id,
		callback = function(self)
			local var_42_0 = {}

			for iter_42_0 = 1, #self do
				local var_42_1 = {}

				for iter_42_1, iter_42_2 in pairs(self[iter_42_0]) do
			...
			self.popUI:updateRankData(var_42_0)
...
function GameRoomPipeView:onBackPressed()
	if self.readyStartFlag then
		return
	end
...
function GameRoomPipeView:OnSendMiniGameOPDone(arg_45_1)
	return
end

function GameRoomPipeView:willExit()
	if self.handle then
