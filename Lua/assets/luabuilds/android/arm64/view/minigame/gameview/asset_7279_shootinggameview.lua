local ShootingGameView = class("ShootingGameView", import("..BaseMiniGameView"))

ShootingGameView.animTime = 0.3333333333333333
ShootingGameView.moveModulus = 120

function ShootingGameView:getUIName()
	return "ShootingGameUI"
end

function ShootingGameView:init()
	self.uiMGR = pg.UIMgr.GetInstance()
	self.blurPanel = self._tf:Find("noAdaptPanel/blur_panel")
	self.top = self.blurPanel:Find("top")
	self.backBtn = self.top:Find("back")
	self.scoreTF = self.top:Find("score/Text")

	setText(self.scoreTF, 0)

	self.bestScoreTF = self.top:Find("score_heightest/Text")
	self.ticketTF = self.top:Find("ticket/Text")
	self.helpBtn = self.top:Find("help_btn")
	self.sightTF = self.blurPanel:Find("MoveArea/Sight")

	setActive(self.sightTF, false)

	self.corners = self.blurPanel:Find("Corners")
	self.shootAreaTF = self._tf:Find("noAdaptPanel/ShootArea")
	self.targetPanel = self.shootAreaTF:Find("target_panel")
	self.targetTpl = {}

	local var_2_0 = self.shootAreaTF:Find("tpl")

	for iter_2_0 = 1, var_2_0.childCount do
		self.targetTpl[iter_2_0] = var_2_0:GetChild(iter_2_0 - 1)
	end

	setActive(var_2_0, false)

	self.startMaskTF = self.shootAreaTF:Find("start_mask")
	self.countdownTF = self.startMaskTF:Find("count")
	self.lastTimeTF = self.shootAreaTF:Find("time_word")
	self.bottomTF = self._tf:Find("noAdaptPanel/bottom")
	self.joyStrickTF = self.bottomTF:Find("Stick")
	self.fireBtn = self.bottomTF:Find("fire/ActCtl")
	self.fireBtnDelegate = GetOrAddComponent(self.fireBtn, "EventTriggerListener")

	setActive(self.fireBtn:Find("block"), false)

	self.resultPanel = self._tf:Find("result_panel")

	setActive(self.resultPanel, false)

	return
end

function ShootingGameView:initData()
	self.tempConfig = self:GetMGData():getConfig("simple_config_data")
	self.tempConfig.waitCountdown = 3
	self.tempConfig.half = 56

	return
end

function ShootingGameView:addTimer(arg_4_1, arg_4_2, arg_4_3)
	self.timerList = self.timerList or {}

	assert(self.timerList[arg_4_1] == nil, "error Timers")
	assert(arg_4_2 > 0, "duration must >0")

	self.timerList[arg_4_1] = {
		timeMark = Time.realtimeSinceStartup + arg_4_2,
		func = arg_4_3
	}

	return
end

function ShootingGameView:updateTimers()
	for iter_5_0, iter_5_1 in pairs(self.timerList) do
		if Time.realtimeSinceStartup > iter_5_1.timeMark then
			self.timerList[iter_5_0] = nil

			iter_5_1.func()
		end
	end

	return
end

function ShootingGameView:stopTimers()
	self.isStopped = true

	for iter_6_0, iter_6_1 in pairs(self.timerList) do
		iter_6_1.timeMark = iter_6_1.timeMark - Time.realtimeSinceStartup
	end

	return
end

function ShootingGameView:restartTimers()
	self.isStopped = false

	for iter_7_0, iter_7_1 in pairs(self.timerList) do
		iter_7_1.timeMark = iter_7_1.timeMark + Time.realtimeSinceStartup
	end

	return
end

function ShootingGameView:clearTimers()
	self.timerList = {}

	return
end

function ShootingGameView:didEnter()
	onButton(self, self.backBtn, function()
		if self.isPlaying then
			self:stopTimers()
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				content = i18n("tips_summergame_exit"),
				onYes = function()
					self:gameFinish(true)
					self:closeView()

					return
				end,
				onNo = function()
					self:restartTimers()

					return
				end
			})
		else
			self:closeView()
		end

		return
	end)
	onButton(self, self.helpBtn, function()
		pg.MsgboxMgr.GetInstance():ShowMsgBox({
			type = MSGBOX_TYPE_HELP,
			helps = pg.gametip.help_summer_shooting.tip
		})

		return
	end, SFX_PANEL)
	onButton(self, self.startMaskTF, function()
		if not self.isPlaying then
			self:gameStart()
		end

		return
	end)
	self:initData()
	self:updateCount()
	self:resetTime()
	self:initFireFunc()
	self:setFireLink(false)
	setActive(self.startMaskTF, true)

	return
end

function ShootingGameView:onBackPressed()
	triggerButton(self.backBtn)

	return
end

local function var_0_1(arg_16_0, arg_16_1)
	return Vector2(math.clamp(arg_16_0.x, -arg_16_1.x, arg_16_1.x), math.clamp(arg_16_0.y, -arg_16_1.y, arg_16_1.y))
end

function ShootingGameView:update()
	local var_17_0 = Time.GetTimestamp()

	if not self.isStopped then
		if self.isAfterCount and self.sightTimeMark then
			local var_17_4
			local var_17_2
			local var_17_3

			if not self.moveRect then
				local var_17_1 = tf(self.sightTF.parent)

				self.moveRect = Vector2(var_17_1.rect.width - self.sightTF.rect.width, var_17_1.rect.height - self.sightTF.rect.height) / 2
				var_17_2 = self.sightTF
				var_17_3 = var_0_1
				var_17_4 = self.sightTF.anchoredPosition
			end

			var_17_2.anchoredPosition = var_17_3(var_17_4 + Vector2(self.uiMGR.hrz, self.uiMGR.vtc) * self.tempConfig.moveSpeed * (var_17_0 - self.sightTimeMark) * ShootingGameView.moveModulus * (self.isDown and 0.5 or 1), self.moveRect)
		end

		self:updateTimers()
	end

	self.sightTimeMark = var_17_0

	return
end

function ShootingGameView:resetTime()
	self.countdown = self.tempConfig.waitCountdown

	setText(self.countdownTF, self.countdown)

	self.lastTime = self.tempConfig.baseTime

	setText(self.lastTimeTF, self.lastTime)

	return
end

function ShootingGameView:gameStart()
	self.isPlaying = true

	UpdateBeat:Add(self.update, self)
	setActive(self.countdownTF, true)
	setActive(self.startMaskTF:Find("word"), false)

	local function var_19_0(arg_20_0)
		self:addTimer("start countdown", 1, function()
			self.countdown = self.countdown - 1

			setText(self.countdownTF, self.countdown)

			if self.countdown > 0 then
				arg_20_0(arg_20_0)
			else
				self:afterCountDown()
			end

			return
		end)

		return
	end

	var_19_0(var_19_0)

	return
end

function ShootingGameView:afterCountDown()
	self.isAfterCount = true

	self.uiMGR:AttachStickOb(self.joyStrickTF)
	setActive(self.sightTF, true)
	setAnchoredPosition(self.sightTF, Vector2.zero)
	self:setFireLink(true)
	setActive(self.startMaskTF, false)

	self.score = 0

	self:flushTarget(true)

	local function var_22_0(arg_23_0)
		self:addTimer("gamefinish", 1, function()
			self.lastTime = self.lastTime - 1

			setText(self.lastTimeTF, self.lastTime)

			if self.lastTime > 0 then
				arg_23_0(arg_23_0)
			else
				self:gameFinish()
			end

			return
		end)

		return
	end

	var_22_0(var_22_0)

	return
end

function ShootingGameView:gameFinish(arg_25_1)
	if self.isAfterCount then
		self:setFireLink(false)
		self.uiMGR:ClearStick()

		self.isAfterCount = false
	end

	self:clearTimers()
	UpdateBeat:Remove(self.update, self)
	setActive(self.sightTF, false)
	setActive(self.countdownTF, false)
	self:resetTime()

	self.isPlaying = false

	if not arg_25_1 then
		for iter_25_0 = 1, 3 do
			for iter_25_1 = 1, 6 do
				if self.cell[iter_25_0][iter_25_1] then
					self.targetPanel:Find("line_" .. iter_25_0):GetChild(iter_25_1 - 1):GetChild(0):GetComponent(typeof(Animator)):Play("targetDown")
				end
			end
		end

		Timer.New(function()
			setActive(self.startMaskTF, true)
			setActive(self.startMaskTF:Find("word"), true)

			return
		end, ShootingGameView.animTime):Start()
		self:resultFinish()
	end

	return
end

function ShootingGameView:resultFinish()
	local var_27_0

	for iter_27_0 = 1, #self.tempConfig.score_level do
		if self.score >= self.tempConfig.score_level[#self.tempConfig.score_level - iter_27_0 + 1] then
			var_27_0 = iter_27_0

			break
		end
	end

	self.awardLevel = var_27_0

	if self:GetMGHubData().count > 0 then
		self:SendSuccess(var_27_0)
	else
		self:showResultPanel({})
	end

	return
end

function ShootingGameView:showResultPanel(arg_28_1, arg_28_2)
	local function var_28_0()
		setActive(self.resultPanel, false)

		if arg_28_2 then
			arg_28_2()
		else
			self:updateCount()
		end

		return
	end

	onButton(self, self.resultPanel:Find("bg"), var_28_0)
	onButton(self, self.resultPanel:Find("main/btn_confirm"), var_28_0)

	local var_28_1 = self.resultPanel:Find("main")

	if self.score > self.bestScore then
		self:StoreDataToServer({
			self.score
		})
		GetImageSpriteFromAtlasAsync("ui/minigameui/shootinggameui_atlas", "new_recode", var_28_1:Find("success"), true)
	else
		GetImageSpriteFromAtlasAsync("ui/minigameui/shootinggameui_atlas", "success", var_28_1:Find("success"), true)
	end

	GetImageSpriteFromAtlasAsync("ui/minigameui/shootinggameui_atlas", "level_" .. #self.tempConfig.score_level - self.awardLevel + 1, var_28_1:Find("success/level"), true)
	setText(var_28_1:Find("right/score/number"), self.score)
	setActive(var_28_1:Find("right/awards/list"), #arg_28_1 > 0)
	setActive(var_28_1:Find("right/awards/nothing"), #arg_28_1 == 0)

	self.itemList = self.itemList or UIItemList.New(var_28_1:Find("right/awards/list"), var_28_1:Find("right/awards/list/item"))

	self.itemList:make(function(arg_30_0, arg_30_1, arg_30_2)
		if arg_30_0 == UIItemList.EventUpdate then
			updateDrop(arg_30_2, arg_28_1[arg_30_1 + 1])
			setText(arg_30_2:Find("number"), "x" .. arg_28_1[arg_30_1 + 1].count)
		end

		return
	end)
	self.itemList:align(#arg_28_1)
	setActive(self.resultPanel, true)

	return
end

function ShootingGameView:updateAfterFinish()
	local var_31_0 = getProxy(MiniGameProxy):GetMiniGameData(MiniGameDataCreator.ShrineGameID):GetRuntimeData("count") or 0

	pg.m02:sendNotification(GAME.MODIFY_MINI_GAME_DATA, {
		id = MiniGameDataCreator.ShrineGameID,
		map = {
			count = var_31_0 + 1
		}
	})

	return
end

function ShootingGameView:OnGetAwardDone(arg_32_1)
	if arg_32_1.cmd == MiniGameOPCommand.CMD_COMPLETE then
		local var_32_0 = self:GetMGHubData()

		if var_32_0.ultimate == 0 and var_32_0.usedtime >= var_32_0:getConfig("reward_need") then
			pg.m02:sendNotification(GAME.SEND_MINI_GAME_OP, {
				hubid = var_32_0.id,
				cmd = MiniGameOPCommand.CMD_ULTIMATE,
				args1 = {}
			})
		end
	elseif arg_32_1.cmd == MiniGameOPCommand.CMD_ULTIMATE then
		pg.NewStoryMgr.GetInstance():Play("TIANHOUYUYI2")
	end

	return
end

function ShootingGameView:OnSendMiniGameOPDone(arg_33_1)
	self:updateCount()

	return
end

function ShootingGameView:updateCount()
	setText(self.ticketTF, self:GetMGHubData().count)

	self.bestScore = checkExist(self:GetMGData():GetRuntimeData("elements"), {
		1
	}) or 0

	setText(self.bestScoreTF, self.bestScore)

	return
end

function ShootingGameView:initFireFunc()
	local var_35_0 = pg.TipsMgr.GetInstance()
	local var_35_1 = pg.TimeMgr.GetInstance()
	local var_35_2 = self.sightTF:Find("sight_base")
	local var_35_3 = self.sightTF:Find("sight_ready")

	setImageAlpha(var_35_2, 1)
	setImageAlpha(var_35_3, 0)

	local function var_35_4()
		setActive(self.corners, true)
		LeanTween.scale(var_35_2, Vector3(1.95, 1.95, 1), 0.1):setOnComplete(System.Action(function()
			LeanTween.alpha(var_35_2, 0, 0.1)
			LeanTween.alpha(var_35_3, 1, 0.1)

			return
		end))

		return
	end

	local function var_35_5()
		setActive(self.corners, false)
		LeanTween.alpha(var_35_2, 1, 0.1)
		LeanTween.alpha(var_35_3, 0, 0.1):setOnComplete(System.Action(function()
			LeanTween.scale(var_35_2, Vector3.one, 0.1)

			return
		end))

		return
	end

	function self._downFunc()
		var_35_4()

		return
	end

	function self._upFunc()
		LeanTween.scale(var_35_3, Vector3(2, 2, 2), 0.03):setOnComplete(System.Action(function()
			LeanTween.scale(var_35_3, Vector3.one, 0.07):setOnComplete(System.Action(function()
				var_35_5()

				return
			end))

			return
		end))

		local var_41_0, var_41_1, var_41_2 = self:checkHit()

		if var_41_0 then
			self.cell[var_41_1][var_41_2] = nil
			self.score = self.score + self.tempConfig.targetScore[self.cell[var_41_1][var_41_2]]
			self.targetCount[self.cell[var_41_1][var_41_2]] = self.targetCount[self.cell[var_41_1][var_41_2]] - 1
			self.lastTime = self.lastTime + self.tempConfig.bonusTime

			setText(self.lastTimeTF, self.lastTime)
			self.targetPanel:Find("line_" .. var_41_1):GetChild(var_41_2 - 1):GetChild(0):GetComponent(typeof(Animator)):Play("targetDown")
			self:addTimer("flush call", 0.2 + ShootingGameView.animTime, function()
				self:flushTarget()

				return
			end)

			if not _.any(self.targetCount, function(arg_45_0)
				return arg_45_0 > 0
			end) then
				self:gameFinish()
			end
		end

		self:setFireLink(false)
		self:addTimer("fire cd", self.tempConfig.fireCD, function()
			self:setFireLink(true)

			return
		end)

		return
	end

	function self._cancelFunc()
		var_35_5()

		return
	end

	self._emptyFunc = nil

	return
end

function ShootingGameView:setFireLink(arg_48_1)
	if arg_48_1 then
		setButtonEnabled(self.fireBtn, true)

		if self._downFunc ~= nil then
			self.fireBtnDelegate:AddPointDownFunc(function()
				self.isDown = true

				if self._main_cannon_sound then
					self._main_cannon_sound:Stop(true)
				end

				self._main_cannon_sound = pg.CriMgr.GetInstance():PlaySE_V3("battle-cannon-main-prepared")

				self._downFunc()

				return
			end)
		end

		if self._upFunc ~= nil then
			self.fireBtnDelegate:AddPointUpFunc(function()
				if self.isDown then
					if self._main_cannon_sound then
						self._main_cannon_sound:Stop(true)
					end

					pg.CriMgr.GetInstance():PlaySoundEffect_V3("event:/battle/boom2")

					self.isDown = false

					self._upFunc()
				end

				return
			end)
		end

		if self._cancelFunc ~= nil then
			self.fireBtnDelegate:AddPointExitFunc(function()
				if self.isDown then
					if self._main_cannon_sound then
						self._main_cannon_sound:Stop(true)
					end

					self.isDown = false

					self._cancelFunc()
				end

				return
			end)
		end
	else
		if self.isDown then
			self.isDown = false

			self._cancelFunc()
		end

		setButtonEnabled(self.fireBtn, false)
		self.fireBtnDelegate:RemovePointDownFunc()
		self.fireBtnDelegate:RemovePointUpFunc()
		self.fireBtnDelegate:RemovePointExitFunc()
	end

	return
end

function ShootingGameView:flushTarget(arg_52_1)
	if arg_52_1 then
		self.targetCount = {
			2,
			4,
			6
		}
	end

	for iter_52_0 = 1, 3 do
		for iter_52_1 = 1, 6 do
			removeAllChildren(self.targetPanel:Find("line_" .. iter_52_0):GetChild(iter_52_1 - 1))
		end
	end

	local var_52_0 = {
		0,
		0,
		0
	}

	self.cell = {
		{},
		{},
		{}
	}

	for iter_52_2, iter_52_3 in ipairs(self.targetCount) do
		for iter_52_4 = 1, iter_52_3 do
			local var_52_1 = math.random(3)
			local var_52_2 = math.random(6)

			while self.cell[var_52_1][var_52_2] or arg_52_1 and var_52_0[var_52_1] >= 4 do
				var_52_2 = math.random(6)
				var_52_1 = math.random(3)
			end

			var_52_0[var_52_1] = var_52_0[var_52_1] + 1
			self.cell[var_52_1][var_52_2] = iter_52_2

			cloneTplTo(self.targetTpl[iter_52_2], self.targetPanel:Find("line_" .. var_52_1):GetChild(var_52_2 - 1))
		end
	end

	setText(self.scoreTF, self.score)

	return
end

function ShootingGameView:checkHit()
	for iter_53_0 = 1, 3 do
		for iter_53_1 = 1, 6 do
			if self.cell[iter_53_0][iter_53_1] then
				local var_53_0 = self.targetPanel:Find("line_" .. iter_53_0):GetChild(iter_53_1 - 1):GetChild(0):Find("icon/face")
				local var_53_1 = self.sightTF:InverseTransformPoint(var_53_0:TransformPoint(var_53_0.position))

				if var_53_1.x * var_53_1.x + var_53_1.y * var_53_1.y < self.tempConfig.half * self.tempConfig.half then
					return true, iter_53_0, iter_53_1
				end
			end
		end
	end

	return
end

function ShootingGameView:willExit()
	return
end

return ShootingGameView
