local CurlingGameView = class("CurlingGameView", import("..BaseMiniGameView"))
...
function CurlingGameView:getUIName()
	return "CurlingGameUI"
end

function CurlingGameView:didEnter()
	self:initEvent()
	self:initData()
	self:initUI()
	self:initGameUI()
...
function CurlingGameView:AutoFitScreen()
...
function CurlingGameView:initEvent()
...
function CurlingGameView:initData()
...
function CurlingGameView:initUI()
	...
	onButton(self, self.mainUI:Find("return_btn"), function()
		self:emit(CurlingGameView.ON_BACK_PRESSED)

		return
	end, SFX_PANEL)
	onButton(self, self.mainUI:Find("main_btn"), function()
		self:emit(CurlingGameView.ON_HOME)
	...
	local var_91_0 = self.mainUI:Find("item_tpl")

	self.itemList = {}

	local var_91_1 = pg.mini_game[self:GetMGData().id].simple_config_data.drop
	...
	for iter_91_0 = 1, #var_91_1 do
		...
		setParent(var_91_2, self.mainUI:Find("item_list/Viewport/Content"))
		...
		table.insert(self.itemList, var_91_2)
		...
		updateDrop(var_91_3, var_91_4)
		onButton(self, var_91_3, function()
			self:emit(BaseUI.ON_DROP, var_91_4)

			return
		end, SFX_PANEL)
	end

	self.rankUI = findTF(self._tf, "ui/rank_ui")

	self:openRankUI(false)
...
function CurlingGameView:initGameUI()
...
function CurlingGameView:initController()
...
function CurlingGameView:updateMainUI()
	local var_114_0 = self:getGameUsedTimes()
	local var_114_1 = self:getGameTimes()

	for iter_114_0 = 1, #self.itemList do
		setActive(self.itemList[iter_114_0]:Find("lock"), false)
		setActive(self.itemList[iter_114_0]:Find("finish"), false)

		if iter_114_0 <= var_114_0 then
			setActive(self.itemList[iter_114_0]:Find("finish"), true)
		elseif iter_114_0 == var_114_0 + 1 and var_114_1 >= 1 then
			-- block empty
		elseif var_114_0 < iter_114_0 and iter_114_0 <= var_114_0 + var_114_1 then
			-- block empty
		else
			setActive(self.itemList[iter_114_0]:Find("lock"), true)
		end
	end

	self.totalTimes = self:getGameTotalTime()

	local var_114_2 = 1 - (self:getGameUsedTimes() - 3 < 0 and 0 or self:getGameUsedTimes() - 3) / (self.totalTimes - 4)

	if var_114_2 > 1 then
		var_114_2 = 1
	end

	scrollTo(self.listScrollRect, 0, var_114_2)
	self:checkGet()

	return
end

function CurlingGameView:updateRankUI(arg_115_1)
	for iter_115_0 = 1, #arg_115_1 do
		if iter_115_0 > #self._rankItems then
			local var_115_1 = tf(instantiate(self._rankItemTpl))

			setActive(var_115_1, false)
			setParent(var_115_1, self._rankContent)
			table.insert(self._rankItems, var_115_1)
		end

		local var_115_2 = self._rankItems[iter_115_0]

		self:setRankItemData(self._rankItems[iter_115_0], arg_115_1[iter_115_0], iter_115_0)
		setActive(var_115_2, true)
	end

	for iter_115_1 = #arg_115_1 + 1, #self._rankItems do
		setActive(self._rankItems, false)
	end

	setActive(self._rankEmpty, #arg_115_1 == 0)
	setActive(self._rankImg, #arg_115_1 > 0)

	return
end

function CurlingGameView:checkGet()
...
function CurlingGameView:openMainUI()
...
function CurlingGameView:openRankUI(arg_118_1)
	setActive(self.rankUI, arg_118_1)
	...
	if arg_118_1 then
		pg.m02:sendNotification(GAME.MINI_GAME_FRIEND_RANK, {
			...
			callback = function(self)
				...
				for iter_119_0 = 1, #self do
					local var_119_1 = {}

					for iter_119_1, iter_119_2 in pairs(self[iter_119_0]) do
						var_119_1[iter_119_1] = iter_119_2
					end

					table.insert(var_119_0, var_119_1)
				end
...
function CurlingGameView:readyStart()
...
function CurlingGameView:resetGame()
...
function CurlingGameView:startGame()
...
function CurlingGameView:staticObsStart()
	...
	if math.random() <= var_0_37.walker.appear then
		...
		local var_124_0 = var_0_37.walker.path[math.random(1, #var_0_37.walker.path)]

		self.walker:SetPath(var_124_0)

		local var_124_1 = {}

		if var_124_0 == var_0_26 then
			var_124_1 = {
				8,
				11,
				12,
		...
		elseif var_124_0 == var_0_24 then
			var_124_1 = {
				5,
				9,
				10,
		...
		local function var_124_2(arg_125_0)
			for iter_125_0, iter_125_1 in ipairs(var_124_1) do
		...
		for iter_124_0, iter_124_1 in ipairs(self.grids) do
			if not var_124_2(iter_124_0) then
				table.insert(var_124_3, iter_124_1)
			end
		end

		self.grids = var_124_3
	...
	for iter_124_2, iter_124_3 in ipairs(var_0_37.oil) do
		if math.random() <= iter_124_3.appear then
			for iter_124_4 = 1, iter_124_3.num do
				local var_124_4 = cloneTplTo(self.obsTpl:Find("oil_Tpl"), self.oilGroups, "oil")

				setActive(var_124_4, true)

				local var_124_5 = math.random(1, #self.grids)

				setLocalPosition(var_124_4, Vector2(self.grids[var_124_5].x, self.grids[var_124_5].y))
				setLocalScale(var_124_4, Vector2(var_0_40.oil, var_0_40.oil))
				table.remove(self.grids, var_124_5)
			end
		end
	end
	...
	for iter_124_5, iter_124_6 in ipairs(var_0_37.cube) do
		if math.random() <= iter_124_6.appear then
			for iter_124_7 = 1, iter_124_6.num do
				local var_124_6 = cloneTplTo(self.obsTpl:Find("cube_Tpl"), self.cubeGroups, "cube")

				setActive(var_124_6, true)

				local var_124_7 = math.random(1, #self.grids)

				setLocalPosition(var_124_6, Vector2(self.grids[var_124_7].x, self.grids[var_124_7].y))
				setLocalScale(var_124_6, Vector2(var_0_40.cube, var_0_40.cube))
				table.remove(self.grids, var_124_7)
			end
		end
	end
	...
	for iter_124_8, iter_124_9 in ipairs(var_0_37.miner) do
		if math.random() <= iter_124_9.appear then
			for iter_124_10 = 1, iter_124_9.num do
				local var_124_8 = cloneTplTo(self.obsTpl:Find("miner_Tpl"), self.minerGroups, "miner")

				setActive(var_124_8, true)
				table.insert(self.minerControls, (var_0_54(var_124_8, self)))

				local var_124_9 = math.random(1, #self.grids)

				setLocalPosition(var_124_8, Vector2(self.grids[var_124_9].x, self.grids[var_124_9].y))
				setLocalScale(var_124_8, Vector2(var_0_40.miner, var_0_40.miner))
				table.remove(self.grids, var_124_9)
			end
		end
	end
...
function CurlingGameView:obsFadeOut()
	self:managedTween(LeanTween.value, function()
		setActive(self.obsTF, false)

		return
	end, go(self.obsTF), 1, 0, 0.5):setOnUpdate(System.Action_float(function(arg_128_0)
		self.obsCanvas.alpha = arg_128_0
...
function CurlingGameView:Update()
	self:AddDebugInput()

	return
end

function CurlingGameView:AddDebugInput()
...
function CurlingGameView:changeSpeed(arg_131_1)
	return
end

function CurlingGameView:onTimer()
	self.curling:Step()
	self.walker:Step()
	self:updateGameUI()
...
function CurlingGameView:timerStart()
...
function CurlingGameView:timerStop()
...
function CurlingGameView:updateGameUI()
...
function CurlingGameView:addScore(arg_136_1, arg_136_2)
	local var_136_0 = cloneTplTo(self.gameUI:Find("score_tf"), self.scoreGroup)
	...
	self.scoreNum = self.scoreNum + arg_136_1

	return
end

function CurlingGameView:pauseGame()
	self.gamePause = true
...
function CurlingGameView:resumeGame()
	self.gamePause = false
...
function CurlingGameView:nextRoundGame()
...
function CurlingGameView:endGame()
...
function CurlingGameView:showEndUI()
	setActive(self.endUI, true)

	local var_142_0 = self.scoreNum
	local var_142_1 = getProxy(MiniGameProxy):GetHighScore(self:GetMGData().id)
	local var_142_2

	if var_142_1 and #var_142_1 > 0 then
		var_142_2 = var_142_1[1] or 0

		local var_142_3

		if var_142_1 and #var_142_1 > 1 then
			var_142_3 = var_142_1[2] or 0
		end
	end

	setActive(self.endUI:Find("ad/panel/cur_score/new"), var_142_2 < var_142_0)

	if var_142_2 <= var_142_0 then
		var_142_2 = var_142_0

		getProxy(MiniGameProxy):UpdataHighScore(self:GetMGData().id, {
			var_142_0,
			var_142_3
		})
	end

	setText(self.endUI:Find("ad/panel/highest_score"), var_142_2)
	setText(self.endUI:Find("ad/panel/cur_score"), var_142_0)
...
function CurlingGameView:CoordinateGrid(arg_143_1)
	...
	for iter_143_0 = 1, var_143_3 do
		for iter_143_1 = 1, var_143_2 do
			table.insert(self.grids, Vector2(var_143_1.x + iter_143_0 * (arg_143_1.rect.width % var_143_0.x / (var_143_3 + 1) + var_143_0.x) - var_143_0.x / 2, var_143_1.y + iter_143_1 * (arg_143_1.rect.height % var_143_0.y / (var_143_2 + 1) + var_143_0.y) - var_143_0.y / 2))
		end
	end
...
function CurlingGameView:getGameTimes()
	return self:GetMGHubData().count
end

function CurlingGameView:getGameUsedTimes()
	return self:GetMGHubData().usedtime
end

function CurlingGameView:getUltimate()
	return self:GetMGHubData().ultimate
end

function CurlingGameView:getGameTotalTime()
	return (self:GetMGHubData():getConfig("reward_need"))
end

function CurlingGameView:onBackPressed()
...
function CurlingGameView:willExit()
	if self.handle then
