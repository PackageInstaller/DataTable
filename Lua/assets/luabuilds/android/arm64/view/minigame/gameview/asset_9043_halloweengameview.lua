local HalloweenGameView = class("HalloweenGameView", import("..BaseMiniGameView"))
local var_0_1 = 1
local var_0_2 = 2
local var_0_3 = 1
local var_0_4 = 2
...
function HalloweenGameView:getUIName()
	return "HalloweenGameUI"
end

function HalloweenGameView:getBGM()
	return "backyard"
end
...
local function var_0_36(arg_4_0, arg_4_1, arg_4_2)
	...
	function var_4_0:PlayAniamtion(arg_28_1, arg_28_2)
		...
		if self.onAniCallback then
			var_0_35(self.onAniamtionName .. "的animation被" .. arg_28_1 .. "中断")
		end
	...
	function var_4_0:onAnimationEnd()
		var_0_35("动作播放结束:" .. self.onAniamtionName)
	...
	function var_91_0:step()
		...
		if self.speedLevel <= #var_0_15 then
			...
			if var_0_30 >= self.nextSpeedUpTime then
				...
				var_0_35("幽灵生成速度提升" .. (self.nextSpeedUpTime or "(已经达到最高速度)"))
			end
		end
...
function HalloweenGameView:init()
	self:initUI()
	self:initData()
	self:gameReadyStart()
...
function HalloweenGameView:initUI()
...
function HalloweenGameView:initData()
	...
	self.charactor = var_0_36(findTF(self.scene, "charactor"), var_0_25, self.scene)

	function self.charactor.charactorIdleCallback(arg_124_0)
		setActive(self.playerIdleTip, arg_124_0)

		return
	end

	self.flyer = var_0_38(findTF(self.scene, "flyCharactor"), var_0_26)
	...
	self.controllerUI = var_0_37(findTF(self._tf, "controller"), findTF(self._tf, "conRight/useLight"))

	function self.controllerUI.pointChangeCallback(arg_126_0)
		self:onControllerDirectChange(arg_126_0)

		return
	end

	function self.controllerUI.pointUpCallback(arg_127_0)
		self:onControllerDirectUp(arg_127_0)

		return
	end

	function self.controllerUI.pointLightCallback(arg_128_0)
		self:onUseLight(arg_128_0)

		return
	end

	self.dropControl = var_0_39()
	...
	self.dropItemController = var_0_40(self.flyer, self.scene)
	...
	self.dropColliderControll = var_0_41(self.charactor, self.dropItemController, self.scene)
	...
	self.ghostBossController = var_0_42(findTF(self._tf, "ghostBoss"))
	...
	self.ghostChildController = var_0_43(findTF(self.scene, "tplGhost"), self.charactor, self.scene)

	function self.ghostChildController.catchCharactorCallback(arg_136_0)
		self:onGhostCatch(arg_136_0)

		return
	end

	self.eyesController = var_0_44(findTF(self._tf, "bg/eyes"), findTF(self.scene, "charactor"))
...
function HalloweenGameView:gameReadyStart()
...
function HalloweenGameView:gameStart()
...
function HalloweenGameView:timerStart()
...
function HalloweenGameView:timerStop()
...
function HalloweenGameView:getGameTimes()
	return self:GetMGHubData().count
end

function HalloweenGameView:getSoundData(arg_144_1)
	CueData.GetCueData().channelName = pg.CriMgr.C_GALLERY_MUSIC
	self.cueData.cueSheetName = arg_144_1
	self.cueData.cueName = ""

	return
end

function HalloweenGameView:onTimer()
...
function HalloweenGameView:updateUI()
	for iter_146_0 = 1, #self.hearts do
		if iter_146_0 <= self.heartNum then
			setActive(findTF(self.hearts[iter_146_0], "img"), true)
		else
			setActive(findTF(self.hearts[iter_146_0], "img"), false)
		end
	end
...
function HalloweenGameView:dropSpeedUp()
...
function HalloweenGameView:loseHeart(arg_148_1)
	if self.heartNum and self.heartNum > 0 then
		...
		if self.heartNum == 0 then
			...
			if arg_148_1 == var_0_5 then
				var_148_0 = var_0_7 or var_0_8
			end

			self.charactor:fail(var_148_0)

			if var_148_0 == var_0_8 then
				self.ghostChildController:createGhostLight(function(arg_149_0)
					if arg_149_0 then
...
function HalloweenGameView:addScore(arg_151_1)
	self.scoreNum = self.scoreNum + arg_151_1
...
function HalloweenGameView:gameOver()
...
function HalloweenGameView:showSettlement()
	setActive(self.settlementUI, true)
	GetComponent(findTF(self.settlementUI, "ad"), typeof(Animator)):Play("settlement", -1, 0)

	local var_153_0 = self:GetMGData():GetRuntimeData("elements")
	local var_153_1 = self.scoreNum
	local var_153_2

	if var_153_0 and #var_153_0 > 0 then
		var_153_2 = var_153_0[1] or 0
	...
	setText(findTF(self.settlementUI, "ad/highText"), var_153_2)
	setText(findTF(self.settlementUI, "ad/currentText"), var_153_1)

	return
end

function HalloweenGameView:lostCandy()
	self:loseHeart(var_0_5)

	return
end

function HalloweenGameView:touchBoom()
	self:loseHeart(var_0_6)

	return
end

function HalloweenGameView:createGhost()
...
function HalloweenGameView:onCreateDropItem()
...
function HalloweenGameView:onRequestDrop()
	if self.flyer then
...
function HalloweenGameView:onGhostCatch(arg_159_1)
	if not self.charactor:getGhostFlag() then
		self.charactor:setGhostFlag(true, function()
			self.ghostChildController:removeChild(arg_159_1)
...
function HalloweenGameView:onUseLight(arg_161_1)
	if not self.gameStartFlag then
		return
	end

	self.charactor:playLight(function(arg_162_0)
		if arg_162_0 and arg_161_1 == var_0_3 then
			self.ghostChildController:createGhostLight(function(arg_163_0)
				if arg_163_0 then
					self.ghostBossController:onGhostDestroy()
...
function HalloweenGameView:onColliderItem(arg_164_1)
	var_0_35("碰撞到了物品，数量:" .. #arg_164_1)

	return
end

function HalloweenGameView:onControllerDirectChange(arg_165_1)
	self:changeDirect(arg_165_1, true)

	return
end

function HalloweenGameView:onControllerDirectUp(arg_166_1)
	self:changeDirect(arg_166_1, false)

	return
end

function HalloweenGameView:changeDirect(arg_167_1, arg_167_2)
	if self.gameStartFlag then
		self.charactor:onDirectChange(arg_167_1, arg_167_2)
	end

	return
end

function HalloweenGameView:Update()
	self:AddDebugInput()

	return
end

function HalloweenGameView:AddDebugInput()
...
function HalloweenGameView:clearUI()
...
function HalloweenGameView:onBackPressed()
	if not self.gameStartFlag then
		self:emit(HalloweenGameView.ON_BACK_PRESSED)
	else
		setActive(self.leaveUI, true)
		self:timerStop()
...
function HalloweenGameView:willExit()
