local GameRoomTreasureView = class("GameRoomTreasureView", import("..BaseMiniGameView"))
...
function GameRoomTreasureView:getUIName()
	return "GameRoomTreasureUI"
end

function GameRoomTreasureView:getBGM()
	return var_0_1
end

function GameRoomTreasureView:didEnter()
	self:initEvent()
	self:initData()
	self:initUI()
	self:initGameUI()
...
function GameRoomTreasureView:initEvent()
...
function GameRoomTreasureView:initData()
...
function GameRoomTreasureView:initUI()
...
function GameRoomTreasureView:initGameUI()
	...
	self.dragDelegate:AddPointDownFunc(function(arg_83_0, arg_83_1)
...
function GameRoomTreasureView:Update()
	self:AddDebugInput()

	return
end

function GameRoomTreasureView:AddDebugInput()
...
function GameRoomTreasureView:updateMenuUI()
	return
end

function GameRoomTreasureView:openMenuUI()
...
function GameRoomTreasureView:clearUI()
...
function GameRoomTreasureView:readyStart()
...
function GameRoomTreasureView:getGameTimes()
	return self:GetMGHubData().count
end

function GameRoomTreasureView:getGameUsedTimes()
	return self:GetMGHubData().usedtime
end

function GameRoomTreasureView:getUltimate()
	return self:GetMGHubData().ultimate
end

function GameRoomTreasureView:gameStart()
...
function GameRoomTreasureView:transformColor(arg_94_1)
	return Color.New(tonumber(string.sub(arg_94_1, 1, 2), 16) / 255, tonumber(string.sub(arg_94_1, 3, 4), 16) / 255, tonumber(string.sub(arg_94_1, 5, 6), 16) / 255)
end

function GameRoomTreasureView:addScore(arg_95_1, arg_95_2)
	if arg_95_1 and arg_95_1 > 0 or arg_95_2 and arg_95_2 > 0 then
	...
	local var_95_0 = findTF(self.sceneScoreTf, "img")
	local var_95_1 = GetComponent(var_95_0, typeof(Text))

	if arg_95_1 then
		local var_95_2

		for iter_95_0 = 1, #var_0_40 do
			if arg_95_1 and arg_95_1 >= var_0_40[iter_95_0].score then
				var_95_2 = var_0_40[iter_95_0].font

				break
			end
		end

		local var_95_4 = self:transformColor("6f1807")

		self.scoreNum = self.scoreNum + arg_95_1

		setText(var_95_0, (arg_95_1 >= 0 and "+" or "") .. arg_95_1)

		var_95_1.fontSize = var_95_2 or 40

		setTextColor(var_95_0, var_95_4)
	elseif arg_95_2 then
		var_95_1.fontSize = 40

		setTextColor(var_95_0, (self:transformColor("66f2fb")))

		local var_95_5

		if self.gameTime > 0 then
			self.gameTime = self.gameTime + arg_95_2
			var_95_5 = setText
		end

		var_95_5(var_95_0, (arg_95_2 > 0 and "+" or "") .. arg_95_2 .. "s")
	end
...
function GameRoomTreasureView:onTimer()
	self:gameStep()

	return
end

function GameRoomTreasureView:gameStep()
...
function GameRoomTreasureView:timerStart()
...
function GameRoomTreasureView:timerStop()
...
function GameRoomTreasureView:updateGameUI()
...
function GameRoomTreasureView:onGameOver()
...
function GameRoomTreasureView:showSettlement()
	setActive(self.settlementUI, true)
	GetComponent(findTF(self.settlementUI, "ad"), typeof(Animator)):Play("settlement", -1, 0)

	local var_103_0 = self:GetMGData():GetRuntimeData("elements")
	local var_103_1 = self.scoreNum
	local var_103_2

	if var_103_0 and #var_103_0 > 0 then
		var_103_2 = var_103_0[1] or 0
	end

	setActive(findTF(self.settlementUI, "ad/new"), var_103_2 < var_103_1)
	...
	setText(findTF(self.settlementUI, "ad/highText"), var_103_2)
	setText(findTF(self.settlementUI, "ad/currentText"), var_103_1)
...
function GameRoomTreasureView:resumeGame()
	self.gameStop = false
...
function GameRoomTreasureView:stopGame()
	self.gameStop = true
...
function GameRoomTreasureView:onBackPressed()
	if not self.gameStartFlag then
		self:emit(GameRoomTreasureView.ON_BACK_PRESSED)
	else
		if self.settlementFlag then
			return
		end
...
function GameRoomTreasureView:willExit()
	if self.handle then
