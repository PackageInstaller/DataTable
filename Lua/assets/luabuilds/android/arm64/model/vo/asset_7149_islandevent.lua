local IslandEvent = class("IslandEvent", import(".BaseVO"))

function IslandEvent:bindConfigTable()
	return pg.activity_map_event_data
end

function IslandEvent:Ctor(arg_2_1)
	self.id = arg_2_1.id
...
function IslandEvent:CheckTrigger(arg_3_1)
	if self:getConfig("type") == 2 then
		local var_3_0 = underscore.detect(self:getConfig("params"), function(arg_4_0)
			return arg_4_0[1] == arg_3_1
		end)

		assert(var_3_0, string.format("event_%d without params option_%d", self.id, arg_3_1))
		...
		if var_3_0[2] then
			...
			if var_3_1:getOwnedCount() < var_3_1.count then
				local var_3_3 = i18n("common_no_item_1")
...
function IslandEvent:AfterTrigger(arg_5_1)
	if self:getConfig("type") == 2 then
		local var_5_0 = underscore.detect(self:getConfig("params"), function(arg_6_0)
