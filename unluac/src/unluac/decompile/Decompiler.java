...
public class Decompiler {
  ...
  private Expression namedCall(String name, Expression... args) {
    return new FunctionCall(new UpvalueExpression(name), args, false);
  }
  
  private boolean isPtnXlua() {
    return function.header.opmap != null && function.header.opmap.get(3) == Op.LOADFIX32;
  }
  ...
  private List<Operation> processLine(State state, int line) {
    ...
    switch(code.op(line)) {
      ...
      case LOADI:
        ...
        break;
      case LOADFIX32:
        operations.add(new RegisterSet(line, A, namedCall("fix32", ConstantExpression.createInteger(code.sBx(line)))));
        break;
      case LOADF:
     ...
     // We can later determine if : syntax was used by comparing subexpressions with ==
      ...
      case BNOT:
        ...
        break;
      case FIX32:
        operations.add(new RegisterSet(line, A, namedCall("fix32", r.getExpression(B, line))));
        break;
      case ASFIX32:
        operations.add(new RegisterSet(line, A, namedCall("asfix32", r.getExpression(B, line))));
        break;
      case CONCAT: {
      ...
      case GETVARG: {
        if(isPtnXlua()) {
          operations.add(new RegisterSet(line, A, namedCall("select", r.getExpression(C, line), new Vararg(1, true))));
        } else {
