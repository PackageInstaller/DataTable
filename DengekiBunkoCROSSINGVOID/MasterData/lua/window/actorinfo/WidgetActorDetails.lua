...
function OnEnable()
  ...
  WU.TriggerUnforcedTutorial(1104)
end
...
function UpdateAttributes()
  REF.LabelAttributes.UIHtmlLabel.text = RenderAttrList(AU.NormalizeAttrs(m_actor.attrs), AU.NormalizeAttrs(m_actor.equipAttrs), AU.NormalizeAttrs(m_actor.weaponAttrs))
end

function RenderAttrList(actorAttrs, equipAttrs, armAttrs)
  ...
  for i = 1, #m_attrIndices do
    ...
    if 0 < attrType then
      ...
      local armAttr = armAttrs[attrType]
      local actorAttrValue = U.FormatAttrValue({
        type = actorAttr.type,
        isRatio = actorAttr.isRatio,
        value = actorAttr.value - equipAttr.value - armAttr.value
      })
      local equipAttrValue = U.FormatAttrValue(equipAttr)
      local armAttrValue = U.FormatAttrValue(armAttr)
      local attrAdd = U.FormatAttrAdd(equipAttr, armAttr)
      ...
      attrString = attrString .. "<c width=10% align=right><i>" .. attrAdd .. "</i></c></font>"
