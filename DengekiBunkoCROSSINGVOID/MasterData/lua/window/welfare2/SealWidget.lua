...
local m_row = 5
local m_col = 5
local m_data, m_allRewardInfo, m_roleReward
...
function InitData()
  ...
  m_activityManager.GetActivityByType(PB.enum.ActivityType.Seal, function(activityList)
    if activityList then
      local strTime = string.format("%s-%s", ACU.RenderTime2(activityList[1].timestampStart, true), ACU.RenderTime2(activityList[1].timestampEnd, true))
      REF.HtmlLabelTime.UIHtmlLabel.text = strTime
    end
  end)
end
