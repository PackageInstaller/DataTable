...
function OnEnable()
  local welfareTab = this:GetData("WelfareTab")
  local welfareConfig = PB.get("WelfareConfig", welfareTab)
  WU.SetGameDataCache("Welfare/" .. welfareTab .. "/firstLogin" .. DB:GetData("playerId"), CS.GameTime.serverUtc)
  RU.SetRedMark("Welfare/" .. welfareTab .. "/firstLogin", false)
  RU.SetRedMark("Welfare/" .. welfareTab .. "/new", false)
  local data = {}
  data.activity = welfareConfig.relatedActivityIds[1]
  data.actorId = math.floor(tonumber(welfareConfig.params[1]))
...
function SetData()
  ...
  if boardActorConfig then
    ...
    REF.TextureDesc.UITexture.width = m_titleSize[m_CurrentActor].x or REF.TextureDesc.UITexture.width
    REF.TextureDesc.UITexture.height = m_titleSize[m_CurrentActor].y or REF.TextureDesc.UITexture.height
