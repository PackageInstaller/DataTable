...
local m_config, m_curGrade
local m_seasonReward = {}
...
function SetConfig(config)
  ...
  LU.Bind(REF.ItemContainer, {
    updateRow = UpdateSeasonRewardNode
  })
  SetSeasonReward()
  SetGradeReward()
...
function SetSeasonReward()
  local seasonreward = PB.all("RacePVPSegReward")
  for i = 1, #seasonreward do
    for j = 1, #seasonreward[i].rewards do
      local reward = {}
      reward.seg = i
      reward.rewards = seasonreward[i].rewards[j]
      table.insert(m_seasonReward, reward)
    end
  end
  LU.Set(REF.ItemContainer, #m_seasonReward)
end
...
function SetGradeReward()
  for i = 1, #REF.Grades do
    ...
    if data ~= nil then
      ...
      item.LabelTitle.UIHtmlLabel.text = WU.GetString("PVPREALRACE_GRADE_" .. data.seg .. "_" .. data.subSeg)
...
function UpdateSeasonRewardNode(rowRef, wrapIndex, rowIndex)
  if rowIndex == nil then
    return
  end
  local trueIndex = rowIndex + 1
  local rewardInfo = m_seasonReward[trueIndex]
  if rewardInfo == nil then
    return
  end
  local reward = rewardInfo.rewards
  rowRef.LabelGradeName.UILabel.text = WU.GetString("PVPREALRACE_GRADE_" .. rewardInfo.seg .. "_1")
  rowRef.WidgetIconSlotTask["$SetData"](reward.type, reward.id, reward.count)
  rowRef.WidgetIconSlotTask["$SetNameVisible"](false)
end
