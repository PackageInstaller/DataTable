...
local m_EffectName = {
  [1502] = "UI_RoleNurturance_K",
  [1514] = "UI_RoleNurturance_H",
  [1511] = nil,
  [1505] = nil
}
local m_EmoteOffset = {
  pos = {
    [1511] = CS.UnityEngine.Vector3(-361, -207, 0),
    [1505] = CS.UnityEngine.Vector3(-366, -162, 0),
    [1401] = CS.UnityEngine.Vector3(-280, -132, 0),
    [1409] = CS.UnityEngine.Vector3(-330, -114, 0),
    [1508] = CS.UnityEngine.Vector3(-347, -181, 0),
    [1515] = CS.UnityEngine.Vector3(-347, -181, 0)
  },
  scale = {
    [1511] = CS.UnityEngine.Vector3(1.16, 1.16, 1),
    [1505] = CS.UnityEngine.Vector3(1.14, 1.14, 1),
    [1508] = CS.UnityEngine.Vector3(1.1, 1.1, 1),
    [1515] = CS.UnityEngine.Vector3(1.1, 1.1, 1)
  }
}
local m_labelColor = {
  [1511] = CS.NGUIMath.HexToColor(1211393791),
  [1505] = CS.NGUIMath.HexToColor(1211393791),
  [1508] = CS.NGUIMath.HexToColor(1211393791),
  [1515] = CS.NGUIMath.HexToColor(757611775)
}
local m_titleSize = {
  [1508] = CS.UnityEngine.Vector2(862, 215),
  [1515] = CS.UnityEngine.Vector2(862, 215)
}
...
function OnEnable()
  local tab = this:GetData("ProgressTask/Tab")
  local config = PB.get("ProgressTaskConfig", tab)
  WU.SetGameDataCache("ProgressTask/" .. tab .. "/firstLogin" .. DB:GetData("playerId"), CS.GameTime.serverUtc)
  RU.SetRedMark("ProgressTask/" .. tab .. "/firstLogin", false)
  RU.SetRedMark("ProgressTask/" .. tab .. "/new", false)
  local data = {}
  data.activity = config.relatedActivityIds[1]
  data.actorId = math.floor(tonumber(config.params[1]))
  this:SetData("ActorCultivation/CurrentInfo", data)
  REF.TextureBG.UITexture.mainTexturePath = "Texture/BackGround/ActorCultivation_bg_" .. data.actorId .. "a"
  ...
  this:RegisterGameEvent("ActivatedVip", OnActivatedVip)
end
...
function OnDisable()
  ...
  this:UnregisterGameEvent("ActivatedVip", OnActivatedVip)
end
...
function OnActorCultivationChange(info)
  ...
  if m_ActorCultivationInfo.endTime ~= 0 then
    local prefabName = "NodeCG_" .. m_CurrentActor
    if reRef(REF.NodeMain)[prefabName] == nil then
      WU.RemoveAllChild(REF.NodeMain)
      local goContent = WU.InstantiateWindowPrefabAttached(REF.NodeMain, "ActorCultivationComplete/" .. prefabName, prefabName)
...
function SetData()
  ...
  if boardActorConfig then
    ...
    REF.EmoteActor.transform.localPosition = m_EmoteOffset.pos[m_CurrentActor] or REF.EmoteActor.transform.localPosition
    REF.EmoteActor.transform.localScale = m_EmoteOffset.scale[m_CurrentActor] or REF.EmoteActor.transform.localScale
    REF.TextureDesc.UITexture.width = m_titleSize[m_CurrentActor] and m_titleSize[m_CurrentActor].x or REF.TextureDesc.UITexture.width
    REF.TextureDesc.UITexture.height = m_titleSize[m_CurrentActor] and m_titleSize[m_CurrentActor].y or REF.TextureDesc.UITexture.height
  ...
  REF.TextureDesc.UITexture.mainTexturePath = "Texture/Welfare/ActorCultivation/ActorCultivation_title_" .. m_CurrentActor .. "a"
  ...
  m_coroutine = this:RepeatInvokeInSeconds(2, function()
    RefreshTimeSpan(endTime)
  end)
  REF.LabelTime2.UILabel.color = m_labelColor[m_CurrentActor] or REF.LabelTime.UILabel.color
  REF.LabelTime.UILabel.color = m_labelColor[m_CurrentActor] or REF.LabelTime.UILabel.color
  local tasks = this:GetData("fci/actorCultivationTasks")
  UpdateTasks(tasks)
  CheckAlternative()
  UpdateProgressReward()
...
function RefreshTimeSpan(endTime)
  local residueTime = endTime - CS.GameTime.serverUtc
  local result = ""
  if residueTime < 0 then
  else
    local timeSpan = CS.System.TimeSpan(0, 0, residueTime)
    if timeSpan.Days ~= 0 then
      result = WU.GetString("Window_Day", timeSpan.Days)
    end
    result = result .. WU.GetString("Window_Hour", timeSpan.Hours)
    if timeSpan.Days == 0 then
      if residueTime < 59 then
        result = result .. WU.GetString("Window_Min", 1)
      else
        result = result .. WU.GetString("Window_Min", timeSpan.Minutes)
      end
    end
    result = WU.GetString("ActorCultivation_ResidueTime") .. result
  end
  REF.LabelTime.UILabel.text = result
end
...
function OnClickSpecialReward()
  local effectName = m_EffectName[m_CurrentActor]
  if effectName then
    ...
    DB:GameRequest("fci/actor-cultivation/progress-reward"):Post({
      ...
      WU.AcquireWindowAsync("ActorCultivationComplete", function(win)
        _ENV["$"](win)["$$SetView"]({
          ...
          atcorID = m_CurrentActor,
          TextureBg = "Texture/BackGround/ActorCultivation_share_" .. m_CurrentActor .. "a"
        })
      end)
      this:SetData("fci/actorCultivationTasks", this:GetData("fci/actorCultivationTasks"))
    end)
  else
    local time = WU.RenderTimeSpan(m_ActorCultivationInfo.endTime - m_ActorCultivationInfo.startTime)
    WU.AcquireWindowAsync("ActorCultivationComplete", function(win)
      _ENV["$"](win)["$$SetView"]({
        time = time,
        effectName = effectName,
...
function OnActivatedVip(data)
  if data and data.info.activityId == m_CurrentActivity then
    local hasExtraRewards = false
    for _, res in pairs(data.resChange) do
      if res.baseRes and res.baseRes.countDelta > 0 then
        hasExtraRewards = true
        break
      end
    end
    if hasExtraRewards then
      WU.ShowRewards(data.resChange, nil, "result_title_09", WU.GetString("ActorCultivation_VipProgressReward"))
    else
