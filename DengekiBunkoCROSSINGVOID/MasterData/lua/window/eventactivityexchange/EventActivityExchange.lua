...
local m_allAwards = {}
local m_cost, m_costCount, m_goods, m_resetPosition, m_shopInfo, m_shopId
...
function InitWindow()
  m_shopId = this:GetData("EventActivityExchange/ShopId")
  ...
  if m_shopInfo then
    for i = 1, #m_shopInfo do
      if m_shopInfo[i].baseInfo.shopId == m_shopId then
        shopInfo = m_shopInfo[i]
        break
      end
    end
    if shopInfo then
      this:BindRemote(DB:GameRequest(string.format("fci/Shop/%s/Goods/", m_shopId)), OnGoodsInfo)
...
function OnGoodsItemShow(refRow, wrapIndex, realIndex)
  ...
  refRow["$$SetData"](wrapIndex, realIndex, m_goods[itemIndex], m_shopId)
end
...
function OnShopPurchaseSuccess(result)
  ...
  DB:SyncRemote(DB:GameRequest(string.format("fci/Shop/%s/Goods/", m_shopId)))
end
