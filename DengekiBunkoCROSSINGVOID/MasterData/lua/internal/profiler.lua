...
local function on_hook(event, func_info_id, source)
  ...
  if not func_info then
    func_info = create_func_info(debug.getinfo(m_funcLayer, "nS"))
...
local function start(func_layer)
  m_funcLayer = func_layer
  func_info_map = {}
  start_time = get_time()
  sethook(on_hook, "cr")
end
...
local function endProfile(reportName)
  if reportName == nil then
    reportName = "lua_profile"
  end
  local str = report()
  stop()
  local file = io.open(reportName, "w")
  file:write(str)
  file:close()
end
