-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/battle/battleflow/component/command/impl/clientonly/BattleAdjustCameraInHotAreaCommand.lua

module("logic.battle.battleflow.component.command.impl.clientonly.BattleAdjustCameraInHotAreaCommand", package.seeall)

local BattleAdjustCameraInHotAreaCommand = class("BattleAdjustCameraInHotAreaCommand", BattleTimingCommandBase)
local kRaycastPosList = {}
local kP1Vector = {}
local kP2Vector = {}

function BattleAdjustCameraInHotAreaCommand:create(fromUnionIndex, toUnionIndex, toUnit, type)
	local cmd = BattleAdjustCameraInHotAreaCommand:createInstance()

	cmd:setFromUnionIndex(fromUnionIndex)
	cmd:setToUnionIndex(toUnionIndex, toUnit)
	cmd:setHotAreaType(type)

	return cmd
end

function BattleAdjustCameraInHotAreaCommand:onInit()
...
function BattleAdjustCameraInHotAreaCommand:onClear()
	self._fromUnionIndex = false
	self._toUnionIndex = false
	self._toUnit = false
	self._kHotAreaWidth = false
...
function BattleAdjustCameraInHotAreaCommand:execute(battleFlow, runningContext)
	...
	local wpos1 = boardMgr:getCellPosition(self._fromUnionIndex)
	local screenPos1 = mainCamera:WorldToScreenPoint(wpos1)
	local wpos2 = boardMgr:getCellPosition(self._toUnionIndex)
...
function BattleAdjustCameraInHotAreaCommand:_handleAsFocusCenter()
	local boardMgr = BattleMgr.instance:getBoardMgr()
...
function BattleAdjustCameraInHotAreaCommand:_doFollow(dx, dz)
	BattleCameraUtil.doFollowPosition(self._curWpos.x + dx, self._curWpos.y, self._curWpos.z + dz, 0.4)
	self:startTimer(0.4)
end

function BattleAdjustCameraInHotAreaCommand:_handleTargetOutside()
	...
	if not nodeWx or not nodeWz then
		if BattleLog.enableWarn then
			BattleLog.warn(string.format("BattleAdjustCameraInHotAreaCommand::cannot raycast go,from:%s,to:%s", self._fromUnionIndex, self._toUnionIndex))
		end
...
function BattleAdjustCameraInHotAreaCommand:_handleStartOutSide()
...
function BattleAdjustCameraInHotAreaCommand:_moveNodeOnLineToHotArea(nodex, nodey)
...
function BattleAdjustCameraInHotAreaCommand:_isNodeInRect(x, y)
	local screenWidth = self._screenWidth
	local screenHeight = self._screenHeight
	local rectx1, rectx2 = self._kHotAreaWidth, screenWidth - self._kHotAreaWidth
	local recty1, recty2 = self._kHotAreaDownHeight, screenHeight - self._kHotAreaUpHeight
...
function BattleAdjustCameraInHotAreaCommand:_getNodeOnRectAndLine(screenPos1, screenPos2)
	local screenWidth = self._screenWidth
	local screenHeight = self._screenHeight
	local rectx1, rectx2 = self._kHotAreaWidth, screenWidth - self._kHotAreaWidth
	local recty1, recty2 = self._kHotAreaDownHeight, screenHeight - self._kHotAreaUpHeight
...
function BattleAdjustCameraInHotAreaCommand:_getWorldPosOnCheckerBoard(screenX, screenY)
	local mainCamera = CameraTargetMgr.instance:getMainCameraTarget():getCamera()
...
function BattleAdjustCameraInHotAreaCommand:_getNeareastNodeOffsetOnRect(nodex, nodey)
	local screenWidth = self._screenWidth
	local screenHeight = self._screenHeight
	local rectx1, rectx2 = self._kHotAreaWidth, screenWidth - self._kHotAreaWidth
	local recty1, recty2 = self._kHotAreaDownHeight, screenHeight - self._kHotAreaUpHeight
...
function BattleAdjustCameraInHotAreaCommand:toString()
	return string.format("[BattleAdjustCameraInHotAreaCommand  fromUnionIndex = %s,toUnionIndex = %s]", self._fromUnionIndex, self._toUnionIndex)
end

function BattleAdjustCameraInHotAreaCommand:setFromUnionIndex(fromUnionIndex)
	self._fromUnionIndex = fromUnionIndex

	return self
end

function BattleAdjustCameraInHotAreaCommand:setToUnionIndex(toUnionIndex, toUnit)
...
function BattleAdjustCameraInHotAreaCommand:setHotAreaType(type)
	type = type or BattleEnum.HotAreaType.Small

	if type == BattleEnum.HotAreaType.Middle then
		self._kHotAreaWidth = 0.282 * UnityEngine.Screen.width
		self._kHotAreaDownHeight = 0.283 * UnityEngine.Screen.height
		self._kHotAreaUpHeight = 0.335 * UnityEngine.Screen.height
	else
		self._kHotAreaWidth = 0.066 * UnityEngine.Screen.width
		self._kHotAreaDownHeight = 0.0677 * UnityEngine.Screen.height
		self._kHotAreaUpHeight = 0.133 * UnityEngine.Screen.height
	end
end

return BattleAdjustCameraInHotAreaCommand
