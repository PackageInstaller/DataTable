-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/ai/impl/condition/AIIsInTargetRoom.lua

module("logic.extensions.ai.impl.condition.AIIsInTargetRoom", package.seeall)

local M = class("AIIsInTargetRoom", AINode)

function M:ctor(context)
	M.super.ctor(self, context)

	self._withTarget = true
end

function M:initFromConfig(aiConfigData)
	M.super.initFromConfig(self, aiConfigData)

	self._withTarget = aiConfigData.withTarget
end

function M:onStart()
	M.super.onStart(self)

	local unit = self:getContext():getUnit()
	local curZoneId = unit.navMesh:getCurZoneId()

	if self._withTarget then
		local targetZoneId = self:getContext():getTargetRoomId()

		if curZoneId == targetZoneId then
			return self:onEnd(AIConst.AIStatus.SUCCESS)
		end
	else
		local zoneType = NavMeshUtil.getZoneTypeByZoneId(curZoneId)

		if zoneType == HouseEnum.ZoneType.Room or zoneType == HouseEnum.ZoneType.Extra then
			return self:onEnd(AIConst.AIStatus.SUCCESS)
		end
	end

	return self:onEnd(AIConst.AIStatus.FAILD)
end

function M:handleEnd()
	return
end

function M:onReset()
	M.super.onReset(self)
end

function M:onReuse(context)
	M.super.onReuse(self, context)
end

function M:onRecycle()
	M.super.onRecycle(self)

	self._withTarget = true
end

function M:onDestroy()
	M.super.onDestroy(self)

	self._withTarget = true
end

return M
