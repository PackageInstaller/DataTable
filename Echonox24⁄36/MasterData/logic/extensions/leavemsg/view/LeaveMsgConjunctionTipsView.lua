-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/leavemsg/view/LeaveMsgConjunctionTipsView.lua

module("logic.extensions.leavemsg.view.LeaveMsgConjunctionTipsView", package.seeall)

local M = class("LeaveMsgConjunctionTipsView", ViewComponent)
...
function M:buildUI()
	...
	self._txtTitle.text = lang("tip_leave_msg_title_joiner")
...
function M:_onClickConfirm()
	...
	info.context:setMsgId(info.paragraphIndex, LeaveMsgEnum.MsgPartType.Joiner, id)
	GlobalDispatcher:dispatchEvent(LeaveMsgEventType.ON_EDIT_MSG_PART_CHANGE, info.paragraphIndex, LeaveMsgEnum.MsgPartType.Joiner, id)
...
function M:refreshPreView()
	...
	self._txtPreview.text = context:getPreviewMsgTmpRelpace(info.paragraphIndex, LeaveMsgEnum.MsgPartType.Joiner, id)
end

function M:refreshView(resetPos)
	self._moLst = AirWorkShopConfig.instance:getConfigList(ConfigName.AirStudioMessageJoiner)
...
function M:_updateCell(go, index)
	...
	theItem.index = index
	theItem.txtContent.text = self._moLst[index].content
