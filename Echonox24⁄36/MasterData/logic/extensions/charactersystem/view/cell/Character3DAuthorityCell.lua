-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/charactersystem/view/cell/Character3DAuthorityCell.lua

module("logic.extensions.charactersystem.view.cell.Character3DAuthorityCell", package.seeall)

local M = class("Character3DAuthorityCell")
local LockAlpha = 0.2
...
function M:_buildUI()
	...
	self._canvasTop = goutil.findChildComponent(self._go, "top", ComponentType.CanvasGroup)
	self._goLock = goutil.findChild(self._go, "lock")
end
...
function M:_refreshView()
	if self._skillCOWrapper then
		...
		self._canvasTop.alpha = self._unlock and 1 or LockAlpha

		goutil.setActive(self._goLock, not self._unlock)
	end
