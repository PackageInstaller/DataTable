-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/battlecalculateeditor/scene/BattleCalculateEditorSceneCamera.lua

module("logic.extensions.battlecalculateeditor.scene.BattleCalculateEditorSceneCamera", package.seeall)

local M = class("BattleCalculateEditorSceneCamera", SceneComponentBase)
...
function M:onEnterScene(sceneId, bornX, bornZ)
	local checkerBoardCO = BattleConfig.instance:getCheckerBoardCO(10001)
...
function M:onReturnScene()
	...
	campOperator:tryOperateNextUnit()
end

function M:_setConfiner(checkerBoardCO)
...
function M:_calculateBoundingBox(checkerBoardCO)
	local minx = INT_MAX
	local minz = INT_MAX
	local maxx = 0
	local maxz = 0
