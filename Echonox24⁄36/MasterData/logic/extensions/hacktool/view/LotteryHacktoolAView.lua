-- chunkname: @/Users/baioo/builds/866EVqtU/3/spacex/spacex-client/UnityProj/Assets/Scripts/Lua/logic/extensions/hacktool/view/LotteryHacktoolAView.lua

module("logic.extensions.hacktool.view.LotteryHacktoolAView", package.seeall)

local M = class("LotteryHacktoolAView")
local SPACE = " "
...
function M:_setEvent(add)
	if add then
		RetrieveDispatcher:addEventListener(RetrieveEventType.onTestLotteryCharacterReply, self._onTestLotteryCardReply, self)
	else
		RetrieveDispatcher:removeEventListener(RetrieveEventType.onTestLotteryCharacterReply, self._onTestLotteryCardReply, self)
	end
end
...
function M:_onClickLottery()
	...
	local data = HackToolUtil.createLotteryCmd1(mallId, time)
...
function M:_onTestLotteryCardReply(e, msg)
	...
	for i, v in ipairs(msg.results) do
		...
		sb:clear()
		sb:append(toLeft(tostring(v.userId), 8))
		sb:append(toLeft(tostring(v.seq), 8))

		if v.ten then
			sb:append(toLeft(lang("tip_ten_card"), 6))
		else
			sb:append(toLeft(lang("tip_one_card"), 6))
		end

		sb:append(toLeft(tostring(v.pool), 13))

		local rate = string.sub(tostring(v.rate * 100), 0, 5)

		sb:append(toLeft(rate .. "%", 8))
		sb:append(toLeft(tostring(v.quality), 8))
		sb:append(toLeft(tostring(v.item), 10))
		sb:append(toLeft(tostring(v.group), 8))
