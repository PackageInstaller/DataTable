...
public class SortedSet<T> : ISet<T>, ICollection<T>, IEnumerable<T>, IEnumerable, ICollection, IReadOnlyCollection<T>, ISerializable, IDeserializationCallback
{
		...
		: this(comparer)
	{
		...
		T[] array = System.Collections.Generic.EnumerableHelpers.ToArray(collection, out var length);
		if (length <= 0)
		{
			return;
		}
	...
	public void SymmetricExceptWith(IEnumerable<T> other)
	{
		...
		T[] array = System.Collections.Generic.EnumerableHelpers.ToArray(other, out var length);
		Array.Sort(array, 0, length, Comparer);
