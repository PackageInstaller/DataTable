...
internal sealed class RegexBoyerMoore
{
	...
	public RegexBoyerMoore(string pattern, bool caseInsensitive, bool rightToLeft, CultureInfo culture)
	{
		if (caseInsensitive)
		{
			StringBuilder stringBuilder = System.Text.StringBuilderCache.Acquire(pattern.Length);
			for (int i = 0; i < pattern.Length; i++)
			{
				stringBuilder.Append(culture.TextInfo.ToLower(pattern[i]));
			}
			pattern = System.Text.StringBuilderCache.GetStringAndRelease(stringBuilder);
		}
		Pattern = pattern;
