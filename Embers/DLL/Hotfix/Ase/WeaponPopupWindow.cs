...
public class WeaponPopupWindow : UGuiWindow
{
	[SerializeField]
	...
	protected override void OnCreate(IBundle bundle)
	{
		...
		_viewModel = GetData<WeaponPopupViewModel>();
		BindingSet<WeaponPopupWindow, WeaponPopupViewModel> bindingSet = this.CreateBindingSet(_viewModel);
		bindingSet.Bind().For((WeaponPopupWindow v) => v.CloseWindow).To((WeaponPopupViewModel vm) => vm.DismissRequest);
		...
		bindingSet.Bind<TextMeshProUGUI>(level).For((Expression<Func<TextMeshProUGUI, string>>)((TextMeshProUGUI v) => ((TMP_Text)v).text)).ToExpression((WeaponPopupViewModel vm) => $"Lv.{vm.WeaponData.Level}<color=#b9aca5>/{vm.WeaponData.MaxLevel}</color>");
		...
		bindingSet.Bind(this).For((WeaponPopupWindow v) => v.RefreshData).To((WeaponPopupViewModel vm) => vm.RefreshDataRequest);
		...
		bindingSet.Build();
		maskRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, _viewModel.Mask2Width);
		lockBtn.onClick.AddListener(_viewModel.Lock);
		lockBtn.gameObject.SetActive(_viewModel.WeaponData.Uid != 0);
		mask1.onClick.AddListener(_viewModel.Close);
		mask2.onClick.AddListener(_viewModel.Close);
		RefreshData();
	}

	public void RefreshData(object sender = null, InteractionEventArgs e = null)
	{
	...
	private void RefreshRoot(WeaponData weaponData)
	{
		if (weaponData != null)
		{
			...
			for (int j = 0; j < _viewModel.WeaponData.EquipAttributeList.Count; j++)
			{
				...
				obj.transform.GetComponent<AttributeItem>().Init(_viewModel.AttributeItemData[j]);
