...
[Name("创建怪物召唤物", 0)]
[Description("根据指定的怪物 ID 和召唤物索引,在场景中创建怪物召唤物，并设置相关属性。")]
[Category("✫ DragonLost/Entity")]
public class CreateMonsterSummonend : ActionTaskBase
{
	[Name("怪物阵营", 0)]
	...
	[Name("召唤物索引", 0)]
	public BBParameter<int> summonendIndex;

	[Name("返回的怪物ID", 0)]
	[Description("生成的实体ID")]
	public BBParameter<int> returnMonsterId;
	...
	protected override void OnExecute()
	{
		...
		{
			OnActionFail("当前实体不是Monster");
			return;
		}
		if (monsterEntity.MonsterAttrConfig == null)
		{
			OnActionFail($"没有找到怪物编号为 {ownerEntity.EntityId} 的怪物配置");
			return;
		}
		List<int> summonMonster = monsterEntity.MonsterAttrConfig.SummonMonster;
		if (summonMonster == null || summonMonster.Count == 0)
		{
			OnActionFail("当前怪物的召唤物列表配置为空");
			return;
		}
		if (summonendIndex.value < 0 || summonendIndex.value >= summonMonster.Count)
		{
			OnActionFail("召唤物索引错误");
			return;
		}
		int num = summonMonster[summonendIndex.value];
		DRMonsterAttr dataRow = GameEntry.DataTable.GetDataRow<DRMonsterAttr>(num);
		if (dataRow == null)
		{
			OnActionFail($"没有找到怪物召唤物 编号为 {num} 的怪物配置");
			return;
		}
		...
		MonsterEntity monsterEntity2 = ownerEntity.GetSystem<EntitySystem>().CreateEntity<MonsterEntity>(dataRow2.Id, new EntityData
		{
		...
		returnMonsterId.value = monsterEntity2.Id;
		if (monsterIds.value != null)
		{
			monsterIds.value.Add(returnMonsterId.value);
		}
		if (GetOwnerEntity().GetWorld().IsStoryWorld)
		{
			...
			{
				data.AddCallMonster(monsterEntity2.Id);
			}
		}
		ownerEntity.GetSystem<BattleStateSystem>()?.CallMonsterHandle(monsterEntity, monsterEntity2);
		OnActionFinish();
	}
}
