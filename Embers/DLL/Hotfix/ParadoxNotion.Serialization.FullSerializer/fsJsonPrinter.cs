...
public static class fsJsonPrinter
{
	...
	public static void ToByte(fsData data, GraphByteData graphByteData)
	{
		CompressedByte(data, graphByteData);
	}

	public static void CompressedByte(fsData data, GraphByteData graphByteData)
	{
		graphByteData.TypeHashEnum.Add((byte)data.Type);
		switch (data.Type)
		{
		case fsDataType.Null:
		{
			graphByteData.DataPosition.Add(graphByteData.ByteGraph.Count);
			byte[] bytes3 = Encoding.UTF8.GetBytes("null");
			graphByteData.DataPosition.Add(bytes3.Length);
			graphByteData.ByteGraph.AddRange(bytes3);
			break;
		}
		case fsDataType.Boolean:
			graphByteData.DataPosition.Add(graphByteData.ByteGraph.Count);
			graphByteData.ByteGraph.AddRange(BitConverter.GetBytes(data.AsBool));
			break;
		case fsDataType.Double:
			graphByteData.DataPosition.Add(graphByteData.ByteGraph.Count);
			graphByteData.ByteGraph.AddRange(BitConverter.GetBytes(data.AsDouble));
			break;
		case fsDataType.Int64:
			graphByteData.DataPosition.Add(graphByteData.ByteGraph.Count);
			graphByteData.ByteGraph.AddRange(BitConverter.GetBytes(data.AsInt64));
			break;
		case fsDataType.String:
		{
			graphByteData.DataPosition.Add(graphByteData.ByteGraph.Count);
			byte[] bytes2 = Encoding.UTF8.GetBytes(data.AsString);
			graphByteData.DataPosition.Add(bytes2.Length);
			graphByteData.ByteGraph.AddRange(bytes2);
			break;
		}
		case fsDataType.Object:
			graphByteData.ShortArrayCount.Add((ushort)data.AsDictionary.Count);
			{
				foreach (KeyValuePair<string, fsData> item in data.AsDictionary)
				{
					graphByteData.DataPosition.Add(graphByteData.ByteGraph.Count);
					byte[] bytes = Encoding.UTF8.GetBytes(item.Key);
					graphByteData.DataPosition.Add(bytes.Length);
					graphByteData.ByteGraph.AddRange(bytes);
					CompressedByte(item.Value, graphByteData);
				}
				break;
			}
		case fsDataType.Array:
			graphByteData.ShortArrayCount.Add((ushort)data.AsList.Count);
			{
				foreach (fsData @as in data.AsList)
				{
					CompressedByte(@as, graphByteData);
				}
				break;
			}
		}
