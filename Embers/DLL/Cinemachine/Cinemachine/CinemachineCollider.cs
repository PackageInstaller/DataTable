...
public class CinemachineCollider : CinemachineExtension
{
	...
	private RaycastHit[] m_CornerBuffer = (RaycastHit[])(object)new RaycastHit[4];

	private const float k_AngleThreshold = 0.1f;

	private static Collider[] s_ColliderBuffer = (Collider[])(object)new Collider[5];
	...
	private Vector3 PreserveLineOfSight(ref CameraState state, ref VcamExtraState extra)
	{
		...
		if (state.HasLookAt && (int)m_CollideAgainst != 0 && (int)m_CollideAgainst != (int)m_TransparentLayers)
		{
			...
			if ((UnityEngine.Object)(object)((RaycastHit)(ref hitInfo)).collider != null)
			{
			...
			else
			{
				...
				if (num2 > 0.0001f)
				{
					if (m_Strategy == ResolutionStrategy.PullCameraForward && m_CameraRadius >= 0.0001f)
					{
						...
						{
							result = ((RaycastHit)(ref hitInfo)).point + ((RaycastHit)(ref hitInfo)).normal * m_CameraRadius - cameraPos;
						}
					}
					else if (RuntimeUtility.RaycastIgnoreTag(ray, out hitInfo, num2, layerMask, in m_IgnoreTag))
					{
						float distance = Mathf.Max(0f, ((RaycastHit)(ref hitInfo)).distance - 0.001f);
	...
	private Vector3 PushCameraBack(Vector3 currentPos, Vector3 pushDir, RaycastHit obstacle, Vector3 lookAtPos, Plane startPlane, float targetDistance, int iterations, ref VcamExtraState extra)
	{
		...
		float num = ClampRayToBounds(ray, pushBackDistance, ((RaycastHit)(ref obstacle)).collider.bounds);
		...
		{
			float distance = ((RaycastHit)(ref hitInfo)).distance - 0.001f;
		...
		if (pushBackDistance > 0.0001f)
		{
			...
			{
				float distance2 = ((RaycastHit)(ref hitInfo)).distance - 0.001f;
	...
	{
		Vector3 normal = ((RaycastHit)(ref obstacle)).normal;
		float num = 0.0050000004f;
		int num2 = Physics.SphereCastNonAlloc(pos, num, pushDir.normalized, m_CornerBuffer, 0f, (int)m_CollideAgainst & ~(int)m_TransparentLayers, (QueryTriggerInteraction)1);
		if (num2 > 1)
		{
			for (int i = 0; i < num2; i++)
			{
				if ((UnityEngine.Object)(object)((RaycastHit)(ref m_CornerBuffer[i])).collider == null || (m_IgnoreTag.Length > 0 && ((Component)(object)((RaycastHit)(ref m_CornerBuffer[i])).collider).CompareTag(m_IgnoreTag)))
				{
					continue;
				}
				Type type = ((object)((RaycastHit)(ref m_CornerBuffer[i])).collider).GetType();
				if (!(type == typeof(BoxCollider)) && !(type == typeof(SphereCollider)) && !(type == typeof(CapsuleCollider)))
				{
					continue;
				}
				Vector3 direction = ((RaycastHit)(ref m_CornerBuffer[i])).collider.ClosestPoint(pos) - pos;
				if (direction.magnitude > 1E-05f && ((RaycastHit)(ref m_CornerBuffer[i])).collider.Raycast(new Ray(pos, direction), ref m_CornerBuffer[i], num))
				{
					if (!(((RaycastHit)(ref m_CornerBuffer[i])).normal - ((RaycastHit)(ref obstacle)).normal).AlmostZero())
					{
						normal = ((RaycastHit)(ref m_CornerBuffer[i])).normal;
					}
					break;
				}
			}
		}
		Vector3 vector = Vector3.Cross(((RaycastHit)(ref obstacle)).normal, normal);
		if (vector.AlmostZero())
		{
			vector = Vector3.ProjectOnPlane(pushDir, ((RaycastHit)(ref obstacle)).normal);
		}
		else
		{
	...
	private Vector3 RespectCameraRadius(Vector3 cameraPos, Vector3 lookAtPos)
	{
		...
		int num = Physics.OverlapSphereNonAlloc(cameraPos, m_CameraRadius, s_ColliderBuffer, (int)m_CollideAgainst, (QueryTriggerInteraction)1);
		RaycastHit hitInfo = default(RaycastHit);
		if (num == 0 && (int)m_TransparentLayers != 0 && magnitude > m_MinimumDistanceFromTarget + 0.0001f)
		{
			...
			{
				Collider collider = ((RaycastHit)(ref hitInfo)).collider;
				if (!collider.Raycast(new Ray(cameraPos, -vector2), ref hitInfo, num2))
				{
		...
		if ((num > 0 && magnitude == 0f) || magnitude > m_MinimumDistanceFromTarget)
		{
			...
			for (int i = 0; i < num; i++)
			{
				Collider val = s_ColliderBuffer[i];
				if (m_IgnoreTag.Length > 0 && ((Component)(object)val).CompareTag(m_IgnoreTag))
				{
					continue;
				}
				if (magnitude > m_MinimumDistanceFromTarget)
				{
					...
					if (magnitude2 > 0.0001f)
					{
						...
						if (val.Raycast(ray, ref hitInfo, magnitude2 + m_CameraRadius))
						{
							vector3 = ray.GetPoint(((RaycastHit)(ref hitInfo)).distance) - vector2 * 0.001f;
						}
					}
				}
				if (Physics.ComputePenetration((Collider)(object)scratchCollider, vector3, Quaternion.identity, val, ((Component)(object)val).transform.position, ((Component)(object)val).transform.rotation, ref vector4, ref num3))
				{
					vector3 += vector4 * num3;
				}
			}
		...
		{
			float num4 = Mathf.Max(m_MinimumDistanceFromTarget, m_CameraRadius) + 0.001f;
			if ((cameraPos + vector - lookAtPos).magnitude < num4)
			{
				vector = lookAtPos - cameraPos + vector2 * num4;
			}
		}
		return vector;
	}
