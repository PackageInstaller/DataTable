...
public class Image : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
{
	...
	protected override void UpdateMaterial()
	{
		...
		if (activeSprite == null)
		{
			base.canvasRenderer.SetAlphaTexture((Texture)null);
			return;
		}
		Texture2D associatedAlphaSplitTexture = activeSprite.associatedAlphaSplitTexture;
		if (associatedAlphaSplitTexture != null)
		{
			base.canvasRenderer.SetAlphaTexture((Texture)associatedAlphaSplitTexture);
		}
	}
	...
	protected override void OnCanvasHierarchyChanged()
	{
		...
		if ((Object)(object)base.canvas == null)
		{
	...
	public virtual bool IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera)
	{
		...
		if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(base.rectTransform, screenPoint, eventCamera, ref local))
		{
			return false;
		}
		Rect pixelAdjustedRect = GetPixelAdjustedRect();
		...
		float u = (textureRect.x + local.x) / (float)activeSprite.texture.width;
		float v = (textureRect.y + local.y) / (float)activeSprite.texture.height;
		try
		{
