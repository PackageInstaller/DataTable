...
public class DataTableConverter : JsonConverter
{
	public override void WriteJson(JsonWriter writer, object? value, JsonSerializer serializer)
	{
		...
		foreach (DataRow item in (InternalDataCollectionBase)obj.Rows)
		{
			writer.WriteStartObject();
			foreach (DataColumn item2 in (InternalDataCollectionBase)item.Table.Columns)
			{
				object obj2 = item[item2];
				if (serializer.NullValueHandling != NullValueHandling.Ignore || (obj2 != null && obj2 != DBNull.Value))
				{
					writer.WritePropertyName((defaultContractResolver != null) ? defaultContractResolver.GetResolvedPropertyName(item2.ColumnName) : item2.ColumnName);
