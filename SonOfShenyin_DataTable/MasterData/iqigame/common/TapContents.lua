-- chunkname: @IQIGame\\Common\\TapContents.lua

TapContents = {}

function TapContents.New(count)
	local o = Clone(TapContents)

	return o
end

function TapContents:Init(root, onToggleChange, repeatedly, updateItem, RepeatedCall, TagDesTable)
	self.tapCount = 0
	self.CurToogle = nil
	self.toggleList = {}
	self.root = root
...
function TapContents:InitFold(root, onToggleChange, repeatedly, updateItem, TagArr, CurTapContents, Part_tagbtnBtn, TagDesTable)
	self.tapCount = 0
	self.CurToogle = nil
	self.toggleList = {}
	self.root = root
	...
	self.tapShow = false
	self.TagArr = TagArr
	self.CurTapContents = CurTapContents
	self.TagDesTable = TagDesTable
...
function TapContents:InitDropdown(root, onToggleChange, DropdownBtn, TargetScale, updateItem)
	self.tapCount = 0
	self.CurToogle = nil
	self.toggleList = {}
...
function TapContents:AddTagDesTable(TagDesTable)
	self.TagDesTable = TagDesTable
end

function TapContents:OnClicktagbtn(_self, go)
	...
	self.CurTapContents.gameObject.transform.localScale = Vector3.New(1, localScale_y, 1)
end

function TapContents:RestTagArr()
	self.tapShow = false

	self:OnClicktagbtn()
end

function TapContents:InitPanel()
	if self.count then
		self:InitItems()
...
function TapContents:InitItems()
...
function TapContents:AddItem(item)
...
function TapContents:DetectionSemblanceTable(name)
...
function TapContents:BindEvent()
...
function TapContents:IsRepeatedlyList(go)
...
function TapContents:OnResetDropdownBtnClick()
	self.DropdownBtnMark = false
...
function TapContents:OnDropdownBtnClick()
...
function TapContents:ToggleSetItemIsOn()
...
function TapContents:ToggleSetOneItemIsOn(i)
	local toggle = self.toggleList[i]
...
function TapContents:ToggleEvent(Index, InitDropdownMark)
	self.semblanceTable = {}
...
function TapContents:ClickEvent()
	self.semblanceTable = {}
	...
	for i = 1, self.tapCount do
		...
		if isOn and self.onToggleChange then
			self.onToggleChange(i, isOn, toggle)
...
function TapContents:UpdateDropdownTagName(toggle_name)
...
function TapContents:SetEnabled(value)
	for i = 1, self.tapCount do
		self.toggleList[i].enabled = value
	end
end

function TapContents:UpdateItemText(textList)
...
function TapContents:OnDestroy()
	return
end
