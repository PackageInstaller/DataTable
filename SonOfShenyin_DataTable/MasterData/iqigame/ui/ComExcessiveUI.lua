...
function ComExcessiveUI:OnOpen(userData)
	if StoryModule.isDebugMod then
		UIModule.Close(Constant.UIControllerName.ComExcessiveUI)
...
function ComExcessiveUI:PlayTween()
	...
	local timer = ModuleTimerUtil.NewTimer(Constant.UIControllerName.ComExcessiveUI, self.onTwAlphaFinishedDelegate, MiscApi:GetComExcessiveFadeDuration())
