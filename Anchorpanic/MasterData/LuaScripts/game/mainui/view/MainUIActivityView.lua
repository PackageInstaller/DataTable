...
module('game.mainui.view.MainUIActivityView', Class.impl("lib.component.BaseContainer"))
...
function checkStateChange(self)
    local list = activity.ActivityManager:getBillboardList()
    for i, billboardVo in ipairs(list) do
        if billboardVo.isOpen ~= (billboardVo:isOpenTime() and activity.ActivityManager:getBillboardIsOpenByFunId(billboardVo.funcOpenId)) then
            self:recover()
            self:updateView()
            break
        end
...
function updateView(self)
    ...
    local page = 1
    for i, billboardVo in ipairs(list) do
        billboardVo.isOpen = (billboardVo:isOpenTime() and activity.ActivityManager:getBillboardIsOpenByFunId(billboardVo.funcOpenId)) -- 缓存自己的开放状态
