--[[ 
-----------------------------------------------------
@filename       : FormationMazeHeroSelectPanel
@Description    : 迷宫·副本选择战员
@date           : 2021-05-18 14:22:12
@Author         : Zzz
@copyright      : (LY) 2021 雷焰网络
-----------------------------------------------------
]]
module('formation.FormationMazeHeroSelectPanel', Class.impl('game/formation/normal/view/FormationHeroSelectPanel'))

--对应的ui文件
UIRes = UrlManager:getUIPrefabPath("formation/FormationMazeHeroSelectPanel.prefab")

function __getHeadSelectItem(self)
    return formation.FormationMazeHeroSelectItem
end
...
function getDataList(self)
    ...
    for i = 1, #heroList do
        ...
        if(isShowRemove)then
            ...
            local mazeHeroVo = self:getMazeHeroHp(scrollVo)
            if (mazeHeroVo.nowHp <= 0) then
                table.insert(dieList, scrollVo)
            else
    ...
    if(monsterUniqueTidList)then
        for i = #monsterUniqueTidList, 1, -1 do
            ...
            local isShowRemove = false
            if(self.tileFormationHeroVo 
            and self.tileFormationHeroVo.sourceType == formation.HERO_SOURCE_TYPE.MAZE_SUPPORT 
    ...
    for i = 1, #dieList do
        table.insert(scrollList, dieList[i])
    end

    return scrollList
end

function getMazeHeroHp(self, scrollVo)
    local manager = scrollVo:getDataVo().manager
    local dataVo = scrollVo:getDataVo().dataVo
