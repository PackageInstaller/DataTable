-- @FileName:   FieldExplorationDupItem_212.lua
-- @Description:   文件描述
-- @Author: ZDH
-- @Date:   2023-08-23 17:20:27
-- @Copyright:   (LY) 2023 雷焰网络

module("game.fieldExploration.view.FieldExplorationDupItem_212", Class.impl(SimpleInsItem))
...
function setData(self, dup_id, lastDup_id)
    ...
    if self.isCanPlay then
        ...
        for k, v in pairs(self.mDupConfigVo.star_list) do
            maxStarCount = maxStarCount + 1
        end
        ...
        local isMaxStar = starCount >= maxStarCount

        local isPass = fieldExploration.FieldExplorationManager:getPlayerDupRecord(dup_id) ~= 0
        self:getChildGO("mImgBg_Normal_02"):SetActive(isPass and not isMaxStar)

        self:getChildGO("mMaxStar"):SetActive(isMaxStar)
    else
        local isOpen = true
        ...
        if not isOpen then
            ...
            self:getChildGO("mTextOpenTime"):GetComponent(ty.Text).text = ""
        end

        self:getChildGO("mImgBg_Normal_02"):SetActive(false)
        self:getChildGO("mMaxStar"):SetActive(false)
    end

    self:getChildGO("mRedImg"):SetActive(self.isCanPlay and not StorageUtil:getBool1(FieldExplorationConst.DupNewOpenStr .. dup_id))

    self:getChildGO("mImgNormal"):SetActive(self.isCanPlay)
    self:getChildGO("mLock"):SetActive(not self.isCanPlay)
    self:getChildGO("mGroupStar"):SetActive(self.isCanPlay)
