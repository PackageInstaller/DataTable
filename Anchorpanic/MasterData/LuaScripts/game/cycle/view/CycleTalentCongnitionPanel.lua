module("cycle.CycleTalentCongnitionPanel", Class.impl(View))
-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("cycle/CycleTalentCongnition.prefab")
...
function updateView(self)
    self:recoverTalentBar()
    local talentNum = cycle.CycleTalentManager:getUnlockNum()
    self.mTxtProgress.text = talentNum .. "/" .. 40
    if talentNum > 0 then 
        -- 三属性
        for k,v in pairs(cycle.CycleTalentManager:getTalentAttr()) do
