module("fashionPermit.FashionPermitBuyPanel", Class.impl(View))

UIRes = UrlManager:getUIPrefabPath("fashionPermit/FashionPermitBuyPanel.prefab")
...
function configUI(self)
    ...
    self.mNumberStepper:Init(1, 1, 1, (#fashionPermit.FashionPermitManager:getFashionPermitList() - fashionPermit.FashionPermitManager:getFashionPermitedLv()), self.onStepChange, self)
end
...
function active(self, args)
    ...
    GameDispatcher:addEventListener(EventName.UPDATE_FASHION_PERMIT_PANEL, self.onCloseView, self)
...
function deActive(self)
    super.deActive(self)
    GameDispatcher:removeEventListener(EventName.UPDATE_FASHION_PERMIT_PANEL, self.onCloseView, self)
...
function updateView(self)
    self.mCurLv = fashionPermit.FashionPermitManager:getFashionPermitedLv()
    self.mTxtCurLv.text = self.mCurLv
 
    self:updatePrice()
    self.mNumberStepper.MaxCount = #fashionPermit.FashionPermitManager:getFashionPermitList() - self.mCurLv
end
...
function onStepChange(self, cusCount, cusType)
    if cusType == 1 then
        -- '最大值'
        if self.mNumberStepper.MaxCount >= (#fashionPermit.FashionPermitManager:getFashionPermitList() - self.mCurLv) then
...
function onClickHandler(self)
    ...
    GameDispatcher:dispatchEvent(EventName.REQ_FASHION_PERMIT_BUY_LV, { buyLv = self.mNumberStepper.CurrCount })
end

function updatePrice(self)
    self:closeAllProps()
    self.mNeedMoney = self.mNumberStepper.CurrCount * sysParam.SysParamManager:getValue(SysParamType.FASHIONPERMIT_UP_NEED_DIAMOND)
    ...
    local list = fashionPermit.FashionPermitManager:getFashionPermitLvStageAwardList(needBuyLv)
