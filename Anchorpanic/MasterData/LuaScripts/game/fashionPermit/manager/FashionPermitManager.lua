module("fashionPermit.FashionPermitManager", Class.impl(Manager))
...
function setFashionPemitInfo(self, msg)
    ...
    self:updateBubble()
    GameDispatcher:dispatchEvent(EventName.UPDATE_FASHION_PERMIT_PANEL)
    GameDispatcher:dispatchEvent(EventName.UPDATE_FASHIONPERMIT_TASK_PANEL)
end
...
function parseFashionPermitData(self)
    ...
    local baseData = RefMgr:getData("fashion_permit_data")
    for key, data in pairs(baseData) do
        local vo = fashionPermit.FashionPermitVo.new()
...
function parseFashionPermitTaskData(self)
    ...
    local baseData = RefMgr:getData("fashion_task_data")
    for id, data in pairs(baseData) do
        local vo = fashionPermit.FashionPermitTaskVo.new()
...
function updateBubble(self)
    local isOpen = funcopen.FuncOpenManager:isOpen(funcopen.FuncOpenConst.FUNC_ID_FASHIONPERMIT)
    if not isOpen then
        return false
    end

    -- if not activity.ActivityManager:checkIsOpenByFuncId(funcopen.FuncOpenConst.FUNC_ID_FASHIONPERMIT) then
    --     return false
    -- end
