...
module("noviceActivity.NoviceActivityAccRechargeItem", Class.impl('lib.component.BaseItemRender'))
--构造函数
function ctor(self)
    super.ctor(self)
    self.mAwardItemList = {}
...
function updateDataView(self)
    self:recoverAllGrid()
    local curNum=noviceActivity.NoviceActivityManager:getRechargeNum()<self.data:getMaxNum() and noviceActivity.NoviceActivityManager:getRechargeNum() or self.data:getMaxNum()
...
function onClickGetHandler(self)
    GameDispatcher:dispatchEvent(EventName.REQ_NOVICE_ACTIVITY_RECHARGE_RECEIVE, self.data.index) 
end
