--[[ 
-----------------------------------------------------
@filename       : RogueLikeBuffSelectItem
@Description    : 迷宫通关物品选择item
@Author         : sxt
@copyright      : (LY) 2021 雷焰网络
-----------------------------------------------------
]] 
module("rogueLike.RogueLikeBuffSelectItem", Class.impl(BaseReuseItem))

-- 对应的ui文件
UIRes = UrlManager:getUIPrefabPath("rogueLike/RogueLikeBuffSelectItem.prefab")
...
function onClick(self)
    ...
    if self.isSelect then
        GameDispatcher:dispatchEvent(EventName.REQ_ROGUE_SELECT_MAP_BUFF, {cellId = self.mCellId, buffId = self.mBuffId})
    end
end
