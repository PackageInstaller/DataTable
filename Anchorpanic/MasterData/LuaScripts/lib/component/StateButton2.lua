module("StateButton2", Class.impl())
...
function initData(self)
    self.m_prefabName = UrlManager:getUIPrefabPath("compent/StateButton2.prefab")
    self.mTxtName = "m_textContent"
    self.m_textNameEn = "m_textContentEn"
...
function setData(self, cusPosX, cusPosY, cusWidth, cusHeight, normalAssetName, selectAssetName, cusContent, cusFontSize, cusIsBlod, cusParamData, fontNormalColor, fontSelectColor, fontOffsetNomal, fontOffsetSelect, fontLineSpace, cusTextAlignment)
    ...
    local content = nil
    local contentEn = nil
    if type(cusContent) == "table" then
        content = cusContent[1]
        contentEn = cusContent[2]
    else
        content = cusContent
    end
    if (content ~= nil) then
        self:setText(content, cusFontSize, cusIsBlod, fontLineSpace, cusTextAlignment)
    end
    if contentEn ~= nil then
        self:setTextEn(contentEn, 10, nil, fontLineSpace)
    end
...
function setTextEn(self, cusContent, cusFontSize, cusIsBlod, cusLineSpace, cusAlignment)
    local textScriptEn = self.m_childTrans[self.m_textNameEn]:GetComponent(ty.Text)
    textScriptEn.text = cusContent
    if (cusFontSize == nil) then
        textScriptEn.fontSize = 24
    else
        textScriptEn.fontSize = cusFontSize
    end
    if (cusIsBlod == nil or cusIsBlod == false) then
        textScriptEn.fontStyle = gs.FontStyle.Normal
    else
        textScriptEn.fontStyle = gs.FontStyle.Bold
    end
    if cusLineSpace then
        textScriptEn.lineSpacing = cusLineSpace
    end
	if cusAlignment then
        textScript.alignment = cusAlignment
	end
...
--     if offset[5] then
--         local rectEn = self.m_childTrans[self.m_textNameEn]:GetComponent(ty.RectTransform)
--         gs.TransQuick:Offset(rectEn, offset[5], offset[6], -offset[7], -offset[8])
--     end
-- end
    ...
    if (imgSelectGo.activeSelf == cusIsSelect) then
        ...
        if (cusIsSelect) then
            ...
            if (self.m_fontSelectColor) then
                ...
                textScriptEn.color = gs.ColorUtil.GetColor(self.m_fontSelectColor)
            end
            ...
            if (self.m_fontNormalColor) then
                ...
                textScriptEn.color = gs.ColorUtil.GetColor(self.m_fontNormalColor)
            end
            --self:setFontOffset(self.m_fontOffsetNomal)
        end
    end
