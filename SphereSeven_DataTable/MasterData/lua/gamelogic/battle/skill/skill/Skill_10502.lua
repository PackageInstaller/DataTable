---
--- 对一条直线的敌人造成${Params[1]}%物理攻击力的伤害,有${Params[2]}%的概赋予目标${Params[3]}回合的【攻击力下降】。对攻击力低于自己的目标获得额外${Params[4]}%的倍率
---

local Skill_10502 = BaseClass("Skill_10502",Skill)
local base = Skill

local function __init(self)
end

local function onParamsInited(self)
    base.onParamsInited(self)

    self.Data.DamageScale = self.Params[1] / 100
    self.Data.Probably = self.Params[2]
    self.Data.DebuffTurn = self.Params[3]
    self.Data.ExtraDamageScale = self.Params[4] / 100
end

local function OnUseActiveSkill(self, targetUnit)
    local result = ActiveSkillResult.New()
    local atk = self.__snc:Get(NumericType.Atk)

    local damageScale = self.Data.DamageScale
    local targetAtk = targetUnit:GetComponent("NumericComponent"):Get(NumericType.Atk)
    local selfAtk = self.__snc:Get(NumericType.Atk)
    if targetAtk < selfAtk then
        damageScale = damageScale + self.Data.ExtraDamageScale
    end

    local damageNum = atk * damageScale / self.MultistageNum
    result.FeixiaoDamage = {
        Type = BattleDamageType.Physical,
        Value = damageNum
    }

    local probably = self.Data.Probably
    if self.__sbc:HasBuff() then
        probably = probably * 2
    end

    if BattleUtil.IsPctProbHit(probably) then
        table.insert(result.Buffs, ActiveSkillBuff.New(BattleBuffType.AtkDecrease, self.Data.DebuffTurn))
    end

    return result
end

Skill_10502.__init = __init
Skill_10502.onParamsInited = onParamsInited
Skill_10502.OnUseActiveSkill = OnUseActiveSkill

return Skill_10502