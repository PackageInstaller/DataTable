...
local function AddPreloadResource(self, path, res_type, inst_count)
	...
	if res_type == typeof(GameObject) then
...
--  预加载ab包
local function AddPreloadAssetBundle(self, path)
	assert(type(path) == "string" and #path > 0)
	table.insert(self.preload_ab, path)
end
...
local function CoOnPrepare(self)
	...
	local ab_count = table.count(self.preload_ab)
	...
	for k, v in pairs(self.preload_ab) do
		ResourcesManager:GetInstance():CoLoadAssetBundleAsync(v, ProgressCallback)
...
BaseScene.AddPreloadAssetBundle = AddPreloadAssetBundle 
