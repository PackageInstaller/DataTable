...
local function GetOrder(self)
	if IsNull(self.unity_canvas) then
		Logger.LogError("self.unity_canvas is null")
		return 0
	end
	return self.unity_canvas.sortingOrder
end
...
local function MoveOrderOneUnit(self)
	self.unity_canvas.sortingOrder = self.unity_canvas.sortingOrder - UIManager:GetInstance().MaxOderPerWindow
end
...
UICanvas.MoveOrderOneUnit = MoveOrderOneUnit
