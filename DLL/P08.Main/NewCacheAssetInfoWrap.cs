...
public class NewCacheAssetInfoWrap
{
	public static void Register(LuaState L)
	{
		L.BeginClass(typeof(NewCacheAssetInfo), typeof(object));
		L.RegFunction("New", _CreateNewCacheAssetInfo);
	...
	private static int _CreateNewCacheAssetInfo(IntPtr L)
	{
		try
		{
			if (LuaDLL.lua_gettop(L) == 0)
			{
				NewCacheAssetInfo o = new NewCacheAssetInfo();
				ToLua.PushObject(L, o);
				return 1;
			}
			return LuaDLL.luaL_throw(L, "invalid arguments to ctor method: NewCacheAssetInfo.New");
		}
		catch (Exception e)
		{
	...
	private static int get_assetPath(IntPtr L)
	{
		...
		try
		{
			...
			string assetPath = ((NewCacheAssetInfo)obj).assetPath;
	...
	private static int get_num(IntPtr L)
	{
		...
		{
			obj = ToLua.ToObject(L, 1);
			int num = ((NewCacheAssetInfo)obj).num;
	...
	private static int set_assetPath(IntPtr L)
	{
		...
		{
			obj = ToLua.ToObject(L, 1);
			NewCacheAssetInfo obj2 = (NewCacheAssetInfo)obj;
