...
public class XmlNodeConverter : JsonConverter
{
	...
	private string GetPropertyName(IXmlNode node, XmlNamespaceManager manager)
	{
		...
		case XmlNodeType.Whitespace:
			return "#whitespace";
		default:
			throw new JsonSerializationException("Unexpected XmlNodeType when getting node name: " + ((object)node.NodeType/*cast due to constrained. prefix*/).ToString());
		}
	}
	...
	private void SerializeNode(JsonWriter writer, IXmlNode node, XmlNamespaceManager manager, bool writePropertyName)
	{
		...
		default:
			throw new JsonSerializationException("Unexpected XmlNodeType when serializing nodes: " + ((object)node.NodeType/*cast due to constrained. prefix*/).ToString());
		}
	}
	...
	private static string? ConvertTokenToXmlValue(JsonReader reader)
	{
		...
		case JsonToken.Integer:
			...
			{
				return bigInteger.ToString((IFormatProvider)CultureInfo.InvariantCulture);
			}
