...
public class JsonTextReader : JsonReader, IJsonLineInfo
{
	...
	private void ParseReadString(char quote, ReadType readType)
	{
		...
		{
			byte[] value2 = ((_stringReference.Length == 0) ? CollectionUtils.ArrayEmpty<byte>() : ((_stringReference.Length != 36 || !ConvertUtils.TryConvertGuid(_stringReference.ToString(), out var g)) ? Convert.FromBase64CharArray(_stringReference.Chars, _stringReference.StartIndex, _stringReference.Length) : g.ToByteArray()));
	...
	private void ParseReadNumber(ReadType readType, char firstChar, int initialPosition)
	{
		...
		case ReadType.ReadAsInt64:
		{
			...
			default:
				...
				{
					ParseResult parseResult = ConvertUtils.DecimalTryParse(_stringReference.Chars, _stringReference.StartIndex, _stringReference.Length, out var value3);
	...
	{
		return BigInteger.Parse(number, (IFormatProvider)culture);
	}
