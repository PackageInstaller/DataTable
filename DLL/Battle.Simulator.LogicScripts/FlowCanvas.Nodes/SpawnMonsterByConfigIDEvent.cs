...
[Name("绘制实体事件(实体类型ID)", 0)]
[Category("Common/事件")]
[Description("创建实体事件 包含该逻辑创建和渲染创建")]
public class SpawnMonsterByConfigIDEvent : EventNode
{
	...
	protected override void RegisterPorts()
	{
		...
		AddValueOutput("皮肤ID", "skinID", () => mSkinID);
		_isRenderInput = AddValueInput<bool>("是否渲染");
		_roleID = AddValueInput<int>("实体类型ID", "角色ID");
	}
	...
	private void SetData(EntitySpawnedEvent triggerEvent)
	{
		...
		mSkinID = triggerEvent.mSkinID;
	}

	private void OnSpawnUnitEventHandler(EntitySpawnedEvent triggerEvent)
	{
		if (triggerEvent.mSpawnType == E_SpawnType.ROLE && _roleID.GetValue() == triggerEvent.mConfigId)
		{
