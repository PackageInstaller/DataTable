...
public class INeedSpawnThrown3AbilitySuffix : FlowControlNode
{
	...
	protected override void RegisterPorts()
	{
		...
		AddFlowInput("In", "In", delegate(Flow f)
		{
			...
			{
				int thrownTimelineID = (entityWithEntityID.entityConfig.mConfig as RoleConfig).ID * 1000 + m_thrownTimelineIDVar.value;
				...
				{
					m_thrownID = ThrownProcessor.SpawnThrown(base.mSimContext, m_casterIDVar.GetValue(), 0, 0, m_position, m_forward, thrownTimelineID, m_useCommonVar.GetValue(), m_producerIDVar.GetValue(), m_creatdRootTimelineIDVar.GetValue(), m_attributeID, out var _);
					m_output.Call(f);
				}
			}
		});
