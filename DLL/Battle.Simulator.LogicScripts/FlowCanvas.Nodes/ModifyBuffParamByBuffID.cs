...
[Name("修改Buff参数(创建ID)(逻辑)", 0)]
[Category("Logic/Buff")]
[Description("修改buff参数(ID), 下标从0开始, 乘法操作的参数是百分比")]
public class ModifyBuffParamByBuffID : FlowNode
{
	private ValueInput<int> m_buffID;
	...
	protected override void RegisterPorts()
	{
		...
		m_buffID = AddValueInput<int>("创建ID", "BuffID");
		...
		AddFlowInput("In", "input", delegate(Flow f)
		{
			...
			{
				int num = -1;
				int value = m_buffID.GetValue();
				for (int i = 0; i < entityWithEntityID.entityBuff.mBuffID.Count; i++)
				{
					if (entityWithEntityID.entityBuff.mBuffID[i] == value && !entityWithEntityID.entityBuff.IsInPendingRemoveArray(entityWithEntityID.entityBuff.mBuffID[i]))
					{
						num = i;
						break;
					}
