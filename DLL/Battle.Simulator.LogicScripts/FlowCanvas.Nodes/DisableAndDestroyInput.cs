...
[Description("将本次输入无效并可选销毁,默认销毁;\n可用于接在输入【逻辑输入事件】后面进行处理;\n如果找不到实体或者实体无输入,则走error流")]
public class DisableAndDestroyInput : FlowControlNode
{
	...
	protected override void RegisterPorts()
	{
		...
		AddFlowInput("In", "input", delegate(Flow f)
		{
			...
			{
				entityWithEntityID.entityActionManualInput.mStatus = EntityActionStatus.E_FAILED;
