...
[Name("是否有Buff(子类型)(全部)(逻辑)", 0)]
[Category("Logic/Buff/通用")]
[Description("只要实体身上的BUFF，只有当buff的子类型与传入数组全部匹配才走true,任意一个不匹配走false")]
public class HasTargetAllSubTypeBuffArr : FlowNode
{
				...
				if (entityWithEntityID.hasEntityBuff)
				{
					...
					for (int j = 0; j < entityWithEntityID.entityBuff.mBuffConfigID.Count; j++)
					{
						...
						if (num != 0)
						{
							...
							if (!entityWithEntityID.entityBuff.IsInPendingRemoveArray(entityWithEntityID.entityBuff.mBuffID[j]) && BuffSubTypeComparer.ContainsAll(subTypeHashSet, config.SubType))
							{
								trueOut.Call(f);
								return;
							}
