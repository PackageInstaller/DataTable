...
public class GuildActivitySPWarFieldBigBossNodeUI : MonoBehaviour
{
	...
	public void UpdateView()
	{
		...
		if (GuildActivitySPManager.GetInstance().luaData.bigBossUnlockMapList.IndexOf(_nodeData.map_id) < 0)
		{
			_stateController.SetSelectedState("lock");
		}
		else if (_data.health <= 0)
		{
			_arrivableController.SetSelectedState("false");
			if (GuildActivitySPManager.GetInstance().luaData.maxNodeId == _nodeData.id)
			{
				_stateController.SetSelectedState("attacking_2");
			}
			else
			{
		...
		else if (_data.health < _data.boss_score)
		{
			if (GuildActivitySPManager.GetInstance().IsNodeCanReach(_nodeData.id))
			{
				_stateController.SetSelectedState("attacking_3");
			}
			else
			{
		...
		else if (GuildActivitySPManager.GetInstance().IsNodeCanReach(_nodeData.id))
		{
			_stateController.SetSelectedState("attacking_2");
		}
		else
