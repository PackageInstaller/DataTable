...
public class BuffConfitionHitSiteLevelEvent : BuffConditionBase
{
	...
	public override BuffConditionType BuffConditionType => BuffConditionType.HitSiteLevelEvent;
	...
	protected override void OnMessage(IObserverParams param)
	{
		...
		if (observerDamageProgressParams.Attacker != null && observerDamageProgressParams.Attacker.Id == entityTarget.Id && observerDamageProgressParams.Defender != null)
		{
			BuffExtension.GetTargetByTeamType(entityFrom, entityTarget, teamType, observerDamageProgressParams.Defender);
			if (observerDamageProgressParams.ContainsKey("SiteLevel"))
			{
				int v = observerDamageProgressParams.Get<int>("SiteLevel");
