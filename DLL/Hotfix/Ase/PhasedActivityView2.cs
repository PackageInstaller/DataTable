...
public class PhasedActivityView2 : ActivityViewBase
{
	[SerializeField]
	...
	[SerializeField]
	private ActivityRewardSliderView activityRewardSliderView;
	...
	public override void Init(ActivityViewModelBase viewModel)
	{
		...
		base.Init(viewModel);
		BindingSet<PhasedActivityView2, PhasedActivityViewModel> bindingSet = this.CreateBindingSet(this.viewModel);
		bindingSet.Bind(this).For((PhasedActivityView2 v) => v.RefreshTaskList).To((PhasedActivityViewModel vm) => vm.RefreshActivityTaskRequest);
		bindingSet.Build();
		...
		activityRewardSliderView.Init(this.viewModel.ActivityRewardSliderViewModel);
	}
