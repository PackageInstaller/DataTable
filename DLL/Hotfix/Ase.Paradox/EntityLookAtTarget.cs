...
[Name("朝向目标或坐标", 0)]
[Description("使实体朝向指定目标或坐标。")]
[Category("✫ DragonLost/Transform")]
public class EntityLookAtTarget : ActionTaskBase
{
	[Name("最大旋转时间", 0)]
	...
	protected override void OnExecute()
	{
		...
		if (IsImmediatelyRotate)
		{
			if (lookAtEntity)
			{
				...
				rotateComponent.ImmediatelyRotate(entity.transform.position);
			...
			{
				rotateComponent.ImmediatelyRotate(targetPosition.value);
				if (LockstepData.Instance != null)
				{
		...
		if (lookAtEntity)
		{
			...
			rotateComponent.SetTarget(entity2, duration.value, finishImmediatelyEnd);
			if (LockstepData.Instance != null)
			{
		...
		{
			rotateComponent.SetTarget(targetPosition.value, duration.value, finishImmediatelyEnd);
			if (LockstepData.Instance != null)
			{
