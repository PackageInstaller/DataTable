...
public class CurveMoveAndRotate : ActionTaskBase
{
	[Name("旋转类型", 0)]
	...
	protected override void OnExecute()
	{
		...
		AnimationCurve rotW = ((rotateType == RotateType.Quaternion) ? curveRotW : null);
		Vector2 vector = new Vector2(multiple_X.value, multiple.value);
		if (useMovement && useRotation)
		{
			component.CurveMove(GetTimeScaleCurve(curveX, timeScale_x.value), GetTimeScaleCurve(curveZ, timeScale_z.value), curveRotX, curveRotY, curveRotZ, rotW, moveTime.value, MoveTick.value, vector, moveDirByRotate: false, isSkillMoveCancel, isSkillMoveCancelXMove, isSkillMoveCancelZMove, stopAngle, checkDistance.value, CheckRadius, IsFrozenMovementCommand, TimeType, IsOpenInterpolation, IsOpenAnimator, colliderPauseMove: false);
		}
		else if (useMovement)
		{
			component.CurveMove(GetTimeScaleCurve(curveX, timeScale_x.value), GetTimeScaleCurve(curveZ, timeScale_z.value), moveTime.value, MoveTick.value, vector, !useRotation, isSkillMoveCancel, isSkillMoveCancelXMove, isSkillMoveCancelZMove, stopAngle, checkDistance.value, CheckRadius, IsFrozenMovementCommand, TimeType, IsOpenInterpolation, IsOpenAnimator, colliderPauseMove: false);
		}
		else
		{
			component.CurveMove(curveRotX, curveRotY, curveRotZ, rotW, moveTime.value, MoveTick.value, vector, isSkillMoveCancel, isSkillMoveCancelXMove, isSkillMoveCancelZMove, stopAngle, checkDistance.value, CheckRadius, IsFrozenMovementCommand, TimeType, IsOpenInterpolation, IsOpenAnimator, colliderPauseMove: false);
		}
		if (!waitUntilFinish)
		{
			OnActionFinish();
