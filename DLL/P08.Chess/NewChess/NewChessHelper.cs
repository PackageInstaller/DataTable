...
public class NewChessHelper
{
	...
	public static List<GameObject> m_FxPathList = new List<GameObject>();

	public static int AstarPathHelpCount = 2;

	public static float m_layHeight = 0.16f;
	...
	public static void SetGridSelectOutline(Vector3 position)
	{
		GameObject gameObject = Asset.Instantiate("NewWarChess/fx_pick_hexagon");
		gameObject.transform.position = position;
		m_FxPathList.Add(gameObject);
	}

	public static void ClearGridSelectOutline(Vector3 position)
	{
		foreach (GameObject fxPath in m_FxPathList)
		{
			if (fxPath.transform.position == position)
			{
				m_FxPathList.Remove(fxPath);
				PooledAsset.DestroyOrReturn(fxPath);
				break;
			}
		}
	}
	...
	public static float CheckFloatPrecision(float f)
	{
