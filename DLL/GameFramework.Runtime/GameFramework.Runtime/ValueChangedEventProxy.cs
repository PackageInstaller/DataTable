...
public class ValueChangedEventProxy<T> : EventTargetProxyBase
{
	...
	{
		INotifyValueChangedExtensions.RegisterValueChangedCallback<T>(target, (EventCallback<ChangeEvent<T>>)(object)new EventCallback<ChangeEvent<ChangeEvent<T>>>(OnValueChangedEvent));
	}

	protected virtual void UnbindEvent()
	{
		INotifyValueChangedExtensions.UnregisterValueChangedCallback<T>(target, (EventCallback<ChangeEvent<T>>)(object)new EventCallback<ChangeEvent<ChangeEvent<T>>>(OnValueChangedEvent));
	}
