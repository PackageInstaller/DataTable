...
public static class BehaviourBindingExtension
{
	...
	public static IBindingContext BindingContext(this Behaviour behaviour)
	{
		...
		{
			bindingContext = (bindingContextLifecycle.BindingContext = new BindingContext(behaviour, Binder));
		}
	...
	{
		IBindingContext bindingContext = behaviour.BindingContext();
		bindingContext.DataContext = dataContext;
		return new BindingSet<TBehaviour, TSource>(bindingContext, behaviour);
	}
	...
	public static void AddBindings(this Behaviour behaviour, IDictionary<object, IEnumerable<BindingDescription>> bindingMap, object key = null)
	{
		...
		IBindingContext bindingContext = behaviour.BindingContext();
		foreach (KeyValuePair<object, IEnumerable<BindingDescription>> item in bindingMap)
		{
			bindingContext.Add(item.Key, item.Value, key);
		}
	}
