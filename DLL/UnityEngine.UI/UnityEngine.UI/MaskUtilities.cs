...
public class MaskUtilities
{
	...
	public static Transform FindRootSortOverrideCanvas(Transform start)
	{
		...
		return ((Component)(object)val).transform;
	}
	...
	public static RectMask2D GetRectMaskForClippable(IClippable clippable)
	{
		...
		if (list.Count > 0)
		{
			for (int i = 0; i < list.Count; i++)
			{
				...
				{
					if (!IsDescendantOrSelf(((Component)(object)list2[num]).transform, rectMask2D.transform) && list2[num].overrideSorting)
					{
						rectMask2D = null;
						break;
					}
		...
		if (list2.Count > 0)
		{
			...
			for (int num = list2.Count - 1; num >= 0; num--)
			{
				if (list2[num].IsActive())
				{
					...
					{
						if (!IsDescendantOrSelf(((Component)(object)list[num2]).transform, list2[num].transform) && list[num2].overrideSorting)
						{
							flag = false;
							break;
						}
