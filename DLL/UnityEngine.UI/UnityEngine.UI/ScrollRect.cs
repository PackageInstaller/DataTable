...
public class ScrollRect : UIBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup, ILayoutController
{
	...
	public virtual void OnDrag(PointerEventData eventData)
	{
		...
		if (!m_Dragging || eventData.button != PointerEventData.InputButton.Left || !IsActive() || !RectTransformUtility.ScreenPointToLocalPointInRectangle(viewRect, eventData.position, eventData.pressEventCamera, ref vector))
		{
			return;
		}
		UpdateBounds();
		Vector2 vector2 = vector - m_PointerStartLocalCursor;
		Vector2 vector3 = m_ContentStartPosition + vector2;
		Vector2 vector4 = CalculateOffset(vector3 - m_Content.anchoredPosition);
		vector3 += vector4;
		if (m_MovementType == MovementType.Elastic)
		{
			if (vector4.x != 0f)
			{
				vector3.x -= RubberDelta(vector4.x, m_ViewBounds.size.x);
			}
			if (vector4.y != 0f)
			{
				vector3.y -= RubberDelta(vector4.y, m_ViewBounds.size.y);
			}
		}
		SetContentAnchoredPosition(vector3);
	}
	...
	protected virtual void LateUpdate()
	{
		...
		if (m_ViewBounds != m_PrevViewBounds || m_ContentBounds != m_PrevContentBounds || m_Content.anchoredPosition != m_PrevPosition)
		{
			...
			UISystemProfilerApi.AddMarker("ScrollRect.value", (Object)this);
