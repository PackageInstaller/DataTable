...
internal class X509ChainImplMono : X509ChainImpl
{
	...
	private void Process(int n)
	{
		...
		{
			X509Certificate monoCertificate2 = X509Helper2.GetMonoCertificate(elements[n + 1].Certificate);
	...
	private X509ChainStatusFlags CheckRevocation(X509Certificate2 certificate, X509Certificate2 ca_cert, bool online)
	{
		...
		X509Crl val = FindCrl(ca_cert);
		_ = (val == null) & online;
		if (val != null)
		{
			if (!val.VerifySignature(ca_cert.PublicKey.Key))
			{
				return X509ChainStatusFlags.RevocationStatusUnknown;
			}
			X509Certificate monoCertificate = X509Helper2.GetMonoCertificate(certificate);
			X509CrlEntry crlEntry = val.GetCrlEntry(monoCertificate);
			if (crlEntry != null)
			{
	...
	private static X509Crl CheckCrls(string subject, string ski, X509Store store)
	{
		...
		{
			X509Crl val = crl;
			if (val.IssuerName == subject && (ski.Length == 0 || ski == GetAuthorityKeyIdentifier(val)))
			{
				return val;
			}
		}
	...
	private X509Crl FindCrl(X509Certificate2 caCertificate)
	{
		...
		X509Crl val = CheckCrls(subject, subjectKeyIdentifier, LMCAStore.Store);
		if (val != null)
		{
			return val;
		}
		if (location == StoreLocation.CurrentUser)
		{
			val = CheckCrls(subject, subjectKeyIdentifier, UserCAStore.Store);
			if (val != null)
			{
				return val;
			}
		}
		val = CheckCrls(subject, subjectKeyIdentifier, LMRootStore.Store);
		if (val != null)
		{
			return val;
		}
		if (location == StoreLocation.CurrentUser)
		{
			val = CheckCrls(subject, subjectKeyIdentifier, UserRootStore.Store);
			if (val != null)
			{
				return val;
			}
	...
	private bool ProcessCrlExtensions(X509Crl crl)
	{
		...
		foreach (X509Extension item in (CollectionBase)(object)crl.Extensions)
		{
			X509Extension val = item;
			if (val.Critical)
			{
				string oid = val.Oid;
	...
	private bool ProcessCrlEntryExtensions(X509CrlEntry entry)
	{
		//IL_0014: Unknown result type (might be due to invalid IL or missing references)
		//IL_001a: Expected O, but got Unknown
		foreach (X509Extension item in (CollectionBase)(object)entry.Extensions)
		{
			X509Extension val = item;
			if (val.Critical && !(val.Oid == "2.5.29.21"))
			{
				return false;
			}
		}
