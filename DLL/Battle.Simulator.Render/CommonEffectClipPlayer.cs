...
public class CommonEffectClipPlayer
{
	...
	public static bool IsMainPlayeOrBossOrElitel(BBHumanoid blackboard)
	{
		if (null == blackboard.mAgent)
		{
			Debug.Log("=====> blackboard.mAgent Is Null");
			return false;
		}
		if (blackboard.m_NextSimFrameData == null)
		{
			Debug.Log("=====>blackboard.m_NextSimFrameData Is Null");
			return false;
		}
		if (blackboard.mAgent.AgentID == (NScene.GetCurrentScene() as BattleScene).GetAgentManager().mPlayerAgentID)
		{
			return (blackboard.m_NextSimFrameData.mEntityType & EnemyType.Other) != 0;
		}
		return false;
	}

	public static bool IsFriend(BBHumanoid blackboard)
	{
		if (null == blackboard.mAgent)
		{
			Debug.Log("=====> blackboard.mAgent Is Null");
		...
		if (blackboard.mAgent.AgentID != (NScene.GetCurrentScene() as BattleScene).GetAgentManager().mFirstAgentID && blackboard.mAgent.AgentID != (NScene.GetCurrentScene() as BattleScene).GetAgentManager().mSecondAgentID)
		{
			return blackboard.mAgent.AgentID == (NScene.GetCurrentScene() as BattleScene).GetAgentManager().mThirdAgentID;
		}
		return true;
	}
