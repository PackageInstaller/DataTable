...
public class TimelineTickEvent4 : EventNode
{
	...
	{
		if ((lastTime == 0 && m_executeTimeVar.GetValue() == 0) || (m_executeTimeVar.GetValue() > lastTime && m_executeTimeVar.GetValue() <= currentTime))
		{
			m_agentID = agentID;
