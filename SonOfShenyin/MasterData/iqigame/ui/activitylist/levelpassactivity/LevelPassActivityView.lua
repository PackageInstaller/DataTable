-- chunkname: @IQIGame\\UI\\ActivityList\\LevelPassActivity\\LevelPassActivityView.lua

local ActivityViewBase = require("IQIGame/UI/ActivityList/ActivityViewBase")
local activityScrollItemView = require("IQIGame.UI.ActivityList.LevelPassActivity.ActivityScrollItemView")
...
function m:__SetData(activityListCfg)
	...
	local activityPod = ActivityModule.GetActivityPodByID(self.activityListCfg.ActivityID[1])
	...
	UGUIUtil.SetText(self.rewardCountText, count)

	local itemCfg = CfgItemTable[self.normalTask[1].BaseData.ActionParam[1]]

	UGUIUtil.SetText(self.rewardName, itemCfg.Name)
	AssetUtil.LoadImage(self, UIGlobalApi.GetIconPath(itemCfg.Icon), self.rewardItemIcon:GetComponent("Image"))
	LuaUtility.SetGameObjectShow(self.Phase1, self.normalTask[#self.normalTask].BaseData.NeedSchedule == 30)
	LuaUtility.SetGameObjectShow(self.Phase2, self.normalTask[#self.normalTask].BaseData.NeedSchedule == 60)
	...
	self.commodityData = ActivityListModule.GetCommodityDataByActivityID(Constant.ShopType.PassShop.Type, activityPod.cid)
...
function m:UpdatePassScroll(OptimizedParams, OptimizedViewsHolder)
	...
	if not self.ItemList[instanceID] then
		self.ItemList[instanceID] = activityScrollItemView.New(viewGameObject, self)
	end
